 ...
 *
 *   node scripts/finalize-scheduled-prefill.cjs --start=today --window=9 --limit=9
...
function parseArgs(argv) {
  ...
  if (!Number.isInteger(args.window) || args.window < 1) args.window = 9;
  if (!Number.isInteger(args.limit) || args.limit < 1) args.limit = 9;
