...
/**
 * Export side-by-side PNGs comparing chroma-only vs chroma+lightness dampen.
 *
 *   npm run chroma:compare
 *   DATE_KEY=2026-06-14 npm run chroma:compare
 *
 * Output: tmp/chroma-dampen-compare/<dateKey>/*.png + contact-sheet.png
 */
const fs = require('fs');
...
const PANELS = [
  {
    ...
    label: 'Original (0%)',
    subtitle: 'No dampen',
    preview: (engine) => engine.withCohesionDampenPreviewBlocks(0)
  },
  {
    key: 'chroma-100',
    label: 'Chroma only 100%',
    subtitle: 'Hue preserved · saturation toward neighbors',
    preview: (engine) => engine.withChromaDampenPreviewBlocks(1)
  },
  {
    key: 'cohesion-50',
    label: 'Chroma + L 50%',
    subtitle: 'Hue preserved · sat + brightness toward neighbors',
    preview: (engine) => engine.withCohesionDampenPreviewBlocks(0.5)
  },
  {
    key: 'cohesion-100',
    label: 'Chroma + L 100%',
    subtitle: 'Hue preserved · sat + brightness toward neighbors',
    preview: (engine) => engine.withCohesionDampenPreviewBlocks(1)
  }
];
const OUT_W = 1070;
const OUT_H = 1340;
...
async function main() {
  ...
  const outDir = path.join(ROOT, 'tmp', 'chroma-dampen-compare', dateKey);
  fs.mkdirSync(outDir, { recursive: true });

  const pngPaths = [];
  for (const { key, label, subtitle, preview } of PANELS) {
    const previewBlocks = preview(engine);
    ...
    console.log(`[chroma-compare] wrote ${pngPath}`);
  }
  ...
  await sharp({
    ...
    .png()
    .toFile(contactPath);
  console.log(`[chroma-compare] wrote ${contactPath}`);
  console.log(`[chroma-compare] done — ${outDir}`);
}

main().catch((err) => {
  console.error('[chroma-compare] failed:', err.message || err);
  process.exit(1);
});
