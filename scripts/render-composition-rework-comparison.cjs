...
/**
 * Side-by-side: stored live quilt vs end-of-day full recompose.
 *
 * Left: quilts/{dateKey} blocks as they grew in the app (baseline live).
 * Right: infer mode from all ordered colors, then compose the full day from
 * color 1 with that mode active (blank canvas, same colors in order).
 *
 *   DATE_KEYS=2026-06-16,2026-06-10 npm run composition:rework-comparison
 */
const fs = require('fs');
...
const MIN_COLORS = Math.max(1, Math.floor(Number(process.env.MIN_COLORS) || 8));
...
async function renderDate(db, dateKey) {
  ...
  const metrics = analyzeColors(day.colors);
  const inferredMode = inferMode(metrics);
  const rework = replaySequence(dateKey, day.colors, inferredMode, 0, { lockPalette: true });
  const modeConfig = MODE_CONFIG[inferredMode] || MODE_CONFIG.baseline;
  ...
  const panels = [
    ...
    {
      mode: inferredMode,
      label: `End-of-day rework — ${dateKey}`,
      subtitle: `${day.colors.length} colors (${day.colorSource}) · mode: ${modeConfig.label} · ${rework.blocks.length} blocks · full-day compose`,
      blocks: rework.blocks,
      submissionCount: rework.submissionCount
    }
  ];

  console.log(
    `[rework-comparison] ${dateKey}: live ${day.liveBlocks.length} blocks vs rework (${inferredMode}) ${rework.blocks.length} blocks`
  );

  const outDir = path.join(ROOT, 'tmp', 'rework-comparison', dateKey);
  ...
  console.log(`[rework-comparison] wrote ${contactPath}`);
  ...
  const summary = {
    ...
    reworkBlockCount: rework.blocks.length,
    inferredMode,
    metrics: {
...
async function main() {
  ...
  for (const dateKey of dateKeys) {
    try {
      ...
      console.error(`[rework-comparison] ${dateKey} FAILED:`, err.message);
    }
  }
  const outDir = path.join(ROOT, 'tmp', 'rework-comparison');
  fs.mkdirSync(outDir, { recursive: true });
  fs.writeFileSync(path.join(outDir, 'summary.json'), `${JSON.stringify(summaries, null, 2)}\n`);
  console.log('[rework-comparison] done');
}

main().catch((err) => {
  console.error('[rework-comparison] failed:', err && err.stack ? err.stack : err);
  process.exit(1);
});
