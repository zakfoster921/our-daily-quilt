...
/**
 * Export side-by-side PNGs of today's quilt at different harmony blend strengths.
 *
 *   npm run harmony:compare
 *   DATE_KEY=2026-06-14 npm run harmony:compare
 *
 * Output: tmp/harmony-compare/<dateKey>/harmony-*.png + contact-sheet.png
 */
const fs = require('fs');
...
const STRENGTHS = [
  { key: '05', strength: 0.05, label: 'Harmony 5%' },
  { key: '10', strength: 0.10, label: 'Harmony 10%' },
  { key: '15', strength: 0.15, label: 'Harmony 15%' },
  { key: '50', strength: 0.50, label: 'Harmony 50%' }
];
const OUT_W = 1070;
const OUT_H = 1340;
...
function blocksToSvg(blocks, Utils, label) {
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="${minX - pad} ${minY - pad} ${vbW + pad * 2} ${vbH + pad * 2}" width="${OUT_W}" height="${OUT_H}">
  ...
  ${shapes}
  <rect x="${minX - pad}" y="${minY - pad}" width="${vbW + pad * 2}" height="52" fill="rgba(246,244,241,0.92)"/>
  <text x="${minX}" y="${minY + 28}" font-family="-apple-system,sans-serif" font-size="22" font-weight="600" fill="#333">${title}</text>
</svg>`;
}
...
async function main() {
  ...
  const outDir = path.join(ROOT, 'tmp', 'harmony-compare', dateKey);
  fs.mkdirSync(outDir, { recursive: true });

  const pngPaths = [];
  for (const { key, strength, label } of STRENGTHS) {
    const previewBlocks = engine.withHarmonyPreviewBlocks(strength);
    const svg = blocksToSvg(previewBlocks, Utils, `${label} — ${dateKey}`);
    const pngPath = path.join(outDir, `harmony-${key}.png`);
    await sharp(Buffer.from(svg)).png().toFile(pngPath);
    pngPaths.push({ pngPath, label, key });
    console.log(`[harmony-compare] wrote ${pngPath}`);
  }
  ...
  await sharp({
    ...
    .png()
    .toFile(contactPath);
  console.log(`[harmony-compare] wrote ${contactPath}`);
  console.log(`[harmony-compare] done — ${outDir}`);
}

main().catch((err) => {
  console.error('[harmony-compare] failed:', err.message || err);
  process.exit(1);
});
