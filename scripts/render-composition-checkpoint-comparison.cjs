...
/**
 * Side-by-side: stored live quilt vs color-20 checkpoint path.
 *
 * Left: live final from Firestore.
 * Right: baseline through color 20, infer mode, surgical tweaks, forward bias for 21+.
 *
 *   DATE_KEYS=2026-06-16,2026-06-10 npm run composition:checkpoint-comparison
 */
const fs = require('fs');
...
async function renderDate(db, dateKey) {
  ...
  const checkpoint = replaySequenceWithCheckpoint(dateKey, day.colors, { lockPalette: true });
  const modeConfig = MODE_CONFIG[checkpoint.mode] || MODE_CONFIG.baseline;
  ...
  const panels = [
    ...
    },
    {
      ...
      label: `Checkpoint @ ${checkpoint.decisionAt} — ${dateKey}`,
      subtitle: `${day.colors.length} colors · mode: ${modeConfig.label} · ${checkpoint.checkpoint.adjustments} tweak(s) · ${checkpoint.blocks.length} blocks · forward bias after ${checkpoint.decisionAt}`,
  ...
  console.log(
    `[checkpoint-comparison] ${dateKey}: live ${day.liveBlocks.length} blocks vs checkpoint (${checkpoint.mode}, ${checkpoint.checkpoint.adjustments} tweaks) ${checkpoint.blocks.length} blocks`
  );

  const outDir = path.join(ROOT, 'tmp', 'checkpoint-comparison', dateKey);
  ...
  console.log(`[checkpoint-comparison] wrote ${contactPath}`);
  ...
  const summary = {
    ...
    checkpointBlockCount: checkpoint.blocks.length,
    decisionAt: checkpoint.decisionAt,
    inferredMode: checkpoint.mode,
    checkpointAdjustments: checkpoint.checkpoint.adjustments,
...
async function main() {
  ...
  for (const dateKey of dateKeys) {
    try {
      ...
      console.error(`[checkpoint-comparison] ${dateKey} FAILED:`, err.message);
    }
  }
  const outDir = path.join(ROOT, 'tmp', 'checkpoint-comparison');
  fs.mkdirSync(outDir, { recursive: true });
  fs.writeFileSync(path.join(outDir, 'summary.json'), `${JSON.stringify(summaries, null, 2)}\n`);
  console.log('[checkpoint-comparison] done');
}

main().catch((err) => {
  console.error('[checkpoint-comparison] failed:', err && err.stack ? err.stack : err);
  process.exit(1);
});
