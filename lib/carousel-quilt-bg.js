...
(function (root, factory) {
  ...
  if (root) {
    ...
    root.drawLayoutBCarouselQuiltPaperTooth = api.drawLayoutBCarouselQuiltPaperTooth;
  }
...
function () {
  ...
  /** Slide 4 quilt-only: slightly more inset so green mat shows above/below the quilt. */
  const CAROUSEL_SLIDE4_MAT_PEEK_PX = 48;
  ...
  function drawLayoutBCarouselQuiltBg(ctx, quiltImg, rect, panelW, panelH, options = {}) {
    ...
    if (usePaperFrame) {
      ...
      const topShadowSeed = options.paperToothSeed || 'carousel-slide-1';
      if (PaperDepth.drawQuiltFrameTopCastShadow) {
        ctx.save();
        ctx.beginPath();
        ctx.rect(0, 0, panelW, frameY + 2);
        ctx.clip();
        PaperDepth.drawQuiltFrameTopCastShadow(
          ctx,
          frameX,
          frameY,
          frameW,
          topShadowSeed,
          options.topCastShadow || {}
        );
        ctx.restore();
      }
      const depthOpts =
      ...
      if (options.paperTextureImg && PaperDepth.drawClusterPaperTooth && !options.deferPaperTooth) {
        drawLayoutBCarouselQuiltPaperTooth(ctx, panelW, panelH, options);
      }
      return;
    }
  ...
  /** Paper grain / fleck over the quilt card — call after quote strips so they sit under the texture. */
  function drawLayoutBCarouselQuiltPaperTooth(ctx, panelW, panelH, options = {}) {
    const matPeek = Math.max(0, Number(options.quiltMatPeekPx) || 0);
    const PaperDepth = globalThis.IgCarouselPaperDepth;
    if (!options.paperTextureImg || !PaperDepth?.drawClusterPaperTooth) return;
    if (panelW <= matPeek * 2 || panelH <= matPeek * 2) return;
    const frameX = matPeek;
    const frameY = matPeek;
    const frameW = panelW - matPeek * 2;
    const frameH = panelH - matPeek * 2;
  ...
  return {
    ...
    drawLayoutBCarouselQuiltPaperTooth,
