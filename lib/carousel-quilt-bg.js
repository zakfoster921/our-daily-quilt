...
function () {
  ...
  function resolveLayoutBCarouselQuiltRect(quiltImg, panelW, panelH, options = {}) {
    ...
    const quiltBgOffsetY = Number(options.quiltBgOffsetY);
    const matPeekPx = Math.max(0, Number(options.quiltMatPeekPx) || 0);

    if (matPeekPx > 0 && (quiltFit === 'cover' || !quiltFit)) {
      const peekRect = resolveQuiltRectForPanelMatPeek(iw, ih, panelW, panelH, matPeekPx, {
        quiltBgZoom,
        quiltBgOffsetY,
        layoutBQuiltBleed: layoutBQuiltBleed / quiltBgZoom
      });
      return { rect: peekRect, dedicationMessageY: null };
    }
