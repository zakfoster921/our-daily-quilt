  ...
  async saveQuilt(blocks, contributorCount, colorReplayEvents = [], contributorsOrOptions = [], saveOptions = null) {
    ...
    try {
      ...
      if (!skipFirestoreSave && window.db && window.firestore) {
        ...
        /**
         * Daytime quilt save: upload quilt-screen 9:16 JPEG only (~400 KB).
         * Story preview stacks that quilt layer + a stable overlay (nightly / tune).
         * Full layout-b-story.png + overlay regen stay on nightly GitHub job and tune save.
         */
        if (!options?.adminMutation) {
          ...
          void (async () => {
            try {
              ...
              try {
                await timed(
                  'Firestore quilt-screen upload on save',
                  Utils.writeInstagramImagesDocForZapier(uploadPayload),
                  180000
                );
              } catch (clientUploadErr) {
                console.warn('Quilt-screen client upload failed, trying backend', clientUploadErr);
                if (typeof Utils.writeInstagramImagesDocForZapierViaServer === 'function') {
                  await timed(
                    'Backend quilt-screen upload on save',
