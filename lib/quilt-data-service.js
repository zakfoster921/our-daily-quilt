...
class QuiltDataService {
  ...
  async loadQuiltFromServer(todayKey = Utils.getTodayKey()) {
    ...
    /**
     * Mirror tune fields ride on this same doc. Copied through as raw field names
     * so QuiltMirrorLayout.odqMirrorTuneFromQuiltData can parse either a Firestore
     * doc or this REST payload identically.
     */
    const pickMirrorFields = (src) => {
      const out = {
        mirrorBottomLayout: src.mirrorBottomLayout,
        mirrorFlipX: src.mirrorFlipX,
        mirrorFlipY: src.mirrorFlipY,
        mirrorBottomLeftFlipX: src.mirrorBottomLeftFlipX,
        mirrorBottomLeftFlipY: src.mirrorBottomLeftFlipY,
        mirrorBottomRightFlipX: src.mirrorBottomRightFlipX,
        mirrorBottomRightFlipY: src.mirrorBottomRightFlipY,
        mirrorSeamNudgeY: src.mirrorSeamNudgeY,
        mirrorFieldNudgeY: src.mirrorFieldNudgeY,
        mirrorTileSeamNudgeX: src.mirrorTileSeamNudgeX,
        mirrorBottomLeftNudgeX: src.mirrorBottomLeftNudgeX,
        mirrorBottomLeftNudgeY: src.mirrorBottomLeftNudgeY,
        mirrorBottomRightNudgeY: src.mirrorBottomRightNudgeY,
        mirrorTuneUpdatedAt: src.mirrorTuneUpdatedAt
      };
      ['TL', 'TR', 'BL', 'BR'].forEach((id) => {
        out[`mirrorQuad${id}FlipX`] = src[`mirrorQuad${id}FlipX`];
        out[`mirrorQuad${id}FlipY`] = src[`mirrorQuad${id}FlipY`];
        out[`mirrorQuad${id}NudgeX`] = src[`mirrorQuad${id}NudgeX`];
        out[`mirrorQuad${id}NudgeY`] = src[`mirrorQuad${id}NudgeY`];
      });
      return out;
    };
    ...
    const loadViaServerApi = async () => {
      ...
      return {
        ...
        data: {
          ...
          macroLayoutMode: payload.macroLayoutMode === 'columns' ? 'columns' : undefined,
          ...pickMirrorFields(payload)
        }
      };
    };
    try {
      ...
      if (isNative) {
        // iOS WebView Firestore getDoc often wins the race from cache with stale mirror tune;
        // Railway REST uses Admin SDK and is the reliable read path on native.
        try {
          return await loadViaServerApi();
        } catch (error) {
          this.logger.warn('⚠️ Native quilt load failed via Railway API:', error);
  ...
  async saveQuilt(blocks, contributorCount, colorReplayEvents = [], contributorsOrOptions = [], saveOptions = null) {
    ...
    try {
      ...
      const requireFirestoreSave = options?.requireFirestore === true;
      ...
      if (options?.macroLayoutMode === 'columns' || options?.macroLayoutMode === 'default') {
        quiltData.macroLayoutMode = options.macroLayoutMode;
      }
      ...
      if (!skipFirestoreSave && (!window.db || !window.firestore)) {
        ...
        if (requireFirestoreSave) {
          console.warn('⚠️ Refusing save because Firestore is required for this write.');
          return false;
        }
        if (src === 'daily') {
        ...
        if (requireFirestoreSave) {
          console.warn('⚠️ Refusing save because Firestore was required but skipped.');
          return false;
        }
      }
      ...
      if (!skipFirestoreSave && window.db && window.firestore) {
        ...
        if (src !== 'admin' && !options?.adminMutation) {
          try {
            ...
            if (existingSnap.exists()) {
              ...
              const existingReplay = Array.isArray(existingData.colorReplayEvents)
                ? existingData.colorReplayEvents
                : [];
              const incomingReplay = Array.isArray(quiltData.colorReplayEvents)
                ? quiltData.colorReplayEvents
                : [];
              if (
                !options?.adminMutation &&
                existingReplay.length > 0 &&
                incomingReplay.length < existingReplay.length
              ) {
                quiltData.colorReplayEvents = existingReplay;
              }
            }
          } catch (guardError) {
        ...
        /**
         * Daytime quilt save: quilt-screen JPEG (tune modal) + full story preview JPEG (app preview).
         * PNG story + overlay stay on nightly GitHub job and tune save.
         */
          ...
          void (async () => {
            try {
              ...
              const timed = (label, promise, timeoutMs) =>
                typeof root.odqPromiseWithTimeout === 'function'
                  ? root.odqPromiseWithTimeout(promise, timeoutMs, label)
                  : promise;
              const baseMeta = {
                dateKey: igDateKey,
                zapierCaption: Utils.formatZapierCaptionFromQuote(todayQuote),
                quiltFingerprint: Utils.computeQuiltFingerprint(igBlocks),
              ...
              /** Quilt-screen first — next visitor needs this even if story JPEG times out. */
              let quiltScreen9x16ImageData = null;
              try {
                quiltScreen9x16ImageData = await timed(
                  'Quilt screen 9:16 on save',
                  arch.generateInstagramQuiltScreen9x16ImageData(igBlocks, igDateKey),
                  90000
                );
              } catch (_) {
              ...
              const quoteFitPreview =
                typeof arch.generateInstagramStoryQuoteFitPreviewJpegImageData === 'function'
                  ? arch.generateInstagramStoryQuoteFitPreviewJpegImageData.bind(arch)
                  : null;
              const layoutBPreview =
                typeof arch.generateInstagramStoryLayoutBPreviewJpegImageData === 'function'
                  ? arch.generateInstagramStoryLayoutBPreviewJpegImageData.bind(arch)
                  : null;
              const generateStoryPreview = quoteFitPreview || layoutBPreview;
              if (generateStoryPreview) {
                try {
                  storyLayoutBPreviewImageData = await timed(
                    quoteFitPreview
                      ? 'Story quote-fit preview JPEG on save'
                      : 'Layout B story preview JPEG on save',
                    generateStoryPreview(igBlocks, todayQuote, igDateKey),
                    120000
                  );
                } catch (_) {
                ...
                if (storyLayoutBPreviewImageData) {
                  await uploadSnapshot({
                    ...
                    ...(quoteFitPreview
                      ? {
                          layoutBStoryPreviewRenderVersion: String(
                            root.ODQ_STORY_QUOTE_FIT_PREVIEW_RENDER_VERSION ||
                              'story-quote-fit-preview-v3'
                          ).trim()
                        }
                      : {})
                  });
              ...
              if (!quiltScreen9x16ImageData && !storyLayoutBPreviewImageData) {
                console.warn('Skipping Instagram upload on save: no quilt-screen or story preview JPEG');
              }
            } catch (imageError) {
              console.warn('Could not save quilt-screen to Firestore:', imageError);
            }
          })();
        }
      }
      ...
      if (fallbackOptions?.requireFirestore === true) {
        console.warn('⚠️ Refusing localStorage fallback because Firestore is required for this write.');
        return false;
      }
