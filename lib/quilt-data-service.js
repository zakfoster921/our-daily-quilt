        ...
        if (!options?.adminMutation) {
          ...
          void (async () => {
            try {
              ...
              if (!arch) {
                console.warn('ArchiveService not available for Instagram snapshot');
                return;
              }
              if (typeof window.app?.ensureLayoutBComposeReady === 'function') {
                await window.app.ensureLayoutBComposeReady();
              }
              ...
              if (arch.generateInstagramStoryLayoutBImage) {
                storyLayoutBImageData = await timed(
                  'Layout B story on save',
                  arch.generateInstagramStoryLayoutBImage(igBlocks, todayQuote, igDateKey),
                  120000
                ).catch(() => null);
              }
              ...
              if (arch.generateInstagramStoryLayoutBOverlayImage) {
                storyLayoutBOverlayImageData = await timed(
                  'Layout B story overlay on save',
                  arch.generateInstagramStoryLayoutBOverlayImage(igBlocks, todayQuote, igDateKey),
                  120000
                ).catch(() => null);
              }
              let postLayoutBSpeakerImageData = null;
              if (arch.generateInstagramPostLayoutBSpeakerImage) {
                postLayoutBSpeakerImageData = await timed(
                  'Layout B speaker post on save',
                  arch.generateInstagramPostLayoutBSpeakerImage(igBlocks, todayQuote, igDateKey),
                  120000
                ).catch(() => null);
              }
              ...
              if (arch.buildIntegratedInstagramCarouselImageData) {
                const integrated = await timed(
                  'Integrated IG carousel on save',
                  arch.buildIntegratedInstagramCarouselImageData(
                    igBlocks,
                    igContributors,
                    todayQuote,
                    igDateKey
              ...
              } else if (arch.generateInstagramCarouselSlideImageData) {
                const quiltCarouselSlides = await timed(
                  'IG carousel slides on save',
                  arch.generateInstagramCarouselSlideImageData(igBlocks, igContributors, igDateKey),
                  120000
                ).catch(() => null);
                let postLayoutBImageData = null;
                if (arch.generateInstagramPostLayoutBImage) {
                  postLayoutBImageData = await timed(
                    'Layout B post on save',
                    arch.generateInstagramPostLayoutBImage(igBlocks, todayQuote, igDateKey),
                    120000
                  ).catch(() => null);
                }
              ...
              const hasStorySnapshot =
                !!storyLayoutBImageData || !!storyLayoutBOverlayImageData || !!quiltScreen9x16ImageData;
              const hasCarousel =
                !!(carouselSlide1ImageData && carouselSlide2ImageData && carouselSlide3ImageData);
              if (!hasStorySnapshot && !hasCarousel) {
                console.warn('Skipping Instagram Firestore doc: no story or carousel images generated');
                return;
              }
              ...
              window.app?.scheduleLayoutBStoryPreviewRefresh?.({ force: true, delayMs: 400 });
            } catch (imageError) {
