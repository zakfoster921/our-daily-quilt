...
class QuiltDataService {
  ...
  async loadQuiltFromServer(todayKey = Utils.getTodayKey()) {
    ...
    try {
      ...
      const readTimeoutMs = isNative ? 20000 : 12000;
      ...
      try {
        ...
        if (!isNative) throw firstError;
        this.logger.warn(
          '⚠️ Firestore server quilt read failed; retrying once:',
        ...
        try {
          ...
          if (typeof getCached !== 'function') throw secondError;
          this.logger.warn(
            '⚠️ Firestore server quilt read failed twice; trying network getDoc:',
            secondError?.message || secondError
          );
          quiltSnap = await withTimeout(getCached(quiltRef), readTimeoutMs, 'Firestore quilt read (network)');
        }
      }
      if (!quiltSnap.exists()) {
