...
function buildPreserveFromNotionMap(notionPage, catalogData) {
  ...
  for (const field of PRESERVE_IF_NOTION_EDITED_AFTER_PREFILL) {
    ...
    const catalogText = readCatalogField(catalogData, field);
    const editedAfterPrefill =
      prefillMs == null || notionEditedMs == null || notionEditedMs > prefillMs;
    // Notion already differs from Firestore (manual edit before sync caught up, or edit mid-workflow).
    const manualNotionCopy = Boolean(catalogText && notionText !== catalogText);
    if (editedAfterPrefill || manualNotionCopy) {
