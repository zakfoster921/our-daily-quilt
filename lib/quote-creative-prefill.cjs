...
function findNotionPropName(schema, ...aliases) {
  for (const alias of aliases) {
    ...
    if (!target) continue;
    const found = Object.keys(schema || {}).find((name) => normKey(name) === target);
...
function chooseScheduledFieldsToPatch(data, { moodOptions = [] } = {}) {
  const missing = SCHEDULED_CREATIVE_FIELDS.filter((field) => {
    ...
    if (field === 'community_prompt' && !isCommunityPromptPrefillValid(value)) return true;
    return false;
  });
...
function readNotionFieldFromPageProperties(properties, field) {
  ...
  for (const alias of aliases) {
    const target = normKey(alias);
    if (!target) continue;
    const found = Object.entries(properties || {}).find(([name]) => normKey(name) === target);
    if (!found) continue;
...
function buildPreserveFromNotionMap(notionPage, catalogData) {
  ...
  for (const field of PRESERVE_IF_NOTION_EDITED_AFTER_PREFILL) {
    ...
    if (field === 'community_prompt' && !isCommunityPromptPrefillValid(notionText)) continue;
    const notionEditedAfterPrefill =
      prefillMs == null || notionEditedMs == null || notionEditedMs > prefillMs;
    // Only keep Notion copy when the page was edited after the last prefill stamp.
    // Do not clobber Firestore from stale Notion just because the two differ.
    if (notionEditedAfterPrefill) {
...
function mapSubmittedQuotePrefillFields(parsed, model) {
  return {
    ...
    good_day: normalizeMoodDayPrefillValue(
      pickPrefillStringLoose(parsed, 'good_day', 'goodDay', 'good day')
    ),
    rough_day: normalizeMoodDayPrefillValue(
      pickPrefillStringLoose(parsed, 'rough_day', 'roughDay', 'rough day')
    ),
...
const REQUIRED_PREFILL_FIELDS = [
  ...
  },
  {
    key: 'watch_for',
    requirement:
      'watch_for is required: one capitalized fragment framed as a moment, opening, or chance for the reader to step into today — prefer "A chance to…" / "An opening to…" over passive third-person observation; no "Watch for" prefix, no feelings, no homework; do not add a trailing period unless the copy needs one.'
  },
  {
    key: 'good_day',
    requirement:
      'good_day is required: a short quirky declarative push, max 50 characters including spaces — one tight line, no questions, no generic pep-talk filler, no trailing period.'
  },
  {
    key: 'rough_day',
    requirement:
      'rough_day is required: a short reframe, max 50 characters including spaces — names no emotions, makes no demands, asks no questions, no trailing period.'
  }
];
...
function findMissingRequiredPrefillFields(out, moodOptions) {
  ...
  if (!isCommunityPromptPrefillValid(out.community_prompt)) {
    missing.push({
      key: 'community_prompt',
      requirement: COMMUNITY_PROMPT_PREFILL_REQUIREMENT
    });
  }
  for (const key of ['good_day', 'rough_day']) {
    const val = String(out?.[key] || '').trim();
    if (val && !isMoodDayPrefillValid(val)) {
      missing.push({
        key,
        requirement: MOOD_DAY_PREFILL_REQUIREMENT
      });
    }
  }
...
async function generateSubmittedQuotePrefillFieldsWithProvider({
  ...
  if (!isCommunityPromptPrefillValid(out.community_prompt)) {
    throw new Error(
      `${providerLabel} returned an invalid community_prompt (too long or compound). Got: ${String(out.community_prompt || '').slice(0, 120)}`
    );
  }
  for (const key of ['good_day', 'rough_day']) {
    if (!isMoodDayPrefillValid(out[key])) {
      throw new Error(
        `${providerLabel} returned invalid ${key} (empty or over 50 characters). Got: ${String(out[key] || '').slice(0, 80)}`
      );
    }
  }
...
async function runScheduledCreativePrefill({
  ...
  // Prefill writes bump Notion last_edited_time, so "edited after prefill" is not a manual edit.
  // On version bump, always regenerate community_prompt with the new spec.
  if (chosen.reason === 'stale_version') {
    delete preserveFromNotion.community_prompt;
  }
  ...
  const existingPortrait = readCatalogField(catalogData, 'speaker_image_url');
  if (existingPortrait && !/^needs\s+manual\s+lookup$/i.test(existingPortrait)) {
    if (!preserveFromNotion.speaker_image_url) {
      catalogWrite.speaker_image_url = existingPortrait;
      catalogWrite.speakerImageUrl = existingPortrait;
    }
    for (const [propName] of Object.entries(notionProperties)) {
      const aliases = FIELD_ALIASES.speaker_image_url || ['speaker_image_url'];
      if (aliases.some((alias) => normKey(alias) === normKey(propName))) delete notionProperties[propName];
    }
  }
