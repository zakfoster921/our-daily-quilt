...
const PREFILL_CREATIVE_PROMPT_VERSION = '2026-08-03-watch-for-active-v5';

const COMMUNITY_PROMPT_MAX_WORDS = 20;
const MOOD_DAY_MAX_CHARS = 50;

const COMPOUND_COMMUNITY_PROMPT_RES = [
  /\s+and\s+(what|why|how|when|where|who|did|does|do|was|were|would|could|should|is|are)\b/i,
  /[—–-]\s*and\s+(what|why|how|when|did|does|do|was|were|would|could|should)\b/i,
  /;\s*(what|why|how|did|does|do|was|were|would|could|should)\b/i
];
...
const SYSTEM_PROMPT = [
  ...
  'If the quote is abstract, translate its theme into one concrete interpersonal gesture (small_act) and one moment or opportunity the reader can catch and act on today (watch_for).'
].join(' ');
...
FIELD: watch_for
- A single phrase naming a moment, opportunity, or chance the reader might recognize and step into today.
- Frame it for the participant — not a scene to watch from the sidelines. Prefer "A chance to…", "An opening to…", "A moment to…", or "An opportunity to…" over passive third-person observation ("Someone does…", "Two people…", "The room…").
- Enacts the quote without explaining it. The noticing is the whole thing — but the reader should feel invited to meet it, not just witness it.
- Catchable today: specific enough that they know it when it shows up. A door they can walk through, not a category or a vibe.
...
- Does not mention the quote text or the author.
- Examples (value only, after the app prefix): "A chance to do something that could only come from you." / "An opening to let two struggles meet instead of choosing sides." / "A moment to notice the obstacle that's actually holding something together." / "An opportunity to ask a question nobody has a clean answer to." / "A chance to stay quiet long enough for something true to get said." / "An opening to make something small and hand it over." / "A moment to skip the obvious path for a stranger one." / "A chance to try again after getting it wrong."
...
- List entries in the order they appear in the quote (left to right).
...
- List entries in the order they appear in speaker_guide_line (left to right).
...
- Max ${MOOD_DAY_MAX_CHARS} characters including spaces. One tight line; only use two very short sentences if the total still fits.
...

{
  ...
  "watch_for": "capitalized fragment framed as a moment/chance/opportunity for the reader (no Watch for prefix; no passive someone-does-X; no forced trailing period)",
  ...
  "good_day": "short declarative push, max ${MOOD_DAY_MAX_CHARS} characters, no question marks, no trailing period",
  "rough_day": "short reframe, max ${MOOD_DAY_MAX_CHARS} characters, no emotion-naming, no demands, no question marks, no trailing period",
...
function countCommunityPromptWords(text) {
  return String(text || '')
    .trim()
    .replace(/[?.,!;:—–-]+$/g, '')
    .split(/\s+/)
...
/** Returns issue codes when community_prompt is too long or compound (e.g. "… — and did it…?"). */
function getCommunityPromptPrefillIssues(value) {
  const text = String(value || '').trim();
  const issues = [];
  if (!text) {
    issues.push('empty');
  ...
  if (!text.endsWith('?')) issues.push('must_end_with_question_mark');
  if (countCommunityPromptWords(text) > COMMUNITY_PROMPT_MAX_WORDS) issues.push('over_20_words');
  if (COMPOUND_COMMUNITY_PROMPT_RES.some((re) => re.test(text))) issues.push('compound_question');
  return issues;
}

function isCommunityPromptPrefillValid(value) {
  return getCommunityPromptPrefillIssues(value).length === 0;
}

const COMMUNITY_PROMPT_PREFILL_REQUIREMENT =
  'community_prompt is required: one short open question under 20 words, one clause and one ask only — no compound "and what/did..." follow-ups, no em-dash second question. Transferable, plain language, does not mention the quote or author, ends with ?';

const MOOD_DAY_PREFILL_REQUIREMENT =
  `good_day and rough_day are required: each max ${MOOD_DAY_MAX_CHARS} characters including spaces — one tight line, no questions, no trailing period. good_day: quirky declarative push. rough_day: reframe without naming emotions or making demands.`;

/** Trim mood-day copy to max length on a word boundary when the model overshoots. */
function normalizeMoodDayPrefillValue(value) {
  const text = String(value || '').replace(/\s+/g, ' ').trim();
  if (!text || text.length <= MOOD_DAY_MAX_CHARS) return text;
  const slice = text.slice(0, MOOD_DAY_MAX_CHARS);
  const lastSpace = slice.lastIndexOf(' ');
  if (lastSpace > 12) return slice.slice(0, lastSpace).trim();
  return slice.trim();
}

function getMoodDayPrefillIssues(value) {
  const text = String(value || '').trim();
  const issues = [];
  if (!text) issues.push('empty');
  else if (text.length > MOOD_DAY_MAX_CHARS) issues.push('over_50_chars');
  return issues;
}

function isMoodDayPrefillValid(value) {
  return getMoodDayPrefillIssues(value).length === 0;
}
