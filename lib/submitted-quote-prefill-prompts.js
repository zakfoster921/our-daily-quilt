 ...
 *   what_if, watch_for, speaker_guide_line, art_recs, good_day, rough_day, prompt_theme
 *
 ...
 *
 * prompt_theme is picked from the Notion "prompt_theme" select column's existing
 * options (passed in as promptThemeOptions) rather than invented, since it's a
 * fixed tag list an editor curates in Notion.
 */
...
const PREFILL_CREATIVE_PROMPT_VERSION = '2026-07-03-quirky-good-day-v1';
...
function buildFieldInstructions(promptThemeOptions) {
  const options = (promptThemeOptions || []).map((o) => String(o || '').trim()).filter(Boolean);
  const promptThemeField = options.length
    ? `
FIELD: prompt_theme
- Pick exactly one tag from this fixed list that best fits the core idea of the quote/community_prompt: ${options.join(', ')}
- Output the tag exactly as written above (same spelling/casing). Do not invent a new tag, combine tags, or explain your choice.
...

{
  ...
  "prompt_theme": "one tag verbatim from the provided list, or \\"\\" if none fit"
}
`.trim();

/**
 * Build the user message for the prefill call.
 * @param {{ quoteText: string, authorName: string, promptThemeOptions?: string[] }} params
 */
function buildSubmittedQuotePrefillPrompt({ quoteText, authorName, promptThemeOptions }) {
  ...
  return [
    ...
    '',
    'FIELD SPECS:',
    buildFieldInstructions(promptThemeOptions),
    '',
    'OUTPUT SCHEMA:',
    OUTPUT_SCHEMA
  ].join('\n');
