...
function (global) {
  ...
  async function composeTerminalQuoteClippingFallbackDataUrl(quote, opts = {}) {
    const dk = String(quote?.dateKey || '').trim();
    const app = global.app || globalThis.app;
    const arch = app?.archiveService;
    if (dk && typeof arch?.generateNewspaperClippingImageData === 'function') {
      try {
        const dataUrl = await arch.generateNewspaperClippingImageData(dk);
