...
function (global) {
  ...
  function resolveQuoteInput(opts = {}) {
    ...
    if (!text) return null;
    const keyword = String(q.keyword ?? q.keywordSnapshot ?? opts.keyword ?? '').trim();
    return {
      text,
      author,
      dateKey: String(q.dateKey ?? opts.dateKey ?? '').trim(),
      firstLineCount:
        Number(q.firstLineCount ?? q.first_line_count ?? opts.firstLineCount ?? opts.first_line_count ?? 0) ||
        undefined,
  ...
  async function composeTerminalQuoteClippingFallbackDataUrl(quote, opts = {}) {
    ...
    const paperTextureUrl =
      typeof QNC.resolveClippingPaperTextureUrl === 'function'
        ? QNC.resolveClippingPaperTextureUrl()
    ...
    return QNC.composeDataUrl({
      ...
      today: {
        ...
        firstLineCount: quote.firstLineCount,
  ...
  function revealMoodMessage(root, prefix, mood, message, opts = {}) {
    ...
    const showBody = () => {
      ...
      // Tighten line-height for long messages that wrap to 3+ lines.
      if (msgEl && bodyRow) {
        const probe = msgEl.cloneNode(false);
        probe.style.cssText = 'position:absolute;visibility:hidden;pointer-events:none;';
        probe.textContent = displayLine;
        bodyRow.appendChild(probe);
        const lineH = parseFloat(getComputedStyle(bodyRow).lineHeight) || 0;
        const probeH = probe.getBoundingClientRect().height;
        probe.remove();
        if (lineH > 0 && probeH / lineH >= 2.5) {
          bodyRow.style.lineHeight = '0.88';
        } else {
          bodyRow.style.removeProperty('line-height');
        }
      }
