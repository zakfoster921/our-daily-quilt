...
(function (root) {
  ...
  function applyWarmSmoothLithographPass(d, width, height, opts = {}) {
    const INK_FLOOR = opts.quiltScreenDarkerDarks === true ? 28 : 58;
    const contrast = opts.quiltScreenDarkerDarks === true ? 1.32 : 1.18;
    const brightness = opts.quiltScreenDarkerDarks === true ? 1.02 : 1.1;
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < width; x += 1) {
        const i = (y * width + x) * 4;
        const alpha = d[i + 3];
        ...
        const v = Math.round(lum);
        const warmBoost = opts.quiltScreenDarkerDarks === true ? 4 : 10;
        d[i] = Math.min(255, v + warmBoost);
        d[i + 1] = Math.min(255, v + (opts.quiltScreenDarkerDarks === true ? 2 : 4));
        d[i + 2] = Math.max(0, v - (opts.quiltScreenDarkerDarks === true ? 22 : 16));
      }
    }
  }
    ...
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < width; x += 1) {
        ...
        if (lum >= HIGHLIGHT_FLOOR) {
          ...
          const t = 1 - lum / SHADOW_CEIL;
          const crush = quiltDark ? 0.34 + t * 0.52 : 0.2 + t * 0.42;
  ...
  function makeSpeakerPortraitCanvas(img, outW, outH, isCutoutPng = false, portraitOpts = {}) {
    ...
    if (portraitOpts.smoothLithograph === true) {
      ...
      applyNewspaperFilmGrainPass(d, c.width, c.height, toneSeed, { grainAmp: 4 });
    }
  ...
  function drawSpeakerCutoutOverQuilt(g, img, rect, options = {}) {
    ...
    const grain =
      options.layers?.grain === false
        ? null
        : buildSpeakerGrainCanvas(portrait, { softGrain: quiltScreen, quiltScreen });
    const radialWash = useRadialWash ? buildSpeakerRadialWashCanvas(portrait, washColor, bgOpts) : null;
    const creamBackdrop =
