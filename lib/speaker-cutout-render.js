 ...
 * Layer order (bottom → top): scanner bed → cream mat → solid wash → paper shapes → B/W portrait → grain.
 */
(function (root) {
  'use strict';
  ...
  function makeSpeakerAlphaSilhouetteCanvas(img, outW, outH, isCutoutPng = false, layout = 'contain') {
  ...
  function applyPaperShapeElement(el, shape) {
  ...
  function applyPaperBackdropElement(el, shape) {
    if (!el || !shape) return;
    el.hidden = false;
    el.removeAttribute('hidden');
    applyPaperShapeElement(el, shape);
  ...
  function buildSpeakerCutoutPlan({
    ...
    if (img && (img.naturalWidth || img.width)) {
      const analysis = makeSpeakerAlphaSilhouetteCanvas(img, width, height, true);
  ...
  function drawSpeakerCutoutStack(g, img, rect, options = {}) {
    ...
    const paperAlpha = options.paperAlpha != null ? options.paperAlpha : isHeroPost ? 0.85 : 0.88;
    const backdropAlpha =
