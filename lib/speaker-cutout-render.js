...
(function (root) {
  ...
  /** IG slide 1 opaque speaker mat — warmer + softer than quote strips so strips stay focal. */
  const IG_SPEAKER_COLLAGE_PAPER = '#f0e9df';
  const BAYER_4 = [
    [0, 8, 2, 10],
    [12, 4, 14, 6],
    [3, 11, 1, 9],
  ...
  const SPEAKER_WASH_PROFILES = Object.freeze({
    ...
    /** IG speaker colored stock — softer/lighter than pop-art paper shapes. */
    igCollagePaper: {
      satMul: 0.42,
      satMin: 10,
      satMax: 32,
  ...
  /** Accent swatch for IG speaker paper — farthest hue from dominant quilt color (palette[0]). */
  function pickIgCollagePaperQuiltSwatch(palette, fallback = '#ea9b9a') {
    const colors = normalizeQuiltPalette(palette, fallback);
    if (colors.length <= 1) return colors[0];
    const primary = colors[0];
    let accent = colors[1];
    let bestDist = hueDistance(primary, accent);
    for (let i = 2; i < colors.length; i += 1) {
      const dist = hueDistance(primary, colors[i]);
      if (dist > bestDist) {
        bestDist = dist;
        accent = colors[i];
      }
  ...
  /** Soft colored stock for IG speaker mat — dominant quilt swatch (palette[0] by area). */
  function resolveIgCollagePaperColor(options = {}) {
    if (options.paper && /^#[0-9A-Fa-f]{6}$/.test(String(options.paper).trim())) {
      return safeHex(options.paper, IG_SPEAKER_COLLAGE_PAPER);
    }
    const fallback = safeHex(
      options.rawWashColor || options.washColor || IG_SPEAKER_COLLAGE_PAPER,
      IG_SPEAKER_COLLAGE_PAPER
    );
    const palette =
      Array.isArray(options.palette) && options.palette.length ? options.palette : [];
    const colors = normalizeQuiltPalette(palette, fallback);
    const rawSource = safeHex(colors[0], IG_SPEAKER_COLLAGE_PAPER);
    const quiltTint = washOutSpeakerQuiltColor(rawSource, 'igCollagePaper');
    return mixHexColors(quiltTint, IG_SPEAKER_COLLAGE_PAPER, 0.22);
  }
  ...
  /** Softer warm paper for IG speaker collage — less sheen/grain so quote strips read first. */
  function fillSpeakerIgCollagePaperInCtx(ctx, w, h, paperHex = IG_SPEAKER_COLLAGE_PAPER) {
    const paper = safeHex(paperHex, IG_SPEAKER_COLLAGE_PAPER);
    const width = Math.max(1, Math.round(Number(w) || 1));
    const height = Math.max(1, Math.round(Number(h) || 1));
    ctx.fillStyle = paper;
    ctx.fillRect(0, 0, width, height);
    applySpeakerIgCollagePaperTextureInCtx(ctx, width, height, paper);
  }

  /** Texture only — call after source-in mask with composite restored to source-over. */
  function applySpeakerIgCollagePaperTextureInCtx(ctx, w, h, paperHex = IG_SPEAKER_COLLAGE_PAPER) {
    const paper = safeHex(paperHex, IG_SPEAKER_COLLAGE_PAPER);
    const { r, g, b } = parseSpeakerWashRgb(paper);
    const width = Math.max(1, Math.round(Number(w) || 1));
    const height = Math.max(1, Math.round(Number(h) || 1));
    ctx.fillStyle = `rgba(${Math.max(0, r - 52)}, ${Math.max(0, g - 52)}, ${Math.max(0, b - 52)}, 0.014)`;
    ...
    sheen.addColorStop(0, 'rgba(255, 252, 245, 0.09)');
    sheen.addColorStop(0.52, 'rgba(255, 252, 245, 0)');
    sheen.addColorStop(1, `rgba(${Math.max(0, r - 28)}, ${Math.max(0, g - 28)}, ${Math.max(0, b - 28)}, 0.05)`);
    ctx.fillStyle = sheen;
    ctx.fillRect(0, 0, width, height);
  ...
  /** Same paper tooth pass as Layout B carousel slide 1 (quiltFramePaperToothOptions). */
  function applyIgSpeakerPaperToothLayer(ctx, w, h, options = {}) {
    const PaperDepth = globalThis.IgCarouselPaperDepth;
    if (!PaperDepth?.drawClusterPaperTooth || !ctx || w < 1 || h < 1) return;
    const seed = String(options.paperToothSeed || options.seed || 'carousel-slide-1').trim();
    const toothOpts =
    ...
    PaperDepth.drawClusterPaperTooth(ctx, 0, 0, w, h, null, seed, toothOpts);
  }
  ...
  function buildSpeakerPlainCutoutLayerCanvas(img, rect, options = {}) {
    ...
    if (options.isCutoutPng !== false) {
      sil = makeSpeakerPlainCutoutAlphaSilhouetteCanvas(img, w, h, resolvedLayout, options.subjectCrop);
      if (sil) flattenCutoutAlphaToOpaque(c, sil);
    }
    if (options.paperTooth === true && options.igCollagePaper === true && sil) {
      drawCanvasLayerClippedToMask(g, sil, 0, 0, w, h, (lx, lw, lh) => {
        applyIgSpeakerPaperToothLayer(lx, lw, lh, options);
      });
    }
    return c;
  }

  /** Flattened Layout B story speaker (lithograph multiply + radial wash) for quilt-screen bake. */
  function buildSpeakerOverQuiltLayerCanvas(img, rect, options = {}) {
    if (!img || !rect || rect.width <= 0 || rect.height <= 0) return null;
    const w = Math.max(1, Math.round(rect.width));
    const h = Math.max(1, Math.round(rect.height));
    const c = document.createElement('canvas');
    ...
    const resolvedLayout = options.subjectCrop ? 'subject-fill' : layout;
    const ok = drawSpeakerCutoutOverQuilt(g, img, inner, {
      ...options,
      isCutoutPng: options.isCutoutPng !== false,
      layout: resolvedLayout,
      skipScannerBed: true,
    ...
    const sourceSil =
      options.isCutoutPng !== false
        ? makeSpeakerPlainCutoutAlphaSilhouetteCanvas(
            img,
            w,
            h,
            resolvedLayout,
            options.subjectCrop || null
          )
        : null;
    const bakedSil = makeAlphaSilhouetteFromCanvasCtx(g, w, h);
    const clipMask = bakedSil || sourceSil;

    if (clipMask) {
      flattenCutoutAlphaToOpaque(c, clipMask);
    }

    if (options.bleedPhase === 'quilt-screen' && clipMask) {
      try {
        const flat = g.getImageData(0, 0, w, h);
        const maskData = clipMask.getContext('2d')?.getImageData(0, 0, w, h)?.data || null;
        applyQuiltScreenCompositeDarkerDarksPass(flat.data, w, h, maskData);
        g.putImageData(flat, 0, 0);
      } catch (_) {
        /* canvas taint or readback blocked */
      }
    }

    if (options.paperTooth === true && clipMask) {
      drawCanvasLayerClippedToMask(g, clipMask, 0, 0, w, h, (lx, lw, lh) => {
        applyIgSpeakerPaperToothLayer(lx, lw, lh, options);
      });
    }
  ...
  function buildSpeakerOverQuiltCompositeDataUrl(img, outW, outH, layout = 'contain', options = {}) {
    try {
      const iw = Math.max(1, img.naturalWidth || img.width);
      const ih = Math.max(1, img.naturalHeight || img.height);
      const w = Math.max(1, Math.round(outW || iw));
      const h = Math.max(1, Math.round(outH || ih));
      const layer = buildSpeakerOverQuiltLayerCanvas(
        img,
  ...
  async function buildSpeakerOverQuiltCompositeDataUrlAsync(
    img,
    outW,
    outH,
    layout = 'contain',
    ...
    return buildSpeakerOverQuiltCompositeDataUrl(img, outW, outH, layout, options);
  }
  ...
  function loadSpeakerImageElement(url, timeoutMs = 8000) {
  ...
  function applyNewspaperFilmGrainPass(d, width, height, seed = 'odq', opts = {}) {
    const blackFloor = opts.blackFloor ?? XEROX_TONE.blackFloor;
    const refPixels = 300 * 450;
    const scale = Math.sqrt((width * height) / refPixels);
    const amp = (opts.grainAmp ?? 7) * Math.min(1.35, Math.max(0.85, scale));
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < width; x += 1) {
        const i = (y * width + x) * 4;
        if (d[i + 3] < 8) continue;
        ...
        const n = xeroxHashSeedToUnit(seed, x, y) * 2 - 1;
  ...
  /** Soft floor for face detail — full style (1) keeps current Xerox; 0 keeps midtones. */
  const XEROX_TONE_SOFT = Object.freeze({
    contrast: 1.08,
    brightness: 1.0,
    blackFloor: 28,
    whiteCap: 252,
  ...
  function normalizeSpeakerStyleAmount(raw) {
    const n = Number(raw);
  ...
  function resolveXeroxToneForStyleAmount(styleAmount, warmTape = false) {
    const a = normalizeSpeakerStyleAmount(styleAmount);
    const hard = warmTape
      ? { ...XEROX_TONE, whiteCap: 255, blackFloor: 58 }
      : { ...XEROX_TONE };
    const soft = warmTape
      ? { ...XEROX_TONE_SOFT, whiteCap: 255, blackFloor: 36 }
      : { ...XEROX_TONE_SOFT };
    return {
      contrast: lerpNum(soft.contrast, hard.contrast, a),
      brightness: lerpNum(soft.brightness, hard.brightness, a),
      blackFloor: Math.round(lerpNum(soft.blackFloor, hard.blackFloor, a)),
      whiteCap: Math.round(lerpNum(soft.whiteCap, hard.whiteCap, a)),
      bandStrength: lerpNum(soft.bandStrength, hard.bandStrength, a),
      bandPeriodPx: hard.bandPeriodPx,
      grainAmp: lerpNum(soft.grainAmp, hard.grainAmp, a)
    };
  }

  /**
   * Pull crushed Xerox highlights back toward face midtones.
   * restoreAmount 0 = no change; 1 = strongest soften.
   */
  function applySpeakerMidtoneRestorePass(d, width, height, restoreAmount) {
    const r = Math.min(1, Math.max(0, Number(restoreAmount) || 0));
    if (r < 0.01) return;
    const contrastPull = lerpNum(1, 0.58, r);
    const highlightPull = 0.55 * r;
    const shadowLift = 0.22 * r;
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < width; x += 1) {
        const i = (y * width + x) * 4;
        if (d[i + 3] < 8) continue;
        ...
        const centered = (lum - 128) * contrastPull + 128;
        let target = centered;
        if (lum > 175) {
          const t = Math.min(1, (lum - 175) / 80);
          target = lerpNum(centered, 168 + (255 - lum) * 0.15, t * highlightPull);
        } else if (lum < 90) {
          const t = 1 - lum / 90;
          target = lerpNum(centered, lum + (110 - lum) * 0.45, t * shadowLift);
        }
        const delta = target - lum;
        d[i] = Math.max(0, Math.min(255, Math.round(rr + delta)));
        d[i + 1] = Math.max(0, Math.min(255, Math.round(gg + delta * 0.98)));
        d[i + 2] = Math.max(0, Math.min(255, Math.round(bb + delta * 0.96)));
      }
    }
  }
  ...
  /** Match server newspaper bake: warm sepia grayscale (not pure R≈G≈B). */
  ...
  function applyXeroxTonePass(d, width, height, seed = 'odq', opts = {}) {
    ...
    const {
      ...
      bandPeriodPx,
      grainAmp
    } = resolveXeroxToneForStyleAmount(opts.styleAmount, warmTape);
    for (let y = 0; y < height; y += 1) {
      const bandMul =
        1 -
        bandStrength *
  ...
  /** Extra shadow crush for quilt-screen speaker widget only (not IG story/post). */
  function applyQuiltScreenDarkerDarksPass(d, width, height) {
    const SHADOW_CEIL = 168;
    // Soften when the portrait is already dark (darker skin / inked bake).
    let sum = 0;
    let count = 0;
    for (let i = 0; i < d.length; i += 4) {
    ...
    const subjectMean = count ? sum / count : 140;
    const darkGap = Math.max(0, Math.min(1, (95 - subjectMean) / 55));
    const INK_FLOOR = Math.round(24 + darkGap * 36);
    const CRUSH = 0.52 * (1 - darkGap * 0.72);
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < width; x += 1) {
        const i = (y * width + x) * 4;
        if (d[i + 3] < 8) continue;
        const lum = 0.299 * d[i] + 0.587 * d[i + 1] + 0.114 * d[i + 2];
        if (lum > SHADOW_CEIL) continue;
        const t = 1 - lum / SHADOW_CEIL;
        const factor = 1 - CRUSH * (0.2 + t * 0.8);
        d[i] = Math.max(INK_FLOOR, Math.round(d[i] * factor));
        d[i + 1] = Math.max(INK_FLOOR, Math.round(d[i + 1] * factor * 0.97));
        d[i + 2] = Math.max(INK_FLOOR - 6, Math.round(d[i + 2] * factor * 1.08));
      }
    }
  }

  function makeAlphaSilhouetteFromCanvasCtx(sourceCtx, w, h, minAlpha = 20) {
    ...
    let image;
    try {
      image = sourceCtx.getImageData(0, 0, w, h);
    } catch (_) {
      return null;
    }
    const d = image.data;
  ...
  /** Final flattened bake — crush visible shadows after cream/wash/grain (quilt widget only). */
  function applyQuiltScreenCompositeDarkerDarksPass(d, width, height, maskData = null) {
    const SHADOW_CEIL = 205;
    const FLOOR = 12;
    const CRUSH = 0.48;
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < width; x += 1) {
        const i = (y * width + x) * 4;
        if (maskData && maskData[i + 3] < 8) {
          d[i] = 0;
          d[i + 1] = 0;
          d[i + 2] = 0;
          d[i + 3] = 0;
        ...
        if (lum >= SHADOW_CEIL) continue;
        const t = 1 - lum / SHADOW_CEIL;
        const factor = 1 - CRUSH * (0.12 + t * 0.88);
        d[i] = Math.max(FLOOR, Math.round(d[i] * factor));
        d[i + 1] = Math.max(FLOOR, Math.round(d[i + 1] * factor * 0.98));
        d[i + 2] = Math.max(FLOOR, Math.round(d[i + 2] * factor * 1.04));
      }
    }
  }

  function applySmoothLithographPass(d, width, height, opts = {}) {
    const a = normalizeSpeakerStyleAmount(opts.styleAmount);
    const contrast = lerpNum(1.06, 1.38, a);
    const brightness = lerpNum(1.0, 1.12, a);
    const floor = Math.round(lerpNum(12, 18, a));
    const ceil = Math.round(lerpNum(255, 252, a));
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < width; x += 1) {
        const i = (y * width + x) * 4;
        const alpha = d[i + 3];
        ...
        let lum = 0.299 * d[i] + 0.587 * d[i + 1] + 0.114 * d[i + 2];
        lum = ((lum / 255 - 0.5) * contrast + 0.5) * 255 * brightness;
        lum = Math.max(floor, Math.min(ceil, lum));
        const v = Math.round(lum);
        d[i] = v;
        d[i + 1] = v;
        d[i + 2] = v;
  ...
  function applyWarmSmoothLithographPass(d, width, height, opts = {}) {
    const a = normalizeSpeakerStyleAmount(opts.styleAmount);
    const hardFloor = opts.quiltScreenDarkerDarks === true ? 28 : 58;
    const hardContrast = opts.quiltScreenDarkerDarks === true ? 1.32 : 1.18;
    const hardBright = opts.quiltScreenDarkerDarks === true ? 1.02 : 1.1;
    const INK_FLOOR = Math.round(lerpNum(18, hardFloor, a));
    const contrast = lerpNum(1.04, hardContrast, a);
    const brightness = lerpNum(1.0, hardBright, a);
    const warmBoostHard = opts.quiltScreenDarkerDarks === true ? 4 : 10;
    const warmBoost = Math.round(lerpNum(2, warmBoostHard, a));
    const gBoostHard = opts.quiltScreenDarkerDarks === true ? 2 : 4;
    const gBoost = Math.round(lerpNum(1, gBoostHard, a));
    const bDropHard = opts.quiltScreenDarkerDarks === true ? 22 : 16;
    const bDrop = Math.round(lerpNum(6, bDropHard, a));
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < width; x += 1) {
        const i = (y * width + x) * 4;
        const alpha = d[i + 3];
        ...
        const v = Math.round(lum);
        d[i] = Math.min(255, v + warmBoost);
        d[i + 1] = Math.min(255, v + gBoost);
        d[i + 2] = Math.max(0, v - bDrop);
      }
    }
  }

  /** IG colored stock: only dark photocopy ink prints — highlights stay paper-colored. */
  function applyIgCollageInkAlphaPass(d, width, height) {
    const HIGH_CUT = 208;
    const LOW_CUT = 152;
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < width; x += 1) {
        ...
        if (lum >= HIGH_CUT) {
          ...
          const t = (lum - LOW_CUT) / (HIGH_CUT - LOW_CUT);
          d[i + 3] = Math.round(alpha * (1 - t));
        }
      }
    }
  }
   ...
   */
  function applyMultiplyDuotonePass(d, width, height, opts = {}) {
    const a = normalizeSpeakerStyleAmount(opts.styleAmount);
    if (a < 0.02) return;
    const quiltDark = opts.quiltScreenDarkerDarks === true;
    const SHADOW_CEIL = quiltDark ? 148 : 132;
    ...
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < width; x += 1) {
        ...
        if (lum >= HIGHLIGHT_FLOOR) {
          ...
          const t = 1 - lum / SHADOW_CEIL;
          const crush = (quiltDark ? 0.34 + t * 0.52 : 0.2 + t * 0.42) * a;
          ...
          if (mid < 0.5) {
            const t = (0.5 - mid) * 2;
            d[i] = Math.round(d[i] * (1 - t * 0.1 * a));
            d[i + 1] = Math.round(d[i + 1] * (1 - t * 0.1 * a));
            d[i + 2] = Math.round(d[i + 2] * (1 - t * 0.12 * a));
          } else {
            const t = (mid - 0.5) * 2;
            d[i] = Math.round(d[i] + (255 - d[i]) * t * 0.18 * a);
            d[i + 1] = Math.round(d[i + 1] + (255 - d[i + 1]) * t * 0.18 * a);
            d[i + 2] = Math.round(d[i + 2] + (255 - d[i + 2]) * t * 0.14 * a);
          }
        }
      }
    }
  ...
  function buildSpeakerCreamBackdropCanvas(portrait, opts = {}) {
    ...
    const g = c.getContext('2d');
    if (!g) return null;
    g.fillStyle = opts.paperColor
      ? safeHex(opts.paperColor, '#f2eee6')
      : opts.brightenBackground === true
        ? '#faf6ee'
        : '#f2eee6';
  ...
  function makeSpeakerPortraitCanvas(img, outW, outH, isCutoutPng = false, portraitOpts = {}) {
    ...
    const styleAmount = normalizeSpeakerStyleAmount(portraitOpts.styleAmount);
    const alreadyXerox = isCutoutPng && rgbaLooksSpeakerCutoutXerox(d, c.width, c.height);
    // Storage cutouts are pre-toned — re-lithographing at screen px destroys edge detail.
    // When styleAmount < 1, restore crushed face midtones instead of leaving the plate as-is.
    if (alreadyXerox && isCutoutPng && portraitOpts.smoothLithograph === true) {
      applySpeakerMidtoneRestorePass(d, c.width, c.height, 1 - styleAmount);
      if (portraitOpts.quiltScreenDarkerDarks === true && styleAmount > 0.35) {
        applyQuiltScreenDarkerDarksPass(d, c.width, c.height);
      }
      g.putImageData(image, 0, 0);
      return c;
    }
    // Treatment is baked into cutout PNGs by the upload pipeline — never re-tone them here.
    const mustToneCutout = false;
    const toneOpts = {
      quiltScreenDarkerDarks: portraitOpts.quiltScreenDarkerDarks === true,
      styleAmount
    };
    if (portraitOpts.smoothLithograph === true) {
      if (portraitOpts.warmLithograph === true) {
        applyWarmSmoothLithographPass(d, c.width, c.height, toneOpts);
      } else {
        applySmoothLithographPass(d, c.width, c.height, toneOpts);
      }
      if (portraitOpts.quiltBleedAlpha === true) {
        ...
        applyMultiplyDuotonePass(d, c.width, c.height, toneOpts);
      }
      ...
      if (portraitOpts.skipHalftone) {
        ...
        if (portraitOpts.filmGrain !== false) {
          applyNewspaperFilmGrainPass(d, c.width, c.height, toneSeed);
        }
      } else {
        applyHalftonePass(d, c.width, c.height);
      }
    } else if (
      portraitOpts.skipHalftone === true &&
      portraitOpts.filmGrain !== false &&
      isCutoutPng &&
      alreadyXerox
    ) {
      applySpeakerMidtoneRestorePass(d, c.width, c.height, 1 - styleAmount);
      applyNewspaperFilmGrainPass(d, c.width, c.height, toneSeed, { grainAmp: 4 * styleAmount });
    } else if (alreadyXerox && isCutoutPng && styleAmount < 0.999) {
      applySpeakerMidtoneRestorePass(d, c.width, c.height, 1 - styleAmount);
    }
    if (portraitOpts.coloredPaperInk === true) {
      applyIgCollageInkAlphaPass(d, c.width, c.height);
    }
    if (portraitOpts.quiltScreenDarkerDarks === true) {
      applyQuiltScreenDarkerDarksPass(d, c.width, c.height);
    }
    g.putImageData(image, 0, 0);
    return c;
  ...
  /**
   * Soften a pre-baked (or live) speaker bitmap for sticker/compose paths.
   * Returns null when styleAmount is full (1) so callers can keep the original image.
   */
  function applySpeakerStyleAmountToImage(img, styleAmount, options = {}) {
    const amount = normalizeSpeakerStyleAmount(styleAmount);
    if (!img || amount >= 0.999) return null;
    const iw = Math.max(1, img.naturalWidth || img.width || 0);
    const ih = Math.max(1, img.naturalHeight || img.height || 0);
    ...
    let image;
    try {
      image = g.getImageData(0, 0, iw, ih);
    } catch (_) {
      return null;
    }
    const d = image.data;
    const isCutoutPng = options.isCutoutPng !== false;
    if (isCutoutPng) maskCutoutFringe(d, iw, ih, true);
    const alreadyXerox = rgbaLooksSpeakerCutoutXerox(d, iw, ih);
    if (alreadyXerox || options.forceMidtoneRestore === true) {
      applySpeakerMidtoneRestorePass(d, iw, ih, 1 - amount);
    } else {
      applyXeroxTonePass(d, iw, ih, String(options.seed || 'odq'), {
        warmTape: options.warmTape === true,
        styleAmount: amount
      });
    }
    g.putImageData(image, 0, 0);
  ...
  function buildSpeakerGrainCanvas(portrait, options = {}) {
    if (!portrait) return null;
    const softGrain = options.softGrain === true || options.quiltScreen === true;
    ...
    gx.strokeStyle = softGrain ? 'rgba(35, 27, 20, 0.28)' : 'rgba(35, 27, 20, 0.78)';
    gx.lineWidth = softGrain ? 0.75 : 1;
    const diag = Math.ceil(Math.sqrt(W * W + H * H));
    const lineStep = softGrain ? 7 : 5;
    for (let y = -diag; y <= diag; y += lineStep) {
      gx.beginPath();
    ...
    gx.restore();
    gx.save();
    gx.strokeStyle = softGrain ? 'rgba(35, 27, 20, 0.16)' : 'rgba(35, 27, 20, 0.42)';
    gx.lineWidth = softGrain ? 0.65 : 0.85;
    const vertStep = softGrain ? 9 : 7;
    for (let x = 0; x <= W; x += vertStep) {
    ...
    gx.strokeStyle = softGrain ? 'rgba(35, 27, 20, 0.22)' : 'rgba(35, 27, 20, 0.65)';
    gx.lineWidth = softGrain ? 0.45 : 0.55;
    const ringStep = softGrain ? 6 : 4;
    for (let r = 4; r <= ringMaxR; r += ringStep) {
      gx.beginPath();
  ...
  function buildSpeakerRadialWashCanvas(portrait, washColor, opts = {}) {
    if (!portrait) return null;
    const brighten = opts.brightenBackground === true;
    const matchPaper = opts.matchNameStripPaper === true;
    const wash = parseSpeakerWashRgb(washColor);
    const washLayer = document.createElement('canvas');
    washLayer.width = portrait.width;
    washLayer.height = portrait.height;
    ...
    if (matchPaper) {
      grad.addColorStop(0, `rgba(${wash.r}, ${wash.g}, ${wash.b}, 0.92)`);
      grad.addColorStop(0.55, `rgba(${wash.r}, ${wash.g}, ${wash.b}, 0.9)`);
      grad.addColorStop(1, `rgba(${wash.r}, ${wash.g}, ${wash.b}, 0.88)`);
    } else if (brighten) {
      grad.addColorStop(0, `rgba(${wash.r}, ${wash.g}, ${wash.b}, ${brighten ? 0.46 : 0.58})`);
      grad.addColorStop(0.52, `rgba(${wash.r}, ${wash.g}, ${wash.b}, ${brighten ? 0.28 : 0.38})`);
      grad.addColorStop(0.82, brighten ? 'rgba(255,248,235,0.34)' : 'rgba(255,244,218,0.22)');
      grad.addColorStop(1, brighten ? 'rgba(255,248,235,0.44)' : 'rgba(255,244,218,0.32)');
    } else {
  ...
  /** IG slide 1 / Layout B carousel: opaque colored paper (tune modal Post + Feed slide 1). ?igSpeakerOpaque=1 overrides. */
  function isIgSpeakerOpaqueCollage(options = {}) {
    if (options.opaqueCollage === true) return true;
    if (options.opaqueCollage === false) return false;
    if (
      options.bleedPhase === 'layout-b-slide1' ||
      options.bleedPhase === 'carousel-seam-slide2' ||
    ...
    try {
      const qp =
        ...
        new URLSearchParams(location.search).get('igSpeakerOpaque');
      if (qp === '1' || qp === 'true') return true;
      if (qp === '0' || qp === 'false') return false;
    } catch (_) {
      /* ignore */
    }
    return globalThis.CONFIG?.APP?.igSpeakerOpaqueCollage === true;
  }
  ...
  function drawSpeakerCutoutPlain(g, img, rect, options = {}) {
    ...
    try {
      ...
      portrait = makeSpeakerPortraitCanvas(img, rect.width, rect.height, isCutoutPng, {
        skipHalftone: true,
        warmTapePaper: !newsprint && options.igCollagePaper !== true,
        smoothLithograph: options.igCollagePaper === true ? false : options.smoothLithograph === true,
        warmLithograph: options.igCollagePaper === true ? false : options.warmLithograph === true,
        coloredPaperInk: options.igCollagePaper === true,
        filmGrain: options.igCollagePaper === true ? false : options.filmGrain,
        seed,
        layout,
        subjectCrop,
        styleAmount: normalizeSpeakerStyleAmount(options.styleAmount)
      });
    ...
    if (!(options.skipPaperMat && isCutoutPng)) {
      ...
      if (csx) {
        ...
        const paper =
          options.igCollagePaper === true
            ? resolveIgCollagePaperColor(options)
            : safeHex(options.paper, SPEAKER_STRIP_PAPER);
        if (newsprint && QNC?.drawNewsprintSurfaceStack) {
          QNC.drawNewsprintSurfaceStack(csx, pw, ph, {
            paper,
            paperTextureUrl: null
          });
        } else if (options.igCollagePaper === true) {
          csx.fillStyle = paper;
          csx.fillRect(0, 0, pw, ph);
          csx.globalCompositeOperation = 'source-over';
          applySpeakerIgCollagePaperTextureInCtx(csx, pw, ph, paper);
        } else {
  ...
  function drawSpeakerCutoutOverQuilt(g, img, rect, options = {}) {
    ...
    const styleAmount = normalizeSpeakerStyleAmount(options.styleAmount);
    let portrait = null;
    try {
      portrait = makeSpeakerPortraitCanvas(img, rect.width, rect.height, isCutoutPng, {
        seed,
        ...
        warmLithograph: true,
        quiltScreenDarkerDarks: options.bleedPhase === 'quilt-screen',
    ...
    const quiltScreen = options.bleedPhase === 'quilt-screen';
    const washColor = options.washColor || '#ea9b9a';
    const matchPaper = options.matchNameStripPaper === true;
    const bgOpts = matchPaper
      ? { paperColor: washColor, matchNameStripPaper: true }
      : quiltScreen
        ? { brightenBackground: true }
        : {};
    const mis = buildMisregistration(createSeededRng(`${seed}:speaker-cutout-quilt`), options.misTune);
    const grain =
      options.layers?.grain === false || styleAmount < 0.05
        ? null
        : buildSpeakerGrainCanvas(portrait, { softGrain: quiltScreen, quiltScreen });
    const radialWash =
      useRadialWash && styleAmount > 0.02
        ? buildSpeakerRadialWashCanvas(portrait, washColor, bgOpts)
        : null;
    const creamBackdrop =
      useRadialWash && options.creamBackdrop !== false && styleAmount > 0.02
        ? buildSpeakerCreamBackdropCanvas(portrait, bgOpts)
        : null;
    const creamBackdropAlphaBase =
      options.creamBackdropAlpha != null
        ? options.creamBackdropAlpha
        : matchPaper
          ? 0.58
          : quiltScreen
            ? 0.5
            : 0.4;
    const creamBackdropAlpha = creamBackdropAlphaBase * (0.15 + 0.85 * styleAmount);
    const multiplyAlpha =
      options.portraitAlpha != null
        ? options.portraitAlpha
        : useRadialWash
          ...
          : 0.85;
    const washAlphaBase =
      options.washAlpha != null ? options.washAlpha : matchPaper ? 0.38 : quiltScreen ? 0.66 : 0.62;
    const washAlpha = washAlphaBase * styleAmount;
    const grainAlphaBase =
      options.grainAlpha != null ? options.grainAlpha : quiltScreen ? 0.12 : 0.2;
    const grainAlpha = grainAlphaBase * styleAmount;
    const skipBed =
  ...
  function drawSpeakerCutoutStack(g, img, rect, options = {}) {
    ...
    if (options.compositeOverQuilt === true) {
      if (isIgSpeakerOpaqueCollage(options)) {
        return drawSpeakerPlainCutoutLayer(g, img, rect, {
          ...options,
          skipScannerBed: true,
          drawScannerBed: false,
          newsprintSurface: false,
          igCollagePaper: true,
          coloredPaperInk: true,
          skipHalftone: true,
          filmGrain: false,
          portraitAlpha: options.portraitAlpha != null ? options.portraitAlpha : 0.86
        });
      }
    ...
    const styleAmount = normalizeSpeakerStyleAmount(options.styleAmount);
    const grain = styleAmount < 0.05 ? null : buildSpeakerGrainCanvas(portrait);
    const contourLayer = buildSpeakerContourCanvas(alphaSil || portrait, plan.contourTune);
    const washLayer =
      layers.wash && styleAmount > 0.02
      ...
      options.portraitAlpha != null
        ...
        : options.colorTestMode
          ...
          : isHeroPost
            ...
            : 0.95;
    const grainAlpha =
      (options.grainAlpha != null ? options.grainAlpha : isHeroPost ? 0.27 : 0.34) * styleAmount;
    const contourAlpha =
      options.contourAlpha != null
        ? options.contourAlpha
        : plan.contourTune?.opacity ?? DEFAULT_CONTOUR_TUNE.opacity;
    const washAlpha =
      (options.washAlpha != null ? options.washAlpha : isHeroPost ? 0.58 : 0.78) * styleAmount;
    const paperAlpha =
  ...
  root.SpeakerCutoutRender = {
    ...
    normalizeSpeakerStyleAmount,
    applySpeakerMidtoneRestorePass,
    applySpeakerStyleAmountToImage,
    ...
    buildSpeakerOverQuiltLayerCanvas,
    buildSpeakerOverQuiltCompositeDataUrl,
    buildSpeakerOverQuiltCompositeDataUrlAsync,
    ...
    fillSpeakerIgCollagePaperInCtx,
    applySpeakerIgCollagePaperTextureInCtx,
