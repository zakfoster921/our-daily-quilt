...
(function (root) {
  ...
  function applyMultiplyDuotonePass(d, width, height) {
    const SHADOW_CEIL = 118;
    const HIGHLIGHT_FLOOR = 158;
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < width; x += 1) {
        const i = (y * width + x) * 4;
        const alpha = d[i + 3];
        ...
        if (lum >= HIGHLIGHT_FLOOR) {
          ...
          const lift = 0.38 + t * 0.58;
          d[i] = Math.round(d[i] + (255 - d[i]) * lift);
          d[i + 1] = Math.round(d[i + 1] + (255 - d[i + 1]) * lift);
          d[i + 2] = Math.round(d[i + 2] + (255 - d[i + 2]) * lift * 0.9);
        } else if (lum <= SHADOW_CEIL) {
          const t = 1 - lum / SHADOW_CEIL;
          d[i] = Math.round(d[i] * (1 - t * 0.14));
          d[i + 1] = Math.round(d[i + 1] * (1 - t * 0.14));
          d[i + 2] = Math.round(d[i + 2] * (1 - t * 0.16));
        }
      }
    }
  }
  ...
  function drawSpeakerCutoutOverQuilt(g, img, rect, options = {}) {
    ...
    const multiplyAlpha =
      options.portraitAlpha != null ? options.portraitAlpha : useRadialWash ? 0.9 : 0.85;
