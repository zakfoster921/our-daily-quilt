...
(function (root) {
  ...
  function fillSpeakerIgCollagePaperInCtx(ctx, w, h, paperHex = IG_SPEAKER_COLLAGE_PAPER) {
    ...
    ctx.fillStyle = paper;
    ctx.fillRect(0, 0, width, height);
    ctx.fillStyle = 'rgba(118, 98, 68, 0.011)';
    ...
    sheen.addColorStop(0, 'rgba(255, 246, 232, 0.11)');
    sheen.addColorStop(0.5, 'rgba(255, 246, 232, 0)');
    ctx.fillStyle = sheen;
    ctx.fillRect(0, 0, width, height);
    ctx.fillStyle = 'rgba(235, 210, 175, 0.07)';
    ctx.fillRect(0, 0, width, height);
  }
  ...
  function drawSpeakerCutoutPlain(g, img, rect, options = {}) {
    ...
    if (!(options.skipPaperMat && isCutoutPng)) {
      ...
      if (csx) {
        ...
        const paper = safeHex(
          options.paper,
          options.igCollagePaper === true ? IG_SPEAKER_COLLAGE_PAPER : SPEAKER_STRIP_PAPER
        );
    ...
    if (options.compositeOverQuilt === true) {
      if (isIgSpeakerOpaqueCollage(options)) {
        return drawSpeakerPlainCutoutLayer(g, img, rect, {
          ...
          igCollagePaper: true,
          paper: options.paper || IG_SPEAKER_COLLAGE_PAPER,
          smoothLithograph: true,
          warmLithograph: true,
          skipHalftone: true,
