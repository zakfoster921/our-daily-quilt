...
(function (root) {
  ...
  /** Soft colored stock for IG speaker mat — quilt swatch washed out, merged with cream base. */
  function resolveIgCollagePaperColor(options = {}) {
    ...
    const fallback = safeHex(options.washColor || IG_SPEAKER_COLLAGE_PAPER);
    const palette =
      Array.isArray(options.palette) && options.palette.length ? options.palette : [fallback];
    const sources = pickSpeakerQuiltSources(palette, fallback);
    const quiltTint = washOutSpeakerQuiltColor(sources.wash, 'paper');
    return mixHexColors(quiltTint, IG_SPEAKER_COLLAGE_PAPER, 0.24);
  }
  ...
  function drawSpeakerCutoutPlain(g, img, rect, options = {}) {
    ...
    try {
      ...
      portrait = makeSpeakerPortraitCanvas(img, rect.width, rect.height, isCutoutPng, {
        ...
        warmLithograph: options.warmLithograph === true,
        seed,
        layout,
        subjectCrop
      });
    ...
    if (!(options.skipPaperMat && isCutoutPng)) {
      ...
      if (csx) {
        ...
        if (newsprint && QNC?.drawNewsprintSurfaceStack) {
          ...
          fillSpeakerIgCollagePaperInCtx(csx, pw, ph, paper);
        } else {
    ...
    if (options.compositeOverQuilt === true) {
      if (isIgSpeakerOpaqueCollage(options)) {
        return drawSpeakerPlainCutoutLayer(g, img, rect, {
          ...
          warmLithograph: true,
          skipHalftone: true,
          portraitAlpha: options.portraitAlpha != null ? options.portraitAlpha : 0.78
        });
      }
