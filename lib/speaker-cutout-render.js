...
(function (root) {
  ...
  function buildSpeakerOverQuiltLayerCanvas(img, rect, options = {}) {
    ...
    if (!ok) return null;
    if (options.paperTooth === true) {
      const sil = makeSpeakerPlainCutoutAlphaSilhouetteCanvas(
        img,
        w,
        h,
        resolvedLayout,
    ...
    if (options.bleedPhase === 'quilt-screen') {
      try {
        const flat = g.getImageData(0, 0, w, h);
        applyQuiltScreenCompositeDarkerDarksPass(flat.data, w, h);
  ...
  function applyQuiltScreenCompositeDarkerDarksPass(d, width, height) {
