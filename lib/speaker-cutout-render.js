...
(function (root) {
    ...
    try {
      ...
      for (let y = 0; y < h; y += stepY) {
        for (let x = 0; x < w; x += stepX) {
          ...
          if (lum > bestLum) {
            bestLum = lum;
            best = { r: data[i], g: data[i + 1], b: data[i + 2], a: data[i + 3] };
          }
        }
      }
      return best;
  ...
  function applyQuiltBleedLuminanceAlpha(d, width, height) {
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < width; x += 1) {
        ...
        if (t > 0.32) {
          const lift = (t - 0.32) / 0.68;
          const liftAmt = 0.42 + lift * 0.48;
          d[i] = Math.round(d[i] + (255 - d[i]) * lift * liftAmt);
          d[i + 1] = Math.round(d[i + 1] + (255 - d[i + 1]) * lift * liftAmt);
          d[i + 2] = Math.round(d[i + 2] + (255 - d[i + 2]) * lift * (liftAmt - 0.08));
        }
        lum = 0.299 * d[i] + 0.587 * d[i + 1] + 0.114 * d[i + 2];
        const t2 = Math.max(0, Math.min(1, lum / 255));
        const bleed = Math.pow(1 - t2, 0.82);
        d[i + 3] = Math.round(Math.min(255, alpha * (0.06 + 0.72 * bleed)));
      }
    }
  }
  ...
  function drawSpeakerCutoutOverQuilt(g, img, rect, options = {}) {
    ...
    const multiplyAlpha =
      options.portraitAlpha != null ? options.portraitAlpha : 0.62;
    const creamTintAlpha = 0;
    const grainAlpha = options.grainAlpha != null ? options.grainAlpha : 0.08;
