...
(function (root) {
  ...
  function applyWarmSmoothLithographPass(d, width, height) {
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < width; x += 1) {
        ...
        let lum = 0.299 * d[i] + 0.587 * d[i + 1] + 0.114 * d[i + 2];
        lum = ((lum / 255 - 0.5) * 1.34 + 0.5) * 255 * 1.14;
        lum = Math.max(22, Math.min(248, lum));
        const v = Math.round(lum);
        d[i] = Math.min(255, v + 6);
        d[i + 1] = Math.min(255, v + 2);
        d[i + 2] = Math.max(0, v - 10);
      }
    }
  }
  ...
  /**
   * Lithograph multiply over quilt (Layout B IG): warm cream ink + soft flatbed bloom + light grain.
   * Scanner bed uses soft-light (not opaque source-over) so quilt colors still tint the portrait.
   */
  function drawSpeakerCutoutOverQuilt(g, img, rect, options = {}) {
    ...
    const multiplyAlpha =
      options.portraitAlpha != null ? options.portraitAlpha : 0.9;
    const grainAlpha = options.grainAlpha != null ? options.grainAlpha : 0.24;
