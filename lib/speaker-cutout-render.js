  ...
  function drawSpeakerCutoutOverQuilt(g, img, rect, options = {}) {
    ...
    try {
      portrait = makeSpeakerPortraitCanvas(img, rect.width, rect.height, isCutoutPng, {
        ...
        warmLithograph: true,
        // Radial wash (June 4) opens cream/transparency — skip per-pixel bleed when both stack.
        quiltBleedAlpha: useRadialWash ? false : true,
        layout,
        subjectCrop
      });
    } catch (_) {
    ...
    const multiplyAlpha =
      options.portraitAlpha != null ? options.portraitAlpha : useRadialWash ? 0.95 : 0.85;
