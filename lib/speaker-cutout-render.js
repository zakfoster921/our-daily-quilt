...
(function (root) {
  ...
  const _odqDbg7240SamplePortrait = (portrait) => {
    try {
      ...
      const x = Math.max(0, Math.round(portrait.width / 2) - 1);
      const y = Math.max(0, Math.round(portrait.height / 2) - 1);
      const d = px.getImageData(x, y, 1, 1).data;
      return { r: d[0], g: d[1], b: d[2], a: d[3] };
    } catch (_) {
      return null;
    }
    ...
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < width; x += 1) {
        ...
        if (t > 0.58) {
          ...
          d[i] = Math.round(d[i] + (255 - d[i]) * lift * 0.62);
          d[i + 1] = Math.round(d[i + 1] + (255 - d[i + 1]) * lift * 0.62);
          d[i + 2] = Math.round(d[i + 2] + (255 - d[i + 2]) * lift * 0.52);
        }
        const bleed = Math.pow(1 - t, 1.25);
        d[i + 3] = Math.round(Math.min(255, alpha * (0.22 + 0.78 * bleed)));
      }
    }
  }
  ...
  function drawSpeakerCutoutOverQuilt(g, img, rect, options = {}) {
    ...
    const creamTint = null;
    const multiplyAlpha =
      options.portraitAlpha != null ? options.portraitAlpha : 0.88;
    const creamTintAlpha = 0;
    const grainAlpha = options.grainAlpha != null ? options.grainAlpha : 0.14;
    const skipBed =
    ...
    if (creamTint) {
      drawOffsetLayer(g, creamTint, rect, mis.portrait, creamTintAlpha, 'multiply');
    }
