...
(function (root) {
  ...
  /** Match clipping newsprint `#f6f4f1` (nightly peek / lab reference). */
  const MAT_RGB = '246, 244, 241';
  /** Same flat field as quilt newspaper clipping paper base. */
  const SPEAKER_STRIP_PAPER = '#f6f4f1';
  const BAYER_4 = [
    [0, 8, 2, 10],
    [12, 4, 14, 6],
    [3, 11, 1, 9],
  ...
  function fillSpeakerStripPaperInCtx(ctx, w, h) {
    ...
    if (QNC?.drawNewsprintSurfaceStack) {
      QNC.drawNewsprintSurfaceStack(ctx, w, h, { paper: SPEAKER_STRIP_PAPER, paperTextureUrl: null });
      return;
    }
    const pad = 24;
  ...
  function speakerNewsprintSurfaceEnabled(options = {}) {
    return options.newsprintSurface !== false;
  }
   ...
   */
  function createSpeakerPortraitScannerBedDrawer(seedKey) {
    const QNC = root.QuiltNewspaperClipping || globalThis.QuiltNewspaperClipping;
  ...
  function makeSpeakerPlainCutoutAlphaSilhouetteCanvas(img, outW, outH, layout = 'width-fill') {
    const alpha = makeSpeakerAlphaSilhouetteCanvas(img, outW, outH, true, layout, false);
    if (silhouetteHasOpaquePixels(alpha)) return alpha;
    return makeSpeakerFilledSilhouetteCanvas(img, outW, outH, true, layout);
  }

  function fillPaperIntoSilhouetteCanvas(silhouette, paperHex) {
  ...
  function buildSpeakerFilledPaperSilhouetteDataUrl(img, outW, outH, layout = 'width-fill', options = {}) {
    ...
    const filled = fillPaperIntoSilhouetteCanvas(sil, paper);
    if (!filled) return '';
    try {
  ...
  function buildSpeakerPlainCutoutLayerCanvas(img, rect, options = {}) {
    ...
    if (!ok) return null;
    if (speakerNewsprintSurfaceEnabled(options)) {
      const sil = makeSpeakerPlainCutoutAlphaSilhouetteCanvas(img, w, h, layout);
  ...
  function applyXeroxTonePass(d, width, height, seed = 'odq') {
  ...
  /** B/W cutout only — newsprint mat + lithograph multiply (matches clipping nightly peek). */
  function drawSpeakerCutoutPlain(g, img, rect, options = {}) {
    ...
    try {
      portrait = makeSpeakerPortraitCanvas(img, rect.width, rect.height, isCutoutPng, {
        skipHalftone: newsprint,
        smoothLithograph: options.smoothLithograph === true,
        seed,
        layout
      });
      alphaSil = makeSpeakerAlphaSilhouetteCanvas(img, rect.width, rect.height, isCutoutPng, layout);
      backingSil = isCutoutPng
        ? makeSpeakerFilledSilhouetteCanvas(img, rect.width, rect.height, isCutoutPng, layout)
        : alphaSil;
      clipSil = isCutoutPng
        ? makeSpeakerPlainCutoutAlphaSilhouetteCanvas(img, rect.width, rect.height, layout)
        : alphaSil;
    } catch (_) {
      return false;
    }
...

    {
      ...
      if (csx) {
        ...
        if (newsprint && QNC?.drawNewsprintSurfaceStack) {
          ...
          csx.fillStyle = safeHex(options.paper, SPEAKER_STRIP_PAPER);
