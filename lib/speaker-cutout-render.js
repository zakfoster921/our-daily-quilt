...
(function (root) {
  ...
  /** Warm cream lithograph ink (R>G>B) — quilt multiply reads creamy, not cold gray. */
  function applyWarmSmoothLithographPass(d, width, height) {
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < width; x += 1) {
        ...
        let lum = 0.299 * d[i] + 0.587 * d[i + 1] + 0.114 * d[i + 2];
        lum = ((lum / 255 - 0.5) * 1.32 + 0.5) * 255 * 1.16;
        lum = Math.max(20, Math.min(255, lum));
        const v = Math.round(lum);
        d[i] = Math.min(255, v + 8);
        d[i + 1] = Math.min(255, v + 3);
        d[i + 2] = Math.max(0, v - 14);
      }
    }
  }
  ...
  function drawSpeakerCutoutOverQuilt(g, img, rect, options = {}) {
    ...
    const creamTint =
      options.creamTint === false ? null : buildSpeakerWarmCreamTintCanvas(portrait);
    const multiplyAlpha =
      options.portraitAlpha != null ? options.portraitAlpha : 0.86;
    const creamTintAlpha =
      options.creamTintAlpha != null ? options.creamTintAlpha : 0.18;
    const grainAlpha = options.grainAlpha != null ? options.grainAlpha : 0.22;
    const skipBed =
