...
/** Split carousel cards only fit brief single-line responses (both strips must qualify). */
function reflectionStripsEligibleForSplit(strips) {
  const list = Array.isArray(strips) ? strips : [];
  if (list.length !== 2) return false;
  return list.every((strip) => isShortReflectionForSplit(strip?.text ?? strip?.body ?? strip?.theme));
}
...
function buildReflectionCurationPrompt({ reflectionPrompt, items }) {
  ...
  const instructions = [
    ...
    '',
    'Priority: pair only responses that mean the same thing (similar ideas, not necessarily identical words).',
    ...
    '- Identical wording from multiple people is consolidated server-side into one card with combined names.',
    '- When two brief responses (about six words or fewer each) mean the same thing but use different words, pair them in one group of exactly two ids (top + bottom on one split card).',
    '- Do not pair longer responses on a split card — give each its own single-id group.',
