...
(function (root, factory) {
  ...
  function (global) {
    ...
    function responseCardAnchors(count) {
      ...
      const bottomY = PROMPT_CARD_Y + PROMPT_CARD_H + gap;
      const slotOffsetX = (PROMPT_CARD_W - RESPONSE_CARD_W) / 2;
      const slotOffsetY = (PROMPT_CARD_H - RESPONSE_CARD_H) / 2;
      const anchor = (x, y) => ({
        x: x + slotOffsetX,
        y: y + slotOffsetY
      });
      if (count === 1) {
        return [anchor(rightX, PROMPT_CARD_Y)];
      }
      if (count === 2) {
        return [
          anchor(rightX, PROMPT_CARD_Y),
          anchor(PROMPT_CARD_X, bottomY)
        ];
      }
