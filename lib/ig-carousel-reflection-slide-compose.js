  ...
  function (global) {
    ...
    function buildJoinTapes(placements, dateKey) {
      ...
      if (placements.length >= 2) {
        ...
        const b = placements[1];
        if (sameRow(a, b)) {
          const { top, span } = patchOverlapSpan(a, b);
          tapes.push({
            kind: 'vertical',
            centerX: b.x,
            overlapTop: top,
      ...
      if (placements.length >= 3) {
        const upper = placements[1];
        const lower = placements[2];
        if (sameCol(upper, lower)) {
          tapes.push({
            kind: 'horizontal',
            left: upper.x,
            width: upper.width,
            centerY: lower.y,
            seedKey: `${dateKey}:reflection-split:1`
          });
        } else if (sameRow(upper, lower)) {
          const { top, span } = patchOverlapSpan(upper, lower);
          tapes.push({
            kind: 'vertical',
            centerX: lower.x,
            overlapTop: top,
            overlapSpan: span,
            seedKey: `${dateKey}:reflection-join:2`
          });
        }
      }
      return tapes;
    ...
    function backwardsLClusterOrigin() {
      const gap = 8;
      return {
        x: PROMPT_CARD_X + PROMPT_CARD_W + gap,
        y: PROMPT_CARD_Y
      };
    }

    /** Backwards L: top row left→right, third patch drops below the second (crook opens toward question). */
    function responseCardAnchors(count) {
      const origin = backwardsLClusterOrigin();
      const stepX = RESPONSE_CARD_W - PATCH_JOIN_OVERLAP;
      const stepY = RESPONSE_CARD_H - PATCH_JOIN_OVERLAP;
      if (count === 1) {
      ...
      if (count === 2) {
        return [
          ...
          { x: origin.x + stepX, y: origin.y }
        ];
      }
      return [
        { x: origin.x, y: origin.y },
        { x: origin.x + stepX, y: origin.y },
        { x: origin.x + stepX, y: origin.y + stepY }
      ];
    }
    ...
    async function composeCarouselReflectionSlideFromQuiltBlob(quiltBlob, options = {}) {
      ...
      const patchDrawOrder = placements.length >= 3 ? [0, 2, 1] : placements.map((_, index) => index);
