...
(function (root, factory) {
  ...
  function (global) {
    ...
    const PATCH_FONT_PX = 32;
    const PATCH_LINE_HEIGHT = 1.32;
    const PATCH_PAD_X = 30;
    const PATCH_PAD_Y = 34;
    const PATCH_MIN_W = 300;
    const PATCH_MIN_H = 200;
    ...
    function estimatePatchSize(ctx, entry, slotW, slotH) {
      const body = String(entry.text || '').trim();
      const author = String(entry.author || '').trim();
      const w = Math.max(PATCH_MIN_W, Math.round(slotW));
      const h = Math.max(PATCH_MIN_H, Math.round(slotH));
      const innerW = Math.max(120, w - PATCH_PAD_X * 2);
      const lines = wrapTextLines(ctx, body, innerW, 8);
    ...
    function drawFabricPatch(ctx, x, y, w, h, rgbCsv, seedKey, highlighted, layout) {
      ...
      const textBlockH =
        layout.lines.length * layout.lineStep +
        (layout.author ? PATCH_FONT_PX * 0.85 * PATCH_LINE_HEIGHT + 10 : 0);
      let cy = Math.max(PATCH_PAD_Y, (h - textBlockH) / 2) + PATCH_FONT_PX;
      layout.lines.forEach((line) => {
        ctx.fillText(line, PATCH_PAD_X, cy);
        cy += layout.lineStep;
      });
      if (layout.author) {
        ctx.font = `400 ${Math.round(PATCH_FONT_PX * 0.82)}px "DM Sans", system-ui, -apple-system, "Segoe UI", sans-serif`;
        ctx.textAlign = 'right';
        ctx.fillText(`— ${layout.author}`, w - PATCH_PAD_X, h - PATCH_PAD_Y + 6);
      }
      if (highlighted) {
    ...
    function patchWallSlots(count, questionRect) {
      const m = PANEL_MARGIN;
      const gap = 22;
      const bottomBandY = questionRect.y + questionRect.height - 40;
      const slots = [];
      ...
      if (count === 1) {
        slots.push({
          x: questionRect.x + questionRect.width + gap,
          y: m,
          w: PANEL_W - m - (questionRect.x + questionRect.width + gap),
          h: PANEL_H - m * 2
        });
        return slots;
      }
      ...
      if (count === 2) {
        const colX = questionRect.x + questionRect.width + gap;
        const colW = PANEL_W - m - colX;
        const colH = Math.floor((PANEL_H - m * 2 - gap) / 2);
        slots.push({ x: colX, y: m, w: colW, h: colH });
        slots.push({ x: colX, y: m + colH + gap, w: colW, h: colH });
        return slots;
      }

      const rightColX = questionRect.x + questionRect.width - 24;
      const rightColW = PANEL_W - m - rightColX;
      const upperH = Math.round((bottomBandY - m - gap) / 2);
      slots.push({ x: rightColX, y: m, w: rightColW, h: upperH });
      slots.push({
        x: rightColX - 36,
        y: m + upperH + gap,
        w: rightColW,
      ...
      const bottomY = bottomBandY;
      const bottomH = PANEL_H - m - bottomY;
      slots.push({
        x: m,
        y: bottomY,
        w: PANEL_W - m * 2,
    ...
    function layoutWallPatches(entries, measureCtx, dateKey) {
      const questionRect = questionOccupiedRect();
      const slots = patchWallSlots(entries.length, questionRect);
      const placements = [];

      entries.forEach((entry, index) => {
        const slot = slots[index] || slots[slots.length - 1];
        const inset = 12;
        const slotW = Math.max(PATCH_MIN_W, slot.w - inset * 2);
        const slotH = Math.max(PATCH_MIN_H, slot.h - inset * 2);
        const layout = estimatePatchSize(measureCtx, entry, slotW, slotH);
        const w = layout.width;
        const h = layout.height;
        const jitterX = (seededUnit(dateKey, `${entry.seedKey}:jx:${index}`) - 0.5) * 28;
        const jitterY = (seededUnit(dateKey, `${entry.seedKey}:jy:${index}`) - 0.5) * 20;
        const x = Math.round(slot.x + (slot.w - w) / 2 + jitterX);
        const y = Math.round(slot.y + (slot.h - h) / 2 + jitterY);
        const tiltDeg =
          patchTiltDeg(index) + (seededUnit(dateKey, `tilt:${index}`) - 0.5) * 2.8;
        placements.push({ x, y, width: w, height: h, layout, tiltDeg });
      });
      return placements;
    }
