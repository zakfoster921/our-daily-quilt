/**
 * IG carousel slide 2 alternate: mirror-flipped quilt bg + reflection cards
 * rasterized from quilt-screen DOM (same CSS/markup as the in-app wall).
 */
(function (root, factory) {
  const api = factory(root);
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = api;
  }
  if (root) {
    root.IgCarouselReflectionSlideCompose = api;
    root.composeCarouselReflectionSlideFromQuiltBlob = api.composeCarouselReflectionSlideFromQuiltBlob;
  }
})(
  typeof globalThis !== 'undefined' ? globalThis : typeof window !== 'undefined' ? window : {},
  function (global) {
    'use strict';

    const PANEL_W = 1080;
    const PANEL_H = 1350;
    const CARD_LAYER_MARGIN_X = 16;
    const CARD_LAYER_MARGIN_Y = 48;
    const CARD_LAYER_WIDTH_RATIO = 0.96;
    const CARD_LAYER_HEIGHT_RATIO = 0.9;
    const PAPER_TEXTURE_URL = 'assets/quilt-paper-card-texture.png';

    let paperTextureImagePromise = null;

    function loadImageFromUrl(url) {
      return new Promise((resolve) => {
        const img = new Image();
        img.crossOrigin = 'anonymous';
        img.onload = () => resolve(img);
        img.onerror = () => resolve(null);
        img.src = url;
      });
    }

    async function loadPaperTextureImage() {
      if (paperTextureImagePromise) return paperTextureImagePromise;
      paperTextureImagePromise = loadImageFromUrl(PAPER_TEXTURE_URL);
      return paperTextureImagePromise;
    }

    function resolveCardsLayerLayout(cardsLayer, panelW, panelH) {
      const { canvas, logicalWidth, logicalHeight } = cardsLayer;
      if (!canvas || !logicalWidth || !logicalHeight) return null;

      const maxW = Math.min(panelW - CARD_LAYER_MARGIN_X * 2, panelW * CARD_LAYER_WIDTH_RATIO);
      const maxH = Math.round(panelH * CARD_LAYER_HEIGHT_RATIO);
      const scale = Math.min(maxW / logicalWidth, maxH / logicalHeight);
      const drawW = logicalWidth * scale;
      const drawH = logicalHeight * scale;
      const drawX = Math.round((panelW - drawW) / 2);
      const drawY = Math.round(Math.max(CARD_LAYER_MARGIN_Y, (panelH - drawH) * 0.38));
      return { canvas, scale, drawW, drawH, drawX, drawY, logicalWidth, logicalHeight };
    }

    function drawCardsLayerOnPanel(ctx, cardsLayer, panelW, panelH, paperTextureImg, dateKey) {
      const layout = resolveCardsLayerLayout(cardsLayer, panelW, panelH);
      if (!layout) return null;

      const PaperDepth = global.IgCarouselPaperDepth || globalThis.IgCarouselPaperDepth;
      if (!PaperDepth?.extractAlphaBlobsFromCanvas || !PaperDepth?.drawCanvasRegionWithPaperDepth) {
        ctx.drawImage(layout.canvas, layout.drawX, layout.drawY, layout.drawW, layout.drawH);
        return {
          x: layout.drawX,
          y: layout.drawY,
          width: layout.drawW,
          height: layout.drawH,
          scale: layout.scale,
          sourceLogicalWidth: layout.logicalWidth,
          sourceLogicalHeight: layout.logicalHeight
        };
      }

      const cardPieces = Array.isArray(cardsLayer.cardPieceRects) && cardsLayer.cardPieceRects.length
        ? cardsLayer.cardPieceRects
        : null;
      const blobs = cardPieces || PaperDepth.extractAlphaBlobsFromCanvas(layout.canvas, 2200, 18);
      const rng = PaperDepth.createSeededRng(`${dateKey}:reflection-card-pieces`);

      if (!blobs.length) {
        ctx.drawImage(layout.canvas, layout.drawX, layout.drawY, layout.drawW, layout.drawH);
      } else {
        ctx.save();
        ctx.globalAlpha = 0.98;
        blobs.forEach((blob, index) => {
          const sx = blob.minX;
          const sy = blob.minY;
          const sw = blob.width;
          const sh = blob.height;
          const destX = layout.drawX + (sx / layout.canvas.width) * layout.drawW;
          const destY = layout.drawY + (sy / layout.canvas.height) * layout.drawH;
          const destW = (sw / layout.canvas.width) * layout.drawW;
          const destH = (sh / layout.canvas.height) * layout.drawH;
          const area = destW * destH;
          const depthScale = area > 18000 ? 1.15 : area > 8000 ? 1 : 0.82;
          PaperDepth.drawCanvasRegionWithPaperDepth(
            ctx,
            layout.canvas,
            sx,
            sy,
            sw,
            sh,
            destX,
            destY,
            destW,
            destH,
            {
              seed: `${dateKey}:reflection-piece:${index}`,
              rng: PaperDepth.createSeededRng(`${dateKey}:reflection-piece:${index}`),
              paperTextureImg,
              depthOptions: PaperDepth.scrapLikeDepthOptions(
                paperTextureImg,
                `${dateKey}:reflection-piece:${index}`,
                depthScale
              )
            }
          );
        });
        ctx.restore();
      }

      return {
        x: layout.drawX,
        y: layout.drawY,
        width: layout.drawW,
        height: layout.drawH,
        scale: layout.scale,
        sourceLogicalWidth: layout.logicalWidth,
        sourceLogicalHeight: layout.logicalHeight,
        pieceCount: blobs.length,
        paperDepth: true,
        pieceSource: cardPieces ? 'dom-rects' : 'alpha-blobs'
      };
    }

    function loadImageFromBlob(blob) {
      const url = URL.createObjectURL(blob);
      return new Promise((resolve, reject) => {
        const img = new Image();
        img.onload = () => {
          URL.revokeObjectURL(url);
          resolve(img);
        };
        img.onerror = () => {
          URL.revokeObjectURL(url);
          reject(new Error('Could not decode image'));
        };
        img.src = url;
      });
    }

    function canvasToBlob(canvas) {
      return new Promise((resolve) => {
        canvas.toBlob((blob) => resolve(blob || null), 'image/png', 0.95);
      });
    }

    function hashSeed(input) {
      const QNC = global.QuiltNewspaperClipping;
      if (typeof QNC?.hashDateKeySeed === 'function') {
        return QNC.hashDateKeySeed(String(input || 'seed'));
      }
      let h = 2166136261;
      const s = String(input || '');
      for (let i = 0; i < s.length; i += 1) {
        h ^= s.charCodeAt(i);
        h = Math.imul(h, 16777619);
      }
      return h >>> 0;
    }

    function seededUnit(seed, salt) {
      const h = hashSeed(`${seed}:${salt}`);
      return (h % 10000) / 10000;
    }

    function themeText(entry) {
      const app = global.app;
      if (typeof app?.reflectionWallThemeText === 'function') {
        return app.reflectionWallThemeText(entry);
      }
      if (entry && typeof entry === 'object') {
        if (entry.split && Array.isArray(entry.strips)) {
          return entry.strips.map((s) => s.text).filter(Boolean).join(' / ');
        }
        return String(entry.text || '').trim();
      }
      return String(entry || '').trim();
    }

    function themeAuthor(entry) {
      if (!entry || typeof entry !== 'object') return '';
      if (entry.split && Array.isArray(entry.strips)) {
        const authors = entry.strips.map((s) => String(s.author || '').trim()).filter(Boolean);
        return authors[0] || '';
      }
      return String(entry.author || '').trim();
    }

    function flattenWallThemes(themes) {
      const out = [];
      (Array.isArray(themes) ? themes : []).forEach((entry, index) => {
        if (!entry) return;
        if (entry.split && Array.isArray(entry.strips)) {
          entry.strips.forEach((strip, stripIndex) => {
            const text = String(strip.text || '').trim();
            if (!text) return;
            out.push({
              text,
              author: String(strip.author || '').trim(),
              heartCount: Math.max(0, Number(strip.heartCount) || 0),
              adminHighlight: strip?.adminHighlight === true,
              seedKey: `split:${index}:${stripIndex}`
            });
          });
          return;
        }
        const text = themeText(entry);
        if (!text) return;
        out.push({
          text,
          author: themeAuthor(entry),
          heartCount: Math.max(0, Number(entry.heartCount) || 0),
          adminHighlight: entry?.adminHighlight === true,
          seedKey: `solo:${index}`
        });
      });
      return out.filter((row) => row.text && !/add yours/i.test(row.text));
    }

    function flattenHighlightedThemes(themes) {
      const app = global.app;
      const isHighlighted = (entry) =>
        typeof app?.reflectionThemeIsAdminHighlighted === 'function'
          ? app.reflectionThemeIsAdminHighlighted(entry)
          : entry?.adminHighlight === true;
      const out = [];
      (Array.isArray(themes) ? themes : []).forEach((entry, index) => {
        if (!entry) return;
        if (entry.split && Array.isArray(entry.strips)) {
          entry.strips.forEach((strip, stripIndex) => {
            if (strip?.adminHighlight === true) {
              out.push({
                text: String(strip.text || '').trim(),
                author: String(strip.author || '').trim(),
                heartCount: Math.max(0, Number(strip.heartCount) || 0),
                adminHighlight: true,
                seedKey: `split:${index}:${stripIndex}`
              });
            }
          });
          return;
        }
        if (isHighlighted(entry)) {
          out.push({
            text: themeText(entry),
            author: themeAuthor(entry),
            heartCount: Math.max(0, Number(entry.heartCount) || 0),
            adminHighlight: true,
            seedKey: `solo:${index}`
          });
        }
      });
      return out.filter((row) => row.text);
    }

    const IG_REFLECTION_SLIDE_MAX_HIGHLIGHTS = 6;

    function pickFallbackReflectionThemes(themes, dateKey, limit = 3) {
      const pool = flattenWallThemes(themes);
      if (pool.length <= limit) return pool;
      const scored = pool.map((row, index) => {
        const len = row.text.length;
        let score = 0;
        if (row.author) score += 12;
        score += Math.min(row.heartCount, 24) * 2;
        if (len >= 8 && len <= 48) score += 14;
        else if (len <= 72) score += 7;
        else score -= Math.min(24, Math.round((len - 72) / 3));
        score += seededUnit(dateKey, `pick:${row.seedKey}`) * 4;
        return { row, score, index };
      });
      scored.sort((a, b) => b.score - a.score || a.index - b.index);
      return scored.slice(0, limit).map((item) => ({ ...item.row, adminHighlight: false }));
    }

    function resolveReflectionSlideThemes(themes, dateKey) {
      const highlighted = flattenHighlightedThemes(themes);
      if (highlighted.length) {
        return {
          entries: highlighted.slice(0, IG_REFLECTION_SLIDE_MAX_HIGHLIGHTS),
          source: 'highlight'
        };
      }
      const fallback = pickFallbackReflectionThemes(themes, dateKey, 3);
      return { entries: fallback, source: fallback.length ? 'fallback' : 'none' };
    }

    function entriesToWallThemes(entries) {
      return (Array.isArray(entries) ? entries : []).map((entry) => ({
        text: String(entry.text || '').trim(),
        author: String(entry.author || '').trim(),
        heartCount: Math.max(0, Number(entry.heartCount) || 0),
        adminHighlight: entry?.adminHighlight === true
      }));
    }

    /**
     * @param {object} options
     * @param {string} options.reflectionPrompt
     * @param {Array} options.highlightedThemes
     * @returns {Promise<{ blob: Blob|null, meta: object }|null>}
     */
    async function composeCarouselReflectionSlideFromQuiltBlob(quiltBlob, options = {}) {
      const promptText = String(options.reflectionPrompt || '').trim();
      const dateKey = String(options.dateKey || 'our-daily').trim() || 'our-daily';
      const { entries: responses, source: responseSource } = resolveReflectionSlideThemes(
        options.highlightedThemes,
        dateKey
      );
      if (!quiltBlob || !promptText || !responses.length) return null;

      const app = global.app;
      if (typeof app?.rasterizeIgReflectionSlideCardsLayer !== 'function') {
        throw new Error('app.rasterizeIgReflectionSlideCardsLayer missing');
      }

      const IgCompose = global.IgContributorCarouselCompose || globalThis.IgContributorCarouselCompose;
      const resolveRect = IgCompose?.resolveSharedQuiltRect;
      const MatBg = global.CarouselCuttingMatBg || globalThis.CarouselCuttingMatBg;
      if (typeof resolveRect !== 'function' || !MatBg?.drawCarouselCuttingMatBackground) {
        throw new Error('CarouselCuttingMatBg.drawCarouselCuttingMatBackground missing');
      }

      const cardsPngBase64 = String(
        options.cardsPngBase64 || globalThis.__igReflectionPlaywrightCardsPng || ''
      ).trim();
      if (!cardsPngBase64) {
        throw new Error(
          'Reflection slide cards PNG missing — capture via Playwright (scripts/ig-reflection-playwright-capture.cjs)'
        );
      }

      const [quiltImg, cardsLayer, cuttingMatImg, paperTextureImg] = await Promise.all([
        loadImageFromBlob(quiltBlob),
        app.rasterizeIgReflectionSlideCardsLayer({
          cardsPngBase64,
          logicalWidth: options.cardsLayerLogicalWidth,
          logicalHeight: options.cardsLayerLogicalHeight,
          deviceScaleFactor: options.cardsLayerDeviceScaleFactor,
          cardPieceRects: options.cardPieceRects,
          stackLogicalWidth: options.cardsLayerLogicalWidth,
          stackLogicalHeight: options.cardsLayerLogicalHeight
        }),
        MatBg.loadCarouselCuttingMatImage?.() || Promise.resolve(null),
        loadPaperTextureImage()
      ]);
      const quiltRect = resolveRect(quiltImg, options);
      if (!quiltRect || !cardsLayer?.canvas) return null;

      const cardsLayout = resolveCardsLayerLayout(cardsLayer, PANEL_W, PANEL_H);
      const StatsCompose =
        global.IgCarouselYesterdayStatsSlideCompose ||
        globalThis.IgCarouselYesterdayStatsSlideCompose;

      const canvas = document.createElement('canvas');
      canvas.width = PANEL_W;
      canvas.height = PANEL_H;
      const ctx = canvas.getContext('2d');
      if (!ctx) return null;

      MatBg.drawCarouselCuttingMatBackground(
        ctx,
        cuttingMatImg,
        PANEL_W,
        PANEL_H,
        MatBg.CAROUSEL_MAT_PAN?.SLIDE_3 ?? PANEL_W * 2
      );

      let collageScraps = [];
      if (typeof StatsCompose?.drawCarouselCutoffScraps === 'function' && cardsLayout) {
        const protectedBounds = StatsCompose.protectedBoundsFromLayout(
          cardsLayout.drawX,
          cardsLayout.drawY,
          cardsLayout.drawW,
          cardsLayout.drawH,
          24
        );
        collageScraps = StatsCompose.drawCarouselCutoffScraps(
          ctx,
          quiltImg,
          quiltRect,
          PANEL_W,
          PANEL_H,
          dateKey,
          paperTextureImg,
          protectedBounds,
          { seedTag: 'reflection-scraps' }
        );
      }

      const cardsMeta = drawCardsLayerOnPanel(
        ctx,
        cardsLayer,
        PANEL_W,
        PANEL_H,
        paperTextureImg,
        dateKey
      );
      const blob = await canvasToBlob(canvas);
      if (!blob) return null;

      return {
        blob,
        meta: {
          panelWidth: PANEL_W,
          panelHeight: PANEL_H,
          carouselQuiltBgMode: 'cutting-mat-reflection-wall',
          cuttingMatAsset: cuttingMatImg ? 'loaded' : 'procedural-fallback',
          paperTextureAsset: paperTextureImg ? 'loaded' : 'missing',
          reflectionPrompt: promptText,
          responseSource,
          responseCount: responses.length,
          highlightCount: responses.filter((row) => row.adminHighlight === true).length,
          cardsLayer: cardsMeta,
          responseClusterLayout: 'playwright-dom-screenshot',
          collageScraps: collageScraps.map((s) => s.name),
          collageScrapRecords:
            typeof StatsCompose?.scrapRecordFromScrap === 'function'
              ? collageScraps.map((s) => StatsCompose.scrapRecordFromScrap(s))
              : [],
          collageScrapLayoutSeed: `${dateKey}:reflection-scraps`
        }
      };
    }

    return {
      composeCarouselReflectionSlideFromQuiltBlob,
      PANEL_W,
      PANEL_H,
      IG_REFLECTION_SLIDE_MAX_HIGHLIGHTS,
      flattenHighlightedThemes,
      flattenWallThemes,
      pickFallbackReflectionThemes,
      entriesToWallThemes,
      resolveReflectionSlideThemes
    };
  }
);
