...
(function (root, factory) {
  ...
  function (global) {
    ...
    const PATCH_PAD_X = Math.round(24 * SLIDE_TEXT_SCALE);
    const PATCH_PAD_Y = Math.round(28 * SLIDE_TEXT_SCALE);
    ...
    function wrapTextLinesFull(ctx, text, maxWidth) {
    ...
    function longestLineWidth(ctx, lines) {
      return lines.reduce((max, line) => Math.max(max, ctx.measureText(line).width), 0);
    }

    function estimatePatchLayout(ctx, entry, fontPx, maxWidthCap) {
      const body = String(entry.text || '').trim();
      const author = String(entry.author || '').trim();
      const lineStep = fontPx * PATCH_LINE_HEIGHT;
      const authorGap = author ? Math.round(PATCH_AUTHOR_GAP * SLIDE_TEXT_SCALE) : 0;
      const minW = RESPONSE_CARD_W;
      const minH = RESPONSE_CARD_H;
      const maxW = Math.min(
        maxWidthCap || PANEL_W,
        PANEL_W - PROMPT_CARD_X - PANEL_MARGIN,
        Math.round(PANEL_W * 0.78)
      );
      ctx.font = `400 ${fontPx}px "DM Sans", system-ui, -apple-system, "Segoe UI", sans-serif`;

      let lines = wrapTextLinesFull(ctx, body, maxW - PATCH_PAD_X * 2);
      let w = Math.ceil(Math.max(minW, longestLineWidth(ctx, lines) + PATCH_PAD_X * 2 + 16));
      w = Math.min(w, maxW);
      lines = wrapTextLinesFull(ctx, body, w - PATCH_PAD_X * 2);
      w = Math.ceil(Math.max(minW, longestLineWidth(ctx, lines) + PATCH_PAD_X * 2 + 16));

      const bodyH = lines.length * lineStep;
      const authorH = author ? lineStep : 0;
      const h = Math.ceil(Math.max(minH, bodyH + authorGap + authorH + PATCH_PAD_Y * 2));
      return { width: w, height: h, lines, author, lineStep, fontPx };
    }

    function measurePromptCardLayout(ctx, text) {
      const prompt = String(text || '').trim();
      const fontPx = questionFontPx(prompt);
      const lineStep = fontPx * PATCH_LINE_HEIGHT;
      ctx.font = `400 ${fontPx}px "DM Sans", system-ui, -apple-system, "Segoe UI", sans-serif`;
      let cardW = PROMPT_CARD_W;
      let cardH = PROMPT_CARD_H;
      const maxW = Math.min(PANEL_W - PROMPT_CARD_X * 2, Math.round(PANEL_W * 0.82));

      const measureBlock = (w, h) => {
        const insetX = w * PROMPT_INSET_SIDE;
        const insetTop = h * PROMPT_INSET_TOP;
        const insetBottom = h * PROMPT_INSET_BOTTOM;
        const boxW = w - insetX * 2;
        const boxH = h - insetTop - insetBottom;
        const lines = wrapTextLinesFull(ctx, prompt, boxW - 8);
        const blockH = lines.length * lineStep;
        return { insetX, insetTop, insetBottom, boxW, boxH, lines, blockH };
      };

      let block = measureBlock(cardW, cardH);
      cardW = Math.ceil(
        Math.max(PROMPT_CARD_W, longestLineWidth(ctx, block.lines) + block.insetX * 2 + 20)
      );
      cardW = Math.min(cardW, maxW);
      block = measureBlock(cardW, cardH);
      while (block.blockH > block.boxH && cardH < PANEL_H - PROMPT_CARD_Y - 80) {
        cardH += Math.ceil(lineStep);
        block = measureBlock(cardW, cardH);
      }
      return {
        width: cardW,
        ...
        lineStep,
        lines: block.lines,
        insetTop: block.insetTop,
        insetBottom: block.insetBottom,
        boxH: block.boxH
      };
    }
    ...
    function drawPromptCard(ctx, promptText, cardImg, promptLayout) {
      const text = String(promptText || '').trim();
      if (!text || !promptLayout) return null;
      const cardW = promptLayout.width;
      const cardH = promptLayout.height;
      ctx.save();
      ctx.translate(PROMPT_CARD_X + cardW / 2, PROMPT_CARD_Y + cardH / 2);
      ...
      ctx.shadowBlur = 0;
      ctx.shadowOffsetY = 0;
      const insetX = cardW * PROMPT_INSET_SIDE;
      const insetTop = promptLayout.insetTop;
      const boxH = promptLayout.boxH;
      ctx.fillStyle = 'rgba(42, 34, 28, 0.86)';
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      const lineStep = promptLayout.lineStep;
      const blockH = promptLayout.lines.length * lineStep;
      let cy = -cardH / 2 + insetTop + boxH / 2 - blockH / 2 + lineStep / 2;
      ctx.font = `400 ${promptLayout.fontPx}px "DM Sans", system-ui, -apple-system, "Segoe UI", sans-serif`;
      promptLayout.lines.forEach((line) => {
     ...
     */
    function responseCardAnchors(layouts, promptCard) {
      const count = layouts.length;
      const topRightX = PROMPT_CARD_X + promptCard.width + QUESTION_RESPONSE_GAP;
      const topRightY = PROMPT_CARD_Y;
      const bottomLeftX = PROMPT_CARD_X;
      const bottomRowY = PROMPT_CARD_Y + promptCard.height + QUESTION_RESPONSE_GAP;
      const anchors = [];
      if (count >= 1) anchors.push({ x: topRightX, y: topRightY });
      if (count >= 2) anchors.push({ x: bottomLeftX, y: bottomRowY });
      if (count >= 3) {
        let x = bottomLeftX + layouts[1].width - PATCH_JOIN_OVERLAP;
        anchors.push({ x, y: bottomRowY });
        for (let i = 3; i < count; i += 1) {
          x += layouts[i - 1].width - PATCH_JOIN_OVERLAP;
          anchors.push({ x, y: bottomRowY });
        }
      }
      return anchors;
    ...
    function layoutWallPatches(entries, measureCtx, fontPx, promptCard) {
      const topRightX = PROMPT_CARD_X + promptCard.width + QUESTION_RESPONSE_GAP;
      const topRightMaxW = PANEL_W - PANEL_MARGIN - topRightX;
      const layouts = entries.map((entry, index) =>
        estimatePatchLayout(measureCtx, entry, fontPx, index === 0 ? topRightMaxW : undefined)
      );
      const anchors = responseCardAnchors(layouts, promptCard);
    ...
    async function composeCarouselReflectionSlideFromQuiltBlob(quiltBlob, options = {}) {
      ...
      const promptLayout = measurePromptCardLayout(measureCtx, promptText);
      measureCtx.font = `400 ${responseFontPx}px "DM Sans", system-ui, -apple-system, "Segoe UI", sans-serif`;

      const placements = layoutWallPatches(responses, measureCtx, responseFontPx, promptLayout);
      ...
      const promptMeta = drawPromptCard(ctx, promptText, promptCardImg, promptLayout);
