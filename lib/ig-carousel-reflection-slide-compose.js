...
(function (root, factory) {
  ...
  function (global) {
    ...
    function buildJoinTapes(placements, dateKey) {
      ...
      const byRow = placements.filter((_, index) => index > 0).sort((a, b) => a.y - b.y);
      const bottomRow =
        byRow.length >= 2 && sameRow(byRow[0], byRow[byRow.length - 1])
          ? byRow.slice(-2).sort((a, b) => a.x - b.x)
          : null;
      const rightCol = placements.length >= 3 ? [placements[0], placements[2]] : null;

      if (bottomRow?.length === 2) {
        const [left, right] = bottomRow;
        ...
        tapes.push({
          ...
          overlapTop: top,
          overlapSpan: span,
          seedKey: `${dateKey}:reflection-join:bottom`
        });
      ...
      if (rightCol && sameCol(rightCol[0], rightCol[1])) {
        const [upper, lower] = rightCol.sort((a, b) => a.y - b.y);
        tapes.push({
          kind: 'horizontal',
          left: upper.x,
          width: upper.width,
    ...
    /**
     * Nestled backwards-L (user layout):
     *   [question][R0]
     *   [R1     ][R2]
     * Vertical tape between R1↔R2; horizontal tape between R0↔R2.
     */
    function responseCardAnchors(count) {
      const tuck = PATCH_JOIN_OVERLAP;
      const gap = 6;
      const topRightX = PROMPT_CARD_X + PROMPT_CARD_W - tuck + gap;
      const topRightY = PROMPT_CARD_Y;
      const bottomLeftX = PROMPT_CARD_X;
      const bottomLeftY = PROMPT_CARD_Y + PROMPT_CARD_H - tuck + gap;
    ...
    async function composeCarouselReflectionSlideFromQuiltBlob(quiltBlob, options = {}) {
      ...
      if (typeof QNC?.ensureNewspaperClippingFonts === 'function') {
        await QNC.ensureNewspaperClippingFonts([responseFontPx, 26, 30, 34, 38], '"DM Sans", system-ui, sans-serif');
      }
      ...
      const placements = layoutWallPatches(highlighted, measureCtx, dateKey, responseFontPx);
      const joinTapes = buildJoinTapes(placements, dateKey);
      const patchDrawOrder = placements.length >= 3 ? [1, 2, 0] : placements.map((_, index) => index);
      let previousRgb = null;
      patchDrawOrder.forEach((index) => {
        const placement = placements[index];
        if (!placement) return;
        const entry = highlighted[index];
