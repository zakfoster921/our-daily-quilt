...
(function (root, factory) {
  ...
  function (global) {
    ...
    const PATCH_PAD_X = 26;
    const PATCH_PAD_Y = 24;
    const PATCH_MAX_W = 520;
    ...
    function estimatePatchSize(ctx, entry, maxWidthPx) {
      ...
      const maxW = Math.max(120, Math.min(PATCH_MAX_W, Math.round(maxWidthPx)));
      const innerMax = Math.max(80, maxW - PATCH_PAD_X * 2);
      const lineStep = PATCH_FONT_PX * PATCH_LINE_HEIGHT;
      const authorFontPx = Math.round(PATCH_FONT_PX * 0.82);
      ctx.font = `400 ${PATCH_FONT_PX}px "DM Sans", system-ui, -apple-system, "Segoe UI", sans-serif`;
      let lines;
      if (ctx.measureText(body).width <= innerMax) {
        lines = [body];
      } else {
        lines = wrapTextLines(ctx, body, innerMax, 6);
      }
      ...
      const w = Math.min(maxW, Math.ceil(Math.max(longest, authorW) + PATCH_PAD_X * 2));
      let h = PATCH_PAD_Y * 2 + lines.length * lineStep;
      if (author) {
        h += authorFontPx * PATCH_LINE_HEIGHT + 6;
      }
      return { width: w, height: Math.ceil(h), lines, author, lineStep, authorFontPx };
    }
    ...
    function layoutWallPatches(entries, measureCtx, dateKey) {
      ...
      entries.forEach((entry, index) => {
        ...
        const maxW = Math.min(PATCH_MAX_W, slot.w - 16);
        const layout = estimatePatchSize(measureCtx, entry, maxW);
        const w = layout.width;
        const h = layout.height;
        const anchor = anchors[index % anchors.length];
        const jitterX = (seededUnit(dateKey, `${entry.seedKey}:jx:${index}`) - 0.5) * 36;
        const jitterY = (seededUnit(dateKey, `${entry.seedKey}:jy:${index}`) - 0.5) * 28;
        const x = Math.round(
          slot.x + (slot.w - w) * anchor.ax + jitterX
        );
        const y = Math.round(
          slot.y + (slot.h - h) * anchor.ay + jitterY
        );
        const tiltDeg =
