...
(function (root, factory) {
  ...
  function (global) {
    ...
    function responseCardAnchors(count) {
      const gap = 16;
      const rightX = PANEL_W - PANEL_MARGIN - RESPONSE_CARD_W;
      if (count === 1) {
      ...
      if (count === 2) {
        return [
          ...
          { x: PROMPT_CARD_X, y: PROMPT_CARD_Y + RESPONSE_CARD_H + gap }
        ];
      }
      return [
        { x: rightX, y: PROMPT_CARD_Y },
        { x: rightX - 52, y: Math.round(PROMPT_CARD_Y + RESPONSE_CARD_H * 0.46) },
        { x: PROMPT_CARD_X + 36, y: PROMPT_CARD_Y + RESPONSE_CARD_H + gap }
      ];
    }
    ...
    function layoutWallPatches(entries, measureCtx, dateKey) {
      ...
      entries.forEach((entry, index) => {
        ...
        const jitterX = (seededUnit(dateKey, `${entry.seedKey}:jx:${index}`) - 0.5) * 22;
        const jitterY = (seededUnit(dateKey, `${entry.seedKey}:jy:${index}`) - 0.5) * 18;
        const x = Math.round(anchor.x + jitterX);
        const y = Math.round(anchor.y + jitterY);
        const tiltDeg =
          patchTiltDeg(index) + (seededUnit(dateKey, `tilt:${index}`) - 0.5) * 2.4;
