...
class QuiltRendererV2 {
  ...
  /** Extra vertical stretch from dup×2 tile transforms: scale(1, fillY). */
  _splitBandVerticalStretchFromTransform(transformStr) {
    const s = String(transformStr || '');
    const matches = [...s.matchAll(/scale\s*\(\s*1\s+([+-]?\d*\.?\d+(?:e[+-]?\d+)?)\s*\)/gi)];
    let stretch = 1;
    for (const match of matches) {
      const sy = Math.abs(Number(match[1]));
      if (Number.isFinite(sy) && sy > stretch) stretch = sy;
    }
    return stretch;
  }

  _splitBandMirrorCircleAspectRatio(layout, doubleSideBySide, layerTransform) {
    if (!layout) return 1;
    let scaleY = Math.abs(Number(layout.mirrorBandScaleY) || 1);
    if (doubleSideBySide) {
      const parsedStretch = this._splitBandVerticalStretchFromTransform(layerTransform);
      const fillY = Number(layout.mirrorDupFillY) || 1;
      const tileStretch = parsedStretch > 1 + 1e-6 ? parsedStretch : fillY > 1 + 1e-6 ? fillY : 1;
      scaleY *= tileStretch;
    }
    return this._splitBandCircleAspectRatio(layout.mirrorBandScaleX, scaleY);
  }

  _applySplitBandCircleFixToLayer(layer, ratio) {
    if (!layer || typeof layer.querySelectorAll !== 'function') return;
    const r = Number(ratio);
    if (!Number.isFinite(r)) return;
    layer.querySelectorAll('[data-split-band-circle-fix="1"]').forEach((fixGroup) => {
      ...
      const transform = this._splitBandCircleFixTransform(cx, cy, r);
      if (transform) {
  ...
  renderBlocks(blocks, userPieces = [], submissionCount = 0) {
    ...
    const horizontalFieldStretchRatio =
      !splitBandLayout && effectiveStretch !== 1 ? Number(effectiveStretch) || 1 : 1;
    const splitBandPrimaryCircleRatio = splitBandLayout
      ...
      : horizontalFieldStretchRatio;
    const splitBandMirrorCircleRatio = splitBandLayout
      ? this._splitBandMirrorCircleAspectRatio(splitBandLayout, splitBandDoubleSideBySide)
      : horizontalFieldStretchRatio;
    ...
    renderQueue.forEach(({ block, originalIndex }, i) => {
      ...
      if (
        ...
        if (block.specialPatternType === 'insetCircle') {
          ...
          if (splitBandLayout || horizontalFieldStretchRatio !== 1) {
            ...
            if (splitBandLayout) {
              fixGroup.setAttribute('data-split-band-circle-ratio-mirror', String(splitBandMirrorCircleRatio));
            }
        ...
        const isAxisPolygonSpecialCell =
          ...
          ['checkerboard', 'stripes', 'railfence', 'framed', 'cross'].includes(block.specialPatternType);
      ...
      let regularVisibilityScale = isNewestRegularSubmissionBlock
    ...
    if (renderMirroredField) {
      ...
      const prepareDuplicateLayer = (layer, transform, options = {}) => {
        ...
        layer.setAttribute('pointer-events', 'none');
        const layerTransform = withMirrorFieldNudge(transform);
        layer.setAttribute('transform', layerTransform);
        if (options.dataAttr) {
        ...
        if (mirrorBand) {
          ...
          const mirrorCircleRatio = splitBandLayout
            ? this._splitBandMirrorCircleAspectRatio(
                splitBandLayout,
                splitBandDoubleSideBySide,
                layerTransform
              )
            : horizontalFieldStretchRatio;
          this._applySplitBandCircleFixToLayer(layer, mirrorCircleRatio);
        } else {
          fieldLayer.insertBefore(layer, parallaxLayer);
          if (horizontalFieldStretchRatio !== 1) {
            this._applySplitBandCircleFixToLayer(layer, horizontalFieldStretchRatio);
          }
        }
      };
    ...
    if (splitBandLayout && mirrorBand) {
      const mirrorLayerSelectors = [
        '#quiltMirroredFieldLayer',
        '#quiltDuplicateBottomLayer1',
        '#quiltDuplicateBottomLayer2'
      ];
      mirrorLayerSelectors.forEach((selector) => {
        const layer = mirrorParent.querySelector(selector);
        if (!layer) return;
        const layerTransform = layer.getAttribute('transform') || '';
