  ...
  renderBlocks(blocks, userPieces = [], submissionCount = 0) {
    ...
    const mirrorLayout =
      renderMirroredField && typeof QuiltMirrorLayout !== 'undefined'
        ? QuiltMirrorLayout.computeFromBounds(
            { minX, minY, width: actualQuiltWidth, height: actualQuiltHeight },
            {
              dateKey,
              viewportW,
              viewportH
    ...
    renderQueue.forEach(({ block, originalIndex }, i) => {
      ...
      if (
        ...
        if (block.specialPatternType === 'insetCircle') {
          ...
          this._applySeamClosingStroke(handCutPolygon, block.color);
        ...
        specialSeamUnderpaint.setAttribute('fill', block.color);
