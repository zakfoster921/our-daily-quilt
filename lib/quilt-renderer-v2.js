  ...
  renderBlocks(blocks, userPieces = [], submissionCount = 0) {
    ...
    if (renderMirroredField) {
      ...
      if (quadrantFourUpPreview) {
        ...
        const leftFlags = savedDoubleBottomTileFlags.left;
        const rightFlags = savedDoubleBottomTileFlags.right;
        let quadTransforms =
          typeof QuiltMirrorLayout !== 'undefined' &&
        ...
        if (tileSeamNudgeX) {
          const nudgeRight = (transform) =>
            typeof QuiltMirrorLayout !== 'undefined' &&
            typeof QuiltMirrorLayout.odqAppendTileSeamNudgeTransform === 'function'
              ? QuiltMirrorLayout.odqAppendTileSeamNudgeTransform(transform, tileSeamNudgeX, viewBoxWidth)
              : `translate(${tileSeamNudgeX * viewBoxWidth} 0) ${transform}`;
          quadTransforms[1] = nudgeRight(quadTransforms[1]);
          quadTransforms[3] = nudgeRight(quadTransforms[3]);
        }
        if (leftTileNudgeX) {
          const nudgeLeft = (transform) =>
            typeof QuiltMirrorLayout !== 'undefined' &&
            typeof QuiltMirrorLayout.odqAppendTileSeamNudgeTransform === 'function'
              ? QuiltMirrorLayout.odqAppendTileSeamNudgeTransform(transform, leftTileNudgeX, viewBoxWidth)
              : `translate(${leftTileNudgeX * viewBoxWidth} 0) ${transform}`;
          quadTransforms[0] = nudgeLeft(quadTransforms[0]);
          quadTransforms[2] = nudgeLeft(quadTransforms[2]);
        }
        if (doubleBottomTileNudgeY.left || doubleBottomTileNudgeY.right) {
          const appendTileVerticalNudge = (transform, nudgeTileY) =>
            ...
            typeof QuiltMirrorLayout.odqAppendTileVerticalNudgeTransform === 'function'
              ...
              : `translate(0 ${nudgeTileY * viewBoxHeight}) ${transform}`;
          if (doubleBottomTileNudgeY.left) {
            quadTransforms[0] = appendTileVerticalNudge(
              quadTransforms[0],
              doubleBottomTileNudgeY.left
            );
            quadTransforms[2] = appendTileVerticalNudge(
              quadTransforms[2],
              doubleBottomTileNudgeY.left
            );
          }
          if (doubleBottomTileNudgeY.right) {
            quadTransforms[1] = appendTileVerticalNudge(
              quadTransforms[1],
              doubleBottomTileNudgeY.right
            );
            quadTransforms[3] = appendTileVerticalNudge(
              quadTransforms[3],
              doubleBottomTileNudgeY.right
            );
          }
        ...
        parallaxLayer.setAttribute('transform', withMirrorFieldNudge(quadTransforms[0]));
        parallaxLayer.setAttribute(quadMeta[0].dataAttr, '1');
        parallaxLayer.setAttribute(
        ...
        cloneLayers.forEach((layer, cloneIndex) => {
          ...
          layer.setAttribute('aria-hidden', 'true');
          layer.setAttribute('pointer-events', 'none');
          layer.setAttribute('transform', withMirrorFieldNudge(quadTransforms[index]));
          layer.setAttribute(meta.dataAttr, '1');
          layer.querySelectorAll('[data-new-block="1"]').forEach((node) => {
            node.classList.remove('new-block');
            node.removeAttribute('opacity');
          ...
          if (meta.role === 'tr' || meta.role === 'br') {
            layer.setAttribute(
              'data-mirror-bottom-right-flip-x',
              rightFlags.flipX ? '1' : '0'
            );
            layer.setAttribute(
              'data-mirror-bottom-right-flip-y',
              rightFlags.flipY ? '1' : '0'
            );
          } else {
            layer.setAttribute(
              'data-mirror-bottom-left-flip-x',
