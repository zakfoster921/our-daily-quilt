...
class QuiltRendererV2 {
  ...
  _buildLiveGapUnderpaintGroup(blocks, id, bleed = 2.5) {
    ...
    blocks.forEach((block) => {
      ...
      const fill =
        typeof Utils !== 'undefined' && typeof Utils.validateHexColor === 'function' && Utils.validateHexColor(block.color)
          ? block.color
          : block.color || '#d8d4cf';
      const rect = document.createElementNS('http://www.w3.org/2000/svg', 'rect');
  ...
  renderBlocks(blocks, userPieces = [], submissionCount = 0) {
    ...
    renderQueue.forEach(({ block, originalIndex }, i) => {
      ...
      const gridSeamBleed = this._gridSeamUnderpaintBleed(effectiveStretch);
      const seamUnderpaintRect =
        gridSeamBleed > 0
          ? this._createGridSeamUnderpaintRect(block.x, block.y, block.width, block.height, gridSeamBleed)
          : null;
      if (seamUnderpaintRect) {
        seamUnderpaintRect.setAttribute('fill', jitteredBlock.color);
