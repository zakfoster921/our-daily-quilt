...
class QuiltRendererV2 {
  ...
  /** Local quilt builder: exact cell fit, smaller blocks on top — no hand-cut overlap hiding neighbors. */
  ...
  renderBlocks(blocks, userPieces = [], submissionCount = 0) {
    ...
    const renderQueue = blocksForRender
      ...
      .sort((a, b) => {
        if (this._builderPreviewMode) {
          const areaA =
            Math.max(0, Number(a.block?.width) || 0) * Math.max(0, Number(a.block?.height) || 0);
          const areaB =
            Math.max(0, Number(b.block?.width) || 0) * Math.max(0, Number(b.block?.height) || 0);
    ...
    renderQueue.forEach(({ block, originalIndex }, i) => {
      ...
      const rotationRadians = jitterRotation * Math.PI / 180;
      const maxExtension =
        Math.max(block.width, block.height) *
