...
class QuiltRendererV2 {
  ...
  _boundsIntersect(a, b, padding = 0) {
    const A = this._normalizeBounds(a);
    const B = this._normalizeBounds(b);
    if (!A || !B) return false;
  ...
  _lightOverDarkOpacity(topBlock, underlyingBlocks) {
    const minTopLuminance = SimpleQuiltEngine.LIGHT_OVER_DARK_MIN_TOP_LUMINANCE;
    const minDelta = SimpleQuiltEngine.LIGHT_OVER_DARK_MIN_LUMINANCE_DELTA;
    const baseOpacity = SimpleQuiltEngine.LIGHT_OVER_DARK_FILL_OPACITY;
    const adjacencyPadding = SimpleQuiltEngine.LIGHT_OVER_DARK_ADJACENCY_PADDING;
    if (!topBlock || typeof topBlock.color !== 'string') return 1;

    const topLuminance = this._blockLuminance(topBlock.color);
    if (topLuminance < minTopLuminance) return 1;
    if (!this._normalizeBounds(topBlock)) return 1;

    let maxDelta = 0;
    (Array.isArray(underlyingBlocks) ? underlyingBlocks : []).forEach((underBlock) => {
      if (!underBlock || typeof underBlock.color !== 'string') return;
      if (!this._boundsIntersect(topBlock, underBlock, adjacencyPadding)) return;
      const delta = topLuminance - this._blockLuminance(underBlock.color);
      if (delta >= minDelta) maxDelta = Math.max(maxDelta, delta);
    });

    if (maxDelta < minDelta) return 1;

    const scaled = baseOpacity - Math.min(0.08, (maxDelta - minDelta) / 320);
    return Math.max(0.74, Math.min(0.88, scaled));
  }

  _buildLightOverDarkOpacityMap(renderQueue) {
    const map = new Map();
    const entries = Array.isArray(renderQueue) ? renderQueue : [];
    entries.forEach(({ block, originalIndex }, index) => {
      if (!block) return;
      const key = block.id != null ? String(block.id) : `__idx_${originalIndex}`;
      const underlying = entries.slice(0, index).map((entry) => entry.block);
      const opacity = this._lightOverDarkOpacity(block, underlying);
      if (opacity < 1) map.set(key, opacity);
    });
    return map;
  }

  _lightOverDarkOpacityForBlock(block, originalIndex, opacityMap) {
    if (!opacityMap || !block) return 1;
    const key = block.id != null ? String(block.id) : `__idx_${originalIndex}`;
    return opacityMap.get(key) ?? 1;
  }

  _multiplySolidFillOpacity(node, opacity) {
    if (!node || !(opacity < 1)) return;
    const fill = String(node.getAttribute('fill') || '');
    if (!fill || fill.startsWith('url(')) return;
    const base = parseFloat(node.getAttribute('data-base-fill-opacity') || node.getAttribute('fill-opacity') || '1');
    if (!node.hasAttribute('data-base-fill-opacity')) {
      node.setAttribute('data-base-fill-opacity', String(Number.isFinite(base) ? base : 1));
    }
    const baseOp = parseFloat(node.getAttribute('data-base-fill-opacity') || '1');
    node.setAttribute('fill-opacity', (baseOp * opacity).toFixed(3));
  }

  /** Per-shape fill-opacity — group opacity rasterizes as 2-triangle quads and shows lighter diagonal wedges on WKWebView. */
  _applyLightOverDarkGroupOpacity(group, opacity) {
    if (!group || !(opacity < 1)) return;
    const op = opacity.toFixed(3);
    group.dataset.overlapOpacity = op;
    Array.from(group.children).forEach((child) => {
      const tag = child.localName;
      if (tag === 'polygon' || tag === 'rect') {
        this._multiplySolidFillOpacity(child, opacity);
      } else if (tag === 'g') {
  ...
  renderBlocks(blocks, userPieces = [], submissionCount = 0) {
    ...
    const lightOverDarkOpacityById = this._buildLightOverDarkOpacityMap(renderQueue);
    ...
    renderQueue.forEach(({ block, originalIndex }, i) => {
      ...
      const overlapOpacity = this._lightOverDarkOpacityForBlock(block, originalIndex, lightOverDarkOpacityById);
      ...
      if (
        ...
        if (block.specialPatternType === 'diagonalAxis' && Array.isArray(block.polygonPieces)) {
          ...
          this._applyLightOverDarkGroupOpacity(specialGroup, overlapOpacity);
        ...
        if (block.specialPatternType === 'hst') {
          ...
          const hstEdgeBleed = effectiveStretch > 1 ? 1.35 : 0.75;
     ...
     // Add the pattern group to the SVG
      ...
      this._applyLightOverDarkGroupOpacity(blockGroup, overlapOpacity);
  ...
  _applyNewBlockAnimation(group) {
    ...
    group.addEventListener('animationend', () => {
      ...
      const overlapOpacity = parseFloat(group.dataset.overlapOpacity || '');
      if (Number.isFinite(overlapOpacity) && overlapOpacity < 1) {
        this._applyLightOverDarkGroupOpacity(group, overlapOpacity);
      }
