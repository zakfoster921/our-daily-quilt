    ...
    renderQueue.forEach(({ block, originalIndex }, i) => {
      ...
      if (
        ...
        if (block.specialPatternType === 'diagonalAxis' && Array.isArray(block.polygonPieces)) {
          ...
          if (this._builderPreviewMode) {
            const underpaint = this._createGridSeamUnderpaintRect(renderX, renderY, w, h, 1.75);
            if (underpaint) {
              underpaint.setAttribute('fill', this._underpaintFillForBlock(block));
          ...
          block.polygonPieces.forEach((piece, pi) => {
            ...
            if (this._builderPreviewMode) {
              poly.setAttribute('stroke-width', '1.85');
            }
        ...
        const isGaplessTilingPattern =
          ...
          ['checkerboard', 'stripes', 'railfence', 'nestedGrid', 'bandedColumns'].includes(block.specialPatternType);
        const isAxisPolygonSpecialCell =
          Array.isArray(block.polygonPieces) &&
          block.polygonPieces.length &&
          ['checkerboard', 'stripes', 'railfence', 'framed', 'cross', 'diamond', 'nestedGrid', 'bandedColumns'].includes(block.specialPatternType);
