...
class QuiltRendererV2 {
  ...
  renderBlocks(blocks, userPieces = [], submissionCount = 0) {
    ...
    const viewBoxWidth = primaryOnlyPreview
      ? actualQuiltWidth * primaryPreviewStretch + primaryPreviewPad * 2
      : viewBox?.width ?? actualQuiltWidth * horizontalStretch + padding * 2;
    const viewBoxHeight = primaryOnlyPreview
      ? actualQuiltHeight + primaryPreviewPad * 2
      : viewBox?.height ?? mirroredFieldHeight + padding * 2;
    const viewBoxX = primaryOnlyPreview ? minX - primaryPreviewPad : viewBox?.x ?? minX - padding;
    const viewBoxY = primaryOnlyPreview ? minY - primaryPreviewPad : viewBox?.y ?? minY - padding;
    ...
    this.quiltSVG.setAttribute(
      'preserveAspectRatio',
      primaryOnlyPreview ? 'xMidYMid meet' : 'xMidYMin meet'
    );
    ...
    if (liveGapPrimary) {
      fieldLayer.appendChild(liveGapPrimary);
    }
    ...
    if (renderMirroredField) {
      ...
      const mirroredLayerEl = fieldLayer.querySelector('#quiltMirroredFieldLayer');
      if (mirroredLayerEl) {
        ...
        if (liveGapMirrored) {
          ...
          fieldLayer.insertBefore(liveGapMirrored, mirroredLayerEl);
        }
      }

      ['#quiltDuplicateBottomLayer1', '#quiltDuplicateBottomLayer2'].forEach((selector, index) => {
        const duplicateLayerEl = fieldLayer.querySelector(selector);
        ...
        fieldLayer.insertBefore(liveGapDuplicate, duplicateLayerEl);
      });
    }
