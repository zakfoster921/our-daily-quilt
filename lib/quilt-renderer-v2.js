...
class QuiltRendererV2 {
  ...
  /** Local quilt builder: exact cell fit, natural paint order — flat mosaic preview. */
  setBuilderPreviewMode(enabled) {
    this._builderPreviewMode = enabled === true;
  }

  _builderExactRect(x, y, width, height) {
  ...
  renderBlocks(blocks, userPieces = [], submissionCount = 0) {
    ...
    diagonalAxisRenderByKey.forEach((group, key) => {
      ...
      group.jitterX = this._builderPreviewMode ? 0 : (this.seededUnit(`${axisSeed}:x`) - 0.5) * 6 * jitterMultiplier;
      group.jitterY = this._builderPreviewMode ? 0 : (this.seededUnit(`${axisSeed}:y`) - 0.5) * 6 * jitterMultiplier;
      group.jitterRotation = this._builderPreviewMode
        ? 0
    ...
    renderQueue.forEach(({ block, originalIndex }, i) => {
      ...
      if (
        ...
        if (block.specialPatternType === 'insetCircle') {
          const w = safeWidth;
          const h = safeHeight;
          const jitterMultiplier = this._builderPreviewMode
            ? 0
            : this.calculateJitterMultiplier(w, h) * (isAxisCollageLayer ? 1.14 : 1);
          const rotationMultiplier = this._builderPreviewMode
            ? 0
            : this.calculateRotationMultiplier(w, h);
          const jitterX = this._builderPreviewMode ? 0 : (randForBlock('inset:x') - 0.5) * 6 * jitterMultiplier;
          const jitterY = this._builderPreviewMode ? 0 : (randForBlock('inset:y') - 0.5) * 6 * jitterMultiplier;
          const jitterRotation = this._builderPreviewMode
            ? 0
          ...
          const handCutPolygon = this._builderPreviewMode
            ? this._builderExactRect(renderX, renderY, w, h)
            : this.createHandCutPolygon(renderX, renderY, w, h, jitterMultiplier, `${renderSeed}:inset:bg`);
      ...
      const jitterX = this._builderPreviewMode ? 0 : (randForBlock('regular:x') - 0.5) * 6 * jitterMultiplier;
      const jitterY = this._builderPreviewMode ? 0 : (randForBlock('regular:y') - 0.5) * 6 * jitterMultiplier;
      const jitterRotation = this._builderPreviewMode
        ? 0
      ...
      const blockScale =
        this._builderPreviewMode || regularVisibilityScale < 1
      ...
      const handCutPolygon = this._builderPreviewMode
        ? this._builderExactRect(scaledX, scaledY, scaledWidth, scaledHeight)
        : this.createHandCutPolygon(scaledX, scaledY, scaledWidth, scaledHeight, jitterMultiplier, `${renderSeed}:regular-main`);
