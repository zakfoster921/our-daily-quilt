...
class QuiltRendererV2 {
  ...
  _buildLiveGapUnderpaintGroup(blocks, id, bleed = 2.5) {
    ...
    blocks.forEach((block) => {
      ...
      const fill = this._underpaintFillForBlock(block);
      const rect = document.createElementNS('http://www.w3.org/2000/svg', 'rect');
  ...
  renderBlocks(blocks, userPieces = [], submissionCount = 0) {
    ...
    const wobbleUnderpaintPad = 26;
    const builderGapBoost = this._builderPreviewMode ? 8 : 0;
    const liveGapBleed =
    ...
    renderQueue.forEach(({ block, originalIndex }, i) => {
      ...
      if (
        ...
        if (block.specialPatternType === 'diagonalAxis' && Array.isArray(block.polygonPieces)) {
          ...
          const diagSeamBleed = this._gridSeamUnderpaintBleed(effectiveStretch) + (this._builderPreviewMode ? 4 : 0);
          const diagSeamUnderpaint = this._createGridSeamUnderpaintRect(
            renderX,
            renderY,
            w,
            h,
          ...
          diagSeamUnderpaint.setAttribute('fill', this._underpaintFillForBlock(block));
          diagSeamUnderpaint.setAttribute('stroke', 'none');
          specialGroup.appendChild(diagSeamUnderpaint);
      ...
      const gridSeamBleed =
        this._gridSeamUnderpaintBleed(effectiveStretch) + (this._builderPreviewMode && allowLayering ? 3 : 0);
      const seamUnderpaintRect =
        gridSeamBleed > 0
          ? this._createGridSeamUnderpaintRect(
              this._builderPreviewMode && allowLayering ? jitteredBlock.x : block.x,
              this._builderPreviewMode && allowLayering ? jitteredBlock.y : block.y,
              this._builderPreviewMode && allowLayering ? jitteredBlock.width : block.width,
              this._builderPreviewMode && allowLayering ? jitteredBlock.height : block.height,
              gridSeamBleed
            )
          : null;
