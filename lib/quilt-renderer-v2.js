...
class QuiltRendererV2 {
  ...
  _isMacroRegionDebugEnabled() {
    try {
      ...
      if (params.get('macroDebug') === '0') return false;
      if (localStorage.getItem('odqDebugMacroRegions') === '1') return true;
      if (localStorage.getItem('odqDebugMacroRegions') === '0') return false;
      // Default on for admin so region jitter is easy to inspect.
  ...
  renderBlocks(blocks, userPieces = [], submissionCount = 0) {
    ...
    const liveGapBleed = this._gridSeamUnderpaintBleed(effectiveStretch) + (effectiveStretch > 1 ? 1 : 0.5);
