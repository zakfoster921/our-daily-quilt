...
class QuiltRendererV2 {
  ...
  renderBlocks(blocks, userPieces = [], submissionCount = 0) {
    ...
    const renderQueue = displayBlocks
    ...
    renderQueue.forEach(({ block, originalIndex }, i) => {
      ...
      if (
        ...
        if (block.specialPatternType === 'diagonalAxis' && Array.isArray(block.polygonPieces)) {
          ...
          if (originalIndex === this.lastAddedIndex) {
            specialGroup.setAttribute('opacity', '0');
            specialGroup.classList.add('new-block');
            specialGroup.addEventListener('animationend', () => {
              specialGroup.classList.remove('new-block');
              specialGroup.removeAttribute('opacity');
            }, { once: true });
          }
        ...
        if (block.specialPatternType === 'hst') {
          ...
          const jitterX = (randForBlock('hst:x') - 0.5) * 6 * jitterMultiplier;
          const jitterY = (randForBlock('hst:y') - 0.5) * 6 * jitterMultiplier;
          const jitterRotation = (randForBlock('hst:rot') - 0.5) * (isAxisCollageLayer ? 4.2 : 3) * rotationMultiplier;
          ...
          const tris = Utils.getHstOrganicRenderTriangles(block, jitterMultiplier);
          const blockRect = { x: renderX, y: renderY, width: w, height: h };
          const watercolorStrength = watercolorStrengthForSize(w, h);
          const specialGroup = document.createElementNS('http://www.w3.org/2000/svg', 'g');
          specialGroup.classList.add('quilt-parallax-block');
          ...
          tris.forEach((t, ti) => {
            ...
            poly.setAttribute(
              'points',
              (t.points || []).map((p) => `${renderX + p[0]},${renderY + p[1]}`).join(' ')
            );
     ...
     // Add the pattern group to the SVG
      ...
      if (originalIndex === this.lastAddedIndex) {
        ...
        blockGroup.classList.add('new-block');
        blockGroup.addEventListener('animationend', () => {
          blockGroup.classList.remove('new-block');
          blockGroup.removeAttribute('opacity');
        }, { once: true });
      }
    ...
    if (renderMirroredField) {
      const addMirroredLayer = (id, transform) => {
        ...
        mirroredLayer.querySelectorAll('.new-block').forEach((node) => {
