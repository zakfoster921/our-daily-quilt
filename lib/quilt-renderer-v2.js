...
class QuiltRendererV2 {
  ...
  /**
   * Paint-order policy (2026-06): default blocks[] index (structural stack — preserves aligned edges).
   * Optional ?paintOrder=submission for submissionIndex recency (legacy).
   */
  _paintOrderMode() {
    try {
      const param = new URLSearchParams(window.location.search || '').get('paintOrder');
      if (param === 'submission') return 'submission';
      return 'array';
    } catch (_) {
      return 'array';
    }
