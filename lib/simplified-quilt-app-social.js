...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Social {
    ...
    isSocialPostFeedVideoAudiblyUnmuted(video) {
      return video?.dataset?.socialUserUnmuted === '1';
    }
    ...
    syncSocialPostFeedVideoMuteUi(video) {
      ...
      if (!muteBtn) return;
      const unmuted = this.isSocialPostFeedVideoAudiblyUnmuted(video);
      muteBtn.setAttribute('aria-pressed', unmuted ? 'false' : 'true');
      muteBtn.setAttribute('aria-label', unmuted ? 'Mute video' : 'Unmute video');
      muteBtn.classList.toggle('social-post-video-mute--unmuted', unmuted);
    }
