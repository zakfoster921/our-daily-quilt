...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Social {
    isSocialPostsEnabled() {
      return CONFIG.APP.socialPostsEnabled === true;
    }

    /** Beta: entire feature is admin-only until public launch. */
    canAccessSocialPosts() {
      return this.isSocialPostsEnabled() && !!this.isCurrentUserAdmin?.();
    }

    syncSocialPostsEntryVisibility() {
      const enabled = this.canAccessSocialPosts();
      if (enabled) {
        delete document.documentElement.dataset.socialPostsDisabled;
      } else {
        document.documentElement.dataset.socialPostsDisabled = '1';
      }
      const link = document.getElementById('quiltSocialPostsLink');
      if (link) {
        link.hidden = !enabled;
        link.setAttribute('aria-hidden', enabled ? 'false' : 'true');
        link.style.display = enabled ? '' : 'none';
        link.style.visibility = enabled ? '' : 'hidden';
        link.style.pointerEvents = enabled ? '' : 'none';
      }
      ['screen-social-posts', 'screen-social-post-compose', 'screen-social-post-manage'].forEach((screenId) => {
    ...
    guardSocialPostsAccess({ redirect = true } = {}) {
      ...
      if (redirect) {
        this.uiService?.showToast?.(
          this.isSocialPostsEnabled() ? 'Admin access required' : 'Community posts are not available yet'
        );
    ...
    async submitSocialPost({ status }) {
      ...
      try {
        ...
        this.uiService.showToast('Post save failed');
      } finally {
