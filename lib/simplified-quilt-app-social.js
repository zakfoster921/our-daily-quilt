...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Social {
    ...
    /** Cream paper band like Before you go — no torn tape caps. */
    drawSocialPostStoryHeaderPaper(ctx, layout) {
      ...
      ctx.fillRect(-slabW / 2, -slabH / 2, slabW, slabH);
      ctx.restore();
      return { slabX, slabY, slabW, slabH, rotateDeg };
    }
    ...
    drawSocialPostStoryHeader(ctx, { padX, textMaxW, padTop = 192, storyW = 1080 } = {}) {
      const layout = this.measureSocialPostStoryHeader(ctx, { padX, textMaxW, padTop, storyW });
      this.drawSocialPostStoryHeaderPaper(ctx, layout);
