  ...
  class SimplifiedQuiltAppV2Social {
    ...
    buildSocialPostCarouselHtml(items, { classPrefix = 'social-post' } = {}) {
      ...
      if (media.length === 1) {
        ...
        return `<div class="${classPrefix}-media-frame"><img class="${classPrefix}-media${zoomableClass}" src="${safeUrl}" alt="" loading="lazy" /></div>`;
      }
      const slides = media
        .map((item, index) => {
          const safeUrl = this.escapeQuiltFortuneText(item.url || item.previewUrl || '');
          ...
          const inner = isVideo
            ...
            : `<img class="${classPrefix}-media${classPrefix === 'social-post' ? ' social-post-media--zoomable' : ''}" src="${safeUrl}" alt="" loading="lazy" />`;
    ...
    _bindSocialPostImageZoomOverlay(overlay) {
      ...
      stage?.addEventListener(
        ...
        (event) => {
          ...
          if (s.scale > 1.01) {
            s.panning = true;
            s.panStartClient = { x: touches[0].clientX, y: touches[0].clientY };
            s.startTranslateX = s.translateX;
            s.startTranslateY = s.translateY;
          }
        },
      ...
      stage?.addEventListener(
        ...
        (event) => {
          ...
          const s = state();
          if (!touches?.length) return;
          if (touches.length >= 2 && s.pinching) {
            event.preventDefault();
            const distance = this._socialPostImageZoomDistance(touches[0], touches[1]);
            const ratio = s.startDistance > 0 ? distance / s.startDistance : 1;
            s.scale = clampScale(s.startScale * ratio);
            if (s.scale <= 1.01) {
              s.translateX = 0;
              s.translateY = 0;
            }
          ...
          if (touches.length === 1 && s.panning && s.panStartClient && s.scale > 1.01) {
            event.preventDefault();
            const dx = touches[0].clientX - s.panStartClient.x;
            const dy = touches[0].clientY - s.panStartClient.y;
            s.translateX = s.startTranslateX + dx;
            s.translateY = s.startTranslateY + dy;
            this._socialPostImageZoomApplyTransform();
          }
        },
        { passive: false }
