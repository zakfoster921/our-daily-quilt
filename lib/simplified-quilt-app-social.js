  ...
  class SimplifiedQuiltAppV2Social {
    ...
    _bindSocialPostImageZoomOverlay(overlay) {
      ...
      const handleTouchStart = (event) => {
        if (event.target.closest('.social-post-image-zoom__close')) return;
      ...
      closeBtn?.addEventListener('click', handleCloseButtonActivate);
      closeBtn?.addEventListener('touchend', (event) => {
        if (event.cancelable) event.preventDefault();
        handleCloseButtonActivate(event);
      });

      overlay.addEventListener('click', (event) => {
        if (event.target.closest('.social-post-image-zoom__close')) {
          close();
          return;
        }
