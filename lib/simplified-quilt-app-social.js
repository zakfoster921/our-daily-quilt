...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Social {
    ...
    canAccessSocialPosts() {
      return this.canManageSocialPosts();
    }

    guardSocialPostsManageAccess({ redirect = true } = {}) {
      if (this.canManageSocialPosts()) return true;
      if (redirect) {
    ...
    guardSocialPostsAccess({ redirect = true } = {}) {
      return this.guardSocialPostsManageAccess({ redirect });
    }
    ...
    setupSocialEventHandlers() {
      document.addEventListener('screenChange', (e) => {
        ...
        if (screenId === 'screen-social-posts') {
          this.markStudioFloorOpened();
          this._reportStudioFloorOpenOnce?.();
        ...
        if (
          ...
          screenId === 'screen-reflection-themes-archive'
        ) {
          this._scheduleStudioFloorUnreadBadgeReveal?.();
        }
      });
      this.setupSocialPostAppIconBadgeResumeCheck();
    }

    getStudioFloorOpenedStorageKey() {
      return 'ourDailyHasOpenedStudioFloor';
    }

    hasOpenedStudioFloor() {
      try {
        if (localStorage.getItem(this.getStudioFloorOpenedStorageKey()) === 'true') return true;
        // Opened before the discover-wiggle flag shipped.
        if (this.readLastSeenSocialPostId()) return true;
        return false;
      } catch (_) {
        return true;
      }
    ...
    markStudioFloorOpened() {
      if (this.hasOpenedStudioFloor()) return;
      try {
        localStorage.setItem(this.getStudioFloorOpenedStorageKey(), 'true');
      } catch (_) {
        /* ignore */
      }
      this.clearStudioFloorIconDiscoverWiggle();
    }

    /** Fire-and-forget anonymous Studio Floor open count (once per device/day). */
    _reportStudioFloorOpenOnce() {
      const dk = String(
      ...
      if (!dk) return;
      const storageKey = 'ourDailyQuiltStudioFloorOpenV1';
      try {
      ...
      if (this._studioFloorOpenReportInFlightKey === dk) return;
      ...
      this._studioFloorOpenReportInFlightKey = dk;
      void fetch(`${baseUrl}/api/studio-floor-open`, {
        method: 'POST',
        ...
        .finally(() => {
          if (this._studioFloorOpenReportInFlightKey === dk) {
            this._studioFloorOpenReportInFlightKey = '';
          }
        });
    }

    shouldOfferStudioFloorIconDiscoverWiggle() {
      if (this.hasOpenedStudioFloor()) return false;
      if (this._studioFloorIconWigglePlayedThisQuiltVisit) return false;
      if (window.matchMedia?.('(prefers-reduced-motion: reduce)')?.matches) return false;
      if (!document.getElementById('screen-quilt')?.classList.contains('active')) return false;
      return true;
    }

    noteStudioFloorFooterBottomVisit(footerVisible) {
      if (this.hasOpenedStudioFloor()) return;
      if (footerVisible) {
        if (this._studioFloorFooterWasVisible) return;
        this._studioFloorFooterWasVisible = true;
        this._studioFloorFooterBottomVisitCount =
          (this._studioFloorFooterBottomVisitCount || 0) + 1;
        if (this._studioFloorFooterBottomVisitCount >= 2) {
          this.maybePlayStudioFloorIconDiscoverWiggle();
        }
        this._scheduleStudioFloorUnreadBadgeReveal?.();
        return;
      }
      this._studioFloorFooterWasVisible = false;
    }

    resetStudioFloorFooterBottomVisitTracking() {
      this._studioFloorFooterBottomVisitCount = 0;
      this._studioFloorFooterWasVisible = false;
      this._studioFloorIconWigglePlayedThisQuiltVisit = false;
    }

    maybePlayStudioFloorIconDiscoverWiggle() {
      if (!this.shouldOfferStudioFloorIconDiscoverWiggle()) return false;
      const buttons = document.querySelectorAll('#screen-quilt .quilt-studio-floor-icon-btn');
      if (!buttons.length) return false;

      this._studioFloorIconWigglePlayedThisQuiltVisit = true;
      buttons.forEach((btn) => {
        btn.classList.remove('quilt-studio-floor-icon-btn--discover-wiggle');
        void btn.offsetWidth;
        btn.classList.add('quilt-studio-floor-icon-btn--discover-wiggle');
    ...
    clearStudioFloorIconDiscoverWiggle() {
      document.querySelectorAll('.quilt-studio-floor-icon-btn--discover-wiggle').forEach((btn) => {
        btn.classList.remove('quilt-studio-floor-icon-btn--discover-wiggle');
      });
    }
    ...
    getSocialPostLastSeenCommentsStorageKey() {
      return 'ourDailyLastSeenCommentsAt';
    }
    ...
    readLastSeenCommentsAt() {
      try {
        return String(localStorage.getItem(this.getSocialPostLastSeenCommentsStorageKey()) || '').trim();
      } catch (_) {
        return '';
      }
    }

    writeLastSeenCommentsAt(iso) {
      try {
        localStorage.setItem(this.getSocialPostLastSeenCommentsStorageKey(), String(iso || '').trim());
      } catch (_) {
        /* ignore */
      }
    }
    ...
    parseSocialIsoTimestamp(iso) {
      const ms = Date.parse(String(iso || '').trim());
      return Number.isNaN(ms) ? null : ms;
    }

    isSocialIsoAfter(iso, seenIso) {
      const value = String(iso || '').trim();
      const seen = String(seenIso || '').trim();
      if (!value) return false;
      if (!seen) return true;
      const valueMs = this.parseSocialIsoTimestamp(value);
      const seenMs = this.parseSocialIsoTimestamp(seen);
      if (valueMs != null && seenMs != null) return valueMs > seenMs;
      return value > seen;
    }

    pickLatestSocialIso(...values) {
      let latest = '';
      let latestMs = -Infinity;
      for (const raw of values) {
        const iso = String(raw || '').trim();
        if (!iso) continue;
        const ms = this.parseSocialIsoTimestamp(iso);
        if (ms != null) {
          if (ms >= latestMs) {
            latestMs = ms;
            latest = iso;
    ...
    beginSocialPostsCommentsDigestRefresh() {
      this._socialPostsCommentsDigestSeq = (this._socialPostsCommentsDigestSeq || 0) + 1;
      return this._socialPostsCommentsDigestSeq;
    }

    cancelSocialPostsCommentsDigestRefresh() {
      this._socialPostsCommentsDigestSeq = (this._socialPostsCommentsDigestSeq || 0) + 1;
    }

    isSocialPostsCommentsDigestRefreshCurrent(seq) {
      return seq === this._socialPostsCommentsDigestSeq;
    }

    postHasUnreadComments(post) {
      if (!this.canManageSocialPosts()) return false;
      const lastComment = String(post?.lastCommentAtIso || '').trim();
      if (!lastComment) return false;
      const lastSeen = this.readLastSeenCommentsAt();
      if (!lastSeen) return true;
      return this.isSocialIsoAfter(lastComment, lastSeen);
    }

    hasUnseenSocialCommentsInPosts(posts) {
      if (!this.canManageSocialPosts()) return false;
      const list = Array.isArray(posts) ? posts : [];
      return list.some((post) => this.postHasUnreadComments(post));
    }

    isSocialCommentUnread(comment, lastSeen = null) {
      if (!this.canManageSocialPosts()) return false;
      const createdAt = String(comment?.createdAtIso || '').trim();
      if (!createdAt) return false;
      const seenAt = lastSeen == null ? this.readLastSeenCommentsAt() : String(lastSeen || '').trim();
      if (!seenAt) return true;
      return this.isSocialIsoAfter(createdAt, seenAt);
    }

    getPushNotificationTypes() {
      return ['daily_quote', 'studio_floor'];
    }
    ...
    refreshStudioFloorUnreadBadge({ unseen = null, immediate = false } = {}) {
      if (unseen == null) {
        const posts = this._socialPostsFeedPosts || [];
        const latest = this.getLatestSocialPostIdFromPosts(posts);
        const unseenPost = latest ? this.hasUnseenSocialPostsForLatestId(latest) : false;
        const unseenComments = this.canManageSocialPosts() && this.hasUnseenSocialCommentsInPosts(posts);
        unseen = unseenPost || unseenComments;
      }
      this._studioFloorBadgeUnseen = !!unseen;
      const label = unseen
        ? this.canManageSocialPosts()
          ? 'Studio floor, new activity'
          : 'Studio floor, new post'
        : 'Studio floor';
      ...
      if (!unseen) {
        this._clearStudioFloorUnreadBadgeRevealTimers();
        document.querySelectorAll('.quilt-studio-floor-icon-btn').forEach((btn) => {
          btn.classList.remove('quilt-studio-floor-icon-btn--unread');
          btn.classList.remove('quilt-studio-floor-icon-btn--unread-reveal');
        });
        return;
      }
      ...
      if (immediate) {
        this._clearStudioFloorUnreadBadgeRevealTimers();
        this._applyStudioFloorUnreadBadgeVisual({ animate: false });
        return;
      }
    ...
    _clearStudioFloorUnreadBadgeRevealTimers() {
      if (this._studioFloorUnreadBadgeRevealTimer != null) {
        clearTimeout(this._studioFloorUnreadBadgeRevealTimer);
        this._studioFloorUnreadBadgeRevealTimer = null;
      }
      if (this._studioFloorUnreadBadgeRevealRaf != null) {
        cancelAnimationFrame(this._studioFloorUnreadBadgeRevealRaf);
        this._studioFloorUnreadBadgeRevealRaf = null;
      }
    }

    _isStudioFloorIconBarVisibleForButton(btn) {
      if (!btn) return false;
      if (btn.closest('#screen-quilt .quilt-scroll-icon-footer')) {
        if (!document.getElementById('screen-quilt')?.classList.contains('active')) return false;
        return !!this.isQuiltFooterVisibleInViewport?.();
      }
      if (btn.closest('.quilt-viewport-footer-icon-row')) {
        return !!btn.closest('.screen.active');
      }
      if (btn.closest('#screen-exit-chamber')) {
        return document.getElementById('screen-exit-chamber')?.classList.contains('active');
      }
      return false;
    }

    _studioFloorUnreadBadgeRevealTargets() {
      return [...document.querySelectorAll('.quilt-studio-floor-icon-btn')].filter(
        (btn) =>
          this._isStudioFloorIconBarVisibleForButton(btn) &&
          !btn.classList.contains('quilt-studio-floor-icon-btn--unread')
      );
    }

    _scheduleStudioFloorUnreadBadgeReveal() {
      if (!this._studioFloorBadgeUnseen) return;
      this._clearStudioFloorUnreadBadgeRevealTimers();

      const attempt = () => {
        this._studioFloorUnreadBadgeRevealRaf = null;
        if (!this._studioFloorBadgeUnseen) return;
        if (!this._studioFloorUnreadBadgeRevealTargets().length) return;

        this._studioFloorUnreadBadgeRevealTimer = setTimeout(() => {
          this._studioFloorUnreadBadgeRevealTimer = null;
          if (!this._studioFloorBadgeUnseen) return;
          this._applyStudioFloorUnreadBadgeVisual({ animate: true });
        }, 420);
      };

      this._studioFloorUnreadBadgeRevealRaf = requestAnimationFrame(() => {
        requestAnimationFrame(attempt);
      });
    }

    _applyStudioFloorUnreadBadgeVisual({ animate = true } = {}) {
      if (!this._studioFloorBadgeUnseen) return;
      const targets = this._studioFloorUnreadBadgeRevealTargets();
      ...
      targets.forEach((btn) => {
        btn.classList.add('quilt-studio-floor-icon-btn--unread');
        if (!shouldAnimate) return;
        btn.classList.remove('quilt-studio-floor-icon-btn--unread-reveal');
        void btn.offsetWidth;
        btn.classList.add('quilt-studio-floor-icon-btn--unread-reveal');
        btn.addEventListener(
          'animationend',
          () => {
            btn.classList.remove('quilt-studio-floor-icon-btn--unread-reveal');
    ...
    markSocialPostsFeedSeen(posts) {
      ...
      if (this.canManageSocialPosts()) {
        const list = Array.isArray(posts) ? posts : Array.isArray(this._socialPostsFeedPosts) ? this._socialPostsFeedPosts : [];
        const seenIso = this.pickLatestSocialIso(
          new Date().toISOString(),
          ...list.map((post) => post?.lastCommentAtIso)
        );
        this.writeLastSeenCommentsAt(seenIso);
        this._socialPostsUnreadCommentCountByPostId = {};
      }
    ...
    async checkForUnseenSocialPosts() {
      ...
      try {
        const page = await this.fetchSocialPostsFeedPage('', { limit: 10 });
        const latestId = this.getLatestSocialPostIdFromPosts(page.posts);
        const unseenPost = this.hasUnseenSocialPostsForLatestId(latestId);
        const unseenComments = this.canManageSocialPosts() && this.hasUnseenSocialCommentsInPosts(page.posts);
        const unseen = unseenPost || unseenComments;
    ...
    clearSocialPostsNewCommentsDigest() {
      const host = document.getElementById('socialPostsNewComments');
      if (!host) return;
      host.hidden = true;
      host.innerHTML = '';
    }

    buildSocialPostDigestCaptionSnippet(post) {
      const caption = String(post?.caption || '').replace(/\s+/g, ' ').trim();
      const dateLabel = this.formatSocialPostCaptionDate(post?.publishedAtIso || post?.createdAtIso);
      const snippet = caption ? caption.slice(0, 72) + (caption.length > 72 ? '…' : '') : 'Studio floor post';
      return dateLabel ? `${dateLabel}: ${snippet}` : snippet;
    }

    renderSocialPostsNewCommentsDigest(items) {
      const host = document.getElementById('socialPostsNewComments');
      if (!host) return;
      const entries = Array.isArray(items) ? items : [];
      if (!entries.length || !this.canManageSocialPosts()) {
        this.clearSocialPostsNewCommentsDigest();
        return;
      }
      const countLabel = entries.length === 1 ? '1 new since last visit' : `${entries.length} new since last visit`;
      host.innerHTML = `
        <div class="social-posts-new-comments__inner">
          <div class="social-posts-new-comments-heading-row">
            <p class="social-posts-new-comments-heading">${this.escapeQuiltFortuneText(countLabel)}</p>
            <button type="button" class="social-posts-new-comments-clear-all">Clear all</button>
          </div>
          <ul class="social-posts-new-comments-list">
            ${entries
              .map((item) => {
                const author = String(item.displayName || '').trim() || 'Friend';
                const text = String(item.text || '').replace(/\s+/g, ' ').trim();
                const snippet = text.slice(0, 120) + (text.length > 120 ? '…' : '');
                const postLabel = this.buildSocialPostDigestCaptionSnippet(item.post);
                return `<li>
                  <button type="button" class="social-posts-new-comments-item" data-post-id="${this.escapeQuiltFortuneText(item.postId)}">
                    <span class="social-posts-new-comments-author">${this.escapeQuiltFortuneText(author)}</span>
                    <span class="social-posts-new-comments-text">${this.escapeQuiltFortuneText(snippet)}</span>
                    <span class="social-posts-new-comments-post">${this.escapeQuiltFortuneText(postLabel)}</span>
                  </button>
                </li>`;
              })
              .join('')}
      ...
      `;
      host.hidden = false;
      if (host.dataset.digestBound !== '1') {
        host.dataset.digestBound = '1';
        host.addEventListener('click', (event) => {
          if (event.target.closest('.social-posts-new-comments-clear-all')) {
            this.cancelSocialPostsCommentsDigestRefresh();
            this.markSocialPostsFeedSeen(this._socialPostsFeedPosts);
            this.clearSocialPostsNewCommentsDigest();
            this.renderSocialPostFeedEntries(this._socialPostsFeedPosts);
            return;
          }
          const btn = event.target.closest('.social-posts-new-comments-item');
          if (!btn) return;
          const postId = btn.getAttribute('data-post-id') || '';
          const entry = document.querySelector(`.social-post-entry[data-post-id="${CSS.escape(postId)}"]`);
          if (!entry) return;
          entry.scrollIntoView({ behavior: 'smooth', block: 'start' });
          void this.openSocialPostEntryComments(entry);
        });
      }
    }

    async refreshSocialPostsNewCommentsDigest(posts) {
      const refreshSeq = this.beginSocialPostsCommentsDigestRefresh();
      if (!this.canManageSocialPosts()) {
        this._socialPostsUnreadCommentCountByPostId = {};
        this.clearSocialPostsNewCommentsDigest();
        return [];
      ...
      const list = Array.isArray(posts) ? posts : [];
      const lastSeen = this.readLastSeenCommentsAt();
      const unreadPosts = list.filter((post) => this.postHasUnreadComments(post));
      const countByPostId = {};
      const digestItems = [];
      ...
      await Promise.all(
        unreadPosts.map(async (post) => {
          ...
          try {
            ...
            const unread = allComments.filter((comment) => this.isSocialCommentUnread(comment, lastSeen));
            if (unread.length) {
              countByPostId[postId] = unread.length;
            ...
            /* ignore per-post digest failures */
          }
        })
      );

      digestItems.sort((a, b) => String(b.createdAtIso || '').localeCompare(String(a.createdAtIso || '')));
      if (!this.isSocialPostsCommentsDigestRefreshCurrent(refreshSeq)) return digestItems;
      this._socialPostsUnreadCommentCountByPostId = countByPostId;
      this.renderSocialPostsNewCommentsDigest(digestItems);
      return digestItems;
    }
    ...
    resolveSocialPostCarouselElement(el) {
      return el?.closest?.('.social-post-carousel, .social-post-compose-carousel') || null;
    }
    ...
    syncSocialPostImageFrameLayout(img) {
      if (!img) return;
      const width = Number(img.naturalWidth || img.getAttribute('width'));
      const height = Number(img.naturalHeight || img.getAttribute('height'));
      if (!width || !height) return;
      const ratio = `${width} / ${height}`;
      const slide = img.closest('.social-post-carousel-slide, .social-post-compose-carousel-slide');
      const carousel = img.closest('.social-post-carousel, .social-post-compose-carousel');
      const mediaFrame = img.closest('.social-post-media-frame, .social-post-compose-media-frame');
      if (mediaFrame) mediaFrame.style.aspectRatio = ratio;
      if (
        slide &&
        carousel &&
    ...
    syncSocialPostCarouselLayout(carousel) {
      ...
      const activeSlide = carousel.querySelector(
        '.social-post-carousel-slide--active, .social-post-compose-carousel-slide--active'
      );
      ...
      const activeImg = activeSlide?.querySelector('img.social-post-media');
      if (activeImg) {
        if (activeImg.naturalWidth && activeImg.naturalHeight) {
          this.syncSocialPostImageFrameLayout(activeImg);
          return;
        }
        const w = Number(activeImg.getAttribute('width'));
        const h = Number(activeImg.getAttribute('height'));
        if (w > 0 && h > 0) {
          const ratio = `${w} / ${h}`;
          carousel.style.aspectRatio = ratio;
          const mediaFrame = activeImg.closest('.social-post-media-frame');
          if (mediaFrame) mediaFrame.style.aspectRatio = ratio;
          return;
        }
      }
      carousel.style.removeProperty('aspect-ratio');
    }
    ...
    buildSocialPostFeedImageHtml(item, { priority = 'auto', loading = 'lazy' } = {}) {
      ...
      const srcset = hasDims
        ? ` srcset="${safeUrl} ${Math.round(width)}w" sizes="100vw"`
        : '';
      const loadingAttr = loading === 'eager' ? ' loading="eager"' : ' loading="lazy"';
      const priorityAttr = priority === 'high' ? ' fetchpriority="high"' : '';
      return `<img class="social-post-media social-post-media--zoomable" src="${safeUrl}"${srcset}${dimAttrs} alt=""${loadingAttr}${priorityAttr} decoding="async" />`;
    }
    ...
    buildSocialPostCarouselHtml(items, { classPrefix = 'social-post', imagePriority = 'auto' } = {}) {
      ...
      const renderMediaItem = (item) => {
        ...
        if (classPrefix === 'social-post') {
          return this.buildSocialPostFeedImageHtml(item, { priority: imagePriority, loading: imageLoading });
        }
      ...
      const slides = media
        .map(
          (item, index) =>
            `<div class="${classPrefix}-carousel-slide${index === 0 ? ` ${classPrefix}-carousel-slide--active` : ''}" data-slide-index="${index}">${renderMediaItem(item)}</div>`
        )
        .join('');
      const dots = media
        .map(
        ...
        .join('');
      const carouselStyle = classPrefix === 'social-post' ? this.buildSocialPostMediaFrameStyle(media[0]) : '';
      return `<div class="${classPrefix}-carousel" data-slide-count="${media.length}" data-active-slide-index="0"${carouselStyle}><div class="${classPrefix}-carousel-track">${slides}</div><div class="${classPrefix}-carousel-dots">${dots}</div></div>`;
    }

    getSocialPostCarouselActiveIndex(carousel) {
      if (!carousel) return 0;
      const stored = Number(carousel.dataset.activeSlideIndex);
      if (Number.isFinite(stored) && stored >= 0) return stored;
      const active = carousel.querySelector('[class*="-carousel-slide--active"]');
      return Number(active?.getAttribute('data-slide-index') || 0);
    }
    ...
    setSocialPostCarouselSlide(carousel, index) {
      ...
      carousel.dataset.activeSlideIndex = String(clamped);
      const track = carousel.querySelector('[class*="-carousel-track"]');
      ...
      carousel.querySelectorAll('[data-slide-index]').forEach((el) => {
        ...
        const isActive = Number(el.getAttribute('data-slide-index')) === clamped;
    ...
    bindSocialPostFeedImages(rootEl) {
      ...
      scope.querySelectorAll('img.social-post-media--zoomable').forEach((img) => {
        ...
        const applyLayout = () => {
          ...
          this.syncSocialPostImageFrameLayout(img);
          const carousel = img.closest('.social-post-carousel');
          if (carousel) this.syncSocialPostCarouselLayout(carousel);
        };
    ...
    bindSocialPostCarouselSwipe(rootEl) {
      ...
      rootEl.addEventListener('click', (event) => {
        ...
        if (!dot) return;
        const carousel = this.resolveSocialPostCarouselElement(dot);
        this.setSocialPostCarouselSlide(carousel, Number(dot.getAttribute('data-slide-index') || 0));
      });
      rootEl.addEventListener(
        'touchstart',
        (event) => {
          const carousel = this.resolveSocialPostCarouselElement(event.target);
          if (!carousel || Number(carousel.dataset.slideCount || 1) <= 1) return;
          carousel._touchStartX = event.touches?.[0]?.clientX ?? null;
          carousel._touchStartY = event.touches?.[0]?.clientY ?? null;
        },
        { passive: true }
      );
      rootEl.addEventListener(
        ...
        (event) => {
          ...
          const endY = event.changedTouches?.[0]?.clientY ?? carousel._touchStartY ?? endX;
          const deltaX = endX - carousel._touchStartX;
          const deltaY = endY - (carousel._touchStartY ?? endY);
          carousel._touchStartX = null;
          carousel._touchStartY = null;
          if (Math.abs(deltaX) < 40 || Math.abs(deltaX) <= Math.abs(deltaY)) return;
          carousel._suppressCarouselClick = true;
          const count = Number(carousel.dataset.slideCount || 1);
          const active = this.getSocialPostCarouselActiveIndex(carousel);
          const next = deltaX < 0 ? Math.min(count - 1, active + 1) : Math.max(0, active - 1);
    ...
    /** Studio floor byline for Zak — not the portal greeting name. */
    getSocialPostCommentDisplayName() {
      if (this.canManageSocialPosts?.()) return 'Zak';
      try {
        if (String(localStorage.getItem('ourDailyResetToken') || '').trim()) return 'Zak';
      } catch (_) {
        /* ignore */
      }
      return typeof Utils.getNameThanksDisplayName === 'function' ? Utils.getNameThanksDisplayName() : '';
    }

    async buildSocialPostCommentRequestHeaders() {
      const headers = { 'Content-Type': 'application/json' };
      if (this.getSocialPostCommentDisplayName() !== 'Zak') return headers;
      const token = await this.getSocialPostAdminToken({ promptIfMissing: false });
      if (token) Object.assign(headers, this.buildSocialPostAdminHeaders(token));
      return headers;
    }
    ...
    resolveSocialPostsFeedHasMore(page, { limit = 10 } = {}) {
      const posts = Array.isArray(page?.posts) ? page.posts : [];
      const pageLimit = Math.max(1, Number(limit) || 10);
      if (!posts.length || !page?.hasMore) return false;
      return posts.length >= pageLimit;
    }

    syncSocialPostsLoadMoreButton() {
    ...
    getSocialCommentHeartsStorageKey() {
      return 'ourDailySocialCommentHearts';
    }

    socialCommentHeartKey(postId, commentId) {
      return `${String(postId || '').trim()}:${String(commentId || '').trim()}`;
    }

    readSocialCommentHeartedKeys() {
      try {
        const raw = localStorage.getItem(this.getSocialCommentHeartsStorageKey());
    ...
    isSocialCommentHeartedLocally(postId, commentId) {
      const key = this.socialCommentHeartKey(postId, commentId);
      if (!key || key === ':') return false;
      return this.readSocialCommentHeartedKeys().includes(key);
    }

    setSocialCommentHeartedLocally(postId, commentId, hearted) {
      const key = this.socialCommentHeartKey(postId, commentId);
      if (!key || key === ':') return hearted;
      const keys = new Set(this.readSocialCommentHeartedKeys());
      if (hearted) keys.add(key);
      else keys.delete(key);
      try {
        localStorage.setItem(this.getSocialCommentHeartsStorageKey(), JSON.stringify(Array.from(keys)));
      } catch (_) {
        /* ignore */
      }
      return hearted;
    ...
    resolveSocialCommentHeartState(comment, postId = '') {
      const commentId = String(comment?.commentId || '').trim();
      const resolvedPostId = String(postId || comment?.postId || '').trim();
      const localHearted = this.isSocialCommentHeartedLocally(resolvedPostId, commentId);
      const apiHearted = !!comment?.hearted;
      const hearted = apiHearted || localHearted;
      let heartCount = Math.max(0, Number(comment?.heartCount) || 0);
      if (hearted && heartCount === 0) heartCount = 1;
      return { hearted, heartCount };
    }

    renderSocialPostCommentHeartHtml(comment, postId = '') {
      const { hearted, heartCount } = this.resolveSocialCommentHeartState(comment, postId);
      const countHtml =
        heartCount > 0
          ? `<span class="social-post-comment-heart-count">${heartCount}</span>`
          : '<span class="social-post-comment-heart-count" hidden aria-hidden="true">0</span>';
      return `<button type="button" class="social-post-comment-heart${hearted ? ' social-post-comment-heart--active' : ''}" aria-label="${hearted ? 'Unlike comment' : 'Heart comment'}" aria-pressed="${hearted ? 'true' : 'false'}">
        <span class="material-symbols-outlined" aria-hidden="true" translate="no">favorite</span>
        ${countHtml}
      </button>`;
    }

    syncSocialPostCommentHeart(commentEl, { hearted, heartCount } = {}) {
      if (!commentEl) return;
      const button = commentEl.querySelector('.social-post-comment-heart');
      if (!button) return;
      const active = !!hearted;
      const count = Math.max(0, Number(heartCount) || 0);
      button.classList.toggle('social-post-comment-heart--active', active);
      button.setAttribute('aria-pressed', active ? 'true' : 'false');
      button.setAttribute('aria-label', active ? 'Unlike comment' : 'Heart comment');
      const countEl = button.querySelector('.social-post-comment-heart-count');
      if (countEl) {
    ...
    async toggleSocialPostCommentHeart({ postId, commentId, commentEl } = {}) {
      const baseUrl = this.getSocialPostApiBaseUrl();
      if (!baseUrl || !postId || !commentId) return;
      const clientId = UtilsCore.getOrCreateUserId();
      const button = commentEl?.querySelector('.social-post-comment-heart');
      if (button?.dataset.heartPending === '1') return;
      const wasHearted = button?.classList.contains('social-post-comment-heart--active');
      const priorCount = Math.max(0, Number(button?.querySelector('.social-post-comment-heart-count')?.textContent) || 0);
      const optimisticHearted = !wasHearted;
      const optimisticCount = Math.max(0, priorCount + (optimisticHearted ? 1 : -1));
      if (button) button.dataset.heartPending = '1';
      this.syncSocialPostCommentHeart(commentEl, {
        hearted: optimisticHearted,
        heartCount: optimisticCount
      });
      try {
        const res = await fetch(
          `${baseUrl}/api/social-posts/${encodeURIComponent(postId)}/comments/${encodeURIComponent(commentId)}/heart`,
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        this.setSocialCommentHeartedLocally(postId, commentId, !!data.hearted);
      } catch (error) {
        this.setSocialCommentHeartedLocally(postId, commentId, optimisticHearted);
        this.syncSocialPostCommentHeart(
          commentEl,
          this.resolveSocialCommentHeartState(
            {
              commentId,
              postId,
              hearted: optimisticHearted,
        ...
        if (button) delete button.dataset.heartPending;
      }
    }
    ...
    normalizeSocialPostCaptionText(value) {
      return String(value || '')
        .replace(/\r\n/g, '\n')
        .replace(/\r/g, '\n')
        .split('\n')
    ...
    renderSocialPostCaptionHtml(caption, iso) {
      ...
      const text = this.escapeQuiltFortuneText(this.normalizeSocialPostCaptionText(caption));
    ...
    renderSocialPostMedia(post, { imagePriority = 'auto' } = {}) {
    ...
    layoutSocialPostStoryDateCaption(ctx, { dateLabel = '', caption = '', textMaxW, textSize, font }) {
      ...
      const safeCaption = this.normalizeSocialPostCaptionText(caption);
      ...
      paragraphs.forEach((paragraph, paragraphIndex) => {
        ...
        const wrapped =
          datePrefix && isFirstParagraph && bodyLines.length === 0
            ...
            : this.wrapSocialPostStoryLines(ctx, trimmed, textMaxW);
        bodyLines.push(...wrapped);
      });
      return { datePrefix, bodyLines, lineH: textSize * 1.45 };
    }

    fitSocialPostStoryLineWithEllipsis(ctx, text, maxWidth, textSize, font) {
      const ellipsis = '…';
    ...
    /** Keep caption readable: fixed type size, max 2 lines, ellipsis when longer. */
    truncateSocialPostStoryCaptionLayout(
      ctx,
      layout,
      { textMaxW, textSize, font, maxLines = 2 } = {}
    ) {
      if (!layout) return layout;
      const bodyLines = Array.isArray(layout.bodyLines) ? layout.bodyLines : [];
      if (bodyLines.length <= maxLines) return layout;
      const kept = bodyLines.slice(0, maxLines);
      const lastIdx = maxLines - 1;
      kept[lastIdx] = this.fitSocialPostStoryLineWithEllipsis(
        ctx,
        kept[lastIdx],
        textMaxW,
        textSize,
    ...
    drawSocialPostStoryDateCaption(ctx, { datePrefix, bodyLines, padX, top, textSize, font, lineH, inkColor }) {
      ...
      try {
        ctx.letterSpacing = `${Math.round(textSize * 0.03)}px`;
      } catch (_) {
        /* ignore */
      }
      let ty = top;
    ...
    /** Match Before you go / settings display title scale on a phone-width viewport. */
    ...
    measureSocialPostStoryHeader(ctx, { padX, textMaxW, padTop = 192, storyW = 1080 } = {}) {
      const HEADER_FONT = "'Barlow Condensed', 'Arial Narrow', 'Helvetica Neue', Arial, sans-serif";
      const HEADER_COLOR = '#241f19';
      ...
      let headerSize = this.getSocialPostStoryHeaderFontSize(storyW);
    ...
    getSocialPostStoryHeaderPaperFallback() {
      return '#e8e4dc';
    }

    /** Same cream tape scrap as Before you go pinned cards. */
    getSocialPostStoryHeaderTapeUrl() {
      const rel = 'assets/before-you-go-tape-alpha.png';
      try {
    ...
    async loadSocialPostStoryHeaderTapeImage() {
      if (this._socialPostStoryBygTapeImage) {
        return this._socialPostStoryBygTapeImage;
      }
      const url = this.getSocialPostStoryHeaderTapeUrl();
      return new Promise((resolve, reject) => {
        const im = new Image();
        im.onload = () => {
          this._socialPostStoryBygTapeImage = im;
          resolve(im);
        };
    ...
    /** Soft paper tint from a photo accent — readable under dark title type. */
    socialPostStoryAccentToPaperFill(rgb) {
      const fallback = this.getSocialPostStoryHeaderPaperFallback();
      if (!rgb) return fallback;
      const base = { r: 232, g: 228, b: 220 };
      const t = 0.48;
      let r = Math.round(base.r * (1 - t) + Number(rgb.r || 0) * t);
      let g = Math.round(base.g * (1 - t) + Number(rgb.g || 0) * t);
      let b = Math.round(base.b * (1 - t) + Number(rgb.b || 0) * t);
    ...
    /** Weighted mid-tone chromatic sample from the post image. */
    sampleSocialPostStoryAccentFromImage(image) {
      if (!image) return null;
      const iw = Math.max(1, image.naturalWidth || image.width || 0);
      const ih = Math.max(1, image.naturalHeight || image.height || 0);
    ...
    resolveSocialPostStoryHeaderPaperFill(image) {
      return this.socialPostStoryAccentToPaperFill(this.sampleSocialPostStoryAccentFromImage(image));
    }

    /** Paper band like Before you go — tinted from image accent when available. */
    drawSocialPostStoryHeaderPaper(ctx, layout, fillColor, tapeImage = null) {
      if (!layout) return null;
      const { slabX, slabY, slabW, slabH } = layout;
      const rotateDeg = -1.15;
      const fill = fillColor || this.getSocialPostStoryHeaderPaperFallback();

      ctx.save();
      ctx.translate(slabX + slabW / 2, slabY + slabH / 2);
      ...
      ctx.shadowOffsetY = 3;
      ctx.fillStyle = fill;
      ctx.fillRect(-slabW / 2, -slabH / 2, slabW, slabH);
      ctx.shadowColor = 'transparent';
      ctx.shadowBlur = 0;
      ctx.shadowOffsetY = 0;

      const wash = ctx.createLinearGradient(-slabW / 2, 0, slabW / 2, 0);
      ...
      if (tapeImage) {
        ...
        const tapeW = Math.round(Math.min(Math.max(slabW * 0.26, 160), 280));
        const tapeH = Math.round(tapeW * (srcH / srcW));
        const tapeX = -tapeW / 2;
        // Mostly above the strip — only a thin bite overlaps the paper, clear of the title.
        const tapeOverlap = Math.round(Math.min(18, tapeH * 0.18));
        const tapeY = -slabH / 2 - tapeH + tapeOverlap;
        ctx.shadowColor = 'rgba(45, 36, 29, 0.14)';
        ctx.shadowBlur = 6;
        ctx.shadowOffsetX = 0;
        ctx.shadowOffsetY = 2;
        ctx.drawImage(tapeImage, tapeX, tapeY, tapeW, tapeH);
        ctx.shadowColor = 'transparent';
        ctx.shadowBlur = 0;
        ctx.shadowOffsetY = 0;
      }
      ctx.restore();
      return { slabX, slabY, slabW, slabH, rotateDeg, fill };
    }

    drawSocialPostStoryHeaderText(ctx, layout) {
      if (!layout) return;
      const centerX = Math.round((layout.storyW || 1080) / 2);
      ...
      ctx.fillText(layout.headerText, centerX, layout.padTop);
    }

    drawSocialPostStoryHeader(
      ctx,
      { padX, textMaxW, padTop = 192, storyW = 1080, paperFill, tapeImage = null } = {}
    ) {
      const layout = this.measureSocialPostStoryHeader(ctx, { padX, textMaxW, padTop, storyW });
      this.drawSocialPostStoryHeaderPaper(ctx, layout, paperFill, tapeImage);
    ...
    async createSocialPostStoryShareBlob(imageBlob, { caption = '', dateLabel = '' } = {}) {
      ...
      const paperFill = this.resolveSocialPostStoryHeaderPaperFill(postImage);
      let headerTape = null;
      try {
      ...
      // Match caption width to the photo mat — not the full story width.
      ...
      let captionLayout = this.layoutSocialPostStoryDateCaption(ctx, {
        dateLabel: safeDate,
        caption: safeCaption,
        textMaxW: captionTextMaxW,
        textSize,
        font: FONT
      });
      captionLayout = this.truncateSocialPostStoryCaptionLayout(ctx, captionLayout, {
        textMaxW: captionTextMaxW,
        textSize,
        font: FONT,
      ...
      ctx.textBaseline = 'top';
      try {
        ctx.letterSpacing = `${Math.round(ctaSize * 0.045)}px`;
      } catch (_) {
        /* ignore */
      }
    ...
    renderSocialPostComment(comment, { isReply = false, isNew = false, postId = '' } = {}) {
      ...
      const canAdminDelete = this.canManageSocialPosts();
      const canEdit = isOwn || this.canManageSocialPosts();
      ...
      const editLink = canEdit
        ? `<button type="button" class="social-post-comment-edit-link" data-comment-id="${this.escapeQuiltFortuneText(comment.commentId)}"${!isOwn ? ' data-admin-edit="true"' : ''}>edit</button>`
        : '';
      const adminDeleteLink = canAdminDelete
        ? `<button type="button" class="social-post-comment-delete-link" data-comment-id="${this.escapeQuiltFortuneText(comment.commentId)}" data-admin-delete="true">delete</button>`
        : '';
      const metaLinks = [replyLink, editLink, adminDeleteLink].filter(Boolean).join('');
      const metaHtml = metaLinks ? `<span class="social-post-comment-meta">${metaLinks}</span>` : '';
      const newClass = isNew ? ' social-post-comment--new' : '';
      return `
        <article class="social-post-comment${isReply ? ' social-post-comment--reply' : ''}${newClass}" data-comment-id="${this.escapeQuiltFortuneText(comment.commentId)}">
          <div class="social-post-comment-row">
            <p class="social-post-comment-body">
              ${authorLabel}
              <span class="social-post-comment-text">${text}</span>
              ${metaHtml}
            </p>
            ${this.renderSocialPostCommentHeartHtml(comment, postId || comment.postId)}
          </div>
    ...
    async loadSocialPostThread(postId, container) {
      ...
      try {
        const lastSeen = this.canManageSocialPosts() ? this.readLastSeenCommentsAt() : '';
        const topLevel = await this.fetchSocialPostComments(postId);
        const blocks = [];
        for (const comment of topLevel) {
          blocks.push(
            this.renderSocialPostComment(comment, {
              postId,
              isNew: this.isSocialCommentUnread(comment, lastSeen)
            })
          );
          const replies = await this.fetchSocialPostComments(postId, comment.commentId);
          if (replies.length) {
            blocks.push(
              `<div class="social-post-comment-replies">${replies
                .map((reply) =>
                  this.renderSocialPostComment(reply, {
                    postId,
                    isReply: true,
                    isNew: this.isSocialCommentUnread(reply, lastSeen)
                  })
                )
                .join('')}</div>`
            );
    ...
    buildSocialPostCommentLabel(post) {
      const postId = String(post?.postId || '');
      const totalCount = Number(post?.commentCount) || 0;
      const unreadCount =
        this.canManageSocialPosts() && postId ? Number(this._socialPostsUnreadCommentCountByPostId?.[postId] || 0) : 0;
      const hasUnread = this.canManageSocialPosts() && this.postHasUnreadComments(post);
      if (unreadCount === 1) return '1 new';
      if (unreadCount > 1) return `${unreadCount} new`;
      if (hasUnread) return 'New comments';
    ...
    renderSocialPostFeedEntries(posts) {
      ...
      feed.innerHTML = entries
        .map((post, index) => {
          ...
          const commentLabel = this.buildSocialPostCommentLabel(post);
          const hasUnread = this.canManageSocialPosts() && this.postHasUnreadComments(post);
          const isLatestPost = index === 0;
          const commentToggle = isLatestPost
            ? ''
            : `<button type="button" class="social-post-entry-toggle${hasUnread ? ' social-post-entry-toggle--new' : ''}" aria-expanded="false" aria-controls="${detailsId}">
          ...
          const entryClasses = [
            'settings-item',
            'social-post-entry',
            isLatestPost ? 'social-post-entry--open social-post-entry--latest' : '',
            hasUnread ? 'social-post-entry--has-new-comments' : ''
          ]
            .filter(Boolean)
            .join(' ');
          return `
            <article class="${entryClasses}" data-post-id="${this.escapeQuiltFortuneText(postId)}">
              <div class="settings-item__slab social-post-entry__slab">
              ${this.renderSocialPostMedia(post, { imagePriority: index === 0 ? 'high' : 'auto' })}
      ...
      const openEntries = [];
      feed.querySelectorAll('.social-post-entry').forEach((entry, index) => {
        const post = entries[index];
        if (index === 0 || (this.canManageSocialPosts() && post && this.postHasUnreadComments(post))) {
    ...
    _bindSocialPostFeedEvents(feed) {
      feed.addEventListener('click', async (event) => {
        ...
        if (carouselDot) {
          const carousel = this.resolveSocialPostCarouselElement(carouselDot);
        ...
        const commentHeartBtn = event.target.closest('.social-post-comment-heart');
        if (commentHeartBtn) {
          event.preventDefault();
          const comment = commentHeartBtn.closest('.social-post-comment');
          const entry = commentHeartBtn.closest('.social-post-entry');
          const postId = entry?.getAttribute('data-post-id') || '';
          const commentId = comment?.getAttribute('data-comment-id') || '';
          await this.toggleSocialPostCommentHeart({ postId, commentId, commentEl: comment });
          return;
        }
        ...
        if (zoomImage) {
          const zoomCarousel = this.resolveSocialPostCarouselElement(zoomImage);
          if (zoomCarousel?._suppressCarouselClick) {
            zoomCarousel._suppressCarouselClick = false;
            return;
          }
        ...
        if (editLink) {
          ...
          if (form && body) {
            form.dataset.adminEdit = editLink.getAttribute('data-admin-edit') === 'true' ? 'true' : '';
            body.hidden = true;
            form.hidden = false;
        ...
        const deleteBtn = event.target.closest('.social-post-comment-delete-btn, .social-post-comment-delete-link');
        if (deleteBtn) {
          const entry = deleteBtn.closest('.social-post-entry');
          const postId = entry?.getAttribute('data-post-id') || '';
          const commentId = deleteBtn.getAttribute('data-comment-id') || '';
          await this.deleteSocialPostComment(postId, commentId, entry, {
            adminDelete: deleteBtn.getAttribute('data-admin-delete') === 'true'
          });
        }
      });
      ...
      feed.addEventListener('submit', async (event) => {
        ...
        if (editForm) {
          ...
          await this.editSocialPostComment(postId, commentId, String(input?.value || ''), entry, {
            adminEdit: editForm.dataset.adminEdit === 'true'
          });
          return;
        }
    ...
    async submitSocialPostComment(postId, { text, parentCommentId = '', form, entry, isReply = false } = {}) {
      ...
      // Guard double-taps: moderation can take a beat, and Post stayed clickable.
      if (form?.dataset?.submitting === '1') return;
      const submitBtn = form?.querySelector(
        isReply ? '.social-post-reply-submit' : '.social-post-comment-submit'
      );
      const input = form?.querySelector(
        isReply ? '.social-post-reply-input' : '.social-post-comment-input'
      );
      ...
      try {
        const res = await fetch(`${baseUrl}/api/social-posts/${encodeURIComponent(postId)}/comments`, {
          method: 'POST',
          headers: await this.buildSocialPostCommentRequestHeaders(),
          body: JSON.stringify({
            text: trimmed,
            parentCommentId: parentCommentId || undefined,
            clientId: UtilsCore.getOrCreateUserId(),
            displayName: this.getSocialPostCommentDisplayName()
          })
        });
        const data = await res.json().catch(() => ({}));
        if (!res.ok || !data.success) {
        ...
        if (submitBtn) {
          ...
          submitBtn.textContent = submitBtn.dataset.idleLabel || (isReply ? 'Post reply' : 'Post');
    ...
    async editSocialPostComment(postId, commentId, text, entry, { adminEdit = false } = {}) {
      const trimmed = String(text || '').replace(/\s+/g, ' ').trim();
      const baseUrl = this.getSocialPostApiBaseUrl();
      if (!trimmed) {
        this.uiService.showToast('Write a comment first');
      ...
      try {
        ...
        if (adminEdit) {
          const token = await this.getSocialPostAdminToken();
          if (!token) {
            this.uiService.showToast('Edit cancelled: missing admin token');
            return;
          }
          Object.assign(headers, this.buildSocialPostAdminHeaders(token));
        }
        const res = await fetch(
    ...
    async deleteSocialPostComment(postId, commentId, entry, { adminDelete = false } = {}) {
      const baseUrl = this.getSocialPostApiBaseUrl();
      ...
      try {
        ...
        if (adminDelete) {
          const token = await this.getSocialPostAdminToken();
          if (!token) {
            this.uiService.showToast('Delete cancelled: missing admin token');
            return;
          }
          Object.assign(headers, this.buildSocialPostAdminHeaders(token));
        }
    ...
    async reloadSocialPostsFeed() {
      ...
      try {
        ...
        this._socialPostsFeedHasMore = this.resolveSocialPostsFeedHasMore(page);
        if (this.canManageSocialPosts()) {
          await this.refreshSocialPostsNewCommentsDigest(this._socialPostsFeedPosts);
        } else {
          this.clearSocialPostsNewCommentsDigest();
        }
        this.renderSocialPostFeedEntries(this._socialPostsFeedPosts);
        ...
        this.markSocialPostsFeedSeen(this._socialPostsFeedPosts);
        this.syncSocialPostsLoadMoreButton();
      } catch (error) {
    ...
    ensureSocialPostImageZoomOverlay() {
      const existing = document.getElementById('socialPostImageZoom');
      if (existing && existing.dataset.zoomUiVersion !== '3') {
        existing.remove();
      }
      ...
      overlay.dataset.zoomUiVersion = '3';
      overlay.hidden = true;
      overlay.innerHTML = `
    ...
    _socialPostImageZoomCloseButtonRect(closeBtn, pad = 10) {
    ...
    _socialPostImageZoomPointInRect(x, y, rect) {
    ...
    _bindSocialPostImageZoomOverlay(overlay) {
      ...
      const close = () => this.closeSocialPostImageZoom();
      ...
      const tryCloseFromCloseButtonPointer = (event) => this._tryCloseSocialPostImageZoomFromCloseButton(event);
      ...
      overlay.addEventListener('click', (event) => {
        if (tryCloseFromCloseButtonPointer(event)) return;
        if (event.target === overlay) close();
      });

      if (!this._socialPostImageZoomClosePointerBound) {
        this._socialPostImageZoomClosePointerBound = true;
        document.addEventListener(
          'touchend',
          (event) => {
            this._tryCloseSocialPostImageZoomFromCloseButton(event);
          },
    ...
    _tryCloseSocialPostImageZoomFromCloseButton(event) {
      const overlay = document.getElementById('socialPostImageZoom');
      ...
      if (x == null || y == null || !closeBtn) return false;
      const rect = this._socialPostImageZoomCloseButtonRect(closeBtn);
      if (!this._socialPostImageZoomPointInRect(x, y, rect)) return false;
      if (event.cancelable) event.preventDefault();
      event.stopPropagation();
      this.closeSocialPostImageZoom();
      return true;
    }
    ...
    bindSocialPostsFeedControls() {
      ...
      loadMoreBtn?.addEventListener('click', async () => {
        ...
        try {
          ...
          this._socialPostsFeedHasMore = nextPosts.length
            ? this.resolveSocialPostsFeedHasMore(page)
            : false;
          this.renderSocialPostFeedEntries(this._socialPostsFeedPosts);
        } catch (error) {
