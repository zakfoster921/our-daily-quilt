...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Social {
    ...
    /** Same cream torn-end tape as `.quilt-before-you-go__slab`. */
    ...
    async loadSocialPostStoryHeaderTapeImage() {
      ...
      return new Promise((resolve, reject) => {
        ...
        im.onerror = () => reject(new Error('Failed to load before-you-go header tape'));
        im.src = url;
      });
    }
    ...
    drawSocialPostStoryHeaderTape(ctx, tapeImage, layout) {
      if (!layout) return null;
      const { slabX, slabY, slabW, slabH } = layout;
      const rotateDeg = -1.15;
      const fill = '#f2eee6';
      const capFrac = 0.13;
      const capW = Math.max(24, Math.round(slabW * capFrac));
      ...
      if (tapeImage) {
        ctx.drawImage(tapeImage, -slabW / 2, -slabH / 2, capW, slabH);
        ctx.drawImage(tapeImage, slabW / 2 - capW, -slabH / 2, capW, slabH);
      }
      ctx.restore();
    ...
    drawSocialPostStoryHeader(ctx, { padX, textMaxW, padTop = 192, tapeImage = null, storyW = 1080 } = {}) {
      ...
      this.drawSocialPostStoryHeaderTape(ctx, tapeImage, layout);
