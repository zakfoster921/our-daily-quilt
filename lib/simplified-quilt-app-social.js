...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Social {
    ...
    async createSocialPostStoryShareBlob(imageBlob, { caption = '', dateLabel = '' } = {}) {
      ...
      const maxImageH = Math.round(STORY_H * 0.58);
      ...
      const ctaBlockH = ctaLineH;
      const footerTop = STORY_H - padBottom - ctaBlockH;
      ...
      let dateSize = 38;
      let captionSize = 38;
      let dateLineH = dateSize * 1.35;
      let captionLineH = captionSize * 1.45;
      let dateLines = [];
      let captionLines = [];
      ...
      for (let attempt = 0; attempt < 24; attempt++) {
        ...
        captionLines = [];
        if (safeDate) {
          ctx.font = `700 ${dateSize}px ${FONT}`;
          dateLines = this.wrapSocialPostStoryLines(ctx, `${safeDate}:`, textMaxW);
        }
        if (safeCaption) {
          ctx.font = `400 ${captionSize}px ${FONT}`;
          captionLines = this.wrapSocialPostStoryLines(ctx, safeCaption, textMaxW);
        } else if (!safeDate) {
          captionLines = [''];
        }
        const totalTextH =
          (dateLines.length ? dateLines.length * dateLineH : 0) +
          (safeCaption && dateLines.length ? 8 : 0) +
          captionLines.length * captionLineH;
        if (totalTextH <= textAreaH || (dateSize <= 24 && captionSize <= 24)) break;
        if (captionSize > dateSize && captionSize > 24) captionSize -= 2;
        else if (dateSize > 24) dateSize -= 2;
        else break;
        dateLineH = dateSize * 1.35;
        captionLineH = captionSize * 1.45;
      }
