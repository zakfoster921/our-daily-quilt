...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Social {
        ...
        if (this.isSocialPostVideoMedia(item)) {
          const videoClass =
            classPrefix === 'social-post'
              ? `${classPrefix}-media ${classPrefix}-media--video`
              : `${classPrefix}-media`;
          const inlineAttr = this._socialPostVideoPreferFullscreen() ? '' : ' playsinline';
          return `<video class="${videoClass}" src="${safeUrl}" controls${inlineAttr} preload="metadata"></video>`;
        }
    ...
    _socialPostVideoPreferFullscreen() {
      return /iPhone|iPad|iPod/i.test(String(navigator.userAgent || ''));
    }

    bindSocialPostVideoElement(video) {
      ...
      video.addEventListener('play', ensureAudible);
      video.addEventListener('playing', ensureAudible);
      video.addEventListener('volumechange', ensureAudible);
      video.addEventListener('click', ensureAudible);
    }

    bindSocialPostFeedVideos(rootEl) {
      const scope = rootEl || document.getElementById('socialPostsFeed');
      if (!scope) return;
      scope.querySelectorAll('video.social-post-media--video, video.social-post-media').forEach((video) => {
        this.bindSocialPostVideoElement(video);
      });
    }
