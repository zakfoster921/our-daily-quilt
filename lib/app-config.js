...
const CONFIG = {
  APP: {
    ...
    /** Feature-feedback scroll nudge stops offering itself after this date (time-boxed collection push). */
    featureFeedbackNudgeSunsetIso: '2026-08-01T00:00:00',
    /** Delay before color picker pane reveals on quote screen (ms). */
    quoteColorPickerRevealMs: 1500,
    /**
     * Max wait for quote Layout B/clipping before revealing the color picker anyway (ms).
     * Prevents a stuck quote screen when pin/paint never marks ready (seen on Android).
     */
    quoteColorPickerLayoutBMaxWaitMs: 5000,
    ...
    /**
     * Post-color cut reveal (mat + scrap cuts) between picker and quilt.
     * Off in production — lives in post-color-lab.html + lib/post-color-cut-reveal.js.
     * To try again in-app: set true, add <script src="lib/post-color-cut-reveal.js"> in
     * our-daily-beta.html, or use ?postColorCutReveal=1 with that script loaded.
     */
    postColorCutRevealEnabled: false,
    /** IG slide 1 speaker: opaque colored paper on carousel slide 1 (tune Post/Feed + nightly). ?igSpeakerOpaque=0 to disable. */
    igSpeakerOpaqueCollage: false,
    /** Split-band quilt fit: primary width-fit + mirror fills lower band. ?splitBand=1/0 overrides. ?splitBandSimple=1 forces single mirror for QA (ignores dup×2 tune). */
    quiltSplitBandFit: true,
    /** Mood quote spread: 'collage' (scratch strips) or 'triptych' (flip cards). Override: ?moodSpread=triptych */
    moodSpreadMode: 'collage',
    /** Footer icon bar: bookmark_heart reminders shortcut. Temporary off — re-enable when ready. */
    footerRemindersIconEnabled: false,
  ...
  COLOR_PICKER: {
    wheelSize: 280,
    /** Initial slider positions: hue/value right, saturation ~10%; background stays neutral until a slide. */
    defaultHue: 360,
    defaultSaturation: 10,
    defaultLightness: 90,
    /** Pastel first-swatch hint only; hue bar uses hueBarSaturation. */
    saturation: 21.7,
    /** Hue slider rainbow + saturation applied when hue moves before user touches sat. */
    hueBarSaturation: 50
  },
  QUILT: {
    viewBoxWidth: 800,
  ...
  function odqProxyImageBases() {
    ...
    try {
      if (typeof location !== 'undefined' && location.origin) {
        ...
        // Native apps (Capacitor) serve the bundle from capacitor://localhost, which
        // resolves like a real host but isn't a network server — only real http(s)
        // dev servers (e.g. `npm start` on :3000) should qualify here.
        const isHttpProtocol = protocol === 'http:' || protocol === 'https:';
        if (isHttpProtocol && (host === 'localhost' || host === '127.0.0.1')) {
