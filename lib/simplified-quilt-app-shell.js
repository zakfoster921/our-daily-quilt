      ...
      handleWindowResize() {
        ...
        this.resizeTimeout = setTimeout(() => {
          if (this.quiltEngine.blocks && this.quiltEngine.blocks.length > 0) {
            ...
            const rect = quiltContainer?.getBoundingClientRect?.();
            const cw = rect?.width || 0;
            const ch = rect?.height || 0;
            const last = this._quiltMirrorViewportSize || { w: 0, h: 0 };
            const containerStable =
              cw > 0 &&
              ch > 0 &&
              Math.abs(cw - last.w) < 2 &&
              Math.abs(ch - last.h) < 2;
            if (!containerStable) {
              this._quiltMirrorViewportSize = { w: cw, h: ch };
          ...
          if (quiltScreen && quiltScreen.classList.contains('active')) {
            ...
            const contributorStage = document.getElementById('quiltContributors');
            const contributorList = document.getElementById('quiltContributorList');
            if (contributorList && contributorStage && !contributorStage.hidden) {
              this.fitContributorCloudToViewport?.(contributorList);
            }
          }
        }, 250); // 250ms debounce
      }
