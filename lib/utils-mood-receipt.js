...
(function (root) {
  ...
  Object.assign(UtilsCore, {
    ...
    /** Cool neutral chip stock — dense cardstock, barely tinted. */
    colorCardstockPaperFromUserColor(hex) {
      const neutral = '#eef1f4';
      const raw = String(hex || '').trim();
      if (!UtilsCore.validateHexColor(raw)) return neutral;
      const hsv = UtilsCore.hexToHsv(raw);
      const hintSat = hsv.s < 4 ? 3 : Math.min(7, Math.round(hsv.s * 0.05));
      const hint = UtilsCore.hsvToHex(hsv.h, hintSat, 96);
      const mix = 0.05;
      const parse = (h) => {
        const n = parseInt(String(h).replace('#', ''), 16);
        return { r: (n >> 16) & 255, g: (n >> 8) & 255, b: n & 255 };
      ...
      const a = parse(hint);
      const b = parse(neutral);
      const ch = (k) =>
        Math.round(a[k] * mix + b[k] * (1 - mix))
          .toString(16)
          .padStart(2, '0');
      return '#' + ch('r') + ch('g') + ch('b');
    },
    ...
    moodReceiptRuleLine(containerEl) {
      ...
      const sample =
        containerEl.querySelector('.quilt-mood-widget__paper-text') ||
        containerEl;
    ...
    /** Portal footer label from marketing version (e.g. v2.1). Build is ignored for display. */
    formatPortalVersionLabel(version, _build) {
      const versionStr = String(version || '').trim();
      if (!versionStr) return '';
      const parts = versionStr.split('.').filter(Boolean);
      if (parts.length >= 1) return `v${parts.join('.')}`;
      return '';
    },
    refreshPortalVersionLabel() {
      const labels = [
        { el: root.document.getElementById('portal-version-label'), prefix: '' },
        { el: root.document.getElementById('settings-version-label'), prefix: 'You are running ' }
      ].filter((item) => item.el);
      if (!labels.length || typeof root.CONFIG === 'undefined' || !root.CONFIG.APP) return;
    
      const fallbackVersion = root.CONFIG.APP.version || '?';
      const fallbackBuild = root.CONFIG.APP.buildId || 'dev';
      const setLabels = (version, build) => {
        const displayVersion = UtilsCore.formatPortalVersionLabel(version, build);
        labels.forEach(({ el, prefix }) => {
          el.textContent = displayVersion ? `${prefix}${displayVersion}` : '';
        });
      };
      setLabels(fallbackVersion, fallbackBuild);
      ...
      appPlugin.getInfo()
        .then((info) => {
          setLabels(info?.version || fallbackVersion, info?.build || fallbackBuild);
        })
        .catch(() => {});
    }
  });
