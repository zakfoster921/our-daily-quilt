...
function (global) {
  ...
  const STYLE_ID = 'quilt-mood-triptych-widget-styles-v113';
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      #screen-quilt .quilt-mood-triptych__card.is-dismissed {
        position: absolute;
        top: 0;
        width: var(--triptych-card-w, calc((100% - var(--triptych-mood-gap)) / 2));
  ...
  function mount(host, opts = {}) {
    ...
    host.classList.remove('is-ready', 'is-good', 'is-rough', 'is-open', 'is-dismiss-sliding', 'is-instant', 'is-locked');
    ...
    function syncTypeWindow(faceEl, contentEl) {
      ...
      const gutterXStart = resolveTriptychLengthPx(
        isBackMessage
          ? '--triptych-halftone-clear-gutter-back-x'
          : '--triptych-halftone-clear-gutter-x',
        isBackMessage ? 5 : 12
      );
      const gutterXEnd = resolveTriptychLengthPx(
        isBackMessage
    ...
    function resetMood({ instant = false } = {}) {
      ...
      host.classList.remove('is-good', 'is-rough', 'is-open', 'is-dismiss-sliding', 'is-locked');
  ...
  function unmount(host) {
    ...
    host.setAttribute('hidden', 'hidden');
    host.setAttribute('aria-hidden', 'true');
    host.classList.remove('is-good', 'is-rough', 'is-open', 'is-ready', 'is-dismiss-sliding', 'is-instant', 'is-locked');
