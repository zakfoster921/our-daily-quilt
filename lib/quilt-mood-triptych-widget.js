...
function (global) {
  ...
  const STYLE_ID = 'quilt-mood-triptych-widget-styles-v44';
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      /* Steady open state — not during dismiss slide or unfold handoff (see rules above). */
      #screen-quilt .quilt-mood-triptych.is-open:not(.is-dismiss-sliding):not(.is-opening) .quilt-mood-triptych__card.is-chosen .quilt-mood-triptych__stack {
        opacity: 0;
        visibility: hidden;
        pointer-events: none;
        z-index: 1;
  ...
  function mount(host, opts = {}) {
    ...
    function moodUiSettled(next) {
      ...
      if (host.classList.contains('is-open')) return true;
      const sibling = next === 'good' ? cardRough.btn : cardGood.btn;
      return (
        sibling.classList.contains('is-dismissed')
        || sibling.classList.contains('is-dismissed-gone')
      );
    }
    ...
    function applyMoodClasses() {
      ...
      const hasMood = mood === 'good' || mood === 'rough';
      const inDismissOrUnfold =
        host.classList.contains('is-dismiss-sliding')
        || host.classList.contains('is-opening');
      const fullyOpen = hasMood && (
        !ENABLE_OPEN_ANIMATION
        || host.classList.contains('is-instant')
        || !inDismissOrUnfold
      );
    ...
    function syncSheetMetrics() {
      ...
      if (rowW < 48) return;
      const nextH = `${Math.round(rowW / 2)}px`;
