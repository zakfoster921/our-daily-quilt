...
function (global) {
  ...
  const STYLE_ID = 'quilt-mood-triptych-widget-styles-v166';
  const TRIPTYCH_BUILD = 'v166';
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      #screen-quilt .quilt-mood-triptych.is-flipping
        .quilt-mood-triptych__card.is-chosen
        .quilt-mood-triptych__face--back
        .quilt-mood-triptych__type-window,
  ...
  function mount(host, opts = {}) {
    ...
    let lockedChosenBackRect = null;
    let lockedChosenFrontRect = null;
    ...
    function clearLockedChosenTypeRects() {
      lockedChosenBackRect = null;
      lockedChosenFrontRect = null;
    }

    function clearPickVisualState() {
      clearActiveFlipAnimation();
      clearLockedChosenTypeRects();
    ...
    function cellHeightForCard(cardBtn, cardWidthPx, messageText) {
      ...
      const typeWindowPadY =
        2 *
        resolveTriptychLengthPx('--triptych-halftone-clear-gutter-back-y', 1);
      return Math.ceil(pad.top + pad.bottom + 2 * pad.border + articleH + typeWindowPadY + 4);
    }
    ...
    function syncCardTypeWindows(faceFront, faceBack, frontMark, messageEl, cardBtn, { isChosenCard = false } = {}) {
      ...
      if (unifyForDesktopFlip) {
        const backRect = lockedChosenBackRect || measureTypeWindowRect(faceBack, messageEl);
        const unified =
          lockedChosenFrontRect || unionTypeWindowRects(frontRect, backRect);
    ...
    /** Size back wash to message ink before flip so copy never sits on halftone dots. */
    function primeChosenTypeWindows(chosen) {
      ...
      const frontWin = faceFront?.querySelector('.quilt-mood-triptych__type-window');
      const backWin = faceBack?.querySelector('.quilt-mood-triptych__type-window');
      if (!backWin || !faceBack) return;

      const frontRect = measureTypeWindowRect(faceFront, frontMark);
      const backRect = measureTypeWindowRect(faceBack, chosen.messageEl);
      if (!backRect) return;

      lockedChosenBackRect = backRect;
      lockedChosenFrontRect = frontRect
        ? unionTypeWindowRects(frontRect, backRect)
        : backRect;

      if (frontWin && lockedChosenFrontRect) {
        applyTypeWindowRect(frontWin, lockedChosenFrontRect);
      }
      applyTypeWindowRect(backWin, backRect);
      backWin.style.removeProperty('visibility');
    }
