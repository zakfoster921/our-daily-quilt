...
function (global) {
  ...
  const STYLE_ID = 'quilt-mood-triptych-widget-styles-v17';
  const ENABLE_OPEN_ANIMATION = false;
  const DISMISS_MS = 720;
  const UNFOLD_DELAY_MS = 260;
  const UNFOLD_MS = 420;
  const REDUCED_DISMISS_MS = 320;
  ...
  function injectStyles() {
    ...
    style.textContent = `
      #screen-quilt .quilt-mood-triptych {
        ...
        --triptych-closed-front-y: 11deg;
        --triptych-closed-back-y: -15deg;
        --triptych-open-tilt-left: 2.5deg;
        --triptych-open-tilt-right: -2.5deg;
        --triptych-card-tilt-good: -2deg;
        --triptych-card-tilt-rough: 2deg;
        ...
        --triptych-unfold-delay-ms: ${UNFOLD_DELAY_MS}ms;
        --triptych-unfold-ms: ${UNFOLD_MS}ms;
        --triptych-dismiss-ease: cubic-bezier(0.22, 0.68, 0.36, 1);
        ...
        --triptych-font: var(--quilt-courier-font, 'Courier Prime', 'Courier New', Courier, monospace);
      ...
      #screen-quilt .quilt-mood-triptych.is-dismissing .quilt-mood-triptych__row,
      ...
      #screen-quilt .quilt-mood-triptych:not(.is-open):not(.is-opening):not(.is-dismissing) .quilt-mood-triptych__card--good .quilt-mood-triptych__stack {
        transform: rotate(var(--triptych-card-tilt-good));
      }

      #screen-quilt .quilt-mood-triptych:not(.is-open):not(.is-opening):not(.is-dismissing) .quilt-mood-triptych__card--rough .quilt-mood-triptych__stack {
      ...
      #screen-quilt .quilt-mood-triptych__back-leaf {
        ...
        transform: rotateY(var(--triptych-closed-back-y));
      ...
      #screen-quilt .quilt-mood-triptych.is-dismissing .quilt-mood-triptych__card.is-chosen,
      ...
      #screen-quilt .quilt-mood-triptych.is-dismissing .quilt-mood-triptych__card.is-dismissed,
      #screen-quilt .quilt-mood-triptych.is-opening .quilt-mood-triptych__card.is-dismissed,
      #screen-quilt .quilt-mood-triptych.is-good .quilt-mood-triptych__card.is-dismissed,
      #screen-quilt .quilt-mood-triptych.is-rough .quilt-mood-triptych__card.is-dismissed {
        position: absolute;
        top: 0;
      ...
      #screen-quilt .quilt-mood-triptych.is-dismissing .quilt-mood-triptych__card--good.is-dismissed,
      #screen-quilt .quilt-mood-triptych.is-opening .quilt-mood-triptych__card--good.is-dismissed,
      #screen-quilt .quilt-mood-triptych.is-good .quilt-mood-triptych__card--good.is-dismissed,
      #screen-quilt .quilt-mood-triptych.is-rough .quilt-mood-triptych__card--good.is-dismissed {
        left: 0;
        transform: translateX(calc(-100% - clamp(1.5rem, 12vw, 4rem)));
      }

      #screen-quilt .quilt-mood-triptych.is-dismissing .quilt-mood-triptych__card--rough.is-dismissed,
      #screen-quilt .quilt-mood-triptych.is-opening .quilt-mood-triptych__card--rough.is-dismissed,
      #screen-quilt .quilt-mood-triptych.is-good .quilt-mood-triptych__card--rough.is-dismissed,
      #screen-quilt .quilt-mood-triptych.is-rough .quilt-mood-triptych__card--rough.is-dismissed {
        right: 0;
        left: auto;
        transform: translateX(calc(100% + clamp(1.5rem, 12vw, 4rem)));
      }
      ...
      #screen-quilt .quilt-mood-triptych.is-open .quilt-mood-triptych__card.is-chosen {
        ...
        aspect-ratio: 2 / 1;
        height: auto;
        min-height: var(--triptych-cell-h, 10rem);
        max-height: none;
        overflow: hidden;
      ...
      #screen-quilt .quilt-mood-triptych__half {
        ...
        transform-style: preserve-3d;
        -webkit-transform-style: preserve-3d;
        transition: transform var(--triptych-unfold-ms) cubic-bezier(0.33, 1, 0.58, 1);
      }
  ...
  function buildCard(mood, message, iconName) {
    ...
    frontLeaf.appendChild(frontIcon);
  ...
  function applyHandCutClips(cards, dateKey) {
    ...
    cards.forEach((card) => {
      ...
      const openCard =
        ...
        (host?.classList.contains('is-open') || host?.classList.contains('is-opening') || host?.classList.contains('is-dismissing'));
      if (!openCard) {
        fold.style.clipPath = '';
  ...
  function mount(host, opts = {}) {
    ...
    host.classList.remove('is-ready', 'is-good', 'is-rough', 'is-open', 'is-opening', 'is-dismissing', 'is-instant');
    ...
    function applyMoodClasses() {
      ...
      const fullyOpen = ENABLE_OPEN_ANIMATION
        ...
        && !host.classList.contains('is-dismissing');
    ...
    function openMood(next, { instant = false, fromUser = false } = {}) {
      ...
      const unfoldDelay = reducedMotion ? 0 : UNFOLD_DELAY_MS;
      const totalMs = reducedMotion
        ? REDUCED_UNFOLD_MS
        : Math.max(DISMISS_MS, UNFOLD_DELAY_MS + UNFOLD_MS);
    ...
    function resetMood({ instant = false } = {}) {
      ...
      host.classList.remove('is-good', 'is-rough', 'is-open', 'is-opening', 'is-dismissing');
  ...
  function unmount(host) {
    ...
    host.setAttribute('hidden', 'hidden');
    host.setAttribute('aria-hidden', 'true');
    host.classList.remove('is-good', 'is-rough', 'is-open', 'is-ready', 'is-opening', 'is-dismissing', 'is-instant');
