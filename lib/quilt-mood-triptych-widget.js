...
function (global) {
  ...
  const STYLE_ID = 'quilt-mood-triptych-widget-styles-v152';
  const TRIPTYCH_BUILD = 'v152';
  ...
  /** Map nightly/client PNG pixels to triptych display cap (~68% sheet — smaller than mood row). */
  function applyTriptychQuoteDisplaySize(quoteImg, quoteWrap, opts = {}) {
      ...
      if (nw > 0 && nh > 0) {
        ...
        const layout = QNC?.resolveQuoteClippingDisplayLayout?.(
          ...
          QNC.resolveTriptychClippingLayoutOpts?.() || {
            displayScale: Number(QNC.TRIPTYCH_QUOTE_DISPLAY_SCALE) || 0.551
          }
        );
    ...
    const fallbackScale = Number(global.QuiltNewspaperClipping?.TRIPTYCH_QUOTE_DISPLAY_SCALE) || 0.551;
  ...
  function injectStyles() {
    ...
    style.textContent = `
      #screen-quilt .quilt-mood-triptych {
        ...
        --triptych-mood-gap: clamp(0.85rem, 4.8vw, 1.45rem);
      ...
      /* Dot-free riso wash behind type/icon — sized by syncTypeWindow to hug ink + gutter. */
      ...
      #screen-quilt .quilt-mood-triptych__face--back .quilt-mood-triptych__message {
        padding: clamp(0.1rem, 0.9vw, 0.22rem)
  ...
  function mount(host, opts = {}) {
    ...
    /** Face-local back type-window rect — frozen at flip prime so remeasure cannot resize it. */
    let lockedChosenBackTypeRect = null;

    // #region agent log
    function debugTriptychLog(phase, hypothesisId, data) {
      try {
        console.info('[triptych-debug]', phase, hypothesisId, {
          triptychBuild: TRIPTYCH_BUILD,
          ...data
        });
      } catch (_) {
        /* */
      ...
      fetch('http://127.0.0.1:7433/ingest/0ed8adaa-5aed-4571-811f-aadcc7a8fddc', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-Debug-Session-Id': '6d64f7' },
        body: JSON.stringify({
          sessionId: '6d64f7',
          runId: 'pre-fix',
          hypothesisId,
          location: 'quilt-mood-triptych-widget.js:debugTriptychLog',
          message: phase,
          data,
          timestamp: Date.now()
        })
    ...
    function sampleTypeWindow(win, label) {
      if (!win) return { label, missing: true };
    ...
    function sampleFlipStack(chosen) {
      const btn = chosen.btn;
      const card3d = btn.querySelector('.quilt-mood-triptych__card-3d');
      ...
      const stackHost = btn.closest('.quilt-mood-triptych');
      ...
      return {
        hostClasses: stackHost ? [...stackHost.classList] : [],
        btnClasses: [...btn.classList],
        stack: [
          sample(stackHost, 'host'),
          sample(btn, 'btn'),
          sample(card3d, 'card3d'),
          sample(flip, 'flip'),
          sample(faceFront, 'faceFront'),
          sample(faceBack, 'faceBack')
        ]
      };
    }
    // #endregion
    ...
    function clearLockedChosenBackTypeRect() {
      lockedChosenBackTypeRect = null;
    }

    function clearPickVisualState() {
      clearActiveFlipAnimation();
      clearLockedChosenBackTypeRect();
    ...
    function readFaceArticleBox(cardBtn, faceEl) {
      const cs = getComputedStyle(cardBtn);
      const faceR = faceEl.getBoundingClientRect();
      return {
        top: parseFloat(cs.getPropertyValue('--triptych-article-inset-top')) || 18,
        right: parseFloat(cs.getPropertyValue('--triptych-article-inset-right')) || 18,
        bottom: parseFloat(cs.getPropertyValue('--triptych-article-inset-bottom')) || 18,
        left: parseFloat(cs.getPropertyValue('--triptych-article-inset-left')) || 18,
        faceW: faceR.width,
        faceH: faceR.height
      };
    }

    function measureBackTypeWindowRectFromProbe(faceBack, messageEl, cardBtn) {
      if (!faceBack || !messageEl || !cardBtn) return null;
      const live = measureTypeWindowRect(faceBack, messageEl);
      if (live && live.width >= 8 && live.height >= 8) return live;

      const box = readFaceArticleBox(cardBtn, faceBack);
      const articleW = Math.max(48, box.faceW - box.left - box.right);
      const gutters = readTypeWindowGutters(messageEl);
      messageMeasure.textContent = messageEl.textContent || '';
      messageMeasure.style.width = `${Math.round(articleW)}px`;
      const ink = measureMessageInkRect(messageMeasure);
      if (!ink || ink.width < 1) return live;

      const blockH = ink.height + 2 * gutters.gutterY;
      const blockW = ink.width + gutters.gutterXStart + gutters.gutterXEnd;
      const innerH = Math.max(0, box.faceH - box.top - box.bottom);
      const innerW = Math.max(0, articleW);
      return {
        top: box.top + Math.max(0, (innerH - blockH) * 0.5),
        left: box.left + Math.max(0, (innerW - blockW) * 0.5),
        width: Math.min(blockW, innerW),
        height: Math.min(blockH, innerH)
      };
    }
    ...
    function syncCardTypeWindows(faceFront, faceBack, frontMark, messageEl, cardBtn, { isChosenCard = false } = {}) {
      ...
      let backRect = null;
      let backRectSource = 'measured';
      if (isChosenCard && lockedChosenBackTypeRect) {
        backRect = lockedChosenBackTypeRect;
        backRectSource = 'locked';
      } else {
        backRect = measureTypeWindowRect(faceBack, messageEl);
        const needsBackProbe =
          cardBtn &&
          messageEl &&
          (!backRect || backRect.width < 8 || backRect.height < 8);
        if (needsBackProbe) {
          backRect =
            measureBackTypeWindowRectFromProbe(faceBack, messageEl, cardBtn) || backRect;
          backRectSource = 'probe';
        }
      }
      ...
      if (unifyForDesktopFlip) {
        ...
        applyTypeWindowRect(backWin, unified);
        return;
      }

      applyTypeWindowRect(frontWin, frontRect);
      applyTypeWindowRect(backWin, backRect);

      if (backWin && isChosenCard) {
        backWin.style.removeProperty('visibility');
      }

      if (isChosenCard && host.classList.contains('is-open')) {
        debugTriptychLog('syncCardTypeWindows:open', 'H6', {
          useIos2dFlip,
          backRectSource,
          backRect,
          frontRect,
          backWin: sampleTypeWindow(backWin, 'backWin'),
          messageBox: messageEl
            ? {
                w: Math.round(messageEl.getBoundingClientRect().width),
                h: Math.round(messageEl.getBoundingClientRect().height)
              }
            : null
        });
      }
    ...
    function syncSheetMetrics() {
      ...
      const rowInnerW = Math.max(96, Math.min(hostW, row.getBoundingClientRect().width) - padInline);
      ...
      let wEqual =
        hardMin <= maxCardW
          ...
          : maxCardW;
      const maxFitW = Math.floor(available / 2);
      const maxHostW = Math.max(hardMin, Math.floor((hostW - gap - padInline) / 2));
      wEqual = Math.min(
    ...
    /** Probe-size back dot-window before/during flip so it can appear with the message, not after. */
    function primeChosenBackTypeWindow(chosen) {
      const faceBack = chosen.flip.querySelector('.quilt-mood-triptych__face--back');
      const backWin = faceBack?.querySelector('.quilt-mood-triptych__type-window');
      if (!backWin || !chosen.messageEl) return null;
      const rect =
        measureBackTypeWindowRectFromProbe(faceBack, chosen.messageEl, chosen.btn) ||
        measureTypeWindowRect(faceBack, chosen.messageEl);
      if (rect && rect.width >= 8 && rect.height >= 8) {
        lockedChosenBackTypeRect = {
          top: rect.top,
          left: rect.left,
          width: rect.width,
          height: rect.height
      ...
      applyTypeWindowRect(backWin, lockedChosenBackTypeRect || rect);
      backWin.style.removeProperty('visibility');
      debugTriptychLog('primeChosenBackTypeWindow', 'H9', {
        lockedChosenBackTypeRect,
        backWin: sampleTypeWindow(backWin, 'backWin')
      });
      return lockedChosenBackTypeRect || rect;
    }
    ...
    function finishPick(fromUser) {
      ...
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          ...
          debugTriptychLog('finishPick:type-window', 'H7', {
            useIos2dFlip,
            triptychBuild: TRIPTYCH_BUILD,
            backWin: sampleTypeWindow(backWin, 'backWin'),
            faceBackDisplay: getComputedStyle(
              chosen.flip.querySelector('.quilt-mood-triptych__face--back') || chosen.flip
            ).display
          });
        });
      });
    ...
    function openMood(next, { instant = false, fromUser = false } = {}) {
      ...
      // #region agent log
      debugTriptychLog('openMood:start', 'H2', {
        next,
        fromUser,
        reducedMotion,
        instant,
        ...
        triptychBuild: TRIPTYCH_BUILD,
        styleId: STYLE_ID,
        hasCard3d: !!chosen.btn.querySelector('.quilt-mood-triptych__card-3d'),
        hasCardTilt: !!chosen.btn.querySelector('.quilt-mood-triptych__card-tilt')
      });
      // #endregion
      ...
      const primedBackRect = primeChosenBackTypeWindow(chosen);
      debugTriptychLog('openMood:back-primed', 'H8', {
        primedBackRect,
        backWin: sampleTypeWindow(
          chosen.flip
            .querySelector('.quilt-mood-triptych__face--back')
            ?.querySelector('.quilt-mood-triptych__type-window'),
          'backWin'
      ...
      // #region agent log
      debugTriptychLog('openMood:after-is-chosen', 'H1', sampleFlipStack(chosen));
      // #endregion
      ...
      function armFlipTransition() {
        ...
        function onFlipTransitionEnd(e) {
          ...
          // #region agent log
          debugTriptychLog('openMood:transitionend', 'H5', {
            propertyName: e.propertyName,
            elapsedTime: e.elapsedTime,
            pseudoElement: e.pseudoElement,
            ...sampleFlipStack(chosen)
          });
          // #endregion
          clearFlipTransitionListener();
          flipDone = true;
      ...
      function beginPickAnimation() {
        ...
        // #region agent log
        debugTriptychLog('openMood:pre-is-open', 'H3', { useIos2dFlip, ...sampleFlipStack(chosen) });
        // #endregion

        const runCssFlip = () => {
          requestAnimationFrame(() => {
            requestAnimationFrame(() => {
              const flipBeforeOpen = getComputedStyle(chosen.flip).transform;
              host.classList.add('is-open');
              applyMoodClasses();
              const flipRightAfterOpen = getComputedStyle(chosen.flip).transform;
              // #region agent log
              debugTriptychLog('openMood:is-open-applied', 'H3', {
                flipEngine: 'css',
                flipBeforeOpen,
                flipRightAfterOpen,
                sameTransform: flipBeforeOpen === flipRightAfterOpen,
                flipMs,
                ...sampleFlipStack(chosen)
              });
              // #endregion
              armFlipTransition();
              scheduleAnimation(() => {
                debugTriptychLog('openMood:mid-flip', 'H2', {
                  flipEngine: 'css',
                  atMs: Math.round(flipMs * 0.5),
                  ...sampleFlipStack(chosen)
                });
              }, Math.round(flipMs * 0.5));
            });
          });
        };
            ...
            requestAnimationFrame(() => {
              ...
              const revealIosBackHalfway = () => {
                ...
                const backWin = chosen.flip
                  .querySelector('.quilt-mood-triptych__face--back')
                  ?.querySelector('.quilt-mood-triptych__type-window');
                debugTriptychLog('openMood:ios-mid-reveal', 'H8', {
                  atMs: Math.round(iosFlipMs * 0.48),
                  backWin: sampleTypeWindow(backWin, 'backWin')
                });
              };
              ...
              const finishIosRotate = () => {
                ...
                applyMoodClasses();
                flipDone = true;
                debugTriptychLog('openMood:ios-rotate-finish', 'H12', {
                  iosFlipMs,
                  animationName: getComputedStyle(flipEl).animationName,
                  transform: getComputedStyle(flipEl).transform,
                  ...sampleFlipStack(chosen)
                });
                maybeFinishPick();
              };
              ...
              debugTriptychLog('openMood:ios-rotate-start', 'H12', {
                iosFlipMs,
                flipEngine: 'ios-rotate3d',
                triptychBuild: TRIPTYCH_BUILD,
                animationName: getComputedStyle(flipEl).animationName,
                ...sampleFlipStack(chosen)
              });
