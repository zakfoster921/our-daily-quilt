...
function (global) {
  ...
  const STYLE_ID = 'quilt-mood-triptych-widget-styles-v1103';
  ...
  function injectStyles() {
    ...
    style.textContent = `
      #screen-quilt .quilt-mood-triptych {
        ...
        --triptych-or-breathe: clamp(0.85rem, 3.2vw, 1.35rem);
  ...
  function mount(host, opts = {}) {
    ...
    function cellHeightForCard(cardBtn, cardWidthPx, messageText) {
      ...
      const cardLh = cardBtn.style.getPropertyValue('--triptych-copy-line-height').trim();
      if (cardLh) messageMeasure.style.lineHeight = cardLh;
      else messageMeasure.style.removeProperty('line-height');
      ...
      messageMeasure.style.removeProperty('line-height');
    ...
    function syncSheetMetrics() {
      ...
      // Tighten line-height per card when message wraps to 3+ lines.
      const BASE_LINE_HEIGHT = 1.2;
      const TIGHT_LINE_HEIGHT = 1.0;
      for (const [btn, text, fontPx] of [[goodBtn, goodDay, goodFontPx], [roughBtn, roughDay, roughFontPx]]) {
        applyMeasureFontSizePx(fontPx);
        const pad = readArticleInsets(btn);
        const articleW = Math.max(1, wEqual - pad.left - pad.right - 2 * pad.border);
        const lineH = fontPx * BASE_LINE_HEIGHT;
        const lines = lineH > 0 ? Math.round(measureMessageHeight(text, articleW) / lineH) : 1;
        const lh = lines >= 3 ? TIGHT_LINE_HEIGHT : BASE_LINE_HEIGHT;
btn.style.setProperty('--triptych-copy-line-height', String(lh));
        // Set directly on message elements in case CSS var cascade doesn't reach them.
        btn.querySelectorAll('.quilt-mood-triptych__message, .quilt-mood-triptych__bleed-imprint--message')
          .forEach(el => { el.style.lineHeight = String(lh); });
      }
      applyMeasureFontSizePx(Math.max(goodFontPx, roughFontPx));
      const wPx = `${wEqual}px`;
      if (SCRATCH_MODE) {
