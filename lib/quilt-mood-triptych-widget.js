...
function (global) {
  ...
  const STYLE_ID = 'quilt-mood-triptych-widget-styles-v119';
  ...
  function injectStyles() {
    ...
    style.textContent = `
      #screen-quilt .quilt-mood-triptych {
        ...
        --triptych-halftone-overlay-scale: 108%;
        --triptych-scale: 0.85;
        ...
        --triptych-halftone-clear-gutter-x: clamp(0.75rem, 6.5vw, 1.15rem);
        --triptych-halftone-clear-gutter-x-end: clamp(0.32rem, 2.8vw, 0.52rem);
        --triptych-halftone-clear-gutter-y: clamp(0.65rem, 5.5vw, 1rem);
        --triptych-halftone-clear-gutter-back-x: clamp(0.28rem, 2.4vw, 0.48rem);
        --triptych-halftone-clear-gutter-back-x-end: clamp(0.14rem, 1.2vw, 0.26rem);
        --triptych-halftone-clear-gutter-back-y: clamp(0.22rem, 2vw, 0.38rem);
        ...
        --triptych-card-shadow:
          drop-shadow(0 1px 0 rgba(255, 252, 246, 0.36))
          drop-shadow(0 3px 10px rgba(45, 36, 29, 0.11))
          drop-shadow(0 7px 16px rgba(26, 20, 16, 0.08));
        --triptych-card-face-highlight:
          inset 0 1px 0 rgba(255, 252, 246, 0.34),
          inset 0 -1px 0 rgba(26, 20, 16, 0.05);
        width: 100%;
        max-width: 100%;
        margin: 0 auto;
        padding: 0;
      ...
      #screen-quilt .quilt-mood-triptych__card--good {
        z-index: 2;
        --triptych-card-tilt: -2.4deg;
        --triptych-card-origin: 36% 92%;
        --triptych-card-transform: translate(clamp(-4px, -1vw, -1px), clamp(3px, 1.2vw, 8px))
          rotate(var(--triptych-card-tilt));
      }

      #screen-quilt .quilt-mood-triptych__card--rough {
        z-index: 1;
        --triptych-card-tilt: 2.8deg;
        --triptych-card-origin: 64% 90%;
        --triptych-card-transform: translate(clamp(1px, 1vw, 5px), clamp(-3px, -0.8vw, 2px))
          rotate(var(--triptych-card-tilt));
      }
      ...
      #screen-quilt .quilt-mood-triptych.is-flipping .quilt-mood-triptych__card.is-chosen {
        /* filter on an ancestor flattens preserve-3d — drop shadow during flip only. */
        filter: none;
      }
      ...
      #screen-quilt .quilt-mood-triptych__face {
        ...
        border: 0;
        border-radius: 0;
        box-shadow: var(--triptych-card-face-highlight);
        overflow: hidden;
      }
      ...
      /* Riso: light ink wash + halftone-masked dark ink (same hue). */
      ...
      #screen-quilt .quilt-mood-triptych__face::before {
        ...
        background-size: var(--triptych-halftone-overlay-scale, 108%)
          var(--triptych-halftone-overlay-scale, 108%);
  ...
  function mount(host, opts = {}) {
    ...
    const quoteIsRemote = /^https?:\/\//i.test(quoteDataUrl);
    if (quoteIsRemote) {
      quoteImg.style.width = '100%';
      quoteImg.style.maxWidth = '100%';
      quoteImg.style.height = 'auto';
    } else if (quoteDisplayWidthPx > 0) {
      const layoutW = `${Math.round(quoteDisplayWidthPx)}px`;
      quoteWrap.style.maxWidth = layoutW;
      quoteImg.style.width = layoutW;
      quoteImg.style.height = 'auto';
    }
    ...
    function syncTypeWindow(faceEl, contentEl) {
      const win = faceEl?.querySelector('.quilt-mood-triptych__type-window');
      if (!win || !contentEl || !faceEl) return;

      const isBackMessage = contentEl.classList.contains('quilt-mood-triptych__message');
      const gutterY = resolveTriptychLengthPx(
        isBackMessage
      ...
      let left = aLeft;
      let width = aRight - aLeft;
      const frame = faceEl.querySelector('.quilt-mood-triptych__border-frame');
      if (frame) {
        const fr = frame.getBoundingClientRect();
        const fcs = getComputedStyle(frame);
        const borderL = parseFloat(fcs.borderLeftWidth) || 0;
        const borderR = parseFloat(fcs.borderRightWidth) || 0;
        left = fr.left - faceRect.left + borderL;
        width = Math.max(0, fr.width - borderL - borderR);
      }

      /* Inner border edge to inner border edge; height hugs ink + vertical gutter. */
      ...
      const widthPx = `${Math.round(Math.max(0, width))}px`;
      const heightPx = `${Math.round(Math.max(0, height))}px`;
    ...
    function syncAllTypeWindows() {
      ...
      [cardGood, cardRough].forEach(({ btn, messageEl }) => {
        ...
        syncTypeWindow(faceFront, faceFront?.querySelector('.quilt-mood-triptych__front-mark'));
        syncTypeWindow(faceBack, messageEl);
      });
    }
