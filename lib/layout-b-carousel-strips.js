...
(function (global) {
  ...
  function odqApplyCarouselSpeakerStripLayout(plan, layoutW, layoutH, opts = {}) {
    ...
    const needsVerticalCompression = opts.carouselNeedsVerticalCompression === true;
    const phase = opts.phase === 'constraints' ? 'constraints' : 'full';
    const minGap = needsVerticalCompression ? ODQ_CAROUSEL_STRIP_TEXT_MIN_GAP : 12;
    ...
    if (needsVerticalCompression && quoteText && deps.odqCarouselSpeakerRestackQuoteStripsVertically) {
      const bounds = deps.odqLayoutBPostStripScatterBoundsForCarouselSpeaker(
        layoutW,
        layoutH,
        speakerRect
    ...
    if (needsVerticalCompression && deps.odqCarouselSpeakerEnforceReadingOrderStripGaps) {
