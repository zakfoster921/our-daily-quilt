...
function (global) {
  ...
  function minClippingDisplayWidthDomPx(opts = {}) {
    const bodyDom = clippingBodyDomPx(opts);
    const minBody = Number(opts.minBodyDomPx) > 0 ? Number(opts.minBodyDomPx) : 11;
  ...
  async function composeDataUrlAttempt(opts = {}) {
    ...
    const hasNeighborColumns = !!(
      normalizeText(opts.yesterday?.text) ||
      normalizeText(opts.tomorrow?.text) ||
    ...
    const layoutOpts = {
      ...
      maxDisplayWidthPx: opts.maxDisplayWidthPx,
      minDisplayWidthPx: opts.minDisplayWidthPx,
      minBodyDomPx: opts.minBodyDomPx,
      sheetDomPx: opts.sheetDomPx,
      absoluteMinPx: opts.absoluteMinPx
    };
    let renderCfg = cfg;
  ...
  async function composeDataUrlWithLayout(opts = {}) {
    const attempts = [
      { label: 'full', opts },
      { label: 'no-paper-texture', opts: { ...opts, paperTextureUrl: null } }
    ];
    let lastErr = null;
