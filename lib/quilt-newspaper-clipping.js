...
function (global) {
  ...
  const CONTRIBUTOR_TITLE_COLOR = '#2c2622';
  ...
  function drawContributorTitleLine(ctx, x, y, titlePx, titleText, colW, align = 'center') {
    ...
    ctx.save();
    ctx.font = contributorTitleFontSpec(titlePx);
    ctx.fillStyle = CONTRIBUTOR_TITLE_COLOR;
  ...
  function contributorListRunsFromNames(names) {
    ...
    return plainRunsFromText(list.join(' · '));
  }
  ...
  function resolveContributorListExportWidth(cfg, mctx, namesRuns, centerPx, centerPads) {
    const targetW = Math.max(
      Math.round(Number(cfg.width) || CONTRIBUTOR_LIST_DEFAULT_WIDTH),
      Math.round(Number(cfg.centerColMinW) || 0)
    );
    const measuredW = fitCenterColumnWidthForHardBreaks(mctx, namesRuns, centerPx, centerPads, cfg);
    return Math.max(targetW, measuredW);
  }
  ...
  function resolveContributorListBodyPxForHandCut(mctx, names, opts, cfgIn) {
    ...
    for (let pass = 0; pass < 8; pass += 1) {
      const cfg = withClippingTypography({ ...cfgIn, centerBodyPx: centerPx, bodyPx: centerPx });
      const centerPads = resolveColumnPadPair(cfg, 'center');
      const exportW = resolveContributorListExportWidth(cfg, mctx, namesRuns, centerPx, centerPads);
  ...
  function renderContributorListSpread(mctx, names, cfgIn, dateKey) {
    ...
    const cfg = typography;
    const namesRuns = contributorListRunsFromNames(names);
    if (!namesRuns.length) return null;
    ...
    const ruleH = ruleLW;
    const centerPads = resolveColumnPadPair(cfg, 'center');
    const W = resolveContributorListExportWidth(cfg, mctx, namesRuns, centerPx, centerPads);
    const centerColW = W;
    const centerX = 0;
    const namesLayout = layoutColumn(mctx, namesRuns, centerColW, centerPx, centerPads);
    const textBetweenRulesPad = Math.max(cfg.ruleTextGap, Math.round(centerPx * 0.35));
    ...
    const ruleY2 = textZoneTop + textBetweenRulesH;
    const ruleSpanW = Math.max(8, Math.round(centerColW * cfg.centerRuleSpanRatio));
    const ruleX0 = (centerColW - ruleSpanW) / 2;
    ...
    drawPaperBase(ctx, W, spreadH, cfg);
    drawScannerBed(ctx, W, spreadH, `${clipDateKey}:contributor-list`, 'newsprint');
    drawCardGrain(ctx, W, spreadH, cfg);

    drawContributorTitleLine(ctx, 0, titleBaselineY, titlePx, CONTRIBUTOR_CAROUSEL_TITLE, centerColW, 'center');
  ...
  function composeContributorListClippingCanvas(spread, cfg, dateKey) {
    ...
    const handCutKey = `${String(dateKey || 'our-daily').trim()}:contributor-carousel`;
  ...
  async function composeContributorListDataUrl(opts = {}) {
    ...
    const cfgBase = withClippingTypography({
      ...
      ...opts,
      width: targetWidth,
      centerColMaxW: Math.max(targetWidth, Number(opts.centerColMaxW) || 880),
      centerColMinW: Number(opts.centerColMinW) > 0 ? Number(opts.centerColMinW) : 720,
      handCutSideInwardMaxDomPx: 4,
    ...
    if (!spread?.canvas) return null;
    const clipped = composeContributorListClippingCanvas(spread, cfg, dateKey);
    if (!clipped) return null;
