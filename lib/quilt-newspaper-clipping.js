...
function (global) {
  ...
  /** Left paper margin scale (1 = symmetric; 0.6 = 40% narrower left gutter). */
  const CONTRIBUTOR_LIST_LEFT_SIDE_BLEED_SCALE = 0.6;
  ...
  function renderContributorListSpread(mctx, names, cfgIn, dateKey) {
    ...
    const visualCenterNudge = Math.round(
      domToExportPx(cfg, CONTRIBUTOR_LIST_HAND_CUT_LEFT_SIDE_INSET_DOM_PX) * 0.72
    );
  ...
  function composeContributorStripClippingCanvas(spread, cfg, dateKey, handCutSuffix = 'contributor-carousel') {
    ...
    return {
      ...
      removedLeft: trimmed.removedLeft || 0,
      removedTop: trimmed.removedTop || 0,
