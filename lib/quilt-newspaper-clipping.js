...
function (global) {
  ...
  /** Logged on Firestore uploads only — app does not gate on this; use newspaperClippingGeneratedAt + URL ?odq_t= for cache bust. */
  const CLIPPING_EXPORT_REV = '80';
  ...
  const DEFAULTS = {
    ...
    /** Paper above top rule — hand-cut wobble only; never trim into the rule stroke. */
  ...
  function renderFullSpread(mctx, opts, cfgIn) {
    ...
    /** Anchor crop flush to the top rule — no ruleGap paper above the band. */
    const handCutTopPad = Math.max(
      0,
      Math.round(domToExportPx(cfg, cfg.cropBandTopHandCutPadDomPx ?? 0))
    );
