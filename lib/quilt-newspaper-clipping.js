...
function (global) {
  ...
  /** Widest inner width that still keeps line 1 equal to `targetPhrase`. */
  function maxInnerWidthForFirstLinePhrase(mctx, runs, px, targetPhrase, innerLo, maxInner) {
    let maxOk = innerLo;
    for (let tryW = innerLo; tryW <= maxInner; tryW += 1) {
      const wrapped = wrapRuns(mctx, runs, tryW, px);
      if (firstWrappedLineText(wrapped) !== targetPhrase) break;
      maxOk = tryW;
    }
    return maxOk;
  }
  ...
  function fitCenterColumnWidth(mctx, runs, px, padOpts, cfg, firstLineMaxWords) {
    ...
    const maxOk = maxInnerWidthForFirstLinePhrase(mctx, runs, px, targetPhrase, innerLo, maxInner);
