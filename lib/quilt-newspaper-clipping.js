...
function (global) {
  ...
  /** Vertical divider rules for peek crop output (full clipping height). */
  function drawPeekClippingDividerRules(ctx, outW, outH, spread, cfg) {
    ...
    const ruleLW = exportRuleThickness(cfg);
    if (digitalExportPolishEnabled(cfg)) {
      strokeCrispVLine(ctx, x1, 0, outH, ruleInk, ruleLW);
      strokeCrispVLine(ctx, x2, 0, outH, ruleInk, ruleLW);
    } else {
      ctx.strokeStyle = ruleInk;
      ctx.lineWidth = ruleLW;
      ctx.beginPath();
  ...
  /** Vertical divider rules for full-column crop output (full clipping height). */
  function drawFullColumnsDividerRules(ctx, outW, outH, spread, cfg) {
    const { centerColW, centerX, sideColW, canvas: spreadCanvas } = spread;
    if (!spreadCanvas?.width || !centerColW) return;
    const gutterPx = domToExportPx(cfg, cfg.columnGutterDomPx ?? DEFAULTS.columnGutterDomPx);
    const sx = outW / spreadCanvas.width;
  ...
  /**
   * Pad clipped art for hand-cut pivots — newsprint surface + texture under peeks, rules full height.
   */
  function buildHandCutPaddedCanvas(canvas, cfg, opts = {}) {
    const contentPad = resolveHandCutEdgePadDomPx(cfg);
    const w = canvas.width + contentPad * 2;
    const h = canvas.height + contentPad * 2;
    const out = document.createElement('canvas');
    out.width = w;
    out.height = h;
    const octx = out.getContext('2d');
    ...
    const padCfg = withClippingTypography({ ...cfg, width: w, exportScale: 1 });
    drawPaperBase(octx, w, h, padCfg);
    drawCardGrain(octx, w, h, padCfg);
    octx.drawImage(canvas, contentPad, contentPad);
    drawNewsprintTextureOverlay(octx, w, h, padCfg);

    if (opts.peekCrop && opts.spread) {
      drawPeekClippingDividerRules(octx, w, h, opts.spread, padCfg);
    } else if (opts.fullColumns && opts.spread) {
      drawFullColumnsDividerRules(octx, w, h, opts.spread, padCfg);
    }
