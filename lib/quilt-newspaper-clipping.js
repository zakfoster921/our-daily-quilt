...
function (global) {
  ...
  /**
   * One off-center scallop on top (0) or bottom (4).
   * Outward/convex only — shallow enough to stay in hand-cut pad, never bite into horiz rules.
   */
  function handCutTopBottomEdgePoint(edgeIdx, seed, edgeLen, cfg) {
    ...
    const depthBase = domToExportPx(cfg, cfg.handCutEdgePointDomPx ?? 4) * 0.42;
    const depth = Math.max(1, depthBase * (0.85 + edgeRand() * 0.2));
    const side = edgeRand() > 0.5 ? 1 : -1;
    const center = Math.max(0.34, Math.min(0.66, 0.5 + side * (0.14 + edgeRand() * 0.12)));
    const maxDepth = Math.min(depth, Math.max(1, pad * 0.9), (edgeLen || 1) * 0.032);
    return {
      longBulge: {
        t0: center,
        t1: null,
