...
function (global) {
  ...
  function renderFullSpread(mctx, opts, cfgIn) {
    ...
    const textBetweenRulesPad = cfg.quoteOnlyTightPack
      ? Math.max(cfg.ruleTextGap, Math.round(centerPx * 0.22))
    ...
    const bandPad = cfg.quoteOnlyTightPack
      ? Math.max(
          Math.round(domToExportPx(cfg, Number(cfg.quoteOnlyBandPadDomPx) || 4)),
          Math.round(centerPx * 0.06)
        )
      : Math.max(
  ...
  function composeLabPeekClippingCanvas(spread, cfg, dateKey) {
    ...
    return trimCanvasAlphaBounds(clipped, cfg, cfg.quoteOnlyTightPack ? 2 : 6);
  }
  ...
  function normalizeComposeExportOpts(opts = {}) {
    ...
    return {
      ...
      quoteOnlyBandPadDomPx: 3,
      ruleTextGap: 7,
      ruleGap: 5,
      spreadPadY: 8,
      cropVerticalBleedDomPx: 0,
      cropBandTopHandCutPadDomPx: 1,
