...
function (global) {
  ...
  const DEFAULTS = {
    ...
    /** 1–2 wide shallow outward pivots on 1–2 sides (replaces corner chamfers). */
  ...
  /**
   * Seeded outward pivot tabs on 1–2 sides — wide shallow bumps that break up straight edges.
   * edgeIdx: 0 top, 2 right, 4 bottom, 6 left.
   */
  function resolveHandCutEdgePivotTabs(seed, mainEdgeLen, cfg) {
    const tabs = new Map();
    if (cfg.handCutTopBottomEdgePointEnabled === false) return tabs;

    const pickRand = seededRandom(hashDateKeySeed(`${seed >>> 0}:edge-pivot-pick`));
    const pivotCount = pickRand() < 0.52 ? 1 : 2;
    const sides = [0, 2, 4, 6].slice().sort(() => pickRand() - 0.5);
    const selected = sides.slice(0, pivotCount);

    for (const edgeIdx of selected) {
      const edgeSeed = hashDateKeySeed(`${seed >>> 0}:edge-pivot:${edgeIdx}`);
      const edgeRand = seededRandom(edgeSeed);
      const pad = domToExportPx(cfg, cfg.handCutCanvasPadDomPx ?? 1);
      const depthBase = domToExportPx(cfg, cfg.handCutEdgePointDomPx ?? 6) * 0.36;
      const depth = Math.max(1, depthBase * (0.8 + edgeRand() * 0.25));
      const side = edgeRand() > 0.5 ? 1 : -1;
      const center = Math.max(0.3, Math.min(0.7, 0.5 + side * (0.12 + edgeRand() * 0.14)));
      const edgeLen = mainEdgeLen[edgeIdx] || 1;
      const maxDepth = Math.min(depth, Math.max(1, pad * 0.88), edgeLen * 0.03);
      tabs.set(edgeIdx, {
        t: center,
        half: 0.13 + edgeRand() * 0.09,
        depth: maxDepth
      });
    }
    return tabs;
  ...
  /**
   * Square-ish rect + optional edge pivot tabs (no corner chamfers).
   * Optional inward bites when handCutNotchesEnabled.
   */
  function buildHandCutPolygon(w, h, seed, cfg) {
    ...
    const mainEdgeLen = {
      ...
      2: edgeLen(cTopR.x, cTopR.y, cBotR.x, cBotR.y),
      4: edgeLen(cBotR.x, cBotR.y, cBotL.x, cBotL.y),
      6: edgeLen(cBotL.x, cBotL.y, cTopL.x, cTopL.y)
    };
    const pivotTabs = resolveHandCutEdgePivotTabs(seed, mainEdgeLen, cfg);
    ...
    const notchPool = mainEdges.filter((e) => !pivotTabs.has(e));
    ...
    for (const idx of notchEdges) {
      ...
      const len = mainEdgeLen[idx] || w;
      notchSpec.set(idx, {
        t: 0.28 + rand() * 0.44,
        half: 0.09 + rand() * 0.07,
        depth: Math.min(depth * (0.42 + rand() * 0.18), len * 0.075)
      });
    }

    const segments = [
      { ax: cTopL.x, ay: cTopL.y, bx: cTopR.x, by: cTopR.y, edgeIdx: 0, isSide: false },
      { ax: cTopR.x, ay: cTopR.y, bx: cBotR.x, by: cBotR.y, edgeIdx: 2, isSide: true },
      { ax: cBotR.x, ay: cBotR.y, bx: cBotL.x, by: cBotL.y, edgeIdx: 4, isSide: false },
      { ax: cBotL.x, ay: cBotL.y, bx: cTopL.x, by: cTopL.y, edgeIdx: 6, isSide: true }
    ];
    ...
    const strokeOpts = (seg) => {
      const outwardTab = pivotTabs.get(seg.edgeIdx) || null;
      return {
        rand,
        cx,
        cy,
        slip: outwardTab ? 0 : longSlip,
        kinkAmp: longKink,
        bite: notchSpec.get(seg.edgeIdx) || null,
        outwardTab,
        longBulge: null,
        twoCuts: false,
        topBottomTrim: seg.edgeIdx === 0 || seg.edgeIdx === 4 ? trimTopBottom : 0,
        isSide: !!seg.isSide,
        sideInMax: trimSides,
        cleanLongEdge: !!outwardTab
      };
    };
