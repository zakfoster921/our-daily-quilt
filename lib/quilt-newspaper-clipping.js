...
function (global) {
  ...
  /** Keep emphasized keyword phrases on one wrap token (e.g. "very simple"). */
  function runWrapParts(run) {
    const t = String(run.text || '');
    if (!t) return [];
    if (runHasEmphasis(run) && t === t.trim()) return [t.trim()];
    return t.split(/(\s+)/).filter((p) => p.length);
  }
  ...
  /** Runs for the first N word units (emphasized keyword phrases count as one unit). */
  function extractFirstWordsRuns(runs, wordCount) {
    ...
    for (const run of runs) {
      ...
      const t = String(run.text || '');
      if (!t.trim()) continue;
      if (runHasEmphasis(run) && t === t.trim()) {
  ...
  /** Visible word count on wrapped line 1 (punctuation stripped per token). */
  function visualWordCountOnFirstLine(wrapped) {
    return firstWrappedLineText(wrapped)
      .split(/\s+/)
      .filter(Boolean)
      .map((w) => w.replace(/^[("'[\{]+|[)"'"\}\],.;:!?]+$/g, ''))
  ...
  /** Line 1 must stay within `cap` units; visible-word cap applies when line 1 is short on units. */
  function firstWrappedLineMatchesWordCap(wrapped, cap, targetWords) {
    ...
    const visual = visualWordCountOnFirstLine(wrapped);
    if (got.length < cap && visual > cap) return false;
  ...
  /** Word units from runs — emphasized keyword phrases stay one unit (e.g. "very simple"). */
  ...
  function fitCenterColumnWidth(mctx, runs, px, padOpts, cfg, firstLineMaxWords) {
    ...
    for (let tryW = floorW; tryW <= maxInner; tryW += 1) {
      ...
      const units = firstLineWordsFromWrapped(wrapped).length;
      const visual = visualWordCountOnFirstLine(wrapped);
      if (
        units > bestUnits ||
        (units === bestUnits && visual > bestVisual) ||
        foundLo < 0
      ) {
        bestUnits = units;
        bestVisual = visual;
