...
function (global) {
  ...
  const CLIPPING_EXPORT_REV = '60';
  /** Mood triptych quote PNG — on-screen cap below `--triptych-scale` sheet width. */
  const TRIPTYCH_QUOTE_DISPLAY_SCALE = 0.551;
  /** Export center-column type scale after hand-cut fit (triptych / nightly quote PNG). */
  const TRIPTYCH_QUOTE_TYPE_SCALE = 0.761;
  ...
  function resolveClippingPaperTextureUrl(raw) {
    ...
    if (typeof document !== 'undefined') {
      const script = document.querySelector('script[src*="quilt-newspaper-clipping.js"]');
      const scriptSrc = script?.getAttribute('src');
      if (scriptSrc) {
        try {
          bases.push(new URL(scriptSrc, location.href).href);
        } catch (_) {
          /* */
        }
      }
    ...
    for (const base of bases) {
      try {
        if (src.startsWith('assets/') && /\/lib\//i.test(base)) {
   ...
   * Floor uses the same displayScale as max (triptych ≈ 0.85× sheet) so short quotes do not
  ...
  function estimateHandCutSafePaddingDomPx(cfg, dateKey) {
    ...
    const bite = Number(cfg.handCutBiteMaxDomPx ?? 8) * (0.55 + rand() * 0.42);
  ...
  /**
   * Chamfered rect + scissor cuts: ~1 big chop per side, optional second shallow notch.
   */
  function buildHandCutPolygon(w, h, seed, cfg) {
    ...
    const notchCount = rand() < 0.22 ? 2 : 1;
  ...
  async function composeDataUrlAttempt(opts = {}) {
    ...
    let clipped = composePeekClippingCanvas(spread, cfg, opts, usePeekSides);
    if (!clipped) return null;
    ...
    for (let pass = 0; pass < 3; pass += 1) {
      ...
      const sharpClipped = composePeekClippingCanvas(spreadSharp, renderCfg, opts, usePeekSides);
    ...
    const centerOnlyCanvas =
      usePeekSides && refSpread ? cropSpreadCenterOnly(refSpread, renderCfg) : null;
    const centerOnlyWidth = centerOnlyCanvas?.width || 0;
    const peekWidthRatio =
      centerOnlyWidth > 0 ? clipped.width / centerOnlyWidth : usePeekSides ? 1 : 0;
    ...
    return {
      ...
      peekCrop: usePeekSides && peekWidthRatio >= 1.08,
      centerOnlyWidth,
      peekWidthRatio,
