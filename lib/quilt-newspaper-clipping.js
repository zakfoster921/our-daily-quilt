...
function (global) {
  ...
  /** Word tokens on wrapped line 1 (same rules as `extractFirstWordsRuns`). */
  function firstLineWordsFromWrapped(wrapped) {
    const words = [];
    for (const run of wrapped[0] || []) {
      const parts = String(run.text || '')
        .split(/(\s+)/)
        .filter((p) => p.length);
      for (const part of parts) {
  ...
  /** Word tokens from runs (preserves boundaries across emphasized runs). */
