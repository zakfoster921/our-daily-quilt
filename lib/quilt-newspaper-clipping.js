...
function (global) {
  ...
  function resolveCropBandTopHandCutPadDomPx(cfg = DEFAULTS) {
    ...
    const ruleClearance = 10;
    const ruleGap = Number(cfg.ruleGap ?? DEFAULTS.ruleGap);
    return Math.max(configured, canvasPad + maxIntoDom + ruleClearance + ruleGap);
  }
