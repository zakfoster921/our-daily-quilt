...
function (global) {
  ...
  const CLIPPING_EXPORT_REV = '68';
  /** Mood triptych quote PNG — on-screen cap below `--triptych-scale` sheet width (~72% sheet). */
  const TRIPTYCH_QUOTE_DISPLAY_SCALE = 0.72;
  /** Quote color-picker top half — tighter than triptych so tall peek PNGs fit without overflow. */
  const QUOTE_SCREEN_DISPLAY_SCALE = 0.58;
  ...
  function renderFullSpread(mctx, opts, cfgIn) {
    ...
    /** Hand-cut top wobble can clip the upper horizontal rule — extra paper above center block. */
    const bandTopPad =
      bandPad + domToExportPx(cfg, cfg.ruleGap + 8);
    const bandTop = Math.max(0, centerBlockTop - bandTopPad);
  ...
  function composePeekClippingCanvas(spread, cfg, opts, usePeekSides) {
    ...
    return trimCanvasAlphaBounds(clipped, cfg, 6, { preserveWidth: !!usePeekSides });
  }
  ...
  function composeFullColumnsClippingCanvas(spread, cfg, opts) {
    ...
    const dateKey =
      ...
      '';
    clipped = applyHandCutSilhouette(clipped, dateKey, cfg);
    return trimCanvasAlphaBounds(clipped, cfg, 6, { preserveWidth: true });
  }
  ...
  function trimCanvasAlphaBounds(canvas, cfg, bleedDomPx = 6, trimOpts = {}) {
    ...
    const bleed = Math.ceil(domToExportPx(cfg, bleedDomPx));
  ...
  function cropSpreadCenterOnly(spread, cfg) {
    ...
    const bleedSrcY = Math.ceil(bleedY);
  ...
  function cropSpreadToClipping(spread, cfg) {
    ...
    const scale = 1;
    const bleedSrcY = Math.ceil(bleedY / scale);
    const peekSrcW = Math.max(1, peekBaseOut);
    const srcY = Math.max(0, bandTop - bleedSrcY);
    const srcH = Math.min(spreadCanvas.height - srcY, bandH + bleedSrcY * 2);
    const outH = Math.max(1, Math.ceil(srcH * scale));
