...
function (global) {
  ...
  function contributorListTitleCropTopPadPx(cfg, spread) {
    ...
    return Math.max(
      handCutTopPad + Math.round(spreadPadExportPx * 0.65),
      visiblePaperExportPx,
      Math.round(drawTitlePx * CONTRIBUTOR_LIST_TITLE_TOP_PAD_RATIO),
      handCutInsetExportPx + handCutPivotExportPx + Math.round(drawTitlePx * 0.14)
    );
  }
  ...
  function padContributorHandCutTopMargin(canvas, cfg, spread) {
    ...
    const extraTop = Math.max(
      domToExportPx(cfg, 14),
      Math.round(titlePx * 0.42),
      Math.round(handCutInsetExportPx * 0.9),
      Math.round(domToExportPx(cfg, CONTRIBUTOR_LIST_TITLE_VISIBLE_PAPER_DOM_PX) * 0.65)
    );
  ...
  /** Blit paper to both sides so hand-cut side insets cannot cover contributor text. */
  ...
  function contributorListTightHorizontalBleedsExportPx(cfg) {
    const insetExportPx = domToExportPx(cfg, resolveHandCutPolygonInsetDomPx(cfg));
    const trimExportPx = domToExportPx(cfg, CONTRIBUTOR_LIST_TRIM_SIDE_BLEED_DOM_PX);
    const handCutPadPx = Math.ceil(insetExportPx + trimExportPx * 0.45);
    return {
  ...
  function composeContributorStripClippingCanvas(spread, cfg, dateKey, handCutSuffix = 'contributor-carousel') {
    ...
    if (isContributorList) {
      ...
      // Add equal bottom padding to vertically center the text within the clipping
      if (handCutTopPadPx > 0) {
        const w = clipped.width, h = clipped.height;
        const bottomPadded = document.createElement('canvas');
        bottomPadded.width = w;
        bottomPadded.height = h + handCutTopPadPx;
        const bctx = bottomPadded.getContext('2d');
        bctx.drawImage(clipped, 0, 0, w, h, 0, 0, w, h);
        bctx.drawImage(clipped, 0, h - 1, w, 1, 0, h, w, handCutTopPadPx);
        clipped = bottomPadded;
      }
    } else {
