...
function (global) {
  ...
  function imageSrcUsesCrossOrigin(src) {
    ...
    try {
      const u = new URL(s, typeof location !== 'undefined' ? location.href : 'http://127.0.0.1/');
  ...
  function resolveMoodSpreadCropMetrics(cfg, spread = null) {
    ...
    const gap = domToExportPx(cfg, cfg.moodFlankCenterGapDomPx ?? DEFAULTS.moodFlankCenterGapDomPx);
    const outW =
      sideOutW + flankW + gap + centerOutW + gap + flankW + sideOutW;
    const viewportW = flankW + gap + centerOutW + gap + flankW;
    return {
      baseW,
      centerOutW,
      sideOutW,
  ...
  /** Same hand-cut pass as `composeDataUrl` (quote clipping). */
  function finishMoodSpreadClipping(clipped, cfg, dateKey) {
    if (!clipped) return null;
    if (cfg.moodSpreadHandCutExport === false || cfg.handCutEnabled === false) return clipped;
    return applyHandCutSilhouette(clipped, moodSpreadHandCutDateKey(dateKey), cfg);
  }
  ...
  async function composeMoodSpreadDataUrl(opts = {}) {
    ...
    const dateKey =
      ...
      String(opts.today?.dateKey || '').trim() ||
      '';
    clipped = finishMoodSpreadClipping(clipped, cfg, dateKey);
  ...
  async function composeMoodSpreadWithMetrics(opts = {}) {
    ...
    return {
      dataUrl: clipped.toDataURL('image/png', 0.92),
