...
function (global) {
  ...
  function buildHandCutPolygon(w, h, seed, cfg) {
    ...
    const coupledSideLean = cfg.handCutSideLeanCoupled === true;
    const sharedSideTilt = tiltSign();
    const applySideLean = (top, bottom, edgeLen) => {
      const tilt = coupledSideLean ? sharedSideTilt : tiltSign();
      bottom.x = top.x + minSideLean(edgeLen) * tilt;
    };
    ...
    if (leftInsetPx > 0) {
      cTopL.x = Math.min(x1 - skew, cTopL.x + leftInsetPx);
      cBotL.x = Math.min(x1 - skew, cBotL.x + leftInsetPx);
    }
    const leftTopAlign = Number(cfg.handCutLeftSideTopAlignScale);
    if (Number.isFinite(leftTopAlign) && leftTopAlign > 0 && leftTopAlign < 1) {
      cTopL.x = cBotL.x + (cTopL.x - cBotL.x) * leftTopAlign;
    }

    const corners = [cTopL, cTopR, cBotR, cBotL];
    const pivotsEnabled = cfg.handCutTopBottomEdgePointEnabled !== false;
    const leftPivotDepthScale = Number(cfg.handCutLeftSidePivotDepthScale);
    ...
    function outwardPivotForSide(sideIdx) {
      ...
      const lenCap =
        sideIdx === 3 && leftPivotLenCap > 0.065 ? leftPivotLenCap : 0.065;
      ...
      if (
        sideIdx === 3 &&
        Number.isFinite(leftPivotDepthScale) &&
        leftPivotDepthScale > 1
      ) {
        maxDepth = Math.min(maxDepth * leftPivotDepthScale, len * (lenCap + 0.038));
      }
      return {
  ...
  function trimCanvasAlphaBoundsWithMeta(canvas, cfg, bleedDomPx = 6, trimOpts = {}) {
    ...
    if (tw >= w && th >= h) {
      if (trimOpts.fillHandCutCorners !== false) {
        fillHandCutCornerNotches(canvas, cfg, trimOpts.handCutCornerFrac);
      }
    ...
    if (trimOpts.fillHandCutCorners !== false) {
      fillHandCutCornerNotches(out, cfg, trimOpts.handCutCornerFrac);
    }
  ...
  function parsePaperRgb(paper) {
    const raw = String(paper || '#f6f4f1').trim();
    if (/^rgba?\(/i.test(raw)) {
      const nums = raw.match(/[\d.]+/g) || [];
      return {
        r: Math.round(Number(nums[0]) || 246),
        g: Math.round(Number(nums[1]) || 244),
        b: Math.round(Number(nums[2]) || 241)
      };
    }
    ...
    return { r: 246, g: 244, b: 241 };
  }

  /** Hand-cut bbox corners sit outside the polygon — fill those notches with paper, not transparency. */
  function fillHandCutCornerNotches(canvas, cfg, cornerFrac = 0.085) {
    ...
    const cw = Math.max(6, Math.round(Math.min(w, h) * cornerFrac));
    const ch = cw;
    const paper = parsePaperRgb(cfg.paper || '#f6f4f1');
    let data;
    try {
    ...
    for (const [rx, ry] of regions) {
      for (let y = ry; y < ry + ch; y += 1) {
        for (let x = rx; x < rx + cw; x += 1) {
          const i = (y * w + x) * 4;
  ...
  function padContributorHandCutSideMargins(canvas, cfg, spread) {
    if (!canvas) return { canvas, padSidePx: 0 };
    const extraSide = Math.max(
      domToExportPx(cfg, CONTRIBUTOR_LIST_TEXT_SAFE_SIDE_DOM_PX),
      Math.round((Number(spread?.titlePx) || 12) * 0.32)
    );
    if (extraSide <= 0) return { canvas, padSidePx: 0 };
    const w = canvas.width;
    const h = canvas.height;
    const out = document.createElement('canvas');
    ...
    out.height = h;
    const ctx = out.getContext('2d');
    if (!ctx) return { canvas, padSidePx: 0 };
    ...
    return { canvas: out, padSidePx: extraSide };
  }
  ...
  function trimContributorListLeftToInk(canvas, cfg, spread, trimMeta = {}) {
    ...
    const cropLeft = Math.max(0, Math.min(canvas.width - 48, inkLeft - safePx));
  ...
  /** Side safety before hand-cut so contributor text never sits under the torn paper edge. */
  ...
  function renderContributorListSpread(mctx, names, cfgIn, dateKey) {
    ...
    {
      const innerW = Math.max(1, contentW - centerPads.padLeft - centerPads.padRight);
      const fillTarget = 0.85;
      const maxPx = Math.round(centerPx * 2.4);
      let guard = 0;
      while (guard < 60 && centerPx < maxPx) {
        const probeLayout = layoutContributorNameColumn(mctx, nameList, contentW, centerPx + 1, centerPads);
    ...
    const sideBleeds = contributorListHorizontalBleedsExportPx(cfg, bottomBandPad, handCutTopPad);
    ...
    const centerColW = W;
    const centerX = 0;
    const visualCenterNudge = Math.round(
      domToExportPx(cfg, CONTRIBUTOR_LIST_HAND_CUT_LEFT_SIDE_INSET_DOM_PX) * 0.5
    );
    const textColX = Math.round((W - contentW) / 2 + visualCenterNudge);
  ...
  function composeContributorStripClippingCanvas(spread, cfg, dateKey, handCutSuffix = 'contributor-carousel') {
    ...
    const trimSideDom = isContributorCarouselStrip ? 1 : trimSideBleedDomPx;
    ...
    if (isContributorList && spread) {
      const trimMeta = { removedLeft, removedTop, handCutTopPadPx, handCutSidePadPx };
      const leftInkTrim = trimContributorListLeftToInk(outCanvas, cfg, spread, trimMeta);
      outCanvas = leftInkTrim.canvas;
      removedLeft += leftInkTrim.extraRemovedLeft || 0;
    }
    if (outCanvas) fillHandCutCornerNotches(outCanvas, cfg);
    return {
      canvas: outCanvas,
      removedLeft,
      removedTop,
  ...
  async function composeContributorListDataUrl(opts = {}) {
    ...
    const cfgBase = withClippingTypography({
      ...
      handCutSideInwardMaxDomPx: 0,
      handCutMacroDomPx: 5,
      handCutSideLeanCoupled: true,
      contributorListLeftSideBleedScale: CONTRIBUTOR_LIST_LEFT_SIDE_BLEED_SCALE,
      handCutLeftSideTopAlignScale: CONTRIBUTOR_LIST_HAND_CUT_LEFT_TOP_ALIGN_SCALE,
      handCutLeftSideInsetDomPx: CONTRIBUTOR_LIST_HAND_CUT_LEFT_SIDE_INSET_DOM_PX,
      handCutLeftSidePivotDepthScale: CONTRIBUTOR_LIST_HAND_CUT_LEFT_PIVOT_DEPTH_SCALE,
      handCutLeftSidePivotLenCap: CONTRIBUTOR_LIST_HAND_CUT_LEFT_PIVOT_LEN_CAP,
      paperTextureUrl: resolveClippingPaperTextureUrl(opts.paperTextureUrl)
    });
