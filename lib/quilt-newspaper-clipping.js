...
function (global) {
  ...
  function composePeekClippingCanvas(spread, cfg, opts, usePeekSides) {
    ...
    clipped = restrokeCenterBandRulesOnClipped(clipped, spread, cfg, cropKind);
    const dateKey =
      String(opts.dateKey || '').trim() ||
      String(opts.today?.dateKey || '').trim() ||
      '';
    const handCut = applyHandCutSilhouette(clipped, dateKey, cfg);
    const trimmed = trimCanvasAlphaBoundsWithMeta(handCut, cfg, 3, trimOpts);
    const centerStripTopPx = measureCenterStripTopOpaqueRow(trimmed.canvas, trimOpts);
    clipped = restrokeCenterBandRulesOnClipped(trimmed.canvas, spread, cfg, cropKind, {
      afterHandCut: true,
      centerStripTopPx,
      removedLeft: trimmed.removedLeft
    });
    const skipTopInkTrim =
      cfg.handCutEnabled !== false && cfg.handCutTopBottomEdgePointEnabled !== false;
    return skipTopInkTrim
      ? clipped
  ...
  function composeFullColumnsClippingCanvas(spread, cfg, opts) {
    ...
    const trimmed = trimCanvasAlphaBoundsWithMeta(handCut, cfg, 3, trimOpts);
    const centerStripTopPx = measureCenterStripTopOpaqueRow(trimmed.canvas, trimOpts);
    clipped = restrokeCenterBandRulesOnClipped(trimmed.canvas, spread, cfg, 'full', {
      afterHandCut: true,
      centerStripTopPx,
      removedLeft: trimmed.removedLeft
    });
