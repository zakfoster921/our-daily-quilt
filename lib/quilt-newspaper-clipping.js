...
function (global) {
  ...
  function buildHandCutPaddedCanvas(canvas, cfg, opts = {}) {
    ...
    // Stretch top/bottom edge rows into the pad so pivot bumps carry paper texture.
    // Left/right sides use the plain fill to avoid stretching peek-column text at 3x.
    if (contentPad > 0) {
      octx.drawImage(canvas, 0, 0, cw, 1, contentPad, 0, cw, contentPad);
      octx.drawImage(canvas, 0, ch - 1, cw, 1, contentPad, contentPad + ch, cw, contentPad);
    }
  ...
  function composeContributorStripClippingCanvas(spread, cfg, dateKey, handCutSuffix = 'contributor-carousel') {
    ...
    // Trim to zero bleed for contributor carousel to eliminate white paper gaps at all edges
    const trimBleedDom = isContributorList ? 0 : pivotBleedDom;
    const trimSideDom = isContributorList ? 0 : trimSideBleedDomPx;
