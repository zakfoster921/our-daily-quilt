...
function (global) {
  ...
  const DEFAULTS = {
    ...
    /** Paper above top rule — must clear top-edge inward hand-cut bulge; see resolveCropBandTopHandCutPadDomPx. */
    ...
    handCutMacroDomPx: 5,
    /** Inward triangular scissor bites on long edges — off by default (chamfer + wobble only). */
    ...
    /** One off-center bump per top/bottom edge (convex outward or concave inward dip). */
    handCutTopBottomEdgePointEnabled: true,
    handCutEdgePointDomPx: 5,
  ...
  /** Min dom px above center top rule so hand-cut top inward bulge never clips the stroke. */
  function resolveCropBandTopHandCutPadDomPx(cfg = DEFAULTS) {
    ...
    const maxIntoDom = 5.5 + 2.5;
    const ruleClearance = 2;
    return Math.max(configured, canvasPad + maxIntoDom + ruleClearance);
  }

  /** Re-ink center column rules on the cropped band (after crop, before hand-cut mask). */
  function restrokeCenterBandRulesOnClipped(clipped, spread, cfg, cropKind = 'peek') {
    ...
    if (cropKind === 'full') {
      ...
      y1 = (ruleY1 - srcY) * scale;
      y2 = (ruleY2 - srcY) * scale;
      x0 = (centerX + (Number.isFinite(ruleX0Center) ? ruleX0Center : 0)) * scale;
      spanW *= scale;
    } else {
      y1 = ruleY1 - bandTop;
      y2 = ruleY2 - bandTop;
      const peekW =
        cropKind === 'peek'
          ...
          : 0;
      x0 = peekW + (Number.isFinite(ruleX0Center) ? ruleX0Center : 0);
    }
  ...
  function resolveCenterLineAlign(ctx, runs, px, innerW, cfg, { isLastLine = false } = {}) {
    const words = tokenizeLineForJustify(runs);
  ...
  function fitCenterColumnWidth(mctx, runs, px, padX, cfg, firstLineMaxWords) {
    ...
    for (let guard = 0; guard < 16; guard += 1) {
      ...
      if (firstWrappedLineText(wrapped) !== targetPhrase) {
        innerW += 1;
        continue;
      }
      const widest = widestWrappedLineWidth(mctx, wrapped, px);
      const next = Math.max(innerLo, widest);
      if (next <= innerW) break;
      innerW = Math.min(next, maxInner);
    }
    ...
    if (Number.isFinite(maxGapRatio) && maxGapRatio > 0) {
      ...
      for (let tryW = innerLo; tryW <= maxInner; tryW += 1) {
        ...
        if (firstWrappedLineText(wrapped) !== targetPhrase) continue;
      ...
      innerW = Math.max(innerW, chosen);
    }

    let colW = Math.ceil(innerW + padX * 2);
  ...
  function renderFullSpread(mctx, opts, cfgIn) {
    ...
    for (let i = 0; i < centerLines.length; i++) {
      const lineAlign = resolveCenterLineAlign(
        ctx,
        centerLines[i],
        centerPx,
    ...
    /** Paper above top rule — sized for hand-cut top inward bulge, not ruleGap. */
  ...
  function estimateHandCutSafePaddingDomPx(cfg, dateKey) {
    ...
    const chamfer = Number(cfg.handCutCornerChamferDomPx ?? 12) * (0.42 + rand() * 0.38);
    ...
    const horizontal = margin + chamfer + bite + sideIn + macro + 1.5;
    const vertical = margin + chamfer + bite + topBot + macro + 1.5;
  ...
  function resolveCenterBodyPxForHandCut(mctx, opts, cfgIn) {
    ...
    for (let pass = 0; pass < 6; pass += 1) {
      ...
      const colLayout = layoutColumn(mctx, todayRuns, centerColW, centerPx, cfg.spreadPadX);
      ...
      const innerW = Math.max(1, centerColW - cfg.spreadPadX * 2);
      ...
      centerPx = Math.max(8, Math.floor(centerPx * scale * 0.97));
    }
    return {
  ...
  function walkScissorStroke(ring, ax, ay, bx, by, opts) {
    ...
    if (longBulge) {
      ...
      // Top edge outward bulges extend into blank paper and get erased by trimCanvasTopInk.
  ...
  /**
   * One off-center point on top (0) or bottom (4); chamfer corners are the other two anchors.
   */
  function handCutTopBottomEdgePoint(edgeIdx, seed, edgeLen, cfg) {
    if (edgeIdx !== 0 && edgeIdx !== 4) return null;
    if (cfg.handCutTopBottomEdgePointEnabled === false) return null;
    const edgeSeed = hashDateKeySeed(`${seed >>> 0}:tb-edge:${edgeIdx}`);
    const edgeRand = seededRandom(edgeSeed);
    const isTop = edgeIdx === 0;
    const depthBase = domToExportPx(cfg, cfg.handCutEdgePointDomPx ?? 4) * 0.52;
    const depth = Math.max(1.5, depthBase * (0.9 + edgeRand() * 0.15));
    const side = edgeRand() > 0.5 ? 1 : -1;
    const center = Math.max(0.34, Math.min(0.66, 0.5 + side * (0.14 + edgeRand() * 0.12)));
    const convex = isTop ? false : edgeRand() > 0.45;
    let maxDepth = Math.min(depth, (edgeLen || 1) * 0.04);
    if (isTop) {
      const pad = domToExportPx(cfg, cfg.handCutCanvasPadDomPx ?? 1);
      // Must reach opaque content (pad zone is transparent); keep shallow for one-kink cut.
      const intoContent = pad + domToExportPx(cfg, 5.5 + edgeRand() * 2.5);
      const cap = (edgeLen || 1) * 0.055;
      maxDepth = Math.min(cap, Math.max(maxDepth, intoContent));
    }
    const longBulge = {
      t0: center,
      t1: null,
  ...
  /**
   * Chamfered rect + scissor wobble (optional inward bites when handCutNotchesEnabled).
   */
  function buildHandCutPolygon(w, h, seed, cfg) {
    ...
    const slip = domToExportPx(cfg, cfg.handCutMacroDomPx ?? 5) * 0.18;
    const kinkAmp = domToExportPx(cfg, cfg.handCutMacroDomPx ?? 5) * 0.2;
    const bitePrimary = domToExportPx(cfg, cfg.handCutBiteMaxDomPx ?? 8);
    const biteSecondary = domToExportPx(cfg, cfg.handCutSecondaryBiteDomPx ?? 5);
    ...
    const cornerJitter = () => (rand() - 0.5) * 2 * skew;
    const nominalTopLen = x1 - x0 - 2 * cfX;
    const nominalSideLen = y1 - y0 - 2 * cfY;
    const minAxisDelta = (edgeLen) => Math.max(skew * 0.85, edgeLen * 0.038);
    /** Left/right cuts lean visibly — both corners were pinned to x0/x1 and read as vertical. */
    const minSideLean = (edgeLen) => Math.max(skew * 1.45, edgeLen * 0.085);

    const tiltSign = () => (rand() > 0.5 ? 1 : -1);
    const ensureAxisTilt = (a, b, horizontal, edgeLen) => {
      const min = minAxisDelta(edgeLen);
      if (horizontal) {
        if (Math.abs(b.y - a.y) < min) b.y = a.y + min * tiltSign();
      }
    };
    const applySideLean = (top, bottom, edgeLen) => {
      bottom.x = top.x + minSideLean(edgeLen) * tiltSign();
    };
    const ensureChamferTilt = (a, b) => {
      const span = Math.hypot(b.x - a.x, b.y - a.y) || cfX;
      const min = Math.max(skew * 0.55, span * 0.22);
      if (Math.abs(b.x - a.x) < min) b.x = a.x + min * tiltSign();
      if (Math.abs(b.y - a.y) < min) b.y = a.y + min * tiltSign();
    };

    const cTopL = { x: x0 + cfX + cornerJitter(), y: y0 + cornerJitter() };
    const cTopR = { x: x1 - cfX + cornerJitter(), y: y0 + cornerJitter() };
    const cRightT = { x: x1 + cornerJitter(), y: y0 + cfY + cornerJitter() };
    const cRightB = { x: x1 + cornerJitter(), y: y1 - cfY + cornerJitter() };
    applySideLean(cRightT, cRightB, nominalSideLen);
    const cBotR = { x: x1 - cfX + cornerJitter(), y: y1 + cornerJitter() };
    const cBotL = { x: x0 + cfX + cornerJitter(), y: y1 + cornerJitter() };
    const cLeftB = { x: x0 + cornerJitter(), y: y1 - cfY + cornerJitter() };
    const cLeftT = { x: x0 + cornerJitter(), y: y0 + cfY + cornerJitter() };
    applySideLean(cLeftT, cLeftB, nominalSideLen);

    ensureAxisTilt(cTopL, cTopR, true, nominalTopLen);
    if (cfg.handCutTopBottomEdgePointEnabled !== false) {
      const avgTopY = (cTopL.y + cTopR.y) * 0.5;
      const maxTopTilt = skew * 0.28;
      cTopL.y = avgTopY + Math.max(-maxTopTilt, Math.min(maxTopTilt, cTopL.y - avgTopY));
      cTopR.y = avgTopY + Math.max(-maxTopTilt, Math.min(maxTopTilt, cTopR.y - avgTopY));
    }
    ensureChamferTilt(cTopR, cRightT);
    ensureChamferTilt(cRightB, cBotR);
    ensureAxisTilt(cBotR, cBotL, true, nominalTopLen);
    if (cfg.handCutTopBottomEdgePointEnabled !== false) {
      const avgBotY = (cBotR.y + cBotL.y) * 0.5;
      const maxBotTilt = skew * 0.28;
      cBotR.y = avgBotY + Math.max(-maxBotTilt, Math.min(maxBotTilt, cBotR.y - avgBotY));
      cBotL.y = avgBotY + Math.max(-maxBotTilt, Math.min(maxBotTilt, cBotL.y - avgBotY));
    }
    ensureChamferTilt(cBotL, cLeftB);
    ensureChamferTilt(cLeftT, cTopL);
    applySideLean(cRightT, cRightB, nominalSideLen);
    applySideLean(cLeftT, cLeftB, nominalSideLen);

    const edgeLen = (ax, ay, bx, by) => Math.hypot(bx - ax, by - ay) || 1;
    const mainEdgeLen = {
      0: edgeLen(cTopL.x, cTopL.y, cTopR.x, cTopR.y),
      2: edgeLen(cRightT.x, cRightT.y, cRightB.x, cRightB.y),
      4: edgeLen(cBotR.x, cBotR.y, cBotL.x, cBotL.y),
      6: edgeLen(cLeftB.x, cLeftB.y, cLeftT.x, cLeftT.y)
    };
    const mainEdges = [0, 2, 4, 6];
    const notchesEnabled =
      cfg.handCutNotchesEnabled !== false && bitePrimary > 0.5 && biteSecondary > 0.5;
    const notchCount = notchesEnabled ? (rand() < 0.22 ? 2 : 1) : 0;
    const notchPool =
      cfg.handCutTopBottomEdgePointEnabled !== false
        ? mainEdges.filter((e) => e !== 0 && e !== 4)
        : mainEdges;
    const shuffled = notchPool.slice().sort(() => rand() - 0.5);
    const notchEdges = notchCount > 0 ? new Set(shuffled.slice(0, notchCount)) : new Set();
    const notchSpec = new Map();
    for (const idx of notchEdges) {
      const depth = idx === shuffled[0] ? bitePrimary : biteSecondary;
      const edgeLen = mainEdgeLen[idx] || w;
      notchSpec.set(idx, {
        t: 0.28 + rand() * 0.44,
        half: 0.09 + rand() * 0.07,
        depth: Math.min(depth * (0.42 + rand() * 0.18), edgeLen * 0.075)
      });
    }

    const segments = [
      { ax: cTopL.x, ay: cTopL.y, bx: cTopR.x, by: cTopR.y, edgeIdx: 0, isSide: false, long: true },
      { ax: cTopR.x, ay: cTopR.y, bx: cRightT.x, by: cRightT.y, edgeIdx: 1, chamfer: true },
      { ax: cRightT.x, ay: cRightT.y, bx: cRightB.x, by: cRightB.y, edgeIdx: 2, isSide: true, long: false },
      { ax: cRightB.x, ay: cRightB.y, bx: cBotR.x, by: cBotR.y, edgeIdx: 3, chamfer: true },
      { ax: cBotR.x, ay: cBotR.y, bx: cBotL.x, by: cBotL.y, edgeIdx: 4, isSide: false, long: true },
      { ax: cBotL.x, ay: cBotL.y, bx: cLeftB.x, by: cLeftB.y, edgeIdx: 5, chamfer: true },
      { ax: cLeftB.x, ay: cLeftB.y, bx: cLeftT.x, by: cLeftT.y, edgeIdx: 6, isSide: true, long: false },
      { ax: cLeftT.x, ay: cLeftT.y, bx: cTopL.x, by: cTopL.y, edgeIdx: 7, chamfer: true }
    ];

    const ring = [];
    const longSlip = notchesEnabled ? slip : 0;
    const longKink = notchesEnabled ? kinkAmp : 0;
    const chamferSlip = notchesEnabled ? slip * 0.3 : 0;
    const trimSides = notchesEnabled ? sideInMax : 0;
    const trimTopBottom = notchesEnabled ? topBottomTrim : 0;
    const strokeOpts = (seg) => {
      const edgePoint = handCutTopBottomEdgePoint(
        seg.edgeIdx,
        seed,
        mainEdgeLen[seg.edgeIdx] || w,
        cfg
      );
      const longBulge = edgePoint?.longBulge || null;
      return {
        rand,
        cx,
        cy,
        slip: longBulge ? 0 : longSlip,
        kinkAmp: longKink,
        ...
        twoCuts: false,
        topBottomTrim:
          (seg.edgeIdx === 0 || seg.edgeIdx === 4) && !seg.chamfer ? trimTopBottom : 0,
        isSide: !!seg.isSide,
        sideInMax: trimSides,
        cleanLongEdge: !!longBulge
      };
    };
  ...
  function composePeekClippingCanvas(spread, cfg, opts, usePeekSides) {
    ...
    clipped = restrokeCenterBandRulesOnClipped(
      clipped,
      spread,
      cfg,
      usePeekSides ? 'peek' : 'center'
    );
    const dateKey =
    ...
    clipped = trimCanvasAlphaBounds(clipped, cfg, 3, {
      preserveWidth: !!usePeekSides,
      bleedTopDomPx: 0,
      bleedBottomDomPx: 3
    });
    const skipTopInkTrim =
      cfg.handCutEnabled !== false && cfg.handCutTopBottomEdgePointEnabled !== false;
    const trimmed = skipTopInkTrim
      ? clipped
      : trimCanvasTopInk(clipped, cfg, {
          bleedTopDomPx: 1,
  ...
  function composeFullColumnsClippingCanvas(spread, cfg, opts) {
    ...
    if (!clipped) return null;
    clipped = restrokeCenterBandRulesOnClipped(clipped, spread, cfg, 'full');
    const dateKey =
      String(opts.dateKey || '').trim() ||
      String(opts.today?.dateKey || '').trim() ||
      '';
    ...
    return trimCanvasTopInk(clipped, cfg, {
      bleedTopDomPx: 1,
      usePeekSides: true,
      peekRatio: cfg.peekRatio
    });
  ...
  function verticalCropBleedSrcPx(cfg, scale = 1) {
    ...
    return {
      top: Math.ceil(domToExportPx(cfg, Math.max(0, topDom)) / safeScale),
      bottom: Math.ceil(domToExportPx(cfg, Math.max(0, bottomDom)) / safeScale)
    };
  }
  ...
  async function composeDataUrlAttempt(opts = {}) {
    ...
    let cfg = resolveCenterBodyPxForHandCut(mctx, opts, { ...DEFAULTS, ...opts });
    ...
    const layoutOpts = triptychQuote
