...
function (global) {
  ...
  /** Line 1 must be exactly the first `cap` quote words — not just a string match. */
  function firstWrappedLineMatchesWordCap(wrapped, cap, targetWords) {
    ...
    const lineText = firstWrappedLineText(wrapped);
    if ((/[.;]\s*$/.test(lineText) || /[.;]$/.test(lineText)) && got.length < want.length) {
  ...
  function fitCenterColumnWidth(mctx, runs, px, padOpts, cfg, firstLineMaxWords) {
    ...
    for (let tryW = innerLo; tryW <= maxInner; tryW += 1) {
      const wrapped = wrapRuns(mctx, runs, tryW, px);
      if (firstWrappedLineMatchesWordCap(wrapped, cap, words)) {
        foundLo = tryW;
        break;
      }
    }
    if (foundLo < 0) {
      ...
      for (let tryW = innerLo - 1; tryW >= floorW; tryW -= 1) {
        const wrapped = wrapRuns(mctx, runs, tryW, px);
        if (firstWrappedLineMatchesWordCap(wrapped, cap, words)) {
          foundLo = tryW;
          break;
