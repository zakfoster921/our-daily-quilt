...
function (global) {
  ...
  function resolveCarouselSpeakerSeamRect(
    ...
    let y;
    if (quiltAnchored) {
      const quiltLeft = Math.round(Number(quiltRect.x));
      const quiltBottom = Math.round(Number(quiltRect.y) + Number(quiltRect.height));
      x = quiltLeft;
      y = quiltBottom - h;
    } else {
    ...
    if (alphaMetrics) {
      ...
      if (quiltAnchored) {
        const quiltLeft = Math.round(Number(quiltRect.x));
        const quiltBottom = Math.round(Number(quiltRect.y) + Number(quiltRect.height));
        x = Math.round(quiltLeft - alphaMetrics.opaqueBottomLeftX * sx);
        y = Math.round(quiltBottom - alphaMetrics.opaqueBottomLeftY * sy);
      }
