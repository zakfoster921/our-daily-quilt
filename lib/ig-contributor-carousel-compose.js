...
function (global) {
  ...
  function resolveQuiltDrawBounds(quiltImg, panelX, panelW, panelH, minLeftGutterPx = 0) {
    ...
    const gutter = Math.max(0, Math.round(Number(minLeftGutterPx) || 0));
    const maxQuiltW = Math.max(1, innerW - gutter);
    const scale = Math.min(maxQuiltW / iw, innerH / ih);
  ...
  function resolveMirroredQuiltExtensionPlacement(quiltImg, quiltBounds, options = {}) {
    ...
    return {
      ...
      sliceSrcH: ih,
      destX: 0,
      destY: quiltBounds.dy,
      destW,
      destH: quiltBounds.dh,
      scale
    };
  }

  /** Right-edge source strip, mirrored L→R, left-justified on slide 1 (behind clipping). */
  function drawMirroredQuiltExtension(ctx, quiltImg, quiltBounds, options = {}) {
    const placement = resolveMirroredQuiltExtensionPlacement(quiltImg, quiltBounds, options);
  ...
  function drawRightJustifiedQuilt(ctx, quiltImg, panelX, panelW, panelH, minLeftGutterPx = 0, boundsOverride) {
    const bounds =
      boundsOverride || resolveQuiltDrawBounds(quiltImg, panelX, panelW, panelH, minLeftGutterPx);
  ...
  async function composeInstagramContributorCarouselFromQuiltBlob(
    ...
    const dateStripResult =
      typeof composeDateStrip === 'function'
        ? await composeDateStrip({
            dateKey,
            width: clippingWidth,
            paperTextureUrl: options.paperTextureUrl
          })
    ...
    const [quiltImg, clippingImg, dateStripImg] = await Promise.all([
      loadImageFromBlob(quiltBlob),
      loadImageFromDataUrl(clippingDataUrl),
      dateStripDataUrl ? loadImageFromDataUrl(dateStripDataUrl) : Promise.resolve(null)
    ]);
    ...
    const quiltBounds = resolveQuiltDrawBounds(quiltImg, PANEL_W, PANEL_W, MASTER_H, seamOverlapPx);
    const mirrorPlacement = drawMirroredQuiltExtension(ctx, quiltImg, quiltBounds, options);
    drawRightJustifiedQuilt(ctx, quiltImg, PANEL_W, PANEL_W, MASTER_H, seamOverlapPx, quiltBounds);
    ...
    const dateOpaque = dateStripImg ? measureOpaqueBoundsFromImage(dateStripImg) : null;
    ...
    if (stack.dateStrip && dateStripImg && dateOpaque) {
      ctx.drawImage(
        dateStripImg,
        dateOpaque.x,
        dateOpaque.y,
