...
function (global) {
  ...
  function drawRightJustifiedQuilt(ctx, quiltImg, panelX, panelW, panelH, minLeftGutterPx = 0) {
    const bounds = resolveQuiltDrawBounds(quiltImg, panelX, panelW, panelH, minLeftGutterPx);
  ...
  async function composeInstagramContributorCarouselFromQuiltBlob(
    ...
    const quiltBounds = drawRightJustifiedQuilt(ctx, quiltImg, PANEL_W, PANEL_W, MASTER_H, seamOverlapPx);
    ...
    const placement = resolveContributorClippingPlacement(opaque, seamOverlapPx, quiltBounds.dx);
    ctx.save();
    ctx.beginPath();
