...
function (global) {
  ...
  /** Slide 3: full quilt on bold warm off-white mat over shared cover bg. */
  const SLIDE3_MATTE_FILL = '#f6f4f1';
  const SLIDE3_BG_GUTTER_PX = 24;
  const SLIDE3_MAT_BORDER_PX = 56;
  ...
  /** Slide 3: shared cover bg + entire quilt contained on bold off-white mat. */
  function drawSlide3FramedFullQuilt(ctx, quiltImg, quiltBgRect, panelW, panelH, options = {}) {
    ...
    const gutter = Math.max(12, Math.round(Number(options.slide3BgGutterPx) || SLIDE3_BG_GUTTER_PX));
    const matBorder = Math.max(
      32,
      Math.round(Number(options.slide3MatBorderPx) || SLIDE3_MAT_BORDER_PX)
    );
    const matteFill = String(options.slide3MatteFill || SLIDE3_MATTE_FILL).trim() || SLIDE3_MATTE_FILL;

    const matX = gutter;
    const matY = gutter;
    const matW = Math.max(1, panelW - gutter * 2);
    const matH = Math.max(1, panelH - gutter * 2);

    ctx.fillStyle = matteFill;
    ctx.fillRect(matX, matY, matW, matH);

    const innerX = matX + matBorder;
    const innerY = matY + matBorder;
    const innerW = Math.max(1, matW - matBorder * 2);
    const innerH = Math.max(1, matH - matBorder * 2);
    ...
    const dx = Math.round(innerX + (innerW - dw) / 2);
    const dy = Math.round(innerY + (innerH - dh) / 2);
  ...
  function resolveContributorClippingPlacement(bounds, seamOverlapPx, _quiltLeftX, placementOpts = {}) {
    ...
    const maxRight = straddleSeam ? quiltCap : Math.min(seamRight, quiltCap);
    const minDestX =
  ...
  async function composeInstagramContributorCarouselFromQuiltBlob(
    ...
    if (Number.isFinite(titleEndX) && titleEndX > 0) {
      placementOpts.titleEndX = titleEndX;
      if (Number.isFinite(titlePx) && titlePx > 0) {
        placementOpts.titleSlackPx = Math.max(8, Math.round(titlePx * 0.1));
      }
    }
