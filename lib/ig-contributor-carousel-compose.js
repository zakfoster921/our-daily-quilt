/**
 * IG carousel slides 2–3: slide 2 = framed quilt + title strip (speaker seam from slide 1);
 * slide 3 = contributor clipping + date strip on quilt bg.
...
function (global) {
  ...
  /** Slide 2 (IG): framed quilt + winning-quilt title strip; speaker seam bleeds on left edge. */
  ...
  async function composeInstagramContributorCarouselFromQuiltBlob(
    ...
    drawSharedQuiltBg(slide3Ctx, quiltImg, quiltRect, PANEL_W, MASTER_H, { flip: true });
    ...
    const gapPx =
      Number(options.dateStripGapPx) > 0
        ? Math.round(Number(options.dateStripGapPx))
        : DATE_STRIP_GAP_PX;
    ...
    const widestClipping = Math.max(opaque.width, dateOpaque?.width || 0);
    const naturalStackH = opaque.height + (dateOpaque ? gapPx + dateOpaque.height : 0);
    const clippingScale = Math.min(
      1,
      maxStackW / Math.max(1, widestClipping),
      maxStackH / Math.max(1, naturalStackH)
    );
    const scaledGapPx = Math.max(0, Math.round(gapPx * clippingScale));
    const drawOpaque = {
      x: 0,
      y: 0,
    ...
    return {
      ...
      meta: {
        ...
        carouselQuiltBgMode: 'layout-b-quilt-then-clipping',
  ...
  async function duplicateClassicToCarouselSlides(classicDataUrl, options = {}) {
    ...
    const slide2Ctx = slide2Canvas.getContext('2d');
    if (!slide2Ctx) return null;
    drawSlide3FramedFullQuilt(slide2Ctx, img, quiltRect, PANEL_W, MASTER_H, options);
    ...
    const slide3Ctx = slide3Canvas.getContext('2d');
    if (!slide3Ctx) return null;
    drawSharedQuiltBg(slide3Ctx, img, quiltRect, PANEL_W, MASTER_H, { flip: true });
    ...
    return {
      slide1Blob,
      slide2Blob,
      meta: { fallbackClassic: true, carouselQuiltBgMode: 'layout-b-quilt-then-clipping' }
    };
  }
