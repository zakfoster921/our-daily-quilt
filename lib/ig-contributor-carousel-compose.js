...
function (global) {
  ...
  /** White gutter between contributor clipping and quilt (both sides of the slide 2–3 seam). */
  const QUILT_CLIPPING_GAP_PX = 8;
  ...
  /** Headline ink must stay on slide 1 when not straddling the seam. */
  const CONTRIBUTOR_TITLE_SLIDE1_MARGIN_PX = 20;
  /** Integrated carousel: center contributor clipping on slide 2–3 seam (master x). */
  ...
  function resolveContributorClippingPlacement(bounds, seamOverlapPx, quiltLeftX, placementOpts = {}) {
    ...
    const quiltCap = Math.max(
      PANEL_W,
      Math.round(Number(quiltLeftX) || seamRight) - quiltMargin
    );
    const straddleSeam =
  ...
  async function composeInstagramContributorCarouselFromQuiltBlob(
    ...
    if (placement.straddleSeam) {
      ctx.fillStyle = '#ffffff';
      ctx.fillRect(CAROUSEL_SEAM_X - QUILT_CLIPPING_GAP_PX, 0, QUILT_CLIPPING_GAP_PX, MASTER_H);
    }
    ctx.restore();
