...
function (global) {
  ...
  function drawQuiltNameStrip(ctx, name, centerX, centerY, tiltRad) {
  ...
  function drawSlide3FramedFullQuilt(ctx, quiltImg, quiltBgRect, panelW, panelH, options = {}) {
    ...
    if (quiltName) {
      const stripH = SLIDE3_NAME_STRIP_FONT_PX + SLIDE3_NAME_STRIP_PAD_Y * 2;
      // Top-left, half the strip height above the card top so it overlaps the frame
      const stripCX = cardX + 48;
  ...
  async function composeInstagramContributorCarouselFromQuiltBlob(
    ...
    const maxClippingW = Math.max(1, PANEL_W - CONTRIBUTOR_CLIPPING_SAFE_MARGIN_PX * 2);
