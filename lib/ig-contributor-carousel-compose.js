...
function (global) {
  ...
  /** Bottom-left of the quilt rect as it appears on the slide (not off-canvas bleed). */
  function resolveQuiltVisibleBottomLeftAnchor(quiltRect, panelW, panelH, minPad = 8) {
    ...
    if (![qx, qy, qw, qh].every(Number.isFinite) || qw <= 0 || qh <= 0) {
      return { x: minPad, y: panelH - minPad };
    }
    const pad = Math.max(0, Number(minPad) || 0);
    const quiltRight = qx + qw;
    const quiltBottom = qy + qh;
    const visLeft = Math.max(pad, Math.min(panelW - pad, qx));
    const visBottom = Math.min(panelH - pad, quiltBottom);
    if (visBottom <= pad || visLeft >= panelW - pad) {
      return { x: pad, y: panelH - pad };
    }
    return { x: Math.round(visLeft), y: Math.round(visBottom) };
  }
  ...
  function resolveCarouselSpeakerSeamRect(
    ...
    let y;
    if (quiltAnchored) {
      const anchor = resolveQuiltVisibleBottomLeftAnchor(quiltRect, panelW, panelH, minPad);
      x = anchor.x;
      y = anchor.y - h;
    } else {
    ...
    if (alphaMetrics) {
      ...
      if (quiltAnchored) {
        const anchor = resolveQuiltVisibleBottomLeftAnchor(quiltRect, panelW, panelH, minPad);
        x = Math.round(anchor.x - alphaMetrics.opaqueBottomLeftX * sx);
        y = Math.round(anchor.y - alphaMetrics.opaqueBottomLeftY * sy);
      }
  ...
  async function applyCarouselSpeakerSeamOverlap(options = {}) {
    ...
    const speakerDrawOpts = {
      ...
      palette:
        options.palette ||
        global.app?.getSpeakerCutoutPaperPalette?.() ||
