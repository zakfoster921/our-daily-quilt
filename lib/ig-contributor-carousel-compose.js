...
function (global) {
  ...
  /** Slide 4 quilt-only: shrink framed card so green mat peeks above/below. */
  const QUILT_ONLY_SLIDE4_QUILT_SCALE = 0.86;
  ...
  /** Temporary slide 4: green cutting mat + centered framed quilt (no strips, speaker, or stats card). */
  async function composeCarouselQuiltOnlyMatSlideFromQuiltBlob(quiltBlob, options = {}) {
    ...
    const MatBg = global.CarouselCuttingMatBg || globalThis.CarouselCuttingMatBg;
    const cuttingMatImg = MatBg?.loadCarouselCuttingMatImage
      ? await MatBg.loadCarouselCuttingMatImage()
      : null;
    ...
    const frameMeta = drawSlide3FramedFullQuilt(ctx, quiltImg, quiltRect, PANEL_W, MASTER_H, {
      ...options,
      omitBackgroundQuilt: true,
      winningQuiltName: '',
      slide3QuiltScale:
        Number(options.slide3QuiltScale) > 0
          ? Number(options.slide3QuiltScale)
          : QUILT_ONLY_SLIDE4_QUILT_SCALE
    });
    ...
    return {
      ...
      meta: {
        panelWidth: PANEL_W,
        panelHeight: MASTER_H,
        carouselQuiltBgMode: 'cutting-mat-quilt-only',
        cuttingMatAsset: cuttingMatImg ? 'loaded' : 'procedural-fallback',
        carouselMatPan: MatBg?.CAROUSEL_MAT_PAN?.SLIDE_4 ?? PANEL_W * 3,
