...
(function (root, factory) {
  ...
  if (root) {
    ...
    root.composeCarouselQuiltOnlyMatSlideFromQuiltBlob = api.composeCarouselQuiltOnlyMatSlideFromQuiltBlob;
  }
...
function (global) {
  ...
  function drawSlide3FramedFullQuilt(ctx, quiltImg, quiltBgRect, panelW, panelH, options = {}) {
    if (!options.omitBackgroundQuilt) {
  ...
  /** Visible quilt card bounds on the slide (ignores off-canvas bleed). */
  function resolveQuiltVisibleBounds(quiltRect, panelW, panelH) {
    ...
    if (![qx, qy, qw, qh].every(Number.isFinite) || qw <= 0 || qh <= 0) {
      return { left: 0, right: panelW, bottom: panelH, centerX: panelW / 2, width: panelW };
    }
    const left = Math.max(0, Math.round(qx));
    const right = Math.min(panelW, Math.round(qx + qw));
    const bottom = Math.min(panelH, Math.round(qy + qh));
  ...
  function resolveCarouselSpeakerSeamRect(
    ...
    const quiltBox = quiltAnchored
      ? resolveQuiltVisibleBounds(quiltRect, panelW, panelH)
      : null;
    if (quiltAnchored && quiltBox) {
      x = Math.round(quiltBox.centerX - w / 2);
      y = Math.round(quiltBox.bottom - h);
    } else {
    ...
    if (alphaMetrics) {
      ...
      const sy = h / ih;
      if (quiltAnchored && quiltBox) {
        /** Bottom-center of visible cutout ink on quilt bottom edge (not canvas). */
        const contentBottomCenterX = alphaMetrics.minX + alphaMetrics.width / 2;
        x = Math.round(quiltBox.centerX - contentBottomCenterX * sx);
  ...
  /** Temporary slide 4: green mat + layout-B quilt styling (same paper depth as slide 1). */
  async function composeCarouselQuiltOnlyMatSlideFromQuiltBlob(quiltBlob, options = {}) {
    ...
    const matPeekPx =
      Number(options.quiltMatPeekPx) > 0
        ...
        : CQB.CAROUSEL_SLIDE4_MAT_PEEK_PX ?? CQB.CAROUSEL_SLIDE1_MAT_PEEK_PX ?? 48;
    const matPanX = MatBg?.CAROUSEL_MAT_PAN?.SLIDE_4 ?? PANEL_W * 3;
    ...
    CQB.drawLayoutBCarouselQuiltBg(ctx, quiltImg, quiltCanvasRect, PANEL_W, MASTER_H, {
      greenCuttingMatUnderQuilt: true,
      quiltMatPeekPx: matPeekPx,
      cuttingMatImg,
      paperTextureImg,
      paperToothSeed: `${dateKey}:carousel-slide-4`,
      matPanX,
      topCastShadow: {
        offsetY: 8,
        bandUp: 12,
    ...
    return {
      ...
      meta: {
        panelWidth: PANEL_W,
        panelHeight: MASTER_H,
        carouselQuiltBgMode: 'cutting-mat-layout-b-quilt',
        quiltCanvasRect,
        quiltMatPeekPx: matPeekPx,
        cuttingMatAsset: cuttingMatImg ? 'loaded' : 'procedural-fallback',
        paperTextureAsset: paperTextureImg ? 'loaded' : 'missing',
        carouselMatPan: matPanX,
        dateKey
      }
    };
  }
  ...
  return {
    ...
    composeCarouselQuiltOnlyMatSlideFromQuiltBlob,
