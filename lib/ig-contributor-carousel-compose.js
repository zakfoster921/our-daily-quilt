...
function (global) {
  ...
  /** Nudge framed quilt right so the winning title strip has more room at the seam. */
  const SLIDE3_FRAME_OFFSET_X_PX = 34;
  /** Keep the winning title strip close to the quilt's left border. */
  const SLIDE3_NAME_STRIP_RIGHT_EDGE_INSET_PX = 8;
  ...
  function drawSlide3FramedFullQuilt(ctx, quiltImg, quiltBgRect, panelW, panelH, options = {}) {
    ...
    const targetScale = scale * quiltScale;
    let dw = Math.round(iw * targetScale);
    let dh = Math.round(dw / sourceAspect);
    if (dh > Math.round(ih * targetScale)) {
      dh = Math.round(ih * targetScale);
      dw = Math.round(dh * sourceAspect);
    }
    const cardW = dw + framePx * 2;
    const cardH = dh + framePx * 2;
    const requestedOffsetX = Number(options.slide3FrameOffsetXPx);
    const frameOffsetX = Number.isFinite(requestedOffsetX)
      ? Math.round(requestedOffsetX)
      : SLIDE3_FRAME_OFFSET_X_PX;
    const cardX = Math.round((panelW - cardW) / 2 + frameOffsetX);
    ...
    if (quiltName) {
      const stripH = SLIDE3_NAME_STRIP_FONT_PX + SLIDE3_NAME_STRIP_PAD_Y * 2;
      // Anchor the strip at the quilt's lower-left border; left overflow is redrawn onto slide 2.
      const stripRightEdgeX = cardX + framePx + SLIDE3_NAME_STRIP_RIGHT_EDGE_INSET_PX;
      const stripCY = cardY + framePx + dh - Math.round(stripH * 0.5);
  ...
  async function composeInstagramContributorCarouselFromQuiltBlob(
    ...
    const slide3QuiltImg = options.slide3QuiltBlob
    ...
    const slide3Frame = drawSlide3FramedFullQuilt(
      ...
      PANEL_W,
      MASTER_H,
      { ...options, slide3BgQuiltImg: quiltImg }
    );

    // Draw the name strip's left overflow onto slide 2's right edge for a seamless seam
    const ns = slide3Frame?.nameStrip;
    if (ns && String(options.winningQuiltName || '').trim()) {
      const stripTiltRad = ((slide3Frame.tiltDeg || 0) * Math.PI) / 180;
      // centerX in slide-3 local coords; in slide-2 local coords it's PANEL_W + centerX
      drawQuiltNameStrip(slide2Ctx, String(options.winningQuiltName).trim(),
        PANEL_W + ns.centerX, ns.centerY, stripTiltRad);
    }
