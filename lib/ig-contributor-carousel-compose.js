...
function (global) {
  ...
  /** Fraction of clipping width that sits past the seam (0.5 = centered on seam). */
  const CAROUSEL_SEAM_STRADDLE_RIGHT_FRAC = 0.34;
  ...
  /** Inset from quilt edges when placing the title strip on slide 3. */
  ...
  const SLIDE3_NAME_STRIP_FONT_PX = 49;
  const SLIDE3_NAME_STRIP_PAD_X = 44;
  const SLIDE3_NAME_STRIP_PAD_Y = 19;
  const SLIDE3_NAME_STRIP_LETTER_SPACING_EM = 0.12;
  const SLIDE3_NAME_STRIP_GAP_PX = 27;
    ...
    if (straddleSeam) {
      const pastSeamFrac =
        ...
        Number(placementOpts.pastSeamFraction) <= 0.5
          ...
          : CAROUSEL_SEAM_STRADDLE_RIGHT_FRAC;
