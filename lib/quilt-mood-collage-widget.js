...
function (global) {
  ...
  const STYLE_ID = 'quilt-mood-collage-widget-styles-v44';
  const COLLAGE_BUILD = 'v152';
  ...
  function injectStyles() {
    ...
    const css = `
      ...
      #screen-quilt .quilt-mood-collage__tape {
        ...
        overflow: hidden;
        isolation: isolate;
        /* Gold plate before foil canvas — strips used to stay invisible until webps landed. */
        background-color: #c9a227;
      }

      #screen-quilt .quilt-mood-collage__tape.is-scratch-ready,
      #screen-quilt .quilt-mood-collage__tape.is-revealed,
  ...
  function mountTapeScratch(cfg) {
    ...
    return {
      ...
      setFoilAssets(nextFoil, nextMarks) {
        foilImg = nextFoil || foilImg;
        marksImg = nextMarks || marksImg;
        if (revealed) return;
  ...
  function mount(host, opts = {}) {
    ...
    engines.good = bindTapeEngine('good', tapeGood, null, null);
    engines.rough = bindTapeEngine('rough', tapeRough, null, null);
    enginesReady = true;
    tryApplyPendingMood();
    ...
    void loadTapeScratchAssets()
      .then(([goodFoilImg, goodMarksImg, roughFoilImg, roughMarksImg]) => {
        ...
        engines.good?.setFoilAssets?.(goodFoilImg, goodMarksImg);
        engines.rough?.setFoilAssets?.(roughFoilImg, roughMarksImg);
