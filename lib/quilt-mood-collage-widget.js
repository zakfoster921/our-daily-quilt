...
function (global) {
  ...
  function mount(host, opts = {}) {
    ...
    /** Quote sits above scratch strips — close to visual text, never overlapping tape stack. */
    function syncQuoteLayout() {
      ...
      const narrow = stageRect.width <= PENNY_NARROW_STAGE_MAX_PX;
      const insetBottom = QUOTE_TEXT_INSET_BOTTOM + (narrow ? 0.1 : 0.04);
      const quoteVisualHPx = quoteHPx * (1 - insetBottom);
      const quoteGap = stageRect.height * TAPE_LAYOUT_QUOTE_GAP;
      const maxQuoteBottomPx = tapeLayoutPx.goodTop - quoteGap;
      const desiredTopPx = tapeLayoutPx.goodTop - quoteGap - quoteVisualHPx;
      const maxTopPx = maxQuoteBottomPx - quoteHPx;
      let quoteTopPx = Math.min(desiredTopPx, maxTopPx);
      quoteTopPx = Math.max(0, quoteTopPx);

      host.style.setProperty('--collage-quote-top', `${(quoteTopPx / stageRect.height) * 100}%`);

      for (let i = 0; i < 40; i++) {
        const quoteBottomStage = quoteWrap.getBoundingClientRect().bottom - stageRect.top;
        if (quoteBottomStage <= maxQuoteBottomPx + 0.5) break;
        quoteTopPx -= Math.max(1, quoteBottomStage - maxQuoteBottomPx);
        if (quoteTopPx <= 0) {
          host.style.setProperty('--collage-quote-top', '0%');
          break;
        }
        host.style.setProperty('--collage-quote-top', `${(quoteTopPx / stageRect.height) * 100}%`);
      }
