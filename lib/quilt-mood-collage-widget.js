...
function (global) {
  ...
  const STYLE_ID = 'quilt-mood-collage-widget-styles-v45';
  const COLLAGE_BUILD = 'v153';
  ...
  function mountTapeScratch(cfg) {
    ...
    function ensureFoilMask() {
      if (foilMask || !scratchCtx || !foilDrawn || canvas.width < 1) return;
    ...
    function syncScratchCanvas({ forceFoil = false } = {}) {
      ...
      if (foilDrawn) zoneEl.classList.add('is-foil-live');
