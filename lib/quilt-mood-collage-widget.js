...
function (global) {
  ...
  const STYLE_ID = 'quilt-mood-collage-widget-styles-v43';
  const COLLAGE_BUILD = 'v151';
  ...
  function readFoilHorizontalSpan(foilImg, pxW, pxH) {
    if (!foilImg?.naturalWidth || pxW < 1 || pxH < 1) {
    ...
    if (!ctx) return { left: 0, width: pxW };
    ctx.drawImage(foilImg, 0, 0, pxW, pxH);
    const data = ctx.getImageData(0, 0, pxW, pxH).data;
    let minX = pxW;
    let maxX = 0;
    for (let y = 0; y < pxH; y++) {
      for (let x = 0; x < pxW; x++) {
        if (data[(y * pxW + x) * 4 + 3] >= 48) {
          minX = Math.min(minX, x);
          maxX = Math.max(maxX, x);
        }
      }
    }
    if (maxX <= minX) return { left: 0, width: pxW };
