...
(function (root, factory) {
  ...
  // Styles before mount so the HTML yellow-card shell can paint on first scroll.
  try {
    api.injectStyles?.();
  } catch (_) {
    /* */
...
function (global) {
  ...
  const STYLE_ID = 'quilt-mood-collage-widget-styles-v46';
  const COLLAGE_BUILD = 'v154';
  ...
  const COLLAGE_SHOW_PENNY = false;
  const COLLAGE_QUOTE_DISPLAY_SCALE = 1;
  ...
  /** Stamp tint drawn onto mark canvas before CSS blend/filter. */
  const TAPE_MARK_TINT = 'rgba(38, 28, 20, 0.58)';
  ...
  const ASSETS = {
    ...
    tapeGoodFoil: `${ASSET_BASE}/tape-good-foil.webp?${COLLAGE_BUILD}`,
    tapeGoodMarks: `${ASSET_BASE}/tape-good-marks.webp?${COLLAGE_BUILD}`,
    tapeRoughFoil: `${ASSET_BASE}/tape-rough-foil.webp?${COLLAGE_BUILD}`,
    tapeRoughMarks: `${ASSET_BASE}/tape-rough-marks.webp?${COLLAGE_BUILD}`,
  ...
  function injectStyles() {
    ...
    const css = `
      #screen-quilt .quilt-mood-collage {
        ...
        --collage-tape-mark-opacity: 0.86;
        --collage-tape-mark-filter: brightness(0.7) contrast(1.14) saturate(0.78);
        position: relative;
        display: block;
        width: 100%;
      ...
      /* Native: CSS filters force expensive offscreen raster while scrolling. */
      html.odq-capacitor-native #screen-quilt .quilt-mood-collage {
        filter: none;
      }
      html.odq-capacitor-native #screen-quilt .quilt-mood-collage__mood-card-wrap,
      html.odq-capacitor-native #screen-quilt .quilt-mood-collage__quote-wrap,
      html.odq-capacitor-native #screen-quilt .quilt-mood-collage__tape {
        filter: none;
      }

      #screen-quilt .quilt-mood-collage[hidden] { display: none !important; }

      /* Keep the yellow card visible while size settles — hiding until ready left a blank scroll gap. */
      ...
      #screen-quilt .quilt-mood-collage__layer.quilt-mood-collage__mood-card-wrap {
        ...
        z-index: 2;
        display: block;
        /* Keep a yellow plate even if the webp is slow/fails — strips used to float alone. */
        aspect-ratio: 928 / 607;
      ...
      #screen-quilt .quilt-mood-collage__penny-tape {
        display: none !important;
      ...
      #screen-quilt .quilt-mood-collage__or-strip {
        ...
        font-weight: 700;
        font-size: clamp(0.85rem, 3.15vw, 1.09rem);
      ...
      #screen-quilt .quilt-mood-collage__tape-foil {
        position: absolute;
        inset: 0;
        width: 100%;
        height: 100%;
      ...
      #screen-quilt .quilt-mood-collage__tape.is-foil-live .quilt-mood-collage__tape-foil,
      #screen-quilt .quilt-mood-collage__tape.is-revealed .quilt-mood-collage__tape-foil,
      #screen-quilt .quilt-mood-collage__tape.is-dismissed .quilt-mood-collage__tape-foil {
        visibility: hidden;
      }
      ...
      #screen-quilt .quilt-mood-collage__tape.is-foil-live .quilt-mood-collage__scratch-canvas {
        opacity: 1;
      }

      #screen-quilt .quilt-mood-collage__tape.is-foil-live .quilt-mood-collage__tape-marks {
        opacity: var(--collage-tape-mark-opacity, 0.86);
    ...
    if (preset > 0) {
      ...
      if (naturalW > 0 && naturalH > 0) {
        ...
        const effectiveBodyDomPx =
          Number(opts.quoteEffectiveBodyDomPx) ||
          Number(quoteWrap?.closest?.('#quiltMoodSpread')?.dataset?.quoteEffectiveBodyDomPx) ||
          0;
        const layoutOpts = {
  ...
  function isCapacitorNativeClient() {
    try {
      return typeof global.odqIsCapacitorNative === 'function' && global.odqIsCapacitorNative();
    } catch (_) {
      return false;
    }
  }

  function applyMoodCardMacroFreezeTint(host, { frozen = false, color = null, img: imgOpt = null } = {}) {
    if (!host) return;
    const img = imgOpt || moodCardImgEl(host);
    const hex = normalizeHexColor(color);
    host._moodCardRecolorToken = (host._moodCardRecolorToken || 0) + 1;
    const token = host._moodCardRecolorToken;
    const ensureYellowSrc = (el) => {
      if (!el) return;
      el.onload = () => host._moodSpreadWidget?.remeasure?.();
      if (!el.getAttribute('src') || el.dataset.moodCardBroken === '1') {
        el.dataset.moodCardBroken = '';
        el.src = ASSETS.moodCard;
      } else if (!String(el.src || '').includes('mood-card.webp') && !String(el.src || '').startsWith('data:')) {
        el.src = ASSETS.moodCard;
      }
    };
    // Native: skip canvas recolor — WKWebView often leaves the <img> blank while recolor runs/fails.
    if (!frozen || !hex || isCapacitorNativeClient()) {
      host.classList.remove('is-macro-frozen');
      if (img) {
        img.onload = () => host._moodSpreadWidget?.remeasure?.();
        img.onerror = () => {
          img.dataset.moodCardBroken = '1';
          img.removeAttribute('src');
          host.classList.add('is-mood-card-fallback');
        };
    ...
    void buildMoodCardRecolorDataUrl(hex)
      .then((url) => {
        ...
        const liveImg = imgOpt || moodCardImgEl(host);
        if (!liveImg) return;
        liveImg.onload = () => host._moodSpreadWidget?.remeasure?.();
        liveImg.onerror = () => {
          liveImg.src = ASSETS.moodCard;
        };
        liveImg.src = url;
      })
      .catch(() => {
        const liveImg = imgOpt || moodCardImgEl(host);
        if (liveImg) liveImg.src = ASSETS.moodCard;
      });
  }
  ...
  let tapeScratchAssetsPromise = null;
  ...
  function loadTapeScratchAssets() {
    if (!tapeScratchAssetsPromise) {
      tapeScratchAssetsPromise = Promise.all([
        loadImage(ASSETS.tapeGoodFoil, { highPriority: true }),
        loadImage(ASSETS.tapeGoodMarks),
        loadImage(ASSETS.tapeRoughFoil, { highPriority: true }),
        loadImage(ASSETS.tapeRoughMarks)
      ]).catch((err) => {
        tapeScratchAssetsPromise = null;
        throw err;
      });
    }
    return tapeScratchAssetsPromise;
  }
  ...
  function preloadAssets() {
    ...
    for (const [key, src] of Object.entries(ASSETS)) {
      const img = new Image();
      img.decoding = 'async';
      if (key === 'tapeGoodFoil' || key === 'tapeRoughFoil' || key === 'moodCard') {
        try {
          img.fetchPriority = 'high';
        } catch (_) {
          /* */
  ...
  /** Horizontal span of visible gold foil — marks stay inside this, not the full tap rect. */
  function readFoilHorizontalSpan(_foilImg, pxW, pxH) {
    if (pxW < 1 || pxH < 1) {
      return { left: 0, width: pxW };
    }
    return {
      left: Math.round(pxW * (10 / 480)),
      width: Math.max(1, Math.round(pxW * (459 / 480)))
    };
  }
  ...
  function mountTapeScratch(cfg) {
    ...
    let foilImg = cfg.foilImg || null;
    let marksImg = cfg.marksImg || null;
    let painting = false;
    let lastX = 0;
    let lastY = 0;
    let moveTotal = 0;
    let strokeCount = 0;
    let revealed = !!startCommitted;
    ...
    let scratchPersistDirty = false;

    function persistScratchNow(options = {}) {
      if (!scratchPersistEnabled || revealed || !scratchCtx) return;
      // Unload/destroy paths must not re-export unless strokes changed — canvas
      // export is sync and can stall reload for many seconds.
      if (options.onlyIfDirty === true && !scratchPersistDirty) return;
    ...
    function foilMaskMatchesCanvas() {
      return !!(
        foilMask &&
        canvas.width > 16 &&
        canvas.height > 16 &&
        foilMask.length === canvas.width * canvas.height
      );
    }

    function ensureFoilMask() {
      if (!scratchCtx || !foilDrawn || canvas.width <= 16 || canvas.height <= 16) return;
      if (foilMaskMatchesCanvas()) return;
      scratchCtx.setTransform(1, 0, 0, 1, 0, 0);
      foilMask = captureFoilMask(scratchCtx, canvas.width, canvas.height);
      const dpr = Math.min(global.devicePixelRatio || 1, 2);
      scratchCtx.setTransform(dpr, 0, 0, dpr, 0, 0);
    }

    function scratchAt(x, y) {
      if (foilImg && !foilImg.naturalWidth) return;
      if (!scratchCtx && !syncScratchCanvas({ forceFoil: true })) return;
      if (!foilDrawn && foilImg?.naturalWidth) {
    ...
    function drawTapeMarks(ctx, img, pxW, pxH, markSpan) {
      if (!ctx || !img?.naturalWidth) return;
      const span = markSpan || { left: 0, width: pxW };
      const maxW = Math.max(1, Math.round(span.width));
      const maxH = pxH;
      ...
      const drawX = Math.round(span.left + (maxW - drawW) * 0.5);
      const drawY = Math.round((pxH - drawH) * 0.5);
      ctx.save();
      ctx.drawImage(img, drawX, drawY, drawW, drawH);
      ctx.globalCompositeOperation = 'source-atop';
      ctx.fillStyle = TAPE_MARK_TINT;
    ...
    function drawFreshFoil(pxW, pxH, dpr) {
      ...
      drawTapeMarks(marksCtx, marksImg, pxW, pxH, readFoilHorizontalSpan(foilImg, pxW, pxH));
    ...
    function syncScratchCanvas({ forceFoil = false } = {}) {
      ...
      const shouldRedrawFoil =
        forceFoil || !foilDrawn || progressBefore < 0.005 || sizeChanged;
      ...
      if (foilDrawn && cssW > 8 && cssH > 8) zoneEl.classList.add('is-foil-live');
      else zoneEl.classList.remove('is-foil-live');
      if (foilMaskMatchesCanvas() && progressBefore >= SCRATCH_PERSIST_MIN_PROGRESS) {
        zoneEl.classList.add('is-scratch-progress');
      }
    ...
    function foilScratchProgress() {
      if (!foilMaskMatchesCanvas()) return 0;
    ...
    function onPointerUp(e) {
      ...
      // Defer export so a refresh right after scratching is less likely to hitch.
      scratchPersistDirty = true;
      const schedule =
        typeof global.requestIdleCallback === 'function'
          ? (cb) => global.requestIdleCallback(cb, { timeout: 1200 })
          : (cb) => global.setTimeout(cb, 0);
      schedule(() => persistScratchNow({ onlyIfDirty: true }));
    }
    ...
    function kickFoilFromImg() {
      if (revealed || !foilImg?.naturalWidth) return;
      syncScratchCanvas({ forceFoil: true });
    }
    if (foilImg) {
      if (foilImg.naturalWidth) {
        ...
        foilImg.addEventListener('load', kickFoilFromImg, { once: true });
        try {
          void foilImg.decode?.().then(kickFoilFromImg);
        } catch (_) {
          /* */
        }
      }
    ...
    return {
      ...
      revealInstant,
      persistScratchNow,
      needsPersistScratch: () => scratchPersistDirty,
      setFoilAssets(nextFoil, nextMarks) {
        if (nextFoil) foilImg = nextFoil;
        if (nextMarks) marksImg = nextMarks;
        if (revealed) return;
        persistScratchNow({ onlyIfDirty: true });
        syncScratchCanvas({ forceFoil: true });
      },
      reset() {
  ...
  function buildTapeZone(mood, message) {
    ...
    const foilEl = document.createElement('img');
    foilEl.className = 'quilt-mood-collage__tape-foil';
    foilEl.src = mood === 'good' ? ASSETS.tapeGoodFoil : ASSETS.tapeRoughFoil;
    foilEl.alt = '';
    foilEl.draggable = false;
    foilEl.decoding = 'sync';
    foilEl.loading = 'eager';
    try {
      foilEl.fetchPriority = 'high';
    } catch (_) {
      /* */
    }
    ...
    zone.append(surface, msg, foilEl, canvas, marksCanvas);
    return { zone, canvas, marksCanvas, msg, foilEl };
  }
  ...
  function collageHostWidthPx(hostEl) {
    ...
    if (stack) {
      ...
      if (stackRect && stackRect.width > 8) return stackRect.width;
      if (stack.offsetWidth > 8) return stack.offsetWidth;
    }

    const screen = hostEl?.closest?.('#screen-quilt');
    const vw = global.innerWidth || screen?.clientWidth || document.documentElement?.clientWidth || 0;
    let max = 460;
    let gutter = 16;
    if (screen && typeof global.getComputedStyle === 'function') {
      const cs = global.getComputedStyle(screen);
      max = parseFloat(cs.getPropertyValue('--quilt-float-card-max')) || max;
      gutter = parseFloat(cs.getPropertyValue('--quilt-float-card-gutter')) || gutter;
    }
    // Always size from viewport when host is still [hidden]/zero-width — native cold start.
    if (vw > 1) return Math.max(8, Math.min(max, vw - gutter * 2));
    return 390;
  }
  ...
  function mount(host, opts = {}) {
    ...
    if (duoQuoteWrap && !duoQuoteImg) {
      ...
      duoQuoteImg.loading = 'eager';
      try {
        duoQuoteImg.fetchPriority = 'high';
      } catch (_) {
        /* */
      }
    ...
    if (moodCardWrap) {
      ...
      moodCardImg.decoding = 'sync';
      moodCardImg.loading = 'eager';
      moodCardImg.fetchPriority = 'high';
    ...
    orWord.textContent = 'SCRATCH ONE';
    ...
    // Tint only after the card is in the host tree — earlier querySelector missed the img.
    if (moodCardImg) {
      ...
      try {
        if (typeof moodCardImg.decode === 'function') void moodCardImg.decode();
      } catch (_) {
        /* */
      }
    }
    ...
    function scheduleRemeasureCollage(force = false) {
      if (document.getElementById('postColorCutRevealHost')) return;
    ...
    function syncDuoQuoteLayout() {
      ...
      applyCollageQuoteDisplaySize(duoQuoteImg, duoQuoteWrap, {
        ...
        quoteEffectiveBodyDomPx: opts.quoteEffectiveBodyDomPx
      });
    }
    ...
    function syncPennyLayout() {
      if (!COLLAGE_SHOW_PENNY) {
        pennyImg.style.visibility = 'hidden';
        if (pennyTapeEl) pennyTapeEl.style.visibility = 'hidden';
        return;
      }
      const stageRect = stage.getBoundingClientRect();
    ...
    function bindTapeEngine(moodName, tape, foil, marks) {
      return mountTapeScratch({
        zoneEl: tape.zone,
        canvas: tape.canvas,
        marksCanvas: tape.marksCanvas,
        foilImg: foil,
        marksImg: marks,
        mood: moodName,
        reducedMotion,
        ...
        scratchPersist,
        startCommitted: initialMood === moodName
      });
    }

    const mountGen = (Number(host._moodCollageMountGen) || 0) + 1;
    host._moodCollageMountGen = mountGen;
    engines.good = bindTapeEngine('good', tapeGood, tapeGood.foilEl, null);
    engines.rough = bindTapeEngine('rough', tapeRough, tapeRough.foilEl, null);
    enginesReady = true;
    tryApplyPendingMood();

    void loadTapeScratchAssets()
      .then(([, goodMarksImg, , roughMarksImg]) => {
        if (host._moodCollageMountGen !== mountGen) return;
        engines.good?.setFoilAssets?.(null, goodMarksImg);
        engines.rough?.setFoilAssets?.(null, roughMarksImg);
    ...
    function applyQuoteImage(
      nextUrl,
      { quoteDisplayWidthPx = 0, quoteRenderWidth = 0, quoteEffectiveBodyDomPx = 0, onReady } = {}
    ) {
      ...
      if (quoteEffectiveBodyDomPx > 0) opts.quoteEffectiveBodyDomPx = quoteEffectiveBodyDomPx;
    ...
    if (quiltScreen && typeof MutationObserver !== 'undefined') {
      screenObserver = new MutationObserver(() => {
        // Cut-reveal mat owns the main thread — skip remasure while it's up.
        if (document.getElementById('postColorCutRevealHost')) return;
    ...
    // Do not flush scratch masks on pagehide/visibility — exportEraserMaskDataUrl
    // is sync and blocks reload. Strokes already persist via debounced pointerup saves.
