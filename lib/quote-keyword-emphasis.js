...
const SPEAKER_GUIDE_KEYWORD_PROMPT = [
  'Pick 2–4 key words or short phrases from the speaker guideline below.',
  'Each entry must be an exact substring of the guideline (same words, same order, same spelling).',
  'Pick load-bearing words: what they did, cared about, or are known for — concrete nouns/verbs, not glue words.',
  'Phrases may be 2–4 words when they appear verbatim in the guideline.',
  'Exclude articles (a, an, the) and conjunctions unless part of a meaningful phrase.',
  'Aim for 2–4 entries — only add more when each carries equal weight.',
  'List entries in the order they appear in the guideline (left to right).',
  'Do not change any wording. Return only JSON: {"keywords":["heritage","identity","memory"]}.'
].join('\n');

function buildSpeakerGuideKeywordPrompt(guideText) {
  const g = String(guideText || '').replace(/\s+/g, ' ').trim();
  return `${SPEAKER_GUIDE_KEYWORD_PROMPT}\n\nSpeaker guideline:\n"${g}"`;
}
...
function findPhraseIndexInQuote(quoteText, phrase) {
  const exact = findPhraseInQuote(quoteText, phrase);
  if (!exact) return null;
  const original = String(quoteText || '');
  const normalized = normalizeQuoteForKeywordMatch(original);
  const pat = phraseRegexFromText(normalizeQuoteForKeywordMatch(exact));
  if (!pat) return null;
  const re = new RegExp(pat, 'i');
  ...
  if (normalized.length === original.length) {
    return {
      exact: original.slice(m.index, m.index + m[0].length),
      index: m.index,
      len: m[0].length
    };
  }
  return { exact: m[0], index: m.index, len: m[0].length };
}

function normalizeEmphasisWords(words, quoteText, maxCount = 3) {
  const quote = String(quoteText || '');
  const cap = Math.max(1, parseInt(maxCount, 10) || 3);
  if (!quote.trim()) return [];

  const hits = [];
  const seenKeys = new Set();
  for (const item of parseEmphasisInputList(words)) {
    const key = phraseMatchKey(item);
    if (!key || seenKeys.has(key)) continue;
    const found = findPhraseIndexInQuote(quote, item);
    if (!found) continue;
    seenKeys.add(phraseMatchKey(found.exact));
    hits.push({
      exact: found.exact,
      key: phraseMatchKey(found.exact),
      index: found.index,
      len: found.len
    });
  }

  // Resolve overlaps by preferring longer phrases, then emit left-to-right.
  hits.sort((a, b) => b.len - a.len || a.index - b.index);

  const picked = [];
  for (const hit of hits) {
    const start = hit.index;
    const end = start + hit.len;
...
function suggestKeywordsHeuristic(quoteText, dateKey, maxCount = 3) {
  ...
  const cap = short ? 1 : Math.max(1, parseInt(maxCount, 10) || 3);
  const candidates = [];
...
function collectPhraseMatchesInLine(line, emphasisTargets) {
  ...
  for (const t of targets) {
    const pat = phraseRegexFromText(normalizeQuoteForKeywordMatch(t.text));
    if (!pat) continue;
