...
class ArchiveService {
  ...
  async getInstagramQuiltSourceBlob(blocks, options = {}) {
    ...
    console.log('[LB-DEBUG] mirror-check: quiltBlob=', quiltBlob?.size, 'fromLiveSvg=', quiltBlobFromLiveSvg, 'hasComposeFn=', typeof window?.app?.composeMirroredQuiltFieldBlob === 'function');
    if (
      quiltBlob &&
      ...
      try {
        console.log('[LB-DEBUG] mirror-start');
        ...
        console.log('[LB-DEBUG] mirror-done: mirrored=', mirrored?.size);
        if (mirrored) {
          quiltBlob = mirrored;
          usedMirroredComposeFallback = true;
        }
      } catch (mirrorErr) {
        console.log('[LB-DEBUG] mirror-err:', mirrorErr?.message);
  ...
  async _buildLayoutBStoryComposeOptions(blocks, quoteObj, dateKey, buildOptions = {}) {
    ...
    if (typeof globalThis.odqReadLayoutBPostStripPlan === 'function') {
      try {
    ...
    const speakerCutoutUrl = preloadedSpeaker
  ...
  async generateInstagramStoryLayoutBBlob(blocks, quoteObj = null, dateKey = null, options = {}) {
    if (!blocks || blocks.length === 0) return null;
    console.log('[LB-DEBUG] 1: start, blocks=', blocks.length, 'forPreview=', options.forPreview);
    await globalThis.ensureLayoutBComposeReady?.();
    console.log('[LB-DEBUG] 2: ensureLayoutBComposeReady done');
    const dk = dateKey || Utils.getTodayKey();
    ...
    console.log('[LB-DEBUG] 3: waiting for optionsPromise + sourceBlobPromise');
    const [{ quoteText, quoteAuthor, composeOptions }, highResBlob] = await Promise.all([
      optionsPromise,
      sourceBlobPromise
    ]);
    console.log('[LB-DEBUG] 4: highResBlob=', highResBlob?.size, highResBlob?.type, 'quoteText=', String(quoteText || '').slice(0, 40));
    if (!highResBlob) return null;
    ...
    const isNativeCtx =
      ...
      typeof window.odqIsCapacitorNative === 'function' &&
      window.odqIsCapacitorNative();
    console.log('[LB-DEBUG] 5: isNative=', isNativeCtx, 'converting to dataUrl=', isNativeCtx);
    const quiltSource = isNativeCtx ? await this._blobToDataUrl(highResBlob) : highResBlob;
    console.log('[LB-DEBUG] 6: quiltSource type=', typeof quiltSource, typeof quiltSource === 'string' ? quiltSource.slice(0, 40) : quiltSource?.size);
    const compose = globalThis.composeInstagramLayoutBFromQuiltBlob;
    if (typeof compose !== 'function') {
      throw new Error('composeInstagramLayoutBFromQuiltBlob missing (app module must finish loading)');
    }
    console.log('[LB-DEBUG] 7: compose fn found, ensuring fonts');
    if (typeof globalThis.ensureOdqCanvasFontsReady === 'function') {
      await globalThis.ensureOdqCanvasFontsReady();
    }
    console.log('[LB-DEBUG] 8: fonts ready, ensuring speaker assets');
    const SCR = globalThis.SpeakerCutoutRender;
    ...
    console.log('[LB-DEBUG] 9: calling compose...');
    const storyBlob = await compose(quiltSource, quoteText, quoteAuthor, 1080, 1920, dk, composeOptions);
    console.log('[LB-DEBUG] 10: compose done, storyBlob=', storyBlob?.size);
    return storyBlob;
  }
