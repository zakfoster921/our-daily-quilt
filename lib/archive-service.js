...
class ArchiveService {
  ...
  async getInstagramQuiltSourceBlob(blocks, options = {}) {
    ...
    const cacheKey = `${String(options.dateKey || '').trim()}:${fingerprint}:${cacheVariant}`;
    if (
    ...
    const rasterOpts = {
      targetAspect: IG_QUILT_SCREEN_ASPECT,
    ...
    if (!quiltBlob) {
      quiltBlob = await this.generateQuiltRasterBlobFromBlocks(blocks, rasterOpts);
    }
    if (
      quiltBlob &&
  ...
  /** Same today-only payload as quilt mood triptych (not 3-column neighbor peek). */
  ...
  async _prepareSpeakerImageUrlForCanvas(speakerImageUrl, options = {}) {
    ...
    const proxyToDataUrl = async (url) => {
      ...
      if (this._proxyImageFailedUrls.has(source)) throw new Error('Proxy skipped (cached failure)');
      ...
      const baseUrl =
        typeof CONFIG !== 'undefined' && CONFIG.BACKEND && CONFIG.BACKEND.baseUrl
          ...
          : '';
      if (!baseUrl) return '';
      const res = await fetch(`${baseUrl}/api/proxy-image?url=${encodeURIComponent(source)}`, {
        cache: 'no-store'
      });
      if (!res.ok) {
  ...
  async _buildLayoutBPostComposeOptions(blocks, quoteObj, dateKey) {
    ...
    try {
      keywordEmphasis = await odqReadLayoutBKeywordEmphasis(dk, 'post');
    } catch (_) {
      /* optional */
    }
    ...
    const speakerPortraitUrl = this._layoutBSpeakerPortraitUrl(q);
    ...
    if (speakerImageUrl) {
      const speakerImageForCanvas = await this._prepareSpeakerImageUrlForCanvas(speakerImageUrl, {
      ...
      const presetTransform = await odqSpeakerCutoutTransformForDateAsync(dk, 'post');
    ...
    return { quoteText, quoteAuthor, composeOptions, hasSpeaker: !!composeOptions.speakerOverlay };
  }

  async _buildLayoutBStoryComposeOptions(blocks, quoteObj, dateKey) {
    const dk = dateKey || Utils.getTodayKey();
    const q = await this._quoteForLayoutBSpeakerImage(quoteObj, dk);
    const quoteText = String(q.text ?? q.body ?? '').trim();
    const quoteAuthor = String(q.author ?? '').trim();
    ...
    if (speakerImageUrl) {
      ...
      const presetTransform = await odqSpeakerCutoutTransformForDateAsync(dk, 'story');
      composeOptions.speakerOverlay = {
        enabled: true,
        imageUrl: speakerImageForCanvas,
        name: speakerName,
   ...
   */
  async generateInstagramStoryLayoutBBlob(blocks, quoteObj = null, dateKey = null) {
    if (!blocks || blocks.length === 0) return null;
    const dk = dateKey || Utils.getTodayKey();
    const { quoteText, quoteAuthor, composeOptions } = await this._buildLayoutBStoryComposeOptions(
      blocks,
      quoteObj,
      dk
    );
    const highResBlob = await this.getInstagramQuiltSourceBlob(blocks, {
      dateKey: dk,
      exportModeLabel: 'layout_b_story_9x16_source'
    });
  ...
  async generateInstagramPostLayoutBBlob(blocks, quoteObj = null, dateKey = null, { requireSpeaker = false } = {}) {
    ...
    const highResBlob = await this.getInstagramQuiltSourceBlob(blocks, {
      dateKey: dk,
      exportModeLabel: hasSpeaker
        ? 'layout_b_speaker_post_9x16_cover_source'
  ...
  /**
   * Instagram feed post (4:5,1080×1350): Layout B with speaker cutout when available.
   */
          ...
          <div class="archive-post-content">
            <div class="archive-post-public">
              ...
              <div class="archive-post-meta">
                ...
                <button class="archive-share-btn" data-date="${archiveEntry.date}" title="Share OUR DAILY QUILT">
                  📤 Share
                </button>
              </div>
            </div>
      ...
      modalDelete.onclick = async () => {
        if (confirm(`Are you sure you want to delete the archive from ${formattedDate}?`)) {
          try {
            ...
            this.logger.log(`🗑️ Archive deleted: ${archiveEntry.date}`);
          } catch (error) {
            this.logger.error('Failed to delete archive:', error);
          }
        }
  ...
  async shareArchiveQuilt(date) {
    try {
      ...
      if (navigator.share) {
        try {
          await navigator.share({
            title: `OUR DAILY QUILT — ${formattedDate}`,
            text: `${archiveEntry.quote.text} — ${archiveEntry.quote.author}\n\nMade by ${archiveEntry.userCount} contributors — OUR DAILY QUILT`,
            url: `${window.location.href}?date=${date}`
          });
          
          this.logger.log(`📤 Archive quilt shared: ${date}`);
          return;
        } catch (shareError) {
      ...
      try {
        ...
        this.logger.log(`📤 Archive link copied: ${date}`);
      } catch (clipboardError) {
        // Final fallback: Just show a message
        const shareUrl = `${window.location.href}?date=${date}`;
        this.logger.log(`📤 Archive share link: ${shareUrl}`);
      }
