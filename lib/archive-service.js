...
class ArchiveService {
  ...
  _liveQuiltSvgHasMirrorField(quiltSVG) {
    return !!quiltSVG?.querySelector?.('#quiltMirroredFieldLayer');
  }
  ...
  async rasterizeQuiltBlocksViaRenderer(blocks, options = {}) {
    ...
    try {
      ...
      if (!this.hasRenderedQuiltSvgForBlocks(offscreenSvg, blocks)) return null;
  ...
  async getInstagramQuiltSourceBlob(blocks, options = {}) {
    ...
    const liveSvgMirrorReady =
      !isNative &&
      this.hasRenderedQuiltSvgForBlocks(quiltSVG, blocks) &&
    ...
    if (liveSvgMirrorReady) {
      try {
        ...
        this.logger.warn('SVG raster failed for Instagram quilt source, using renderer export:', e);
      }
    } else if (!isNative && quiltSVG) {
      this.logger.warn(
        'Live quilt SVG is not mirror-ready for Instagram quilt source; using renderer export'
      );
    }
    if (!quiltBlob) {
      try {
        ...
        this.logger.warn('Renderer quilt export failed, using block fallback:', rendererErr);
      }
    }
    if (!quiltBlob) {
    ...
    if (quiltBlob && !quiltBlobFromLiveSvg && !quiltBlobFromRendererExport && !exportPrimaryQuiltOnly) {
