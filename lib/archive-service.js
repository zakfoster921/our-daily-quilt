...
class ArchiveService {
  ...
  async generateInstagramPostLayoutBBlob(
    ...
    if (!composeOptions.postQuoteStyleIndependent) {
    ...
    console.log('[ARCHIVE-POST]', 'carouselStoryStyle='+carouselStoryStyle, 'hasFn='+(typeof globalThis.odqReadLayoutBPostStripPlan === 'function'));
    if (carouselStoryStyle && typeof globalThis.odqReadLayoutBPostStripPlan === 'function') {
      try {
        const savedPlan = await globalThis.odqReadLayoutBPostStripPlan(dk);
        console.log('[ARCHIVE-POST-PLAN]', savedPlan ? savedPlan.length + ' strips' : 'null');
        if (savedPlan && savedPlan.length) {
          composeOptions.savedPostStripPlan = savedPlan;
        }
      } catch (e) { console.warn('[ARCHIVE-POST-PLAN-ERR]', e?.message); }
    }
