...
class ArchiveService {
  ...
  async composeMirroredQuiltFieldBlob(quiltBlob, opts = {}) {
    ...
    try {
      ...
      const drawBottomTile = (clipX, clipW, clipH, flipX, flipY, duplicateOnly = false) => {
        ctx.save();
        ctx.beginPath();
      ...
      if (
        ...
        if (useDouble) {
          ...
          tiles.forEach((tile, index) => {
            ...
            const duplicateOnly = !tile.flipX && !tile.flipY;
            drawBottomTile(clipX, halfW, p.drawH, tile.flipX, tile.flipY, duplicateOnly);
          });
        } else {
          const flipX = tune?.flipX !== false;
          const flipY = tune?.flipY !== false;
          const duplicateOnly = !flipX && !flipY;
          drawBottomTile(p.startX, p.drawW, p.drawH, flipX, flipY, duplicateOnly);
        }
      } else {
  ...
  /**
   * Seamless IG carousel slides 2–3: slide 2 = framed quilt (+ speaker seam from slide 1), slide 3 = contributor clipping.
   */
  ...
  async applyCarouselSpeakerSeamToSlides(
    slide1DataUrl,
    slide2DataUrl,
    quoteObj,
    blocks,
    ...
    const overlay = await this._resolveCarouselSpeakerSeamOverlay(quoteObj, blocks, dateKey);
    if (!overlay) return null;
    const shortQuote = this._isShortCarouselQuote(quoteObj);
    const composedSpeakerRect =
      seamOpts.composedSpeakerRect ||
      this._lastLayoutBPostComposeMeta?.composedSpeakerRect ||
      globalThis._lastLayoutBComposeMeta?.composedSpeakerRect ||
      null;
    const skipSlide1SpeakerDraw =
      seamOpts.skipSlide1SpeakerDraw === true || !!composedSpeakerRect;
    try {
      const result = await apply({
        slide1DataUrl,
        slide2DataUrl,
        speakerImageUrl: overlay.imageUrl,
        cutoutSourceUrl: overlay.cutoutSourceUrl,
        washColor: overlay.washColor,
        dateKey: overlay.dateKey,
        shortQuote,
        composedSpeakerRect,
        skipSlide1SpeakerDraw,
        palette:
          typeof window !== 'undefined' && window.app?.getSpeakerCutoutPaperPalette
            ? window.app.getSpeakerCutoutPaperPalette()
            : []
      });
      if (!result?.slide1Blob || !result?.slide2Blob) return null;
      const [slide1DataUrlOut, slide2DataUrlOut] = await Promise.all([
        this._blobToDataUrl(result.slide1Blob),
        this._blobToDataUrl(result.slide2Blob)
      ]);
      if (!slide1DataUrlOut || !slide2DataUrlOut) return null;
      return {
        slide1DataUrl: slide1DataUrlOut,
        slide2DataUrl: slide2DataUrlOut,
        meta: result.meta || null
      };
    } catch (err) {
      this.logger.warn('Carousel speaker seam skipped:', err?.message || err);
      return null;
    }
  }

  /**
   * Integrated 3-slide carousel: slide 1 = layout B (+ speaker seam into slide 2),
   * Integrated IG carousel: slide 1 = layout B (+ speaker seam into slide 2),
   * slide 2 = yesterday stats (mirrored quilt), slide 3 = reflection wall (plain quilt),
   * slide 4 = contributor clipping (mirrored quilt).
   */
  async buildIntegratedInstagramCarouselImageData(
    ...
    const composedSpeakerRectFromLayout = layoutMeta?.composedSpeakerRect || null;
    ...
    const [quiltSlides, yesterdaySlide, reflectionSlide] = await Promise.all([
    ...
    const yesterdaySlideDataUrl = yesterdaySlide?.dataUrl || null;
    const yesterdayStatsMeta = yesterdaySlide?.meta || null;
    const reflectionSlideDataUrl = reflectionSlide?.dataUrl || null;
    const reflectionSlideMeta = reflectionSlide?.meta || null;

    if (!layoutBSlide1 || !yesterdaySlideDataUrl || !quiltSlides?.slide2) return null;

    let carouselSlide1 = layoutBSlide1;
    let carouselSlide2 = yesterdaySlideDataUrl;
    let speakerSeamMeta = null;

    const composedSpeakerRect =
      composedSpeakerRectFromLayout ||
      this._lastLayoutBPostComposeMeta?.composedSpeakerRect ||
      globalThis._lastLayoutBComposeMeta?.composedSpeakerRect ||
      null;
    const seam = await this.applyCarouselSpeakerSeamToSlides(
      layoutBSlide1,
      yesterdaySlideDataUrl,
      quoteObj,
      blocks,
      dk,
      {
        composedSpeakerRect,
        /** Slide 1 already has speaker under strips from carouselStoryStyle compose (matches tune Post preview). */
        skipSlide1SpeakerDraw: true
      }
    );
    if (seam?.slide1DataUrl && seam?.slide2DataUrl) {
      carouselSlide1 = seam.slide1DataUrl;
      carouselSlide2 = seam.slide2DataUrl;
      speakerSeamMeta = seam.meta || null;
    }

    let carouselSlide3 = reflectionSlideDataUrl || null;
    let scrapSeamMeta = null;
    const scrapRecords = yesterdayStatsMeta?.collageScrapRecords || [];
    if (carouselSlide3 && scrapRecords.length) {
      const applyScrapSeam =
        globalThis.IgCarouselYesterdayStatsSlideCompose?.applyCarouselRightScrapSeamToReflectionSlide ||
        globalThis.applyCarouselRightScrapSeamToReflectionSlide;
      if (typeof applyScrapSeam === 'function') {
        try {
          const scrapSeamQuiltBlob = await this._getInstagramQuiltScreenSourceBlob(
            blocks,
            dk,
            'ig_carousel_scrap_seam_source'
          );
          if (scrapSeamQuiltBlob) {
            const scrapSeam = await applyScrapSeam({
              reflectionDataUrl: carouselSlide3,
              scrapRecords,
              quiltBlob: scrapSeamQuiltBlob,
              dateKey: dk,
              quiltCanvasRect,
              quiltFit: 'cover',
              quiltBgZoom: carouselOptions?.quiltBgZoom
            });
            if (scrapSeam?.dataUrl) {
              carouselSlide3 = scrapSeam.dataUrl;
              scrapSeamMeta = scrapSeam.meta || null;
            }
          }
        } catch (err) {
          this.logger.warn('Carousel right scrap seam skipped:', err?.message || err);
        }
      }
    }

    const carouselSlide4 = quiltSlides.slide2;
    ...
    return {
      ...
      carouselSlide3,
      carouselSlide4,
      /** @deprecated preview alias — reflection is slide 3 now */
      carouselSlide2Reflection: carouselSlide3,
      meta: {
        ...(quiltSlides.meta || {}),
        speakerSeam: speakerSeamMeta,
        scrapSeam: scrapSeamMeta,
        reflectionSlide: reflectionSlideMeta,
        yesterdayStatsSlide: yesterdayStatsMeta
      }
    };
  }
    ...
    if (
      ...
      try {
        ...
        if (savedPlan && savedPlan.length) {
          ...
          // Carousel export always runs flow layout; tuned x/y replay is tune-modal preview only
          // (replaySavedCarouselStripPlan) so stale Firestore plans cannot skip typography/layout.
        }
      } catch (_) { /* optional */ }
    } else if (
      omitQuoteStrips !== true &&
