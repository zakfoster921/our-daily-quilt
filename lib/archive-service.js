...
class ArchiveService {
  ...
  _quiltSvgRenderLooksReady(quiltSVG, blocks) {
    ...
    if (this._liveQuiltSvgHasMirrorField(quiltSVG)) return true;
  ...
  async rasterizeQuiltBlocksViaRenderer(blocks, options = {}) {
    ...
    const igAspect =
      typeof globalThis.IG_QUILT_SCREEN_ASPECT === 'number'
        ...
        : 9 / 16;
    const exportMatchQuiltScreen = options.exportMatchQuiltScreen !== false;
    ...
    try {
      ...
      return await this.rasterizeQuiltSvgToPngBlob(offscreenSvg, {
        ...
        targetAspect: exportMatchQuiltScreen ? screenAspect : igAspect,
  ...
  async getInstagramQuiltSourceBlob(blocks, options = {}) {
    ...
    if (!exportPrimaryQuiltOnly && !exportFullQuiltField) {
      try {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (exportFullQuiltField && exportBlocks) {
          ...
          // Match the live quilt screen crop (xMidYMin slice) — do not recompute export viewBox.
        ...
        if (exportViewBoxParts) {
          ...
          if (exportMatchQuiltScreen) {
            // Same anchor as quilt-renderer-v2 on screen (xMidYMin slice).
            par = 'xMidYMin slice';
          } else {
            par =
