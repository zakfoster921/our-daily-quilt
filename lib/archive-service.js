...
class ArchiveService {
  ...
  async _resolveReflectionWallThemesForIg(dateKey, quoteObj = null) {
    ...
    let firstResponse = '';
    let userName = '';
    let firstHeartCount = 0;
    let highlightMeta = null;
    if (app?.loadReflectionThemesForToday) {
      try {
        await app.loadReflectionThemesForToday(dk, { force: false });
      } catch (_) {
        /* non-fatal */
      }
    }
    const cached = app?._readLocalReflectionThemesCache?.(dk);
    if (cached) {
      communityThemes = Array.isArray(cached.themes) ? cached.themes : [];
      firstResponse = String(cached.first_response || '').trim();
      userName = String(cached.user_name || '').trim();
      firstHeartCount = Math.max(0, Number(cached.firstResponseHeartCount) || 0);
      highlightMeta = {
        adminFirstResponseHighlight: cached.adminFirstResponseHighlight === true,
        adminFirstResponseHighlightAtIso: String(cached.adminFirstResponseHighlightAtIso || '').trim()
      };
    }
    const prompt =
    ...
    const wallThemes =
      typeof app?.buildReflectionWallThemes === 'function'
        ? app.buildReflectionWallThemes(
            communityThemes,
            firstResponse,
            userName,
            null,
  ...
  async buildIntegratedInstagramCarouselImageData(
    ...
    if (
      ...
      if (reflectionSlide?.dataUrl) {
        carouselSlide2Reflection = reflectionSlide.dataUrl;
        reflectionSlideMeta = reflectionSlide.meta || null;
        if (carouselOptions?.slide2Variant === 'reflection') {
          carouselSlide2 = reflectionSlide.dataUrl;
        }
      }
    }
