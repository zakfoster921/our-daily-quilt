...
class ArchiveService {
   ...
   * slides 2–3 = framed quilt (slide 2) + contributor clipping (slide 3).
   */
  async buildIntegratedInstagramCarouselImageData(
    ...
    const quiltSlides = await this.generateInstagramCarouselSlideImageData(blocks, contributors, dk, {
      quiltCanvasRect,
      quiltFit: 'cover',
      winningQuiltName: carouselOptions?.winningQuiltName || ''
    });
    if (!layoutBSlide1 || !quiltSlides?.slide1 || !quiltSlides?.slide2) return null;

    const wantsReflection =
      carouselOptions?.slide2Variant === 'reflection' ||
      carouselOptions?.includeSlide2Reflection === true;
    const useReflectionAsSlide2 = carouselOptions?.slide2Variant === 'reflection';

    let reflectionSlideDataUrl = null;
    let reflectionSlideMeta = null;
    if (wantsReflection) {
      const reflectionSlide = await this.generateInstagramCarouselReflectionSlideImageData(
        blocks,
        quoteObj,
        dk,
        {
      ...
      if (reflectionSlide?.dataUrl) {
        reflectionSlideDataUrl = reflectionSlide.dataUrl;
        reflectionSlideMeta = reflectionSlide.meta || null;
      }
    }

    const slide2ForSeam =
      useReflectionAsSlide2 && reflectionSlideDataUrl
        ? reflectionSlideDataUrl
        : quiltSlides.slide1;

    let carouselSlide1 = layoutBSlide1;
    let carouselSlide2 = quiltSlides.slide1;
    ...
    } else if (useReflectionAsSlide2 && reflectionSlideDataUrl) {
      carouselSlide2 = reflectionSlideDataUrl;
    }

    let carouselSlide2Reflection = null;
    if (reflectionSlideDataUrl) {
      if (useReflectionAsSlide2) {
        carouselSlide2Reflection = carouselSlide2;
      } else {
        const reflectionSeam = await this.applyCarouselSpeakerSeamToSlides(
          carouselSlide1,
          reflectionSlideDataUrl,
          quoteObj,
          blocks,
        ...
        carouselSlide2Reflection = reflectionSeam?.slide2DataUrl || reflectionSlideDataUrl;
      }
    }

    let carouselSlide4 = null;
    let yesterdayStatsMeta = null;
    ...
    const yesterdaySlide = await this.generateInstagramCarouselYesterdayStatsSlideImageData(
      blocks,
      dk,
      {
    ...
    if (yesterdaySlide?.dataUrl) {
      carouselSlide4 = yesterdaySlide.dataUrl;
      yesterdayStatsMeta = yesterdaySlide.meta || null;
    }

    return {
      carouselSlide1,
      carouselSlide2,
      carouselSlide3: quiltSlides.slide2,
      carouselSlide4,
      carouselSlide2Reflection,
      meta: {
        ...(quiltSlides.meta || {}),
