...
class ArchiveService {
  ...
  async _prepareSpeakerImageUrlForCanvas(speakerImageUrl, options = {}) {
      ...
      const fetchPromise = (async () => {
        ...
        for (const baseUrl of bases) {
          ...
          try {
            ...
            const exportUrl = await this._finalizeSpeakerCutoutExportDataUrl(dataUrl, source);
            this._proxyImageDataUrlCache.set(source, exportUrl);
            if (storagePath) {
              this._proxyImageDataUrlCache.set(`firebase-storage:${storagePath}`, exportUrl);
            }
            return exportUrl;
          } catch (fetchErr) {
    ...
    if (/^(?:data|blob):/i.test(primary)) {
      return this._finalizeSpeakerCutoutExportDataUrl(primary, cutoutSourceRef);
    }
    return '';
  }
