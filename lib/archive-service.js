...
class ArchiveService {
  ...
  async applyCarouselSpeakerSeamToSlides(
    ...
    try {
      const result = await apply({
        ...
        shortQuote,
        composedSpeakerRect: seamOpts.composedSpeakerRect || null,
        skipSlide1SpeakerDraw: seamOpts.skipSlide1SpeakerDraw === true
      });
  ...
  async buildIntegratedInstagramCarouselImageData(
    ...
    const composedSpeakerRect = this._lastLayoutBPostComposeMeta?.composedSpeakerRect || null;
    ...
    if (!postQuoteStyleIndependent && (speakerCutoutUrl || speakerPortraitUrl)) {
      ...
      if (speakerImageForCanvas) {
        ...
        if (typeof globalThis.odqSpeakerCutoutTransformForDateAsync === 'function') {
          try {
            presetTransform = await globalThis.odqSpeakerCutoutTransformForDateAsync(dk, 'story');
          } catch (_) {
            /* optional */
          }
        }
    ...
    if (carouselStoryStyle) {
      ...
      if (
        ...
        try {
          ...
          if (postTransform) composeOptions.speakerOverlay.transform = postTransform;
        } catch (_) {
