...
class ArchiveService {
  ...
  async generateInstagramCarouselSlideBlobs(blocks, contributors = [], dateKey = null, carouselOptions = {}) {
    ...
    const highResBlob = await this.getInstagramQuiltSourceBlob(blocks, {
      dateKey: dk,
      exportModeLabel: 'ig_carousel_quilt_source',
      skipFilmGrain: true
    });
  ...
  async buildIntegratedInstagramCarouselImageData(
    ...
    const layoutBSlide1 = overlay
      ...
      : await this.generateInstagramPostLayoutBImage(blocks, quoteObj, dk);
    ...
    try {
      console.log(
        '[nightly-ig:layout-b-speaker]',
        JSON.stringify({
          phase: 'integrated-carousel-done',
          hasOverlay: !!overlay,
          hasLayoutBSlide1: !!layoutBSlide1,
          seamApplied: !!(seam?.slide1DataUrl && seam?.slide2DataUrl),
          skipSlide1SpeakerRedraw: !!composedSpeakerRect,
          diagCount: (globalThis.__odqSpeakerDrawDiag || []).length,
          lastBranch:
            (globalThis.__odqSpeakerDrawDiag || []).slice(-1)[0]?.data?.branch || null
        })
      );
    } catch (_) {
      /* ignore */
          ...
          try {
            ...
            fetch('http://127.0.0.1:7433/ingest/0ed8adaa-5aed-4571-811f-aadcc7a8fddc', {
              ...
              body: JSON.stringify({
                ...
                runId: 'pre-fix',
                hypothesisId: 'B',
                location: 'archive-service.js:_prepareSpeakerImageUrlForCanvas',
                message: 'speaker proxy prepared',
                data: {
                  skipCutoutExportFinalize,
                  finalizedSkipped: skipCutoutExportFinalize,
                  sameAsRaw: exportUrl === dataUrl,
                  sourceIsCutout: /speaker-cutouts(?:%2F|\/)/i.test(source),
