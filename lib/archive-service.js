/**
 * Archive, Instagram image generation, and quilt export helpers.
 * Exposes globalThis.ArchiveService.
 */
(function (root) {
  'use strict';

/** Temporary: carousel post slide 4 = green mat + framed quilt (yesterday stats hidden). */
const CAROUSEL_USE_QUILT_ONLY_SLIDE_4 = true;
/** Quilt-screen stored story preview (quote-fit JPEG). Bump when bake shape changes. */
const ODQ_STORY_QUOTE_FIT_PREVIEW_RENDER_VERSION = 'story-quote-fit-preview-v3';
root.ODQ_STORY_QUOTE_FIT_PREVIEW_RENDER_VERSION = ODQ_STORY_QUOTE_FIT_PREVIEW_RENDER_VERSION;

class ArchiveService {
  constructor(logger, dataService) {
    this.logger = logger;
    this.dataService = dataService;
    this.archives = new Map(); // Date string -> archive data (cached)
    this.isLoading = false;
    this.currentPage = 0;
    this.postsPerPage = 10;
    this.lastQuiltExportDebug = null;
    this.lastQuiltExportRawDataUrl = null;
    /** Story 9:16 strip plan captured for the next layout-b post compose (tune modal parity). */
    this._layoutBStoryRefStripPlan = null;
    /** Last 4:5 compose metadata (e.g. composedSpeakerRect for carousel seam). */
    this._lastLayoutBPostComposeMeta = null;
  }

  _clearLayoutBStoryRefStripPlan() {
    this._layoutBStoryRefStripPlan = null;
  }

  _stashLayoutBStoryRefStripPlan(plan) {
    const clone =
      typeof globalThis.odqCloneLayoutBStripPlan === 'function'
        ? globalThis.odqCloneLayoutBStripPlan(plan)
        : plan;
    this._layoutBStoryRefStripPlan =
      Array.isArray(clone) && clone.length ? clone : null;
  }

  /** Story compose (9:16) to capture strip positions for inherited post export. */
  async _captureLayoutBStoryRefStripPlanForPost(highResBlob, quoteText, quoteAuthor, dateKey, seedOpts) {
    if (!highResBlob) return null;
    await globalThis.ensureLayoutBComposeReady?.();
    const compose = globalThis.composeInstagramLayoutBFromQuiltBlob;
    if (typeof compose !== 'function') return null;
    let captured = null;
    const storyOpts = {
      tuneAspect: 'story',
      stripLayoutSeed: seedOpts.stripLayoutSeed,
      stripLayoutSeedExplicit: seedOpts.stripLayoutSeedExplicit,
      speakerOverlay: seedOpts.speakerOverlay,
      speakerCutoutQuote: seedOpts.speakerCutoutQuote,
      captureStoryRefStripPlan: true,
      onStoryRefStripPlan: (plan) => {
        captured = plan;
      }
    };
    await compose(highResBlob, quoteText, quoteAuthor, 1080, 1920, dateKey, storyOpts);
    if (Array.isArray(captured) && captured.length) {
      this._stashLayoutBStoryRefStripPlan(captured);
    }
    return this._layoutBStoryRefStripPlan;
  }

  // Archive data structure for each day
  createArchiveEntry(date, quilt, quote, userCount) {
    return {
      date: date, // "2024-01-15"
      quilt: quilt, // SVG data/state
      quote: quote, // { text: "...", author: "..." }
      userCount: userCount, // Number of users who contributed
      shareCount: 0, // Number of times this quilt has been shared
      thumbnail: null, // Will be generated
      isComplete: true // Whether the day finished properly
    };
  }

  // Get archives for feed (with pagination)
  async getArchives(page = 0, limit = this.postsPerPage) {
    try {
              // Try to load from Firestore if we don't have archives cached
    // if (this.archives.size === 0 && window.db && window.firestore) {
    //   await this.loadArchivesFromFirestore();
    // }
      
      const allArchives = Array.from(this.archives.values())
        .sort((a, b) => new Date(b.date) - new Date(a.date)); // Newest first
      
      const start = page * limit;
      const end = start + limit;
      const pageArchives = allArchives.slice(start, end);
      
      return {
        archives: pageArchives,
        hasMore: end < allArchives.length,
        totalCount: allArchives.length
      };
    } catch (error) {
      this.logger.error('Failed to get archives:', error);
      return {
        archives: [],
        hasMore: false,
        totalCount: 0
      };
    }
  }

  // Load archives from Firestore
  async loadArchivesFromFirestore() {
    try {
      if (!window.db || !window.firestore) {
        this.logger.warn('Firestore not available for loading archives');
        return;
      }

      const archivesRef = window.firestore.collection(window.db, 'archives');
      const q = window.firestore.query(archivesRef, window.firestore.orderBy('date', 'desc'), window.firestore.limit(50));
      const querySnapshot = await window.firestore.getDocs(q);
      
      querySnapshot.forEach((doc) => {
        const data = doc.data();
        this.archives.set(data.date, data);
      });
      
      this.logger.log(`📖 Loaded ${this.archives.size} archives from Firestore`);
    } catch (error) {
      this.logger.error('Failed to load archives from Firestore:', error);
    }
  }

  // Add new archive entry
  async addArchive(archiveEntry) {
    try {
      // Add to Firestore if available
      // if (window.db && window.firestore) {
      //   const archiveRef = window.firestore.doc(window.db, 'archives', archiveEntry.date);
      //   await window.firestore.setDoc(archiveRef, {
      //     ...archiveEntry,
      //     createdAt: new Date().toISOString()
      //   });
      //   this.logger.log(`📝 Archive saved to Firestore: ${archiveEntry.date}`);
      // }
      
      // Add to local cache
      this.archives.set(archiveEntry.date, archiveEntry);
      this.logger.log(`📝 Archive cached locally: ${archiveEntry.date}`);
    } catch (error) {
      this.logger.error('Failed to save archive to Firestore:', error);
      // Still cache locally as fallback
      this.archives.set(archiveEntry.date, archiveEntry);
    }
  }

  /**
   * Mirrored-quilt viewBox for export (positive padding). Screen SVG uses negative padding for meet fit;
   * re-use block bounds + renderer mirror math so exports show the full field, not a tight crop.
   */
  viewBoxPartsForMirroredQuiltScreen(blocks, paddingRatio = 0.02, dateKey = null) {
    if (typeof QuiltMirrorLayout !== 'undefined' && typeof QuiltMirrorLayout.viewBoxPartsFromBlocks === 'function') {
      const dk =
        String(dateKey || '').trim() ||
        (typeof Utils !== 'undefined' && typeof Utils.getTodayKey === 'function' ? Utils.getTodayKey() : '');
      return QuiltMirrorLayout.viewBoxPartsFromBlocks(blocks, {
        dateKey: dk,
        forExport: true,
        paddingRatio,
        preferFullMirroredField: true
      });
    }
    if (!Array.isArray(blocks) || blocks.length === 0) return null;
    const minX = Math.min(...blocks.map((b) => Number(b.x)));
    const minY = Math.min(...blocks.map((b) => Number(b.y)));
    const maxX = Math.max(...blocks.map((b) => Number(b.x) + Number(b.width)));
    const maxY = Math.max(...blocks.map((b) => Number(b.y) + Number(b.height)));
    const quiltW = maxX - minX;
    const quiltH = maxY - minY;
    if (!(quiltW > 0) || !(quiltH > 0)) return null;
    const horizontalStretch = 1.16;
    const mirrorRatio =
      typeof window !== 'undefined' && typeof window.app?.renderer?.getMirrorSeamOffsetRatio === 'function'
        ? window.app.renderer.getMirrorSeamOffsetRatio()
        : 0.68;
    const mirrorSeamOffset = quiltH * mirrorRatio;
    const mirroredFieldWidth = quiltW * horizontalStretch;
    const mirroredFieldHeight = quiltH + mirrorSeamOffset;
    const pad = Math.max(4, quiltW * paddingRatio);
    return [minX - pad, minY - pad, mirroredFieldWidth + pad * 2, mirroredFieldHeight + pad * 2];
  }

  /**
   * Canonical 9:16 quilt capture for Zapier (classic + Layout B). Matches the quilt screen:
   * primary field + mirrored fold, crop to container slice, letterboxed into 9:16. Cached per fingerprint.
   */
  async _prefetchQuiltExportTune(dateKey) {
    const dk = String(dateKey || '').trim();
    if (!dk) return;
    if (typeof globalThis.odqPrefetchMirrorTune === 'function') {
      try {
        await globalThis.odqPrefetchMirrorTune(dk);
      } catch (err) {
        this.logger?.warn?.('Mirror tune prefetch failed before quilt export:', err);
      }
    }
    if (typeof globalThis.odqPrefetchLayoutBTuneFields === 'function') {
      try {
        await globalThis.odqPrefetchLayoutBTuneFields(dk, 8000);
      } catch (err) {
        this.logger?.warn?.('Layout B tune prefetch failed before quilt export:', err);
      }
    }
  }

  _liveQuiltSvgHasMirrorField(quiltSVG) {
    if (!quiltSVG?.querySelector) return false;
    return !!(
      quiltSVG.querySelector('#quiltMirroredFieldLayer') ||
      quiltSVG.querySelector('#quiltDuplicateBottomLayer1') ||
      quiltSVG.querySelector('#quiltDuplicateBottomLayer2')
    );
  }

  _quiltSvgRenderLooksReady(quiltSVG, blocks) {
    if (!quiltSVG || !Array.isArray(blocks) || !blocks.length) return false;
    if (this.hasRenderedQuiltSvgForBlocks(quiltSVG, blocks)) return true;
    const shapeCount = quiltSVG.querySelectorAll('polygon, path, rect').length;
    return shapeCount >= Math.max(1, blocks.length);
  }

  /** Primary field + mirrored bottom tiles into 9:16 (used when SVG renderer export is unavailable). */
  async composeMirroredQuiltFieldBlob(quiltBlob, opts = {}) {
    if (!quiltBlob) return null;
    const bitmap = await createImageBitmap(quiltBlob);
    try {
      const maxEdge = Math.max(320, Number(opts.maxEdge) || 960);
      const outW = Math.round(maxEdge * 9 / 16);
      const outH = maxEdge;
      const canvas = document.createElement('canvas');
      canvas.width = outW;
      canvas.height = outH;
      const ctx = canvas.getContext('2d');
      if (!ctx) return null;
      ctx.fillStyle = '#f6f4f1';
      ctx.fillRect(0, 0, outW, outH);
      ctx.imageSmoothingEnabled = true;
      ctx.imageSmoothingQuality = opts.imageSmoothingQuality || 'medium';

      const dateKey =
        String(opts.dateKey || '').trim() ||
        (typeof Utils !== 'undefined' && typeof Utils.getTodayKey === 'function'
          ? Utils.getTodayKey()
          : '');
      const blocks = Array.isArray(opts.blocks) ? opts.blocks : [];

      const drawTileInRect = (clipX, clipY, clipW, clipH, flipX, flipY) => {
        ctx.save();
        ctx.beginPath();
        ctx.rect(clipX, clipY, clipW, clipH);
        ctx.clip();
        if (!flipX && !flipY) {
          ctx.drawImage(bitmap, clipX, clipY, clipW, clipH);
        } else {
          ctx.translate(clipX + (flipX ? clipW : 0), clipY + (flipY ? clipH : 0));
          ctx.scale(flipX ? -1 : 1, flipY ? -1 : 1);
          ctx.drawImage(bitmap, 0, 0, clipW, clipH);
        }
        ctx.restore();
      };

      if (
        typeof QuiltMirrorLayout !== 'undefined' &&
        typeof QuiltMirrorLayout.computeComposePlacements === 'function'
      ) {
        let sourceW = bitmap.width;
        let sourceH = bitmap.height;
        if (blocks.length) {
          const minX = Math.min(...blocks.map((b) => Number(b.x)));
          const minY = Math.min(...blocks.map((b) => Number(b.y)));
          const maxX = Math.max(...blocks.map((b) => Number(b.x) + Number(b.width)));
          const maxY = Math.max(...blocks.map((b) => Number(b.y) + Number(b.height)));
          sourceW = maxX - minX;
          sourceH = maxY - minY;
        }
        const tune =
          typeof globalThis.odqReadMirrorTuneFromLocal === 'function'
            ? globalThis.odqReadMirrorTuneFromLocal(dateKey)
            : null;
        const useQuad =
          typeof globalThis.odqMirrorBottomLayoutIsQuad === 'function' &&
          globalThis.odqMirrorBottomLayoutIsQuad(tune);
        const useDouble =
          typeof globalThis.odqMirrorBottomLayoutIsDouble === 'function' &&
          globalThis.odqMirrorBottomLayoutIsDouble(tune);

        if (useQuad) {
          const cellW = outW / 2;
          const cellH = outH / 2;
          const quad =
            typeof QuiltMirrorLayout !== 'undefined' &&
            typeof QuiltMirrorLayout.odqQuadTileFlagsFromTune === 'function'
              ? QuiltMirrorLayout.odqQuadTileFlagsFromTune(tune)
              : {
                  tl: { flipX: tune?.tlFlipX === true, flipY: tune?.tlFlipY === true },
                  tr: { flipX: tune?.trFlipX === true, flipY: tune?.trFlipY === true },
                  bl: { flipX: tune?.blFlipX === true, flipY: tune?.blFlipY === true },
                  br: { flipX: tune?.brFlipX === true, flipY: tune?.brFlipY === true }
                };
          [
            { x: 0, y: 0, ...quad.tl },
            { x: cellW, y: 0, ...quad.tr },
            { x: 0, y: cellH, ...quad.bl },
            { x: cellW, y: cellH, ...quad.br }
          ].forEach((tile) => {
            drawTileInRect(tile.x, tile.y, cellW, cellH, tile.flipX, tile.flipY);
          });
        } else {
          const p = QuiltMirrorLayout.computeComposePlacements(
            outW,
            outH,
            sourceW,
            sourceH,
            dateKey,
            { blocks }
          );
          ctx.drawImage(bitmap, p.startX, p.primaryY, p.drawW, p.drawH);
          const clipY = p.mirrorY;

          if (useDouble) {
            const halfW = p.drawW / 2;
            const tiles = [
              { flipX: tune?.leftFlipX === true, flipY: tune?.leftFlipY === true },
              { flipX: tune?.rightFlipX === true, flipY: tune?.rightFlipY === true }
            ];
            tiles.forEach((tile, index) => {
              const clipX = p.startX + index * halfW;
              drawTileInRect(clipX, clipY, halfW, p.drawH, tile.flipX, tile.flipY);
            });
          } else {
            const flipX = tune?.flipX !== false;
            const flipY = tune?.flipY !== false;
            drawTileInRect(p.startX, clipY, p.drawW, p.drawH, flipX, flipY);
          }
        }
      } else {
        const fit = opts.quiltFit === 'cover' ? 'cover' : 'meet';
        const scale =
          fit === 'cover'
            ? Math.max(outW / Math.max(1, bitmap.width), outH / (Math.max(1, bitmap.height) * 2))
            : Math.min(outW / Math.max(1, bitmap.width), outH / (Math.max(1, bitmap.height) * 2));
        const drawW = bitmap.width * scale;
        const drawH = bitmap.height * scale;
        const startY = (outH - drawH * 2) / 2;
        const startX = (outW - drawW) / 2;
        [
          { y: startY, flipX: false, flipY: false },
          { y: startY + drawH, flipX: true, flipY: true }
        ].forEach((placement) => {
          ctx.save();
          ctx.beginPath();
          ctx.rect(0, placement.y, outW, drawH);
          ctx.clip();
          if (placement.flipX || placement.flipY) {
            ctx.translate(
              placement.flipX ? startX + drawW : startX,
              placement.flipY ? placement.y + drawH : placement.y
            );
            ctx.scale(placement.flipX ? -1 : 1, placement.flipY ? -1 : 1);
            ctx.drawImage(bitmap, 0, 0, drawW, drawH);
          } else {
            ctx.drawImage(bitmap, startX, placement.y, drawW, drawH);
          }
          ctx.restore();
        });
      }

      const mimeType = opts.mimeType || 'image/png';
      const quality = typeof opts.quality === 'number' ? opts.quality : undefined;
      return await new Promise((resolve) => canvas.toBlob(resolve, mimeType, quality));
    } finally {
      if (typeof bitmap.close === 'function') bitmap.close();
    }
  }

  /**
   * Offscreen quilt-renderer-v2 export — full SVG textures + mirror field (matches quilt screen).
   * @returns {Promise<Blob|null>}
   */
  async rasterizeQuiltBlocksViaRenderer(blocks, options = {}) {
    if (!Array.isArray(blocks) || !blocks.length || typeof document === 'undefined') return null;
    const app = typeof window !== 'undefined' ? window.app : null;
    const renderer = app?.renderer;
    if (!renderer || typeof renderer.renderBlocks !== 'function') return null;

    const dk =
      String(options.dateKey || '').trim() ||
      (typeof Utils.getTodayKey === 'function' ? Utils.getTodayKey() : '');
    if (options.skipTunePrefetch !== true) {
      await this._prefetchQuiltExportTune(dk);
    }

    const igAspect =
      typeof globalThis.IG_QUILT_SCREEN_ASPECT === 'number'
        ? globalThis.IG_QUILT_SCREEN_ASPECT
        : 9 / 16;
    const exportFullQuiltField = options.exportFullQuiltField === true;
    const exportMatchQuiltScreen =
      options.exportMatchQuiltScreen !== false && !exportFullQuiltField;
    const screenAspect = exportMatchQuiltScreen ? this.getLiveQuiltScreenAspect(igAspect) : igAspect;
    let viewportW;
    let viewportH;
    if (exportFullQuiltField) {
      const exportVb = this.viewBoxPartsForMirroredQuiltScreen(blocks, 0.02, dk);
      if (exportVb && exportVb[2] > 0 && exportVb[3] > 0) {
        const vbAspect = exportVb[2] / exportVb[3];
        viewportH = 960;
        viewportW = Math.max(320, Math.round(viewportH * vbAspect));
        viewportH = Math.max(320, Math.round(viewportW / Math.max(0.01, vbAspect)));
      }
    }
    if (!(viewportW > 0) || !(viewportH > 0)) {
      const containerRect =
        document.querySelector('#screen-quilt .quilt-container')?.getBoundingClientRect?.() || null;
      viewportW =
        containerRect && containerRect.width > 0
          ? containerRect.width
          : Math.max(320, Math.round(720 * screenAspect));
      viewportH =
        containerRect && containerRect.height > 0
          ? containerRect.height
          : Math.max(480, Math.round(viewportW / Math.max(0.01, screenAspect)));
    }

    const offscreenSvg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    offscreenSvg.setAttribute('xmlns', 'http://www.w3.org/2000/svg');
    offscreenSvg.setAttribute('width', String(Math.round(viewportW)));
    offscreenSvg.setAttribute('height', String(Math.round(viewportH)));
    offscreenSvg.setAttribute('aria-hidden', 'true');
    offscreenSvg.style.cssText = 'position:fixed;left:-99999px;top:0;width:1px;height:1px;overflow:hidden;pointer-events:none;';
    document.body.appendChild(offscreenSvg);

    const prevSvg = renderer.quiltSVG;
    const prevOverride = renderer._renderViewportOverride;
    const prevDateKey = renderer._exportDateKeyOverride;
    const prevDisableSplitBand = renderer._exportDisableSplitBandFit;
    const prevExportFullQuiltField = renderer._exportFullQuiltField;
    const prevUserPieces = renderer.userPieces;
    const prevLastAddedIndex = renderer.lastAddedIndex;
    const prevBackside = renderer.backsidePreviewEnabled;

    try {
      renderer._exportDateKeyOverride = dk;
      /** Full-quilt-field IG exports use classic mirror viewBox; split-band screen layout misaligns. */
      renderer._exportDisableSplitBandFit = exportFullQuiltField === true;
      renderer._exportFullQuiltField = exportFullQuiltField === true;
      renderer._renderViewportOverride = { w: viewportW, h: viewportH };
      renderer.quiltSVG = offscreenSvg;
      renderer.lastAddedIndex = null;
      renderer.setBacksidePreviewEnabled?.(false);
      const state = app?.quiltEngine?.getState?.() || {};
      renderer.renderBlocks(
        blocks,
        Array.isArray(state.userPieces) ? state.userPieces : [],
        state.submissionCount ?? blocks.length
      );
      if (!this._liveQuiltSvgHasMirrorField(offscreenSvg)) {
        await new Promise((resolve) => requestAnimationFrame(resolve));
      }
      if (!this._quiltSvgRenderLooksReady(offscreenSvg, blocks)) return null;
      return await this.rasterizeQuiltSvgToPngBlob(offscreenSvg, {
        underpaintBlocks: blocks,
        targetAspect: exportMatchQuiltScreen ? screenAspect : null,
        maxEdge: options.maxEdge,
        dateKey: dk,
        exportModeLabel: options.exportModeLabel || 'renderer_offscreen_quilt_screen',
        exportFullQuiltField,
        exportMatchQuiltScreen,
        exportKeepRendererViewBox: exportFullQuiltField === true,
        skipExportUnderpaint: true,
        exportMatteFill:
          typeof options.exportMatteFill === 'string' ? options.exportMatteFill : '#f6f4f1',
        quiltFingerprint: options.quiltFingerprint || Utils.computeQuiltFingerprint(blocks)
      });
    } finally {
      renderer.quiltSVG = prevSvg;
      renderer._renderViewportOverride = prevOverride;
      renderer._exportDateKeyOverride = prevDateKey;
      renderer._exportDisableSplitBandFit = prevDisableSplitBand;
      renderer._exportFullQuiltField = prevExportFullQuiltField;
      renderer.userPieces = prevUserPieces;
      renderer.lastAddedIndex = prevLastAddedIndex;
      renderer.setBacksidePreviewEnabled?.(prevBackside);
      offscreenSvg.remove();
    }
  }

  async getInstagramQuiltSourceBlob(blocks, options = {}) {
    if (!blocks || blocks.length === 0) return null;
    const fingerprint = Utils.computeQuiltFingerprint(blocks);
    const exportDateKey =
      String(options.dateKey || '').trim() ||
      (typeof Utils.getTodayKey === 'function' ? Utils.getTodayKey() : '');
    if (options.skipTunePrefetch !== true) {
      await this._prefetchQuiltExportTune(exportDateKey);
    }
    if (typeof window !== 'undefined' && options.skipDeferredAdminSlice !== true) {
      await window?.app?.loadDeferredAdminSlice?.();
    }
    const exportPrimaryQuiltOnly = options.exportPrimaryQuiltOnly === true;
    const exportFullQuiltField = options.exportFullQuiltField === true;
    /** Honor explicit false — meet-fit screen rasters letterbox quilt on tall paper. */
    const exportMatchQuiltScreen =
      options.exportMatchQuiltScreen !== false &&
      !exportPrimaryQuiltOnly &&
      !exportFullQuiltField;
    const cacheVariant = exportPrimaryQuiltOnly
      ? 'primary-quilt-v1'
      : exportFullQuiltField
        ? 'full-quilt-field-v3'
        : options.exportFillFrame === true
          ? 'mood-card-fill-v2'
          : options.tunePreviewExport === true
            ? 'quilt-screen-tune-v1'
            : 'quilt-screen-v3';
    const grainVariant = options.skipFilmGrain === true ? 'clean' : 'grain';
    const maxEdgePart =
      options.maxEdge != null ? `:e${Math.min(4096, Math.max(64, Number(options.maxEdge) || 0))}` : '';
    const mirrorSig =
      typeof globalThis.odqMirrorTuneExportSignature === 'function'
        ? globalThis.odqMirrorTuneExportSignature(options.dateKey)
        : '';
    const mirrorPart = mirrorSig ? `:m${mirrorSig}` : '';
    const igAspect =
      typeof globalThis.IG_QUILT_SCREEN_ASPECT === 'number'
        ? globalThis.IG_QUILT_SCREEN_ASPECT
        : 9 / 16;
    const screenAspectPart = exportMatchQuiltScreen
      ? `:a${Math.round(this.getLiveQuiltScreenAspect(igAspect) * 10000)}`
      : '';
    const cacheKey = `${String(options.dateKey || '').trim()}:${fingerprint}:${cacheVariant}:${grainVariant}${maxEdgePart}${mirrorPart}${screenAspectPart}`;
    if (
      this._igQuiltSourceCacheKey === cacheKey &&
      this._igQuiltSourceCacheBlob instanceof Blob &&
      this._igQuiltSourceCacheBlob.size > 0
    ) {
      return this._igQuiltSourceCacheBlob;
    }
    if (!this._igQuiltSourceInflight) this._igQuiltSourceInflight = new Map();
    const inflightExisting = this._igQuiltSourceInflight.get(cacheKey);
    if (inflightExisting) return inflightExisting;

    const inflightPromise = (async () => {
    const quiltSVG = typeof document !== 'undefined' ? document.getElementById('quilt') : null;
    const screenAspect = exportMatchQuiltScreen ? this.getLiveQuiltScreenAspect(igAspect) : null;
    const rasterOpts = {
      targetAspect: screenAspect,
      underpaintBlocks: blocks,
      dateKey: options.dateKey,
      exportPrimaryQuiltOnly,
      exportFullQuiltField,
      exportMatchQuiltScreen,
      exportFillFrame: options.exportFillFrame === true,
      exportMatteFill: '#f6f4f1',
      exportModeLabel: options.exportModeLabel || 'instagram_quilt_source_9x16_quilt_screen',
      instagramTrueWhiteMatte: false,
      skipFilmGrain: options.skipFilmGrain === true,
      quiltFingerprint: fingerprint
    };
    if (options.maxEdge != null) {
      rasterOpts.maxEdge = Math.min(4096, Math.max(64, Number(options.maxEdge) || 2400));
    }
    if (options.imageSmoothingQuality === 'low') {
      rasterOpts.imageSmoothingQuality = 'low';
    }

    let quiltBlob = null;
    let quiltBlobFromLiveSvg = false;
    let usedMirroredComposeFallback = false;
    let quiltBlobFromRendererExport = false;
    const isNative =
      typeof window !== 'undefined' &&
      typeof window.odqIsCapacitorNative === 'function' &&
      window.odqIsCapacitorNative();
    if (!exportPrimaryQuiltOnly && options.skipRendererExport !== true) {
      try {
        quiltBlob = await this.rasterizeQuiltBlocksViaRenderer(blocks, {
          dateKey: exportDateKey,
          maxEdge: options.maxEdge,
          exportModeLabel: rasterOpts.exportModeLabel,
          exportFullQuiltField,
          exportMatchQuiltScreen,
          skipExportUnderpaint: true,
          exportMatteFill: rasterOpts.exportMatteFill,
          quiltFingerprint: fingerprint,
          skipTunePrefetch: options.skipTunePrefetch === true
        });
        quiltBlobFromRendererExport = !!quiltBlob;
      } catch (rendererErr) {
        this.logger.warn('Renderer quilt export failed, trying live SVG:', rendererErr);
      }
    }
    const exportRenderer =
      typeof window !== 'undefined' ? window.app?.renderer : null;
    const portalSplitBandBlocksFullFieldExport =
      exportFullQuiltField &&
      !isNative &&
      typeof exportRenderer?.isQuiltSplitBandFitEnabled === 'function' &&
      exportRenderer.isQuiltSplitBandFitEnabled();
    const liveSvgMirrorReady =
      !quiltBlob &&
      !isNative &&
      !portalSplitBandBlocksFullFieldExport &&
      this._quiltSvgRenderLooksReady(quiltSVG, blocks) &&
      this._liveQuiltSvgHasMirrorField(quiltSVG);
    if (liveSvgMirrorReady) {
      try {
        quiltBlob = await this.rasterizeVisibleQuiltSvgToPngBlob(quiltSVG, blocks, {
          ...rasterOpts,
          skipExportUnderpaint: true
        });
        quiltBlobFromLiveSvg = !!quiltBlob;
      } catch (e) {
        this.logger.warn('SVG raster failed for Instagram quilt source, using block fallback:', e);
      }
    } else if (!quiltBlob && !isNative && quiltSVG) {
      this.logger.warn(
        'Live quilt SVG is not mirror-ready for Instagram quilt source; using block fallback if needed'
      );
    }
    let quiltBlobFromBlockRaster = false;
    if (!quiltBlob) {
      // Last-resort flat block raster + mirrored compose (headless without renderer).
      const blockRasterOpts = {
        underpaintBlocks: blocks,
        exportMatchQuiltScreen: false,
        exportFillFrame: options.exportFillFrame === true,
        exportMatteFill: '#f6f4f1',
        exportModeLabel: `${rasterOpts.exportModeLabel || 'instagram_quilt_source'}_block_primary`,
        instagramTrueWhiteMatte: false,
        skipFilmGrain: options.skipFilmGrain === true,
        quiltFingerprint: fingerprint
      };
      if (options.maxEdge != null) {
        blockRasterOpts.maxEdge = rasterOpts.maxEdge;
      }
      if (rasterOpts.imageSmoothingQuality === 'low') {
        blockRasterOpts.imageSmoothingQuality = 'low';
      }
      quiltBlob = await this.generateQuiltRasterBlobFromBlocks(blocks, blockRasterOpts);
      quiltBlobFromBlockRaster = !!quiltBlob;
    }
    if (
      quiltBlob &&
      quiltBlobFromBlockRaster &&
      !exportPrimaryQuiltOnly &&
      !exportFullQuiltField
    ) {
      try {
        const mirrored = await this.composeMirroredQuiltFieldBlob(quiltBlob, {
          maxEdge: options.maxEdge != null ? rasterOpts.maxEdge : 2400,
          mimeType: 'image/png',
          imageSmoothingQuality: rasterOpts.imageSmoothingQuality || 'high',
          quiltFit: options.exportFillFrame === true ? 'cover' : 'meet',
          dateKey: exportDateKey,
          blocks
        });
        if (mirrored) {
          quiltBlob = mirrored;
          usedMirroredComposeFallback = true;
        }
      } catch (mirrorErr) {
        this.logger.warn('Mirrored quilt compose fallback failed:', mirrorErr);
      }
    }
    this._igQuiltSourceExportMeta = {
      quiltBlobFromLiveSvg,
      quiltBlobFromRendererExport,
      quiltBlobFromBlockRaster,
      usedMirroredComposeFallback,
      exportModeLabel: rasterOpts.exportModeLabel,
      capturedAt: new Date().toISOString(),
      lastQuiltExportDebug: this.lastQuiltExportDebug ? { ...this.lastQuiltExportDebug } : null
    };
    if (quiltBlob) {
      this._igQuiltSourceCacheKey = cacheKey;
      this._igQuiltSourceCacheBlob = quiltBlob;
    }
    return quiltBlob;
    })();

    this._igQuiltSourceInflight.set(cacheKey, inflightPromise);
    try {
      return await inflightPromise;
    } finally {
      if (this._igQuiltSourceInflight.get(cacheKey) === inflightPromise) {
        this._igQuiltSourceInflight.delete(cacheKey);
      }
    }
  }

  clearInstagramQuiltSourceCache() {
    this._igQuiltSourceCacheKey = '';
    this._igQuiltSourceCacheBlob = null;
    this._igQuiltSourceExportMeta = null;
    this._igQuiltSourceInflight = null;
  }

  /**
   * Re-encodes an opaque raster blob as JPEG — the quilt-screen story raster has no
   * transparency (matte-filled), so lossless PNG just bloats upload/load size for no benefit.
   */
  async _reencodeBlobAsJpeg(blob, quality = 0.9) {
    if (!blob || typeof document === 'undefined') return null;
    let bitmap = null;
    let objectUrl = '';
    try {
      if (typeof createImageBitmap === 'function') {
        bitmap = await createImageBitmap(blob);
      } else {
        objectUrl = URL.createObjectURL(blob);
        bitmap = await new Promise((resolve, reject) => {
          const el = new Image();
          el.onload = () => resolve(el);
          el.onerror = () => reject(new Error('Could not decode blob for JPEG re-encode'));
          el.src = objectUrl;
        });
      }
      const w = bitmap.width || bitmap.naturalWidth;
      const h = bitmap.height || bitmap.naturalHeight;
      if (!w || !h) return null;
      const canvas = document.createElement('canvas');
      canvas.width = w;
      canvas.height = h;
      const ctx = canvas.getContext('2d');
      if (!ctx) return null;
      ctx.fillStyle = '#f6f4f1';
      ctx.fillRect(0, 0, w, h);
      ctx.drawImage(bitmap, 0, 0, w, h);
      return await new Promise((resolve) => canvas.toBlob(resolve, 'image/jpeg', quality));
    } catch (_) {
      return null;
    } finally {
      if (typeof bitmap?.close === 'function') bitmap.close();
      if (objectUrl) URL.revokeObjectURL(objectUrl);
    }
  }

  /**
   * Quilt-screen raster (primary + mirror, edge-filled) — same source as quiltScreen9x16Url.
   * @returns {Promise<Blob|null>}
   */
  async _getInstagramQuiltScreenSourceBlob(blocks, dateKey = null, exportModeLabel = 'quilt_screen_source') {
    if (!blocks || blocks.length === 0) return null;
    const dk = dateKey || Utils.getTodayKey();
    return this.getInstagramQuiltSourceBlob(blocks, {
      dateKey: dk,
      exportModeLabel,
      skipFilmGrain: true
    });
  }

  /**
   * Quilt-only 9:16 JPEG (1080×1920 class) matching the quilt screen — uploaded nightly as quilt-screen-9x16.jpg.
   * @returns {Promise<string|null>} data URL
   */
  async generateInstagramQuiltScreen9x16ImageData(blocks, dateKey = null) {
    if (!blocks || blocks.length === 0) return null;
    const dk = dateKey || Utils.getTodayKey();
    const blob = await this._getInstagramQuiltScreenSourceBlob(blocks, dk, 'quilt_screen_9x16_nightly');
    if (!blob) return null;
    const jpegBlob = await this._reencodeBlobAsJpeg(blob, 0.9);
    return Utils.blobToDataUrl(jpegBlob || blob);
  }

  /** Today payload for quilt mood triptych / nightly newspaper clipping PNG. */
  async _resolveNewspaperClippingTodayPayload(dateKey = null, options = {}) {
    const qs = window.app?.quoteService;
    const dk =
      String(dateKey || '').trim() ||
      qs?.getQuoteCalendarKeyNow?.() ||
      Utils.getTodayKey?.() ||
      '';
    if (!dk) return null;
    const app = window.app;
    let raw = null;
    if (qs && typeof qs.getQuoteResolvedForInstagramDateKey === 'function') {
      try {
        raw = await qs.getQuoteResolvedForInstagramDateKey(dk);
      } catch (_) {
        /* */
      }
    }
    if (!raw && qs) raw = qs.getTodayQuote?.();
    const q = app?._quoteForMoodWidget?.(raw) || raw;
    const payload =
      app?._quiltMoodTerminalQuotePayload?.(q, dk) ||
      (q && String(q.text ?? q.body ?? '').trim()
        ? {
            text: String(q.text ?? q.body ?? '').trim(),
            author: String(q.author ?? q.authorSnapshot ?? '').trim(),
            dateKey: dk
          }
        : null);
    if (!payload?.text) return null;
    const out = { ...payload, dateKey: String(payload.dateKey || dk).trim() || dk };
    const QNC = globalThis.QuiltNewspaperClipping;
    const fromFs =
      options.skipFirestore === true ? null : await this._readQuoteForClippingDateKeyFromFirestore(dk);
    const catalogFlc =
      typeof QNC?.firstLineCountFromQuoteFields === 'function'
        ? QNC.firstLineCountFromQuoteFields(fromFs)
        : Number(fromFs?.first_line_count ?? fromFs?.firstLineCount);
    if (Number.isFinite(catalogFlc) && catalogFlc > 0) {
      out.first_line_count = Math.round(catalogFlc);
      out.firstLineCount = Math.round(catalogFlc);
    } else {
      const hasFlc =
        (typeof QNC?.firstLineCountFromQuoteFields === 'function'
          ? QNC.firstLineCountFromQuoteFields(out)
          : null) ??
        (Number(out.first_line_count ?? out.firstLineCount) > 0
          ? Math.round(Number(out.first_line_count ?? out.firstLineCount))
          : null);
      if (hasFlc == null) {
        const fsFlc = Number(fromFs?.first_line_count ?? fromFs?.firstLineCount);
        if (Number.isFinite(fsFlc) && fsFlc > 0) {
          out.first_line_count = Math.round(fsFlc);
          out.firstLineCount = Math.round(fsFlc);
        }
      }
    }
    return out;
  }

  _newspaperClippingPaperTextureUrl() {
    const QNC = globalThis.QuiltNewspaperClipping;
    if (typeof QNC?.resolveClippingPaperTextureUrl === 'function') {
      return QNC.resolveClippingPaperTextureUrl();
    }
    return 'assets/quilt-paper-card-texture.png';
  }

  async _readQuoteForClippingDateKeyFromFirestore(dk) {
    const dateKey = String(dk || '').trim();
    if (!dateKey || !window.db || !window.firestore) return null;
    const pick = (...vals) => {
      for (const v of vals) {
        const s = String(v ?? '').trim();
        if (s) return s;
      }
      return '';
    };
    const firstLineCountFromDoc = (data) => {
      const QNC = globalThis.QuiltNewspaperClipping;
      if (typeof QNC?.firstLineCountFromQuoteFields === 'function') {
        return QNC.firstLineCountFromQuoteFields(data);
      }
      let flc = Number(data?.firstLineCount ?? data?.first_line_count);
      if (!Number.isFinite(flc) || flc <= 0) {
        flc = Number(data?.firstLineCountSnapshot ?? data?.first_line_count_snapshot);
      }
      if (!Number.isFinite(flc) || flc <= 0) {
        flc = Number(data?.notionProperties?.first_line_count?.value ?? data?.notionProperties?.firstLineCount?.value);
      }
      return Number.isFinite(flc) && flc > 0 ? Math.round(flc) : null;
    };
    const fromDoc = (data) => {
      if (!data || typeof data !== 'object') return null;
      const text = pick(data.text, data.quote, data.body, data.textSnapshot);
      const author = pick(data.author, data.authorSnapshot);
      if (!text && !author) return null;
      const flc = firstLineCountFromDoc(data);
      return {
        text,
        body: text,
        author,
        dateKey,
        keyword: pick(data.keyword, data.keywordSnapshot),
        ...(flc != null ? { firstLineCount: flc, first_line_count: flc } : {})
      };
    };
    try {
      const read = async (col, id) => {
        const ref = window.firestore.doc(window.db, col, id);
        const snap = await window.firestore.getDoc(ref);
        return snap.exists() ? snap.data() || {} : null;
      };
      const assignment = await read('dailyQuoteAssignments', dateKey);
      const sourceId = pick(assignment?.sourceId);
      const merged = [fromDoc(assignment), fromDoc(await read('quotes', dateKey)), fromDoc(await read('quotes', sourceId))]
        .filter(Boolean)
        .reduce((acc, row) => ({ ...acc, ...row }), {});
      return merged.text ? merged : null;
    } catch (_) {
      return null;
    }
  }

  async _resolveAdjacentQuotesForClipping(dateKey) {
    const dk = String(dateKey || '').trim();
    if (!dk) return { yesterday: null, tomorrow: null };
    const qs = window.app?.quoteService;
    if (qs && typeof qs.getAdjacentQuotesForClippingDateKey === 'function') {
      try {
        if (typeof qs.offsetQuoteCalendarKey === 'function' && typeof qs.resolveAndPinCalendarKey === 'function') {
          const yKey = qs.offsetQuoteCalendarKey(dk, -1);
          const tKey = qs.offsetQuoteCalendarKey(dk, 1);
          await Promise.all([
            yKey ? qs.resolveAndPinCalendarKey(yKey).catch(() => null) : null,
            tKey ? qs.resolveAndPinCalendarKey(tKey).catch(() => null) : null
          ]);
        }
        const adj = await qs.getAdjacentQuotesForClippingDateKey(dk);
        const yesterday = adj?.yesterday || null;
        const tomorrow = adj?.tomorrow || null;
        const neighborLine = (q) => String(q?.text ?? q?.body ?? '').trim();
        const yKey = qs.offsetQuoteCalendarKey?.(dk, -1);
        const tKey = qs.offsetQuoteCalendarKey?.(dk, 1);
        const yOut =
          neighborLine(yesterday) ? yesterday : await this._readQuoteForClippingDateKeyFromFirestore(yKey);
        const tOut =
          neighborLine(tomorrow) ? tomorrow : await this._readQuoteForClippingDateKeyFromFirestore(tKey);
        if (!neighborLine(yOut) && !neighborLine(tOut)) {
          this.logger.warn(
            `[archive] newspaper clipping ${dk}: no yesterday/tomorrow quote text — side columns may be blank`
          );
        }
        return { yesterday: yOut || yesterday, tomorrow: tOut || tomorrow };
      } catch (_) {
        /* */
      }
    }
    if (typeof qs?.getAdjacentQuotesForClipping === 'function') {
      const adj = qs.getAdjacentQuotesForClipping() || {};
      return {
        yesterday: adj.yesterday || null,
        tomorrow: adj.tomorrow || null
      };
    }
    return { yesterday: null, tomorrow: null };
  }

  /** Pre-rendered newspaper clipping PNG — matches live quilt mood triptych quote card. */
  async generateNewspaperClippingImageData(dateKey = null, options = {}) {
    const QNC = globalThis.QuiltNewspaperClipping;
    const compose = QNC?.composeDataUrlWithLayout || QNC?.composeDataUrl;
    if (typeof compose !== 'function') {
      this.logger.warn('QuiltNewspaperClipping.composeDataUrl missing');
      return null;
    }
    const tunePreview = options.tunePreview === true;
    const payload = await this._resolveNewspaperClippingTodayPayload(dateKey, {
      skipFirestore: tunePreview
    });
    if (!payload) return null;
    const dk = payload.dateKey;
    const composed = await compose({
      today: payload,
      dateKey: dk,
      width: 0,
      exportProfile: 'nightlyPeek',
      centerOnly: true,
      ...(tunePreview
        ? {
            exportDensity: 1,
            paperTextureUrl: null
          }
        : {})
    });
    const dataUrl = typeof composed === 'string' ? composed : composed?.dataUrl || null;
    if (composed && typeof composed === 'object') {
      const bytes = dataUrl
        ? Math.max(0, Math.round(((dataUrl.length - 22) * 3) / 4))
        : 0;
      const clippedW = Number(composed.clippedWidth) || 0;
      const clippedH = Number(composed.clippedHeight) || 0;
      this._lastNewspaperClippingComposeMeta = {
        dateKey: dk,
        bytes,
        composeAttempt: composed.composeAttempt,
        composeAttemptLabel: composed.composeAttemptLabel,
        composePipeline: composed.composePipeline || null,
        exportDiagnostics: composed.exportDiagnostics,
        hasKeyword: !!String(payload.keyword || '').trim(),
        firstLineCount:
          globalThis.QuiltNewspaperClipping?.firstLineCountFromQuoteFields?.(payload) ??
          payload.first_line_count ??
          payload.firstLineCount ??
          null,
        firstLineUnits: Number(composed.firstLineUnits) || null,
        firstLineText: String(composed.firstLineText || '').trim() || null,
        peekPxEachSide: Number(composed.peekPxEachSide) || null,
        peekRatio: Number(composed.peekRatio) || null,
        exportRev: globalThis.QuiltNewspaperClipping?.CLIPPING_EXPORT_REV || null,
        spreadCrop: String(composed.spreadCrop || '').trim(),
        peekCrop: composed.peekCrop === true,
        quoteOnlyCrop: composed.quoteOnlyCrop === true,
        quoteOnlyPadPxEachSide: Number(composed.quoteOnlyPadPxEachSide) || null,
        hasYesterdayText: !!composed.hasYesterdayText,
        hasTomorrowText: !!composed.hasTomorrowText,
        clippedWidth: clippedW,
        clippedHeight: clippedH,
        renderWidth: Number(composed.renderWidth) || 0,
        effectiveBodyDomPx: Number(composed.effectiveBodyDomPx) || 0,
        handCutTypographyScale: Number(composed.handCutTypographyScale) || 1,
        centerOnlyWidth: Number(composed.centerOnlyWidth) || 0,
        peekWidthRatio: Number(composed.peekWidthRatio) || 0,
        aspectRatio: clippedW > 0 && clippedH > 0 ? clippedW / clippedH : 0
      };
      console.log(
        '[archive] newspaper clipping compose',
        JSON.stringify(this._lastNewspaperClippingComposeMeta)
      );
    }
    return dataUrl;
  }

  /** Pre-rendered mood-spread clipping: full good/rough columns + gapped flanks + center quote. */
  async generateMoodSpreadClippingImageData(dateKey = null) {
    const compose = globalThis.QuiltNewspaperClipping?.composeMoodSpreadDataUrl;
    if (typeof compose !== 'function') {
      this.logger.warn('QuiltNewspaperClipping.composeMoodSpreadDataUrl missing');
      return null;
    }
    const qs = window.app?.quoteService;
    const dk =
      String(dateKey || '').trim() ||
      qs?.getQuoteCalendarKeyNow?.() ||
      Utils.getTodayKey?.() ||
      '';
    if (!dk) return null;
    let today =
      (typeof qs?.getQuoteResolvedForInstagramDateKey === 'function'
        ? await qs.getQuoteResolvedForInstagramDateKey(dk)
        : null) || qs?.getTodayQuote?.();
    if (today && typeof qs?.hydrateMoodFieldsForCalendarKey === 'function') {
      try {
        const hydrated = await qs.hydrateMoodFieldsForCalendarKey(dk, today);
        if (hydrated) today = hydrated;
      } catch (_) {
        /* */
      }
    }
    if (!today || !String(today.text || today.body || '').trim()) return null;
    const moods = qs?._moodLinesFromQuote?.(today || {}) || {};
    const goodDay = String(moods.goodDay ?? today.good_day ?? today.goodDay ?? '').trim();
    const roughDay = String(moods.roughDay ?? today.rough_day ?? today.roughDay ?? '').trim();
    if (!goodDay && !roughDay) return null;
    const paperTextureUrl = this._newspaperClippingPaperTextureUrl();
    return compose({
      today,
      goodDay,
      roughDay,
      dateKey: dk,
      paperTextureUrl,
      width: 0
    });
  }

  async _measureDataUrlHeightPx(dataUrl) {
    const size = await this._measureDataUrlSizePx(dataUrl);
    return size?.h ?? 0;
  }

  async _measureDataUrlSizePx(dataUrl) {
    const src = String(dataUrl || '').trim();
    if (!src.startsWith('data:image/')) return { w: 0, h: 0 };
    if (typeof Image === 'undefined') return { w: 0, h: 0 };
    return new Promise((resolve) => {
      const img = new Image();
      img.onload = () =>
        resolve({
          w: Math.max(0, Math.round(img.naturalWidth || 0)),
          h: Math.max(0, Math.round(img.naturalHeight || 0))
        });
      img.onerror = () => resolve({ w: 0, h: 0 });
      img.src = src;
    });
  }

  /** Pre-rendered mood clipping PNG (`good_day` / `rough_day`) for quilt stack. */
  async generateMoodClippingImageData(
    dateKey = null,
    { variant = 'good', quoteClippingHeightPx = 0, quoteClippingWidthPx = 0 } = {}
  ) {
    const composeMood = globalThis.QuiltMoodClipping?.composeMoodDataUrl;
    if (typeof composeMood !== 'function') {
      this.logger.warn('QuiltMoodClipping.composeMoodDataUrl missing');
      return null;
    }
    const moodVariant = String(variant || 'good').toLowerCase() === 'rough' ? 'rough' : 'good';
    const qs = window.app?.quoteService;
    const dk =
      String(dateKey || '').trim() ||
      qs?.getQuoteCalendarKeyNow?.() ||
      Utils.getTodayKey?.() ||
      '';
    if (!dk) return null;
    let quote =
      (typeof qs?.getQuoteResolvedForInstagramDateKey === 'function'
        ? await qs.getQuoteResolvedForInstagramDateKey(dk)
        : null) || qs?.getTodayQuote?.();
    if (quote && typeof qs?.hydrateMoodFieldsForCalendarKey === 'function') {
      try {
        const hydrated = await qs.hydrateMoodFieldsForCalendarKey(dk, quote);
        if (hydrated) quote = hydrated;
      } catch (_) {
        /* */
      }
    }
    const moods = qs?._moodLinesFromQuote?.(quote || {}) || {};
    const line = String(
      moodVariant === 'rough'
        ? moods.roughDay ?? quote?.rough_day ?? quote?.roughDay ?? ''
        : moods.goodDay ?? quote?.good_day ?? quote?.goodDay ?? ''
    ).trim();
    if (!line) return null;

    let quoteH = Number(quoteClippingHeightPx);
    let quoteW = Number(quoteClippingWidthPx);
    if (!Number.isFinite(quoteH) || quoteH <= 0 || !Number.isFinite(quoteW) || quoteW <= 0) {
      const quoteDataUrl = await this.generateNewspaperClippingImageData(dk);
      const size = await this._measureDataUrlSizePx(quoteDataUrl);
      if (!Number.isFinite(quoteH) || quoteH <= 0) quoteH = size.h;
      if (!Number.isFinite(quoteW) || quoteW <= 0) quoteW = size.w;
    }

    const paperTextureUrl = this._newspaperClippingPaperTextureUrl();
    return composeMood({
      line,
      dateKey: dk,
      variant: moodVariant,
      quoteClippingHeightPx: quoteH,
      quoteClippingWidthPx: quoteW,
      paperTextureUrl,
      width: quoteW
    });
  }

  // Generate Instagram-ready image (4:5 portrait ratio)
  async generateInstagramImage(blocks, _quote = null) {
    try {
      if (!blocks || blocks.length === 0) {
        this.logger.warn('No blocks provided for Instagram image generation');
        return null;
      }

      const quiltBlob = await this.getInstagramQuiltSourceBlob(blocks, {
        exportModeLabel: 'classic_9x16_from_shared_source',
        skipFilmGrain: true
      });
      if (!quiltBlob) return null;

      const W = 1080;
      const H = 1350;
      /** Minimal symmetric inset so IG classic matches thin top/bottom (contain centers in full inner box). */
      const pad = 2;
      const innerW = W - pad * 2;
      const innerH = H - pad * 2;

      const canvas = document.createElement('canvas');
      const ctx = canvas.getContext('2d');
      canvas.width = W;
      canvas.height = H;
      ctx.fillStyle = '#ffffff';
      ctx.fillRect(0, 0, W, H);

      const bitmapUrl = URL.createObjectURL(quiltBlob);
      try {
        const img = await new Promise((resolve, reject) => {
          const im = new Image();
          im.onload = () => resolve(im);
          im.onerror = () => reject(new Error('Could not decode quilt bitmap'));
          im.src = bitmapUrl;
        });
        const iw = Math.max(1, img.naturalWidth || img.width);
        const ih = Math.max(1, img.naturalHeight || img.height);
        const scale = Math.min(innerW / iw, innerH / ih);
        const dw = Math.round(iw * scale);
        const dh = Math.round(ih * scale);
        const dx = pad + (innerW - dw) / 2;
        const dy = pad + (innerH - dh) / 2;
        ctx.imageSmoothingEnabled = true;
        ctx.imageSmoothingQuality = 'high';
        ctx.drawImage(img, 0, 0, iw, ih, dx, dy, dw, dh);
      } finally {
        URL.revokeObjectURL(bitmapUrl);
      }

      return canvas.toDataURL('image/png', 0.95);
    } catch (error) {
      this.logger.error('Instagram image generation failed:', error);
      return null;
    }
  }

  async _blobToDataUrl(blob) {
    if (!blob) return null;
    return new Promise((resolve) => {
      const fr = new FileReader();
      fr.onload = () => resolve(fr.result || null);
      fr.onerror = () => resolve(null);
      fr.readAsDataURL(blob);
    });
  }

  /**
   * IG carousel contributor pair: slide 2 = framed quilt + title strip, slide 3 = contributor clipping.
   */
  async generateInstagramCarouselSlideBlobs(blocks, contributors = [], dateKey = null, carouselOptions = {}) {
    if (!blocks || blocks.length === 0) return null;
    const compose = globalThis.composeInstagramContributorCarouselFromQuiltBlob;
    const duplicateFallback = globalThis.IgContributorCarouselCompose?.duplicateClassicToCarouselSlides;
    const dk = dateKey || Utils.getTodayKey();

    const quiltScreenBlob = await this._getInstagramQuiltScreenSourceBlob(
      blocks,
      dk,
      'ig_carousel_quilt_screen_source'
    );
    if (!quiltScreenBlob) return null;

    const normalized =
      typeof globalThis.ContributorCloudCompose?.normalizeContributors === 'function'
        ? globalThis.ContributorCloudCompose.normalizeContributors(contributors)
        : Array.isArray(contributors)
          ? contributors
          : [];

    if (normalized.length && typeof compose === 'function') {
      const result = await compose(quiltScreenBlob, normalized, {
        dateKey: dk,
        paperTextureUrl: this._newspaperClippingPaperTextureUrl?.(),
        quiltCanvasRect: carouselOptions.quiltCanvasRect || null,
        quiltBgZoom: carouselOptions.quiltBgZoom,
        quiltFit: carouselOptions.quiltFit || 'cover',
        slide3QuiltBlob: quiltScreenBlob,
        winningQuiltName: carouselOptions.winningQuiltName || '',
        contributorBgFlip: carouselOptions.contributorBgFlip === true
      });
      if (result?.slide1Blob && result?.slide2Blob) {
        this._lastInstagramCarouselComposeMeta = result.meta || null;
        return { slide1: result.slide1Blob, slide2: result.slide2Blob, meta: result.meta || null };
      }
    }

    if (typeof duplicateFallback !== 'function') {
      this.logger.warn('Carousel compose unavailable and classic duplicate fallback missing');
      return null;
    }
    const classicDataUrl = await this.generateInstagramImage(blocks);
    const fallback = await duplicateFallback(classicDataUrl);
    if (!fallback?.slide1Blob || !fallback?.slide2Blob) return null;
    this._lastInstagramCarouselComposeMeta = { ...(fallback.meta || {}), contributorFallback: true };
    return { slide1: fallback.slide1Blob, slide2: fallback.slide2Blob, meta: this._lastInstagramCarouselComposeMeta };
  }

  async generateInstagramCarouselSlideImageData(blocks, contributors = [], dateKey = null, carouselOptions = {}) {
    try {
      const blobs = await this.generateInstagramCarouselSlideBlobs(
        blocks,
        contributors,
        dateKey,
        carouselOptions
      );
      if (!blobs?.slide1 || !blobs?.slide2) return null;
      const [slide1, slide2] = await Promise.all([
        this._blobToDataUrl(blobs.slide1),
        this._blobToDataUrl(blobs.slide2)
      ]);
      if (!slide1 || !slide2) return null;
      return { slide1, slide2, meta: blobs.meta || null };
    } catch (error) {
      this.logger.error('Instagram carousel slide generation failed:', error);
      return null;
    }
  }

  async _resolveReflectionWallThemesForIg(dateKey, quoteObj = null) {
    const dk = dateKey || Utils.getTodayKey();
    const app = typeof window !== 'undefined' ? window.app : null;
    if (typeof app?.buildReflectionWallThemesForDateKey === 'function') {
      try {
        return await app.buildReflectionWallThemesForDateKey(dk, quoteObj);
      } catch (_) {
        /* fall through */
      }
    }
    const prompt =
      (typeof app?.getQuiltReflectionPromptText === 'function' && app.getQuiltReflectionPromptText(quoteObj)) ||
      String(
        quoteObj?.communityPrompt ||
          quoteObj?.community_prompt ||
          quoteObj?.reflectionPrompt ||
          quoteObj?.reflection_prompt ||
          ''
      ).trim();
    return { prompt, wallThemes: [] };
  }

  /** Alternate carousel slide 2: flipped quilt bg + reflection prompt + highlighted response patches. */
  async generateInstagramCarouselReflectionSlideImageData(
    blocks,
    quoteObj = null,
    dateKey = null,
    carouselOptions = {}
  ) {
    const compose =
      globalThis.composeCarouselReflectionSlideFromQuiltBlob ||
      globalThis.IgCarouselReflectionSlideCompose?.composeCarouselReflectionSlideFromQuiltBlob;
    if (typeof compose !== 'function') return null;
    const dk = dateKey || Utils.getTodayKey();
    const { prompt, wallThemes } = await this._resolveReflectionWallThemesForIg(dk, quoteObj);
    const flatten =
      globalThis.IgCarouselReflectionSlideCompose?.flattenHighlightedThemes ||
      ((themes) => (Array.isArray(themes) ? themes : []).filter((t) => t?.adminHighlight === true));
    const highlighted = flatten(wallThemes);
    if (!prompt || (!highlighted.length && !wallThemes.length)) return null;

    const quiltScreenBlob = await this._getInstagramQuiltScreenSourceBlob(
      blocks,
      dk,
      'ig_carousel_reflection_slide_source'
    );
    if (!quiltScreenBlob) return null;

    try {
      const result = await compose(quiltScreenBlob, {
        dateKey: dk,
        reflectionPrompt: prompt,
        highlightedThemes: wallThemes,
        quiltCanvasRect: carouselOptions.quiltCanvasRect || null,
        quiltBgZoom: carouselOptions.quiltBgZoom,
        quiltFit: carouselOptions.quiltFit || 'cover',
        promptCardUrl: carouselOptions.promptCardUrl,
        cardsPngBase64: carouselOptions.cardsPngBase64,
        cardsLayerLogicalWidth: carouselOptions.cardsLayerLogicalWidth,
        cardsLayerLogicalHeight: carouselOptions.cardsLayerLogicalHeight,
        cardsLayerDeviceScaleFactor: carouselOptions.cardsLayerDeviceScaleFactor,
        cardPieceRects: carouselOptions.cardPieceRects
      });
      if (!result?.blob) return null;
      const dataUrl = await this._blobToDataUrl(result.blob);
      if (!dataUrl) return null;
      return { dataUrl, meta: result.meta || null };
    } catch (error) {
      this.logger.warn('Reflection carousel slide 2 generation failed:', error);
      return null;
    }
  }

  _resolveIgCarouselContributorCount(contributors, blocks) {
    const fromList = Array.isArray(contributors) ? contributors.length : 0;
    const fromBlocks = Math.max(0, (Array.isArray(blocks) ? blocks.length : 0) - 1);
    return Math.max(1, fromList || fromBlocks || 1);
  }

  /** Temporary: carousel post slide 4 = green mat + framed quilt (yesterday stats hidden). */

  /** Carousel slide 4 (temporary): green mat + centered framed quilt only. */
  async generateInstagramCarouselQuiltOnlySlideImageData(blocks, dateKey = null, carouselOptions = {}) {
    const compose =
      globalThis.composeCarouselQuiltOnlyMatSlideFromQuiltBlob ||
      globalThis.IgContributorCarouselCompose?.composeCarouselQuiltOnlyMatSlideFromQuiltBlob;
    if (typeof compose !== 'function') return null;
    const dk = dateKey || Utils.getTodayKey();

    const quiltScreenBlob = await this._getInstagramQuiltScreenSourceBlob(
      blocks,
      dk,
      'ig_carousel_quilt_only_slide_source'
    );
    if (!quiltScreenBlob) return null;

    try {
      const result = await compose(quiltScreenBlob, {
        dateKey: dk,
        quiltCanvasRect: carouselOptions.quiltCanvasRect || null,
        quiltBgZoom: carouselOptions.quiltBgZoom,
        quiltFit: carouselOptions.quiltFit || 'cover'
      });
      if (!result?.blob) return null;
      const dataUrl = await this._blobToDataUrl(result.blob);
      if (!dataUrl) return null;
      return { dataUrl, meta: result.meta || null };
    } catch (error) {
      this.logger.warn('Quilt-only carousel slide 4 generation failed:', error);
      return null;
    }
  }

  /**
   * Carousel slide 0 (alternate slide 1): mat + quilt frame with Helvetica quote
   * scaled and fully justified to fill the quilt frame. No speaker / strips.
   */
  async generateInstagramCarouselSlide0QuoteFitImageData(
    blocks,
    quoteObj = null,
    dateKey = null,
    carouselOptions = {}
  ) {
    const compose =
      globalThis.composeCarouselSlide0QuoteFitFromQuiltBlob ||
      globalThis.IgCarouselSlide0QuoteFitCompose?.composeCarouselSlide0QuoteFitFromQuiltBlob;
    if (typeof compose !== 'function') return null;
    const dk = dateKey || Utils.getTodayKey();
    const nestedQuote =
      quoteObj?.quote && typeof quoteObj.quote === 'object' ? quoteObj.quote : null;
    const quoteText = String(
      carouselOptions.quoteText ||
        quoteObj?.text ||
        quoteObj?.body ||
        nestedQuote?.text ||
        nestedQuote?.body ||
        (typeof quoteObj?.quote === 'string' ? quoteObj.quote : '') ||
        ''
    ).trim();
    if (!quoteText || quoteText === '[object Object]') return null;

    /** Fill-frame source so quilt blocks cover the paper (screen meet-fit leaves a cream bar). */
    const quiltScreenBlob = await this.getInstagramQuiltSourceBlob(blocks, {
      dateKey: dk,
      exportModeLabel: 'ig_carousel_slide0_quote_fit_source',
      skipFilmGrain: true,
      exportMatchQuiltScreen: false,
      exportFillFrame: true
    });
    if (!quiltScreenBlob) return null;

    try {
      const result = await compose(quiltScreenBlob, {
        dateKey: dk,
        quoteText,
        quiltBgZoom: carouselOptions.quiltBgZoom,
        quiltBgOffsetY: carouselOptions.quiltBgOffsetY,
        quiltFit: carouselOptions.quiltFit || 'cover',
        quiltMatPeekPx: carouselOptions.quiltMatPeekPx,
        textPadFrac: carouselOptions.textPadFrac
      });
      if (!result?.blob) return null;
      const dataUrl = await this._blobToDataUrl(result.blob);
      if (!dataUrl) return null;
      return { dataUrl, meta: result.meta || null };
    } catch (error) {
      this.logger.warn('Carousel slide 0 quote-fit generation failed:', error);
      return null;
    }
  }

  /**
   * Story 9:16 alternate of carousel slide 0: tight top vinyl quote-fit +
   * slide-0b speaker cutout/name on the bottom half.
   */
  async generateInstagramStoryQuoteFitImageData(
    blocks,
    quoteObj = null,
    dateKey = null,
    storyOptions = {}
  ) {
    const compose =
      globalThis.composeStoryQuoteFitFromQuiltBlob ||
      globalThis.IgCarouselSlide0QuoteFitCompose?.composeStoryQuoteFitFromQuiltBlob;
    if (typeof compose !== 'function') return null;
    const dk = dateKey || Utils.getTodayKey();
    const q = await this._quoteForLayoutBSpeakerImage(quoteObj, dk);
    const nestedQuote =
      quoteObj?.quote && typeof quoteObj.quote === 'object' ? quoteObj.quote : null;
    const quoteText = String(
      storyOptions.quoteText ||
        quoteObj?.text ||
        quoteObj?.body ||
        nestedQuote?.text ||
        nestedQuote?.body ||
        q?.text ||
        q?.body ||
        (typeof quoteObj?.quote === 'string' ? quoteObj.quote : '') ||
        ''
    ).trim();
    if (!quoteText || quoteText === '[object Object]') return null;

    const quoteAuthor = String(q.author ?? '').trim();
    const app = typeof window !== 'undefined' ? window.app : null;
    const profileName = app?.getQuoteSpeakerProfile?.(q)?.name || '';
    const speakerName = String(
      storyOptions.speakerName ||
        q.speakerName ||
        q.speaker_name ||
        profileName ||
        quoteAuthor ||
        ''
    )
      .replace(/^\s*[—-]\s*/, '')
      .trim();

    let speakerImageForCanvas = String(storyOptions.speakerImageUrl || '').trim();
    let speakerCutoutUrl = this._isFirebaseSpeakerCutoutUrl(storyOptions.cutoutSourceUrl)
      ? String(storyOptions.cutoutSourceUrl || '').trim()
      : '';
    let speakerPortraitUrl = '';
    if (speakerName) {
      speakerPortraitUrl = this._layoutBSpeakerPortraitUrlResolved(q);
      if (!speakerCutoutUrl) {
        speakerCutoutUrl = await this.resolveLayoutBSpeakerCutoutUrl(q, quoteAuthor);
      }
      if (
        this._isFirebaseSpeakerCutoutUrl(speakerCutoutUrl) &&
        speakerImageForCanvas &&
        /^https?:/i.test(speakerImageForCanvas) &&
        !this._isFirebaseSpeakerCutoutUrl(speakerImageForCanvas)
      ) {
        speakerImageForCanvas = '';
      }
      if (!speakerImageForCanvas && (speakerCutoutUrl || speakerPortraitUrl)) {
        speakerImageForCanvas = await this._speakerOverlayImageUrlForLayoutBCompose(
          speakerCutoutUrl,
          speakerPortraitUrl,
          q
        );
      }
    }

    const washColor = String(
      storyOptions.washColor ||
        (typeof window !== 'undefined' && window.app?.getSpeakerCutoutWashColor?.()) ||
        (typeof window !== 'undefined' && window.app?.getMostPopularQuiltColor?.(blocks)?.color) ||
        (typeof globalThis.CONFIG !== 'undefined' &&
          globalThis.CONFIG.APP &&
          globalThis.CONFIG.APP.defaultColor) ||
        '#ea9b9a'
    ).trim();

    let quiltScreenBlob = storyOptions.quiltBlob || null;
    if (!quiltScreenBlob && storyOptions.quiltSourceUrl) {
      try {
        const quiltUrl = String(storyOptions.quiltSourceUrl || '').trim();
        if (quiltUrl) {
          const res = await fetch(quiltUrl, { mode: 'cors', credentials: 'omit', cache: 'no-store' });
          if (res.ok) quiltScreenBlob = await res.blob();
        }
      } catch (_) {
        quiltScreenBlob = null;
      }
    }
    if (!quiltScreenBlob) {
      quiltScreenBlob = await this._getInstagramQuiltScreenSourceBlob(
        blocks,
        dk,
        'ig_story_quote_fit_source'
      );
    }
    if (!quiltScreenBlob) return null;

    try {
      let styleAmount = storyOptions.styleAmount ?? storyOptions.speakerStyleAmount;
      if (styleAmount == null && typeof globalThis.odqGetCachedLayoutBSpeakerStyleAmount === 'function') {
        styleAmount = globalThis.odqGetCachedLayoutBSpeakerStyleAmount(dk);
      } else if (
        styleAmount == null &&
        typeof globalThis.odqNormalizeSpeakerStyleAmount === 'function'
      ) {
        styleAmount = 1;
      }
      if (typeof globalThis.odqNormalizeSpeakerStyleAmount === 'function') {
        styleAmount = globalThis.odqNormalizeSpeakerStyleAmount(styleAmount);
      }
      const result = await compose(quiltScreenBlob, {
        dateKey: dk,
        quoteText,
        speakerName,
        speakerImageUrl: speakerImageForCanvas || '',
        cutoutSourceUrl: speakerCutoutUrl || speakerPortraitUrl || speakerImageForCanvas || '',
        washColor,
        isCutoutPng: !!(
          this._isFirebaseSpeakerCutoutUrl(speakerCutoutUrl) ||
          this._isFirebaseSpeakerCutoutUrl(speakerImageForCanvas)
        ),
        styleAmount,
        quiltCanvasRect: storyOptions.quiltCanvasRect || null,
        quiltBgZoom: storyOptions.quiltBgZoom,
        quiltBgOffsetY: storyOptions.quiltBgOffsetY,
        quiltFit: storyOptions.quiltFit || 'cover',
        quiltMatPeekPx: storyOptions.quiltMatPeekPx,
        textPadFrac: storyOptions.textPadFrac,
        speakerFrameFrac: storyOptions.speakerFrameFrac,
        speakerTopFrac: storyOptions.speakerTopFrac,
        panelWidth: storyOptions.panelWidth,
        panelHeight: storyOptions.panelHeight,
        exportMime: storyOptions.exportMime,
        exportQuality: storyOptions.exportQuality
      });
      if (!result?.blob) return null;
      const dataUrl = await this._blobToDataUrl(result.blob);
      if (!dataUrl) return null;
      return { dataUrl, meta: result.meta || null };
    } catch (error) {
      this.logger.warn('Story quote-fit generation failed:', error);
      return null;
    }
  }

  /**
   * Compact JPEG for quilt-screen predecessor preview (~100–200 KB).
   * Full-res PNG compose stays available via generateInstagramStoryQuoteFitImageData.
   * Bump ODQ_STORY_QUOTE_FIT_PREVIEW_RENDER_VERSION when the bake shape changes.
   */
  async generateInstagramStoryQuoteFitPreviewJpegImageData(
    blocks,
    quoteObj = null,
    dateKey = null,
    storyOptions = {}
  ) {
    try {
      const result = await this.generateInstagramStoryQuoteFitImageData(blocks, quoteObj, dateKey, {
        ...storyOptions,
        panelWidth: Number(storyOptions.panelWidth) > 0 ? Number(storyOptions.panelWidth) : 810,
        panelHeight: Number(storyOptions.panelHeight) > 0 ? Number(storyOptions.panelHeight) : 1440,
        exportMime: 'image/jpeg',
        exportQuality:
          Number(storyOptions.exportQuality) > 0 ? Number(storyOptions.exportQuality) : 0.72
      });
      return result?.dataUrl || null;
    } catch (error) {
      this.logger.warn('Story quote-fit preview JPEG generation failed:', error);
      return null;
    }
  }

  /** Carousel slide 0b: green mat + centered speaker cutout + vinyl speaker name. */
  async generateInstagramCarouselSlide0bSpeakerNameImageData(
    blocks,
    quoteObj = null,
    dateKey = null,
    carouselOptions = {}
  ) {
    const compose =
      globalThis.composeCarouselSlide0bSpeakerName ||
      globalThis.IgCarouselSlide0bSpeakerNameCompose?.composeCarouselSlide0bSpeakerName;
    if (typeof compose !== 'function') return null;
    const dk = dateKey || Utils.getTodayKey();
    const q = await this._quoteForLayoutBSpeakerImage(quoteObj, dk);
    const quoteAuthor = String(q.author ?? '').trim();
    const app = typeof window !== 'undefined' ? window.app : null;
    const profileName = app?.getQuoteSpeakerProfile?.(q)?.name || '';
    const speakerName = String(
      carouselOptions.speakerName ||
        q.speakerName ||
        q.speaker_name ||
        profileName ||
        quoteAuthor ||
        ''
    )
      .replace(/^\s*[—-]\s*/, '')
      .trim();
    if (!speakerName) return null;

    const speakerPortraitUrl = this._layoutBSpeakerPortraitUrlResolved(q);
    const speakerCutoutUrl = await this.resolveLayoutBSpeakerCutoutUrl(q, quoteAuthor);
    if (!speakerCutoutUrl && !speakerPortraitUrl) return null;

    const speakerImageForCanvas = await this._speakerOverlayImageUrlForLayoutBCompose(
      speakerCutoutUrl,
      speakerPortraitUrl,
      q
    );
    if (!speakerImageForCanvas) return null;

    const washColor = String(
      carouselOptions.washColor ||
        (typeof window !== 'undefined' && window.app?.getSpeakerCutoutWashColor?.()) ||
        (typeof window !== 'undefined' && window.app?.getMostPopularQuiltColor?.(blocks)?.color) ||
        (typeof globalThis.CONFIG !== 'undefined' &&
          globalThis.CONFIG.APP &&
          globalThis.CONFIG.APP.defaultColor) ||
        '#ea9b9a'
    ).trim();

    try {
      const quiltScreenBlob = await this._getInstagramQuiltScreenSourceBlob(
        blocks,
        dk,
        'ig_carousel_slide0b_speaker_name_source'
      );
      const result = await compose({
        dateKey: dk,
        speakerName,
        speakerImageUrl: speakerImageForCanvas,
        cutoutSourceUrl: speakerCutoutUrl || speakerPortraitUrl || '',
        washColor,
        isCutoutPng: !!(speakerCutoutUrl && /speaker-cutouts(?:%2F|\/)/i.test(speakerCutoutUrl)),
        speakerFrameFrac: carouselOptions.speakerFrameFrac,
        speakerTopFrac: carouselOptions.speakerTopFrac,
        speakerNameGapPx: carouselOptions.speakerNameGapPx,
        speakerRotateDeg: carouselOptions.speakerRotateDeg,
        quiltBlob: quiltScreenBlob || null,
        quiltBgZoom: carouselOptions.quiltBgZoom,
        quiltFit: carouselOptions.quiltFit || 'cover'
      });
      if (!result?.blob) return null;
      const dataUrl = await this._blobToDataUrl(result.blob);
      if (!dataUrl) return null;
      return { dataUrl, meta: result.meta || null };
    } catch (error) {
      this.logger.warn('Carousel slide 0b speaker-name generation failed:', error);
      return null;
    }
  }

  /** Carousel slide 4: quilt bg + centered yesterday contributor-count card. */
  async generateInstagramCarouselYesterdayStatsSlideImageData(
    blocks,
    dateKey = null,
    carouselOptions = {}
  ) {
    const compose =
      globalThis.composeCarouselYesterdayStatsSlideFromQuiltBlob ||
      globalThis.IgCarouselYesterdayStatsSlideCompose?.composeCarouselYesterdayStatsSlideFromQuiltBlob;
    if (typeof compose !== 'function') return null;
    const dk = dateKey || Utils.getTodayKey();
    const contributorCount = Math.max(
      1,
      Math.floor(
        Number(carouselOptions.contributorCount) ||
          this._resolveIgCarouselContributorCount(carouselOptions.contributors, blocks)
      )
    );

    const quiltScreenBlob = await this._getInstagramQuiltScreenSourceBlob(
      blocks,
      dk,
      'ig_carousel_yesterday_stats_slide_source'
    );
    if (!quiltScreenBlob) return null;

    try {
      const result = await compose(quiltScreenBlob, {
        dateKey: dk,
        contributorCount,
        quiltCanvasRect: carouselOptions.quiltCanvasRect || null,
        quiltBgZoom: carouselOptions.quiltBgZoom,
        quiltFit: carouselOptions.quiltFit || 'cover',
        cardsPngBase64: carouselOptions.cardsPngBase64,
        cardsLayerLogicalWidth: carouselOptions.cardsLayerLogicalWidth,
        cardsLayerLogicalHeight: carouselOptions.cardsLayerLogicalHeight,
        cardsLayerDeviceScaleFactor: carouselOptions.cardsLayerDeviceScaleFactor,
        cardPieceRects: carouselOptions.cardPieceRects
      });
      if (!result?.blob) return null;
      const dataUrl = await this._blobToDataUrl(result.blob);
      if (!dataUrl) return null;
      return { dataUrl, meta: result.meta || null };
    } catch (error) {
      this.logger.warn('Yesterday stats carousel slide generation failed:', error);
      return null;
    }
  }

  _isShortCarouselQuote(quoteObj) {
    if (!quoteObj) return false;
    const text = String(quoteObj.text ?? quoteObj.body ?? quoteObj.quote ?? '').trim();
    if (!text) return false;
    const words = text.split(/\s+/).filter(Boolean);
    return text.length <= 90 || words.length <= 14;
  }

  async _resolveCarouselSpeakerSeamOverlay(quoteObj, blocks, dateKey) {
    const dk = dateKey || Utils.getTodayKey();
    const q = await this._quoteForLayoutBSpeakerImage(quoteObj, dk);
    const quoteAuthor = String(q.author ?? '').trim();
    const speakerPortraitUrl = this._layoutBSpeakerPortraitUrlResolved(q);
    const speakerCutoutUrl = await this.resolveLayoutBSpeakerCutoutUrl(q, quoteAuthor);
    if (!speakerCutoutUrl && !speakerPortraitUrl) return null;
    const imageUrl = await this._speakerOverlayImageUrlForLayoutBCompose(
      speakerCutoutUrl,
      speakerPortraitUrl,
      q
    );
    if (!imageUrl) return null;
    const washColor = String(
      (typeof window !== 'undefined' && window.app?.getSpeakerCutoutWashColor?.()) ||
        (typeof window !== 'undefined' && window.app?.getMostPopularQuiltColor?.(blocks)?.color) ||
        (typeof globalThis.CONFIG !== 'undefined' &&
          globalThis.CONFIG.APP &&
          globalThis.CONFIG.APP.defaultColor) ||
        '#ea9b9a'
    ).trim();
    return {
      imageUrl,
      cutoutSourceUrl: speakerCutoutUrl || speakerPortraitUrl || '',
      washColor,
      dateKey: dk
    };
  }

  async _resolveCarouselSpeakerSeamRectFromOverlay(overlay, { shortQuote = false } = {}) {
    if (!overlay?.imageUrl) return null;
    const resolve =
      globalThis.resolveCarouselSpeakerSeamRectFromImageUrl ||
      globalThis.IgContributorCarouselCompose?.resolveCarouselSpeakerSeamRectFromImageUrl;
    if (typeof resolve !== 'function') return null;
    try {
      return await resolve(overlay.imageUrl, { shortQuote });
    } catch (err) {
      this.logger.warn('Carousel speaker rect resolve skipped:', err?.message || err);
      return null;
    }
  }

  /**
   * Integrated IG carousel generation (internal roles):
   *   layout B → reflection → contributors → slide 4 quilt-only mat (each slide composed independently).
   * Post order (Instagram swipe / carouselSlideNUrl):
   *   1 = layout B, 2 = reflection, 3 = contributors, 4 = quilt-only mat (1, 3, 4, 2 content).
   */
  async buildIntegratedInstagramCarouselImageData(
    blocks,
    contributors = [],
    quoteObj = null,
    dateKey = null,
    carouselOptions = {}
  ) {
    const dk = dateKey || Utils.getTodayKey();
    await this._prefetchQuiltExportTune(dk);
    const overlay = await this._resolveCarouselSpeakerSeamOverlay(quoteObj, blocks, dk);
    const shortCarouselQuote = this._isShortCarouselQuote(quoteObj);
    const layoutBSlide1 = overlay
      ? await this._generateInstagramPostLayoutBImageData(blocks, quoteObj, dk, {
          carouselStoryStyle: true,
          carouselShortQuote: shortCarouselQuote,
          carouselFullWidthQuiltSource: true
        })
      : await this._generateInstagramPostLayoutBImageData(blocks, quoteObj, dk, {
          carouselFullWidthQuiltSource: true
        });
    const layoutMeta = this._lastLayoutBPostComposeMeta || globalThis._lastLayoutBComposeMeta || null;
    const quiltCanvasRect = layoutMeta?.quiltCanvasRect || null;

    const igContributorCount = Math.max(
      1,
      Math.floor(
        Number(carouselOptions?.contributorCount) ||
          this._resolveIgCarouselContributorCount(contributors, blocks)
      )
    );

    const wantsReflection =
      carouselOptions?.slide2Variant === 'reflection' ||
      carouselOptions?.includeSlide2Reflection === true ||
      carouselOptions?.includeSlide3Reflection === true;

    const [quiltSlides, slide4Result, reflectionSlide, slide0Result, slide0bResult] = await Promise.all([
      this.generateInstagramCarouselSlideImageData(blocks, contributors, dk, {
        quiltCanvasRect,
        quiltFit: 'cover',
        winningQuiltName: carouselOptions?.winningQuiltName || '',
        contributorBgFlip: true
      }),
      CAROUSEL_USE_QUILT_ONLY_SLIDE_4
        ? this.generateInstagramCarouselQuiltOnlySlideImageData(blocks, dk, {
            quiltCanvasRect,
            quiltFit: 'cover',
            quiltBgZoom: carouselOptions?.quiltBgZoom
          })
        : this.generateInstagramCarouselYesterdayStatsSlideImageData(blocks, dk, {
            contributorCount: igContributorCount,
            contributors,
            quiltCanvasRect,
            quiltFit: 'cover',
            quiltBgZoom: carouselOptions?.quiltBgZoom,
            cardsPngBase64: carouselOptions?.yesterdayStatsCardsPngBase64,
            cardsLayerLogicalWidth: carouselOptions?.yesterdayStatsCardsLayerLogicalWidth,
            cardsLayerLogicalHeight: carouselOptions?.yesterdayStatsCardsLayerLogicalHeight,
            cardsLayerDeviceScaleFactor: carouselOptions?.yesterdayStatsCardsLayerDeviceScaleFactor
          }),
      wantsReflection
        ? this.generateInstagramCarouselReflectionSlideImageData(blocks, quoteObj, dk, {
            quiltCanvasRect,
            quiltFit: 'cover',
            quiltBgZoom: carouselOptions?.quiltBgZoom,
            cardsPngBase64: carouselOptions?.cardsPngBase64,
            cardsLayerLogicalWidth: carouselOptions?.cardsLayerLogicalWidth,
            cardsLayerLogicalHeight: carouselOptions?.cardsLayerLogicalHeight,
            cardsLayerDeviceScaleFactor: carouselOptions?.cardsLayerDeviceScaleFactor,
            cardPieceRects: carouselOptions?.cardPieceRects
          })
        : Promise.resolve(null),
      this.generateInstagramCarouselSlide0QuoteFitImageData(blocks, quoteObj, dk, {
        quiltFit: 'cover',
        quiltBgZoom: carouselOptions?.quiltBgZoom,
        quiltBgOffsetY: carouselOptions?.quiltBgOffsetY
      }),
      this.generateInstagramCarouselSlide0bSpeakerNameImageData(blocks, quoteObj, dk, {
        quiltCanvasRect,
        quiltFit: 'cover',
        quiltBgZoom: carouselOptions?.quiltBgZoom
      })
    ]);

    const slide4DataUrl = slide4Result?.dataUrl || null;
    const slide4Meta = slide4Result?.meta || null;
    const reflectionSlideDataUrl = reflectionSlide?.dataUrl || null;
    const reflectionSlideMeta = reflectionSlide?.meta || null;
    const slide0DataUrl = slide0Result?.dataUrl || null;
    const slide0Meta = slide0Result?.meta || null;
    const slide0bDataUrl = slide0bResult?.dataUrl || null;
    const slide0bMeta = slide0bResult?.meta || null;

    if (!layoutBSlide1 || !slide4DataUrl || !quiltSlides?.slide2) return null;

    const carouselSlideContributors = quiltSlides.slide2;

    return {
      /** Quote-fit Helvetica on quilt frame — preferred first carousel image when present. */
      carouselSlide0: slide0DataUrl,
      /** Speaker cutout + vinyl name on green mat. */
      carouselSlide0b: slide0bDataUrl,
      carouselSlide1: layoutBSlide1,
      carouselSlide2: reflectionSlideDataUrl || null,
      carouselSlide3: carouselSlideContributors,
      carouselSlide4: slide4DataUrl,
      /** @deprecated preview alias — reflection is carousel post slide 2 */
      carouselSlide2Reflection: reflectionSlideDataUrl || null,
      meta: {
        ...(quiltSlides.meta || {}),
        quoteFitSlide0: slide0Meta,
        speakerNameSlide0b: slide0bMeta,
        reflectionSlide: reflectionSlideMeta,
        quiltOnlySlide4: CAROUSEL_USE_QUILT_ONLY_SLIDE_4 ? slide4Meta : null,
        yesterdayStatsSlide: CAROUSEL_USE_QUILT_ONLY_SLIDE_4 ? null : slide4Meta,
        carouselPostOrder: slide0DataUrl ? [0, 3, 4, 2] : [1, 3, 4, 2]
      }
    };
  }

  /** Primary Zapier feed image — seamless contributor carousel (replaces centered classic). */
  async generateInstagramCarouselPrimaryImageData(blocks, contributors = [], dateKey = null) {
    const slides = await this.generateInstagramCarouselSlideImageData(blocks, contributors, dateKey);
    if (!slides?.slide1 || !slides?.slide2) return null;
    return slides;
  }

  /** Match the live quilt container aspect — not IG 9:16 — so mirror crop matches the quilt screen. */
  getLiveQuiltScreenAspect(fallbackAspect = 9 / 16) {
    const fallback =
      Number.isFinite(fallbackAspect) && fallbackAspect > 0 ? fallbackAspect : 9 / 16;
    if (typeof document === 'undefined') return fallback;
    const container =
      document.querySelector('#screen-quilt .quilt-container') ||
      document.querySelector('.quilt-container');
    const rect = container?.getBoundingClientRect?.();
    if (rect && rect.width > 0 && rect.height > 0) {
      return rect.width / rect.height;
    }
    const svg = document.getElementById('quilt');
    const svgRect = svg?.getBoundingClientRect?.();
    if (svgRect && svgRect.width > 0 && svgRect.height > 0) {
      return svgRect.width / svgRect.height;
    }
    return fallback;
  }

  hasRenderedQuiltSvgForBlocks(quiltSVG, blocks) {
    if (!quiltSVG || !Array.isArray(blocks) || blocks.length === 0) return false;
    const renderedIds = Array.from(quiltSVG.querySelectorAll('[data-block-id]'))
      .map((node) => node.getAttribute('data-block-id'))
      .filter(Boolean);
    const expectedIds = blocks.map((block) => block?.id).filter(Boolean);
    if (expectedIds.length === blocks.length) {
      const renderedSet = new Set(renderedIds);
      return expectedIds.every((id) => renderedSet.has(id));
    }
    if (renderedIds.length) {
      return new Set(renderedIds).size >= blocks.length;
    }
    return quiltSVG.querySelectorAll('polygon, rect').length >= blocks.length;
  }

  /** Rasterize the live quilt crop as it appears on the quilt screen. */
  async rasterizeVisibleQuiltSvgToPngBlob(quiltSVG, blocks = null, options = {}) {
    if (!quiltSVG) return null;
    const rect = quiltSVG.getBoundingClientRect?.();
    const requestedAspect =
      Number.isFinite(options.targetAspect) && options.targetAspect > 0
        ? options.targetAspect
        : null;
    const screenAspect =
      requestedAspect ||
      (rect && rect.width > 0 && rect.height > 0
        ? rect.width / rect.height
        : null);
    return this.rasterizeQuiltSvgToPngBlob(quiltSVG, {
      underpaintBlocks: blocks,
      targetAspect: screenAspect || undefined,
      maxEdge: options.maxEdge,
      dateKey: options.dateKey,
      exportModeLabel:
        options.exportModeLabel || (screenAspect ? 'zapier_screen_aspect_crop' : undefined),
      skipExportUnderpaint: options.skipExportUnderpaint === true,
      exportPrimaryQuiltOnly: options.exportPrimaryQuiltOnly === true,
      exportFullQuiltField: options.exportFullQuiltField === true,
      exportMatchQuiltScreen: options.exportMatchQuiltScreen === true,
      exportFillFrame: options.exportFillFrame === true,
      exportMatteFill: options.exportMatteFill,
      viewBoxPaddingRatio: options.viewBoxPaddingRatio,
      preserveAspectRatio: options.preserveAspectRatio,
      instagramTrueWhiteMatte: options.instagramTrueWhiteMatte === true,
      quiltFingerprint: options.quiltFingerprint
    });
  }

  cropViewBoxForSliceAspect(viewBoxParts, targetAspect) {
    if (
      !Array.isArray(viewBoxParts) ||
      viewBoxParts.length !== 4 ||
      !Number.isFinite(targetAspect) ||
      targetAspect <= 0
    ) {
      return null;
    }
    const [x, y, width, height] = viewBoxParts;
    if (![x, y, width, height].every(Number.isFinite) || width <= 0 || height <= 0) return null;
    const sourceAspect = width / height;
    if (Math.abs(sourceAspect - targetAspect) < 0.0001) return [x, y, width, height];
    if (targetAspect > sourceAspect) {
      const croppedHeight = width / targetAspect;
      const ch = Math.min(height, croppedHeight);
      /** Center vertically (top-anchored crops left a huge empty band above the quilt in 9:16 exports). */
      const y0 = y + Math.max(0, (height - ch) / 2);
      return [x, y0, width, ch];
    }
    const croppedWidth = height * targetAspect;
    return [x + (width - croppedWidth) / 2, y, Math.min(width, croppedWidth), height];
  }

  /** Block bounds viewBox for exports (excludes on-screen mirror field). */
  viewBoxPartsFromQuiltBlocks(blocks, paddingRatio = 0.02) {
    if (!Array.isArray(blocks) || !blocks.length) return null;
    const minX = Math.min(...blocks.map((b) => Number(b.x)));
    const minY = Math.min(...blocks.map((b) => Number(b.y)));
    const maxX = Math.max(...blocks.map((b) => Number(b.x) + Number(b.width)));
    const maxY = Math.max(...blocks.map((b) => Number(b.y) + Number(b.height)));
    const quiltW = maxX - minX;
    const quiltH = maxY - minY;
    if (!(quiltW > 0) || !(quiltH > 0)) return null;
    const padded = this.padViewBoxParts([minX, minY, quiltW, quiltH], paddingRatio);
    return padded || [minX, minY, quiltW, quiltH];
  }

  /**
   * Expand (never shrink) a viewBox so every block stays inside a target aspect (e.g. 9:16).
   */
  expandViewBoxToAspect(viewBoxParts, targetAspect) {
    if (
      !Array.isArray(viewBoxParts) ||
      viewBoxParts.length !== 4 ||
      !Number.isFinite(targetAspect) ||
      targetAspect <= 0
    ) {
      return null;
    }
    const [x, y, width, height] = viewBoxParts;
    if (![x, y, width, height].every(Number.isFinite) || width <= 0 || height <= 0) return null;
    const sourceAspect = width / height;
    if (Math.abs(sourceAspect - targetAspect) < 0.0001) return [x, y, width, height];
    if (sourceAspect > targetAspect) {
      const nh = width / targetAspect;
      const dy = (nh - height) / 2;
      return [x, y - dy, width, nh];
    }
    const nw = height * targetAspect;
    const dx = (nw - width) / 2;
    return [x - dx, y, nw, height];
  }

  stripMirroredQuiltFieldFromSvgClone(svgClone) {
    if (!svgClone) return;
    svgClone
      .querySelectorAll(
        '#quiltMirroredFieldLayer, #quiltExportMirroredGapUnderpaint, [data-export-mirrored-field="1"]'
      )
      .forEach((node) => node.remove());
  }

  padViewBoxParts(viewBoxParts, paddingRatio) {
    if (
      !Array.isArray(viewBoxParts) ||
      viewBoxParts.length !== 4 ||
      !Number.isFinite(paddingRatio) ||
      paddingRatio <= 0
    ) {
      return null;
    }
    const [x, y, width, height] = viewBoxParts;
    if (![x, y, width, height].every(Number.isFinite) || width <= 0 || height <= 0) return null;
    const dx = width * paddingRatio;
    const dy = height * paddingRatio;
    return [x - dx, y - dy, width + dx * 2, height + dy * 2];
  }

  addQuiltExportUnderpaint(svgClone, explicitBlocks = null) {
    if (!svgClone) return;
    const blocks = Array.isArray(explicitBlocks) && explicitBlocks.length
      ? explicitBlocks
      : (typeof window !== 'undefined' && Array.isArray(window.app?.quiltEngine?.blocks)
        ? window.app.quiltEngine.blocks
        : []);
    if (!blocks.length) return;
    const ns = 'http://www.w3.org/2000/svg';
    const fieldLayer = svgClone.querySelector('#quiltFieldLayer') || svgClone;
    const parallaxLayer = fieldLayer.querySelector('#quiltParallaxLayer');
    const fieldTransform = String(fieldLayer.getAttribute('transform') || '');
    const transformMatch = /translate\(\s*([-0-9.]+)(?:[\s,]+[-0-9.]+)?\s*\)\s*scale\(\s*([-0-9.]+)/.exec(
      fieldTransform
    );
    const stretchScale = transformMatch ? Number(transformMatch[2]) : 1;
    const effectiveStretch =
      Number.isFinite(stretchScale) && stretchScale > 1 ? stretchScale : 1;
    const bleed = effectiveStretch > 1 ? 3.5 + 1 : 2 + 0.5;

    const buildUnderpaintGroup = (id) => {
      const group = document.createElementNS(ns, 'g');
      group.setAttribute('id', id);
      group.setAttribute('aria-hidden', 'true');
      group.setAttribute('pointer-events', 'none');
      group.setAttribute('data-export-underpaint', '1');
      blocks.forEach((block) => {
        if (!block) return;
        const x = Number(block.x);
        const y = Number(block.y);
        const w = Math.max(0, Number(block.width));
        const h = Math.max(0, Number(block.height));
        const fill = Utils.validateHexColor(block.color) ? block.color : '#d8d4cf';
        if (![x, y, w, h].every(Number.isFinite) || w <= 0 || h <= 0) return;
        const rect = document.createElementNS(ns, 'rect');
        rect.setAttribute('x', String(x - bleed));
        rect.setAttribute('y', String(y - bleed));
        rect.setAttribute('width', String(w + bleed * 2));
        rect.setAttribute('height', String(h + bleed * 2));
        rect.setAttribute('fill', fill);
        rect.setAttribute('stroke', 'none');
        group.appendChild(rect);
      });
      return group.childNodes.length ? group : null;
    };

    const insertLayerUnderpaint = (layerEl, id) => {
      if (!layerEl) return;
      const layerUnderpaint = buildUnderpaintGroup(id);
      if (!layerUnderpaint) return;
      const layerTransform = layerEl.getAttribute('transform');
      if (layerTransform) layerUnderpaint.setAttribute('transform', layerTransform);
      fieldLayer.insertBefore(layerUnderpaint, layerEl);
    };

    const underpaint = buildUnderpaintGroup('quiltExportGapUnderpaint');
    if (underpaint) {
      fieldLayer.insertBefore(underpaint, parallaxLayer || fieldLayer.firstChild);
    }
    insertLayerUnderpaint(
      fieldLayer.querySelector('#quiltMirroredFieldLayer'),
      'quiltExportMirroredGapUnderpaint'
    );
    insertLayerUnderpaint(
      fieldLayer.querySelector('#quiltDuplicateBottomLayer1'),
      'quiltExportDuplicateGapUnderpaint1'
    );
    insertLayerUnderpaint(
      fieldLayer.querySelector('#quiltDuplicateBottomLayer2'),
      'quiltExportDuplicateGapUnderpaint2'
    );
  }

  _svgTransformAttributeToMatrix(transformStr) {
    if (!transformStr || typeof document === 'undefined' || typeof DOMMatrix === 'undefined') {
      return null;
    }
    try {
      const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
      svg.setAttribute('width', '1');
      svg.setAttribute('height', '1');
      const g = document.createElementNS('http://www.w3.org/2000/svg', 'g');
      g.setAttribute('transform', transformStr);
      svg.appendChild(g);
      const host = document.createElement('div');
      host.style.cssText =
        'position:fixed;left:-9999px;top:-9999px;width:0;height:0;overflow:hidden;pointer-events:none;visibility:hidden;';
      host.appendChild(svg);
      document.documentElement.appendChild(host);
      const ctm = g.getCTM();
      host.remove();
      return ctm ? new DOMMatrix(ctm) : null;
    } catch (_) {
      return null;
    }
  }

  drawQuiltExportCanvasUnderpaint(ctx, width, height, viewBoxParts, explicitBlocks = null, svgClone = null) {
    if (!ctx || !Array.isArray(viewBoxParts) || viewBoxParts.length !== 4) return;
    const blocks = Array.isArray(explicitBlocks) && explicitBlocks.length
      ? explicitBlocks
      : (typeof window !== 'undefined' && Array.isArray(window.app?.quiltEngine?.blocks)
        ? window.app.quiltEngine.blocks
        : []);
    if (!blocks.length) return;
    const [vbX, vbY, vbW, vbH] = viewBoxParts;
    if (![vbX, vbY, vbW, vbH].every(Number.isFinite) || vbW <= 0 || vbH <= 0) return;
    const fieldTransform = String(svgClone?.querySelector?.('#quiltFieldLayer')?.getAttribute('transform') || '');
    const transformMatch = /translate\(\s*([-0-9.]+)(?:[\s,]+[-0-9.]+)?\s*\)\s*scale\(\s*([-0-9.]+)/.exec(fieldTransform);
    const stretchAnchor = transformMatch ? Number(transformMatch[1]) : 0;
    const stretchScale = transformMatch ? Number(transformMatch[2]) : 1;
    const hasHorizontalStretch =
      Number.isFinite(stretchAnchor) &&
      Number.isFinite(stretchScale) &&
      Math.abs(stretchScale - 1) > 1e-6;
    const sx = width / vbW;
    const sy = height / vbH;
    const bleed = 8;
    const mapRectToCanvas = (x1, y1, x2, y2) => {
      const px = (Math.min(x1, x2) - vbX) * sx;
      const py = (Math.min(y1, y2) - vbY) * sy;
      const pw = Math.abs(x2 - x1) * sx;
      const ph = Math.abs(y2 - y1) * sy;
      ctx.fillRect(px, py, pw, ph);
    };
    const transformPoint = (x, y, matrix) => {
      if (!matrix || typeof DOMPoint === 'undefined') return { x, y };
      const p = matrix.transformPoint(new DOMPoint(x, y));
      return { x: p.x, y: p.y };
    };
    const drawFieldBlocks = (layerMatrix, useLegacyMirrorY) => {
      blocks.forEach((block) => {
        if (!block) return;
        const bx = Number(block.x);
        const by = Number(block.y);
        const bw = Math.max(0, Number(block.width));
        const bh = Math.max(0, Number(block.height));
        if (![bx, by, bw, bh].every(Number.isFinite) || bw <= 0 || bh <= 0) return;
        const fill = Utils.validateHexColor(block.color) ? block.color : '#d8d4cf';
        let x1 = bx - bleed;
        let x2 = bx + bw + bleed;
        let y1 = by - bleed;
        let y2 = by + bh + bleed;
        if (hasHorizontalStretch) {
          x1 = stretchAnchor + (x1 - stretchAnchor) * stretchScale;
          x2 = stretchAnchor + (x2 - stretchAnchor) * stretchScale;
        }
        if (useLegacyMirrorY) {
          const sourceMinY = Math.min(...blocks.map((b) => Number(b?.y) || 0));
          const sourceMaxY = Math.max(...blocks.map((b) => (Number(b?.y) || 0) + (Number(b?.height) || 0)));
          const mirrorSeam = sourceMaxY - sourceMinY;
          const oldY1 = y1;
          const oldY2 = y2;
          y1 = sourceMinY + mirrorSeam + (sourceMaxY - oldY2);
          y2 = sourceMinY + mirrorSeam + (sourceMaxY - oldY1);
        } else if (layerMatrix) {
          const corners = [
            transformPoint(x1, y1, layerMatrix),
            transformPoint(x2, y1, layerMatrix),
            transformPoint(x1, y2, layerMatrix),
            transformPoint(x2, y2, layerMatrix)
          ];
          x1 = Math.min(...corners.map((p) => p.x));
          x2 = Math.max(...corners.map((p) => p.x));
          y1 = Math.min(...corners.map((p) => p.y));
          y2 = Math.max(...corners.map((p) => p.y));
        }
        ctx.fillStyle = fill;
        mapRectToCanvas(x1, y1, x2, y2);
      });
    };
    ctx.save();
    drawFieldBlocks(null, false);
    if (svgClone?.querySelector?.('#quiltMirroredFieldLayer')) {
      drawFieldBlocks(null, true);
    }
    ['#quiltDuplicateBottomLayer1', '#quiltDuplicateBottomLayer2'].forEach((selector) => {
      const layer = svgClone?.querySelector?.(selector);
      if (!layer) return;
      const matrix = this._svgTransformAttributeToMatrix(layer.getAttribute('transform') || '');
      drawFieldBlocks(matrix, false);
    });
    ctx.restore();
  }

  healQuiltExportRasterSeams(ctx, width, height) {
    if (!ctx || !(width > 0) || !(height > 0)) return;
    let img;
    try {
      img = ctx.getImageData(0, 0, width, height);
    } catch (_) {
      return;
    }
    const data = img.data;
    const source = new Uint8ClampedArray(data);
    const idxFor = (x, y) => (y * width + x) * 4;
    const sat = (i) => Math.max(source[i], source[i + 1], source[i + 2]) - Math.min(source[i], source[i + 1], source[i + 2]);
    const luma = (i) => 0.299 * source[i] + 0.587 * source[i + 1] + 0.114 * source[i + 2];
    const isNeutralSeam = (i) => {
      if (source[i + 3] < 220) return true;
      const nearPaper =
        Math.abs(source[i] - 246) <= 18 &&
        Math.abs(source[i + 1] - 244) <= 18 &&
        Math.abs(source[i + 2] - 241) <= 18;
      return nearPaper || (luma(i) >= 224 && sat(i) <= 26);
    };
    const isQuiltColor = (i) => source[i + 3] >= 220 && luma(i) < 246 && sat(i) >= 28;
    const hasNearbyColor = (x, y) => {
      const probes = [
        [-3, 0], [3, 0], [0, -3], [0, 3],
        [-2, -2], [2, -2], [-2, 2], [2, 2]
      ];
      return probes.some(([dx, dy]) => {
        const nx = x + dx;
        const ny = y + dy;
        return nx >= 0 && ny >= 0 && nx < width && ny < height && isQuiltColor(idxFor(nx, ny));
      });
    };
    const radius = 5;
    let changed = 0;
    for (let y = 1; y < height - 1; y++) {
      for (let x = 1; x < width - 1; x++) {
        const i = idxFor(x, y);
        if (!isNeutralSeam(i) || !hasNearbyColor(x, y)) continue;
        let r = 0;
        let g = 0;
        let b = 0;
        let count = 0;
        for (let dy = -radius; dy <= radius; dy++) {
          const ny = y + dy;
          if (ny < 0 || ny >= height) continue;
          for (let dx = -radius; dx <= radius; dx++) {
            const nx = x + dx;
            if (nx < 0 || nx >= width) continue;
            const ni = idxFor(nx, ny);
            if (!isQuiltColor(ni)) continue;
            r += source[ni];
            g += source[ni + 1];
            b += source[ni + 2];
            count++;
          }
        }
        if (count < 3) continue;
        data[i] = Math.round(r / count);
        data[i + 1] = Math.round(g / count);
        data[i + 2] = Math.round(b / count);
        data[i + 3] = 255;
        changed++;
      }
    }
    if (changed > 0) {
      ctx.putImageData(img, 0, 0);
    }
  }

  /**
   * After film grain, restore pure white only in the viewBox margin outside the blocks' bounds.
   * Does not touch pixels inside the block envelope (quilt + internal gaps keep grain/texture).
   */
  bleachInstagramOuterMatteAroundBlockBounds(
    ctx,
    outW,
    outH,
    viewBoxParts,
    blocks,
    svgClone = null,
    matteFill = '#ffffff'
  ) {
    if (!ctx || !Array.isArray(viewBoxParts) || viewBoxParts.length !== 4) return;
    if (!Array.isArray(blocks) || !blocks.length) return;
    const [vbX, vbY, vbW, vbH] = viewBoxParts;
    if (![vbX, vbY, vbW, vbH].every(Number.isFinite) || vbW <= 0 || vbH <= 0) return;
    const fieldTransform = String(svgClone?.querySelector?.('#quiltFieldLayer')?.getAttribute('transform') || '');
    const transformMatch = /translate\(\s*([-0-9.]+)(?:[\s,]+[-0-9.]+)?\s*\)\s*scale\(\s*([-0-9.]+)/.exec(fieldTransform);
    const stretchAnchor = transformMatch ? Number(transformMatch[1]) : 0;
    const stretchScale = transformMatch ? Number(transformMatch[2]) : 1;
    const hasHorizontalStretch =
      Number.isFinite(stretchAnchor) &&
      Number.isFinite(stretchScale) &&
      Math.abs(stretchScale - 1) > 1e-6;
    let qx1 = Infinity;
    let qy1 = Infinity;
    let qx2 = -Infinity;
    let qy2 = -Infinity;
    for (const b of blocks) {
      if (!b) continue;
      const bx = Number(b.x);
      const by = Number(b.y);
      const bw = Number(b.width);
      const bh = Number(b.height);
      if (![bx, by, bw, bh].every(Number.isFinite) || bw <= 0 || bh <= 0) continue;
      let x1 = bx;
      let x2 = bx + bw;
      if (hasHorizontalStretch) {
        x1 = stretchAnchor + (x1 - stretchAnchor) * stretchScale;
        x2 = stretchAnchor + (x2 - stretchAnchor) * stretchScale;
      }
      qx1 = Math.min(qx1, x1);
      qy1 = Math.min(qy1, by);
      qx2 = Math.max(qx2, x2);
      qy2 = Math.max(qy2, by + bh);
    }
    /**
     * Screen exports include the mirrored field in the SVG viewBox; extend bleach to that full height.
     * Primary-only Instagram exports strip the mirror and must not extend past block bounds.
     */
    if (svgClone?.querySelector?.('#quiltMirroredFieldLayer')) {
      qy2 = Math.max(qy2, vbY + vbH);
    }
    if (!Number.isFinite(qx1) || qx2 <= qx1 || qy2 <= qy1) return;
    const bleed = Math.max(6, Math.min(outW, outH) * 0.006);
    const sx = outW / vbW;
    const sy = outH / vbH;
    let left = (qx1 - vbX) * sx - bleed;
    let top = (qy1 - vbY) * sy - bleed;
    let right = (qx2 - vbX) * sx + bleed;
    let bottom = (qy2 - vbY) * sy + bleed;
    left = Math.max(0, Math.floor(left));
    top = Math.max(0, Math.floor(top));
    right = Math.min(outW, Math.ceil(right));
    bottom = Math.min(outH, Math.ceil(bottom));
    ctx.save();
    ctx.fillStyle = matteFill;
    if (top > 0) ctx.fillRect(0, 0, outW, top);
    if (bottom < outH) ctx.fillRect(0, bottom, outW, outH - bottom);
    if (left > 0) ctx.fillRect(0, top, left, Math.max(0, bottom - top));
    if (right < outW) ctx.fillRect(right, top, outW - right, Math.max(0, bottom - top));
    ctx.restore();
  }

  /** Same outer-matte bleach as `bleachInstagramOuterMatteAroundBlockBounds` for the block-only raster (no SVG viewBox). */
  bleachInstagramOuterMatteForBlockRaster(
    ctx,
    outW,
    outH,
    sourceX,
    sourceY,
    scaleX,
    scaleY,
    blocks,
    matteFill = '#ffffff'
  ) {
    if (!ctx || !Array.isArray(blocks) || !blocks.length) return;
    let qx1 = Infinity;
    let qy1 = Infinity;
    let qx2 = -Infinity;
    let qy2 = -Infinity;
    for (const b of blocks) {
      if (!b) continue;
      const bx = Number(b.x);
      const by = Number(b.y);
      const bw = Number(b.width);
      const bh = Number(b.height);
      if (![bx, by, bw, bh].every(Number.isFinite) || bw <= 0 || bh <= 0) continue;
      qx1 = Math.min(qx1, bx);
      qy1 = Math.min(qy1, by);
      qx2 = Math.max(qx2, bx + bw);
      qy2 = Math.max(qy2, by + bh);
    }
    if (!Number.isFinite(qx1) || qx2 <= qx1 || qy2 <= qy1) return;
    const bleed = Math.max(6, Math.min(outW, outH) * 0.006);
    let left = (qx1 - sourceX) * scaleX - bleed;
    let top = (qy1 - sourceY) * scaleY - bleed;
    let right = (qx2 - sourceX) * scaleX + bleed;
    let bottom = (qy2 - sourceY) * scaleY + bleed;
    left = Math.max(0, Math.floor(left));
    top = Math.max(0, Math.floor(top));
    right = Math.min(outW, Math.ceil(right));
    bottom = Math.min(outH, Math.ceil(bottom));
    ctx.save();
    ctx.fillStyle = matteFill;
    if (top > 0) ctx.fillRect(0, 0, outW, top);
    if (bottom < outH) ctx.fillRect(0, bottom, outW, outH - bottom);
    if (left > 0) ctx.fillRect(0, top, left, Math.max(0, bottom - top));
    if (right < outW) ctx.fillRect(right, top, outW - right, Math.max(0, bottom - top));
    ctx.restore();
  }

  /** High-res PNG blob from the live quilt SVG (same path as layout B story share). */
  async rasterizeQuiltSvgToPngBlob(quiltSVG, options = {}) {
    return new Promise((resolve, reject) => {
      try {
        this.lastQuiltExportDebug = null;
        this.lastQuiltExportRawDataUrl = null;
        const svgClone = quiltSVG.cloneNode(true);
        svgClone.setAttribute('xmlns', 'http://www.w3.org/2000/svg');
        svgClone.querySelectorAll('.new-block').forEach((node) => {
          node.classList.remove('new-block');
          node.removeAttribute('opacity');
          node.style.animation = 'none';
        });

        const exportBlocks =
          Array.isArray(options.underpaintBlocks) && options.underpaintBlocks.length
            ? options.underpaintBlocks
            : null;
        const exportPrimaryQuiltOnly = options.exportPrimaryQuiltOnly === true;
        const exportFullQuiltField = options.exportFullQuiltField === true;
        const exportMatchQuiltScreen = options.exportMatchQuiltScreen === true;
        const exportKeepRendererViewBox = options.exportKeepRendererViewBox === true;
        if (exportPrimaryQuiltOnly && !exportMatchQuiltScreen) {
          this.stripMirroredQuiltFieldFromSvgClone(svgClone);
        }

        const viewBox = (quiltSVG.getAttribute('viewBox') || '').trim();
        let viewBoxParts = null;
        let usedLiveScreenViewBox = false;
        let sourceWidth = 1080;
        let sourceHeight = 1080;
        if (exportFullQuiltField && exportKeepRendererViewBox && viewBox) {
          const rendererParts = viewBox.split(/\s+/).map(Number);
          if (rendererParts.length === 4 && rendererParts.every(Number.isFinite)) {
            viewBoxParts = rendererParts;
            usedLiveScreenViewBox = true;
            sourceWidth = Math.max(1, rendererParts[2]);
            sourceHeight = Math.max(1, rendererParts[3]);
          }
        } else if (exportFullQuiltField && exportBlocks) {
          viewBoxParts = this.viewBoxPartsForMirroredQuiltScreen(
            exportBlocks,
            Number(options.viewBoxPaddingRatio) > 0 ? Number(options.viewBoxPaddingRatio) : 0.02,
            options.dateKey
          );
          if (viewBoxParts) {
            sourceWidth = Math.max(1, viewBoxParts[2]);
            sourceHeight = Math.max(1, viewBoxParts[3]);
          }
        } else if (exportPrimaryQuiltOnly && exportBlocks) {
          viewBoxParts = this.viewBoxPartsFromQuiltBlocks(
            exportBlocks,
            Number(options.viewBoxPaddingRatio) > 0 ? Number(options.viewBoxPaddingRatio) : 0.02
          );
          if (viewBoxParts) {
            sourceWidth = Math.max(1, viewBoxParts[2]);
            sourceHeight = Math.max(1, viewBoxParts[3]);
          }
        } else if (exportMatchQuiltScreen && exportBlocks) {
          // Match the live quilt screen fit (xMidYMin meet) — do not recompute export viewBox.
          const liveParts = viewBox.split(/\s+/).map(Number);
          if (liveParts.length === 4 && liveParts.every(Number.isFinite)) {
            viewBoxParts = liveParts;
            usedLiveScreenViewBox = true;
            sourceWidth = Math.max(1, liveParts[2]);
            sourceHeight = Math.max(1, liveParts[3]);
          } else if (typeof this.viewBoxPartsForMirroredQuiltScreen === 'function') {
            viewBoxParts = this.viewBoxPartsForMirroredQuiltScreen(
              exportBlocks,
              Number(options.viewBoxPaddingRatio) > 0 ? Number(options.viewBoxPaddingRatio) : 0.02,
              options.dateKey
            );
            if (viewBoxParts) {
              sourceWidth = Math.max(1, viewBoxParts[2]);
              sourceHeight = Math.max(1, viewBoxParts[3]);
            }
          }
        } else if (viewBox) {
          const parts = viewBox.split(/\s+/).map(Number);
          if (parts.length === 4 && parts.every(Number.isFinite)) {
            viewBoxParts = parts;
            sourceWidth = Math.max(1, parts[2]);
            sourceHeight = Math.max(1, parts[3]);
          }
        } else {
          const rect = quiltSVG.getBoundingClientRect();
          sourceWidth = Math.max(1, Math.round(rect.width));
          sourceHeight = Math.max(1, Math.round(rect.height));
        }

        const targetAspect =
          Number.isFinite(options.targetAspect) && options.targetAspect > 0
            ? options.targetAspect
            : null;
        let croppedViewBoxParts = null;
        let expandedViewBoxParts = null;
        if (targetAspect && viewBoxParts && !exportMatchQuiltScreen) {
          if (exportPrimaryQuiltOnly) {
            expandedViewBoxParts = this.expandViewBoxToAspect(viewBoxParts, targetAspect);
          } else {
            croppedViewBoxParts = this.cropViewBoxForSliceAspect(viewBoxParts, targetAspect);
          }
        }
        const paddedViewBoxParts =
          options.viewBoxPaddingRatio &&
          !exportPrimaryQuiltOnly &&
          !exportMatchQuiltScreen &&
          (croppedViewBoxParts || viewBoxParts)
            ? this.padViewBoxParts(croppedViewBoxParts || viewBoxParts, Number(options.viewBoxPaddingRatio))
            : null;
        let screenExpandedViewBoxParts = null;
        if (exportMatchQuiltScreen && viewBoxParts && targetAspect && !usedLiveScreenViewBox) {
          screenExpandedViewBoxParts = this.cropViewBoxForSliceAspect(viewBoxParts, targetAspect);
        }
        const exportViewBoxParts =
          exportMatchQuiltScreen && viewBoxParts
            ? screenExpandedViewBoxParts || viewBoxParts
            : exportFullQuiltField && viewBoxParts
              ? viewBoxParts
              : expandedViewBoxParts || paddedViewBoxParts || croppedViewBoxParts;
        if (exportViewBoxParts) {
          viewBoxParts = exportViewBoxParts;
          sourceWidth = Math.max(1, exportViewBoxParts[2]);
          sourceHeight = Math.max(1, exportViewBoxParts[3]);
          svgClone.setAttribute('viewBox', exportViewBoxParts.join(' '));
          let par;
          if (options.exportFillFrame === true) {
            // Cover the export canvas — no cream letterbox under IG paper frames.
            par = 'xMidYMin slice';
          } else if (exportMatchQuiltScreen) {
            // Same anchor as quilt-renderer-v2 on screen (xMidYMin meet).
            par = 'xMidYMin meet';
          } else if (exportFullQuiltField) {
            // Carousel / layout B: center primary + mirror in the export frame.
            par = 'xMidYMid meet';
          } else {
            par =
              typeof options.preserveAspectRatio === 'string' && options.preserveAspectRatio.trim()
                ? options.preserveAspectRatio.trim()
                : 'xMidYMin meet';
          }
          svgClone.setAttribute('preserveAspectRatio', par);
        }
        const rect = quiltSVG.getBoundingClientRect?.();
        const maxEdge = Math.min(
          4096,
          Math.max(64, Number(options.maxEdge) || 2400)
        );
        const aspect = targetAspect || (sourceWidth / sourceHeight);
        let outW;
        let outH;
        if (aspect >= 1) {
          outW = maxEdge;
          outH = Math.round(maxEdge / aspect);
        } else {
          outH = maxEdge;
          outW = Math.round(maxEdge * aspect);
        }

        svgClone.removeAttribute('style');
        svgClone.setAttribute('width', String(outW));
        svgClone.setAttribute('height', String(outH));
        if (!options.skipExportUnderpaint) {
          this.addQuiltExportUnderpaint(svgClone, options.underpaintBlocks);
        }
        this.lastQuiltExportDebug = {
          mode: options.exportModeLabel || (targetAspect ? 'target_aspect_crop' : 'default_svg_raster'),
          exportPrimaryQuiltOnly,
          exportFullQuiltField,
          exportMatchQuiltScreen,
          exportViewBoxFromBlocks: !!(exportMatchQuiltScreen && exportBlocks),
          originalViewBox: viewBox || '',
          croppedViewBox: croppedViewBoxParts ? croppedViewBoxParts.join(' ') : '',
          expandedViewBox: expandedViewBoxParts ? expandedViewBoxParts.join(' ') : '',
          screenExpandedViewBox: screenExpandedViewBoxParts
            ? screenExpandedViewBoxParts.join(' ')
            : '',
          paddedViewBox: paddedViewBoxParts ? paddedViewBoxParts.join(' ') : '',
          viewBoxPaddingRatio: Number(options.viewBoxPaddingRatio) || null,
          targetAspect: targetAspect || null,
          sourceWidth,
          sourceHeight,
          outputWidth: outW,
          outputHeight: outH,
          svgClientRect: rect
            ? {
              width: Math.round(rect.width),
              height: Math.round(rect.height),
              top: Math.round(rect.top),
              left: Math.round(rect.left)
            }
            : null,
          preserveAspectRatio: svgClone.getAttribute('preserveAspectRatio') || '',
          usedLiveSvg: true,
          backsidePreview: !!(typeof window !== 'undefined' && window.app?._isBacksidePreviewMode)
        };

        const svgData = new XMLSerializer().serializeToString(svgClone);
        const svgBlob = new Blob([svgData], { type: 'image/svg+xml;charset=utf-8' });
        const svgUrl = URL.createObjectURL(svgBlob);

        const exportMatteFill =
          typeof options.exportMatteFill === 'string' && /^#[0-9a-f]{3,8}$/i.test(options.exportMatteFill.trim())
            ? options.exportMatteFill.trim()
            : exportMatchQuiltScreen
              ? '#f6f4f1'
              : '#ffffff';
        const img = new Image();
        img.onload = () => {
          try {
            const canvas = document.createElement('canvas');
            const ctx = canvas.getContext('2d');
            canvas.width = outW;
            canvas.height = outH;
            ctx.fillStyle = exportMatteFill;
            ctx.fillRect(0, 0, outW, outH);
            if (viewBoxParts && options.underpaintBlocks && !options.skipExportUnderpaint) {
              this.drawQuiltExportCanvasUnderpaint(
                ctx,
                outW,
                outH,
                viewBoxParts,
                options.underpaintBlocks,
                svgClone
              );
            }
            ctx.imageSmoothingEnabled = true;
            ctx.imageSmoothingQuality = 'high';
            const iw = Math.max(1, img.naturalWidth || outW);
            const ih = Math.max(1, img.naturalHeight || outH);
            if (exportMatchQuiltScreen || exportFullQuiltField) {
              ctx.drawImage(img, 0, 0, outW, outH);
            } else {
              const scale = Math.min(outW / iw, outH / ih);
              const dw = Math.round(iw * scale);
              const dh = Math.round(ih * scale);
              const ox = Math.floor((outW - dw) / 2);
              const oy = Math.floor((outH - dh) / 2);
              ctx.drawImage(img, 0, 0, iw, ih, ox, oy, dw, dh);
            }
            if (options.underpaintBlocks) {
              this.healQuiltExportRasterSeams(ctx, outW, outH);
            }
            if (options.skipFilmGrain !== true) {
              Utils.applyFilmGrain(ctx, outW, outH, 0, { vignette: false });
            }
            if (
              options.instagramTrueWhiteMatte === true &&
              viewBoxParts &&
              Array.isArray(options.underpaintBlocks) &&
              options.underpaintBlocks.length
            ) {
              this.bleachInstagramOuterMatteAroundBlockBounds(
                ctx,
                outW,
                outH,
                viewBoxParts,
                options.underpaintBlocks,
                svgClone,
                exportMatteFill
              );
            }
            canvas.toBlob(async (blob) => {
              URL.revokeObjectURL(svgUrl);
              if (!blob) {
                reject(new Error('Could not convert canvas to blob'));
                return;
              }
              if (options.captureDebugRawDataUrl !== false) {
                try {
                  this.lastQuiltExportRawDataUrl = await Utils.blobToDataUrl(blob);
                } catch (_) {
                  this.lastQuiltExportRawDataUrl = null;
                }
              }
              resolve(blob);
            }, 'image/png');
          } catch (drawError) {
            URL.revokeObjectURL(svgUrl);
            reject(drawError);
          }
        };
        img.onerror = () => {
          URL.revokeObjectURL(svgUrl);
          reject(new Error('Failed to render SVG for sharing'));
        };
        img.src = svgUrl;
      } catch (err) {
        reject(err);
      }
    });
  }

  /** Fallback raster when SVG is not available (e.g. headless / Zapier). */
  async generateQuiltRasterBlobFromBlocks(blocks, options = {}) {
    if (!blocks || blocks.length === 0) return null;
    const minX = Math.min(...blocks.map((b) => b.x));
    const minY = Math.min(...blocks.map((b) => b.y));
    const maxX = Math.max(...blocks.map((b) => b.x + b.width));
    const maxY = Math.max(...blocks.map((b) => b.y + b.height));
    const quiltW = maxX - minX;
    const quiltH = maxY - minY;
    let sourceX = minX;
    let sourceY = minY;
    let sourceW = quiltW;
    let sourceH = quiltH;
    const targetAspect =
      Number.isFinite(options.targetAspect) && options.targetAspect > 0
        ? Number(options.targetAspect)
        : null;
    if (targetAspect && quiltW > 0 && quiltH > 0) {
      const sourceAspect = quiltW / quiltH;
      if (targetAspect > sourceAspect) {
        sourceH = Math.min(quiltH, quiltW / targetAspect);
        sourceY = minY + (quiltH - sourceH) / 2;
      } else {
        sourceW = Math.min(quiltW, quiltH * targetAspect);
        sourceX = minX + (quiltW - sourceW) / 2;
      }
    }
    const maxEdge = Math.min(
      4096,
      Math.max(64, Number(options.maxEdge) || 2400)
    );
    const aspect = targetAspect || sourceW / Math.max(1, sourceH);
    let outW;
    let outH;
    if (aspect >= 1) {
      outW = maxEdge;
      outH = Math.round(maxEdge / aspect);
    } else {
      outH = maxEdge;
      outW = Math.round(maxEdge * aspect);
    }
    const canvas = document.createElement('canvas');
    const ctx = canvas.getContext('2d');
    canvas.width = outW;
    canvas.height = outH;
    const bgColor =
      typeof options.backgroundColor === 'string' && /^#[0-9a-f]{3,8}$/i.test(options.backgroundColor.trim())
        ? options.backgroundColor.trim()
        : '#f6f4f1';
    ctx.fillStyle = bgColor;
    ctx.fillRect(0, 0, outW, outH);
    ctx.imageSmoothingEnabled = true;
    ctx.imageSmoothingQuality =
      options.imageSmoothingQuality === 'low' ? 'low' : 'high';
    let scaleX = outW / Math.max(1, sourceW);
    let scaleY = outH / Math.max(1, sourceH);
    let offsetX = 0;
    let offsetY = 0;
    if (options.exportMatchQuiltScreen === true) {
      const scale =
        options.exportFillFrame === true
          ? Math.max(scaleX, scaleY)
          : Math.min(scaleX, scaleY);
      scaleX = scale;
      scaleY = scale;
      offsetX = (outW - sourceW * scale) / 2;
      offsetY = (outH - sourceH * scale) / 2;
    }
    if (!options.skipExportUnderpaint) {
      const bleed = 8;
      blocks.forEach((block) => {
        if (!block) return;
        const bx = Number(block.x);
        const by = Number(block.y);
        const bw = Math.max(0, Number(block.width));
        const bh = Math.max(0, Number(block.height));
        if (![bx, by, bw, bh].every(Number.isFinite) || bw <= 0 || bh <= 0) return;
        const fill = Utils.validateHexColor(block.color) ? block.color : '#d8d4cf';
        const x1 = (bx - bleed - sourceX) * scaleX + offsetX;
        const y1 = (by - bleed - sourceY) * scaleY + offsetY;
        const x2 = (bx + bw + bleed - sourceX) * scaleX + offsetX;
        const y2 = (by + bh + bleed - sourceY) * scaleY + offsetY;
        ctx.fillStyle = fill;
        ctx.fillRect(
          Math.min(x1, x2),
          Math.min(y1, y2),
          Math.abs(x2 - x1),
          Math.abs(y2 - y1)
        );
      });
    }
    const fp = String(options.quiltFingerprint || Utils.computeQuiltFingerprint(blocks) || '');
    let jitterSeed = 1;
    for (let i = 0; i < fp.length; i++) {
      jitterSeed = (Math.imul(jitterSeed, 31) + fp.charCodeAt(i)) | 0;
    }
    const jitterRng = Utils._mulberry32(jitterSeed || 1);
    blocks.forEach((block) => {
      SimpleQuiltEngine.ensureInsetClassificationFromGeometry(block);
      const jitterX = (jitterRng() - 0.5) * 8;
      const jitterY = (jitterRng() - 0.5) * 8;
      const jitterRotation = (jitterRng() - 0.5) * 4;
      const jitteredX = block.x + jitterX;
      const jitteredY = block.y + jitterY;
      const canvasX = (jitteredX - sourceX) * scaleX + offsetX;
      const canvasY = (jitteredY - sourceY) * scaleY + offsetY;
      const canvasWidth = block.width * scaleX;
      const canvasHeight = block.height * scaleY;
      const isHst =
        block.patternType === 'special' &&
        block.specialPatternType === 'hst';
      if (isHst) {
        const tris = Utils.getHstOrganicRenderTriangles(block, 1);
        const bw = Math.max(1e-6, block.width);
        const bh = Math.max(1e-6, block.height);
        const centerX = canvasX + canvasWidth / 2;
        const centerY = canvasY + canvasHeight / 2;
        ctx.save();
        ctx.translate(centerX, centerY);
        ctx.rotate((jitterRotation * Math.PI) / 180);
        const x0 = -canvasWidth / 2;
        const y0 = -canvasHeight / 2;
        const cw = canvasWidth;
        const ch = canvasHeight;
          tris.forEach((t, tux) => {
            const pts = t.points || [];
            if (pts.length < 3) return;
            ctx.fillStyle = t.color || block.color;
            ctx.beginPath();
            ctx.moveTo(x0 + (pts[0][0] / bw) * cw, y0 + (pts[0][1] / bh) * ch);
            for (let k = 1; k < pts.length; k++) {
              ctx.lineTo(x0 + (pts[k][0] / bw) * cw, y0 + (pts[k][1] / bh) * ch);
            }
            ctx.closePath();
            ctx.fill();
            const anch = Utils.hstOrganicTriangleRadialAnchorLocal(pts);
            if (!anch) return;
            const tcx = x0 + (anch.cx / bw) * cw;
            const tcy = y0 + (anch.cy / bh) * ch;
            const spanPx = Math.max((anch.span / bw) * cw, (anch.span / bh) * ch, 1e-6);
            const washHx = /^#[0-9a-f]{6}$/i.test(String(t.color || '').trim())
              ? String(t.color).trim()
              : /^#[0-9a-f]{6}$/i.test(String(block.color || '').trim())
                ? String(block.color).trim()
                : '#c8c4bf';
            ctx.save();
            ctx.beginPath();
            ctx.moveTo(x0 + (pts[0][0] / bw) * cw, y0 + (pts[0][1] / bh) * ch);
            for (let k = 1; k < pts.length; k++) {
              ctx.lineTo(x0 + (pts[k][0] / bw) * cw, y0 + (pts[k][1] / bh) * ch);
            }
            ctx.closePath();
            ctx.clip();
            Utils.canvasWatercolorWashAxisAlignedRect(
              ctx,
              x0,
              y0,
              cw,
              ch,
              washHx,
              `${block.id || 'blk'}|rasterblob|${tux}`,
              { radial: { cx: tcx, cy: tcy, span: spanPx }, skipClip: true }
            );
            ctx.restore();
          });
          ctx.restore();
        return;
      }
      const isInset =
        block.patternType === 'special' &&
        block.specialPatternType === 'insetCircle';
      if (isInset) {
        const bw = Math.max(1e-6, block.width);
        const bh = Math.max(1e-6, block.height);
        const centerX = canvasX + canvasWidth / 2;
        const centerY = canvasY + canvasHeight / 2;
        ctx.save();
        ctx.translate(centerX, centerY);
        ctx.rotate((jitterRotation * Math.PI) / 180);
        const x0 = -canvasWidth / 2;
        const y0 = -canvasHeight / 2;
        Utils.fillInsetCircleBlockCanvasCtx(ctx, block, x0, y0, canvasWidth, canvasHeight);
        ctx.restore();
        return;
      }
      ctx.save();
      const centerX = canvasX + canvasWidth / 2;
      const centerY = canvasY + canvasHeight / 2;
      ctx.translate(centerX, centerY);
      ctx.rotate((jitterRotation * Math.PI) / 180);
      ctx.fillStyle = block.color;
      ctx.fillRect(-canvasWidth / 2, -canvasHeight / 2, canvasWidth, canvasHeight);
      ctx.restore();
    });
    if (!options.skipExportUnderpaint) {
      this.healQuiltExportRasterSeams(ctx, outW, outH);
    }
    if (options.skipFilmGrain !== true) {
      Utils.applyFilmGrain(ctx, outW, outH, 0, { vignette: false });
    }
    if (options.instagramTrueWhiteMatte === true) {
      const exportMatteFill =
        typeof options.exportMatteFill === 'string' && /^#[0-9a-f]{3,8}$/i.test(options.exportMatteFill.trim())
          ? options.exportMatteFill.trim()
          : '#ffffff';
      this.bleachInstagramOuterMatteForBlockRaster(
        ctx,
        outW,
        outH,
        sourceX,
        sourceY,
        scaleX,
        scaleY,
        blocks,
        exportMatteFill
      );
    }
    const mimeType =
      typeof options.mimeType === 'string' && options.mimeType.length
        ? options.mimeType
        : 'image/png';
    const quality =
      typeof options.quality === 'number'
        ? options.quality
        : mimeType === 'image/jpeg' || mimeType === 'image/webp'
          ? 0.88
          : undefined;
    return new Promise((resolve, reject) => {
      canvas.toBlob(
        (blob) => {
          if (!blob) {
            reject(new Error('Could not create quilt raster blob'));
            return;
          }
          resolve(blob);
        },
        mimeType,
        mimeType === 'image/png' ? undefined : quality
      );
    });
  }

  /**
   * Classic 4:5 IG exports use a white card matte; reflection archive uses warm neutral (#f6f4f1).
   */
  async compositeImageUrlWithWarmNeutralMatte(imageUrl, matteHex = '#f6f4f1') {
    const url = String(imageUrl || '').trim();
    if (!url) return null;
    const matteMatch = /^#([0-9a-f]{6})$/i.exec(String(matteHex || '').trim());
    const mr = matteMatch ? parseInt(matteMatch[1].slice(0, 2), 16) : 246;
    const mg = matteMatch ? parseInt(matteMatch[1].slice(2, 4), 16) : 244;
    const mb = matteMatch ? parseInt(matteMatch[1].slice(4, 6), 16) : 241;
    const loadCandidates = [];
    const baseUrl =
      typeof globalThis.odqBackendBaseUrl === 'function'
        ? globalThis.odqBackendBaseUrl()
        : String(CONFIG.BACKEND?.baseUrl || '').replace(/\/$/, '');
    if (baseUrl && /^https?:\/\//i.test(url)) {
      loadCandidates.push(`${baseUrl}/api/proxy-image?url=${encodeURIComponent(url)}`);
    }
    loadCandidates.push(url);
    let img = null;
    let lastErr = null;
    for (const candidate of loadCandidates) {
      try {
        img = await new Promise((resolve, reject) => {
          const im = new Image();
          im.crossOrigin = 'anonymous';
          im.referrerPolicy = 'no-referrer';
          im.onload = () => resolve(im);
          im.onerror = () => reject(new Error('Could not load quilt image for matte pass'));
          im.src = candidate;
        });
        break;
      } catch (err) {
        lastErr = err;
      }
    }
    if (!img) throw lastErr || new Error('Could not load quilt image for matte pass');
    const w = Math.max(1, img.naturalWidth || img.width);
    const h = Math.max(1, img.naturalHeight || img.height);
    const canvas = document.createElement('canvas');
    canvas.width = w;
    canvas.height = h;
    const ctx = canvas.getContext('2d', { willReadFrequently: true });
    ctx.fillStyle = matteHex;
    ctx.fillRect(0, 0, w, h);
    ctx.drawImage(img, 0, 0, w, h);
    const threshold = 240;
    const maxSat = 0.14;
    const imageData = ctx.getImageData(0, 0, w, h);
    const px = imageData.data;
    for (let i = 0; i < px.length; i += 4) {
      const a = px[i + 3];
      if (a < 12) {
        px[i] = mr;
        px[i + 1] = mg;
        px[i + 2] = mb;
        px[i + 3] = 255;
        continue;
      }
      const r = px[i];
      const g = px[i + 1];
      const b = px[i + 2];
      const maxC = Math.max(r, g, b);
      const minC = Math.min(r, g, b);
      const sat = maxC === 0 ? 0 : (maxC - minC) / maxC;
      if (r >= threshold && g >= threshold && b >= threshold && sat <= maxSat) {
        px[i] = mr;
        px[i + 1] = mg;
        px[i + 2] = mb;
      }
    }
    ctx.putImageData(imageData, 0, 0);
    return new Promise((resolve, reject) => {
      canvas.toBlob(
        (blob) => (blob ? resolve(blob) : reject(new Error('Could not build warm-neutral quilt image'))),
        'image/png',
        0.92
      );
    });
  }

  _firebaseStoragePathFromDownloadUrl(url) {
    try {
      const u = new URL(String(url || '').trim());
      const host = String(u.hostname || '').toLowerCase();
      if (
        !/(^|\.)firebasestorage\.googleapis\.com$/i.test(host) &&
        !/\.firebasestorage\.app$/i.test(host)
      ) {
        return '';
      }
      const match = u.pathname.match(/\/o\/(.+)$/);
      if (!match) return '';
      return decodeURIComponent(match[1]);
    } catch (_) {
      return '';
    }
  }

  _isFirebaseStorageDownloadUrl(url) {
    return !!this._firebaseStoragePathFromDownloadUrl(url);
  }

  /** Canvas-safe data URL via /api/proxy-image (never Firebase client getBlob — CORS + minute-long retries on web). */
  async _fetchFirebaseStorageImageAsDataUrl(downloadUrl, options = {}) {
    return this._prepareSpeakerImageUrlForCanvas(downloadUrl, options);
  }

  _normalizeProxyImageSource(url) {
    return typeof globalThis.odqNormalizeProxyImageSourceUrl === 'function'
      ? globalThis.odqNormalizeProxyImageSourceUrl(url)
      : String(url || '').trim();
  }

  /** Allow a later story-preview pass to retry after an early proxy miss (race with speaker card). */
  _forgetProxyImageFailure(url) {
    const source = this._normalizeProxyImageSource(url);
    if (!source || !this._proxyImageFailedUrls) return;
    this._proxyImageFailedUrls.delete(source);
    const storagePath = this._firebaseStoragePathFromDownloadUrl(source);
    if (storagePath) {
      this._proxyImageFailedUrls.delete(`firebase-storage:${storagePath}`);
    }
  }

  /**
   * Proxy remote speaker cutouts to data URLs so canvas compose can read pixels (CORS-safe).
   */
  async _prepareSpeakerImageUrlForCanvas(speakerImageUrl, options = {}) {
    if (!this._proxyImageDataUrlCache) this._proxyImageDataUrlCache = new Map();
    if (!this._proxyImageFailedUrls) this._proxyImageFailedUrls = new Set();
    if (!this._proxyImageInFlight) this._proxyImageInFlight = new Map();
    /** Layout B quilt multiply needs raw cutout pixels — not cream hole-fill export PNG. */
    const skipCutoutExportFinalize = options.skipCutoutExportFinalize === true;
    const cacheKeyFor = (src) => (skipCutoutExportFinalize ? `${src}#rawCutout` : src);
    const finalizeDataUrl = async (dataUrl, source) => {
      if (skipCutoutExportFinalize) return dataUrl;
      return this._finalizeSpeakerCutoutExportDataUrl(dataUrl, source);
    };
    const proxyToDataUrl = async (url) => {
      const source = this._normalizeProxyImageSource(url);
      if (!source) return '';
      if (/^(?:data|blob):/i.test(source)) return source;
      if (!/^https?:\/\//i.test(source)) return source;
      if (options.retry === true) this._forgetProxyImageFailure(source);
      if (this._proxyImageFailedUrls.has(source)) return '';
      const cacheKey = cacheKeyFor(source);
      if (this._proxyImageDataUrlCache.has(cacheKey)) return this._proxyImageDataUrlCache.get(cacheKey);
      const storagePath = this._firebaseStoragePathFromDownloadUrl(source);
      if (storagePath) {
        const storageCacheKey = cacheKeyFor(`firebase-storage:${storagePath}`);
        if (this._proxyImageDataUrlCache.has(storageCacheKey)) {
          return this._proxyImageDataUrlCache.get(storageCacheKey);
        }
      }
      const bases =
        typeof globalThis.odqProxyImageBases === 'function'
          ? globalThis.odqProxyImageBases()
          : [String(CONFIG.BACKEND?.baseUrl || '').replace(/\/$/, '')].filter(Boolean);
      if (this._proxyImageInFlight.has(cacheKey)) {
        return this._proxyImageInFlight.get(cacheKey);
      }
      const fetchPromise = (async () => {
        let lastStatus = 0;
        for (const baseUrl of bases) {
          if (!baseUrl) continue;
          try {
            const proxyUrl =
              typeof globalThis.odqProxyImageFetchUrl === 'function'
                ? globalThis.odqProxyImageFetchUrl(baseUrl, source)
                : `${baseUrl}/api/proxy-image?url=${encodeURIComponent(source)}`;
            const proxyController = typeof AbortController === 'function' ? new AbortController() : null;
            const proxyAbortTimer = proxyController ? setTimeout(() => proxyController.abort(), 12000) : 0;
            let res;
            try {
              res = await fetch(proxyUrl, {
                cache: 'no-store',
                signal: proxyController?.signal
              });
            } finally {
              if (proxyAbortTimer) clearTimeout(proxyAbortTimer);
            }
            if (!res.ok) {
              lastStatus = res.status;
              continue;
            }
            const blob = await res.blob();
            if (!String(blob.type || '').startsWith('image/')) {
              continue;
            }
            const dataUrl = await Utils.blobToDataUrl(blob);
            const exportUrl = await finalizeDataUrl(dataUrl, source);
            this._proxyImageDataUrlCache.set(cacheKey, exportUrl);
            if (storagePath) {
              this._proxyImageDataUrlCache.set(cacheKeyFor(`firebase-storage:${storagePath}`), exportUrl);
            }
            return exportUrl;
          } catch (fetchErr) {
            lastStatus = lastStatus || 0;
          }
        }
        this._proxyImageFailedUrls.add(source);
        if (lastStatus) {
          this.logger?.warn?.(`Layout B speaker proxy failed (${lastStatus}):`, source.slice(0, 120));
        }
        return '';
      })();
      this._proxyImageInFlight.set(cacheKey, fetchPromise);
      try {
        return await fetchPromise;
      } finally {
        this._proxyImageInFlight.delete(cacheKey);
      }
    };
    let primary = String(speakerImageUrl || '').trim();
    let fallback = String(options.fallbackUrl || '').trim();
    const cutoutSourceRef = this._isFirebaseSpeakerCutoutUrl(primary)
      ? primary
      : String(options.cutoutSourceUrl || '').trim() || primary;
    const primaryIsStorageCutout = this._isFirebaseSpeakerCutoutUrl(primary);
    /**
     * Do not swap a resolved Storage cutout for portrait here — resolveLayoutBSpeakerCutoutUrl
     * already validated it. Re-checking portrait hash on phone often differs (assignment vs
     * catalog URL) and wrongly forces the full portrait → B/W rectangle on iOS.
     */
    if (
      !primaryIsStorageCutout &&
      primary &&
      fallback &&
      /speaker-cutouts(?:%2F|\/)/i.test(primary) &&
      typeof this._speakerCutoutSnapMatchesPortraitUrl === 'function'
    ) {
      const q = options.quote && typeof options.quote === 'object' ? options.quote : null;
      const source = String(
        q?.speakerCutoutSourceUrlSnapshot ??
          q?.speaker_cutout_source_url_snapshot ??
          q?.speakerCutoutSourceUrl ??
          q?.speaker_cutout_source_url ??
          ''
      ).trim();
      const trustedBySource = !!(source && source === fallback);
      if (
        !trustedBySource &&
        !(await this._speakerCutoutSnapMatchesPortraitUrl(primary, fallback))
      ) {
        primary = fallback;
        fallback = '';
      }
    }
    try {
      const dataUrl = await proxyToDataUrl(primary);
      if (dataUrl) return dataUrl;
    } catch (err) {
      this.logger.warn('Layout B speaker proxy failed:', err);
    }
    if (fallback && fallback !== primary) {
      try {
        const dataUrl = await proxyToDataUrl(fallback);
        if (dataUrl) return dataUrl;
      } catch (err) {
        this.logger.warn('Layout B speaker portrait fallback proxy failed:', err);
      }
    }
    if (/^(?:data|blob):/i.test(primary)) {
      return skipCutoutExportFinalize
        ? primary
        : this._finalizeSpeakerCutoutExportDataUrl(primary, cutoutSourceRef);
    }
    return '';
  }

  _isFirebaseSpeakerCutoutUrl(url) {
    return /speaker-cutouts(?:%2F|\/)/i.test(String(url || '').trim());
  }

  /** One export-time hole-fill so card, story preview, and IG post share the same cutout bytes. */
  async _finalizeSpeakerCutoutExportDataUrl(dataUrl, sourceUrl) {
    const raw = String(dataUrl || '').trim();
    const source = String(sourceUrl || '').trim();
    if (!raw || !this._isFirebaseSpeakerCutoutUrl(source)) return raw;
    const SCR = globalThis.SpeakerCutoutRender;
    if (!SCR?.prepareSpeakerCutoutExportDataUrl) return raw;
    try {
      const exportUrl = await SCR.prepareSpeakerCutoutExportDataUrl(raw);
      return exportUrl && exportUrl.length > 2000 ? exportUrl : raw;
    } catch (_) {
      return raw;
    }
  }

  /**
   * Speaker URL for Layout B canvas compose (data: or blob: after proxy / Firebase SDK).
   * Prefers cutout; portrait fallback only when no Storage cutout was resolved.
   */
  async _speakerOverlayImageUrlForLayoutBCompose(speakerCutoutUrl, speakerPortraitUrl, quote = null) {
    const cutout = String(speakerCutoutUrl || '').trim();
    const portrait = String(speakerPortraitUrl || '').trim();
    const primary = cutout || portrait;
    if (!primary) return '';
    const hasResolvedStorageCutout = !!(cutout && this._isFirebaseSpeakerCutoutUrl(cutout));
    const prepared = await this._prepareSpeakerImageUrlForCanvas(primary, {
      fallbackUrl: hasResolvedStorageCutout ? '' : portrait && primary !== portrait ? portrait : '',
      cutoutSourceUrl: cutout || '',
      quote,
      skipCutoutExportFinalize: true
    });
    if (prepared) return prepared;
    if (!hasResolvedStorageCutout && portrait && portrait !== primary) {
      const portraitPrepared = await this._prepareSpeakerImageUrlForCanvas(portrait, {
        quote,
        skipCutoutExportFinalize: true
      });
      if (portraitPrepared) return portraitPrepared;
    }
    return !hasResolvedStorageCutout && /^https?:\/\//i.test(primary) ? primary : '';
  }

  async _buildLayoutBPostComposeOptions(blocks, quoteObj, dateKey) {
    const dk = dateKey || Utils.getTodayKey();
    const q = await this._quoteForLayoutBSpeakerImage(quoteObj, dk);
    const quoteText = String(q.text ?? q.body ?? '').trim();
    const quoteAuthor = String(q.author ?? '').trim();
    let postQuoteStyleIndependent = false;
    try {
      if (typeof globalThis.odqReadInstagramImagesDocWithFallback === 'function') {
        const ig = await globalThis.odqReadInstagramImagesDocWithFallback(dk, 8000);
        if (ig?.data && typeof globalThis.odqLayoutBPostQuoteStyleIndependent === 'function') {
          postQuoteStyleIndependent = globalThis.odqLayoutBPostQuoteStyleIndependent(ig.data);
        }
      }
    } catch (_) {
      /* optional */
    }
    const quoteStyleAspect = postQuoteStyleIndependent ? 'post' : 'story';
    let stripLayoutSeed = 0;
    try {
      stripLayoutSeed = await odqReadLayoutBStripLayoutSeed(dk, quoteStyleAspect);
    } catch (_) {
      stripLayoutSeed = odqGetCachedLayoutBStripLayoutSeed(dk, quoteStyleAspect) ?? 0;
    }
    let quiltBgZoom = ODQ_LAYOUT_B_QUILT_BG_ZOOM_MIN;
    try {
      quiltBgZoom = await odqReadLayoutBQuiltBgZoom(dk, 'post');
    } catch (_) {
      quiltBgZoom = odqGetCachedLayoutBQuiltBgZoom(dk, 'post') ?? ODQ_LAYOUT_B_QUILT_BG_ZOOM_MIN;
    }
    let quiltBgOffsetY = 0;
    try {
      if (typeof globalThis.odqReadLayoutBQuiltBgOffsetY === 'function') {
        quiltBgOffsetY = await globalThis.odqReadLayoutBQuiltBgOffsetY(dk, 'post');
      }
    } catch (_) {
      quiltBgOffsetY = globalThis.odqNormalizeQuiltBgOffsetY?.(
        globalThis.odqGetCachedLayoutBQuiltBgOffsetY?.(dk, 'post')
      ) ?? 0;
    }
    let quoteStripOffset = { x: 0, y: 0 };
    try {
      if (typeof globalThis.odqReadLayoutBQuoteStripOffset === 'function') {
        quoteStripOffset = await globalThis.odqReadLayoutBQuoteStripOffset(dk, 'post');
      }
    } catch (_) {
      quoteStripOffset = globalThis.odqGetCachedLayoutBQuoteStripOffset?.(dk, 'post') || { x: 0, y: 0 };
    }
    let keywordEmphasis = null;
    try {
      keywordEmphasis = await odqReadLayoutBKeywordEmphasis(dk, quoteStyleAspect);
    } catch (_) {
      keywordEmphasis = odqGetCachedLayoutBKeywordEmphasis(dk, quoteStyleAspect) ?? null;
    }
    const composeOptions = {
      quiltFit: 'cover',
      tuneAspect: 'post',
      stripLayoutSeed,
      stripLayoutSeedExplicit: true,
      quiltBgZoom,
      quiltBgZoomExplicit: true,
      quiltBgOffsetY,
      quiltBgOffsetYExplicit: true,
      quoteStripOffset,
      quoteStripOffsetExplicit: true,
      postQuoteStyleIndependent,
      postStripLayoutFromStory: false
    };
    if (keywordEmphasis) {
      composeOptions.keywordEmphasis = keywordEmphasis;
      composeOptions.keywordEmphasisExplicit = true;
    }
    const speakerPortraitUrl = this._layoutBSpeakerPortraitUrlResolved(q);
    const speakerCutoutUrl = await this.resolveLayoutBSpeakerCutoutUrl(q, quoteAuthor);
    let hasSpeaker = false;
    if (speakerCutoutUrl || speakerPortraitUrl) {
      const speakerImageForCanvas = await this._speakerOverlayImageUrlForLayoutBCompose(
        speakerCutoutUrl,
        speakerPortraitUrl,
        q
      );
      if (speakerImageForCanvas) {
        const app = typeof window !== 'undefined' ? window.app : null;
        const profileName = app?.getQuoteSpeakerProfile?.(q)?.name || '';
        const speakerName = String(q.speakerName ?? q.speaker_name ?? profileName ?? quoteAuthor)
          .replace(/^\s*[—-]\s*/, '')
          .trim();
        const washColor = String(
          (typeof window !== 'undefined' && window.app?.getSpeakerCutoutWashColor?.()) ||
            (typeof window !== 'undefined' && window.app?.getMostPopularQuiltColor?.(blocks)?.color) ||
            (typeof globalThis.CONFIG !== 'undefined' &&
              globalThis.CONFIG.APP &&
              globalThis.CONFIG.APP.defaultColor) ||
            '#ea9b9a'
        ).trim();
        let presetTransform = null;
        if (typeof globalThis.odqSpeakerCutoutTransformForDateAsync === 'function') {
          try {
            presetTransform = await globalThis.odqSpeakerCutoutTransformForDateAsync(dk, 'post');
          } catch (_) {
            /* optional */
          }
        }
        if (
          !presetTransform &&
          typeof globalThis.odqReadInstagramImagesDocWithFallback === 'function' &&
          typeof globalThis.odqSpeakerCutoutTransformFromInstagramData === 'function'
        ) {
          try {
            const ig = await globalThis.odqReadInstagramImagesDocWithFallback(dk, 8000);
            if (ig?.data) {
              presetTransform = globalThis.odqSpeakerCutoutTransformFromInstagramData(ig.data, 'post');
            }
          } catch (_) {
            /* optional */
          }
        }
        composeOptions.speakerOverlay = {
          enabled: true,
          imageUrl: speakerImageForCanvas,
          cutoutSourceUrl: speakerCutoutUrl || speakerPortraitUrl || '',
          name: speakerName,
          washColor,
          transform: presetTransform || undefined
        };
        composeOptions.speakerCutoutQuote = q;
        hasSpeaker = true;
      }
    }
    return { quoteText, quoteAuthor, composeOptions, hasSpeaker };
  }

  async _buildLayoutBSpeakerHeroPostComposeOptions(blocks, quoteObj, dateKey) {
    const dk = dateKey || Utils.getTodayKey();
    const q = await this._quoteForLayoutBSpeakerImage(quoteObj, dk);
    const quoteAuthor = String(q.author ?? '').trim();
    let quiltBgZoom = ODQ_LAYOUT_B_QUILT_BG_ZOOM_MIN;
    try {
      quiltBgZoom = await odqReadLayoutBQuiltBgZoom(dk, 'post');
    } catch (_) {
      quiltBgZoom = odqGetCachedLayoutBQuiltBgZoom(dk, 'post') ?? ODQ_LAYOUT_B_QUILT_BG_ZOOM_MIN;
    }
    const composeOptions = {
      quiltFit: 'cover',
      tuneAspect: 'post',
      speakerHeroPost: true,
      quiltBgZoom,
      quiltBgZoomExplicit: true
    };
    const speakerPortraitUrl = this._layoutBSpeakerPortraitUrlResolved(q);
    const speakerCutoutUrl = await this.resolveLayoutBSpeakerCutoutUrl(q, quoteAuthor);
    if (!speakerCutoutUrl && !speakerPortraitUrl) {
      return { quoteText: '', quoteAuthor, composeOptions, hasSpeaker: false };
    }
    const speakerImageForCanvas = await this._speakerOverlayImageUrlForLayoutBCompose(
      speakerCutoutUrl,
      speakerPortraitUrl,
      q
    );
    if (!speakerImageForCanvas) {
      return { quoteText: '', quoteAuthor, composeOptions, hasSpeaker: false };
    }
    const speakerName = String(q.speakerName ?? q.speaker_name ?? quoteAuthor)
      .replace(/^\s*[—-]\s*/, '')
      .trim();
          const washColor = String(
            (typeof window !== 'undefined' && window.app?.getSpeakerCutoutWashColor?.()) ||
              (typeof window !== 'undefined' && window.app?.getMostPopularQuiltColor?.(blocks)?.color) ||
        (typeof globalThis.CONFIG !== 'undefined' &&
          globalThis.CONFIG.APP &&
          globalThis.CONFIG.APP.defaultColor) ||
        '#ea9b9a'
    ).trim();
    composeOptions.speakerOverlay = {
      enabled: true,
      imageUrl: speakerImageForCanvas,
      cutoutSourceUrl: speakerCutoutUrl || speakerPortraitUrl || '',
      name: speakerName,
      washColor
    };
    return { quoteText: '', quoteAuthor, composeOptions, hasSpeaker: true };
  }

  _layoutBSpeakerPortraitUrlResolved(quote) {
    const q = quote && typeof quote === 'object' ? quote : {};
    let portrait = this._layoutBSpeakerPortraitUrl(q);
    const app = typeof window !== 'undefined' ? window.app : null;
    if (!portrait && app?.getQuoteSpeakerProfile) {
      portrait = String(app.getQuoteSpeakerProfile(q)?.portraitUrl || '').trim();
    }
    if (!portrait && app?.quoteService?._speakerPortraitFromQuoteAndAssignment) {
      portrait = String(app.quoteService._speakerPortraitFromQuoteAndAssignment(q) || '').trim();
    }
    if (portrait && /speaker-cutouts(?:%2F|\/)/i.test(portrait)) portrait = '';
    return portrait;
  }

  async _buildLayoutBStoryComposeOptions(blocks, quoteObj, dateKey, buildOptions = {}) {
    const previewFast = buildOptions.previewFast === true;
    const dk = dateKey || Utils.getTodayKey();
    const q = await this._quoteForLayoutBSpeakerImage(quoteObj, dk);
    const quoteText = String(q.text ?? q.body ?? '').trim();
    const quoteAuthor = String(q.author ?? '').trim();
    let stripLayoutSeed = 0;
    if (previewFast) {
      stripLayoutSeed = odqGetCachedLayoutBStripLayoutSeed(dk, 'story') ?? 0;
    } else {
      try {
        stripLayoutSeed = await odqReadLayoutBStripLayoutSeed(dk, 'story');
      } catch (_) {
        stripLayoutSeed = odqGetCachedLayoutBStripLayoutSeed(dk, 'story') ?? 0;
      }
    }
    let quiltBgZoom = ODQ_LAYOUT_B_QUILT_BG_ZOOM_MIN;
    if (previewFast) {
      quiltBgZoom = odqGetCachedLayoutBQuiltBgZoom(dk, 'story') ?? ODQ_LAYOUT_B_QUILT_BG_ZOOM_MIN;
    } else {
      try {
        quiltBgZoom = await odqReadLayoutBQuiltBgZoom(dk, 'story');
      } catch (_) {
        quiltBgZoom = odqGetCachedLayoutBQuiltBgZoom(dk, 'story') ?? ODQ_LAYOUT_B_QUILT_BG_ZOOM_MIN;
      }
    }
    let quiltBgOffsetY = 0;
    if (previewFast) {
      quiltBgOffsetY = odqGetCachedLayoutBQuiltBgOffsetY(dk, 'story') ?? 0;
    } else {
      try {
        quiltBgOffsetY = await odqReadLayoutBQuiltBgOffsetY(dk, 'story');
      } catch (_) {
        quiltBgOffsetY = odqGetCachedLayoutBQuiltBgOffsetY(dk, 'story') ?? 0;
      }
    }
    let quoteStripOffset = { x: 0, y: 0 };
    if (previewFast) {
      quoteStripOffset = odqGetCachedLayoutBQuoteStripOffset(dk, 'story') ?? { x: 0, y: 0 };
    } else {
      try {
        quoteStripOffset = await odqReadLayoutBQuoteStripOffset(dk, 'story');
      } catch (_) {
        quoteStripOffset = odqGetCachedLayoutBQuoteStripOffset(dk, 'story') ?? { x: 0, y: 0 };
      }
    }
    let keywordEmphasis = null;
    if (previewFast) {
      keywordEmphasis = odqGetCachedLayoutBKeywordEmphasis(dk, 'story') ?? null;
    } else {
      try {
        keywordEmphasis = await odqReadLayoutBKeywordEmphasis(dk, 'story');
      } catch (_) {
        keywordEmphasis = odqGetCachedLayoutBKeywordEmphasis(dk, 'story') ?? null;
      }
    }
    const composeOptions = {
      tuneAspect: 'story',
      stripLayoutSeed,
      stripLayoutSeedExplicit: true,
      quiltBgZoom,
      quiltBgZoomExplicit: true,
      quiltBgOffsetY,
      quiltBgOffsetYExplicit: true,
      quoteStripOffset,
      quoteStripOffsetExplicit: true
    };
    if (keywordEmphasis) {
      composeOptions.keywordEmphasis = keywordEmphasis;
      composeOptions.keywordEmphasisExplicit = true;
    }
    // Pass saved post strip plan so story uses same arrangement as post
    // Skip Firestore reads for fast preview — getDoc hangs on WKWebView.
    if (!previewFast && typeof globalThis.odqReadLayoutBPostStripPlan === 'function') {
      try {
        const savedPlan = await globalThis.odqReadLayoutBPostStripPlan(dk);
        if (savedPlan && savedPlan.length) composeOptions.savedPostStripPlan = savedPlan;
      } catch (_) { /* optional */ }
    }
    const speakerPortraitUrl = this._layoutBSpeakerPortraitUrlResolved(q);
    const preloadedSpeaker = String(buildOptions.preloadedSpeakerImageUrl || '').trim();
    // For fast preview, skip the Firebase Storage CDN lookup (hangs on WKWebView) and use
    // only locally-cached data. Full export still resolves from storage.
    const speakerCutoutUrl =
      preloadedSpeaker || previewFast
        ? String(this._resolveLayoutBSpeakerImageUrl(q) || '').trim()
        : await this.resolveLayoutBSpeakerCutoutUrl(q, quoteAuthor);
    let speakerImageForCanvas = preloadedSpeaker;
    // For fast preview skip the proxy-fetch (un-timeboxed network call that hangs on WKWebView).
    // Pass the raw URL so compose can render it with its own load-timeout if available.
    if (!speakerImageForCanvas && !previewFast && (speakerCutoutUrl || speakerPortraitUrl)) {
      speakerImageForCanvas = await this._speakerOverlayImageUrlForLayoutBCompose(
        speakerCutoutUrl,
        speakerPortraitUrl,
        q
      );
    } else if (!speakerImageForCanvas && previewFast) {
      speakerImageForCanvas = speakerCutoutUrl || speakerPortraitUrl || '';
    }
    if (speakerImageForCanvas) {
      const isLocalSpeakerSrc = /^(?:data|blob):/i.test(String(speakerImageForCanvas || ''));
      if (!(previewFast && !isLocalSpeakerSrc)) {
        const app = typeof window !== 'undefined' ? window.app : null;
        const profileName = app?.getQuoteSpeakerProfile?.(q)?.name || '';
        const speakerName = String(q.speakerName ?? q.speaker_name ?? profileName ?? quoteAuthor)
          .replace(/^\s*[—-]\s*/, '')
          .trim();
        const washColor = String(
          (typeof window !== 'undefined' && window.app?.getSpeakerCutoutWashColor?.()) ||
            (typeof window !== 'undefined' && window.app?.getMostPopularQuiltColor?.(blocks)?.color) ||
            (typeof globalThis.CONFIG !== 'undefined' &&
              globalThis.CONFIG.APP &&
              globalThis.CONFIG.APP.defaultColor) ||
            '#ea9b9a'
        ).trim();
        let presetTransform = null;
        if (previewFast) {
          // Never make server calls for preview — getDocFromServer hangs on WKWebView.
          if (typeof globalThis.odqReadSpeakerCutoutPreset === 'function') {
            const preset = globalThis.odqReadSpeakerCutoutPreset(dk, 'story');
            const nudge =
              typeof globalThis.odqReadSpeakerCutoutTweakFromLocal === 'function'
                ? globalThis.odqReadSpeakerCutoutTweakFromLocal(dk, 'story')
                : null;
            if (preset && typeof globalThis.odqSpeakerCutoutTransformResolved === 'function') {
              presetTransform = globalThis.odqSpeakerCutoutTransformResolved(preset, nudge);
            }
          }
        } else {
          presetTransform = await odqSpeakerCutoutTransformForDateAsync(dk, 'story');
        }
        composeOptions.speakerOverlay = {
          enabled: true,
          imageUrl: speakerImageForCanvas,
          cutoutSourceUrl: speakerCutoutUrl || speakerPortraitUrl || '',
          name: speakerName,
          washColor,
          transform: presetTransform || undefined
        };
        composeOptions.speakerCutoutQuote = q;
      }
    }
    return { quoteText, quoteAuthor, composeOptions };
  }

  /**
   * Same pipeline as nightly `generateInstagramStoryLayoutBImage` (Zapier layout-b-story.png).
   * @returns {Promise<Blob|null>}
   */
  async generateInstagramStoryLayoutBBlob(blocks, quoteObj = null, dateKey = null, options = {}) {
    if (!blocks || blocks.length === 0) return null;
    await globalThis.ensureLayoutBComposeReady?.();
    const dk = dateKey || Utils.getTodayKey();
    const forPreview = options.forPreview === true;
    const fastPreview = forPreview;
    const quiltSourceOpts = {
      dateKey: dk,
      exportModeLabel: forPreview ? 'layout_b_story_9x16_preview' : 'layout_b_story_9x16_source',
      skipFilmGrain: !forPreview,
      /** Story collage uses cover+bleed; meet-fit quilt-screen letterboxing reads as milky overcast. */
      exportMatchQuiltScreen: false,
      exportFillFrame: true
    };
    if (forPreview) {
      const defaultEdge = fastPreview ? 720 : 1200;
      quiltSourceOpts.maxEdge = Math.min(2400, Math.max(512, Number(options.maxEdge) || defaultEdge));
      if (fastPreview) quiltSourceOpts.imageSmoothingQuality = 'low';
    }
    const buildOpts = forPreview
      ? {
          previewFast: true,
          preloadedSpeakerImageUrl: String(options.preloadedSpeakerImageUrl || '').trim()
        }
      : {};
    const optionsPromise = this._buildLayoutBStoryComposeOptions(blocks, quoteObj, dk, buildOpts);
    const sourceBlobPromise = this.getInstagramQuiltSourceBlob(blocks, quiltSourceOpts);
    const [{ quoteText, quoteAuthor, composeOptions }, highResBlob] = await Promise.all([
      optionsPromise,
      sourceBlobPromise
    ]);
    if (!highResBlob) return null;
    // WKWebView cannot load blob: URLs in new Image() — convert to data URL on native.
    const isNativeCtx =
      typeof window !== 'undefined' &&
      typeof window.odqIsCapacitorNative === 'function' &&
      window.odqIsCapacitorNative();
    const quiltSource = isNativeCtx ? await this._blobToDataUrl(highResBlob) : highResBlob;
    const compose = globalThis.composeInstagramLayoutBFromQuiltBlob;
    if (typeof compose !== 'function') {
      throw new Error('composeInstagramLayoutBFromQuiltBlob missing (app module must finish loading)');
    }
    if (typeof globalThis.ensureOdqCanvasFontsReady === 'function') {
      await globalThis.ensureOdqCanvasFontsReady();
    }
    const SCR = globalThis.SpeakerCutoutRender;
    if (!forPreview && SCR?.ensureSpeakerNewsprintAssets) {
      await SCR.ensureSpeakerNewsprintAssets();
    }
    if (!forPreview) {
      composeOptions.captureStoryRefStripPlan = true;
      composeOptions.onStoryRefStripPlan = (plan) => {
        this._stashLayoutBStoryRefStripPlan(plan);
      };
    }
    if (options.transparentOverlay === true) {
      composeOptions.transparentOverlay = true;
    }
    if (typeof options.exportMime === 'string' && options.exportMime.trim()) {
      composeOptions.exportMime = options.exportMime.trim();
      if (typeof options.exportQuality === 'number') {
        composeOptions.exportQuality = options.exportQuality;
      }
    }
    if (forPreview) {
      composeOptions.tunePreviewFast = true;
      composeOptions.skipSpeakerImageNetwork = true;
      composeOptions.storyPanelPreview = true;
    }
    const storyBlob = await compose(quiltSource, quoteText, quoteAuthor, 1080, 1920, dk, composeOptions);
    return storyBlob;
  }

  async _generateInstagramStoryLayoutBImageData(blocks, quoteObj, dateKey) {
    const storyBlob = await this.generateInstagramStoryLayoutBBlob(blocks, quoteObj, dateKey);
    if (!storyBlob) return null;
    return await new Promise((resolve) => {
      const fr = new FileReader();
      fr.onload = () => resolve(fr.result || null);
      fr.onerror = () => resolve(null);
      fr.readAsDataURL(storyBlob);
    });
  }

  /**
   * Instagram story (9:16, 1080×1920): Layout B collage (speaker cutout when available).
   */
  async generateInstagramStoryLayoutBImage(blocks, quoteObj = null, dateKey = null) {
    try {
      return await this._generateInstagramStoryLayoutBImageData(blocks, quoteObj, dateKey);
    } catch (error) {
      this.logger.error('Instagram story layout B generation failed:', error);
      return null;
    }
  }

  async generateInstagramStoryLayoutBOverlayImage(blocks, quoteObj = null, dateKey = null) {
    try {
      const storyBlob = await this.generateInstagramStoryLayoutBBlob(blocks, quoteObj, dateKey, {
        transparentOverlay: true
      });
      return storyBlob ? await this._blobToDataUrl(storyBlob) : null;
    } catch (error) {
      this.logger.error('Instagram story layout B overlay generation failed:', error);
      return null;
    }
  }

  /**
   * Full-quality 1080×1920 story JPEG for in-app preview (~200–400 KB). PNG nightly/Zapier unchanged.
   * @returns {Promise<string|null>} data URL
   */
  async generateInstagramStoryLayoutBPreviewJpegImageData(blocks, quoteObj = null, dateKey = null) {
    try {
      const storyBlob = await this.generateInstagramStoryLayoutBBlob(blocks, quoteObj, dateKey, {
        exportMime: 'image/jpeg',
        exportQuality: 0.88
      });
      return storyBlob ? await this._blobToDataUrl(storyBlob) : null;
    } catch (error) {
      this.logger.error('Instagram story layout B preview JPEG generation failed:', error);
      return null;
    }
  }

  async generateInstagramPostLayoutBBlob(
    blocks,
    quoteObj = null,
    dateKey = null,
    {
      requireSpeaker = false,
      speakerHeroPost = false,
      carouselSpeakerAvoidRect = null,
      carouselShortQuote = false,
      carouselStoryStyle = false,
      carouselFullWidthQuiltSource = false,
      postStripLayoutFromStory = false,
      omitQuoteStrips = false
    } = {}
  ) {
    if (!blocks || blocks.length === 0) return null;
    await globalThis.ensureLayoutBComposeReady?.();
    const dk = dateKey || Utils.getTodayKey();
    await this._prefetchQuiltExportTune(dk);
    const { quoteText, quoteAuthor, composeOptions, hasSpeaker } = speakerHeroPost
      ? await this._buildLayoutBSpeakerHeroPostComposeOptions(blocks, quoteObj, dk)
      : await this._buildLayoutBPostComposeOptions(blocks, quoteObj, dk);
    if (requireSpeaker && !hasSpeaker) return null;

    const highResBlob = carouselFullWidthQuiltSource
      ? await this._getInstagramQuiltScreenSourceBlob(
          blocks,
          dk,
          'layout_b_carousel_post_quilt_screen_cover_source'
        )
      : await this.getInstagramQuiltSourceBlob(blocks, {
          dateKey: dk,
          exportModeLabel: speakerHeroPost
            ? 'layout_b_speaker_hero_post_9x16_cover_source'
            : 'layout_b_post_9x16_cover_source',
          skipFilmGrain: true
        });
    if (!highResBlob) return null;

    const compose = globalThis.composeInstagramLayoutBFromQuiltBlob;
    if (typeof compose !== 'function') {
      throw new Error('composeInstagramLayoutBFromQuiltBlob missing (app module must finish loading)');
    }
    /** Carousel slide 1 is the Post tune surface — match tune modal, not story strip inheritance. */
    if (carouselStoryStyle && omitQuoteStrips !== true) {
      composeOptions.postQuoteStyleIndependent = true;
      try {
        composeOptions.stripLayoutSeed = await odqReadLayoutBStripLayoutSeed(dk, 'post');
        composeOptions.stripLayoutSeedExplicit = true;
      } catch (_) {
        composeOptions.stripLayoutSeed =
          globalThis.odqGetCachedLayoutBStripLayoutSeed?.(dk, 'post') ?? composeOptions.stripLayoutSeed;
      }
      try {
        const postKeywords = await odqReadLayoutBKeywordEmphasis(dk, 'post');
        if (postKeywords) {
          composeOptions.keywordEmphasis = postKeywords;
          composeOptions.keywordEmphasisExplicit = true;
        }
      } catch (_) {
        const cachedKeywords = globalThis.odqGetCachedLayoutBKeywordEmphasis?.(dk, 'post');
        if (cachedKeywords) {
          composeOptions.keywordEmphasis = cachedKeywords;
          composeOptions.keywordEmphasisExplicit = true;
        }
      }
    }
    if (
      omitQuoteStrips !== true &&
      carouselStoryStyle &&
      typeof globalThis.odqReadLayoutBPostStripPlan === 'function'
    ) {
      try {
        const savedPlan = await globalThis.odqReadLayoutBPostStripPlan(dk);
        if (savedPlan && savedPlan.length) {
          composeOptions.savedPostStripPlan = savedPlan;
          composeOptions.replaySavedCarouselStripPlan = true;
        }
      } catch (_) { /* optional */ }
    } else if (
      omitQuoteStrips !== true &&
      typeof globalThis.odqReadLayoutBPostStripPlan === 'function'
    ) {
      try {
        const savedPlan = await globalThis.odqReadLayoutBPostStripPlan(dk);
        if (savedPlan && savedPlan.length) {
          composeOptions.savedPostStripPlan = savedPlan;
        }
      } catch (_) { /* optional */ }
    }
    if (omitQuoteStrips !== true && !composeOptions.postQuoteStyleIndependent) {
      const skipStoryRefForSavedStripPlan =
        composeOptions.replaySavedCarouselStripPlan === true &&
        Array.isArray(composeOptions.savedPostStripPlan) &&
        composeOptions.savedPostStripPlan.length > 0;
      if (!skipStoryRefForSavedStripPlan) {
        let refPlan = this._layoutBStoryRefStripPlan;
        if (!refPlan || !refPlan.length) {
          await this._captureLayoutBStoryRefStripPlanForPost(
            highResBlob,
            quoteText,
            quoteAuthor,
            dk,
            composeOptions
          );
          refPlan = this._layoutBStoryRefStripPlan;
        }
        if (refPlan && refPlan.length) {
          composeOptions.storyRefStripPlan =
            typeof globalThis.odqCloneLayoutBStripPlan === 'function'
              ? globalThis.odqCloneLayoutBStripPlan(refPlan)
              : refPlan;
        }
      }
    }
    if (omitQuoteStrips === true) {
      composeOptions.omitQuoteStrips = true;
      composeOptions.savedPostStripPlan = null;
      composeOptions.storyRefStripPlan = null;
    }
    if (carouselStoryStyle) {
      composeOptions.carouselStoryStyle = true;
      if (omitQuoteStrips !== true) {
        composeOptions.postStripLayoutFromStory = true;
      }
      if (!carouselFullWidthQuiltSource) {
        composeOptions.quiltFit = undefined;
      }
      if (
        composeOptions.speakerOverlay &&
        typeof globalThis.odqSpeakerCutoutTransformForDateAsync === 'function'
      ) {
        try {
          const postTransform = await globalThis.odqSpeakerCutoutTransformForDateAsync(dk, 'post');
          composeOptions.speakerOverlay.transform = postTransform || undefined;
        } catch (_) {
          /* keep existing transform */
        }
        if (
          !composeOptions.speakerOverlay.transform &&
          typeof globalThis.odqReadInstagramImagesDocWithFallback === 'function' &&
          typeof globalThis.odqSpeakerCutoutTransformFromInstagramData === 'function'
        ) {
          try {
            const ig = await globalThis.odqReadInstagramImagesDocWithFallback(dk, 8000);
            if (ig?.data) {
              composeOptions.speakerOverlay.transform =
                globalThis.odqSpeakerCutoutTransformFromInstagramData(ig.data, 'post') || undefined;
            }
          } catch (_) {
            /* optional */
          }
        }
      }
    } else if (postStripLayoutFromStory) {
      composeOptions.postStripLayoutFromStory = true;
    }
    if (carouselFullWidthQuiltSource === true) {
      composeOptions.quiltFit = 'cover';
    }
    if (carouselSpeakerAvoidRect && !carouselStoryStyle) {
      composeOptions.carouselSpeakerAvoidRect = carouselSpeakerAvoidRect;
    }
    if (carouselShortQuote) {
      composeOptions.carouselShortQuote = true;
    }
    this._lastLayoutBPostComposeMeta = null;
    if (typeof globalThis._lastLayoutBComposeMeta !== 'undefined') {
      globalThis._lastLayoutBComposeMeta = null;
    }
    const blob = await compose(
      highResBlob,
      quoteText,
      quoteAuthor,
      1080,
      1350,
      dk,
      composeOptions
    );
    this._lastLayoutBPostComposeMeta = globalThis._lastLayoutBComposeMeta || null;
    return blob;
  }

  async _generateInstagramPostLayoutBImageData(
    blocks,
    quoteObj,
    dateKey,
    {
      requireSpeaker = false,
      speakerHeroPost = false,
      carouselSpeakerAvoidRect = null,
      carouselShortQuote = false,
      carouselStoryStyle = false,
      carouselFullWidthQuiltSource = false,
      postStripLayoutFromStory = false,
      omitQuoteStrips = false
    } = {}
  ) {
    const postBlob = await this.generateInstagramPostLayoutBBlob(blocks, quoteObj, dateKey, {
      requireSpeaker,
      speakerHeroPost,
      carouselSpeakerAvoidRect,
      carouselShortQuote,
      carouselStoryStyle,
      carouselFullWidthQuiltSource,
      postStripLayoutFromStory,
      omitQuoteStrips
    });
    if (!postBlob) return null;
    return await new Promise((resolve) => {
      const fr = new FileReader();
      fr.onload = () => resolve(fr.result || null);
      fr.onerror = () => resolve(null);
      fr.readAsDataURL(postBlob);
    });
  }

  /**
   * Carousel slide 1 alternate: quilt + carousel speaker (no Layout B strips / name strip),
   * with the already-produced newspaper quote clipping overlaid (letter-size + post floor).
   */
  async composeCarouselSlide1QuoteClippingImageData(
    blocks,
    quoteObj,
    dateKey,
    {
      clippingDataUrl,
      clippingMeta = null,
      carouselShortQuote = false,
      clippingOffset = null,
      clippingOffsetExplicit = false
    } = {}
  ) {
    const clipUrl = String(clippingDataUrl || '').trim();
    if (!clipUrl) return null;
    const baseDataUrl = await this._generateInstagramPostLayoutBImageData(blocks, quoteObj, dateKey, {
      carouselStoryStyle: true,
      carouselShortQuote: carouselShortQuote === true,
      carouselFullWidthQuiltSource: true,
      omitQuoteStrips: true
    });
    if (!baseDataUrl) return null;
    let resolvedOffset = clippingOffset;
    if (!clippingOffsetExplicit) {
      const getCached = globalThis.odqGetCachedLayoutBCarouselClippingOffset;
      const normalize = globalThis.odqNormalizeCarouselClippingOffset;
      if (typeof getCached === 'function') {
        resolvedOffset = getCached(dateKey) || { x: 0, y: 0 };
      }
      if (typeof normalize === 'function') {
        resolvedOffset = normalize(resolvedOffset);
      }
    }
    return await this._overlayQuoteClippingOnCarouselSlide1(baseDataUrl, clipUrl, clippingMeta, {
      offset: resolvedOffset
    });
  }

  async _overlayQuoteClippingOnCarouselSlide1(
    baseDataUrl,
    clippingDataUrl,
    clippingMeta = null,
    overlayOptions = {}
  ) {
    const W = 1080;
    const H = 1350;
    const load = (src) =>
      new Promise((resolve, reject) => {
        const img = new Image();
        img.onload = () => resolve(img);
        img.onerror = () => reject(new Error('clipping overlay image load failed'));
        img.src = src;
      });
    try {
      const [bg, clip] = await Promise.all([load(baseDataUrl), load(clippingDataUrl)]);
      const QNC = globalThis.QuiltNewspaperClipping;
      const sheetW = Number(QNC?.CLIPPING_DOM_SHEET_W) || 428;
      const postColumnW = W * 0.86;
      const sheetToPost = postColumnW / sheetW;
      const POST_MIN_COLUMN_FRACTION = 0.78;
      const layoutOpts = QNC?.resolveClippingLayoutOpts
        ? QNC.resolveClippingLayoutOpts({ minBodyDomPx: 8 })
        : { minBodyDomPx: 8 };
      const effectiveBody = Number(clippingMeta?.effectiveBodyDomPx);
      if (Number.isFinite(effectiveBody) && effectiveBody > 0) {
        layoutOpts.effectiveBodyDomPx = effectiveBody;
      }
      const renderWidth =
        Number(clippingMeta?.renderWidth) > 0
          ? Number(clippingMeta.renderWidth)
          : Number(QNC?.resolveDefaultClippingRenderWidthPx?.()) || clip.width;
      const letterLayout =
        typeof QNC?.resolveQuoteClippingDisplayLayout === 'function'
          ? QNC.resolveQuoteClippingDisplayLayout(clip.width, clip.height, renderWidth, layoutOpts)
          : null;
      const sheetDisplayW = Number(letterLayout?.displayWidthPx) || sheetW;
      const letterSizedW = Math.round(sheetDisplayW * sheetToPost);
      const postFloorW = Math.round(postColumnW * POST_MIN_COLUMN_FRACTION);
      let cw = Math.min(postColumnW, Math.max(postFloorW, letterSizedW));
      let ch = Math.max(1, Math.round((clip.height / Math.max(1, clip.width)) * cw));
      const maxH = H * 0.62;
      if (ch > maxH) {
        cw = Math.max(postFloorW, Math.round(cw * (maxH / ch)));
        ch = Math.max(1, Math.round((clip.height / Math.max(1, clip.width)) * cw));
      }
      const canvas = document.createElement('canvas');
      canvas.width = W;
      canvas.height = H;
      const ctx = canvas.getContext('2d');
      if (!ctx) return null;
      ctx.drawImage(bg, 0, 0, W, H);
      const normalize =
        globalThis.odqNormalizeCarouselClippingOffset || globalThis.odqNormalizeQuoteStripOffset;
      const offset =
        typeof normalize === 'function'
          ? normalize(overlayOptions?.offset || overlayOptions)
          : { x: 0, y: 0 };
      const tilt = -3.4;
      const cx = W * 0.46 + (Number(offset?.x) || 0) * W;
      const cy = Math.min(H * 0.42, H * 0.22 + ch / 2) + (Number(offset?.y) || 0) * H;
      ctx.save();
      ctx.translate(cx, cy);
      ctx.rotate((tilt * Math.PI) / 180);
      ctx.shadowColor = 'rgba(0,0,0,0.42)';
      ctx.shadowBlur = 36;
      ctx.shadowOffsetX = 6;
      ctx.shadowOffsetY = 16;
      ctx.drawImage(clip, -cw / 2, -ch / 2, cw, ch);
      ctx.restore();
      this._lastCarouselSlide1ClippingMeta = {
        displayWidthPx: cw,
        displayHeightPx: ch,
        sheetDisplayW,
        letterSizedW,
        postFloorW,
        renderWidth,
        offsetX: Number(offset?.x) || 0,
        offsetY: Number(offset?.y) || 0,
        effectiveBodyDomPx: Number(letterLayout?.effectiveBodyDomPx) || effectiveBody || 0
      };
      return canvas.toDataURL('image/png');
    } catch (err) {
      this.logger?.warn?.('Carousel slide 1 quote-clipping overlay failed:', err?.message || err);
      return null;
    }
  }

  /**
   * Instagram feed post (4:5, 1080×1350): Layout B quilt + quote strips + speaker cutout when available.
   */
  async generateInstagramPostLayoutBImage(blocks, quoteObj = null, dateKey = null) {
    try {
      return await this._generateInstagramPostLayoutBImageData(blocks, quoteObj, dateKey);
    } catch (error) {
      this.logger.error('Instagram post layout B generation failed:', error);
      return null;
    }
  }

  /**
   * Catalog/assignment-aware speaker URL for Layout B (transparent cutout before portrait).
   * Matches exit chamber + `getQuoteSpeakerProfile`.
   */
  _layoutBSpeakerPortraitUrl(quote) {
    const q = quote && typeof quote === 'object' ? quote : {};
    const candidates = [
      q.speakerImageUrlSnapshot,
      q.speaker_image_url_snapshot,
      q.speakerImageUrl,
      q.speaker_image_url,
      q.speakerImage,
      q.speaker_image
    ];
    for (const value of candidates) {
      const url = String(value ?? '').trim();
      if (!url) continue;
      if (/speaker-cutouts(?:%2F|\/)/i.test(url)) continue;
      return url;
    }
    return '';
  }

  _speakerStorageSlug(speakerName) {
    return (
      String(speakerName || 'speaker')
        .trim()
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-+|-+$/g, '')
        .slice(0, 80) || 'speaker'
    );
  }

  async _ensureFirebaseStorageReady() {
    if (window.firebaseStorage) return window.firebaseStorage;
    if (!window.firebaseApp) return null;
    try {
      const { getStorage } = await import('https://www.gstatic.com/firebasejs/10.7.1/firebase-storage.js');
      window.firebaseStorage = getStorage(window.firebaseApp);
      return window.firebaseStorage;
    } catch (_) {
      return null;
    }
  }

  /** Resolve cutout download URL from Storage path when Firestore snapshot is stale. */
  async resolveSpeakerCutoutFromStorage(portraitUrl, speakerName) {
    const portrait = String(portraitUrl || '').trim();
    if (!portrait) return '';
    const hash = await this._portraitUrlHash12(portrait);
    if (!hash) return '';
    const cacheKey = `${this._speakerStorageSlug(speakerName)}:${hash}`;
    if (!this._speakerCutoutStorageUrlCache) this._speakerCutoutStorageUrlCache = new Map();
    if (this._speakerCutoutStorageUrlCache.has(cacheKey)) {
      return this._speakerCutoutStorageUrlCache.get(cacheKey);
    }
    const storage = await this._ensureFirebaseStorageReady();
    if (!storage) return '';
    const slug = this._speakerStorageSlug(speakerName);
    const paths = [
      `speaker-cutouts/${slug}-${hash}-manual.png`,
      `speaker-cutouts/${slug}-${hash}.png`,
      `speaker-cutouts/${slug}-${hash}-crop10.png`
    ];
    try {
      const { ref, getDownloadURL } = await import('https://www.gstatic.com/firebasejs/10.7.1/firebase-storage.js');
      for (const storagePath of paths) {
        try {
          const url = await getDownloadURL(ref(storage, storagePath));
          this._speakerCutoutStorageUrlCache.set(cacheKey, url);
          return url;
        } catch (_) {
          /* try next suffix */
        }
      }
    } catch (_) {
      /* storage SDK unavailable */
    }
    this._speakerCutoutStorageUrlCache.set(cacheKey, '');
    return '';
  }

  async _portraitUrlHash12(portraitUrl) {
    const portrait = String(portraitUrl || '').trim();
    if (!portrait) return '';
    if (!this._portraitUrlHashCache) this._portraitUrlHashCache = new Map();
    if (this._portraitUrlHashCache.has(portrait)) return this._portraitUrlHashCache.get(portrait);
    const promise = crypto.subtle
      .digest('SHA-256', new TextEncoder().encode(portrait))
      .then((buf) =>
        Array.from(new Uint8Array(buf))
          .map((b) => b.toString(16).padStart(2, '0'))
          .join('')
          .slice(0, 12)
      )
      .catch(() => '');
    this._portraitUrlHashCache.set(portrait, promise);
    return promise;
  }

  async _speakerCutoutSnapMatchesPortraitUrl(cutoutUrl, portraitUrl) {
    const cutout = String(cutoutUrl || '').trim();
    const portrait = String(portraitUrl || '').trim();
    if (!cutout || !portrait) return false;
    if (!/speaker-cutouts(?:%2F|\/)/i.test(cutout)) return true;
    const hash = await this._portraitUrlHash12(portrait);
    if (!hash) return false;
    return cutout.includes(`-${hash}.`) || cutout.includes(`-${hash}-`);
  }

  /** Pick portrait vs Storage cutout for on-screen display (avoids proxying deleted cutouts). */
  async resolveSpeakerDisplayUrl({ portraitUrl, cutoutUrl, cutoutSnap, speakerName } = {}) {
    let portrait = String(portraitUrl || '').trim();
    if (/speaker-cutouts(?:%2F|\/)/i.test(portrait)) portrait = '';
    const cutout = String(cutoutUrl || cutoutSnap || '').trim();
    if (!portrait && !cutout) return '';
    if (!cutout) return portrait;
    if (!/speaker-cutouts(?:%2F|\/)/i.test(cutout)) return cutout;
    if (!portrait) return cutout;
    if (await this._speakerCutoutSnapMatchesPortraitUrl(cutout, portrait)) {
      return cutout;
    }
    const storageCutout = await this.resolveSpeakerCutoutFromStorage(portrait, speakerName);
    return storageCutout || portrait;
  }

  async resolveLayoutBSpeakerCutoutUrl(quote, fallbackName = '') {
    const q = quote && typeof quote === 'object' ? quote : {};
    const author = String(fallbackName || q.author || '').trim();
    const snap = String(q.speakerCutoutUrlSnapshot ?? q.speaker_cutout_url_snapshot ?? '').trim();
    const portrait = this._layoutBSpeakerPortraitUrlResolved(q);
    const qs = typeof window !== 'undefined' ? window.app?.quoteService : null;
    const snapAuthorOk =
      !snap ||
      !qs ||
      typeof qs.speakerStorageCutoutUrlMatchesAuthor !== 'function' ||
      qs.speakerStorageCutoutUrlMatchesAuthor(snap, author);
    if (!portrait) {
      const directCutout = String(
        q.speakerCutoutUrl ??
          q.speaker_cutout_url ??
          q.speakerCutoutUrlSnapshot ??
          q.speaker_cutout_url_snapshot ??
          snap ??
          ''
      ).trim();
      if (directCutout && this._isFirebaseSpeakerCutoutUrl(directCutout) && snapAuthorOk) {
        return directCutout;
      }
      return snap && this._isFirebaseSpeakerCutoutUrl(snap) && snapAuthorOk ? snap : '';
    }
    if (snap && snapAuthorOk && (await this._speakerCutoutSnapMatchesPortraitUrl(snap, portrait))) {
      return snap;
    }
    const resolved = qs ? String(qs._resolveSpeakerCutoutForQuote(q) || '').trim() : '';
    const resolvedAuthorOk =
      !resolved ||
      !qs ||
      typeof qs.speakerStorageCutoutUrlMatchesAuthor !== 'function' ||
      qs.speakerStorageCutoutUrlMatchesAuthor(resolved, author);
    if (resolved && resolvedAuthorOk && (await this._speakerCutoutSnapMatchesPortraitUrl(resolved, portrait))) {
      return resolved;
    }
    const name = String(q.speakerName ?? q.speaker_name ?? fallbackName ?? q.author ?? '')
      .replace(/^\s*[—-]\s*/, '')
      .trim();
    const fromStorage = await this.resolveSpeakerCutoutFromStorage(portrait, name);
    if (fromStorage) return fromStorage;
    /**
     * Same PNG the speaker card uses. Portrait-hash can disagree (assignment vs catalog URL)
     * and used to drop the cutout, leaving the full Wikimedia rectangle on story preview.
     */
    if (resolved && resolvedAuthorOk && this._isFirebaseSpeakerCutoutUrl(resolved)) {
      return resolved;
    }
    if (snap && snapAuthorOk && this._isFirebaseSpeakerCutoutUrl(snap)) {
      return snap;
    }
    return '';
  }

  _resolveLayoutBSpeakerImageUrl(quote) {
    const q = quote && typeof quote === 'object' ? quote : {};
    const portrait = this._layoutBSpeakerPortraitUrl(q);
    const app = typeof window !== 'undefined' ? window.app : null;
    const qs = app?.quoteService;
    if (qs && typeof qs._resolveSpeakerCutoutForQuote === 'function') {
      const cutout = String(qs._resolveSpeakerCutoutForQuote(q) || '').trim();
      if (cutout && this._isFirebaseSpeakerCutoutUrl(cutout)) return cutout;
    }
    if (app && typeof app.getQuoteSpeakerProfile === 'function') {
      const profile = app.getQuoteSpeakerProfile(q);
      const profileCutout = String(profile?.cutoutUrl || '').trim();
      if (profileCutout && this._isFirebaseSpeakerCutoutUrl(profileCutout)) return profileCutout;
      if (profile?.imageUrl) return String(profile.imageUrl).trim();
    }
    const cutoutSnap = String(q.speakerCutoutUrlSnapshot || q.speaker_cutout_url_snapshot || '').trim();
    if (cutoutSnap && this._isFirebaseSpeakerCutoutUrl(cutoutSnap)) return cutoutSnap;
    return portrait;
  }

  /** Pinned/catalog quote for a date — same path as Zapier caption + nightly IG job. */
  async _quoteForLayoutBSpeakerImage(quoteObj, dateKey) {
    const dk = dateKey || Utils.getTodayKey();
    const app = typeof window !== 'undefined' ? window.app : null;
    if (app?.isAdminTomorrowPreviewActive?.() && app?.getEffectiveAppDateKey?.() === dk) {
      const previewQuote = app.getEffectiveQuiltQuote?.();
      if (previewQuote?.text) return previewQuote;
    }
    if (quoteObj && typeof quoteObj === 'object') {
      const explicitText = String(quoteObj.text ?? quoteObj.body ?? '').trim();
      if (explicitText) return quoteObj;
    }
    const qs = typeof window !== 'undefined' ? window.app?.quoteService : null;
    if (qs && typeof qs.getQuoteResolvedForInstagramDateKey === 'function') {
      try {
        const resolved = await qs.getQuoteResolvedForInstagramDateKey(dk);
        if (resolved) return resolved;
      } catch (e) {
        this.logger.warn('Layout B speaker quote resolve failed:', e);
      }
    }
    if (quoteObj && typeof quoteObj === 'object') return quoteObj;
    if (qs && typeof qs.getTodayQuote === 'function') {
      const today = qs.getTodayQuote();
      if (today) return today;
    }
    return { text: '', author: '' };
  }

  /**
   * Instagram feed post (4:5): contributor name cloud on full-bleed quilt (contributor-cloud.png).
   * @returns {Promise<Blob|null>} null when contributors normalize to empty
   */
  async generateInstagramContributorCloudBlob(blocks, contributors = [], dateKey = null) {
    if (!blocks || blocks.length === 0) return null;
    const compose = globalThis.composeContributorCloudPostFromQuiltBlob;
    if (typeof compose !== 'function') {
      throw new Error('composeContributorCloudPostFromQuiltBlob missing (contributor-cloud-compose.js must load)');
    }
    const normalized =
      typeof globalThis.ContributorCloudCompose?.normalizeContributors === 'function'
        ? globalThis.ContributorCloudCompose.normalizeContributors(contributors)
        : (Array.isArray(contributors) ? contributors : []);
    if (!normalized.length) return null;

    const dk = dateKey || Utils.getTodayKey();
    const highResBlob = await this.getInstagramQuiltSourceBlob(blocks, {
      dateKey: dk,
      exportModeLabel: 'contributor_cloud_post_9x16_cover_source',
      skipFilmGrain: true
    });
    if (!highResBlob) return null;

    if (typeof globalThis.ensureContributorCloudFontsReady === 'function') {
      await globalThis.ensureContributorCloudFontsReady();
    }
    return compose(highResBlob, normalized, blocks, 1080, 1350, { dateKey: dk });
  }

  async _generateInstagramContributorCloudImageData(blocks, contributors, dateKey) {
    const blob = await this.generateInstagramContributorCloudBlob(blocks, contributors, dateKey);
    if (!blob) return null;
    return await new Promise((resolve) => {
      const fr = new FileReader();
      fr.onload = () => resolve(fr.result || null);
      fr.onerror = () => resolve(null);
      fr.readAsDataURL(blob);
    });
  }

  /**
   * Instagram feed post (4:5): contributor name cloud on full-bleed quilt.
   */
  async generateInstagramContributorCloudImage(blocks, contributors = [], dateKey = null) {
    try {
      return await this._generateInstagramContributorCloudImageData(blocks, contributors, dateKey);
    } catch (error) {
      this.logger.error('Instagram contributor cloud generation failed:', error);
      return null;
    }
  }

  /**
   * Instagram feed post (4:5): large speaker cutout on quilt + name strip (layout-b-speaker.png).
   */
  async generateInstagramPostLayoutBSpeakerImage(blocks, quoteObj = null, dateKey = null) {
    try {
      return await this._generateInstagramPostLayoutBImageData(blocks, quoteObj, dateKey, {
        requireSpeaker: true,
        speakerHeroPost: true
      });
    } catch (error) {
      this.logger.error('Instagram post layout B speaker generation failed:', error);
      return null;
    }
  }

  // Delete archive entry
  async deleteArchive(date) {
    try {
      // Delete from Firestore if available
      // if (window.db && window.firestore) {
      //   const archiveRef = window.firestore.doc(window.db, 'archives', date);
      //   await window.firestore.deleteDoc(archiveRef);
      //   this.logger.log(`🗑️ Archive deleted from Firestore: ${date}`);
      // }
      
      // Remove from local cache
      this.archives.delete(date);
      this.logger.log(`🗑️ Archive removed from local cache: ${date}`);
      
    } catch (error) {
      this.logger.error('Failed to delete archive:', error);
      throw error;
    }
  }

  // Generate thumbnail from SVG
  async generateThumbnail(svgElement) {
    try {
      if (!svgElement) {
        svgElement = document.getElementById('quilt');
      }
      
      if (!svgElement) {
        this.logger.warn('No SVG element found for thumbnail generation');
        return 'data:image/svg+xml;base64,';
      }

      // SAFEGUARD: Check if SVG has content
      if (!svgElement.children || svgElement.children.length === 0) {
        this.logger.warn('SVG element has no children - using fallback thumbnail');
        return 'data:image/svg+xml;base64,';
      }

      // Clone SVG to avoid modifying original
      const svgClone = svgElement.cloneNode(true);
      
      // Get SVG dimensions
      const svgRect = svgElement.getBoundingClientRect();
      const width = Math.max(400, svgRect.width);
      const height = Math.max(400, svgRect.height);
      
      // Ensure SVG has proper attributes
      svgClone.setAttribute('width', width);
      svgClone.setAttribute('height', height);
      svgClone.setAttribute('xmlns', 'http://www.w3.org/2000/svg');
      
      // Convert SVG to string
      const svgData = new XMLSerializer().serializeToString(svgClone);
      const svgBlob = new Blob([svgData], { type: 'image/svg+xml;charset=utf-8' });
      
      // Create canvas for conversion
      const canvas = document.createElement('canvas');
      const ctx = canvas.getContext('2d');
      canvas.width = width;
      canvas.height = height;
      
      // Create image from SVG
      const img = new Image();
      const url = URL.createObjectURL(svgBlob);
      
      return new Promise((resolve) => {
        img.onload = () => {
          // Draw to canvas
          ctx.fillStyle = '#f6f4f1'; // Background color
          ctx.fillRect(0, 0, width, height);
          ctx.drawImage(img, 0, 0, width, height);
          
          // Convert to data URL
          const dataURL = canvas.toDataURL('image/png', 0.8);
          URL.revokeObjectURL(url);
          
          this.logger.log(`✅ Generated thumbnail: ${width}x${height}`);
          resolve(dataURL);
        };
        
        img.onerror = () => {
          URL.revokeObjectURL(url);
          this.logger.error('Failed to generate thumbnail');
          resolve('data:image/svg+xml;base64,');
        };
        
        img.src = url;
      });
      
    } catch (error) {
      this.logger.error('Thumbnail generation failed:', error);
      return 'data:image/svg+xml;base64,';
    }
  }

  // Generate full quilt image from blocks data
  async generateFullQuiltImage(blocks) {
    try {
      if (!blocks || blocks.length === 0) {
        this.logger.warn('No blocks provided for full quilt image generation');
        return 'data:image/svg+xml;base64,';
      }

      // Calculate quilt bounds
      const minX = Math.min(...blocks.map(b => b.x));
      const minY = Math.min(...blocks.map(b => b.y));
      const maxX = Math.max(...blocks.map(b => b.x + b.width));
      const maxY = Math.max(...blocks.map(b => b.y + b.height));
      
      const quiltWidth = maxX - minX;
      const quiltHeight = maxY - minY;
      
      // Create canvas for full quilt image
      const canvas = document.createElement('canvas');
      const ctx = canvas.getContext('2d');
      
      // Set canvas size (tall and narrow like the quilt)
      canvas.width = 800;
      canvas.height = 1200;
      
      // Fill background
      ctx.fillStyle = '#f6f4f1';
      ctx.fillRect(0, 0, canvas.width, canvas.height);
      
      // Calculate scale to fit quilt in canvas
      const scaleX = canvas.width / quiltWidth;
      const scaleY = canvas.height / quiltHeight;
      const scale = Math.min(scaleX, scaleY, 1) * 1.2; // 20% larger to reduce white space
      
      // Center the quilt
      const offsetX = (canvas.width - quiltWidth * scale) / 2;
      const offsetY = (canvas.height - quiltHeight * scale) / 2;
      
      // Draw each block
      blocks.forEach(block => {
        SimpleQuiltEngine.ensureInsetClassificationFromGeometry(block);
        // Add jitter effects (same as current quilt)
        const jitterX = (Math.random() - 0.5) * 6;
        const jitterY = (Math.random() - 0.5) * 6;
        const jitterRotation = (Math.random() - 0.5) * 3;
        
        const jitteredX = block.x + jitterX;
        const jitteredY = block.y + jitterY;
        
        // Transform to canvas coordinates
        const canvasX = offsetX + (jitteredX - minX) * scale;
        const canvasY = offsetY + (jitteredY - minY) * scale;
        const canvasWidth = block.width * scale;
        const canvasHeight = block.height * scale;
        
        const isHst = block.patternType === 'special' && block.specialPatternType === 'hst';
        if (isHst) {
          ctx.save();
          ctx.shadowColor = 'rgba(0, 0, 0, 0.15)';
          ctx.shadowBlur = 3;
          ctx.shadowOffsetX = 2;
          ctx.shadowOffsetY = 4;
          const tris = Utils.getHstOrganicRenderTriangles(block, 1);
          const centerX = canvasX + canvasWidth / 2;
          const centerY = canvasY + canvasHeight / 2;
          const bw = Math.max(1e-6, block.width);
          const bh = Math.max(1e-6, block.height);
          const cw = canvasWidth;
          const ch = canvasHeight;
          ctx.translate(centerX, centerY);
          ctx.rotate((jitterRotation * Math.PI) / 180);
          const x0 = -cw / 2;
          const y0 = -ch / 2;
          tris.forEach((t, tux) => {
            const pts = t.points || [];
            if (pts.length < 3) return;
            ctx.fillStyle = t.color || block.color;
            ctx.beginPath();
            ctx.moveTo(x0 + (pts[0][0] / bw) * cw, y0 + (pts[0][1] / bh) * ch);
            for (let k = 1; k < pts.length; k++) {
              ctx.lineTo(x0 + (pts[k][0] / bw) * cw, y0 + (pts[k][1] / bh) * ch);
            }
            ctx.closePath();
            ctx.fill();
            const anch = Utils.hstOrganicTriangleRadialAnchorLocal(pts);
            if (!anch) return;
            const tcx = x0 + (anch.cx / bw) * cw;
            const tcy = y0 + (anch.cy / bh) * ch;
            const spanPx = Math.max((anch.span / bw) * cw, (anch.span / bh) * ch, 1e-6);
            const washHx = /^#[0-9a-f]{6}$/i.test(String(t.color || '').trim())
              ? String(t.color).trim()
              : /^#[0-9a-f]{6}$/i.test(String(block.color || '').trim())
                ? String(block.color).trim()
                : '#c8c4bf';
            ctx.save();
            ctx.beginPath();
            ctx.moveTo(x0 + (pts[0][0] / bw) * cw, y0 + (pts[0][1] / bh) * ch);
            for (let k = 1; k < pts.length; k++) {
              ctx.lineTo(x0 + (pts[k][0] / bw) * cw, y0 + (pts[k][1] / bh) * ch);
            }
            ctx.closePath();
            ctx.clip();
            Utils.canvasWatercolorWashAxisAlignedRect(
              ctx,
              x0,
              y0,
              cw,
              ch,
              washHx,
              `${block.id || 'blk'}|fullquilt|${tux}`,
              { radial: { cx: tcx, cy: tcy, span: spanPx }, skipClip: true }
            );
            ctx.restore();
          });
          ctx.restore();
          return;
        }

        const isInset = block.patternType === 'special' && block.specialPatternType === 'insetCircle';
        if (isInset) {
          ctx.save();
          ctx.shadowColor = 'rgba(0, 0, 0, 0.15)';
          ctx.shadowBlur = 3;
          ctx.shadowOffsetX = 2;
          ctx.shadowOffsetY = 4;
          const centerX = canvasX + canvasWidth / 2;
          const centerY = canvasY + canvasHeight / 2;
          const bw = Math.max(1e-6, block.width);
          const bh = Math.max(1e-6, block.height);
          const cw = canvasWidth;
          const ch = canvasHeight;
          ctx.translate(centerX, centerY);
          ctx.rotate((jitterRotation * Math.PI) / 180);
          const x0 = -cw / 2;
          const y0 = -ch / 2;
          Utils.fillInsetCircleBlockCanvasCtx(ctx, block, x0, y0, cw, ch);
          ctx.restore();
          return;
        }

        // Save context for rotation
        ctx.save();
        
        // Move to center of block for rotation
        const centerX = canvasX + canvasWidth / 2;
        const centerY = canvasY + canvasHeight / 2;
        ctx.translate(centerX, centerY);
        ctx.rotate(jitterRotation * Math.PI / 180);
        
        // Draw block with shadow
        ctx.shadowColor = 'rgba(0, 0, 0, 0.15)';
        ctx.shadowBlur = 3;
        ctx.shadowOffsetX = 2;
        ctx.shadowOffsetY = 4;
        
        ctx.fillStyle = block.color;
        ctx.fillRect(-canvasWidth / 2, -canvasHeight / 2, canvasWidth, canvasHeight);
        
        // Restore context
        ctx.restore();
      });
      
      // Convert to data URL
      const dataURL = canvas.toDataURL('image/png', 0.9);
      
      this.logger.log(`✅ Generated full quilt image: 800x1200`);
      return dataURL;
      
    } catch (error) {
      this.logger.error('Full quilt image generation failed:', error);
      return 'data:image/svg+xml;base64,';
    }
  }

  // Load more archives (for "Load More" button)
  async loadMore() {
    if (this.isLoading) return null;
    
    this.isLoading = true;
    this.currentPage++;
    
    try {
      const result = await this.getArchives(this.currentPage);
      this.isLoading = false;
      return result;
    } catch (error) {
      this.isLoading = false;
      this.logger.error('Failed to load more archives:', error);
      return null;
    }
  }

}

  root.ArchiveService = ArchiveService;
})(typeof globalThis !== 'undefined' ? globalThis : typeof window !== 'undefined' ? window : this);
