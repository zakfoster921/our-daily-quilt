...
class ArchiveService {
  ...
  async _resolveCarouselSpeakerSeamRectFromOverlay(overlay) {
    ...
    try {
      return await resolve(overlay.imageUrl);
    } catch (err) {
  ...
  async applyCarouselSpeakerSeamToSlides(slide1DataUrl, slide2DataUrl, quoteObj, blocks, dateKey = null) {
    const apply =
  ...
  async buildIntegratedInstagramCarouselImageData(
    ...
    const carouselSpeakerAvoidRect = overlay
      ? await this._resolveCarouselSpeakerSeamRectFromOverlay(overlay)
      : null;
    const layoutBSlide1 = carouselSpeakerAvoidRect
  ...
  async generateInstagramPostLayoutBBlob(
    ...
    quoteObj = null,
    dateKey = null,
    { requireSpeaker = false, speakerHeroPost = false, carouselSpeakerAvoidRect = null } = {}
  ) {
    if (!blocks || blocks.length === 0) return null;
    const dk = dateKey || Utils.getTodayKey();
