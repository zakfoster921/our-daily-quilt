...
class ArchiveService {
  ...
  async generateInstagramCarouselSlideBlobs(blocks, contributors = [], dateKey = null, carouselOptions = {}) {
    ...
    const highResBlob = await this.getInstagramQuiltSourceBlob(blocks, {
      ...
      exportModeLabel: 'ig_carousel_full_quilt_field_source',
      skipFilmGrain: true
    });
    if (!highResBlob) return null;
    ...
    if (normalized.length && typeof compose === 'function') {
      const slide3QuiltBlob = await this.getInstagramQuiltSourceBlob(blocks, {
        dateKey: dk,
        exportModeLabel: 'ig_carousel_slide3_quilt_screen_source',
        skipFilmGrain: true
      });
      const result = await compose(highResBlob, normalized, {
        dateKey: dk,
        ...
        quiltFit: carouselOptions.quiltFit,
        slide3QuiltBlob,
  ...
  async generateInstagramPostLayoutBBlob(
    ...
    const highResBlob = await this.getInstagramQuiltSourceBlob(blocks, {
      dateKey: dk,
      exportFullQuiltField: carouselFullWidthQuiltSource === true,
      exportModeLabel: speakerHeroPost
        ? 'layout_b_speaker_hero_post_9x16_cover_source'
        : carouselFullWidthQuiltSource === true
          ? 'layout_b_carousel_post_full_quilt_field_source'
