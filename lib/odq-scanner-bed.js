...
(function (global) {
  ...
  const PROFILES = {
    ...
    /** User color Pantone chip — clean flatbed lift, not vintage riso wash. */
    colorCard: {
      bloomOpacity: 0.058,
      irisOpacity: 0.032,
      bloomX: 24,
  ...
  function bootstrapTriptych(host, dateKey) {
    ...
    host.querySelectorAll('.quilt-mood-triptych__zone-surface').forEach((surface) => {
      const mood = surface.closest('.quilt-mood-triptych__zone')?.dataset?.mood || 'zone';
      mountSurface(surface, `${dk}:triptych-scratch:${mood}`, {
        profile: 'newsprint',
        insertBefore: surface.firstElementChild,
        bedClass: 'odq-scanner-bed--riso'
      });
    });
  }

  function bootstrapCollage(host, dateKey) {
    if (!host) return;
    const dk = String(dateKey || 'odq');
    host.querySelectorAll('.quilt-mood-collage__tape-surface').forEach((surface) => {
      const mood = surface.closest('.quilt-mood-collage__tape')?.dataset?.mood || 'tape';
      mountSurface(surface, `${dk}:collage-scratch:${mood}`, {
        profile: 'newsprint',
        insertBefore: surface.firstElementChild,
        bedClass: 'odq-scanner-bed--riso'
      });
  ...
  function bootstrapQuiltPaper(scope, dateKey) {
    ...
    const collage = screen.querySelector('.quilt-mood-collage');
    if (collage) bootstrapCollage(collage, dk);
    ...
    screen.querySelectorAll('.quilt-user-shape-card__tab').forEach((host, i) => {
      mountSurface(host, `${dk}:user-color-tab:${i}`, {
        profile: 'colorCard',
        insertBefore: host.firstElementChild
      });
    });
