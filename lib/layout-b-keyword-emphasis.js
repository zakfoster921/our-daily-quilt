...
function buildTextRunsForLine(line, keywords, styles) {
  ...
  const segments = glueLeadingPunctToPreviousEmphasisSegments(
    stripTrailingPunctAfterEmphasis(QKE.buildLineSegments(line, keywords))
  );
...
const LEADING_PUNCT_AFTER_EMPHASIS_RE = /^([.,;:!?]+)/;

function isEmphasisPartKind(kind) {
  return kind === 'emphasis-strip' || kind === 'inline-emphasis';
}

/** Move leading punctuation from a plain gap onto the preceding emphasized keyword. */
function glueLeadingPunctToPreviousEmphasisParts(parts) {
  if (!Array.isArray(parts) || parts.length < 2) return parts;
  const out = [];
  for (const part of parts) {
    const prev = out[out.length - 1];
    if (part.kind === 'plain' && prev && isEmphasisPartKind(prev.kind)) {
      const m = LEADING_PUNCT_AFTER_EMPHASIS_RE.exec(String(part.text || ''));
      if (m) {
        prev.text = String(prev.text || '') + m[1];
        const rest = String(part.text || '').slice(m[0].length);
        if (rest.length) out.push({ ...part, text: rest });
        continue;
      }
    }
    out.push({ ...part });
...
function glueLeadingPunctToPreviousEmphasisSegments(segments) {
  if (!Array.isArray(segments) || segments.length < 2) return segments;
  const out = [];
  for (const seg of segments) {
    const prev = out[out.length - 1];
    if (!seg.emphasis && prev && prev.emphasis) {
      const m = LEADING_PUNCT_AFTER_EMPHASIS_RE.exec(String(seg.text || ''));
      if (m) {
        prev.text = String(prev.text || '') + m[1];
        const rest = String(seg.text || '').slice(m[0].length);
        if (rest.length) out.push({ ...seg, text: rest });
        continue;
      }
    }
    out.push({ ...seg });
...
function expandSegmentForKeywordEmphasis(segment, keywords, styles) {
  ...
  const glued = glueLeadingPunctToPreviousEmphasisParts(out);
  const filtered = dropPlainPunctPartsAfterEmphasis(glued);
