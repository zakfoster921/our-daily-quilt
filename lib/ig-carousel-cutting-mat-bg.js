...
(function (root, factory) {
  ...
  function () {
    ...
    /** Uniform scale on source mat (<1 zooms out slightly; height-fit base keeps grid natural). */
    const CAROUSEL_MAT_UNIFORM_ZOOM = 0.9;
    const CAROUSEL_MAT_LETTERBOX_FILL = '#174830';
    ...
    function drawCarouselCuttingMatBackground(ctx, matImg, panelW, panelH, panX = 0) {
      ...
      const scale = (panelH / ih) * CAROUSEL_MAT_UNIFORM_ZOOM;
      const tileW = iw * scale;
      const tileH = ih * scale;
      const dy = (panelH - tileH) / 2;

      ctx.fillStyle = CAROUSEL_MAT_LETTERBOX_FILL;
      ctx.fillRect(0, 0, panelW, panelH);
      ...
      for (let t = startTile; t <= endTile; t += 1) {
        ...
        ctx.drawImage(matImg, 0, 0, iw, ih, canvasX, dy, tileW, tileH);
      }
    }
