...
(function (root) {
  ...
  Object.assign(UtilsCore, {
    ...
    CONTRIBUTOR_NAME_MODERATION_ERROR: 'There seems to be a problem with this name. Please try another.',
    ...
    /** In-progress mood scratch foil (per zone) before a pick is committed. */
    QUILT_MOOD_SCRATCH_KEY: 'ourDailyQuiltMoodScratchV1',
    ...
    /** Server check for profanity / hate speech before saving a contributor name. */
    async validateContributorName(rawName) {
      const name = UtilsCore.normalizeStoredFirstName(rawName);
      if (!name || name.toLowerCase() === 'friend') return { ok: true };
      const baseUrl = String(root.CONFIG?.BACKEND?.baseUrl || '').replace(/\/$/, '');
      if (!baseUrl) return { ok: true };
      try {
        const res = await fetch(`${baseUrl}/api/validate-contributor-name`, {
          method: 'POST',
        ...
        return {
          ok: false,
          error: data?.error || UtilsCore.CONTRIBUTOR_NAME_MODERATION_ERROR
        };
      } catch (_) {
        return { ok: false, error: 'Could not verify name. Please try again.' };
      }
    },
    /**
