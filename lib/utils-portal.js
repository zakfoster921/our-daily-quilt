  ...
  Object.assign(UtilsCore, {
    ...
    refreshPortalGreeting() {
      ...
      if (portal?.classList.contains('portal-mode--first-visit')) {
        const welcomeLine = UtilsCore.getPortalFirstVisitGreetingLine();
        const run = () => root.app?.quoteService?.mountPortalGreetingStrips?.(welcomeLine);
        run();
      ...
      if (!line) {
        ...
        root.app?.quoteService?.mountPortalGreetingStrips?.('');
        return;
      }
      const run = () => root.app?.quoteService?.mountPortalGreetingStrips?.(line);
      run();
      root.requestAnimationFrame(run);
    }
  });
