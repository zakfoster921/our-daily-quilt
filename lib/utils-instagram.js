...
(function (root) {
  ...
  const layoutBStoryRenderVersion = () =>
    String(root.ODQ_LAYOUT_B_STORY_RENDER_VERSION || 'story-strip-cluster-v3').trim();
  ...
  Object.assign(UtilsCore, {
    ...
    applyNewspaperClippingDisplayMetaToDoc(docPayload, meta = {}) {
      if (!docPayload || typeof docPayload !== 'object') return docPayload;
      const row = meta && typeof meta === 'object' ? meta : {};
      const renderWidth = Number(row.renderWidth);
      const effectiveBodyDomPx = Number(row.effectiveBodyDomPx);
      const clippedWidth = Number(row.clippedWidth);
     ...
     * `quilt-screen-9x16.jpg`, `contributor-cloud.png` overwrite on each push.
     * `carousel-slide-1.png` = quote clipping on quilt+speaker (Zapier feed). `carousel-slide-2.png` + `carousel-slide-3.png` = seamless quilt contributor pair.
     * `layout-b.png` = Layout B strips post (when postLayoutBImageData provided); otherwise aliases to slide 1.
     * `layout-b-speaker.png` = speaker hero post (separate file).
    ...
    async writeInstagramImagesDocForZapierViaServer({
      ...
      storyLayoutBOverlayImageData = null,
      storyLayoutBPreviewImageData = null,
      quiltScreen9x16ImageData = null,
      newspaperClippingImageData = null,
      newspaperClippingDisplayMeta = null,
      moodClippingGoodImageData = null,
      moodClippingRoughImageData = null,
      contributorCloudImageData = null,
      carouselSlide0ImageData = null,
      carouselSlide0bImageData = null,
      carouselSlide1ImageData = null,
      carouselSlide2ImageData = null,
      carouselSlide3ImageData = null,
      carouselSlide4ImageData = null,
      zapierCaption = '',
      quiltFingerprint = '',
      blockCount,
      contributorCount,
      ...
      layoutBStoryPreviewRenderVersion = ''
    }) {
      const baseUrl = String(apiBaseOverride || root.CONFIG.BACKEND?.baseUrl || '').replace(/\/$/, '');
      if (!baseUrl) throw new Error('Backend base URL not configured');

      const approxBytes = (...parts) =>
        parts.reduce((sum, part) => sum + (typeof part === 'string' ? part.length : 0), 0);

      const normalizeDocPayload = (docPayload = {}) => {
        if (!docPayload.carouselSlide0Url && docPayload.carouselSlide0ImageStorageUrl) {
          docPayload.carouselSlide0Url = docPayload.carouselSlide0ImageStorageUrl;
        }
        if (!docPayload.carouselSlide0bUrl && docPayload.carouselSlide0bImageStorageUrl) {
          docPayload.carouselSlide0bUrl = docPayload.carouselSlide0bImageStorageUrl;
        }
        if (!docPayload.carouselSlide4Url && docPayload.carouselSlide4ImageStorageUrl) {
          docPayload.carouselSlide4Url = docPayload.carouselSlide4ImageStorageUrl;
        }
        return docPayload;
      };

      const postPayload = async (payload, label = 'IG push') => {
        ...
        try {
          ...
          if (!res.ok || data.success === false) {
            throw new Error(
              data.error || `Backend ${label} failed (${res.status}, ~${mb}MB body)`
            );
          }
          return normalizeDocPayload(data.docPayload || {});
        } catch (err) {
          const msg = err?.message || String(err);
          if (/Failed to fetch|NetworkError|Load failed/i.test(msg)) {
            throw new Error(
              `Backend ${label} network error (~${mb}MB body): ${msg}. Often means the request exceeded the API body limit.`
            );
          }
          throw err;
        }
      ...
      const basePayload = {
        ...
        markReadyForInstagram,
        layoutBStoryRenderVersion: storyLayoutBImageData ? layoutBStoryRenderVersion() : '',
        layoutBStoryPreviewRenderVersion: String(layoutBStoryPreviewRenderVersion || '').trim(),
        lastIgPushStartedAt: new Date().toISOString()
      };

      // Keep under older ~30MB Railway limits: send quote/speaker lead slides as a second merge when needed.
      const SOFT_SPLIT_BYTES = 28 * 1024 * 1024;
      const estimated = approxBytes(
        instagramImage,
        postLayoutBImageData,
        postLayoutBSpeakerImageData,
        storyLayoutBImageData,
      ...
      const shouldSplitLeadSlides =
        estimated > SOFT_SPLIT_BYTES && (carouselSlide0ImageData || carouselSlide0bImageData);
      if (shouldSplitLeadSlides) {
        const primary = await postPayload(
          {
            ...basePayload,
            carouselSlide0ImageData: null,
            carouselSlide0bImageData: null
        ...
        const leadOnly = await postPayload(
          {
            dateKey,
            carouselSlide0ImageData,
            carouselSlide0bImageData,
        ...
        return normalizeDocPayload({ ...primary, ...leadOnly });
      }

      return postPayload(basePayload);
    },
    async writeInstagramImagesDocForZapier({
      ...
      aliasLayoutBSpeakerUrl = false,
      /** Optional override for preview JPEG render version (e.g. quote-fit preview). */
      layoutBStoryPreviewRenderVersion = ''
    }) {
      ...
      const timedStep = async (label, promise, timeoutMs) => {
        try {
      ...
      const writeInstagramDocWithFallback = async (label, payload, sdkTimeoutMs) => {
        try {
          ...
          await timedStep(`${label} REST`, writeInstagramDocViaRest(payload), 30000);
          return 'rest';
        }
      };

      await timedStep('Firebase init', root.app.initializeFirebaseForImages(), 30000);
      ...
      let storyLayoutBOverlayImageStorageUrl = null;
      let storyLayoutBPreviewImageStorageUrl = null;
      let quiltScreen9x16ImageStorageUrl = null;
      let newspaperClippingImageStorageUrl = null;
      let contributorCloudImageStorageUrl = null;
      let carouselSlide0ImageStorageUrl = null;
      let carouselSlide0bImageStorageUrl = null;
      let carouselSlide1ImageStorageUrl = null;
      let carouselSlide2ImageStorageUrl = null;
      let carouselSlide3ImageStorageUrl = null;
      let carouselSlide4ImageStorageUrl = null;
      ...
      if (carouselSlide0ImageData) {
        carouselSlide0ImageStorageUrl = await uploadDataUrlToStorage(
          'carousel slide 0',
          `${basePath}/carousel-slide-0.png`,
          carouselSlide0ImageData,
          {
            contentType: 'image/png',
            cacheControl: pngCacheControl
      ...
      if (carouselSlide0bImageData) {
        carouselSlide0bImageStorageUrl = await uploadDataUrlToStorage(
          'carousel slide 0b',
          `${basePath}/carousel-slide-0b.png`,
          carouselSlide0bImageData,
          {
            contentType: 'image/png',
            cacheControl: pngCacheControl
      ...
      if (carouselSlide1ImageData) {
        carouselSlide1ImageStorageUrl = await uploadDataUrlToStorage(
          'carousel slide 1',
      ...
      if (carouselSlide2ImageData) {
        carouselSlide2ImageStorageUrl = await uploadDataUrlToStorage(
          'carousel slide 2',
      ...
      if (carouselSlide3ImageData) {
        carouselSlide3ImageStorageUrl = await uploadDataUrlToStorage(
          'carousel slide 3',
      ...
      if (carouselSlide4ImageData) {
        carouselSlide4ImageStorageUrl = await uploadDataUrlToStorage(
          'carousel slide 4',
          `${basePath}/carousel-slide-4.png`,
          carouselSlide4ImageData,
          {
            contentType: 'image/png',
            cacheControl: pngCacheControl
      ...
      if (storyLayoutBOverlayImageData) {
        storyLayoutBOverlayImageStorageUrl = await uploadDataUrlToStorage(
          'layout B story overlay',
          `${basePath}/layout-b-story-overlay.png`,
          storyLayoutBOverlayImageData,
          {
            contentType: 'image/png',
            cacheControl: pngCacheControl
      ...
      if (storyLayoutBPreviewImageData) {
        storyLayoutBPreviewImageStorageUrl = await uploadDataUrlToStorage(
          'layout B story preview',
          `${basePath}/layout-b-story-preview.jpg`,
          storyLayoutBPreviewImageData,
          {
      ...
      if (quiltScreen9x16ImageData) {
        quiltScreen9x16ImageStorageUrl = await uploadDataUrlToStorage(
          'quilt screen 9x16',
          `${basePath}/quilt-screen-9x16.jpg`,
          quiltScreen9x16ImageData,
          {
            contentType: 'image/jpeg',
            cacheControl: pngCacheControl
      ...
      if (reelWebmBlob && reelWebmBlob.size > 0) {
        ...
        reelWebmStorageUrl = await timedStep('reel WebM URL', getDownloadURL(r), 30000);
      }
      ...
      if (carouselSlide0ImageStorageUrl) {
        docPayload.carouselSlide0ImageStorageUrl = carouselSlide0ImageStorageUrl;
        docPayload.carouselSlide0Url = carouselSlide0ImageStorageUrl;
      }
      if (carouselSlide0bImageStorageUrl) {
        docPayload.carouselSlide0bImageStorageUrl = carouselSlide0bImageStorageUrl;
        docPayload.carouselSlide0bUrl = carouselSlide0bImageStorageUrl;
      }
      if (carouselSlide1ImageStorageUrl) {
        docPayload.carouselSlide1ImageStorageUrl = carouselSlide1ImageStorageUrl;
        docPayload.carouselSlide1Url = carouselSlide1ImageStorageUrl;
        docPayload.imageStorageUrl = carouselSlide1ImageStorageUrl;
        docPayload.classicUrl = carouselSlide1ImageStorageUrl;
        // Quote-clipping slide 1 may differ from strips Layout B (layout-b.png).
        if (!postLayoutBImageStorageUrl) {
          docPayload.postLayoutBImageStorageUrl = carouselSlide1ImageStorageUrl;
          docPayload.layoutBUrl = carouselSlide1ImageStorageUrl;
          docPayload.postLayoutBPlainImageStorageUrl = carouselSlide1ImageStorageUrl;
          docPayload.layoutBPlainUrl = carouselSlide1ImageStorageUrl;
        }
      }
      if (carouselSlide2ImageStorageUrl) {
        docPayload.carouselSlide2ImageStorageUrl = carouselSlide2ImageStorageUrl;
        docPayload.carouselSlide2Url = carouselSlide2ImageStorageUrl;
      }
      if (carouselSlide3ImageStorageUrl) {
        docPayload.carouselSlide3ImageStorageUrl = carouselSlide3ImageStorageUrl;
        docPayload.carouselSlide3Url = carouselSlide3ImageStorageUrl;
      }
      if (carouselSlide4ImageStorageUrl) {
        docPayload.carouselSlide4ImageStorageUrl = carouselSlide4ImageStorageUrl;
        docPayload.carouselSlide4Url = carouselSlide4ImageStorageUrl;
      }
      const carouselLeadUrl =
        carouselSlide0ImageStorageUrl || carouselSlide1ImageStorageUrl || imageStorageUrl;
      const carouselPostOrderUrls = [
        carouselLeadUrl,
        carouselSlide3ImageStorageUrl,
        carouselSlide4ImageStorageUrl,
        carouselSlide2ImageStorageUrl
      ].filter(Boolean);
      if (carouselPostOrderUrls.length) {
        docPayload.carouselPostOrderUrls = carouselPostOrderUrls;
      }
      const imageUrlList = [
        carouselSlide0ImageStorageUrl,
        carouselSlide0bImageStorageUrl,
        carouselSlide1ImageStorageUrl || imageStorageUrl,
        carouselSlide2ImageStorageUrl,
        carouselSlide3ImageStorageUrl,
        carouselSlide4ImageStorageUrl,
        postLayoutBImageStorageUrl &&
        postLayoutBImageStorageUrl !== carouselSlide1ImageStorageUrl
          ? postLayoutBImageStorageUrl
          : null,
        postLayoutBSpeakerImageStorageUrl,
        storyLayoutBImageStorageUrl,
        storyLayoutBOverlayImageStorageUrl,
        quiltScreen9x16ImageStorageUrl,
        contributorCloudImageStorageUrl
      ].filter(Boolean);
      if (imageUrlList.length) {
        docPayload.imageUrls = imageUrlList;
      }
      if (
        postLayoutBImageStorageUrl &&
        postLayoutBImageStorageUrl !== carouselSlide1ImageStorageUrl
      ...
      if (storyLayoutBImageStorageUrl) {
        ...
        docPayload.layoutBStoryUrl = this.cacheBustInstagramStorageUrl(storyLayoutBImageStorageUrl);
        docPayload.storyLayoutBUrl = docPayload.layoutBStoryUrl;
        docPayload.layoutBStoryRenderVersion = layoutBStoryRenderVersion();
      }
      if (storyLayoutBOverlayImageStorageUrl) {
        docPayload.storyLayoutBOverlayImageStorageUrl = storyLayoutBOverlayImageStorageUrl;
        docPayload.layoutBStoryOverlayUrl = this.cacheBustInstagramStorageUrl(storyLayoutBOverlayImageStorageUrl);
        docPayload.storyLayoutBOverlayUrl = docPayload.layoutBStoryOverlayUrl;
        docPayload.layoutBStoryOverlayRenderVersion = layoutBStoryRenderVersion();
      }
      if (storyLayoutBPreviewImageStorageUrl) {
        docPayload.storyLayoutBPreviewImageStorageUrl = storyLayoutBPreviewImageStorageUrl;
        docPayload.layoutBStoryPreviewUrl = this.cacheBustInstagramStorageUrl(
          storyLayoutBPreviewImageStorageUrl
        );
        docPayload.storyLayoutBPreviewUrl = docPayload.layoutBStoryPreviewUrl;
        docPayload.layoutBStoryPreviewRenderVersion =
          String(layoutBStoryPreviewRenderVersion || '').trim() || layoutBStoryRenderVersion();
      }
      if (quiltScreen9x16ImageStorageUrl) {
        docPayload.quiltScreen9x16ImageStorageUrl = quiltScreen9x16ImageStorageUrl;
        docPayload.quiltScreen9x16Url = this.cacheBustInstagramStorageUrl(quiltScreen9x16ImageStorageUrl);
        docPayload.quiltScreenUrl = docPayload.quiltScreen9x16Url;
        docPayload.quiltStoryImageStorageUrl = quiltScreen9x16ImageStorageUrl;
        docPayload.quiltStoryUrl = docPayload.quiltScreen9x16Url;
        docPayload.storyQuiltImageStorageUrl = quiltScreen9x16ImageStorageUrl;
        docPayload.storyQuiltUrl = docPayload.quiltScreen9x16Url;
      }
      if (newspaperClippingImageStorageUrl) {
        ...
        this.applyNewspaperClippingDisplayMetaToDoc(docPayload, newspaperClippingDisplayMeta);
      }
      ...
      if (markReadyForInstagram) {
        ...
        // Daytime quilt saves must not leave a stale readyForInstagram from an earlier nightly pass.
        docPayload.readyForInstagram = false;
        docPayload.igImagesSource = 'daytime_quilt_save';
      }
