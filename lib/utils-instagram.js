...
(function (root) {
  ...
  Object.assign(UtilsCore, {
    /** Fresh query params so phones/CDNs do not keep a flat legacy newspaper-clipping.png. */
    cacheBustInstagramStorageUrl(url, { rev = '' } = {}) {
      const u = String(url || '').trim();
      if (!u) return u;
      const sep = u.includes('?') ? '&' : '?';
      const r = String(
        rev || globalThis.QuiltNewspaperClipping?.CLIPPING_EXPORT_REV || '0'
      ).trim();
      return `${u}${sep}odq_rev=${encodeURIComponent(r)}&odq_t=${Date.now()}`;
    },
     ...
     * `quilt-screen-9x16.png` overwrite on each push.
     * When Layout B includes a speaker cutout, `layoutBSpeakerUrl` aliases the same `layout-b.png` URL
     * (no separate `layout-b-speaker.png` upload). Optional `debug-raw-quilt-{timestamp}-….png` stays versioned.
     */
    ...
    async writeInstagramImagesDocForZapier({
      ...
      const bustStorageDownloadUrl = (url, rev = '') => {
        const u = String(url || '').trim();
        if (!u) return u;
        const sep = u.includes('?') ? '&' : '?';
        const r = String(rev || globalThis.QuiltNewspaperClipping?.CLIPPING_EXPORT_REV || '0').trim();
        return `${u}${sep}odq_rev=${encodeURIComponent(r)}&odq_t=${Date.now()}`;
      };
      ...
      if (newspaperClippingImageStorageUrl) {
        ...
        docPayload.newspaperClippingUrl = bustStorageDownloadUrl(
