...
(function (root, factory) {
  ...
  function (global) {
    ...
    function packWordsIntoLines(words, lineCount) {
      ...
      /**
       * First-line rule (readability):
       * - Keep it lighter than body lines (cap at 3 words).
       * - Prefer exactly 3 when possible — 1–2 short words full-justify into
       *   huge gaps ("SO I" is hard to read; "SO I TRY" still feels airy).
       * - Body lines may stay denser.
       */
      const maxFirst = Math.max(1, words.length - (n - 1));
      const firstTake = Math.min(3, maxFirst);
