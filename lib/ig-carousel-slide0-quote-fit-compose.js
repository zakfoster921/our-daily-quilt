...
(function (root, factory) {
  ...
  function (global) {
    ...
    /** Story 9:16 alternate of slide 0 — same vinyl lettering, taller quilt frame. */
    function composeStoryQuoteFitFromQuiltBlob(quiltBlob, options = {}) {
      return composeCarouselSlide0QuoteFitFromQuiltBlob(quiltBlob, {
        ...options,
        panelWidth: Number(options.panelWidth) > 0 ? Number(options.panelWidth) : STORY_PANEL_W,
        panelHeight: Number(options.panelHeight) > 0 ? Number(options.panelHeight) : STORY_PANEL_H
      });
    }
