...
(function (root, factory) {
  ...
  function (global) {
    ...
    /**
     * Pack words into `lineCount` lines. First line is width-based (not word count):
     * add words until natural width sits in ~40–58% of maxW so full-justify
     * stays readable (not "SO I" gappy, not overcrowded). Body lines share the rest.
     */
    function packWordsWithLightFirstLine(ctx, words, lineCount, maxW, fontPx) {
      ...
      let firstTake = 0;
      let width = 0;
      while (firstTake < maxFirst) {
        const nextW = ctx.measureText(words[firstTake]).width;
        const newWidth = firstTake === 0 ? nextW : width + spaceW + nextW;
        const newFrac = newWidth / Math.max(1, maxW);
        if (
          firstTake > 0 &&
          width / Math.max(1, maxW) >= FIRST_MIN_FRAC &&
          newFrac > FIRST_MAX_FRAC
        ) {
          break;
        }
        if (firstTake > 0 && newFrac > 0.9) break;
        width = newWidth;
        firstTake += 1;
      }
      if (firstTake < 1) firstTake = 1;
      while (
        firstTake < maxFirst &&
        width / Math.max(1, maxW) < FIRST_MIN_FRAC
      ) {
        width += spaceW + ctx.measureText(words[firstTake]).width;
        firstTake += 1;
      }
    ...
    /** Even pack fallback when we lack a measure context. */
