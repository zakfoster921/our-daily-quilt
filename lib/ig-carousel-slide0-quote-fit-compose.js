...
(function (root, factory) {
  ...
  function (global) {
    ...
    /** Cap justify stretch — beyond this, keep natural spacing (no giant "IS····NOT"). */
    const MAX_JUSTIFY_GAP_EM = 0.42;
    ...
    function loadImageFromUrl(url) {
      return new Promise((resolve, reject) => {
        ...
        img.onerror = () => reject(new Error('slide0: image load failed'));
        img.src = src;
      });
    }
    ...
    function maxFontForLines(ctx, lines, maxW, maxH, options = {}) {
      ...
      const widthFirst = options.widthFirst === true;
      const leadEm =
      ...
      if (widthFirst) {
        /** Size by width; only shrink if the tight block won't fit the band height. */
        let lo = 8;
        let hi = 220;
        let best = lo;
        while (lo <= hi) {
        ...
        const blockH = best * leadEm * lines.length;
        if (blockH <= maxH + 0.5) return best;
        /** Long quote: shrink until height fits (width still enforced). */
        lo = 8;
        hi = best;
        best = 8;
        while (lo <= hi) {
          const mid = (lo + hi) >> 1;
          const fitsH = mid * leadEm * lines.length <= maxH + 0.5;
    ...
    function drawJustifiedVinylLine(ctx, line, x, y, width, fontPx, rng, options = {}) {
      const words = wordsOf(line);
      ...
      const maxGap =
        fontPx *
        (Number(options.maxJustifyGapEm) > 0
          ? Number(options.maxJustifyGapEm)
          : MAX_JUSTIFY_GAP_EM);
      let gap = 0;
      if (words.length > 1) {
        const justifyGap =
          (width - wordWidths.reduce((a, b) => a + b, 0)) / (words.length - 1);
        /** If full justify would look sparse, keep natural spacing (left-ragged). */
        gap = justifyGap > maxGap ? minGap : Math.max(minGap, justifyGap);
      }
      let cx = x;
    ...
    function drawQuoteFitInQuiltRect(ctx, quoteText, quiltRect, options = {}) {
      ...
      const fontPx = maxFontForLines(ctx, lines, textRect.width, textRect.height, {
        widthFirst: alignTop || options.widthFirst === true,
    ...
    async function composeStoryQuoteFitFromQuiltBlob(quiltBlob, options = {}) {
      ...
      const [quiltImg, speakerImg] = await Promise.all([
      ...
      const quoteTop = typeRect.y + Math.max(0, Math.round(quoteTopPad));
      const quoteBottom = speakerLayout?.previewRect
        ? Math.max(quoteTop + 120, Math.round(speakerLayout.previewRect.y - quoteGap))
        : quoteTop + Math.round(typeRect.height * 0.5);
      const quoteFrame = {
        x: typeRect.x,
        y: quoteTop,
