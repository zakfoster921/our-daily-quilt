...
(function (root, factory) {
  ...
  function (global) {
    ...
    const INK = '#1a1c20';
    const BEVEL_HI = 'rgba(255, 255, 255, 0.55)';
    const BEVEL_LO = 'rgba(0, 0, 0, 0.58)';
    ...
    /** Vinyl sticker placement jitter. */
    const JITTER_X_EM = 0.032;
    const JITTER_Y_EM = 0.042;
    const JITTER_ROT_DEG = 3.0;
    const KERN_JITTER_EM = 0.018;
    const BEVEL_EM = 0.028;
    const CONTACT_BLUR_EM = 0.05;
    const CONTACT_OX_EM = 0.018;
    const CONTACT_OY_EM = 0.03;
    const SOFT_EDGE_BLUR_PX = 0.55;
    ...
    function buildVinylGlyphCanvas(ch, fontPx, rng) {
      ...
      sheen.addColorStop(0.78, 'rgba(0,0,0,0.06)');
    ...
    function drawVinylChar(ctx, ch, x, y, fontPx, rng) {
      ...
      ctx.shadowBlur = Math.max(1.5, fontPx * CONTACT_BLUR_EM);
