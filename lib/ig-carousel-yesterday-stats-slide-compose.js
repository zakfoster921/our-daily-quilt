...
(function (root, factory) {
  ...
  function (global) {
    ...
    function clipToFrameRing(ctx, outerX, outerY, outerW, outerH, innerX, innerY, innerW, innerH) {
      ctx.beginPath();
    ...
    /** Paper tooth on the chunky quilt border only — not the inner text card. */
    function drawFrameBorderPaperTooth(
      ctx,
      frameX,
      frameY,
      frameW,
      ...
      ctx.save();
      clipToFrameRing(ctx, frameX, frameY, frameW, frameH, cardX, cardY, cardW, cardH);
      drawPaperToothLayers(
        ctx,
        { minX: frameX, minY: frameY, width: frameW, height: frameH },
        {
          paperImg,
          seed: `${seed}:frame-border`,
          toothOpacity: 0.58,
          pngOpacity: 0.3,
          fleckIntensity: 0.34
        }
