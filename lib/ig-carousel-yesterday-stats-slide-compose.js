...
(function (root, factory) {
  ...
  function (global) {
    ...
    function resolveScrapQuiltPan(scrap, panelW, panelH, dateKey) {
      if (Number.isFinite(scrap.quiltPanX) && Number.isFinite(scrap.quiltPanY)) {
        return { panX: scrap.quiltPanX, panY: scrap.quiltPanY };
      }
      const bounds = polygonBounds(scrap.points);
      const cx = bounds.minX + bounds.width / 2;
      const cy = bounds.minY + bounds.height / 2;
      const nx = cx / panelW - 0.5;
      const ny = cy / panelH - 0.5;
      const rng = createSeededRng(`${dateKey}:${scrap.name}:quilt-pan`);
      return {
        panX: -nx * panelW * 0.74 + (rng() - 0.5) * 180,
        panY: -ny * panelH * 0.74 + (rng() - 0.5) * 180
      };
    }
      ...
      return [
        {
          ...
          points: scrapPoly(rng, jitter, [
            [-42, 12, panelH * 0.908, 22],
            [88, 10, panelH * 0.882, 20],
            [132, 8, panelH * 0.962, 18],
            [76, 12, panelH * 1.038, 20],
            [-34, 14, panelH * 0.948, 24]
          ])
        },
        {
          ...
          quiltPanY: -380,
          points: scrapPoly(rng, jitter, [
            [panelW * 0.928, 18, -38, 24],
            [panelW * 1.048, 14, -24, 20],
            [panelW * 1.018, 16, panelH * 0.082, 22],
            [panelW * 0.892, 20, panelH * 0.058, 26]
          ])
        },
        {
          name: 'top-left-strip',
    ...
    function drawCollageScraps(ctx, quiltImg, quiltRect, panelW, panelH, dateKey, paperTextureImg) {
      ...
      for (const scrap of scraps) {
        ...
        drawQuiltScrap(
          ...
          paperTextureImg,
          `${dateKey}:${scrap.name}`,
          resolveScrapQuiltPan(scrap, panelW, panelH, dateKey)
        );
      }
      ctx.restore();
