...
(function (root, factory) {
  ...
  function (global) {
    ...
    function applyPaperToothToCardCanvas(sourceCanvas, seed, paperImg) {
      ...
      const tooth = buildPaperToothTile(`${seed}:card-tooth`);
      ctx.globalCompositeOperation = 'overlay';
      ...
      drawPaperFlecksOnRect(ctx, w, h, `${seed}:card-flecks`, 0.42);
      ctx.restore();
    ...
    function drawCenterCluster(ctx, cardsLayer, quiltImg, quiltRect, panelW, panelH, paperTextureImg, dateKey) {
      ...
      const cardCanvas = applyPaperToothToCardCanvas(
        scaled.canvas,
        `${dateKey || 'yesterday-stats'}:card`,
        paperTextureImg
      );
      ctx.drawImage(cardCanvas, cardX, cardY, scaled.drawW, scaled.drawH);
