...
(function (root, factory) {
  ...
  function (global) {
    ...
    /** Daily tilt range for the frame + card cluster (single unit). */
    const CLUSTER_TILT_MIN_DEG = -11;
    const CLUSTER_TILT_MAX_DEG = 11;
    const CUTTING_MAT_ASSETS =
      (global.CarouselCuttingMatBg || globalThis.CarouselCuttingMatBg)?.CUTTING_MAT_ASSETS || [
    ...
    function getCarouselMatApi() {
      return global.CarouselCuttingMatBg || globalThis.CarouselCuttingMatBg || null;
    }

    async function loadCuttingMatImage() {
      const shared = getCarouselMatApi();
      if (shared?.loadCarouselCuttingMatImage) {
        return shared.loadCarouselCuttingMatImage();
      }
    ...
    function drawCuttingMatBackground(ctx, matImg, panelW, panelH) {
      const shared = getCarouselMatApi();
      const panX = shared?.CAROUSEL_MAT_PAN?.SLIDE_2 ?? PANEL_W;
      if (shared?.drawCarouselCuttingMatBackground) {
        shared.drawCarouselCuttingMatBackground(ctx, matImg, panelW, panelH, panX);
        return;
      }
      if (matImg) {
      ...
      shared?.drawProceduralCuttingMat?.(ctx, panelW, panelH, panX);
    }
    ...
    function resolveDailyClusterTiltDeg(dateKey) {
      const rng = createSeededRng(`${dateKey}:yesterday-stats-cluster-tilt`);
      let deg = CLUSTER_TILT_MIN_DEG + rng() * (CLUSTER_TILT_MAX_DEG - CLUSTER_TILT_MIN_DEG);
      if (Math.abs(deg) < 3.5) {
        deg = (deg >= 0 ? 1 : -1) * (3.5 + rng() * 5.5);
      }
      return deg;
    }

    function computeClusterProtectedBounds(panelW, panelH, cardsLayer, tiltDeg) {
    ...
    function protectedBoundsFromLayout(x, y, width, height, pad = 20) {
    ...
    function buildScrapLayouts(panelW, panelH, dateKey, protectedBounds = null, seedTag = 'yesterday-stats-scraps') {
      const tag = String(seedTag || 'yesterday-stats-scraps').trim() || 'yesterday-stats-scraps';
      const rng = createSeededRng(`${dateKey}:${tag}`);
      const placementRng = createSeededRng(`${dateKey}:${tag}-placement`);
    ...
    /** Edge-bleed quilt scraps under carousel center content (slides 2–4). */
    function drawCarouselCutoffScraps(
      ctx,
      quiltImg,
      quiltRect,
      panelW,
      ...
      const seedTag = opts.seedTag || 'yesterday-stats-scraps';
      const scraps = buildScrapLayouts(panelW, panelH, dateKey, protectedBounds, seedTag);
    ...
    function drawCenterCluster(ctx, cardsLayer, quiltImg, quiltRect, panelW, panelH, paperTextureImg, dateKey) {
      ...
      const centerX = panelW / 2;
      const centerY = panelH * 0.5;
      const tiltDeg = resolveDailyClusterTiltDeg(dateKey || 'yesterday-stats');
      const tiltRad = (tiltDeg * Math.PI) / 180;
    ...
    async function composeCarouselYesterdayStatsSlideFromQuiltBlob(quiltBlob, options = {}) {
      ...
      const clusterTiltDeg = resolveDailyClusterTiltDeg(dateKey);
      const protectedBounds = computeClusterProtectedBounds(
        PANEL_W,
        PANEL_H,
        cardsLayer,
        clusterTiltDeg
    ...
    return {
      ...
      buildScrapLayouts,
      drawCarouselCutoffScraps,
      protectedBoundsFromLayout,
      scrapRecordFromScrap,
      PANEL_W,
      PANEL_H,
      CUTTING_MAT_ASSETS
    };
