...
(function (root, factory) {
  ...
  function (global) {
    ...
    function drawPaperFlecksInClip(ctx, points, seedStr, intensity = 0.2) {
      const bounds = polygonBounds(points);
      if (!bounds.width || !bounds.height) return;
      const rng = createSeededRng(`${seedStr}:paper-flecks`);
      const count = Math.max(10, Math.floor((bounds.width * bounds.height) / 3800));
      ctx.save();
      ctx.globalCompositeOperation = 'multiply';
      for (let i = 0; i < count; i += 1) {
        const x = bounds.minX + rng() * bounds.width;
        const y = bounds.minY + rng() * bounds.height;
        const r = 0.35 + rng() * 1.6;
        ctx.globalAlpha = intensity * (0.4 + rng() * 0.6);
        ctx.fillStyle = rng() < 0.18 ? 'rgba(38, 32, 28, 0.58)' : 'rgba(52, 44, 38, 0.34)';
        ctx.beginPath();
    ...
    function drawPaperTextureInClip(ctx, points, paperImg, options = {}) {
      if (!paperImg?.naturalWidth && !paperImg?.width) return;
      const bounds = polygonBounds(points);
      const pad = 6;
      const intensity = Number(options.intensity) || 0.32;
      const blendMode = options.blendMode || 'soft-light';

      ctx.save();
      ctx.globalCompositeOperation = blendMode;
      ctx.globalAlpha = intensity;
      const pattern = ctx.createPattern(paperImg, 'repeat');
      if (pattern) {
        ...
        ctx.drawImage(
          paperImg,
          bounds.minX + (bounds.width - sw) / 2,
          bounds.minY + (bounds.height - sh) / 2,
          sw,
          sh
        );
      }
      ...
      if (options.flecks !== false) {
        drawPaperFlecksInClip(
          ctx,
          points,
          options.seed || 'paper-texture',
          Number(options.fleckIntensity) || 0.18
        );
      }
    }
    ...
    function drawPaperPieceWithDepth(ctx, points, rotateDeg, drawContent, depthOptions = {}) {
      withPolygonTransform(ctx, points, rotateDeg, () => {
        ...
        if (paperTexture?.img) {
          drawPaperTextureInClip(ctx, points, paperTexture.img, paperTexture);
        }
        ctx.restore();
    ...
    function drawCenterCluster(ctx, cardsLayer, quiltImg, quiltRect, panelW, panelH, paperTextureImg) {
