...
(function (root, factory) {
  ...
  function (global) {
    ...
    function drawPaperEdgeHighlights(ctx, points, options = {}) {
      const highlightColor = options.highlightColor || 'rgba(255, 255, 255, 0.62)';
      const highlightWidth = Number(options.highlightWidth) || 2.4;
      const shadowEdgeColor = options.shadowEdgeColor || 'rgba(0, 0, 0, 0.16)';
      const litThreshold = Number(options.litThreshold) || 0.28;
    ...
    function drawQuiltScrap(ctx, quiltImg, quiltRect, points, panelW, panelH, rotateDeg = 0, depthScale = 1) {
      ...
      drawPaperPieceWithDepth(
        ...
        {
          offsetX: 4 * depthScale,
          offsetY: 6 * depthScale,
          blur: 12 * depthScale,
          alpha: 0.3,
          secondaryAlpha: 0.12,
          highlightWidth: 2 * depthScale
        }
      );
    }
