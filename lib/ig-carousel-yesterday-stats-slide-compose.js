...
(function (root, factory) {
  ...
  function (global) {
    ...
    /** Daily tilt range for the quilt frame; card counter-rotates to stay upright. */
    ...
    function drawCuttingMatBackground(ctx, matImg, panelW, panelH) {
      const shared = getCarouselMatApi();
      const panX = shared?.CAROUSEL_MAT_PAN?.SLIDE_2 ?? 0;
    ...
    function computeClusterProtectedBounds(panelW, panelH, cardsLayer, tiltDeg) {
      ...
      const cardBounds = {
        minX: centerX - scaled.drawW / 2 - pad,
        maxX: centerX + scaled.drawW / 2 + pad,
        minY: centerY - scaled.drawH / 2 - pad,
        maxY: centerY + scaled.drawH / 2 + pad
      };
      return {
        minX: Math.min(frameBounds.minX, cardBounds.minX),
        maxX: Math.max(frameBounds.maxX, cardBounds.maxX),
        minY: Math.min(frameBounds.minY, cardBounds.minY),
        maxY: Math.max(frameBounds.maxY, cardBounds.maxY)
      };
    }
    ...
    function drawCenterCluster(ctx, cardsLayer, quiltImg, quiltRect, panelW, panelH, paperTextureImg, dateKey) {
      ...
      const cardCx = cardX + scaled.drawW / 2;
      const cardCy = cardY + scaled.drawH / 2;
      ctx.save();
