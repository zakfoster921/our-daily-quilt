...
(function (root) {
  ...
  function odqLayoutBIgStripsSkipKeywordEmphasis() {
    ...
    /** True when a saved strip plan includes quote line breaks (not just Firestore x/y/angle). */
    function odqLayoutBStripPlanHasTypography(plan) {
      return (
        Array.isArray(plan) &&
        plan.some((s) => Array.isArray(s.lines) && s.lines.length > 0)
      );
    }
    ...
    /** Carousel slide 1: keep quilt-aligned speaker on-screen after manual nudge. */
    function odqClampCarouselSlide1SpeakerRect(rect, layoutW, layoutH, quiltRect) {
      ...
      const oldY = y;
      const quiltLeft =
        quiltRect && Number.isFinite(Number(quiltRect.x))
          ? Math.max(0, Math.round(Number(quiltRect.x)))
          : 0;
      if (Number.isFinite(Number(rect.contentX))) {
        const contentLeft = Number(rect.contentX);
        if (contentLeft < quiltLeft) {
          x += quiltLeft - contentLeft;
        }
      } else if (x < quiltLeft) {
        x = quiltLeft;
      }
      x = Math.max(quiltLeft, Math.min(x, layoutW - Math.round(w * 0.15)));
    ...
    /** Positions only — nightly/tune replay supplies quote text from the day’s quote. */
    function odqCompactLayoutBPostStripPlanForFirestore(stripPlan) {
      ...
      return stripPlan.map((s) => {
        ...
        if (s.angle != null && Number.isFinite(Number(s.angle))) row.angle = Number(s.angle);
        return row;
      });
    }
