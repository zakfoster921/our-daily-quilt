...
(function (root) {
  ...
  if (!CONFIG || typeof CONFIG !== 'object') {
    ...
    /** Carousel slide 1: translate the whole strip cluster left — no per-strip relayout. */
            ...
            loadQuiltImage()
              .then(async (quiltImg) => {
                ...
                const finishLayoutBCanvas = () => {
                  ...
                  const composeMeta = composedSpeakerRectForMeta
                    ? { composedSpeakerRect: composedSpeakerRectForMeta }
                    : null;
                  if (typeof options.onComposeMeta === 'function') {
                    options.onComposeMeta(composeMeta);
                  }
                  globalThis._lastLayoutBComposeMeta = composeMeta;
                  const exportMime =
                  ...
                  if (speakerDrawn) {
                    ...
                    try {
                      console.log(
                        ...
                        JSON.stringify({
                          speakerDrawn: true,
                          carouselStoryStyle: !!carouselStoryStyle,
                          isPostLayout: !!isPostLayout,
                          skipSpeakerDrawOnPost: !!skipSpeakerDrawOnPost,
