...
(function (root) {
  ...
  root.ODQ_LAYOUT_B_STORY_RENDER_VERSION = ODQ_LAYOUT_B_STORY_RENDER_VERSION;
    ...
    const ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_MAX = 32;
    const ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_MIN = 6;
    ...
    function odqCarouselStoryStyleStackStripsByReadingOrder(strips, quoteText, layoutW, layoutH) {
      ...
      const topPad = 12;
      const bottomPad = 8;
      const avail = Math.max(40, safe.bottom - safe.top - topPad - bottomPad);
      const totalStripH = ordered.reduce((sum, s) => sum + Number(s.h || 0), 0);
      ...
      for (let i = 0; i < ordered.length - 1; i++) {
        ...
        } else if (cur.role !== 'author' && next.role !== 'author') {
          quoteGapCount += 1;
        }
      }
      let quoteGap =
        quoteGapCount > 0
          ? (avail - totalStripH - authorGapTotal) / quoteGapCount
          : ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_MAX;
      quoteGap = Math.max(
        ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_MIN,
        Math.min(ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_MAX, quoteGap)
      );
      const rowGapForPair = (cur, next, gap) => {
        if (!next) return 0;
        if (cur.role !== 'author' && next.role === 'author') {
          return ODQ_CAROUSEL_STORY_STYLE_AUTHOR_ROW_GAP;
        }
        if (cur.role !== 'author' && next.role !== 'author') return gap;
        return gap;
      };
      const stackBottomForGap = (gap) => {
        ...
        for (let i = 0; i < ordered.length; i++) {
          ...
          if (i < ordered.length - 1) {
            cursorTop += rowGapForPair(s, ordered[i + 1], gap);
          }
        }
        return cursorTop;
      };
      while (
        quoteGapCount > 0 &&
        stackBottomForGap(quoteGap) > safe.bottom - bottomPad &&
        quoteGap > ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_MIN
      ) {
        quoteGap = Math.max(ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_MIN, quoteGap - 1);
      }
      let cursorTop = safe.top + topPad;
      for (let i = 0; i < ordered.length; i++) {
        const s = ordered[i];
        ...
        s.y = cursorTop + h / 2;
        if (i < ordered.length - 1) {
          cursorTop = s.y + h / 2 + rowGapForPair(s, ordered[i + 1], quoteGap);
        }
      }
