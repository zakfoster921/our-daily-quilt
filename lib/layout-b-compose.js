...
(function (root) {
  ...
  function odqLayoutBIgStripsSkipKeywordEmphasis() {
    ...
    function odqCarouselStoryStyleAdaptiveRowGap(rowHeights, safeSpan, reserveBottom = 0) {
      ...
      gap = Math.max(
        ...
        Math.min(ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_MAX, gap)
      );
      const stackHeight = (g) => totalH + g * gapCount;
      while (
        gapCount > 0 &&
        stackHeight(gap) > usable &&
        gap > ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_MIN
      ) {
        gap = Math.max(ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_MIN, gap - 1);
      }
