...
(function (root) {
  ...
  if (!CONFIG || typeof CONFIG !== 'object') {
    ...
    /** Story 9:16: zoom past cover so warm matte in the quilt source raster does not peek at edges. */
    const ODQ_LAYOUT_B_STORY_QUILT_BLEED = 1.588;
    ...
    function getLayoutBStoryQuotePlan(ctx, options) {
      ...
      const buildStripSpecs = () => {
        ...
        const measureStoryUnitPaperW = (unit, font) => {
          ...
          ctx.font = font;
          odqSetLayoutBStripLetterSpacing(ctx, quoteFontPx());
          return Math.min(STRIP_MAX_W, Math.ceil(ctx.measureText(txt).width + PAPER_PAD_X * 2 + 6));
        };
          ...
          while (ui < list.length) {
            ...
            if (row.length >= ODQ_LAYOUT_B_STORY_ROW_MIN_STRIPS) {
              ...
              row.forEach((unit, slot) => {
                ...
                if (unit.emphasis) measureEmphasisStrip(unit.text, unit.rank, meta);
                else measureStrip([unit.text], font, 1.38, 'quote', meta);
              });
            } else {
              const unit = row[0];
              if (unit.emphasis) measureEmphasisStrip(unit.text, unit.rank);
              else measureStrip([unit.text], font, 1.38, 'quote');
            }
          }
        };
            ...
            for (const part of fullParts) {
              ...
              if (part.kind === 'inline-emphasis') {
                for (const seg of segmentQuoteForLayout(chunkText)) {
                  for (const micro of odqMicroSplitSegmentForStripCollage(seg, rnd, microSplitOpts)) {
                    const lines = wrapLines(ctx, micro, innerMaxW);
                    for (const ln of lines) storyUnits.push({ emphasis: false, text: ln });
                  }
                }
                continue;
              }
              storyUnits.push(
                ...flattenSegmentsToStoryTextPieces(segmentQuoteForLayout(chunkText)).map((text) => ({
                  emphasis: false,
                  text
                }))
              );
      ...
      for (let i = 0; i < stripSpecs.length; ) {
        ...
        else cx = STORY_W / 2 + (rnd() - 0.5) * (STORY_W * 0.14);
            ...
            const drawSpeakerOverlay = (g, img, rect, washColor, opts = {}) => {
              ...
              return SCR.drawSpeakerCutoutStack(g, img, rect, {
                ...
                palette:
                  ...
                  [],
                drawScannerBed: QNC?.drawScannerBed
                  ? (targetCtx, dx, dy, pw, ph, portrait) => {
