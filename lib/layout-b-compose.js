...
(function (root) {
  ...
  if (!CONFIG || typeof CONFIG !== 'object') {
    ...
    /** Undo 9:16→4:5 font shrink on inherited story strip plans (quote-only post; then refit + safe spread). */
    ...
    /**
     * After story→post scale: safe-zone spread; quote-only posts also restore type size (positions unchanged).
     */
    function odqFinishInheritedStoryToPostStripPlan(
      ...
      let strips = odqApplyPostQuoteStripVerticalSpread(plan, layoutW, layoutH);
      if (!quoteOnlyTypeBoost || !strips.length) return strips;
