...
(function (root) {
  ...
  const ODQ_LAYOUT_B_STORY_RENDER_VERSION = 'story-strip-keyword-order-v4';
  root.ODQ_LAYOUT_B_STORY_RENDER_VERSION = ODQ_LAYOUT_B_STORY_RENDER_VERSION;
    ...
    /** Repeated single-word keyword strips stay attached to the preceding phrase (right or below). */
    function odqAttachRepeatedKeywordStripsToStoryPhrases(plan, layoutW, edgePad = 44) {
      const strips = Array.isArray(plan) ? plan : [];
      if (!strips.length) return strips;
      const w = Math.max(1, Number(layoutW) || 1080);
      const pad = Math.max(12, Number(edgePad) || 44);
      const safePad = ODQ_CAROUSEL_STRIP_TEXT_SAFE_PAD;
      const gap = Math.max(ODQ_CAROUSEL_STRIP_TEXT_MIN_GAP, 20);
      for (let i = 0; i < strips.length; i++) {
        const spec = strips[i];
        if (!spec?.isKeywordEmphasis || spec.role !== 'quote') continue;
        let prev = null;
        for (let j = i - 1; j >= 0; j--) {
          const candidate = strips[j];
          if (candidate && candidate.role === 'quote' && !candidate.isKeywordEmphasis) {
            prev = candidate;
            break;
          }
        }
        if (!prev) continue;
        const specW = Math.max(1, Number(spec.w) || 0);
        const specH = Math.max(1, Number(spec.h) || 0);
        const prevR = odqStripPlanAxisRect(prev, safePad);
        let x = prevR.right + gap + specW / 2;
        let y = Number(prev.y) || 0;
        if (x + specW / 2 > w - pad) {
          y = prevR.bottom + gap + specH / 2;
          x = Math.max(prevR.left + specW / 2, Number(prev.x) || prevR.left + specW / 2);
        }
        y = Math.max(y, Number(prev.y) || 0);
        spec.y = y;
        spec.x = Math.max(pad + specW / 2, Math.min(w - pad - specW / 2, x));
      }
      return strips;
    }

    /** Story: re-anchor keyword strips and enforce LTR / row-stack gaps after clamp/overlap passes. */
    function odqFinalizeStoryQuoteStripReadingOrder(plan, layoutW, edgePad = 44, minGap = 12) {
      let strips = odqAttachRepeatedKeywordStripsToStoryPhrases(plan, layoutW, edgePad);
      strips = odqCarouselSpeakerEnforceReadingOrderStripGaps(strips, minGap);
      return strips;
    }
            ...
            loadQuiltImage()
              .then(async (quiltImg) => {
                ...
                if (!isPostLayout && quoteStripPlan.length) {
                  ...
                  quoteStripPlan = odqCarouselSpeakerEnforceReadingOrderStripGaps(quoteStripPlan, 12);
                  quoteStripPlan = odqClampLayoutBStoryStripPlanToCanvas(
                    quoteStripPlan,
                    STORY_W,
                    STORY_H
                  );
                  quoteStripPlan = odqFinalizeStoryQuoteStripReadingOrder(
                    quoteStripPlan,
                    STORY_W
                  );
                }
