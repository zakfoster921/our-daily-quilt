...
(function (root) {
  ...
  if (!CONFIG || typeof CONFIG !== 'object') {
    ...
    function getLayoutBStoryQuotePlan(ctx, options) {
      ...
      const buildStripSpecs = () => {
        ...
        const emitQuoteLine = (ln, bodyFont, lineSalt) => {
          ...
          const pair =
            stripBounds && isPostLayout ? null : tryPairQuoteLine(ln, bodyFont, lineSalt);
          if (pair) {
      ...
      const postQuoteStripCount = postReadableStripStack
        ? stripSpecs.filter((s) => s.role !== 'author').length
        : 0;
      let postQuoteStripIdx = 0;
      ...
      const storyLikeStripCenterX = (spec, pseudoWidth) => {
        ...
        if (postReadableStripStack && spec.role === 'author') {
          const minCx = w / 2 + hBounds.left + 8;
          const maxCx = hBounds.right - w / 2 - 8;
          const centers = odqPostStripReadingBandCenters(STORY_W, STORY_H, postQuoteStripCount);
    ...
    function odqApplyPostStripReadingBands(plan, layoutW, layoutH) {
      ...
      if (!n) return strips;
      const bandCenters = odqPostStripReadingBandCenters(layoutW, layoutH, n);
      const innerSpan = layout ? Math.max(1, layout.right - layout.left) : 400;
      let prevCx = bandCenters[0];
      for (let i = 0; i < n; i++) {
        const s = quotes[i];
        const targetCx = odqPostStripReadingBandTargetCx(i, n, prevCx, layoutW, layoutH);
        const half = Number(s.w || 20) / 2 + 10;
        let cx =
          targetCx + odqDeterministicStripJitterPx(s, ODQ_LAYOUT_B_POST_READING_BAND_JITTER_PX);
        if (safe) {
          cx = Math.max(safe.x + half, Math.min(safe.x + safe.width - half, cx));
        }
        s.x = cx;
        prevCx = targetCx;
      }
      for (const a of authors) {
        const half = Number(a.w || 20) / 2 + 10;
