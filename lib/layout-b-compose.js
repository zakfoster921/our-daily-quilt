...
(function (root) {
  ...
  function odqLayoutBIgStripsSkipKeywordEmphasis() {
    ...
    function odqFlowLayoutCarouselQuoteStrips(plan, quoteText, layoutW, layoutH, opts = {}) {
      ...
      // Staggered jitter: fewer strips → more scatter/tilt (collage feel with room to spare);
      // more strips → jitter tapers toward 0 so dense long quotes stay legible and compact.
      // x/y jitter is capped well under half the row/strip gap so a strip can never cross
      // into reading-order violation; rotation is purely cosmetic and always safe to vary.
      const jitterT = Math.max(0, Math.min(1, 1 - (ordered.length - 6) / 18));
              ...
              .then(async (quiltImg) => {
                ...
                if (
                  ...
                  if (authorStrips.length && quoteStrips.length) {
                    ...
                    const orderedQuotes = odqSortCarouselQuoteStripsByTextOrder(
                      quoteStrips.filter((s) => quotePos(s) !== Number.MAX_SAFE_INTEGER),
                      layoutQuoteText
                    ).concat(quoteStrips.filter((s) => quotePos(s) === Number.MAX_SAFE_INTEGER));
                    ...
                    const rowHeights9 = rows9.map((row) =>
                      Math.max(...row.map((s) => Number(s.h || 0)))
                    );
                    const authorReserve9 = authorStrips.length
                      ? Math.max(...authorStrips.map((a) => Number(a.h || 0))) +
                        odqCarouselStoryStyleAuthorRowGap(
                          carouselTightVerticalStack
                            ? ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_MIN
                            : ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_COMFORTABLE
                        )
                      : 0;
                    const { gap: rowGap9, startOffset: startOffset9 } = odqCarouselStoryStyleAdaptiveRowGap(
                      rowHeights9,
                      safe9 ? safe9.bottom - safe9.top : 1320,
                      authorReserve9,
                      { tightStack: carouselTightVerticalStack }
                    );
                    let cursor = safe9 ? safe9.top + startOffset9 : 30;
                    const NR = rows9.length;
                    for (let ri2 = 0; ri2 < NR; ri2++) {
                      const row = rows9[ri2];
                      const rowH = rowHeights9[ri2];
                      const rowY = cursor + rowH / 2;
                      if (ri2 < NR - 1) cursor = rowY + rowH / 2 + rowGap9;
                      else cursor = rowY + rowH / 2;
                      if (row.length === 1) {
                        const q = row[0];
                        const w = Number(q.w || 300);
                    ...
                    cursor += odqCarouselStoryStyleAuthorRowGap(rowGap9);
                    const quoteLeft = Math.min(
                      ...orderedQuotes
                        .map((q) => Number(q.x || 0) - Number(q.w || 0) / 2)
                    ...
                    for (const a of authorStrips) {
                      ...
                      const targetLeft = Number.isFinite(quoteLeft) ? quoteLeft : sL + 20;
                      const left = Math.max(sL + 20, Math.min(sR - aw - 20, targetLeft));
                      a.y = cursor + h / 2;
                      a.x = Math.round(left + aw / 2);
                      cursor = a.y + h / 2 + 8;
                    }
                  }
                }
                if (!useSavedPostStripPlan && carouselStoryStyle && isPostLayout && quoteStripPlan.length) {
                ...
                if (!useSavedPostStripPlan && (quoteStripDx || quoteStripDy)) {
