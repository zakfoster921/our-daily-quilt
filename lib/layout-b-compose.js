  ...
  if (!CONFIG || typeof CONFIG !== 'object') {
    ...
    function getLayoutBStoryQuotePlan(ctx, options) {
      ...
      const buildStripSpecs = () => {
        ...
        const measureStrip = (lines, font, lineLead, role, rowMeta = null) => {
          ...
          const o = { lines: cleanLines, font, lh, w, h, role };
        ...
        const emitQuoteSegmentsForLayout = (segments) => {
          const segs = (Array.isArray(segments) ? segments : [])
            ...
            .filter(Boolean);
          if (!isPostLayout) {
            emitStoryQuoteTextPiecesInRows(flattenSegmentsToStoryTextPieces(segs), bodyFont);
            return;
          }
      ...
      for (let s = 0; s < shrinkIters; s++) {
        ...
        if ((stackH <= stripBudget && textOverflow <= 0) || bodyPx <= minBodyPx) break;
        bodyPx -= 2;
      }
      ...
      for (let i = 0; i < stripSpecs.length; ) {
        ...
        if (isPostLayout) {
          ...
          cx = STORY_W / 2 + (rnd() - 0.5) * (STORY_W * 0.14);
          const minCx = spec.w / 2 + stripEdgeMargin;
          const maxCx = STORY_W - spec.w / 2 - stripEdgeMargin;
          cx = Math.max(minCx, Math.min(maxCx, cx));
        }
    ...
    function odqLayoutBStripPlanFitsPostSpreadBounds(plan, layoutW, layoutH, opts = {}) {
      ...
      if (opts.carouselSpeakerAvoidRect) {
        const laid = odqApplyCarouselSpeakerStripFlowLayout(
        ...
        return maxBottom <= bounds.boundsBottom + 1;
      }
    ...
    function odqApplyPostQuoteStripVerticalSpread(plan, layoutW, layoutH, opts = {}) {
      ...
      let out = opts.carouselSpeakerAvoidRect
      ...
      if (out.length > 1 && !opts.carouselSpeakerAvoidRect) {
    ...
    function odqCarouselStripPlanLayoutOk(plan, speakerRect, minGap = 10) {
      if (odqStripPlanHasOverlaps(plan, minGap)) return false;
    ...
    function odqCarouselSpeakerExpandedRect(
      ...
      const pad =
        typeof padSpec === 'number'
          ...
          : padSpec;
      return {
        left: content.x - pad.left,
        top: content.y - pad.top,
        right: content.x + content.width + pad.right,
        bottom: content.y + content.height + pad.bottom
      };
    }
    ...
    function odqApplyCarouselSpeakerStripFlowLayout(plan, layoutW, layoutH, opts = {}) {
      ...
      const stripLayoutSeed = odqNormalizeStripLayoutSeed(opts.stripLayoutSeed);
      const quoteStrips = strips.filter((s) => s.role !== 'author');
      const authors = strips.filter((s) => s.role === 'author');
      const quoteRows = odqGroupLayoutBStripPlanRowsByReadingOrder(quoteStrips);
      const speakerPad = ODQ_CAROUSEL_SPEAKER_AVOID_PAD;
      const shelves = odqCarouselSpeakerLayoutShelves(bounds, speakerRect, speakerPad);
      const hGapBase = 14;
      let shelfIndex = 0;
      let rowIndex = 0;
      let shelf = shelves[0];
      let rowTop = shelf.top;
      let rowBottom = rowTop;
      let cursorX = bounds.left;

      const rowStartX = (ri, sh) => {
        const hash = odqPostStripStaggerHash(layoutSeed, stripLayoutSeed, ri, 'carousel_row');
        const roll = odqPostStripStaggerRoll(hash, 3);
        const span = Math.max(1, sh.maxRight - bounds.left);
        if (sh.fullWidth) {
          return bounds.left + roll * span * 0.52;
        }
        return bounds.left + roll * Math.min(88, span * 0.22);
      };

      const rowGapFor = (ri) => {
        const hash = odqPostStripStaggerHash(layoutSeed, stripLayoutSeed, ri, 'carousel_gap');
        return 10 + Math.round(odqPostStripStaggerRoll(hash, 9) * 16);
      };

      const hGapFor = (ri, spec) => {
        const hash = odqPostStripStaggerHash(
          layoutSeed,
          stripLayoutSeed,
          ri,
          spec?.seed || 'carousel_hgap'
        );
        return hGapBase + Math.round(odqPostStripStaggerRoll(hash, 11) * 12);
      };

      const advanceShelf = (rowH) => {
        if (shelfIndex >= shelves.length - 1) return false;
        shelfIndex += 1;
        shelf = shelves[shelfIndex];
        rowIndex += 1;
        rowTop = shelf.top;
        rowBottom = rowTop;
        cursorX = rowStartX(rowIndex, shelf);
        return true;
      };

      const wrapRow = (rowH) => {
        const nextTop = rowBottom + rowGapFor(rowIndex + 1);
        if (nextTop + rowH > shelf.bottom) {
          if (advanceShelf(rowH)) return rowTop + rowH / 2;
          rowTop = Math.max(shelf.top, shelf.bottom - rowH);
          rowBottom = rowTop;
          cursorX = rowStartX(++rowIndex, shelf);
      ...
      const bumpRowBottom = (spec, centerY) => {
        rowBottom = Math.max(rowBottom, centerY + odqStripPlanFootprintH(spec, 8) / 2);
      };

      cursorX = rowStartX(0, shelf);
      ...
      for (let r = 0; r < quoteRows.length; r++) {
        ...
        if (isPair) {
          ...
          b.angle =
            odqPostStripStaggerTiltRad(r, b, layoutSeed, stripLayoutSeed) * -0.85 +
            odqDeterministicStripJitterPx(b, 0.012);
          const pairGap = 18 + Math.round(Math.abs(odqDeterministicStripJitterPx(a, 8)));
          const wA = odqStripPlanFootprintW(a, 8);
          const wB = odqStripPlanFootprintW(b, 8);
          const totalW = wA + pairGap + wB;
          const rowH = Math.max(odqStripPlanFootprintH(a, 8), odqStripPlanFootprintH(b, 8));
          let centerY = rowTop + rowH / 2;
          let maxRight = shelf.maxRight;
          if (centerY + rowH / 2 > shelf.bottom && !advanceShelf(rowH)) {
            centerY = Math.min(shelf.bottom - rowH / 2, rowTop + rowH / 2);
          }
          if (cursorX + totalW > maxRight && cursorX > bounds.left + 4) {
            centerY = wrapRow(rowH);
            maxRight = shelf.maxRight;
          }
          a.x = cursorX + wA / 2;
          a.y = centerY;
          b.x = cursorX + wA + pairGap + wB / 2;
          b.y = centerY;
          bumpRowBottom(a, centerY);
          bumpRowBottom(b, centerY);
          cursorX += totalW + hGapFor(r, a);
        } else {
          const s = row[0];
          s.angle = odqPostStripStaggerTiltRad(r, s, layoutSeed, stripLayoutSeed);
          const fw = odqStripPlanFootprintW(s, 8);
          const fh = odqStripPlanFootprintH(s, 8);
          let centerY = rowTop + fh / 2;
          let maxRight = shelf.maxRight;
          if (centerY + fh / 2 > shelf.bottom && !advanceShelf(fh)) {
            centerY = Math.min(shelf.bottom - fh / 2, rowTop + fh / 2);
          }
          if (cursorX + fw > maxRight && cursorX > bounds.left + 4) {
            centerY = wrapRow(fh);
            maxRight = shelf.maxRight;
          }
          s.x = cursorX + fw / 2;
          s.y = centerY;
          bumpRowBottom(s, centerY);
          cursorX += fw + hGapFor(r, s);
        }
      }

      const lastShelf = shelves[shelves.length - 1];
      let authorRowTop = Math.max(rowBottom + rowGapFor(rowIndex + 1), lastShelf.top);
      let authorCursorX = rowStartX(rowIndex + 1, lastShelf);
      let authorShelf = lastShelf;
      for (let aIdx = 0; aIdx < authors.length; aIdx++) {
        ...
        let centerY = authorRowTop + fh / 2;
        let maxRight = authorShelf.maxRight;
        if (centerY + fh / 2 > authorShelf.bottom) {
          authorRowTop = authorShelf.top;
          centerY = authorRowTop + fh / 2;
        }
        if (authorCursorX + fw > maxRight && authorCursorX > bounds.left + 4) {
          authorRowTop += rowGapFor(rowIndex + 2 + aIdx) + fh;
          authorCursorX = rowStartX(rowIndex + 2 + aIdx, authorShelf);
          centerY = authorRowTop + fh / 2;
        }
        a.x = authorCursorX + fw / 2;
        a.y = centerY;
        authorCursorX += fw + hGapFor(quoteRows.length + aIdx, a);
      }

      return odqApplyCarouselSpeakerStripScatterNudge(
        [...quoteStrips, ...authors],
        bounds,
        speakerRect,
        layoutSeed,
    ...
    function odqApplyCarouselSpeakerStripScatterNudge(
      ...
      for (let i = 0; i < ordered.length; i++) {
        ...
        const jx = (odqPostStripStaggerRoll(hash, 5) - 0.5) * 36;
        const jy = (odqPostStripStaggerRoll(hash, 13) - 0.5) * 22;
        ...
        if (i > 0) {
          ...
          if (sameRow && curAr.left < prevAr.right + 6) {
            cx = Number(spec.x || 0);
            cy = Number(spec.y || 0);
    ...
    function odqApplyLayoutBTuneFieldsFromInstagramDoc(dateKey, data, aspect = 'story') {
      ...
      return { keywordEmphasis, stripLayoutSeed, quiltBgZoom };
    }
            ...
            loadQuiltImage()
              .then(async (quiltImg) => {
                ...
                if (
                  ...
                  if (isPostLayout && useSpeakerUnifiedStripPlan && quoteStripPlan.length) {
                    ...
                    if (carouselSpeakerAvoidRect) {
                      ...
                      for (let shrinkPass = 0; shrinkPass < 10; shrinkPass++) {
                        ...
                        if (
                          odqCarouselStripPlanLayoutOk(quoteStripPlan, carouselSpeakerAvoidRect, 10) &&
                        ...
                        quoteStripPlan = refitLayoutBStripPlanToText(
                          boostLayoutBStripPlanTypography(quoteStripPlan, 0.88, FONT_SERIF),
                          ctx,
                          carouselStripMaxW,
                          FONT_SERIF
                        );
                ...
                if (carouselSpeakerAvoidRect && quoteStripPlan.length) {
                  quoteStripPlan = odqReflowLayoutBStripPlanAroundCarouselSpeaker(
                    quoteStripPlan,
                    carouselSpeakerAvoidRect,
                    STORY_W,
                    STORY_H
