...
(function (root) {
  ...
  if (!CONFIG || typeof CONFIG !== 'object') {
    ...
    function getLayoutBStoryQuotePlan(ctx, options) {
      ...
      /** Post (4:5): quote/author strips sit near horizontal center with light jitter. */
      const postStripCenterX = (spec) => {
        const edgeL = stripBounds ? stripBounds.left + 8 : stripEdgeMargin;
        const edgeR = stripBounds ? stripBounds.right - 8 : STORY_W - stripEdgeMargin;
        const minCx = spec.w / 2 + edgeL;
        const maxCx = edgeR - spec.w / 2;
        const mid = stripBounds ? (stripBounds.left + stripBounds.right) / 2 : STORY_W / 2;
        const jitter = (rnd() - 0.5) * (stripBounds ? 18 : 48);
        return Math.max(minCx, Math.min(maxCx, mid + jitter));
      };
      ...
      for (let i = 0; i < stripSpecs.length; ) {
        ...
        let cx;
        if (isPostLayout) cx = postStripCenterX(spec);
    ...
    /**
     * Largest 3:4 rect centered inside a 4:5 IG post. Profile grid center-crops sides from 4:5
     * (full height preserved), so quote strips belong in this inset — not the full 1080px width.
     */
    ...
    async function composeInstagramLayoutBFromQuiltBlob(highResBlob, quoteText, quoteAuthor, layoutW, layoutH, dateKey, options = {}) {
      ...
      return new Promise((resolve, reject) => {
            ...
            /** 4:5 speaker-hero post: cutout fills the frame down to the name strip (grid-safe width). */
            const getSpeakerHeroPostRect = (img) => {
              ...
              const maxW = gridSafe ? gridSafe.width : STORY_W * 0.96;
              const topPad = 22;
              const bottomReserve = 112;
              const maxH = STORY_H - topPad - bottomReserve;
              let w = maxW;
              let h = w / aspect;
              if (h > maxH) {
                h = maxH;
              ...
              .then(async (quiltImg) => {
                ...
                if (options.speakerHeroPost === true) {
                  ...
                  if (rawName) {
                    ...
                    drawStripContent(nameSpec, nameCx, nameCy, (rnd() - 0.5) * 0.045, 'dark');
                  }
                  finishLayoutBCanvas();
                  return;
                }
                ...
                /** Speaker layouts: plan strips on 9:16, then scale to 4:5 post so tune modal story ↔ post match. */
                const useSpeakerUnifiedStripPlan =
                  speakerActive || (speakerRequested && options.stripLayoutSeedExplicit);
                ...
                if (
                  ...
                  if (speakerActive && isPostLayout && useSpeakerUnifiedStripPlan) {
                    quoteStripPlan = scaleLayoutBStripPlan(
                      quoteStripPlan,
                      LAYOUT_B_STORY_REF_W,
                      LAYOUT_B_STORY_REF_H,
