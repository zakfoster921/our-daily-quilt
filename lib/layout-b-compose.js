...
(function (root) {
  ...
  function odqLayoutBIgStripsSkipKeywordEmphasis() {
    ...
    /** Story 9:16 AUTO: default cutout width/height vs legacy base (3 = three times as large). */
    const ODQ_LAYOUT_B_STORY_SPEAKER_AUTO_LINEAR_MUL = 3;
    ...
    /** Story 9:16 AUTO: allow cutout to outgrow the author name strip (3× post cap). */
    const ODQ_LAYOUT_B_STORY_SPEAKER_MAX_NAME_STRIP_WIDTH_FRAC = 2.7;
    ...
    async function composeInstagramLayoutBFromQuiltBlob(highResBlob, quoteText, quoteAuthor, layoutW, layoutH, dateKey, options = {}) {
      ...
      return new Promise((resolve, reject) => {
            ...
            const getSpeakerOverlayRect = (img, strips) => {
              ...
              const fitted = odqConstrainSpeakerRectToNameStripWidth(
                best,
                strips,
                isPostLayout
                  ? ODQ_LAYOUT_B_SPEAKER_MAX_NAME_STRIP_WIDTH_FRAC
                  : ODQ_LAYOUT_B_STORY_SPEAKER_MAX_NAME_STRIP_WIDTH_FRAC
              );
              if (!isPostLayout) {
              ...
              .then(async (quiltImg) => {
                ...
                for (const speakerImg of speakerImgs) {
                  ...
                  if (speakerRect && speakerTransform) {
                    speakerRect = odqConstrainSpeakerRectToNameStripWidth(
                      speakerRect,
                      quoteStripPlan,
                      isPostLayout
                        ? ODQ_LAYOUT_B_SPEAKER_MAX_NAME_STRIP_WIDTH_FRAC
