...
(function (root) {
  ...
  root.ODQ_LAYOUT_B_STORY_RENDER_VERSION = ODQ_LAYOUT_B_STORY_RENDER_VERSION;
    ...
    /** Repeated single-word keyword strips should stay attached to their source phrase, not stack together. */
    function odqAttachRepeatedKeywordStripsToStoryPhrases(plan, layoutW, edgePad = 44) {
      ...
      for (let i = 0; i < strips.length; i++) {
        ...
        const next = strips
          .slice(i + 1)
          .find((s) => s && s.role === 'quote' && !s.isKeywordEmphasis);
        if (!next) continue;
        const specW = Math.max(1, Number(spec.w) || 0);
        const nextW = Math.max(1, Number(next.w) || 0);
        const gap = 24;
        spec.y = Number(next.y) || Number(spec.y) || 0;
        let x = Number(next.x || 0) - nextW / 2 - gap - specW / 2;
        if (x - specW / 2 < pad) {
          x = Number(next.x || 0) + nextW / 2 + gap + specW / 2;
        }
