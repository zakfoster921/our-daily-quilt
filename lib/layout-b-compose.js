...
(function (root) {
  ...
  if (!CONFIG || typeof CONFIG !== 'object') {
    ...
    /** Post quote: loose horizontal bands (strips per band before shifting lane). */
    const ODQ_LAYOUT_B_POST_READING_STRIPS_PER_BAND = 2;
    const ODQ_LAYOUT_B_POST_READING_BAND_MIN = 3;
    const ODQ_LAYOUT_B_POST_READING_BAND_MAX = 5;
    /** Band centers span this fraction of the inner layout column (not full width). */
    const ODQ_LAYOUT_B_POST_READING_BAND_USE_FRAC = 0.58;
    const ODQ_LAYOUT_B_POST_READING_BAND_JITTER_PX = 16;
    /** Max horizontal step between consecutive quote strips (fraction of inner column). */
    const ODQ_LAYOUT_B_POST_READING_MAX_CONSEC_STEP_FRAC = 0.07;
    ...
    function getLayoutBStoryQuotePlan(ctx, options) {
      ...
      const buildStripSpecs = () => {
        ...
        const tryPairQuoteLine = (ln, font, lineSalt) => {
          ...
          const lim =
            postInnerSpan > 0
              ? Math.max(72, postInnerSpan * 0.46 - PAPER_PAD_X)
              : innerMaxW;
          const pairRowMaxW = postInnerSpan > 0 ? postInnerSpan * 0.9 : innerMaxW * 2;
          if (fullW < lim * 0.22 && words.length < 5) return null;
          let best = null;
          ...
          const pickPct = isPostLayout && stripBounds ? 52 : 48;
      ...
      const storyLikeStripCenterX = (spec, pseudoWidth) => {
        ...
        if (postReadableStripStack && spec.role === 'author') {
          const minCx = w / 2 + hBounds.left + 8;
          const maxCx = hBounds.right - w / 2 - 8;
          const centers = odqPostStripReadingBandCenters(STORY_W, STORY_H, postQuoteRowCount);
          ...
          let cx = centers[centers.length - 1] + innerSpan * 0.1;
          return Math.max(minCx, Math.min(maxCx, cx));
        }
      ...
      for (let i = 0; i < stripSpecs.length; ) {
        ...
        if (
          ...
          const pairTilt = postReadableStripStack ? (rnd() - 0.5) * 0.02 : (rnd() - 0.5) * 0.055;
          strips.push({
            ...a,
            x: leftCx,
            y: topOfRow + a.h / 2,
          ...
          strips.push({
            ...
            y: topOfRow + b.h / 2,
            padX: PAPER_PAD_X,
            angle: (rnd() - 0.5) * (postReadableStripStack ? 0.02 : 0.055),
            seed: `${b.role}_${i + 1}`
          });
    ...
    function odqPostStripReadingBandCount(quoteCount) {
      const n = Math.max(1, Number(quoteCount) || 1);
      return Math.max(
        ODQ_LAYOUT_B_POST_READING_BAND_MIN,
        Math.min(
          ODQ_LAYOUT_B_POST_READING_BAND_MAX,
          Math.ceil(n / ODQ_LAYOUT_B_POST_READING_STRIPS_PER_BAND)
        )
      );
    }

    function odqPostStripReadingBandCenters(layoutW, layoutH, quoteCount) {
      const layout = odqLayoutBPostStripInnerColumn(layoutW, layoutH);
      if (!layout) return [layoutW / 2];
      const bandCount = odqPostStripReadingBandCount(quoteCount);
      const innerSpan = Math.max(1, layout.right - layout.left);
      const useSpan = innerSpan * ODQ_LAYOUT_B_POST_READING_BAND_USE_FRAC;
      const mid = (layout.left + layout.right) / 2;
      const useLeft = mid - useSpan / 2;
      const centers = [];
      for (let b = 0; b < bandCount; b++) {
        const t = bandCount === 1 ? 0.5 : b / (bandCount - 1);
        centers.push(useLeft + useSpan * t);
      }
      return centers;
    }

    /** Target X for quote strip i: loose lane bands + capped step strip-to-strip. */
    function odqPostStripReadingBandTargetCx(
      ...
      const centers = odqPostStripReadingBandCenters(layoutW, layoutH, quoteCount);
      const layout = odqLayoutBPostStripInnerColumn(layoutW, layoutH);
      const innerSpan = layout ? Math.max(1, layout.right - layout.left) : 400;
      const bandCount = centers.length;
      const stripsPerBand = Math.max(1, Math.ceil(quoteCount / bandCount));
      const bandIdx = Math.min(bandCount - 1, Math.floor(stripIndex / stripsPerBand));
      let targetCx = centers[bandIdx];
      const maxStep = innerSpan * ODQ_LAYOUT_B_POST_READING_MAX_CONSEC_STEP_FRAC;
      const prev = Number.isFinite(prevSmoothedCx) ? prevSmoothedCx : centers[0];
      const delta = targetCx - prev;
      targetCx = prev + Math.max(-maxStep, Math.min(maxStep, delta));
      return targetCx;
    }

    /**
     * Post 4:5: assign strips to loose horizontal bands so reading order stays top→bottom
     * without heavy left/right zig-zag between consecutive phrases.
     */
    ...
    function odqApplyPostStripReadingBands(plan, layoutW, layoutH) {
      const strips = (Array.isArray(plan) ? plan : []).map((s) => ({ ...s }));
      if (!strips.length) return strips;
      const safe = odqLayoutBPostGridSafeRect(layoutW, layoutH, 8);
      const layout = odqLayoutBPostStripInnerColumn(layoutW, layoutH);
      ...
      const bandCenters = odqPostStripReadingBandCenters(layoutW, layoutH, unitCount);
      const innerSpan = layout ? Math.max(1, layout.right - layout.left) : 400;
      let prevCx = bandCenters[0];
      for (let r = 0; r < quoteRows.length; r++) {
        const row = quoteRows[r];
        const targetCx = odqPostStripReadingBandTargetCx(r, unitCount, prevCx, layoutW, layoutH);
        const isPair =
          row.length === 2 &&
          row[0].rowGroup != null &&
      ...
      for (const a of authors) {
        let cx = bandCenters[bandCenters.length - 1] + innerSpan * 0.1;
    ...
    function odqApplyPostQuoteStripVerticalSpread(plan, layoutW, layoutH, opts = {}) {
      ...
      let out = opts.readingStack
        ? odqApplyPostStripReadingBands(spread, layoutW, layoutH)
