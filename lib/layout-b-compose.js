...
(function (root) {
  ...
  if (!CONFIG || typeof CONFIG !== 'object') {
    ...
    /** layout-b-speaker.png: hand-cut silhouette mat behind cutout (in-app card uses neutral-silhouette + clip). */
    const ODQ_SPEAKER_CUTOUT_MAT_RGB = '246, 244, 241';
    const ODQ_LAYOUT_B_SPEAKER_HERO_MAT_OPACITY = 1;
    /** Export hero mat: slight peek past multiply cutout (closer to in-app neutral-silhouette offset). */
    const ODQ_SPEAKER_HERO_MAT_OFFSET_X_FRAC = 0.009;
    const ODQ_SPEAKER_HERO_MAT_OFFSET_Y_FRAC = 0.007;
    const ODQ_SPEAKER_HERO_MAT_SCALE = 1.01;
    const ODQ_SPEAKER_HERO_MAT_ROTATE_DEG = 0.15;
    const ODQ_SPEAKER_HERO_MAT_HAND_CUT_CFG = {
      handCutEnabled: true,
      handCutMarginDomPx: 0.15,
      handCutCornerChamferDomPx: 10,
      handCutMacroDomPx: 5,
      handCutBiteMaxDomPx: 14,
      handCutSecondaryBiteDomPx: 10,
      handCutSideInwardMaxDomPx: 13,
      handCutTopBottomTrimDomPx: 10,
    ...
    async function composeInstagramLayoutBFromQuiltBlob(highResBlob, quoteText, quoteAuthor, layoutW, layoutH, dateKey, options = {}) {
      ...
      return new Promise((resolve, reject) => {
            ...
            const makeSpeakerHandCutSolidMatCanvas = (portraitCanvas, seedKey) => {
              if (!portraitCanvas) return null;
              ...
              bctx.drawImage(portraitCanvas, 0, 0);
              bctx.globalCompositeOperation = 'source-in';
              bctx.fillStyle = `rgb(${ODQ_SPEAKER_CUTOUT_MAT_RGB})`;
              ...
              const peekPx = Math.max(2, Math.round(m * 0.004));
              for (let y = 0; y < h; y += 1) {
                for (let x = 0; x < w; x += 1) {
                  if (alphaAt(bd, x, y) < 128) continue;
                  for (let dy = -peekPx; dy <= peekPx; dy += 1) {
                    for (let dx = -peekPx; dx <= peekPx; dx += 1) {
                      if (Math.abs(dx) + Math.abs(dy) > peekPx) continue;
                      setOpaque(bd, x + dx, y + dy);
                    }
                  }
                }
              }
              ...
              const chopCount = 20 + Math.floor(rnd() * 16);
              const minLen = Math.max(14, Math.round(m * 0.04));
              const maxLen = Math.max(minLen + 8, Math.round(m * 0.13));
              const carveLine = (sx, sy, angle, len, thick) => {
                const cos = Math.cos(angle);
                const sin = Math.sin(angle);
                const steps = Math.ceil(len);
                const start = -len * (0.15 + rnd() * 0.2);
                for (let t = 0; t <= steps; t += 1) {
                  const along = start + (len * t) / steps;
                  const cx = Math.round(sx + cos * along);
                  const cy = Math.round(sy + sin * along);
                  for (let oy = -thick; oy <= thick; oy += 1) {
                    for (let ox = -thick; ox <= thick; ox += 1) {
                      if (Math.abs(ox) + Math.abs(oy) > thick) continue;
                      clearPixel(bd, cx + ox, cy + oy);
                    }
                  }
                }
              };

              for (let c = 0; c < chopCount; c += 1) {
                const pick = edgePts[Math.floor(rnd() * edgePts.length)];
                const sx = pick[0];
                const sy = pick[1];
                const angle = rnd() * Math.PI * 2;
                const len = minLen + rnd() * (maxLen - minLen);
                carveLine(sx, sy, angle, len, rnd() < 0.35 ? 2 : 1);
              }
              ...
              const finalMat = document.createElement('canvas');
              finalMat.width = w;
              finalMat.height = h;
              const fctx = finalMat.getContext('2d');
              if (!fctx) return null;
            ...
            const drawSpeakerOverlay = (g, img, rect, washColor, opts = {}) => {
              ...
              const solidMatOpacity = Math.max(0, Math.min(1, Number(opts.solidMatOpacity) || 0));
              let portrait = null;
              try {
              ...
              if (solidMatOpacity > 0) {
                const mat = makeSpeakerHandCutSolidMatCanvas(
                  portrait,
                  `${rngDateKey}:speaker-cutout:mat`
                );
                if (mat) {
                  g.save();
                  g.globalCompositeOperation = 'source-over';
                  ...
                  g.translate(
                    rect.width * ODQ_SPEAKER_HERO_MAT_OFFSET_X_FRAC,
                    rect.height * ODQ_SPEAKER_HERO_MAT_OFFSET_Y_FRAC
                  );
                  g.rotate((ODQ_SPEAKER_HERO_MAT_ROTATE_DEG * Math.PI) / 180);
                  g.scale(ODQ_SPEAKER_HERO_MAT_SCALE, ODQ_SPEAKER_HERO_MAT_SCALE);
                  g.drawImage(mat, -rect.width / 2, -rect.height / 2, rect.width, rect.height);
