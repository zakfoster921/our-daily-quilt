...
(function (root) {
  ...
  const ODQ_LAYOUT_B_STORY_RENDER_VERSION = 'story-speaker-25pct-v10';
  root.ODQ_LAYOUT_B_STORY_RENDER_VERSION = ODQ_LAYOUT_B_STORY_RENDER_VERSION;
  /** Layout B IG story/post strips: plain text only (clipping + quilt screen keep keywords). */
  const ODQ_LAYOUT_B_IG_STRIPS_SKIP_KEYWORD_EMPHASIS = true;
  root.ODQ_LAYOUT_B_IG_STRIPS_SKIP_KEYWORD_EMPHASIS = ODQ_LAYOUT_B_IG_STRIPS_SKIP_KEYWORD_EMPHASIS;

  function odqLayoutBIgStripsSkipKeywordEmphasis() {
    return ODQ_LAYOUT_B_IG_STRIPS_SKIP_KEYWORD_EMPHASIS === true;
  }
    ...
    const ODQ_LAYOUT_B_QUOTE_STRIP_FONT_WEIGHT = 400;
    /** Post 4:5 quote strips: lighter weight to match story’s printed look. */
    const ODQ_LAYOUT_B_POST_QUOTE_STRIP_FONT_WEIGHT = 300;
    /** Wider tracking on strip type (em of font size). */
    const ODQ_LAYOUT_B_STRIP_LETTER_SPACING_EM = 0.045;
    /** Global strip body size multiplier (IG canvas + in-app plan). */
    const ODQ_LAYOUT_B_STRIP_BODY_SCALE = 0.9;
    ...
    const ODQ_LAYOUT_B_QUOTE_AUTHOR_GAP_STORY_SPEAKER = 108;
    const ODQ_LAYOUT_B_STORY_SPEAKER_VPAD_BOTTOM = 84;
    /** Story: preferred quote→speaker-name separation when vertical room allows. */
    const ODQ_LAYOUT_B_STORY_AUTHOR_NAME_MIN_GAP = 112;
    const ODQ_LAYOUT_B_STORY_AUTHOR_NAME_MAX_GAP = 196;
    const ODQ_LAYOUT_B_STORY_AUTHOR_BOTTOM_RESERVE = 108;
    ...
    /** Layout B AUTO speaker (story 9:16 + post 4:5): cutout area vs full canvas (aspect preserved). */
    const ODQ_LAYOUT_B_SPEAKER_AUTO_TARGET_AREA_FRAC = 0.20;
    const ODQ_LAYOUT_B_SPEAKER_AUTO_MAX_AREA_FRAC = 0.25;
    ...
    function odqLayoutBStripStartingBodyPx(usePostQuoteLayout, isPostLayout = false) {
      let bodyPx = Math.round(38 * 1.25 * ODQ_LAYOUT_B_STRIP_BODY_SCALE);
      if (usePostQuoteLayout) bodyPx += Math.round(10 * ODQ_LAYOUT_B_STRIP_BODY_SCALE);
      if (isPostLayout) bodyPx = Math.round(bodyPx * ODQ_LAYOUT_B_POST_STRIP_FONT_SCALE);
      return bodyPx;
    }

    /** Post 4:5 quote strips: never shrink below this bodyPx (~63px paper strip at 1080×1350). */
    const ODQ_LAYOUT_B_POST_MIN_BODY_PX = Math.round(38 * ODQ_LAYOUT_B_STRIP_BODY_SCALE);
    ...
    /** Carousel slide 1 quote/author strips — bright white paper (not cream post fill). */
    const ODQ_CAROUSEL_STRIP_PAPER_FILL = '#ffffff';
    ...
    const ODQ_CAROUSEL_STORY_STYLE_MIN_BODY_PX = Math.round(60 * ODQ_LAYOUT_B_STRIP_BODY_SCALE);
    ...
    /** Carousel / tune post preview: quote-row gap clamps (adaptive stack fills safe-zone height). */
    const ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_COMFORTABLE = 28;
    const ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_MAX = 8;
    const ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_MIN = 0;
    /** Extra space before author strip beyond the quote-row gap. */
    const ODQ_CAROUSEL_STORY_STYLE_AUTHOR_ROW_GAP_EXTRA = 14;
    /** Breathing room between quote cluster and post safe-zone top/bottom. */
    const ODQ_CAROUSEL_STORY_STYLE_STACK_EDGE_MARGIN = 48;
    ...
    /** Story 9:16: keep single/emphasis strips in a collage cluster instead of edge-to-edge scatter. */
    const ODQ_LAYOUT_B_STORY_CLUSTER_CENTER_FRAC = 0.28;
    const ODQ_LAYOUT_B_STORY_CLUSTER_SIDE_NUDGE_FRAC = 0.09;
    ...
    /** Admin tune: vertical pan on quilt background (fraction of canvas height; + = down). */
    const ODQ_LAYOUT_B_QUILT_BG_OFFSET_Y_STEP = 0.01;
    const ODQ_LAYOUT_B_QUILT_BG_OFFSET_Y_BIG_STEP = 0.05;
    ...
    const ODQ_LAYOUT_B_SPEAKER_MAX_NAME_STRIP_WIDTH_FRAC = 0.9;
    ...
    function odqLayoutBQuoteStripFontWeight(isPostLayout, role) {
      if (isPostLayout && role === 'quote') return ODQ_LAYOUT_B_POST_QUOTE_STRIP_FONT_WEIGHT;
      return ODQ_LAYOUT_B_QUOTE_STRIP_FONT_WEIGHT;
    }

    function odqLayoutBQuoteStripFont(bodyPx, fontSerif, isPostLayout, role = 'quote') {
      const px = Math.max(1, Math.round(Number(bodyPx) || 48));
      const weight = odqLayoutBQuoteStripFontWeight(isPostLayout, role);
      return `${weight} ${px}px ${fontSerif || ODQ_CANVAS_SERIF_FONT}`;
    }

    /** Post inherits story strip plans at weight 400 — rewrite quote strip fonts at draw/measure time. */
    function odqLayoutBStripFontForRender(fontStr, isPostLayout, role) {
      const fs = String(fontStr || '');
      if (!isPostLayout || role !== 'quote' || /^italic\s/i.test(fs)) return fs;
      const weight = odqLayoutBQuoteStripFontWeight(isPostLayout, role);
      if (/\b\d{3}\b/.test(fs)) return fs.replace(/\b\d{3}\b/, String(weight));
      return `${weight} ${fs}`.replace(/\s+/g, ' ').trim();
    }
    ...
    /** Sentence boundaries for strip layout — each sentence starts its own strip row. */
    function odqExtractQuoteSentences(text) {
    ...
    function odqPackStoryQuoteStripsIntoRows(specList, opts = {}) {
      ...
      while (i < specList.length) {
        const a = specList[i];
        if (a.role !== 'quote' || a.isKeywordEmphasis) {
          out.push(a);
          i += 1;
          continue;
        }
        ...
        while (i < specList.length && row.length < maxStrips) {
          ...
          if (next.sentenceStart && row.length) break;
          if (next.role !== 'quote' || next.rowGroup != null || next.isKeywordEmphasis) break;
    ...
    function getLayoutBStoryQuotePlan(ctx, options) {
      ...
      /** Row-budget strip splitting uses compressed typography — only when post layout still overflows after comfortable specs. */
      const carouselRowBudget = false;
      ...
      // Longer quotes get chunkier phrase strips so total strip/row count doesn't
      // blow past what fits at the carousel's fixed min font size. Based on the
      // full quote (not per-sentence — splitIntoPhraseSegments runs once per sentence).
      const phraseSegmentTakeLo = quoteText.length > 230 ? 4 : quoteText.length > 165 ? 3 : 2;
      const splitIntoPhraseSegments = (text) => {
        const t = String(text || '').replace(/\s+/g, ' ').trim();
        if (!t) return [];
        const takeLo = phraseSegmentTakeLo;
        ...
        for (const sentence of sentences) {
          ...
          for (const chunk of chunks) {
            ...
            while (i < words.length) {
              // Pick takeLo or takeLo+1 words; the larger size ~40% of the time.
              let take = (rnd() > 0.60 && i + takeLo < words.length) ? takeLo + 1 : takeLo;
      ...
      const stackVertPad = stripBounds
        ...
        : usePostQuoteLayout
          ? 19
          : 14;
      let bodyPx = odqLayoutBStripStartingBodyPx(usePostQuoteLayout, isPostLayout);
      ...
      const buildStripSpecs = () => {
        ...
        const emitStoryQuoteUnitsInRows = (units, font) => {
          ...
          while (ui < list.length) {
            ...
            while (ui < list.length && row.length < ODQ_LAYOUT_B_STORY_ROW_MAX_STRIPS) {
              ...
              if (list[ui].sentenceStart && row.length) break;
            ...
            if (row.length >= ODQ_LAYOUT_B_STORY_ROW_MIN_STRIPS) {
              ...
              row.forEach((unit, slot) => {
                const meta = {
                  rowGroup: gid,
                  rowSlot: slot,
                  ...(unit.sentenceStart ? { sentenceStart: true } : {})
                };
              ...
              if (unit.inlineRuns && Array.isArray(unit.textRuns) && unit.textRuns.length) {
                ...
                measureStrip(
                  ...
                  1.38,
                  'quote',
                  unit.isKeywordConnector
                    ? { isKeywordConnector: true, ...(unit.sentenceStart ? { sentenceStart: true } : {}) }
                    : unit.sentenceStart
                      ? { sentenceStart: true }
                      : null
                );
        ...
        const emitQuoteSentencesForLayout = (text) => {
          for (const sentence of odqExtractQuoteSentences(text)) {
            emitQuoteSegmentsForLayout(segmentQuoteForLayout(sentence), { sentenceStart: true });
          }
        };
        ...
        const emitQuoteSegmentsForLayout = (segments, opts = {}) => {
          let sentenceStartNext = opts.sentenceStart === true;
          const quoteMeta = (base) => {
            if (!sentenceStartNext) return base ?? null;
            sentenceStartNext = false;
            return base ? { ...base, sentenceStart: true } : { sentenceStart: true };
          };
          ...
          for (let si = 0; si < mergedSegs.length; ) {
            if (carouselRowBudget) {
              ...
              for (const ln of lines) {
                measureStrip([ln], bodyFont, quoteLineLead, 'quote', quoteMeta());
              }
              si += 1;
              continue;
            }
            ...
            if (microPieces.length === 1 && si + 1 < mergedSegs.length) {
              ...
              if (adjacentPair) {
                const gid = nextRowPairGroup++;
                measureStrip([adjacentPair[0]], bodyFont, quoteLineLead, 'quote', quoteMeta({
                  rowGroup: gid,
                  rowSlot: 0
                }));
        ...
        const qPx = quoteFontPx();
        const bodyFont = odqLayoutBQuoteStripFont(qPx, FONT_SERIF, isPostLayout, 'quote');
        ctx.font = bodyFont;
        odqSetLayoutBStripLetterSpacing(ctx, qPx);
        let quoteLineSalt = 0;
        const layoutKeywordPart = (part) => {
          ...
          if (part.kind === 'plain') {
            emitQuoteSentencesForLayout(chunkText);
            return;
          }
          emitQuoteSentencesForLayout(chunkText);
        };
        if (kwActive) {
          ...
          if (!isPostLayout) {
            const storyUnits = [];
            const flushStoryUnits = () => {
              if (!storyUnits.length) return;
              emitStoryQuoteUnitsInRows(storyUnits.splice(0), bodyFont);
            };
            for (const part of fullParts) {
              ...
              if (part.kind === 'plain') {
                for (const sentence of odqExtractQuoteSentences(chunkText)) {
                  const pieces = flattenSegmentsToStoryTextPieces(segmentQuoteForLayout(sentence));
                  emitStoryQuoteUnitsInRows(
                    pieces.map((t, i) => ({
                      emphasis: false,
                      text: t,
            ...
            const flushPlainSegs = () => {
              ...
              emitQuoteSegmentsForLayout(plainSegs.splice(0));
            };
            for (const part of fullParts) {
              if (part.kind === 'emphasis-strip' && kwOwnStrip) {
                flushPlainSegs();
          ...
          emitQuoteSentencesForLayout(quoteText);
        }
        if (layoutAuthor) {
      ...
      const staggerStripCenterX = (spec) => {
        ...
        let cx;
        if (!isPostLayout) {
          const centerSpan = STORY_W * ODQ_LAYOUT_B_STORY_CLUSTER_CENTER_FRAC;
          cx = STORY_W / 2 + (rnd() - 0.5) * centerSpan;
          if (roll < 0.22) {
            const side = rnd() < 0.5 ? -1 : 1;
            cx += side * STORY_W * ODQ_LAYOUT_B_STORY_CLUSTER_SIDE_NUDGE_FRAC * rnd();
          }
        } else if (roll < 0.34) cx = stripEdgeMargin + spec.w / 2 + rnd() * 95;
      ...
      /**
       * Post 4:5: keep full strip type for long quotes — vertical fit is manual (tune drag).
       * Still shrink for horizontal text overflow / carousel row budget.
       */
      const skipVerticalTypeShrink = isPostLayout && !carouselRowBudget && !stripBounds;
      ...
      for (let s = 0; s < shrinkIters; s++) {
        ...
        const heightOk = skipVerticalTypeShrink || stackH <= stripBudget;
        if (
          (heightOk && textOverflow <= 0 && !rowBudgetOverflow) ||
          bodyPx <= minBodyPx
        ) {
          break;
        }
    ...
    /** True when a saved strip plan can replay without refitting typography (lines or font + paper size). */
    function odqLayoutBStripPlanHasTypography(plan) {
      if (!Array.isArray(plan) || !plan.length) return false;
      return plan.some((s) => {
        if (Array.isArray(s.lines) && s.lines.length) return true;
        const hasFont = /(\d+)px/.test(String(s.font || ''));
        const hasPaper = Number(s.w) > 0 && Number(s.h) > 0;
        return hasFont && hasPaper;
      });
    }
    ...
    /** Final story guard: speaker lift/offsets must never push quote strips outside the 9:16 canvas. */
    function odqClampLayoutBStoryStripPlanToCanvas(plan, layoutW, layoutH, edgePad = 44) {
      const strips = Array.isArray(plan) ? plan : [];
      if (!strips.length) return strips;
      const w = Math.max(1, Number(layoutW) || 1080);
      const h = Math.max(1, Number(layoutH) || 1920);
      const pad = Math.max(12, Number(edgePad) || 44);
      const visualPad = 24;
      let minTop = Infinity;
      let maxBottom = -Infinity;
      for (const spec of strips) {
        const r = odqStripPlanAxisRect(spec, visualPad);
      ...
      if (maxBottom > h - pad && minTop > pad) {
        dy -= Math.min(maxBottom - (h - pad), minTop - pad);
      }
      if (Math.abs(dy) > 0.5) {
        for (const spec of strips) spec.y = Number(spec.y || 0) + dy;
      }
      for (const spec of strips) {
        const r = odqStripPlanAxisRect(spec, visualPad);
        const dxLeft = pad - r.left;
        const dxRight = r.right - (w - pad);
        if (dxLeft > 0) spec.x = Number(spec.x || 0) + dxLeft;
        else if (dxRight > 0) spec.x = Number(spec.x || 0) - dxRight;
      }
      return strips;
    }

    /** Story 9:16: push speaker/author name down when slack exists so it reads apart from quote strips. */
    function odqSpreadStoryAuthorNameBelowQuotes(plan, layoutW, layoutH, opts = {}) {
      const strips = (Array.isArray(plan) ? plan : []).map((s) => ({ ...s }));
      const authors = strips.filter((s) => s.role === 'author' && !s.authorCutoutLabel);
      const quotes = strips.filter((s) => s.role === 'quote');
      if (!authors.length || !quotes.length) return strips;
      const h = Math.max(1, Number(layoutH) || 1920);
      const pad = ODQ_CAROUSEL_STRIP_TEXT_SAFE_PAD;
      const edgePad = Math.max(12, Number(opts.edgePad) || 44);
      const bottomReserve = Math.max(
        72,
        Number(opts.bottomReserve) || ODQ_LAYOUT_B_STORY_AUTHOR_BOTTOM_RESERVE
      );
      const minGap = Math.max(
        ODQ_CAROUSEL_STRIP_TEXT_MIN_GAP,
        Number(opts.minGap) || ODQ_LAYOUT_B_STORY_AUTHOR_NAME_MIN_GAP
      );
      const maxGap = Math.max(
        minGap,
        Number(opts.maxGap) || ODQ_LAYOUT_B_STORY_AUTHOR_NAME_MAX_GAP
      );
      const quoteBottom = Math.max(
        ...quotes.map((q) => odqStripPlanAxisRect(q, pad).bottom)
      );
      const maxAuthorBottom = h - edgePad - bottomReserve;
      for (const author of authors) {
        const ar = odqStripPlanAxisRect(author, pad);
        const fh = Math.max(1, ar.bottom - ar.top);
        let gap = minGap;
        for (let g = maxGap; g >= minGap; g -= 6) {
          const bottom = quoteBottom + g + fh;
          if (bottom <= maxAuthorBottom - 2) {
            gap = g;
            break;
          }
        }
        author.y = Math.min(quoteBottom + gap + fh / 2, maxAuthorBottom - fh / 2);
      }
      return strips;
    }
    ...
    function odqApplyCarouselStoryStyleAuthorFontScale(plan, ctx, maxStripW, fontSerif, opts = {}) {
      ...
      return refitLayoutBStripPlanToText(arr, ctx, maxStripW, fontSerif, {
        lockBodyPx: true,
        uniformAuthorPx: authorPx,
        paperPadY: opts.tightStack === true ? ODQ_CAROUSEL_STRIP_PAPER_PAD_Y : 15
      });
    }
    ...
    function odqSortCarouselQuoteStripsByTextOrder(quotes, quoteText) {
      ...
      const stripText = (s) =>
        String(s.text || (Array.isArray(s.lines) ? s.lines.join(' ') : '') || '')
          .trim()
          .toLowerCase();
      const findPos = (needle, fromIndex = 0) => {
        const t = String(needle || '').trim().toLowerCase();
        if (!t) return Number.MAX_SAFE_INTEGER;
        let idx = src.indexOf(t, Math.max(0, fromIndex));
        if (idx >= 0) return idx;
        if (t.length > 10) {
          idx = src.indexOf(t.slice(0, 10), Math.max(0, fromIndex));
          if (idx >= 0) return idx;
        }
        for (const w of t.split(/\s+/).filter((word) => word.length > 3)) {
          idx = src.indexOf(w, Math.max(0, fromIndex));
          if (idx >= 0) return idx;
        }
      ...
      let cursor = 0;
      for (const s of Array.isArray(quotes) ? quotes : []) {
        const t = stripText(s);
        let idx = findPos(t, cursor);
        if (idx === Number.MAX_SAFE_INTEGER) idx = findPos(t, 0);
        positions.set(s, idx);
        if (idx !== Number.MAX_SAFE_INTEGER) {
          cursor = Math.min(src.length, idx + Math.max(1, t.length));
        }
      }
      const posOf = (s) => positions.get(s) ?? Number.MAX_SAFE_INTEGER;
    ...
    function odqLayoutBStripPlanLabel(s) {
      return String(s.text || (Array.isArray(s.lines) ? s.lines.join(' ') : '') || '')
        .replace(/^\s*[—-]\s*/, '')
        .replace(/\s+/g, ' ')
        .trim()
        .toLowerCase();
    ...
    /** Count saved vs current strips (reading order) without mutating the live plan. */
    function odqCountSavedLayoutBStripPlanMatch(plan, savedPlan, quoteText, authorName) {
      const strips = Array.isArray(plan) ? plan : [];
      const saved = Array.isArray(savedPlan) ? savedPlan : [];
      if (!strips.length || !saved.length) return 0;
      const authorNameLower = String(authorName || '')
        .replace(/^\s*[—-]\s*/, '')
        .replace(/\s+/g, ' ')
        .trim()
        .toLowerCase();
      const isAuthorStrip = (s) =>
        s.role === 'author' || (!!authorNameLower && odqLayoutBStripPlanLabel(s) === authorNameLower);
      const curQuotes = strips.filter((s) => !isAuthorStrip(s));
      const savedQuotes = saved.filter((s) => !isAuthorStrip(s));
      const orderedCur = odqSortCarouselQuoteStripsByTextOrder(curQuotes, quoteText);
      const orderedSaved = odqSortCarouselQuoteStripsByTextOrder(savedQuotes, quoteText);
      let matched = 0;
      for (let i = 0; i < orderedCur.length; i++) {
        const from = orderedSaved[i];
        if (from && from.x != null && from.y != null) matched++;
      }
      const curAuthors = strips.filter(isAuthorStrip);
      const savedAuthors = saved.filter(isAuthorStrip);
      for (let i = 0; i < curAuthors.length; i++) {
        const from = savedAuthors[i];
        if (from && from.x != null && from.y != null) matched++;
      }
      return matched;
    }

    /** Tune modal saved plan: replay only when every strip has a saved x/y pair. */
    function odqSavedLayoutBPostStripPlanUsable(plan, savedPlan, quoteText, authorName) {
      const n = Array.isArray(plan) ? plan.length : 0;
      const savedN = Array.isArray(savedPlan) ? savedPlan.length : 0;
      if (!n || !savedN || n !== savedN) return false;
      return (
        odqCountSavedLayoutBStripPlanMatch(plan, savedPlan, quoteText, authorName) === n
      );
    }

    /** Match saved strip x/y by quote reading order — repeated phrases must not share one text key. */
    function odqApplySavedLayoutBStripPlanPositionsByReadingOrder(plan, savedPlan, quoteText, authorName, opts = {}) {
      const strips = Array.isArray(plan) ? plan : [];
      const saved = Array.isArray(savedPlan) ? savedPlan : [];
      if (!strips.length || !saved.length) return 0;
      const xOnly = opts.xOnly === true;
      const anglesOnly = opts.anglesOnly === true;
      const authorNameLower = String(authorName || '')
        .replace(/^\s*[—-]\s*/, '')
        .replace(/\s+/g, ' ')
        .trim()
      ...
      for (let i = 0; i < orderedCur.length; i++) {
        const from = orderedSaved[i];
        if (!from) continue;
        const s = orderedCur[i];
        if (!anglesOnly) {
          if (from.x != null) s.x = from.x;
          if (!xOnly && from.y != null) s.y = from.y;
        }
        if (from.role && !anglesOnly) s.role = from.role;
        if (from.angle != null && Number.isFinite(Number(from.angle))) s.angle = Number(from.angle);
        matched++;
      }
      const curAuthors = strips.filter(isAuthorStrip);
      const savedAuthors = saved.filter(isAuthorStrip);
    ...
    function odqResolveCarouselStoryStyleSpeakerSeamRect(img, layoutW, layoutH, shortQuote, quiltRect) {
      const resolve =
      ...
      return resolve(layoutW, layoutH, img, undefined, {
        shortQuote: shortQuote === true,
        quiltRect: quiltRect || null
      });
    }

    function odqSpeakerNameStripWidthFromPlan(strips) {
      const authors = (Array.isArray(strips) ? strips : []).filter((s) => s?.role === 'author');
      const preferred = authors.find((s) => s.authorCutoutLabel) || authors[0];
      const width = Number(preferred?.w);
      return Number.isFinite(width) && width > 0 ? width : 0;
    }

    function odqConstrainSpeakerRectToNameStripWidth(
      rect,
      strips,
      maxWidthFrac = ODQ_LAYOUT_B_SPEAKER_MAX_NAME_STRIP_WIDTH_FRAC
    ) {
      if (!rect || !Number.isFinite(Number(rect.width)) || Number(rect.width) <= 0) return rect;
      const nameStripW = odqSpeakerNameStripWidthFromPlan(strips);
      const maxW = nameStripW * maxWidthFrac;
      if (!Number.isFinite(maxW) || maxW <= 0 || Number(rect.width) <= maxW) return rect;
      const scale = maxW / Number(rect.width);
      const oldX = Number(rect.x) || 0;
      const oldY = Number(rect.y) || 0;
      const oldW = Number(rect.width);
      const oldH = Number(rect.height) || 0;
      const cx = oldX + oldW / 2;
      const bottom = oldY + oldH;
      const nextW = oldW * scale;
      const nextH = oldH * scale;
      const next = {
        ...rect,
        x: cx - nextW / 2,
        y: bottom - nextH,
      ...
      if (
        Number.isFinite(Number(rect.contentX)) &&
        Number.isFinite(Number(rect.contentY)) &&
        Number.isFinite(Number(rect.contentWidth)) &&
        Number.isFinite(Number(rect.contentHeight))
      ) {
        next.contentX = next.x + (Number(rect.contentX) - oldX) * scale;
        next.contentY = next.y + (Number(rect.contentY) - oldY) * scale;
        next.contentWidth = Number(rect.contentWidth) * scale;
        next.contentHeight = Number(rect.contentHeight) * scale;
      }
      if (
        Number.isFinite(Number(rect.opaqueAnchorX)) &&
        Number.isFinite(Number(rect.opaqueAnchorY))
      ) {
        next.opaqueAnchorX = next.x + (Number(rect.opaqueAnchorX) - oldX) * scale;
        next.opaqueAnchorY = next.y + (Number(rect.opaqueAnchorY) - oldY) * scale;
      }
      return next;
    }

    /** Keep speaker rect inside the canvas without changing chosen placement. */
    function odqClampStorySpeakerRectInCanvas(rect, layoutW, layoutH, minPad = 28, allowOffCanvas = false) {
      if (!rect) return rect;
      let w = Math.max(1, Number(rect.width) || 0);
      let h = Math.max(1, Number(rect.height) || 0);
      if (!w || !h) return rect;
      const pad = Math.max(0, Number(minPad) || 0);
      const oldX = Number(rect.x) || 0;
      const oldY = Number(rect.y) || 0;
      const oldW = Number(rect.width) || w;
      const oldH = Number(rect.height) || h;
      if (allowOffCanvas) {
        const marginX = w * 0.98;
        const marginY = h * 0.98;
        const next = {
          ...rect,
          x: Math.max(-marginX, Math.min(Number(rect.x) || 0, layoutW - w + marginX)),
          y: Math.max(-marginY, Math.min(Number(rect.y) || 0, layoutH - h + marginY)),
          width: w,
          height: h
        };
        if (
          ...
          next.contentX = next.x + (Number(rect.contentX) - oldX);
          next.contentY = next.y + (Number(rect.contentY) - oldY);
          next.contentWidth = Number(rect.contentWidth);
          next.contentHeight = Number(rect.contentHeight);
        }
        if (
          Number.isFinite(Number(rect.opaqueAnchorX)) &&
          Number.isFinite(Number(rect.opaqueAnchorY))
        ) {
          next.opaqueAnchorX = next.x + (Number(rect.opaqueAnchorX) - oldX);
          next.opaqueAnchorY = next.y + (Number(rect.opaqueAnchorY) - oldY);
        }
        return next;
      }
      const maxW = Math.max(1, layoutW - pad * 2);
      const maxH = Math.max(1, layoutH - pad * 2);
      ...
      const next = {
        ...rect,
        x: Math.max(pad, Math.min(Number(rect.x) || 0, layoutW - pad - w)),
        y: Math.max(pad, Math.min(Number(rect.y) || 0, layoutH - pad - h)),
        width: w,
        height: h
      };
      if (
    ...
    /** Carousel slide 1: keep quilt-anchored speaker on-screen after manual nudge. */
    function odqClampCarouselSlide1SpeakerRect(rect, layoutW, layoutH, quiltRect) {
      if (!rect) return rect;
      ...
      const oldX = x;
      const oldY = y;
      x = Math.max(-Math.round(w * 0.15), Math.min(x, layoutW - Math.round(w * 0.15)));
      y = Math.max(-Math.round(h * 0.1), Math.min(y, layoutH - Math.round(h * 0.12)));
      const next = { ...rect, x, y, width: w, height: h };
      const dx = x - oldX;
      const dy = y - oldY;
      if (dx || dy) {
        if (Number.isFinite(Number(rect.contentX))) next.contentX = Number(rect.contentX) + dx;
        if (Number.isFinite(Number(rect.contentY))) next.contentY = Number(rect.contentY) + dy;
        if (Number.isFinite(Number(rect.contentWidth))) next.contentWidth = Number(rect.contentWidth);
        if (Number.isFinite(Number(rect.contentHeight))) next.contentHeight = Number(rect.contentHeight);
        if (Number.isFinite(Number(rect.opaqueAnchorX))) next.opaqueAnchorX = Number(rect.opaqueAnchorX) + dx;
        if (Number.isFinite(Number(rect.opaqueAnchorY))) next.opaqueAnchorY = Number(rect.opaqueAnchorY) + dy;
      }
      return next;
    }

    /** Story 9:16 AUTO: keep the full cutout inside the frame (bottom-right, aspect preserved). */
    function odqFitStorySpeakerRectInCanvas(rect, layoutW, layoutH, minPad = 28) {
      if (!rect) return rect;
      let w = Math.max(1, Number(rect.width) || 0);
      let h = Math.max(1, Number(rect.height) || 0);
      if (!w || !h) return rect;
      ...
      const next = {
        ...rect,
        x: Math.max(pad, layoutW - pad - w),
        y: Math.max(pad, layoutH - pad - h),
        width: w,
        height: h
      };
      if (
    ...
    function odqCarouselStoryStyleSpeakerRectToComposeMeta(seamRect) {
      ...
      const panelW = 1080;
      const overlapPx =
        typeof globalThis.resolveCarouselSpeakerSeamOverlapPx === 'function'
          ? globalThis.resolveCarouselSpeakerSeamOverlapPx(seamRect, panelW)
          : Math.max(0, Math.min(width, Math.round(Number(seamRect.x) + width - panelW)));
      const overlap =
        width > 0
          ? overlapPx / width
          : Math.max(
      ...
      return {
        ...
        opaqueAnchorX: seamRect.opaqueAnchorX,
        opaqueAnchorY: seamRect.opaqueAnchorY,
    ...
    function odqCarouselStoryStyleSpeakerRectAfterTransform(speakerRect, seamRectForMeta) {
      ...
      if (Number.isFinite(Number(seamRectForMeta.opaqueAnchorX))) {
        const ratio = Number(speakerRect.width) / seamRectForMeta.width;
        out.opaqueAnchorX = Math.round(
          Number(speakerRect.x) + (seamRectForMeta.opaqueAnchorX - seamRectForMeta.x) * ratio
        );
        out.opaqueAnchorY = Math.round(
          Number(speakerRect.y) + (seamRectForMeta.opaqueAnchorY - seamRectForMeta.y) * ratio
        );
      }
      return out;
    }
    ...
    const ODQ_CAROUSEL_STORY_STYLE_AUTHOR_CUTOUT_OVERLAP_INTO_X = 14;
    const ODQ_CAROUSEL_STORY_STYLE_AUTHOR_CUTOUT_OVERLAP_UP_PX = 16;

    /** Carousel slide 1: name strip kisses opaque bottom-left ink (tiny overlap, not on face). */
    function odqCarouselStoryStyleAnchorAuthorToCutoutBottomLeft(
      plan,
      layoutW,
      layoutH,
      speakerRect,
      ...
      const content = odqCarouselSpeakerContentRect(speakerRect);
      const opaqueBL = odqCarouselSpeakerOpaqueBottomLeft(speakerRect);
      if (!opaqueBL) return plan;
      const bounds = odqLayoutBPostSafeBounds(layoutW, layoutH, 10);
      const pad = ODQ_CAROUSEL_STRIP_TEXT_SAFE_PAD;
      const layoutSeed = String(opts.layoutSeed || '');
      const stripLayoutSeed = odqNormalizeStripLayoutSeed(opts.stripLayoutSeed);
      const cornerX = Number(opaqueBL.x);
      const cornerY = Number(opaqueBL.y);
      const overlapIntoX = Number.isFinite(Number(opts.overlapIntoX))
        ? Number(opts.overlapIntoX)
        : ODQ_CAROUSEL_STORY_STYLE_AUTHOR_CUTOUT_OVERLAP_INTO_X;
      const overlapUpPx = Number.isFinite(Number(opts.overlapUpPx))
        ? Number(opts.overlapUpPx)
        : ODQ_CAROUSEL_STORY_STYLE_AUTHOR_CUTOUT_OVERLAP_UP_PX;
      const targetRight = cornerX + overlapIntoX;
      const targetTop = cornerY - overlapUpPx;
      ...
      for (let aIdx = 0; aIdx < authors.length; aIdx++) {
        ...
        const fw = odqStripPlanFootprintW(a, pad);
        const fh = odqStripPlanFootprintH(a, pad);
        a.x = targetRight - fw / 2;
        a.y = targetTop + fh / 2;
        a.angle =
          odqPostStripStaggerTiltRad(aIdx + 23, a, layoutSeed, stripLayoutSeed) * 0.22;
        for (let pass = 0; pass < 6; pass++) {
          const ar = odqStripPlanAxisRect(a, pad);
          a.x += targetRight - ar.right;
          a.y += targetTop - ar.top;
        }
        if (content) {
          const faceTop = Number(content.y) + Number(content.height) * 0.52;
          for (let pass = 0; pass < 8; pass++) {
            const ar = odqStripPlanAxisRect(a, pad);
            if (ar.top >= faceTop) break;
            a.y += faceTop - ar.top;
          }
        }
        const fw2 = odqStripPlanFootprintW(a, pad);
        const fh2 = odqStripPlanFootprintH(a, pad);
        if (bounds) {
          a.x = Math.max(bounds.left + fw2 / 2, Math.min(bounds.right - fw2 / 2, a.x));
          a.y = Math.max(bounds.top + fh2 / 2, Math.min(bounds.bottom - fh2 / 2 - 2, a.y));
        }
      }
      return [...quotes, ...authors];
    }
    ...
    function odqApplyPostQuoteStripVerticalSpread(plan, layoutW, layoutH, opts = {}) {
      const carouselSafe = opts.carouselSpeakerAvoidRect
        ...
        : null;
      const needsVerticalCompression =
        opts.carouselSpeakerAvoidRect &&
        odqCarouselStoryStyleNeedsTightVerticalStack(plan, layoutW, layoutH, opts);
      const spreadOpts = {
        minRowGap: Number.isFinite(opts.minRowGap)
          ? opts.minRowGap
          : opts.carouselSpeakerAvoidRect
            ? needsVerticalCompression
              ? ODQ_CAROUSEL_STRIP_TEXT_MIN_GAP
              : 12
            : 10,
      ...
      const longCarouselQuote = needsVerticalCompression;
      ...
      if (opts.carouselSpeakerAvoidRect && typeof globalThis.odqApplyCarouselSpeakerStripLayout === 'function') {
        return globalThis.odqApplyCarouselSpeakerStripLayout(out, layoutW, layoutH, {
          ...opts,
          carouselNeedsVerticalCompression: odqCarouselStoryStyleNeedsTightVerticalStack(
            out,
            layoutW,
            layoutH,
            opts
    ...
    /**
     * Quote-only layout-b post: undo story→post scale, then optionally grow type.
     * Tall / long quotes keep full type (no shrink-to-fit) — strip spacing is tuned manually.
     */
    ...
    function odqCarouselStoryStyleAuthorRowGap(quoteGap) {
      const q = Math.max(0, Number(quoteGap) || 0);
      return Math.max(16, q + ODQ_CAROUSEL_STORY_STYLE_AUTHOR_ROW_GAP_EXTRA);
    }

    /** Greedy row pairing estimate — matches post overflow repacker when side-by-side strips fit. */
    function odqCarouselStoryStyleEstimateQuoteRowHeights(quotes, layoutW, layoutH, quoteText = '') {
      let list = (Array.isArray(quotes) ? quotes : []).filter(
        (s) => s && s.role !== 'author' && !s.authorCutoutLabel
      );
      if (!list.length) return [];
      if (String(quoteText || '').trim()) {
        list = odqSortCarouselQuoteStripsByTextOrder(list, quoteText);
      }
      const safe = odqLayoutBPostSafeBounds(layoutW, layoutH, 10);
      const safeW = safe ? Math.max(120, safe.right - safe.left) : Math.max(120, Number(layoutW) || 1080);
      const rows = [];
      let i = 0;
      while (i < list.length) {
        const cur = list[i];
        const w = Number(cur.w || 300);
        const next = list[i + 1];
        const nw = next ? Number(next.w || 300) : 0;
        if (next && w + 32 + nw <= safeW * 0.88) {
          rows.push(Math.max(Number(cur.h || 0), Number(next.h || 0)));
          i += 2;
        } else {
          rows.push(Math.max(0, Number(cur.h || 0)));
          i += 1;
        }
      }
      return rows;
    ...
    /** True when quote strips cannot fit in the post safe zone at comfortable row gaps (28px) and strip heights. */
    function odqCarouselStoryStyleNeedsTightVerticalStack(plan, layoutW, layoutH, opts = {}) {
      const safe = odqLayoutBPostSafeBounds(layoutW, layoutH, 10);
      if (!safe || !Array.isArray(plan) || plan.length < 2) return false;
      const quotes = plan.filter((s) => s.role !== 'author' && !s.authorCutoutLabel);
      if (quotes.length < 2) return false;
      const edge = ODQ_CAROUSEL_STORY_STYLE_STACK_EDGE_MARGIN;
      const comfortableGap = ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_COMFORTABLE;
      const authors = plan.filter((s) => s.role === 'author' || s.authorCutoutLabel);
      const authorReserve = authors.length
        ? Math.max(...authors.map((a) => Number(a.h || 0))) +
          odqCarouselStoryStyleAuthorRowGap(ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_COMFORTABLE)
        : 0;
      const rowHeights = odqCarouselStoryStyleEstimateQuoteRowHeights(
        quotes,
        layoutW,
        layoutH,
        opts.quoteText
      );
      const totalH = rowHeights.reduce((sum, h) => sum + Math.max(0, Number(h) || 0), 0);
      const needed =
        edge * 2 +
        totalH +
        Math.max(0, rowHeights.length - 1) * comfortableGap +
        authorReserve;
      return needed > safe.bottom - safe.top;
    }

    function odqCarouselStoryStyleAdaptiveRowGap(rowHeights, safeSpan, reserveBottom = 0, opts = {}) {
      const heights = Array.isArray(rowHeights) ? rowHeights : [];
      const edge = ODQ_CAROUSEL_STORY_STYLE_STACK_EDGE_MARGIN;
      const usable = Math.max(
        40,
        Number(safeSpan) - edge * 2 - Math.max(0, Number(reserveBottom) || 0)
      );
      const totalH = heights.reduce((sum, h) => sum + Math.max(0, Number(h) || 0), 0);
      const gapCount = Math.max(0, heights.length - 1);
      const tight = opts.tightStack === true;
      const gapMin = tight
        ? ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_MIN
        : ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_COMFORTABLE;
      const gapMax = tight
        ? ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_MAX
        : ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_COMFORTABLE;
      const stackHeight = (g) => totalH + g * gapCount;
      const comfortableGap = ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_COMFORTABLE;
      let gap;
      if (tight && gapCount > 0) {
        gap = gapMin;
        if (stackHeight(gap) > usable) {
          gap = Math.max(gapMin, Math.min(gapMax, (usable - totalH) / gapCount));
          while (stackHeight(gap) > usable && gap > gapMin) {
            gap = Math.max(gapMin, gap - 1);
          }
        } else if (stackHeight(comfortableGap) <= usable) {
          gap = Math.min(comfortableGap, (usable - totalH) / gapCount);
        }
      } else {
        gap =
          gapCount > 0
            ? (usable - totalH) / gapCount
            : gapMax;
        gap = Math.max(gapMin, Math.min(gapMax, gap));
        while (gapCount > 0 && stackHeight(gap) > usable && gap > gapMin) {
          gap = Math.max(gapMin, gap - 1);
        }
      }
      const leftover = Math.max(0, usable - stackHeight(gap));
      const startOffset = edge + leftover / 2;
      return { gap, startOffset, edgeMargin: edge, usable, stackHeight: stackHeight(gap) };
    }

    /** Preserve X; re-stack quote strip Y so long quotes fit the post safe zone. */
    function odqCarouselStoryStyleCompressQuoteStripYs(plan, layoutW, layoutH, opts = {}) {
      if (!odqCarouselStoryStyleNeedsTightVerticalStack(plan, layoutW, layoutH, opts)) {
        return plan;
      }
      const safe = odqLayoutBPostSafeBounds(layoutW, layoutH, 10);
      if (!safe || !Array.isArray(plan) || plan.length < 2) return plan;
      const quotes = plan.filter((s) => s.role !== 'author' && !s.authorCutoutLabel);
      const authors = plan.filter((s) => s.role === 'author' || s.authorCutoutLabel);
      if (quotes.length < 2) return plan;
      const sorted = quotes
        .slice()
      ...
      for (const s of sorted) {
        ...
        if (last && Math.abs(Number(s.y || 0) - Number(last[0].y || 0)) < 20) last.push(s);
        else rows.push([s]);
      }
      const rowHeights = rows.map((row) => Math.max(...row.map((s) => Number(s.h || 0))));
      const authorReserve = authors.length
        ? Math.max(...authors.map((a) => Number(a.h || 0))) +
          odqCarouselStoryStyleAuthorRowGap(ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_COMFORTABLE)
        : 0;
      const { gap, startOffset, edgeMargin } = odqCarouselStoryStyleAdaptiveRowGap(
        rowHeights,
        safe.bottom - safe.top,
        authorReserve,
        { tightStack: true }
      );
      let cursor = safe.top + startOffset;
      for (let i = 0; i < rows.length; i++) {
        const rowH = rowHeights[i];
        const rowY = cursor + rowH / 2;
        for (const s of rows[i]) s.y = rowY;
        cursor += rowH + (i < rows.length - 1 ? gap : 0);
      }
      if (authors.length) {
        cursor += odqCarouselStoryStyleAuthorRowGap(gap);
        for (const a of authors) {
    ...
    function odqCarouselStoryStyleStackStripsByReadingOrder(strips, quoteText, layoutW, layoutH, opts = {}) {
      const src = String(quoteText || '').trim();
      ...
      const ordered = [
        ...odqSortCarouselQuoteStripsByTextOrder(
          input.filter((s) => s.role !== 'author' && stripTextPosition(s) !== Number.MAX_SAFE_INTEGER),
          quoteText
      ...
      const anchorAuthorToCutout = opts.anchorAuthorToCutout === true;
      const authorStrips = ordered.filter((s) => s.role === 'author');
      const stackStrips = anchorAuthorToCutout
        ? ordered.filter((s) => s.role !== 'author')
        : ordered;
      if (!stackStrips.length) return ordered;
      let authorGapTotal = 0;
      const tightStack = odqCarouselStoryStyleNeedsTightVerticalStack(
        anchorAuthorToCutout ? stackStrips : ordered,
        layoutW,
        layoutH,
        opts
      );
      const estimatedQuoteGap = tightStack
        ? ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_MIN
        : ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_COMFORTABLE;
      for (let i = 0; i < ordered.length - 1; i++) {
        const cur = ordered[i];
        const next = ordered[i + 1];
        if (!anchorAuthorToCutout && cur.role !== 'author' && next.role === 'author') {
          authorGapTotal += odqCarouselStoryStyleAuthorRowGap(estimatedQuoteGap);
        }
      }
      const pairedRowHeights = odqCarouselStoryStyleEstimateQuoteRowHeights(
        stackStrips,
        layoutW,
        layoutH,
        quoteText
      );
      const rowHeights = pairedRowHeights.length
        ? pairedRowHeights
        : stackStrips.map((s) => Number(s.h || 0));
      const { gap: quoteGap, startOffset, edgeMargin } = odqCarouselStoryStyleAdaptiveRowGap(
        rowHeights,
        safe.bottom - safe.top,
        authorGapTotal,
        { tightStack }
      );
      let cursorTop = safe.top + startOffset;
      for (let i = 0; i < stackStrips.length; i++) {
        const s = stackStrips[i];
        const h = Number(s.h || 0);
        s.y = cursorTop + h / 2;
        if (i < stackStrips.length - 1) {
          cursorTop = s.y + h / 2 + quoteGap;
        }
      }
      const lastS = stackStrips[stackStrips.length - 1];
      const overflowBottom = lastS
        ? lastS.y + (lastS.h || 0) / 2 - (safe.bottom - edgeMargin)
        : 0;
      if (overflowBottom > 0) {
        const firstS = ordered[0];
        const headroom = firstS
          ? Math.max(0, firstS.y - (firstS.h || 0) / 2 - (safe.top + edgeMargin))
          : 0;
        const lift = Math.min(overflowBottom, headroom);
        if (lift > 0) {
          for (const s of stackStrips) s.y = Number(s.y || 0) - lift;
        }
      }
      return anchorAuthorToCutout ? [...stackStrips, ...authorStrips] : ordered;
    }

    /**
     * Final carousel quote placement: pack strips left-to-right in reading order, wrapping to a
     * new row (never left/above the previous strip) when a row runs out of width. Overrides the
     * scatter-collage x/y from earlier passes so long quotes stay legible regardless of strip count.
     */
    function odqFlowLayoutCarouselQuoteStrips(plan, quoteText, layoutW, layoutH, opts = {}) {
      const layoutSeed = opts.layoutSeed || '';
      const stripLayoutSeed = opts.stripLayoutSeed || 0;
      const input = Array.isArray(plan) ? plan : [];
      const quotes = input.filter((s) => s.role !== 'author');
      if (quotes.length < 2) return plan;
      const safe = odqLayoutBPostSafeBounds(layoutW, layoutH, 10);
      if (!safe) return plan;
      const authors = input.filter((s) => s.role === 'author');
      const ordered = odqSortCarouselQuoteStripsByTextOrder(quotes, quoteText);
      const safeW = Math.max(80, safe.right - safe.left);
      const stripGapX = 14;
      ...
      for (const s of ordered) {
        ...
        const addW = curRow.length ? stripGapX + w : w;
        if (curRow.length && curRowW + addW > safeW) {
          rows.push(curRow);
          curRow = [s];
          curRowW = w;
        } else {
      ...
      const authorReserve = authors.length
        ? Math.max(...authors.map((a) => Number(a.h || 0))) + 24
        : 0;
      const usableH = Math.max(60, safe.bottom - safe.top - authorReserve);
      let rowGap =
        gapCount > 0
          ? (usableH - totalH) / gapCount
          : ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_COMFORTABLE;
      rowGap = Math.max(
        ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_MIN,
        Math.min(ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_COMFORTABLE, rowGap)
      );
      const stackedH = totalH + rowGap * gapCount;
      const startY = safe.top + Math.max(0, (usableH - stackedH) / 2);

      // Few strips → readable vertical stack; more rows → light collage jitter.
      const jitterT = Math.max(0, Math.min(0.28, (ordered.length - 7) / 16));
      const maxXJitter = 7 * jitterT;
      const maxYJitter = 5 * jitterT;
      ...
      for (let r = 0; r < rows.length; r++) {
        const row = rows[r];
        const rowH = rowHeights[r];
        const rowContentW = row.reduce(
          (sum, s, i) => sum + Number(s.w || 0) + (i ? stripGapX : 0),
          0
        );
        let cursorX = safe.left + Math.max(0, (safeW - rowContentW) / 2);
        for (const s of row) {
          const w = Number(s.w || 0);
          const hash = odqPostStripStaggerHash(layoutSeed, stripLayoutSeed, idx, s.seed || s.role || 'quote');
          const jx = jitterT > 0 ? (odqPostStripStaggerRoll(hash, 3) - 0.5) * 2 * maxXJitter : 0;
          const jy = jitterT > 0 ? (odqPostStripStaggerRoll(hash, 11) - 0.5) * 2 * maxYJitter : 0;
          s.x = cursorX + w / 2 + jx;
          s.y = cursorY + rowH / 2 + jy;
          s.angle = odqPostStripStaggerTiltRad(idx, s, layoutSeed, stripLayoutSeed) * jitterT;
          cursorX += w + stripGapX;
          idx++;
        }
        cursorY += rowH + (r < rows.length - 1 ? rowGap : 0);
      }
      return input;
    }
    ...
    function odqCarouselStoryStyleFinalizeStripSpread(strips, layoutW, layoutH, opts = {}) {
      ...
      const orderedInput = quoteText
        ? [
            ...odqSortCarouselQuoteStripsByTextOrder(
              input.filter((s) => !isAuthorStrip(s) && stripTextPosition(s) !== Number.MAX_SAFE_INTEGER),
              quoteText
            ).map((s) => ({ ...s, rowGroup: undefined, rowSlot: undefined })),
            ...input
      ...
      out = odqCarouselStoryStyleStackStripsByReadingOrder(out, quoteText, layoutW, layoutH, {
        carouselShortQuote:
          opts.carouselShortQuote === true || opts.spreadOpts?.carouselShortQuote === true,
        anchorAuthorToCutout: !!opts.carouselStoryStyleSpeakerSeamRect
      });
    ...
    /** Bottom-left opaque ink on canvas — real silhouette touch point, not bbox corner. */
    function odqCarouselSpeakerOpaqueBottomLeft(speakerRect) {
      if (!speakerRect) return null;
      if (
        Number.isFinite(Number(speakerRect.opaqueAnchorX)) &&
        Number.isFinite(Number(speakerRect.opaqueAnchorY))
      ) {
        return {
          x: Number(speakerRect.opaqueAnchorX),
          y: Number(speakerRect.opaqueAnchorY)
        };
      }
      const content = odqCarouselSpeakerContentRect(speakerRect);
      if (content) {
        return { x: Number(content.x), y: Number(content.y) + Number(content.height) };
      }
      const frame = odqCarouselSpeakerCutoutFrameRect(speakerRect);
      if (frame) {
        return { x: Number(frame.x), y: Number(frame.y) + Number(frame.height) };
      }
      return null;
    }
    ...
    /** Max fractional nudge offset (±100% of canvas width/height). */
    const ODQ_SPEAKER_NUDGE_MIN = -1;
    const ODQ_SPEAKER_NUDGE_MAX = 1;
    /** Preset center (% of canvas) after nudge — allow mostly off-screen placement. */
    const ODQ_SPEAKER_CENTER_PCT_MIN = -0.5;
    const ODQ_SPEAKER_CENTER_PCT_MAX = 1.5;
    ...
    function odqNormalizeSpeakerNudgeComponent(raw) {
      ...
      return Math.round(Math.min(ODQ_SPEAKER_NUDGE_MAX, Math.max(ODQ_SPEAKER_NUDGE_MIN, n)) * 1000) / 1000;
    }

    function odqSpeakerTransformAllowsOffCanvas(speakerTransform) {
      if (!speakerTransform) return false;
      if (speakerTransform.autoRelative) {
        return !!(
          odqNormalizeSpeakerNudgeComponent(speakerTransform.nudgeCx) ||
          odqNormalizeSpeakerNudgeComponent(speakerTransform.nudgeCy)
        );
      }
      const cx = Number(speakerTransform.cxPct);
      const cy = Number(speakerTransform.cyPct);
      return (
        (Number.isFinite(cx) && (cx < 0.02 || cx > 0.98)) ||
        (Number.isFinite(cy) && (cy < 0.02 || cy > 0.98))
      );
    }
    ...
    function odqReadSpeakerCutoutTweakFromLocal(dateKey, aspect = 'story') {
      ...
      const a = odqNormalizeTuneAspect(aspect);
      const keys = [ODQ_SPEAKER_TWEAK_KEY(key, a)];
      if (a === 'story') keys.push(`odq.speakerCutoutTweak.${key}`);
      let local = { preset: 'AUTO', nudgeCx: 0, nudgeCy: 0, nudgeRotateDeg: 0, nudgeScale: 1, updatedAt: '' };
      try {
        for (const storageKey of keys) {
          const raw = localStorage.getItem(storageKey);
          if (!raw) continue;
          local = odqParseSpeakerCutoutTweakRaw(JSON.parse(raw));
          break;
        }
      } catch {
        /* ignore */
      ...
      const mem = odqSpeakerCutoutTweakMemoryCache[cacheId];
      if (!mem || !odqSpeakerCutoutTweakIsCustomized(mem)) return local;
      if (!odqSpeakerCutoutTweakIsCustomized(local)) return mem;
      if (odqIsoTimestampIsNewer(mem.updatedAt, local.updatedAt)) return mem;
      return local;
    }
    ...
    async function odqReadSpeakerCutoutTweak(dateKey, aspect = 'story') {
      ...
      const readInstagramDoc = async (preferServer) => {
        ...
        const readSnap = async (readFn) => {
          const snapPromise = readFn(ref);
          return odqPromiseWithTimeout(snapPromise, 5000, `Speaker tweak ${a} read`);
        };
        if (preferServer && typeof window.firestore.getDocFromServer === 'function') {
          try {
            return await readSnap(window.firestore.getDocFromServer.bind(window.firestore));
          } catch (serverErr) {
            console.warn('odqReadSpeakerCutoutTweak server read failed, using cache:', serverErr);
          }
        }
        return readSnap(window.firestore.getDoc.bind(window.firestore));
      };
      try {
        ...
        if (remoteCustom && !localCustom) {
          ...
          // layoutBTuneUpdatedAt can advance without speaker fields (keywords/strips/nightly);
          // never drop a customized local story/post placement for a non-custom remote doc.
          useRemote = false;
        } else if (remoteCustom && localCustom) {
    ...
    /** Pull the full Layout B tune doc into local caches before opening the tune modal. */
    async function odqPrefetchLayoutBTuneFields(dateKey, timeoutMs = 8000) {
      const key = String(dateKey || '').trim();
      if (!key) return { ok: false, reason: 'dateKey required' };
      const read = await odqReadInstagramImagesDocWithFallback(key, timeoutMs);
      const data = read?.data || null;
      if (!data) return { ok: false, reason: 'doc-missing', source: read?.source || '' };
      const localTuneAt = odqGetLayoutBTuneLocalUpdatedAt(key);
      const remoteTuneAt = String(data.layoutBTuneUpdatedAt || '').trim();
      /** Phone saved after last cloud sync — keep local caches (incl. strip arrangement). */
      if (localTuneAt && !odqIsoTimestampIsNewer(remoteTuneAt, localTuneAt)) {
        const localPlan = odqGetCachedLayoutBPostStripPlan(key);
        const remotePlan = data.layoutBPostStripPlan;
        if (
          (!Array.isArray(localPlan) || !localPlan.length) &&
          Array.isArray(remotePlan) &&
          remotePlan.length
        ) {
          odqSetCachedLayoutBPostStripPlan(
            key,
            remotePlan,
            data.layoutBPostStripPlanUpdatedAt || remoteTuneAt
          );
        }
        await Promise.all(
          ['story', 'post'].map((aspect) =>
            odqReadSpeakerCutoutTweak(key, aspect).catch((err) => {
              console.warn(`odqPrefetchLayoutBTuneFields speaker (${aspect}) failed:`, err);
              return null;
            })
          )
        );
        return {
          ok: true,
          source: 'local-newer',
          layoutBTuneUpdatedAt: localTuneAt
        };
      }
      await Promise.all(
        ['story', 'post'].map((aspect) =>
      ...
      for (const aspect of ['story', 'post']) {
        odqApplyLayoutBTuneFieldsFromInstagramDoc(key, data, aspect);
        odqHydrateSpeakerCutoutTweakFromInstagramDoc(key, data, aspect);
      }
      const savedStripPlan = data.layoutBPostStripPlan;
      const remotePlanAt = String(
        data.layoutBPostStripPlanUpdatedAt || data.layoutBTuneUpdatedAt || ''
      ).trim();
      const localPack = odqGetCachedLayoutBPostStripPlanPack(key);
      if (Array.isArray(savedStripPlan) && savedStripPlan.length) {
        if (
          !localPack.plan?.length ||
          odqIsoTimestampIsNewer(remotePlanAt, localPack.updatedAt)
        ) {
          odqSetCachedLayoutBPostStripPlan(key, savedStripPlan, remotePlanAt);
        }
      }
      return { ok: true, source: read?.source || 'unknown', layoutBTuneUpdatedAt: data.layoutBTuneUpdatedAt || null };
    }
    ...
    function odqWriteSpeakerCutoutPreset(dateKey, presetName, aspect = 'story', nudge) {
      ...
      if (!presetValid && !hasTweak) {
        ...
        delete odqSpeakerCutoutTweakMemoryCache[odqTuneAspectCacheId(String(dateKey || '').trim(), aspect)];
        return;
      }
      ...
      const cacheId = odqTuneAspectCacheId(String(dateKey || '').trim(), aspect);
      odqSpeakerCutoutTweakMemoryCache[cacheId] = {
    ...
    function odqSpeakerCutoutTransformResolved(presetName, nudge) {
      ...
      const out = { ...base };
      if (dx && Number.isFinite(+out.cxPct)) {
        out.cxPct = clamp(out.cxPct + dx, ODQ_SPEAKER_CENTER_PCT_MIN, ODQ_SPEAKER_CENTER_PCT_MAX);
      }
      if (dy && Number.isFinite(+out.cyPct)) {
        out.cyPct = clamp(out.cyPct + dy, ODQ_SPEAKER_CENTER_PCT_MIN, ODQ_SPEAKER_CENTER_PCT_MAX);
      }
    ...
    /** Compact signature for Layout B story preview cache busting (speaker placement tweaks). */
    function odqSpeakerCutoutTuneExportSignature(dateKey, aspect = 'story') {
      const t = odqReadSpeakerCutoutTweakFromLocal(dateKey, aspect);
      return [
        String(t.preset || 'AUTO').trim().toUpperCase(),
        odqNormalizeSpeakerNudgeComponent(t.nudgeCx),
        odqNormalizeSpeakerNudgeComponent(t.nudgeCy),
        odqNormalizeSpeakerRotateDeg(t.nudgeRotateDeg),
        odqNormalizeSpeakerScaleMul(t.nudgeScale)
      ].join(':');
    }

    /** Layout B tune fields for quilt-screen story preview cache busting (story aspect). */
    function odqLayoutBTuneExportSignature(dateKey, aspect = 'story') {
      const a = odqNormalizeTuneAspect(aspect);
      const speakerSig = odqSpeakerCutoutTuneExportSignature(dateKey, a);
      const stripSeed = odqNormalizeStripLayoutSeed(odqGetCachedLayoutBStripLayoutSeed(dateKey, a) ?? 0);
      const quiltZoom = odqNormalizeQuiltBgZoom(
        odqGetCachedLayoutBQuiltBgZoom(dateKey, a) ?? ODQ_LAYOUT_B_QUILT_BG_ZOOM_MIN
      );
      const quiltOffsetY = odqNormalizeQuiltBgOffsetY(odqGetCachedLayoutBQuiltBgOffsetY(dateKey, a) ?? 0);
      const stripOffset = odqNormalizeQuoteStripOffset(
        odqGetCachedLayoutBQuoteStripOffset(dateKey, a) ?? { x: 0, y: 0 }
      );
      const styleAmount = odqNormalizeSpeakerStyleAmount(
        odqGetCachedLayoutBSpeakerStyleAmount(dateKey) ?? 1
      );
      const kw = odqGetCachedLayoutBKeywordEmphasis(dateKey, a);
      const kwSig =
        kw?.keywords?.length ? JSON.stringify({ k: kw.keywords, s: kw.styles || [] }) : '';
      return [
        speakerSig,
        stripSeed,
        quiltZoom,
    ...
    const ODQ_QUILT_BG_OFFSET_Y_KEY = (dateKey, aspect) =>
      `odq.layoutBQuiltBgOffsetY.${String(dateKey || '').trim()}.${odqNormalizeTuneAspect(aspect)}`;
    ...
    /** Feed carousel slide 1 quote-clipping position (fraction of 4:5 canvas). */
    const ODQ_CAROUSEL_CLIPPING_OFFSET_KEY = (dateKey) =>
      `odq.layoutBCarouselClippingOffset.${String(dateKey || '').trim()}`;
    /** Manual / captured post strip arrangement (x/y/angle + type) — survives reopen when cloud save fails. */
    const ODQ_POST_STRIP_PLAN_KEY = (dateKey) =>
      `odq.layoutBPostStripPlan.${String(dateKey || '').trim()}`;
    /** Phone-only tune save timestamp — prefetch must not clobber newer local caches. */
    const ODQ_LAYOUT_B_TUNE_LOCAL_UPDATED_AT_KEY = (dateKey) =>
      `odq.layoutBTuneLocalUpdatedAt.${String(dateKey || '').trim()}`;
    /** Per-day speaker Xerox/wash strength (shared story+post — about the image). */
    const ODQ_SPEAKER_STYLE_AMOUNT_KEY = (dateKey) =>
      `odq.layoutBSpeakerStyleAmount.${String(dateKey || '').trim()}`;
    ...
    const odqQuiltBgOffsetYMemoryCache = Object.create(null);
    const odqQuoteStripOffsetMemoryCache = Object.create(null);
    const odqCarouselClippingOffsetMemoryCache = Object.create(null);
    const odqPostStripPlanMemoryCache = Object.create(null);
    const odqPostStripPlanUpdatedAtMemoryCache = Object.create(null);
    const odqSpeakerCutoutTweakMemoryCache = Object.create(null);
    const odqSpeakerStyleAmountMemoryCache = Object.create(null);

    function odqGetLayoutBTuneLocalUpdatedAt(dateKey) {
      const key = String(dateKey || '').trim();
      if (!key) return '';
      try {
        return String(localStorage.getItem(ODQ_LAYOUT_B_TUNE_LOCAL_UPDATED_AT_KEY(key)) || '').trim();
      } catch {
        return '';
      }
    }

    function odqSetLayoutBTuneLocalUpdatedAt(dateKey, updatedAt) {
      const key = String(dateKey || '').trim();
      const at = String(updatedAt || '').trim();
      if (!key || !at) return;
      try {
        localStorage.setItem(ODQ_LAYOUT_B_TUNE_LOCAL_UPDATED_AT_KEY(key), at);
      } catch (_) {
        /* ignore */
      }
    }

    function odqGetCachedLayoutBPostStripPlanPack(dateKey) {
      const key = String(dateKey || '').trim();
      if (!key) return { plan: null, updatedAt: '' };
      const memPlan = odqPostStripPlanMemoryCache[key];
      const memAt = String(odqPostStripPlanUpdatedAtMemoryCache[key] || '').trim();
      if (Array.isArray(memPlan) && memPlan.length) {
        return { plan: memPlan, updatedAt: memAt };
      }
      try {
        const raw = localStorage.getItem(ODQ_POST_STRIP_PLAN_KEY(key));
        if (!raw) return { plan: null, updatedAt: '' };
        ...
        const plan = Array.isArray(parsed)
          ...
          : Array.isArray(parsed?.plan)
            ? parsed.plan
            : null;
        const updatedAt = Array.isArray(parsed)
          ? ''
          : String(parsed?.updatedAt || '').trim();
        if (Array.isArray(plan) && plan.length) {
          odqPostStripPlanMemoryCache[key] = plan;
          if (updatedAt) odqPostStripPlanUpdatedAtMemoryCache[key] = updatedAt;
          return { plan, updatedAt };
        }
      } catch (_) {
        /* ignore */
      }
      return { plan: null, updatedAt: '' };
    }

    function odqGetCachedLayoutBPostStripPlan(dateKey) {
      return odqGetCachedLayoutBPostStripPlanPack(dateKey).plan;
    }

    function odqSetCachedLayoutBPostStripPlan(dateKey, stripPlan, updatedAt) {
      const key = String(dateKey || '').trim();
      if (!key) return;
      const compact = odqCompactLayoutBPostStripPlanForFirestore(stripPlan);
      if (!compact?.length) {
        delete odqPostStripPlanMemoryCache[key];
        delete odqPostStripPlanUpdatedAtMemoryCache[key];
        try {
          localStorage.removeItem(ODQ_POST_STRIP_PLAN_KEY(key));
        } catch (_) {
          /* ignore */
        }
        return;
      }
      const at = String(updatedAt || '').trim() || new Date().toISOString();
      odqPostStripPlanMemoryCache[key] = compact;
      odqPostStripPlanUpdatedAtMemoryCache[key] = at;
      try {
        localStorage.setItem(
          ODQ_POST_STRIP_PLAN_KEY(key),
          JSON.stringify({ updatedAt: at, plan: compact })
        );
      } catch (_) {
        /* memory still holds plan */
      }
    }

    /** Pull speaker preset/nudge/rotate from instagram-images into memory + localStorage. */
    function odqHydrateSpeakerCutoutTweakFromInstagramDoc(dateKey, data, aspect = 'story') {
      const key = String(dateKey || '').trim();
      if (!key || !data || typeof data !== 'object') return null;
      const a = odqNormalizeTuneAspect(aspect);
      const remote = odqSpeakerCutoutTweakFromInstagramData(data, a);
      const cacheId = odqTuneAspectCacheId(key, a);
      odqSpeakerCutoutTweakMemoryCache[cacheId] = remote;
      if (odqSpeakerCutoutTweakIsCustomized(remote)) {
        odqWriteSpeakerCutoutPreset(key, remote.preset, a, {
          cx: remote.nudgeCx,
          cy: remote.nudgeCy,
          rotateDeg: remote.nudgeRotateDeg,
          nudgeScale: remote.nudgeScale,
          updatedAt: remote.updatedAt || data.layoutBTuneUpdatedAt || undefined
        });
      }
      return remote;
    }

    function odqSpeakerCutoutTransformFromInstagramData(data, aspect = 'story') {
      const remote = odqSpeakerCutoutTweakFromInstagramData(data, aspect);
      return odqSpeakerCutoutTransformResolved(remote.preset, {
        cx: remote.nudgeCx,
        cy: remote.nudgeCy,
        rotateDeg: remote.nudgeRotateDeg,
        nudgeScale: remote.nudgeScale
    ...
    function odqPostSpeakerTuneDiffersFromStory(tuneDraftByAspect) {
      const p = tuneDraftByAspect?.post || {};
      const s = tuneDraftByAspect?.story || {};
      return (
        String(p.preset || 'AUTO').trim().toUpperCase() !== String(s.preset || 'AUTO').trim().toUpperCase() ||
        odqNormalizeSpeakerNudgeComponent(p.nudgeCx) !== odqNormalizeSpeakerNudgeComponent(s.nudgeCx) ||
        odqNormalizeSpeakerNudgeComponent(p.nudgeCy) !== odqNormalizeSpeakerNudgeComponent(s.nudgeCy) ||
        odqNormalizeSpeakerRotateDeg(p.nudgeRotateDeg) !== odqNormalizeSpeakerRotateDeg(s.nudgeRotateDeg) ||
        odqNormalizeSpeakerScaleMul(p.nudgeScale) !== odqNormalizeSpeakerScaleMul(s.nudgeScale)
      );
    }
    ...
    /** 0 = soft face detail, 1 = full Xerox/wash (default). */
    function odqNormalizeSpeakerStyleAmount(raw) {
      const SCR = root.SpeakerCutoutRender || globalThis.SpeakerCutoutRender;
      if (typeof SCR?.normalizeSpeakerStyleAmount === 'function') {
        return SCR.normalizeSpeakerStyleAmount(raw);
      }
      const n = Number(raw);
    ...
    function odqFormatSpeakerStyleAmountLabel(amount) {
      const a = odqNormalizeSpeakerStyleAmount(amount);
      if (a >= 0.999) return 'Full';
      if (a <= 0.001) return 'Soft';
      return `${Math.round(a * 100)}%`;
    }

    function odqGetCachedLayoutBSpeakerStyleAmount(dateKey) {
      const key = String(dateKey || '').trim();
      if (!key) return undefined;
      if (Object.prototype.hasOwnProperty.call(odqSpeakerStyleAmountMemoryCache, key)) {
        return odqSpeakerStyleAmountMemoryCache[key];
      }
      try {
        const raw = localStorage.getItem(ODQ_SPEAKER_STYLE_AMOUNT_KEY(key));
        if (raw == null) return undefined;
        const a = odqNormalizeSpeakerStyleAmount(raw);
        odqSpeakerStyleAmountMemoryCache[key] = a;
        return a;
      } catch {
        return undefined;
      }
    ...
    function odqSetCachedLayoutBSpeakerStyleAmount(dateKey, amount) {
      const key = String(dateKey || '').trim();
      if (!key) return;
      const a = odqNormalizeSpeakerStyleAmount(amount);
      odqSpeakerStyleAmountMemoryCache[key] = a;
      try {
        const storageKey = ODQ_SPEAKER_STYLE_AMOUNT_KEY(key);
        if (a < 0.999) {
          localStorage.setItem(storageKey, String(a));
        } else {
          localStorage.removeItem(storageKey);
        }
      } catch (_) {
    ...
    async function odqReadLayoutBSpeakerStyleAmount(dateKey) {
      const key = String(dateKey || '').trim();
      if (!key) return 1;
      const cached = odqGetCachedLayoutBSpeakerStyleAmount(key);
      if (cached != null) return cached;
      try {
        if (typeof odqReadInstagramImagesDocWithFallback === 'function') {
          const read = await odqReadInstagramImagesDocWithFallback(key, 5000);
          if (read?.data && read.data.layoutBSpeakerStyleAmount != null) {
            const a = odqNormalizeSpeakerStyleAmount(read.data.layoutBSpeakerStyleAmount);
            odqSetCachedLayoutBSpeakerStyleAmount(key, a);
            return a;
          }
        }
      } catch (_) {
    ...
    async function odqWriteLayoutBSpeakerStyleAmount(dateKey, amount) {
      const key = String(dateKey || '').trim();
      if (!key) return;
      const a = odqNormalizeSpeakerStyleAmount(amount);
      odqSetCachedLayoutBSpeakerStyleAmount(key, a);
    ...
    /** Same fraction range as strip offset — used for Zapier feed slide 1 clipping nudge. */
    function odqNormalizeCarouselClippingOffset(raw) {
      return odqNormalizeQuoteStripOffset(raw);
    }
    ...
    function odqNormalizeQuiltBgOffsetY(raw) {
      const n = Number(raw);
      if (!Number.isFinite(n) || Math.abs(n) < 0.0005) return 0;
      return Math.round(Math.min(0.35, Math.max(-0.35, n)) * 1000) / 1000;
    }

    function odqFormatQuiltBgOffsetYLabel(offsetY) {
      const y = odqNormalizeQuiltBgOffsetY(offsetY);
      if (!y) return 'Centered';
    ...
    function odqGetCachedLayoutBQuiltBgOffsetY(dateKey, aspect = 'story') {
      const cacheId = odqTuneAspectCacheId(dateKey, aspect);
      if (!cacheId || cacheId.startsWith(':')) return undefined;
      if (Object.prototype.hasOwnProperty.call(odqQuiltBgOffsetYMemoryCache, cacheId)) {
        return odqQuiltBgOffsetYMemoryCache[cacheId];
      }
      try {
        const raw = localStorage.getItem(ODQ_QUILT_BG_OFFSET_Y_KEY(dateKey, aspect));
        if (raw == null) return undefined;
        const y = odqNormalizeQuiltBgOffsetY(raw);
        odqQuiltBgOffsetYMemoryCache[cacheId] = y;
        return y;
      } catch {
        return undefined;
      }
    ...
    function odqSetCachedLayoutBQuiltBgOffsetY(dateKey, offsetY, aspect = 'story') {
      const cacheId = odqTuneAspectCacheId(dateKey, aspect);
      if (!cacheId || cacheId.startsWith(':')) return;
      const y = odqNormalizeQuiltBgOffsetY(offsetY);
      odqQuiltBgOffsetYMemoryCache[cacheId] = y;
      try {
        const storageKey = ODQ_QUILT_BG_OFFSET_Y_KEY(dateKey, aspect);
        if (y) {
    ...
    function odqGetCachedLayoutBCarouselClippingOffset(dateKey) {
      const key = String(dateKey || '').trim();
      if (!key) return undefined;
      if (Object.prototype.hasOwnProperty.call(odqCarouselClippingOffsetMemoryCache, key)) {
        return odqCarouselClippingOffsetMemoryCache[key];
      }
      try {
        const raw = localStorage.getItem(ODQ_CAROUSEL_CLIPPING_OFFSET_KEY(key));
        if (raw == null) return undefined;
        const offset = odqNormalizeCarouselClippingOffset(JSON.parse(raw));
        odqCarouselClippingOffsetMemoryCache[key] = offset;
        return offset;
      } catch {
        return undefined;
      }
    ...
    function odqSetCachedLayoutBCarouselClippingOffset(dateKey, offset) {
      const key = String(dateKey || '').trim();
      if (!key) return;
      const next = odqNormalizeCarouselClippingOffset(offset);
      odqCarouselClippingOffsetMemoryCache[key] = next;
      try {
        const storageKey = ODQ_CAROUSEL_CLIPPING_OFFSET_KEY(key);
        if (next.x || next.y) {
          localStorage.setItem(storageKey, JSON.stringify(next));
        } else {
          localStorage.removeItem(storageKey);
    ...
    async function odqReadLayoutBCarouselClippingOffset(dateKey) {
      const key = String(dateKey || '').trim();
      ...
      if (window.db && window.firestore) {
        try {
          ...
          if (snap.exists()) {
            const data = snap.data() || {};
            const offset = odqNormalizeCarouselClippingOffset({
              x: data.layoutBCarouselClippingOffsetX,
              y: data.layoutBCarouselClippingOffsetY
            });
            odqSetCachedLayoutBCarouselClippingOffset(key, offset);
            return offset;
          }
        } catch (err) {
          console.warn('odqReadLayoutBCarouselClippingOffset Firestore read failed:', err);
        }
      }
      return odqGetCachedLayoutBCarouselClippingOffset(key) ?? { x: 0, y: 0 };
    }

    async function odqWriteLayoutBCarouselClippingOffset(dateKey, offset, writeOptions = {}) {
      const key = String(dateKey || '').trim();
      if (!key) throw new Error('dateKey required');
      const normalized = odqNormalizeCarouselClippingOffset(offset);
      odqSetCachedLayoutBCarouselClippingOffset(key, normalized);
      ...
      if (!normalized.x && !normalized.y && typeof deleteFieldFn === 'function') {
        patch.layoutBCarouselClippingOffsetX = deleteFieldFn();
        patch.layoutBCarouselClippingOffsetY = deleteFieldFn();
        patch.layoutBCarouselClippingOffsetXUpdatedAt = deleteFieldFn();
        patch.layoutBCarouselClippingOffsetYUpdatedAt = deleteFieldFn();
        patch.layoutBCarouselClippingOffsetXUpdatedBy = deleteFieldFn();
        patch.layoutBCarouselClippingOffsetYUpdatedBy = deleteFieldFn();
      } else {
        patch.layoutBCarouselClippingOffsetX = normalized.x;
        patch.layoutBCarouselClippingOffsetY = normalized.y;
        patch.layoutBCarouselClippingOffsetXUpdatedAt = now;
        patch.layoutBCarouselClippingOffsetYUpdatedAt = now;
        patch.layoutBCarouselClippingOffsetXUpdatedBy = 'admin-tune-modal';
        patch.layoutBCarouselClippingOffsetYUpdatedBy = 'admin-tune-modal';
      }
    ...
    async function odqReadLayoutBQuiltBgZoom(dateKey, aspect = 'story') {
      ...
      if (window.db && window.firestore) {
        try {
          ...
          if (snap.exists()) {
            const data = snap.data() || {};
            if (Object.prototype.hasOwnProperty.call(data, firestoreField)) {
    ...
    async function odqReadLayoutBQuiltBgOffsetY(dateKey, aspect = 'story') {
      const key = String(dateKey || '').trim();
      if (!key) return 0;
      const a = odqNormalizeTuneAspect(aspect);
      const cacheId = odqTuneAspectCacheId(key, a);
      ...
      const firestoreField = a === 'post' ? 'layoutBQuiltBgOffsetYPost' : 'layoutBQuiltBgOffsetYStory';
      if (window.db && window.firestore) {
        try {
          const ref = window.firestore.doc(window.db, 'instagram-images', key);
          const snap = await window.firestore.getDoc(ref);
          if (snap.exists()) {
            const data = snap.data() || {};
            if (Object.prototype.hasOwnProperty.call(data, firestoreField)) {
              const y = odqNormalizeQuiltBgOffsetY(data[firestoreField]);
              odqSetCachedLayoutBQuiltBgOffsetY(key, y, a);
              return y;
            }
          }
        } catch (err) {
          console.warn('odqReadLayoutBQuiltBgOffsetY Firestore read failed:', err);
        }
      }
      return odqGetCachedLayoutBQuiltBgOffsetY(key, a) ?? 0;
    }
      ...
      if (window.db && window.firestore) {
        try {
          ...
          if (snap.exists()) {
            ...
            if (
              Object.prototype.hasOwnProperty.call(data, xField) ||
              Object.prototype.hasOwnProperty.call(data, yField)
            ) {
    ...
    async function odqWriteLayoutBQuiltBgOffsetY(dateKey, offsetY, aspect = 'story', writeOptions = {}) {
      const key = String(dateKey || '').trim();
      if (!key) throw new Error('dateKey required');
      const a = odqNormalizeTuneAspect(aspect);
      const y = odqNormalizeQuiltBgOffsetY(offsetY);
      ...
      if (!window.db || !window.firestore) throw new Error('Firestore not ready');
      const ref = window.firestore.doc(window.db, 'instagram-images', key);
      const field = a === 'post' ? 'layoutBQuiltBgOffsetYPost' : 'layoutBQuiltBgOffsetYStory';
      const deleteFieldFn = window.firestore.deleteField;
      if (!y) {
        if (typeof deleteFieldFn === 'function') {
          const patch = {
    ...
    /** Post quote style is independent when Post fields differ or were saved at/after Story. */
    function odqLayoutBPostQuoteStyleOverridden(data, kind) {
      if (!data || typeof data !== 'object') return false;
      if (data.layoutBPostTuneIndependent === true) return true;
      ...
      if (kind === 'strip') {
        const postSeed = odqNormalizeStripLayoutSeed(data.layoutBStripLayoutSeedPost);
        const storySeed = odqLayoutBStoryStripSeedFromDoc(data);
        if (postSeed !== storySeed) return true;
      } else {
        const postKw = JSON.stringify(data.layoutBKeywordEmphasisPost || null);
        const storyKw = JSON.stringify(odqLayoutBStoryKeywordRawFromDoc(data) || null);
        if (postKw !== storyKw) return true;
      }
    ...
    function odqApplyLayoutBTuneFieldsFromInstagramDoc(dateKey, data, aspect = 'story') {
      ...
      if (!key || !data || typeof data !== 'object') {
        return { keywordEmphasis: null, stripLayoutSeed: 0, quiltBgZoom: ODQ_LAYOUT_B_QUILT_BG_ZOOM_MIN, quiltBgOffsetY: 0 };
      }
      const a = odqNormalizeTuneAspect(aspect);
      ...
      const quiltBgOffsetY = odqNormalizeQuiltBgOffsetY(
        data[a === 'post' ? 'layoutBQuiltBgOffsetYPost' : 'layoutBQuiltBgOffsetYStory']
      );
      odqSetCachedLayoutBQuiltBgOffsetY(key, quiltBgOffsetY, a);
      const suffix = a === 'post' ? 'Post' : 'Story';
      ...
      const carouselClippingOffset = odqNormalizeCarouselClippingOffset({
        x: data.layoutBCarouselClippingOffsetX,
        y: data.layoutBCarouselClippingOffsetY
      });
      odqSetCachedLayoutBCarouselClippingOffset(key, carouselClippingOffset);
      if (data.layoutBSpeakerStyleAmount != null) {
        odqSetCachedLayoutBSpeakerStyleAmount(
          key,
          odqNormalizeSpeakerStyleAmount(data.layoutBSpeakerStyleAmount)
        );
      }
      odqHydrateSpeakerCutoutTweakFromInstagramDoc(key, data, a);
      return {
        keywordEmphasis,
        stripLayoutSeed,
        quiltBgZoom,
        quiltBgOffsetY,
        quoteStripOffset,
        carouselClippingOffset,
        speakerStyleAmount: odqNormalizeSpeakerStyleAmount(
          odqGetCachedLayoutBSpeakerStyleAmount(key) ?? 1
        )
      };
    }
    ...
    async function odqWriteLayoutBPostStripPlan(dateKey, stripPlan, writeOptions = {}) {
      ...
      if (!stripPlan || !stripPlan.length) {
        odqSetCachedLayoutBPostStripPlan(key, null, now);
        if (!window.db || !window.firestore) throw new Error('Firestore not ready');
      ...
      odqSetCachedLayoutBPostStripPlan(key, stripPlan, now);
      if (!window.db || !window.firestore) throw new Error('Firestore not ready');
      const ref = window.firestore.doc(window.db, 'instagram-images', key);
      const compact = odqCompactLayoutBPostStripPlanForFirestore(stripPlan);
      if (!compact) return;
    ...
    async function odqReadLayoutBPostStripPlan(dateKey) {
      ...
      if (window.db && window.firestore) {
        try {
          const ref = window.firestore.doc(window.db, 'instagram-images', key);
          const readDoc =
            typeof window.firestore.getDocFromServer === 'function'
              ? window.firestore.getDocFromServer.bind(window.firestore)
              : window.firestore.getDoc.bind(window.firestore);
          const snap = await readDoc(ref);
          if (snap.exists()) {
            const data = snap.data() || {};
            const plan = data.layoutBPostStripPlan;
            const remoteTuneAt = String(data.layoutBTuneUpdatedAt || '').trim();
            /** Strip freshness only — do not use layoutBTuneUpdatedAt (speaker-only saves). */
            const remotePlanAt = String(data.layoutBPostStripPlanUpdatedAt || '').trim();
            /**
             * Phone saved at/after last cloud sync — keep local strips (splits/tilts).
             * Matches odqPrefetchLayoutBTuneFields local-newer short-circuit.
             */
            if (
              localPack.plan?.length &&
              localTuneAt &&
              !odqIsoTimestampIsNewer(remoteTuneAt, localTuneAt)
            ) {
              return localPack.plan;
            }
            if (Array.isArray(plan) && plan.length) {
              if (
                !localPack.plan?.length ||
                (remotePlanAt && odqIsoTimestampIsNewer(remotePlanAt, localPack.updatedAt))
              ) {
                odqSetCachedLayoutBPostStripPlan(key, plan, remotePlanAt || remoteTuneAt);
                return plan;
              }
            }
          }
    ...
    globalThis.odqSetCachedLayoutBPostStripPlan = odqSetCachedLayoutBPostStripPlan;
    globalThis.odqGetCachedLayoutBPostStripPlan = odqGetCachedLayoutBPostStripPlan;
    globalThis.odqSetLayoutBTuneLocalUpdatedAt = odqSetLayoutBTuneLocalUpdatedAt;
    globalThis.odqGetLayoutBTuneLocalUpdatedAt = odqGetLayoutBTuneLocalUpdatedAt;
    ...
    function odqBuildLayoutBTuneFirestorePayload(dateKey, tuneDraftByAspect, updatedAt, options = {}) {
      const key = String(dateKey || '').trim();
      if (!key) throw new Error('dateKey required');
      const now = String(updatedAt || '').trim() || new Date().toISOString();
      const aspects = Array.isArray(options.aspects) && options.aspects.length
        ? options.aspects.map((a) => odqNormalizeTuneAspect(a)).filter(Boolean)
        : ['story', 'post'];
      const set = {
        layoutBTuneUpdatedAt: now,
        layoutBTuneUpdatedBy: 'admin-tune-modal',
      ...
      if (
        options.postTuneIndependent === true ||
        odqPostSpeakerTuneDiffersFromStory(tuneDraftByAspect)
      ) {
        set.layoutBPostTuneIndependent = true;
        set.layoutBPostTuneIndependentUpdatedAt = now;
      }
      ...
      for (const aspect of aspects) {
        ...
        const quiltOffsetYField =
          a === 'post' ? 'layoutBQuiltBgOffsetYPost' : 'layoutBQuiltBgOffsetYStory';
        ...
        const quiltOffsetY = odqNormalizeQuiltBgOffsetY(d.quiltBgOffsetY);
        if (!quiltOffsetY) {
          del(quiltOffsetYField);
          del(`${quiltOffsetYField}UpdatedAt`);
          del(`${quiltOffsetYField}UpdatedBy`);
        } else {
          set[quiltOffsetYField] = quiltOffsetY;
          set[`${quiltOffsetYField}UpdatedAt`] = now;
          set[`${quiltOffsetYField}UpdatedBy`] = 'admin-tune-modal';
        }
      ...
      const styleAmountRaw =
        tuneDraftByAspect?.story?.speakerStyleAmount ??
        tuneDraftByAspect?.post?.speakerStyleAmount ??
        1;
      const styleAmount = odqNormalizeSpeakerStyleAmount(styleAmountRaw);
      odqSetCachedLayoutBSpeakerStyleAmount(key, styleAmount);
      if (styleAmount >= 0.999) {
        del('layoutBSpeakerStyleAmount');
        del('layoutBSpeakerStyleAmountUpdatedAt');
        del('layoutBSpeakerStyleAmountUpdatedBy');
      } else {
        set.layoutBSpeakerStyleAmount = styleAmount;
        set.layoutBSpeakerStyleAmountUpdatedAt = now;
        set.layoutBSpeakerStyleAmountUpdatedBy = 'admin-tune-modal';
      }

      if (!aspects.includes('post')) {
        return { set, deleteFields };
      }

      const clippingOffset = odqNormalizeCarouselClippingOffset(
        tuneDraftByAspect?.post?.carouselClippingOffset
      );
      if (!clippingOffset.x && !clippingOffset.y) {
        del('layoutBCarouselClippingOffsetX');
        del('layoutBCarouselClippingOffsetY');
        del('layoutBCarouselClippingOffsetXUpdatedAt');
        del('layoutBCarouselClippingOffsetYUpdatedAt');
        del('layoutBCarouselClippingOffsetXUpdatedBy');
        del('layoutBCarouselClippingOffsetYUpdatedBy');
      } else {
        set.layoutBCarouselClippingOffsetX = clippingOffset.x;
        set.layoutBCarouselClippingOffsetY = clippingOffset.y;
        set.layoutBCarouselClippingOffsetXUpdatedAt = now;
        set.layoutBCarouselClippingOffsetYUpdatedAt = now;
        set.layoutBCarouselClippingOffsetXUpdatedBy = 'admin-tune-modal';
        set.layoutBCarouselClippingOffsetYUpdatedBy = 'admin-tune-modal';
      }

      return { set, deleteFields };
    }

    /** Positions + typography so nightly/export matches tune-modal preview (legacy docs: x/y/angle only). */
    function odqCompactLayoutBPostStripPlanForFirestore(stripPlan) {
    ...
    async function odqWriteLayoutBTuneViaServer(dateKey, tuneDraftByAspect, updatedAt, extra = {}) {
      const bases =
      ...
      const { set, deleteFields } = odqBuildLayoutBTuneFirestorePayload(
        ...
        now,
        {
          ...
          postTuneIndependent: extra.postTuneIndependent
        }
      );
      const compactStripPlan = odqCompactLayoutBPostStripPlanForFirestore(extra.postStripPlan);
      if (compactStripPlan) {
        set.layoutBPostStripPlan = compactStripPlan;
        set.layoutBPostStripPlanUpdatedAt = now;
      }
    ...
    async function odqVerifyLayoutBTuneOnServer(dateKey, tuneDraftByAspect) {
      ...
      for (const aspect of ['story', 'post']) {
        ...
        const quiltOffsetYField =
          aspect === 'post' ? 'layoutBQuiltBgOffsetYPost' : 'layoutBQuiltBgOffsetYStory';
        const stripOffsetXField =
        ...
        const serverQuiltBgOffsetY = odqNormalizeQuiltBgOffsetY(data[quiltOffsetYField]);
        ...
        if (serverQuiltBgOffsetY) fields.push(quiltOffsetYField);
        ...
        const expectedQuiltBgOffsetY = odqNormalizeQuiltBgOffsetY(d.quiltBgOffsetY);
        if (Math.abs(expectedQuiltBgOffsetY - serverQuiltBgOffsetY) > 0.0005) {
          mismatches.push(`${aspect}-quilt-offset-y`);
        }
      ...
      const serverClippingOffset = odqNormalizeCarouselClippingOffset({
        x: data.layoutBCarouselClippingOffsetX,
        y: data.layoutBCarouselClippingOffsetY
      });
      if (serverClippingOffset.x || serverClippingOffset.y) {
        fields.push('layoutBCarouselClippingOffsetX', 'layoutBCarouselClippingOffsetY');
      }
      const expectedClippingOffset = odqNormalizeCarouselClippingOffset(
        tuneDraftByAspect?.post?.carouselClippingOffset
      );
      if (
        Math.abs(expectedClippingOffset.x - serverClippingOffset.x) > 0.0005 ||
        Math.abs(expectedClippingOffset.y - serverClippingOffset.y) > 0.0005
      ) {
        mismatches.push('post-clipping-offset');
      }
      return {
    ...
    function odqKeywordEmphasisFromQuoteObject(quote, dateKey) {
      ...
      const hasKeywordField =
        Object.prototype.hasOwnProperty.call(quote, 'keyword') ||
        Object.prototype.hasOwnProperty.call(quote, 'keywordSnapshot');
      const rawKeyword = String(quote.keyword ?? quote.keywordSnapshot ?? '').trim();
      if (hasKeywordField && !rawKeyword) return ODQ_LAYOUT_B_NO_KEYWORD;
    ...
    const ODQ_LAYOUT_B_NO_KEYWORD = Object.freeze({ noLayoutBKeyword: true });

    function odqIsLayoutBNoKeyword(value) {
      return value === ODQ_LAYOUT_B_NO_KEYWORD || value?.noLayoutBKeyword === true;
    }
    ...
    async function odqResolveLayoutBQuoteKeywordEmphasis(dateKey, quoteHint = null) {
      ...
      for (const candidate of candidates) {
        ...
        if (odqIsLayoutBNoKeyword(fromObject)) return fromObject;
    ...
    async function odqKeywordEmphasisFromQuoteFirestore(dateKey) {
      ...
      const tryDoc = (data, textOverride) => {
        ...
        const hasKeywordField =
          Object.prototype.hasOwnProperty.call(data, 'keyword') ||
          Object.prototype.hasOwnProperty.call(data, 'keywordSnapshot');
        const raw = String(data.keyword ?? data.keywordSnapshot ?? '').trim();
        if (hasKeywordField && !raw && text) return ODQ_LAYOUT_B_NO_KEYWORD;
      ...
      if (assignmentData) {
        ...
        if (sid && sid !== dateKey) {
          try {
            ...
            if (srcSnap.exists()) {
              ...
              if (odqIsLayoutBNoKeyword(fromCatalog)) return fromCatalog;
        ...
        if (odqIsLayoutBNoKeyword(fromAssign)) return fromAssign;
      ...
      for (const collectionName of collections) {
        try {
          ...
          if (odqIsLayoutBNoKeyword(built)) return built;
    ...
    async function odqReadLayoutBKeywordEmphasis(dateKey, aspect = 'story', quoteHint = null) {
      ...
      if (odqIsLayoutBNoKeyword(fromQuote)) {
        odqSetCachedLayoutBKeywordEmphasis(key, null, a);
        return null;
      }
    ...
    async function composeInstagramLayoutBFromQuiltBlob(highResBlob, quoteText, quoteAuthor, layoutW, layoutH, dateKey, options = {}) {
      ...
      const skipLayoutBStripKeywords = odqLayoutBIgStripsSkipKeywordEmphasis();
      let resolvedKeywordEmphasis = null;
      if (!skipLayoutBStripKeywords) {
      ...
      let resolvedQuiltBgOffsetY = 0;
      if (options.quiltBgOffsetYExplicit) {
        resolvedQuiltBgOffsetY = odqNormalizeQuiltBgOffsetY(options.quiltBgOffsetY);
      } else if (options.quiltBgOffsetY != null) {
        resolvedQuiltBgOffsetY = odqNormalizeQuiltBgOffsetY(options.quiltBgOffsetY);
      } else {
        try {
          resolvedQuiltBgOffsetY = await odqReadLayoutBQuiltBgOffsetY(rngDateKey, tuneAspect);
        } catch (_) {
          resolvedQuiltBgOffsetY = odqGetCachedLayoutBQuiltBgOffsetY(rngDateKey, tuneAspect) ?? 0;
        }
      }
      let resolvedSpeakerStyleAmount = 1;
      if (options.styleAmountExplicit || options.speakerStyleAmountExplicit) {
        resolvedSpeakerStyleAmount = odqNormalizeSpeakerStyleAmount(
          options.styleAmount ?? options.speakerStyleAmount
        );
      } else if (options.styleAmount != null || options.speakerStyleAmount != null) {
        resolvedSpeakerStyleAmount = odqNormalizeSpeakerStyleAmount(
          options.styleAmount ?? options.speakerStyleAmount
        );
      } else {
        try {
          resolvedSpeakerStyleAmount = await odqReadLayoutBSpeakerStyleAmount(rngDateKey);
        } catch (_) {
          resolvedSpeakerStyleAmount = odqNormalizeSpeakerStyleAmount(
            odqGetCachedLayoutBSpeakerStyleAmount(rngDateKey) ?? 1
          );
        }
      }
      ...
      return new Promise((resolve, reject) => {
            const STORY_W = layoutW;
            const STORY_H = layoutH;
            const storyPanelPreview = options.storyPanelPreview === true;
            /** Tune modal keeps fast path; quilt-screen story preview needs full strip type. */
            const tunePreviewFast = options.tunePreviewFast === true && !storyPanelPreview;
            const previewScale = tunePreviewFast ? odqTuneModalPreviewScale() : 1;
            const transparentOverlay = options.transparentOverlay === true;
            ...
            /** Carousel post tune preview must use same shrink loop as export (story-only fast path stays at 4). */
            const carouselShrinkPasses =
              tunePreviewFast && !(carouselStoryStyle && isPostLayout) ? 4 : 16;
            ...
            // blow past what fits at the carousel's fixed min font size. Based on the
            // full quote (not per-sentence — splitIntoPhraseSegments runs once per sentence).
            const phraseSegmentTakeLo =
              layoutQuoteText.length > 230 ? 4 : layoutQuoteText.length > 165 ? 3 : 2;
            const splitIntoPhraseSegments = (text) => {
            ...
            const loadSpeakerOverlayImages = async (url) => {
              ...
              if (!sourceUrl) return [];
              if (
                options.skipSpeakerImageNetwork === true &&
                !/^(?:data|blob):/i.test(sourceUrl)
              ) {
                return [];
              }
            ...
            /** Story overlay + post: strip-only finish softens type without washing quilt colors. */
            const applyLayoutBMagazinePrintFinishOnStrips = (g, specs) => {
              if (!Array.isArray(specs) || !specs.length || tunePreviewFast) return;
              for (const spec of specs) {
                const cx = Number(spec.x) || 0;
                const cy = Number(spec.y) || 0;
                const sw = Math.max(1, Number(spec.w) || 0);
                const sh = Math.max(1, Number(spec.h) || 0);
                const ang = Number(spec.angle) || 0;
                if (!(sw > 0 && sh > 0)) continue;
                g.save();
                g.translate(cx, cy);
                g.rotate(ang);
                handCutStripPath(g, sw, sh, rnd);
                g.clip();
                g.translate(-sw / 2, -sh / 2);
                applyLayoutBMagazinePrintFinish(g, sw, sh);
                g.restore();
              }
            };
            ...
            const drawPaperStrip = (g, cx, cy, w, h, angle, fillPaper, stripVariant = 'light') => {
              ...
              if (!tunePreviewFast) {
                if (stripVariant === 'dark') {
                  ...
                  g.shadowOffsetX = 2;
                  g.shadowOffsetY = 3;
                } else if (stripVariant === 'bright') {
                  /** Carousel slide 1: lighter contact shadow — quilt grain pass follows. */
            ...
            const drawSpeakerOverlay = (g, img, rect, washColor, opts = {}) => {
              ...
              return SCR.drawSpeakerCutoutStack(g, img, rect, {
                ...
                styleAmount:
                  opts.styleAmount != null
                    ? odqNormalizeSpeakerStyleAmount(opts.styleAmount)
                    : resolvedSpeakerStyleAmount,
                palette:
                  opts.palette ||
            ...
            const getSpeakerOverlayRect = (img, strips) => {
              ...
              const hardAreaMax = canvasArea * ODQ_LAYOUT_B_SPEAKER_AUTO_MAX_AREA_FRAC;
              const targetArea = canvasArea * ODQ_LAYOUT_B_SPEAKER_AUTO_TARGET_AREA_FRAC;
              let w = Math.sqrt(targetArea * aspect);
              let h = w / aspect;
              const canvasPad = 28;
              const maxW = STORY_W - canvasPad * 2;
              const maxH = STORY_H - canvasPad * 2;
              ...
              if (!carouselStoryStyle) {
                return odqFitStorySpeakerRectInCanvas(fitted, STORY_W, STORY_H, minPad);
              }
              return fitted;
            };
            ...
            loadQuiltImage()
              .then(async (quiltImg) => {
                ...
                const carouselSlide1GreenMat =
                  carouselStoryStyle && isPostLayout && !stripPlanOnly && !transparentOverlay;
                let carouselCuttingMatImg = null;
                let carouselPaperTextureImg = null;
                if (carouselSlide1GreenMat) {
                  const MatBg = globalThis.CarouselCuttingMatBg;
                  if (MatBg?.loadCarouselCuttingMatImage) {
                    carouselCuttingMatImg = await MatBg.loadCarouselCuttingMatImage();
                  }
                  const PaperDepth = globalThis.IgCarouselPaperDepth;
                  if (PaperDepth?.loadPaperTextureImage) {
                    carouselPaperTextureImg = await PaperDepth.loadPaperTextureImage();
                  }
                }
                const carouselQuiltMatPeekPx =
                  carouselSlide1GreenMat &&
                  globalThis.CarouselQuiltBg?.CAROUSEL_SLIDE1_MAT_PEEK_PX != null
                    ? globalThis.CarouselQuiltBg.CAROUSEL_SLIDE1_MAT_PEEK_PX
                    : 0;
                const segmentQuoteForLayout = (text) => {
                  const raw = String(text ?? '');
                ...
                if (!transparentOverlay && !carouselSlide1GreenMat) {
                ...
                const quiltBgOffsetY = resolvedQuiltBgOffsetY;
                const quiltBgOffsetYPx = Math.round(odqNormalizeQuiltBgOffsetY(quiltBgOffsetY) * STORY_H);
                ...
                if (!stripPlanOnly && !transparentOverlay) {
                if (isPostLayout) {
                  ...
                  if (CQB?.resolveLayoutBCarouselQuiltRect && CQB?.drawLayoutBCarouselQuiltBg) {
                    const resolved = CQB.resolveLayoutBCarouselQuiltRect(quiltImg, STORY_W, STORY_H, {
                      ...
                      quiltMatPeekPx: carouselQuiltMatPeekPx
                    });
                    ...
                    CQB.drawLayoutBCarouselQuiltBg(
                      ...
                      STORY_H,
                      {
                        ...
                        greenCuttingMatUnderQuilt: carouselSlide1GreenMat,
                        quiltMatPeekPx: carouselQuiltMatPeekPx,
                        cuttingMatImg: carouselCuttingMatImg,
                        paperTextureImg: carouselPaperTextureImg,
                        paperToothSeed: dateKey || options?.dateKey || 'carousel-slide-1',
                        matPanX: globalThis.CarouselCuttingMatBg?.CAROUSEL_MAT_PAN?.SLIDE_1 ?? 0,
                        deferPaperTooth: carouselSlide1GreenMat
                      }
                    );
                    if (
                      focusBlockRect &&
                  ...
                  if (dedicationBox) {
                    ...
                    dy = Math.round(dy + quiltBgOffsetYPx);
                  ...
                  if (options?.quiltFit === 'contain') {
                    const scale = Math.min(STORY_W / iw, STORY_H / ih) * quiltBgZoom;
                    const qw = Math.round(iw * scale);
                    const qh = Math.round(ih * scale);
                    const qx = Math.round((STORY_W - qw) / 2);
                    const qy = Math.round((STORY_H - qh) / 2 + quiltBgOffsetYPx);
                    ctx.drawImage(quiltImg, qx, qy, qw, qh);
                  } else {
                    ...
                    const qy = Math.round(rect.y - (ch - rect.height) / 2 + quiltBgOffsetYPx);
                ...
                const drawStripContent = (spec, cx, cy, jitterA, stripVariant = 'light') => {
                  const renderFont = odqLayoutBStripFontForRender(spec.font, isPostLayout, spec.role);
                  let paper;
                  let ink;
                  ...
                  if (stripVariant === 'dark') {
                    ...
                    paper = carouselStoryStyle ? ODQ_CAROUSEL_STRIP_PAPER_FILL : PAPER_FILL;
                    ink = '#404040';
                    paperTexVariant = carouselStoryStyle ? 'bright' : 'light';
                  }
                  ...
                  const drawRunsLineAt = (ox, oy, y, fillStyle, runFilter = null) => {
                    ...
                    for (const run of spec.textRuns) {
                      ...
                      ctx.font = odqLayoutBStripFontForRender(
                        LBKE.layoutBFontForEmphasis(basePx, FONT_SERIF, runFlags),
                        isPostLayout,
                        spec.role
                      );
                      odqSetLayoutBStripLetterSpacing(ctx, basePx);
                  ...
                  const drawLinesAt = (ox, oy, drawTy) => {
                    ...
                    let t = drawTy;
                    ctx.font = renderFont;
                    const stripPx = parseInt(/(\d+)px/.exec(String(renderFont || ''))?.[1] || '48', 10);
                  ...
                  odqSetLayoutBStripLetterSpacing(
                    ctx,
                    parseInt(/(\d+)px/.exec(String(renderFont || ''))?.[1] || '48', 10)
                  );
                ...
                let storyTitleStripFinishSpec = null;
                /** Declared early so speakerHeroPost early-return can call finishLayoutBCanvas safely. */
                let quoteStripPlan = null;
                const finishLayoutBCanvas = () => {
                  drawDedicationMessageBox(ctx, dedicationBox);
                  /** Overlay-only PNGs stack on quilt-screen; full-canvas print finish reads as milky wash. */
                  if (!isPostLayout && !tunePreviewFast && transparentOverlay) {
                    const stripFinishSpecs = [
                      ...(Array.isArray(quoteStripPlan) ? quoteStripPlan : []),
                      ...(storyTitleStripFinishSpec ? [storyTitleStripFinishSpec] : [])
                    ];
                    applyLayoutBMagazinePrintFinishOnStrips(ctx, stripFinishSpecs);
                  } else if (!isPostLayout && !tunePreviewFast && !transparentOverlay) {
                    applyLayoutBMagazinePrintFinish(ctx, STORY_W, STORY_H);
                  } else if (isPostLayout && Array.isArray(quoteStripPlan) && quoteStripPlan.length) {
                    applyLayoutBMagazinePrintFinishOnStrips(ctx, quoteStripPlan);
                  }
                  const composeMeta = {
                ...
                if (options.speakerHeroPost === true) {
                  ...
                  if (speakerImgHero) {
                    let heroRect = getSpeakerHeroPostRect(speakerImgHero);
                    if (heroRect && nameSpec) {
                      heroRect = odqConstrainSpeakerRectToNameStripWidth(heroRect, [
                        { ...nameSpec, role: 'author' }
                      ]);
                    }
                    if (heroRect) {
                ...
                // quoteStripPlan already declared above (speakerHeroPost-safe).
                const carouselQuiltRectForSpeaker = () => {
                  if (quiltCanvasRect) return quiltCanvasRect;
                  if (!carouselStoryStyle || !quiltImg) return null;
                  const CQB = globalThis.CarouselQuiltBg;
                  if (!CQB?.resolveLayoutBCarouselQuiltRect) return null;
                  return (
                    CQB.resolveLayoutBCarouselQuiltRect(quiltImg, STORY_W, STORY_H, {
                      quiltBgZoom,
                      quiltBgOffsetY: resolvedQuiltBgOffsetY,
                      quiltFit: options?.quiltFit,
                      dedicationBoxHeight: dedicationBox?.height || 0,
                      layoutBQuiltBleed: isPostLayout ? 1.04 : layoutBQuiltBleed / quiltBgZoom,
                      quiltMatPeekPx: carouselQuiltMatPeekPx
                ...
                const cachedStoryRefPlan = Array.isArray(options.storyRefStripPlan)
                  ...
                  : null;
                /** Feed slide 1 clipping alternate: skip all strip planning (quilt + speaker only). */
                if (options.omitQuoteStrips === true) {
                  quoteStripPlan = [];
                } else if (
                  inheritStoryStripPlanForPost &&
                  cachedStoryRefPlan &&
                  ...
                  if (carouselStoryStyle && quoteStripPlan.length) {
                    ...
                    const kwCarousel =
                      skipLayoutBStripKeywords
                        ? null
                        : options.keywordEmphasisExplicit
                    ...
                    if (
                      odqCarouselStoryStyleNeedsTightVerticalStack(quoteStripPlan, STORY_W, STORY_H, {
                        quoteText: layoutQuoteText
                      })
                    ) {
                      quoteStripPlan = refitLayoutBStripPlanToText(
                    ...
                    for (let shrinkPass = 0; shrinkPass < carouselShrinkPasses; shrinkPass++) {
                      const needsVerticalCompression = odqCarouselStoryStyleNeedsTightVerticalStack(
                        quoteStripPlan,
                        STORY_W,
                        STORY_H,
                        { quoteText: layoutQuoteText }
                      ...
                      quoteStripPlan = odqShrinkStripPlanTypographyUntilPostSafe(
                        ...
                        FONT_SERIF,
                        {
                          carouselSpeakerAvoidRect,
                          quoteText: layoutQuoteText,
                          authorCutoutLabel: needsVerticalCompression
                        }
                      );
                      ...
                      quoteStripPlan = refitLayoutBStripPlanToText(
                        ...
                        carouselStripMaxW,
                        FONT_SERIF,
                        needsVerticalCompression ? odqCarouselLongQuoteRefitOpts() : {}
                      );
                    }
                  }
                } else if (
                  ...
                  quoteStripPlan = getLayoutBStoryQuotePlan(ctx, {
                    ...
                    maxQuoteCanvasCoverage: null,
                    keywordEmphasis: skipLayoutBStripKeywords
                      ? null
                      : options.keywordEmphasisExplicit
                        ? options.keywordEmphasis || null
                        : resolvedKeywordEmphasis || null,
                    skipKeywordEmphasis:
                      skipLayoutBStripKeywords || options.skipKeywordEmphasis === true,
                    stripLayoutSeed: resolvedStripLayoutSeed,
                  ...
                  if (!isPostLayout && speakerActive && quoteStripPlan.length) {
                    if (Array.isArray(options.savedPostStripPlan) && options.savedPostStripPlan.length) {
                      odqApplySavedLayoutBStripPlanPositionsByReadingOrder(
                        quoteStripPlan,
                        options.savedPostStripPlan,
                        layoutQuoteText,
                        layoutBPostAuthorName || quoteAuthor
                      );
                    }
                  }
                ...
                for (const speakerImg of speakerImgs) {
                  ...
                  if (carouselStoryStyle) {
                    ...
                    speakerRect = seamRectForMeta
                      ? {
                          ...
                          opaqueAnchorX: seamRectForMeta.opaqueAnchorX,
                          opaqueAnchorY: seamRectForMeta.opaqueAnchorY
                        }
                  ...
                  if (speakerRect && !carouselStoryStyle) {
                    const allowOffCanvas =
                      tunePreviewFast || odqSpeakerTransformAllowsOffCanvas(speakerTransform);
                    speakerRect = odqClampStorySpeakerRectInCanvas(
                      speakerRect,
                      STORY_W,
                      STORY_H,
                      28,
                  ...
                  } else if (speakerRect && carouselStoryStyle) {
                    speakerRect = odqClampCarouselSlide1SpeakerRect(
                      speakerRect,
                      STORY_W,
                      STORY_H,
                      carouselQuiltRectForSpeaker()
                ...
                const carouselTightVerticalStack =
                  carouselStoryStyle &&
                  odqCarouselStoryStyleNeedsTightVerticalStack(quoteStripPlan, STORY_W, STORY_H, {
                    quoteText: layoutQuoteText
                  });
                if (carouselStoryStyle && carouselTightVerticalStack && quoteStripPlan.length) {
                  quoteStripPlan = refitLayoutBStripPlanToText(
                    odqCompactCarouselLongQuoteStripTypography(quoteStripPlan),
                    ctx,
                    STRIP_MAX_W,
                ...
                if (carouselStoryStyle && quoteStripPlan.length) {
                  quoteStripPlan = odqApplyCarouselStoryStyleAuthorFontScale(
                    ...
                    STRIP_MAX_W,
                    FONT_SERIF,
                    { tightStack: carouselTightVerticalStack }
                  );
                }
                const savedPostStripPlan = Array.isArray(options.savedPostStripPlan)
                  ? options.savedPostStripPlan
                  : null;
                /** Tune modal: replay manual x/y even when strip count drifted — merge by reading order. */
                const replaySavedCarouselStripPlan =
                  options.replaySavedCarouselStripPlan === true &&
                  carouselStoryStyle &&
                  isPostLayout &&
                  quoteStripPlan.length > 0 &&
                  Array.isArray(savedPostStripPlan) &&
                  savedPostStripPlan.length > 0;
                if (
                  carouselStoryStyle &&
                  isPostLayout &&
                  quoteStripPlan.length
                ) {
                  quoteStripPlan = odqCarouselStoryStyleStackStripsByReadingOrder(
                    quoteStripPlan,
                    layoutQuoteText,
                    STORY_W,
                    STORY_H,
                    {
                      carouselShortQuote: options.carouselShortQuote === true,
                      anchorAuthorToCutout: !!carouselStoryStyleSpeakerSeamRect
                    }
                  );
                }
                if (carouselStoryStyle && isPostLayout && quoteStripPlan.length) {
                  quoteStripPlan = odqCarouselStoryStyleCompressQuoteStripYs(
                    quoteStripPlan,
                    STORY_W,
                    STORY_H,
                    {
                ...
                /** Post/carousel: tune quoteStripOffset can push the top strip off-canvas (story already clamps). */
                if (isPostLayout && quoteStripPlan.length) {
                  quoteStripPlan = odqClampLayoutBStoryStripPlanToCanvas(
                    quoteStripPlan,
                    STORY_W,
                    STORY_H,
                    carouselStoryStyle ? 14 : 20
                  );
                }
                if (!isPostLayout && quoteStripPlan.length) {
                  quoteStripPlan = odqResolveLayoutBStripPlanOverlaps(
                    quoteStripPlan,
                    STORY_W,
                    STORY_H,
                    12,
                  ...
                  if (speakerActive) {
                    quoteStripPlan = odqSpreadStoryAuthorNameBelowQuotes(
                      quoteStripPlan,
                      STORY_W,
                      STORY_H,
                      { speakerActive: true }
                ...
                if (
                  ...
                  lastSpeakerRectDrawn &&
                  quoteStripPlan.length &&
                  !replaySavedCarouselStripPlan
                ) {
                  quoteStripPlan = odqCarouselStoryStyleAnchorAuthorToCutoutBottomLeft(
                    quoteStripPlan,
                    STORY_W,
                    STORY_H,
                    lastSpeakerRectDrawn,
                ...
                if (
                  ...
                  const carouselSafe = odqLayoutBPostSafeBounds(STORY_W, STORY_H, 10);
                  if (carouselSafe) {
                    quoteStripPlan = odqResolveLayoutBStripPlanOverlaps(
                      quoteStripPlan,
                      STORY_W,
                      ...
                      10,
                      {
                        x: carouselSafe.left,
                        y: carouselSafe.top,
                        width: carouselSafe.right - carouselSafe.left,
                        height: carouselSafe.bottom - carouselSafe.top
                      }
                    );
                  }
                }
                if (
                  ...
                  quoteStripPlan = odqFlowLayoutCarouselQuoteStrips(
                    quoteStripPlan,
                    layoutQuoteText,
                    STORY_W,
                    STORY_H,
                    { layoutSeed: rngDateKey, stripLayoutSeed: resolvedStripLayoutSeed }
                  );
                  const postFlowNeedsFix = () =>
                    odqLayoutBStripPlanHasTextOverflow(quoteStripPlan, ctx, FONT_SERIF) ||
                    odqStripPlanHasOverlaps(quoteStripPlan, ODQ_CAROUSEL_STRIP_TEXT_MIN_GAP);
                  if (postFlowNeedsFix()) {
                    const postFlowMaxStripW = odqCarouselStoryStyleStripMaxWidth(
                      STORY_W,
                      STORY_H,
                      carouselStoryStyleSpeakerSeamRect,
                      STRIP_MAX_W
                    );
                    for (let postFlowPass = 0; postFlowPass < carouselShrinkPasses + 2; postFlowPass++) {
                      if (!postFlowNeedsFix()) break;
                      const shrinkPx = Math.max(
                        ODQ_CAROUSEL_STORY_STYLE_MIN_BODY_PX,
                        Math.round(odqLayoutBStripPlanQuoteBodyPx(quoteStripPlan) * 0.88)
                      );
                      quoteStripPlan = odqBuildUniformCarouselStoryStripPlan(
                        quoteStripPlan,
                        shrinkPx,
                        ctx,
                        postFlowMaxStripW,
                ...
                if (!replaySavedCarouselStripPlan && (quoteStripDx || quoteStripDy)) {
                  quoteStripPlan = quoteStripPlan.map((s) => {
                    if (carouselStoryStyle && s.role === 'author') return s;
                    return {
                      ...s,
                ...
                if (replaySavedCarouselStripPlan) {
                  if (odqLayoutBStripPlanHasTypography(savedPostStripPlan)) {
                    quoteStripPlan = odqCloneLayoutBStripPlan(savedPostStripPlan);
                    for (const s of quoteStripPlan) {
                      if (s.role === 'author') s.authorCutoutLabel = true;
                    }
                    quoteStripPlan = odqClampLayoutBStoryStripPlanToCanvas(
                      quoteStripPlan,
                      STORY_W,
                  ...
                  const savedPlanStoryMaxStripW = odqCarouselStoryStyleStripMaxWidth(
                    STORY_W,
                    STORY_H,
                    carouselStoryStyleSpeakerSeamRect,
                    STRIP_MAX_W
                  ...
                  quoteStripPlan = quoteStripPlan.map((s) => {
                    if (s.role === 'author') return s;
                    const stripMaxW = Math.max(
                      120,
                      Math.min(Number(s.w) || savedPlanStoryMaxStripW, savedPlanStoryMaxStripW)
                    );
                    return refitLayoutBStripPlanToText([s], ctx, stripMaxW, FONT_SERIF, {
                      hardMinBodyPx: ODQ_CAROUSEL_STORY_STYLE_MIN_BODY_PX
                    })[0];
                  });
                  quoteStripPlan = odqNormalizeCarouselStoryStyleUniformTypography(
                    quoteStripPlan,
                    ctx,
                    savedPlanStoryMaxStripW,
                    FONT_SERIF,
                    { noWrap: true }
                  );
                  const savedPlanSafe = odqLayoutBPostSafeBounds(STORY_W, STORY_H, 10);
                  if (savedPlanSafe) {
                    quoteStripPlan = odqResolveLayoutBStripPlanOverlaps(
                      quoteStripPlan,
                      STORY_W,
                      ...
                      10,
                      {
                        x: savedPlanSafe.left,
                        y: savedPlanSafe.top,
                        width: savedPlanSafe.right - savedPlanSafe.left,
                        height: savedPlanSafe.bottom - savedPlanSafe.top
                      }
                    );
                  }
                  quoteStripPlan = odqClampLayoutBStoryStripPlanToCanvas(
                ...
                /** Carousel slide 1 quote-clipping alternate: quilt + speaker only (no paper strips / name). */
                if (options.omitQuoteStrips === true) {
                  quoteStripPlan = [];
                }
                ...
                if (isPostLayout && quoteStripPlan.length) {
                  quoteStripPlan = quoteStripPlan.map((s) =>
                    s.role === 'quote'
                      ? { ...s, font: odqLayoutBStripFontForRender(s.font, true, 'quote') }
                      : s
                  );
                }
                ...
                if (
                  carouselSlide1GreenMat &&
                  carouselPaperTextureImg &&
                  globalThis.CarouselQuiltBg?.drawLayoutBCarouselQuiltPaperTooth
                ) {
                  globalThis.CarouselQuiltBg.drawLayoutBCarouselQuiltPaperTooth(
                    ctx,
                    STORY_W,
                ...
                if (!isPostLayout) {
                  ...
                  const titleAngle = (rnd() - 0.5) * 0.045;
                  storyTitleStripFinishSpec = {
                    x: cxStrip,
                    y: cyStrip,
                    w: titleSpec.w,
                    h: titleSpec.h,
                  ...
                  drawStripContent(titleSpec, cxStrip, cyStrip, titleAngle, 'title-invert');
                }
                finishLayoutBCanvas();
              })
              .catch(reject);
    ...
    globalThis.odqLayoutBStripPlanHasTypography = odqLayoutBStripPlanHasTypography;
    globalThis.odqApplySavedLayoutBStripPlanPositionsByReadingOrder =
      odqApplySavedLayoutBStripPlanPositionsByReadingOrder;
    ...
    globalThis.odqPrefetchLayoutBTuneFields = odqPrefetchLayoutBTuneFields;
    globalThis.odqSpeakerCutoutTransformForDateAsync = odqSpeakerCutoutTransformForDateAsync;
    globalThis.odqSpeakerCutoutTransformFromInstagramData = odqSpeakerCutoutTransformFromInstagramData;
    globalThis.odqHydrateSpeakerCutoutTweakFromInstagramDoc = odqHydrateSpeakerCutoutTweakFromInstagramDoc;
    globalThis.odqPostSpeakerTuneDiffersFromStory = odqPostSpeakerTuneDiffersFromStory;
    globalThis.odqSpeakerCutoutTransformResolved = odqSpeakerCutoutTransformResolved;
    globalThis.odqSpeakerCutoutTuneExportSignature = odqSpeakerCutoutTuneExportSignature;
    globalThis.odqLayoutBTuneExportSignature = odqLayoutBTuneExportSignature;
    ...
    globalThis.odqReadSpeakerCutoutTweak = odqReadSpeakerCutoutTweak;
    ...
    globalThis.odqNormalizeSpeakerStyleAmount = odqNormalizeSpeakerStyleAmount;
    globalThis.odqFormatSpeakerStyleAmountLabel = odqFormatSpeakerStyleAmountLabel;
    globalThis.odqGetCachedLayoutBSpeakerStyleAmount = odqGetCachedLayoutBSpeakerStyleAmount;
    globalThis.odqSetCachedLayoutBSpeakerStyleAmount = odqSetCachedLayoutBSpeakerStyleAmount;
    globalThis.odqReadLayoutBSpeakerStyleAmount = odqReadLayoutBSpeakerStyleAmount;
    globalThis.odqWriteLayoutBSpeakerStyleAmount = odqWriteLayoutBSpeakerStyleAmount;
    globalThis.odqNormalizeQuiltBgOffsetY = odqNormalizeQuiltBgOffsetY;
    globalThis.odqFormatQuiltBgOffsetYLabel = odqFormatQuiltBgOffsetYLabel;
    globalThis.odqGetCachedLayoutBQuiltBgOffsetY = odqGetCachedLayoutBQuiltBgOffsetY;
    globalThis.odqSetCachedLayoutBQuiltBgOffsetY = odqSetCachedLayoutBQuiltBgOffsetY;
    globalThis.odqReadLayoutBQuiltBgOffsetY = odqReadLayoutBQuiltBgOffsetY;
    globalThis.odqWriteLayoutBQuiltBgOffsetY = odqWriteLayoutBQuiltBgOffsetY;
    ...
    globalThis.odqNormalizeCarouselClippingOffset = odqNormalizeCarouselClippingOffset;
    globalThis.odqGetCachedLayoutBCarouselClippingOffset = odqGetCachedLayoutBCarouselClippingOffset;
    globalThis.odqSetCachedLayoutBCarouselClippingOffset = odqSetCachedLayoutBCarouselClippingOffset;
    globalThis.odqReadLayoutBCarouselClippingOffset = odqReadLayoutBCarouselClippingOffset;
    globalThis.odqWriteLayoutBCarouselClippingOffset = odqWriteLayoutBCarouselClippingOffset;
    ...
    globalThis.ODQ_LAYOUT_B_QUILT_BG_OFFSET_Y_STEP = ODQ_LAYOUT_B_QUILT_BG_OFFSET_Y_STEP;
    globalThis.ODQ_LAYOUT_B_QUILT_BG_OFFSET_Y_BIG_STEP = ODQ_LAYOUT_B_QUILT_BG_OFFSET_Y_BIG_STEP;
    ...
    globalThis.odqCarouselStoryStyleAnchorAuthorToCutoutBottomLeft =
      odqCarouselStoryStyleAnchorAuthorToCutoutBottomLeft;
     ...
     * Used by carousel slide 2 to label the quilt with its winning name.
     *
