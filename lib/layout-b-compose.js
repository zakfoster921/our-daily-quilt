...
(function (root) {
  ...
  if (!CONFIG || typeof CONFIG !== 'object') {
    ...
    function getLayoutBStoryQuotePlan(ctx, options) {
      ...
      /** Post (4:5) in safe zone: one centered column for readable order; story keeps stagger. */
      const postReadableStripStack = isPostLayout && stripBounds;
      const postReadingCenterX = () => odqPostStripReadingCenterX(STORY_W, STORY_H);
      const storyLikeStripCenterX = (spec, pseudoWidth) => {
        ...
        if (postReadableStripStack) {
          const readCx = postReadingCenterX();
          const minCx = w / 2 + hBounds.left + 8;
          const maxCx = hBounds.right - w / 2 - 8;
          return Math.max(minCx, Math.min(maxCx, readCx));
        }
    ...
    /** Center every strip on one column so post quote reads top → bottom (no zig-zag). */
    function odqAlignPostStripPlanReadingStack(plan, layoutW, layoutH) {
      const strips = Array.isArray(plan) ? plan : [];
      if (!strips.length) return strips;
      const readCx = odqPostStripReadingCenterX(layoutW, layoutH);
      const safe = odqLayoutBPostGridSafeRect(layoutW, layoutH, 8);
      return strips.map((s) => {
        const half = Number(s.w || 20) / 2 + 10;
        let x = readCx;
        if (safe) {
          x = Math.max(safe.x + half, Math.min(safe.x + safe.width - half, readCx));
        }
        return { ...s, x };
      });
    }
    ...
    function odqApplyPostQuoteStripVerticalSpread(plan, layoutW, layoutH, opts = {}) {
      ...
      let out = opts.readingStack
        ? odqAlignPostStripPlanReadingStack(spread, layoutW, layoutH)
