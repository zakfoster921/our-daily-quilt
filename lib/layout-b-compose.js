...
(function (root) {
  ...
  if (!CONFIG || typeof CONFIG !== 'object') {
    ...
    /** layout-b-speaker.png: linear scale applied to cover-fit cutout (0.8 = 20% smaller). */
    ...
    function getLayoutBStoryQuotePlan(ctx, options) {
      ...
      const buildStripSpecs = () => {
        ...
        const layoutKeywordPart = (part) => {
          ...
          if (part.kind === 'plain') {
            measureStrip([chunkText], bodyFont, 1.38, 'quote', { isKeywordConnector: true });
            return;
          }
            ...
            for (const part of fullParts) {
              ...
              if (part.kind === 'inline-emphasis') {
                ...
                if (runs) {
                  ...
                  storyUnits.push({ emphasis: false, text: phrase });
                }
                continue;
              }
              if (part.kind === 'plain') {
                storyUnits.push({ emphasis: false, text: chunkText.trim(), isKeywordConnector: true });
              }
            }
    ...
    async function composeInstagramLayoutBFromQuiltBlob(highResBlob, quoteText, quoteAuthor, layoutW, layoutH, dateKey, options = {}) {
      ...
      return new Promise((resolve, reject) => {
            ...
            const getSpeakerHeroPostRect = (img) => {
              ...
              const coverScale =
                Math.max(maxW / aspect, maxH) * ODQ_LAYOUT_B_SPEAKER_HERO_POST_CUTOUT_SCALE;
              ...
              .then(async (quiltImg) => {
                ...
                const layoutBQuiltBleed = isPostLayout
                  ...
                  : odqLayoutBSpeakerActiveFromOverlay(speakerOverlay)
                    ...
                    : ODQ_LAYOUT_B_STORY_QUILT_BLEED_NO_SPEAKER;
                if (isPostLayout) {
                  let postScale;
                  let dx;
                  let dy;
