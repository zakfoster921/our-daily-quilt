...
(function (root) {
  ...
  const ODQ_LAYOUT_B_STORY_RENDER_VERSION = 'story-speaker-3x-v8';
    ...
    async function composeInstagramLayoutBFromQuiltBlob(highResBlob, quoteText, quoteAuthor, layoutW, layoutH, dateKey, options = {}) {
      ...
      return new Promise((resolve, reject) => {
            ...
            const getSpeakerOverlayRect = (img, strips) => {
              ...
              /** Story 9:16 AUTO uses the same bold sizing for quilt-screen preview and full export. */
              const storyAutoLinearMul = !isPostLayout ? ODQ_LAYOUT_B_STORY_SPEAKER_AUTO_LINEAR_MUL : 1;
              const speakerLinearBoost = 1.25 * storyAutoLinearMul;
              const speakerAreaMult = speakerLinearBoost * speakerLinearBoost;
              const hardAreaMax =
                canvasArea * (isPostLayout ? 0.25 : 0.33) * speakerAreaMult;
              const targetArea =
                canvasArea * (isPostLayout ? 0.18 : 0.24) * speakerAreaMult;
              ...
              const fitted = isPostLayout
                ? odqConstrainSpeakerRectToNameStripWidth(best, strips)
                : best;
              if (!isPostLayout) {
            ...
            loadQuiltImage()
              .then(async (quiltImg) => {
                ...
                for (const speakerImg of speakerImgs) {
                  ...
                  if (speakerRect && speakerTransform && isPostLayout) {
