...
(function (root) {
  ...
  if (!CONFIG || typeof CONFIG !== 'object') {
    ...
    const ODQ_CAROUSEL_STRIP_STACK_PAD = 10;
    ...
    const ODQ_CAROUSEL_STORY_STYLE_MIN_BODY_PX = 38;
    ...
    function getLayoutBStoryQuotePlan(ctx, options) {
      ...
      const segmentQuoteForLayout = (text) => {
        ...
        if (paragraphs.length === 1) {
          const segs = applyPoeticMicroSplit(runOne(paragraphs[0]));
          if (carouselRowBudget) return segs;
          return odqCarouselSpeakerMergeSegmentsToRowBudget(segs, carouselMaxQuoteRows());
        }
        const out = [];
        for (const p of paragraphs) out.push(...runOne(p));
        const merged = applyPoeticMicroSplit(out);
        if (carouselRowBudget) return merged;
        return odqCarouselSpeakerMergeSegmentsToRowBudget(merged, carouselMaxQuoteRows());
      };
    ...
    /** Carousel slide 1: one body size for every quote + author strip (keywords stay caps/bold, not smaller). */
    function odqApplyUniformCarouselStoryBodyPx(plan, bodyPx, fontSerif) {
      const px = Math.max(1, Math.round(Number(bodyPx) || ODQ_LAYOUT_B_POST_MIN_BODY_PX));
      return (Array.isArray(plan) ? plan : []).map((s) => {
        if (s.role !== 'quote' && s.role !== 'author') return s;
        const next = odqLayoutBStripReplaceFontPx(s, px, fontSerif);
        if (s.role === 'quote') {
    ...
    function odqCarouselUniformMaybeWrapQuoteStrip(spec, ctx, px, maxInnerW, fontSerif) {
      const serif = fontSerif || ODQ_CANVAS_SERIF_FONT;
      ...
      const wrapped = odqLayoutBWrapTextLines(ctx, lineText, maxInnerW, fontStr, px);
      const next = {
        ...spec,
    ...
    function odqCarouselUniformQuoteSpecForFit(spec, ctx, px, maxStripW, fontSerif) {
    ...
    function odqCarouselQuoteBodyPxFitsAllStrips(plan, ctx, bodyPx, maxStripW, fontSerif) {
      ...
      for (const s of Array.isArray(plan) ? plan : []) {
        ...
        const measureSpec = odqCarouselUniformQuoteSpecForFit(s, ctx, px, maxStripW, fontSerif);
    ...
    function odqBuildUniformCarouselStoryStripPlan(plan, bodyPx, ctx, maxStripW, fontSerif, refitOpts = {}) {
      ...
      strips = strips.map((s) => {
        ...
        const extraPadX =
          ...
          (s.isKeywordConnector && s.w < maxW ? ODQ_LAYOUT_B_KEYWORD_CONNECTOR_EXTRA_PAD : 0);
        const innerAtMaxW = maxW - 26 * 2 - extraPadX - 6;
        return odqCarouselUniformMaybeWrapQuoteStrip(s, ctx, px, innerAtMaxW, fontSerif);
      });
    ...
    function odqNormalizeCarouselStoryStyleUniformTypography(plan, ctx, maxStripW, fontSerif) {
    ...
    function refitLayoutBStripPlanToText(plan, ctx, stripMaxW, fontSerif, opts = {}) {
      ...
      return (Array.isArray(plan) ? plan : []).map((s) => {
        ...
        if (uniformBodyPx != null && (s.role === 'quote' || s.role === 'author')) {
          fontStr = fontStr.includes('px')
            ? fontStr.replace(/\d+px/, `${uniformBodyPx}px`)
            : `400 ${uniformBodyPx}px ${serif}`;
        }
        const extraPadX =
          Number(s.extraPaperPadX) ||
          (s.isKeywordConnector && s.w < maxW ? ODQ_LAYOUT_B_KEYWORD_CONNECTOR_EXTRA_PAD : 0);
    ...
    function odqCarouselStoryStyleStripMaxWidth(layoutW, layoutH, speakerSeamRect, defaultMax) {
      ...
      const sp = speakerSeamRect
        ? odqCarouselSpeakerExpandedRect(speakerSeamRect, ODQ_CAROUSEL_SPEAKER_AVOID_PAD)
        : null;
      if (!safe || !sp) return defaultMax;
      return Math.max(160, Math.round(sp.left - safe.left - 36));
    }
    ...
    function odqCarouselStoryStyleFinalizeStripSpread(strips, layoutW, layoutH, opts = {}) {
      let out = odqApplyPostQuoteStripVerticalSpread(strips, layoutW, layoutH, opts.spreadOpts || {});
      const speakerRect = opts.carouselStoryStyleSpeakerSeamRect;
      const safe = odqLayoutBPostGridSafeRect(layoutW, layoutH, 8);
      out = odqResolveLayoutBStripPlanOverlaps(out, layoutW, layoutH, 18, safe);
    ...
    function odqMaximizeCarouselStoryStyleStripTypography(
      ...
      const fitsAtPx = (bodyPx) => {
        ...
        if (!odqCarouselQuoteBodyPxFitsAllStrips(boosted, ctx, px, maxStripW, fontSerif)) return false;
                ...
                if (
                  ...
                  if (
                    ...
                    for (const s of quoteStripPlan) {
                      ...
                      s.textRuns = runs;
                      s.lines = [LBKE.runsToDisplayLine(runs)];
                    }
                    quoteStripPlan = refitLayoutBStripPlanToText(
                      quoteStripPlan,
                      ctx,
