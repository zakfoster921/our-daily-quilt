...
(function (root) {
  ...
  function odqLayoutBIgStripsSkipKeywordEmphasis() {
    ...
    const ODQ_CAROUSEL_STORY_STYLE_AUTHOR_ROW_GAP = 12;
    ...
    function odqCarouselStoryStyleNeedsTightVerticalStack(plan, layoutW, layoutH, opts = {}) {
      ...
      const authorReserve = authors.length
        ? Math.max(...authors.map((a) => Number(a.h || 0))) + ODQ_CAROUSEL_STORY_STYLE_AUTHOR_ROW_GAP
        : 0;
    ...
    function odqCarouselStoryStyleCompressQuoteStripYs(plan, layoutW, layoutH, opts = {}) {
      ...
      if (authors.length) {
        cursor += ODQ_CAROUSEL_STORY_STYLE_AUTHOR_ROW_GAP;
        for (const a of authors) {
    ...
    function odqCarouselStoryStyleStackStripsByReadingOrder(strips, quoteText, layoutW, layoutH, opts = {}) {
      ...
      for (let i = 0; i < ordered.length - 1; i++) {
        ...
        if (cur.role !== 'author' && next.role === 'author') {
          authorGapTotal += ODQ_CAROUSEL_STORY_STYLE_AUTHOR_ROW_GAP;
        }
      }
      const rowHeights = ordered.map((s) => Number(s.h || 0));
      const tightStack = odqCarouselStoryStyleNeedsTightVerticalStack(ordered, layoutW, layoutH, opts);
      ...
      for (let i = 0; i < ordered.length; i++) {
        ...
        if (i < ordered.length - 1) {
          const next = ordered[i + 1];
          const pairGap =
            s.role !== 'author' && next.role === 'author'
              ? ODQ_CAROUSEL_STORY_STYLE_AUTHOR_ROW_GAP
              : quoteGap;
          cursorTop = s.y + h / 2 + pairGap;
        }
      }
              ...
              .then(async (quiltImg) => {
                ...
                if (isPostLayout) {
                  ...
                  if (options?.quiltFit === 'contain') {
                    ...
                    const qy = Math.round((STORY_H - qh) / 2);
