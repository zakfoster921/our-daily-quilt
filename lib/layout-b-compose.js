...
(function (root) {
  ...
  if (!CONFIG || typeof CONFIG !== 'object') {
    ...
    /** layout-b-speaker.png: linear scale applied to contain-fit cutout when name strip width unknown. */
    const ODQ_LAYOUT_B_SPEAKER_HERO_POST_CUTOUT_SCALE = 0.8;
    /** Speaker-hero post: cutout width as a fraction of the fitted author name strip width. */
    const ODQ_LAYOUT_B_SPEAKER_HERO_CUTOUT_NAME_STRIP_RATIO = 0.9;
    ...
    async function composeInstagramLayoutBFromQuiltBlob(highResBlob, quoteText, quoteAuthor, layoutW, layoutH, dateKey, options = {}) {
      ...
      return new Promise((resolve, reject) => {
            ...
            const drawSpeakerOverlay = (g, img, rect, washColor, opts = {}) => {
              ...
              return SCR.drawSpeakerCutoutStack(g, img, rect, {
                ...
                palette:
                  ...
                  [],
                drawScannerBed: SCR?.createSpeakerPortraitScannerBedDrawer?.(rngDateKey),
            ...
            /** 4:5 speaker-hero post: contain-fit cutout; width tied to author name strip when known. */
            const getSpeakerHeroPostRect = (img, opts = {}) => {
              if (!img) return null;
              const iw = Math.max(1, img.naturalWidth || img.width);
              const ih = Math.max(1, img.naturalHeight || img.height);
              const aspect = iw / ih;
              ...
              const nameStripW = Number(opts.nameStripWidthPx);
              let w =
                Number.isFinite(nameStripW) && nameStripW > 0
                  ? nameStripW * ODQ_LAYOUT_B_SPEAKER_HERO_CUTOUT_NAME_STRIP_RATIO
                  : maxW * ODQ_LAYOUT_B_SPEAKER_HERO_POST_CUTOUT_SCALE;
              w = Math.min(w, maxW);
              let h = w / aspect;
              if (h > maxH) {
                h = maxH;
            ...
            loadQuiltImage()
              .then(async (quiltImg) => {
                ...
                if (!isPostLayout || options.speakerHeroPost === true) {
