...
(function (root) {
  ...
  if (!CONFIG || typeof CONFIG !== 'object') {
    ...
    function odqCarouselStoryStyleStripTypographyOk(strips, layoutW, layoutH, opts = {}, ctx, fontSerif) {
      const spreadStrips = odqApplyPostQuoteStripVerticalSpread(
        strips,
        layoutW,
        layoutH,
        opts.spreadOpts || {}
      ...
      if (speakerRect && !odqCarouselStripPlanLayoutOk(spreadStrips, speakerRect)) return false;
    ...
    function odqMaximizeCarouselStoryStyleStripTypography(
      ...
      const refitOpts = { hardMinBodyPx: 22 };
      let base = refitLayoutBStripPlanToText(
        boostLayoutBStripPlanTypography(plan, ODQ_LAYOUT_B_POST_STORY_INHERIT_TYPE_BOOST, fontSerif),
        ctx,
        maxStripW,
      ...
      if (!odqCarouselStoryStyleStripTypographyOk(base, layoutW, layoutH, opts, ctx, fontSerif)) {
        let fixed = base;
        for (let i = 0; i < 28; i++) {
          if (odqCarouselStoryStyleStripTypographyOk(fixed, layoutW, layoutH, opts, ctx, fontSerif)) break;
          fixed = refitLayoutBStripPlanToText(
            boostLayoutBStripPlanTypography(fixed, 0.94, fontSerif),
            ctx,
            maxStripW,
            fontSerif,
            refitOpts
        ...
        return odqApplyPostQuoteStripVerticalSpread(fixed, layoutW, layoutH, opts.spreadOpts || {});
      }
      let lo = 1;
      let hi = ODQ_LAYOUT_B_POST_QUOTE_ONLY_MAX_TYPE_SCALE;
      let best = base;
      for (let i = 0; i < 12; i++) {
        ...
        if (odqCarouselStoryStyleStripTypographyOk(trial, layoutW, layoutH, opts, ctx, fontSerif)) {
          best = trial;
          lo = mid;
        } else {
          hi = mid;
      ...
      best = refitLayoutBStripPlanToText(best, ctx, maxStripW, fontSerif, refitOpts);
      return odqApplyPostQuoteStripVerticalSpread(best, layoutW, layoutH, opts.spreadOpts || {});
    }
