...
(function (root) {
  ...
  if (!CONFIG || typeof CONFIG !== 'object') {
      ...
      return (Array.isArray(plan) ? plan : []).map((s) => {
        ...
        const lh =
          Number.isFinite(capLineLead) && capLineLead > 0
            ? Math.min(lhRaw, refitPx * capLineLead)
            : lhRaw;
    ...
    function odqCarouselStoryStyleStripTypographyOk(strips, layoutW, layoutH, opts = {}) {
    ...
    function odqMaximizeCarouselStoryStyleStripTypography(
      ...
      if (!odqCarouselStoryStyleStripTypographyOk(base, layoutW, layoutH, opts)) {
        return odqShrinkStripPlanTypographyUntilPostSafe(
          base,
          ctx,
          layoutW,
      ...
      for (let i = 0; i < 12; i++) {
        ...
        if (odqCarouselStoryStyleStripTypographyOk(trial, layoutW, layoutH, opts)) {
          best = trial;
          lo = mid;
        } else {
          hi = mid;
