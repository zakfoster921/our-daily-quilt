  ...
  if (!CONFIG || typeof CONFIG !== 'object') {
    ...
    function odqCarouselStoryStyleStackStripsByReadingOrder(strips, quoteText, layoutW, layoutH) {
      ...
      for (let i = 0; i < ordered.length; i++) {
        const s = ordered[i];
        const arAtZero = odqStripPlanAxisRect({ ...s, y: 0 }, pad);
        s.y = cursorTop - arAtZero.top;
        const ar = odqStripPlanAxisRect(s, pad);
        const nextIsAuthor = ordered[i + 1]?.role === 'author';
        cursorTop = ar.bottom + (nextIsAuthor ? 26 : 16);
      }
      const bottom = Math.max(...ordered.map((s) => odqStripPlanAxisRect(s, pad).bottom));
      const overflow = bottom - (safe.bottom - 8);
      if (overflow > 0) {
        const top = Math.min(...ordered.map((s) => odqStripPlanAxisRect(s, pad).top));
        const lift = Math.min(overflow, Math.max(0, top - safe.top - 8));
      ...
      return new Promise((resolve, reject) => {
            ...
            loadQuiltImage()
              .then(async (quiltImg) => {
                ...
                if (carouselStoryStyle && quoteStripPlan.length && layoutBPostAuthorName) {
                  ...
                  if (authorStrips.length && quoteStrips.length) {
                    ...
                    let cursor = Math.min(...orderedQuotes.map((s) => odqStripPlanAxisRect(s, 18).top));
                    for (const q of orderedQuotes) {
                      const arAtZero = odqStripPlanAxisRect({ ...q, y: 0 }, 18);
                      q.y = cursor - arAtZero.top;
                      cursor = odqStripPlanAxisRect(q, 18).bottom + 16;
                    }
                    cursor += 12;
                    for (const a of authorStrips) {
                      a.role = 'author';
                      const arAtZero = odqStripPlanAxisRect({ ...a, y: 0 }, 18);
                      a.y = cursor - arAtZero.top;
                      cursor = odqStripPlanAxisRect(a, 18).bottom + 16;
                    }
                  }
                }
