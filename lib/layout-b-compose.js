            ...
            loadQuiltImage()
              .then(async (quiltImg) => {
                ...
                if (
                  ...
                  for (let postFlowPass = 0; postFlowPass < carouselShrinkPasses + 4; postFlowPass++) {
                    quoteStripPlan = quoteStripPlan.map((s) => {
                      if (s.role === 'author') return s;
                      const stripMaxW = Math.max(
                        120,
                        Math.min(Number(s.w) || postFlowMaxStripW, postFlowMaxStripW)
                      );
                    ...
                    const typographyOk =
                      ...
                      odqCarouselStoryStyleStripTypographyOk(
                        ...
                        STORY_W,
                        STORY_H,
                        postFlowTypographyOpts,
                        ctx,
                        FONT_SERIF
                      );
                    if (typographyOk) break;
                    const shrinkFloorPx =
                      postFlowPass >= carouselShrinkPasses
                        ? ODQ_LAYOUT_B_POST_MIN_BODY_PX
                        : ODQ_CAROUSEL_STORY_STYLE_MIN_BODY_PX;
                    const shrinkPx = Math.max(
                      shrinkFloorPx,
