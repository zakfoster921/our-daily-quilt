...
(function (root) {
  ...
  if (!CONFIG || typeof CONFIG !== 'object') {
    ...
    /** Post quote: compositional serpentine inside safe zone (readable, not a rigid column). */
    const ODQ_LAYOUT_B_POST_COMPOSITIONAL_WAVE_CYCLES = 1.35;
    const ODQ_LAYOUT_B_POST_COMPOSITIONAL_AMPLITUDE_FRAC = 0.26;
    const ODQ_LAYOUT_B_POST_COMPOSITIONAL_HARMONIC_FRAC = 0.32;
    const ODQ_LAYOUT_B_POST_COMPOSITIONAL_ROW_JITTER_FRAC = 0.09;
    const ODQ_LAYOUT_B_POST_READING_BAND_JITTER_PX = 26;
    /** Max horizontal step between consecutive rows (fraction of inner column). */
    const ODQ_LAYOUT_B_POST_READING_MAX_CONSEC_STEP_FRAC = 0.14;
    const ODQ_LAYOUT_B_POST_STRIP_TILT_RAD = 0.042;
    /** Post: chance adjacent comma-split phrase segments share one row (two strips). */
    const ODQ_LAYOUT_B_POST_ADJACENT_PAIR_PICK_PCT = 64;
    ...
    function getLayoutBStoryQuotePlan(ctx, options) {
      ...
      const buildStripSpecs = () => {
        ...
        const tryPairQuoteLine = (ln, font, lineSalt) => {
          ...
          const pickPct =
            isPostLayout && stripBounds ? ODQ_LAYOUT_B_POST_ADJACENT_PAIR_PICK_PCT : 48;
          const pick = (((lineSalt * 1103515245 + 12345) ^ dch * 31) >>> 0) % 100 < pickPct;
          return pick ? [best.left, best.right] : null;
        };
        /** Post: pair consecutive phrase segments (comma splits) on one row when both fit. */
        const tryPairAdjacentSegmentsForPost = (segA, segB, font, lineSalt) => {
          ...
          const pairGap = 30;
          if (paperA + pairGap + paperB > inner * 0.94) return null;
          const dch = rngDateKey.charCodeAt(lineSalt % rngDateKey.length);
          const pick =
            (((lineSalt * 1103515245 + 12345) ^ dch * 31) >>> 0) % 100 <
            ODQ_LAYOUT_B_POST_ADJACENT_PAIR_PICK_PCT;
          return pick ? [textA, textB] : null;
        };
        const emitQuoteSegmentsForLayout = (segments) => {
          ...
          for (let si = 0; si < segs.length; ) {
            ...
            const lines = wrapLines(ctx, segs[si], innerMaxW);
        ...
        const pairAdjacentPostQuoteSpecsInPlace = (specList) => {
          ...
          for (let i = 0; i < specList.length; ) {
            ...
            if (
              ...
              !b.isKeywordEmphasis &&
              b.rowSlot == null &&
              Number(a.w || 0) + pairGap + Number(b.w || 0) <= inner * 0.94
            ) {
              const dch = rngDateKey.charCodeAt(i % rngDateKey.length);
              const pick =
                (((i * 1103515245 + 12345) ^ dch * 31) >>> 0) % 100 <
                ODQ_LAYOUT_B_POST_ADJACENT_PAIR_PICK_PCT;
              if (pick) {
                const gid = gidBase++;
      ...
      /** Post (4:5) in safe zone: loose reading bands; story keeps stagger. */
      const postReadableStripStack = isPostLayout && stripBounds;
      const postQuoteRowCount = postReadableStripStack
        ? odqCountLayoutBQuoteLayoutRows(stripSpecs)
        : 0;
      let postQuoteRowIdx = 0;
      let postQuoteBandPrevCx = null;
      const storyLikeStripCenterX = (spec, pseudoWidth) => {
        ...
        if (postReadableStripStack && spec.role !== 'author') {
          ...
          const targetCx = odqPostStripReadingBandTargetCx(
            idx,
            postQuoteRowCount,
            postQuoteBandPrevCx,
            STORY_W,
          ...
          postQuoteBandPrevCx = targetCx;
          const minCx = w / 2 + hBounds.left + 8;
          const maxCx = hBounds.right - w / 2 - 8;
          const cx =
            targetCx + odqDeterministicStripJitterPx(
              { seed: `plan_${idx}` },
              ODQ_LAYOUT_B_POST_READING_BAND_JITTER_PX * 0.65
            );
          return Math.max(minCx, Math.min(maxCx, cx));
        }
        if (postReadableStripStack && spec.role === 'author') {
          const minCx = w / 2 + hBounds.left + 8;
          const maxCx = hBounds.right - w / 2 - 8;
          const layout = odqLayoutBPostStripInnerColumn(STORY_W, STORY_H);
          const innerSpan = layout ? Math.max(1, layout.right - layout.left) : 400;
          let cx =
            odqPostStripCompositionalTargetCx(
              postQuoteRowCount,
              postQuoteRowCount + 1,
              postQuoteBandPrevCx,
              STORY_W,
      ...
      for (let i = 0; i < stripSpecs.length; ) {
        ...
        if (
          ...
          const pairTiltA = postReadableStripStack
            ? odqPostStripCompositionalTiltRad(pairRowIdx, a, rngDateKey)
            : (rnd() - 0.5) * 0.055;
          const pairTiltB = postReadableStripStack
            ? odqPostStripCompositionalTiltRad(pairRowIdx, b, rngDateKey) * -0.85
            : (rnd() - 0.5) * 0.055;
          strips.push({
            ...a,
            x: leftCx,
        ...
        const stripTilt = postReadableStripStack
          ? odqPostStripCompositionalTiltRad(rowIdx, spec, rngDateKey)
          : (rnd() - 0.5) * 0.055;
        strips.push({
          ...spec,
          x: cx,
    ...
    function odqPostStripCompositionalTiltRad(rowIndex, spec, layoutSeed = '') {
      const sign = rowIndex % 2 === 0 ? 1 : -1;
      const wobble = odqDeterministicStripJitterPx(
        { seed: `${spec?.seed || 'strip'}_tilt_${layoutSeed}` },
        1
      );
      return sign * ODQ_LAYOUT_B_POST_STRIP_TILT_RAD * (0.55 + Math.abs(wobble) * 0.45);
    }
    ...
    function odqPostStripCompositionalTargetCx(
      ...
      const mid = (layout.left + layout.right) / 2;
      const amp = innerSpan * ODQ_LAYOUT_B_POST_COMPOSITIONAL_AMPLITUDE_FRAC;
      const t = rowCount <= 1 ? 0.5 : rowIndex / Math.max(1, rowCount - 1);
      const seedPhase =
        odqDeterministicStripJitterPx({ seed: `comp_phase_${layoutSeed}` }, 1) * Math.PI;
      const wave =
        Math.sin(t * Math.PI * ODQ_LAYOUT_B_POST_COMPOSITIONAL_WAVE_CYCLES + seedPhase) * amp;
      const wave2 =
        Math.sin(
          t * Math.PI * ODQ_LAYOUT_B_POST_COMPOSITIONAL_WAVE_CYCLES * 2.08 + seedPhase * 0.65
        ) *
        amp *
        ODQ_LAYOUT_B_POST_COMPOSITIONAL_HARMONIC_FRAC;
      let targetCx = mid + wave + wave2;
      targetCx += odqDeterministicStripJitterPx(
        { seed: `comp_row_${rowIndex}_${layoutSeed}` },
        innerSpan * ODQ_LAYOUT_B_POST_COMPOSITIONAL_ROW_JITTER_FRAC
      );
      const maxStep = innerSpan * ODQ_LAYOUT_B_POST_READING_MAX_CONSEC_STEP_FRAC;
      const prev = Number.isFinite(prevSmoothedCx) ? prevSmoothedCx : targetCx;
      const delta = targetCx - prev;
      return prev + Math.max(-maxStep, Math.min(maxStep, delta));
    }
    ...
    function odqApplyPostStripReadingBands(plan, layoutW, layoutH, opts = {}) {
      ...
      if (!unitCount) return strips;
      const innerSpan = layout ? Math.max(1, layout.right - layout.left) : 400;
      let prevCx = odqPostStripCompositionalTargetCx(0, unitCount, null, layoutW, layoutH, layoutSeed);
      for (let r = 0; r < quoteRows.length; r++) {
        const row = quoteRows[r];
        const targetCx = odqPostStripCompositionalTargetCx(
          r,
          unitCount,
          prevCx,
          layoutW,
        ...
        if (isPair) {
          ...
          a.x = leftCx;
          b.x = rightCx;
          a.angle = odqPostStripCompositionalTiltRad(r, a, layoutSeed);
          b.angle =
            odqPostStripCompositionalTiltRad(r, b, layoutSeed) * -0.85 +
            odqDeterministicStripJitterPx(b, 0.012);
        } else {
          const s = row[0];
          let cx =
            targetCx + odqDeterministicStripJitterPx(s, ODQ_LAYOUT_B_POST_READING_BAND_JITTER_PX);
          cx = odqClampStripPlanCxToSafe(s, cx, safe);
          s.x = cx;
          s.angle = odqPostStripCompositionalTiltRad(r, s, layoutSeed);
        }
        prevCx = targetCx;
      }
      for (const a of authors) {
        let cx = odqPostStripCompositionalTargetCx(
          unitCount,
          unitCount + 1,
          prevCx,
          layoutW,
        ...
        a.x = cx;
        a.angle = odqPostStripCompositionalTiltRad(unitCount + 1, a, layoutSeed) * 0.7;
      }
    ...
    function odqFitLayoutBStripPlanToPostLayoutColumn(plan, layoutW, layoutH) {
      ...
      if (!Number.isFinite(minX)) return strips;
      ...
      return odqClampLayoutBStripPlanXToSafe(strips, safe);
    }
    ...
    async function odqPrefetchSpeakerCutoutTweak(dateKey) {
      ...
      try {
        ...
        if (app && typeof app.scheduleLayoutBStoryPreviewRefresh === 'function') {
      ...
      return new Promise((resolve, reject) => {
            ...
            const makeSpeakerSilhouetteCanvas = (img, outW, outH) => {
              ...
              const inMarginBand = (x, y) =>
                x < c.width * 0.2 || x > c.width * 0.8 || y < c.height * 0.1 || y > c.height * 0.9;
              ...
              /** Xerox filter — match in-app quote card (no extra cream offset layer). */
              for (let y = 0; y < c.height; y += 1) {
                for (let x = 0; x < c.width; x += 1) {
                  const i = (y * c.width + x) * 4;
                  const alpha = d[i + 3];
              ...
              .then(async (quiltImg) => {
                ...
                if (options.speakerHeroPost === true) {
                  ...
                  const rawName = String(speakerOverlay?.name || quoteAuthor || '')
                    ...
                    .trim();
                  if (rawName) {
                    const namePx = Math.round(Math.max(36, bodyPx * 0.92));
                    const nameFont = `italic 400 ${namePx}px ${FONT_SERIF}`;
                    ctx.font = nameFont;
                    const nameLines = wrapLines(ctx, rawName, innerMaxW);
                    let tw = 0;
                    for (const ln of nameLines) tw = Math.max(tw, ctx.measureText(ln).width);
                    const lh = namePx * 1.38;
                    const nameSpec = {
                      lines: nameLines,
                      font: nameFont,
                      lh,
                      w: Math.min(STRIP_MAX_W, Math.ceil(tw + PAPER_PAD_X * 2 + 6)),
                      h: Math.ceil(nameLines.length * lh + PAPER_PAD_Y * 2),
                      role: 'author'
                    };
