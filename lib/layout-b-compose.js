...
(function (root) {
  ...
  const ODQ_LAYOUT_B_STORY_RENDER_VERSION = 'story-plain-strips-v7';
  ...
  function odqLayoutBIgStripsSkipKeywordEmphasis() {
    ...
    /** Story 9:16 AUTO: default cutout width/height vs legacy base (2 = twice as large). */
    const ODQ_LAYOUT_B_STORY_SPEAKER_AUTO_LINEAR_MUL = 2;
    ...
    function odqConstrainSpeakerRectToNameStripWidth(rect, strips) {
      ...
      const maxW = nameStripW * ODQ_LAYOUT_B_SPEAKER_MAX_NAME_STRIP_WIDTH_FRAC;
    ...
    /** Carousel slide 1 debug overlay — frame, content, anchor targets, name strip box. */
    function odqDrawCarouselSpeakerLayoutDebug(ctx, speakerRect, authorStrip, opts = {}) {
      if (!ctx || !speakerRect) return;
      ...
      const strokeBox = (x, y, w, h, color, label, dash = []) => {
        ...
        ctx.fillStyle = color;
        ctx.fillText(label, x + 4, Math.max(22, y - 8));
        ctx.restore();
      };

      const crosshair = (x, y, color, label) => {
      ...
      if (frame) {
        strokeBox(
          ...
          frame.height,
          '#ff2d2d',
          `cutout frame (${Math.round(frame.x)},${Math.round(frame.y)} ${Math.round(frame.width)}×${Math.round(frame.height)})`,
          [10, 6]
        );
      }
      if (content) {
        strokeBox(
          ...
          content.height,
          '#39ff14',
          `portrait content (${Math.round(content.x)},${Math.round(content.y)} ${Math.round(content.width)}×${Math.round(content.height)})`
        );
        const opaqueBL = odqCarouselSpeakerOpaqueBottomLeft(speakerRect);
        const bboxCornerX = Number(content.x);
        const bboxCornerY = Number(content.y) + Number(content.height);
        const cornerX = opaqueBL ? Number(opaqueBL.x) : bboxCornerX;
        const cornerY = opaqueBL ? Number(opaqueBL.y) : bboxCornerY;
        ...
        crosshair(bboxCornerX, bboxCornerY, '#ffff00', `bbox BL ${Math.round(bboxCornerX)},${Math.round(bboxCornerY)}`);
        crosshair(
          cornerX,
          cornerY,
          '#ffffff',
          `opaque BL ${Math.round(cornerX)},${Math.round(cornerY)}`
        );
        crosshair(
          targetRight,
          targetTop,
          '#ff00ff',
          `name target TR ${Math.round(targetRight)},${Math.round(targetTop)}`
        );
        ctx.save();
        ...
        ctx.setLineDash([8, 5]);
        ctx.beginPath();
        ctx.moveTo(content.x, faceTop);
        ctx.lineTo(content.x + content.width, faceTop);
        ctx.stroke();
        ctx.fillStyle = '#ff8800';
        ctx.font = 'bold 15px system-ui, sans-serif';
        ctx.fillText(`face guard y=${Math.round(faceTop)}`, content.x + 6, faceTop - 8);
        ctx.restore();
      }
      if (authorStrip) {
        const ar = odqStripPlanAxisRect(authorStrip, pad);
        strokeBox(
          ar.left,
          ar.top,
          ar.right - ar.left,
          ar.bottom - ar.top,
          '#00d4ff',
          `name strip center (${Math.round(authorStrip.x)},${Math.round(authorStrip.y)})`
        );
      }
    }
    ...
    function odqCarouselStoryStyleNeedsTightVerticalStack(plan, layoutW, layoutH, opts = {}) {
      if (opts.carouselShortQuote === true) return false;
      const safe = odqLayoutBPostSafeBounds(layoutW, layoutH, 10);
      ...
      const authorReserve = authors.length
        ...
        : 0;
      const totalH = quotes.reduce((sum, s) => sum + Math.max(0, Number(s.h || 0)), 0);
      const needed =
        edge * 2 +
        totalH +
        Math.max(0, quotes.length - 1) * comfortableGap +
        authorReserve;
    ...
    function odqCarouselStoryStyleStackStripsByReadingOrder(strips, quoteText, layoutW, layoutH, opts = {}) {
      ...
      const rowHeights = stackStrips.map((s) => Number(s.h || 0));
    ...
    async function composeInstagramLayoutBFromQuiltBlob(highResBlob, quoteText, quoteAuthor, layoutW, layoutH, dateKey, options = {}) {
      ...
      return new Promise((resolve, reject) => {
            ...
            const getSpeakerOverlayRect = (img, strips) => {
              ...
              const fastPreviewAutoFallback =
                !isPostLayout &&
                options.fastPreviewSpeakerAutoSize === true &&
                !speakerOverlay?.transform;
              /** Layout B + speaker: full renders stay bold; fast mobile previews use calmer AUTO fallback sizing. */
              const storyAutoLinearMul =
                !isPostLayout && !fastPreviewAutoFallback
                  ? ODQ_LAYOUT_B_STORY_SPEAKER_AUTO_LINEAR_MUL
                  : 1;
              const speakerLinearBoost = (fastPreviewAutoFallback ? 1 : 1.25) * storyAutoLinearMul;
              const speakerAreaMult = speakerLinearBoost * speakerLinearBoost;
              const hardAreaMax =
                canvasArea * (isPostLayout ? 0.25 : fastPreviewAutoFallback ? 0.24 : 0.33) * speakerAreaMult;
              const targetArea =
                canvasArea * (isPostLayout ? 0.18 : fastPreviewAutoFallback ? 0.17 : 0.24) * speakerAreaMult;
              let w = Math.sqrt(targetArea * aspect);
              let h = w / aspect;
              const canvasPad = 28;
              const maxW = Math.min(
                STORY_W - canvasPad * 2,
                STORY_W * (isPostLayout ? 0.48 : 0.62) * speakerLinearBoost
              );
              const maxH = Math.min(
                STORY_H - canvasPad * 2,
                STORY_H * (isPostLayout ? 0.48 : 0.46) * speakerLinearBoost
              );
              ...
              const fitted = odqConstrainSpeakerRectToNameStripWidth(best, strips);
              if (!isPostLayout) {
            ...
            loadQuiltImage()
              .then(async (quiltImg) => {
                ...
                for (const speakerImg of speakerImgs) {
                  ...
                  if (speakerRect && speakerTransform) {
                  ...
                  if (speakerRect && !isPostLayout && !carouselStoryStyle) {
                ...
                const debugCarouselSpeakerLayout =
                  options.debugCarouselSpeakerLayout === true ||
                  globalThis.ODQ_DEBUG_CAROUSEL_SPEAKER_LAYOUT === true;
                if (
                  debugCarouselSpeakerLayout &&
                  carouselStoryStyle &&
                  isPostLayout &&
                  lastSpeakerRectDrawn
                ) {
                  const debugAuthor =
                    cutoutLabelAuthors[0] ||
                    quoteStripPlan.find((s) => s.role === 'author') ||
                    null;
                  odqDrawCarouselSpeakerLayoutDebug(ctx, lastSpeakerRectDrawn, debugAuthor, {
                    overlapIntoX: ODQ_CAROUSEL_STORY_STYLE_AUTHOR_CUTOUT_OVERLAP_INTO_X,
                    overlapUpPx: ODQ_CAROUSEL_STORY_STYLE_AUTHOR_CUTOUT_OVERLAP_UP_PX
                  });
                }
    ...
    globalThis.odqDrawCarouselSpeakerLayoutDebug = odqDrawCarouselSpeakerLayoutDebug;
