                ...
                const _useSavedPlan = (() => {
                  ...
                  const stripText = (s) =>
                    String(s.text || (Array.isArray(s.lines) ? s.lines.join(' ') : '') || '')
                      ...
                      .trim()
                      .toLowerCase();
                  const authorNameLower = String(layoutBPostAuthorName || quoteAuthor || '')
                    .replace(/^\s*[—-]\s*/, '')
                    .replace(/\s+/g, ' ')
                    .trim()
                    .toLowerCase();
                  const isAuthorStrip = (s) =>
                    s.role === 'author' || (!!authorNameLower && stripText(s) === authorNameLower);
                  const saved = options.savedPostStripPlan;
                  const curQuotes = quoteStripPlan.filter((s) => !isAuthorStrip(s));
                  const savedQuotes = saved.filter((s) => !isAuthorStrip(s));
                  const orderedCur = odqSortCarouselQuoteStripsByTextOrder(curQuotes, layoutQuoteText);
                  const orderedSaved = odqSortCarouselQuoteStripsByTextOrder(savedQuotes, layoutQuoteText);
                  let matched = 0;
                  ...
                  const curAuthors = quoteStripPlan.filter(isAuthorStrip);
