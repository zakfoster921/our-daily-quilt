...
(function (root) {
  ...
  function odqLayoutBIgStripsSkipKeywordEmphasis() {
    ...
    /** Carousel slide 1: keep a meaningful slice of the cutout on-screen after quilt anchor + nudge. */
    function odqClampCarouselSlide1SpeakerRect(rect, layoutW, layoutH) {
      if (!rect) return rect;
      const w = Math.max(1, Number(rect.width) || 0);
      const h = Math.max(1, Number(rect.height) || 0);
      const oldX = Number(rect.x) || 0;
      const oldY = Number(rect.y) || 0;
      const minVisibleX = Math.max(56, Math.round(w * 0.32));
      const minVisibleY = Math.max(56, Math.round(h * 0.26));
      const x = Math.max(-w + minVisibleX, Math.min(oldX, layoutW - minVisibleX));
      const y = Math.max(-h + minVisibleY, Math.min(oldY, layoutH - minVisibleY));
              ...
              .then(async (quiltImg) => {
                ...
                for (const speakerImg of speakerImgs) {
                  ...
                  if (speakerRect && !carouselStoryStyle) {
                    ...
                    speakerRect = odqClampCarouselSlide1SpeakerRect(speakerRect, STORY_W, STORY_H);
                  }
