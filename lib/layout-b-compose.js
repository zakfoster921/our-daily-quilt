...
(function (root) {
  ...
  root.ODQ_LAYOUT_B_STORY_RENDER_VERSION = ODQ_LAYOUT_B_STORY_RENDER_VERSION;
    ...
    const ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_MAX = 28;
    const ODQ_CAROUSEL_STORY_STYLE_QUOTE_ROW_GAP_MIN = 4;
    const ODQ_CAROUSEL_STORY_STYLE_AUTHOR_ROW_GAP = 26;
    const ODQ_CAROUSEL_STORY_STYLE_STACK_TOP_PAD = 12;
    const ODQ_CAROUSEL_STORY_STYLE_STACK_BOTTOM_PAD = 8;
    ...
    function odqCarouselStoryStyleAdaptiveRowGap(rowHeights, safeSpan, reserveBottom = 0) {
      ...
      const topPad = ODQ_CAROUSEL_STORY_STYLE_STACK_TOP_PAD;
      const bottomPad = ODQ_CAROUSEL_STORY_STYLE_STACK_BOTTOM_PAD;
      const usable = Math.max(
        40,
        Number(safeSpan) - topPad - bottomPad - Math.max(0, Number(reserveBottom) || 0)
      );
    ...
    function odqCarouselStoryStyleCompressQuoteStripYs(plan, layoutW, layoutH, opts = {}) {
      ...
      const authorReserve = authors.length
        ...
        : 0;
      const { gap, topPad } = odqCarouselStoryStyleAdaptiveRowGap(
        rowHeights,
        safe.bottom - safe.top,
        authorReserve
      );
      let cursor = safe.top + topPad;
    ...
    function odqCarouselStoryStyleStackStripsByReadingOrder(strips, quoteText, layoutW, layoutH) {
      ...
      let quoteGap = odqCarouselStoryStyleAdaptiveRowGap(
        rowHeights,
        safe.bottom - safe.top,
        authorGapTotal
      ).gap;
      let cursorTop = safe.top + topPad;
      ...
      const overflowBottom = lastS ? lastS.y + (lastS.h || 0) / 2 - (safe.bottom - bottomPad) : 0;
      if (overflowBottom > 0) {
        const firstS = ordered[0];
        const headroom = firstS
          ? Math.max(0, firstS.y - (firstS.h || 0) / 2 - (safe.top + topPad))
          : 0;
                  ...
                  if (authorStrips.length && quoteStrips.length) {
                    ...
                    const authorReserve9 = authorStrips.length
                      ? Math.max(...authorStrips.map((a) => Number(a.h || 0))) +
                        ODQ_CAROUSEL_STORY_STYLE_AUTHOR_ROW_GAP
                      : 0;
                    const { gap: rowGap9, topPad: topPad9 } = odqCarouselStoryStyleAdaptiveRowGap(
                      rowHeights9,
                      safe9 ? safe9.bottom - safe9.top : 1320,
                      authorReserve9
                    );
                    let cursor = safe9 ? safe9.top + topPad9 : 30;
