...
(function (root) {
  ...
  if (!CONFIG || typeof CONFIG !== 'object') {
    ...
    /** Undo 9:16→4:5 font shrink on inherited story strip plans, then grow for 4:5 (quote-only post). */
    const ODQ_LAYOUT_B_POST_STORY_INHERIT_TYPE_BOOST = 1920 / 1350;
    /** Extra scale on quote-only post after undo — tuned so long quotes shrink back into safe zone. */
    const ODQ_LAYOUT_B_POST_QUOTE_ONLY_EXTRA_TYPE_BOOST = 1.18;
    ...
    function getLayoutBStoryQuotePlan(ctx, options) {
      ...
      /** Post (4:5): cluster in inner layout column; story uses full-width stagger. */
      const storyLikeStripCenterX = (spec, pseudoWidth) => {
        ...
        if (isPostLayout && hBounds) {
          ...
          const jitterSpan = Math.max(40, (hBounds.right - hBounds.left) * 0.36);
    ...
    function odqApplyPostQuoteStripVerticalSpread(plan, layoutW, layoutH) {
      ...
      return odqFitLayoutBStripPlanToPostLayoutColumn(spread, layoutW, layoutH);
    }
    ...
    /** Step typography down until inherited quote-only stack fits post safe-zone height. */
    ...
    /**
     * After story→post scale: quote-only boosts type (then shrinks if needed), then safe-zone spread.
     * Speaker posts: spread only (positions from story, no extra type boost).
     */
    function odqFinishInheritedStoryToPostStripPlan(
      ...
      if (quoteOnlyTypeBoost && strips.length) {
        const typeBoost =
          ODQ_LAYOUT_B_POST_STORY_INHERIT_TYPE_BOOST * ODQ_LAYOUT_B_POST_QUOTE_ONLY_EXTRA_TYPE_BOOST;
        strips = refitLayoutBStripPlanToText(
          boostLayoutBStripPlanTypography(strips, typeBoost, fontSerif),
          ctx,
          maxStripW,
          fontSerif
        );
        strips = odqShrinkStripPlanTypographyUntilPostSafe(
          strips,
          ctx,
          layoutW,
          layoutH,
      ...
      return odqApplyPostQuoteStripVerticalSpread(strips, layoutW, layoutH);
    }
      ...
      return new Promise((resolve, reject) => {
            ...
            loadQuiltImage()
              .then(async (quiltImg) => {
                ...
                /** Quote-only inherit: keep story strip positions, boost type after scale (speaker post unchanged). */
                const quoteOnlyPostInherit =
                  inheritStoryStripPlanForPost && !speakerActive && !speakerRequested;
                /** Plan strips on 9:16, then scale to 4:5 post so story quote arrangement carries over. */
                ...
                if (
                  ...
                  quoteStripPlan = getLayoutBStoryQuotePlan(ctx, {
                    ...
                    stripBoundsRect: (() => {
                      ...
                      if (isPostLayout && !speakerRequested && options.speakerHeroPost !== true) {
