...
(function (root) {
  ...
  if (!CONFIG || typeof CONFIG !== 'object') {
    ...
    /** layout-b-speaker.png: cutout fills frame width, cover-scales into band above name strip. */
    const ODQ_LAYOUT_B_SPEAKER_HERO_SIDE_PAD = 8;
    const ODQ_LAYOUT_B_SPEAKER_HERO_TOP_PAD = 8;
    ...
    function odqLayoutBPostStripSpreadBounds(layoutW, layoutH) {
      ...
      return {
        boundsTop: safe.y + 18,
        boundsBottom: safe.y + safe.height - 22
      };
    }
    ...
    async function composeInstagramLayoutBFromQuiltBlob(highResBlob, quoteText, quoteAuthor, layoutW, layoutH, dateKey, options = {}) {
      ...
      return new Promise((resolve, reject) => {
            ...
            /** 4:5 speaker-hero post (layout-b-speaker.png): cover-fit cutout, full width, anchored above name strip. */
            const getSpeakerHeroPostRect = (img) => {
              if (!img) return null;
              const iw = Math.max(1, img.naturalWidth || img.width);
              const ih = Math.max(1, img.naturalHeight || img.height);
              const aspect = iw / ih;
              const maxW = STORY_W - ODQ_LAYOUT_B_SPEAKER_HERO_SIDE_PAD * 2;
              const availTop = ODQ_LAYOUT_B_SPEAKER_HERO_TOP_PAD;
              const availBottom = STORY_H - ODQ_LAYOUT_B_SPEAKER_HERO_BOTTOM_RESERVE;
              ...
              .then(async (quiltImg) => {
                ...
                if (options.speakerHeroPost === true) {
                  ...
                  if (rawName) {
                    ...
                    const nameCx = STORY_W / 2 + (rnd() - 0.5) * 32;
                ...
                if (
                  ...
                  // Inherited story→post: scale only. Boost/refit widens strips at fixed x and closes pair gaps.
