...
(function (root) {
  ...
  class SimplifiedQuiltAppV2FeatureFeedback {
    ...
    async openFeatureFeedbackSummary() {
      ...
      if (!ok) return false;
      const screen = document.getElementById('screen-feature-feedback-summary');
      if (screen) {
        screen.hidden = false;
        screen.removeAttribute('hidden');
        screen.setAttribute('aria-hidden', 'false');
    ...
    async loadFeatureFeedbackSummary() {
      ...
      if (!root.db || !root.firestore?.collection || !root.firestore?.getDocs) {
        this.setFeatureFeedbackSummaryStatus('Firestore is not ready yet.', 'error');
        return false;
      }
    ...
    featureFeedbackApiBaseUrl() {
      const configured = String(root.CONFIG?.APP?.baseUrl || '').trim().replace(/\/+$/, '');
