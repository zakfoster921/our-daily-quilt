  ...
  function (global) {
    ...
    function drawSpeakerInRect(ctx, img, rect, options = {}) {
      ...
      if (poly && poly.length >= 4) {
        ...
        shadow.width = dw;
        shadow.height = dh;
        const sctx2 = shadow.getContext('2d');
        if (sctx2) {
          fillHardPolygon(sctx2, poly, '#000000');
          ctx.save();
          ctx.globalAlpha = 0.24;
          if ('filter' in ctx) {
            ctx.filter = `blur(${Math.max(5, Math.round(rect.height * 0.01))}px)`;
          }
          ctx.drawImage(
            shadow,
            dx + Math.round(rect.width * 0.01),
            dy + Math.round(rect.height * 0.014)
          );
          ctx.restore();
        }
      }
