...
(function (root, factory) {
  ...
  function (global) {
    ...
    function drawSpeakerNameStack(ctx, speakerImg, options = {}) {
      ...
      const nameAlignLeft = speakerAlign === 'left';
      /** Left stack: name sits under the cutout, not centered on the full panel. */
      const nameRect = nameAlignLeft
        ? {
            x: Math.max(namePadX, speakerRect.x),
            y: Math.min(
              nameBottom - 80,
            ...
            width: Math.max(
              120,
              Math.min(panelW - namePadX, speakerRect.x + speakerRect.width + panelW * 0.12) -
                Math.max(namePadX, speakerRect.x)
            ),
            height: 40
          }
        : {
      ...
      if (speakerName) {
        nameDrawn = drawNameVinylInRect(ctx, speakerName, nameRect, {
          dateKey,
          align: nameAlignLeft ? 'left' : 'center'
        });
      }
