...
(function (root, factory) {
  ...
  function (global) {
    ...
    /** Near-white paper peek under the cutout (hand-cut sticker). */
    const PAPER_EDGE = '#f6f2ea';
    /** Paper margin as fraction of speaker height. */
    const PAPER_MARGIN_FRAC = 0.028;
    /** Floor/ceiling for paper margin in px. */
    const PAPER_MARGIN_MIN_PX = 14;
    const PAPER_MARGIN_MAX_PX = 34;
    ...
    /**
     * Dilate opaque alpha by radius px (circular). Mutates ImageData.data.
     */
    function dilateAlphaMask(data, width, height, radius, minAlpha = 24) {
      if (radius < 1) return;
    ...
    /**
     * Nibble the outer paper ring so the margin reads as a rough scissors cut.
     * `origKeep` = pre-dilate opaque mask.
     */
    function roughenPaperMargin(data, width, height, origKeep, rng) {
      const paperOnly = new Uint8Array(width * height);
      for (let i = 0; i < width * height; i += 1) {
        if (!origKeep[i] && data[i * 4 + 3] >= 24) paperOnly[i] = 1;
      }
      /** Two passes of edge nibbles — deeper bites + fine fray. */
      for (let pass = 0; pass < 2; pass += 1) {
        const dropChance = pass === 0 ? 0.38 : 0.22;
        const clear = [];
        for (let y = 1; y < height - 1; y += 1) {
          for (let x = 1; x < width - 1; x += 1) {
            const idx = y * width + x;
            if (!paperOnly[idx]) continue;
            let openNeighbor = false;
            for (let dy = -1; dy <= 1; dy += 1) {
              for (let dx = -1; dx <= 1; dx += 1) {
                if (!dx && !dy) continue;
                const n = (y + dy) * width + (x + dx);
                if (!paperOnly[n] && !origKeep[n]) {
                  openNeighbor = true;
                  break;
                }
              }
              if (openNeighbor) break;
            }
            if (openNeighbor && rng() < dropChance) clear.push(idx);
          }
        }
      ...
      /** Occasional deeper bites into the paper ring (not into the portrait). */
    ...
    /**
     * Build speaker sticker: rough cream paper margin under the cutout PNG.
     * Returns a canvas larger than the draw box (includes pad for the margin).
     */
    function buildSpeakerWithRoughPaperEdge(img, drawW, drawH, options = {}) {
      ...
      try {
        ...
        return { canvas: src, pad, marginPx };
      }
      const data = imgData.data;
      const origKeep = new Uint8Array(cw * ch);
      for (let i = 0; i < cw * ch; i += 1) {
        if (data[i * 4 + 3] >= 24) origKeep[i] = 1;
      }

      const paperMask = new ImageData(new Uint8ClampedArray(data), cw, ch);
      dilateAlphaMask(paperMask.data, cw, ch, marginPx, 24);
      const seed = hashSeed(`${options.dateKey || 'slide0b'}:paper-edge:${cw}x${ch}:${marginPx}`);
      const rng = mulberry32(seed);
      roughenPaperMargin(paperMask.data, cw, ch, origKeep, rng);
      ...
      if (!pctx) return { canvas: src, pad, marginPx };
      pctx.fillStyle = paperHex;
      pctx.fillRect(0, 0, cw, ch);
      /** Soft paper tooth — slight warm noise on the face. */
      try {
        const grain = pctx.getImageData(0, 0, cw, ch);
        const gd = grain.data;
        for (let i = 0; i < gd.length; i += 4) {
          const n = (rng() - 0.5) * 10;
          gd[i] = Math.max(0, Math.min(255, gd[i] + n));
          gd[i + 1] = Math.max(0, Math.min(255, gd[i + 1] + n * 0.9));
          gd[i + 2] = Math.max(0, Math.min(255, gd[i + 2] + n * 0.75));
        }
        pctx.putImageData(grain, 0, 0);
      } catch (_) {
        /* skip */
      }
      ...
      maskCanvas.getContext('2d').putImageData(paperMask, 0, 0);
      ...
      if (!octx) return { canvas: src, pad, marginPx };
      octx.drawImage(paper, 0, 0);
      octx.drawImage(src, 0, 0);
      return { canvas: out, pad, marginPx };
    }
    ...
    function drawSpeakerInRect(ctx, img, rect, options = {}) {
      ...
      /** Soft contact shadow on the mat (paper edge sits like a sticker). */
