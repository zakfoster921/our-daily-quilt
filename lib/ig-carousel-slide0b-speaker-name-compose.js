...
(function (root, factory) {
  ...
  function (global) {
    ...
    function resolveSpeakerRect(img, options = {}) {
      ...
      const cx = PANEL_W / 2;
      const y = Math.round(PANEL_H * topFrac);
      return {
        x: Math.round(cx - w / 2),
        y,
        width: Math.round(w),
    ...
    async function composeCarouselSlide0bSpeakerName(options = {}) {
      ...
      /** Precompute name band so scraps can protect it before type is drawn. */
      ...
      /** Recompute name top from actual paper margin + rotation extent. */
