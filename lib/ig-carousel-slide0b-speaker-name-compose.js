...
(function (root, factory) {
  ...
  function (global) {
    ...
    /**
     * Mild scissors jitter on the outer contour only — keep the paper band solid.
     * Avoids chalky fringe from deep nibbles.
     */
    function scissorsCutOuterEdge(data, width, height, origKeep, rng) {
      ...
      for (let y = 1; y < height - 1; y += 1) {
        for (let x = 1; x < width - 1; x += 1) {
          ...
          if (!openNeighbor) continue;
          /** Shallow bites only — most edge stays put so the band stays thick. */
          if (rng() < 0.28) clear.push(idx);
        }
      }
      for (const idx of clear) {
        data[idx * 4 + 3] = 0;
      }
      /** A few slightly deeper scissor bites (1–2px), still rare. */
      for (let y = 2; y < height - 2; y += 2) {
        for (let x = 2; x < width - 2; x += 2) {
          const idx = y * width + x;
          if (!paperOnly[idx] || data[idx * 4 + 3] < 24) continue;
          let onRim = false;
          for (let dy = -1; dy <= 1; dy += 1) {
            for (let dx = -1; dx <= 1; dx += 1) {
              if (data[((y + dy) * width + (x + dx)) * 4 + 3] < 24) {
                onRim = true;
                break;
              }
            }
    ...
    /**
     * Build speaker sticker: thick solid tan paper backing under the cutout PNG.
    ...
    function buildSpeakerWithRoughPaperEdge(img, drawW, drawH, options = {}) {
      ...
      for (let i = 0; i < cw * ch; i += 1) {
        if (data[i * 4 + 3] >= 28) origKeep[i] = 1;
      }

      /** Threshold soft hair edges so the paper band follows a cleaner silhouette. */
      const paperMask = new ImageData(new Uint8ClampedArray(data), cw, ch);
      const md = paperMask.data;
      for (let i = 0; i < md.length; i += 4) {
        md[i + 3] = md[i + 3] >= 40 ? 255 : 0;
      }
      dilateAlphaMask(paperMask.data, cw, ch, marginPx, 24);
      const seed = hashSeed(`${options.dateKey || 'slide0b'}:paper-edge:${cw}x${ch}:${marginPx}`);
      const rng = mulberry32(seed);
      scissorsCutOuterEdge(paperMask.data, cw, ch, origKeep, rng);
      ...
      /** Flat solid paper — no grain (grain read as chalk). */
