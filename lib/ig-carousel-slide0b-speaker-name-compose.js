...
(function (root, factory) {
  ...
  function (global) {
    ...
    const SPEAKER_TOP_FRAC = 0.05;
    /** Gap between speaker paper bottom and name block. */
    const SPEAKER_NAME_GAP_PX = 36;
    ...
    async function composeCarouselSlide0bSpeakerName(options = {}) {
      ...
      const nameTop = Math.min(
        nameBottom - 80,
        Math.round(speakerRect.y + speakerRect.height + paperPadEstimate + gap)
      );
      ...
      /** Recompute name top from actual paper margin (may differ slightly from estimate). */
      const paperPad = Math.max(0, Number(speakerDraw?.marginPx) || paperPadEstimate);
      nameRect.y = Math.min(
        nameBottom - 80,
        Math.round(speakerRect.y + speakerRect.height + paperPad + gap)
      );
