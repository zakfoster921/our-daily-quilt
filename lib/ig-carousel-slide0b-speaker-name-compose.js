...
(function (root, factory) {
  ...
  function (global) {
    ...
    function drawSpeakerNameStack(ctx, speakerImg, options = {}) {
      ...
      /** Side stacks: name sits under the cutout, not centered on the full panel. */
      let nameRect;
      if (speakerAlign === 'left') {
        const x = Math.max(namePadX, speakerRect.x);
        nameRect = {
          x,
          y: nameY,
          width: Math.max(
            120,
            Math.min(panelW - namePadX, speakerRect.x + speakerRect.width + panelW * 0.12) - x
          ),
          height: 40
        };
      } else if (speakerAlign === 'right') {
        const right = Math.min(panelW - namePadX, speakerRect.x + speakerRect.width);
        const x = Math.max(
          namePadX,
          Math.round(speakerRect.x - panelW * 0.12)
        );
        nameRect = {
          x,
          y: nameY,
