...
(function (root) {
  ...
  class SimplifiedQuiltAppV2QuoteUi {
      ...
      /** Startup slider positions; background preview stays neutral until first slide. */
      _pickerStartupHSV() {
        const cp = CONFIG.COLOR_PICKER || {};
        const h = Number(cp.defaultHue);
        const s = Number(cp.defaultSaturation);
        const v = Number(cp.defaultLightness);
        return {
          h: Number.isFinite(h) ? Math.max(0, Math.min(360, Math.round(h))) : 360,
          s: Number.isFinite(s) ? Math.max(0, Math.min(100, Math.round(s))) : 10,
          v: Number.isFinite(v) ? Math.max(0, Math.min(100, Math.round(v))) : 90
        };
      }

      /** Saturation for slider tracks / preview — neutral (0) until user moves a slider. */
      _pickerPreviewSaturation() {
        if (!this.colorHasBeenSelected) return 0;
        return this.selectedSaturation;
      }

      _applyPickerStartupHSV() {
        const { h, s, v } = this._pickerStartupHSV();
        this.selectedHue = h;
        this.selectedSaturation = s;
        this.selectedLightness = v;
        this._userExplicitSatPick = false;
        this._hsvWheelCanvasKey = '';
      }

      _getTodayQuoteMoodHex() {
        const quote = this.quoteService?.getTodayQuote?.();
        const mood = String(quote?.mood ?? '').trim();
        if (!mood) return null;
        return globalThis.QuoteMoodColors?.resolveQuoteMoodColor?.(mood) || null;
      }

      /** Apply a hex as the suggested picker color (user can still adjust before submit). */
      _applyPickerHexColor(hex) {
        const normalized = String(hex || '').trim();
        if (!Utils.validateHexColor(normalized)) return false;
        const { h, s, v } = Utils.hexToHsv(normalized);
        this.selectedHue = Math.round(h);
        this.selectedSaturation = Math.round(s);
        this.selectedLightness = Math.round(v);
      ...
      _bindPickerRangeSlider(el, handler) {
        if (!el || typeof handler !== 'function' || el.dataset.pickerBound === '1') return;
        el.dataset.pickerBound = '1';
        el.addEventListener('input', handler);
        el.addEventListener('change', handler);
      }

      /** WebKit often ignores range `background` updates unless the track is forced to repaint. */
      _repaintPickerRangeTrack(el) {
        if (!el) return;
        if (el.type !== 'range') {
      ...
      _pickerTrackForSlider(el) {
        if (!el) return null;
        const prev = el.previousElementSibling;
        if (prev?.classList?.contains('picker-slider-track')) return prev;
        return el.parentElement?.querySelector('.picker-slider-track') || null;
      }

      /**
       * Paints the gradient on the sibling `.picker-slider-track` div, not the range input.
       * iOS WebKit skips repainting a range's own background when it wasn't the touched
       * control (sat/value tracks during a hue drag); a plain div always repaints.
       */
      _applyPickerTrackGradient(el, gradient) {
        if (!el || !gradient) return;
        const track = this._pickerTrackForSlider(el);
        if (track) {
          track.style.background = gradient;
          el.style.removeProperty('background');
          el.style.removeProperty('--picker-track-bg');
          void track.offsetHeight;
          return;
        }
        el.style.background = gradient;
        this._repaintPickerRangeTrack(el);
      }
      ...
      setupColorPicker() {
        ...
        if (this.useThreeSliderPicker()) {
          if (!this._boundHueSliderChange) {
            this._boundHueSliderChange = this.handleHueSliderChange.bind(this);
            this._boundSatSliderChange = this.handleSatSliderChange.bind(this);
            this._boundValueSliderChange = this.handleValueSliderChange.bind(this);
          }
          const hueSlider = document.getElementById('hueSlider');
          const satSlider = document.getElementById('satSlider');
          const valueSlider = document.getElementById('valueSlider');
          this._bindPickerRangeSlider(hueSlider, this._boundHueSliderChange);
          this._bindPickerRangeSlider(satSlider, this._boundSatSliderChange);
          this._bindPickerRangeSlider(valueSlider, this._boundValueSliderChange);
        } else {
      ...
      handleHueSliderChange(e) {
        ...
        if (!this._userExplicitSatPick) {
          this.selectedSaturation = this._pickerStartupHSV().s;
        }
        this.colorHasBeenSelected = true;
        this.updateColorPickerUI();
      }
      ...
      renderHueSlider() {
        ...
        this._applyPickerTrackGradient(el, `linear-gradient(to right, ${stops.join(', ')})`);
      }

      renderSatSlider() {
        const el = document.getElementById('satSlider');
        if (!el) return;
        this._applyPickerTrackGradient(
          el,
          `linear-gradient(to right, ${Utils.hsvToHex(this.selectedHue, 0, this.selectedLightness)}, ${Utils.hsvToHex(this.selectedHue, 100, this.selectedLightness)})`
        );
      }
      ...
      renderValueSlider() {
        ...
        const s = this._pickerPreviewSaturation();
        this._applyPickerTrackGradient(
          el,
          `linear-gradient(to right, ${Utils.hsvToHex(this.selectedHue, s, vMin)}, ${Utils.hsvToHex(this.selectedHue, s, vMax)})`
        );
      }
      ...
      updateColorPickerUI() {
        ...
        // Paint slider tracks before theme/mood side effects so a failure there
        // cannot leave sat/value stuck on the previous hue.
        if (this.useThreeSliderPicker()) {
          const hueSlider = document.getElementById('hueSlider');
          const satSlider = document.getElementById('satSlider');
        ...
        if (this.colorHasBeenSelected && hexColor && Utils.validateHexColor(hexColor) && this.isColorPickerActive()) {
          try {
            ...
            console.warn('Color picker theme update failed:', err);
          }
        }

        if (this.useThreeSliderPicker()) return;
      ...
      advanceIntroPersonaTo(targetScreenId, fromScreenId) {
        this.clearIntroPersonaAdvanceTimer();
        this.snapIntroPersonaFadeIns(fromScreenId);
      ...
      snapIntroPersonaFadeIns(screenId) {
        const el = document.getElementById(screenId);
        if (!el) return;
        el.querySelectorAll('.intro-zak-line-fade-in').forEach((node) => node.classList.add('visible'));
      }

      snapWelcomeIntroFadeIns() {
        document
          .querySelectorAll('#screen-welcome .welcome-screen-fade-in')
          .forEach((node) => node.classList.add('visible'));
      }

      skipPortalIntroDwell() {
        const portal = document.getElementById('screen-portal');
        if (!portal?.classList.contains('active')) return;
        if (!this._portalToQuoteTimer && !this._blockPortalScrollNav) return;
        this.clearPortalToQuoteIntroTimer();
        this._portalToQuoteIntroScheduled = false;
        void this.autoTransitionFromPortal();
      }

      async goToQuoteWithColorPicker() {
        this.clearPortalToQuoteIntroTimer();
        this.clearIntroPersonaAdvanceTimer();
        this.clearFirstQuoteBridgeTimer();
        this.clearQuoteColorPickerSchedule();
        this.markIntroPersonaSeen?.();
        try {
      ...
      clearQuoteColorPickerSchedule() {
        ...
        if (this._quoteColorPickerLayoutBFallbackTimer) {
          clearTimeout(this._quoteColorPickerLayoutBFallbackTimer);
          this._quoteColorPickerLayoutBFallbackTimer = null;
        }
      ...
      revealQuoteColorPickerPane() {
        ...
        // Warm cut-reveal mat photo while the user still picks a color.
        try {
      ...
      _beginQuoteColorPickerRevealCountdown() {
        ...
        if (!this._quoteScreenLayoutBStripsReady()) {
          this._quoteColorPickerRevealWaitingLayoutB = true;
          this._ensureQuoteColorPickerLayoutBFallback();
          return;
        }
        this._quoteColorPickerRevealWaitingLayoutB = false;
        if (this._quoteColorPickerLayoutBFallbackTimer) {
      ...
      /** If quote clipping never reports ready, still reveal the picker so users aren't stuck. */
      _ensureQuoteColorPickerLayoutBFallback() {
        if (this._quoteColorPickerLayoutBFallbackTimer) return;
        const maxWait = Math.max(
          0,
          Number(CONFIG?.APP?.quoteColorPickerLayoutBMaxWaitMs) || 5000
        );
        this._quoteColorPickerLayoutBFallbackTimer = setTimeout(() => {
          this._quoteColorPickerLayoutBFallbackTimer = null;
          if (this.isColorPickerActive()) return;
          if (document.querySelector('.screen.active')?.id !== 'screen-quote') return;
          this._quoteColorPickerRevealWaitingLayoutB = false;
