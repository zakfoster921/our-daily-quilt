...
(function (root) {
  ...
  class SimplifiedQuiltAppV2QuoteUi {
      ...
      renderHueSlider() {
        ...
        if (!el) return;
        const disp = this._hueBarDisplay || { s: 68, v: 91 };
      ...
      clearNameThanksAdvanceTimer() {
        if (this._nameThanksAdvanceTimer) {
          clearTimeout(this._nameThanksAdvanceTimer);
      ...
      revealQuoteColorPickerPane() {
        ...
        const startSlideIn = () => {
          ...
          const onDone = (ev) => {
            if (ev.target !== picker || ev.propertyName !== 'transform') return;
        ...
        requestAnimationFrame(() => requestAnimationFrame(startSlideIn));
      }

      /**
       * Quote in top half; after `readPauseMs` (default 1.5s) wheel slides into bottom half.
