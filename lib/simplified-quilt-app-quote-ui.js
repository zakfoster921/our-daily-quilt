...
(function (root) {
  ...
  class SimplifiedQuiltAppV2QuoteUi {
      ...
      /** Fade in the quote bridge line when the screen is shown (tap advances to quote). */
      primeFirstQuoteBridgeScreen() {
      ...
      /** Tap on quote bridge: instant cut to quote screen. */
      advanceFromFirstQuoteBridge() {
        this.clearFirstQuoteBridgeTimer();
        const bridge = document.getElementById('screen-first-quote-bridge');
        if (!bridge?.classList?.contains('active')) return;
        if (this.uiService?._flowTransitioning) return;
