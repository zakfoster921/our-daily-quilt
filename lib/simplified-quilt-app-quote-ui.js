...
(function (root) {
  ...
  class SimplifiedQuiltAppV2QuoteUi {
      ...
      /** Auto-advance between intro persona screens (Zak → mission → welcome). */
      scheduleIntroPersonaAdvance(targetScreenId, options = {}) {
        this.clearIntroPersonaAdvanceTimer();
        const reducedMotion = window.matchMedia?.('(prefers-reduced-motion: reduce)')?.matches;
        const contentFadeMs = reducedMotion ? 0 : (options.contentFadeMs ?? 700);
        const dwellMs = reducedMotion ? 0 : (options.dwellMs ?? 1400);
        const exitGapMs = reducedMotion ? 0 : (options.exitGapMs ?? 720);
        const onComplete = typeof options.onComplete === 'function' ? options.onComplete : null;
        const activeEl = document.querySelector('.screen.active');

        this._introPersonaAdvanceTimer = setTimeout(() => {
          if (activeEl?.classList?.contains('intro-persona-screen')) {
            activeEl.classList.remove('intro-persona-visible');
          }
          this._introPersonaAdvanceTimer = setTimeout(() => {
            this._introPersonaAdvanceTimer = null;
