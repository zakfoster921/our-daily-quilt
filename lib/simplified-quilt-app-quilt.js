...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    /**
     * Quilt-screen story preview: vinyl quote-fit + speaker (same as story quote-fit compose).
     */
    async _refreshStoryQuoteFitPreviewCore(options = {}) {
      ...
      const currentFp =
        typeof Utils.computeQuiltFingerprint === 'function' && Array.isArray(blocks)
          ...
          : String(blockCount);
      const contentKey = `quote-fit:${dk}:${currentFp}`;

      this._maybeInvalidateLayoutBStoryPreviewForQuiltGrowth();
      if (!force && this._layoutBStoryPreviewHeavyDoneThisVisit && !this.isAdminTomorrowPreviewActive?.()) {
        if (this._layoutBStoryPreviewContentKey === contentKey) return;
      }
      ...
      try {
        ...
        const composePromise = arch.generateInstagramStoryQuoteFitImageData(blocks, quote, dk);
        const result =
     ...
     * Exception: USE_STORY_QUOTE_FIT_PREVIEW uses live vinyl quote-fit compose.
     */
