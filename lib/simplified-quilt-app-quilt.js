...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    /** Stored snapshot previews can live-upgrade when speaker bytes arrive on the card. */
    scheduleLayoutBStoryPreviewRefreshAfterSpeaker(paintUrl) {
      ...
      if (this._layoutBStoryPreviewHeavyDoneThisVisit && !this._layoutBStoryPreviewSpeakerUpgradeDone) {
    ...
    async _tryStoredLayoutBStoryPreview(dateKey, previewContentKey = '', options = {}) {
      ...
      const useLayeredPreview = !!(overlayUrl && quiltScreenUrl);
      let url = '';
      if (preferQuiltScreenPlaceholder) {
        url = quiltScreenUrl;
      } else if (useLayeredPreview) {
        url = quiltScreenUrl;
      } else if (quiltScreenUrl && !overlayUrl) {
        // Split quilt screen exists but overlay not written yet — never use baked monolithic story.
        url = quiltScreenUrl;
        if (allowStalePlaceholder && finalAllowed) placeholderOnly = true;
      } else {
        url = storyUrl || quiltScreenUrl;
      }
      if (!url) {
        clearUnsafePlaceholder();
        return false;
        ...
        const finish = (ok) => {
          ...
          if (ok) {
            ...
            if (useLayeredPreview && overlayImg) {
              this._setLayoutBStoryPreviewImageCrossOrigin(img, url);
              this._setLayoutBStoryPreviewImageCrossOrigin(overlayImg, overlayUrl);
     ...
     * "re-compose from scratch" code further down only exists as a bootstrap fallback for the
     * rare case where a day has *no* stored image at all yet (nightly job hasn't run once).
      ...
      if (!this.isAdminTomorrowPreviewActive?.()) {
        ...
        if (!skipStoredPreview) {
          ...
          if (this._layoutBStoryPreviewDisplayedStoredPlaceholder) {
            ...
            this._layoutBStoryPreviewHeavyDoneThisVisit = false;
            this._scheduleLayoutBStoryPreviewRefreshAfterFirstScroll();
            return;
          }
        }
      }
