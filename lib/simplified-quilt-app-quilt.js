...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    _layoutBStoryPreviewContentKeyFor(blocks, dateKey, speakerSig, options = {}) {
      ...
      const tuneSig = String(
        options.tuneSignature || this._layoutBStoryPreviewTuneSignatureFor?.(dk) || ''
      ).trim();
      return `${dk}|${fp}|${speaker}|${speakerReady}|${tuneSig || 'tune0'}|stripRows3`;
    }

    _layoutBStoryPreviewTuneSignatureFor(dateKey) {
      const dk = String(dateKey || '').trim();
      if (!dk) return '';
      const normalizeZoom =
        typeof globalThis.odqNormalizeQuiltBgZoom === 'function'
          ? globalThis.odqNormalizeQuiltBgZoom
          : (value) => Math.round(Math.max(1, Number(value) || 1) * 1000) / 1000;
      const zoom =
        typeof globalThis.odqGetCachedLayoutBQuiltBgZoom === 'function'
          ? normalizeZoom(globalThis.odqGetCachedLayoutBQuiltBgZoom(dk, 'story'))
          : 1;
      const stripSeed =
        typeof globalThis.odqGetCachedLayoutBStripLayoutSeed === 'function'
          ? Number(globalThis.odqGetCachedLayoutBStripLayoutSeed(dk, 'story')) || 0
          : 0;
      const offset =
        typeof globalThis.odqGetCachedLayoutBQuoteStripOffset === 'function'
          ? globalThis.odqGetCachedLayoutBQuoteStripOffset(dk, 'story') || {}
          : {};
      const offsetX = Math.round((Number(offset.x) || 0) * 1000) / 1000;
      const offsetY = Math.round((Number(offset.y) || 0) * 1000) / 1000;
      return `tune:z${zoom}:s${stripSeed}:o${offsetX},${offsetY}`;
    }
    ...
    scheduleLayoutBStoryPreviewRefresh(options = {}) {
      ...
      if (options.force !== true && this._layoutBStoryPreviewHeavyDoneThisVisit) {
        // Allow refresh if quilt blocks have changed since the preview was last rendered.
    ...
    scheduleLayoutBStoryPreviewRefreshAfterSpeaker(paintUrl) {
      ...
      const contentKey = String(this._layoutBStoryPreviewContentKey || '');
      if (!contentKey.startsWith('stored:') && !contentKey.endsWith('|0')) {
        return;
      }
    ...
    async _tryStoredLayoutBStoryPreview(dateKey, previewContentKey = '', options = {}) {
      ...
      const url = preferQuiltScreenPlaceholder ? quiltScreenUrl : storyUrl || quiltScreenUrl;
      if (!url) {
        clearUnsafePlaceholder();
        return false;
      }
    ...
    _quoteHasRepeatedLayoutBKeyword(quote) {
      const q = quote && typeof quote === 'object' ? quote : {};
      const text = String(q.text ?? q.body ?? '').replace(/\s+/g, ' ').trim().toLowerCase();
      const raw = String(q.keyword ?? q.keywordSnapshot ?? '').trim();
      if (!text || !raw) return false;
      const keywords =
        globalThis.QuoteKeywordEmphasis?.parseEmphasisWordsInput?.(raw, text) ||
        raw.split(/[,;\n]+/).map((s) => s.trim()).filter(Boolean).slice(0, 3);
      for (const keyword of keywords) {
        const needle = String(keyword || '').replace(/\s+/g, ' ').trim().toLowerCase();
        if (!needle) continue;
        let count = 0;
        let idx = 0;
    ...
    async _refreshLayoutBStoryPreviewCore() {
      ...
      const currentFp =
        typeof Utils.computeQuiltFingerprint === 'function' && Array.isArray(blocks)
          ? Utils.computeQuiltFingerprint(blocks)
          : String(blockCount);
      const tuneSignature = this._layoutBStoryPreviewTuneSignatureFor?.(dk) || '';
      const storedPreviewKey = `stored:${dk}|${currentFp}|${tuneSignature || 'tune0'}`;
      let quote = this.isAdminTomorrowPreviewActive()
        ? this.getEffectiveQuiltQuote() || { text: '', author: '' }
        : this.quoteService?.getTodayQuote?.() || { text: '', author: '' };
      const quoteHasKeywordMeta = (q) => !!String(q?.keyword ?? q?.keywordSnapshot ?? '').trim();
      if (
        (!String(quote?.text || '').trim() || !quoteHasKeywordMeta(quote)) &&
        this.quoteService &&
        typeof this.quoteService.getQuoteResolvedForInstagramDateKey === 'function'
      ) {
        const resolvedQuote = await this.quoteService.getQuoteResolvedForInstagramDateKey(dk);
        quote = {
          ...(resolvedQuote || {}),
          ...(quote || {}),
          keyword: String(quote?.keyword ?? resolvedQuote?.keyword ?? '').trim(),
          keywordSnapshot: String(
            quote?.keywordSnapshot ??
              quote?.keyword ??
              resolvedQuote?.keywordSnapshot ??
              resolvedQuote?.keyword ??
              ''
          ).trim()
        };
      }
      if (!this.isAdminTomorrowPreviewActive?.()) {
        ...
        const hasRepeatedLayoutBKeyword = this._quoteHasRepeatedLayoutBKeyword(quote);
        const existingPreviewSrc = String(img.getAttribute('src') || '').trim();
        const existingLivePreview =
          !!existingPreviewSrc &&
          !img.hidden &&
          !!String(this._layoutBStoryPreviewContentKey || '').trim() &&
          !String(this._layoutBStoryPreviewContentKey || '').startsWith('stored:');
        const allowStoredPreview = !skipStoredPreview && (!hasRepeatedLayoutBKeyword || !existingLivePreview);
        const usedStoredPreview = allowStoredPreview
          ? await this._tryStoredLayoutBStoryPreview(dk, storedPreviewKey, {
      ...
      // No stored preview was available, so fall back to the expensive live render.
      ...
      try {
        if (gen !== this._layoutBPreviewGeneration) {
          return;
        }
        if (!arch || typeof arch.generateInstagramStoryLayoutBBlob !== 'function') {
          throw new Error('Layout B story export unavailable');
        }
        // Ensure compose is loaded — globalThis.ensureLayoutBComposeReady is not set on mobile,
        // so call the instance method directly here.
        ...
        // Capacitor 7 supports blob: URLs in WKWebView via the URL scheme handler.
        ...
        if (hint) {
          ...
          hint.textContent = `Preview error: ${msg.slice(0, 80)}`;
        }
      }
    }
