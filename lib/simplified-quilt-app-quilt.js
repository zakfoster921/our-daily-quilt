    ...
    async handleAddColor() {
      ...
      try {
        ...
        if (!usedServerSubmission && !useCutReveal) {
          ...
          try {
            // Spinner covers network + a short min hold only — not apply/render.
            pendingServerResult = await this.submitColorToServer(selectedColor);
        ...
        if (result || pendingServerResult || cutRevealFinished) {
          if (typeof window.odqTrack === 'function') {
          ...
          if (pendingServerResult && !result) {
            try {
              ...
              this.logger.warn('Apply after color submit failed:', applyErr);
              this.uiService.showToast(applyErr.message || failureMessage);
              return;
            }
          }
          ...
          if (cutRevealQuiltHandoffPromise) {
            ...
            void (async () => {
              try {
                await new Promise((resolve) => requestAnimationFrame(resolve));
                await this.renderQuilt({ force: true, viewportOnly: true });
                this.ensureQuiltScreenInteractive?.();
                void this.refreshAdminDailyTaskBanner?.();
                this._scheduleAdminDailyTaskBannerPolling?.();
                this.ensureFooterIconStripHandCut?.();
