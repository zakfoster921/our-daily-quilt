...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    async _tryStoredLayoutBStoryPreview(dateKey, previewContentKey = '', options = {}) {
      ...
      /** Prefer app preview JPEG (~400 KB), then nightly PNG, then layered fallback. */
    ...
    async refreshStoredQuiltScreenLayerAfterQuiltUpdate(options = {}) {
      ...
      try {
        ...
        /** Best-effort full story preview JPEG — never blocks the quilt-screen upload above. */
        let storyLayoutBPreviewImageData = null;
        if (typeof arch.generateInstagramStoryLayoutBPreviewJpegImageData === 'function') {
          try {
            storyLayoutBPreviewImageData = await timed(
              'Layout B story preview JPEG after quilt update',
              arch.generateInstagramStoryLayoutBPreviewJpegImageData(blocks, quote, dateKey),
              120000
            );
          } catch (err) {
            this.logger?.warn?.('Layout B story preview JPEG refresh after quilt update skipped:', err);
          }
          if (storyLayoutBPreviewImageData) {
            await uploadSnapshot({ ...baseMeta, storyLayoutBPreviewImageData });
            this.logger?.log?.('✅ Stored story preview JPEG refreshed after quilt update');
          }
        }
