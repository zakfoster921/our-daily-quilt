...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    scheduleStoredStorySnapshotRefreshAfterQuiltUpdate(options = {}) {
      ...
      this.logger?.log?.('Stored quilt-screen layer auto-refresh skipped to keep quilt scrolling smooth', { dateKey });
    }
    ...
    async refreshStoredStorySnapshotAfterServerColorSubmission(options = {}) {
      ...
      if (!Array.isArray(blocks) || blocks.length <= 1) return;
      const arch = this.archiveService;
      if (!arch?.generateInstagramStoryLayoutBImage || typeof Utils.writeInstagramImagesDocForZapier !== 'function') {
        return;
      }
      ...
      try {
        ...
        const storyLayoutBImageData = await arch.generateInstagramStoryLayoutBImage(blocks, quote, dateKey);
        const quiltScreen9x16ImageData =
          typeof arch.generateInstagramQuiltScreen9x16ImageData === 'function'
            ? await arch.generateInstagramQuiltScreen9x16ImageData(blocks, dateKey).catch(() => null)
            : null;
        if (!storyLayoutBImageData && !quiltScreen9x16ImageData) return;
        await Utils.writeInstagramImagesDocForZapier({
          dateKey,
