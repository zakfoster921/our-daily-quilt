...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    getEffectiveQuiltQuote() {
      if (this._adminPreview?.quote) return this._adminPreview.quote;
    ...
    applyQuiltMoodTriptychPalette(blocks = this.quiltEngine?.blocks || []) {
      ...
      const vars = {
        '--quilt-mood-good-fill': good.ink || goodFill,
        '--quilt-mood-good-fill-light': good.light,
        '--quilt-mood-good-fill-dark': good.dark,
        '--quilt-mood-rough-fill': rough.ink || roughFill,
        '--quilt-mood-rough-fill-light': rough.light,
        '--quilt-mood-rough-fill-dark': rough.dark
      };
      for (const el of [
    ...
    async _layoutBStoryPreviewSpeakerSig(quote) {
      const cached = String(
        this._layoutBStoryPreviewSpeakerPaintUrl || this._quoteSpeakerCutoutUrl || ''
      ).trim();
    ...
    async _layoutBStoryPreviewPreloadedSpeakerDataUrl(paintUrl, quote = null, options = {}) {
      ...
      if (arch._proxyImageDataUrlCache?.has(normalized)) {
        return String(arch._proxyImageDataUrlCache.get(normalized) || '').trim();
      }
      const storagePath =
      ...
      if (storagePath && arch._proxyImageDataUrlCache?.has(`firebase-storage:${storagePath}`)) {
        return String(arch._proxyImageDataUrlCache.get(`firebase-storage:${storagePath}`) || '').trim();
      }
    ...
    async _refreshLayoutBStoryPreviewCore() {
      ...
      const paintUrl = String(
        this._layoutBStoryPreviewSpeakerPaintUrl || this._quoteSpeakerCutoutUrl || speakerSig || ''
      ).trim();
    ...
    async _primeQuiltQuoteChrome() {
      ...
      void this.applyQuoteScreenClipping({
        dateKey,
        quote: todayQ,
        forceRefresh: !!freshness?.assignmentNewerThanClippingMeta
      });
    ...
    async resolveNightlyNewspaperClippingUrl(dateKey, opts = {}) {
      const dk = String(dateKey || '').trim();
      if (dk && !opts.ignoreAssignmentFreshness) {
        const freshness = await this._readNightlyClippingFreshness(dk);
        if (freshness?.assignmentNewerThanClippingMeta) {
          console.info(
            `[our-daily] Nightly clipping metadata for ${dk} predates assignment; skipping Storage PNG`
          );
          return null;
        }
      }
      const url = await this.fetchNewspaperClippingUrl(dateKey, opts);
      if (!url || !(await this._probeRemoteImageUrl(url))) return null;
    ...
    remeasureQuoteScreenClipping() {
      ...
      if (!img || img.hidden || !(img.naturalWidth > 0)) return;
    ...
    async _applyQuoteScreenClippingWork({ dateKey = null, quote = null, forceRefresh = false } = {}) {
      ...
      const clippingMeta = dk
        ? await this.fetchNewspaperClippingMeta(dk, { forceRefresh, waitForFirebaseMs: 4000 })
        : { url: '', exportRev: '' };
      const freshness = dk ? await this._readNightlyClippingFreshness(dk) : null;
      const skipStoragePng = !!freshness?.assignmentNewerThanClippingMeta;
      let todayQ = await this._resolveQuoteForClippingCenter(dk, quote, {
        preferLiveAssignment: skipStoragePng
      });
      if (!todayQ) {
        todayQ =
          this.quoteService?.getTodayQuote?.() ||
      ...
      const showPng = async (layoutMeta = null, stamp = '') => {
        ...
        requestAnimationFrame(() => {
          this.remeasureQuoteScreenClipping();
        });
        if (document.getElementById('screen-quote')?.classList.contains('active')) {
          window.app?.notifyQuoteScreenLayoutBPainted?.();
        }
      ...
      const url =
        !skipStoragePng && dk ? clippingMeta.url || (await this.fetchNewspaperClippingUrl(dk)) : null;
