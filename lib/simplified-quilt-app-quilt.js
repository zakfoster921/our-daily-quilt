  ...
  class SimplifiedQuiltAppV2Quilt {
      ...
      this._quiltLiveListenerUnsub = window.firestore.onSnapshot(
        ...
        (snap) => {
          if (this.isAdminTomorrowPreviewActive?.()) {
            // #region agent log
            fetch('http://127.0.0.1:7433/ingest/0ed8adaa-5aed-4571-811f-aadcc7a8fddc',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'6959a3'},body:JSON.stringify({sessionId:'6959a3',runId:'initial',hypothesisId:'H4',location:'lib/simplified-quilt-app-quilt.js:attachQuiltLiveListener:snapshotDuringPreview',message:'live quilt snapshot ignored during admin preview',data:{listenerKey:key,previewDateKey:this.getEffectiveAppDateKey?.(),snapshotExists:snap.exists(),currentBlocks:this.quiltEngine?.blocks?.length||0},timestamp:Date.now()})}).catch(()=>{});
            // #endregion
            return;
          }
    ...
    async renderQuilt(opts = {}) {
      ...
      if (
        ...
        if (typeof this._scheduleStoryPreviewOnce === 'function') {
    ...
    async _refreshLayoutBStoryPreviewCore() {
      ...
      if (await this._tryStoredNativeLayoutBStoryPreview(dk, `stored:${dk}`)) {
        return;
      }
      ...
      if (await this._tryStoredNativeLayoutBStoryPreview(dk, previewContentKey)) {
        return;
      }
    ...
    _openDailyQuoteFromNotification() {
    ...
    setupPushNotificationListeners() {
      ...
      messaging?.addListener('notificationActionPerformed', (event) => {
        ...
        this._openDailyQuoteFromNotification();
      });
