...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    applyContributorNameContrast() {
      ...
      if (stage) {
        stage.style.setProperty('--contributor-name-color', '#3d687f');
        stage.style.setProperty('--contributor-name-current-color', '#2f5368');
        stage.dataset.quiltTone = 'light';
      }
    }
    ...
    fitContributorCloudToViewport(listEl) {
      ...
      const maxWidth = Math.floor(panelBody.clientWidth);
      ...
      const bleedH = 14;
      const bleedV = 10;
      const availW = Math.max(1, maxWidth - bleedH * 2);
      const availH = Math.max(1, maxHeight - bleedV * 2);

      const measureBounds = () => {
        const names = listEl.querySelectorAll('.quilt-contributor-name');
        const heading = panelBody.querySelector('.quilt-contributors-heading');
        let left = Infinity;
        let right = -Infinity;
        let top = Infinity;
        let bottom = -Infinity;
        [heading, ...names].filter(Boolean).forEach((el) => {
        ...
        if (!Number.isFinite(left)) return { width: 0, height: 0 };
        return {
      ...
      while (!fits(scale) && scale > minScale) {
        scale = Math.max(minScale, scale * 0.94);
      }

      while (scale < maxScale) {
        const grown = Math.min(maxScale, scale * 1.03);
        if (!fits(grown)) break;
        scale = grown;
      }
    }
