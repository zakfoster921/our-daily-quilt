...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    async runPostColorCutRevealSubmit(selectedColor) {
      ...
      let quiltActivatedUnderMat = false;
      try {
        ...
        const activateQuiltUnderMat = () => {
          if (quiltActivatedUnderMat) return;
          quiltActivatedUnderMat = true;
          // After land, before cuts — not during scrap holds (showScreen hitchs stop-motion).
        ...
        const paintQuiltUnderMat = () => {
          ...
          paintUnderMatInFlight = (async () => {
            try {
              activateQuiltUnderMat();
              // Don't mark painted until the new color is applied — avoids fading to a stale quilt.
              if (!serverResult?.success) return;
              await startApplyIfNeeded();
              if (applyPromise) {
                applyResult = (await applyPromise) || applyResult;
              }
              if (quiltPaintedUnderMat) return;
        ...
        // Hydrate under the mat while cuts still run so the last frame only fades.
        void Promise.resolve(submitPromise).then((serverData) => {
          if (!serverData?.success || handoffFired) return;
          const tryPaint = () => {
            if (handoffFired || quiltPaintedUnderMat) return;
            if (!quiltActivatedUnderMat) {
              setTimeout(tryPaint, 40);
              return;
            }
            void paintQuiltUnderMat();
          };
          tryPaint();
        });
        ...
        const fireQuiltHandoff = () => {
          ...
          // Fade immediately when quilt is already under the mat — no last-frame stall.
          const startFadeNow = () => {
        ...
        try {
          playResult = await controller.playWithSubmission(submitPromise, {
            ...
            onBeforeCuts: () => {
              activateQuiltUnderMat();
              // Kick paint as soon as the quilt is live under the mat.
              void paintQuiltUnderMat();
              return new Promise((resolve) => {
                requestAnimationFrame(() => requestAnimationFrame(resolve));
              });
