...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    async _ensureLayoutBStoryPreviewShareBlobCore() {
      ...
      const quiltUrl = String(layerUrls?.quiltUrl || this._layoutBPreviewObjectUrl || '').trim();
      const overlayUrl = String(
        layerUrls?.overlayUrl || this._layoutBPreviewOverlayObjectUrl || ''
      ).trim();
      if (quiltUrl && overlayUrl) {
