...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    hasSeenCurrentWhatsNew() {
      const config = this.getWhatsNewConfig();
      if (!config) return true;
      try {
        return localStorage.getItem('ourDailyLastSeenWhatsNewVersion') === String(config.version);
      } catch (_) {
        return true;
      }
    }
    ...
    markWhatsNewSeen() {
      ...
      if (!config) return;
      try {
        localStorage.setItem('ourDailyLastSeenWhatsNewVersion', String(config.version));
      } catch (_) {
        /* ignore */
      }
    }
    ...
    shouldOfferWhatsNewAnnouncement() {
      const config = this.getWhatsNewConfig();
      if (!config || this.hasSeenCurrentWhatsNew()) return false;
    ...
    async maybeShowWhatsNewAnnouncement() {
      ...
      if (typeof window.odqTrack === 'function') {
        window.odqTrack('whats_new_shown', { version });
      }
      ...
      if (result === 'primary') {
        if (typeof window.odqTrack === 'function') {
          window.odqTrack('whats_new_open_settings', { version });
        }
      ...
      if (typeof window.odqTrack === 'function') {
        window.odqTrack('whats_new_dismissed', { version, action: result });
      }
      return true;
    }
    ...
    /** Pale tinted paper for speaker cutout — washed from today's quilt palette. */
    getPaleQuiltPaperColor(blocks = this.quiltEngine?.blocks || []) {
      ...
      const add = (color) => {
        ...
        if (!/^#[0-9A-Fa-f]{6}$/.test(value) || seen.has(value.toLowerCase())) return;
        seen.add(value.toLowerCase());
        palette.push(value);
      };
      (this.getQuiltPaletteSummary(blocks)?.palette || []).forEach((item) => add(item?.color));
      (Array.isArray(blocks) ? blocks : []).forEach((block) => add(block?.color));
      if (SCR?.resolveSpeakerCardColorsFromQuilt && SCR.washOutSpeakerQuiltColor) {
        const resolved = SCR.resolveSpeakerCardColorsFromQuilt({ palette, washColor: fallback });
        return SCR.washOutSpeakerQuiltColor(
          resolved.washPlateColor || resolved.torsoColor || resolved.faceColor || fallback,
          'paper'
        );
      }
      return '#f6f4f1';
