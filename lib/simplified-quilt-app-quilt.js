...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    _isRectInViewport(rect, margin = 32) {
    ...
    _mountColorSubmitFlyerBox(flyer, fromRect, toRect) {
      flyer.style.left = `${fromRect.left}px`;
      flyer.style.top = `${fromRect.top}px`;
      flyer.style.width = `${Math.max(fromRect.width, 1)}px`;
      flyer.style.height = `${Math.max(fromRect.height, 1)}px`;
      flyer.style.transform = 'none';
      flyer.style.transition = 'none';
      flyer.dataset.endLeft = String(toRect.left);
      flyer.dataset.endTop = String(toRect.top);
      flyer.dataset.endWidth = String(Math.max(toRect.width, 1));
      flyer.dataset.endHeight = String(Math.max(toRect.height, 1));
    }
    ...
    async playColorSubmitFlyToQuilt(block, color = '') {
      ...
      const markerSvg = targetMarker ? this._cloneColorSubmitFlyMarkerSvg(targetMarker) : null;
      if (markerSvg) {
        flyerInner.appendChild(markerSvg);
      } else {
      ...
      const mountFlyer = (startRect, endRect) => {
        this._mountColorSubmitFlyerBox(flyer, startRect, endRect);
      };
      ...
      await new Promise((resolve) => {
        ...
        const startFlight = () => {
          let startRect = pieceFrame.getBoundingClientRect();
          if (startRect.width < 2 || startRect.height < 2) startRect = fromRect;

          const refreshed = this._resolveColorSubmitFlyTarget(engineBlock, color);
          if (refreshed?.rect && this._isRectInViewport(refreshed.rect)) {
            ...
            if (groupTarget?.group) {
              ...
              if (this._isRectInViewport(toRect) && toRect.width > 2 && toRect.height > 2) {
                mountFlyer(startRect, toRect);
              }
            }
          }
          ...
          flyer.style.transition = `left ${duration}ms ${easing}, top ${duration}ms ${easing}, width ${duration}ms ${easing}, height ${duration}ms ${easing}`;
          overlay.style.transition = `opacity ${Math.round(duration * 0.94)}ms ease-out`;
          requestAnimationFrame(() => {
            requestAnimationFrame(() => {
              flyer.style.left = `${flyer.dataset.endLeft}px`;
              flyer.style.top = `${flyer.dataset.endTop}px`;
