...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    async playColorSubmitPieceReveal(block) {
      if (!block) return;
      ...
      const colorSheet = overlay.querySelector('.color-submit-transition__color-sheet');
      const messageEl = overlay.querySelector('.color-submit-transition__message');
      if (!previewSvg || !pieceStage || !pieceFrame) return;

      const shrinkFrom = this._colorSubmitShrinkStartScale();
      overlay.classList.remove('color-submit-transition--shrink-settled', 'color-submit-transition--piece-exit');
      ...
      pieceFrame.appendChild(previewSvg);
      ...
      await this.wait(reduceMotion ? 850 : 1100);
    ...
    async handleAddColor() {
      try {
        ...
        try {
          ...
          serverDuplicate = serverResult?.duplicate === true || result?.duplicate === true;
          this.scheduleStoredStorySnapshotRefreshAfterQuiltUpdate?.({
        ...
        if (result) {
          ...
          if (dedicatedBlock && !serverDuplicate && !result.duplicate) {
            await this.playColorSubmitPieceReveal(dedicatedBlock);
          } else if (!usedServerSubmission) {
            await minimumTransitionHold;
          }
