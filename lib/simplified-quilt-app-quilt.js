  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    async _composeStoredLayoutBStoryPreviewLayers(quiltUrl, overlayUrl) {
      const loadImage = async (url) => {
        ...
        if (!res.ok) throw new Error(`Could not fetch story layer (${res.status})`);
    ...
    async _prepareStoredLayoutBStoryPreviewShareBlob(url, contentKey) {
      ...
      try {
        ...
        if (!res.ok) return;
        const blob = await res.blob();
        if (!blob || !/^image\//i.test(String(blob.type || 'image/png'))) return;
    ...
    async _ensureLayoutBStoryPreviewShareBlobCore() {
      ...
      if (shareUrl && !hasOverlay) {
        await this._prepareStoredLayoutBStoryPreviewShareBlob(shareUrl, this._layoutBStoryPreviewContentKey);
