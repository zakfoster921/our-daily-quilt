    ...
    async runPostColorCutRevealSubmit(selectedColor) {
      ...
      try {
        ...
        const fireQuiltHandoff = () => {
          ...
          // Fade the mat immediately — do not call showScreen/chrome first (that
          // was freezing the last cut frame for several seconds).
          const host = document.getElementById('postColorCutRevealHost');
          if (host) host.classList.add('post-color-cut-reveal-host--handoff');
