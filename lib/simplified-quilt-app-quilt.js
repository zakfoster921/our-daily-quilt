...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    fitContributorCloudToViewport(listEl) {
      ...
      const measure = () => Math.ceil(panelBody.getBoundingClientRect().height);
      let scale = 1;
      let height = measure();

      if (height > viewportHeight) {
        scale = Math.max(minScale, (viewportHeight / height) * 0.98);
        listEl.style.setProperty('--contributor-cloud-base-size', String(scale));
        height = measure();
        while (height > viewportHeight && scale > minScale) {
