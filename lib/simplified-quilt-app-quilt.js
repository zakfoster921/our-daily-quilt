...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    async _waitForQuiltBlockScreenRect(blockId, attempts = 20) {
      ...
      for (let attempt = 0; attempt < attempts; attempt += 1) {
        ...
        if (target?.group) {
          ...
          const measureEl = target.marker || target.group;
          const rect = measureEl.getBoundingClientRect();
          if (rect.width > 2 && rect.height > 2) {
            return { rect, group: target.group, marker: target.marker || null };
          }
        }
        await this.wait(40);
      }
    ...
    async _resolveColorSubmitFlyTarget(blockId, userColor = '') {
      const id = String(blockId || '').trim();
      if (!id) return null;
      for (let attempt = 0; attempt < 50; attempt += 1) {
        ...
        if (attempt > 8 && typeof this.getMyBlockGroupElement === 'function') {
          const group = this.getMyBlockGroupElement();
          if (group) {
            const rect = group.getBoundingClientRect();
            if (rect.width > 2 && rect.height > 2) {
              return { rect, group, marker: null };
            }
          }
        }
        await this.wait(50);
    ...
    _isDedicatedHstBlock(block) {
      return block?.patternType === 'special' && block?.specialPatternType === 'hst';
    }
    ...
    _createColorSubmitPieceNode(block, color) {
      ...
      if (previewSvg) return previewSvg;
      const liteSvg = this._buildDedicatedBlockPreviewSvg(block, color);
      if (liteSvg) return liteSvg;
    ...
    async playColorSubmitPieceReveal(block, color = '') {
      ...
      const pieceNode = block ? this._createColorSubmitPieceNode(block, color) : null;
      const pieceStage = overlay.querySelector('.color-submit-transition__piece-stage');
      const pieceFrame = overlay.querySelector('.color-submit-transition__piece-frame');
      const colorSheet = overlay.querySelector('.color-submit-transition__color-sheet');
      const messageEl = overlay.querySelector('.color-submit-transition__message');
      ...
      const settledRect = pieceFrame.getBoundingClientRect();
      if (settledRect.width > 2 && settledRect.height > 2) {
        overlay.dataset.colorSubmitFlyFrom = JSON.stringify({
    ...
    _colorSubmitFlyTransformAt(motion, progress = 0) {
      const t = Math.min(1, Math.max(0, progress));
      const eased = 1 - (1 - t) ** 3;
      const scaleX = 1 + (motion.scaleX - 1) * eased;
      const scaleY = 1 + (motion.scaleY - 1) * eased;
      return `translate(-50%, -50%) translate(${motion.translateX * eased}px, ${motion.translateY * eased}px) scale(${scaleX}, ${scaleY})`;
    }

    _animateColorSubmitFlyer(flyer, motion, duration) {
      return new Promise((resolve) => {
        const start = performance.now();
        let rafId = 0;
        const tick = (now) => {
          const raw = Math.min(1, (now - start) / duration);
          flyer.style.transform = this._colorSubmitFlyTransformAt(motion, raw);
          if (raw < 1) {
        ...
        flyer.style.transform = this._colorSubmitFlyTransformAt(motion, 0);
        rafId = requestAnimationFrame(tick);
    ...
    async playColorSubmitFlyToQuilt(block, color = '') {
      ...
      let fromRect = pieceFrame.getBoundingClientRect();
      const storedFrom = overlay.dataset.colorSubmitFlyFrom;
      if (storedFrom) {
        try {
          const parsed = JSON.parse(storedFrom);
          if (parsed?.width > 2 && parsed?.height > 2) fromRect = parsed;
        } catch (_) {
          /* ignore */
        }
      }
      ...
      const target = await this._resolveColorSubmitFlyTarget(blockId, color);
      ...
      await this.waitForNextPaint();
      await this.waitForNextPaint();
      const measureEl = targetMarker || targetGroup;
      toRect = measureEl.getBoundingClientRect();
      ...
      const motion = this._computeColorSubmitFlyTransform(fromRect, toRect);
      const prevGroupOpacity = targetGroup.style.opacity;
      targetGroup.style.opacity = '0';

      const sourceSvg = pieceFrame.querySelector('.color-submit-transition__piece-svg, svg');
      const flyer = document.createElement('div');
      flyer.className = 'color-submit-transition__flyer color-submit-transition__flyer--animating';
      ...
      if (sourceSvg) {
        ...
        flyerInner.appendChild(svgClone);
      } else {
      ...
      await new Promise((resolve) => {
        ...
        const done = () => {
          ...
          delete overlay.dataset.colorSubmitFlyFrom;
          this.hideColorSubmitTransition();
          resolve();
        };
        ...
        requestAnimationFrame(() => {
          requestAnimationFrame(async () => {
            overlay.style.transition = `opacity ${Math.round(duration * 0.68)}ms ease-out`;
            overlay.style.transitionDelay = `${Math.round(duration * 0.3)}ms`;
            overlay.style.opacity = '0';
            try {
              await this._animateColorSubmitFlyer(flyer, motion, duration);
            } catch (flyErr) {
              this.logger.warn('Color submit fly animation failed:', flyErr);
            }
            done();
          });
        });
