...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    async _waitForQuiltBlockScreenRect(blockId, attempts = 20) {
      const id = String(blockId || '').trim();
      if (!id) return null;
      for (let attempt = 0; attempt < attempts; attempt += 1) {
        const target =
          typeof this.findQuiltBlockGroup === 'function' ? this.findQuiltBlockGroup(id) : null;
        if (target?.group) {
          this._scrollQuiltBlockIntoView(target.group);
          await this.waitForNextPaint();
          const rect = target.group.getBoundingClientRect();
          if (rect.width > 2 && rect.height > 2) {
            return { rect, group: target.group };
          }
        }
        await this.wait(40);
      }
      ...
      if (block.specialPatternType === 'hst' || (Array.isArray(block.hstTriangles) && block.hstTriangles.length)) {
        let tris =
          typeof Utils.getHstDrawTriangles === 'function'
            ? Utils.getHstDrawTriangles(block, { exactPlacement: true, forceClassicPair: true })
            : [];
        ...
        if (!picked.length && tris.length === 1) {
          picked = tris;
        }
    ...
    async playColorSubmitFlyToQuilt(block, color = '') {
      ...
      const target = await this._waitForQuiltBlockScreenRect(blockId);
      ...
      let { rect: toRect, group: targetGroup } = target;
      const prevGroupOpacity = targetGroup.style.opacity;
      ...
      await new Promise((resolve) => {
        ...
        const done = () => {
          ...
          finished = true;
          flyer.remove();
          targetGroup.style.opacity = prevGroupOpacity || '';
        ...
        const startFlight = () => {
          ...
          if (refreshedTarget?.group) {
            ...
            toRect = refreshedTarget.group.getBoundingClientRect();
