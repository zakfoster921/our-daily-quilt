  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    fitContributorCloudToViewport(listEl) {
      ...
      const measureAtScale = (scale, compact) => {
        ...
        return {
          ...
          overflows: contentOverflowsPanel(),
        };
      };

      const fitsAtScale = (scale, compact = true) => {
        const probe = measureAtScale(scale, compact);
        return probe.contentHeight <= maxPanelHeight && !probe.overflows;
      };

      // Important: seed with minScale. If nothing fits, the old code left scale at 1
      // and overflow:hidden clipped the bottom names out of view.
      let lo = minScale;
      let hi = maxScale;
      let bestScale = minScale;
      let foundFit = fitsAtScale(minScale, true);
      if (foundFit) {
        for (let i = 0; i < 32 && hi - lo > 0.004; i += 1) {
          const mid = (lo + hi) / 2;
          if (fitsAtScale(mid, true)) {
            bestScale = mid;
            lo = mid;
          } else {
            hi = mid;
      ...
      const bestProbe = measureAtScale(bestScale, true);
      const fillRatio = bestProbe.contentHeight / Math.max(1, maxPanelHeight);
      // Dense / heavily scaled clouds stay packed (compact). Spreading with
      // space-between clips names once the frame is full.
      const useCompact = fillRatio < 0.72 || bestScale < 0.55 || !foundFit;
      let finalProbe = measureAtScale(bestScale, useCompact);
      if (finalProbe.overflows || finalProbe.contentHeight > maxPanelHeight) {
        ...
        while (scale > absoluteMinScale && guard < 40) {
          ...
          if (retry.contentHeight <= maxPanelHeight && !retry.overflows) {
            bestScale = scale;
            finalProbe = retry;
            break;
          }
    ...
    async _refreshLayoutBStoryPreviewCore(options = {}) {
      ...
      const allowLiveCompose =
        this._layoutBStoryPreviewAllowLiveCompose === true || this.isAdminTomorrowPreviewActive?.();

      if (!allowLiveCompose) {
        if (!this._layoutBStoryPreviewStoredUrlsMissing) {
          showPreviewFailure();
          return;
        }
      } else if (this._layoutBStoryPreviewAllowLiveCompose) {
        this._layoutBStoryPreviewAllowLiveCompose = false;
      }
    ...
    async refreshStoredQuiltScreenLayerAfterQuiltUpdate(options = {}) {
      ...
      try {
        ...
        if (window.app && typeof window.app.initializeFirebaseForImages === 'function') {
          await window.app.initializeFirebaseForImages();
        }
        const dateKey = String(options.dateKey || this.getEffectiveAppDateKey?.() || Utils.getTodayKey()).trim();
        const qs = this.quoteService;
        const quote =
        ...
        const timed = (label, promise, timeoutMs) =>
          typeof globalThis.odqPromiseWithTimeout === 'function'
            ...
            : promise;
        const [quiltScreen9x16ImageData, storyLayoutBPreviewImageData] = await Promise.all([
          timed(
          ...
          arch.generateInstagramStoryLayoutBPreviewJpegImageData
            ? timed(
        ...
        if (!quiltScreen9x16ImageData && !storyLayoutBPreviewImageData) return;
        const uploadPayload = {
          dateKey,
        ...
        try {
          ...
          this.logger?.warn?.('IG preview client upload after quilt update failed, trying backend', clientUploadErr);
        ...
        this.logger?.log?.('✅ Stored quilt-screen + story preview refreshed after quilt update');
      } catch (error) {
