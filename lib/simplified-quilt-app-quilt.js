    ...
    async runPostColorCutRevealSubmit(selectedColor) {
      ...
      try {
        ...
        const fireQuiltHandoff = () => {
          ...
          // Quilt under the still-opaque mat (fast path skips heavy chrome). Keep the
          // mat covering until blocks paint — fading earlier shows a white flash.
          this.hideQuoteColorPickerPane?.();
          this.uiService.showScreen('screen-quilt', { fastQuiltHandoff: true });

          quiltHandoffPromise = (async () => {
            const host = document.getElementById('postColorCutRevealHost');
            const fadeMatAway = async () => {
              if (host?.isConnected) {
                host.classList.add('post-color-cut-reveal-host--handoff');
                await this.wait(280);
              }
              this._destroyPostColorCutRevealHost();
            };
            try {
              if (!applyPromise && serverResult) {
                applyPromise = this.applyServerColorSubmissionResult(
                  serverResult,
                  selectedColor
                )
                  .then((result) => {
        ...
        const submitPromise = this.submitColorToServer(selectedColor).then((serverData) => {
          if (serverData?.success) {
            ...
            // Hydrate quilt data during remaining cuts so handoff only needs a fast paint.
            if (!applyPromise && !handoffFired) {
              applyPromise = this.applyServerColorSubmissionResult(serverData, selectedColor)
                .then((result) => {
                  applyResult = result;
                  return result;
                })
