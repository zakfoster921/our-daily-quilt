...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    async _holdColorSubmitFindingOverlay(findingStartedAt, shapeConfirmedAt = findingStartedAt) {
      const minEndAt = Math.max(
        findingStartedAt + 320,
        shapeConfirmedAt + 180
      );
      const waitMs = Math.max(0, minEndAt - Date.now());
    ...
    async handleAddColor() {
      try {
        ...
        let shapeConfirmedAt = findingStartedAt;
        const previewBlock = this._previewDedicatedBlockForColor(selectedColor);
        if (previewBlock) shapeConfirmedAt = Date.now();
        ...
        const showTryAgain = async () => {
          await this._holdColorSubmitFindingOverlay(findingStartedAt, shapeConfirmedAt);
