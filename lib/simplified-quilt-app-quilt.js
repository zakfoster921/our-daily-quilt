...
(function (root) {
  ...
  /** Bundled compose pipeline vs Firestore nightly PNG — prefer client compose when app is ahead. */
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    /** HTTP Storage PNG already decoded on quote screen or an existing triptych mount. */
    _reuseLoadedNewspaperClippingSrc(dateKey = '') {
      ...
      const candidates = [
        ...
        document.getElementById('quiltMoodSpread')?.querySelector('.quilt-mood-triptych__quote-img')
      ];
      for (const img of candidates) {
        if (!img) continue;
        const src = String(img.currentSrc || img.src || '').trim();
        if (!isHttp(src) || !(img.naturalWidth > 0 && img.naturalHeight > 0)) continue;
        return src;
      }
      if (dk && this._quoteScreenClippingMountedKey === dk) {
    ...
    async resolveNightlyNewspaperClippingUrl(dateKey, opts = {}) {
      ...
      if (isStoredClippingExportStale(meta?.exportRev)) {
        console.info(
          `[our-daily] Nightly clipping PNG rev ${meta.exportRev} < bundled ${globalThis.QuiltNewspaperClipping?.CLIPPING_EXPORT_REV}; client compose`
        );
        return null;
      }
      const rev =
    ...
    async _applyQuoteScreenClippingWork({ dateKey = null, quote = null, forceRefresh = false } = {}) {
      ...
      const skipStoragePng = skipForAssignment || skipForStaleExport;
