    ...
    fitContributorCloudToViewport(listEl) {
      ...
      const measureBlockHeight = () => {
        ...
        if (heading) {
          ...
          top = Math.min(top, headingRect.top);
          bottom = Math.max(bottom, headingRect.bottom);
        }
        names.forEach((el) => {
        ...
        if (top === bodyRect.bottom && bottom === bodyRect.top) return 0;
        return Math.ceil(bottom - top + bleedPad);
      };

      let scale = 1;
      let height = measureBlockHeight();

      if (height > maxHeight) {
        scale = Math.max(minScale, (maxHeight / height) * 0.96);
