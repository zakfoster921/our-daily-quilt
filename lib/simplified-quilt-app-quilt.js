...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    getQuiltMoodTriptychFills(blocks = this.quiltEngine?.blocks || []) {
      const DEFAULT_GOOD = '#df9368';
      const DEFAULT_ROUGH = '#7eb0c8';
      ...
      if (n === 1) {
        ...
        const hsv = Utils.hexToHsv(lone);
        const complement = Utils.hsvToHex(
          (hsv.h + 165) % 360,
          Math.max(35, Math.min(70, hsv.s * 0.62 + 10)),
          Math.max(58, Math.min(84, hsv.v * 0.88))
        );
        return {
          goodFill: this._softenQuiltMoodTriptychFill(lone) || DEFAULT_GOOD,
          roughFill: this._softenQuiltMoodTriptychFill(complement) || DEFAULT_ROUGH
        };
      }

      let goodIdx = rotate % n;
      let roughIdx = (rotate + 1) % n;
      if (this._quiltMoodTriptychHueDistance(candidates[goodIdx], candidates[roughIdx]) < 36) {
        roughIdx = (goodIdx + Math.min(2, n - 1)) % n;
        if (roughIdx === goodIdx) roughIdx = (roughIdx + 1) % n;
      }

      return {
        goodFill: this._softenQuiltMoodTriptychFill(candidates[goodIdx]) || DEFAULT_GOOD,
        roughFill: this._softenQuiltMoodTriptychFill(candidates[roughIdx]) || DEFAULT_ROUGH
      };
    }
    ...
    applyQuiltMoodTriptychPalette(blocks = this.quiltEngine?.blocks || []) {
      ...
      screen.style.setProperty('--quilt-mood-good-fill', goodFill);
      screen.style.setProperty('--quilt-mood-rough-fill', roughFill);
    }
