...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    async _tryStoredLayoutBStoryPreview(dateKey, previewContentKey = '', options = {}) {
      ...
      let url = storyUrl || quiltScreenUrl;
      let layeredBlob = null;
      let layeredObjectUrl = '';
      if (overlayUrl && quiltScreenUrl) {
        layeredBlob = await this._composeStoredLayoutBStoryPreviewLayers(quiltScreenUrl, overlayUrl).catch(() => null);
        if (layeredBlob) {
          layeredObjectUrl = URL.createObjectURL(layeredBlob);
          url = layeredObjectUrl;
        }
      }
      if (!url) return false;
      ...
      return await new Promise((resolve) => {
        ...
        const finish = (ok) => {
          ...
          if (ok) {
            ...
            if (layeredObjectUrl) {
              this._revokeLayoutBPreviewObjectUrl();
              this._layoutBPreviewObjectUrl = layeredObjectUrl;
            }
            ...
            this._layoutBStoryPreviewFrozenDateKey = dk;
            this._layoutBStoryPreviewShareBlob = layeredBlob || null;
            if (!layeredBlob) {
    ...
    scheduleStoredStorySnapshotRefreshAfterQuiltUpdate(options = {}) {
      ...
      if (!dateKey) return;
      const quietMs = Math.max(5000, Number(options.quietMs) || 10000);
      const firstDelayMs = Math.max(5000, Number(options.delayMs) || 12000);
      if (!this._quiltScreenLayerRefreshInteractionTrackingReady && typeof window !== 'undefined') {
        this._quiltScreenLayerRefreshInteractionTrackingReady = true;
        this._quiltScreenLayerRefreshLastInteractionAt = Date.now();
      ...
      const scheduleAttempt = (delayMs) => {
        this._storySnapshotRefreshScheduledTimer = setTimeout(() => {
          this._storySnapshotRefreshScheduledTimer = null;
          const run = () => {
            const lastInteractionAt = Number(this._quiltScreenLayerRefreshLastInteractionAt) || 0;
            const quietForMs = Date.now() - lastInteractionAt;
            if (lastInteractionAt && quietForMs < quietMs) {
              scheduleAttempt(Math.max(1000, quietMs - quietForMs + 500));
              return;
            }
            void this.refreshStoredQuiltScreenLayerAfterQuiltUpdate({
              dateKey: this._storySnapshotRefreshScheduledDateKey || dateKey
            });
          };
