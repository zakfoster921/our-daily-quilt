...
(function (root) {
  ...
  /** Quote screen Firebase PNG — horizontal inset (each side), matches mood collage. */
  const QUOTE_SCREEN_CLIPPING_SIDE_MARGIN_PX = 12;
  /**
   * Quilt-screen IG story preview (below reflection / popular color).
   */
  const SHOW_QUILT_SCREEN_STORY_PREVIEW = false;
  /**
   * When true, quilt-screen preview uses vinyl quote-fit story (slide 0 + 0b speaker)
   * instead of Layout B collage. Loads a stored predecessor JPEG; uploads on save for the next visitor.
   */
  const USE_STORY_QUOTE_FIT_PREVIEW = true;
  const STORY_QUOTE_FIT_PREVIEW_RENDER_VERSION = () =>
    String(globalThis.ODQ_STORY_QUOTE_FIT_PREVIEW_RENDER_VERSION || 'story-quote-fit-preview-v3').trim();



  function quoteScreenClippingSideMarginPx() {
    const vw = typeof window !== 'undefined' ? Number(window.innerWidth) : 390;
    return Math.max(
      QUOTE_SCREEN_CLIPPING_SIDE_MARGIN_PX,
      Math.min(18, Math.round((vw || 390) * 0.032))
    );
  }

  function quoteScreenClippingCappedWidthPx(slotEl, layoutW) {
    const target = Math.max(1, Math.round(Number(layoutW) || 0));
    if (!target) return 0;
    const margin = quoteScreenClippingSideMarginPx() * 2;
    const screen = typeof document !== 'undefined' ? document.getElementById('screen-quote') : null;
    const containerW = Math.max(
      1,
      slotEl?.clientWidth || screen?.clientWidth || vwFallback()
    );
    const viewportW = Math.max(1, vwFallback());
    const cap = Math.min(target, containerW - margin, viewportW - margin);
    return Math.max(220, Math.round(cap));

    function vwFallback() {
      return typeof window !== 'undefined' ? Number(window.innerWidth) || 390 : 390;
    }
  }
  ...
  function readNewspaperClippingDisplayMetaFromDoc(data) {
    const row = data && typeof data === 'object' ? data : {};
    const renderWidth = Number(row.newspaperClippingRenderWidth);
    const effectiveBodyDomPx = Number(row.newspaperClippingEffectiveBodyDomPx);
    const clippedWidth = Number(row.newspaperClippingClippedWidth);
    return {
      renderWidth: Number.isFinite(renderWidth) && renderWidth > 0 ? Math.round(renderWidth) : 0,
      effectiveBodyDomPx:
        Number.isFinite(effectiveBodyDomPx) && effectiveBodyDomPx > 0
          ? Math.round(effectiveBodyDomPx * 10) / 10
          : 0,
      clippedWidth:
        Number.isFinite(clippedWidth) && clippedWidth > 0 ? Math.round(clippedWidth) : 0
    };
  }

  function newspaperClippingMetaFromDoc(data) {
    return {
      url: readNewspaperClippingUrlFromDoc(data) || null,
      exportRev: readNewspaperClippingExportRevFromDoc(data),
      ...readNewspaperClippingDisplayMetaFromDoc(data)
    };
  }
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    getEffectiveQuiltQuote() {
      ...
      if (previewKey) {
        // Admin preview resolves Notion date_scheduled first; do not let a stale pin win.
    ...
    _syncAdminPreviewQuoteFromPin() {
      ...
      if (!dk || !this._adminPreview?.quote?.text) return;
      const resolution = String(this._adminPreview?.resolution || '');
      const previewQuote = this._adminPreview.quote;
      if (
        resolution === 'notion_schedule' ||
        resolution === 'notion_live' ||
        resolution === 'backend_catalog' ||
        resolution.includes('notion_live_same_quote') ||
        resolution === 'notion_live_source'
      ) {
        if (this.quoteService?._pinnedByDateKey) {
          this.quoteService._pinnedByDateKey[dk] = previewQuote;
        }
        return;
      }
    ...
    _invalidateQuiltChromeForAdminDatePreview() {
      ...
      if (spreadHost) {
        ...
        spreadHost.classList.remove('is-ready', 'is-layer-ready', 'is-open');
      }
      ...
      const speakerImg = document.getElementById('quoteSpeakerImage');
      if (speakerImg) {
        speakerImg.hidden = true;
        speakerImg.removeAttribute('src');
        speakerImg.classList.remove('is-loaded');
        delete speakerImg.dataset.plainCutoutComposite;
        delete speakerImg.dataset.plainCutoutDirect;
      }
      ...
      if (imageWrap) {
        ...
        imageWrap._plainCutoutBackingInFlight = false;
        imageWrap._plainCutoutBackingQueuedSig = '';
        imageWrap._plainCutoutPaperSig = '';
        imageWrap._plainCutoutPaperGen = (imageWrap._plainCutoutPaperGen || 0) + 1;
    ...
    hasLoggedDailyQuoteSeenAnalytics(currentDate = this.getDailyVisitDateKey()) {
      try {
        return localStorage.getItem('ourDailyQuoteSeenAnalyticsDate') === currentDate;
      } catch (_) {
        return false;
      }
    }

    markDailyQuoteSeenAnalyticsSent(currentDate = this.getDailyVisitDateKey()) {
      try {
        localStorage.setItem('ourDailyQuoteSeenAnalyticsDate', currentDate);
      } catch (_) {
        /* */
      }
    }

    _trackDailyQuoteSeenAnalytics(currentDate = this.getDailyVisitDateKey()) {
      if (this.hasLoggedDailyQuoteSeenAnalytics(currentDate)) return;
      if (typeof window.odqTrack !== 'function') return;
      const sent = window.odqTrack('daily_quote_seen', { app_date_key: currentDate });
      if (sent) this.markDailyQuoteSeenAnalyticsSent(currentDate);
    }

    replayPendingAnalyticsEvents() {
      const currentDate = this.getDailyVisitDateKey();
      if (this.hasSeenDailyQuoteToday(currentDate)) {
        this._trackDailyQuoteSeenAnalytics(currentDate);
      }
    }
    ...
    resetFirstTimeUserState() {
      try {
        ...
        localStorage.removeItem('ourDailyQuoteSeenAnalyticsDate');
        ...
        localStorage.removeItem('ourDailyIntroPersonaVersionSeen');
        localStorage.removeItem('ourDailyVisitCount');
        localStorage.removeItem('ourDailyWhatsNewSeen');
        localStorage.removeItem('ourDailyLastSeenWhatsNewVersion');
    ...
    readAppVisitCount() {
      try {
        const raw = localStorage.getItem('ourDailyVisitCount');
        if (raw != null && raw !== '') {
          const count = parseInt(raw, 10);
          if (Number.isFinite(count) && count >= 0) return count;
        }
        if (localStorage.getItem('ourDailyHasVisited')) {
          localStorage.setItem('ourDailyVisitCount', '2');
          return 2;
        }
      } catch (_) {
        /* ignore */
    ...
    incrementAppVisitCount() {
      try {
        const next = this.readAppVisitCount() + 1;
        localStorage.setItem('ourDailyVisitCount', String(next));
        return next;
      } catch (_) {
        return 0;
      }
    ...
    /** Fire-and-forget anonymous daily open (once per device/day) for funnel + return rate. */
    _reportDayVisitOnce() {
      const dk = String(Utils.getTodayKey?.() || this.getDailyVisitDateKey?.() || '').trim();
      if (!dk) return;
      const storageKey = 'ourDailyQuiltDayVisitV1';
      try {
      ...
      if (this._dayVisitReportInFlightKey === dk) return;
      ...
      this._dayVisitReportInFlightKey = dk;
      void fetch(`${baseUrl}/api/day-visit`, {
        method: 'POST',
        ...
        .finally(() => {
          if (this._dayVisitReportInFlightKey === dk) {
            this._dayVisitReportInFlightKey = '';
          }
        });
    }
    ...
    async autoTransitionFromPortal() {
      try {
        ...
        const appVisitCount = this.readAppVisitCount();
        const hasVisitedBefore = localStorage.getItem('ourDailyHasVisited');
        const lastVisitDate = localStorage.getItem('ourDailyLastVisitDate');
        ...
        console.log('🔄 Visit check:', {
          ...
          isNewDay,
          hasSeenQuoteToday,
          hasCompletedColorToday
        });

        const showQuoteScreen = async () => {
        ...
        if (!hasVisitedBefore) {
          ...
          this.uiService.showScreen('screen-first-name');
        } else if (this.shouldOfferIntroPersonaForReturningUser(currentDate)) {
          console.log('🔄 Returning user — showing one-time Zak intro persona');
          if (isNewDay) {
    ...
    getIntroPersonaConfig() {
      const config = CONFIG?.APP?.introPersona;
    ...
    hasSeenCurrentIntroPersona() {
      const config = this.getIntroPersonaConfig();
      if (!config) return true;
      try {
        return localStorage.getItem('ourDailyIntroPersonaVersionSeen') === String(config.version);
      } catch (_) {
        return true;
      }
    }

    markIntroPersonaSeen() {
      const config = this.getIntroPersonaConfig();
      if (!config) return;
      try {
        localStorage.setItem('ourDailyIntroPersonaVersionSeen', String(config.version));
      } catch (_) {
        /* ignore */
      }
    }

    shouldOfferIntroPersonaForReturningUser(currentDate = this.getDailyVisitDateKey()) {
      const config = this.getIntroPersonaConfig();
      if (!config || this.hasSeenCurrentIntroPersona()) return false;
      if (this.hasCompletedDailyColorToday(currentDate)) return false;
      try {
        if (!localStorage.getItem('ourDailyHasVisited')) return false;
      } catch (_) {
        return false;
      }
      if (typeof Utils !== 'undefined' && typeof Utils.needsFirstNamePrompt === 'function' && Utils.needsFirstNamePrompt()) {
    ...
    getWhatsNewConfig() {
      const config = CONFIG?.APP?.whatsNew;
      if (!config || typeof config !== 'object') return null;
      return config;
    }

    hasSeenWhatsNewAnnouncement() {
      try {
        if (localStorage.getItem('ourDailyWhatsNewSeen') === '1') return true;
        if (localStorage.getItem('ourDailyLastSeenWhatsNewVersion')) return true;
      } catch (_) {
        return true;
      }
      return false;
    ...
    markWhatsNewSeen() {
      try {
        localStorage.setItem('ourDailyWhatsNewSeen', '1');
      } catch (_) {
        /* ignore */
      }
    }

    shouldOfferWhatsNewAnnouncement() {
      const config = this.getWhatsNewConfig();
      if (!config || this.hasSeenWhatsNewAnnouncement()) return false;
      if (this.readAppVisitCount() < 2) return false;
      if (this._whatsNewAnnouncementInFlight || this._whatsNewModalOpen) return false;
      if (typeof this.isIntroFlowEnabled === 'function' && this.isIntroFlowEnabled()) return false;
      if (this._skipFastPathForRememberToday) return false;
      if (document.getElementById('screen-remember-today-view')?.classList.contains('active')) return false;
      const activeId = document.querySelector('.screen.active')?.id || '';
      return activeId === 'screen-quilt';
    }

    openSettingsForDailyQuoteTime() {
      this.prepareSettingsScreen({ renderPreview: false });
      this.uiService.showScreen('screen-settings');
      ...
      window.setTimeout(() => {
        ...
        if (timeRow) {
          try {
            timeRow.scrollIntoView({ block: 'center', behavior: 'smooth' });
          } catch (_) {
            timeRow.scrollIntoView();
          }
        }
        if (hourSelect && !hourSelect.disabled) {
          try {
            hourSelect.focus({ preventScroll: true });
          } catch (_) {
            hourSelect.focus();
          }
        }
    ...
    async maybeShowWhatsNewAnnouncement() {
      if (!this.shouldOfferWhatsNewAnnouncement()) return false;
      const config = this.getWhatsNewConfig();
      if (!config || !this.uiService?.showWhatsNewAnnouncement) return false;

      this._whatsNewAnnouncementInFlight = true;
      this._whatsNewModalOpen = true;
      this._quiltBottomModalShownThisVisit = true;
      const visitCount = this.readAppVisitCount();

      if (typeof window.odqTrack === 'function') {
        window.odqTrack('whats_new_shown', { visitCount });
      }

      let result = 'dismiss';
      try {
        result = await this.uiService.showWhatsNewAnnouncement({
          title: config.title,
          body: config.body,
          primaryLabel: config.primaryLabel,
          secondaryLabel: config.secondaryLabel,
          revealDelayMs: CONFIG.APP.whatsNewRevealDelayMs
        });
      } catch (error) {
        this.logger.warn('Whats-new announcement failed:', error);
      } finally {
        this._whatsNewAnnouncementInFlight = false;
        this._whatsNewModalOpen = false;
        this.markWhatsNewSeen();
      }

      if (result === 'primary') {
        if (typeof window.odqTrack === 'function') {
          window.odqTrack('whats_new_open_settings', { visitCount });
        }
        this.openSettingsForDailyQuoteTime();
        return true;
      }

      if (typeof window.odqTrack === 'function') {
        window.odqTrack('whats_new_dismissed', { visitCount, action: result });
      }
      return true;
    }

    featureFeedbackNudgeShownCount() {
      try {
        const raw = localStorage.getItem('ourDailyFeatureFeedbackNudgeShownCount_v1');
        const count = parseInt(raw, 10);
    ...
    recordFeatureFeedbackNudgeShown() {
      try {
        const next = this.featureFeedbackNudgeShownCount() + 1;
        localStorage.setItem('ourDailyFeatureFeedbackNudgeShownCount_v1', String(next));
        localStorage.setItem('ourDailyFeatureFeedbackNudgeShownAtVisit_v1', String(this.readAppVisitCount()));
      } catch (_) {
        /* ignore */
      }
    }

    /** Time-boxed feedback push: stops offering itself after CONFIG.APP.featureFeedbackNudgeSunsetIso. */
    shouldOfferFeatureFeedbackNudge() {
      const sunsetIso = CONFIG.APP.featureFeedbackNudgeSunsetIso;
      if (sunsetIso && Date.now() >= new Date(sunsetIso).getTime()) return false;
      if (this.readFeatureFeedbackState?.().submittedAtIso) return false;

      const shownCount = this.featureFeedbackNudgeShownCount();
      if (shownCount >= 2) return false;
      if (this.readAppVisitCount() < 2) return false;

      if (shownCount === 1) {
        const shownAtVisit = parseInt(
          localStorage.getItem('ourDailyFeatureFeedbackNudgeShownAtVisit_v1'),
          10
        ) || 0;
        if (this.readAppVisitCount() < shownAtVisit + 2) return false;
      }

      return document.querySelector('.screen.active')?.id === 'screen-quilt';
    }

    async maybeOfferFeatureFeedbackNudge() {
      if (this._quiltBottomModalShownThisVisit) return false;
      if (!this.uiService?.showWhatsNewAnnouncement) return false;
      if (!this.shouldOfferFeatureFeedbackNudge()) return false;

      this._quiltBottomModalShownThisVisit = true;
      this.recordFeatureFeedbackNudgeShown();

      let choice = 'dismiss';
      try {
        choice = await this.uiService.showWhatsNewAnnouncement({
          title: "It's time to streamline!",
          body: 'Help me shape ODQ for the future with a few quick taps.',
          primaryLabel: 'Give feedback',
    ...
    detachQuiltLiveListener() {
      if (typeof this._quiltLiveListenerUnsub === 'function') {
        try {
          this._quiltLiveListenerUnsub();
        } catch (_) {
          /* */
        }
      }
      this._quiltLiveListenerUnsub = null;
      this._quiltLiveListenerKey = null;
    }

    attachQuiltLiveListener(dateKey = Utils.getTodayKey()) {
      const key = String(dateKey || '').trim();
      if (!key || !window.db || !window.firestore?.doc || typeof window.firestore.onSnapshot !== 'function') {
        return;
      }
      if (this._quiltLiveListenerKey === key && typeof this._quiltLiveListenerUnsub === 'function') {
        return;
      }
      this.detachQuiltLiveListener();
      const ref = window.firestore.doc(window.db, 'quilts', key);
      this._quiltLiveListenerKey = key;
      this._quiltLiveListenerUnsub = window.firestore.onSnapshot(
        ref,
        (snap) => {
          ...
          this._reconcileMirrorTuneFromRemotePayload?.(key, data);

          const blocks = Array.isArray(data.blocks) ? data.blocks : [];
          if (blocks.length === 0) return;
          const nextFp = Utils.computeQuiltFingerprint(blocks);
          const currentFp = Utils.computeQuiltFingerprint(this.quiltEngine?.blocks || []);
          if (nextFp === currentFp) return;
          void this.applyQuiltDataFromPayload({
            dateKey: key,
            date: data.date || key,
            blocks,
            ...
            this.logger.log('🔄 Quilt updated from live Firestore listener');
            this.renderQuilt?.();
            this.updateSquareCounter();
            this.renderQuiltContributorList?.();
            this._scheduleQuiltScreenIgUploadIfStale?.({ dateKey: key, delayMs: 1500 });
          });
        },
        (error) => {
          this.logger.warn('Quilt live listener error:', error?.message || error);
        }
      );
    }

    _scheduleLaunchQuiltSyncRetry(delayMs = 2000) {
      clearTimeout(this._launchQuiltSyncRetryTimer);
      this._launchQuiltSyncRetryTimer = setTimeout(() => {
        this._launchQuiltSyncRetryTimer = null;
        if (!this._isAppForegroundForQuiltSync()) return;
        void this.syncLiveDailyData({ mode: 'launch-core' }).then((result) => {
          if (!result.ok) {
            this._scheduleLaunchQuiltSyncRetry(Math.min(Math.round(delayMs * 1.5), 30000));
            return;
          }
          ...
          if (!this._postLiveSyncInitialized) {
            void this._continueAfterLiveSync();
            return;
          }
          this.renderQuilt?.();
        });
    ...
    _isAppForegroundForQuiltSync() {
      const native =
        typeof window.odqIsCapacitorNative === 'function' && window.odqIsCapacitorNative();
      if (native && this._capacitorAppIsActive === false) return false;
    ...
    setupQuiltResumeRefresh() {
      ...
      this._capacitorAppIsActive = true;
      const schedule = () => {
      ...
      try {
        ...
        if (capacitorApp && typeof capacitorApp.addListener === 'function') {
          capacitorApp.addListener('appStateChange', (state) => {
            this._capacitorAppIsActive = state?.isActive !== false;
      ...
      const native =
        typeof window.odqIsCapacitorNative === 'function' && window.odqIsCapacitorNative();
      if (native) {
        // iOS WebViews often miss visibility/focus; poll while the native app is active.
        this._nativeQuiltSyncPollTimer = setInterval(() => {
          if (!this._liveDailyDataConfirmed || this._isPersonalQuiltMode) return;
          schedule();
        }, 90000);
      }
    }
    ...
    formatDailyQuiltResetLocalLabel(from = new Date()) {
      return this.getNextQuiltResetTime(from).toLocaleTimeString([], {
        hour: 'numeric',
        minute: '2-digit'
      });
    }
          ...
          if (!archiveSuccess) {
            ...
            try {
              ...
              const emergencyEntry = {
                ...
                contributorCount: this._humanContributorPickCount(),
    ...
    _scheduleResumeQuiltSyncRetry(delayMs = 8000) {
      ...
      this._resumeQuiltSyncRetryTimer = setTimeout(() => {
        ...
        if (!this._isAppForegroundForQuiltSync()) return;
        if (this.isAdminTomorrowPreviewActive?.()) return;
        if (!this._hasCachedTodayQuilt() && !this._liveDailyDataConfirmed) return;
    ...
    getAdminSampleContributorName(index = 0) {
      const names = [
        ...
        'Abby', 'Aiden', 'Alex', 'Aliyah', 'Bella', 'Blake', 'Bryce', 'Caleb',
        'Chloe', 'Cole', 'Dana', 'Drew', 'Elena', 'Eli', 'Emma', 'Ethan',
        'Grace', 'Harper', 'Hazel', 'Hudson', 'Hunter', 'Jace', 'Jamie',
        'Jaylen', 'Jesse', 'Jordan', 'Josie', 'Juliet', 'Kaia', 'Kellen', 'Leah',
        'Liam', 'Lily', 'Logan', 'Lydia', 'Maeve', 'Mason', 'Maya',
        'Miles', 'Nadia', 'Nate', 'Noel', 'Olivia', 'Parker', 'Payton', 'Riley',
        'River', 'Ruby', 'Ryan', 'Sam', 'Savannah', 'Skylar', 'Sofia', 'Spencer',
        'Tatum', 'Taylor', 'Tobias', 'Violet', 'Wyatt',
        'Audrey', 'Beau', 'Brielle', 'Camden', 'Delilah', 'Emmett', 'Finley', 'Gracie',
        'Harlow', 'Isaiah', 'Jenna', 'Knox', 'Lacey', 'Lennon', 'Maci', 'Nathaniel',
        'Presley', 'Reese', 'Sloane', 'Stellan', 'Tristan', 'Waverly'
      ];
      const displayOrder = [
        98, 34, 26, 5, 45, 88, 96, 18, 32, 55,
        66, 100, 105, 11, 74, 60, 58, 0, 75, 83,
        77, 21, 14, 41, 35, 3, 53, 23, 39, 61,
        27, 44, 33, 7, 108, 25, 87, 109, 106, 20,
        54, 103, 71, 56, 36, 79, 91, 47, 68, 15,
        17, 30, 104, 10, 76, 64, 19, 59, 80, 93,
        67, 107, 102, 37, 99, 95, 29, 97, 6, 31,
        92, 72, 73, 51, 43, 110, 52, 78, 65, 42,
        1, 4, 38, 69, 94, 70, 82, 22, 48, 16,
        50, 81, 85, 89, 13, 8, 57, 84, 63, 49,
        86, 101, 90, 12, 46, 2, 40, 24, 62, 9, 28
      ];
    ...
    recordAdminGeneratedContributor(count = 1) {
      ...
      for (let i = 0; i < normalizedCount; i++) {
        ...
        next.push({
          ...
          firstContributedAt: new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000).toISOString()
        });
      }
    ...
    getContributorNameContrastPalette() {
      const frameInk = '#43677b';
      return {
        tone: 'light',
        color: frameInk,
        currentColor: frameInk,
      };
    }
    ...
    fitContributorCloudToViewport(listEl) {
      if (!listEl) return;
      const panelBody = listEl.closest('.quilt-contributor-panel-body');
      const stage = listEl.closest('.quilt-contributors-stage');
      if (!panelBody || !stage || stage.hidden) return;

      const maxPanelHeight = Math.floor(panelBody.clientHeight);
      if (!maxPanelHeight) return;

      // Prefer a readable floor; if the cloud is still too dense, keep shrinking.
      const minScale = 0.22;
      const absoluteMinScale = 0.08;
      const maxScale = 1.45;
      const edgePad = 2;
      const bottomPad = 2;
      const headingEl = panelBody.querySelector('.quilt-contributors-heading:not(.quilt-contributors-heading--footer)');
      const footerEl = panelBody.querySelector('.quilt-contributors-heading--footer');
      const applyScale = (value) => {
        listEl.style.setProperty('--contributor-cloud-base-size', String(value));
      };

      const getContentHeight = () => {
        let height = 0;
        if (headingEl) height += Math.ceil(headingEl.offsetHeight);
        height += Math.ceil(listEl.scrollHeight);
        if (footerEl) height += Math.ceil(footerEl.offsetHeight);
        return height;
      };
      ...
      const getFitBounds = () => {
        ...
        return {
          ...
          right: rect.right - edgePad,
          bottom: rect.bottom - bottomPad,
        };
      };

      const contentOverflowsPanel = () => {
        const bounds = getFitBounds();
        return Array.from(panelBody.querySelectorAll('.quilt-contributor-name, .quilt-contributors-heading')).some((el) => {
      ...
      const measureAtScale = (scale, compact) => {
        applyScale(scale);
        panelBody.classList.toggle('quilt-contributor-panel-body--compact', compact);
        void listEl.offsetHeight;
        const listOverflows = listEl.scrollHeight > listEl.clientHeight + 1;
        return {
          contentHeight: getContentHeight(),
          overflows: contentOverflowsPanel() || listOverflows,
          listOverflows,
        };
      };

      const fitsAtScale = (scale) => {
        const probe = measureAtScale(scale, true);
        return probe.contentHeight <= maxPanelHeight && !probe.overflows;
      };

      // Always measure/fit in compact (packed) mode. The old path switched to
      // space-between once the cloud was ~full; that stretched the list and
      // overflow:hidden clipped the bottom names — even around ~40 contributors.
      let lo = minScale;
      let hi = maxScale;
      let bestScale = minScale;
      let foundFit = fitsAtScale(minScale);
      if (foundFit) {
        for (let i = 0; i < 32 && hi - lo > 0.004; i += 1) {
          const mid = (lo + hi) / 2;
          if (fitsAtScale(mid)) {
            bestScale = mid;
            lo = mid;
        ...
        let scale = minScale;
        let guard = 0;
        while (scale > absoluteMinScale && guard < 40) {
          scale = Math.max(absoluteMinScale, scale * 0.9);
        ...
        if (!foundFit) bestScale = absoluteMinScale;
      }

      let finalProbe = measureAtScale(bestScale, true);
      if (finalProbe.overflows || finalProbe.contentHeight > maxPanelHeight) {
        let guard = 0;
        let scale = bestScale;
        while (scale > absoluteMinScale && guard < 40) {
          scale = Math.max(absoluteMinScale, scale * 0.94);
          const retry = measureAtScale(scale, true);
          bestScale = scale;
          finalProbe = retry;
          if (retry.contentHeight <= maxPanelHeight && !retry.overflows) break;
          guard += 1;
        }
      }
      measureAtScale(bestScale, true);
    }
    ...
    renderContributorCloud(listEl, entries, variant, currentUserId) {
      ...
      (Array.isArray(entries) ? entries : []).forEach((entry, index) => {
        ...
        const washOpacity = 0.86 + ((Math.floor(styleHash / 5) % 14) / 100);
        const inkOpacity = isCurrent ? Math.min(1, washOpacity + 0.05) : washOpacity;
        ...
        item.style.setProperty('--contributor-ink-opacity', inkOpacity.toFixed(3));
        if (glimmerActive) {
        ...
        if (isCurrent) {
          ...
          heartSvg.setAttribute('class', 'quilt-contributor-name__heart');
    ...
    renderQuiltContributorList() {
      ...
      if (show) {
        const runFit = () => this.fitContributorCloudToViewport(list);
        requestAnimationFrame(runFit);
        // Refit after Zakarack loads — first paint often measures with a fallback font.
        if (typeof document !== 'undefined' && document.fonts?.ready?.then) {
          document.fonts.ready.then(runFit).catch(() => {});
        }
      } else {
        list.style.removeProperty('--contributor-cloud-base-size');
        list.closest('.quilt-contributor-panel-body')
          ?.classList.remove('quilt-contributor-panel-body--compact');
      }
    }
    ...
    async handleSettingsNameSave() {
      ...
      if (normalized.toLowerCase() !== 'friend' && typeof Utils.validateContributorName === 'function') {
        const validation = await Utils.validateContributorName(normalized);
        if (!validation.ok) {
          this.uiService.showToast(validation.error || Utils.CONTRIBUTOR_NAME_MODERATION_ERROR);
          return;
        }
      }
    ...
    async saveQuilt(saveOptions = null) {
      try {
        ...
        const blocks = this.quiltEngine.blocks.map((block) => {
          const serialized = this.quiltEngine._serializeBlockForReplay(block);
          const retroOriginal = this.quiltEngine.harmonyRetroOriginalColor?.(block.id);
          if (retroOriginal) serialized.color = retroOriginal;
          return serialized;
        });
        const persistOpts =
        ...
        if (this.quiltEngine.macroLayoutMode === 'columns' || this.quiltEngine.macroLayoutMode === 'default') {
          persistOpts.macroLayoutMode = this.quiltEngine.macroLayoutMode;
        }
        ...
        .then(async () => {
          ...
          const beforeFingerprint =
            typeof Utils.computeQuiltFingerprint === 'function'
              ? Utils.computeQuiltFingerprint(this.quiltEngine?.blocks || [])
              : '';
          try {
            ...
            this._scheduleQuiltScreenIgRefreshAfterBlocksChange?.({
              beforeFingerprint,
    ...
    async getAdminServerMutationToken() {
      ...
      if (!token) {
        const entered = window.prompt('Paste the Railway RESET_TOKEN for admin quilt saves. It will be saved in this browser.');
    ...
    buildAdminQuiltMutationStatePayload(label) {
      const blocks = this.quiltEngine.blocks.map((block) => {
        const serialized = this.quiltEngine._serializeBlockForReplay(block);
        const retroOriginal = this.quiltEngine.harmonyRetroOriginalColor?.(block.id);
        if (retroOriginal) serialized.color = retroOriginal;
      ...
      return {
        ...
        label: String(label || 'Admin quilt mutation'),
        appDateKey: Utils.getTodayKey(),
        clientBuildId: CONFIG.APP?.buildId || '',
        blocks,
        contributorCount: this._humanContributorPickCount(),
        colorReplayEvents: this.quiltEngine.getColorReplayEvents(),
        contributors: this.dailyContributors,
        macroStructureFrozen: this.quiltEngine.macroStructureFrozen === true
      };
    }

    async saveAdminQuiltMutationViaServer(label) {
      ...
      const res = await fetch(`${baseUrl}/api/admin/quilt-mutation`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-reset-token': token
        },
        body: JSON.stringify(this.buildAdminQuiltMutationStatePayload(label))
      });
      ...
      if (!res.ok || !data?.success) {
        throw new Error(data?.error || `Admin quilt mutation failed (${res.status})`);
      }
      await this.applyQuiltDataFromPayload(data);
      this._scheduleQuiltScreenIgRefreshAfterBlocksChange?.({
        dateKey: data.appDateKey || data.dateKey || data.date || Utils.getTodayKey()
      });
      return data;
    }

    async runAdminQuiltServerCommand(command, options = {}) {
      const baseUrl = String(CONFIG.BACKEND?.baseUrl || '').replace(/\/$/, '');
      if (!baseUrl) throw new Error('Backend URL is not configured');
      const token = await this.getAdminServerMutationToken();
      if (!token) throw new Error('Missing admin token');
    ...
    async saveAdminQuiltMutation(label) {
      ...
      try {
        await this.saveAdminQuiltMutationViaServer(label);
        return true;
      } catch (serverError) {
        this.logger.warn(`${label} server admin save failed`, serverError);
    ...
    getQuiltColorFamilySubmissionBreakdown(blocks = this.quiltEngine?.blocks || []) {
      ...
      (Array.isArray(blocks) ? blocks : []).forEach((block, index) => {
        const color = this.normalizeHexColor(block?.contributorColor || block?.color);
        if (!color) return;
    ...
    /** Speaker cutout paper — same tape field as `.quote-speaker-slab` name strip (`--odq-paper-tape`). */
    getPaleQuiltPaperColor() {
      const SCR = globalThis.SpeakerCutoutRender;
      return SCR?.SPEAKER_STRIP_PAPER || '#f2eee6';
    }
    ...
    _quiltMoodTriptychPaletteCandidates(blocks = this.quiltEngine?.blocks || []) {
      ...
      (Array.isArray(blocks) ? blocks : []).forEach((block) => {
        const hex = this._normalizeQuiltMoodTriptychHex(block?.contributorColor || block?.color);
    ...
    _getQuiltMoodTriptychUserPickHex(blocks = this.quiltEngine?.blocks || []) {
      ...
      if (dedicatedId) {
        ...
        const fromBlock = this._normalizeQuiltMoodTriptychHex(block?.contributorColor || block?.color);
    ...
    applyQuiltMoodTriptychPalette(blocks = this.quiltEngine?.blocks || []) {
      ...
      const spreadHost = document.getElementById('quiltMoodSpread');
      const frozen = this.quiltEngine?.macroStructureFrozen === true;
      const dateKey =
        typeof this.getEffectiveAppDateKey === 'function'
          ? this.getEffectiveAppDateKey()
          : Utils.getTodayKey();
      const cardColor = frozen
        ? globalThis.QuiltMoodCollageWidget?.pickMacroFreezeMoodCardColor?.(blocks, dateKey)
        : null;
      const tintOpts = { frozen, color: cardColor };
      if (spreadHost?._moodSpreadWidget?.setMacroFreezeTint) {
        spreadHost._moodSpreadWidget.setMacroFreezeTint(tintOpts);
      } else {
        globalThis.QuiltMoodCollageWidget?.applyMoodCardMacroFreezeTint?.(spreadHost, tintOpts);
      }
    }
    ...
    _shouldDeferSplitBandQuiltRender() {
      const screen = document.getElementById('screen-quilt');
      // Only defer while quilt screen is hidden (portal/quote preload). Once active,
      // render immediately — QuiltMirrorLayout.resolveViewport() falls back to window size.
      return !screen?.classList.contains('active');
    }

    _quiltContainerHasLayoutSize() {
      const rect = document.querySelector('#screen-quilt .quilt-container')?.getBoundingClientRect?.();
      return !!(rect && rect.width > 8 && rect.height > 8);
    }

    _scheduleSplitBandViewportReflowAfterLayout() {
      if (!this.renderer?.isQuiltSplitBandFitEnabled?.()) return;
      if (this._splitBandViewportReflowRetry) return;
      this._splitBandViewportReflowRetry = true;
      let attempts = 0;
      const maxAttempts = 120;
      const attempt = () => {
        attempts += 1;
        if (this._quiltContainerHasLayoutSize()) {
          this._splitBandViewportReflowRetry = false;
          void this.renderQuilt({ viewportOnly: true });
          return;
        }
    ...
    _scheduleSplitBandQuiltRenderRetry(opts = {}) {
      if (this._splitBandQuiltRenderRetry) {
        if (opts.force !== true) return;
        this._splitBandQuiltRenderRetry = false;
      }
      this._splitBandQuiltRenderRetry = true;
      let attempts = 0;
      const maxAttempts = 180;
      const attempt = () => {
        attempts += 1;
        const screenActive = document.getElementById('screen-quilt')?.classList.contains('active') === true;
        const ready = !this._shouldDeferSplitBandQuiltRender();
        if (ready || attempts >= maxAttempts) {
          this._splitBandQuiltRenderRetry = false;
          if (ready || screenActive) {
            void this.renderQuilt(opts);
          }
          return;
        }
        requestAnimationFrame(attempt);
    ...
    _recordQuiltMirrorViewportSize() {
      const rect = document.querySelector('#screen-quilt .quilt-container')?.getBoundingClientRect?.();
      if (rect && rect.width > 0 && rect.height > 0) {
        this._quiltMirrorViewportSize = { w: rect.width, h: rect.height };
      }
    }

    /**
     * @param {{ viewportOnly?: boolean, force?: boolean }} [opts]
     *   viewportOnly — iOS toolbar / visualViewport resize: reflow mosaic only, skip quote/speaker/preview chrome.
     *   force — paint even when split-band preload would defer (e.g. after color submit).
     */
    async renderQuilt(opts = {}) {
      ...
      const splitBandNeedsViewportReflow =
        !viewportOnly &&
        this.renderer?.isQuiltSplitBandFitEnabled?.() &&
        document.getElementById('screen-quilt')?.classList.contains('active') &&
        !this._quiltContainerHasLayoutSize();
      if (this.renderer?.isQuiltSplitBandFitEnabled?.()) {
        if (!forceRender && this._shouldDeferSplitBandQuiltRender()) {
          this._scheduleSplitBandQuiltRenderRetry(opts);
          return;
        }
      }
      ...
      if (splitBandNeedsViewportReflow) {
        this._scheduleSplitBandViewportReflowAfterLayout();
      }
      this._recordQuiltMirrorViewportSize();
      this.resetQuiltParallaxVisualState();
      this.syncQuiltZoomAfterRender();
      // Cut-reveal handoff: skip footer/grain work — mat is still covering; chrome runs after fade.
      if (opts.cutRevealHandoff !== true) {
      ...
      const adminPreviewChromeFirst = this.isAdminTomorrowPreviewActive();
      if (!adminPreviewChromeFirst || effectiveQuote?.text) {
        this.refreshQuoteSpeakerWidget(effectiveQuote);
        if (!adminPreviewChromeFirst) {
          this.refreshSeamsidePodcastWidget?.(effectiveQuote);
        }
      }
      if (!adminPreviewChromeFirst) {
        if (typeof this._scheduleStoryPreviewOnce === 'function' && !this._layoutBStoryPreviewLaunchScheduled) {
          this._scheduleStoryPreviewOnce();
        }
        this.scheduleLayoutBStoryPreviewRefresh({ defer: true });
    ...
    resetQuiltParallaxVisualState() {
      ...
      this._blockInflateCycleT = 0;
      document.getElementById('quilt')?.classList.remove('quilt--wobble-live');
      document.querySelectorAll('#quilt .quilt-parallax-block').forEach((node) => {
        if (node.closest('#quiltSpotlightOverlay')) return;
        node.setAttribute('transform', node.dataset.baseTransform || '');
      });
      // Re-renders clear transforms and can stall the RAF loop — restart scroll zoom.
    ...
    _revokeLayoutBPreviewObjectUrl() {
      ...
      if (this._layoutBPreviewOverlayObjectUrl) {
        try {
          URL.revokeObjectURL(this._layoutBPreviewOverlayObjectUrl);
        } catch (e) {
          /* ignore */
        }
        this._layoutBPreviewOverlayObjectUrl = null;
      }
    }

    _layoutBPreviewOverlayImg() {
      const frame = document.querySelector('#screen-quilt .quilt-layout-b-preview-frame');
      if (!frame) return null;
      let overlay = frame.querySelector('#quiltLayoutBPreviewOverlayImg');
      if (!overlay) {
        overlay = document.createElement('img');
        overlay.id = 'quiltLayoutBPreviewOverlayImg';
        overlay.className = 'quilt-layout-b-preview-img quilt-layout-b-preview-img--overlay';
        overlay.alt = '';
    ...
    _clearLayoutBPreviewOverlayImg() {
      const overlay = document.getElementById('quiltLayoutBPreviewOverlayImg');
      if (!overlay) return;
    ...
    /** Force quilt-screen story preview to rebuild after admin tune save. */
    _invalidateLayoutBStoryPreviewAfterTuneSave() {
      this._layoutBStoryPreviewFrozenDateKey = '';
      this._invalidateLayoutBStoryPreviewForAppDayChange({ force: true });
    }

    /** Drop cached Layout B story preview so a new app-day cannot keep yesterday's bitmap. */
    _invalidateLayoutBStoryPreviewForAppDayChange(options = {}) {
      const force = options.force === true;
      const currentDateKey =
        typeof this.getEffectiveAppDateKey === 'function' ? String(this.getEffectiveAppDateKey() || '') : '';
      const img = document.getElementById('quiltLayoutBPreviewImg');
      if (
        !force &&
        currentDateKey &&
        this._layoutBStoryPreviewFrozenDateKey === currentDateKey
      ) {
        return;
      }
      ...
      this._layoutBStoryPreviewDisplayedBlockCount = 0;
      this._layoutBStoryPreviewDisplayedFingerprint = '';
      this._layoutBStoryPreviewShareBlob = null;
      this._layoutBStoryPreviewSharePrepPromise = null;
      this._layoutBStoryPreviewLayerUrls = null;
      this._layoutBStoryPreviewFrozenDateKey = '';
      this._layoutBStoryPreviewSpeakerPaintUrl = '';
      this._layoutBStoryPreviewSpeakerUpgradeDone = false;
      this._layoutBStoryPreviewUsedCutout = false;
      this._layoutBStoryPreviewDidLiveCompose = false;
      this._layoutBStoryPreviewRerunQueued = false;
      this._layoutBStoryPreviewDisplayedStoredPlaceholder = false;
      this._layoutBStoryPreviewSkipStoredOnce = false;
      this._layoutBStoryPreviewAllowLiveCompose = false;
      this._layoutBStoryPreviewStoredUrlsMissing = false;
      this._layoutBPreviewGeneration++;
      this._lastQuiltScreenIgUploadFingerprint = '';
    ...
    scheduleLayoutBStoryPreviewRefresh(options = {}) {
      ...
      this._maybeInvalidateLayoutBStoryPreviewForQuiltGrowth();
      /** Predecessor snapshot stays on screen for the session once loaded. Tune save clears heavyDone first. */
      if (this._layoutBStoryPreviewHeavyDoneThisVisit && !this.isAdminTomorrowPreviewActive?.()) {
        return;
      }
      void this._scheduleLayoutBStoryPreviewRefreshDebounced(options);
    }

    /** Stored snapshot previews do not live-upgrade when speaker bytes arrive on the card. */
    scheduleLayoutBStoryPreviewRefreshAfterSpeaker(paintUrl) {
      ...
      if (!USE_STORY_QUOTE_FIT_PREVIEW) return;
      if (this._layoutBStoryPreviewUsedCutout) return;
      if (!this._layoutBStoryPreviewDidLiveCompose) return;
      if (this._layoutBStoryPreviewSpeakerUpgradeDone) return;
      ...
      void this._scheduleLayoutBStoryPreviewRefreshDebounced({ force: true, delayMs: 0 });
    }

    _scheduleLayoutBStoryPreviewRefreshAfterFirstScroll(options = {}) {
      this._layoutBStoryPreviewAfterScrollCleanup?.();
      const screen = document.getElementById('screen-quilt');
      let done = false;
      let timer = 0;
      let fallback = 0;
      const cleanup = () => {
        ...
        screen?.removeEventListener?.('scroll', arm);
        screen?.removeEventListener?.('wheel', arm);
        screen?.removeEventListener?.('touchend', arm);
        screen?.removeEventListener?.('pointerup', arm);
        this._layoutBStoryPreviewAfterScrollCleanup = null;
      };
      const run = () => {
        if (done) return;
        done = true;
        cleanup();
        this.scheduleLayoutBStoryPreviewRefresh({ delayMs: 0 });
      };
      function arm() {
        if (done || timer) return;
        timer = setTimeout(run, 700);
      }
      this._layoutBStoryPreviewAfterScrollCleanup = cleanup;
      if (screen) {
        screen.addEventListener('scroll', arm, { passive: true });
        screen.addEventListener('wheel', arm, { passive: true });
        screen.addEventListener('touchend', arm, { passive: true });
        screen.addEventListener('pointerup', arm, { passive: true });
      }
      fallback = setTimeout(run, Math.max(2500, Number(options.fallbackMs) || 6500));
    }

    async _scheduleLayoutBStoryPreviewRefreshDebounced(options = {}) {
      ...
      if (options.force === true) {
        this._layoutBStoryPreviewPendingRefreshOptions = { ...options, force: true };
      } else if (!this._layoutBStoryPreviewPendingRefreshOptions?.force) {
        this._layoutBStoryPreviewPendingRefreshOptions = options;
      }
      const delayMs = Number.isFinite(Number(options.delayMs))
        ? Math.max(0, Number(options.delayMs))
        : options.defer === true
          ? 1200
          : this._layoutBStoryPreviewHeavyDoneThisVisit ? 400 : 0;
      this._layoutBPreviewDebounceTimer = setTimeout(() => {
        this._layoutBPreviewDebounceTimer = null;
        const pending = this._layoutBStoryPreviewPendingRefreshOptions || options;
        this._layoutBStoryPreviewPendingRefreshOptions = null;
        void this.refreshLayoutBStoryPreview(pending);
      }, delayMs);
    }
    ...
    async resolveLayoutBStoryQuoteAndComposeOptions(dateKey, blocks, aspect = 'story') {
      ...
      try {
        const stripLayoutSeed =
          globalThis.odqGetCachedLayoutBStripLayoutSeed?.(dk, tuneAspect) ??
          (await globalThis.odqReadLayoutBStripLayoutSeed(dk, tuneAspect));
        composeExtras.stripLayoutSeed =
          globalThis.odqNormalizeStripLayoutSeed?.(stripLayoutSeed) ?? stripLayoutSeed;
        composeExtras.stripLayoutSeedExplicit = true;
        composeExtras.quiltBgZoom = globalThis.odqNormalizeQuiltBgZoom?.(
          globalThis.odqGetCachedLayoutBQuiltBgZoom?.(dk, tuneAspect) ??
            globalThis.ODQ_LAYOUT_B_QUILT_BG_ZOOM_MIN
        ) ?? globalThis.ODQ_LAYOUT_B_QUILT_BG_ZOOM_MIN;
        composeExtras.quiltBgZoomExplicit = true;
        composeExtras.quiltBgOffsetY =
          globalThis.odqNormalizeQuiltBgOffsetY?.(
            globalThis.odqGetCachedLayoutBQuiltBgOffsetY?.(dk, tuneAspect) ?? 0
          ) ?? 0;
        composeExtras.quiltBgOffsetYExplicit = true;
        if (typeof globalThis.odqGetCachedLayoutBQuoteStripOffset === 'function') {
          composeExtras.quoteStripOffset =
            globalThis.odqGetCachedLayoutBQuoteStripOffset(dk, tuneAspect) || { x: 0, y: 0 };
          composeExtras.quoteStripOffsetExplicit = true;
        } else if (typeof globalThis.odqReadLayoutBQuoteStripOffset === 'function') {
          composeExtras.quoteStripOffset = await globalThis.odqReadLayoutBQuoteStripOffset(dk, tuneAspect);
          composeExtras.quoteStripOffsetExplicit = true;
        }
        const cachedKw = globalThis.odqGetCachedLayoutBKeywordEmphasis?.(dk, tuneAspect);
        if (cachedKw) {
          composeExtras.keywordEmphasis = cachedKw;
    ...
    _renderBeforeYouGoDom() {
      ...
      const exploreTape = document.getElementById('beforeYouGoExploreTape');
      ...
      const { titleLine, blurb, iconName, iconSvg } = qs.getArtRecBeforeYouGoParts(quote);
      const showCompanion = !!String(titleLine || '').trim();
      if (exploreTape) {
        exploreTape.hidden = !showCompanion;
        exploreTape.setAttribute('aria-hidden', showCompanion ? 'false' : 'true');
      }
      if (exploreTitleEl) {
        ...
        if (showCompanion) {
          this._appendArtRecIcon(exploreTitleEl, { iconName, iconSvg });
          this._appendArtRecTitleColonItalic(exploreTitleEl, titleLine);
        }
      }
      if (exploreDescEl) {
        if (showCompanion && blurb) {
      ...
      /** 0 = friend, 1 = App Store review — paused until Android store links exist. */
      const variants = [
        { mode: 'friend-disabled', lead: 'Tell someone', label: 'Share coming soon' },
        { mode: 'review-disabled', lead: 'Help us grow', label: 'Reviews coming soon' }
      ];
      ...
      if (shareBtn) {
        ...
        shareBtn.setAttribute('aria-label', `${v.label} while Android app links are prepared`);
      }
    }
    ...
    async handleBeforeYouGoShareClick() {
      ...
      if (mode === 'friend-disabled' || mode === 'review-disabled') {
        this.uiService.showToast('Coming soon');
        return;
      }
      if (mode === 'friend') {
    ...
    _layoutBStoryPreviewIsStoredImageOlderThanTune(data) {
      if (!data || typeof data !== 'object') return false;
      const parseTime = (value) => {
        const ms = Date.parse(String(value || '').trim());
        return Number.isFinite(ms) ? ms : 0;
      };
      const tuneMs = parseTime(data.layoutBTuneUpdatedAt);
      if (!tuneMs) return false;
      const imageMs = Math.max(
        parseTime(data.lastNightlyIgImagesAt),
        parseTime(data.lastIgPushCompletedAt),
        parseTime(data.lastUpdated)
      );
      return !imageMs || tuneMs > imageMs + 1000;
    }

    async _fetchInstagramImageDocRest(dateKey) {
      const dk = String(dateKey || '').trim();
      const projectId = String(globalThis.CONFIG?.FIREBASE?.projectId || '').trim();
      const apiKey = String(globalThis.CONFIG?.FIREBASE?.apiKey || '').trim();
      if (!dk || !projectId) return null;
      const restValueToPlain = (value) => {
        ...
        if (value.mapValue?.fields) {
          ...
          Object.entries(value.mapValue.fields).forEach(([key, child]) => {
            out[key] = restValueToPlain(child);
          });
          return out;
        }
        if (Array.isArray(value.arrayValue?.values)) return value.arrayValue.values.map(restValueToPlain);
        return undefined;
      };
      const query = apiKey ? `?key=${encodeURIComponent(apiKey)}` : '';
      const url =
        `https://firestore.googleapis.com/v1/projects/${encodeURIComponent(projectId)}` +
        `/databases/(default)/documents/instagram-images/${encodeURIComponent(dk)}${query}`;
      const controller = typeof AbortController === 'function' ? new AbortController() : null;
      const abortTimer = controller ? setTimeout(() => controller.abort(), 6000) : 0;
      let res;
      try {
      ...
      Object.entries(fields).forEach(([key, value]) => {
        data[key] = restValueToPlain(value);
      });
      return data;
    }

    _layoutBStoryPreviewStoredQuiltBlockCount(data) {
      const m = String(data?.quiltFingerprint || '').match(/^qfp-v1-(\d+)-/);
      if (m) return Number(m[1]);
      const bc = Number(data?.blockCount);
      return Number.isFinite(bc) ? bc : NaN;
    }

    _layoutBStoryPreviewStoredSnapshotAcceptable(data, currentBlockCount) {
      if (currentBlockCount <= 1) return false;
      const effectiveStored = this._layoutBStoryPreviewStoredQuiltBlockCount(data);
      if (!Number.isFinite(effectiveStored) || effectiveStored <= 0) return false;
      if (effectiveStored > currentBlockCount) return false;
      /** Predecessor snapshot may trail live by at most one save — not an arbitrary old quilt. */
      return currentBlockCount - effectiveStored <= 1;
    }

    _stampLayoutBStoryPreviewDisplayedSnapshot(blockCount, fingerprint = '') {
      this._layoutBStoryPreviewDisplayedBlockCount = Math.max(0, Number(blockCount) || 0);
      this._layoutBStoryPreviewDisplayedFingerprint = String(fingerprint || '').trim();
    }

    /** Allow a new stored/live fetch when the shared quilt grew since the preview on screen. */
    _maybeInvalidateLayoutBStoryPreviewForQuiltGrowth() {
      if (!this._layoutBStoryPreviewHeavyDoneThisVisit) return;
      const blocks = this.quiltEngine?.blocks;
      const blockCount = Array.isArray(blocks) ? blocks.length : 0;
      const displayed = Number(this._layoutBStoryPreviewDisplayedBlockCount) || 0;
      if (blockCount <= displayed) return;
      this._layoutBStoryPreviewHeavyDoneThisVisit = false;
    }

    _layoutBStoryPreviewImageLoadTimeoutMs() {
    ...
    /**
     * Load stored quilt-screen (+ overlay when present). Normal users prefer layered assets
     * (~400 KB quilt JPEG + stable overlay) over the full baked story PNG (~4 MB).
     * Freshness accepts predecessor snapshots: stored block count may trail live by one.
     */
    async _tryStoredLayoutBStoryPreview(dateKey, previewContentKey = '', options = {}) {
      const dk = String(dateKey || '').trim();
      const wrap = document.getElementById('quiltLayoutBPreviewWrap');
      const img = document.getElementById('quiltLayoutBPreviewImg');
      const hint = document.getElementById('quiltLayoutBPreviewHint');
      if (!dk || !wrap || !img) return false;
      const allowStalePlaceholder = options.allowStalePlaceholder === true;
      const finalAllowed = options.finalAllowed !== false;
      const preferQuiltScreenPlaceholder = options.preferQuiltScreenPlaceholder === true;
      const preferStoryPreviewJpeg = options.preferStoryPreviewJpeg === true;
      const expectedPreviewRenderVersion = String(options.expectedPreviewRenderVersion || '').trim();
      let placeholderOnly = options.forcePlaceholderOnly === true;
      const currentBlockCount = Math.max(0, Number(options.currentBlockCount) || 0);
      let storedBlockCount = NaN;
      const clearUnsafePlaceholder = () => {
        this._layoutBStoryPreviewShareBlob = null;
        img.removeAttribute('src');
        img.hidden = true;
        this._clearLayoutBPreviewOverlayImg();
      };
      const markPlaceholderOrReject = () => {
        if (!allowStalePlaceholder) return false;
        if (
          !preferQuiltScreenPlaceholder &&
          currentBlockCount > 1 &&
          Number.isFinite(storedBlockCount) &&
          storedBlockCount <= 0
        ) {
          clearUnsafePlaceholder();
          return false;
        }
      ...
      try {
        data = await (typeof globalThis.odqPromiseWithTimeout === 'function'
          ? globalThis.odqPromiseWithTimeout(
              ...
              6000,
              `Firestore REST story preview ${dk}`
            )
          : this._fetchInstagramImageDocRest(dk));
      } catch (_) {
        this._layoutBStoryPreviewStoredUrlsMissing = true;
        return false;
      }
      storedBlockCount = Number(data?.blockCount);
      /** Speaker tune is separate from quilt block/fingerprint staleness — never show a baked
       *  story PNG when a newer tune save exists (tune modal vs quilt-screen mismatch). */
      if (this._layoutBStoryPreviewIsStoredImageOlderThanTune(data)) {
        this._layoutBStoryPreviewAllowLiveCompose = true;
        return false;
      }
      if (
        options.ignoreFreshnessChecks !== true &&
        typeof globalThis.odqMirrorTuneIsCustomized === 'function' &&
        typeof globalThis.odqReadMirrorTuneFromLocal === 'function'
      ) {
        const tune = globalThis.odqReadMirrorTuneFromLocal(dk);
        if (globalThis.odqMirrorTuneIsCustomized(tune) && !markPlaceholderOrReject()) {
          return false;
        }
      }
      if (options.ignoreFreshnessChecks !== true) {
        if (!this._layoutBStoryPreviewStoredSnapshotAcceptable(data, currentBlockCount)) {
          if (!markPlaceholderOrReject()) return false;
        }
      }
      const quiltScreenUrl = String(
      ...
      const storyPreviewUrl = String(
        data?.layoutBStoryPreviewUrl ||
          data?.storyLayoutBPreviewUrl ||
          data?.storyLayoutBPreviewImageStorageUrl ||
          ''
      ).trim();
      const storyNightlyUrl = String(
        data?.layoutBStoryUrl ||
          data?.storyLayoutBUrl ||
          data?.storyLayoutBImageStorageUrl ||
          ''
      ).trim();
      const storyUrl = storyPreviewUrl || storyNightlyUrl;
      const overlayUrl = String(
        data?.layoutBStoryOverlayUrl ||
          data?.storyLayoutBOverlayUrl ||
          data?.storyLayoutBOverlayImageStorageUrl ||
          ''
      ).trim();
      const expectedStoryRenderVersion = String(
        globalThis.ODQ_LAYOUT_B_STORY_RENDER_VERSION || 'story-strip-cluster-v3'
      ).trim();
      const storedStoryRenderVersion = String(data?.layoutBStoryRenderVersion || '').trim();
      const storedPreviewRenderVersion = String(data?.layoutBStoryPreviewRenderVersion || '').trim();
      const storyRendererVersionReleasedAt = Date.parse('2026-07-04T13:41:00.000Z');
      const storedStoryGeneratedAt = Math.max(
        Date.parse(String(data?.lastNightlyIgImagesAt || '').trim()) || 0,
        Date.parse(String(data?.lastIgPushCompletedAt || '').trim()) || 0,
        Date.parse(String(data?.lastUpdated || '').trim()) || 0
      );
      const missingVersionButFresh =
        !storedStoryRenderVersion &&
        Number.isFinite(storedStoryGeneratedAt) &&
        storedStoryGeneratedAt >= storyRendererVersionReleasedAt;
      let usableStoryPreviewUrl = storyPreviewUrl;
      if (preferStoryPreviewJpeg && expectedPreviewRenderVersion) {
        const previewVersionOk =
          !!storyPreviewUrl && storedPreviewRenderVersion === expectedPreviewRenderVersion;
        if (!previewVersionOk) {
          /** Missing / old Layout B JPEG — caller should seed upload, not live-compose. */
          this._layoutBStoryPreviewStoredRenderVersionStale = true;
          this._layoutBStoryPreviewStoredUrlsMissing = !storyPreviewUrl;
          return false;
        }
      }
      const storyVersionMismatch =
        !preferStoryPreviewJpeg &&
        !!storyUrl &&
        !preferQuiltScreenPlaceholder &&
        !(overlayUrl && quiltScreenUrl) &&
        !!expectedStoryRenderVersion &&
        !missingVersionButFresh &&
        storedStoryRenderVersion !== expectedStoryRenderVersion;
      if (storyVersionMismatch) {
        this._layoutBStoryPreviewStoredRenderVersionStale = true;
        if (!markPlaceholderOrReject()) return false;
      }
      const cacheBustKey = Math.max(
        Date.parse(String(data?.layoutBTuneUpdatedAt || '').trim()) || 0,
        Date.parse(String(data?.lastIgPushCompletedAt || '').trim()) || 0,
        Date.parse(String(data?.lastNightlyIgImagesAt || '').trim()) || 0,
        Date.parse(String(data?.lastUpdated || '').trim()) || 0,
        0
      );
      const bustStorageUrl = (url) => {
        ...
        const bust =
          typeof Utils.cacheBustInstagramStorageUrl === 'function'
            ...
            : raw.includes('?')
              ? `${raw}&odq_t=${cacheBustKey || Date.now()}`
              : `${raw}?odq_t=${cacheBustKey || Date.now()}`;
        if (cacheBustKey > 0) {
          return bust.replace(/([?&]odq_t=)\d+/, `$1${cacheBustKey}`);
        }
        return bust;
      };
      /** Prefer layered quilt-screen + speaker overlay when both exist — quilt updates without re-baking overlay. */
      let useLayeredPreview = false;
      let url = '';
      const bustedQuiltScreenUrl = bustStorageUrl(quiltScreenUrl);
      const bustedStoryPreviewUrl = bustStorageUrl(usableStoryPreviewUrl);
      const bustedStoryNightlyUrl = bustStorageUrl(storyNightlyUrl);
      const bustedStoryUrl = bustStorageUrl(
        usableStoryPreviewUrl || (preferStoryPreviewJpeg ? '' : storyUrl)
      );
      const bustedOverlayUrl = bustStorageUrl(overlayUrl);
      if (preferQuiltScreenPlaceholder) {
        url = bustedQuiltScreenUrl;
      } else if (preferStoryPreviewJpeg && usableStoryPreviewUrl) {
        url = bustedStoryPreviewUrl;
        useLayeredPreview = false;
      } else if (!preferStoryPreviewJpeg && overlayUrl && quiltScreenUrl) {
        useLayeredPreview = true;
        url = bustedQuiltScreenUrl;
      } else if (usableStoryPreviewUrl) {
        url = bustedStoryPreviewUrl;
        useLayeredPreview = false;
      } else if (!preferStoryPreviewJpeg && storyNightlyUrl) {
        url = bustedStoryNightlyUrl;
        useLayeredPreview = false;
      } else if (quiltScreenUrl) {
        url = bustedQuiltScreenUrl;
        if (!overlayUrl && allowStalePlaceholder && finalAllowed) placeholderOnly = true;
      }
      if (!url) {
        this._layoutBStoryPreviewStoredUrlsMissing = true;
        clearUnsafePlaceholder();
      ...
      this._layoutBStoryPreviewStoredUrlsMissing = false;
      const overlayImg = this._layoutBPreviewOverlayImg();
      this._revokeLayoutBPreviewObjectUrl();
      this._layoutBStoryPreviewLayerUrls = useLayeredPreview
        ? { quiltUrl: bustedQuiltScreenUrl, overlayUrl: bustedOverlayUrl }
        : null;
      this._layoutBStoryPreviewShareUrl = useLayeredPreview ? '' : url;
      ...
      return await new Promise((resolve) => {
        ...
        const finish = async (ok) => {
          ...
          if (ok) {
            const satisfiesPreview = finalAllowed && !placeholderOnly;
            img.hidden = false;
            if (useLayeredPreview && overlayImg) {
              overlayImg.removeAttribute('crossorigin');
              overlayImg.src = bustedOverlayUrl;
            ...
            if (satisfiesPreview) {
              ...
              this._layoutBStoryPreviewContentKey = previewContentKey || `stored:${dk}`;
              this._layoutBStoryPreviewSatisfiedForDateKey = dk;
              this._stampLayoutBStoryPreviewDisplayedSnapshot(
                Number.isFinite(this._layoutBStoryPreviewStoredQuiltBlockCount(data))
                  ? this._layoutBStoryPreviewStoredQuiltBlockCount(data)
                  : storedBlockCount,
                String(data?.quiltFingerprint || '').trim()
              );
              this._layoutBStoryPreviewShareBlob = null;
              if (useLayeredPreview) {
                await this._prepareStoredLayoutBStoryPreviewLayeredShareBlob(
                  quiltScreenUrl,
                  overlayUrl,
                  this._layoutBStoryPreviewContentKey
                );
              } else {
                await this._prepareStoredLayoutBStoryPreviewShareBlob(url, this._layoutBStoryPreviewContentKey);
              }
              if (hint) {
                hint.textContent = '';
                hint.hidden = true;
              ...
              this._layoutBStoryPreviewDisplayedStoredPlaceholder = true;
              this._layoutBStoryPreviewHeavyDoneThisVisit = true;
              this._layoutBStoryPreviewContentKey = previewContentKey || `stored:${dk}`;
              this._layoutBStoryPreviewSatisfiedForDateKey = dk;
              this._layoutBStoryPreviewShareBlob = null;
          ...
          resolve(ok && finalAllowed && !placeholderOnly);
        };
        img.onload = () => finish(img.naturalWidth > 0);
        img.onerror = () => {
          if (!useLayeredPreview && storyPreviewUrl && overlayUrl && quiltScreenUrl && url === bustedStoryPreviewUrl) {
            useLayeredPreview = true;
            this._layoutBStoryPreviewLayerUrls = {
              quiltUrl: bustedQuiltScreenUrl,
              overlayUrl: bustedOverlayUrl
            };
            this._layoutBStoryPreviewShareUrl = '';
            url = bustedQuiltScreenUrl;
            img.onerror = () => {
              if (storyNightlyUrl) {
                useLayeredPreview = false;
                this._layoutBStoryPreviewLayerUrls = null;
                this._layoutBStoryPreviewShareUrl = bustedStoryNightlyUrl;
                this._clearLayoutBPreviewOverlayImg();
                img.onerror = () => finish(false);
                img.onload = () => finish(img.naturalWidth > 0);
                img.src = bustedStoryNightlyUrl;
                return;
              }
              if (useLayeredPreview && overlayUrl) {
                useLayeredPreview = false;
                this._layoutBStoryPreviewLayerUrls = null;
                this._layoutBStoryPreviewShareUrl = bustedQuiltScreenUrl;
                this._clearLayoutBPreviewOverlayImg();
                img.onerror = () => finish(false);
                img.onload = () => finish(img.naturalWidth > 0);
                img.src = bustedQuiltScreenUrl;
                if (allowStalePlaceholder) placeholderOnly = true;
                return;
              }
              finish(false);
            };
          ...
          if (!useLayeredPreview && storyUrl && overlayUrl && quiltScreenUrl && url === bustedStoryUrl) {
            useLayeredPreview = true;
            this._layoutBStoryPreviewLayerUrls = {
              quiltUrl: bustedQuiltScreenUrl,
              overlayUrl: bustedOverlayUrl
        ...
        }, this._layoutBStoryPreviewImageLoadTimeoutMs());
      });
    }

    _layoutBStoryPreviewHasVisibleOverlay() {
      const overlay = document.getElementById('quiltLayoutBPreviewOverlayImg');
      return !!(overlay && !overlay.hidden && overlay.naturalWidth > 0);
    }

    _layoutBStoryPreviewResolveLayerUrls() {
      const quiltImg = document.getElementById('quiltLayoutBPreviewImg');
      const overlayImg = document.getElementById('quiltLayoutBPreviewOverlayImg');
      const quiltFromDom =
        quiltImg && !quiltImg.hidden
          ? String(quiltImg.currentSrc || quiltImg.src || '').trim()
          : '';
      const overlayFromDom =
        overlayImg && !overlayImg.hidden
          ? String(overlayImg.currentSrc || overlayImg.src || '').trim()
          : '';
      const layerUrls = this._layoutBStoryPreviewLayerUrls;
      const quiltUrl = String(
        quiltFromDom || layerUrls?.quiltUrl || this._layoutBPreviewObjectUrl || ''
      ).trim();
      const overlayUrl = String(
        overlayFromDom || layerUrls?.overlayUrl || this._layoutBPreviewOverlayObjectUrl || ''
      ).trim();
      return { quiltUrl, overlayUrl };
    }

    _setLayoutBStoryPreviewImageCrossOrigin(img, url) {
      if (!img) return;
    ...
    async _composeLayoutBStoryPreviewLayersFromDom() {
      const quiltImg = document.getElementById('quiltLayoutBPreviewImg');
      const overlayImg = document.getElementById('quiltLayoutBPreviewOverlayImg');
      if (!quiltImg || quiltImg.hidden || !(quiltImg.naturalWidth > 0)) return null;
      if (!overlayImg || overlayImg.hidden || !(overlayImg.naturalWidth > 0)) return null;
      const canvas = document.createElement('canvas');
      canvas.width = Math.max(1, quiltImg.naturalWidth || 1080);
      canvas.height = Math.max(1, quiltImg.naturalHeight || 1920);
      const ctx = canvas.getContext('2d');
      if (!ctx) return null;
      try {
        ctx.drawImage(quiltImg, 0, 0, canvas.width, canvas.height);
        ctx.drawImage(overlayImg, 0, 0, canvas.width, canvas.height);
      } catch (_) {
        return null;
      }
      return await new Promise((resolve) => {
        canvas.toBlob((blob) => resolve(blob?.size > 0 ? blob : null), 'image/png');
      });
    }

    async _composeStoredLayoutBStoryPreviewLayers(quiltUrl, overlayUrl) {
      const loadImage = async (url) => {
        const blob = await this._fetchLayoutBStoryPreviewImageBlob(url);
        if (!blob) throw new Error('Could not fetch story layer');
        if (typeof createImageBitmap === 'function') {
          return { img: await createImageBitmap(blob), objectUrl: '' };
        }
        const objectUrl = URL.createObjectURL(blob);
        const img = await new Promise((resolve, reject) => {
          const el = new Image();
          el.onload = () => resolve(el);
          el.onerror = () => reject(new Error('Could not decode story layer image'));
          el.src = objectUrl;
        });
        return { img, objectUrl };
      };
      const [quiltLayer, overlayLayer] = await Promise.all([loadImage(quiltUrl), loadImage(overlayUrl)]);
      try {
        const canvas = document.createElement('canvas');
        canvas.width = 1080;
        canvas.height = 1920;
        const ctx = canvas.getContext('2d');
        if (!ctx) throw new Error('Could not compose story preview layers');
        ctx.drawImage(quiltLayer.img, 0, 0, canvas.width, canvas.height);
        ctx.drawImage(overlayLayer.img, 0, 0, canvas.width, canvas.height);
        return await new Promise((resolve, reject) => {
          canvas.toBlob((blob) => {
            if (blob) resolve(blob);
            else reject(new Error('Could not export layered story preview'));
          }, 'image/png');
        });
      } finally {
        if (typeof quiltLayer.img.close === 'function') quiltLayer.img.close();
        if (typeof overlayLayer.img.close === 'function') overlayLayer.img.close();
        if (quiltLayer.objectUrl) URL.revokeObjectURL(quiltLayer.objectUrl);
        if (overlayLayer.objectUrl) URL.revokeObjectURL(overlayLayer.objectUrl);
      }
    }

    /** Fetch a story-preview layer for share (archive proxy first, then direct /api/proxy-image). */
    async _fetchLayoutBStoryPreviewImageBlob(url) {
      ...
      const arch = this.archiveService;
      if (arch && /^https?:/i.test(src)) {
        try {
          const dataUrl = await arch._prepareSpeakerImageUrlForCanvas(src, {
          ...
          /* Fall through to fetch helpers below. */
        }
      }
      const readBlob = async (fetchUrl) => {
        const res = await fetch(fetchUrl, { cache: 'no-store', credentials: 'omit' });
        if (!res.ok) return null;
        const blob = await res.blob();
        if (!blob || blob.size <= 0) return null;
        if (!/^image\//i.test(String(blob.type || 'image/png'))) return null;
        return blob;
      };
      try {
        const direct = await readBlob(src);
        if (direct) return direct;
      } catch (_) {
        /* Fall through to proxy-image. */
      }
      if (!/^https?:/i.test(src)) return null;
      const bases =
        typeof globalThis.odqProxyImageBases === 'function' ? globalThis.odqProxyImageBases() : [];
      const proxyFor =
        typeof globalThis.odqProxyImageFetchUrl === 'function'
          ? globalThis.odqProxyImageFetchUrl
          : null;
      if (!proxyFor || !bases.length) return null;
      for (const base of bases) {
        const proxyUrl = proxyFor(base, src);
        if (!proxyUrl) continue;
        try {
          const proxied = await readBlob(proxyUrl);
    ...
    async _prepareStoredLayoutBStoryPreviewShareBlob(url, contentKey) {
      const src = String(url || '').trim();
      if (!src) return;
      try {
        const blob = await this._fetchLayoutBStoryPreviewImageBlob(src);
        if (!blob?.size) return;
        this._layoutBStoryPreviewShareBlob = blob;
      } catch (_) {
        /* Display still works even if the stored image cannot be fetched as a share blob. */
      }
    }

    async _prepareStoredLayoutBStoryPreviewLayeredShareBlob(quiltUrl, overlayUrl, contentKey) {
      const quiltSrc = String(quiltUrl || '').trim();
      const overlaySrc = String(overlayUrl || '').trim();
      if (!quiltSrc || !overlaySrc) return;
      try {
        const blob = await this._composeStoredLayoutBStoryPreviewLayers(quiltSrc, overlaySrc);
        if (blob?.size > 0) {
          this._layoutBStoryPreviewShareBlob = blob;
        }
      } catch (_) {
        /* Long-press can retry via _ensureLayoutBStoryPreviewShareBlob. */
      }
    }

    /** Sync read of a share blob already in memory or the archive proxy cache. */
    _resolveLayoutBStoryPreviewShareBlobSync() {
      if (this._layoutBStoryPreviewShareBlob?.size > 0) {
        return this._layoutBStoryPreviewShareBlob;
      }
      const arch = this.archiveService;
      const quiltImg = document.getElementById('quiltLayoutBPreviewImg');
      const candidates = [
        ...
        .filter(Boolean);
      if (!arch || typeof Utils?.dataUrlToBlob !== 'function') return null;
      for (const url of candidates) {
        ...
        const norm =
          typeof arch._normalizeProxyImageSource === 'function'
            ? arch._normalizeProxyImageSource(url)
            : url;
        const cacheKeys = [`${norm}#rawCutout`, norm];
        const storagePath =
          typeof arch._firebaseStoragePathFromDownloadUrl === 'function'
            ? arch._firebaseStoragePathFromDownloadUrl(norm)
            : '';
        if (storagePath) {
          cacheKeys.push(`firebase-storage:${storagePath}#rawCutout`, `firebase-storage:${storagePath}`);
        }
        for (const key of cacheKeys) {
          const dataUrl = arch._proxyImageDataUrlCache?.get(key);
          if (!dataUrl || !/^data:/i.test(String(dataUrl))) continue;
          try {
            const blob = Utils.dataUrlToBlob(dataUrl);
            if (blob?.size > 0) {
              this._layoutBStoryPreviewShareBlob = blob;
    ...
    async _ensureLayoutBStoryPreviewShareBlobCore() {
      const { quiltUrl, overlayUrl } = this._layoutBStoryPreviewResolveLayerUrls();
      const hasOverlay = this._layoutBStoryPreviewHasVisibleOverlay() && !!overlayUrl;

      if (hasOverlay && quiltUrl) {
        const domBlob = await this._composeLayoutBStoryPreviewLayersFromDom();
        if (domBlob?.size > 0) {
          this._layoutBStoryPreviewShareBlob = domBlob;
          return domBlob;
        }
        try {
          const blob = await this._composeStoredLayoutBStoryPreviewLayers(quiltUrl, overlayUrl);
          this._layoutBStoryPreviewShareBlob = blob;
          return blob;
        } catch (_) {
          /* Fall through to single-layer blob only when overlay is not on screen. */
        }
      }

      if (!hasOverlay && this._layoutBStoryPreviewShareBlob?.size > 0) {
        return this._layoutBStoryPreviewShareBlob;
      }

      const shareUrl = String(this._layoutBStoryPreviewShareUrl || '').trim();
      const quiltImg = document.getElementById('quiltLayoutBPreviewImg');
      const imgSrc =
        quiltImg && !quiltImg.hidden
          ? String(quiltImg.currentSrc || quiltImg.src || '').trim()
          : '';
      const fetchSrc = shareUrl || imgSrc;
      if (fetchSrc && !hasOverlay) {
        await this._prepareStoredLayoutBStoryPreviewShareBlob(
          fetchSrc,
          this._layoutBStoryPreviewContentKey
        );
        if (this._layoutBStoryPreviewShareBlob?.size > 0) {
        ...
        try {
          const blob = await this._fetchLayoutBStoryPreviewImageBlob(fetchSrc);
          if (blob?.size > 0) {
            this._layoutBStoryPreviewShareBlob = blob;
            return blob;
          }
        } catch (_) {
          /* Long-press handler may retry. */
        }
      }
    ...
    async _ensureLayoutBStoryPreviewShareBlob() {
      const hasOverlay = this._layoutBStoryPreviewHasVisibleOverlay();
      if (!hasOverlay && this._layoutBStoryPreviewShareBlob?.size > 0) {
        return this._layoutBStoryPreviewShareBlob;
      }
      if (this._layoutBStoryPreviewSharePrepPromise) {
        return this._layoutBStoryPreviewSharePrepPromise;
      }
      this._layoutBStoryPreviewSharePrepPromise = this._ensureLayoutBStoryPreviewShareBlobCore()
        .catch(() => null)
        .finally(() => {
          this._layoutBStoryPreviewSharePrepPromise = null;
        });
    ...
    async _composeLayoutBStoryPreviewLayersFromBlobs(quiltBlob, overlayBlob) {
      const decodeBlob = async (blob) => {
        ...
        const img = await new Promise((resolve, reject) => {
          const el = new Image();
          el.onload = () => resolve(el);
          el.onerror = () => reject(new Error('Could not decode story preview layer'));
          el.src = objectUrl;
        });
        return { img, objectUrl };
      };
      const [quiltLayer, overlayLayer] = await Promise.all([
        decodeBlob(quiltBlob),
        decodeBlob(overlayBlob)
      ]);
      try {
        const canvas = document.createElement('canvas');
        canvas.width = 1080;
    ...
    async refreshLayoutBStoryPreview(options = {}) {
      if (this.isDesktopRedirect) return;
      if (!SHOW_QUILT_SCREEN_STORY_PREVIEW) {
        const wrap = document.getElementById('quiltLayoutBPreviewWrap');
        if (wrap) {
      ...
      try {
        await this._refreshLayoutBStoryPreviewCore(options);
      } finally {
        this._layoutBStoryPreviewInFlight = false;
        if (this._layoutBStoryPreviewRerunQueued) {
          this._layoutBStoryPreviewRerunQueued = false;
          const pending = this._layoutBStoryPreviewPendingRefreshOptions || { force: true, delayMs: 0 };
          this._layoutBStoryPreviewPendingRefreshOptions = null;
          void this._scheduleLayoutBStoryPreviewRefreshDebounced(pending);
        }
      }
    }

    async _waitForQuiltSvgReadyForStoryPreview(blocks, maxMs = 5000) {
      if (!Array.isArray(blocks) || blocks.length <= 1) return false;
      const arch = this.archiveService;
      if (!arch?.hasRenderedQuiltSvgForBlocks) return true;
      const started = Date.now();
      while (Date.now() - started < maxMs) {
        const svg = document.getElementById('quilt');
        if (arch.hasRenderedQuiltSvgForBlocks(svg, blocks)) return true;
        await new Promise((resolve) => setTimeout(resolve, 180));
      }
      return arch.hasRenderedQuiltSvgForBlocks(document.getElementById('quilt'), blocks);
    }

    /**
     * Quilt-screen story preview: load stored predecessor quote-fit JPEG only.
     * Live compose is admin-tomorrow / tune-only — never the normal visitor path.
     */
    async _refreshStoryQuoteFitPreviewCore(options = {}) {
      if (this.isDesktopRedirect) return;
      const force = options.force === true;
      const wrap = document.getElementById('quiltLayoutBPreviewWrap');
      const img = document.getElementById('quiltLayoutBPreviewImg');
      ...
      if (blockCount <= 1) {
        ...
        if (hint) {
          hint.hidden = false;
          hint.textContent = "Add another color to see today's story preview.";
        }
        return;
      }
      ...
      const currentFp =
        typeof Utils.computeQuiltFingerprint === 'function' && Array.isArray(blocks)
          ...
          : String(blockCount);
      const previewVersion = STORY_QUOTE_FIT_PREVIEW_RENDER_VERSION();
      const storedPreviewKey = `stored-quote-fit:${dk}|${currentFp}|${previewVersion}`;
      const adminTomorrow = this.isAdminTomorrowPreviewActive?.() === true;
      let allowLiveCompose =
        adminTomorrow ||
        this._layoutBStoryPreviewSkipStoredOnce === true ||
        this._layoutBStoryPreviewAllowLiveCompose === true;

      this._maybeInvalidateLayoutBStoryPreviewForQuiltGrowth();
      if (!force && this._layoutBStoryPreviewHeavyDoneThisVisit && !adminTomorrow) {
        return;
      }

      if (!adminTomorrow) {
        const skipStoredPreview = this._layoutBStoryPreviewSkipStoredOnce === true;
        this._layoutBStoryPreviewSkipStoredOnce = false;
        if (!skipStoredPreview) {
          this._layoutBStoryPreviewStoredRenderVersionStale = false;
          this._layoutBStoryPreviewStoredUrlsMissing = false;
          wrap.setAttribute('aria-busy', 'true');
          if (hint && (img.hidden || !img.getAttribute('src'))) {
            hint.hidden = false;
            hint.textContent = 'Loading preview…';
          }
          const usedStoredPreview = await this._tryStoredLayoutBStoryPreview(dk, storedPreviewKey, {
            currentFingerprint: currentFp,
            currentBlockCount: blockCount,
            allowStalePlaceholder: true,
            finalAllowed: true,
            preferStoryPreviewJpeg: true,
            expectedPreviewRenderVersion: previewVersion
          });
          if (usedStoredPreview) return;
          if (this._layoutBStoryPreviewDisplayedStoredPlaceholder) {
            return;
          }
          /**
           * No Firebase quote-fit JPEG yet — compose once so the slot isn’t stuck empty,
           * then upload that bake for the next visitor.
           */
          allowLiveCompose = true;
          this._layoutBStoryPreviewAllowLiveCompose = true;
        }
      }
      ...
      const showPreviewFailure = () => {
        wrap.setAttribute('aria-busy', 'false');
        const hasVisiblePreview = !img.hidden && !!img.getAttribute('src');
        if (!hasVisiblePreview) {
          ...
          if (hint) {
            hint.hidden = false;
            hint.textContent = 'Story preview unavailable.';
          }
        } else if (hint) {
          hint.hidden = true;
          hint.textContent = '';
      ...
      const contentKey = `live-quote-fit:${dk}:${currentFp}`;
      const generation = this._layoutBPreviewGeneration || 0;
      let quote =
        (adminTomorrow ? this.getEffectiveQuiltQuote?.() : null) ||
        this.quoteService?.getTodayQuote?.() ||
        null;
      if (!String(quote?.text ?? quote?.body ?? '').trim()) {
        try {
          quote =
            (typeof this.quoteService?.getQuoteResolvedForInstagramDateKey === 'function'
              ? await this.quoteService.getQuoteResolvedForInstagramDateKey(dk).catch(() => null)
              : null) ||
            this.quoteService?.getTodayQuote?.() ||
            null;
        } catch (_) {
          quote = this.quoteService?.getTodayQuote?.() || null;
        }
      }
      if (!String(quote?.text ?? quote?.body ?? '').trim()) {
        showPreviewFailure();
        if (hint) {
          hint.hidden = false;
          hint.textContent = 'Waiting for today’s quote…';
        }
        this._layoutBStoryPreviewHeavyDoneThisVisit = false;
        return;
      }
      ...
      const clearBusyIfStale = () => {
        if (generation !== (this._layoutBPreviewGeneration || 0)) {
          wrap.setAttribute('aria-busy', 'false');
          return true;
        }
        return false;
      ...
      const proxiedQuiltUrl = (rawUrl) => {
        ...
        const toProxy =
          typeof globalThis.odqProxyImageFetchUrl === 'function'
            ? globalThis.odqProxyImageFetchUrl
            : null;
        if (toProxy && bases[0]) return toProxy(bases[0], source);
        const base = String(globalThis.CONFIG?.BACKEND?.baseUrl || '').replace(/\/$/, '');
        return base ? `${base}/api/proxy-image?url=${encodeURIComponent(source)}` : '';
      };

      const showQuiltScreenFallback = async (quiltUrl) => {
        const url = String(quiltUrl || '').trim();
        if (!url) return false;
        this._revokeLayoutBPreviewObjectUrl();
        this._clearLayoutBPreviewOverlayImg();
        this._layoutBStoryPreviewLayerUrls = null;
        this._layoutBStoryPreviewShareUrl = url;
        this._layoutBStoryPreviewShareBlob = null;
        ...
        wrap.setAttribute('aria-busy', 'false');
        this._layoutBStoryPreviewHeavyDoneThisVisit = true;
        this._layoutBStoryPreviewContentKey = `quilt-screen-fallback:${dk}`;
        this._layoutBStoryPreviewSatisfiedForDateKey = dk;
        this._stampLayoutBStoryPreviewDisplayedSnapshot(blockCount, currentFp);
        if (hint) {
          hint.textContent = '';
          hint.hidden = true;
        }
      ...
      try {
        const timed = (label, promise, timeoutMs) =>
          typeof globalThis.odqPromiseWithTimeout === 'function'
            ? globalThis.odqPromiseWithTimeout(promise, timeoutMs, label)
            : promise;

        let quiltSourceUrl = '';
        let quiltSourceRawUrl = '';
        try {
          const igDoc = await timed(
            'Story preview quilt-screen lookup',
            this._fetchInstagramImageDocRest(dk),
            5000
          );
          quiltSourceRawUrl = String(
            igDoc?.quiltScreen9x16Url ||
              igDoc?.quiltScreenUrl ||
              igDoc?.quiltStoryUrl ||
              igDoc?.quiltScreen9x16ImageStorageUrl ||
              ''
          ).trim();
          quiltSourceUrl = proxiedQuiltUrl(quiltSourceRawUrl) || quiltSourceRawUrl;
        } catch (_) {
          quiltSourceUrl = '';
          quiltSourceRawUrl = '';
        }
        if (clearBusyIfStale()) return;

        let dataUrl = '';
        /** Local SVG export first — Storage CORS often blocks direct Firebase fetch in canvas. */
        await this._waitForQuiltSvgReadyForStoryPreview(blocks, 3000);
        if (clearBusyIfStale()) return;
        if (typeof arch?.generateInstagramStoryQuoteFitPreviewJpegImageData !== 'function') {
          throw new Error('Story quote-fit compose unavailable');
        }
        const paintUrl = String(
          this._layoutBStoryPreviewSpeakerPaintUrl ||
            this._quoteSpeakerCutoutUrl ||
            this.quoteService?._resolveSpeakerCutoutForQuote?.(quote) ||
            ''
        ).trim();
        const expectsCutout = /speaker-cutouts(?:%2F|\/)/i.test(paintUrl);
        let preloadedSpeakerImageUrl = '';
        if (paintUrl) {
          preloadedSpeakerImageUrl = await this._layoutBStoryPreviewPreloadedSpeakerDataUrl(
            paintUrl,
            quote
          );
        }
        const quoteFitSpeakerOpts = {
          speakerImageUrl: preloadedSpeakerImageUrl || undefined,
          cutoutSourceUrl: expectsCutout ? paintUrl : undefined
        };
        this._layoutBStoryPreviewUsedCutout = expectsCutout && !!preloadedSpeakerImageUrl;
        dataUrl = String(
          await timed(
            'Story quote-fit preview',
            arch.generateInstagramStoryQuoteFitPreviewJpegImageData(blocks, quote, dk, quoteFitSpeakerOpts),
            35000
          ).catch((err) => {
            this.logger?.warn?.('Story quote-fit local compose failed:', err);
            return null;
          }) || ''
        ).trim();
        if (clearBusyIfStale()) return;

        if (!dataUrl && quiltSourceUrl) {
          dataUrl = String(
            await timed(
              'Story quote-fit from proxied quilt-screen',
              arch.generateInstagramStoryQuoteFitPreviewJpegImageData(blocks, quote, dk, {
                quiltSourceUrl,
                ...quoteFitSpeakerOpts
              }),
              20000
            ).catch((err) => {
              this.logger?.warn?.('Quote-fit from proxied quilt-screen failed:', err);
              return null;
            }) || ''
          ).trim();
        }
        ...
        if (!dataUrl || typeof Utils?.dataUrlToBlob !== 'function') {
          if (await showQuiltScreenFallback(quiltSourceRawUrl)) return;
          throw new Error('Story quote-fit preview empty');
        }
        const blob = Utils.dataUrlToBlob(dataUrl);
        if (!blob?.size) {
          if (await showQuiltScreenFallback(quiltSourceRawUrl)) return;
          throw new Error('Story quote-fit preview blob empty');
        }
        ...
        this._layoutBPreviewObjectUrl = previewUrl;
        this._layoutBStoryPreviewShareUrl = '';
        this._layoutBStoryPreviewShareBlob = blob;
        ...
        wrap.setAttribute('aria-busy', 'false');
        this._layoutBStoryPreviewHeavyDoneThisVisit = true;
        this._layoutBStoryPreviewDidLiveCompose = true;
        if (!this._layoutBStoryPreviewUsedCutout) {
          const resolvedCutout = String(
            this.quoteService?._resolveSpeakerCutoutForQuote?.(quote) || ''
          ).trim();
          this._layoutBStoryPreviewUsedCutout = /speaker-cutouts(?:%2F|\/)/i.test(resolvedCutout);
        }
        this._layoutBStoryPreviewContentKey = contentKey;
        this._layoutBStoryPreviewSatisfiedForDateKey = dk;
        this._stampLayoutBStoryPreviewDisplayedSnapshot(blockCount, currentFp);
        if (hint) {
          hint.textContent = '';
        ...
        /** Upload this bake so later visits load from Firebase instead of composing again. */
        if (!adminTomorrow) {
          this._uploadQuoteFitPreviewJpegInBackground?.(dataUrl, dk, currentFp);
        }
      } catch (err) {
        this.logger?.warn?.('Story quote-fit preview failed:', err);
        if (generation === (this._layoutBPreviewGeneration || 0)) showPreviewFailure();
        else wrap.setAttribute('aria-busy', 'false');
      }
    }

    /** Persist a just-composed quote-fit JPEG for the next visitor (non-blocking). */
    _uploadQuoteFitPreviewJpegInBackground(dataUrl, dateKey = '', fingerprint = '') {
      if (!USE_STORY_QUOTE_FIT_PREVIEW) return;
      const dk = String(dateKey || this.getEffectiveAppDateKey?.() || '').trim();
      const fp = String(fingerprint || '').trim();
      const imageData = String(dataUrl || '').trim();
      if (!dk || !imageData.startsWith('data:image/')) return;
      if (
        fp &&
        fp === String(this._lastStoryQuoteFitPreviewUploadFingerprint || '').trim()
      ) {
        return;
      }
      if (this._quoteFitPreviewUploadInFlight) return;
      this._quoteFitPreviewUploadInFlight = true;
      const blocks = this.quiltEngine?.blocks;
      const quote =
        this.quoteService?.getTodayQuote?.() ||
        { text: '', author: '' };
      void (async () => {
        try {
          const uploadPayload = {
            dateKey: dk,
            storyLayoutBPreviewImageData: imageData,
            layoutBStoryPreviewRenderVersion: STORY_QUOTE_FIT_PREVIEW_RENDER_VERSION(),
            zapierCaption: Utils.formatZapierCaptionFromQuote?.(quote) || '',
            quiltFingerprint: fp,
            blockCount: Array.isArray(blocks) ? blocks.length : 0,
            contributorCount: Math.max(1, this._humanContributorPickCount?.() || 1)
          };
          const timed = (label, promise, timeoutMs) =>
            typeof globalThis.odqPromiseWithTimeout === 'function'
              ? globalThis.odqPromiseWithTimeout(promise, timeoutMs, label)
              : promise;
          if (typeof Utils.writeInstagramImagesDocForZapierViaServer === 'function') {
            try {
              await timed(
                'Backend quote-fit preview upload',
                Utils.writeInstagramImagesDocForZapierViaServer(uploadPayload),
                180000
              );
              this._lastStoryQuoteFitPreviewUploadFingerprint = fp;
              this.logger?.log?.('✅ Stored story quote-fit preview after live compose');
              return;
            } catch (backendErr) {
              this.logger?.warn?.(
                'Backend quote-fit preview upload failed, trying client',
                backendErr
              );
            }
          }
          if (typeof Utils.writeInstagramImagesDocForZapier === 'function') {
            await timed(
              'Client quote-fit preview upload',
              Utils.writeInstagramImagesDocForZapier(uploadPayload),
              180000
            );
            this._lastStoryQuoteFitPreviewUploadFingerprint = fp;
            this.logger?.log?.('✅ Stored story quote-fit preview after live compose (client)');
          }
        } catch (err) {
          this.logger?.warn?.('Quote-fit preview background upload failed:', err);
        } finally {
          this._quoteFitPreviewUploadInFlight = false;
        }
      })();
    }

    /**
     * DESIGN RULE — do not revert: normal users load a stored predecessor snapshot
     * (quote-fit JPEG, or quilt-screen + overlay for Layout B), not a live re-compose.
     * Each save uploads for the *next* visitor; the preview after contributing stays
     * one save behind until relaunch. Live compose is admin/tune / blank fallback only.
     */
    async _refreshLayoutBStoryPreviewCore(options = {}) {
      if (this.isDesktopRedirect) return;
      if (USE_STORY_QUOTE_FIT_PREVIEW) {
        return this._refreshStoryQuoteFitPreviewCore(options);
      }
      const force = options.force === true;
      const wrap = document.getElementById('quiltLayoutBPreviewWrap');
      const img = document.getElementById('quiltLayoutBPreviewImg');
      ...
      const currentFp =
        typeof Utils.computeQuiltFingerprint === 'function' && Array.isArray(blocks)
          ...
          : String(blockCount);
      const mirrorSig =
        typeof globalThis.odqMirrorTuneExportSignature === 'function'
          ? globalThis.odqMirrorTuneExportSignature(dk)
          : '';
      const layoutBTuneSig =
        typeof globalThis.odqLayoutBTuneExportSignature === 'function'
          ? globalThis.odqLayoutBTuneExportSignature(dk, 'story')
          : typeof globalThis.odqSpeakerCutoutTuneExportSignature === 'function'
            ? globalThis.odqSpeakerCutoutTuneExportSignature(dk, 'story')
            : '';
      const storedPreviewKey = `stored:${dk}|${currentFp}|${mirrorSig || 'tune0'}|${layoutBTuneSig || 'tune0'}`;

      this._maybeInvalidateLayoutBStoryPreviewForQuiltGrowth();
      if (!force && this._layoutBStoryPreviewHeavyDoneThisVisit && !this.isAdminTomorrowPreviewActive?.()) {
        return;
      }
      ...
      if (!this.isAdminTomorrowPreviewActive?.()) {
        const skipStoredPreview = this._layoutBStoryPreviewSkipStoredOnce === true;
        this._layoutBStoryPreviewSkipStoredOnce = false;
        const localStoryTune =
          typeof globalThis.odqReadSpeakerCutoutTweakFromLocal === 'function'
            ? globalThis.odqReadSpeakerCutoutTweakFromLocal(dk, 'story')
            : null;
        const localTuneCustomized =
          typeof globalThis.odqSpeakerCutoutTweakIsCustomized === 'function' &&
          globalThis.odqSpeakerCutoutTweakIsCustomized(localStoryTune);
        if (localTuneCustomized) {
          this._layoutBStoryPreviewAllowLiveCompose = true;
        }
        if (!skipStoredPreview && !localTuneCustomized) {
          this._layoutBStoryPreviewStoredRenderVersionStale = false;
          const usedStoredPreview = await this._tryStoredLayoutBStoryPreview(dk, storedPreviewKey, {
            currentFingerprint: currentFp,
            currentBlockCount: blockCount,
          ...
          if (usedStoredPreview) return;
          if (this._layoutBStoryPreviewStoredRenderVersionStale) {
            this._layoutBStoryPreviewAllowLiveCompose = true;
          }
          if (this._layoutBStoryPreviewDisplayedStoredPlaceholder) {
            return;
          }
          /** No usable stored snapshot on screen — allow live compose so the slot is not blank all day. */
          const hasVisibleStored = !img.hidden && !!img.getAttribute('src');
          if (!hasVisibleStored) {
            this._layoutBStoryPreviewAllowLiveCompose = true;
          }
        }
      ...
      const showPreviewFailure = () => {
        wrap.setAttribute('aria-busy', 'false');
        /** If a stored/previous image is already visible, leave it up rather than
         *  blanking the preview just because a background re-compose attempt failed. */
        const hasVisiblePreview = !img.hidden && !!img.getAttribute('src');
        if (!hasVisiblePreview) {
          wrap.classList.add('quilt-layout-b-preview-wrap--empty');
          img.removeAttribute('src');
      ...
      const contentKey = `live:${dk}:${currentFp}:${mirrorSig || 'tune0'}:${layoutBTuneSig || 'tune0'}`;
      const generation = this._layoutBPreviewGeneration || 0;
      const allowLiveCompose =
        this._layoutBStoryPreviewAllowLiveCompose === true ||
        this._layoutBStoryPreviewStoredUrlsMissing === true ||
      ...
      if (this._layoutBStoryPreviewAllowLiveCompose) {
        this._layoutBStoryPreviewAllowLiveCompose = false;
      }

      const applyLiveLayeredPreviewBlobs = async (quiltBlob, overlayBlob, previewContentKey) => {
        if (!quiltBlob || !(quiltBlob.size > 0)) throw new Error('Story preview quilt layer empty');
        if (!overlayBlob || !(overlayBlob.size > 0)) throw new Error('Story preview overlay layer empty');
        this._revokeLayoutBPreviewObjectUrl();
        let quiltUrl = '';
        let overlayUrl = '';
        try {
          if (typeof createImageBitmap === 'function') {
            const [quiltBmp, overlayBmp] = await Promise.all([
              createImageBitmap(quiltBlob),
              createImageBitmap(overlayBlob)
            ]);
            if (typeof quiltBmp.close === 'function') quiltBmp.close();
            if (typeof overlayBmp.close === 'function') overlayBmp.close();
          }
          quiltUrl = URL.createObjectURL(quiltBlob);
          overlayUrl = URL.createObjectURL(overlayBlob);
        } catch (decodeErr) {
          throw new Error(`Story preview layers invalid: ${decodeErr?.message || decodeErr}`);
        }
        this._layoutBPreviewObjectUrl = quiltUrl;
        this._layoutBPreviewOverlayObjectUrl = overlayUrl;
        this._layoutBStoryPreviewLayerUrls = { quiltUrl, overlayUrl };
        this._layoutBStoryPreviewShareUrl = '';
        const overlayImg = this._layoutBPreviewOverlayImg();
        await new Promise((resolve) => {
          let quiltDone = false;
          ...
          const finish = () => {
            if (quiltDone && overlayDone) resolve();
          };
          img.onload = () => {
            quiltDone = true;
            finish();
        ...
        if (generation !== (this._layoutBPreviewGeneration || 0)) return false;
        wrap.setAttribute('aria-busy', 'false');
        this._layoutBStoryPreviewHeavyDoneThisVisit = true;
        ...
        this._layoutBStoryPreviewShareBlob = null;
        try {
          this._layoutBStoryPreviewShareBlob = await this._composeLayoutBStoryPreviewLayersFromBlobs(
            quiltBlob,
            overlayBlob
          );
        } catch (_) {
      ...
      const applyLivePreviewBlob = async (blob, previewContentKey) => {
        if (!blob || !(blob.size > 0)) throw new Error('Story preview blob empty');
        this._revokeLayoutBPreviewObjectUrl();
        let previewUrl = '';
        try {
          if (typeof createImageBitmap === 'function') {
          ...
          throw new Error(`Story preview blob invalid: ${decodeErr?.message || decodeErr}`);
        }
        this._layoutBPreviewObjectUrl = previewUrl;
        this._layoutBStoryPreviewLayerUrls = null;
        this._layoutBStoryPreviewShareUrl = '';
      ...
      this._layoutBStoryPreviewShareBlob = null;
      this._layoutBStoryPreviewLayerUrls = null;
      /** Keep whatever's already on screen (e.g. the stored preview) visible while we
       *  attempt a live re-compose in the background — don't blank it just to gamble on
       *  a slower, heavier render that may take much longer than the old image is stale by. */
      const hadVisiblePreview = !img.hidden && !!img.getAttribute('src');
      ...
      const previewCutout = this.isAdminTomorrowPreviewActive?.()
        ? String(this.quoteService?._resolveSpeakerCutoutForQuote?.(quote) || '').trim()
        : '';
      const paintUrl = String(
        cachedPaint ||
          previewCutout ||
      ...
      const previewContentKey = `${contentKey}:${speakerReady ? 'sp1' : 'sp0'}`;

      if (hint) {
        hint.hidden = false;
        hint.textContent =
          expectsCutout && !preloadedSpeakerImageUrl ? 'Loading speaker…' : 'Updating preview…';
      }
      ...
      try {
        ...
        await this._waitForQuiltSvgReadyForStoryPreview(blocks, 3000);
        if (generation !== (this._layoutBPreviewGeneration || 0)) return;
        if (!arch?.getInstagramQuiltSourceBlob || !arch?.generateInstagramStoryLayoutBBlob) {
          throw new Error('Story layered compose unavailable');
        }
        const quiltPromise = arch.getInstagramQuiltSourceBlob(blocks, {
          dateKey: dk,
          exportModeLabel: 'layout_b_story_preview_quilt',
          skipFilmGrain: true,
          maxEdge: 720,
          imageSmoothingQuality: 'low',
          exportMatchQuiltScreen: false,
          exportFillFrame: true
        });
        const overlayPromise = arch.generateInstagramStoryLayoutBBlob(blocks, quote, dk, {
          forPreview: true,
          maxEdge: 720,
          transparentOverlay: true,
          preloadedSpeakerImageUrl
        });
        const layeredPromise = Promise.all([quiltPromise, overlayPromise]);
        const [quiltBlob, overlayBlob] =
          typeof globalThis.odqPromiseWithTimeout === 'function'
            ? await globalThis.odqPromiseWithTimeout(layeredPromise, 25000, 'Layout B layered story preview')
            : await layeredPromise;
        if (generation !== (this._layoutBPreviewGeneration || 0)) return;
        ...
        const ok = await applyLiveLayeredPreviewBlobs(quiltBlob, overlayBlob, previewContentKey);
        if (ok) return;
      } catch (previewErr) {
        this.logger?.warn?.('Live layered Layout B story preview failed:', previewErr);
      }

      if (generation !== (this._layoutBPreviewGeneration || 0)) return;
      try {
        if (!arch?.getInstagramQuiltSourceBlob) {
          throw new Error('Quilt capture unavailable');
        }
        const quiltBlob = await arch.getInstagramQuiltSourceBlob(blocks, {
          dateKey: dk,
          exportModeLabel: 'layout_b_story_preview_quilt_only',
          skipFilmGrain: true,
          maxEdge: 720,
          imageSmoothingQuality: 'low',
          exportMatchQuiltScreen: false,
        ...
        if (quiltBlob) {
          const ok = await applyLivePreviewBlob(quiltBlob, `quilt-only:${contentKey}`);
          if (ok) return;
        }
      } catch (quiltOnlyErr) {
        this.logger?.warn?.('Quilt-only story preview fallback failed:', quiltOnlyErr);
      }

      if (generation !== (this._layoutBPreviewGeneration || 0)) return;
      const storedFallback = await this._tryStoredLayoutBStoryPreview(
        dk,
        `stored-fallback:${dk}|${currentFp}`,
        {
          currentFingerprint: currentFp,
          currentBlockCount: blockCount,
          allowStalePlaceholder: true,
    ...
    _humanContributorPickCount() {
      const engine = this.quiltEngine;
      if (!engine) return 1;
      if (typeof engine.getContributorPickCount === 'function') {
        return Math.max(1, engine.getContributorPickCount());
      }
      return Math.max(1, Number(engine.submissionCount) || 1);
    }
    ...
    async _primeQuiltQuoteChrome() {
      ...
      const adminPreview = !!this.isAdminTomorrowPreviewActive?.();
      let todayQ = adminPreview
        ? this.getEffectiveQuiltQuote()
        : (dateKey &&
          this.quoteService &&
          typeof this.quoteService.getQuoteResolvedForInstagramDateKey === 'function'
      ...
      if (!todayQ) return;
      const isNativeQuoteChrome =
        typeof window !== 'undefined' &&
        typeof window.odqIsCapacitorNative === 'function' &&
        window.odqIsCapacitorNative();
      const hasReflectionPromptForChrome = (q) =>
        !!String(
          q?.reflectionPrompt ??
            q?.reflection_prompt ??
            q?.communityPrompt ??
            q?.community_prompt ??
            q?.reflectionPromptSnapshot ??
            q?.communityPromptSnapshot ??
            ''
        ).trim();
      if (
        isNativeQuoteChrome &&
        dateKey &&
        !adminPreview &&
        !hasReflectionPromptForChrome(todayQ) &&
        this.quoteService?._resolveTodayQuoteForLaunchViaServerApi
      ) {
        const apiOk = await this.quoteService._resolveTodayQuoteForLaunchViaServerApi(dateKey);
        const apiQuote = this.isAdminTomorrowPreviewActive?.()
          ? this.quoteService?._pinnedByDateKey?.[dateKey] ||
            this.getEffectiveQuiltQuote?.() ||
            null
          : this.quoteService?.getTodayQuote?.() || null;
        if (apiOk && apiQuote) todayQ = apiQuote;
      }
      ...
      if (dateKey && (!mood.goodDay || !mood.roughDay) && this.quoteService?.hydrateMoodFieldsForCalendarKey) {
        if (adminPreview) {
          void this.quoteService
            .hydrateMoodFieldsForCalendarKey(dateKey, todayQ)
            .then((hydrated) => {
              if (!hydrated || this.getEffectiveAppDateKey?.() !== dateKey) return;
              if (this._adminPreview) this._adminPreview.quote = hydrated;
              this.refreshQuiltMoodWidget(hydrated);
            })
            .catch(() => {});
        } else {
          todayQ =
      ...
      const isNative =
        ...
        window.odqIsCapacitorNative();
      const freshness =
        dateKey && !isNative && !adminPreview
          ? await this._readNightlyClippingFreshness(dateKey)
          : null;
      this.populateQuiltQuote();
      if (this.isAdminTomorrowPreviewActive?.()) {
        todayQ = this.getEffectiveQuiltQuote() || todayQ;
      }
      if (adminPreview) {
        this._refreshQuoteSpeakerWidgetNow?.(todayQ);
      }
      this.refreshQuoteSpeakerWidget(adminPreview ? null : todayQ);
      this.refreshQuiltMoodWidget(todayQ);
      if (!adminPreview) {
        this.refreshQuiltReflectionScrapWidget?.(todayQ);
      }
      void this.applyQuoteScreenClipping({
        dateKey,
        quote: todayQ,
        forceRefresh:
          !!freshness?.assignmentNewerThanClippingMeta || adminPreview
      }).catch(() => {});
    }

    _quoteSpeakerPriorityUrl(quote = null) {
      const q = quote || this.getEffectiveQuiltQuote?.() || null;
      if (!q) return '';
      return String(
        q.speakerCutoutUrlSnapshot ??
          q.speaker_cutout_url_snapshot ??
          q.speakerCutoutUrl ??
    ...
    /** Admin preview: speaker name/image + quote clipping first, before quilt extras. */
    _primeAdminPreviewQuoteChrome() {
      if (!this.isAdminTomorrowPreviewActive?.()) return;
      const dateKey = String(this.getEffectiveAppDateKey?.() || '').trim();
      const quote = this.getEffectiveQuiltQuote?.() || this._adminPreview?.quote || null;
      if (!quote?.text) return;
      this.populateQuiltQuote();
      this._refreshQuoteSpeakerWidgetNow?.(quote);
      void this._refreshQuoteSpeakerWidgetEntry?.(quote);
      this._preloadHighPriorityImage(this._quoteSpeakerPriorityUrl(quote));
      if (dateKey) {
        void this.fetchNewspaperClippingMeta?.(dateKey, { waitForFirebaseMs: 900 })
          .then((meta) => {
            const url = String(meta?.url || '').trim();
            if (!url) return;
            this._preloadQuoteScreenClippingUrl?.(url);
            this._preloadHighPriorityImage(url);
          })
          .catch(() => {});
      }
      this.refreshQuiltMoodWidget?.(quote);
      void this.applyQuoteScreenClipping?.({
        dateKey,
        quote,
        forceRefresh: true
    ...
    async fetchNewspaperClippingMeta(dateKey, { forceRefresh = false, waitForFirebaseMs = 5000 } = {}) {
      ...
      const restValueToPlain = (value) => {
        ...
        if (Array.isArray(value.arrayValue?.values)) {
          return value.arrayValue.values.map(restValueToPlain);
        }
        return undefined;
      };
      const fetchMetaViaRest = async () => {
        const projectId = String(globalThis.CONFIG?.FIREBASE?.projectId || '').trim();
        const apiKey = String(globalThis.CONFIG?.FIREBASE?.apiKey || '').trim();
        ...
        const meta = newspaperClippingMetaFromDoc(data);
        if (meta.url) {
          this._newspaperClippingMetaCache[dk] = meta;
          if (!this._newspaperClippingUrlCache) this._newspaperClippingUrlCache = {};
          this._newspaperClippingUrlCache[dk] = meta.url;
        }
        return meta;
      };
      if (isNative && !forceRefresh) {
        try {
          const meta = await (typeof globalThis.odqPromiseWithTimeout === 'function'
            ? globalThis.odqPromiseWithTimeout(fetchMetaViaRest(), 6000, `Firestore REST clipping ${dk}`)
            : fetchMetaViaRest());
          if (meta?.url) return meta;
        } catch (_) {
          /* fall through to SDK/cache path */
        }
      }
      if (!window.db || !window.firestore) {
    ...
    async _upgradeTriptychQuoteToNightlyClipping(dk, goodDay, roughDay) {
      const spreadHost = document.getElementById('quiltMoodSpread');
      const img = spreadHost?.querySelector(
        '.quilt-mood-spread__quote-img, .quilt-mood-triptych__quote-img, .quilt-mood-collage__quote img'
      );
      const spreadApi =
    ...
    _quotePayloadForNewspaperClipping(quote, dateKey = null) {
      const raw = quote || this.quoteService?.getTodayQuote?.() || null;
      if (!raw) return null;
      const merged = this._quoteForMoodWidget?.(raw) || raw;
      const dk =
        String(dateKey || '').trim() ||
        String(merged?.dateKey || '').trim() ||
        this.quoteService?.getQuoteCalendarKeyNow?.() ||
        '';
      const payload = this._quiltMoodTerminalQuotePayload?.(merged, dk);
      if (payload?.text) return payload;
      const text = String(merged?.text ?? merged?.body ?? '').trim();
      if (!text) return null;
      const out = {
        text,
        author: String(merged?.author ?? merged?.authorSnapshot ?? '').trim(),
        dateKey: dk
      };
      const flc = globalThis.QuiltNewspaperClipping?.firstLineCountFromQuoteFields?.(merged);
      if (flc != null) {
        out.firstLineCount = flc;
        out.first_line_count = flc;
      }
      const keyword = String(merged?.keyword ?? merged?.keywordSnapshot ?? '').trim();
    ...
    async _composeQuiltNewspaperClippingDataUrl(dateKey, todayQ) {
      ...
      const dk =
        String(dateKey || '').trim() ||
        String(todayQ?.dateKey || '').trim() ||
        this.quoteService?.getQuoteCalendarKeyNow?.() ||
        '';
      const today = this._quotePayloadForNewspaperClipping(todayQ, dk);
    ...
    _clippingLayoutOptsForImage(img, layout = {}) {
      ...
      if (
        img?.closest?.(
          '.quilt-mood-collage__quote, #quiltMoodDuoQuote, .quilt-mood-duo__quote-img'
        )
      ) {
        return QNC?.resolveCollageClippingLayoutOpts?.() || null;
      }
      if (img?.closest?.('.quilt-mood-triptych__quote, #quiltMoodSpread')) {
        return QNC?.resolveTriptychClippingLayoutOpts?.() || null;
      }
      return QNC?.resolveClippingLayoutOpts?.({ minBodyDomPx: 8 }) || null;
    }
    ...
    _resolveNewspaperClippingDisplayLayout(img, layout = {}) {
      ...
      const renderWidth =
        Number(layout?.renderWidth) > 0
          ? Number(layout.renderWidth)
          : Number(img?.dataset?.quoteRenderWidth) > 0
            ? Number(img.dataset.quoteRenderWidth)
            : Number(spreadHost?.dataset?.quoteRenderWidth) > 0
              ? Number(spreadHost.dataset.quoteRenderWidth)
              : 0;
      const effectiveBodyDomPx =
        Number(layout?.effectiveBodyDomPx) > 0
          ? Number(layout.effectiveBodyDomPx)
          : Number(img?.dataset?.quoteEffectiveBodyDomPx) > 0
            ? Number(img.dataset.quoteEffectiveBodyDomPx)
            : Number(spreadHost?.dataset?.quoteEffectiveBodyDomPx) > 0
              ? Number(spreadHost.dataset.quoteEffectiveBodyDomPx)
              : 0;
      const layoutOpts = this._clippingLayoutOptsForImage(img, layout) || {};
      if (effectiveBodyDomPx > 0) layoutOpts.effectiveBodyDomPx = effectiveBodyDomPx;
      return (
    ...
    _fitClippingDisplayToSlot(img, resolved = {}) {
      ...
      const readabilityBoost = Number(resolved?.readabilityScale) > 1.02;
      if (isQuoteScreen) {
        const slot = this._quoteScreenClippingSlotEl(img);
        w = quoteScreenClippingCappedWidthPx(slot, w);
        h = Math.max(1, Math.round((nh / nw) * w));
        const screen = document.getElementById('screen-quote');
        const viewportH =
          Number(window.visualViewport?.height) ||
          Number(window.innerHeight) ||
          Number(screen?.clientHeight) ||
          760;
        const pickerRevealed = screen?.classList.contains('quote-picker-revealed');
        const slotMaxH =
          slot && this._quoteScreenClippingSlotUsable(slot) ? slot.clientHeight - 10 : Infinity;
        const viewportMaxH = viewportH * (pickerRevealed ? 0.33 : 0.42);
        const maxH = Math.max(96, Math.min(slotMaxH, viewportMaxH));
        if (h > maxH) {
          w = Math.max(96, Math.round(w * (maxH / h)));
          h = Math.max(1, Math.round((nh / nw) * w));
        }
        return { widthPx: Math.max(1, Math.round(w)), heightPx: h };
      }
    ...
    _applyNewspaperClippingDisplaySize(img, layout) {
      ...
      const clipping = img.closest?.('.quote-screen-clipping');
      const isQuoteScreen = !!clipping;
      if (isQuoteScreen) {
        clipping.classList.toggle(
          'is-readable-boost',
          Number(resolved?.readabilityScale) > 1.02
        );
      }
      ...
      if (w > 0) {
        ...
        const readableBoost = isQuoteScreen && clipping?.classList.contains('is-readable-boost');
        if (h > 0) {
          img.style.height = `${Math.round(h)}px`;
          img.style.maxHeight = '100%';
        } else if (readableBoost) {
    ...
    remeasureQuoteScreenClipping() {
      ...
      const effectiveBodyDomPx = Number(img.dataset.quoteEffectiveBodyDomPx) || 0;
      const layout = {};
      if (renderWidth > 0) layout.renderWidth = renderWidth;
      if (effectiveBodyDomPx > 0) layout.effectiveBodyDomPx = effectiveBodyDomPx;
    ...
    async _applyQuoteScreenClippingWork({ dateKey = null, quote = null, forceRefresh = false } = {}) {
      ...
      const isNative =
        ...
        typeof window.odqIsCapacitorNative === 'function' &&
        window.odqIsCapacitorNative();
      const metaWaitMs = isNative ? 0 : 8000;

      this.quoteService?.primeTodayQuoteFromLocalAssignment?.();
      let todayQ =
        quote && String(quote.text ?? quote.body ?? '').trim() ? quote : null;
      if (!todayQ) {
        todayQ = await this._resolveQuoteForClippingCenter(dk, quote, {
          preferLiveAssignment: false
        });
      }
      if (!todayQ && isNative && dk && this.quoteService) {
        await this.quoteService._resolveTodayQuoteForLaunchViaServerApi(dk);
        todayQ = this.quoteService.getTodayQuote?.() || null;
      }
      if (!todayQ) {
        todayQ =
          this.quoteService?.getTodayQuote?.() ||
      ...
      if (String(todayQ?.text ?? todayQ?.body ?? '').trim()) {
        showTextFallback();
        return true;
      }

      const clippingMeta = dk
        ? await this.fetchNewspaperClippingMeta(dk, { forceRefresh, waitForFirebaseMs: metaWaitMs })
        : { url: '', exportRev: '' };
      ...
      if (!todayQ || !String(todayQ?.text ?? todayQ?.body ?? '').trim()) {
        todayQ = await this._resolveQuoteForClippingCenter(dk, todayQ, {
          preferLiveAssignment: skipForAssignment
        });
        if (!todayQ) {
          todayQ =
            this.quoteService?.getTodayQuote?.() ||
      ...
      const showPng = async (layoutMeta = null, stamp = '') => {
        ...
        if (Number(layoutMeta?.effectiveBodyDomPx) > 0) {
          img.dataset.quoteEffectiveBodyDomPx = String(layoutMeta.effectiveBodyDomPx);
        }
        this._quoteScreenClippingMountedKey = dk;
      ...
      if (forceRefresh && dk) {
        await this.fetchNewspaperClippingMeta(dk, { forceRefresh: true, waitForFirebaseMs: metaWaitMs || 4000 });
      }
    ...
    async applyQuiltNewspaperClipping({ dateKey = null, quote = null } = {}) {
      ...
      const dk = dateKey || this.getEffectiveAppDateKey?.() || this.quoteService?.getQuoteCalendarKeyNow?.();
      const refreshScannerBedPaper = () => {
        globalThis.OdqScannerBed?.bootstrapQuiltPaper?.(document, dk);
      };

      const clippingMeta = dk
        ? await this.fetchNewspaperClippingMeta(dk, { waitForFirebaseMs: 8000 })
        : { exportRev: '' };
      const skipStoragePng = false;
          ...
          requestAnimationFrame(() => {
            requestAnimationFrame(() => {
              if (document.getElementById('screen-quilt')?.classList.contains('active')) {
                ...
                } else if (this._quiltUserColorCardVisible?.()) {
      ...
      const url =
        !skipStoragePng && dk ? await this.resolveNightlyNewspaperClippingUrl(dk) : null;
    ...
    populateQuiltQuote() {
      ...
      try {
        ...
        void this.applyQuiltMoodClippings({
          dateKey: this.getEffectiveAppDateKey(),
          quote: todayQ
        });
        this.renderQuiltContributorList();
    ...
    async _hydrateSettingsDailyQuoteReminderAsync() {
      ...
      const timeRow = document.getElementById('settingsDailyQuoteTimeRow');
      const hourSelect = document.getElementById('settingsDailyQuoteHourSelect');
      const resetNote = document.getElementById('settingsDailyQuoteResetNote');
      if (!toggle || !hourSelect) return;

      const hour = this.getDailyQuotePreferredHour();
      hourSelect.value = String(hour);
      if (resetNote) {
        resetNote.textContent = `New quilt starts around ${this.formatDailyQuiltResetLocalLabel()} your time`;
      }
    ...
    async handleSettingsDailyQuoteToggleChange(enabled) {
      if (enabled) {
        ...
        const accepted = await this.uiService.showPushNotificationOptIn({
          sampleBody,
        ...
        try {
          const ok = await this.finalizeDailyQuoteReminderOptIn({ showSuccess: true });
          if (ok) {
    ...
    async registerDailyQuotePushToken(token, { enabled = true } = {}) {
      ...
      const res = await fetch(`${baseUrl}/api/push/register`, {
        ...
        body: JSON.stringify({
          ...
          notificationTypes: this.getPushNotificationTypes?.() || ['daily_quote'],
    ...
    async maybePromptForDailyQuoteNotifications() {
      ...
      if (
        promptedState === '1' &&
        (localStorage.getItem('ourDailyPushDailyQuoteEnabled') === '1' ||
          localStorage.getItem('ourDailyLocalDailyQuoteEnabled') === '1')
      ) {
      ...
      try {
        await this.finalizeDailyQuoteReminderOptIn();
      } catch (error) {
    ...
    /** Custom opt-in modal must finish before any native iOS permission prompt. */
    async finalizeDailyQuoteReminderOptIn({ showSuccess = true } = {}) {
      await this.waitForPushOptInModalToDismiss();
      this.setupPushNotificationListeners();
      if (!this.getFirebaseMessagingPlugin() && !this.getLocalNotificationsPlugin()) {
        throw new Error('Notification plugins unavailable');
      }
      try {
        const ok = await this.enableDailyQuotePushNotifications({ showSuccess });
        if (ok) return true;
      } catch (error) {
        this.logger.warn('Daily quote push opt-in failed:', error);
      }
      try {
        return await this.enableLocalDailyQuoteNotification({ showSuccess });
      } catch (localError) {
    ...
    waitForPushOptInModalToDismiss(delayMs = 360) {
    ...
    async refreshDailyQuotePushRegistration() {
      ...
      if (localStorage.getItem('ourDailyPushDailyQuotePrompted_v2') !== '1') return;
      const messaging = this.getFirebaseMessagingPlugin();
      if (!messaging) return;
    ...
    async maybeShowPersonalQuiltFallback() {
      if (!this.isMilestoneQuiltsEnabled()) return false;
      if (this._whatsNewModalOpen || this._whatsNewAnnouncementInFlight) return false;
      try {
    ...
    _getRememberTodayCompanionParts(quote = null) {
      const qs = this.quoteService;
      const today = this._getRememberTodayQuote(quote);
      return qs?.getArtRecBeforeYouGoParts?.(today) || { titleLine: '', blurb: '', iconName: '', iconSvg: '' };
    }

    /** Material Symbol ligature or inline default star when art_recs_type is empty. */
    _appendArtRecIcon(container, { iconName = '', iconSvg = '' } = {}) {
      if (!container) return;
      const svg = String(iconSvg || '').trim();
      if (svg) {
        const wrap = document.createElement('span');
        wrap.className = 'art-rec-icon art-rec-icon--svg';
        wrap.setAttribute('aria-hidden', 'true');
        wrap.innerHTML = svg;
    ...
    _mountRememberTodayArtRecText(el, { titleLine = '', blurb = '', iconName = '', iconSvg = '' } = {}) {
      if (!el) return;
      ...
      this._appendArtRecIcon(line, { iconName, iconSvg });
    ...
    _stashDailyQuoteNotificationOpenIntent() {
      this._pendingDailyQuoteNotificationOpen = true;
      try {
        sessionStorage.setItem(
          'odqDailyQuoteOpen',
    ...
    _takeDailyQuoteNotificationOpenIntent() {
      if (this._pendingDailyQuoteNotificationOpen) {
        this._pendingDailyQuoteNotificationOpen = false;
        return true;
      }
      try {
        const raw = sessionStorage.getItem('odqDailyQuoteOpen');
        sessionStorage.removeItem('odqDailyQuoteOpen');
        if (!raw) return false;
        const parsed = JSON.parse(raw);
        const ageMs = Date.now() - Number(parsed?.at || 0);
    ...
    _peekColdStartNotificationIntent() {
      const rememberPending = String(this._pendingRememberTodayViewDateKey || '').trim();
      if (rememberPending) {
        return { kind: 'remember_today', dateKey: rememberPending };
      }
      try {
        const raw = sessionStorage.getItem('odqRememberTodayOpen');
        if (raw) {
          ...
          const dk = String(parsed?.dateKey || '').trim();
          const ageMs = Date.now() - Number(parsed?.at || 0);
          if (dk && Number.isFinite(ageMs) && ageMs <= 120000) {
            return { kind: 'remember_today', dateKey: dk };
          }
        }
      } catch (_) {
        /* no-op */
    ...
    _takeColdStartNotificationIntent() {
      const rememberDateKey = this._takeRememberTodayViewOpenIntent();
      if (rememberDateKey) {
        return { kind: 'remember_today', dateKey: rememberDateKey };
      }
      if (this._takeDailyQuoteNotificationOpenIntent()) {
        return { kind: 'daily_quote' };
      }
      return null;
    }

    _dailyQuoteNotificationTargetScreenId() {
      return this._shouldFastPathReturningSameDayQuote?.() ? 'screen-quilt' : 'screen-quote';
    }

    _clearNotificationOpenNavigationGuards() {
      this.clearPortalToQuoteIntroTimer();
      if (this.uiService) this.uiService._flowTransitioning = false;
    }

    _openDailyQuoteFromNotification(event) {
      if (typeof window.odqTrack === 'function') {
        const data = event?.notification?.data || {};
        const has_studio_floor_post = data.has_studio_floor_post === '1' ? 1 : 0;
        window.odqTrack('daily_notification_tap', { has_studio_floor_post });
      }
      this._clearNotificationOpenNavigationGuards();
      if (!this._postLiveSyncInitialized) {
        this._stashDailyQuoteNotificationOpenIntent();
        return;
      }
      this.uiService.showScreen(this._dailyQuoteNotificationTargetScreenId());
    }

    async _awaitColdStartNotificationIntent(maxWaitMs = 500) {
      const immediate = this._takeColdStartNotificationIntent();
      if (immediate) return immediate;

      const waitMs = Math.max(0, Number(maxWaitMs) || 0);
      if (!waitMs) return null;

      // Push/local tap handlers may attach just before or during live sync on cold start.
      const deadline = Date.now() + waitMs;
      while (Date.now() < deadline) {
        const peek = this._peekColdStartNotificationIntent();
        if (peek) return this._takeColdStartNotificationIntent();
        await this.wait(16);
      }
      return this._takeColdStartNotificationIntent();
    }

    /** @deprecated Use _awaitColdStartNotificationIntent */
    async _awaitRememberTodayNotificationOpenIntent(maxWaitMs = 500) {
      const intent = await this._awaitColdStartNotificationIntent(maxWaitMs);
      return intent?.kind === 'remember_today' ? intent.dateKey : null;
    }
    ...
    setupPushNotificationListeners() {
      ...
      messaging?.addListener('notificationReceived', (event) => {
        if (this.isStudioFloorPostPushNotification?.(event)) {
          this.handleStudioFloorPostPushNotification?.({ openFeed: false });
        }
      });
      messaging?.addListener('notificationActionPerformed', (event) => {
        if (this.isStudioFloorPostPushNotification?.(event)) {
          this.handleStudioFloorPostPushNotification?.({ openFeed: true });
          return;
        }
        ...
        this._openDailyQuoteFromNotification(event);
      });
    ...
    hideColorSubmitTransition(options = {}) {
      const overlay = document.getElementById('colorSubmitTransition');
      if (!overlay) return;
      overlay.classList.remove('color-submit-transition--visible');
      // Immediate remove: awaiting renderQuilt right after a CSS fade freezes the
      // overlay on-screen for the whole mosaic paint (felt like 5s+ of spinner).
    ...
    async submitColorToServer(selectedColor) {
      const baseUrl = String(CONFIG.BACKEND?.baseUrl || '').replace(/\/$/, '');
      if (!baseUrl) {
        const err = new Error('CONFIG.BACKEND.baseUrl is not set');
        err.transient = true;
        throw err;
      }
      const userId = this.currentUserId || Utils.getOrCreateUserId();
      const controller = typeof AbortController !== 'undefined' ? new AbortController() : null;
      const timer = controller ? setTimeout(() => controller.abort(), 16000) : null;
      try {
        const res = await fetch(`${baseUrl}/api/color-submission`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            color: selectedColor,
            appDateKey: Utils.getTodayKey(),
            clientId: userId,
            displayName: Utils.getNameThanksDisplayName?.() || '',
            clientBuildId: CONFIG.APP?.buildId || '',
            clientQuiltFingerprint:
              typeof Utils.computeQuiltFingerprint === 'function'
                ? Utils.computeQuiltFingerprint(this.quiltEngine?.blocks || [])
                : ''
          }),
        ...
        const data = await res.json().catch(() => null);
        if (!res.ok || !data?.success) {
          const err = new Error(data?.error || `Color submission failed (${res.status})`);
          err.status = res.status;
          err.transient = res.status === 503;
          throw err;
        }
        return data;
      } catch (error) {
    ...
    async applyServerColorSubmissionResult(result, selectedColor) {
      if (!result || !Array.isArray(result.blocks) || result.blocks.length === 0) {
        throw new Error('Server color submission returned no quilt blocks');
      }
      await this.applyQuiltDataFromPayload({
        dateKey: result.appDateKey || Utils.getTodayKey(),
        date: result.appDateKey || Utils.getTodayKey(),
        blocks: result.blocks,
        contributorCount: result.contributorCount || 1,
        colorReplayEvents: Array.isArray(result.colorReplayEvents) ? result.colorReplayEvents : [],
        contributors: Array.isArray(result.contributors) ? result.contributors : [],
        macroStructureFrozen: result.macroStructureFrozen === true
      });

      const dedicatedId = String(result.dedicatedBlockId || '').trim();
      let dedicatedBlock = dedicatedId
        ? this.quiltEngine?.blocks?.find((block) => String(block.id || '') === dedicatedId)
        : null;
      if (!dedicatedBlock) {
        const submissionIndex = Number(result.submissionIndex) || Number(this.quiltEngine?.submissionCount) || 0;
        dedicatedBlock = (this.quiltEngine?.blocks || []).find((block) => {
          if (Number(block?.submissionIndex) !== submissionIndex) return false;
          if (String(block?.contributorId || '').trim() === String(this.currentUserId || '').trim()) return true;
          const ids = Array.isArray(block?.contributorIds) ? block.contributorIds : [];
          return ids.some((id) => String(id || '').trim() === String(this.currentUserId || '').trim());
        }) || null;
      }
      if (dedicatedBlock?.id) {
        this._latestDedicatedBlockId = dedicatedBlock.id;
        try {
        ...
        const index = this.quiltEngine.blocks.findIndex((block) => block.id === dedicatedBlock.id);
        if (index !== -1) this.renderer.setLastAddedIndex(index);
        if (!result.duplicate && typeof this.quiltEngine?.recordUserContribution === 'function') {
          this.quiltEngine.recordUserContribution(
            dedicatedBlock.id,
            result.appliedColor || selectedColor,
            Number(result.submissionIndex) || Number(this.quiltEngine.submissionCount) || 0
          );
        }
      }
      this.recordDailyContributorForCurrentUser();
      return {
        appliedColor: result.appliedColor || selectedColor,
        submissionIndex: Number(result.submissionIndex) || Number(this.quiltEngine?.submissionCount) || 0,
        dedicatedBlockId: dedicatedBlock?.id || dedicatedId || '',
        dedicatedBlock: dedicatedBlock || null,
        duplicate: result.duplicate === true
      };
    }

    /** After admin or user quilt changes: upload quilt-screen-9x16 + blockCount when blocks actually changed. */
    _scheduleQuiltScreenIgRefreshAfterBlocksChange(options = {}) {
      if (this.isAdminTomorrowPreviewActive?.()) return;
      this._scheduleQuiltScreenIgUploadIfStale(options);
    }

    /** Upload quilt-screen-9x16 when live quilt fingerprint differs from last successful IG push this session. */
    _scheduleQuiltScreenIgUploadIfStale(options = {}) {
      if (this.isAdminTomorrowPreviewActive?.()) return;
      if (typeof this._isAppForegroundForQuiltSync === 'function' && !this._isAppForegroundForQuiltSync()) {
        return;
      }
      const blocks = this.quiltEngine?.blocks;
      ...
      if (
        afterFingerprint &&
        afterFingerprint === String(this._lastQuiltScreenIgUploadFingerprint || '').trim()
      ) {
        return;
      }
      this.scheduleStoredStorySnapshotRefreshAfterQuiltUpdate?.({
        dateKey: options.dateKey || this.getEffectiveAppDateKey?.() || Utils.getTodayKey(),
        delayMs: Number.isFinite(Number(options.delayMs)) ? Number(options.delayMs) : 400,
        force: options.force === true
      });
    }

    /** On launch/resume: upload when instagram-images fingerprint trails live quilt. */
    async _maybeScheduleQuiltScreenIgCatchUpFromFirestore(dateKey) {
      const dk = String(dateKey || this.getEffectiveAppDateKey?.() || Utils.getTodayKey()).trim();
      if (!dk || this.isAdminTomorrowPreviewActive?.()) return;
      const blocks = this.quiltEngine?.blocks;
      if (!Array.isArray(blocks) || blocks.length <= 1) return;
      ...
      if (liveFingerprint === String(this._lastQuiltScreenIgUploadFingerprint || '').trim()) return;
      let storedFingerprint = '';
      try {
        if (window.db && window.firestore?.doc && window.firestore?.getDoc) {
          const snap = await window.firestore.getDoc(
            window.firestore.doc(window.db, 'instagram-images', dk)
          );
          const data = snap.exists?.() ? snap.data() : null;
          storedFingerprint = String(data?.quiltFingerprint || '').trim();
        }
      } catch (err) {
        this.logger?.warn?.('Quilt-screen IG catch-up Firestore read skipped:', err);
      }
      if (storedFingerprint && storedFingerprint === liveFingerprint) {
        this._lastQuiltScreenIgUploadFingerprint = liveFingerprint;
        return;
      }
      this._scheduleQuiltScreenIgUploadIfStale?.({ dateKey: dk, delayMs: 2500, force: true });
    }

    scheduleStoredStorySnapshotRefreshAfterQuiltUpdate(options = {}) {
      const dateKey = String(options.dateKey || this.getEffectiveAppDateKey?.() || Utils.getTodayKey()).trim();
      if (!dateKey) return;
      const blocks = this.quiltEngine?.blocks;
      if (!Array.isArray(blocks) || blocks.length <= 1) return;
      if (this._storySnapshotRefreshScheduledTimer) {
        clearTimeout(this._storySnapshotRefreshScheduledTimer);
        this._storySnapshotRefreshScheduledTimer = null;
      }
      this._storySnapshotRefreshScheduledDateKey = dateKey;
      const delayMs = Number.isFinite(Number(options.delayMs))
        ? Math.max(400, Number(options.delayMs))
        : 900;
      this._storySnapshotRefreshScheduledTimer = setTimeout(() => {
        this._storySnapshotRefreshScheduledTimer = null;
        if (this._storySnapshotRefreshScheduledDateKey !== dateKey) return;
        void this.refreshStoredQuiltScreenLayerAfterQuiltUpdate({
          dateKey,
          force: options.force === true
        });
      }, delayMs);
    ...
    async refreshStoredQuiltScreenLayerAfterQuiltUpdate(options = {}) {
      if (this._quiltScreenLayerRefreshInProgress) {
        this._quiltScreenLayerRefreshRerunQueued = true;
        return;
      }
      const blocks = this.quiltEngine?.blocks;
      if (!Array.isArray(blocks) || blocks.length <= 1) return;
      const arch = this.archiveService;
      if (
        !arch?.generateInstagramQuiltScreen9x16ImageData ||
        (typeof Utils.writeInstagramImagesDocForZapier !== 'function' &&
          typeof Utils.writeInstagramImagesDocForZapierViaServer !== 'function')
      ) {
        return;
      }
      this._quiltScreenLayerRefreshInProgress = true;
      try {
        if (typeof this.ensureLayoutBComposeReady === 'function') {
          await this.ensureLayoutBComposeReady().catch((err) => {
            this.logger?.warn?.('Layout B compose preload for story snapshot skipped:', err);
          });
        }
        /** Optional — client Storage auth helper lives on the admin slice. Never abort the upload pipeline if it fails. */
        ...
        try {
          quote =
            (qs && typeof qs.getQuoteResolvedForInstagramDateKey === 'function'
              ? await qs.getQuoteResolvedForInstagramDateKey(dateKey, { requireLive: true })
        ...
        const baseMeta = {
          ...
          contributorCount: Math.max(1, this._humanContributorPickCount())
        };
        const quiltScreenFpFresh =
          !!baseMeta.quiltFingerprint &&
          baseMeta.quiltFingerprint === String(this._lastQuiltScreenIgUploadFingerprint || '').trim();
        const quoteFitFpFresh =
          !USE_STORY_QUOTE_FIT_PREVIEW ||
          (!!baseMeta.quiltFingerprint &&
            baseMeta.quiltFingerprint ===
              String(this._lastStoryQuoteFitPreviewUploadFingerprint || '').trim());
        if (options.force !== true && quiltScreenFpFresh && quoteFitFpFresh) {
          return;
        }
        const uploadSnapshot = async (uploadPayload) => {
          const tryBackendFirst =
            typeof Utils.writeInstagramImagesDocForZapierViaServer === 'function';
          if (tryBackendFirst) {
            try {
              await timed(
                'Backend IG preview upload after quilt update',
                Utils.writeInstagramImagesDocForZapierViaServer(uploadPayload),
                180000
              );
              return;
              ...
              this.logger?.warn?.(
                'Backend IG preview upload after quilt update failed, trying client',
                backendErr
              );
            }
          }
          await timed(
            'Firestore IG preview upload after quilt update',
            Utils.writeInstagramImagesDocForZapier(uploadPayload),
            180000
          );
        };

        /** Critical path: quilt-screen first so the next visitor has a predecessor even if story JPEG is slow. */
        let quiltScreen9x16ImageData = null;
        if (!quiltScreenFpFresh || options.force === true) {
          const generateQuiltScreen = () =>
            timed(
              'Quilt screen 9:16 after quilt update',
              arch.generateInstagramQuiltScreen9x16ImageData(blocks, dateKey),
              90000
            );
          try {
            quiltScreen9x16ImageData = await generateQuiltScreen();
          } catch (err) {
            this.logger?.warn?.('Quilt screen 9:16 refresh after quilt update skipped:', err);
          }
          if (!quiltScreen9x16ImageData) {
            ...
            try {
              ...
              this.logger?.warn?.('Quilt screen 9:16 retry after quilt update skipped:', retryErr);
            }
          }
          if (quiltScreen9x16ImageData) {
            await uploadSnapshot({ ...baseMeta, quiltScreen9x16ImageData });
            this._lastQuiltScreenIgUploadFingerprint = String(baseMeta.quiltFingerprint || '').trim();
            this.logger?.log?.('✅ Stored quilt-screen layer refreshed after quilt update');
          }
        }

        /** Quote-fit: bake a compact predecessor JPEG for the next visitor (skip Layout B overlay). */
        let storyLayoutBPreviewImageData = null;
        if (USE_STORY_QUOTE_FIT_PREVIEW) {
          if (
            (!quoteFitFpFresh || options.force === true) &&
            typeof arch.generateInstagramStoryQuoteFitPreviewJpegImageData === 'function'
          ) {
            try {
              storyLayoutBPreviewImageData = await timed(
                'Story quote-fit preview JPEG after quilt update',
                arch.generateInstagramStoryQuoteFitPreviewJpegImageData(blocks, quote, dateKey),
                120000
              );
            } catch (err) {
              this.logger?.warn?.(
                'Story quote-fit preview JPEG refresh after quilt update skipped:',
                err
              );
            }
            if (storyLayoutBPreviewImageData) {
              ...
              this._lastStoryQuoteFitPreviewUploadFingerprint = String(
                baseMeta.quiltFingerprint || ''
              ).trim();
              this.logger?.log?.('✅ Stored story quote-fit preview refreshed after quilt update');
            }
          }
        } else if (typeof arch.generateInstagramStoryLayoutBOverlayImage === 'function') {
          /** Keep speaker/quote strips on the transparent overlay layer (stable across quilt growth). */
          let storyLayoutBOverlayImageData = null;
          try {
            storyLayoutBOverlayImageData = await timed(
              'Layout B story overlay after quilt update',
              arch.generateInstagramStoryLayoutBOverlayImage(blocks, quote, dateKey),
              120000
            );
          } catch (err) {
            this.logger?.warn?.('Layout B story overlay refresh after quilt update skipped:', err);
          }
          if (storyLayoutBOverlayImageData) {
            await uploadSnapshot({ ...baseMeta, storyLayoutBOverlayImageData });
            this.logger?.log?.('✅ Stored story overlay refreshed after quilt update');
          }
          if (!quiltScreen9x16ImageData && !storyLayoutBOverlayImageData) {
            this.logger?.warn?.('Stored quilt-screen layer refresh produced no images');
          } else {
            this._layoutBStoryPreviewHeavyDoneThisVisit = false;
            this.scheduleLayoutBStoryPreviewRefresh?.({ force: true, delayMs: 300 });
          }
        }

        if (USE_STORY_QUOTE_FIT_PREVIEW) {
          const attemptedQuiltScreen = !quiltScreenFpFresh || options.force === true;
          const attemptedQuoteFit = !quoteFitFpFresh || options.force === true;
          if (
            (attemptedQuiltScreen && !quiltScreen9x16ImageData) &&
            (attemptedQuoteFit && !storyLayoutBPreviewImageData)
          ) {
            this.logger?.warn?.('Stored quilt-screen / quote-fit preview refresh produced no images');
          }
          /** Keep this visitor on the predecessor already on screen; new JPEG is for the next launch.
           *  Exception: seed upload path force-refreshes once so a missing Firebase JPEG can appear. */
        }
      } catch (error) {
        this.logger?.warn?.('Stored quilt-screen layer refresh after quilt update failed:', error);
      } finally {
        this._quiltScreenLayerRefreshInProgress = false;
        if (this._quiltScreenLayerRefreshRerunQueued) {
          this._quiltScreenLayerRefreshRerunQueued = false;
          void this.refreshStoredQuiltScreenLayerAfterQuiltUpdate(options);
        }
      }
    }

    async refreshStoredStorySnapshotAfterServerColorSubmission(options = {}) {
      if (this._storySnapshotRefreshInProgress) {
        this._storySnapshotRefreshRerunQueued = true;
        return;
      }
      const blocks = this.quiltEngine?.blocks;
      if (!Array.isArray(blocks) || blocks.length <= 1) return;
      ...
      const afterTuneSave = options.afterTuneSave === true;
      this._storySnapshotRefreshInProgress = true;
      try {
        if (typeof this.ensureLayoutBComposeReady === 'function') {
          await this.ensureLayoutBComposeReady().catch((err) => {
            this.logger?.warn?.('Layout B compose preload for story snapshot skipped:', err);
        ...
        if (afterTuneSave) {
          ...
          if (USE_STORY_QUOTE_FIT_PREVIEW) {
            ...
            if (typeof arch?.generateInstagramStoryQuoteFitPreviewJpegImageData === 'function') {
              storyLayoutBPreviewImageData = await timed(
                'Story quote-fit preview JPEG after tune',
                arch.generateInstagramStoryQuoteFitPreviewJpegImageData(blocks, quote, dateKey),
                120000
              ).catch((err) => {
                this.logger?.warn?.('Story quote-fit preview JPEG after tune skipped:', err);
                return null;
              });
            }
            if (!storyLayoutBPreviewImageData) return;
            uploadPayload = {
              ...uploadPayload,
              storyLayoutBPreviewImageData,
              layoutBStoryPreviewRenderVersion: STORY_QUOTE_FIT_PREVIEW_RENDER_VERSION()
            ...
            if (arch?.generateInstagramStoryLayoutBOverlayImage) {
              storyLayoutBOverlayImageData = await timed(
                'Layout B story overlay after tune',
                arch.generateInstagramStoryLayoutBOverlayImage(blocks, quote, dateKey),
                120000
              ).catch((err) => {
                this.logger?.warn?.('Layout B story overlay after tune skipped:', err);
                return null;
              });
            }
            if (!storyLayoutBOverlayImageData) return;
            uploadPayload = { ...uploadPayload, storyLayoutBOverlayImageData };
          }
        } else {
          let quiltScreen9x16ImageData = null;
          if (arch?.generateInstagramQuiltScreen9x16ImageData) {
            quiltScreen9x16ImageData = await timed(
              'Quilt screen 9:16 snapshot',
              arch.generateInstagramQuiltScreen9x16ImageData(blocks, dateKey),
              90000
            ).catch((err) => {
              this.logger?.warn?.('Quilt screen 9:16 snapshot skipped:', err);
              return null;
            });
          }
          if (!quiltScreen9x16ImageData) return;
          uploadPayload = { ...uploadPayload, quiltScreen9x16ImageData };
        }
        try {
          await timed(
            'Firestore story snapshot upload',
            Utils.writeInstagramImagesDocForZapier(uploadPayload),
            180000
          );
        } catch (clientUploadErr) {
          this.logger?.warn?.('Story snapshot client upload failed, trying backend', clientUploadErr);
          if (typeof Utils.writeInstagramImagesDocForZapierViaServer === 'function') {
            await timed(
              'Backend story snapshot upload',
              Utils.writeInstagramImagesDocForZapierViaServer(uploadPayload),
              180000
            );
          } else {
            throw clientUploadErr;
          }
        }
        this.logger?.log?.(
          afterTuneSave
            ? USE_STORY_QUOTE_FIT_PREVIEW
              ? '✅ Stored story quote-fit preview refreshed after tune save'
              : '✅ Stored story overlay refreshed after tune save'
            : '✅ Stored quilt-screen layer refreshed after quilt update'
        );
        if (afterTuneSave) {
          this._invalidateLayoutBStoryPreviewAfterTuneSave?.();
          this._layoutBStoryPreviewSkipStoredOnce = false;
          this._layoutBStoryPreviewHeavyDoneThisVisit = false;
          this.scheduleLayoutBStoryPreviewRefresh?.({ force: true, delayMs: 400 });
        }
      } catch (error) {
        this.logger?.warn?.('Stored story snapshot refresh after quilt update failed:', error);
      } finally {
        this._storySnapshotRefreshInProgress = false;
        if (this._storySnapshotRefreshRerunQueued) {
          this._storySnapshotRefreshRerunQueued = false;
          void this.refreshStoredStorySnapshotAfterServerColorSubmission(options);
        }
      }
    }

    async retryColorSaveOnFreshSharedQuilt(selectedColor) {
      if (this._isPersonalQuiltMode || this.isAdminTomorrowPreviewActive?.()) return false;
      if (!Utils.validateHexColor(selectedColor)) return false;
      try {
        const todayKey = Utils.getTodayKey();
        const latest = await this.dataService.loadQuiltFromServer(todayKey);
        if (!latest?.ok) {
          this.logger.warn('Color save retry could not load latest shared quilt:', latest?.reason || latest?.error);
          return false;
        }

        await this.applyQuiltDataFromPayload(latest.data);
        const retryResult = this.quiltEngine?.addColor?.(selectedColor);
        if (!retryResult) {
          this.logger.warn('Color save retry could not place selected color on latest quilt');
          return false;
        }

        this.recordDailyContributorForCurrentUser();
        const saved = await this.saveQuilt({ sourceFunction: 'color-submit-stale-retry' });
        if (saved) {
          this.logger.log('✅ Color saved after refreshing stale shared quilt');
        }
        return saved;
      } catch (error) {
        this.logger.warn('Color save retry failed:', error);
        return false;
      }
    }

    isPostColorCutRevealEnabled() {
      try {
        const params = new URLSearchParams(typeof location !== 'undefined' ? location.search : '');
        const qp = String(params.get('postColorCutReveal') || '').trim().toLowerCase();
        if (qp === '0' || qp === 'false' || qp === 'no') return false;
        if (qp === '1' || qp === 'true' || qp === 'yes') {
          return typeof window.PostColorCutReveal !== 'undefined';
        }
      } catch (_) {
        /* ignore */
      }
      ...
      if (typeof window.PostColorCutReveal === 'undefined') {
        return false;
      }
      return CONFIG?.APP?.postColorCutRevealEnabled === true;
    }

    _destroyPostColorCutRevealHost() {
      try {
        this._postColorCutController?.abort?.();
        this._postColorCutController?.destroy?.();
      } catch (_) {
        /* ignore */
      }
      this._postColorCutController = null;
      document.getElementById('postColorCutRevealHost')?.remove();
    }

    _resolveColorSubmissionFromLivePreview(livePreview) {
      const candidate = livePreview?.submission;
      if (!candidate || typeof candidate !== 'object') return null;
      if (candidate.success === true && Array.isArray(candidate.blocks)) return candidate;
      const nested = candidate.submission;
      if (nested && nested.success === true && Array.isArray(nested.blocks)) return nested;
      return candidate.success === true ? candidate : null;
    }

    async runPostColorCutRevealSubmit(selectedColor) {
      this.hideColorSubmitTransition();
      this._destroyPostColorCutRevealHost();

      // Full viewport so the cutting-mat photo fills the screen (no flat green letterbox).
      const panelW = Math.max(320, Math.round(window.innerWidth || 390));
      const panelH = Math.max(480, Math.round(window.innerHeight || 844));
      const clientId = this.currentUserId || Utils.getOrCreateUserId();
      const appDateKey = Utils.getTodayKey();

      // Start server work immediately — don't wait on mat assets / showScreen.
      let serverResult = null;
      const submitPromise = this.submitColorToServer(selectedColor).then((serverData) => {
      ...
      // Warm mat photo in the background so it can upgrade once cached.
      try {
        void window.CarouselCuttingMatBg?.loadCarouselCuttingMatImage?.();
      } catch (_) {
        /* optional */
      }

      const host = document.createElement('div');
      host.id = 'postColorCutRevealHost';
      host.className = 'post-color-cut-reveal-host';
      host.setAttribute('role', 'status');
      host.setAttribute('aria-live', 'polite');
      document.body.appendChild(host);

      const controller = window.PostColorCutReveal.mount(host, {
        panelW,
        panelH,
        color: selectedColor,
        appDateKey,
      ...
      this._postColorCutController = controller;

      let applyResult = null;
      let applyPromise = null;
      let quiltHandoffPromise = null;
      let playResult = null;
      ...
      try {
        await controller.prepare({
          ...
          appDateKey,
          clientId,
          lightweightPieces: true
        });

        const waitForMatFade = (matHost) =>
          new Promise((resolve) => {
            if (!matHost?.isConnected) {
              resolve();
              return;
            }
            let done = false;
            const finish = () => {
              ...
              matHost.removeEventListener('transitionend', onEnd);
              resolve();
            };
            const onEnd = (e) => {
              if (e.target !== matHost || e.propertyName !== 'opacity') return;
              finish();
            };
            matHost.addEventListener('transitionend', onEnd);
            setTimeout(finish, 180);
          });

        let quiltPaintedUnderMat = false;
        let paintUnderMatInFlight = null;

        const startApplyIfNeeded = () => {
          if (applyPromise || !serverResult?.success) return applyPromise;
          // Runs in the still gap between cut poses (onAfterCut) — not mid-scrap.
          applyPromise = this.applyServerColorSubmissionResult(serverResult, selectedColor)
            .then((result) => {
              applyResult = result;
              return result;
            })
            .catch((applyErr) => {
              this.logger.warn('Apply during cut reveal failed:', applyErr);
              return null;
            });
          return applyPromise;
        };

        const paintQuiltUnderMat = () => {
          if (quiltPaintedUnderMat) return Promise.resolve();
          if (paintUnderMatInFlight) return paintUnderMatInFlight;
          paintUnderMatInFlight = (async () => {
            try {
              if (!serverResult?.success) {
                const data = await submitPromise;
                if (data?.success) serverResult = data;
              }
              if (!serverResult?.success) return;
              this.hideQuoteColorPickerPane?.();
              this.uiService.showScreen('screen-quilt', { fastQuiltHandoff: true });
              startApplyIfNeeded();
              if (applyPromise) {
                applyResult = (await applyPromise) || applyResult;
              }
              await this.renderQuilt({ force: true, viewportOnly: true, cutRevealHandoff: true });
              await new Promise((resolve) => requestAnimationFrame(resolve));
              quiltPaintedUnderMat = true;
            } finally {
              paintUnderMatInFlight = null;
            }
          })();
          return paintUnderMatInFlight;
        };

        const finishQuiltChrome = () => {
          void (async () => {
            try {
              // Let the fast fade settle before heavier chrome work.
              ...
              await this.renderQuilt({ force: true });
              this.applyQuiltMoodTriptychPalette?.();
              this.refreshQuiltFortuneReveal?.();
              this.mountQuiltNameLeaderboard?.();
              this._syncQuiltScrollIconFooterLeakGuard?.();
            } catch (chromeErr) {
              this.logger.warn('Cut reveal quilt chrome refresh failed:', chromeErr);
            }
          })();
        };

        /**
         * Paint happens in the still gap before the last cut.
         * Handoff should be fade-only when that pre-paint finished.
         */
        const fireQuiltHandoff = () => {
          if (handoffFired) return quiltHandoffPromise;
          handoffFired = true;
          const matHost = document.getElementById('postColorCutRevealHost');
          const startFadeNow = () => {
            matHost?.classList.add('post-color-cut-reveal-host--handoff');
          };
          if (quiltPaintedUnderMat) {
            startFadeNow();
          }

          quiltHandoffPromise = (async () => {
            try {
              if (!quiltPaintedUnderMat) {
                await paintQuiltUnderMat();
                startFadeNow();
              }
              await waitForMatFade(matHost);
              this._destroyPostColorCutRevealHost();
              ...
              this.logger.warn('Cut reveal quilt handoff failed:', handoffErr);
              this._destroyPostColorCutRevealHost();
              return { applyResult: applyResult || null, error: handoffErr };
            }
          })();
          return quiltHandoffPromise;
        };

        // When the server answers, remember it for the next between-cut apply gap.
        void Promise.resolve(submitPromise).then((data) => {
          if (data?.success) serverResult = data;
        });

        try {
          playResult = await controller.playWithSubmission(submitPromise, {
            color: selectedColor,
            appDateKey,
            clientId,
            onHandoff: fireQuiltHandoff,
            onAfterCut: () => {
              if (handoffFired) return;
              startApplyIfNeeded();
            },
            onBeforeLastCut: () => paintQuiltUnderMat()
          });
        } catch (playErr) {
          this.logger.warn('Cut reveal animation failed after color save:', playErr);
          if (!serverResult?.success) throw playErr;
          if (!handoffFired) fireQuiltHandoff();
        }
        if (playResult?.aborted && !handoffFired) {
          throw new Error('Color reveal interrupted');
        }
        if (!serverResult) {
          serverResult = this._resolveColorSubmissionFromLivePreview(playResult?.livePreview);
        }
        if (!serverResult?.success) {
          throw new Error(serverResult?.error || 'Color submission failed');
        }
        return {
          playResult,
          serverResult,
          ...
          applyPromise,
          quiltHandoffPromise,
          cutRevealFinished: handoffFired
        };
      } finally {
        if (!handoffFired) {
          this._destroyPostColorCutRevealHost();
    ...
    async handleAddColor() {
      if (this._colorSubmitInProgress) return;
      this._colorSubmitInProgress = true;
      let cutRevealFinished = false;
      let cutRevealQuiltHandoffPromise = null;
      let useCutReveal = false;
      try {
        ...
        let result = null;
        let usedServerSubmission = false;
        useCutReveal = this.isPostColorCutRevealEnabled();
        this.hideColorSubmitTransition();

        if (useCutReveal) {
          try {
            const revealOutcome = await this.runPostColorCutRevealSubmit(selectedColor);
            cutRevealFinished = revealOutcome.cutRevealFinished === true;
            cutRevealQuiltHandoffPromise = revealOutcome.quiltHandoffPromise || null;
            usedServerSubmission = true;
            result = revealOutcome.applyResult;
            if (!result && !cutRevealQuiltHandoffPromise && revealOutcome.serverResult) {
              try {
                result = await this.applyServerColorSubmissionResult(
                  revealOutcome.serverResult,
                  selectedColor
                );
              } catch (applyErr) {
                this.logger.warn('Retry apply after cut reveal failed:', applyErr);
                this.uiService.showToast(applyErr.message || failureMessage);
                return;
              }
            } else if (!result && cutRevealQuiltHandoffPromise && revealOutcome.applyPromise) {
              void revealOutcome.applyPromise
                .then((applied) => {
            ...
            this.scheduleStoredStorySnapshotRefreshAfterQuiltUpdate?.({
              dateKey: revealOutcome.serverResult?.appDateKey || Utils.getTodayKey(),
              force: true,
              delayMs: 400
            });
          } catch (revealErr) {
            this.logger.warn(
              'Post-color cut reveal failed; falling back to silent server submit:',
              revealErr
            );
            console.warn('[post-color-cut-reveal]', revealErr?.message || revealErr);
            this._destroyPostColorCutRevealHost();
            this.hideColorSubmitTransition();
          }
        }

        const COLOR_SUBMIT_MIN_HOLD_MS = 350;
        const ensureColorSubmitMinHold = async (startedAtMs) => {
          const elapsed =
            (typeof performance !== 'undefined' ? performance.now() : Date.now()) - startedAtMs;
          const remaining = COLOR_SUBMIT_MIN_HOLD_MS - elapsed;
          if (remaining > 0) await this.wait(remaining);
        };
        const showTryAgain = async () => {
          if (cutRevealFinished || useCutReveal) {
            this.uiService.showToast(failureMessage);
            return;
          }
          if (!document.getElementById('colorSubmitTransition')) {
            this.showColorSubmitTransition(selectedColor, failureMessage);
          } else {
            this.updateColorSubmitTransition(failureMessage);
          }
          await ensureColorSubmitMinHold(
        ...
        const applySelectedColorToCurrentQuilt = () => {
          ...
          if (typeof this._getTodayQuoteMoodHex === 'function') {
            this.quiltEngine.dailyMoodColorHex = this._getTodayQuoteMoodHex() || null;
          }
          let result;
          try {
        ...
        let serverSubmitPromise = null;
        let serverSubmitInFlight = false;
        let usedOptimisticLocalApply = false;
        // Cap the solid-color interstitial — slow networks used to pin it for the
        // whole /api/color-submission round-trip (often multi-second).
        const COLOR_SUBMIT_SPINNER_CAP_MS = 480;

        if (!usedServerSubmission && !useCutReveal) {
          const submitStartedAt =
            typeof performance !== 'undefined' ? performance.now() : Date.now();
          this.showColorSubmitTransition(selectedColor, this.getColorSubmitPlacementMessage());
          await this.waitForNextPaint();

          let serverSubmitError = null;
          serverSubmitInFlight = true;
          serverSubmitPromise = this.submitColorToServer(selectedColor)
            .then((data) => {
              pendingServerResult = data;
              usedServerSubmission = true;
              return data;
            })
            .catch((err) => {
              serverSubmitError = err;
              throw err;
            });

          const raceOutcome = await Promise.race([
            serverSubmitPromise.then(() => 'ok').catch(() => 'err'),
            this.wait(COLOR_SUBMIT_SPINNER_CAP_MS).then(() => 'cap')
          ]);

          if (raceOutcome === 'ok') {
            await ensureColorSubmitMinHold(submitStartedAt);
          } else if (raceOutcome === 'err') {
            serverSubmitInFlight = false;
            this.logger.warn('Server color submission failed:', serverSubmitError);
            if (!serverSubmitError?.transient) {
              this.uiService.showToast(
                serverSubmitError?.message || 'Could not save your color yet. Please try again.'
              );
              await showTryAgain();
              return;
            }

            result = applySelectedColorToCurrentQuilt();
            if (!result) {
              try {
            ...
            // Spinner cap hit: paint locally and leave — reconcile when server returns.
            result = applySelectedColorToCurrentQuilt();
            usedOptimisticLocalApply = !!result;
            if (!result) {
              try {
                pendingServerResult = await serverSubmitPromise;
                usedServerSubmission = true;
                await ensureColorSubmitMinHold(submitStartedAt);
              } catch (serverError) {
                serverSubmitInFlight = false;
                this.logger.warn('Server color submission failed:', serverError);
                if (!serverError?.transient) {
                  this.uiService.showToast(
                    serverError.message || 'Could not save your color yet. Please try again.'
                  );
                  await showTryAgain();
                  return;
                }
        ...
        } else if (useCutReveal && !usedServerSubmission && !cutRevealFinished) {
          try {
            const serverResult = await this.submitColorToServer(selectedColor);
            result = await this.applyServerColorSubmissionResult(serverResult, selectedColor);
            usedServerSubmission = true;
            this.scheduleStoredStorySnapshotRefreshAfterQuiltUpdate?.({
              dateKey: serverResult?.appDateKey || Utils.getTodayKey(),
              force: true,
              delayMs: 400
            });
          } catch (fallbackErr) {
            this.logger.warn('Cut reveal fallback submit failed:', fallbackErr);
            this.uiService.showToast(fallbackErr.message || failureMessage);
            await showTryAgain();
            return;
          }
        }

        if (result || pendingServerResult || cutRevealFinished || serverSubmitInFlight) {
          if (typeof window.odqTrack === 'function') {
            window.odqTrack('add_color', {
              block_count: Number(this.quiltEngine?.submissionCount) || 0,
              submission_index:
                Number(result?.submissionIndex) ||
                Number(pendingServerResult?.submissionIndex) ||
                0
            });
          }
          // Only client-save when server path never started (true offline fallback).
          if (!usedServerSubmission && !useCutReveal && !serverSubmitInFlight) {
            await ensureColorSubmitMinHold(
              typeof performance !== 'undefined' ? performance.now() : Date.now()
            );
            let saved = await this.saveQuilt({ sourceFunction: 'color-submit-fallback' });
            if (!saved) {
              saved = await this.retryColorSaveOnFreshSharedQuilt(selectedColor);
            }
            if (!saved) {
              this.uiService.showToast('Could not save your color yet. Please try again.');
              try {
                await this.loadQuilt();
                this.renderQuilt();
                this.updateSquareCounter();
              } catch (reloadError) {
                this.logger.warn('Could not reload quilt after failed color save:', reloadError);
              }
              await showTryAgain();
              return;
            }
          ...
          if (!cutRevealFinished) {
            this.markDailyColorCompleted();
            this.uiService.showScreen('screen-quilt', { fastQuiltHandoff: true });
          }
          // Rip spinner off before apply/render — those were freezing it on screen.
          this.hideColorSubmitTransition({ immediate: true });

          if (cutRevealQuiltHandoffPromise) {
            void cutRevealQuiltHandoffPromise
              .then((handoffOutcome) => {
                if (handoffOutcome?.applyResult) {
                  result = handoffOutcome.applyResult;
                }
              })
              .catch((handoffErr) => {
                this.logger.warn('Cut reveal quilt handoff failed:', handoffErr);
            ...
            void (async () => {
              try {
                // If we left the spinner early, paint the local placement now so the
                // quilt isn't blank while the server finishes.
                if (usedOptimisticLocalApply && !pendingServerResult) {
                  await this.renderQuilt({
                    force: true,
                    viewportOnly: true,
                    cutRevealHandoff: true
                  });
                  this.ensureQuiltScreenInteractive?.();
                }

                if (serverSubmitPromise && !pendingServerResult) {
                  try {
                    pendingServerResult = await serverSubmitPromise;
                    usedServerSubmission = true;
                  } catch (serverError) {
                    this.logger.warn('Background color submission failed:', serverError);
                    if (usedOptimisticLocalApply) {
                      if (!serverError?.transient) {
                        this.uiService.showToast(
                          serverError?.message ||
                            'Could not save your color yet. Please try again.'
                        );
                        try {
                          await this.loadQuilt();
                        } catch (reloadError) {
                          this.logger.warn(
                            'Could not reload quilt after failed background color save:',
                            reloadError
                          );
                        }
                      } else {
                        let saved = await this.saveQuilt({
                          sourceFunction: 'color-submit-optimistic-fallback'
                        });
                        if (!saved) {
                          saved = await this.retryColorSaveOnFreshSharedQuilt(selectedColor);
                        }
                        if (!saved) {
                          ...
                          try {
                            ...
                            this.logger.warn(
                              'Could not reload quilt after failed optimistic save:',
                              reloadError
                            );
                          }
                        }
                ...
                if (pendingServerResult) {
                  ...
                  this.scheduleStoredStorySnapshotRefreshAfterQuiltUpdate?.({
                    dateKey: pendingServerResult?.appDateKey || Utils.getTodayKey(),
                    force: true,
                    delayMs: 400
                  });
                }
          ...
          const submissionCountAfter = Number(this.quiltEngine?.submissionCount) || 0;
          void (async () => {
            if (submissionCountAfter === 5) {
              try {
                const baseUrl = typeof this._getPublicQuiltNameApiBaseUrl === 'function'
                  ? this._getPublicQuiltNameApiBaseUrl()
                  : '';
                if (baseUrl) {
                  const d = new Date();
                  const dateKey = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
                  await fetch(`${baseUrl}/api/quilt-name-generate`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ dateKey, blockCount: submissionCountAfter })
                  });
                }
        ...
        if (useCutReveal || cutRevealFinished) {
          this.hideColorSubmitTransition();
          this.uiService.showToast('We hit a snag. Mind trying again?');
        } else {
        ...
        this._colorSubmitInProgress = false;
        this.hideColorSubmitTransition();
      }
    }
  }
