...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    async _refreshStoryQuoteFitPreviewCore(options = {}) {
      ...
      if (!adminTomorrow) {
        ...
        if (!skipStoredPreview) {
          ...
          /** No usable Firebase JPEG — seed one in the background; do not live-compose here. */
          this._scheduleQuoteFitPreviewSeedUpload?.(dk, currentFp);
          wrap.setAttribute('aria-busy', 'false');
          ...
          if (!hasVisibleStored) {
            ...
            if (hint) {
              hint.hidden = false;
              hint.textContent = 'Story preview preparing…';
            }
          }
          this._layoutBStoryPreviewHeavyDoneThisVisit = true;
          this._layoutBStoryPreviewContentKey = storedPreviewKey;
          return;
        }
      }
    ...
    /** One background bake+upload when Firebase has no quote-fit JPEG yet. */
    _scheduleQuoteFitPreviewSeedUpload(dateKey = '', fingerprint = '') {
      if (!USE_STORY_QUOTE_FIT_PREVIEW) return;
      if (this.isAdminTomorrowPreviewActive?.()) return;
      const dk = String(dateKey || this.getEffectiveAppDateKey?.() || '').trim();
      const fp = String(fingerprint || '').trim();
      if (!dk) return;
      const seedKey = `${dk}|${fp || 'fp0'}`;
      if (this._quoteFitPreviewSeedKey === seedKey || this._quoteFitPreviewSeedInFlight) return;
      this._quoteFitPreviewSeedKey = seedKey;
      this._quoteFitPreviewSeedInFlight = true;
      const delayMs = 400;
      setTimeout(() => {
        void (async () => {
          try {
            await this.refreshStoredQuiltScreenLayerAfterQuiltUpdate?.({
              dateKey: dk,
              force: true
            });
            this._layoutBStoryPreviewHeavyDoneThisVisit = false;
            this.scheduleLayoutBStoryPreviewRefresh?.({ force: true, delayMs: 200 });
          } catch (err) {
            this.logger?.warn?.('Quote-fit preview seed upload failed:', err);
            this._quoteFitPreviewSeedKey = '';
          } finally {
            this._quoteFitPreviewSeedInFlight = false;
          }
        })();
      }, delayMs);
    }
