...
(function (root) {
  ...
  const STORY_QUOTE_FIT_PREVIEW_RENDER_VERSION = () =>
    String(globalThis.ODQ_STORY_QUOTE_FIT_PREVIEW_RENDER_VERSION || 'story-quote-fit-preview-v1').trim();
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    async _tryStoredLayoutBStoryPreview(dateKey, previewContentKey = '', options = {}) {
      ...
      if (preferStoryPreviewJpeg && expectedPreviewRenderVersion) {
        ...
        if (!previewVersionOk) {
          /** Wrong-format / missing quote-fit JPEG — let caller live-compose rather than show quilt-only. */
    ...
    /**
     * Quilt-screen story preview: stored predecessor quote-fit JPEG (compact).
     * Live compose only for admin tomorrow / tune force / blank fallback.
     */
    ...
    async refreshStoredQuiltScreenLayerAfterQuiltUpdate(options = {}) {
      ...
      try {
        ...
        if (USE_STORY_QUOTE_FIT_PREVIEW) {
          if (!quiltScreen9x16ImageData && !storyLayoutBPreviewImageData) {
