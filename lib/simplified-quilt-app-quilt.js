...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    fitContributorCloudToViewport(listEl) {
      ...
      const panelOverflows = () => getContentHeight() > maxPanelHeight;
      const needsShrink = () => panelOverflows() || nameOverflowsPanel();

      let scale = 1;
      if (needsShrink()) {
        const ratio = maxPanelHeight / Math.max(1, getContentHeight());
        scale = Math.max(minScale, Math.min(1, ratio * 0.98));
        applyScale(scale);
      }

      let guard = 0;
      while (needsShrink() && scale > minScale && guard < 48) {
