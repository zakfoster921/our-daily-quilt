...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    async runPostColorCutRevealSubmit(selectedColor) {
      ...
      const panelW = Math.min(Math.round(window.innerWidth || 390), 430);
      const panelH = Math.min(Math.round(window.innerHeight || 844), 920);
      ...
      let quiltPreparedUnderMat = false;
      let prepareUnderMatInFlight = false;
      try {
        await controller.prepare({ color: selectedColor, appDateKey, clientId });

        // Quilt is live under the opaque mat for the whole animation so handoff
        // is CSS fade only — no showScreen on the last scrap.
        ...
        const prepareQuiltUnderMat = async () => {
          if (handoffFired || quiltPreparedUnderMat || prepareUnderMatInFlight) return;
          if (!serverResult?.success) return;
          prepareUnderMatInFlight = true;
          try {
            await startApplyIfNeeded();
            if (handoffFired) return;
            await this.renderQuilt({ force: true, viewportOnly: true });
            quiltPreparedUnderMat = true;
          } catch (prepareErr) {
            this.logger.warn('Prepare quilt under cut mat failed:', prepareErr);
          } finally {
            prepareUnderMatInFlight = false;
          }
        };
        ...
        const fireQuiltHandoff = () => {
          ...
          // Fade only — no showScreen / apply / chrome in this turn.
        ...
        try {
          playResult = await controller.playWithSubmission(submitPromise, {
            ...
            onAfterCut: async (info) => {
              // After mocks: start hydration without blocking the first real cut.
              // After later cuts: await paint under the mat between scrap pulls.
              if (info?.phase === 'after-mocks') {
                startApplyIfNeeded();
                return;
              }
              await prepareQuiltUnderMat();
            }
          });
        } catch (playErr) {
