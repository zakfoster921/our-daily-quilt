  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    syncContributorCloudDebug(panelBody, listEl, metrics = {}) {
      ...
      if (!layer) {
        ...
        layer.innerHTML = [
          '<div class="quilt-contributor-debug-line quilt-contributor-debug-line--bottom-limit" data-debug-label="bottom guard (-5px)"></div>',
      ...
      const bottomLimit = Number(metrics.bottomLimit) || (listRect.bottom - 5);
      ...
      const overflowing = names.filter((el) => el.getBoundingClientRect().bottom > bottomLimit);
      names.forEach((el) => {
    ...
    fitContributorCloudToViewport(listEl) {
      ...
      const maxHeight = Math.floor(panelBody.clientHeight);
      if (!maxHeight) return;

      const minScale = 0.28;
      listEl.style.setProperty('--contributor-cloud-base-size', '1');

      const measure = () => Math.ceil(panelBody.scrollHeight);
      let scale = 1;
      let height = measure();

      if (height > maxHeight) {
        scale = Math.max(minScale, (maxHeight / height) * 0.98);
        listEl.style.setProperty('--contributor-cloud-base-size', String(scale));
        height = measure();
        while (height > maxHeight && scale > minScale) {
      ...
      let guard = 0;
      let bottomLimit = listEl.getBoundingClientRect().bottom - 5;
      while (guard < 16) {
        bottomLimit = listEl.getBoundingClientRect().bottom - 5;
        const overflows = Array.from(listEl.querySelectorAll('.quilt-contributor-name')).some(
          (el) => el.getBoundingClientRect().bottom > bottomLimit
        );
        if (!overflows || scale <= minScale) break;
        scale = Math.max(minScale, scale * 0.995);
        listEl.style.setProperty('--contributor-cloud-base-size', String(scale));
        guard += 1;
      }
