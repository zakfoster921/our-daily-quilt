...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    fitContributorCloudToViewport(listEl) {
      ...
      const contentFits = (scale) => {
        listEl.style.setProperty('--contributor-cloud-base-size', String(scale));
        const box = panelBody.getBoundingClientRect();
        const left = box.left + padH;
        const right = box.right - padH;
        const top = box.top + padV;
        const bottom = box.bottom - padV;
        const nodes = [heading, ...listEl.querySelectorAll('.quilt-contributor-name')].filter(Boolean);
      ...
      if (!contentFits(1)) {
        ...
        listEl.style.setProperty('--contributor-cloud-base-size', String(best));
      }

      let lo = Number(listEl.style.getPropertyValue('--contributor-cloud-base-size')) || 1;
      let hi = maxScale;
      let best = lo;
      for (let i = 0; i < 18; i += 1) {
        const mid = (lo + hi) / 2;
