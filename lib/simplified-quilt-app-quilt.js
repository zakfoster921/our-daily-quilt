...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    async fetchNewspaperClippingUrl(dateKey, { forceRefresh = false, waitForFirebaseMs = 5000 } = {}) {
      const dk = String(dateKey || this.quoteService?.getQuoteCalendarKeyNow?.() || '').trim();
      if (!dk) {
        console.warn('[our-daily] fetchNewspaperClippingUrl: missing dateKey');
        return null;
      }
      if (!window.db || !window.firestore) {
      ...
      if (!window.db || !window.firestore) {
        console.warn('[our-daily] fetchNewspaperClippingUrl: Firebase not ready');
        return null;
      }
      ...
      try {
        ...
        if (!firestoreSnapExists(snap)) {
          console.warn(`[our-daily] fetchNewspaperClippingUrl: no instagram-images/${dk} doc`);
          return null;
        }
        const data = snap.data() || {};
        const stamp = newspaperClippingCacheStamp(data);
        const cacheKey = stamp ? `${dk}:${stamp}` : dk;
        if (!forceRefresh && this._newspaperClippingUrlCache[cacheKey]) {
          return this._newspaperClippingUrlCache[cacheKey];
        }
        const url = readNewspaperClippingUrlFromDoc(data);
        if (url) this._newspaperClippingUrlCache[cacheKey] = url;
        if (!url) {
          console.warn(
            `[our-daily] fetchNewspaperClippingUrl: doc exists but no newspaperClippingUrl on instagram-images/${dk}`
          );
        }
        return url || null;
      } catch (e) {
        console.warn('[our-daily] fetchNewspaperClippingUrl failed:', e?.message || e);
        return null;
      }
    }

    /** Swap triptych quote from client data: URL to nightly Storage when Firestore becomes available. */
    async _upgradeTriptychQuoteToNightlyClipping(dk, goodDay, roughDay) {
      ...
      const storageUrl = await this.fetchNewspaperClippingUrl(dk, {
        forceRefresh: true,
        waitForFirebaseMs: 8000
      });
      if (!storageUrl || storageUrl.startsWith('data:')) return false;
      img.style.width = '100%';
      img.style.maxWidth = '100%';
      img.style.height = 'auto';
      img.src = storageUrl;
      spreadHost._moodTriptychWidget.remeasure?.();
      console.info('[our-daily] Triptych quote upgraded to nightly Storage PNG');
      return true;
    }
    ...
    _applyNewspaperClippingDisplaySize(img, layout) {
      if (!img) return;
      const w = Number(layout?.displayWidthPx);
      if (w > 0) {
    ...
    async _composeQuiltNewspaperClippingLayout(dateKey, todayQ) {
      const QNC = globalThis.QuiltNewspaperClipping;
    ...
    async applyQuiltNewspaperClipping({ dateKey = null, quote = null } = {}) {
      ...
      if (url) {
        this.logger.warn(
          'Newspaper clipping Storage URL failed; composing in browser as fallback'
        );
      }
      ...
      entries.forEach(({ kind, kicker, text }) => {
        ...
        if (kind === 'color') {
          ...
          if (text) {
            ...
            const parts = this._getRememberTodayCompanionParts(
              dk === this._rememberTodayDateKey() ? this._getRememberTodayQuote() : null
            );
