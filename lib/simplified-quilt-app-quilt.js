...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    _storedLayoutBStoryPreviewIsStale(data, dateKey) {
      ...
      const effectiveKey =
        typeof this.getEffectiveAppDateKey === 'function'
          ? String(this.getEffectiveAppDateKey() || '')
          : '';
      if (
        effectiveKey &&
        dk === effectiveKey &&
        Array.isArray(liveBlocks) &&
        liveBlocks.length > 1
      ) {
        return true;
      }
    ...
    async _tryStoredLayoutBStoryPreview(dateKey, previewContentKey = '', options = {}) {
      ...
      if (this._storedLayoutBStoryPreviewIsStale(data, dk)) {
        return false;
      }
    ...
    async _refreshLayoutBStoryPreviewCore() {
      ...
      try {
        ...
        const { qt } = await this.resolveLayoutBStoryQuoteAndComposeOptions(dk, blocks, 'story');
        const blob = await arch.generateInstagramStoryLayoutBBlob(blocks, qt, dk, { forPreview: true });
