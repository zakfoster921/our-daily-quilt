...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    async _tryStoredLayoutBStoryPreview(dateKey, previewContentKey = '', options = {}) {
      ...
      const preferLayoutBPostPlaceholder = options.preferLayoutBPostPlaceholder === true;
      let placeholderOnly = options.forcePlaceholderOnly === true;
      const clearUnsafePlaceholder = () => {
        if (!preferQuiltScreenPlaceholder && !preferLayoutBPostPlaceholder) return;
        this._layoutBStoryPreviewShareBlob = null;
        img.classList.remove('quilt-layout-b-preview-img--contain');
        img.removeAttribute('src');
        img.hidden = true;
      };
      ...
      const layoutBPostUrl = String(
        data?.postLayoutBImageStorageUrl ||
          data?.layoutBUrl ||
          data?.layoutBPlainUrl ||
          data?.carouselSlide1ImageStorageUrl ||
          data?.carouselSlide1Url ||
          data?.classicUrl ||
          ''
      ).trim();
      const url = preferLayoutBPostPlaceholder
        ? layoutBPostUrl || quiltScreenUrl
      ...
      img.classList.toggle('quilt-layout-b-preview-img--contain', preferLayoutBPostPlaceholder);
    ...
    async _refreshLayoutBStoryPreviewCore() {
      ...
      const currentFp =
        typeof Utils.computeQuiltFingerprint === 'function' && Array.isArray(blocks)
          ? Utils.computeQuiltFingerprint(blocks)
          : String(blockCount);
      const storedPreviewKey = `stored:${dk}|${currentFp}`;
      ...
      if (!this.isAdminTomorrowPreviewActive?.()) {
        ...
        const finalAllowed = !hasRepeatedLayoutBKeyword;
        const usedStoredPreview = await this._tryStoredLayoutBStoryPreview(dk, storedPreviewKey, {
          currentFingerprint: currentFp,
          currentBlockCount: blockCount,
          allowStalePlaceholder: true,
          finalAllowed,
          forcePlaceholderOnly: hasRepeatedLayoutBKeyword,
          preferLayoutBPostPlaceholder: hasRepeatedLayoutBKeyword
        });
