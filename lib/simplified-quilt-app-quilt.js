  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    _buildDedicatedBlockPreviewSvg(block, userColor = '') {
      ...
      if (block.specialPatternType === 'hst') {
        ...
        if (!picked.length) {
          picked = tris.filter((tri) => colorsMatch(tri.color, block.hstColorB));
        }
        ...
        shapes.push({
          ...
          fill: block.insetInnerColor || block.color || targetColor
        });
        ...
        if (raw.length >= 3) shapes.push({ points: raw, fill: block.color || targetColor });
      }

      if (!shapes.length) {
        shapes.push({ rect: { x: bx, y: by, width: bw, height: bh }, fill: block.color || targetColor });
      }
    ...
    _computeColorSubmitFlyTransform(fromRect, toRect) {
      ...
      const translateX = toCx - fromCx;
      const translateY = toCy - fromCy;
      return {
        fromCx,
        fromCy,
        translateX,
        translateY,
        endScale,
        midScale: 1 + (endScale - 1) * 0.58,
        midTranslateX: translateX * 0.56,
        midTranslateY: translateY * 0.5
      };
    }

    _colorSubmitFlyTransformAt(motion, phase = 'start') {
      if (phase === 'end') {
        return `translate3d(-50%, -50%, 0) translate3d(${motion.translateX}px, ${motion.translateY}px, 0) scale(${motion.endScale})`;
      }
      if (phase === 'mid') {
        return `translate3d(-50%, -50%, 0) translate3d(${motion.midTranslateX}px, ${motion.midTranslateY}px, 0) scale(${motion.midScale})`;
      }
      return 'translate3d(-50%, -50%, 0) translate3d(0, 0, 0) scale(1)';
    }
    ...
    async playColorSubmitFlyToQuilt(block, color = '') {
      ...
      await this.waitForNextPaint();
      await this.waitForNextPaint();
      toRect = targetGroup.getBoundingClientRect();
      ...
      flyer.style.transform = this._colorSubmitFlyTransformAt(motion, 'start');
      ...
      const duration = 980;
      const easing = 'cubic-bezier(0.22, 0.98, 0.36, 1)';
      ...
      await new Promise((resolve) => {
        ...
        const runCssFallback = () => {
          ...
          flyer.style.transform = this._colorSubmitFlyTransformAt(motion, 'end');
          overlay.style.transition = `opacity ${Math.round(duration * 0.72)}ms ease-out`;
          overlay.style.transitionDelay = `${Math.round(duration * 0.24)}ms`;
          requestAnimationFrame(() => {
          ...
          requestAnimationFrame(() => {
            requestAnimationFrame(() => {
              if (typeof flyer.animate !== 'function') {
                runCssFallback();
                window.setTimeout(done, duration + 280);
                return;
              }

              const flight = flyer.animate(
                [
                  { transform: this._colorSubmitFlyTransformAt(motion, 'start') },
                  {
                    transform: this._colorSubmitFlyTransformAt(motion, 'mid'),
                    offset: 0.54
                  },
                  { transform: this._colorSubmitFlyTransformAt(motion, 'end') }
                ],
                { duration, easing, fill: 'forwards' }
              );
              overlay.animate([{ opacity: 1 }, { opacity: 0 }], {
                duration: Math.round(duration * 0.72),
                delay: Math.round(duration * 0.24),
                easing: 'ease-out',
                fill: 'forwards'
              });
              ...
              window.setTimeout(done, duration + 360);
            });
          });
        };
