...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    async saveAdminQuiltMutationViaServer(label) {
      ...
      await this.refreshStoredStorySnapshotAfterAdminServerMutation?.({
    ...
    async refreshStoredStorySnapshotAfterAdminServerMutation(options = {}) {
      try {
        const dateKey = String(options.dateKey || Utils.getTodayKey()).trim();
        const latest =
          this.dataService && typeof this.dataService.loadQuiltFromServer === 'function'
            ? await this.dataService.loadQuiltFromServer(dateKey)
            : null;
        if (latest?.ok && latest.data) {
          ...
          this.logger?.warn?.('Admin story snapshot refresh could not load latest quilt:', latest?.reason || latest?.error);
        }
        await this.refreshStoredStorySnapshotAfterServerColorSubmission({ dateKey });
      } catch (error) {
        this.logger?.warn?.('Stored story snapshot refresh after admin mutation failed:', error);
      }
    }
