  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    async runPostColorCutRevealSubmit(selectedColor) {
      ...
      try {
        ...
        const submitPromise = this.submitColorToServer(selectedColor).then(async (serverResult) => {
          applyResult = await this.applyServerColorSubmissionResult(serverResult, selectedColor);
          return serverResult;
        });
        const playResult = await controller.playWithSubmission(submitPromise, {
          color: selectedColor,
          appDateKey,
          clientId
        });
        ...
        const serverResult = playResult?.livePreview?.submission;
    ...
    async handleAddColor() {
      ...
      try {
        ...
        const useCutReveal = this.isPostColorCutRevealEnabled();
        this.hideColorSubmitTransition();
        ...
        if (useCutReveal) {
          try {
            ...
            this.logger.warn(
              'Post-color cut reveal failed; falling back to classic submit overlay:',
              revealErr
            );
        ...
        if (!usedServerSubmission && !cutRevealFinished) {
