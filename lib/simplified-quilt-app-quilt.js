...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    _clippingLayoutOptsForImage(img, layout = {}) {
      ...
      if (img?.closest?.('.quilt-mood-triptych__quote, .quilt-mood-collage__quote, #quiltMoodSpread')) {
    ...
    async _applyQuoteScreenClippingWork({ dateKey = null, quote = null, forceRefresh = false } = {}) {
      ...
      const skipStoragePng = skipForStaleExport;
      if (skipForStaleExport) {
        console.info(
          `[our-daily] Quote clipping: Storage rev ${clippingMeta.exportRev || '?'} < bundled ${globalThis.QuiltNewspaperClipping?.CLIPPING_EXPORT_REV}; composing locally`
        );
      }
    ...
    async applyQuiltNewspaperClipping({ dateKey = null, quote = null } = {}) {
      ...
      const clippingMeta = dk
        ...
        : { exportRev: '' };
      const skipStoragePng = isStoredClippingExportStale(clippingMeta.exportRev);
      if (skipStoragePng) {
        console.info(
          `[our-daily] Quilt quote clipping: Storage rev ${clippingMeta.exportRev || '?'} < bundled ${globalThis.QuiltNewspaperClipping?.CLIPPING_EXPORT_REV}; composing locally`
        );
      }
