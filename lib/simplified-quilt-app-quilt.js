...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    _colorSubmitFlyTransformAt(motion, phase = 'start') {
      if (phase === 'end') {
        return `translate(-50%, -50%) translate(${motion.translateX}px, ${motion.translateY}px) scale(${motion.scaleX}, ${motion.scaleY})`;
      }
      return 'translate(-50%, -50%) translate(0, 0) scale(1, 1)';
    }
    ...
    async playColorSubmitFlyToQuilt(block, color = '') {
      ...
      const duration = 920;
      const easing = 'cubic-bezier(0.25, 0.92, 0.35, 1)';
      ...
      await new Promise((resolve) => {
        ...
        const runCssFlight = () => {
          ...
          overlay.style.transitionDelay = `${Math.round(duration * 0.28)}ms`;
          requestAnimationFrame(() => {
            requestAnimationFrame(() => {
