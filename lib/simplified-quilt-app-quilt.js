  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    setupQuiltResumeRefresh() {
      ...
      if (native) {
        ...
        // Mirror tune is admin-only and doesn't change blocks — Firestore listeners are
        // unreliable in iOS WebView, so poll Railway for fresh mirror fields.
        this._nativeMirrorTunePollTimer = setInterval(() => {
          if (!this._liveDailyDataConfirmed || this._isPersonalQuiltMode) return;
          if (!this._isAppForegroundForQuiltSync()) return;
          void this.syncMirrorTuneFromServer?.();
        }, 20000);
      }
    }
