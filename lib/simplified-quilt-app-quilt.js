...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    fitContributorCloudToViewport(listEl) {
      ...
      const useCompact = fillRatio < 0.72;
      const finalProbe = measureAtScale(bestScale, useCompact);
      if (!useCompact && finalProbe.overflows) {
        let guard = 0;
        let scale = bestScale;
        while (scale > minScale && guard < 24) {
          scale = Math.max(minScale, scale * 0.97);
          const retry = measureAtScale(scale, false);
          if (!retry.overflows) {
            bestScale = scale;
            break;
          }
        ...
        measureAtScale(bestScale, false);
      } else if (useCompact) {
    ...
    renderQuiltContributorList() {
      ...
      if (show) {
        requestAnimationFrame(() => {
          this.fitContributorCloudToViewport(list);
        });
      } else {
