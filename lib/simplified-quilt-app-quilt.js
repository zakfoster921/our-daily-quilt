...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    fitContributorCloudToViewport(listEl) {
      ...
      const nameOverflowsList = () => {
        const bounds = getFitBounds();
        return Array.from(listEl.querySelectorAll('.quilt-contributor-name')).some((el) => {
      ...
      const panelOverflows = () => Math.ceil(panelBody.scrollHeight) > maxPanelHeight;
      const needsShrink = () => panelOverflows() || nameOverflowsList();

      let scale = 1;
      if (needsShrink()) {
        const ratio = maxPanelHeight / Math.max(1, Math.ceil(panelBody.scrollHeight));
