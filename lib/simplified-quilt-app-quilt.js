    ...
    async runPostColorCutRevealSubmit(selectedColor) {
      ...
      try {
        ...
        const fireQuiltHandoff = () => {
          ...
          // Fast path: no admin pill / footer paper on screenChange.
          ...
          // Start the fade in this same turn — do not wait another paint on the last cut.
          if (quiltHasPaint && host) {
          ...
          quiltHandoffPromise = (async () => {
            const fadeMatAway = async () => {
              if (host?.isConnected) {
                if (!host.classList.contains('post-color-cut-reveal-host--handoff')) {
                  host.classList.add('post-color-cut-reveal-host--handoff');
                }
                await this.wait(280);
              }
