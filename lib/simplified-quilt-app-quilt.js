...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    getColorSubmitThankYouMessage() {
      return 'Thank you for this color';
    }

    getColorSubmitFindingSpotMessage() {
      return 'Finding a place for your shape';
    }

    getColorSubmitPlacementMessage() {
      return this.getColorSubmitFindingSpotMessage();
    }
    ...
    showColorSubmitThankYou(color) {
      ...
      overlay.classList.remove(
        'color-submit-transition--finding',
        'color-submit-transition--thank-you',
        ...
        'color-submit-transition--reveal-quilt'
      );
      ...
      if (pieceFrame) pieceFrame.innerHTML = '';
      if (colorSheet) {
        colorSheet.style.backgroundColor = color;
        colorSheet.style.opacity = '';
      }
      overlay.style.setProperty('--submit-color', color);
      overlay.style.backgroundColor = color;
      overlay.style.color = this.getReadableTextColorForBackground(color);
      overlay.classList.add('color-submit-transition--thank-you', 'color-submit-transition--message-visible');
      if (messageEl) {
        messageEl.textContent = this.getColorSubmitThankYouMessage();
    ...
    showColorSubmitFindingStage(color, block) {
      if (!block) return;
      let overlay = document.getElementById('colorSubmitTransition');
      if (!overlay) {
        this.showColorSubmitThankYou(color);
        overlay = document.getElementById('colorSubmitTransition');
      }
      this._ensureColorSubmitOverlayStructure(overlay);
      const pieceStage = overlay.querySelector('.color-submit-transition__piece-stage');
      const pieceFrame = overlay.querySelector('.color-submit-transition__piece-frame');
      const colorSheet = overlay.querySelector('.color-submit-transition__color-sheet');
      const messageEl = overlay.querySelector('.color-submit-transition__message');
      if (!pieceStage || !pieceFrame) return;
      const previewBlock = this._freshDedicatedBlockForPreview(block) || this._normalizeColorSubmitPreviewBlock(block);
      const pieceNode = this._createColorSubmitPieceNode(previewBlock, color);
      if (!pieceNode) return;
      overlay.classList.remove('color-submit-transition--thank-you');
      overlay.classList.add(
        'color-submit-transition--finding',
        'color-submit-transition--message-visible',
        'color-submit-transition--shrink-settled'
      );
      overlay.style.setProperty('--color-submit-shrink-from', '1');
      overlay.style.setProperty('--submit-color', color);
      overlay.style.backgroundColor = '#f6f4f1';
      overlay.style.color = '#2f2925';
      if (colorSheet) {
        colorSheet.style.backgroundColor = color;
        colorSheet.style.opacity = '0';
      }
      pieceFrame.innerHTML = '';
      pieceFrame.appendChild(pieceNode);
      pieceStage.hidden = false;
      if (messageEl) {
        messageEl.textContent = this.getColorSubmitFindingSpotMessage();
        messageEl.hidden = false;
      }
      overlay.classList.add('color-submit-transition--visible');
    }

    showColorSubmitTransition(color, message = this.getColorSubmitPlacementMessage(), previewBlock = null) {
      let overlay = document.getElementById('colorSubmitTransition');
      if (!overlay) {
        overlay = document.createElement('div');
        overlay.id = 'colorSubmitTransition';
      ...
      const showFinding = !!trimmedMessage;
      const previewPiece = previewBlock
        ? this._createColorSubmitPieceNode(this._normalizeColorSubmitPreviewBlock(previewBlock), color)
        : null;
      const showPiece = !!previewPiece;
      if (pieceStage) pieceStage.hidden = !showPiece;
      if (pieceFrame) {
        pieceFrame.innerHTML = '';
        if (colorSheet) colorSheet.style.opacity = showFinding ? '0' : '';
      }
      ...
      if (showFinding) {
        ...
        overlay.style.backgroundColor = '#f6f4f1';
        overlay.style.color = '#2f2925';
        if (pieceFrame && previewPiece) pieceFrame.appendChild(previewPiece);
      } else {
        overlay.style.backgroundColor = color;
        overlay.style.color = this.getReadableTextColorForBackground(color);
      }
    ...
    updateColorSubmitTransitionPiece(block, color) {
      if (!block) return;
      const overlay = document.getElementById('colorSubmitTransition');
      if (!overlay) return;
      this._ensureColorSubmitOverlayStructure(overlay);
    ...
    _previewDedicatedBlockForColor(selectedColor) {
      if (!this.quiltEngine || typeof this.quiltEngine.addColor !== 'function') return null;
      if (typeof SimpleQuiltEngine === 'undefined') return null;
      try {
        const probe = new SimpleQuiltEngine(this.quiltEngine.deviceId, {
          recordColorReplayEvents: false,
          quiltDateKey: this.quiltEngine.quiltDateKey || Utils.getTodayKey()
        });
        probe.blocks = JSON.parse(JSON.stringify(this.quiltEngine.blocks || []));
        probe.submissionCount = Number(this.quiltEngine.submissionCount) || 0;
        probe.macroStructureFrozen = this.quiltEngine.macroStructureFrozen === true;
        probe.macroLayoutMode = this.quiltEngine.macroLayoutMode;
        probe.dailyMoodColorHex =
          (typeof this._getTodayQuoteMoodHex === 'function' && this._getTodayQuoteMoodHex()) ||
          this.quiltEngine.dailyMoodColorHex ||
          null;
        probe.recordUserContribution = () => {};
        const result = probe.addColor(selectedColor);
        const dedicatedId = String(result?.dedicatedBlockId || '').trim();
        if (!dedicatedId) return null;
        const dedicatedBlock = probe.blocks.find((block) => String(block?.id || '') === dedicatedId);
        return dedicatedBlock ? JSON.parse(JSON.stringify(dedicatedBlock)) : null;
      } catch (previewErr) {
        this.logger.warn('Color placement preview failed:', previewErr);
        return null;
      }
    }
    ...
    async _holdColorSubmitFindingBeat(ms = 550) {
    ...
    async playColorSubmitPieceReveal(block, color = '') {
      ...
      const alreadyFinding = overlay.classList.contains('color-submit-transition--finding');
      if (alreadyFinding) {
        pieceFrame.innerHTML = '';
        pieceFrame.appendChild(pieceNode);
        pieceStage.hidden = false;
    ...
    async playColorSubmitRevealQuilt(blockId = '') {
      const overlay = document.getElementById('colorSubmitTransition');
      const reduceMotion = window.matchMedia?.('(prefers-reduced-motion: reduce)')?.matches;
      if (!overlay) return;

      const id = String(blockId || '').trim();
      if (id && typeof this.findQuiltBlockGroup === 'function') {
        const target = this.findQuiltBlockGroup(id);
        if (target?.group) this._scrollQuiltBlockIntoView(target.group);
      }

      await this.waitForNextPaint();
      overlay.classList.add('color-submit-transition--reveal-quilt', 'color-submit-transition--piece-exit');
      await this.wait(reduceMotion ? 150 : 360);
    ...
    async handleAddColor() {
      try {
        ...
        const thankYouMinMs = 450;
        const findingBeatMs = 550;
        this.showColorSubmitThankYou(selectedColor);
        await this.waitForNextPaint();
        ...
        try {
          ...
          if (serverDedicatedBlock) {
            this.showColorSubmitFindingStage(selectedColor, serverDedicatedBlock);
            await this._holdColorSubmitFindingBeat(findingBeatMs);
          }
          this.scheduleStoredStorySnapshotRefreshAfterQuiltUpdate?.({
          ...
          const fallbackDedicatedBlock = this._findDedicatedBlockInEngine(
            result.dedicatedBlockId || this._latestDedicatedBlockId,
            result.submissionIndex
          );
          if (fallbackDedicatedBlock) {
            this.showColorSubmitFindingStage(selectedColor, fallbackDedicatedBlock);
            await this._holdColorSubmitFindingBeat(findingBeatMs);
          }
        }
        ...
        if (result) {
          ...
          if (dedicatedBlock?.id && typeof this.findQuiltBlockGroup === 'function') {
            const target = this.findQuiltBlockGroup(dedicatedBlock.id);
            if (target?.group) this._scrollQuiltBlockIntoView(target.group);
          }
          this.hideColorSubmitTransition();
          await this.waitForNextPaint();
