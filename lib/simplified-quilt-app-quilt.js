...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    _storedLayoutBStoryPreviewIsStale(data, dateKey) {
      const dk = String(dateKey || '').trim();
      if (!dk || !data) return true;
      const liveBlocks = this.quiltEngine?.blocks;
      const liveBlockCount = Array.isArray(liveBlocks) ? liveBlocks.length : 0;
      const storedBlockCount = Number(data.blockCount);
      if (Number.isFinite(storedBlockCount) && storedBlockCount > 0 && liveBlockCount !== storedBlockCount) {
        return true;
      }
      if (typeof globalThis.odqMirrorTuneIsCustomized === 'function') {
        const tune =
          typeof globalThis.odqReadMirrorTuneFromLocal === 'function'
            ? globalThis.odqReadMirrorTuneFromLocal(dk)
            : null;
        if (globalThis.odqMirrorTuneIsCustomized(tune)) return true;
      }
      return false;
    }
    ...
    async _tryStoredLayoutBStoryPreview(dateKey, previewContentKey = '', options = {}) {
      ...
      if (options.ignoreStale !== true && this._storedLayoutBStoryPreviewIsStale(data, dk)) {
        return false;
      }
    ...
    async _refreshLayoutBStoryPreviewCore() {
      ...
      const applyLivePreviewBlob = async (blob, contentKey) => {
      ...
      if (hint) {
        hint.hidden = false;
        hint.textContent = 'Building story preview…';
      }
      ...
      const mirrorSig =
        typeof globalThis.odqMirrorTuneExportSignature === 'function'
          ...
          : '';
      const contentKey = `live:${dk}:${fingerprint}:${mirrorSig}`;
      ...
      try {
        ...
        await this._waitForQuiltSvgReadyForStoryPreview(blocks);
