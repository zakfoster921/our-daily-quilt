...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    _isRectNearViewport(rect, margin = 120) {
    ...
    _getColorSubmitFlyAnchorRect(block) {
      ...
      if (newBlockGroup) {
        ...
        if (this._isRectNearViewport(rect)) return rect;
      }
    ...
    _resolveColorSubmitFlyTarget(block, userColor = '') {
      ...
      if ((!marker || !this._isRectNearViewport(rect)) && typeof this.findQuiltBlockGroup === 'function') {
        const groupTarget = this.findQuiltBlockGroup(blockId);
        if (groupTarget?.group) {
          ...
          if (!marker || !this._isRectNearViewport(rect)) {
      ...
      if (!rect || !this._isRectNearViewport(rect)) return null;
    ...
    async _waitForQuiltBlockScreenRect(blockId, userColor = '', block = null, attempts = 30) {
      ...
      for (let attempt = 0; attempt < attempts; attempt += 1) {
        ...
        if (resolved?.rect) {
          if (resolved.group) this._scrollQuiltBlockIntoView(resolved.group);
          ...
          if (resolved.rect?.width > 2 && resolved.rect?.height > 2 && this._isRectNearViewport(resolved.rect)) {
            return resolved;
          }
        }
        await this.wait(50);
    ...
    _computeColorSubmitFlyTransform(fromRect, toRect) {
    ...
    async playColorSubmitFlyToQuilt(block, color = '') {
      ...
      const mountFlyer = (startRect) => {
        const motion = this._computeColorSubmitFlyTransform(startRect, toRect);
        flyer.style.left = `${motion.fromCx}px`;
        flyer.style.top = `${motion.fromCy}px`;
        flyer.style.width = `${startRect.width}px`;
        flyer.style.height = `${startRect.height}px`;
        flyer.style.transform = 'translate(-50%, -50%) scale(1, 1)';
        flyer.style.transition = 'none';
        flyer.dataset.flyTransform = `translate(-50%, -50%) translate(${motion.translateX}px, ${motion.translateY}px) scale(${motion.scaleX}, ${motion.scaleY})`;
        return motion;
      };
      ...
      await new Promise((resolve) => {
        ...
        const startFlight = () => {
          ...
          if (refreshed?.rect && this._isRectNearViewport(refreshed.rect)) {
            ...
            if (groupTarget?.group) {
              ...
              if (this._isRectNearViewport(toRect) && toRect.width > 2 && toRect.height > 2) {
                mountFlyer(fromRect);
              }
            }
          }

          const finalTransform = flyer.dataset.flyTransform;
          flyer.style.willChange = 'transform';
          flyer.style.transition = `transform ${duration}ms ${easing}`;
          overlay.style.transition = `opacity ${Math.round(duration * 0.94)}ms ease-out`;
          requestAnimationFrame(() => {
            requestAnimationFrame(() => {
              if (finalTransform) flyer.style.transform = finalTransform;
