...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    async _tryStoredLayoutBStoryPreview(dateKey, previewContentKey = '', options = {}) {
      ...
      const url = storyUrl || quiltScreenUrl;
      if (!url) return false;
    ...
    scheduleStoredStorySnapshotRefreshAfterQuiltUpdate(options = {}) {
      ...
      if (!dateKey) return;
      const quietMs = Math.max(4000, Number(options.quietMs) || 9000);
      const firstDelayMs = Math.max(2000, Number(options.delayMs) || 9000);
      if (!this._storySnapshotRefreshInteractionTrackingReady && typeof window !== 'undefined') {
        this._storySnapshotRefreshInteractionTrackingReady = true;
        this._storySnapshotRefreshLastInteractionAt = Date.now();
      ...
      const scheduleAttempt = (delayMs) => {
        this._storySnapshotRefreshScheduledTimer = setTimeout(() => {
          ...
          const queuedDateKey = this._storySnapshotRefreshScheduledDateKey || dateKey;
          const run = () => {
            const lastInteractionAt = Number(this._storySnapshotRefreshLastInteractionAt) || 0;
            ...
            void this.refreshStoredStorySnapshotAfterServerColorSubmission({ dateKey: queuedDateKey });
          };
