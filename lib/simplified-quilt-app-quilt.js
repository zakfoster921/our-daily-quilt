...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    async _waitForQuiltSvgReadyForStoryPreview(blocks, maxMs = 10000) {
    ...
    async _refreshLayoutBStoryPreviewCore() {
      ...
      const applyLivePreviewBlob = async (blob, contentKey) => {
        if (!blob) throw new Error('Story preview blob empty');
        this._revokeLayoutBPreviewObjectUrl();
        const url = URL.createObjectURL(blob);
        ...
        if (!(img.naturalWidth > 0)) throw new Error('Story preview image failed to decode');
        wrap.setAttribute('aria-busy', 'false');
        this._layoutBStoryPreviewHeavyDoneThisVisit = true;
      ...
      try {
        ...
        const blob =
          typeof globalThis.odqPromiseWithTimeout === 'function'
            ? await globalThis.odqPromiseWithTimeout(previewPromise, 45000, 'Layout B story preview')
