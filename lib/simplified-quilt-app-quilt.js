  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    async runPostColorCutRevealSubmit(selectedColor) {
      ...
      try {
        ...
        const submitPromise = this.submitColorToServer(selectedColor);
        playResult = await controller.playWithSubmission(submitPromise, {
          color: selectedColor,
          appDateKey,
          clientId,
          onCutsComplete: async (livePreview) => {
            const resolved = this._resolveColorSubmissionFromLivePreview(livePreview);
            if (!resolved?.success) return;
            serverResult = resolved;
            try {
              applyResult = await this.applyServerColorSubmissionResult(resolved, selectedColor);
            } catch (applyErr) {
        ...
        if (!applyResult) {
          try {
            applyResult = await this.applyServerColorSubmissionResult(serverResult, selectedColor);
          } catch (applyErr) {
            this.logger.warn('Apply after cut reveal failed (server saved):', applyErr);
            return {
              playResult,
              serverResult,
              applyResult: null,
        ...
        if (applyResult) {
          try {
            ...
            this.logger.warn('Cut reveal quilt render failed (non-fatal):', renderErr);
          }
        }
        return {
          playResult,
    ...
    async handleAddColor() {
      ...
      let cutRevealQuiltRendered = false;
      ...
      try {
        ...
        if (useCutReveal) {
          try {
            ...
            cutRevealQuiltRendered = revealOutcome.quiltRendered === true;
            usedServerSubmission = true;
            result = revealOutcome.applyResult;
            if (!result && revealOutcome.serverResult) {
              try {
