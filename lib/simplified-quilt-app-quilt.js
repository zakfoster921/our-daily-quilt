  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    async handleAddColor() {
      ...
      try {
        ...
        const minimumTransitionHold = this.wait(600);
        ...
        if (!usedServerSubmission && !useCutReveal) {
          ...
          try {
            // Spinner covers network (+ short min hold) only — not apply/render.
            const [serverResult] = await Promise.all([
              this.submitColorToServer(selectedColor),
              minimumTransitionHold
            ]);
            pendingServerResult = serverResult;
            usedServerSubmission = true;
          } catch (serverError) {
