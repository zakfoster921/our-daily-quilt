...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    fitContributorCloudToViewport(listEl) {
      ...
      const bottomLimit = panelBody.getBoundingClientRect().bottom - bottomPad;
      const names = listEl.querySelectorAll('.quilt-contributor-name');
      const bottomOverflows = () => Array.from(names).some((el) => (
        el.getBoundingClientRect().bottom > bottomLimit
      ));
      while (bottomOverflows() && scale > minScale) {
        scale = Math.max(minScale, scale * 0.985);
        listEl.style.setProperty('--contributor-cloud-base-size', String(scale));
      }
    }
