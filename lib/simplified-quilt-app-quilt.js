...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    async playColorSubmitFlyToQuilt(block, color = '') {
      ...
      const { rect: toRect, group: targetGroup } = target;
      ...
      await new Promise((resolve) => {
        ...
        move.addEventListener('finish', done, { once: true });
        move.addEventListener('cancel', done, { once: true });
        window.setTimeout(done, duration + 140);
      });
    }
