...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    showColorSubmitTransition(color, message = this.getColorSubmitPlacementMessage()) {
      ...
      if (pieceStage) pieceStage.hidden = !showFinding;
      if (pieceFrame) {
      ...
      if (showFinding) {
        ...
        const waitingPiece = this._createColorSubmitWaitingPiece(color);
        if (pieceFrame && waitingPiece) pieceFrame.appendChild(waitingPiece);
      } else {
    ...
    _createColorSubmitWaitingPiece(color) {
      const userColor = this.normalizeHexColor(color) || '#DF9368';
      let partnerColor = '#c8c4bf';
      if (typeof this._getTodayQuoteMoodHex === 'function') {
        const mood = this.normalizeHexColor(this._getTodayQuoteMoodHex());
        if (mood) partnerColor = mood;
      }
      const waitingBlock = {
        id: 'color-submit-waiting',
        x: 0,
        y: 0,
        width: 100,
        height: 100,
        color: userColor,
        contributorColor: userColor,
        hstColorB: partnerColor,
        hstDiagonal: 'nw-se',
        patternType: 'special',
        specialPatternType: 'hst'
      };
      return (
        this._buildDedicatedBlockPreviewSvg(waitingBlock, userColor) ||
        this._createColorSubmitFallbackPiece(
          { id: 'color-submit-waiting-rect', x: 0, y: 0, width: 100, height: 100 },
          userColor
        )
      );
    }
