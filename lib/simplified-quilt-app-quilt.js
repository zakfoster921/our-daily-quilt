...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    getContributorNameGlimmerEffectId() {
      return SimplifiedQuiltAppV2Quilt.CONTRIBUTOR_NAME_GLIMMER_EFFECT_ID;
    }
    ...
    async fetchNewspaperClippingUrl(dateKey, { forceRefresh = false } = {}) {
      ...
      if (forceRefresh) delete this._newspaperClippingUrlCache[dk];
      if (this._newspaperClippingUrlCache[dk]) return this._newspaperClippingUrlCache[dk];
      try {
        const ref = window.firestore.doc(window.db, 'instagram-images', dk);
        const snap = await window.firestore.getDoc(ref);
        const data = snap.exists() ? snap.data() || {} : {};
        ...
        if (url) this._newspaperClippingUrlCache[dk] = url;
        return url || null;
      } catch (e) {
    ...
    async applyQuoteScreenClipping({ dateKey = null, quote = null, forceRefresh = false } = {}) {
      ...
      const loadClippingSrc = (src) =>
        new Promise((resolve) => {
          ...
          if (!forceRefresh && img.src === src && img.naturalWidth > 0) {
            showPng();
            resolve(true);
            return;
          }
