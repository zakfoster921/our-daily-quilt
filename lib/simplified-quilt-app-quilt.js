  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    async handleAddColor() {
      try {
        ...
        const previewBlock = this._previewDedicatedBlockForColor(selectedColor);
        this.showColorSubmitTransition(
          selectedColor,
          this.getColorSubmitPlacementMessage(),
          previewBlock
        );
        /* Hold long enough to read the message; shape may already be visible during the server round-trip. */
        const minimumTransitionHold = this.wait(650);
        await this.waitForNextPaint();
        ...
        try {
          ...
          if (serverDedicatedBlock) {
            this.updateColorSubmitTransitionPiece(serverDedicatedBlock, selectedColor);
          }
          await minimumTransitionHold;
          this.scheduleStoredStorySnapshotRefreshAfterQuiltUpdate?.({
          ...
          if (fallbackDedicatedBlock) {
            this.updateColorSubmitTransitionPiece(fallbackDedicatedBlock, selectedColor);
          }
          await minimumTransitionHold;
        }
        ...
        if (result) {
          ...
          const revealBlockId =
            dedicatedBlock?.id ||
            result.dedicatedBlockId ||
            this._latestDedicatedBlockId ||
            '';
          await this.playColorSubmitRevealQuilt(revealBlockId);
          await this.waitForNextPaint();
          if (this.shouldDeferQuiltScrollHintsForCoach?.()) {
            this.scheduleBlockSpotlightCoach?.();
          } else {
