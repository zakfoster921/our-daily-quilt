...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    getQuiltMoodTriptychFills(blocks = this.quiltEngine?.blocks || []) {
      ...
      const baseHex = userHex || candidates[0] || DEFAULT_BASE;
      ...
      if (blockCount > 1 && this._layoutBStoryPreviewHeavyDoneThisVisit) {
        ...
        const quote =
          ...
          typeof this.quoteService.getQuoteResolvedForInstagramDateKey === 'function'
            ? await this.quoteService.getQuoteResolvedForInstagramDateKey(calendarKey)
            : null) ||
          this.quoteService?.getTodayQuote?.() ||
          {};
        const speakerSig = await this._layoutBStoryPreviewSpeakerSig(quote);
        const nextKey = this._layoutBStoryPreviewContentKeyFor(blocks, calendarKey, speakerSig);
