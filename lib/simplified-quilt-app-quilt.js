  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    fitContributorCloudToViewport(listEl) {
      ...
      const fits = (scale) => {
        ...
        const nodes = [
          heading,
          ...listEl.querySelectorAll('.quilt-contributor-name')
        ].filter(Boolean);
        if (!nodes.length) return true;
        return nodes.every((el) => {
          const rect = el.getBoundingClientRect();
          return rect.left >= box.left + bleed
            && rect.right <= box.right - bleed
            && rect.top >= box.top + bleed
            && rect.bottom <= box.bottom - bleed;
        });
      };
