...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    _findClosestFlyMarkerToEngineRect(block, userColor, engineRect) {
      if (!engineRect) return null;
      ...
      const engineCx = engineRect.left + engineRect.width / 2;
      const engineCy = engineRect.top + engineRect.height / 2;
      const maxDist = Math.max(140, Math.min(window.innerWidth, window.innerHeight) * 0.22);
      ...
      for (const el of searchRoot.querySelectorAll('[data-block-id]')) {
        ...
        if (targetColor && fill && fill !== targetColor) continue;
        const rect = el.getBoundingClientRect();
        if (rect.width < 2 || rect.height < 2) continue;
        const cx = rect.left + rect.width / 2;
        const cy = rect.top + rect.height / 2;
        const dist = Math.hypot(cx - engineCx, cy - engineCy);
        if (dist < bestDist) {
          bestDist = dist;
          best = el;
        }
    ...
    _resolveColorSubmitFlyTarget(block, userColor = '') {
      ...
      const engineRect = this._computeEngineBlockScreenRect(block, userColor);
      let marker = this._findColorSubmitFlyMarker(block, userColor, engineRect);
      if (!marker && engineRect) {
        marker = this._findClosestFlyMarkerToEngineRect(block, userColor, engineRect);
      }
      ...
      return { rect, marker, group, block, engineRect };
    }

    _pickClosestMarkerToRect(markers, engineRect) {
      if (!Array.isArray(markers) || !markers.length || !engineRect) return markers?.[0] || null;
      const engineCx = engineRect.left + engineRect.width / 2;
      const engineCy = engineRect.top + engineRect.height / 2;
      let best = null;
      let bestDist = Infinity;
    ...
    _findColorSubmitFlyMarker(block, userColor = '', engineRect = null) {
      if (!block) return null;
      ...
      const searchRoot =
        ...
        (typeof this._getPrimaryQuiltMarkerRoot === 'function' ? this._getPrimaryQuiltMarkerRoot() : null) ||
        root;
      const engineRectForPick =
        engineRect || this._computeEngineBlockScreenRect(block, userColor);
      ...
      if (newBlockGroup) {
        ...
        if (newBlockMarkers.length === 1) return newBlockMarkers[0];
        if (newBlockMarkers.length > 1 && engineRectForPick) {
          return this._pickClosestMarkerToRect(newBlockMarkers, engineRectForPick);
        }
      }
      ...
      if (targetColor && candidates.length > 1) {
        ...
        if (colorMatches.length > 1) {
          return engineRectForPick
            ? this._pickClosestMarkerToRect(colorMatches, engineRectForPick)
      ...
      if (highlighted.length > 1 && engineRectForPick) {
        return this._pickClosestMarkerToRect(highlighted, engineRectForPick);
      }
      if (candidates.length === 1) return candidates[0];
      if (candidates.length > 1) {
        return engineRectForPick
          ? this._pickClosestMarkerToRect(candidates, engineRectForPick)
          : typeof this._pickTopPrimaryScreenMarker === 'function'
    ...
    async _waitForQuiltBlockScreenRect(blockId, userColor = '', block = null, attempts = 30) {
      ...
      for (let attempt = 0; attempt < attempts; attempt += 1) {
        ...
        if (resolved?.rect) {
          ...
          } else if (resolved.engineRect) {
            resolved.rect = this._computeEngineBlockScreenRect(resolved.block, userColor) || resolved.rect;
          }
          if (resolved.rect.width > 2 && resolved.rect.height > 2) {
            return resolved;
          }
        }
        await this.wait(50);
    ...
    async playColorSubmitFlyToQuilt(block, color = '') {
      ...
      let { rect: toRect, marker: targetMarker } = target;
      let hideEl = targetMarker || null;
      ...
      await new Promise((resolve) => {
        ...
        const startFlight = () => {
          ...
          if (refreshed?.rect) {
            ...
            if (refreshed.marker) {
              toRect = refreshed.marker.getBoundingClientRect();
            } else {
              toRect = refreshed.rect;
            }
