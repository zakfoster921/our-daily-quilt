...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    isContributorCloudDebugEnabled() {
      try {
        if (new URLSearchParams(window.location.search).get('contributorDebug') === '1') return true;
        return window.localStorage?.getItem('contributorCloudDebug') === '1';
      } catch (_error) {
        return false;
      }
    }

    syncContributorCloudDebug(panelBody, listEl, metrics = {}) {
      const stage = listEl?.closest('.quilt-contributors-stage');
      const frame = listEl?.closest('.quilt-contributor-frame');
      const enabled = this.isContributorCloudDebugEnabled();
      if (stage) {
        if (enabled) stage.setAttribute('data-contributor-debug', 'true');
        else stage.removeAttribute('data-contributor-debug');
      }
      if (!enabled || !frame || !listEl || !panelBody) {
        listEl?.querySelectorAll('.quilt-contributor-name--debug-overflow').forEach((el) => {
          el.classList.remove('quilt-contributor-name--debug-overflow');
        });
        frame?.querySelector('.quilt-contributor-debug-layer')?.remove();
        return;
      }

      let layer = frame.querySelector('.quilt-contributor-debug-layer');
      if (!layer) {
        layer = document.createElement('div');
        layer.className = 'quilt-contributor-debug-layer';
        layer.setAttribute('aria-hidden', 'true');
        layer.innerHTML = [
          '<div class="quilt-contributor-debug-line quilt-contributor-debug-line--bottom-limit" data-debug-label="bottom guard (-2px)"></div>',
          '<pre class="quilt-contributor-debug-readout"></pre>',
        ].join('');
      ...
      const heading = panelBody.querySelector('.quilt-contributors-heading');
      const headingRect = heading?.getBoundingClientRect();
      const bottomLimit = Number(metrics.bottomLimit) || (listRect.bottom - 2);
      const fitBounds = metrics.fitBounds || {
        top: listRect.top + 2,
        left: listRect.left + 2,
        right: listRect.right - 2,
        bottom: bottomLimit,
      };
      const bottomLine = layer.querySelector('.quilt-contributor-debug-line--bottom-limit');
      if (bottomLine) {
        bottomLine.style.top = `${Math.round(bottomLimit - frameRect.top)}px`;
      }

      const names = Array.from(listEl.querySelectorAll('.quilt-contributor-name'));
      const overflowing = names.filter((el) => {
        const n = el.getBoundingClientRect();
        return (
          n.top < fitBounds.top
          || n.left < fitBounds.left
          || n.right > fitBounds.right
          || n.bottom > fitBounds.bottom
        );
      });
      names.forEach((el) => {
        el.classList.toggle('quilt-contributor-name--debug-overflow', overflowing.includes(el));
      });

      const readout = layer.querySelector('.quilt-contributor-debug-readout');
      if (readout) {
        const lines = [
          'contributor cloud debug',
          'green = panel-body (height fit box)',
          'magenta = heading',
          'cyan = name list (clip + center box)',
          'red dashed = bottom guard limit',
          'orange = names outside cyan box',
          '',
          `scale: ${Number(metrics.scale ?? 1).toFixed(4)}`,
          `guard steps: ${Number(metrics.guardSteps ?? 0)}`,
          `panel clientHeight: ${Math.round(panelRect.height)}px`,
          `panel scrollHeight: ${Number(metrics.panelScrollHeight ?? panelBody.scrollHeight)}px`,
          `list height: ${Math.round(listRect.height)}px`,
          `heading height: ${headingRect ? Math.round(headingRect.height) : 0}px`,
          `bottom limit Y: ${Math.round(bottomLimit - frameRect.top)}px from frame top`,
          `overflowing names: ${overflowing.length}`,
        ];
    ...
    fitContributorCloudToViewport(listEl) {
      ...
      this.syncContributorCloudDebug(panelBody, listEl, {
        scale,
        guardSteps: guard,
        panelScrollHeight: Math.ceil(panelBody.scrollHeight),
        bottomLimit: fitBounds.bottom,
        fitBounds,
      });
    }
