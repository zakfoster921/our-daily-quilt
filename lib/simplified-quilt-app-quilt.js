...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    async _tryStoredLayoutBStoryPreview(dateKey, previewContentKey = '', options = {}) {
      ...
      const canUseStalePlaceholder = () => {
        if (preferQuiltScreenPlaceholder) return true;
        return (
          currentBlockCount > 1 &&
          Number.isFinite(storedBlockCount) &&
          storedBlockCount > 0 &&
          storedBlockCount === currentBlockCount - 1
        );
      };
      const markPlaceholderOrReject = () => {
        if (!allowStalePlaceholder) return false;
        if (!canUseStalePlaceholder()) {
    ...
    async _refreshLayoutBStoryPreviewCore() {
      ...
      if (!this.isAdminTomorrowPreviewActive?.()) {
        ...
        const allowStoredPreview = !hasRepeatedLayoutBKeyword || !existingLivePreview;
