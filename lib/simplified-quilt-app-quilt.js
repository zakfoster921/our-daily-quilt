      ...
      try {
        ...
        const generatePreview =
          typeof arch?.generateInstagramStoryQuoteFitPreviewJpegImageData === 'function'
            ...
            : typeof arch?.generateInstagramStoryQuoteFitImageData === 'function'
              ? async () => {
                  const result = await arch.generateInstagramStoryQuoteFitImageData(blocks, quote, dk);
                  return result?.dataUrl || null;
                }
              : null;
        if (!generatePreview) throw new Error('Story quote-fit compose unavailable');
        const composePromise = generatePreview();
        const dataUrlRaw =
          typeof globalThis.odqPromiseWithTimeout === 'function'
            ? await globalThis.odqPromiseWithTimeout(composePromise, 35000, 'Story quote-fit preview')
