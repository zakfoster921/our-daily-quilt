    ...
    async runPostColorCutRevealSubmit(selectedColor) {
      ...
      try {
        ...
        const fireQuiltHandoff = () => {
          ...
          // Activate quilt under the still-opaque mat (fast path skips heavy chrome).
          // Otherwise the mat fades onto the color picker and can sit there for seconds.
          ...
          if (typeof console !== 'undefined') {
            const showMs =
              (typeof performance !== 'undefined' ? performance.now() : Date.now()) -
              handoffStarted;
            console.log('[post-color-cut-reveal] showScreen(ms)', Math.round(showMs));
          }

          const host = document.getElementById('postColorCutRevealHost');
          // Fade immediately — do not wait another frame after showScreen.
