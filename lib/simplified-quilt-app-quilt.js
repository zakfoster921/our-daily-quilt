...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    _findColorSubmitFlyMarker(block, userColor = '') {
      ...
      if (targetColor && candidates.length > 1) {
        ...
        if (colorMatches.length > 1 && typeof this._pickTopPrimaryScreenMarker === 'function') {
          return this._pickTopPrimaryScreenMarker(colorMatches);
        }
      }

      const highlighted = candidates.find((el) => el.classList?.contains('user-piece-highlight'));
      if (highlighted) return highlighted;
      if (candidates.length === 1) return candidates[0];
      if (candidates.length > 1 && typeof this._pickTopPrimaryScreenMarker === 'function') {
        return this._pickTopPrimaryScreenMarker(candidates);
      }
      if (typeof this._findRenderedMarkerForBlock === 'function') {
        return this._findRenderedMarkerForBlock(block, userColor);
      }
      return typeof this.findQuiltBlockMarker === 'function' ? this.findQuiltBlockMarker(blockId) : null;
    }
    ...
    async _waitForQuiltBlockScreenRect(blockId, userColor = '', block = null, attempts = 24) {
      const id = String(blockId || '').trim();
      if (!id) return null;
      const engineBlock =
        block || this._findDedicatedBlockInEngine(id) || (this.quiltEngine?.blocks || []).find((b) => String(b?.id) === id);
      for (let attempt = 0; attempt < attempts; attempt += 1) {
        const marker = this._findColorSubmitFlyMarker(engineBlock, userColor);
        const group =
          marker && typeof this._groupFromSpotlightMarker === 'function'
            ? this._groupFromSpotlightMarker(marker)
            : typeof this.findQuiltBlockGroup === 'function'
              ? this.findQuiltBlockGroup(id)?.group
              : null;
        const measureEl = marker || group;
        if (measureEl) {
          if (group) this._scrollQuiltBlockIntoView(group);
          await this.waitForNextPaint();
          const rect = measureEl.getBoundingClientRect();
          if (rect.width > 2 && rect.height > 2) {
            return { rect, group: group || measureEl.parentElement, marker: marker || null };
          }
        }
        await this.wait(40);
      }
    ...
    async playColorSubmitFlyToQuilt(block, color = '') {
      ...
      const pieceFrame = overlay?.querySelector('.color-submit-transition__piece-frame');
      const messageEl = overlay?.querySelector('.color-submit-transition__message');
      const engineBlock = this._freshDedicatedBlockForPreview(block);
      ...
      let { rect: toRect, group: targetGroup, marker: targetMarker } = target;
      const hideEl = targetMarker || targetGroup;
      ...
      await new Promise((resolve) => {
        ...
        const startFlight = () => {
          const refreshedMarker = this._findColorSubmitFlyMarker(engineBlock, color);
          if (refreshedMarker) {
            targetMarker = refreshedMarker;
            toRect = refreshedMarker.getBoundingClientRect();
            ...
            const refreshedTarget =
              typeof this.findQuiltBlockGroup === 'function'
                ? this.findQuiltBlockGroup(blockId)
                : null;
            if (refreshedTarget?.group) {
              targetGroup = refreshedTarget.group;
              toRect = (refreshedTarget.marker || refreshedTarget.group).getBoundingClientRect();
              if (toRect.width > 2 && toRect.height > 2) {
                mountFlyer(fromRect);
              }
            }
    ...
    async handleAddColor() {
      try {
        ...
        if (result) {
          ...
          await this.waitForNextPaint();
          if (dedicatedBlock) {
            await this.playColorSubmitFlyToQuilt(
              dedicatedBlock,
              selectedColor
            );
          } else {
