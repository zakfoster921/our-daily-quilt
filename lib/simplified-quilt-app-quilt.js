  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    async _refreshStoryQuoteFitPreviewCore(options = {}) {
      ...
      try {
        ...
        /** Prefer Firebase quilt-screen JPEG (already exists) over a slow SVG re-export. */
        let quiltSourceUrl = '';
        try {
        ...
        if (
          ...
          dataUrl = String(
            await timed(
              'Story quote-fit from stored quilt-screen',
              arch.generateInstagramStoryQuoteFitPreviewJpegImageData(blocks, quote, dk, {
                quiltSourceUrl
              }),
              18000
            ).catch((err) => {
              this.logger?.warn?.('Quote-fit from stored quilt-screen failed:', err);
              return null;
            }) || ''
          ).trim();
        }
        ...
        if (!dataUrl) {
          await this._waitForQuiltSvgReadyForStoryPreview(blocks, 2500);
          if (clearBusyIfStale()) return;
        ...
        const blob = Utils.dataUrlToBlob(dataUrl);
        if (!blob?.size) throw new Error('Story quote-fit preview blob empty');
