...
(function (root) {
  ...
  const STORY_QUOTE_FIT_PREVIEW_RENDER_VERSION = () =>
    String(globalThis.ODQ_STORY_QUOTE_FIT_PREVIEW_RENDER_VERSION || 'story-quote-fit-preview-v2').trim();
  const isStoryQuoteFitPreviewRenderVersion = (value) =>
    /^story-quote-fit-preview-v\d+$/i.test(String(value || '').trim());
    ...
    async _tryStoredLayoutBStoryPreview(dateKey, previewContentKey = '', options = {}) {
      ...
      if (preferStoryPreviewJpeg && expectedPreviewRenderVersion) {
        const previewVersionOk =
          !!storyPreviewUrl &&
          (storedPreviewRenderVersion === expectedPreviewRenderVersion ||
            isStoryQuoteFitPreviewRenderVersion(storedPreviewRenderVersion));
        if (!previewVersionOk) {
