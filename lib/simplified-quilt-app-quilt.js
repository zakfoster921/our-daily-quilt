    ...
    async runPostColorCutRevealSubmit(selectedColor) {
      ...
      try {
        ...
        const fireQuiltHandoff = () => {
          ...
          // Fast path: no admin pill / footer paper on screenChange (those froze the last cut).
          ...
          quiltHandoffPromise = (async () => {
            ...
            try {
              // Prefer fading onto the already-painted quilt (no white flash, no last-frame
              // wait on apply). Only block on apply+paint when the SVG is empty.
              const quiltHasPaint =
                (document.getElementById('quilt')?.childElementCount || 0) > 0;
              if (quiltHasPaint) {
