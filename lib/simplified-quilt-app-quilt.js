...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    /** Stored snapshot previews do not live-upgrade when speaker bytes arrive. */
    ...
    async _tryStoredLayoutBStoryPreview(dateKey, previewContentKey = '', options = {}) {
      ...
      const url = preferQuiltScreenPlaceholder
        ? quiltScreenUrl
        : useLayeredPreview
          ? quiltScreenUrl
          : storyUrl || quiltScreenUrl;
      if (!url) {
        clearUnsafePlaceholder();
        return false;
      }
    ...
    async _refreshLayoutBStoryPreviewCore() {
      ...
      try {
        ...
        if (!arch?.generateInstagramStoryLayoutBBlob) {
          throw new Error('Story compose unavailable');
        }
        const qt = this.quoteService?.getTodayQuote?.() || { text: '', author: '' };
        const previewPromise = arch.generateInstagramStoryLayoutBBlob(blocks, qt, dk, {
          forPreview: true,
          maxEdge: 720
        });
        const blob =
          typeof globalThis.odqPromiseWithTimeout === 'function'
            ? await globalThis.odqPromiseWithTimeout(previewPromise, 25000, 'Layout B story preview')
            : await previewPromise;
        if (generation !== (this._layoutBPreviewGeneration || 0)) return;
        const ok = await applyLivePreviewBlob(blob, contentKey);
        if (ok) return;
      } catch (previewErr) {
        this.logger?.warn?.('Live Layout B story preview failed:', previewErr);
      }
