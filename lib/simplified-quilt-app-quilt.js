...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    scheduleLayoutBStoryPreviewRefresh(options = {}) {
      ...
      const force = options.force === true;
      /** Stored snapshot is final for normal users this session — unless force (tune save / new upload). */
      if (!force && this._layoutBStoryPreviewHeavyDoneThisVisit && !this.isAdminTomorrowPreviewActive?.()) {
        return;
      }
      if (!force && this._layoutBStoryPreviewHeavyDoneThisVisit) {
        ...
        const currentFp =
          typeof Utils.computeQuiltFingerprint === 'function' && Array.isArray(blocks)
            ...
            : '';
        const storedFp = String(this._layoutBStoryPreviewContentKey || '').split('|')[1] || '';
        if (!currentFp || currentFp === storedFp) return;
        this._layoutBStoryPreviewHeavyDoneThisVisit = false;
      }
    ...
    /**
     * `options.ignoreFreshnessChecks: true` (passed by the primary caller in
     * _refreshLayoutBStoryPreviewCore) makes a stored image "final" even when it's stale
     * relative to the live in-memory quilt. Do not make any staleness check below unconditional
     * again — that reintroduces forcing a live client-side re-compose for normal users, which
     * we deliberately removed (see the DESIGN RULE comment on _refreshLayoutBStoryPreviewCore).
     */
    async _tryStoredLayoutBStoryPreview(dateKey, previewContentKey = '', options = {}) {
      ...
      if (options.ignoreFreshnessChecks !== true) {
        const currentFingerprint = String(options.currentFingerprint || '').trim();
        const storedFingerprint = String(data?.quiltFingerprint || '').trim();
        if (currentFingerprint && storedFingerprint && storedFingerprint !== currentFingerprint) {
          if (!markPlaceholderOrReject()) return false;
        }
        const hasUsableFreshnessMeta =
          !!storedFingerprint || (Number.isFinite(storedBlockCount) && storedBlockCount > 0);
        if (currentBlockCount > 1 && !hasUsableFreshnessMeta) {
          if (!markPlaceholderOrReject()) return false;
        }
        if (
          currentBlockCount > 1 &&
          Number.isFinite(storedBlockCount) &&
          storedBlockCount > 0 &&
          storedBlockCount < currentBlockCount
        ) {
          if (!markPlaceholderOrReject()) return false;
        }
      }
      ...
      /** Prefer one baked story PNG when present — fastest load, matches pre-layered preview. */
      ...
      if (preferQuiltScreenPlaceholder) {
        ...
        url = bustedQuiltScreenUrl;
        if (allowStalePlaceholder && finalAllowed) placeholderOnly = true;
      }
      if (!url) {
        this._layoutBStoryPreviewStoredUrlsMissing = true;
        clearUnsafePlaceholder();
      ...
      return await new Promise((resolve) => {
        ...
        img.onerror = () => {
          if (storyUrl && url !== bustedStoryUrl) {
            useLayeredPreview = false;
            this._layoutBStoryPreviewLayerUrls = null;
            this._layoutBStoryPreviewShareUrl = bustedStoryUrl;
            this._clearLayoutBPreviewOverlayImg();
            url = bustedStoryUrl;
            img.onerror = () => finish(false);
            img.onload = () => finish(img.naturalWidth > 0);
            img.src = bustedStoryUrl;
            return;
          }
          finish(false);
        };
    ...
    /**
     * DESIGN RULE — do not revert: a normal user session must only ever display the
     * nightly-generated stored story image (Storage/Firestore), never a live client-side
     * re-compose. The nightly job regenerates the image for the *next* session, not the
     * current one — a user should not see the story preview change mid-session unless they
     * fully close and relaunch the app. This is why the primary stored-preview lookup below
     * passes `ignoreFreshnessChecks: true`: it makes the stored image "final" even if the
     * live quilt has more blocks / a different fingerprint than what's stored. The live
     * "re-compose from scratch" code further down is admin-tomorrow preview only. Normal users
     * never get a live client-side compose during a session.
     * This has regressed before by re-adding freshness/staleness checks that force a live
     * rebuild — don't reintroduce that without re-reading this comment and the surrounding
    ...
    async _refreshLayoutBStoryPreviewCore(options = {}) {
      ...
      if (!this.isAdminTomorrowPreviewActive?.()) {
        ...
        if (!skipStoredPreview && !localTuneCustomized) {
          /** Story preview only ever shows the nightly-generated stored image for a given
           *  session — the nightly job regenerates it for the *next* visit, not the current
           *  one. Never trigger a live client-side re-compose just because the live quilt has
           *  moved on since the stored image was generated. */
    ...
    scheduleStoredStorySnapshotRefreshAfterQuiltUpdate(options = {}) {
      ...
      this._storySnapshotRefreshScheduledTimer = setTimeout(() => {
        ...
        void this.refreshStoredStorySnapshotAfterServerColorSubmission({ dateKey });
      }, delayMs);
    }
    ...
    async refreshStoredQuiltScreenLayerAfterQuiltUpdate(options = {}) {
      ...
      if (!Array.isArray(blocks) || blocks.length <= 1) return;
      const arch = this.archiveService;
      if (!arch?.generateInstagramQuiltScreen9x16ImageData || typeof Utils.writeInstagramImagesDocForZapier !== 'function') {
        return;
      }
      ...
      try {
        ...
        const dateKey = String(options.dateKey || this.getEffectiveAppDateKey?.() || Utils.getTodayKey()).trim();
        const quiltScreen9x16ImageData = await arch.generateInstagramQuiltScreen9x16ImageData(blocks, dateKey);
    ...
    async refreshStoredStorySnapshotAfterServerColorSubmission(options = {}) {
      ...
      try {
        ...
        if (arch?.generateInstagramStoryLayoutBImage) {
          storyLayoutBImageData = await timed(
            'Layout B story snapshot',
            arch.generateInstagramStoryLayoutBImage(blocks, quote, dateKey),
            120000
          ).catch((err) => {
            this.logger?.warn?.('Layout B story snapshot skipped:', err);
            return null;
          });
        }
        ...
        if (arch?.generateInstagramStoryLayoutBOverlayImage) {
          storyLayoutBOverlayImageData = await timed(
            'Layout B story overlay snapshot',
            arch.generateInstagramStoryLayoutBOverlayImage(blocks, quote, dateKey),
            120000
          ).catch((err) => {
            this.logger?.warn?.('Layout B story overlay snapshot skipped:', err);
            return null;
          });
        }
        if (!storyLayoutBImageData && !storyLayoutBOverlayImageData && !quiltScreen9x16ImageData) {
          return;
        }
        const uploadPayload = {
          dateKey,
        ...
        this.logger?.log?.('✅ Stored story snapshot refreshed after quilt update');
