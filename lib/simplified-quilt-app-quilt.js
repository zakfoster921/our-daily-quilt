...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Quilt {
    ...
    _scheduleSplitBandQuiltRenderRetry(opts = {}) {
      if (this._splitBandQuiltRenderRetry) return;
      this._splitBandQuiltRenderRetry = true;
      let attempts = 0;
      const maxAttempts = 180;
      const attempt = () => {
        attempts += 1;
        if (!this._shouldDeferSplitBandQuiltRender() || attempts >= maxAttempts) {
          this._splitBandQuiltRenderRetry = false;
          if (!this._shouldDeferSplitBandQuiltRender()) {
     ...
     * (quilt-screen JPEG + nightly/tune overlay), not a live client-side re-compose.
     * Each save uploads a quilt layer for the *next* visitor; the preview you see after
     * contributing stays one save behind until you relaunch. Live compose is admin/tune only.
     */
    ...
    async _primeQuiltQuoteChrome() {
      ...
      const isNative =
        ...
        window.odqIsCapacitorNative();
      const freshness =
        dateKey && !isNative ? await this._readNightlyClippingFreshness(dateKey) : null;
      this.populateQuiltQuote();
      if (this.isAdminTomorrowPreviewActive?.()) {
        todayQ = this.getEffectiveQuiltQuote() || todayQ;
      }
      this.refreshQuoteSpeakerWidget(this.isAdminTomorrowPreviewActive?.() ? null : todayQ);
    ...
    getColorSubmitFindingSpotMessage() {
      return 'Finding a spot for your block';
    }
    ...
    _ensureColorSubmitOverlayStructure(overlay) {
      if (!overlay) return null;
      const needsRebuild =
        !overlay.querySelector('.color-submit-transition__color-sheet') ||
        !overlay.querySelector('.color-submit-transition__piece-stage') ||
        !overlay.querySelector('.color-submit-transition__message');
      if (needsRebuild) {
        overlay.innerHTML = `
          <div class="color-submit-transition__paper" aria-hidden="true"></div>
          <div class="color-submit-transition__color-sheet" aria-hidden="true"></div>
          <div class="color-submit-transition__piece-stage" hidden>
            <div class="color-submit-transition__piece-frame" aria-hidden="true"></div>
          </div>
    ...
    showColorSubmitTransition(color, message = this.getColorSubmitPlacementMessage()) {
      ...
      this._ensureColorSubmitOverlayStructure(overlay);
      overlay.classList.remove(
        'color-submit-transition--shrinking',
        'color-submit-transition--shrink-settled',
        'color-submit-transition--message-visible',
        'color-submit-transition--piece-exit'
      );
      overlay.style.removeProperty('--color-submit-shrink-from');
      const pieceStage = overlay.querySelector('.color-submit-transition__piece-stage');
      const pieceFrame = overlay.querySelector('.color-submit-transition__piece-frame');
      const colorSheet = overlay.querySelector('.color-submit-transition__color-sheet');
      const messageEl = overlay.querySelector('.color-submit-transition__message');
      if (pieceStage) pieceStage.hidden = true;
      if (pieceFrame) {
        pieceFrame.innerHTML = '';
        if (colorSheet) colorSheet.style.opacity = '';
      }
      if (colorSheet) colorSheet.style.backgroundColor = color;
      overlay.style.setProperty('--submit-color', color);
      overlay.style.backgroundColor = color;
      overlay.style.color = this.getReadableTextColorForBackground(color);
      if (messageEl) {
        messageEl.textContent = message;
        messageEl.hidden = !String(message || '').trim();
      }
    ...
    hideColorSubmitTransition() {
      const overlay = document.getElementById('colorSubmitTransition');
      document.querySelectorAll('.color-submit-transition__flyer').forEach((node) => node.remove());
      if (!overlay) return;
      overlay.classList.remove(
        'color-submit-transition--visible',
        'color-submit-transition--shrinking',
        'color-submit-transition--shrink-settled',
        'color-submit-transition--message-visible',
        'color-submit-transition--piece-exit',
        'color-submit-transition--fly-prep'
      );
      const pieceFrame = overlay.querySelector('.color-submit-transition__piece-frame');
      if (pieceFrame) pieceFrame.style.visibility = '';
      setTimeout(() => overlay.remove(), 150);
    }

    _scrollQuiltBlockIntoView(group) {
      const quiltScreen = document.getElementById('screen-quilt');
      if (!group || !quiltScreen) return;
      try {
      ...
      if (typeof quiltScreen.scrollTop === 'number') {
        const rect = group.getBoundingClientRect?.();
        const screenRect = quiltScreen.getBoundingClientRect?.();
        if (rect && screenRect) {
          const delta =
            rect.top + rect.height / 2 - (screenRect.top + screenRect.height * 0.42);
          if (Math.abs(delta) > 8) {
            quiltScreen.scrollTop += delta;
          }
        }
      }
    }

    _pickUserContributionDrawTriangles(block, userColor = '') {
      block = this._normalizeColorSubmitPreviewBlock(block);
      const targetColor =
        this.normalizeHexColor(userColor || block.contributorColor || block.color || block.macroFrozenColor) ||
        '#DF9368';
      const colorsMatch = (a, b) => {
        const left = this.normalizeHexColor(a);
        const right = this.normalizeHexColor(b);
        return !!(left && right && left === right);
      };
      const hasStoredTris = Array.isArray(block.hstTriangles) && block.hstTriangles.length;
      let tris =
        typeof Utils.getHstDrawTriangles === 'function'
          ? Utils.getHstDrawTriangles(block, {
              exactPlacement: true,
              forceClassicPair: !hasStoredTris
            })
          : [];
      if (!tris.length && typeof Utils.getHstRenderTriangles === 'function') {
        tris = Utils.getHstRenderTriangles(block);
      }
      if (!tris.length) return [];

      let picked = tris.filter((tri) => colorsMatch(tri.color, targetColor));
      if (!picked.length) {
        picked = tris.filter((tri) => colorsMatch(tri.color, block.contributorColor));
      }
      if (!picked.length) {
        picked = tris.filter((tri) => colorsMatch(tri.color, block.color));
      }
      if (!picked.length) {
        const partnerColor = this.normalizeHexColor(block.hstColorB);
        if (partnerColor) {
          picked = tris.filter((tri) => !colorsMatch(tri.color, partnerColor));
        }
      }
      if (!picked.length && tris.length === 1) picked = tris;
      if (!picked.length && tris.length > 0) {
        picked = [tris.find((tri) => colorsMatch(tri.color, block.color)) || tris[0]];
      }
      return picked;
    }

    _getColorSubmitFlyBlock(preferredBlock) {
      const idx = this.renderer?.lastAddedIndex;
      const blocks = this.quiltEngine?.blocks;
      if (Number.isInteger(idx) && idx >= 0 && blocks?.[idx]) {
        return blocks[idx];
      }
      return this._freshDedicatedBlockForPreview(preferredBlock);
    }

    _clientRectFromPoints(points) {
    ...
    _engineBlockToSvgPoints(block, userColor = '') {
      block = this._normalizeColorSubmitPreviewBlock(block);
      const bx = Number(block.x) || 0;
      const by = Number(block.y) || 0;
      const bw = Math.max(1, Number(block.width) || 1);
      const bh = Math.max(1, Number(block.height) || 1);
      const targetColor = this.normalizeHexColor(userColor || block.contributorColor || block.color);

      if (this._isDedicatedHstBlock(block) || (Array.isArray(block.hstTriangles) && block.hstTriangles.length)) {
        const tris = this._pickUserContributionDrawTriangles(block, userColor);
        if (tris.length) {
          const points = [];
          tris.forEach((tri) => {
            (tri.points || []).forEach((p) => {
              points.push([bx + Number(Array.isArray(p) ? p[0] : p?.x), by + Number(Array.isArray(p) ? p[1] : p?.y)]);
            });
          });
          return points;
        }
      ...
      if (Array.isArray(block.polygonPieces) && block.polygonPieces.length) {
        const pieces = block.polygonPieces.filter(
          (piece) => this.normalizeHexColor(piece.color) === targetColor
        );
        const usePieces = pieces.length ? pieces : block.polygonPieces;
        const points = [];
        usePieces.forEach((piece) => {
    ...
    _computeEngineBlockScreenRect(block, userColor = '') {
      const svg = document.getElementById('quilt');
      if (!svg || !block || typeof svg.createSVGPoint !== 'function') return null;
      const toClient = (x, y) => {
      ...
      const clientPts = this._engineBlockToSvgPoints(block, userColor)
        .map(([x, y]) => toClient(x, y))
        .filter(Boolean);
      return this._clientRectFromPoints(clientPts);
    }

    _isRectInViewport(rect, margin = 160) {
    ...
    _isRectNearViewport(rect, margin = 32) {
      return this._isRectInViewport(rect, margin);
    }

    _getColorSubmitFlyAnchorRect(block) {
      const root = document.getElementById('quilt');
      if (!root) return null;
      const searchRoot =
        root.querySelector('#quiltPrimaryBand') ||
        (typeof this._getPrimaryQuiltMarkerRoot === 'function' ? this._getPrimaryQuiltMarkerRoot() : null) ||
        root;
      const newBlockGroup = searchRoot.querySelector('[data-new-block="1"]');
      if (newBlockGroup) {
        const rect = newBlockGroup.getBoundingClientRect();
        if (this._isRectInViewport(rect)) return rect;
      }
      const blockId = String(block?.id || '').trim();
      if (blockId && typeof this.findQuiltBlockGroup === 'function') {
        const groupTarget = this.findQuiltBlockGroup(blockId);
        const measureEl = groupTarget?.group || groupTarget?.marker;
        if (measureEl) {
    ...
    _resolveColorSubmitFlyTarget(block, userColor = '') {
      block = this._getColorSubmitFlyBlock(block);
      ...
      const normalizeFill = (fill) =>
        typeof this._spotlightNormalizeColor === 'function'
          ? this._spotlightNormalizeColor(fill)
          : this.normalizeHexColor(fill);

      const newBlockGroup = searchRoot.querySelector('[data-new-block="1"]');
      if (newBlockGroup) {
        const markers = Array.from(newBlockGroup.querySelectorAll('[data-block-id]')).filter(
          (el) => typeof this._isPrimaryQuiltMarker !== 'function' || this._isPrimaryQuiltMarker(el)
        );
        let marker = null;
        if (targetColor && markers.length > 1) {
          const byColor = markers.filter((el) => normalizeFill(el.getAttribute('fill')) === targetColor);
          if (byColor.length) marker = byColor[0];
        }
        if (!marker && markers.length) marker = markers[0];
        const measureEl = marker || newBlockGroup;
        const rect = measureEl.getBoundingClientRect();
        if (this._isRectInViewport(rect)) {
          return { rect, marker, group: newBlockGroup, block };
        }
      }

      const anchorRect = this._getColorSubmitFlyAnchorRect(block);
      let marker = this._findColorSubmitFlyMarker(block, userColor, anchorRect);
      let group = null;
      let rect = null;

      if (marker) {
        group =
          typeof this._groupFromSpotlightMarker === 'function'
            ? this._groupFromSpotlightMarker(marker)
            : marker.parentElement;
        rect = marker.getBoundingClientRect();
      }

      if ((!marker || !this._isRectInViewport(rect)) && typeof this.findQuiltBlockGroup === 'function') {
        const groupTarget = this.findQuiltBlockGroup(blockId);
        if (groupTarget?.group) {
          group = groupTarget.group;
          if (targetColor) {
            const inGroup = Array.from(group.querySelectorAll('[data-block-id]')).filter((el) => {
              if (typeof this._isPrimaryQuiltMarker === 'function' && !this._isPrimaryQuiltMarker(el)) {
                return false;
              }
              return normalizeFill(el.getAttribute('fill')) === targetColor;
            });
            if (inGroup.length === 1) {
              marker = inGroup[0];
              rect = marker.getBoundingClientRect();
            } else if (inGroup.length > 1 && anchorRect) {
              marker = this._pickClosestMarkerToRect(inGroup, anchorRect);
              rect = marker?.getBoundingClientRect?.() || null;
            }
          }
          if (!marker || !this._isRectInViewport(rect)) {
            marker = groupTarget.marker || marker;
            rect = (marker || group).getBoundingClientRect();
          }
        }
      }

      if (!rect || !this._isRectInViewport(rect)) return null;
      return { rect, marker, group, block };
    }

    _pickClosestMarkerToRect(markers, anchorRect) {
      if (!Array.isArray(markers) || !markers.length || !anchorRect) return markers?.[0] || null;
      ...
      return best || markers[0];
    }

    _findColorSubmitFlyMarker(block, userColor = '', anchorRect = null) {
      if (!block) return null;
      const blockId = String(block.id || '').trim();
      ...
      const searchRoot =
        ...
        (typeof this._getPrimaryQuiltMarkerRoot === 'function' ? this._getPrimaryQuiltMarkerRoot() : null) ||
        root;
      const anchorRectForPick = anchorRect || this._getColorSubmitFlyAnchorRect(block);

      const newBlockGroup = searchRoot.querySelector('[data-new-block="1"]');
      if (newBlockGroup) {
        const newBlockMarkers = [];
        const looseColorMarkers = [];
        for (const el of newBlockGroup.querySelectorAll('[data-block-id]')) {
          if (typeof this._isPrimaryQuiltMarker === 'function' && !this._isPrimaryQuiltMarker(el)) continue;
          const markerId = String(el.getAttribute('data-block-id') || '').trim();
          const related =
            markerId === blockId ||
            (typeof this._spotlightBlockIdsRelated === 'function' &&
              this._spotlightBlockIdsRelated(markerId, blockId));
          if (targetColor) {
            const fill = normalizeFill(el.getAttribute('fill'));
            if (fill && fill === targetColor) looseColorMarkers.push(el);
          }
          if (!related) continue;
          if (targetColor) {
            const fill = normalizeFill(el.getAttribute('fill'));
            if (fill && fill !== targetColor) continue;
          }
          newBlockMarkers.push(el);
        }
        const pickFrom = newBlockMarkers.length ? newBlockMarkers : looseColorMarkers;
        if (pickFrom.length === 1) return pickFrom[0];
        if (pickFrom.length > 1 && anchorRectForPick) {
          return this._pickClosestMarkerToRect(pickFrom, anchorRectForPick);
        }
      }
      ...
      for (const el of searchRoot.querySelectorAll('[data-block-id]')) {
        ...
        const related =
          ...
          (typeof this._spotlightBlockIdsRelated === 'function' &&
            this._spotlightBlockIdsRelated(markerId, blockId));
        if (related) candidates.push(el);
      }

      if (targetColor && candidates.length > 1) {
        const colorMatches = candidates.filter(
          (el) => normalizeFill(el.getAttribute('fill')) === targetColor
        );
        if (colorMatches.length === 1) return colorMatches[0];
        if (colorMatches.length > 1) {
          return anchorRectForPick
            ? this._pickClosestMarkerToRect(colorMatches, anchorRectForPick)
            : typeof this._pickTopPrimaryScreenMarker === 'function'
              ? this._pickTopPrimaryScreenMarker(colorMatches)
              : colorMatches[0];
        }
      }

      const highlighted = candidates.filter((el) => el.classList?.contains('user-piece-highlight'));
      if (highlighted.length === 1) return highlighted[0];
      if (highlighted.length > 1 && anchorRectForPick) {
        return this._pickClosestMarkerToRect(highlighted, anchorRectForPick);
      }
      if (candidates.length === 1) return candidates[0];
      if (candidates.length > 1) {
        return anchorRectForPick
          ? this._pickClosestMarkerToRect(candidates, anchorRectForPick)
          : typeof this._pickTopPrimaryScreenMarker === 'function'
            ? this._pickTopPrimaryScreenMarker(candidates)
            : candidates[0];
      }
      return null;
    }

    _cloneColorSubmitFlyMarkerSvg(marker) {
      if (!marker || typeof document === 'undefined') return null;
      const tag = String(marker.tagName || '').toLowerCase();
      if (tag !== 'polygon' && tag !== 'path' && tag !== 'rect' && tag !== 'circle') return null;
      ...
      let maxY = -Infinity;
      if (tag === 'polygon') {
        const pts = String(marker.getAttribute('points') || '')
          .trim()
          .split(/\s+/)
        ...
        svg.classList.add('color-submit-transition__piece-svg');
      ...
      const pad = Math.max(8, Math.min(maxX - minX, maxY - minY) * 0.12);
      const svg = document.createElementNS(NS, 'svg');
      svg.classList.add('color-submit-transition__piece-svg');
      svg.setAttribute(
        'viewBox',
    ...
    async _waitForQuiltBlockScreenRect(blockId, userColor = '', block = null, attempts = 30) {
      const engineBlock =
        block ||
        this._getColorSubmitFlyBlock(
          this._findDedicatedBlockInEngine(blockId) ||
            (this.quiltEngine?.blocks || []).find((b) => String(b?.id) === String(blockId || ''))
        );
      if (!engineBlock) return null;

      for (let attempt = 0; attempt < attempts; attempt += 1) {
        const resolved = this._resolveColorSubmitFlyTarget(engineBlock, userColor);
        if (resolved?.rect) {
          const scrollTarget = resolved.group || resolved.marker;
          if (scrollTarget) this._scrollQuiltBlockIntoView(scrollTarget);
          await this.waitForNextPaint();
          await this.wait(80);
          if (resolved.marker) {
            resolved.rect = resolved.marker.getBoundingClientRect();
          } else if (resolved.group) {
            resolved.rect = resolved.group.getBoundingClientRect();
          }
          if (resolved.rect?.width > 2 && resolved.rect?.height > 2 && this._isRectInViewport(resolved.rect)) {
            return resolved;
          }
        }
        await this.wait(50);
      ...
      const lastTry = this._resolveColorSubmitFlyTarget(engineBlock, userColor);
      if (lastTry?.rect?.width > 2 && lastTry.rect.height > 2) {
        return lastTry;
      }
      if (typeof this.findQuiltBlockGroup === 'function') {
        const groupTarget = this.findQuiltBlockGroup(String(blockId || '').trim());
        const measureEl = groupTarget?.marker || groupTarget?.group;
        if (measureEl) {
          const rect = measureEl.getBoundingClientRect();
          if (rect.width > 2 && rect.height > 2) {
            return {
              rect,
              marker: groupTarget.marker || null,
              group: groupTarget.group || null,
              block: engineBlock
            };
          }
        }
    ...
    _isDedicatedHstBlock(block) {
      return block?.specialPatternType === 'hst';
    }

    _colorSubmitPreviewUsesLiteBuilder(block) {
      if (!block) return false;
      if (this._isDedicatedHstBlock(block)) return true;
      if (Array.isArray(block.hstTriangles) && block.hstTriangles.length) return true;
      if (Array.isArray(block.polygonPieces) && block.polygonPieces.length) return true;
      return false;
    }

    _normalizeColorSubmitPreviewBlock(block) {
      if (!block || typeof block !== 'object') return block;
      const normalized = { ...block };
      if (normalized.specialPatternType === 'hst' && normalized.patternType !== 'special') {
        normalized.patternType = 'special';
      }
      return normalized;
    }

    _freshDedicatedBlockForPreview(block) {
      if (!block) return null;
      const fromEngine = this._findDedicatedBlockInEngine(
        block.id,
        Number(block.submissionIndex) || Number(this.quiltEngine?.submissionCount) || 0
      );
      return fromEngine || block;
    }

    _trimColorSubmitPreviewSvg(svg, block, userColor = '') {
      const target = this.normalizeHexColor(userColor || block?.color);
      if (!svg || !target || !this._isDedicatedHstBlock(block)) return svg;
      const matches = (fill) => this.normalizeHexColor(fill) === target;
      svg.querySelectorAll('polygon, rect, path, circle').forEach((node) => {
        if (node.closest('defs')) return;
        const fill = node.getAttribute('fill');
        if (fill && !matches(fill)) {
          node.remove();
          return;
        }
        node.setAttribute('fill', target);
        if (node.hasAttribute('stroke') && node.getAttribute('stroke') !== 'none') {
          node.setAttribute('stroke', target);
        }
      });
      return svg;
    }

    _findDedicatedBlockInEngine(blockId, submissionIndex = 0) {
      const blocks = this.quiltEngine?.blocks;
      if (!Array.isArray(blocks) || !blocks.length) return null;
      const id = String(blockId || '').trim();
      if (id) {
        const byId = blocks.find((block) => String(block?.id || '') === id);
        if (byId) return byId;
      }
      ...
      return (
        blocks.find((block) => {
          if (Number(block?.submissionIndex) !== Number(submissionIndex)) return false;
          if (userIds.includes(String(block?.contributorId || '').trim())) return true;
          const ids = Array.isArray(block?.contributorIds) ? block.contributorIds : [];
          return ids.some((uid) => userIds.includes(String(uid || '').trim()));
        }) || null
      );
    }

    _buildDedicatedBlockPreviewSvg(block, userColor = '') {
      if (!block || typeof document === 'undefined' || typeof Utils === 'undefined') return null;
      block = this._normalizeColorSubmitPreviewBlock(block);
      const NS = 'http://www.w3.org/2000/svg';
      const targetColor =
        this.normalizeHexColor(userColor || block.color || block.macroFrozenColor) || '#DF9368';
      const colorsMatch = (a, b) => {
        const left = this.normalizeHexColor(a);
        const right = this.normalizeHexColor(b);
        return !!(left && right && left === right);
      ...
      const pushPolygon = (points, fill) => {
        ...
        if (pts.length >= 3) shapes.push({ points: pts, fill: fill || targetColor });
      };

      if (block.specialPatternType === 'hst' || (Array.isArray(block.hstTriangles) && block.hstTriangles.length)) {
        const picked = this._pickUserContributionDrawTriangles(block, userColor);
        picked.forEach((tri) => {
          pushPolygon(
            (tri.points || []).map((p) => [bx + Number(Array.isArray(p) ? p[0] : p?.x), by + Number(Array.isArray(p) ? p[1] : p?.y)]),
            targetColor
          );
        });
      } else if (Array.isArray(block.polygonPieces) && block.polygonPieces.length) {
        let pieces = block.polygonPieces;
        const colorPieces = pieces.filter((piece) => colorsMatch(piece.color, targetColor));
        if (colorPieces.length) pieces = colorPieces;
        pieces.forEach((piece) => {
          pushPolygon(
            (piece.points || []).map((p) => [bx + Number(Array.isArray(p) ? p[0] : p?.x), by + Number(Array.isArray(p) ? p[1] : p?.y)]),
            targetColor
          );
        });
      } else if (
        ...
        typeof SimpleQuiltEngine.hasPersistedInsetCircleGeometry === 'function' &&
        SimpleQuiltEngine.hasPersistedInsetCircleGeometry(block)
      ) {
        shapes.push({
          circle: {
            cx: Number(block.insetCx) || bx + bw / 2,
            cy: Number(block.insetCy) || by + bh / 2,
            r: Math.max(1, Number(block.insetR) || Math.min(bw, bh) / 2)
          },
          fill: targetColor
        });
      } else if (this.renderer && typeof this.renderer.createHandCutPolygon === 'function') {
        const poly = this.renderer.createHandCutPolygon(bx, by, bw, bh, 1, String(block.id || 'preview'));
        const raw = String(poly.getAttribute('points') || '')
          .trim()
          .split(/\s+/)
          .map((pair) => pair.split(',').map(Number))
          .filter((p) => p.length === 2 && p.every(Number.isFinite));
        if (raw.length >= 3) shapes.push({ points: raw, fill: targetColor });
      }

      if (!shapes.length) {
        shapes.push({ rect: { x: bx, y: by, width: bw, height: bh }, fill: targetColor });
      }
      ...
      shapes.forEach((shape) => {
        if (shape.points) {
          ...
          minX = Math.min(minX, shape.rect.x);
          minY = Math.min(minY, shape.rect.y);
          maxX = Math.max(maxX, shape.rect.x + shape.rect.width);
          maxY = Math.max(maxY, shape.rect.y + shape.rect.height);
        } else if (shape.circle) {
          minX = Math.min(minX, shape.circle.cx - shape.circle.r);
          minY = Math.min(minY, shape.circle.cy - shape.circle.r);
          maxX = Math.max(maxX, shape.circle.cx + shape.circle.r);
          maxY = Math.max(maxY, shape.circle.cy + shape.circle.r);
        }
      });
      ...
      const pad = Math.max(10, Math.min(spanX, spanY) * 0.16);
      const svg = document.createElementNS(NS, 'svg');
      svg.setAttribute('xmlns', NS);
      svg.setAttribute(
        'viewBox',
        `${minX - pad} ${minY - pad} ${spanX + pad * 2} ${spanY + pad * 2}`
      );
      svg.setAttribute('width', '100%');
      svg.setAttribute('height', '100%');
      svg.setAttribute('preserveAspectRatio', 'xMidYMid meet');
      ...
      group.setAttribute('filter', 'drop-shadow(0 8px 18px rgba(28, 32, 40, 0.14))');
      shapes.forEach((shape) => {
        if (shape.points) {
          const poly = document.createElementNS(NS, 'polygon');
          poly.setAttribute('points', shape.points.map((p) => `${p[0]},${p[1]}`).join(' '));
          poly.setAttribute('fill', shape.fill || targetColor);
          poly.setAttribute('stroke', shape.fill || targetColor);
          ...
          rect.setAttribute('fill', shape.fill || targetColor);
          ...
          circle.setAttribute('fill', shape.fill || targetColor);
    ...
    renderDedicatedBlockPreviewSvg(block, userColor = '') {
      block = this._freshDedicatedBlockForPreview(block);
      if (this._colorSubmitPreviewUsesLiteBuilder(block)) {
        const lite = this._buildDedicatedBlockPreviewSvg(block, userColor);
        if (lite) return lite;
      }
      if (!block || !this.renderer || typeof this.renderer.renderBlocks !== 'function') return null;
      const NS = 'http://www.w3.org/2000/svg';
      ...
      const prevSvg = this.renderer.quiltSVG;
      const prevOverride = this.renderer._renderViewportOverride;
      const prevForcePrimary = this.renderer._forcePrimaryOnlyPreview;
      const prevDisableSplit = this.renderer._exportDisableSplitBandFit;
      const prevLastAdded = this.renderer.lastAddedIndex;
      const prevBackside = this.renderer.backsidePreviewEnabled;

      try {
        const width = Math.max(1, Number(block.width) || 1);
        const height = Math.max(1, Number(block.height) || 1);
        const viewport = Math.max(280, Math.min(420, Math.max(width, height) * 2.8));
        this.renderer.quiltSVG = svg;
        this.renderer._renderViewportOverride = { w: viewport, h: viewport };
        this.renderer._forcePrimaryOnlyPreview = true;
        this.renderer._exportDisableSplitBandFit = true;
        this.renderer.lastAddedIndex = null;
        this.renderer.setBacksidePreviewEnabled?.(false);
        this.renderer.renderBlocks(
          [this._normalizeColorSubmitPreviewBlock(block)],
          [block],
          Number(block.submissionIndex) || 1
        );
        const viewBox = svg.getAttribute('viewBox');
        const hasContent = svg.childNodes.length > 0;
        if (!viewBox || !hasContent) return null;
        const clone = svg.cloneNode(true);
        this._trimColorSubmitPreviewSvg(clone, block, userColor);
        clone.classList.add('color-submit-transition__piece-svg');
        ...
        this.logger.warn('Dedicated block preview render failed:', previewErr);
        return null;
      } finally {
        this.renderer.quiltSVG = prevSvg;
        this.renderer._renderViewportOverride = prevOverride;
        this.renderer._forcePrimaryOnlyPreview = prevForcePrimary;
        this.renderer._exportDisableSplitBandFit = prevDisableSplit;
        this.renderer.lastAddedIndex = prevLastAdded;
        this.renderer.setBacksidePreviewEnabled?.(prevBackside);
        host.remove();
      }
    }

    _colorSubmitShrinkStartScale() {
      const stageMax = Math.min(window.innerWidth * 0.74, 340);
      const coverW = window.innerWidth / Math.max(stageMax, 1);
      const coverH = window.innerHeight / Math.max(stageMax, 1);
      return Math.max(coverW, coverH) * 1.18;
    }

    _createColorSubmitFallbackPiece(block, color) {
      const previewSvg = this._buildDedicatedBlockPreviewSvg(block, color);
      if (previewSvg) return previewSvg;
      const el = document.createElement('div');
      el.className = 'color-submit-transition__piece-fallback';
      const width = Math.max(1, Number(block?.width) || 1);
      const height = Math.max(1, Number(block?.height) || 1);
      el.style.aspectRatio = `${width} / ${height}`;
      el.style.backgroundColor = color || '#df9368';
      return el;
    }

    _createColorSubmitPieceNode(block, color) {
      const previewSvg = this.renderDedicatedBlockPreviewSvg(block, color);
      if (previewSvg) return previewSvg;
      this.logger.warn('Color submit piece preview SVG unavailable; using fallback swatch.');
      return this._createColorSubmitFallbackPiece(block, color);
    }

    async playColorSubmitPieceReveal(block, color = '') {
      const reduceMotion = window.matchMedia?.('(prefers-reduced-motion: reduce)')?.matches;
      let overlay = document.getElementById('colorSubmitTransition');
      if (!overlay) return;
      this._ensureColorSubmitOverlayStructure(overlay);

      const pieceNode = block
        ? this._createColorSubmitPieceNode(this._freshDedicatedBlockForPreview(block), color)
        : null;
      const pieceStage = overlay.querySelector('.color-submit-transition__piece-stage');
      const pieceFrame = overlay.querySelector('.color-submit-transition__piece-frame');
      const colorSheet = overlay.querySelector('.color-submit-transition__color-sheet');
      const messageEl = overlay.querySelector('.color-submit-transition__message');
      if (!pieceNode || !pieceStage || !pieceFrame) {
        this.logger.warn('Color submit shrink skipped: overlay piece mount unavailable.');
        return;
      }

      const shrinkFrom = this._colorSubmitShrinkStartScale();
      overlay.classList.remove(
        'color-submit-transition--shrink-settled',
        'color-submit-transition--message-visible',
        'color-submit-transition--piece-exit'
      );
      overlay.style.setProperty('--color-submit-shrink-from', String(shrinkFrom));
      overlay.style.backgroundColor = '#f6f4f1';
      overlay.style.color = '#2f2925';
      if (messageEl) messageEl.hidden = true;
      pieceFrame.innerHTML = '';
      pieceFrame.appendChild(pieceNode);
      pieceStage.hidden = false;

      if (reduceMotion) {
        overlay.classList.add('color-submit-transition--shrinking', 'color-submit-transition--shrink-settled');
        if (colorSheet) colorSheet.style.opacity = '0';
      } else {
        overlay.classList.add('color-submit-transition--shrinking');
        await this.waitForNextPaint();
        await this.waitForNextPaint();
        overlay.classList.add('color-submit-transition--shrink-settled');
        await this.wait(820);
      }

      this.updateColorSubmitTransition(this.getColorSubmitFindingSpotMessage());
      overlay.classList.add('color-submit-transition--message-visible');
      await this.wait(reduceMotion ? 900 : 1200);
    }

    _colorSubmitFlyMotion(fromRect, toRect) {
      return {
        fromX: fromRect.left,
        fromY: fromRect.top,
        toX: toRect.left,
        toY: toRect.top,
        fromW: Math.max(fromRect.width, 1),
        fromH: Math.max(fromRect.height, 1),
        toW: Math.max(toRect.width, 1),
        toH: Math.max(toRect.height, 1)
      };
    }

    _mountColorSubmitFlyer(flyer, flyerInner, motion) {
      flyer.style.left = '0';
      flyer.style.top = '0';
      flyer.style.width = `${motion.fromW}px`;
      flyer.style.height = `${motion.fromH}px`;
      flyer.style.opacity = '1';
      flyer.style.transform = `translate3d(${motion.fromX}px, ${motion.fromY}px, 0)`;
      flyer.style.transition = 'none';
      flyerInner.style.width = '100%';
      flyerInner.style.height = '100%';
      flyerInner.style.transformOrigin = 'top left';
      flyerInner.style.transform = 'scale(1, 1)';
      flyerInner.style.transition = 'none';
      flyer.dataset.endTransform = `translate3d(${motion.toX}px, ${motion.toY}px, 0)`;
      flyer.dataset.endWidth = String(motion.toW);
      flyer.dataset.endHeight = String(motion.toH);
      flyerInner.dataset.endTransform = `scale(${motion.toW / motion.fromW}, ${motion.toH / motion.fromH})`;
    }

    async playColorSubmitFlyToQuilt(block, color = '') {
      const reduceMotion = window.matchMedia?.('(prefers-reduced-motion: reduce)')?.matches;
      const overlay = document.getElementById('colorSubmitTransition');
      const pieceFrame = overlay?.querySelector('.color-submit-transition__piece-frame');
      const messageEl = overlay?.querySelector('.color-submit-transition__message');
      const engineBlock = this._getColorSubmitFlyBlock(block);
      const blockId = String(engineBlock?.id || block?.id || '').trim();

      if (!overlay || !pieceFrame || !blockId) {
        this.hideColorSubmitTransition();
        return;
      }

      const fromRect = pieceFrame.getBoundingClientRect();
      if (fromRect.width < 2 || fromRect.height < 2) {
        this.hideColorSubmitTransition();
        return;
      }

      const target = await this._waitForQuiltBlockScreenRect(blockId, color, engineBlock);
      if (!target || reduceMotion) {
        overlay.classList.add('color-submit-transition--piece-exit');
        await this.wait(reduceMotion ? 180 : 480);
        this.hideColorSubmitTransition();
        return;
      }

      let { rect: toRect, marker: targetMarker, group: targetGroup } = target;
      let hideEl = targetMarker || targetGroup || null;
      const prevHideOpacity = hideEl?.style?.opacity || '';

      const flyer = document.createElement('div');
      flyer.className = 'color-submit-transition__flyer';
      flyer.setAttribute('aria-hidden', 'true');
      const flyerInner = document.createElement('div');
      flyerInner.className = 'color-submit-transition__flyer-inner';
      const pieceClone = pieceFrame.cloneNode(true);
      pieceClone.style.visibility = 'visible';
      pieceClone.style.transform = 'none';
      pieceClone.style.opacity = '1';
      flyerInner.appendChild(pieceClone);
      flyer.appendChild(flyerInner);
      document.body.appendChild(flyer);

      overlay.classList.add('color-submit-transition--fly-prep');
      pieceFrame.style.visibility = 'hidden';
      if (messageEl) messageEl.hidden = true;
      ...
      const mountFlyer = (startRect, endRect) => {
        this._mountColorSubmitFlyer(flyer, flyerInner, this._colorSubmitFlyMotion(startRect, endRect));
      };

      mountFlyer(fromRect, toRect);
      ...
      await new Promise((resolve) => {
        ...
        const done = () => {
          ...
          finished = true;
          flyer.remove();
          if (hideEl) hideEl.style.opacity = prevHideOpacity || '';
        ...
        const startFlight = () => {
          const startRect = fromRect;

          const refreshed = this._resolveColorSubmitFlyTarget(engineBlock, color);
          if (refreshed?.rect) {
            if (refreshed.marker && refreshed.marker !== targetMarker) {
              if (hideEl) hideEl.style.opacity = prevHideOpacity || '';
              targetMarker = refreshed.marker;
              targetGroup = refreshed.group || targetGroup;
              hideEl = targetMarker || targetGroup;
              if (hideEl) hideEl.style.opacity = '0';
            } else if (refreshed.group && refreshed.group !== targetGroup) {
              if (hideEl) hideEl.style.opacity = prevHideOpacity || '';
              targetGroup = refreshed.group;
              hideEl = targetMarker || targetGroup;
              if (hideEl) hideEl.style.opacity = '0';
            }
            toRect = refreshed.marker
              ? refreshed.marker.getBoundingClientRect()
              : refreshed.group
                ? refreshed.group.getBoundingClientRect()
                : refreshed.rect;
            if (toRect.width > 2 && toRect.height > 2) {
              mountFlyer(startRect, toRect);
            }
          } else if (typeof this.findQuiltBlockGroup === 'function') {
            const groupTarget = this.findQuiltBlockGroup(blockId);
            if (groupTarget?.group) {
              if (hideEl) hideEl.style.opacity = prevHideOpacity || '';
              targetGroup = groupTarget.group;
              targetMarker = groupTarget.marker || targetMarker;
              hideEl = targetMarker || targetGroup;
              if (hideEl) hideEl.style.opacity = '0';
              toRect = (targetMarker || targetGroup).getBoundingClientRect();
              if (toRect.width > 2 && toRect.height > 2) {
                mountFlyer(startRect, toRect);
              }
            }
          ...
          flyer.offsetHeight;
          flyer.style.willChange = 'transform, width, height';
          flyerInner.style.willChange = 'transform';
          flyer.style.transition = `transform ${duration}ms ${easing}, width ${duration}ms ${easing}, height ${duration}ms ${easing}`;
          flyerInner.style.transition = `transform ${duration}ms ${easing}`;
          overlay.style.transition = `opacity ${Math.round(duration * 0.94)}ms ease-out`;
          requestAnimationFrame(() => {
            requestAnimationFrame(() => {
              flyer.style.transform = flyer.dataset.endTransform || flyer.style.transform;
              flyer.style.width = `${flyer.dataset.endWidth}px`;
              flyer.style.height = `${flyer.dataset.endHeight}px`;
              if (flyerInner.dataset.endTransform) {
                flyerInner.style.transform = flyerInner.dataset.endTransform;
              }
        ...
        const onTransitionEnd = (event) => {
          if (event.target !== flyer || event.propertyName !== 'transform') return;
    ...
    async refreshStoredQuiltScreenLayerAfterQuiltUpdate(options = {}) {
      ...
      try {
        ...
        if (typeof arch.generateInstagramStoryLayoutBOverlayImage === 'function') {
          try {
      ...
      try {
        ...
        try {
          ...
          if (!serverError?.transient) {
            this.uiService.showToast(serverError.message || 'Could not save your color yet. Please try again.');
            await showTryAgain();
            return;
          }
        ...
        if (result) {
          ...
          const dedicatedBlock =
            result.dedicatedBlock ||
            this._findDedicatedBlockInEngine(
              result.dedicatedBlockId || this._latestDedicatedBlockId,
              result.submissionIndex
            );
          if (dedicatedBlock) {
            await this.playColorSubmitPieceReveal(
              dedicatedBlock,
              selectedColor
            );
          } else {
            this.logger.warn('Color submit shrink skipped: dedicated block not found after placement.');
            if (!usedServerSubmission) await minimumTransitionHold;
          }
          ...
          if (dedicatedBlock) {
            const flyBlock =
              Number.isInteger(this.renderer?.lastAddedIndex) &&
              this.quiltEngine?.blocks?.[this.renderer.lastAddedIndex]
                ? this.quiltEngine.blocks[this.renderer.lastAddedIndex]
                : dedicatedBlock;
            await this.playColorSubmitFlyToQuilt(flyBlock, selectedColor);
          } else {
            this.hideColorSubmitTransition();
          }
          await this.waitForNextPaint();
