...
class UIService {
  ...
  /**
   * WKWebView can leave `#screen-exit-chamber > .quilt-footer-icon-row`
   * (viewport-fixed) in the hit-test path while the chamber section is
   * `display: none`, which blocks taps on the quilt scroll-end footer.
   * When the chamber is inactive, force the row inert via inline `!important`
   * rules; clear them when the chamber is shown so CSS controls layout again.
   */
  _syncExitChamberFixedFooterLeakGuard() {
    try {
      const exit = document.getElementById('screen-exit-chamber');
      const row = exit?.querySelector?.(':scope > .quilt-footer-icon-row');
      if (!exit || !row) return;
  ...
  _purgeReflectionArchiveGhostLayers() {
    try {
      ...
      const quiltArchive = document.getElementById('screen-archive');
      if (quiltArchive && !quiltArchive.classList.contains('active')) {
        quiltArchive.style.setProperty('display', 'none', 'important');
        quiltArchive.style.setProperty('visibility', 'hidden', 'important');
        quiltArchive.style.setProperty('pointer-events', 'none', 'important');
        quiltArchive.style.setProperty('opacity', '0', 'important');
      }
  ...
  showScreen(screenId) {
    try {
      ...
      const restoreQuiltScroll =
        screenId === 'screen-quilt' &&
        previousId &&
        this._quiltScrollPreserveScreens.has(previousId) &&
        this._savedQuiltScrollPosition;
      if (previousId === 'screen-exit-chamber' && screenId !== 'screen-exit-chamber') {
        window.app?.clearExitChamberSpeakerTexture?.();
      }
      ...
      const thanksToWelcomeFade =
        previousActive &&
        previousId === 'screen-name-thanks' &&
      ...
      if (target) {
        ...
        if (isFlowTransition) {
          ...
          const finishInterstitialOverlay = () => {
            if (previousActive) {
              previousActive.classList.remove('active', ...flowClassCleanup);
              previousActive.style.display = 'none';
            }
          ...
          requestAnimationFrame(() => {
            requestAnimationFrame(() => {
              ...
              target.addEventListener('transitionend', onTransEnd);
              const fallbackId = setTimeout(settle, 2400);
            });
          });
          ...
          if (screenId === 'screen-quilt' && restoreQuiltScroll) {
        ...
        if (screenId === 'screen-welcome') {
          ...
          // After name-thanks crossfade, start stagger once welcome is easing in
          const staggerBaseMs = thanksToWelcomeFade ? 320 : 0;
          fadeElements.forEach((el, index) => {
            const staggerDelay = staggerBaseMs + index * 200;
            setTimeout(() => {
        ...
        this._syncExitChamberFixedFooterLeakGuard();
        ...
        document.dispatchEvent(new CustomEvent('screenChange', {
          detail: {
            screenId: screenId,
            skipQuiltScrollCue: restoreQuiltScroll === true
          }
        }));
