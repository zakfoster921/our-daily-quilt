...
class UIService {
  ...
  showScreen(screenId) {
    try {
      const socialScreens = new Set([
        'screen-social-posts',
        'screen-social-post-compose',
        'screen-social-post-manage'
      ]);
      if (socialScreens.has(screenId) && window.app?.canAccessSocialPosts && !window.app.canAccessSocialPosts()) {
        window.app?.guardSocialPostsAccess?.();
        return;
      }
