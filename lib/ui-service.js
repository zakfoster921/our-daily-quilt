...
class UIService {
  ...
  showScreen(screenId, options = {}) {
    try {
      ...
      if (
        screenId === 'screen-quilt' &&
        !returnToQuiltFromPreserve &&
        options.fastQuiltHandoff !== true
      ) {
        try {
      ...
      if (target) {
        ...
        if (isFlowTransition) {
          ...
          if (options.fastQuiltHandoff !== true) {
            target.offsetHeight;
          }
        }
        ...
        // fastQuiltHandoff: cut-reveal mat is still covering the viewport; defer chrome until after fade.
        if (screenId === 'screen-quilt' && options.fastQuiltHandoff !== true) {
          window.app?.applyQuiltMoodTriptychPalette?.();
          window.app?.ensureQuiltScreenInteractive?.();
          window.app?._syncQuiltScrollIconFooterLeakGuard?.();
          window.app?.mountQuiltNameLeaderboard?.();
        ...
        document.dispatchEvent(new CustomEvent('screenChange', {
          detail: {
            ...
            fastQuiltHandoff: options.fastQuiltHandoff === true
          }
        }));
