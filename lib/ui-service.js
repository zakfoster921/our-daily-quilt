...
class UIService {
  constructor(logger) {
    ...
    /** Full-screen overlays that keep #screen-quilt visible underneath (quilt art shows through). */
    this._quiltUnderlayScreens = new Set(['screen-quilt-name-leaderboard']);
  ...
  showScreen(screenId) {
    try {
      ...
      const quiltNameLeaderboardOverlay = screenId === 'screen-quilt-name-leaderboard';
      document.documentElement?.classList?.toggle('odq-qnlb-overlay-active', quiltNameLeaderboardOverlay);
      document.body?.classList?.toggle('odq-qnlb-overlay-active', quiltNameLeaderboardOverlay);
      ...
      if (target) {
        ...
        if (isFlowTransition) {
          ...
          const quiltUnderlay = this._quiltUnderlayScreens.has(screenId);
          document.querySelectorAll(".screen").forEach(s => {
            if (quiltUnderlay && s.id === 'screen-quilt') {
        ...
        if (screenId === 'screen-quilt') {
          ...
          window.app?._syncQuiltScrollIconFooterLeakGuard?.();
          window.app?.mountPublicQuiltNameBallot?.();
