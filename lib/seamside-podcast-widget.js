...
function (global) {
  ...
  function hideHost(host) {
    ...
    host.innerHTML = '';
    host._seamsidePresentationKey = 'hide';
  }

  function isSeamsideQuote(quote, quoteService) {
    return !!(quoteService && typeof quoteService.isSeamsideQuote === 'function' && quoteService.isSeamsideQuote(quote));
  }

  /** player | link-only | hide — submitted_via SEAMSIDE always shows chrome; guest match plays audio. */
  function resolvePresentation(quote, quoteService) {
    const author = String(quote?.author || '').trim();
    const episode =
      quoteService && typeof quoteService.lookupSeamsideEpisodeForAuthor === 'function'
        ? quoteService.lookupSeamsideEpisodeForAuthor(author)
        : null;
    if (episode) return { mode: 'player', episode, author };
    if (isSeamsideQuote(quote, quoteService)) {
      return { mode: 'link-only', episode: {}, author };
    }
    return { mode: 'hide', episode: null, author };
  }

  function presentationKey(presentation) {
    if (!presentation || presentation.mode === 'hide') return 'hide';
    const ep = presentation.episode || {};
    return [
      presentation.mode,
      String(ep.audioUrl || ep.audio_url || '').trim(),
      String(ep.applePodcastsUrl || ep.apple_podcasts_url || '').trim(),
      String(ep.episodeTitle || ep.episode_title || '').trim()
    ].join('|');
  }

  function refresh(host, quote, quoteService) {
    if (!host) return;
    const presentation = resolvePresentation(quote, quoteService);
    const key = presentationKey(presentation);
    if (presentation.mode === 'hide') {
      hideHost(host);
      return;
    }
    if (!host.hidden && host._seamsidePresentationKey === key) return;
    pauseActiveAudio(null);
    host._seamsidePresentationKey = key;
    if (presentation.mode === 'link-only') {
      renderLinkOnly(host, presentation.episode, presentation.author);
      return;
    }
    renderPlayer(host, presentation.episode, presentation.author);
  }
