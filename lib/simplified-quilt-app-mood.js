  ...
  class SimplifiedQuiltAppV2Mood {
      _refreshQuiltMoodWidgetImpl(quote = null) {
        ...
        const previewDateKey =
          this.isAdminTomorrowPreviewActive?.() && typeof this.getEffectiveAppDateKey === 'function'
            ? String(this.getEffectiveAppDateKey() || '').trim()
            : '';
        if (
          previewDateKey &&
          todayQuote?._moodHydratedDateKey !== previewDateKey &&
          host.dataset.moodHydratePending !== '1'
        ) {
        ...
        if (!goodDay || !roughDay) {
          this._scheduleQuiltMoodFieldsHydrate(todayQuote, host);
          // Do not unmount while hydrating — yellow card + strips must stay on screen.
          // Provisional copy is replaced when Firestore mood fields arrive.
          if (!goodDay) goodDay = String(host.dataset.goodDay || '').trim() || '…';
          if (!roughDay) roughDay = String(host.dataset.roughDay || '').trim() || '…';
        }
      ...
      _persistQuiltMoodPick(
        ...
        if (!dk) return false;
        let scratchReported = false;
        try {
          const existingRaw = localStorage.getItem(this._quiltMoodStorageKey());
          if (existingRaw) {
            const existing = JSON.parse(existingRaw);
            scratchReported =
              String(existing?.dateKey || '') === dk && existing?.scratchReported === true;
          }
        } catch (_) {
          scratchReported = false;
        }
        try {
          ...
          if (!scratchReported) this._reportMoodScratchOnce?.(mood, dk);
          return true;
        } catch (err) {
      ...
      /** Fire-and-forget anonymous scratch count (once per device/day). */
      _reportMoodScratchOnce(mood, dateKey) {
        if (mood !== 'good' && mood !== 'rough') return;
        const dk = String(dateKey || this._quiltMoodPickDateKey() || '').trim();
        if (!dk) return;
        if (this._moodScratchReportInFlightKey === dk) return;
        ...
        const markReported = () => {
          try {
            ...
            if (String(parsed?.dateKey || '') !== dk) return;
            parsed.scratchReported = true;
            localStorage.setItem(this._quiltMoodStorageKey(), JSON.stringify(parsed));
          } catch (_) {
            /* ignore */
          }
        };

        this._moodScratchReportInFlightKey = dk;
        void fetch(`${baseUrl}/api/mood-scratch`, {
          method: 'POST',
          ...
          .finally(() => {
            if (this._moodScratchReportInFlightKey === dk) {
              this._moodScratchReportInFlightKey = '';
            }
          });
      }
