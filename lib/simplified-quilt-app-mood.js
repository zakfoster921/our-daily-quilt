  ...
  class SimplifiedQuiltAppV2Mood {
      ...
      _resetQuiltMoodForNewDay() {
        ...
        if (spread) {
          ...
          spread.classList.remove('is-ready', 'is-open');
      ...
      /** Dev / QA: clear today's mood pick and reopen the triptych picker. */
      resetQuiltMoodSelection() {
        ...
        if (triptych?.setMood && (triptych.getMood?.() || spread?.classList.contains('is-open'))) {
          triptych.setMood(null, { instant: true, force: true });
        }
      ...
      _resolveEffectiveQuiltMoodPick(host, spreadHost, sig, hintMood = null) {
        ...
        const spreadStale = !!(spreadDate && today && spreadDate !== today);
