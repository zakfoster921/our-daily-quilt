...
(function (root) {
  ...
  class SimplifiedQuiltAppV2QuiltNameLeaderboard {
    ...
    /** Orange sticky-note squares for rank badges (mockup). */
    applyQuiltNameLeaderboardPositionBadgeColor(wrap) {
      if (!wrap) return;
      wrap.style.setProperty('--qnlb-position-badge-bg', '#e9a23d');
      wrap.style.setProperty('--qnlb-position-badge-fg', '#241f19');
    }

    renderQuiltNameLeaderboard(data = {}, options = {}) {
      const wrap = this.quiltNameLeaderboardWrap();
      this.applyQuiltNameLeaderboardPositionBadgeColor(wrap);
