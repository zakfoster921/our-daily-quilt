...
(function (root) {
  ...
  class SimplifiedQuiltAppV2QuiltNameLeaderboard {
    ...
    syncQuiltNameLeaderboardStripWidths(list, retriesLeft = 20) {
      ...
      const headerWidth = this.qnlbHeaderWidthPx(slabWrap);
    ...
    qnlbHeaderWidthPx(slabWrap) {
      ...
      if (contentNeed <= 0) return 0;

      const remPx = parseFloat(getComputedStyle(document.documentElement).fontSize) || 16;
      const scale = parseFloat(getComputedStyle(slabWrap).getPropertyValue('--qnlb-rank-scale')) || 1;
      const minSidePad = 0.5 * remPx * scale;
      const tapeInner = 0.28 * remPx;
      let width = contentNeed + 2 * (minSidePad + tapeInner) + 2;
      const sidePad = Math.max(minSidePad, width * 0.13 + tapeInner);
      return Math.ceil(contentNeed + 2 * sidePad + 2);
    }
