...
(function (root) {
  ...
  class SimplifiedQuiltAppV2QuiltNameLeaderboard {
    ...
    applyQuiltNameLeaderboardInfoModal(phase = 'submissions') {
      ...
      if (phase === 'voting') {
        title.textContent = 'How voting works';
        text.textContent =
          `When submissions close, ${QNLB_VOTING_ENTRY_CAP} names are chosen at random from everything submitted that day. Tap a name strip to vote. Vote counts stay hidden until you have voted. Whichever name has the most votes when the day ends becomes the quilt title.`;
        return;
      }

      title.textContent = 'How submissions work';
      text.textContent =
    ...
    paintQuiltNameLeaderboardListRow(row, entry, index, viewState) {
      const { myVote, winningVoteCount, showVotes, canVote, demo, badgeTones } = viewState;
      const word = String(entry.word || '').trim();
      const safeWord = this.escapeQuiltNameLeaderboardHtml(word);
      const votes = Number(entry.votes);
    ...
    syncQuiltNameLeaderboardStripWidths(list, retriesLeft = 20) {
      // Set on the shared card ancestor (not just the list) so the "NAME THIS QUILT"
      // title can match the same computed width via inheritance.
      ...
      if (!list?.classList.contains('qnlb-entry-list--final')) {
        ...
        slabWrap?.style.removeProperty('--qnlb-header-width');
        return;
      }
      ...
      const headerWidth = this.qnlbHeaderWidthPx(slabWrap, rowWidth);
      slabWrap.style.setProperty('--qnlb-voting-row-width', `${rowWidth}px`);
      if (headerWidth > 0) {
        slabWrap.style.setProperty('--qnlb-header-width', `${headerWidth}px`);
      } else {
        slabWrap.style.removeProperty('--qnlb-header-width');
      }
    }
    ...
    qnlbHeaderWidthPx(slabWrap, rowWidthPx) {
      const header = slabWrap?.querySelector('.qnlb-rank-board__header');
      if (!header) return rowWidthPx || 0;
      ...
      for (const child of header.children) {
        ...
        contentNeed = Math.max(contentNeed, Math.ceil(el.scrollWidth));
        el.style.width = prevWidth;
      }
      if (contentNeed <= 0) return rowWidthPx || 0;

      const remPx = parseFloat(getComputedStyle(document.documentElement).fontSize) || 16;
      const scale = parseFloat(getComputedStyle(slabWrap).getPropertyValue('--qnlb-rank-scale')) || 1;
      const minSidePad = 0.675 * remPx * scale;
      const tapeInner = 0.47 * remPx;
      let contentFit = contentNeed + 2 * (minSidePad + tapeInner) + 12;
      const sidePad = Math.max(minSidePad, contentFit * 0.13 + tapeInner);
      contentFit = Math.ceil(contentNeed + 2 * sidePad + 12);

      const rowWidth = Math.ceil(rowWidthPx || 0);
      const base = rowWidth <= contentFit
        ? contentFit
        : Math.max(contentFit, Math.ceil((contentFit + rowWidth) / 2));
    ...
    renderQuiltNameLeaderboard(data = {}, options = {}) {
      ...
      if (infoBtn) {
        infoBtn.hidden = phase !== 'submissions' && phase !== 'voting';
      }
      ...
      if (phase === 'submissions') {
        ...
        phaseBanner.innerHTML = 'All our entries are in!<br>Tap to vote';
