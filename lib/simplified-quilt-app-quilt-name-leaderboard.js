...
(function (root) {
  ...
  class SimplifiedQuiltAppV2QuiltNameLeaderboard {
    ...
    applyQuiltNameLeaderboardCloudScatter(row, word, index, { emphasize = false } = {}) {
      const sizeBuckets = [0.88, 0.96, 1.04, 1.14];
      ...
      row.style.setProperty('--cloud-size', emphasize ? maxSize : sizeBuckets[styleHash % sizeBuckets.length]);
    ...
    fitQuiltNameLeaderboardCloud(list) {
      if (!list) return;
      const host = list.closest('.qnlb-name-cloud-stage');
      if (!host) return;
      const maxHeight = Math.floor(host.clientHeight);
      if (!maxHeight) return;
    ...
    paintQuiltNameLeaderboardEntryRow(row, entry, index, viewState) {
      ...
      this.applyQuiltNameLeaderboardCloudScatter(row, word, index, { emphasize: isSelected });
    ...
    renderQuiltNameLeaderboard(data = {}, options = {}) {
      ...
      const submitPrompt = document.getElementById('qnlbSubmitPrompt');
      const list = document.getElementById('qnlbEntryList');
      ...
      const showQuestionHeader = phase === 'submissions' || phase === 'voting' || phase === 'final';
      form.hidden = !showQuestionHeader;
      if (formControls) {
        formControls.hidden = !this.canQuiltNameLeaderboardSubmit(data);
      }
      if (submitPrompt) {
        submitPrompt.textContent = phase === 'submissions'
          ? 'What shall we name our quilt?'
          : 'Thank you!';
      }
