...
(function (root) {
  ...
  class SimplifiedQuiltAppV2QuiltNameLeaderboard {
    ...
    syncQuiltNameLeaderboardStripWidths(list, retriesLeft = 20) {
      ...
      // Longest entry defines the reference width, plus blank space before its vote
      // count — every strip is set to exactly that width via --qnlb-strip-width.
      const QNLB_STRIP_EXTRA_PAD_PX = 50;
      const target = Math.ceil(maxWidth + QNLB_STRIP_EXTRA_PAD_PX);
