...
(function (root) {
  ...
  class SimplifiedQuiltAppV2QuiltNameLeaderboard {
    ...
    qnlbHeaderWidthPx(slabWrap, rowWidthPx) {
      ...
      const title = slabWrap?.querySelector('.qnlb-rank-board__title');
      if (!header || !title) return rowWidthPx;
      const hs = getComputedStyle(header);
      const padX = (parseFloat(hs.paddingLeft) || 0) + (parseFloat(hs.paddingRight) || 0);
      void title.offsetWidth;
      const titleNeed = Math.ceil(title.scrollWidth) + padX + 6;
      return Math.max(rowWidthPx, titleNeed);
    }
