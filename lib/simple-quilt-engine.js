...
class SimpleQuiltEngine {
  ...
  addColor(newColor, retryDepth = 0, rejectedSplitIds = null) {
    ...
    if (!selectedBlock) {
      ...
      } else if (nextAddNumber >= earlyWindowEnd + 1 && nextAddNumber <= midWindowEnd) {
        // Adds 7-24 have mid-window odds unless blocked by the special-pattern cap.
  ...
  selectPatternType(availablePatterns, block, newColor) {
    ...
    const weights = availablePatterns.map((pattern) =>
      bandTypes.includes(pattern) ? bandWeight : 1
    );
