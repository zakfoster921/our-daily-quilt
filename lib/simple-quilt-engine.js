...
class SimpleQuiltEngine {
  ...
  static STARTER_AXIS_SECOND_LINE_CHANCE = 0.82;
  static STARTER_AXIS_MIN_DEGREES = 16;
  static STARTER_AXIS_MAX_DEGREES = 48;
  static RECENT_VISIBLE_SUBMISSION_WINDOW = 4;
  static AXIS_COLLAGE_MAX_VISUAL_EXTENSION = 32;
  static NEW_VISIBLE_REGULAR_RENDER_SCALE = 0.98;
  static RECENT_VISIBLE_REGULAR_RENDER_SCALE = 0.94;
  /** Caps large-block paint-order key so they cannot bury recent/small pieces. */
  static PAINT_ORDER_LARGE_BLOCK_AREA_FRACTION = 0.095;
  ...
  /** Pre-freeze roulette: adds 2–6 / 7–24. */
  static SPECIAL_PATTERN_ODDS_EARLY = 0.82;
  static SPECIAL_PATTERN_ODDS_MID = 0.55;
  static SPECIAL_PATTERN_CAP_FRACTION = 0.65;
  static MIN_SPECIAL_PATTERN_AREA = 18000;
  /** Special motifs only on axis-aligned, mostly-square rects (not triangles / skinny strips / angled shards). */
  static SPECIAL_HOST_MIN_SHORT = 72;
  /** Reject salami strips (see STRIP_ASPECT_FORCE_CROSS_CUT ~2.8); allow portrait half-blocks (~1.6:1). */
  static SPECIAL_HOST_MAX_ASPECT = 2.15;
  /** Band/grid specials that clip messily on diagonal or polygon parents — prefer HST, inset, or regular split. */
  static ANGLED_PARENT_EXCLUDED_SPECIAL_PATTERN_TYPES = [
    'stripes',
    'bandedColumns',
    'nestedGrid',
    'railfence',
  ...
  /** Stripes, columns, nested grids, rail fence — visually “lots of bands/rows”. */
  static BAND_ROW_SPECIAL_PATTERN_TYPES = ['stripes', 'bandedColumns', 'nestedGrid', 'railfence'];
  /** Motif types excluded from new-special roulette (legacy blocks may still split). */
  static DISABLED_SPECIAL_PATTERN_TYPES = ['logcabin', 'railfence'];
  ...
  /** After macro freeze, new-special roulette runs at this fraction of base odds (merges/routing still frozen). */
  static SPECIAL_STRUCTURE_FROZEN_SCALE = 0.32;
  /** When macro freeze is very early (see {@link SimpleQuiltEngine#_effectiveMacroFrozenSpecialStructureScale}). */
  static SPECIAL_STRUCTURE_EARLY_FREEZE_SCALE = 0.85;
  /** Freeze-at block count at/above which post-freeze special scale uses FROZEN_SCALE only. */
  static SPECIAL_STRUCTURE_FREEZE_SCALE_REFERENCE = 25;
  /** Earliest freeze threshold that gets full EARLY_FREEZE_SCALE boost. */
  static SPECIAL_STRUCTURE_EARLY_FREEZE_AT_BLOCKS = 5;
  /** ~1-in-6 days: macro regions follow uneven vertical columns (not default per-block regions). */
  static MACRO_COLUMN_LAYOUT_CHANCE = 0.17;
  static MACRO_COLUMN_MIN_COUNT = 3;
  static MACRO_COLUMN_MAX_COUNT = 6;
  ...
  static MACRO_FREEZE_AT_BLOCK_COUNT = 5;
  ...
  /** Post-freeze macro shards: probability of a slight hand-cut angle instead of a 90° rect split (0 = off). */
  static MACRO_FLATTENED_ANGLED_SPLIT_PROB = 0.85;
  /** Relative roulette weight for band/row specials (0 = off). */
  static BAND_ROW_SPECIAL_WEIGHT = 0;
  /** Scales new-special roulette (0 = motif specials off). */
  static SPECIAL_STRUCTURE_SCALE_MULTIPLIER = 0;
  ...
  static STRUCTURE_ACCENT_PROB = 0.45;
  /** After this many same-orientation regular splits in a lineage, prefer cutting the other axis. */
  static SAME_AXIS_SPLIT_STREAK_FORCE_FLIP = 3;
  /** Aspect (long/short) at/above this counts as a strip — anti-salami only when color mismatches. */
  static STRIP_ASPECT_FORCE_CROSS_CUT = 2.8;
  /** New color at/above this OKLab similarity to stack refs → allow intentional salami banding. */
  static SALAMI_COHESIVE_COLOR_SIM_MIN = 0.78;
  ...
  /** Max OKLab chroma pull for outlier blocks (deferred dampen preview + future production). */
  static CHROMA_DAMPEN_MAX = 0.55;
  /** Max fraction of OKLab L excess (above neighbor/quilt target) to pull down for bright outliers. */
  static LIGHTNESS_DAMPEN_MAX = 0.55;
  /** Production deferred cohesion strength (chroma>p90 blocks, applied on next add). */
  static COHESION_DEFERRED_STRENGTH = 1;
  /** Seam soften: min OKLab ΔE across a shared edge to qualify. */
  static SEAM_SOFTEN_THRESHOLD = 0.35;
  /** Seam soften: pull both sides toward OKLab midpoint (0–1). */
  static SEAM_SOFTEN_STRENGTH = 0.07;

  /**
   * Debug preview: ?harmonyBlend=1 (ramp) | ?harmonyBlend=0.36 (fixed 0–1) | localStorage odqHarmonyBlend.
   * Production default (no override) keeps harmony off.
   */
  static harmonyBlendUrlOverride() {
    try {
      if (typeof window === 'undefined') return null;
      const params = new URLSearchParams(window.location.search || '');
      let raw = params.get('harmonyBlend');
      if (raw == null || raw === '') raw = localStorage.getItem('odqHarmonyBlend');
      if (raw == null || raw === '') return null;
      if (raw === '0' || raw === 'false') return 0;
      if (raw === '1' || raw === 'true' || raw === 'ramp') return 'ramp';
      const n = Number(raw);
  ...
  /** Debug preview: ?harmonyIgnoreFreeze=1 | localStorage odqHarmonyIgnoreFreeze — harmony after macro freeze. */
  static harmonyIgnoreFreezeEnabled() {
    try {
      if (typeof window === 'undefined') return false;
      const params = new URLSearchParams(window.location.search || '');
      const v = params.get('harmonyIgnoreFreeze');
      if (v === '1' || v === 'true') return true;
      return localStorage.getItem('odqHarmonyIgnoreFreeze') === '1';
    } catch (_) {
      return false;
    }
  }
  ...
  /**
   * Re-process loaded quilt colors on reload (in-memory only; originals restored on save).
   * On by default when harmonyBlend is set; disable with ?harmonyRetro=0.
   */
  static harmonyRetroOnLoadEnabled() {
    const override = SimpleQuiltEngine.harmonyBlendUrlOverride();
    if (override == null || override === 0) return false;
    try {
      if (typeof window === 'undefined') return false;
      const params = new URLSearchParams(window.location.search || '');
      const v = params.get('harmonyRetro');
      if (v === '0' || v === 'false') return false;
      if (v === '1' || v === 'true') return true;
      if (localStorage.getItem('odqHarmonyRetro') === '0') return false;
      return true;
    } catch (_) {
      return false;
    }
  ...
  /** Short side at/below this (px) counts as a small piece for shared-edge snapping. */
  static SMALL_PIECE_SNAP_MAX_SHORT = 120;
  /** Area at/below this (px²) counts as a small piece for shared-edge snapping. */
  static SMALL_PIECE_SNAP_MAX_AREA = 14400;
  /** Weld polygon verts within this distance of a neighbor seam (px). */
  static SMALL_PIECE_SNAP_VERTEX_TOL = 2.5;
  /** Prefer neighbor seam coords within this distance when picking a split (px). */
  static SMALL_PIECE_SNAP_CUT_TOL = 18;
  /** Quantize polygon verts to reduce sub-pixel seam drift between siblings. */
  static POLYGON_VERTEX_QUANTIZE_STEP = 0.25;
  /** Drop nearly collinear verts (stairsteps on angled macro edges). */
  static POLYGON_COLLINEAR_COLLAPSE_DEG = 0.55;
  /** Douglas–Peucker tolerance for smoothing clip stairsteps on angled boundaries (px). */
  static POLYGON_STAIRSTEP_SIMPLIFY_EPS = 1.35;
  ...
  /** Pick 1 of each day: Notion mood color (not a human contributor). */
  static DAILY_MOOD_CONTRIBUTOR_ID = 'daily-mood-seed';
  ...
  static pickStarterFieldColorHex() {
    ...
    let probe;
    try {
      probe = new SimpleQuiltEngine('__starter_color_probe', { recordColorReplayEvents: false });
    } catch (_) {
  ...
  constructor(userId = null, options = {}) {
    ...
    /** Record exact split history so admin replay and synthetic reels can rebuild quilt growth. */
    this.recordColorReplayEvents = options?.recordColorReplayEvents !== false;
    ...
    /** @type {'default'|'columns'|undefined} */
    this.macroLayoutMode = undefined;
    /** Quilt date key (YYYY-MM-DD) — seeds occasional column macro layout. */
    this.quiltDateKey = typeof options?.quiltDateKey === 'string' ? options.quiltDateKey.trim() : '';
    /** @type {Array<{regionId:number,x:number,y:number,width:number,height:number}>|null} */
    this._macroColumnBands = null;
    /** @type {Map<string, string>|null} block id → pre-retro hex (preview-only; not persisted) */
    this._harmonyRetroOriginalColors = null;
    ...
    /** Builder / debug: override MACRO_FREEZE_AT_BLOCK_COUNT (null = use static default). */
    this.macroFreezeAtBlockCountOverride =
      Number.isFinite(Number(options?.macroFreezeAtBlockCount)) &&
      Number(options.macroFreezeAtBlockCount) >= 2
        ? Math.floor(Number(options.macroFreezeAtBlockCount))
        : null;
    /** Relative roulette weight for band/row specials (0 = off). */
    this.bandRowSpecialWeight =
      Number.isFinite(Number(options?.bandRowSpecialWeight)) &&
      Number(options.bandRowSpecialWeight) >= 0
        ? Number(options.bandRowSpecialWeight)
        : SimpleQuiltEngine.BAND_ROW_SPECIAL_WEIGHT;
    /** Scales new-special roulette (0 = motif specials off). */
    this.specialStructureScaleMultiplier =
      Number.isFinite(Number(options?.specialStructureScaleMultiplier)) &&
      Number(options.specialStructureScaleMultiplier) >= 0
        ? Number(options.specialStructureScaleMultiplier)
        : SimpleQuiltEngine.SPECIAL_STRUCTURE_SCALE_MULTIPLIER;
    /** Builder / debug: post-macro-freeze special odds scale (null = live default 0.32). */
    this.macroFrozenSpecialStructureScale =
      Number.isFinite(Number(options?.macroFrozenSpecialStructureScale)) &&
      Number(options.macroFrozenSpecialStructureScale) >= 0
        ? Number(options.macroFrozenSpecialStructureScale)
        : null;
    /** 0–1: angled hand-cut splits inside frozen macro shards (builder sets >0). */
    this.macroFlattenedAngledSplitProb =
      Number.isFinite(Number(options?.macroFlattenedAngledSplitProb)) &&
      Number(options.macroFlattenedAngledSplitProb) >= 0
        ? Math.max(0, Math.min(1, Number(options.macroFlattenedAngledSplitProb)))
        : SimpleQuiltEngine.MACRO_FLATTENED_ANGLED_SPLIT_PROB;
  ...
  _sharesAxisAlignedFullEdge(A, B, eps) {
    ...
    // Reject only when blocks are separated on both axes (corner-only touch has ox≈0, oy≈0 → all checks below fail).
  ...
  _blockIsSmallPiece(block) {
    if (!block) return false;
    const w = Math.max(0, Number(block.width) || 0);
    const h = Math.max(0, Number(block.height) || 0);
    if (!(w > 0 && h > 0)) return false;
    return (
      Math.min(w, h) <= SimpleQuiltEngine.SMALL_PIECE_SNAP_MAX_SHORT ||
      w * h <= SimpleQuiltEngine.SMALL_PIECE_SNAP_MAX_AREA
    );
  }

  /** Small piece or touching a small neighbor — use straight seams + weld shared edges. */
  _shouldSnapSmallPieceCluster(block) {
    if (!block) return false;
    if (this._blockIsSmallPiece(block)) return true;
    ...
    for (const other of this.blocks) {
      ...
      if (this._blockIsSmallPiece(other)) return true;
    }
    return false;
  }

  _snapCoordToTargets(val, targets, tol = SimpleQuiltEngine.SMALL_PIECE_SNAP_VERTEX_TOL) {
    const v = Number(val);
  ...
  _sharedEdgeSnapTargetsForBlock(block) {
    const eps = SimpleQuiltEngine.MERGE_EPS_PX * 2;
    const bx = Number(block.x);
    const by = Number(block.y);
    const bw = Number(block.width);
    const bh = Number(block.height);
    const snapX = new Set([bx, bx + bw]);
    const snapY = new Set([by, by + bh]);
    if (![bx, by, bw, bh].every(Number.isFinite)) return { snapX, snapY };
    for (const other of this.blocks) {
      if (!other || other.id === block.id) continue;
      if (!this._sharesAxisAlignedFullEdge(block, other, eps)) continue;
      if (!this._blockIsSmallPiece(other) && !this._blockIsSmallPiece(block)) continue;
      const ox = Number(other.x);
      const oy = Number(other.y);
      const ox2 = ox + Number(other.width);
      const oy2 = oy + Number(other.height);
      if (Math.abs(bx + bw - ox) < eps || Math.abs(ox2 - bx) < eps) {
        snapX.add(Math.abs(bx + bw - ox) < eps ? ox : ox2);
      }
      if (Math.abs(by + bh - oy) < eps || Math.abs(oy2 - by) < eps) {
        snapY.add(Math.abs(by + bh - oy) < eps ? oy : oy2);
      }
      snapX.add(ox, ox2);
      snapY.add(oy, oy2);
    }
  ...
  _clipPolygonWorldPiecesToBox(pieces, box) {
    if (!box || !Array.isArray(pieces) || !pieces.length) return pieces;
    return this._clipPolygonPiecesToRect(pieces, box.x, box.y, box.width, box.height);
  }

  _constrainRectChildToBox(child, box) {
    if (!child || !box) return child;
    const px2 = box.x + box.width;
    const py2 = box.y + box.height;
    let x = Math.max(box.x, Number(child.x));
    let y = Math.max(box.y, Number(child.y));
    let width = Math.max(1, Number(child.width));
    let height = Math.max(1, Number(child.height));
    if (x + width > px2) width = Math.max(1, px2 - x);
    if (y + height > py2) height = Math.max(1, py2 - y);
    if (x >= px2) x = Math.max(box.x, px2 - 1);
    if (y >= py2) y = Math.max(box.y, py2 - 1);
    child.x = x;
    child.y = y;
    child.width = width;
    child.height = height;
  ...
  _constrainPolygonChildToBox(child, box) {
    if (!child || !box || !Array.isArray(child.polygonPieces) || !child.polygonPieces.length) return child;
    const clipped = this._finalizeMacroPolygonWorldPieces(
      this._clipPolygonWorldPiecesToBox(this._polygonPiecesWorld(child), box)
    );
    const bounds = this._polygonWorldBounds(clipped);
    if (!bounds || !clipped.length) return child;
    const px2 = box.x + box.width;
    const py2 = box.y + box.height;
    bounds.x = Math.max(box.x, bounds.x);
    bounds.y = Math.max(box.y, bounds.y);
    bounds.width = Math.max(1, Math.min(px2, bounds.x + bounds.width) - bounds.x);
    bounds.height = Math.max(1, Math.min(py2, bounds.y + bounds.height) - bounds.y);
    child.x = bounds.x;
    child.y = bounds.y;
    child.width = bounds.width;
    child.height = bounds.height;
    child.polygonPieces = this._localPolygonPiecesFromWorldPieces(clipped, bounds);
    return child;
  }

  _constrainSplitChildrenToParent(parent, children) {
    const box = this._parentClipBox(parent);
    if (!box || !Array.isArray(children)) return children;
    children.forEach((child) => {
      if (!child) return;
      if (Array.isArray(child.polygonPieces) && child.polygonPieces.length) {
        this._constrainPolygonChildToBox(child, box);
      } else {
        this._constrainRectChildToBox(child, box);
      }
    });
    return children;
  }

  _snapPolygonWorldPiecesToSharedEdges(worldPieces, block) {
    if (
      block?.macroVisibleFlattened === true &&
      this._polygonHasNonAxisAlignedEdge(worldPieces)
    ) {
      return this._finalizeMacroPolygonWorldPieces(worldPieces);
    }
    if (!this._shouldSnapSmallPieceCluster(block)) return worldPieces;
    const { snapX, snapY } = this._sharedEdgeSnapTargetsForBlock(block);
    const tol = SimpleQuiltEngine.SMALL_PIECE_SNAP_VERTEX_TOL;
    return (worldPieces || []).map((piece) => ({
      color: piece.color,
      points: (piece.points || []).map((p) => [
        this._snapCoordToTargets(Number(p[0]), snapX, tol),
        this._snapCoordToTargets(Number(p[1]), snapY, tol)
      ])
    }));
  }

  _snapAxisSplitCutToNeighborSeams(block, direction, proposedCut) {
    if (block?.macroVisibleFlattened === true && this.macroStructureFrozen) return proposedCut;
    if (!this._shouldSnapSmallPieceCluster(block)) return proposedCut;
    const eps = SimpleQuiltEngine.MERGE_EPS_PX * 2;
    const bx = Number(block.x);
    const by = Number(block.y);
    const bw = Number(block.width);
    const bh = Number(block.height);
    const lo = direction === 'horizontal' ? by : bx;
    const hi = direction === 'horizontal' ? by + bh : bx + bw;
    const candidates = [proposedCut];
    for (const other of this.blocks) {
      if (!other || other.id === block.id) continue;
      if (
        this._compositionColumnsFullHeight === true &&
        direction === 'horizontal' &&
        !this._blocksShareSameMacroColumn(block, other)
      ) {
        continue;
      }
      if (!this._blockIsSmallPiece(other)) continue;
      if (!this._sharesAxisAlignedFullEdge(block, other, eps)) continue;
      const seamCoords =
        direction === 'horizontal'
          ? [Number(other.y), Number(other.y) + Number(other.height)]
          : [Number(other.x), Number(other.x) + Number(other.width)];
      seamCoords.forEach((c) => {
        if (Number.isFinite(c) && c > lo + eps && c < hi - eps) candidates.push(c);
      });
    }
    let best = proposedCut;
    let bestDist = SimpleQuiltEngine.SMALL_PIECE_SNAP_CUT_TOL;
    for (const c of candidates) {
      const d = Math.abs(c - proposedCut);
      if (d < bestDist) {
        bestDist = d;
        best = c;
      }
  ...
  _weldSplitSiblingWorldPieces(worldA, worldB, seam) {
    const weldEps = SimpleQuiltEngine.POLYGON_VERTEX_QUANTIZE_STEP * 1.25;
    const weldPoint = (p) => {
      const x = Number(p[0]);
      const y = Number(p[1]);
      if (!Number.isFinite(x) || !Number.isFinite(y)) return p;
      if (seam?.mode === 'horizontal') {
        return Math.abs(y - seam.cut) <= weldEps ? [x, seam.cut] : [x, y];
      }
      if (seam?.mode === 'vertical') {
        return Math.abs(x - seam.cut) <= weldEps ? [seam.cut, y] : [x, y];
      }
      if (seam?.mode === 'plane') {
        const d = x * seam.nx + y * seam.ny - seam.cut;
        if (Math.abs(d) <= weldEps) {
          return [x - d * seam.nx, y - d * seam.ny];
        }
      }
      return [x, y];
    };
    const weldPieces = (pieces) =>
      (pieces || []).map((piece) => ({
        color: piece.color,
        points: (piece.points || []).map(weldPoint)
      }));
    return [weldPieces(worldA), weldPieces(worldB)];
  }

  _finalizeSplitSiblingWorldPieces(worldA, worldB, seam) {
    const [weldedA, weldedB] = this._weldSplitSiblingWorldPieces(worldA, worldB, seam);
    return [
      this._finalizeMacroPolygonWorldPieces(weldedA),
      this._finalizeMacroPolygonWorldPieces(weldedB)
    ];
  }

  _harmonyReferenceHexForBlock(block) {
    const id = block?.id != null ? String(block.id) : '';
    if (id && this._harmonyRetroReferenceColors instanceof Map && this._harmonyRetroReferenceColors.has(id)) {
      return this._harmonyRetroReferenceColors.get(id);
    }
    return typeof block?.color === 'string' ? block.color : '';
  }
  ...
  _harmonyNeighborHexes(selectedBlock) {
    ...
    for (const b of this.blocks) {
      ...
      const ref = this._harmonyReferenceHexForBlock(b);
      if (!ref || typeof ref !== 'string' || !ref.match(/^#[0-9A-Fa-f]{6}$/)) continue;
  ...
  _sampledQuiltHarmonyOklab(maxN) {
    ...
    for (let i = 0; i < blocks.length && list.length < cap; i += step) {
      const h = this._harmonyReferenceHexForBlock(blocks[i]);
  ...
  _harmonyRampStrengthForBlockCount(n) {
  ...
  _harmonyPreviewActive() {
    const override = SimpleQuiltEngine.harmonyBlendUrlOverride();
    return override != null && override !== 0;
  }

  _harmonyBlendStrength() {
    if (typeof this._harmonyStrengthOverride === 'number' && Number.isFinite(this._harmonyStrengthOverride)) {
      return Math.max(0, Math.min(1, this._harmonyStrengthOverride));
    }
    const override = SimpleQuiltEngine.harmonyBlendUrlOverride();
    if (override == null) return 0;
    if (override === 0) return 0;
    if (override === 'ramp') {
      const ramp = this._harmonyRampStrengthForBlockCount(this.blocks.length);
      // URL preview: before the 45-block ramp starts, use full cap so early quilts still show the effect.
      return ramp > 0 ? ramp : SimpleQuiltEngine.HARMONY_BLEND_MAX;
    }
    if (typeof override === 'number') return override;
    return 0;
  }

  _shouldApplyHarmonyOnSplit(selectedBlock) {
    if (this._harmonyPreviewActive()) return true;
    if (SimpleQuiltEngine.harmonyIgnoreFreezeEnabled()) return true;
    return !(this.macroStructureFrozen && selectedBlock?.macroRegionId != null);
  }
  ...
  _harmonyRetroEligibleBlock(block) {
    if (!block) return false;
    if (SimpleQuiltEngine.hasPersistedInsetCircleGeometry(block)) return false;
    const raw = block.color;
    if (typeof raw !== 'string' || !raw.match(/^#[0-9A-Fa-f]{6}$/)) return false;
    const paper = SimpleQuiltEngine.getEmptyDayPlaceholderColorHex();
    if (this._hexSame(raw, paper) || this._hexSame(raw, '#ebe8e3')) return false;
    return true;
  }

  harmonyRetroOriginalColor(blockId) {
    const id = blockId != null ? String(blockId) : '';
    if (!id || !this._harmonyRetroOriginalColors) return null;
    return this._harmonyRetroOriginalColors.get(id) || null;
  }

  /** Firestore-bound raw color before harmony preview (if any). */
  _blockPersistedRawColor(block) {
    if (!block) return '';
    const id = String(block.id || '');
    if (this._harmonyRetroOriginalColors?.has(id)) {
      return this._harmonyRetroOriginalColors.get(id) || '';
    }
    return typeof block.color === 'string' && block.color.match(/^#[0-9A-Fa-f]{6}$/) ? block.color : '';
  }

  /** Keep persisted raw colors aligned when paint packets swap (group/settle). */
  _swapHarmonyRetroPersistedColors(a, b, rawA, rawB) {
    if (!a || !b) return;
    const eligA = this._harmonyRetroEligibleBlock(a);
    const eligB = this._harmonyRetroEligibleBlock(b);
    if (!eligA && !eligB) return;
    if (!this._harmonyRetroOriginalColors) this._harmonyRetroOriginalColors = new Map();
    const idA = String(a.id);
    const idB = String(b.id);
    if (eligA) {
      if (rawB) this._harmonyRetroOriginalColors.set(idA, rawB);
      else this._harmonyRetroOriginalColors.delete(idA);
    }
    if (eligB) {
      if (rawA) this._harmonyRetroOriginalColors.set(idB, rawA);
      else this._harmonyRetroOriginalColors.delete(idB);
    }
  }

  /** Clone blocks with harmony applied at explicit strength (0 = unchanged). For labs / export scripts. */
  withHarmonyPreviewBlocks(strength) {
    const cloned = JSON.parse(JSON.stringify(this.blocks));
    if (!Array.isArray(cloned) || !cloned.length) return cloned;
    const strengthClamped = Math.max(0, Math.min(1, Number(strength) || 0));
    if (strengthClamped <= 0) return cloned;
    const probe = new SimpleQuiltEngine(`${this.deviceId || 'harmony'}_preview`, { recordColorReplayEvents: false });
    probe.blocks = cloned;
    probe.submissionCount = this.submissionCount;
    probe.macroStructureFrozen = this.macroStructureFrozen;
    probe._harmonyStrengthOverride = strengthClamped;
    probe._harmonyRetroReferenceColors = new Map();
    for (const block of cloned) {
      if (!probe._harmonyRetroEligibleBlock(block)) continue;
      probe._harmonyRetroReferenceColors.set(String(block.id), block.color);
    }
    for (const block of probe.blocks) {
      if (!probe._harmonyRetroEligibleBlock(block)) continue;
      const raw = probe._harmonyRetroReferenceColors.get(String(block.id)) || block.color;
      block.color = probe._harmonyAdjustedSplitColor(block, raw);
    }
    probe._harmonyRetroReferenceColors = null;
    return probe.blocks;
  }

  _chromaDampenEligibleBlock(block) {
    return this._harmonyRetroEligibleBlock(block);
  }

  _oklabChromaForHex(hex) {
    const profile = this._oklabHueProfileForHex(hex);
    return profile?.chroma ?? 0;
  }
  ...
  _quiltChromaPercentiles(referenceColors) {
    const chromas = [];
    for (const block of this.blocks) {
      if (!this._chromaDampenEligibleBlock(block)) continue;
      const raw =
        referenceColors?.get(String(block.id)) ||
        (typeof block.contributorColor === 'string' ? block.contributorColor : block.color);
      if (typeof raw !== 'string' || !raw.match(/^#[0-9A-Fa-f]{6}$/)) continue;
      chromas.push(this._oklabChromaForHex(raw));
    }
    chromas.sort((a, b) => a - b);
    return {
      p50: this._percentile(chromas, 0.5),
      p75: this._percentile(chromas, 0.75),
      p90: this._percentile(chromas, 0.9)
    };
  }

  _chromaTargetForBlock(block, referenceColors) {
    const eps = SimpleQuiltEngine.MERGE_EPS_PX * 2;
    const neighborChromas = [];
    for (const other of this.blocks) {
      if (!other || other.id === block.id) continue;
      const ref =
        referenceColors?.get(String(other.id)) ||
        (typeof other.contributorColor === 'string' ? other.contributorColor : other.color);
      if (typeof ref !== 'string' || !ref.match(/^#[0-9A-Fa-f]{6}$/)) continue;
      if (!this._sharesAxisAlignedFullEdge(block, other, eps)) continue;
      neighborChromas.push(this._oklabChromaForHex(ref));
    }
    if (neighborChromas.length) {
      neighborChromas.sort((a, b) => a - b);
      return this._percentile(neighborChromas, 0.5);
    }
    const stats = this._quiltChromaPercentiles(referenceColors);
    return stats.p75;
  }

  _quiltLightnessPercentiles(referenceColors) {
    const ls = [];
    for (const block of this.blocks) {
      if (!this._chromaDampenEligibleBlock(block)) continue;
      const raw =
        ...
        (typeof block.contributorColor === 'string' ? block.contributorColor : block.color);
      if (typeof raw !== 'string' || !raw.match(/^#[0-9A-Fa-f]{6}$/)) continue;
      ls.push(this._oklabLightnessForHex(raw));
    }
    ls.sort((a, b) => a - b);
    return {
      p50: this._percentile(ls, 0.5),
      p75: this._percentile(ls, 0.75),
      p90: this._percentile(ls, 0.9)
    };
  }

  _lightnessTargetForBlock(block, referenceColors) {
    const eps = SimpleQuiltEngine.MERGE_EPS_PX * 2;
    const neighborLs = [];
    for (const other of this.blocks) {
      if (!other || other.id === block.id) continue;
      ...
      if (typeof ref !== 'string' || !ref.match(/^#[0-9A-Fa-f]{6}$/)) continue;
      if (!this._sharesAxisAlignedFullEdge(block, other, eps)) continue;
      neighborLs.push(this._oklabLightnessForHex(ref));
    }
    if (neighborLs.length) {
      neighborLs.sort((a, b) => a - b);
      return this._percentile(neighborLs, 0.5);
    }
    const stats = this._quiltLightnessPercentiles(referenceColors);
    return stats.p50;
  }

  /**
   * Chroma-only OKLab dampen — hue + lightness preserved. strengthScale 0–1 scales per-block w.
   * For preview scripts; production will use contributorColor as immutable source.
   */
  _chromaDampenOklab(block, lab, strengthScale, stats, referenceColors) {
    const scale = Math.max(0, Math.min(1, Number(strengthScale) || 0));
    if (scale <= 0) return lab;
    const chroma = Math.sqrt(lab.a * lab.a + lab.b * lab.b);
    if (chroma < 1e-6) return lab;
    const hue = Math.atan2(lab.b, lab.a);
    const p75 = stats?.p75 ?? 0;
    const p90 = Math.max(stats?.p90 ?? 0, p75 + 1e-4);
    const outlier = Math.max(0, Math.min(1, (chroma - p75) / (p90 - p75 + 1e-4)));
    const w = outlier * SimpleQuiltEngine.CHROMA_DAMPEN_MAX * scale;
    if (w <= 0) return lab;
    const target = this._chromaTargetForBlock(block, referenceColors);
    const floor = Math.max(0, p75 * 0.7);
    let cOut = chroma * (1 - w) + target * w;
    cOut = Math.max(floor, Math.min(chroma, cOut));
    if (Math.abs(cOut - chroma) < 1e-5) return lab;
    return { l: lab.l, a: cOut * Math.cos(hue), b: cOut * Math.sin(hue) };
  }

  /**
   * Chroma + lightness OKLab dampen — hue preserved; L only darkens toward neighbor/quilt median.
   */
  _cohesionDampenOklab(block, lab, strengthScale, chromaStats, lightnessStats, referenceColors) {
    let out = this._chromaDampenOklab(block, lab, strengthScale, chromaStats, referenceColors);
    const scale = Math.max(0, Math.min(1, Number(strengthScale) || 0));
    if (scale <= 0) return out;
    const p75L = lightnessStats?.p75 ?? 0;
    const p90L = Math.max(lightnessStats?.p90 ?? 0, p75L + 1e-4);
    const lOutlier = Math.max(0, Math.min(1, (out.l - p75L) / (p90L - p75L + 1e-4)));
    if (lOutlier <= 0) return out;
    const targetL = this._lightnessTargetForBlock(block, referenceColors);
    if (out.l <= targetL) return out;
    const excess = out.l - targetL;
    const wL = lOutlier * SimpleQuiltEngine.LIGHTNESS_DAMPEN_MAX * scale;
    const lOut = out.l - excess * wL;
    if (Math.abs(lOut - out.l) < 1e-5) return out;
    return { l: lOut, a: out.a, b: out.b };
  }

  _chromaDampenHex(block, rawHex, strengthScale, stats, referenceColors) {
    if (typeof rawHex !== 'string' || !rawHex.match(/^#[0-9A-Fa-f]{6}$/)) return rawHex;
    const scale = Math.max(0, Math.min(1, Number(strengthScale) || 0));
    if (scale <= 0) return rawHex;
    const lab = this.hexToOklab(rawHex);
    const out = this._chromaDampenOklab(block, lab, scale, stats, referenceColors);
    const chromaIn = Math.sqrt(lab.a * lab.a + lab.b * lab.b);
    const chromaOut = Math.sqrt(out.a * out.a + out.b * out.b);
    if (Math.abs(chromaOut - chromaIn) < 1e-5) return rawHex;
    return this.oklabToHex(out.l, out.a, out.b);
  }

  _cohesionDampenHex(block, rawHex, strengthScale, chromaStats, lightnessStats, referenceColors) {
    if (typeof rawHex !== 'string' || !rawHex.match(/^#[0-9A-Fa-f]{6}$/)) return rawHex;
    const scale = Math.max(0, Math.min(1, Number(strengthScale) || 0));
    if (scale <= 0) return rawHex;
    const lab = this.hexToOklab(rawHex);
    const out = this._cohesionDampenOklab(block, lab, scale, chromaStats, lightnessStats, referenceColors);
    if (
      Math.abs(out.l - lab.l) < 1e-5 &&
      Math.abs(out.a - lab.a) < 1e-5 &&
      Math.abs(out.b - lab.b) < 1e-5
    ) {
      return rawHex;
    }
    return this.oklabToHex(out.l, out.a, out.b);
  ...
  /** Clone blocks with chroma dampen at strengthScale (0 = original, 1 = full per-block dampen). */
  withChromaDampenPreviewBlocks(strengthScale) {
    const cloned = JSON.parse(JSON.stringify(this.blocks));
    if (!Array.isArray(cloned) || !cloned.length) return cloned;
    const scale = Math.max(0, Math.min(1, Number(strengthScale) || 0));
    if (scale <= 0) return cloned;
    const probe = new SimpleQuiltEngine(`${this.deviceId || 'chroma'}_preview`, { recordColorReplayEvents: false });
    probe.blocks = cloned;
    probe.submissionCount = this.submissionCount;
    probe.macroStructureFrozen = this.macroStructureFrozen;
    const referenceColors = new Map();
    for (const block of cloned) {
      if (!probe._chromaDampenEligibleBlock(block)) continue;
      const raw = typeof block.color === 'string' ? block.color : '';
      referenceColors.set(String(block.id), raw);
    }
    const stats = probe._quiltChromaPercentiles(referenceColors);
    for (const block of probe.blocks) {
      if (!probe._chromaDampenEligibleBlock(block)) continue;
      const raw = referenceColors.get(String(block.id)) || block.color;
      block.color = probe._chromaDampenHex(block, raw, scale, stats, referenceColors);
    }
    return probe.blocks;
  }

  /** Chroma + lightness dampen (hue preserved; L only darkens bright outliers). */
  withCohesionDampenPreviewBlocks(strengthScale) {
    const cloned = JSON.parse(JSON.stringify(this.blocks));
    if (!Array.isArray(cloned) || !cloned.length) return cloned;
    const scale = Math.max(0, Math.min(1, Number(strengthScale) || 0));
    if (scale <= 0) return cloned;
    const probe = new SimpleQuiltEngine(`${this.deviceId || 'cohesion'}_preview`, { recordColorReplayEvents: false });
    probe.blocks = cloned;
    probe.submissionCount = this.submissionCount;
    probe.macroStructureFrozen = this.macroStructureFrozen;
    const referenceColors = new Map();
    ...
    const chromaStats = probe._quiltChromaPercentiles(referenceColors);
    const lightnessStats = probe._quiltLightnessPercentiles(referenceColors);
    for (const block of probe.blocks) {
      if (!probe._chromaDampenEligibleBlock(block)) continue;
      const raw = referenceColors.get(String(block.id)) || block.color;
      block.color = probe._cohesionDampenHex(
        block,
        raw,
        scale,
        chromaStats,
  ...
  /** Deferred production preview: cohesion dampen @ strength only on chroma>p90 eligible blocks. */
  withDeferredCohesionPreviewBlocks(strengthScale = 1) {
    const cloned = JSON.parse(JSON.stringify(this.blocks));
    if (!Array.isArray(cloned) || !cloned.length) return cloned;
    const scale = Math.max(0, Math.min(1, Number(strengthScale) || 0));
    if (scale <= 0) return cloned;
    const probe = new SimpleQuiltEngine(`${this.deviceId || 'deferred'}_preview`, { recordColorReplayEvents: false });
    probe.blocks = cloned;
    probe.submissionCount = this.submissionCount;
    probe.macroStructureFrozen = this.macroStructureFrozen;
    const referenceColors = new Map();
    for (const block of cloned) {
      if (!probe._chromaDampenEligibleBlock(block)) continue;
      referenceColors.set(String(block.id), block.color);
    }
    const chromaStats = probe._quiltChromaPercentiles(referenceColors);
    const lightnessStats = probe._quiltLightnessPercentiles(referenceColors);
    const p90 = chromaStats.p90;
    for (const block of probe.blocks) {
      if (!probe._chromaDampenEligibleBlock(block)) continue;
      const raw = referenceColors.get(String(block.id)) || block.color;
      if (probe._oklabChromaForHex(raw) <= p90) continue;
      block.color = probe._cohesionDampenHex(
        block,
        raw,
        scale,
  ...
  /**
   * Seam soften preview: nudge both sides of harsh borders toward OKLab midpoint.
   * threshold = min border ΔE; strength = 0–1 pull toward midpoint.
   */
  _applySeamSoftenInPlace(threshold = SimpleQuiltEngine.SEAM_SOFTEN_THRESHOLD, strength = SimpleQuiltEngine.SEAM_SOFTEN_STRENGTH) {
    const thr = Math.max(0, Number(threshold) || 0);
    const str = Math.max(0, Math.min(1, Number(strength) || 0));
    if (thr <= 0 || str <= 0 || !Array.isArray(this.blocks) || !this.blocks.length) return 0;
    const eps = SimpleQuiltEngine.MERGE_EPS_PX * 2;
    const nudgeLabs = new Map();
    const addNudgeLab = (id, lab) => {
      const cur = nudgeLabs.get(id);
      if (!cur) nudgeLabs.set(id, { l: lab.l, a: lab.a, b: lab.b, n: 1 });
      else {
        cur.l += lab.l;
        cur.a += lab.a;
        cur.b += lab.b;
    ...
    for (const block of this.blocks) {
      for (const other of this.blocks) {
        if (!other || other.id === block.id || block.id >= other.id) continue;
        if (!this._sharesAxisAlignedFullEdge(block, other, eps)) continue;
        const aLab = this.hexToOklab(block.color);
        const bLab = this.hexToOklab(other.color);
        const dE = Math.sqrt(
          (aLab.l - bLab.l) ** 2 + (aLab.a - bLab.a) ** 2 + (aLab.b - bLab.b) ** 2
        );
        if (dE < thr) continue;
        const mid = { l: (aLab.l + bLab.l) / 2, a: (aLab.a + bLab.a) / 2, b: (aLab.b + bLab.b) / 2 };
        addNudgeLab(block.id, {
          l: aLab.l + (mid.l - aLab.l) * str,
          a: aLab.a + (mid.a - aLab.a) * str,
          b: aLab.b + (mid.b - aLab.b) * str
        });
        addNudgeLab(other.id, {
          l: bLab.l + (mid.l - bLab.l) * str,
          a: bLab.a + (mid.a - bLab.a) * str,
          b: bLab.b + (mid.b - bLab.b) * str
        });
      }
    }
    let changed = 0;
    for (const block of this.blocks) {
      const acc = nudgeLabs.get(block.id);
      if (!acc) continue;
      const n = acc.n || 1;
      const next = this.oklabToHex(acc.l / n, acc.a / n, acc.b / n);
      if (next !== block.color) changed++;
      block.color = next;
    }
    return changed;
  }

  _buildCohesionReferenceColors() {
    const referenceColors = new Map();
    for (const block of this.blocks) {
      if (!this._chromaDampenEligibleBlock(block)) continue;
      const raw =
  ...
  /** Apply cohesion dampen to blocks flagged cohesionPending; uses contributorColor as source. */
  _applyDeferredCohesionPassInPlace(strengthScale = SimpleQuiltEngine.COHESION_DEFERRED_STRENGTH) {
    const pending = this.blocks.filter((b) => b && b.cohesionPending === true && this._chromaDampenEligibleBlock(b));
    if (!pending.length) return 0;
    const scale = Math.max(0, Math.min(1, Number(strengthScale) || 0));
    if (scale <= 0) return 0;
    const referenceColors = this._buildCohesionReferenceColors();
    const chromaStats = this._quiltChromaPercentiles(referenceColors);
    const lightnessStats = this._quiltLightnessPercentiles(referenceColors);
    let changed = 0;
    for (const block of pending) {
      const raw =
        typeof block.contributorColor === 'string' && block.contributorColor.match(/^#[0-9A-Fa-f]{6}$/)
          ? block.contributorColor
          : block.color;
      const out = this._cohesionDampenHex(block, raw, scale, chromaStats, lightnessStats, referenceColors);
      if (out !== block.color) changed++;
      block.color = out;
      block.cohesionPending = false;
    }
    return changed;
  }

  /** Before each add: dampen prior pending outliers, then soften harsh seams. */
  runPendingCohesionPasses() {
    const deferredChanged = this._applyDeferredCohesionPassInPlace();
    const seamChanged = this._applySeamSoftenInPlace();
    return { deferredChanged, seamChanged };
  }

  /** Render / export: exact contributor pick when stored, else quilt block color. */
  static displayColorForBlock(block) {
    if (!block) return '';
    const contributor = block.contributorColor;
    if (typeof contributor === 'string' && contributor.match(/^#[0-9A-Fa-f]{6}$/)) {
      return contributor;
    }
    const color = block.color;
    return typeof color === 'string' && color.match(/^#[0-9A-Fa-f]{6}$/) ? color : '';
  }

  /**
   * Exact contributor pick on reveal; flag for deferred cohesion if chroma > quilt p90.
   */
  _stampNewContributorBlock(block, pickedHex) {
    if (!block || typeof pickedHex !== 'string' || !pickedHex.match(/^#[0-9A-Fa-f]{6}$/)) return;
    block.contributorColor = pickedHex;
    if (!this._shouldPreserveFieldColorOnContributorStamp(block, pickedHex)) {
      block.color = pickedHex;
    }
    block.cohesionPending = false;
    if (!this._chromaDampenEligibleBlock(block)) return;
    const referenceColors = this._buildCohesionReferenceColors();
    referenceColors.set(String(block.id), pickedHex);
    const stats = this._quiltChromaPercentiles(referenceColors);
    if (this._oklabChromaForHex(pickedHex) > stats.p90) {
      block.cohesionPending = true;
    }
  }

  withSeamSoftenPreviewBlocks(threshold = SimpleQuiltEngine.SEAM_SOFTEN_THRESHOLD, strength = SimpleQuiltEngine.SEAM_SOFTEN_STRENGTH) {
    const cloned = JSON.parse(JSON.stringify(this.blocks));
    if (!Array.isArray(cloned) || !cloned.length) return cloned;
    const probe = new SimpleQuiltEngine(`${this.deviceId || 'seam'}_preview`, { recordColorReplayEvents: false });
    probe.blocks = cloned;
    probe.submissionCount = this.submissionCount;
    probe.macroStructureFrozen = this.macroStructureFrozen;
    probe._applySeamSoftenInPlace(threshold, strength);
    return probe.blocks;
  }

  /** Preview loaded quilts without adding colors; does not write to Firestore (see saveQuilt). */
  applyHarmonyRetroPreviewPass() {
    if (!SimpleQuiltEngine.harmonyRetroOnLoadEnabled()) {
      return { changed: 0, skipped: true };
    }
    if (this._harmonyBlendStrength() <= 0) {
      return { changed: 0, skipped: true };
    }
    this._harmonyRetroOriginalColors = new Map();
    this._harmonyRetroReferenceColors = new Map();
    let changed = 0;
    for (const block of this.blocks) {
      if (!this._harmonyRetroEligibleBlock(block)) continue;
      this._harmonyRetroReferenceColors.set(String(block.id), block.color);
    }
    for (const block of this.blocks) {
      if (!this._harmonyRetroEligibleBlock(block)) continue;
      const raw = this._harmonyRetroReferenceColors.get(String(block.id)) || block.color;
      const adjusted = this._harmonyAdjustedSplitColor(block, raw);
      if (this._hexSame(raw, adjusted)) continue;
      this._harmonyRetroOriginalColors.set(String(block.id), raw);
      block.color = adjusted;
      changed++;
    }
    this._harmonyRetroReferenceColors = null;
    if (changed > 0) {
      console.log('🎨 harmonyRetro preview on loaded quilt', {
        changed,
        total: this.blocks.length,
        strength: this._harmonyBlendStrength()
      });
    }
  ...
  _blockStripAspect(block) {
    const w = Math.max(1, Number(block?.width) || 0);
    const h = Math.max(1, Number(block?.height) || 0);
    return Math.max(w, h) / Math.min(w, h);
  }

  /** True when newColor fits the block + edge neighbors (intentional tonal salami stack). */
  _salamiStackColorCohesive(block, newColor) {
    if (typeof newColor !== 'string' || !newColor.match(/^#[0-9A-Fa-f]{6}$/)) return false;
    const refs = new Set();
    if (typeof block?.color === 'string' && block.color.match(/^#[0-9A-Fa-f]{6}$/)) refs.add(block.color);
    const anchor = this._macroRegionAnchorColor(block) || block?.macroFrozenColor;
    if (typeof anchor === 'string' && anchor.match(/^#[0-9A-Fa-f]{6}$/)) refs.add(anchor);
    const eps = SimpleQuiltEngine.MERGE_EPS_PX;
    for (const other of this.blocks) {
      if (!other || other.id === block.id) continue;
      if (!this._sharesAxisAlignedFullEdge(block, other, eps)) continue;
      if (typeof other.color === 'string' && other.color.match(/^#[0-9A-Fa-f]{6}$/)) refs.add(other.color);
    }
    if (!refs.size) return false;
    let maxSim = 0;
    refs.forEach((ref) => {
      maxSim = Math.max(maxSim, this.getColorSimilarity(ref, newColor));
    });
    return maxSim >= SimpleQuiltEngine.SALAMI_COHESIVE_COLOR_SIM_MIN;
  }

  /**
   * Prefer flipping the cut axis after repeated parallel splits when colors clash (anti “salami”).
   * Cohesive same-family stacks may keep banding. Returns { direction, flipped, avoidAccent }.
   */
  _chooseRegularSplitDirection(block, minBlockSize = 80, newColor) {
    const w = Number(block?.width) || 0;
    const h = Number(block?.height) || 0;
    let preferred = w > h ? 'vertical' : 'horizontal';
    if (typeof this._compositionSplitDirection === 'function') {
      const forced = this._compositionSplitDirection(block, newColor);
      if (forced === 'horizontal' || forced === 'vertical') {
        preferred = forced;
        if (this._compositionColumnsFullHeight === true) {
          return {
            direction: preferred,
            flipped: false,
            avoidAccent: false,
            colorCohesive: this._salamiStackColorCohesive(block, newColor)
          };
        }
      }
    }
    const alternate = preferred === 'horizontal' ? 'vertical' : 'horizontal';
    const crossSpan = preferred === 'horizontal' ? w : h;
    const canFlip = crossSpan >= minBlockSize * 2;
    const streak = Math.max(0, Number(block?.sameAxisSplitStreak) || 0);
    const last = block?.lastRegularSplitDirection;
    const sameAsLast = last === preferred;
    const alreadyStrip =
      this._blockStripAspect(block) >= SimpleQuiltEngine.STRIP_ASPECT_FORCE_CROSS_CUT;
    const colorCohesive = this._salamiStackColorCohesive(block, newColor);
    const wantsAntiSalami = !colorCohesive;
    const streakForce =
      wantsAntiSalami && sameAsLast && streak >= SimpleQuiltEngine.SAME_AXIS_SPLIT_STREAK_FORCE_FLIP;
    const stripCrossCut = wantsAntiSalami && canFlip && alreadyStrip && sameAsLast;
    if (stripCrossCut || streakForce) {
      return { direction: alternate, flipped: true, avoidAccent: true, colorCohesive };
    }
    return {
      direction: preferred,
      flipped: false,
      avoidAccent: wantsAntiSalami && alreadyStrip && sameAsLast && streak >= 2,
      colorCohesive
    };
  }

  _stampRegularSplitAxisOnChildren(children, splitDirection, parent) {
    const dir = splitDirection === 'vertical' ? 'vertical' : 'horizontal';
    const prevDir = parent?.lastRegularSplitDirection;
    const prevStreak = Math.max(0, Number(parent?.sameAxisSplitStreak) || 0);
    const streak = prevDir === dir ? prevStreak + 1 : 1;
    (Array.isArray(children) ? children : []).forEach((child) => {
      if (!child || typeof child !== 'object') return;
      child.lastRegularSplitDirection = dir;
      child.sameAxisSplitStreak = streak;
    });
    return children;
  }
  ...
  _stampMacroFrozenMetadataOnLeaves() {
    this.blocks.forEach((block) => {
      ...
      let outline = null;
      if (this.macroLayoutMode === 'columns' && Array.isArray(this._macroColumnBands)) {
        const band = this._macroColumnBands.find((b) => b.regionId === block.macroRegionId);
        if (band) {
          outline = { type: 'rect', x: band.x, y: band.y, width: band.width, height: band.height };
        }
      } else if (this.macroLayoutMode === 'ninepatch' && Array.isArray(this._macroNinePatchCells)) {
        const cell = this._macroNinePatchCells.find((c) => c.regionId === block.macroRegionId);
        if (cell) {
          outline = { type: 'rect', x: cell.x, y: cell.y, width: cell.width, height: cell.height };
        }
      }
      if (!outline) outline = this._buildMacroFrozenOutline(block);
  ...
  /** Field rect + clipped inner disk — same two-layer draw as live inset-circle render. */
  _blockInsetCircleWorldPiecesForMacroFreeze(block) {
    if (!block || !SimpleQuiltEngine.hasPersistedInsetCircleGeometry(block)) return [];
    const pieces = this._blockRectWorldPiece(block);
    if (!pieces.length) return [];
    const innerColor =
      typeof block.insetInnerColor === 'string' && block.insetInnerColor.match(/^#[0-9A-Fa-f]{6}$/)
        ? block.insetInnerColor
        : block.color;
    const insetPointsLocal =
      typeof Utils !== 'undefined' && typeof Utils.insetCircleOrganicSectorPointsLocal === 'function'
        ? Utils.insetCircleOrganicSectorPointsLocal(block, 1)
        : null;
    if (
      insetPointsLocal &&
      insetPointsLocal.kind !== 'none' &&
      Array.isArray(insetPointsLocal.points) &&
      insetPointsLocal.points.length >= 3
    ) {
      const bx = Number(block.x) || 0;
      const by = Number(block.y) || 0;
      pieces.push({
        color: innerColor,
        points: insetPointsLocal.points.map((p) => [bx + Number(p[0]), by + Number(p[1])])
      });
    }
    return pieces.filter((piece) => (piece.points || []).length >= 3);
  }
  ...
  _blockCanonicalWorldPiecesForMacroFreeze(block) {
    ...
    if (block.patternType === 'special' && block.specialPatternType === 'insetCircle') {
      const insetPieces = this._blockInsetCircleWorldPiecesForMacroFreeze(block);
      if (insetPieces.length) return insetPieces;
    }
    ...
    this.blocks = this.blocks
      .map((block) => {
        ...
        const out = {
          ...
          polygonPieces: this._localPolygonPiecesFromWorldPieces(
            this._finalizeMacroPolygonWorldPieces(visible.pieces),
            bounds
          ),
        ...
        if (
          ...
          typeof block.insetInnerColor === 'string' &&
          block.insetInnerColor.match(/^#[0-9A-Fa-f]{6}$/) &&
          !this._hexSame(block.insetInnerColor, out.color)
        ) {
          out.color = block.insetInnerColor;
          if (Array.isArray(out.polygonPieces) && out.polygonPieces.length) {
            out.polygonPieces = out.polygonPieces.map((piece) => ({
              ...piece,
              color: block.insetInnerColor
            }));
          }
        }
  ...
  static _hashDateKeySeed(dateKey) {
  ...
  static macroLayoutModeForDateKey(dateKey) {
    const dk = String(dateKey || '').trim();
    if (!dk) return 'default';
    const seed = SimpleQuiltEngine._hashDateKeySeed(`${dk}:macro-layout`);
    const unit = ((seed >>> 16) % 10000) / 10000;
    return unit < SimpleQuiltEngine.MACRO_COLUMN_LAYOUT_CHANCE ? 'columns' : 'default';
  }

  setQuiltDateKey(dateKey) {
    const dk = String(dateKey || '').trim();
    if (dk) this.quiltDateKey = dk;
  }

  hydrateMacroLayoutFromPersistence(mode) {
    if (mode === 'columns' || mode === 'default' || mode === 'ninepatch') this.macroLayoutMode = mode;
  }

  _seededRngFromHash(seed) {
    let state = seed >>> 0;
    return () => {
  ...
  _macroColumnIdForBlock(block) {
    const bands = this._macroColumnBands;
    if (!Array.isArray(bands) || !bands.length || !block) return null;
    const cx = (Number(block.x) || 0) + (Number(block.width) || 0) / 2;
    for (const band of bands) {
      if (cx >= band.x && cx < band.x + band.width) return band.regionId;
    }
    return bands[bands.length - 1]?.regionId ?? null;
  }

  _blocksShareSameMacroColumn(blockA, blockB) {
    if (!blockA || !blockB) return false;
    const a = this._macroColumnIdForBlock(blockA);
    const b = this._macroColumnIdForBlock(blockB);
    if (a == null || b == null) return true;
    return a === b;
  }

  _computeMacroColumnBands() {
    const dims = Utils.getQuiltDimensions();
    const quiltW = Math.max(1, Number(dims?.width) || 1070);
    const quiltH = Math.max(1, Number(dims?.height) || 1340);
    const seed = SimpleQuiltEngine._hashDateKeySeed(`${this.quiltDateKey || 'default'}:macro-columns`);
    const rng = this._seededRngFromHash(seed);
    const minCols = Math.max(
      2,
      Math.floor(
        Number(this.macroColumnMinCount) ||
          SimpleQuiltEngine.MACRO_COLUMN_MIN_COUNT
      )
    );
    const maxCols = Math.max(
      minCols,
      Math.floor(
        Number(this.macroColumnMaxCount) ||
          SimpleQuiltEngine.MACRO_COLUMN_MAX_COUNT
      )
    );
    const colCount = minCols + Math.floor(rng() * (maxCols - minCols + 1));
    const weights = Array.from({ length: colCount }, () => 0.72 + rng() * 0.56);
    const sum = weights.reduce((acc, value) => acc + value, 0) || 1;
    const widths = weights.map((weight) => (weight / sum) * quiltW);
    const bands = [];
    let x = 0;
    for (let i = 0; i < colCount; i++) {
      bands.push({ regionId: i + 1, x, y: 0, width: widths[i], height: quiltH });
      x += widths[i];
    }
    return bands;
  }

  _computeMacroNinePatchCells() {
    const dims = Utils.getQuiltDimensions();
    const quiltW = Math.max(1, Number(dims?.width) || 1070);
    const quiltH = Math.max(1, Number(dims?.height) || 1340);
    const seed = SimpleQuiltEngine._hashDateKeySeed(`${this.quiltDateKey || 'default'}:macro-ninepatch`);
    const rng = this._seededRngFromHash(seed);
    const colWeights = [0.88 + rng() * 0.24, 0.88 + rng() * 0.24, 0.88 + rng() * 0.24];
    const rowWeights = [0.88 + rng() * 0.24, 0.88 + rng() * 0.24, 0.88 + rng() * 0.24];
    const colSum = colWeights.reduce((a, v) => a + v, 0) || 1;
    const rowSum = rowWeights.reduce((a, v) => a + v, 0) || 1;
    const colWidths = colWeights.map((w) => (w / colSum) * quiltW);
    const rowHeights = rowWeights.map((w) => (w / rowSum) * quiltH);
    const cells = [];
    let regionId = 1;
    let y = 0;
  ...
  _assignNinePatchMacroRegionIdsToLeaves() {
    const cells = this._computeMacroNinePatchCells();
    this._macroNinePatchCells = cells;
    this.blocks.forEach((block) => {
      if (!block) return;
      const cx = (Number(block.x) || 0) + (Number(block.width) || 0) / 2;
      const cy = (Number(block.y) || 0) + (Number(block.height) || 0) / 2;
      let regionId = cells[cells.length - 1]?.regionId || 9;
      for (const cell of cells) {
        const x2 = cell.x + cell.width;
        const y2 = cell.y + cell.height;
        if (cx >= cell.x && cx <= x2 && cy >= cell.y && cy <= y2) {
          regionId = cell.regionId;
          break;
        }
      }
      block.macroRegionId = regionId;
    });
  }

  _assignColumnMacroRegionIdsToLeaves() {
    const bands = this._computeMacroColumnBands();
    this._macroColumnBands = bands;
    this.blocks.forEach((block) => {
      if (!block) return;
      const cx = (Number(block.x) || 0) + (Number(block.width) || 0) / 2;
      let regionId = bands[bands.length - 1]?.regionId || 1;
      for (const band of bands) {
        if (cx >= band.x && cx < band.x + band.width) {
  ...
  _resolveMacroLayoutModeForFreeze() {
    if (
      this.macroLayoutMode === 'columns' ||
      this.macroLayoutMode === 'ninepatch' ||
      this.macroLayoutMode === 'default'
    ) {
      return this.macroLayoutMode;
    }
    if (this.quiltDateKey) {
      return SimpleQuiltEngine.macroLayoutModeForDateKey(this.quiltDateKey);
    }
    return 'default';
  }

  _assignMacroRegionIdsToLeaves() {
    this.macroLayoutMode = this._resolveMacroLayoutModeForFreeze();
    if (this.macroLayoutMode === 'columns') {
      this._assignColumnMacroRegionIdsToLeaves();
    } else if (this.macroLayoutMode === 'ninepatch') {
      this._macroColumnBands = null;
      this._assignNinePatchMacroRegionIdsToLeaves();
    } else {
      this._macroColumnBands = null;
      this._macroNinePatchCells = null;
      this._assignSortedMacroRegionIdsToLeaves();
    }
  }

  _macroFreezeAtBlockCount() {
    const override = Number(this.macroFreezeAtBlockCountOverride);
    if (Number.isFinite(override) && override >= 2) return Math.floor(override);
    return SimpleQuiltEngine.MACRO_FREEZE_AT_BLOCK_COUNT;
  }

  /**
   * Post-freeze special odds scale: early macro freeze (e.g. 5 blocks) keeps motifs visible;
   * late freeze (25, live default) stays at the legacy 0.32 taper.
   */
  _effectiveMacroFrozenSpecialStructureScale() {
    if (this.macroFrozenSpecialStructureScale != null) {
      const manual = Number(this.macroFrozenSpecialStructureScale);
      if (Number.isFinite(manual) && manual >= 0) return manual;
    }

    const freezeAt = this._macroFreezeAtBlockCount();
    const lo = SimpleQuiltEngine.SPECIAL_STRUCTURE_FROZEN_SCALE;
    const hi = SimpleQuiltEngine.SPECIAL_STRUCTURE_EARLY_FREEZE_SCALE;
    const ref = SimpleQuiltEngine.SPECIAL_STRUCTURE_FREEZE_SCALE_REFERENCE;
    const earlyAt = SimpleQuiltEngine.SPECIAL_STRUCTURE_EARLY_FREEZE_AT_BLOCKS;

    if (freezeAt >= ref) return lo;
    if (freezeAt <= earlyAt) return hi;
    const t = (ref - freezeAt) / (ref - earlyAt);
    return lo + (hi - lo) * t;
  }

  _specialPatternEarlyWindowEnd() {
    const freezeAt = this._macroFreezeAtBlockCount();
    if (freezeAt <= 10) return Math.max(6, freezeAt + 1);
    return 6;
  }

  _specialPatternMidWindowEnd() {
    const freezeAt = this._macroFreezeAtBlockCount();
    if (freezeAt <= 10) return Math.max(24, freezeAt + 19);
    return 24;
  }

  _maxSpecialPatternAreaForBlock(block, quiltArea) {
    const base = quiltArea * SimpleQuiltEngine.SPECIAL_PATTERN_MAX_AREA_FRACTION;
    const blockArea = Math.max(0, Number(block?.width) * Number(block?.height));
    const freezeAt = this._macroFreezeAtBlockCount();
    if (freezeAt <= 10) {
      return Math.max(base, quiltArea * 0.42, blockArea * 1.02);
    }
    return Math.max(base, blockArea * 1.02);
  }
  ...
  _applyMacroStructureFreeze() {
    ...
    this._assignMacroRegionIdsToLeaves();
  ...
  repairMacroRegionIdsAfterLoadIfFrozen() {
    ...
    if (!idsComplete) {
      if (this.macroLayoutMode === 'columns' && this.quiltDateKey) {
        this._assignColumnMacroRegionIdsToLeaves();
      } else if (this.macroLayoutMode === 'ninepatch' && this.quiltDateKey) {
        this._assignNinePatchMacroRegionIdsToLeaves();
      } else {
        this._assignSortedMacroRegionIdsToLeaves();
      }
    } else if (this.macroLayoutMode === 'columns' && this.quiltDateKey) {
      this._assignColumnMacroRegionIdsToLeaves();
    } else if (this.macroLayoutMode === 'ninepatch' && this.quiltDateKey) {
      this._assignNinePatchMacroRegionIdsToLeaves();
    }
  ...
  /** Admin grouping: any macro-region cell with paint (regular, HST, inset, polygons, etc.). */
  _adminColorGroupingEligibleBlock(block) {
    if (!this.macroStructureFrozen || !block || block.macroRegionId == null) return false;
    if (this._isProtectedAnchorBlock(block)) return false;
    const w = Number(block.width) || 0;
    const h = Number(block.height) || 0;
    if (!(w > 0 && h > 0)) return false;
    return !!this._groupingRepresentativeColor(block);
  }

  _groupingHexColor(color) {
    return typeof color === 'string' && color.match(/^#[0-9A-Fa-f]{6}$/) ? color : '';
  }

  _groupingRepresentativeColor(block) {
    if (!block) return '';
    return (
      this._groupingHexColor(block.contributorColor) ||
      this._groupingHexColor(block.color) ||
      this._groupingHexColor(block.insetInnerColor) ||
      this._groupingHexColor(block.hstColorB) ||
      this._groupingHexColor(block.diagonalAxisAccentColor) ||
      ''
    );
  }

  _groupingRepresentativeColorFromPacket(packet) {
    if (!packet) return '';
    return (
      this._groupingHexColor(packet.contributorColor) ||
      this._groupingHexColor(packet.color) ||
      this._groupingHexColor(packet.insetInnerColor) ||
      this._groupingHexColor(packet.hstColorB) ||
      this._groupingHexColor(packet.diagonalAxisAccentColor) ||
      ''
    );
  }

  _groupingPatternKind(block) {
    if (!block) return 'regular';
    if (block.patternType === 'special' && typeof block.specialPatternType === 'string' && block.specialPatternType) {
      return block.specialPatternType;
    }
    if (SimpleQuiltEngine.hasPersistedInsetCircleGeometry(block)) return 'insetCircle';
    return 'regular';
  }

  _groupingMacroAnchorColor(block) {
    return (
      this._groupingHexColor(block?.macroFrozenColor) ||
      this._groupingRepresentativeColor(block) ||
      ''
    );
  }

  /** Only swap like pattern cells whose macro color anchors already match. */
  _groupingSwapCompatibleBlocks(a, b) {
    if (!a || !b || a.id === b.id) return false;
    if (String(a.macroRegionId) !== String(b.macroRegionId)) return false;
    if (this._groupingPatternKind(a) !== this._groupingPatternKind(b)) return false;
    const anchorA = this._groupingMacroAnchorColor(a);
    const anchorB = this._groupingMacroAnchorColor(b);
    if (!anchorA || !anchorB) return false;
    return this.getColorSimilarity(anchorA, anchorB) >= this._macroRegularStrongColorSimilarityMin();
  }

  _groupingEdgeNeighbors(block, eps = SimpleQuiltEngine.MERGE_EPS_PX * 2) {
    if (!block) return [];
    const out = [];
    for (const other of this.blocks) {
      if (!other || other.id === block.id) continue;
  ...
  _groupingEdgeColorFitness(block, candidateColor, eps = SimpleQuiltEngine.MERGE_EPS_PX * 2) {
    if (!block || !candidateColor) return -0.25;
    let score = 0;
    let n = 0;
    for (const other of this._groupingEdgeNeighbors(block, eps)) {
      const otherColor = this._groupingRepresentativeColor(other);
      if (!otherColor) continue;
      score += this.getColorSimilarity(candidateColor, otherColor);
      n++;
    }
    if (n === 0) return -0.25;
    return score / n;
  ...
  _groupingPairSwapGain(a, b, eps = SimpleQuiltEngine.MERGE_EPS_PX * 2) {
    if (!this._groupingSwapCompatibleBlocks(a, b)) return -Infinity;
    const colorA = this._groupingRepresentativeColor(a);
    const colorB = this._groupingRepresentativeColor(b);
    const candidateOnA = this._groupingRepresentativeColorFromPacket(this._groupingPaintPacketForBlock(b));
    if (!colorA || !colorB || !candidateOnA) return -Infinity;
    const before =
      this._groupingEdgeColorFitness(a, colorA, eps) + this._groupingEdgeColorFitness(b, colorB, eps);
    const after =
      this._groupingEdgeColorFitness(a, candidateOnA, eps) + this._groupingEdgeColorFitness(b, colorA, eps);
    return after - before;
  }

  _groupingQuiltEdgeFitness(blocks, eps = SimpleQuiltEngine.MERGE_EPS_PX * 2) {
    const engineBlocks = Array.isArray(blocks) ? blocks : this.blocks;
    let sum = 0;
    let n = 0;
    for (const block of engineBlocks) {
      if (!this._adminColorGroupingEligibleBlock(block)) continue;
      const color = this._groupingRepresentativeColor(block);
      if (!color) continue;
      const neighbors = [];
      for (const other of engineBlocks) {
        if (!other || other.id === block.id) continue;
        if (!this._sharesAxisAlignedFullEdge(block, other, eps)) continue;
      ...
      for (const other of neighbors) {
        const otherColor = this._groupingRepresentativeColor(other);
        if (!otherColor) continue;
        sum += this.getColorSimilarity(color, otherColor);
        n++;
      }
    }
    return n ? sum / n : 0;
  ...
  _cloneGroupingPaintTriangles(tris) {
    if (!Array.isArray(tris)) return undefined;
    return tris.map((tri) => ({
      color: typeof tri?.color === 'string' ? tri.color : undefined,
      points: Array.isArray(tri?.points)
        ? tri.points.map((p) => (Array.isArray(p) ? [Number(p[0]), Number(p[1])] : [Number(p?.x), Number(p?.y)]))
        : []
    }));
  }

  _cloneGroupingPolygonPieces(pieces) {
    if (!Array.isArray(pieces)) return undefined;
    return pieces.map((piece) => ({
      color: typeof piece?.color === 'string' ? piece.color : undefined,
      points: Array.isArray(piece?.points)
        ? piece.points.map((p) => (Array.isArray(p) ? [Number(p[0]), Number(p[1])] : [Number(p?.x), Number(p?.y)]))
        : []
    }));
  }

  _groupingPaintPacketForBlock(block) {
    return {
      color: block.color,
      contributorColor: block.contributorColor,
      contributorId: block.contributorId,
      contributorIds: Array.isArray(block.contributorIds) ? [...block.contributorIds] : undefined,
      submissionIndex: block.submissionIndex,
      hstColorB: block.hstColorB,
      hstTriangles: this._cloneGroupingPaintTriangles(block.hstTriangles),
      insetInnerColor: block.insetInnerColor,
      diagonalAxisAccentColor: block.diagonalAxisAccentColor,
      polygonPieces: this._cloneGroupingPolygonPieces(block.polygonPieces),
      specialOriginalColor: block.specialOriginalColor,
      specialOriginalInnerColor: block.specialOriginalInnerColor
    };
  }

  _applyGroupingPaintPacketToBlock(block, packet) {
    if (!block || !packet) return;
    ...
    if (this._groupingHexColor(packet.contributorColor)) block.contributorColor = packet.contributorColor;
    else delete block.contributorColor;
    if (this._groupingHexColor(packet.hstColorB)) block.hstColorB = packet.hstColorB;
    else delete block.hstColorB;
    if (Array.isArray(packet.hstTriangles) && packet.hstTriangles.length) {
      block.hstTriangles = this._cloneGroupingPaintTriangles(packet.hstTriangles);
    } else {
      delete block.hstTriangles;
    }
    if (this._groupingHexColor(packet.insetInnerColor)) block.insetInnerColor = packet.insetInnerColor;
    else delete block.insetInnerColor;
    if (this._groupingHexColor(packet.diagonalAxisAccentColor)) {
      block.diagonalAxisAccentColor = packet.diagonalAxisAccentColor;
    } else {
      delete block.diagonalAxisAccentColor;
    }
    if (Array.isArray(packet.polygonPieces) && packet.polygonPieces.length) {
      block.polygonPieces = this._cloneGroupingPolygonPieces(packet.polygonPieces);
    } else {
      delete block.polygonPieces;
    }
    if (this._groupingHexColor(packet.specialOriginalColor)) block.specialOriginalColor = packet.specialOriginalColor;
    else delete block.specialOriginalColor;
    if (this._groupingHexColor(packet.specialOriginalInnerColor)) {
      block.specialOriginalInnerColor = packet.specialOriginalInnerColor;
    } else {
      delete block.specialOriginalInnerColor;
    }
  }

  _groupingPaintPacketSignature(packet) {
    return JSON.stringify(packet);
  }

  _groupingBlocksPaintSignature(blocks) {
    return (Array.isArray(blocks) ? blocks : [])
      .map((block) => `${block?.id || ''}:${this._groupingPaintPacketSignature(this._groupingPaintPacketForBlock(block))}`)
      .sort()
      .join('|');
  }

  _countGroupingPaintChanges(beforeById, blocks) {
    let changed = 0;
    for (const block of Array.isArray(blocks) ? blocks : []) {
      const before = beforeById.get(String(block?.id || ''));
      if (!before) continue;
      const after = this._groupingPaintPacketSignature(this._groupingPaintPacketForBlock(block));
  ...
  /**
   * Admin-only: swap paint packets within a macro region when pattern kind and macro
   * anchor colors match, and only if all shared-edge neighbors (including across regions)
   * score better after the swap.
   */
  runAdminColorGroupingPass(options = {}) {
    if (!this.macroStructureFrozen) {
      return { changed: 0, blocksChanged: 0, skipped: true, reason: 'macro-not-frozen' };
    }
    const minGain = Number(options.minGain) > 0 ? Number(options.minGain) : 0.05;
    const maxSweeps = Math.max(1, Math.min(4, Math.floor(Number(options.maxSweeps) || 1)));
    const eps = SimpleQuiltEngine.MERGE_EPS_PX * 2;
    const beforeById = new Map(
      this.blocks.map((block) => [String(block?.id || ''), this._groupingPaintPacketSignature(this._groupingPaintPacketForBlock(block))])
    );
    const initialBlocks = JSON.parse(JSON.stringify(this.blocks));
    const eligibleBlocks = this.blocks.filter((block) => this._adminColorGroupingEligibleBlock(block)).length;
    const fitnessBefore = this._groupingQuiltEdgeFitness(this.blocks, eps);
    let totalSwaps = 0;
    let sweepsRun = 0;

    for (let sweep = 0; sweep < maxSweeps; sweep++) {
      const signatureBeforeSweep = this._groupingBlocksPaintSignature(this.blocks);
      let sweepSwaps = 0;
      const byRegion = new Map();
      this.blocks.forEach((block) => {
        if (!this._adminColorGroupingEligibleBlock(block)) return;
        const key = String(block.macroRegionId);
      ...
      for (const peers of byRegion.values()) {
        ...
        for (let i = 0; i < ordered.length; i++) {
          ...
          for (let j = i + 1; j < ordered.length; j++) {
            const b = ordered[j];
            const gain = this._groupingPairSwapGain(a, b, eps);
            if (gain > bestGain) {
              bestGain = gain;
              best = b;
            }
          ...
          if (!best) continue;
          const aPacket = this._groupingPaintPacketForBlock(a);
          const bPacket = this._groupingPaintPacketForBlock(best);
          if (this._groupingPaintPacketSignature(aPacket) === this._groupingPaintPacketSignature(bPacket)) {
            continue;
          }
          const rawA = this._blockPersistedRawColor(a);
          const rawB = this._blockPersistedRawColor(best);
          this._applyGroupingPaintPacketToBlock(a, bPacket);
          this._applyGroupingPaintPacketToBlock(best, aPacket);
          this._swapHarmonyRetroPersistedColors(a, best, rawA, rawB);
      ...
      sweepsRun += 1;
      if (sweepSwaps === 0) break;
      if (this._groupingBlocksPaintSignature(this.blocks) === signatureBeforeSweep) break;
    }

    const blocksChanged = this._countGroupingPaintChanges(beforeById, this.blocks);
    let fitnessAfter = this._groupingQuiltEdgeFitness(this.blocks, eps);
    let edgeFitnessDelta = fitnessAfter - fitnessBefore;
    let reverted = false;
    if (blocksChanged > 0 && edgeFitnessDelta <= 1e-6) {
    ...
    return {
      changed: reverted ? 0 : totalSwaps,
      blocksChanged: reverted ? 0 : blocksChanged,
      eligibleBlocks,
      skipped: false,
      sweeps: sweepsRun,
      macroRegionsPreserved: true,
      reverted,
      edgeFitnessBefore: fitnessBefore,
      edgeFitnessAfter: fitnessAfter,
      edgeFitnessDelta
    };
  }
  ...
  _resolveDailyMoodColorHex() {
    const explicit = String(this.dailyMoodColorHex || '').trim();
    if (/^#[0-9A-Fa-f]{6}$/.test(explicit)) return explicit.toLowerCase();
    try {
      const fromApp =
        typeof window !== 'undefined' &&
        window.app &&
        typeof window.app._getTodayQuoteMoodHex === 'function'
          ? window.app._getTodayQuoteMoodHex()
          : null;
      if (fromApp && /^#[0-9a-f]{6}$/i.test(fromApp)) return fromApp.toLowerCase();
      const moodLabel = String(
        (typeof globalThis !== 'undefined' && globalThis.__ODQ_TODAY_MOOD_LABEL__) || ''
      ).trim();
      const resolver =
        typeof globalThis !== 'undefined' && globalThis.QuoteMoodColors?.resolveQuoteMoodColor;
      if (moodLabel && typeof resolver === 'function') {
        const hex = resolver(moodLabel);
        if (hex) return hex;
      }
    } catch (_) {
      /* */
    ...
    const fallback =
      (typeof globalThis !== 'undefined' && globalThis.QuoteMoodColors?.MOOD_COLORS?.Tender) ||
      '#cfadf0';
    return String(fallback).toLowerCase();
  }

  /** Human pick count (excludes the automatic daily mood seed as pick 1). */
  getContributorPickCount() {
    const moodSeeded = (this.blocks || []).some(
      (block) => String(block?.contributorId || '') === SimpleQuiltEngine.DAILY_MOOD_CONTRIBUTOR_ID
    );
    const picks = Math.max(0, Number(this.submissionCount) || 0);
    return moodSeeded ? Math.max(0, picks - 1) : picks;
  }

  /**
   * Second pick of the day: split the single full-canvas block (mood field + first human color).
   */
  _applyStarterAxisSplitFromSingleBlock(parent, newColor) {
    if (!parent || this.blocks.length !== 1) return null;
    const parentBeforeAxis = { ...parent };
    const nextSubmission = this.submissionCount + 1;
    this.submissionCount = nextSubmission;
    const axisBlocks = this.createDiagonalAxisPattern(parent, newColor);
    if (
      ...
      const dedicated = axisBlocks[1];
      if (dedicated) this._stampNewContributorBlock(dedicated, newColor);
      this.recordUserContribution(axisBlocks[1].id, newColor, this.submissionCount);
      this._recordColorReplayMutation(newColor, parentBeforeAxis, axisBlocks);
      return {
    ...
    this.submissionCount = Math.max(1, this.submissionCount - 1);
    const vertical = this._compositionColumnsFullHeight === true ? true : Math.random() < 0.5;
    const ratio = this._starterAxisRatio();
    const children = vertical
      ? [
          this._makeStarterAxisChild(parent, { width: parent.width * ratio }, parent.color, nextSubmission, 'a'),
      ...
      : [
          this._makeStarterAxisChild(parent, { height: parent.height * ratio }, parent.color, nextSubmission, 'a'),
          this._makeStarterAxisChild(
            parent,
    ...
    const dedicated = children[1];
    if (dedicated) this._stampNewContributorBlock(dedicated, newColor);
    this.recordUserContribution(children[1].id, newColor, this.submissionCount);
    this._recordColorReplayMutation(newColor, parentBeforeAxis, children);
    return {
      originalBlock: parent,
      newBlocks: children,
  ...
  /**
   * Pick 1 = today's mood color; first human pick splits against that field.
   */
  _tryApplyMoodSeedAndFirstUserSplit(newColor) {
    if (this.macroStructureFrozen) return null;
    if (this.submissionCount !== 0 || this.blocks.length !== 1) return null;
    const moodHex = this._resolveDailyMoodColorHex();
    if (!moodHex || !/^#[0-9A-Fa-f]{6}$/.test(moodHex)) return null;
    ...
    const parentBeforeFill = { ...parent };
    const originalBlock = { ...parent };
    parent.color = moodHex;
    parent.contributorId = SimpleQuiltEngine.DAILY_MOOD_CONTRIBUTOR_ID;
    parent.submissionIndex = 1;
    parent.patternType = parent.patternType || 'regular';
    delete parent.starterAxisRegion;
    delete parent.starterAxisRegionId;
    this.submissionCount = 1;
    this._recordColorReplayMutation(moodHex, parentBeforeFill, [parent]);

    const splitResult = this._applyStarterAxisSplitFromSingleBlock(parent, newColor);
    if (!splitResult) {
      this.blocks = [parentBeforeFill];
      this.submissionCount = 0;
      this.colorReplayEvents = (this.colorReplayEvents || []).slice(0, -1);
      return null;
    }
    return {
      ...splitResult,
  ...
  /**
   * Legacy: first human fill when mood seed is unavailable (admin / tests).
   */
  _tryApplyFirstSubmissionFill(newColor) {
    ...
    delete parent.starterAxisRegionId;
    this.submissionCount = nextSubmission;
    this._stampNewContributorBlock(parent, newColor);
    this.recordUserContribution(parent.id, newColor, nextSubmission);
    this._recordColorReplayMutation(newColor, parentBeforeFill, [parent]);
    return {
      originalBlock,
      newBlocks: [parent],
  ...
  _tryApplySubmissionAxis(newColor) {
    if (this.macroStructureFrozen) return null;
    // First structural axis: second pick of the day (after mood seed or legacy first fill).
    if (this.submissionCount === 1 && this.blocks.length === 1) {
      return this._applyStarterAxisSplitFromSingleBlock(this.blocks[0], newColor);
    }

    // Second structural axis: third pick, angled split on the least-similar half.
    if (this.submissionCount !== 2 || this.blocks.length !== 2) return null;
    ...
    const diagonalTarget = leastSimilarToNewColor(sortedBlocks);
    if (diagonalTarget) {
      const targetBeforeAxis = { ...diagonalTarget };
      const nextSubmission = this.submissionCount + 1;
      this.submissionCount = nextSubmission;
      const axisBlocks = this.createDiagonalAxisPattern(diagonalTarget, newColor);
      if (
        Array.isArray(axisBlocks) &&
        axisBlocks.length === 2 &&
        axisBlocks.every((block) => block?.specialPatternType === 'diagonalAxis')
      ) {
        const nextBlocks = sortedBlocks.flatMap((block) => (block.id === diagonalTarget.id ? axisBlocks : [block]));
        const newContributionBlock =
          axisBlocks.find((block) => block.submissionIndex === nextSubmission && block.color === newColor) ||
          axisBlocks[1];
        this.blocks = nextBlocks;
        if (newContributionBlock) {
          this._stampNewContributorBlock(newContributionBlock, newColor);
          this.recordUserContribution(newContributionBlock.id, newColor, this.submissionCount);
        }
        this._recordColorReplayMutation(newColor, targetBeforeAxis, axisBlocks);
        return {
          originalBlock: diagonalTarget,
    ...
    const nextBlocks = [];
    let replayParent = null;
    let replayChildren = null;
    ...
    if (secondOrientation === 'vertical') {
      ...
      sortedBlocks.forEach((block) => {
        ...
        const childA = this._makeStarterAxisChild(block, { width: w1 }, block.color, nextSubmission, 'a');
        const childB = this._makeStarterAxisChild(
          block,
        ...
        nextBlocks.push(childA, childB);
        if (!replayParent) {
          replayParent = block;
          replayChildren = [childA, childB];
        }
      });
    } else {
      ...
      sortedBlocks.forEach((block) => {
        ...
        const childA = this._makeStarterAxisChild(block, { height: h1 }, block.color, nextSubmission, 'a');
        const childB = this._makeStarterAxisChild(
          block,
    ...
    if (replayParent && replayChildren && newContributionBlock) {
      const owner = sortedBlocks.find((block) => {
        const prefix = `${String(block.id)}_`;
        return String(newContributionBlock.id).startsWith(prefix);
      });
      if (owner && owner.id !== replayParent.id) {
        const prefix = `${String(owner.id)}_`;
        replayParent = owner;
        replayChildren = nextBlocks.filter((block) => String(block.id).startsWith(prefix));
      }
    }
    if (replayParent && replayChildren) {
      this._recordColorReplayMutation(newColor, replayParent, replayChildren);
    }
    return {
  ...
  initialize() {
    const dimensions = Utils.getQuiltDimensions(
      typeof Utils.getTodayKey === 'function' ? Utils.getTodayKey() : null
    );
    ...
    this.submissionCount = 0;
    this.colorReplayEvents = [];
    this.recordColorReplayEvents = true;
    this.macroStructureFrozen = false;
    this.macroLayoutMode = undefined;
    this._macroColumnBands = null;
    this.macroFreezeHstParentsExplodedCount = 0;
  ...
  _recordColorReplayMutation(newColor, parent, children) {
    if (!this.recordColorReplayEvents || !parent || !Array.isArray(children) || !children.length) return;
    this.colorReplayEvents.push({
      seq: this.submissionCount,
      iso: new Date().toISOString(),
      newHex: newColor,
      parent: this._serializeBlockForReplay(parent),
      children: children.map((child) => this._serializeBlockForReplay(child)).filter(Boolean)
    });
  }
  ...
  _serializeBlockForReplay(block) {
    ...
    if (Array.isArray(block.specialParentClipPieces) && block.specialParentClipPieces.length) {
      o.specialParentClipPieces = block.specialParentClipPieces
        .map((piece) => ({
          points: (piece?.points || [])
            .map((p) => [Number(Array.isArray(p) ? p[0] : p?.x), Number(Array.isArray(p) ? p[1] : p?.y)])
            .filter((p) => Number.isFinite(p[0]) && Number.isFinite(p[1]))
        }))
        .filter((piece) => piece.points.length >= 3);
      if (!o.specialParentClipPieces.length) delete o.specialParentClipPieces;
    }
    ...
    if (typeof block.contributorColor === 'string' && block.contributorColor.match(/^#[0-9A-Fa-f]{6}$/)) {
      o.contributorColor = block.contributorColor;
    }
    if (block.cohesionPending === true) o.cohesionPending = true;
    return o;
  }
  ...
  /** Axis-aligned four-sided host — excludes diagonal shards, triangles, and long skinny rects. */
  _blockHasOrthogonalRectOutline(block) {
    if (!Array.isArray(block?.polygonPieces) || !block.polygonPieces.length) return true;
    const world = this._polygonPiecesWorld(block);
    if (!world.length) return false;
    for (const piece of world) {
      ...
      for (let i = 0; i < pts.length; i++) {
        ...
        const dx = Math.abs(Number(p1[0]) - Number(p0[0]));
        const dy = Math.abs(Number(p1[1]) - Number(p0[1]));
  ...
  _builderSpecialPatternCap(maxSpecialPatterns) {
    if (!this._isQuiltBuilderSession()) return maxSpecialPatterns;
    return Math.min(maxSpecialPatterns, 2);
  }

  _isQuiltBuilderSession() {
    return String(this.deviceId || '') === 'quilt-builder';
  }

  _blockMeetsSpecialHostBBoxGeometry(block) {
    if (!block) return false;
    const w = Number(block.width);
    const h = Number(block.height);
    ...
    const maxAspect = SimpleQuiltEngine.SPECIAL_HOST_MAX_ASPECT;
    return (
      short >= SimpleQuiltEngine.SPECIAL_HOST_MIN_SHORT &&
      aspect <= maxAspect &&
      aspect >= 1 / maxAspect
    );
  }

  _blockCanHostNewSpecialPattern(block) {
    if (!block) return false;
    if (
      block.patternType === 'special' &&
    ...
    const id = String(block.id || '');
    if (id.includes('_pattern_')) return false;
    return true;
  }

  _blockMeetsSpecialPatternHostGeometry(block) {
    if (!block) return false;
    if (!this._blockCanHostNewSpecialPattern(block)) return false;
    if (this._isFrozenHstMacroShard(block)) return false;
    if (
      block.patternType === 'special' &&
      block.specialPatternType === 'hst' &&
    ...
    const builderMacroBboxHost =
      this._isQuiltBuilderSession() &&
      block.macroVisibleFlattened === true &&
      this._blockMeetsSpecialHostBBoxGeometry(block);

    if (block.specialPatternType === 'diagonalAxis') {
      const plainRectHost =
        block.patternType === 'regular' &&
        (!Array.isArray(block.polygonPieces) || !block.polygonPieces.length);
      if (plainRectHost) {
        // Rectangular half from an axis split — still a valid 4-sided host.
      } else if (block.macroVisibleFlattened === true && this._blockHasOrthogonalRectOutline(block)) {
        // Frozen macro cell with an orthogonal outline.
      } else if (builderMacroBboxHost) {
        // Builder: square-ish macro bbox even when angled-split outline is slightly off-axis.
      } else {
        return false;
      }
    }

    if (builderMacroBboxHost) return true;
    if (!this._blockHasOrthogonalRectOutline(block)) return false;
    return this._blockMeetsSpecialHostBBoxGeometry(block);
  }
  ...
  _contributionColorMatchScore(block, color) {
    if (!block || typeof color !== 'string') return 0;
    let best = this.getColorSimilarity(block.color, color);
    if (block.specialPatternType === 'hst' && typeof block.hstColorB === 'string') {
      best = Math.max(best, this.getColorSimilarity(block.hstColorB, color));
    }
    if (block.specialPatternType === 'insetCircle' && typeof block.insetInnerColor === 'string') {
      best = Math.max(best, this.getColorSimilarity(block.insetInnerColor, color));
    }
    const hostAnchor =
      typeof block.specialOriginalColor === 'string' && block.specialOriginalColor.match(/^#[0-9A-Fa-f]{6}$/)
        ? block.specialOriginalColor
        : '';
    if (hostAnchor && this.getColorSimilarity(block.color, hostAnchor) > best) {
      best -= 0.35;
    }
    return best;
  }
  ...
  _contributionBlockFromCandidates(blocks, color, submissionIndex) {
    ...
    return [...candidates].sort((a, b) => {
      const scoreA = this._contributionColorMatchScore(a, color);
      const scoreB = this._contributionColorMatchScore(b, color);
      if (Math.abs(scoreB - scoreA) > 1e-6) return scoreB - scoreA;
      return (b.width * b.height) - (a.width * a.height);
    })[0];
  }

  /** HST / inset / host-field pattern cells keep block.color; only plain patches take the pick. */
  _shouldPreserveFieldColorOnContributorStamp(block, pickedHex) {
    if (!block) return false;
    if (block.specialPatternType === 'hst' || block.specialPatternType === 'insetCircle') return true;
    const hostAnchor =
      typeof block.specialOriginalColor === 'string' && block.specialOriginalColor.match(/^#[0-9A-Fa-f]{6}$/)
        ? block.specialOriginalColor
        : '';
    ...
    return (
      this.getColorSimilarity(block.color, hostAnchor) >= 0.9 &&
      !this._hexSame(block.color, pickedHex)
    );
  }
  ...
  addColor(newColor, retryDepth = 0, rejectedSplitIds = null) {
    ...
    this.runPendingCohesionPasses();

    const moodFirstResult = this._tryApplyMoodSeedAndFirstUserSplit(newColor);
    if (moodFirstResult) {
      return moodFirstResult;
    }
    ...
    const splittableBeforeMacroRoute = splittableBlocks.slice();
    ...
    const structureScale =
      (forceOversizedSplit
        ? 0
        : this.macroStructureFrozen
          ? this._effectiveMacroFrozenSpecialStructureScale()
          : this._growthSpecialStructureScale()) *
      Math.max(0, Number(this.specialStructureScaleMultiplier) || 0) *
      (this._isQuiltBuilderSession() ? 0.75 : 1);
    ...
    if (!selectedBlock) {
      // Special rules for pattern creation (windows scale with early macro freeze):
      // 1) Early window: high odds, no back-to-back specials
      // 2) Mid window: moderate odds unless blocked by the special-pattern cap
      ...
      this.blocks.forEach(block => {
        ...
        if (block.patternType === 'special' && block.specialPatternType) {
          uniquePatternIds.add(block.originalPatternId || block.id);
      ...
      const specialPatternsCreated = uniquePatternIds.size;
      const capFrac = SimpleQuiltEngine.SPECIAL_PATTERN_CAP_FRACTION;
      const maxSpecialPatterns = this._builderSpecialPatternCap(
        Math.min(Math.max(2, Math.floor(this.submissionCount * capFrac)), Math.floor(this.submissionCount * 0.75))
      );
      ...
      const nextAddNumber = this.submissionCount + 1;
      const earlyWindowEnd = this._specialPatternEarlyWindowEnd();
      const midWindowEnd = this._specialPatternMidWindowEnd();

      if (nextAddNumber >= 2 && nextAddNumber <= earlyWindowEnd) {
        ...
        if (wasLastSplitSpecialPattern) {
          ...
          shouldCreateSpecialPattern = Math.random() < SimpleQuiltEngine.SPECIAL_PATTERN_ODDS_EARLY * structureScale;
        }
      } else if (nextAddNumber >= earlyWindowEnd + 1) {
        // Mid + late adds: keep trying until the special-pattern cap (not just adds 7–24).
        shouldCreateSpecialPattern = specialPatternsCreated < maxSpecialPatterns && Math.random() < SimpleQuiltEngine.SPECIAL_PATTERN_ODDS_MID * structureScale;
      }

      }

      if (shouldCreateSpecialPattern) {
        let hostBlocks = splittableBeforeMacroRoute.filter(
          (candidate) =>
            this._blockMeetsSpecialPatternHostGeometry(candidate) &&
            !this._isProtectedAnchorBlock(candidate)
        );
        if (!hostBlocks.length) {
          hostBlocks = this.blocks.filter(
            (candidate) =>
              canTrySplit(candidate) &&
              this._blockMeetsMacroAwareSplitArea(candidate) &&
              this._isBlockSafelySplittableForFrozenMacro(candidate) &&
              protectedAnchorCanSplitForColor(candidate) &&
              this._blockMeetsSpecialPatternHostGeometry(candidate) &&
              !this._isProtectedAnchorBlock(candidate)
          );
        }
        if (this.macroStructureFrozen && hostBlocks.length > 1) {
          const routedHosts = this._routeSplittableBlocksByMacroColor(newColor, hostBlocks);
          if (routedHosts.length) hostBlocks = routedHosts;
        }
        if (!hostBlocks.length) {
          shouldCreateSpecialPattern = false;
        } else {
          const rankedHosts = rankByColorFit(hostBlocks);
          selectedBlock = rankedHosts[0];
          selectedBlock._shouldCreateSpecialPattern = true;
        }
      }

      if (!shouldCreateSpecialPattern && !selectedBlock) {
    ...
    const appliedSplitColor = this._shouldApplyHarmonyOnSplit(selectedBlock)
      ? this._harmonyAdjustedSplitColor(selectedBlock, newColor)
      : newColor;
    if (this._harmonyPreviewActive() && appliedSplitColor !== newColor) {
      console.log('🎨 harmonyBlend color shift', {
        picked: newColor,
        applied: appliedSplitColor,
        strength: this._harmonyBlendStrength(),
    ...
    const contributionBlock =
      this._contributionBlockFromCandidates(splitResult, newColor, this.submissionCount) ||
    ...
    if (this.recordColorReplayEvents && blockIndex !== -1 && splitResult.length > 0) {
      this._recordColorReplayMutation(newColor, parentSnapshot, splitResult);
    }
    ...
    const finalContributionBlock =
      this._currentContributionBlock(newColor, this.submissionCount) ||
      this.blocks.find((block) => block && block.id === contributionBlock?.id) ||
      null;
    if (finalContributionBlock) {
      this._stampNewContributorBlock(finalContributionBlock, newColor);
      this.recordUserContribution(finalContributionBlock.id, newColor, this.submissionCount);
    }
  ...
  splitSpecificBlock(selectedBlock, newColor) {
    ...
    if (adminContribution) {
      this._stampNewContributorBlock(adminContribution, newColor);
      this.recordUserContribution(adminContribution.id, newColor, this.submissionCount);
    }
  ...
  performRegularSplit(block, newColor) {
    if (Array.isArray(block?.polygonPieces) && block.polygonPieces.length > 0) {
      if (block.macroVisibleFlattened === true) {
        const macroSplit = this.splitVisibleFlattenedMacroBlock(block, newColor);
        if (Array.isArray(macroSplit) && macroSplit.length >= 2) return macroSplit;
      }
      const axisSplit = this.splitDiagonalAxisPattern(block, newColor);
      if (Array.isArray(axisSplit) && axisSplit.length >= 2) return axisSplit;
    }

    const minBlockSize = 80;
    const axisChoice = this._chooseRegularSplitDirection(block, minBlockSize, newColor);
    const splitDirection = axisChoice.direction;
    if (axisChoice.flipped) {
      console.log(`🎯 ANTI-SALAMI: Flipping cut on ${block.id} → ${splitDirection} (color mismatch)`);
    } else if (axisChoice.colorCohesive && this._blockStripAspect(block) >= SimpleQuiltEngine.STRIP_ASPECT_FORCE_CROSS_CUT) {
      console.log(`🎯 COHESIVE SALAMI: Keeping ${splitDirection} band on ${block.id}`);
    }
    ...
    const avoidAccent = axisChoice.avoidAccent || axisChoice.flipped;

    const pickPrimaryExtent = (extent) => {
      if (forceOversizedAccentSplit && !avoidAccent) {
        const accentRatio =
      ...
      const accentBias = avoidAccent ? 0 : this._regularSplitAccentBias();
      const ratio =
    ...
    const finishRegularChildren = (children) => {
      if (!Array.isArray(children) || !children.length) return children;
      children.forEach((child) => this._sanitizeRectSplitChild(child));
      this._stampRegularSplitAxisOnChildren(children, splitDirection, block);
      return children;
    };

    if (
      !(Array.isArray(block?.polygonPieces) && block.polygonPieces.length) &&
      typeof this._compositionInColumnDiagonalSplit === 'function'
    ) {
      const diagonal = this._compositionInColumnDiagonalSplit(block, newColor);
      if (Array.isArray(diagonal) && diagonal.length >= 2) {
        return finishRegularChildren(diagonal);
      }
    }
    ...
    if (canonicalHandcutSplit) {
      return finishRegularChildren(canonicalHandcutSplit);
    }

    if (splitDirection === 'horizontal') {
      let splitHeight;
      if (typeof this._compositionHorizontalSplitRatio === 'function') {
        const ratio = Number(this._compositionHorizontalSplitRatio(block, newColor));
        if (Number.isFinite(ratio) && ratio > 0 && ratio < 1) {
          splitHeight = block.height * ratio;
          ...
          if (splitHeight <= 0 || splitHeight >= block.height) {
            splitHeight = pickPrimaryExtent(block.height);
          }
        } else {
          splitHeight = pickPrimaryExtent(block.height);
        }
        if (typeof this._compositionNudgeHorizontalSplitHeight === 'function') {
          splitHeight = this._compositionNudgeHorizontalSplitHeight(block, splitHeight, newColor);
        }
      } else {
        splitHeight = pickPrimaryExtent(block.height);
      }
      if (this._shouldSnapSmallPieceCluster(block)) {
        splitHeight =
          this._snapAxisSplitCutToNeighborSeams(block, 'horizontal', block.y + splitHeight) - block.y;
      }
      ...
      return finishRegularChildren([block1, block2]);
    } else {
      let splitWidth = pickPrimaryExtent(block.width);
      if (this._shouldSnapSmallPieceCluster(block)) {
        splitWidth =
          this._snapAxisSplitCutToNeighborSeams(block, 'vertical', block.x + splitWidth) - block.x;
      }
  ...
  _macroFlattenedAngledSplitProb() {
    const n = Number(this.macroFlattenedAngledSplitProb);
    if (!Number.isFinite(n) || n < 0) return 0;
    return Math.min(1, n);
  }

  _tryMacroFlattenedAngledSplit(block, newColor, ctx) {
    if (!this.macroStructureFrozen || block?.macroVisibleFlattened !== true) return null;
    if (typeof this._compositionAllowAngledMacroSplit === 'function') {
      if (!this._compositionAllowAngledMacroSplit(block, newColor)) return null;
    }
    const prob = this._macroFlattenedAngledSplitProb();
    if (prob <= 0 || (prob < 1 && Math.random() > prob)) return null;

    const world = this._polygonPiecesWorld(block);
    if (!Array.isArray(world) || !world.length) return null;

    const minPart = ctx?.minPart ?? 24;
    const splitDirection = ctx?.splitDirection === 'vertical' ? 'vertical' : 'horizontal';
    const preservedParentColor = ctx?.preservedParentColor ?? block.color;
    const splitColor = ctx?.splitColor ?? newColor;
    const mk = ctx?.mk;
    if (typeof mk !== 'function') return null;
    ...
    if (span < minPart * 2) return null;

    const forceOversized = ctx?.forceOversizedAccentSplit === true;
    const ratio = forceOversized
      ? 1 - (
        SimpleQuiltEngine.OVERSIZED_SPLIT_NEW_COLOR_MIN +
        Math.random() * (SimpleQuiltEngine.OVERSIZED_SPLIT_NEW_COLOR_MAX - SimpleQuiltEngine.OVERSIZED_SPLIT_NEW_COLOR_MIN)
      )
    ...
    let primaryExtent = span * ratio;
    if (primaryExtent < minPart) primaryExtent = minPart;
    if (span - primaryExtent < minPart) primaryExtent = span - minPart;
    if (primaryExtent <= 0 || primaryExtent >= span) primaryExtent = span / 2;
    ...
    if (isHorizontal) {
      ...
      const maxDrift = Math.max(0, Math.min(
        primaryExtent - minPart * 0.55,
        h - primaryExtent - minPart * 0.55,
        h * 0.08,
        28
      ));
    ...
    let worldA = this._clipPolygonPiecesByProjection(world, nx, ny, cut, keepLessForA);
    let worldB = this._clipPolygonPiecesByProjection(world, nx, ny, cut, !keepLessForA);
    if (!worldA.length || !worldB.length) return null;
    [worldA, worldB] = this._finalizeSplitSiblingWorldPieces(worldA, worldB, {
      mode: 'plane',
      nx,
      ny,
      cut
    ...
    const childA = mk('ang_a', worldA, preservedParentColor, block.contributorId, block.submissionIndex);
    const childB = mk('ang_b', worldB, splitColor, this.deviceId, this.submissionCount);
    if (!childA || !childB) return null;

    childA.diagonalAxisUx = ux;
    childA.diagonalAxisUy = uy;
    childB.diagonalAxisUx = ux;
    childB.diagonalAxisUy = uy;
    childB.diagonalAxisAccentColor = newColor;
    return [childA, childB];
  }

  _chooseMacroFlattenedSplitDirection(block, minPart, newColor) {
    const choice = this._chooseRegularSplitDirection(block, minPart, newColor);
    if (!this.macroStructureFrozen || choice.colorCohesive) return choice;
    const last = block?.lastRegularSplitDirection;
    if (last !== choice.direction) return choice;
    const alternate = choice.direction === 'vertical' ? 'horizontal' : 'vertical';
    const crossSpan = alternate === 'horizontal' ? Number(block.width) : Number(block.height);
    if (!(crossSpan >= minPart * 2)) return choice;
    return {
      ...choice,
      direction: alternate,
      flipped: true,
  ...
  splitVisibleFlattenedMacroBlock(block, newColor) {
    ...
    const axisChoice = this._chooseMacroFlattenedSplitDirection(block, minPart, newColor);
    const splitDirection = axisChoice.direction;
    const forceOversizedAccentSplit = block._forceOversizedAccentSplit === true;
    delete block._forceOversizedAccentSplit;
    const splitColor = this._tonalSplitColorForMacroRegion(
    ...
    const parentClipBox = this._parentClipBox(block);
    const mk = (suffix, pieces, color, contributorId, submissionIndex) => {
      const clipped = parentClipBox
        ? this._finalizeMacroPolygonWorldPieces(
          this._clipPolygonWorldPiecesToBox(pieces, parentClipBox)
        )
        : pieces;
      const bounds = this._polygonWorldBounds(clipped);
      if (!bounds || !clipped.length) return null;
      const child = {
        ...block,
        id: `${block.id}_${suffix}`,
        x: bounds.x,
    ...
    const splitBy = (direction) => {
      ...
      const snappedCut = this._snapAxisSplitCutToNeighborSeams(
        block,
        direction === 'vertical' ? 'vertical' : 'horizontal',
        cut
      );
      const aPieces = direction === 'vertical'
        ? this._clipPolygonPiecesByProjection(world, 1, 0, snappedCut, true)
        : this._clipPolygonPiecesByProjection(world, 0, 1, snappedCut, true);
      const bPieces = direction === 'vertical'
        ? this._clipPolygonPiecesByProjection(world, 1, 0, snappedCut, false)
        : this._clipPolygonPiecesByProjection(world, 0, 1, snappedCut, false);
      if (!aPieces.length || !bPieces.length) return null;
      const seamMode = direction === 'vertical' ? 'vertical' : 'horizontal';
      const [weldedA, weldedB] = this._finalizeSplitSiblingWorldPieces(
        this._snapPolygonWorldPiecesToSharedEdges(aPieces, block),
        this._snapPolygonWorldPiecesToSharedEdges(bPieces, block),
        { mode: seamMode, cut: snappedCut }
      );
      const a = mk('1', weldedA, preservedParentColor, block.contributorId, block.submissionIndex);
      const b = mk('2', weldedB, splitColor, this.deviceId, this.submissionCount);
      return a && b ? [a, b] : null;
    };
    const hasAngledBoundary = this._polygonHasNonAxisAlignedEdge(world);
    if (typeof this._compositionInColumnDiagonalSplit === 'function') {
      const diagonal = this._compositionInColumnDiagonalSplit(block, newColor);
      if (Array.isArray(diagonal) && diagonal.length >= 2) {
        diagonal.forEach((child) => {
          if (!child) return;
          child.macroVisibleFlattened = true;
          child.macroOriginalBounds = originalBounds || {
            x: Number(block.x),
            y: Number(block.y),
            width: Number(block.width),
            height: Number(block.height)
        ...
        return this._stampRegularSplitAxisOnChildren(diagonal, splitDirection, block);
      }
    }
    const angled = this._tryMacroFlattenedAngledSplit(block, newColor, {
      minPart,
      splitDirection,
      preservedParentColor,
      splitColor,
      forceOversizedAccentSplit,
      mk
    });
    if (Array.isArray(angled) && angled.length >= 2) {
      return this._stampRegularSplitAxisOnChildren(angled, splitDirection, block);
    }
    if (
      this.macroStructureFrozen &&
      (axisChoice.flipped || this._compositionColumnsFullHeight === true)
    ) {
      const diagonal = this.splitDiagonalAxisPattern(block, newColor);
      if (Array.isArray(diagonal) && diagonal.length >= 2) {
        diagonal.forEach((child) => {
          if (!child) return;
          child.macroVisibleFlattened = true;
        ...
        return this._stampRegularSplitAxisOnChildren(
          diagonal,
          splitDirection,
          block
        );
    ...
    if (hasAngledBoundary) {
      const altDirection = splitDirection === 'vertical' ? 'horizontal' : 'vertical';
      const altAngled = this._tryMacroFlattenedAngledSplit(block, newColor, {
        minPart,
        splitDirection: altDirection,
        preservedParentColor,
        splitColor,
        forceOversizedAccentSplit,
        mk
      });
      if (Array.isArray(altAngled) && altAngled.length >= 2) {
        return this._stampRegularSplitAxisOnChildren(altAngled, altDirection, block);
      }
    }
    const primary = splitBy(splitDirection);
    const fallback = splitBy(splitDirection === 'vertical' ? 'horizontal' : 'vertical');
    const result = primary || fallback;
    if (Array.isArray(result) && result.length >= 2) {
      return this._stampRegularSplitAxisOnChildren(result, splitDirection, block);
    }
    return result || [];
  }
  ...
  splitBlock(block, newColor) {
    ...
    const finishSplit = (result) => {
      ...
      if (Array.isArray(result)) {
        ...
        this._constrainSplitChildrenToParent(block, result);
      }
      return result;
    };
    ...
    if (block._shouldCreateSpecialPattern) {
      ...
      delete block._shouldCreateSpecialPattern;
      const axisFrame = this._resolveAxisPatternFrame(block);
      const specialBlocks = this.createSpecialPattern(block, newColor);
      return finishSplit(this._finalizeNewSpecialPatternSplit(block, specialBlocks, axisFrame, newColor));
    }
  ...
  createSpecialPattern(block, newColor) {
    ...
    switch (selectedPatternType) {
      ...
      case 'doubleFrame':
        return this.createDoubleFramePattern(block, newColor);
      case 'railfence':
      ...
      case 'diamond':
        return this.createDiamondPattern(block, newColor);
      case 'nestedGrid':
        return this.createNestedGridPattern(block, newColor);
      case 'bandedColumns':
        return this.createBandedColumnsPattern(block, newColor);
      default:
  ...
  _filterBandRowSpecialPatterns(availablePatterns) {
    if (!Array.isArray(availablePatterns) || !availablePatterns.length) return availablePatterns;
    let patterns = availablePatterns.filter(
      (pattern) => !SimpleQuiltEngine.DISABLED_SPECIAL_PATTERN_TYPES.includes(pattern)
    );
    const bandWeight = Math.max(0, Number(this.bandRowSpecialWeight));
    if (bandWeight <= 0) {
      patterns = patterns.filter(
        (pattern) => !SimpleQuiltEngine.BAND_ROW_SPECIAL_PATTERN_TYPES.includes(pattern)
      );
    }
    return patterns;
  }
  ...
  getAvailablePatterns(block, newColor) {
    if (!this._blockMeetsSpecialPatternHostGeometry(block)) {
      return [];
    }
    ...
    const maxSpecialPatternArea = this._maxSpecialPatternAreaForBlock(block, quiltArea);
    if (blockArea > maxSpecialPatternArea) {
      return [];
    }
    const minSpecialPatternArea = SimpleQuiltEngine.MIN_SPECIAL_PATTERN_AREA;
    ...
    if (blockArea >= minSpecialPatternArea) {
      ...
      if (block.width > 110 && block.height > 110) {
        const dfAspect = Math.max(block.width, block.height) / Math.min(block.width, block.height);
        if (dfAspect <= 1.55) {
          availablePatterns.push('doubleFrame');
        }
      }
      ...
      // Diamond pinwheel: square-ish blocks with room for four triangles.
      if (block.width > 85 && block.height > 85) {
        const aspectRatio = Math.max(block.width, block.height) / Math.min(block.width, block.height);
        if (aspectRatio <= 1.4) {
          availablePatterns.push('diamond');
        }
      }

      // Nested grid: frame + inner 2×2 checker.
      if (block.width > 100 && block.height > 100) {
        availablePatterns.push('nestedGrid');
      }

      // Banded columns: many uneven vertical strips (wide blocks).
      if (block.width > 110 && block.height > 70) {
        const aspectRatio = block.width / Math.max(1, block.height);
        if (aspectRatio >= 0.75) {
          availablePatterns.push('bandedColumns');
        }
      }
    }
    ...
    if (this._parentNeedsSpecialBoundaryClip(block)) {
      return this._filterBandRowSpecialPatterns(
        availablePatterns.filter(
          (pattern) => !SimpleQuiltEngine.ANGLED_PARENT_EXCLUDED_SPECIAL_PATTERN_TYPES.includes(pattern)
        )
      );
    }

    return this._filterBandRowSpecialPatterns(availablePatterns);
  }
  ...
  selectPatternType(availablePatterns, block, newColor) {
    ...
    const bandTypes = SimpleQuiltEngine.BAND_ROW_SPECIAL_PATTERN_TYPES;
    const bandWeight = Math.max(0, Number(this.bandRowSpecialWeight));
    const gridMotifs = new Set([
      'checkerboard',
      'framed',
      'doubleFrame',
      'cross',
    ...
    const hasGridMotif = availablePatterns.some((pattern) => gridMotifs.has(pattern));
    const motifWeight = this._isQuiltBuilderSession() ? 0.08 : 0.18;
    const weights = availablePatterns.map((pattern) => {
      if (bandTypes.includes(pattern)) return bandWeight;
      if (pattern === 'doubleFrame') return 1.35;
      if (pattern === 'checkerboard') return 0.28;
      if (hasGridMotif && (pattern === 'hst' || pattern === 'insetCircle')) return motifWeight;
      return 1;
    });
    const total = weights.reduce((sum, weight) => sum + weight, 0);
    if (total <= 0) return availablePatterns[0];
    let pick = Math.random() * total;
    for (let i = 0; i < availablePatterns.length; i++) {
      pick -= weights[i];
      if (pick <= 0) return availablePatterns[i];
    }
  ...
  splitInsetCircle(block, newColor) {
    ...
    const field2 = freezeDiskColor ? this._tonalSplitColorForMacroRegion(block, newColor) : newColor;
  ...
  _polygonHasNonAxisAlignedEdge(pieces, angleTolDeg = 0.35) {
    ...
    for (const piece of pieces || []) {
      ...
      for (let i = 0; i < pts.length; i++) {
        ...
        const dx = Math.abs(Number(p1[0]) - Number(p0[0]));
        const dy = Math.abs(Number(p1[1]) - Number(p0[1]));
        if (dx > 0.5 && dy > 0.5 && dy / dx > tanTol && dx / dy > tanTol) return true;
      }
    }
    return false;
  }

  _collapseCollinearVertices(pts, maxTurnDeg = SimpleQuiltEngine.POLYGON_COLLINEAR_COLLAPSE_DEG) {
    if (!Array.isArray(pts) || pts.length < 4) return pts;
    const turnTol = (maxTurnDeg * Math.PI) / 180;
    const out = [];
    const n = pts.length;
    for (let i = 0; i < n; i++) {
      ...
      const v1x = Number(curr[0]) - Number(prev[0]);
      const v1y = Number(curr[1]) - Number(prev[1]);
      const v2x = Number(next[0]) - Number(curr[0]);
      const v2y = Number(next[1]) - Number(curr[1]);
      ...
      if (turn < turnTol) continue;
      out.push([Number(curr[0]), Number(curr[1])]);
    }
  ...
  _douglasPeuckerRecursive(pts, epsilon, start, end, keep) {
    ...
    for (let i = start + 1; i < end; i++) {
      const d = this._pointLineDistance(pts[i], a, b);
      if (d > maxDist) {
        maxDist = d;
        maxIdx = i;
      }
    ...
    if (maxDist > epsilon) {
      keep.add(maxIdx);
      this._douglasPeuckerRecursive(pts, epsilon, start, maxIdx, keep);
      this._douglasPeuckerRecursive(pts, epsilon, maxIdx, end, keep);
    }
  }

  _simplifyClosedPolygonRing(pts, epsilon = SimpleQuiltEngine.POLYGON_STAIRSTEP_SIMPLIFY_EPS) {
    if (!Array.isArray(pts) || pts.length <= 4) return pts;
    const ring = pts.map((p) => [Number(p[0]), Number(p[1])]);
    if (ring.some((p) => !Number.isFinite(p[0]) || !Number.isFinite(p[1]))) return pts;
    ring.push([ring[0][0], ring[0][1]]);
    const keep = new Set([0, ring.length - 1]);
    this._douglasPeuckerRecursive(ring, epsilon, 0, ring.length - 1, keep);
    const out = [...keep]
      .sort((a, b) => a - b)
      .slice(0, -1)
      .map((i) => ring[i]);
  ...
  _finalizeMacroPolygonWorldPieces(worldPieces) {
    const step = SimpleQuiltEngine.POLYGON_VERTEX_QUANTIZE_STEP;
    return (worldPieces || [])
      .map((piece) => {
        let points = (piece.points || []).map((p) => [
          Math.round(Number(p[0]) / step) * step,
          Math.round(Number(p[1]) / step) * step
        ]);
        if (this._polygonHasNonAxisAlignedEdge([{ points }])) {
          points = this._simplifyClosedPolygonRing(points);
        }
        points = this._collapseCollinearVertices(points);
        points = this._dedupePolyVertices(points, step * 0.45);
        return { color: piece.color, points };
      })
      .filter((piece) => Array.isArray(piece.points) && piece.points.length >= 3);
  }
  ...
  _axisCollageMetadataFromBlock(block) {
    ...
    const hasAxisAngle = Number.isFinite(ux) && Number.isFinite(uy) && Math.hypot(ux, uy) > 1e-6;
    const hasAxisLineage =
      block.starterAxisRegion === true ||
      block.specialPatternType === 'diagonalAxis' ||
      block.axisOriginId != null ||
      hasAxisAngle;
    if (!hasAxisLineage) return null;
    return {
      axisLayerMode: hasAxisAngle ? 'collage' : undefined,
      axisOriginId: String(block.axisOriginId || block.starterAxisRegionId || block.originalPatternId || block.id),
      ...
      diagonalAxisUx: hasAxisAngle ? ux : undefined,
      diagonalAxisUy: hasAxisAngle ? uy : undefined
    };
  }

  _applyAxisCollageMetadata(blocks, axisBlock) {
    return this._stampAxisLineageOnPatternCells(blocks, axisBlock);
  }

  /** u/v frame for patterns inside an early-axis region (seam direction + perpendicular). */
  _resolveAxisPatternFrame(block) {
    if (!block) return null;
    const uxRaw = Number(block.diagonalAxisUx);
    const uyRaw = Number(block.diagonalAxisUy);
    const hasAngle = Number.isFinite(uxRaw) && Number.isFinite(uyRaw) && Math.hypot(uxRaw, uyRaw) > 1e-6;
    const hasLineage =
      hasAngle ||
      block.starterAxisRegion === true ||
      block.specialPatternType === 'diagonalAxis' ||
    ...
    let uy;
    if (hasAngle) {
      const axisLen = Math.hypot(uxRaw, uyRaw);
      ux = uxRaw / axisLen;
      uy = uyRaw / axisLen;
    } else {
      return null;
    ...
    let boundaryWorld = this._polygonPiecesWorld(block);
    if (!boundaryWorld.length) {
      boundaryWorld = this._blockRectWorldPiece(block);
    }
    if (!boundaryWorld.length) return null;

    const uvBounds = this._uvBoundsFromWorldPieces(boundaryWorld, ux, uy, vx, vy);
    ...
    return {
      ...
      boundaryWorld,
      uvBounds,
      originId: String(block.axisOriginId || block.starterAxisRegionId || block.originalPatternId || block.id),
  ...
  _uvBoundsFromWorldPieces(pieces, ux, uy, vx, vy) {
    const us = [];
    const vs = [];
  ...
  _clipColoredPiecesToUvBox(pieces, ux, uy, vx, vy, uMin, uMax, vMin, vMax) {
    let out = pieces || [];
    out = this._clipPolygonPiecesByProjection(out, ux, uy, uMin, false);
    out = this._clipPolygonPiecesByProjection(out, ux, uy, uMax, true);
    out = this._clipPolygonPiecesByProjection(out, vx, vy, vMin, false);
    out = this._clipPolygonPiecesByProjection(out, vx, vy, vMax, true);
    return out.filter((piece) => (piece.points || []).length >= 3);
  }

  _recolorWorldPieces(pieces, color) {
    return (pieces || []).map((piece) => ({
      color,
      points: (piece.points || []).map((p) => [Number(p[0]), Number(p[1])])
    }));
  ...
  _patternCellFromWorldPieces(partial, worldPieces, color, minArea = 180) {
    const recolored = this._recolorWorldPieces(worldPieces, color);
    const area = this._coloredPolygonArea(recolored);
    if (area < minArea) return null;
    const bounds = this._polygonWorldBounds(recolored);
    ...
    return {
      ...
      height: bounds.height,
      color,
      polygonPieces: this._localPolygonPiecesFromWorldPieces(recolored, bounds)
    };
  }

  _sanitizeRectSplitChild(child) {
    if (!child || typeof child !== 'object') return child;
    delete child.polygonPieces;
    delete child.specialParentClipPieces;
    if (child.specialPatternType === 'diagonalAxis') {
      delete child.specialPatternType;
      if (child.patternType === 'special') child.patternType = 'regular';
    }
    return child;
  }

  _macroPolygonShardFromSpecialCell(cell, parentBlock) {
    if (!cell || !parentBlock) return cell;
    const onMacroPolygon =
      parentBlock.macroVisibleFlattened === true &&
      Array.isArray(parentBlock.polygonPieces) &&
      parentBlock.polygonPieces.length > 0;
    if (!onMacroPolygon) return cell;
    // Square-ish orthogonal hosts should keep visible motif metadata (checkerboard, HST, etc.).
    if (this._blockHasOrthogonalRectOutline(parentBlock)) return cell;

    let shard = cell;
    if (!Array.isArray(shard.polygonPieces) || !shard.polygonPieces.length) {
      const clipped = this._clipSpecialPatternCellsToParentBoundary(parentBlock, [cell]);
      if (clipped.length === 1 && clipped[0]?.polygonPieces?.length) {
        shard = clipped[0];
      } else {
        return cell;
      }
    ...
    const bounds = this._polygonWorldBounds(this._polygonPiecesWorld(shard));
    if (!bounds) return shard;
    ...
    return {
      ...
      specialPatternType: 'diagonalAxis',
      macroVisibleFlattened: true,
      macroOriginalBounds: parentBlock.macroOriginalBounds || {
        x: Number(parentBlock.x),
        y: Number(parentBlock.y),
        width: Number(parentBlock.width),
        height: Number(parentBlock.height)
      },
      diagonalAxisUx: Number.isFinite(Number(parentBlock.diagonalAxisUx)) ? Number(parentBlock.diagonalAxisUx) : 1,
      diagonalAxisUy: Number.isFinite(Number(parentBlock.diagonalAxisUy)) ? Number(parentBlock.diagonalAxisUy) : 0,
      polygonPieces: shard.polygonPieces,
      originalPatternId: shard.originalPatternId || cell.originalPatternId || cell.id
    };
  }

  _finalizeNewSpecialPatternSplit(parentBlock, specialBlocks, axisFrame, newColor) {
    if (!Array.isArray(specialBlocks) || !specialBlocks.length) return specialBlocks;

    let cells = specialBlocks;
    if (this._parentNeedsSpecialBoundaryClip(parentBlock)) {
      const needsRectClip = !cells.every(
        (cell) => Array.isArray(cell?.polygonPieces) && cell.polygonPieces.length > 0
      );
      if (needsRectClip) {
        const clipped = this._clipSpecialPatternCellsToParentBoundary(parentBlock, cells);
        if (clipped.length) {
          cells = clipped;
        } else {
          return this.performRegularSplit(parentBlock, newColor);
        }
      }
      cells = this._stampSpecialParentClipOnCells(cells, parentBlock);
    }

    if (cells.length === 1) {
      const shard = this._macroPolygonShardFromSpecialCell(cells[0], parentBlock);
      return shard ? [shard] : cells;
    }

    return this._stampAxisLineageOnPatternCells(cells, parentBlock, axisFrame);
  }

  _parentNeedsSpecialBoundaryClip(block) {
    if (!block) return false;
    if (
      block.macroVisibleFlattened === true &&
      this._blockHasOrthogonalRectOutline(block)
    ) {
      return false;
    }
    if (this._resolveAxisPatternFrame(block)) return true;
    if (block.specialPatternType === 'diagonalAxis') return true;
    if (block.starterAxisRegion === true) return true;
    return Array.isArray(block.polygonPieces) && block.polygonPieces.length > 0;
  }

  _parentBoundaryWorldForSpecialClip(block) {
    const frame = this._resolveAxisPatternFrame(block);
    if (frame?.boundaryWorld?.length) return frame.boundaryWorld;
    const world = this._polygonPiecesWorld(block);
    return world.length ? world : null;
  }

  /** Clip rectangular special cells to a diagonal / polygon parent outline. */
  _clipSpecialPatternCellsToParentBoundary(block, cells) {
    if (!this._parentNeedsSpecialBoundaryClip(block)) return cells;
    if (!Array.isArray(cells) || !cells.length) return cells;
    const boundaryWorld = this._parentBoundaryWorldForSpecialClip(block);
    if (!boundaryWorld?.length) return cells;

    const parentArea = Math.max(1, Number(block.width) * Number(block.height));
    const minArea = Math.max(28, Math.min(140, parentArea / Math.max(6, cells.length * 3)));
    const clippedCells = [];

    cells.forEach((cell) => {
      if (!cell) return;
      const rx = Number(cell.x);
      const ry = Number(cell.y);
      const rw = Number(cell.width);
      const rh = Number(cell.height);
      if (![rx, ry, rw, rh].every(Number.isFinite) || rw <= 0 || rh <= 0) return;

      const pieces = this._clipPolygonPiecesToRect(boundaryWorld, rx, ry, rw, rh);
      if (!pieces.length) return;

      const built = this._patternCellFromWorldPieces(
        {
          id: cell.id,
          contributorId: cell.contributorId,
          submissionIndex: cell.submissionIndex,
          patternType: cell.patternType,
          specialPatternType: cell.specialPatternType,
          originalPatternId: cell.originalPatternId,
          specialOriginalBounds: cell.specialOriginalBounds,
          specialOriginalColor: cell.specialOriginalColor,
          visualLayerIndex: cell.visualLayerIndex
        },
        pieces.map((piece) => ({ color: cell.color, points: piece.points })),
        cell.color,
        minArea
      );
      if (built) clippedCells.push(built);
    });

    return clippedCells.length ? clippedCells : cells;
  }

  _uvPositionsFromSpan(min, span, count) {
    const dims = this.generateVariableDimensions(span, count);
    const positions = [min];
    for (let i = 0; i < dims.length; i++) {
      positions.push(positions[i] + dims[i]);
    }
    positions[positions.length - 1] = min + span;
    return positions;
  }

  _stampAxisLineageOnPatternCells(cells, parentBlock, frame) {
    if (!Array.isArray(cells)) return cells;
    const meta = this._axisCollageMetadataFromBlock(parentBlock);
    const originId = frame?.originId || meta?.axisOriginId;
    const stamped = cells.map((cell) => {
      ...
      if (originId) out.axisOriginId = originId;
      if (frame?.ux != null) out.diagonalAxisUx = frame.ux;
      if (frame?.uy != null) out.diagonalAxisUy = frame.uy;
      if (frame?.starterAxisRegion === true) out.starterAxisRegion = true;
      if (frame?.starterAxisRegionId) out.starterAxisRegionId = frame.starterAxisRegionId;
      Object.keys(out).forEach((key) => {
        if (out[key] === undefined) delete out[key];
      });
      return out;
    });
    return this._stampSpecialParentClipOnCells(stamped, parentBlock);
  }

  /** World-space parent outline for renderer clipPath when a special must stay in bounds. */
  _stampSpecialParentClipOnCells(cells, parentBlock) {
    if (!Array.isArray(cells) || !cells.length) return cells;
    if (!this._parentNeedsSpecialBoundaryClip(parentBlock)) return cells;
    const boundaryWorld = this._parentBoundaryWorldForSpecialClip(parentBlock);
    if (!boundaryWorld?.length) return cells;
    const clipPieces = boundaryWorld
      .map((piece) => ({
        points: (piece.points || [])
      ...
      .filter((piece) => piece.points.length >= 3);
    if (!clipPieces.length) return cells;
    cells.forEach((cell) => {
      if (!cell || typeof cell !== 'object') return;
      cell.specialParentClipPieces = clipPieces.map((piece) => ({
        points: piece.points.map((p) => [p[0], p[1]])
      }));
    });
    return cells;
  }

  _createAxisOrientedCheckerboard(block, newColor, frame) {
    const colorPalette = [block.color, newColor];
    const { minU, minV, spanU, spanV } = frame.uvBounds;
    const minSpan = Math.min(spanU, spanV);
    let gridSize = 2;
    if (minSpan >= 250) gridSize = 4;
    else if (minSpan >= 100) gridSize = 3;
    else if (minSpan >= 50) gridSize = 2;
    else return this.performRegularSplit(block, newColor);

    const avgCell = minSpan / gridSize;
    if (avgCell < 15) return this.performRegularSplit(block, newColor);

    const uPositions = this._uvPositionsFromSpan(minU, spanU, gridSize);
    const vPositions = this._uvPositionsFromSpan(minV, spanV, gridSize);
    const { ux, uy, vx, vy, boundaryWorld } = frame;
    const cells = [];

    for (let row = 0; row < gridSize; row++) {
      for (let col = 0; col < gridSize; col++) {
        const cellIndex = row * gridSize + col;
        const color = colorPalette[(row + col) % 2];
        const clipped = this._clipColoredPiecesToUvBox(
          boundaryWorld,
          ux,
          uy,
          vx,
        ...
        const cell = this._patternCellFromWorldPieces(
          {
    ...
    if (cells.length < 2) return this.performRegularSplit(block, newColor);
    console.log(`🎯 CHECKERBOARD (axis): Block ${block.id} → ${gridSize}x${gridSize} grid aligned to seam angle`);
  ...
  _createAxisOrientedStripes(block, newColor, frame) {
    const colorPalette = this.generateColorPalette(block.color, newColor, 3);
    const { minU, minV, spanU, spanV } = frame.uvBounds;
    const alongSeam = spanU >= spanV;
    const stripeCount = alongSeam
      ? Math.min(4, Math.max(3, Math.floor(spanV / 80)))
      : Math.min(4, Math.max(3, Math.floor(spanU / 80)));
    const forcedPickStripe = Math.floor(Math.random() * stripeCount);
    const { ux, uy, vx, vy, boundaryWorld } = frame;
    const cells = [];

    if (alongSeam) {
      const vPositions = this._uvPositionsFromSpan(minV, spanV, stripeCount);
      for (let i = 0; i < stripeCount; i++) {
        const colorIndex = i === forcedPickStripe ? 1 : Math.floor(Math.random() * 3);
        const clipped = this._clipColoredPiecesToUvBox(
          boundaryWorld,
          ux,
          uy,
      ...
      const uPositions = this._uvPositionsFromSpan(minU, spanU, stripeCount);
      for (let i = 0; i < stripeCount; i++) {
        const colorIndex = i === forcedPickStripe ? 1 : Math.floor(Math.random() * 3);
        const clipped = this._clipColoredPiecesToUvBox(
          boundaryWorld,
    ...
    if (cells.length < 2) return this.performRegularSplit(block, newColor);
    console.log(`🎯 STRIPES (axis): Block ${block.id} → ${cells.length} stripes parallel to seam`);
    const specialOriginalBounds = { x: block.x, y: block.y, width: block.width, height: block.height };
    cells.forEach((cell) => {
      cell.specialOriginalBounds = specialOriginalBounds;
      cell.specialOriginalColor = block.color;
  ...
  _createAxisOrientedCross(block, newColor, frame) {
    const colorPalette = this.generateColorPalette(block.color, newColor, 4);
    const { minU, maxU, minV, maxV, spanU, spanV } = frame.uvBounds;
    if (Math.min(spanU, spanV) < 60) return this.performRegularSplit(block, newColor);
    ...
    for (let row = 0; row < gridSize; row++) {
      for (let col = 0; col < gridSize; col++) {
        ...
        let color;
        if (isCenter) color = colorPalette[3];
        else if (isCrossArm) color = colorPalette[1];
        else color = colorPalette[2];
    ...
    if (cells.length < 4) return this.performRegularSplit(block, newColor);
    console.log(`🎯 CROSS (axis): Block ${block.id} → 3x3 cross aligned to seam angle`);
    const specialOriginalBounds = { x: block.x, y: block.y, width: block.width, height: block.height };
    cells.forEach((cell) => {
      cell.specialOriginalBounds = specialOriginalBounds;
      cell.specialOriginalColor = block.color;
  ...
  _createAxisOrientedFramed(block, newColor, frame) {
    const colorPalette = this.generateColorPalette(block.color, newColor, 3);
    const { minU, maxU, minV, maxV, spanU, spanV } = frame.uvBounds;
    const minSpan = Math.min(spanU, spanV);
    const frameThickness = minSpan * 0.15;
    if (minSpan < frameThickness * 3) return this.performRegularSplit(block, newColor);

    const innerUMin = minU + frameThickness;
    const innerUMax = maxU - frameThickness;
    const innerVMin = minV + frameThickness;
    const innerVMax = maxV - frameThickness;
    const { ux, uy, vx, vy, boundaryWorld } = frame;

    const specs = [
      { id: 'top', u0: minU, u1: maxU, v0: minV, v1: minV + frameThickness, color: colorPalette[0] },
      { id: 'bottom', u0: minU, u1: maxU, v0: maxV - frameThickness, v1: maxV, color: colorPalette[2] },
      { id: 'left', u0: minU, u1: minU + frameThickness, v0: innerVMin, v1: innerVMax, color: colorPalette[0] },
      { id: 'right', u0: maxU - frameThickness, u1: maxU, v0: innerVMin, v1: innerVMax, color: colorPalette[2] },
      { id: 'inner', u0: innerUMin, u1: innerUMax, v0: innerVMin, v1: innerVMax, color: colorPalette[1] }
    ];
    ...
    for (const spec of specs) {
      ...
      const cell = this._patternCellFromWorldPieces(
        {
          id: `${block.id}_pattern_framed_${spec.id}`,
          contributorId: this.deviceId,
          submissionIndex: this.submissionCount,
          patternType: 'special',
    ...
    if (cells.length < 3) return this.performRegularSplit(block, newColor);
    console.log(`🎯 FRAMED (axis): Block ${block.id} → ${cells.length} pieces aligned to seam angle`);
    const specialOriginalBounds = { x: block.x, y: block.y, width: block.width, height: block.height };
    cells.forEach((cell) => {
      cell.specialOriginalBounds = specialOriginalBounds;
      cell.specialOriginalColor = block.color;
  ...
  _createAxisOrientedBandedColumns(block, newColor, frame) {
    const numCols = 5 + Math.floor(Math.random() * 2);
    const colorPalette = this.generateColorPalette(block.color, newColor, Math.min(4, numCols));
    const { minU, minV, spanU, spanV } = frame.uvBounds;
    if (spanU / numCols < 14) return this.performRegularSplit(block, newColor);

    const uPositions = this._uvPositionsFromSpan(minU, spanU, numCols);
    const forcedPick = Math.floor(Math.random() * numCols);
    const { ux, uy, vx, vy, boundaryWorld } = frame;
    const cells = [];

    for (let i = 0; i < numCols; i++) {
      const colorIndex = i === forcedPick ? 1 : i % colorPalette.length;
      const clipped = this._clipColoredPiecesToUvBox(
        boundaryWorld,
        ux,
        uy,
      ...
      const cell = this._patternCellFromWorldPieces(
        {
          id: `${block.id}_pattern_bandedColumns_${i}`,
          contributorId: this.deviceId,
          submissionIndex: this.submissionCount,
          patternType: 'special',
          specialPatternType: 'bandedColumns',
          originalPatternId: block.id
        },
        clipped,
        colorPalette[colorIndex]
    ...
    if (cells.length < 2) return this.performRegularSplit(block, newColor);
    console.log(`🎯 BANDED COLUMNS (axis): Block ${block.id} → ${cells.length} columns parallel to seam`);
    const specialOriginalBounds = { x: block.x, y: block.y, width: block.width, height: block.height };
    cells.forEach((cell) => {
      cell.specialOriginalBounds = specialOriginalBounds;
      cell.specialOriginalColor = block.color;
  ...
  _createAxisOrientedNestedGrid(block, newColor, frame) {
    const colorPalette = this.generateColorPalette(block.color, newColor, 3);
    const { minU, maxU, minV, maxV, spanU, spanV } = frame.uvBounds;
    const minSpan = Math.min(spanU, spanV);
    const frameThickness = minSpan * 0.12;
    if (minSpan < frameThickness * 3) return this.performRegularSplit(block, newColor);
    ...
    const innerVMin = minV + frameThickness;
    const innerVMax = maxV - frameThickness;
    const innerSpanU = innerUMax - innerUMin;
    const innerSpanV = innerVMax - innerVMin;
    if (innerSpanU < 40 || innerSpanV < 40) return this.performRegularSplit(block, newColor);
    ...
    for (const spec of frameSpecs) {
      ...
      const cell = this._patternCellFromWorldPieces(
        {
          id: `${block.id}_pattern_nestedGrid_${spec.id}`,
          contributorId: this.deviceId,
          submissionIndex: this.submissionCount,
          patternType: 'special',
          specialPatternType: 'nestedGrid',
          originalPatternId: block.id
        },
        clipped,
        spec.color
    ...
    const colPos = this._uvPositionsFromSpan(innerUMin, innerSpanU, 2);
    const rowPos = this._uvPositionsFromSpan(innerVMin, innerSpanV, 2);
    for (let row = 0; row < 2; row++) {
      for (let col = 0; col < 2; col++) {
        ...
        const cell = this._patternCellFromWorldPieces(
          {
            id: `${block.id}_pattern_nestedGrid_inner_${cellIndex}`,
            contributorId: this.deviceId,
            submissionIndex: this.submissionCount,
            patternType: 'special',
            specialPatternType: 'nestedGrid',
          ...
          clipped,
          (row + col) % 2 === 0 ? colorPalette[0] : colorPalette[1]
        );
        if (cell) cells.push(cell);
      }
    }
    ...
    if (cells.length < 4) return this.performRegularSplit(block, newColor);
    console.log(`🎯 NESTED GRID (axis): Block ${block.id} → frame + 2×2 inner aligned to seam`);
    const specialOriginalBounds = { x: block.x, y: block.y, width: block.width, height: block.height };
    cells.forEach((cell) => {
      cell.specialOriginalBounds = specialOriginalBounds;
      cell.specialOriginalColor = block.color;
  ...
  _pushAxisRailfenceQuadrant(cells, block, frame, spec, colorPalette) {
    const { ux, uy, vx, vy, boundaryWorld } = frame;
    const { name, u0, u1, v0, v1, vertical } = spec;
    const span = vertical ? u1 - u0 : v1 - v0;
    const stripDimensions = this.generateVariableDimensions(span, 3);
    const positions = [0];
    ...
    for (let stripIndex = 0; stripIndex < 3; stripIndex++) {
      ...
      let clipV1 = v1;
      if (vertical) {
        clipU0 = u0 + positions[stripIndex];
        clipU1 = u0 + positions[stripIndex + 1];
      } else {
        clipV0 = v0 + positions[stripIndex];
        clipV1 = v0 + positions[stripIndex + 1];
      }
      const clipped = this._clipColoredPiecesToUvBox(
        boundaryWorld,
        ux,
      ...
      const cell = this._patternCellFromWorldPieces(
        {
          id: `${block.id}_pattern_railfence_${name}_${stripIndex}`,
          contributorId: this.deviceId,
          submissionIndex: this.submissionCount,
          patternType: 'special',
  ...
  _createAxisOrientedRailFence(block, newColor, frame) {
    const colorPalette = this.generateColorPalette(block.color, newColor, 3);
    const { minU, maxU, minV, maxV, spanU, spanV } = frame.uvBounds;
    if (Math.min(spanU, spanV) < 80) return this.performRegularSplit(block, newColor);

    const midU = minU + spanU / 2;
    const midV = minV + spanV / 2;
    const cells = [];
    this._pushAxisRailfenceQuadrant(cells, block, frame, {
      name: 'top-left',
      u0: minU,
      u1: midU,
      v0: minV,
    ...
    if (cells.length < 4) return this.performRegularSplit(block, newColor);
    console.log(`🎯 RAILFENCE (axis): Block ${block.id} → ${cells.length} strips in seam-aligned quadrants`);
    const specialOriginalBounds = { x: block.x, y: block.y, width: block.width, height: block.height };
    cells.forEach((cell) => {
      cell.specialOriginalBounds = specialOriginalBounds;
      cell.specialOriginalColor = block.color;
  ...
  _shouldUseCanonicalHandcutRegularSplit(block) {
    ...
    if (this.macroStructureFrozen) {
      if (
        typeof this._compositionAllowAngledMacroSplit !== 'function' ||
        !this._compositionAllowAngledMacroSplit(block)
      ) {
        return false;
      }
    }
    if (this._shouldSnapSmallPieceCluster(block)) return false;
  ...
  _tryCanonicalHandcutStripPieces(block, isHorizontal, a, b, preservedParentColor, splitColor) {
    if (this._shouldSnapSmallPieceCluster(block)) return null;
    ...
    const maxKink = Math.max(1.2, Math.min(5, span * 0.01));
  ...
  splitDiagonalAxisPattern(block, newColor) {
    ...
    let adjustedCut = cut;
    const snapCluster = this._shouldSnapSmallPieceCluster(block);
    if (snapCluster) {
      const useHorizontal = Number(block.width) >= Number(block.height);
      if (useHorizontal) {
        nx = 0;
        ny = 1;
        adjustedCut = this._snapAxisSplitCutToNeighborSeams(
          block,
          'horizontal',
          Number(block.y) + Number(block.height) * ratio
        );
      } else {
        nx = 1;
        ny = 0;
        adjustedCut = this._snapAxisSplitCutToNeighborSeams(
          block,
          'vertical',
          Number(block.x) + Number(block.width) * ratio
        );
      }
    } else if (
      this._shouldUseCanonicalHandcutRegularSplit(block) ||
      (this.macroStructureFrozen && block.macroVisibleFlattened === true)
    ) {
    ...
    const worldA = this._snapPolygonWorldPiecesToSharedEdges(
      this._clipPolygonPiecesByProjection(world, nx, ny, adjustedCut, true),
      block
    );
    const worldB = this._snapPolygonWorldPiecesToSharedEdges(
      recolorPieces(this._clipPolygonPiecesByProjection(world, nx, ny, adjustedCut, false)),
      block
    );
    const [finalA, finalB] = this._finalizeSplitSiblingWorldPieces(worldA, worldB, {
      mode: 'plane',
      nx,
      ny,
      cut: adjustedCut
    });
    if (!finalA.length || !finalB.length) return [];
    const boundsA = this._polygonWorldBounds(finalA);
    const boundsB = this._polygonWorldBounds(finalB);
    ...
    return [
      mk({
        ...
        polygonPieces: this._localPolygonPiecesFromWorldPieces(finalA, boundsA),
        contributorId: block.contributorId,
        submissionIndex: block.submissionIndex
      }),
      mk({
        ...
        color: splitColor,
        diagonalAxisAccentColor: newColor,
        polygonPieces: this._localPolygonPiecesFromWorldPieces(finalB, boundsB),
        contributorId: this.deviceId,
        submissionIndex: this.submissionCount
      })
    ];
  ...
  createDiagonalAxisPattern(block, newColor) {
    ...
    const minAngle = SimpleQuiltEngine.STARTER_AXIS_MIN_DEGREES;
    const maxAngle = SimpleQuiltEngine.STARTER_AXIS_MAX_DEGREES;
    const angleDegrees = minAngle + Math.floor(Math.random() * (maxAngle - minAngle + 1));
    ...
    const axisOriginId = `${block.starterAxisRegionId || block.id}_axis_${this.submissionCount}`;
    children.forEach((child) => {
      if (!child) return;
      child.axisOriginId = axisOriginId;
    });
  ...
  createOrganicCheckerboard(block, newColor) {
    const axisFrame = this._resolveAxisPatternFrame(block);
    if (axisFrame) {
      return this._createAxisOrientedCheckerboard(block, newColor, axisFrame);
    }
  ...
  createOrganicStripes(block, newColor) {
    const axisFrame = this._resolveAxisPatternFrame(block);
    if (axisFrame) {
      return this._createAxisOrientedStripes(block, newColor, axisFrame);
    }
    ...
    const forcedPickStripe = Math.floor(Math.random() * numStripes);
  ...
  createFramedPattern(block, newColor) {
    const axisFrame = this._resolveAxisPatternFrame(block);
    if (axisFrame) {
      return this._createAxisOrientedFramed(block, newColor, axisFrame);
    }
  ...
  _pushRectFrameRingCells(cells, block, rect, thickness, idPrefix, colors, originalPatternId) {
    const x = Number(rect.x);
    const y = Number(rect.y);
    const w = Number(rect.width);
    const h = Number(rect.height);
    if (![x, y, w, h, thickness].every(Number.isFinite) || w <= 0 || h <= 0 || thickness <= 0) {
      return null;
    }
    if (w < thickness * 3 || h < thickness * 3) return null;

    const innerY0 = y + thickness;
    const innerY1 = y + h - thickness;
    const specs = [
      { id: `${idPrefix}_top`, x, y, width: w, height: thickness, color: colors.top },
      {
        id: `${idPrefix}_bottom`,
        x,
        y: y + h - thickness,
    ...
    specs.forEach((spec) => {
      cells.push({
        ...
        submissionIndex: this.submissionCount,
        patternType: 'special',
        specialPatternType: 'doubleFrame',
        originalPatternId: originalPatternId || block.id
      });
    });
    ...
    return {
      innerX: x + thickness,
      innerY: y + thickness,
      innerW: w - thickness * 2,
      innerH: h - thickness * 2
    };
  }

  _createAxisOrientedDoubleFrame(block, newColor, frame) {
    const colorPalette = this.generateColorPalette(block.color, newColor, 4);
    const { minU, maxU, minV, maxV, spanU, spanV } = frame.uvBounds;
    const minSpan = Math.min(spanU, spanV);
    const outerT = minSpan * 0.12;
    const matGap = minSpan * 0.05;
    const innerT = minSpan * 0.1;
    if (minSpan < outerT * 3 + matGap * 2 + innerT * 3) {
      return this.performRegularSplit(block, newColor);
    }

    const { ux, uy, vx, vy, boundaryWorld } = frame;
    const innerUMin = minU + outerT;
    const innerUMax = maxU - outerT;
    const innerVMin = minV + outerT;
    const innerVMax = maxV - outerT;
    const frameUMin = innerUMin + matGap;
    const frameUMax = innerUMax - matGap;
    const frameVMin = innerVMin + matGap;
    const frameVMax = innerVMax - matGap;
    const centerUMin = frameUMin + innerT;
    const centerUMax = frameUMax - innerT;
    const centerVMin = frameVMin + innerT;
    const centerVMax = frameVMax - innerT;
    if (centerUMax - centerUMin < 12 || centerVMax - centerVMin < 12) {
      return this.performRegularSplit(block, newColor);
    }

    const specs = [
      { id: 'outer_top', u0: minU, u1: maxU, v0: minV, v1: minV + outerT, color: colorPalette[0] },
      { id: 'outer_bottom', u0: minU, u1: maxU, v0: maxV - outerT, v1: maxV, color: colorPalette[2] },
      { id: 'outer_left', u0: minU, u1: minU + outerT, v0: innerVMin, v1: innerVMax, color: colorPalette[0] },
      { id: 'outer_right', u0: maxU - outerT, u1: maxU, v0: innerVMin, v1: innerVMax, color: colorPalette[2] },
      { id: 'mat_top', u0: innerUMin, u1: innerUMax, v0: innerVMin, v1: frameVMin, color: colorPalette[3] },
      { id: 'mat_bottom', u0: innerUMin, u1: innerUMax, v0: frameVMax, v1: innerVMax, color: colorPalette[3] },
      { id: 'mat_left', u0: innerUMin, u1: frameUMin, v0: frameVMin, v1: frameVMax, color: colorPalette[3] },
      { id: 'mat_right', u0: frameUMax, u1: innerUMax, v0: frameVMin, v1: frameVMax, color: colorPalette[3] },
      { id: 'inner_top', u0: frameUMin, u1: frameUMax, v0: frameVMin, v1: frameVMin + innerT, color: colorPalette[1] },
      { id: 'inner_bottom', u0: frameUMin, u1: frameUMax, v0: frameVMax - innerT, v1: frameVMax, color: colorPalette[1] },
      { id: 'inner_left', u0: frameUMin, u1: frameUMin + innerT, v0: centerVMin, v1: centerVMax, color: colorPalette[0] },
      { id: 'inner_right', u0: frameUMax - innerT, u1: frameUMax, v0: centerVMin, v1: centerVMax, color: colorPalette[2] },
      { id: 'center', u0: centerUMin, u1: centerUMax, v0: centerVMin, v1: centerVMax, color: colorPalette[1] }
    ];
    ...
    for (const spec of specs) {
      ...
      const cell = this._patternCellFromWorldPieces(
        {
          id: `${block.id}_pattern_doubleFrame_${spec.id}`,
          contributorId: this.deviceId,
          submissionIndex: this.submissionCount,
          patternType: 'special',
          specialPatternType: 'doubleFrame',
    ...
    if (cells.length < 9) return this.performRegularSplit(block, newColor);
    const specialOriginalBounds = { x: block.x, y: block.y, width: block.width, height: block.height };
    cells.forEach((cell) => {
      cell.specialOriginalBounds = specialOriginalBounds;
      cell.specialOriginalColor = block.color;
  ...
  createDoubleFramePattern(block, newColor) {
    const axisFrame = this._resolveAxisPatternFrame(block);
    if (axisFrame) {
      return this._createAxisOrientedDoubleFrame(block, newColor, axisFrame);
    }

    const minSpan = Math.min(block.width, block.height);
    const outerT = minSpan * 0.12;
    const matGap = minSpan * 0.05;
    const innerT = minSpan * 0.1;
    if (minSpan < outerT * 3 + matGap * 2 + innerT * 3) {
    ...
    const colorPalette = this.generateColorPalette(block.color, newColor, 4);
    const cells = [];
    const outerInner = this._pushRectFrameRingCells(
      cells,
      block,
      ...
      {
        top: colorPalette[0],
        bottom: colorPalette[2],
        sideA: colorPalette[0],
        sideB: colorPalette[2]
      },
      block.id
    );
    if (!outerInner) return this.createFramedPattern(block, newColor);

    if (matGap > 1.5) {
      const matSpecs = [
        {
          id: 'doubleFrame_mat_top',
          x: outerInner.innerX,
          y: outerInner.innerY,
          width: outerInner.innerW,
          height: matGap
        },
        {
          id: 'doubleFrame_mat_bottom',
          x: outerInner.innerX,
          y: outerInner.innerY + outerInner.innerH - matGap,
          width: outerInner.innerW,
          height: matGap
        },
        {
          id: 'doubleFrame_mat_left',
          x: outerInner.innerX,
          y: outerInner.innerY + matGap,
          width: matGap,
          height: outerInner.innerH - matGap * 2
        },
        {
          id: 'doubleFrame_mat_right',
          x: outerInner.innerX + outerInner.innerW - matGap,
          y: outerInner.innerY + matGap,
          width: matGap,
          height: outerInner.innerH - matGap * 2
        }
      ];
      matSpecs.forEach((spec) => {
        cells.push({
          id: `${block.id}_pattern_${spec.id}`,
          x: spec.x,
          y: spec.y,
    ...
    const innerFrameRect = {
      x: outerInner.innerX + matGap,
      y: outerInner.innerY + matGap,
      width: outerInner.innerW - matGap * 2,
      height: outerInner.innerH - matGap * 2
    };
    const centerInner = this._pushRectFrameRingCells(
      cells,
      block,
      innerFrameRect,
      innerT,
      ...
      {
        top: colorPalette[1],
        bottom: colorPalette[1],
        sideA: colorPalette[0],
        sideB: colorPalette[2]
      },
      block.id
    );
    if (!centerInner || centerInner.innerW < 8 || centerInner.innerH < 8) {
      return this.createFramedPattern(block, newColor);
    }

    cells.push({
      id: `${block.id}_pattern_doubleFrame_center`,
      x: centerInner.innerX,
      y: centerInner.innerY,
      width: centerInner.innerW,
      height: centerInner.innerH,
      color: colorPalette[1],
      contributorId: block.contributorId || this.deviceId,
      submissionIndex: this.submissionCount,
      patternType: 'special',
  ...
  createRailFencePattern(block, newColor) {
    const axisFrame = this._resolveAxisPatternFrame(block);
    if (axisFrame) {
      return this._createAxisOrientedRailFence(block, newColor, axisFrame);
    }
  ...
  createCrossPattern(block, newColor) {
    const axisFrame = this._resolveAxisPatternFrame(block);
    if (axisFrame) {
      return this._createAxisOrientedCross(block, newColor, axisFrame);
    }
  ...
  createDiamondPattern(block, newColor) {
    const w = Number(block.width);
    const h = Number(block.height);
    if (!(w > 0 && h > 0)) return this.performRegularSplit(block, newColor);
    const cx = w / 2;
    const cy = h / 2;
    const colorPalette = this.generateColorPalette(block.color, newColor, 4);
    const polygonPieces = [
      { points: [[0, 0], [w, 0], [cx, cy]], color: colorPalette[0] },
      { points: [[w, 0], [w, h], [cx, cy]], color: colorPalette[1] },
      { points: [[w, h], [0, h], [cx, cy]], color: colorPalette[2] },
      { points: [[0, h], [0, 0], [cx, cy]], color: colorPalette[3] }
    ];
    return [
      {
        id: block.id,
  ...
  createNestedGridPattern(block, newColor) {
    const axisFrame = this._resolveAxisPatternFrame(block);
    if (axisFrame) {
      return this._createAxisOrientedNestedGrid(block, newColor, axisFrame);
    }

    const frameThickness = Math.min(block.width, block.height) * 0.12;
    const innerW = block.width - frameThickness * 2;
    const innerH = block.height - frameThickness * 2;
    if (innerW < 40 || innerH < 40) {
      return this.createFramedPattern(block, newColor);
    }
    const colorPalette = this.generateColorPalette(block.color, newColor, 3);
    const innerX = block.x + frameThickness;
    const innerY = block.y + frameThickness;
    const columnWidths = this.generateVariableDimensions(innerW, 2);
    const rowHeights = this.generateVariableDimensions(innerH, 2);
    const colPos = this._positionsFromDimensions(columnWidths);
    const rowPos = this._positionsFromDimensions(rowHeights);
    const cells = [];

    const pushFrame = (spec) => {
      cells.push({
        id: block.id + '_pattern_nestedGrid_' + spec.id,
        x: spec.x,
        y: spec.y,
    ...
    pushFrame({
      id: 'bottom',
      x: block.x,
      y: block.y + block.height - frameThickness,
      width: block.width,
      height: frameThickness,
    ...
    pushFrame({
      id: 'right',
      x: block.x + block.width - frameThickness,
      y: innerY,
      width: frameThickness,
      height: innerH,
      color: colorPalette[2]
    ...
    for (let row = 0; row < 2; row++) {
      for (let col = 0; col < 2; col++) {
        ...
        cells.push({
          ...
          x: innerX + colPos[col],
          y: innerY + rowPos[row],
          width: colPos[col + 1] - colPos[col],
          height: rowPos[row + 1] - rowPos[row],
          color: (row + col) % 2 === 0 ? colorPalette[0] : colorPalette[1],
          contributorId: this.deviceId,
          submissionIndex: this.submissionCount,
          patternType: 'special',
          specialPatternType: 'nestedGrid',
  ...
  createBandedColumnsPattern(block, newColor) {
    const axisFrame = this._resolveAxisPatternFrame(block);
    if (axisFrame) {
      return this._createAxisOrientedBandedColumns(block, newColor, axisFrame);
    }

    const numCols = 5 + Math.floor(Math.random() * 2);
    const colorPalette = this.generateColorPalette(block.color, newColor, Math.min(4, numCols));
    const columnWidths = this.generateVariableDimensions(block.width, numCols);
    const columnPositions = this._positionsFromDimensions(columnWidths);
    const forcedPick = Math.floor(Math.random() * numCols);
    const cells = [];
    ...
    for (let i = 0; i < numCols; i++) {
      ...
      cells.push({
        id: block.id + '_pattern_bandedColumns_' + i,
        x: block.x + columnPositions[i],
        y: block.y,
        width: columnPositions[i + 1] - columnPositions[i],
        height: block.height,
        color: colorPalette[colorIndex],
        contributorId: this.deviceId,
        submissionIndex: this.submissionCount,
  ...
  oklabToHex(L, a, bChannel) {
    ...
    const s_ = L - 0.0894841775 * a - 1.2914855480 * bChannel;
    const lMs = l_ * l_ * l_;
    const mMs = m_ * m_ * m_;
    const sMs = s_ * s_ * s_;
    let rLin = 4.0767416621 * lMs - 3.3077115913 * mMs + 0.2309699292 * sMs;
    let gLin = -1.2684380046 * lMs + 2.6097574011 * mMs - 0.3413193965 * sMs;
    let bLin = -0.0041960863 * lMs - 0.7034186147 * mMs + 1.7076147010 * sMs;
    ...
    const channelHex = (c) =>
      Math.min(255, Math.max(0, Math.round(linearToSrgb(c) * 255)))
        .toString(16)
        .padStart(2, '0');
    return `#${channelHex(rLin)}${channelHex(gLin)}${channelHex(bLin)}`;
  }
  ...
  root.odqHarmonyPreview = () => ({
    override: SimpleQuiltEngine.harmonyBlendUrlOverride(),
    ignoreFreeze: SimpleQuiltEngine.harmonyIgnoreFreezeEnabled(),
    retroOnLoad: SimpleQuiltEngine.harmonyRetroOnLoadEnabled(),
    search: typeof window !== 'undefined' ? String(window.location.search || '') : '',
    href: typeof window !== 'undefined' ? String(window.location.href || '') : ''
  });
  try {
    const harmonyOverride = SimpleQuiltEngine.harmonyBlendUrlOverride();
    if (harmonyOverride != null && harmonyOverride !== 0) {
      console.log(
        '🎨 harmonyBlend preview loaded — reload re-tints the quilt; new adds also blend. Status:',
        root.odqHarmonyPreview()
      );
    }
  } catch (_) {
