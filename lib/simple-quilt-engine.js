...
class SimpleQuiltEngine {
  ...
  addColor(newColor, retryDepth = 0, rejectedSplitIds = null) {
    ...
    const structureScale =
      (forceOversizedSplit
        ? 0
        : this.macroStructureFrozen
          ? SimpleQuiltEngine.SPECIAL_STRUCTURE_FROZEN_SCALE
    ...
    if (!selectedBlock) {
      ...
      // 2) Adds 7-24: 45% chance, unless blocked by the special-pattern cap
  ...
  _longestNonAxisAlignedEdge(pieces, minLen = 8) {
    let best = null;
    let bestLen = 0;
    for (const piece of pieces || []) {
      ...
      for (let i = 0; i < pts.length; i++) {
        ...
        const dx = Number(p1[0]) - Number(p0[0]);
        const dy = Number(p1[1]) - Number(p0[1]);
        const len = Math.hypot(dx, dy);
        if (len < minLen) continue;
        const adx = Math.abs(dx);
        const ady = Math.abs(dy);
        if (adx > 0.5 && ady > 0.5 && len > bestLen) {
          bestLen = len;
          best = { dx, dy, len };
        }
      }
  ...
  _tryMacroFlattenedAngledSplit(block, newColor, ctx) {
    ...
    const boundaryEdge = ctx?.force === true ? this._longestNonAxisAlignedEdge(world) : null;
    if (boundaryEdge) {
      const bx = Number(block.x);
      const by = Number(block.y);
      const bw = Number(block.width);
      ...
      const cx = bx + bw / 2;
      const cy = by + bh / 2;
      const edgeUx = boundaryEdge.dx / boundaryEdge.len;
      const edgeUy = boundaryEdge.dy / boundaryEdge.len;
      const nx = -edgeUy;
      const ny = edgeUx;
      const corners = [
      ...
      if (span >= minPart * 2) {
        ...
        const cut = minProj + primaryExtent;
        const sideAPoint = [cx - nx * span * 0.25, cy - ny * span * 0.25];
