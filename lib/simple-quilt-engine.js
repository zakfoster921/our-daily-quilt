...
class SimpleQuiltEngine {
  ...
  /**
   * Admin-only: swap full paint packets within macro regions so similar hues cluster
   * along shared edges. Seam geometry is unchanged (regular + special pattern cells).
   */
  runAdminColorGroupingPass(options = {}) {
    ...
    const minGain = Number(options.minGain) > 0 ? Number(options.minGain) : 0.02;
    const maxSweeps = Math.max(1, Math.min(12, Math.floor(Number(options.maxSweeps) || 3)));
    const eps = SimpleQuiltEngine.MERGE_EPS_PX * 2;
    ...
    for (let sweep = 0; sweep < maxSweeps; sweep++) {
      ...
      for (const peers of byRegion.values()) {
        ...
        for (let i = 0; i < ordered.length; i++) {
          ...
          if (!colorA) continue;
          const fitA = this._neighborColorFitnessForGrouping(a, colorA, peers, eps);
          let best = null;
          let bestGain = minGain;
          for (let j = i + 1; j < ordered.length; j++) {
            const b = ordered[j];
            if (!b || a.id === b.id) continue;
            const bPacket = this._groupingPaintPacketForBlock(b);
            const candidateColor = this._groupingRepresentativeColorFromPacket(bPacket);
            if (!candidateColor) continue;
            const colorB = this._groupingRepresentativeColor(b);
            const fitB = this._neighborColorFitnessForGrouping(b, colorB, peers, eps);
            const swappedFit =
              this._neighborColorFitnessForGrouping(a, candidateColor, peers, eps) +
              this._neighborColorFitnessForGrouping(b, colorA, peers, eps);
            const gain = swappedFit - (fitA + fitB);
            if (gain > bestGain) {
              bestGain = gain;
              best = b;
            }
