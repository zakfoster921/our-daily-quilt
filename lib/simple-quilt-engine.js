...
class SimpleQuiltEngine {
  ...
  static MIN_SPECIAL_PATTERN_AREA = 28000;
  /** Special motifs only on axis-aligned, mostly-square rects (not triangles / skinny strips / angled shards). */
  static SPECIAL_HOST_MIN_SHORT = 80;
  static SPECIAL_HOST_MAX_ASPECT = 1.45;
  ...
  _blockMeetsSpecialPatternHostGeometry(block) {
    ...
    if (Array.isArray(block.polygonPieces) && block.polygonPieces.length) {
      ...
      if (this._polygonHasNonAxisAlignedEdge(world)) return false;
    }
