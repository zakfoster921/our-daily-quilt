...
class SimpleQuiltEngine {
  ...
  _projectPointToSegment(p, a, b) {
    ...
    if (len2 < 1e-12) return [ax, ay];
    let t = ((px - ax) * dx + (py - ay) * dy) / len2;
    t = Math.max(0, Math.min(1, t));
    return [ax + t * dx, ay + t * dy];
  }

  _blocksSharePolygonNeighborhood(block, other, eps = SimpleQuiltEngine.MERGE_EPS_PX * 3) {
    if (!block || !other || block.id === other.id) return false;
    const ax = Number(block.x);
    const ay = Number(block.y);
    const ax2 = ax + Number(block.width);
    const ay2 = ay + Number(block.height);
    const bx = Number(other.x);
    const by = Number(other.y);
    const bx2 = bx + Number(other.width);
    const by2 = by + Number(other.height);
    if (![ax, ay, ax2, ay2, bx, by, bx2, by2].every(Number.isFinite)) return false;
    return ax <= bx2 + eps && ax2 >= bx - eps && ay <= by2 + eps && ay2 >= by - eps;
  }

  _polygonNeighborBoundarySegments(block, excludeBlockId = null) {
    const segments = [];
    for (const other of this.blocks) {
      if (!other || other.id === block?.id || other.id === excludeBlockId) continue;
      if (!this._blocksSharePolygonNeighborhood(block, other)) continue;
      const world = this._polygonPiecesWorld(other);
      world.forEach((piece) => {
  ...
  _snapPolygonWorldPiecesToNeighborBoundaries(worldPieces, block) {
    const tol = SimpleQuiltEngine.MERGE_EPS_PX * 3.5;
    const segments = this._polygonNeighborBoundarySegments(block);
    if (!segments.length) return worldPieces;
    return (worldPieces || []).map((piece) => ({
      color: piece.color,
      points: (piece.points || []).map((p) => {
        const px = Number(p?.[0]);
        const py = Number(p?.[1]);
        if (!Number.isFinite(px) || !Number.isFinite(py)) return p;
        let best = null;
        let bestDist = tol;
        for (const [a, b] of segments) {
          const projected = this._projectPointToSegment([px, py], a, b);
          if (!projected) continue;
          const d = Math.hypot(px - projected[0], py - projected[1]);
          if (d < bestDist) {
            bestDist = d;
            best = projected;
          }
  ...
  _healPolygonSeamGapsInPlace() {
    const targets = (this.blocks || []).filter(
      (block) => Array.isArray(block?.polygonPieces) && block.polygonPieces.length > 0
    );
    if (targets.length < 2) return;
    for (let pass = 0; pass < 2; pass++) {
      for (const block of targets) {
        const world = this._polygonPiecesWorld(block);
        const healed = this._finalizeMacroPolygonWorldPieces(
          this._snapPolygonWorldPiecesToNeighborBoundaries(world, block)
        );
        const bounds = this._polygonWorldBounds(healed);
        if (!bounds || !healed.length) continue;
        block.polygonPieces = this._localPolygonPiecesFromWorldPieces(healed, bounds);
        block.x = bounds.x;
        block.y = bounds.y;
  ...
  _snapPolygonWorldPiecesToSharedEdges(worldPieces, block) {
    if (
      ...
      return this._finalizeMacroPolygonWorldPieces(
        this._snapPolygonWorldPiecesToNeighborBoundaries(worldPieces, block)
      );
    }
  ...
  _applyMacroStructureFreeze() {
    ...
    this._healPolygonSeamGapsInPlace();
