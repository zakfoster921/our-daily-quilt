...
class SimpleQuiltEngine {
  ...
  static SAME_AXIS_SPLIT_STREAK_FORCE_FLIP = 2;
  /** Aspect (long/short) at/above this counts as a strip — prefer a cross-cut when possible. */
  static STRIP_ASPECT_FORCE_CROSS_CUT = 2.15;
  /** Minimum AABB stripe / frame-rail thickness (px). */
  static MIN_BAND_THICKNESS_PX = 48;
  ...
  _chooseRegularSplitDirection(block, minBlockSize = 80, newColor) {
    ...
    if (canFlip && (streakForce || alreadyStrip)) {
      return { direction: alternate, flipped: true, avoidAccent: true };
    }
    return {
      direction: preferred,
      flipped: false,
      avoidAccent: alreadyStrip || (sameAsLast && streak >= 1)
    };
  }
  ...
  /** Pick split hosts by color fit first; optional value fallback only when no macro hue match exists. */
  ...
  createOrganicStripes(block, newColor) {
    ...
    const specialOriginalBounds = { x: block.x, y: block.y, width: block.width, height: block.height };
    const bandDir = stripeOrientation === 'horizontal' ? 'horizontal' : 'vertical';
    cells.forEach((cell) => {
      cell.specialOriginalBounds = specialOriginalBounds;
      cell.specialOriginalColor = block.color;
      // Next regular split should prefer the cross-axis instead of stacking more parallel bands.
      cell.lastRegularSplitDirection = bandDir;
      cell.sameAxisSplitStreak = 1;
    });
    return cells;
  }
  ...
  createFramedPattern(block, newColor) {
    ...
    const topFrame = {
      ...
      specialPatternType: 'framed',
      originalPatternId: block.id,
      lastRegularSplitDirection: 'horizontal',
      sameAxisSplitStreak: 1
    };
    cells.push(topFrame);
    ...
    const leftFrame = {
      ...
      specialPatternType: 'framed',
      originalPatternId: block.id,
      lastRegularSplitDirection: 'vertical',
      sameAxisSplitStreak: 1
    };
