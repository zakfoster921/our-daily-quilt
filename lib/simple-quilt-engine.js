...
class SimpleQuiltEngine {
   ...
   * Prefer relaxed absorb, then split the largest splittable block on the quilt.
   */
  _forceColorPlacementFallback(newColor, rejectedSplitIds = null) {
    ...
    if (this.blocks.length >= SimpleQuiltEngine.ABSORB_NEAR_MATCH_MIN_BLOCKS) {
      ...
      if (bestBlock) {
        console.log(
          `🎯 ABSORB LAST RESORT: ${newColor} → ${bestBlock.id} (sim ${bestSim.toFixed(3)})`
        );
        return this._commitAbsorbNearMatchColor(newColor, bestBlock);
      }
    }
    ...
    if (!candidates.length) return null;
    console.log(`🎯 GLOBAL SPLIT FALLBACK: ${newColor} on ${candidates[0].id}`);
    return this._executeSelectedBlockColorSplit(candidates[0], newColor);
  }
