...
class SimpleQuiltEngine {
  ...
  static SPECIAL_PATTERN_MAX_AREA_FRACTION = 0.19;
  ...
  static SPECIAL_STRUCTURE_FROZEN_SCALE = 0.25;
  ...
  /** Late regular splits favor a thin new-color slice so most of the cell stays one field (90→190 blocks). */
  static STRUCTURE_ACCENT_START_BLOCKS = 90;
  static STRUCTURE_ACCENT_FULL_BLOCKS = 190;
  static STRUCTURE_ACCENT_PROB = 0.4;
  /** OKLab chroma above this → stripe patterns are banned / pattern fills are softened (stops neon band-only triangles). */
  static INTENSE_PATTERN_CHROMA = 0.135;
  /** Post-freeze diagonal / flattened splits keep parts at least this large (px along cut axis). */
  static FROZEN_AXIS_MIN_SPLIT_PART = 48;
  /** Frozen HST triangle shards refuse further cuts below this half-span. */
  static FROZEN_HST_MIN_SPLIT_PART = 44;
  /** Drop clipped pattern / axis crumbs thinner than this (px). Catches diagonal “teeth” AABB miss. */
  static POLYGON_MIN_THICKNESS_PX = 10;
  /** Minimum keepable polygon area after UV/axis clips (px²). */
  static POLYGON_MIN_AREA_PX = 400;
  /** Reject crumbs whose area is a small fraction of their AABB (skinny diagonal wedges). */
  static POLYGON_MIN_AABB_FILL = 0.22;
  ...
  /** Aspect (long/short) at/above this counts as a strip — soft avoidAccent only (no auto flip). */
  static STRIP_ASPECT_FORCE_CROSS_CUT = 2.8;
  /** Minimum AABB stripe / frame-rail thickness (px). */
  static MIN_BAND_THICKNESS_PX = 36;
  /** Framed rails use at least this fraction of the short side (also floored by MIN_BAND_THICKNESS_PX). */
  static FRAMED_THICKNESS_FRAC = 0.22;
  /** Skip framed/cross when block aspect exceeds this (avoids long thin rails). */
  static FRAMED_MAX_ASPECT = 1.75;
  /** Checkerboard hard cap (AABB + axis). */
  static CHECKERBOARD_MAX_GRID = 3;
  /** Looser width/height match for merging near-aligned strip stacks. */
  static STRIP_MERGE_DIM_TOL_PX = 10;
  ...
  /** Late-quilt density gate: absorb into a near-match regular host instead of splitting. */
  static ABSORB_NEAR_MATCH_MIN_BLOCKS = 55;
  ...
  /**
   * Prefer flipping the cut axis after repeated parallel splits (anti “salami” banding).
   * Returns { direction, flipped, avoidAccent }.
   */
  _chooseRegularSplitDirection(block, minBlockSize = 80, newColor) {
    ...
    const streakForce =
      sameAsLast && streak >= SimpleQuiltEngine.SAME_AXIS_SPLIT_STREAK_FORCE_FLIP;
    if (canFlip && streakForce) {
      return { direction: alternate, flipped: true, avoidAccent: true };
    }
    return {
      direction: preferred,
      flipped: false,
      avoidAccent: alreadyStrip && sameAsLast && streak >= 2
    };
  }
  ...
  _filterMacroCandidatesByColorOrValue(blocks, newColor) {
    ...
    if (strictColorMatches.length) return strictColorMatches;
    const softFloor = this._macroRegularStrongColorSimilarityMin();
    const softMatches = blocks.filter((block) => {
      const anchor = this._macroRegionAnchorColor(block) || block?.color;
      if (typeof anchor !== 'string' || !anchor.match(/^#[0-9A-Fa-f]{6}$/)) return false;
      return this.getColorSimilarity(anchor, newColor) >= softFloor;
    });
    return softMatches.length ? softMatches : blocks;
  }
  ...
  _frozenHstMacroMinSplitPart() {
    return SimpleQuiltEngine.FROZEN_HST_MIN_SPLIT_PART;
  }

  _frozenAxisMinSplitPart() {
    return this.macroStructureFrozen ? SimpleQuiltEngine.FROZEN_AXIS_MIN_SPLIT_PART : 42;
  }

  _blockMeetsMacroAwareSplitArea(block) {
    if (!block) return false;
    const blockArea = Math.max(0, Number(block.width) || 0) * Math.max(0, Number(block.height) || 0);
    return blockArea > (this._isFrozenHstMacroShard(block) ? 2400 : 1600);
  }

  /** Regular splits need at least minBlockSize on both sides of the cut (default 80px → 160px span). */
  _canPerformRegularSplit(block, minBlockSize = 80) {
    if (!block) return false;
    const w = Number(block.width) || 0;
    const h = Number(block.height) || 0;
    const minSpan = minBlockSize * 2;
    return w >= minSpan || h >= minSpan;
  }

  _isIntensePatternColor(hex) {
    return this._oklabChromaForHex(hex) >= SimpleQuiltEngine.INTENSE_PATTERN_CHROMA;
  }

  /** Pull saturation down on neon pattern fills so multi-band triangles do not dominate. */
  _softenIntensePatternColor(hex) {
    if (typeof hex !== 'string' || !hex.match(/^#[0-9A-Fa-f]{6}$/)) return hex;
    const chroma = this._oklabChromaForHex(hex);
    const floor = SimpleQuiltEngine.INTENSE_PATTERN_CHROMA;
    if (!(chroma > floor)) return hex;
    const hsl = this.hexToHsl(hex);
    if (!hsl) return hex;
    const pull = Math.min(0.5, (chroma - floor) / 0.12);
    const s = Math.max(18, hsl.s * (1 - pull * 0.58));
    return Utils.hslToHex(hsl.h, s, hsl.l);
  }

  _patternColorPalette(baseColor1, baseColor2, numColors = 3) {
    return this.generateColorPalette(baseColor1, baseColor2, numColors).map((c) =>
      this._softenIntensePatternColor(c)
    );
  }

  _blockHasAxisPatternLineage(block) {
    if (!block) return false;
    if (block.starterAxisRegion === true) return true;
    if (block.specialPatternType === 'diagonalAxis') return true;
    if (block.axisOriginId != null || block.starterAxisRegionId != null) return true;
    const ux = Number(block.diagonalAxisUx);
    const uy = Number(block.diagonalAxisUy);
    return Number.isFinite(ux) && Number.isFinite(uy) && Math.hypot(ux, uy) > 1e-6;
  }
  ...
  /** Pick split hosts by color fit first; blend lightness when macro frozen; value fallback when no hue match. */
  _rankBlocksByColorFit(blocks, newColor, options = {}) {
    const list = Array.isArray(blocks) ? blocks : [];
    const useValueFallback =
      options.macroValueFallback === true ||
      (this.macroStructureFrozen &&
        list.length > 0 &&
        !list.some((block) => this._macroRegionColorCompatible(block, newColor)));
    const blendValue =
      this.macroStructureFrozen && !useValueFallback && list.length > 1;
    const valueProximityScore = (block) => {
      const dist = this._macroValueDistanceForBlock(block, newColor);
      if (!Number.isFinite(dist)) return 0;
      return 1 / (1 + Math.max(0, dist) * 0.06);
    };
    return [...list].sort((a, b) => {
      ...
      const fitA = blendValue ? simA * 0.68 + valueProximityScore(a) * 0.32 : simA;
      const fitB = blendValue ? simB * 0.68 + valueProximityScore(b) * 0.32 : simB;
      const scoreA = fitA + this._blockSizePressureScore(a) * SimpleQuiltEngine.SIZE_PRESSURE_RANK_WEIGHT;
      const scoreB = fitB + this._blockSizePressureScore(b) * SimpleQuiltEngine.SIZE_PRESSURE_RANK_WEIGHT;
  ...
  _routeSplittableBlocksByMacroColor(newColor, candidateBlocks) {
    ...
    const eligible = !hasStrictColorMatch
      ? (safeNonProtected.length ? safeNonProtected : nonProtected.length ? nonProtected : ranked)
  ...
  _explodeHstBlocksIntoIndependentTriangleShardsAtMacroFreeze() {
    ...
    for (const block of this.blocks) {
      ...
      const mkShard = (tri, shardSuffix, shardId) => {
        ...
        // Tight AABB to the visible triangle (not the full parent box) so later cuts
        // don't stair-step empty corners into jagged seam crumbs.
        ...
        const shardW = maxX - minX;
        const shardH = maxY - minY;
        if (!(shardW > 1 && shardH > 1)) return null;
        const localPts = pts.map((p) => [p[0] - minX, p[1] - minY]);
        const thickness = (2 * this._polygonArea(localPts)) / Math.max(1e-6, this._polygonPerimeter(localPts));
        if (thickness < SimpleQuiltEngine.POLYGON_MIN_THICKNESS_PX * 0.85) return null;
        return {
          id: shardId,
          x: bx + minX,
          y: by + minY,
          ...
          hstTriangles: [{ color: fill, points: localPts }],
  ...
  /** Approximate minimum thickness via 2*area/perimeter (works for skinny diagonal wedges). */
  _piecesMinThickness(pieces) {
    let minT = Number.POSITIVE_INFINITY;
    for (const piece of pieces || []) {
      const pts = piece?.points || [];
      const area = this._polygonArea(pts);
      const peri = this._polygonPerimeter(pts);
      if (!(area > 0) || !(peri > 1e-6)) continue;
      minT = Math.min(minT, (2 * area) / peri);
    }
    return Number.isFinite(minT) ? minT : 0;
  }

  /**
   * True when clipped world pieces are substantial enough to keep as a cell/child.
   * AABB side checks miss long thin wedges along diagonal seams.
   */
  _worldPiecesAreKeepable(pieces, options = {}) {
    const list = Array.isArray(pieces) ? pieces.filter((p) => (p?.points || []).length >= 3) : [];
    if (!list.length) return false;
    const minArea = Number(options.minArea);
    const areaFloor = Number.isFinite(minArea) ? minArea : SimpleQuiltEngine.POLYGON_MIN_AREA_PX;
    const minThickness = Number(options.minThickness);
    const thicknessFloor = Number.isFinite(minThickness)
      ? minThickness
      : SimpleQuiltEngine.POLYGON_MIN_THICKNESS_PX;
    const minFill = Number(options.minFill);
    const fillFloor = Number.isFinite(minFill) ? minFill : SimpleQuiltEngine.POLYGON_MIN_AABB_FILL;
    const area = this._coloredPolygonArea(list);
    if (!(area >= areaFloor)) return false;
    if (this._piecesMinThickness(list) < thicknessFloor) return false;
    const bounds = this._polygonWorldBounds(list);
    if (!bounds || bounds.width < 8 || bounds.height < 8) return false;
    const aabbArea = bounds.width * bounds.height;
    if (aabbArea > 1e-6 && area / aabbArea < fillFloor) return false;
    return true;
  }
  ...
  _subtractCoverPiecesFromPieces(subjectPieces, coverPieces) {
    ...
    return visible.filter((piece) =>
      this._worldPiecesAreKeepable([piece], {
        minArea: 120,
        minThickness: 6,
        minFill: 0.15
      })
  ...
  _flattenMacroFreezeToVisibleOwnership() {
    ...
    for (let i = ordered.length - 1; i >= 0; i--) {
      ...
      if (
        ...
        this._coloredPolygonArea(visiblePieces) < 1 ||
        (clipped && !this._worldPiecesAreKeepable(visiblePieces, {
          minArea: 200,
          minThickness: 8,
          minFill: 0.18
        }))
      ) {
        // Subtraction can wipe a leaf or leave only diagonal “teeth”; keep the intact shape.
  ...
  /**
   * Best regular absorb host for a color at a given similarity floor (read-only; no mutation).
   * @returns {{ block: object, similarity: number, area: number } | null}
   */
  _findAbsorbNearMatchHost(newColor, simMin = SimpleQuiltEngine.MERGE_COLOR_SIMILARITY_MIN) {
    if (!newColor || typeof newColor !== 'string' || !newColor.match(/^#[0-9A-Fa-f]{6}$/)) {
      return null;
    }
    const floor = Number(simMin);
    if (!Number.isFinite(floor)) return null;
    let best = null;
    let bestSim = -1;
    let bestArea = Number.POSITIVE_INFINITY;
    for (const block of this.blocks) {
      if (!this._mergeEligibleRegularBlock(block)) continue;
      const sim = this.getColorSimilarity(block.color, newColor);
      if (sim < floor) continue;
      const area = Math.max(0, Number(block.width) || 0) * Math.max(0, Number(block.height) || 0);
      if (sim > bestSim + 1e-6 || (Math.abs(sim - bestSim) <= 1e-6 && area < bestArea)) {
        best = block;
        bestSim = sim;
        bestArea = area;
      }
    }
    return best ? { block: best, similarity: bestSim, area: bestArea } : null;
  }

  /**
   * Admin probe: estimate absorb rates at several similarity floors against today's quilt.
   * Mixes near-nudges of existing regular colors with distant random hues (read-only).
   */
  probeAbsorbNearMatchThresholds(thresholds = [0.96, 0.92, 0.88], sampleCount = 48) {
    const floors = (Array.isArray(thresholds) ? thresholds : [0.96, 0.92, 0.88])
      .map((n) => Number(n))
      .filter((n) => Number.isFinite(n))
      .sort((a, b) => b - a);
    const nSamples = Math.max(12, Math.min(120, Math.floor(Number(sampleCount) || 48)));
    const regulars = this.blocks.filter((block) => this._mergeEligibleRegularBlock(block));
    const gateOpen = this.blocks.length >= SimpleQuiltEngine.ABSORB_NEAR_MATCH_MIN_BLOCKS;
    const liveFloor = SimpleQuiltEngine.MERGE_COLOR_SIMILARITY_MIN;

    const probes = [];
    const nearCount = Math.floor(nSamples / 2);
    for (let i = 0; i < nearCount; i++) {
      const host = regulars.length
        ? regulars[Math.floor(Math.random() * regulars.length)]
        : null;
      const base = host?.color && /^#[0-9A-Fa-f]{6}$/.test(host.color) ? host.color : '#c87860';
      const hsl = this.hexToHsl(base);
      // Small HSL nudges → typically land near 0.88–0.98 similarity.
      const amount = 0.012 + Math.random() * 0.05;
      const nudged =
        Math.random() < 0.5 ? this.darkenColorHsl(hsl, amount) : this.lightenColorHsl(hsl, amount);
      const hex =
        typeof Utils !== 'undefined' && typeof Utils.hslToHex === 'function'
          ? Utils.hslToHex(nudged.h, nudged.s, nudged.l)
          : base;
      probes.push({
        hex: typeof hex === 'string' && /^#[0-9A-Fa-f]{6}$/.test(hex) ? hex : base,
        kind: 'near'
      });
    }
    while (probes.length < nSamples) {
      const h = Math.floor(Math.random() * 360);
      const s = 35 + Math.floor(Math.random() * 55);
      const l = 28 + Math.floor(Math.random() * 45);
      const hex =
        typeof Utils !== 'undefined' && typeof Utils.hslToHex === 'function'
          ? Utils.hslToHex(h, s, l)
          : '#445566';
      probes.push({
        hex: typeof hex === 'string' && /^#[0-9A-Fa-f]{6}$/.test(hex) ? hex : '#445566',
        kind: 'far'
      });
    }
    ...
    const rows = floors.map((floor) => {
      ...
      for (const probe of probes) {
        if (probe.kind === 'near') nearTotal++;
        else farTotal++;
        const hit = this._findAbsorbNearMatchHost(probe.hex, floor);
        if (!hit) continue;
        absorbCount++;
        if (probe.kind === 'near') nearAbsorb++;
        else farAbsorb++;
        bestSimSum += hit.similarity;
        bestSimN++;
        const id = String(hit.block?.id || '');
        if (id) hostHits.set(id, (hostHits.get(id) || 0) + 1);
      }
      let topHostShare = 0;
      for (const count of hostHits.values()) {
        topHostShare = Math.max(topHostShare, count);
      }
      return {
        threshold: floor,
        isLive: Math.abs(floor - liveFloor) < 1e-9,
        absorbCount,
        absorbRate: probes.length ? absorbCount / probes.length : 0,
        nearAbsorbRate: nearTotal ? nearAbsorb / nearTotal : 0,
        farAbsorbRate: farTotal ? farAbsorb / farTotal : 0,
        uniqueHosts: hostHits.size,
        topHostShare: absorbCount ? topHostShare / absorbCount : 0,
        avgBestSimilarity: bestSimN ? bestSimSum / bestSimN : null
      };
    });

    return {
      blockCount: this.blocks.length,
      regularHostCount: regulars.length,
      gateMinBlocks: SimpleQuiltEngine.ABSORB_NEAR_MATCH_MIN_BLOCKS,
      gateOpen,
      liveThreshold: liveFloor,
      sampleCount: probes.length,
      nearSampleCount: nearCount,
      farSampleCount: probes.length - nearCount,
      rows
    };
  }

  /**
   * Density control: when the quilt is already busy, join a near-match regular host
   * instead of carving a new shard. Geometry unchanged; multi-contributor tags preserved.
   */
  _commitAbsorbNearMatchColor(newColor, hostBlock) {
    const best = hostBlock;
    if (!best) return null;

    const parentSnapshot = this.recordColorReplayEvents
      ? this._serializeBlockForReplay(best)
      : { id: best.id, color: best.color, x: best.x, y: best.y, width: best.width, height: best.height };
    ...
    const pushId = (id) => {
      ...
      if (s && !contributorIds.includes(s)) contributorIds.push(s);
    };
    if (Array.isArray(best.contributorIds)) best.contributorIds.forEach(pushId);
    else pushId(best.contributorId);
    pushId(this.deviceId);

    best.contributorIds = contributorIds;
    best.contributorId = this.deviceId;
    best.submissionIndex = nextSubmission;
    this._stampNewContributorBlock(best, newColor);
    this.recordUserContribution(best.id, newColor, nextSubmission);
    ...
    if (this.recordColorReplayEvents) {
      this.colorReplayEvents.push({
        ...
        children: [this._serializeBlockForReplay(best)]
      });
    }
  ...
  _tryAbsorbNearMatchColor(newColor) {
    if (this.blocks.length < SimpleQuiltEngine.ABSORB_NEAR_MATCH_MIN_BLOCKS) return null;
    const hit = this._findAbsorbNearMatchHost(newColor, SimpleQuiltEngine.MERGE_COLOR_SIMILARITY_MIN);
    if (!hit?.block) return null;
    return this._commitAbsorbNearMatchColor(newColor, hit.block);
  }

  /**
   * Last-resort placement when macro-routed shards are too small to split.
   * Prefer relaxed absorb, then split the best color-fit splittable block on the quilt.
   */
  _forceColorPlacementFallback(newColor, rejectedSplitIds = null) {
    const rejectedIds = rejectedSplitIds instanceof Set ? rejectedSplitIds : new Set();
    if (this.blocks.length >= SimpleQuiltEngine.ABSORB_NEAR_MATCH_MIN_BLOCKS) {
      for (const floor of [0.88, 0.85, 0.82, 0.78, 0.72]) {
        const hit = this._findAbsorbNearMatchHost(newColor, floor);
        if (hit?.block) {
          console.log(
            `🎯 ABSORB FALLBACK: ${newColor} → ${hit.block.id} (floor ${floor.toFixed(2)}, sim ${hit.similarity.toFixed(3)})`
          );
          return this._commitAbsorbNearMatchColor(newColor, hit.block);
        }
      }
    ...
    const candidates = this.blocks.filter((block) => {
      const id = String(block?.id || '').trim();
      return (
        id &&
        !rejectedIds.has(id) &&
        this._canPerformRegularSplit(block) &&
        this._isBlockSafelySplittableForFrozenMacro(block)
      );
    });
    if (!candidates.length) return null;
    const routed = this._filterMacroCandidatesByColorOrValue(candidates, newColor);
    const ranked = this._rankBlocksByColorFit(
      this._routeSplittableBlocksByMacroColor(newColor, routed),
      newColor,
      {
        macroValueFallback:
          this.macroStructureFrozen &&
          routed.length > 0 &&
          !routed.some((block) => this._macroRegionColorCompatible(block, newColor))
      }
    );
    const target = ranked[0];
    if (!target) return null;
    console.log(
      `🎯 GLOBAL SPLIT FALLBACK: ${newColor} on ${target.id} (sim ${this.getColorSimilarity(target.color, newColor).toFixed(3)})`
    );
    return this._executeSelectedBlockColorSplit(target, newColor);
  }

  _executeSelectedBlockColorSplit(selectedBlock, newColor, options = {}) {
    if (!selectedBlock) return null;
    const parentSnapshot = this.recordColorReplayEvents
      ? this._serializeBlockForReplay(selectedBlock)
      : null;
    if (options.forceOversizedSplit === true) {
    ...
    console.log(
      `🔧 SPLIT RESULT: Block ${selectedBlock.id} split into ${splitResult.length} blocks:`,
      splitResult.map((b) => b.id)
    );
    ...
    console.log(
      `🔧 BLOCK COUNT: Before: ${beforeCount}, After: ${afterCount}, Net change: ${afterCount - beforeCount}`
    );
  ...
  _mergeTwoRegularBlocks(a, b) {
    ...
    const stripLike =
      this._blockStripAspect(a) >= 1.8 ||
      this._blockStripAspect(b) >= 1.8 ||
      Math.min(a.height, b.height, a.width, b.width) <= 110;
    const dimTol = stripLike
      ? Math.max(eps, SimpleQuiltEngine.STRIP_MERGE_DIM_TOL_PX)
      : eps;
    ...
    if (
      ...
      Math.abs(a.y - b.y) < dimTol &&
      Math.abs(a.height - b.height) < dimTol
    ) {
      x = a.x;
      y = Math.min(a.y, b.y);
      w = a.width + b.width;
      ...
      Math.abs(a.x - b.x) < dimTol &&
      Math.abs(a.width - b.width) < dimTol
    ) {
      x = Math.min(a.x, b.x);
      y = a.y;
  ...
  _neighborColorFitnessForGrouping(block, candidateColor, peers, eps) {
    let score = 0;
    let n = 0;
  ...
  _tryApplySubmissionAxis(newColor) {
    if (this.macroStructureFrozen) return null;
    // First structural axis: second pick of the day (after first-submission fill).
  ...
  _isPersistedSpecialPatternCell(block) {
    return (
      block?.patternType === 'special' &&
      typeof block.specialPatternType === 'string' &&
      block.specialPatternType.length > 0 &&
  ...
  /** Keep pattern family + render metadata on children after a re-split. */
  _preserveSpecialPatternMetadataOnChildren(parent, children) {
    if (!this._isPersistedSpecialPatternCell(parent)) return children;
    const originalPatternId = parent.originalPatternId || parent.id;
    const specialOriginalBounds =
      this._normalizeMacroOriginalBounds(parent.specialOriginalBounds) || {
        x: Number(parent.x),
        y: Number(parent.y),
        width: Number(parent.width),
        height: Number(parent.height)
      };
    const specialOriginalColor =
      typeof parent.specialOriginalColor === 'string' ? parent.specialOriginalColor : parent.color;
    (Array.isArray(children) ? children : []).forEach((child) => {
      if (!child) return;
      child.patternType = 'special';
      child.specialPatternType = parent.specialPatternType;
      child.originalPatternId = originalPatternId;
      child.specialOriginalBounds = specialOriginalBounds;
      if (typeof specialOriginalColor === 'string') child.specialOriginalColor = specialOriginalColor;
      // Rect fallback on HST: keep family tag, drop triangle geom sized for the old cell.
      if (parent.specialPatternType === 'hst') {
        delete child.hstTriangles;
        delete child.hstDiagonal;
        delete child.hstColorB;
      }
    });
    return children;
  }
  ...
  /**
   * Clip polygon-based pattern cells (checkerboard / stripes / cross / framed on axis polygons)
   * while keeping the parent special identity.
   */
  _splitSpecialPatternPolygonCell(block, newColor) {
    ...
    const minPart = this._frozenAxisMinSplitPart();
    ...
    const specialOriginalBounds =
      this._normalizeMacroOriginalBounds(block.specialOriginalBounds) || {
        x: Number(block.x),
        y: Number(block.y),
        width: Number(block.width),
        height: Number(block.height)
    ...
    const specialOriginalColor =
      typeof block.specialOriginalColor === 'string' ? block.specialOriginalColor : block.color;
    ...
    const mk = (suffix, pieces, color, contributorId, submissionIndex, backsideMeta) => {
      ...
      const child = {
        ...
        polygonPieces: this._localPolygonPiecesFromWorldPieces(
          pieces.map((piece) => ({ color: piece.color, points: piece.points })),
          bounds
        ),
        contributorId,
        submissionIndex
    ...
    const splitBy = (direction) => {
      ...
      if (!Number.isFinite(span) || span < Math.max(minPart, minBlockSize) * 2) return null;
      const origin = direction === 'vertical' ? Number(block.x) : Number(block.y);
      const cut = origin + pickPrimaryExtent(span);
      ...
      if (!this._worldPiecesAreKeepable(aPieces) || !this._worldPiecesAreKeepable(bPieces)) return null;
      const a = mk(
        '1',
        aPieces.map((piece) => ({ color: preservedParentColor, points: piece.points })),
      ...
      const b = mk(
        '2',
        bPieces.map((piece) => ({ color: splitColor, points: piece.points })),
        splitColor,
        this.deviceId,
        this.submissionCount,
      ...
      if (!a || !b) return null;
      this._stampRegularSplitAxisOnChildren([a, b], direction, block);
      return [a, b];
    };
    return (
      splitBy(splitDirection) ||
      splitBy(splitDirection === 'vertical' ? 'horizontal' : 'vertical') ||
      []
    );
  }
  ...
  addColor(newColor, retryDepth = 0, rejectedSplitIds = null) {
    ...
    const absorbResult = this._tryAbsorbNearMatchColor(newColor);
    if (absorbResult) {
      return absorbResult;
    }
    ...
    let splittableBlocks = this.blocks.filter(block => {
      return (
        canTrySplit(block) &&
        this._blockMeetsMacroAwareSplitArea(block) &&
        this._canPerformRegularSplit(block) &&
        this._isBlockSafelySplittableForFrozenMacro(block)
      );
    });
    ...
    const rankByColorFit = (blocks) =>
      this._rankBlocksByColorFit(blocks, newColor, {
        macroValueFallback:
          this.macroStructureFrozen &&
    ...
    if (splittableBlocks.length === 0) {
      ...
      let allBlocks = this.blocks.filter(
        (block) =>
          canTrySplit(block) &&
          this._canPerformRegularSplit(block) &&
          this._isBlockSafelySplittableForFrozenMacro(block)
    ...
    if (!selectedBlock) {
      ...
      if (nextAddNumber >= 2 && nextAddNumber <= 6) {
        ...
        // Adds 7-24 have 45% chance unless blocked by the special-pattern cap.
        shouldCreateSpecialPattern = specialPatternsCreated < maxSpecialPatterns && Math.random() < 0.45 * structureScale;
      }
    ...
    if (!selectedBlock) {
      ...
      if (regularBlocks.length > 0) {
          ...
          selectedBlock = this.blocks
            .filter(
              (b) =>
                b &&
                canTrySplit(b) &&
                this._canPerformRegularSplit(b) &&
                Number(b.width) > 0 &&
                Number(b.height) > 0
            )
            .filter(protectedAnchorCanSplitForColor)
        ...
        if (!selectedBlock) {
          ...
          const fallback = this._forceColorPlacementFallback(newColor, rejectedIds);
    ...
    const splitOutcome = this._executeSelectedBlockColorSplit(selectedBlock, newColor, {
      forceOversizedSplit
    });
    if (!splitOutcome) {
  ...
  performRegularSplit(block, newColor, options = {}) {
    if (!options._skipSpecialRouting) {
      ...
      if (block?.specialPatternType === 'insetCircle') {
        return this.splitInsetCircle(block, newColor);
      }
      if (
        this._isPersistedSpecialPatternCell(block) &&
        Array.isArray(block.polygonPieces) &&
        block.polygonPieces.length
      ) {
        return this._splitSpecialPatternPolygonCell(block, newColor);
      }
    }
    const minBlockSize = 80;
    const axisChoice = this._chooseRegularSplitDirection(block, minBlockSize, newColor);
    ...
    if (axisChoice.flipped) {
      console.log(`🎯 ANTI-SALAMI: Flipping cut on ${block.id} → ${splitDirection}`);
    }
    const splitColor = this._tonalSplitColorForMacroRegion(
      block,
      this._regularSplitColorForFrozenSpecialBlock(block, newColor)
    ...
    const forceOversizedAccentSplit = block._forceOversizedAccentSplit === true;
    delete block._forceOversizedAccentSplit;
    // Strip-like / repeated same-axis lineages: skip thin accent slices (even oversized pressure).
    const avoidAccent = axisChoice.avoidAccent || axisChoice.flipped;
    ...
    const finishRegularChildren = (children) => {
      ...
      this._preserveSpecialPatternMetadataOnChildren(block, children);
      return children;
    };
    ...
    if (this.submissionCount <= 1) {
      ...
      if (block.patternType === 'special' && block.specialPatternType === 'hst') {
        if (this._hstFamilyBlockCount(block) >= SimpleQuiltEngine.HST_MAX_FAMILY_BLOCKS) {
          ...
          return finishSplit(this.performRegularSplit(block, newColor, { _skipSpecialRouting: true }));
        }
  ...
  createSpecialPattern(block, newColor) {
    ...
    switch (selectedPatternType) {
      ...
      default:
        ...
        return this.performRegularSplit(block, newColor, { _skipSpecialRouting: true });
    }
  }
  ...
  getAvailablePatterns(block, newColor) {
    ...
    const organicSpecialsAllowed = blockArea <= maxSpecialPatternArea;
    const minSpecialPatternArea = 35000; // 35,000 square pixels minimum for organic special patterns
    const axisLineage = this._blockHasAxisPatternLineage(block);
    const intensePair =
      this._isIntensePatternColor(block.color) || this._isIntensePatternColor(newColor);
    
    const availablePatterns = [];
    
    // Multi-cell organic specials stay under the area cap (avoids huge banded / checkerboard zones).
    // HST + insetCircle stay eligible on larger cells so big triangles get a real two-tone break.
    if (organicSpecialsAllowed && blockArea >= minSpecialPatternArea) {
      ...
      // Stripes: skip on early-axis triangles (reads as flat neon bands) and on intense chroma pairs.
      // Prefer HST / framed / inset there instead.
      if (
        !axisLineage &&
        !intensePair &&
        block.width > 110 &&
      ...
      // Framed: skip skinny aspect ratios (long thin rails read as line fragmentation).
      if (block.width > 90 && block.height > 90) {
        const framedAr = Math.max(block.width, block.height) / Math.min(block.width, block.height);
        if (framedAr <= SimpleQuiltEngine.FRAMED_MAX_ASPECT) {
    ...
    if (this._blockMeetsHstGeometry(block)) {
  ...
  selectPatternType(availablePatterns, block, newColor) {
    ...
    const weights = availablePatterns.map((pattern) => {
      if (pattern === 'stripes') return intensePair || axisLineage ? 0.15 : 0.45;
      if (pattern === 'cross') return 0.65;
      if (pattern === 'checkerboard') return axisLineage ? 0.75 : 0.95;
      if (pattern === 'hst' || pattern === 'framed' || pattern === 'insetCircle') {
        return axisLineage || intensePair ? 1.55 : 1.25;
      }
      return 1;
    });
    ...
    for (let i = 0; i < availablePatterns.length; i++) {
      roll -= weights[i];
      if (roll <= 0) return availablePatterns[i];
    }
  ...
  splitInsetCircle(block, newColor) {
    ...
    if (useVertical) {
      ...
      if (w1 < minHalf || w2 < minHalf) {
        return freezeDiskColor ? [] : this.performRegularSplit(block, newColor, { _skipSpecialRouting: true });
      }
  ...
  _clipColoredPiecesToUvBox(pieces, ux, uy, vx, vy, uMin, uMax, vMin, vMax) {
    ...
    // Drop skinny diagonal crumbs before they become pattern cells / “teeth.”
    return out.filter((piece) => {
      if (!piece || (piece.points || []).length < 3) return false;
      return this._worldPiecesAreKeepable([piece], {
        minArea: Math.min(220, SimpleQuiltEngine.POLYGON_MIN_AREA_PX),
        minThickness: Math.min(8, SimpleQuiltEngine.POLYGON_MIN_THICKNESS_PX),
        minFill: Math.min(0.18, SimpleQuiltEngine.POLYGON_MIN_AABB_FILL)
      });
    });
  }
  ...
  _patternCellFromWorldPieces(partial, worldPieces, color, minArea = SimpleQuiltEngine.POLYGON_MIN_AREA_PX) {
    const recolored = this._recolorWorldPieces(worldPieces, color);
    if (!this._worldPiecesAreKeepable(recolored, { minArea })) return null;
  ...
  _createAxisOrientedCheckerboard(block, newColor, frame) {
    const colorPalette = [
      this._softenIntensePatternColor(block.color),
      this._softenIntensePatternColor(newColor)
    ];
    const { minU, minV, spanU, spanV } = frame.uvBounds;
    const minSpan = Math.min(spanU, spanV);
    // Cap at 3×3 — 4×4 on axis polygons reads as messy fragmentation.
    let gridSize = 2;
    if (minSpan >= 160) gridSize = 3;
    else if (minSpan >= 50) gridSize = 2;
    else return this.performRegularSplit(block, newColor);

    const avgCell = minSpan / gridSize;
    if (avgCell < 28) return this.performRegularSplit(block, newColor);
  ...
  _createAxisOrientedStripes(block, newColor, frame) {
    // Axis stripes are gated out of pattern roulette; if reached via legacy path, keep to 2 soft bands.
    const colorPalette = this._patternColorPalette(block.color, newColor, 3);
    const { minU, minV, spanU, spanV } = frame.uvBounds;
  ...
  _createAxisOrientedFramed(block, newColor, frame) {
    ...
    const frameThickness = Math.min(
      minSpan * 0.28,
      Math.max(SimpleQuiltEngine.MIN_BAND_THICKNESS_PX, minSpan * SimpleQuiltEngine.FRAMED_THICKNESS_FRAC)
    );
    if (minSpan < frameThickness * 3.2) return this.performRegularSplit(block, newColor);
  ...
  _tryCanonicalHandcutStripPieces(block, isHorizontal, a, b, preservedParentColor, splitColor) {
    ...
    const maxKink = Math.max(1.5, Math.min(9, span * 0.015));
    const innerCount = span > 480 && Math.random() < 0.45 ? 2 : 1;
  ...
  _splitHalfSquareTriangleClipped(block, newColor) {
    ...
    if (w > h) {
      ...
      if (!this._worldPiecesAreKeepable(t1) || !this._worldPiecesAreKeepable(t2)) {
        const preserved = this._preserveFrozenHstMacroShardInsteadOfRegularFallback(block, 'thin clipped vertical child');
        if (preserved) return preserved;
        return [];
      }
      return [
    ...
    if (!this._worldPiecesAreKeepable(t1) || !this._worldPiecesAreKeepable(t2)) {
      const preserved = this._preserveFrozenHstMacroShardInsteadOfRegularFallback(block, 'thin clipped horizontal child');
      if (preserved) return preserved;
      return [];
    }
    return [
  ...
  splitDiagonalAxisPattern(block, newColor) {
    ...
    // Abort rather than mint a skinny wedge child along the diagonal seam.
    if (!this._worldPiecesAreKeepable(worldA) || !this._worldPiecesAreKeepable(worldB)) {
      console.log(`🎯 AXIS SPLIT: Aborting ${block.id} — child would be a thin seam remnant`);
      return [];
    }
    const mk = (partial) => {
      const o = {
  ...
  createDiagonalAxisPattern(block, newColor) {
    ...
    const makeAxisChild = (suffix, color, pieces, contributorId, submissionIndex) => {
      const validPieces = (pieces || [])
        ...
        .filter((piece) => piece.points.length >= 3);
      if (!this._worldPiecesAreKeepable(validPieces)) return null;
  ...
  createOrganicCheckerboard(block, newColor) {
    ...
    // Cap at 3×3 — denser grids + later splits read as messy micro-cells.
    const colorPalette = [
      this._softenIntensePatternColor(block.color),
      this._softenIntensePatternColor(newColor)
    ];
    ...
    let gridSize = 2;
    if (minDimension >= 220) {
      gridSize = SimpleQuiltEngine.CHECKERBOARD_MAX_GRID;
    ...
    gridSize = Math.min(gridSize, SimpleQuiltEngine.CHECKERBOARD_MAX_GRID);

    console.log(`🎯 CHECKERBOARD: Block ${block.id} (${block.width}x${block.height}, min: ${minDimension}px) → ${gridSize}x${gridSize} grid`);

    const minCell = 36;
    const columnWidths = this.generateVariableDimensions(block.width, gridSize, minCell);
    const rowHeights = this.generateVariableDimensions(block.height, gridSize, minCell);

    const avgCellWidth = block.width / gridSize;
    const avgCellHeight = block.height / gridSize;
    if (avgCellWidth < minCell || avgCellHeight < minCell) {
      console.log(`🎯 CHECKERBOARD: Cells too small (${avgCellWidth.toFixed(1)}x${avgCellHeight.toFixed(1)} < ${minCell}px) - falling back to regular split`);
      return this.performRegularSplit(block, newColor);
    }
    
    console.log(`🎯 CHECKERBOARD: Cell size check passed (${avgCellWidth.toFixed(1)}x${avgCellHeight.toFixed(1)} >= ${minCell}px)`);
  ...
  generateVariableDimensions(totalSize, gridSize, minSize = 0) {
    ...
    if (floor > 0 && count * floor > total + 1e-6) {
    ...
    let dims = weights.map((weight) => (weight / sum) * total);
    if (floor > 0) {
      dims = dims.map((d) => Math.max(floor, d));
      const dimSum = dims.reduce((acc, value) => acc + value, 0) || 1;
      dims = dims.map((d) => (d / dimSum) * total);
      // Re-enforce floor after normalize if total allows.
      if (count * floor <= total + 1e-6) {
        dims = dims.map((d) => Math.max(floor, d));
        const dimSum2 = dims.reduce((acc, value) => acc + value, 0) || 1;
        dims = dims.map((d) => (d / dimSum2) * total);
      }
    }
    return dims;
  }
  ...
  createOrganicStripes(block, newColor) {
    const axisFrame = this._resolveAxisPatternFrame(block);
    if (axisFrame) {
      // Prefer a true two-tone triangle / framed fill over flat neon bands on early-axis shapes.
      if (this._blockMeetsHstGeometry(block)) {
        return this.createHalfSquareTriangle(block, newColor);
      }
      return this.createFramedPattern(block, newColor);
    ...
    // Soften neon chroma so multi-band fills do not dominate
    const colorPalette = this._patternColorPalette(block.color, newColor, 3);

    const blockArea = block.width * block.height;
    const minBand = SimpleQuiltEngine.MIN_BAND_THICKNESS_PX;

    // Determine if stripes should be horizontal or vertical based on aspect ratio
    const aspectRatio = block.width / block.height;
    const isHorizontalBand = aspectRatio > 1.2; // Wider blocks get horizontal stripes
    const isVerticalBand = aspectRatio < 0.8;   // Taller blocks get vertical stripes
    const isSquare = !isHorizontalBand && !isVerticalBand;
    ...
    const span = stripeOrientation === 'horizontal' ? block.height : block.width;
    // Keep stripes calmer — 2–3 bands max, and never thinner than MIN_BAND_THICKNESS_PX.
    let numStripes = 2;
    if (blockArea > 28000 && span >= minBand * 3) numStripes = 3;
    numStripes = Math.min(numStripes, Math.max(2, Math.floor(span / minBand)));
    numStripes = Math.min(numStripes, 3);
    if (span < minBand * 2) {
      return this.performRegularSplit(block, newColor);
    }
    ...
    if (stripeOrientation === 'horizontal') {
      ...
      const stripeHeights = this.generateVariableDimensions(block.height, numStripes, minBand);
      ...
      const stripeWidths = this.generateVariableDimensions(block.width, numStripes, minBand);
  ...
  createFramedPattern(block, newColor) {
    ...
    const aspect = Math.max(block.width, block.height) / Math.max(1, shortSide);
    if (aspect > SimpleQuiltEngine.FRAMED_MAX_ASPECT) {
      return this.performRegularSplit(block, newColor);
    }

    // Thicker rails so framed edges do not read as thin line fragmentation.
    const frameThickness = Math.min(
      shortSide * 0.28,
      Math.max(SimpleQuiltEngine.MIN_BAND_THICKNESS_PX, shortSide * SimpleQuiltEngine.FRAMED_THICKNESS_FRAC)
    );
    if (shortSide < frameThickness * 3.2) {
      return this.performRegularSplit(block, newColor);
    }
  ...
  createCrossPattern(block, newColor) {
    ...
    const colorPalette = this._patternColorPalette(block.color, newColor, 4);
    const minCell = 36;
    if (block.width < minCell * 3 || block.height < minCell * 3) {
      return this.performRegularSplit(block, newColor);
    }

    const columnWidths = this.generateVariableDimensions(block.width, 3, minCell);
    const rowHeights = this.generateVariableDimensions(block.height, 3, minCell);
    ...
    for (let row = 0; row < 3; row++) {
      for (let col = 0; col < 3; col++) {
        ...
        // Exact tiling — position jitter created gaps/overlaps that read as messy edges.
        cells.push({
          id: block.id + '_pattern_cross_' + cellIndex,
          x: block.x + columnPositions[col],
          y: block.y + rowPositions[row],
