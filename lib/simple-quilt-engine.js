...
class SimpleQuiltEngine {
  ...
  /**
   * Admin-only: aggressively swap eligible color packets within macro regions so
   * similar hues cluster along shared edges. Seam geometry is unchanged.
   */
  runAdminColorGroupingPass(options = {}) {
    if (!this.macroStructureFrozen) {
      return { changed: 0, skipped: true, reason: 'macro-not-frozen' };
    }
    const minGain = Number(options.minGain) > 0 ? Number(options.minGain) : 0.03;
    const maxSweeps = Math.max(1, Math.min(24, Math.floor(Number(options.maxSweeps) || 8)));
    const pairLimit = Math.max(4, Math.min(80, Math.floor(Number(options.pairLimit) || 40)));
    const eps = SimpleQuiltEngine.MERGE_EPS_PX * 2;
    let totalSwaps = 0;
    ...
    for (let sweep = 0; sweep < maxSweeps; sweep++) {
      ...
      for (const peers of byRegion.values()) {
        ...
        const ordered = [...peers].sort((a, b) => (b.width * b.height) - (a.width * a.height));
        const limit = Math.min(pairLimit, ordered.length);
        for (let i = 0; i < limit; i++) {
          const a = ordered[i];
          const fitA = this._neighborColorFitness(a, a.color, peers, eps);
          let best = null;
    ...
    return { changed: totalSwaps, skipped: false, sweeps: maxSweeps };
  }
