...
class SimpleQuiltEngine {
  ...
  addColor(newColor, retryDepth = 0, rejectedSplitIds = null) {
    ...
    const structureScale =
      ...
      (this._isQuiltBuilderSession() ? 1.5 : 1);
    ...
    if (!selectedBlock) {
      ...
      const maxSpecialPatterns = Math.min(Math.max(2, Math.floor(this.submissionCount * capFrac)), Math.floor(this.submissionCount * 0.75));
      ...
      if (
        ...
        this._isQuiltBuilderSession() &&
        this.macroStructureFrozen &&
        specialPatternsCreated < 3 &&
        nextAddNumber >= this._macroFreezeAtBlockCount() + 4
      ) {
        const needsFirst = specialPatternsCreated === 0;
        const periodicBoost = nextAddNumber % 10 === 0;
        if (needsFirst || periodicBoost) {
