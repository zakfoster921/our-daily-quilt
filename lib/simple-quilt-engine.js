...
class SimpleQuiltEngine {
  ...
  /** Screen-axis row/column band specials — skip on diagonal / polygon parents (stair-step edges). */
  static ROW_BASED_SPECIAL_PATTERN_TYPES = ['stripes', 'bandedColumns', 'nestedGrid', 'railfence', 'logcabin'];
  ...
  getAvailablePatterns(block, newColor) {
    ...
    if (this._parentNeedsSpecialBoundaryClip(block)) {
      return availablePatterns.filter(
        (pattern) => !SimpleQuiltEngine.ROW_BASED_SPECIAL_PATTERN_TYPES.includes(pattern)
      );
    }
