...
class SimpleQuiltEngine {
  ...
  static SPECIAL_PATTERN_MAX_AREA_FRACTION = 0.14;
  ...
  static SPECIAL_STRUCTURE_FROZEN_SCALE = 0.12;
  ...
  static SAME_AXIS_SPLIT_STREAK_FORCE_FLIP = 1;
  ...
  _stripStripesFromSplitChildren(block1, block2, parent) {
    this._stripMultiCellPatternFromSplitChildren(block1, block2, parent, ['stripes']);
  }

  /** Clear multi-cell special identity so re-splits become regular geometry (not denser pattern soup). */
  _stripMultiCellPatternFromSplitChildren(block1, block2, parent, types) {
    const allowed = Array.isArray(types) ? types : [];
    if (!parent || !allowed.includes(parent.specialPatternType)) return;
    [block1, block2].forEach((b) => {
      if (!b) return;
  ...
  performRegularSplit(block, newColor) {
    ...
    if (splitDirection === 'horizontal') {
      ...
      this._stripMultiCellPatternFromSplitChildren(block1, block2, block, [
        'checkerboard',
        'cross',
        'framed'
      ]);
  ...
  _tryCanonicalHandcutStripPieces(block, isHorizontal, a, b, preservedParentColor, splitColor) {
    ...
    // Keep handcut kink subtle — large multi-bend seams read as jagged teeth along later diagonals.
    const maxKink = Math.max(1.2, Math.min(5, span * 0.01));
    const innerCount = span > 520 && Math.random() < 0.35 ? 2 : 1;
