...
class SimpleQuiltEngine {
  ...
  _flattenMacroFreezeToVisibleOwnership() {
    ...
    this.blocks = this.blocks
      .map((block) => {
        ...
        if (preserveSpecialMotif) {
          out.patternType = 'special';
          out.specialPatternType = block.specialPatternType;
        } else {
          out.patternType = 'regular';
          out.specialPatternType = 'diagonalAxis';
          out.originalPatternId = block.originalPatternId || block.id;
          out.diagonalAxisUx = Number.isFinite(Number(block.diagonalAxisUx)) ? Number(block.diagonalAxisUx) : 1;
          out.diagonalAxisUy = Number.isFinite(Number(block.diagonalAxisUy)) ? Number(block.diagonalAxisUy) : 0;
  ...
  splitBlock(block, newColor) {
    ...
    if (block._shouldCreateSpecialPattern) {
      ...
      let specialBlocks = this.createSpecialPattern(block, newColor);
      if (!Array.isArray(specialBlocks) || specialBlocks.length < 2) {
        return finishSplit(specialBlocks);
      }
      if (this._parentNeedsSpecialBoundaryClip(block)) {
        const needsRectClip = !specialBlocks.every(
          (cell) => Array.isArray(cell?.polygonPieces) && cell.polygonPieces.length > 0
        );
        if (needsRectClip) {
          const clipped = this._clipSpecialPatternCellsToParentBoundary(block, specialBlocks);
          if (clipped.length >= 2) {
            specialBlocks = clipped;
          } else {
            return finishSplit(this.performRegularSplit(block, newColor));
          }
        }
        specialBlocks = this._stampSpecialParentClipOnCells(specialBlocks, block);
      }
      return finishSplit(this._stampAxisLineageOnPatternCells(specialBlocks, block, axisFrame));
    }
  ...
  _clipSpecialPatternCellsToParentBoundary(block, cells) {
    ...
    return clippedCells.length >= 2 ? clippedCells : cells;
  }
