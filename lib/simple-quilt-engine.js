...
class SimpleQuiltEngine {
  ...
  constructor(userId = null, options = {}) {
    ...
    this.macroFrozenSpecialStructureScale =
      ...
      Number(options.macroFrozenSpecialStructureScale) >= 0
        ...
        : null;
    /** Builder: one forced motif attempt if freeze passed with none yet. */
    this._builderSeededSpecialAttempted = false;
  ...
  _builderSpecialPatternCap(maxSpecialPatterns) {
    ...
    const freezeAt = this._macroFreezeAtBlockCount();
    const builderCap = freezeAt <= 10 ? 5 : 4;
    return Math.min(maxSpecialPatterns, builderCap);
  }
  ...
  addColor(newColor, retryDepth = 0, rejectedSplitIds = null) {
    ...
    if (!selectedBlock) {
      ...
      if (
        !shouldCreateSpecialPattern &&
        this._isQuiltBuilderSession() &&
        !this._builderSeededSpecialAttempted &&
        this.macroStructureFrozen &&
        specialPatternsCreated === 0 &&
        nextAddNumber >= this._macroFreezeAtBlockCount() + 6
      ) {
        this._builderSeededSpecialAttempted = true;
        shouldCreateSpecialPattern = true;
      }
