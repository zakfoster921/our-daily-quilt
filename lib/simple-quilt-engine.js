...
class SimpleQuiltEngine {
  ...
  /** Snap frozen macro split cuts to neighbor column/row seams within this distance (px). */
  static FROZEN_MACRO_SPLIT_CUT_ALIGN_TOL = 18;
  /** Collapse tiny orthogonal stairstep legs after axis clips (px). */
  static POLYGON_ORTHOGONAL_STAIRSTEP_MAX_LEG = 4.5;
  ...
  _snapAxisSplitCutToNeighborSeams(block, direction, proposedCut) {
    if (block?.macroVisibleFlattened === true && this.macroStructureFrozen) {
      return this._snapFrozenMacroSplitCutToNeighborSeams(block, direction, proposedCut);
    }
  ...
  _snapFrozenMacroSplitCutToNeighborSeams(block, direction, proposedCut) {
    const eps = SimpleQuiltEngine.MERGE_EPS_PX * 2;
    const tol = SimpleQuiltEngine.FROZEN_MACRO_SPLIT_CUT_ALIGN_TOL;
    const bx = Number(block.x);
    const by = Number(block.y);
    const bw = Number(block.width);
    const bh = Number(block.height);
    if (![bx, by, bw, bh].every(Number.isFinite)) return proposedCut;
    const lo = direction === 'horizontal' ? by : bx;
    const hi = direction === 'horizontal' ? by + bh : bx + bw;
    const candidates = [proposedCut];
    for (const other of this.blocks) {
      ...
      const verticalNeighbor =
        Math.abs(bx + bw - ox) < eps || Math.abs(ox + ow - bx) < eps;
      const horizontalNeighbor =
        Math.abs(by + bh - oy) < eps || Math.abs(oy + oh - by) < eps;
      if (direction === 'horizontal' && verticalNeighbor) {
        [oy, oy + oh].forEach((c) => {
          if (Number.isFinite(c) && c > lo + eps && c < hi - eps) candidates.push(c);
        });
      }
      if (direction === 'vertical' && horizontalNeighbor) {
        [ox, ox + ow].forEach((c) => {
          if (Number.isFinite(c) && c > lo + eps && c < hi - eps) candidates.push(c);
        });
      }
  ...
  _simplifyOrthogonalStairsteps(pts, maxLeg = SimpleQuiltEngine.POLYGON_ORTHOGONAL_STAIRSTEP_MAX_LEG) {
    if (!Array.isArray(pts) || pts.length < 4) return pts;
    let out = pts.map((p) => [Number(p[0]), Number(p[1])]);
    let changed = true;
    while (changed && out.length >= 4) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const curr = out[i];
        const nxt = out[(i + 1) % n];
        const l1 = Math.hypot(curr[0] - prev[0], curr[1] - prev[1]);
        const l2 = Math.hypot(nxt[0] - curr[0], nxt[1] - curr[1]);
        const horiz1 = Math.abs(curr[1] - prev[1]) < 0.05;
        const vert1 = Math.abs(curr[0] - prev[0]) < 0.05;
        const horiz2 = Math.abs(nxt[1] - curr[1]) < 0.05;
        const vert2 = Math.abs(nxt[0] - curr[0]) < 0.05;
        const turn90 = (horiz1 && vert2) || (vert1 && horiz2);
        if (turn90 && l1 <= maxLeg && l2 <= maxLeg) {
          changed = true;
          continue;
        }
        next.push(curr);
      }
      out = next.length >= 3 ? next : out;
    }
    return out;
  }
  ...
  _finalizeMacroPolygonWorldPieces(worldPieces) {
    ...
    return (worldPieces || [])
      .map((piece) => {
        ...
        points = this._simplifyOrthogonalStairsteps(points);
