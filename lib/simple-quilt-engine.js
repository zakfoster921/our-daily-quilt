...
class SimpleQuiltEngine {
  ...
  static FROZEN_HST_MIN_SPLIT_PART = 36;
  ...
  _blockMeetsMacroAwareSplitArea(block) {
    ...
    return blockArea > (this._isFrozenHstMacroShard(block) ? 1400 : 1600);
  }
  ...
  _stripStripesFromSplitChildren(block1, block2, parent) {
    ...
    // Always clear stripe identity on re-split (including post-freeze) so band cells
    // do not keep fragmenting as "pattern soup."
    [block1, block2].forEach((b) => {
      if (!b) return;
  ...
  createOrganicStripes(block, newColor) {
    ...
    // Keep stripes calmer — 2–3 bands max (was up to 4)
    let numStripes = 2;
    if (blockArea > 28000) numStripes = 3;
