...
(function (root) {
  ...
  Object.assign(UtilsCore, {
    ...
    hstClassicDiagonalEndpoints(w, h, diagonal) {
      const ww = Number(w);
      const hh = Number(h);
    ...
    _classicHstTemplate(block) {
      const w = Number(block && block.width);
      const h = Number(block && block.height);
      if (!(w > 0 && h > 0)) return [];
      return UtilsCore.getHstRenderTriangles({
        width: w,
        height: h,
        color: block.color,
    ...
    /** Nudge seam vertices slightly toward each wedge centroid to hide SVG AA gaps along the diagonal. */
    sealHstDiagonalOverlap(tris, w, h, seamEps = 0.9) {
      if (!Array.isArray(tris) || tris.length !== 2) return tris;
      const ww = Number(w);
      const hh = Number(h);
      if (!(ww > 0 && hh > 0)) return tris;
      const eps = Number(seamEps);
      if (!(eps > 0)) return tris;
      return tris.map((tri) => {
        ...
        const [cx, cy] = UtilsCore._triangleCentroid(pts);
        const nudged = pts.map((p) => {
          const vx = cx - p[0];
          const vy = cy - p[1];
          const len = Math.hypot(vx, vy);
          if (len < 1e-6) return p;
          const scale = Math.min(eps, len * 0.35);
          return [p[0] + (vx / len) * scale, p[1] + (vy / len) * scale];
        });
        return { color: tri.color, points: nudged };
      });
    },
    /**
     * Triangles for SVG draw: restore missing partner wedge, snap shared seam to straight
     * classic diagonal (stored/clipped shards often disagree → visible line along the seam).
     */
    getHstDrawTriangles(block, options = {}) {
      ...
      const classic = UtilsCore._classicHstTemplate(block);
      const forceClassic =
        options.forceClassicPair === true ||
        block?.axisLayerMode === 'collage' ||
        (typeof block?.hstColorB === 'string' && block.hstColorB.length > 0);

      if (forceClassic && classic.length === 2) {
        /** Classic halves share exact corners — per-wedge seal nudge pulls the diagonal apart and shows a background line. */
        return classic.map((t) => ({
          color: t.color,
          points: normPts(t.points)
        }));
      }

      let tris = exactPlacement
        ? UtilsCore.getHstRenderTriangles(block)
        : UtilsCore.getHstOrganicRenderTriangles(block, jitterMultiplier);
      ...
      const classicPtsForColor = (color) => {
        const want = UtilsCore._normHstColor(color);
        const hit = classic.find((t) => UtilsCore._normHstColor(t.color) === want);
        return hit ? normPts(hit.points) : null;
      };

      if (tris.length === 1) {
        const only = tris[0];
        const partner = classic.find(
          (t) => UtilsCore._normHstColor(t.color) !== UtilsCore._normHstColor(only.color)
        );
        if (partner) {
          tris = [
            { color: only.color, points: normPts(only.points) },
            { color: partner.color, points: normPts(partner.points) }
          ];
        }
      }

      if (exactPlacement && tris.length === 2) {
        tris = tris.map((t) => {
          const snapped = classicPtsForColor(t.color);
          return {
            color: t.color,
            points: snapped && snapped.length >= 3 ? snapped : normPts(t.points)
          };
        });
      }

      if (tris.length === 2) {
        tris = UtilsCore.sealHstDiagonalOverlap(tris, w, h);
      }
