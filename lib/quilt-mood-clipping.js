...
function (global) {
  ...
  const MOOD_DEFAULTS = {
    ...
    spreadPadX: 14,
    moodPadYDomPx: 14,
    /** Grow mood type toward this fraction of inner frame height (up to +10% quote px). */
    moodTextFillRatio: 0.66,
  ...
  /**
   * Mood copy is short; use full quote center-band width and scale px up (max +10% quote)
   * so type fills the tall frame instead of floating small in the middle.
   */
  function fitMoodLayoutForFrame(mctx, runs, cfg, opts) {
    ...
    const targetTextH = innerH * Number(cfg.moodTextFillRatio ?? MOOD_DEFAULTS.moodTextFillRatio);
    const maxPx = Math.round(quoteCenterPx * (1 + MOOD_QUOTE_SIZE_TOLERANCE));
    let px = Math.max(8, Math.round(quoteCenterPx));
    let layout = qnc.layoutColumn(mctx, runs, colW, px, padX);
    while (layout.textH < targetTextH && px < maxPx) {
      px += 1;
  ...
  function renderMoodClipping(mctx, line, cfgIn, opts = {}) {
    ...
    let H = padY * 2 + Math.round(quoteCenterPx * cfg.lineHeight * 3);
    if (Number.isFinite(quoteH) && quoteH > 0) {
    ...
    if (cutPad > 0 && Number.isFinite(quoteW) && quoteW > 0 && Number.isFinite(quoteH) && quoteH > 0) {
      ...
      canvasH = Math.max(minHDom, Math.round(H - 2 * cutPad));
    }

    const fitted = fitMoodLayoutForFrame(mctx, runs, cfg, {
      quoteW,
      canvasW,
      canvasH,
      quoteCenterPx
    });
    const { colW, layout, centerPx } = fitted;
