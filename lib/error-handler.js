...
(function (root) {
  ...
  class ErrorHandler {
    ...
    handleError(error, context = 'Unknown') {
      // User dismissed the share sheet — not an app failure.
      if (error && error.name === 'AbortError') return;
      if (error && error.name === 'NotAllowedError') {
        if (context === 'layoutBPreviewShare') {
          this.uiService.showToast('Could not open share — try again');
        }
        return;
      }
      ...
      const errorMessages = {
        ...
        layoutBPreviewShare: 'Could not prepare story image. Try again in a moment.',
        'adminTuneSpeakerCutout:save':
          'Could not save story settings to the server. Check your connection and tap Save again.',
