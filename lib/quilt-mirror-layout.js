...
(function (root) {
  ...
  function solveViewBox(options) {
    ...
    if (options.preferFullMirroredField === true) {
  ...
  function computeFromBounds(bounds, options = {}) {
    ...
    let quadSideBySide = options.quadSideBySide === true;
    if (options.quadSideBySide == null) {
      quadSideBySide =
        odqNormalizeMirrorBottomLayout(mirrorTune?.bottomLayout) === MIRROR_BOTTOM_LAYOUT_QUAD;
    }
    const fitMode =
  ...
  /**
   * One tile in the Dup ×4 (2×2) grid: offset X/Y, optional flip X/Y about quilt center.
   * Same flip convention as duplicateBottomTileTransform (Dup ×2).
   */
  function quadSideBySideTileTransform(minX, minY, quiltW, quiltH, offsetX, offsetY, flags = {}) {
    const flipX = odqNormalizeMirrorFlipFlag(flags.flipX, false);
    const flipY = odqNormalizeMirrorFlipFlag(flags.flipY, false);
    const cx = minX + quiltW / 2;
    const cy = minY + quiltH / 2;
    let transform = `translate(${Number(offsetX) || 0} ${Number(offsetY) || 0})`;
    if (!flipX && !flipY) return transform;
    if (!flipX && flipY) {
      return `${transform} translate(0 ${cy}) scale(1 -1) translate(0 ${-cy})`;
    }
  ...
  /**
   * Dup ×4 grid-only: independent TL/TR/BL/BR flip flags.
   * tileFlags: { tl, tr, bl, br } each { flipX, flipY }.
   * Legacy: (leftFlags, rightFlags) still accepted as top row; bottom = Flip Y.
   * Returns [TL, TR, BL, BR] transforms (no primary above).
   */
  function quadSideBySideTransformsFromFlags(
    minX,
    minY,
    quiltW,
    quiltH,
    ...
    const halfW = quiltW / 2;
    const rowStep = quiltH - MIRROR_SEAM_OVERLAP_PX;
    let tl;
    let tr;
    let bl;
    let br;
    if (
      rightFlags == null &&
      tileFlagsOrLeft &&
      (tileFlagsOrLeft.tl || tileFlagsOrLeft.tr || tileFlagsOrLeft.bl || tileFlagsOrLeft.br)
    ) {
      tl = tileFlagsOrLeft.tl || {};
      tr = tileFlagsOrLeft.tr || {};
      bl = tileFlagsOrLeft.bl || {};
      br = tileFlagsOrLeft.br || {};
    } else {
      const leftFlags = tileFlagsOrLeft || {};
      const right = rightFlags || {};
      tl = {
        flipX: odqNormalizeMirrorFlipFlag(leftFlags.flipX, false),
        flipY: odqNormalizeMirrorFlipFlag(leftFlags.flipY, false)
      };
      tr = {
        flipX: odqNormalizeMirrorFlipFlag(right.flipX, false),
        flipY: odqNormalizeMirrorFlipFlag(right.flipY, false)
      };
      bl = { flipX: tl.flipX, flipY: !tl.flipY };
      br = { flipX: tr.flipX, flipY: !tr.flipY };
    }
    return [
      quadSideBySideTileTransform(minX, minY, quiltW, quiltH, 0, 0, tl),
      quadSideBySideTileTransform(minX, minY, quiltW, quiltH, halfW, 0, tr),
      quadSideBySideTileTransform(minX, minY, quiltW, quiltH, 0, rowStep, bl),
      quadSideBySideTileTransform(minX, minY, quiltW, quiltH, halfW, rowStep, br)
    ];
  }

  /** ViewBox for Dup ×4: 1.5× quilt width × two stacked rows, meet-fit to viewport. */
  function solveViewBoxQuadSideBySide(options) {
    const minX = Number(options.minX) || 0;
    const minY = Number(options.minY) || 0;
    const quiltW = Math.max(1e-6, Number(options.quiltW) || 1);
    const quiltH = Math.max(1e-6, Number(options.quiltH) || 1);
    ...
    const viewport = resolveViewport(options.viewportW, options.viewportH);
    const fitMode = String(options.fitMode || LIVE_SCREEN_FIT).trim() === 'slice' ? 'slice' : 'meet';
    const contentVbW = contentViewBoxWidth(quiltW, horizontalStretch, padding, true);
    const rowStep = quiltH - MIRROR_SEAM_OVERLAP_PX;
    const contentVbH = rowStep + quiltH + padding * 2;
    const vbX = minX - padding;
    const vbY = minY - padding;
    const scale = fitScale(viewport.width, viewport.height, contentVbW, contentVbH, fitMode);
    return {
      x: vbX,
      y: vbY,
      width: contentVbW,
      ...
      overlapPercent: 0,
      mirrorSeamOffset: 0,
      mirroredFieldHeight: rowStep + quiltH,
      primaryVisibleFraction: 0
    };
  }
  ...
  /** 2×2 grid of Dup×2-style tiles (no primary above); per-tile flips + nudges. */
  const MIRROR_BOTTOM_LAYOUT_QUAD = 'quadSideBySide';
  const MIRROR_QUAD_TILE_IDS = Object.freeze(['tl', 'tr', 'bl', 'br']);
  ...
  const MIRROR_TUNE_DEFAULT = Object.freeze({
    ...
    nudgeLeftTileY: 0,
    nudgeRightTileY: 0,
    // Dup ×4 per-tile (BL/BR default Flip Y of top row when unset on load)
    tlFlipX: false,
    tlFlipY: false,
    trFlipX: false,
    trFlipY: false,
  ...
  const MIRROR_TILE_NUDGE_MIN = -1;
  const MIRROR_TILE_NUDGE_MAX = 1;
  ...
  function odqNormalizeMirrorTileNudge(value) {
    const n = Number(value);
    if (!Number.isFinite(n)) return 0;
    return Math.max(MIRROR_TILE_NUDGE_MIN, Math.min(MIRROR_TILE_NUDGE_MAX, n));
  }
  ...
  function odqAppendTileSeamNudgeTransform(transform, nudgeTileSeamX, viewBoxWidth) {
    const dx = odqNormalizeMirrorTileNudge(nudgeTileSeamX) * Number(viewBoxWidth);
  ...
  function odqAppendTileVerticalNudgeTransform(transform, nudgeTileY, viewBoxHeight) {
    const dy = odqNormalizeMirrorTileNudge(nudgeTileY) * Number(viewBoxHeight);
    if (!Number.isFinite(dy) || Math.abs(dy) < 1e-6) return transform;
  ...
  function odqMirrorBottomLayoutIsQuad(tune) {
    return odqNormalizeMirrorBottomLayout(tune?.bottomLayout) === MIRROR_BOTTOM_LAYOUT_QUAD;
  }

  /** Dup ×2 or Dup ×4 — both use left/right tile flip + position controls. */
  function odqMirrorBottomLayoutUsesDupTiles(tune) {
    const layout = odqNormalizeMirrorBottomLayout(tune?.bottomLayout);
    return layout === MIRROR_BOTTOM_LAYOUT_DOUBLE || layout === MIRROR_BOTTOM_LAYOUT_QUAD;
  }

  function odqQuadTileFieldKey(tileId, kind) {
    const id = String(tileId || '').trim().toLowerCase();
    if (!MIRROR_QUAD_TILE_IDS.includes(id)) return null;
    if (kind === 'flipX') return `${id}FlipX`;
    if (kind === 'flipY') return `${id}FlipY`;
    if (kind === 'nudgeX') return `${id}NudgeX`;
    if (kind === 'nudgeY') return `${id}NudgeY`;
    return null;
  }

  function odqQuadFirestoreFieldKey(tileId, kind) {
    const id = String(tileId || '').trim().toLowerCase();
    if (!MIRROR_QUAD_TILE_IDS.includes(id)) return null;
  ...
  function odqRawHasQuadTileFields(raw) {
    if (!raw || typeof raw !== 'object') return false;
    return MIRROR_QUAD_TILE_IDS.some((id) => {
      return (
        ...
        Object.prototype.hasOwnProperty.call(raw, odqQuadFirestoreFieldKey(id, 'flipX')) ||
        Object.prototype.hasOwnProperty.call(raw, odqQuadFirestoreFieldKey(id, 'flipY')) ||
        Object.prototype.hasOwnProperty.call(raw, odqQuadFirestoreFieldKey(id, 'nudgeX')) ||
        Object.prototype.hasOwnProperty.call(raw, odqQuadFirestoreFieldKey(id, 'nudgeY'))
      );
    });
  }

  function odqDefaultQuadTileFieldsFromDup2(leftFlipX, leftFlipY, rightFlipX, rightFlipY) {
    const lx = odqNormalizeMirrorFlipFlag(leftFlipX, false);
    const ly = odqNormalizeMirrorFlipFlag(leftFlipY, false);
    const rx = odqNormalizeMirrorFlipFlag(rightFlipX, false);
    const ry = odqNormalizeMirrorFlipFlag(rightFlipY, false);
    return {
      tlFlipX: lx,
      tlFlipY: ly,
      trFlipX: rx,
  ...
  function odqParseQuadTileFields(raw, leftFlipX, leftFlipY, rightFlipX, rightFlipY) {
    const defaults = odqDefaultQuadTileFieldsFromDup2(
      leftFlipX,
      leftFlipY,
      rightFlipX,
      rightFlipY
    );
    if (!odqRawHasQuadTileFields(raw)) return defaults;
    const out = { ...defaults };
    MIRROR_QUAD_TILE_IDS.forEach((id) => {
      const fxKey = `${id}FlipX`;
      const fyKey = `${id}FlipY`;
      const nxKey = `${id}NudgeX`;
      const nyKey = `${id}NudgeY`;
      const fsFx = odqQuadFirestoreFieldKey(id, 'flipX');
      const fsFy = odqQuadFirestoreFieldKey(id, 'flipY');
      const fsNx = odqQuadFirestoreFieldKey(id, 'nudgeX');
      const fsNy = odqQuadFirestoreFieldKey(id, 'nudgeY');
      ...
      const nySrc = Object.prototype.hasOwnProperty.call(raw, nyKey)
        ? raw[nyKey]
        : raw[fsNy];
      if (fxSrc !== undefined) out[fxKey] = odqNormalizeMirrorFlipFlag(fxSrc, defaults[fxKey]);
      if (fySrc !== undefined) out[fyKey] = odqNormalizeMirrorFlipFlag(fySrc, defaults[fyKey]);
      if (nxSrc !== undefined) out[nxKey] = odqNormalizeMirrorTileNudge(nxSrc);
      if (nySrc !== undefined) out[nyKey] = odqNormalizeMirrorTileNudge(nySrc);
    });
    return out;
  }

  function odqQuadTileFlagsFromTune(tune) {
    const t = tune || MIRROR_TUNE_DEFAULT;
    const parsed = odqParseQuadTileFields(
      t,
      t.leftFlipX,
      t.leftFlipY,
      t.rightFlipX,
    ...
    MIRROR_QUAD_TILE_IDS.forEach((id) => {
      out[id] = {
        flipX: parsed[`${id}FlipX`] === true,
        flipY: parsed[`${id}FlipY`] === true,
        nudgeX: odqNormalizeMirrorTileNudge(parsed[`${id}NudgeX`]),
        nudgeY: odqNormalizeMirrorTileNudge(parsed[`${id}NudgeY`])
      };
    });
    return out;
  }

  function odqQuadTileFieldsPlain(tune) {
    const flags = odqQuadTileFlagsFromTune(tune);
    const out = {};
    MIRROR_QUAD_TILE_IDS.forEach((id) => {
      out[`${id}FlipX`] = flags[id].flipX;
      out[`${id}FlipY`] = flags[id].flipY;
      out[`${id}NudgeX`] = flags[id].nudgeX;
      out[`${id}NudgeY`] = flags[id].nudgeY;
    });
    return out;
  }

  function odqQuadTileFieldsToFirestore(tune) {
    const plain = odqQuadTileFieldsPlain(tune);
    const out = {};
    MIRROR_QUAD_TILE_IDS.forEach((id) => {
      out[odqQuadFirestoreFieldKey(id, 'flipX')] = plain[`${id}FlipX`];
      out[odqQuadFirestoreFieldKey(id, 'flipY')] = plain[`${id}FlipY`];
      out[odqQuadFirestoreFieldKey(id, 'nudgeX')] = plain[`${id}NudgeX`];
      out[odqQuadFirestoreFieldKey(id, 'nudgeY')] = plain[`${id}NudgeY`];
    });
    return out;
  }
  ...
  function odqParseMirrorTuneRaw(raw) {
    ...
    const leftFlipX = odqNormalizeMirrorFlipFlag(raw.leftFlipX, MIRROR_TUNE_DEFAULT.leftFlipX);
    const leftFlipY = odqNormalizeMirrorFlipFlag(raw.leftFlipY, MIRROR_TUNE_DEFAULT.leftFlipY);
    const rightFlipX = odqNormalizeMirrorFlipFlag(raw.rightFlipX, MIRROR_TUNE_DEFAULT.rightFlipX);
    const rightFlipY = odqNormalizeMirrorFlipFlag(raw.rightFlipY, MIRROR_TUNE_DEFAULT.rightFlipY);
    const quad = odqParseQuadTileFields(raw, leftFlipX, leftFlipY, rightFlipX, rightFlipY);
    return {
      ...
      nudgeTileSeamX: odqNormalizeMirrorTileNudge(raw.nudgeTileSeamX),
      nudgeLeftTileX: odqNormalizeMirrorTileNudge(raw.nudgeLeftTileX),
      nudgeLeftTileY: odqNormalizeMirrorTileNudge(raw.nudgeLeftTileY),
      nudgeRightTileY: odqNormalizeMirrorTileNudge(raw.nudgeRightTileY),
      ...quad,
  ...
  function odqMirrorTuneFromQuiltData(data) {
    ...
    const hasQuad =
      MIRROR_QUAD_TILE_IDS.some((id) =>
        Object.prototype.hasOwnProperty.call(d, odqQuadFirestoreFieldKey(id, 'flipX')) ||
        Object.prototype.hasOwnProperty.call(d, odqQuadFirestoreFieldKey(id, 'flipY')) ||
        Object.prototype.hasOwnProperty.call(d, odqQuadFirestoreFieldKey(id, 'nudgeX')) ||
        Object.prototype.hasOwnProperty.call(d, odqQuadFirestoreFieldKey(id, 'nudgeY'))
      );
    if (!hasFields && !hasQuad) return { ...MIRROR_TUNE_DEFAULT, updatedAt: '' };
    const leftFlipX = odqNormalizeMirrorFlipFlag(d.mirrorBottomLeftFlipX, MIRROR_TUNE_DEFAULT.leftFlipX);
    const leftFlipY = odqNormalizeMirrorFlipFlag(d.mirrorBottomLeftFlipY, MIRROR_TUNE_DEFAULT.leftFlipY);
    const rightFlipX = odqNormalizeMirrorFlipFlag(d.mirrorBottomRightFlipX, MIRROR_TUNE_DEFAULT.rightFlipX);
    const rightFlipY = odqNormalizeMirrorFlipFlag(d.mirrorBottomRightFlipY, MIRROR_TUNE_DEFAULT.rightFlipY);
    const quad = odqParseQuadTileFields(d, leftFlipX, leftFlipY, rightFlipX, rightFlipY);
    return {
      ...
      nudgeTileSeamX: odqNormalizeMirrorTileNudge(d.mirrorTileSeamNudgeX),
      nudgeLeftTileX: odqNormalizeMirrorTileNudge(d.mirrorBottomLeftNudgeX),
      nudgeLeftTileY: odqNormalizeMirrorTileNudge(d.mirrorBottomLeftNudgeY),
      nudgeRightTileY: odqNormalizeMirrorTileNudge(d.mirrorBottomRightNudgeY),
      ...quad,
  ...
  function odqMirrorTuneIsCustomized(tune) {
    const t = tune || MIRROR_TUNE_DEFAULT;
    if (odqMirrorBottomLayoutUsesDupTiles(t)) return true;
    return (
      ...
      odqNormalizeMirrorTileNudge(t.nudgeTileSeamX) !== 0 ||
      odqNormalizeMirrorTileNudge(t.nudgeLeftTileX) !== 0 ||
      odqNormalizeMirrorTileNudge(t.nudgeLeftTileY) !== 0 ||
      odqNormalizeMirrorTileNudge(t.nudgeRightTileY) !== 0
    );
  }

  function odqMirrorTuneModeLabel(tune) {
    const t = tune || MIRROR_TUNE_DEFAULT;
    if (odqMirrorBottomLayoutIsQuad(t)) {
      const q = odqQuadTileFlagsFromTune(t);
      return `Dup ×4 (${odqMirrorTileFlipLabel('TL', q.tl.flipX, q.tl.flipY)} ${odqMirrorTileFlipLabel('TR', q.tr.flipX, q.tr.flipY)} ${odqMirrorTileFlipLabel('BL', q.bl.flipX, q.bl.flipY)} ${odqMirrorTileFlipLabel('BR', q.br.flipX, q.br.flipY)})`;
    }
  ...
  function odqMirrorTuneSnapshotFields(tune) {
    ...
    return {
      ...
      nudgeTileSeamX: odqNormalizeMirrorTileNudge(t.nudgeTileSeamX),
      nudgeLeftTileX: odqNormalizeMirrorTileNudge(t.nudgeLeftTileX),
      nudgeLeftTileY: odqNormalizeMirrorTileNudge(t.nudgeLeftTileY),
      nudgeRightTileY: odqNormalizeMirrorTileNudge(t.nudgeRightTileY),
      ...odqQuadTileFieldsPlain(t)
    };
  }
  ...
  /**
   * Pick whether a remote quilt-doc tune should replace local.
   * On timestamp ties, keep local when remote omits explicit Dup ×4 tile fields
   * (those get synthesized as defaults and would wipe per-tile edits).
   */
  function odqShouldPreferRemoteMirrorTune(local, remote, remoteRaw) {
    ...
    if (!remoteCustom && localCustom) return odqIsoTimestampIsNewer(remoteAt, localAt);
    if (remoteCustom && localCustom) {
      if (odqIsoTimestampIsNewer(remoteAt, localAt)) return true;
      if (odqIsoTimestampIsNewer(localAt, remoteAt)) return false;
      // Tie: don't apply a remote that lacks explicit quad tile fields over a local Dup ×4.
      if (
        odqMirrorBottomLayoutIsQuad(local) &&
        !odqRawHasQuadTileFields(remoteRaw || {})
      ) {
        return false;
      }
      return true;
  ...
  function odqWriteMirrorTuneLocal(dateKey, tune) {
    ...
    const nudgeTileSeamX = odqNormalizeMirrorTileNudge(tune?.nudgeTileSeamX);
    const nudgeLeftTileX = odqNormalizeMirrorTileNudge(tune?.nudgeLeftTileX);
    const nudgeLeftTileY = odqNormalizeMirrorTileNudge(tune?.nudgeLeftTileY);
    const nudgeRightTileY = odqNormalizeMirrorTileNudge(tune?.nudgeRightTileY);
    const quad = odqQuadTileFieldsPlain(tune);
    const payload = {
      bottomLayout,
      flipX,
      flipY,
  ...
  async function odqReadMirrorTune(dateKey) {
    ...
    if (isNative) {
      try {
        ...
        if (payload) {
          ...
          const localCustom = odqMirrorTuneIsCustomized(local);
          const useRemote = odqShouldPreferRemoteMirrorTune(local, remote, payload);
          const winner = useRemote ? remote : local;
          if (useRemote) {
            odqWriteMirrorTuneLocal(key, {
              ...winner,
              ...odqQuadTileFieldsPlain(winner),
              updatedAt: winner.updatedAt || remote.updatedAt || undefined
            });
    ...
    try {
      ...
      const remote = odqMirrorTuneFromQuiltData(rawData);
      const localCustom = odqMirrorTuneIsCustomized(local);
      const useRemote = odqShouldPreferRemoteMirrorTune(local, remote, rawData);
      const winner = useRemote ? remote : local;
      if (useRemote) {
        odqWriteMirrorTuneLocal(key, {
          ...winner,
  ...
  async function odqWriteMirrorTuneViaServer(dateKey, tune, writeOptions = {}) {
    ...
    const quad = odqQuadTileFieldsPlain({
      ...tune,
      leftFlipX,
      leftFlipY,
      rightFlipX,
    ...
    const body = JSON.stringify({
      ...
      ...odqQuadTileFieldsToFirestore(afterTune),
  ...
  /** Floor/cap for how far the mirror tucks under the primary (fraction of primary band). */
  const SPLIT_BAND_SEAM_OVERLAP_PRIMARY_FRACTION_MIN = 0.04;
  const SPLIT_BAND_SEAM_OVERLAP_PRIMARY_FRACTION_MAX = 0.35;

  /**
   * Deepest transparent valley along the fold (quilt units). Sample X columns; for each,
   * notch = foldY − lowest block coverage. We only care about fold skyline holes — those
   * show page white unless the mirror band tucks under the primary far enough to paint them.
   */
  function estimateFoldNotchDepth(blocks, minY, quiltH, samples = 32) {
    if (!Array.isArray(blocks) || !blocks.length || !(quiltH > 0)) return 0;
    const xs = [];
    const rights = [];
    for (const block of blocks) {
      const x = Number(block?.x);
      const w = Number(block?.width);
      if (![x, w].every(Number.isFinite) || w <= 0) continue;
      xs.push(x);
      rights.push(x + w);
    }
    ...
    const foldY = minY + quiltH;
    const n = Math.max(8, Math.min(64, Math.floor(Number(samples) || 32)));
    let maxNotch = 0;
    for (let i = 0; i < n; i++) {
      const x = minX + ((i + 0.5) / n) * width;
      let coverBottom = minY;
      let covered = false;
      for (const block of blocks) {
        ...
        if (![bx, bw, by, bh].every(Number.isFinite) || bw <= 0 || bh <= 0) continue;
        if (x < bx || x > bx + bw) continue;
        covered = true;
        coverBottom = Math.max(coverBottom, by + bh);
      }
      if (!covered) {
        maxNotch = Math.max(maxNotch, quiltH);
        continue;
      }
      maxNotch = Math.max(maxNotch, foldY - coverBottom);
    }
    return Math.max(0, maxNotch);
  }

  /**
   * Pixel overlap so mirror sits under primary and fills fold notches (not a fixed 8px).
   * Floor ≈ 4% of primary band; grows with measured notch depth; capped so the mirror
   * band still owns the lower viewport.
   */
  function computeSplitBandSeamOverlapPx(blocks, minY, quiltH, primaryScaleY, targetPrimaryScreenH) {
    const primaryH = Math.max(1, Number(targetPrimaryScreenH) || 1);
    const scaleY = Math.max(0, Number(primaryScaleY) || 0);
    const floorPx = Math.max(
      MIRROR_SEAM_OVERLAP_PX,
      primaryH * SPLIT_BAND_SEAM_OVERLAP_PRIMARY_FRACTION_MIN
    );
    const capPx = Math.max(floorPx, primaryH * SPLIT_BAND_SEAM_OVERLAP_PRIMARY_FRACTION_MAX);
    const notchQuilt = estimateFoldNotchDepth(blocks, minY, quiltH);
    const notchPx = notchQuilt * scaleY + MIRROR_SEAM_OVERLAP_PX;
    return Math.max(floorPx, Math.min(capPx, notchPx));
  }

  function _blockFillColor(block) {
    const c = block?.color;
  ...
  /**
   * Opaque rects that extend skyline coverage down to the fold. Needed because primary and
   * mirror share the same transparent valleys after flip — overlap alone still shows page white.
   * Returns [{x,y,width,height,fill}, ...] in quilt space.
   */
  function buildFoldNotchUnderpaintRects(blocks, minY, quiltH, bleed = 2) {
    if (!Array.isArray(blocks) || !blocks.length || !(quiltH > 0)) return [];
    const notchDepth = estimateFoldNotchDepth(blocks, minY, quiltH);
    if (!(notchDepth > 0.5)) return [];
    const foldY = minY + quiltH;
    const pad = Math.max(0, Number(bleed) || 0);
    const xs = [];
    const rights = [];
    for (const block of blocks) {
      const x = Number(block?.x);
    ...
    const maxX = Math.max(...rights);
    const width = Math.max(1e-6, maxX - minX);
    const samples = Math.max(32, Math.min(128, Math.ceil(width / 6)));
    const stripW = width / samples;
    const rects = [];
    let run = null;
    const flush = () => {
      if (!run) return;
      rects.push({
        x: run.x0 - pad,
        y: run.y0 - pad,
        width: run.x1 - run.x0 + pad * 2,
        height: foldY - run.y0 + pad * 2,
        fill: run.fill
      });
      run = null;
    };
    for (let i = 0; i < samples; i++) {
      ...
      for (const block of blocks) {
        ...
        const bh = Number(block?.height);
        if (![bx, bw, by, bh].every(Number.isFinite) || bw <= 0 || bh <= 0) continue;
        if (xMid < bx || xMid > bx + bw) continue;
        const bottom = by + bh;
        if (bottom >= coverBottom) {
          coverBottom = bottom;
          fill = _blockFillColor(block);
        }
      }
      if (!(coverBottom > -Infinity) || foldY - coverBottom <= 0.5) {
        flush();
        continue;
      }
      const y0 = coverBottom;
      if (run && run.fill === fill && Math.abs(run.y0 - y0) < 0.75) {
        run.x1 = x1;
      } else {
        flush();
        run = { x0, x1, y0, fill };
  ...
  /**
   * Split-band simple: flip flags for the lower band.
   * Full scale(-1,-1) empties the band — default Mirror keeps seam-anchored horizontal flip.
   */
  function splitBandSimpleMirrorFieldTransform(minX, minY, quiltW, quiltH, seamRel, flags = {}) {
    ...
    const flipY = odqNormalizeMirrorFlipFlag(flags.flipY, MIRROR_TUNE_DEFAULT.flipY);
    if (flipX && flipY) {
      const flipXPart = `translate(${cx} 0) scale(-1 1) translate(${-cx} 0)`;
      const flipYPart = `translate(0 ${cy}) scale(1 -1) translate(0 ${-cy})`;
      return `${flipYPart} ${flipXPart}`;
    }
    if (!flipX && !flipY) {
      return 'translate(0 0)';
    }
    if (!flipX && flipY) {
      return `translate(0 ${cy}) scale(1 -1) translate(0 ${-cy})`;
    }
    return `translate(${cx} 0) scale(-1 1) translate(${-cx} 0)`;
  }
  ...
  function computeSplitBandLayout(options = {}) {
    ...
    void contentSeamPx;
    /**
     * Mirror is painted under primary. Fixed 8px left fold notches (jagged block bottoms)
     * transparent → page white. Tuck by measured notch depth (floor 4% / cap 35% of primary).
     */
    const seamOverlapPx = computeSplitBandSeamOverlapPx(
      options.blocks,
      minY,
      quiltH,
      primaryScaleY,
      targetPrimaryScreenH
    );
    ...
    return {
      ...
      mirrorClipRect: {
        x: 0,
        /** Band-local: clip group is translated to mirrorBandTopPx; includes seam underlap. */
        y: 0,
        width: vpW,
      ...
      contentSeamPx,
      mirrorSeamRel,
      foldNotchDepth: estimateFoldNotchDepth(options.blocks, minY, quiltH),
  ...
  root.QuiltMirrorLayout = {
    ...
    estimateFoldNotchDepth,
    computeSplitBandSeamOverlapPx,
    buildFoldNotchUnderpaintRects,
    ...
    quadSideBySideTileTransform,
    quadSideBySideTransformsFromFlags,
    solveViewBoxQuadSideBySide,
    ...
    MIRROR_BOTTOM_LAYOUT_QUAD,
    MIRROR_QUAD_TILE_IDS,
    odqNormalizeMirrorBottomLayout,
    odqMirrorBottomLayoutIsDouble,
    odqMirrorBottomLayoutIsQuad,
    odqMirrorBottomLayoutUsesDupTiles,
    odqQuadTileFlagsFromTune,
    odqQuadTileFieldsPlain,
    odqQuadTileFieldsToFirestore,
    odqParseQuadTileFields,
    odqRawHasQuadTileFields,
    odqIsoTimestampIsNewer,
    odqShouldPreferRemoteMirrorTune,
  ...
  if (typeof root !== 'undefined') {
    ...
    root.odqMirrorBottomLayoutIsQuad = odqMirrorBottomLayoutIsQuad;
    root.odqMirrorBottomLayoutUsesDupTiles = odqMirrorBottomLayoutUsesDupTiles;
    root.odqQuadTileFlagsFromTune = odqQuadTileFlagsFromTune;
    root.odqQuadTileFieldsPlain = odqQuadTileFieldsPlain;
