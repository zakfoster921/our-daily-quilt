...
(function (root) {
  ...
  function computeSplitBandLayout(options = {}) {
    ...
    void mirrorSeamOffset;
    const mirrorContentAnchorY = Number.isFinite(options.mirrorContentAnchorY)
      ? Number(options.mirrorContentAnchorY)
      : minY;
    const viewport = resolveViewport(options.viewportW, options.viewportH);
    ...
    /** Y uses visual width; X bakes horizontalStretch into the band (fieldLayer stretch is off in split-band). */
    const primaryScaleY = vpW / primaryVisualW;
    const primaryScaleX = vpW / quiltW;
    const primaryScale = primaryScaleY;
    const primaryScreenH = primaryScaleY * quiltH;

    const seamOverlapPx = primaryScaleY * MIRROR_SEAM_OVERLAP_PX;
    ...
    let mirrorBandTopPx = Math.max(0, contentSeamPx - seamOverlapPx);
    if (nudgeSeamY !== 0) {
    ...
    /** Single mirror: stretch quiltH to fill mirror band; dup×2 keeps primary Y + fillY stretch. */
    const mirrorBandScaleY = doubleSideBySide ? primaryScaleY : mirrorBandScreenH / quiltH;
    ...
    /**
     * Single mirror: quilt y=0 (block-heavy edge after XY flip) lands at band top;
     * reflection grows upward — extend clip so it meets primary without a dead zone.
     */
    const mirrorClipOverflowPx = doubleSideBySide
      ? 0
      : Math.min(
          mirrorBandTopPx,
          mirrorBandScaleY * Math.max(contentSeamRel - blockTopRel, blockSpanRel * 0.55)
        );
    const mirrorContentTy = doubleSideBySide
      ? 0
      : -mirrorBandScaleY * (quiltH - MIRROR_SEAM_OVERLAP_PX) +
        mirrorBandScaleY * MIRROR_SEAM_OVERLAP_PX;
    const mirrorContentTransform =
      `translate(${mirrorTx} ${mirrorContentTy}) scale(${mirrorBandScaleX} ${mirrorBandScaleY}) translate(0 ${-mirrorContentAnchorY})`;
    const mirrorBandTransform = `translate(0 ${mirrorBandTopPx})`;
    const mirrorClipY = Math.max(0, mirrorBandTopPx - mirrorClipOverflowPx);
    const mirrorClipHeight = mirrorBandScreenH + mirrorClipOverflowPx;
    ...
    return {
      ...
      mirrorClipRect: {
        ...
        y: mirrorClipY,
        width: vpW,
        height: mirrorClipHeight
      },
      primaryClipRect: doubleSideBySide
        ? null
        : {
            x: 0,
            y: 0,
            width: vpW,
            height: mirrorBandTopPx + seamOverlapPx
          },
      contentSeamRel,
      contentSeamPx,
