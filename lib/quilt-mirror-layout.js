...
(function (root) {
  ...
  function computeSplitBandContentSeamRel(blocks, minY, quiltH) {
    if (!Array.isArray(blocks) || !blocks.length) return quiltH;
    let wSum = 0;
    let bottomSum = 0;
    for (const block of blocks) {
    ...
    const span = Math.max(1e-6, blockBottomRel - blockTopRel);
    /** Bias seam toward block mass (top-heavy quilts start mirror higher). */
    return Math.min(
      quiltH,
      Math.max(blockTopRel + span * 0.35, weightedBottom - span * 0.08)
    );
  }
  ...
  function computeSplitBandLayout(options = {}) {
    ...
    /** Pull mirror blocks up toward content seam without collapsing the band. */
    const mirrorPullUpPx = doubleSideBySide
      ? 0
      : Math.max(0, mirrorBandScreenH - primaryScreenH) +
        Math.max(0, primaryScreenH - contentSeamPx) * 0.35;
    const mirrorContentTy = doubleSideBySide
      ? 0
      : mirrorBandScaleY * MIRROR_SEAM_OVERLAP_PX - mirrorPullUpPx;
