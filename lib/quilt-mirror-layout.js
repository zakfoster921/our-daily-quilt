...
(function (root) {
  ...
  function splitBandSimpleMirrorFieldTransform(minX, minY, quiltW, quiltH, seamRel, flags = {}) {
    ...
    if (!flipX && flipY) {
      return `translate(0 ${seamY}) scale(1 -1) translate(0 ${-seamY})`;
    }
  ...
  function computeSplitBandLayout(options = {}) {
    ...
    let mirrorBandScaleY = doubleSideBySide ? primaryScaleY : mirrorBandScreenH / mirrorSeamRel;
    if (!doubleSideBySide && !mirrorFlipY) {
      mirrorBandScaleY = -Math.abs(mirrorBandScaleY);
    }
