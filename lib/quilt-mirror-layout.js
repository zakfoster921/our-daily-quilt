...
(function (root) {
  ...
  /**
   * Dup ×4 grid-only: top row = Dup ×2 L/R flags; bottom row = Flip Y of that row.
  ...
  function quadSideBySideTransformsFromFlags(
    ...
    const topRight = {
      flipX: odqNormalizeMirrorFlipFlag(rightFlags.flipX, false),
      flipY: odqNormalizeMirrorFlipFlag(rightFlags.flipY, false)
    };
    const bottomLeft = { flipX: topLeft.flipX, flipY: !topLeft.flipY };
    const bottomRight = { flipX: topRight.flipX, flipY: !topRight.flipY };
    return [
      quadSideBySideTileTransform(minX, minY, quiltW, quiltH, 0, 0, topLeft),
      quadSideBySideTileTransform(minX, minY, quiltW, quiltH, halfW, 0, topRight),
      quadSideBySideTileTransform(minX, minY, quiltW, quiltH, 0, rowStep, bottomLeft),
      quadSideBySideTileTransform(minX, minY, quiltW, quiltH, halfW, rowStep, bottomRight)
    ];
  }
  ...
  /** 2×2 grid of Dup×2-style tiles (no primary above). Bottom row = Flip Y of top row. */
  ...
  function odqMirrorTuneModeLabel(tune) {
    ...
    if (odqMirrorBottomLayoutIsQuad(t)) {
      return `Dup ×4 (${odqMirrorTileFlipLabel('L', t.leftFlipX, t.leftFlipY)} ${odqMirrorTileFlipLabel('R', t.rightFlipX, t.rightFlipY)})`;
    }
