...
(function (root) {
  ...
  function computeSplitBandLayout(options = {}) {
    ...
    const horizontalStretch = Number(options.horizontalStretch) || HORIZONTAL_STRETCH;
    const doubleSideBySide = options.doubleSideBySide === true;
    const nudgeMirrorY = Number(options.nudgeMirrorY) || 0;
    ...
    if (nudgeMirrorY !== 0) {
      mirrorBandTopPx += nudgeMirrorY * vpH;
      mirrorBandTopPx = Math.max(0, Math.min(vpH - 1, mirrorBandTopPx));
      mirrorBandScreenH = Math.max(1, vpH - mirrorBandTopPx);
    }

    const mirrorSpanFactor = doubleSideBySide ? DOUBLE_BOTTOM_WIDTH_FACTOR : 1;
    const mirrorVisualW = primaryVisualW * mirrorSpanFactor;
    const mirrorScaleX = vpW / (primaryScale * mirrorVisualW);
    ...
    const foldAnchorY = minY + quiltH - MIRROR_SEAM_OVERLAP_PX;
    const mirrorTx = (vpW - mirrorBandScaleX * mirrorVisualW) / 2 - mirrorBandScaleX * minX;
    const mirrorTransform =
      `translate(${mirrorTx} ${mirrorBandTopPx}) scale(${mirrorBandScaleX} ${mirrorBandScaleY}) translate(0 ${-foldAnchorY})`;
