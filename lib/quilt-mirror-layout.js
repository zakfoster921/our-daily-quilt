...
(function (root) {
  ...
  function odqMirrorTuneSnapshotsEqual(a, b) {
    return (
      a.flipX === b.flipX &&
      a.flipY === b.flipY &&
      odqNormalizeMirrorSeamNudge(a.nudgeSeamY) === odqNormalizeMirrorSeamNudge(b.nudgeSeamY) &&
      odqNormalizeMirrorSeamNudge(a.nudgeMirrorY) === odqNormalizeMirrorSeamNudge(b.nudgeMirrorY)
    );
  }

  function odqBuildMirrorTuneHistoryEntry(beforeTune, afterTune, options = {}) {
    const before = {
      flipX: odqNormalizeMirrorFlipFlag(beforeTune?.flipX, MIRROR_TUNE_DEFAULT.flipX),
      flipY: odqNormalizeMirrorFlipFlag(beforeTune?.flipY, MIRROR_TUNE_DEFAULT.flipY),
      nudgeSeamY: odqNormalizeMirrorSeamNudge(beforeTune?.nudgeSeamY),
      nudgeMirrorY: odqNormalizeMirrorSeamNudge(beforeTune?.nudgeMirrorY)
    };
    const after = {
      flipX: odqNormalizeMirrorFlipFlag(afterTune?.flipX, MIRROR_TUNE_DEFAULT.flipX),
      flipY: odqNormalizeMirrorFlipFlag(afterTune?.flipY, MIRROR_TUNE_DEFAULT.flipY),
      nudgeSeamY: odqNormalizeMirrorSeamNudge(afterTune?.nudgeSeamY),
      nudgeMirrorY: odqNormalizeMirrorSeamNudge(afterTune?.nudgeMirrorY)
    };
  ...
  function odqWriteMirrorTuneLocal(dateKey, tune) {
    ...
    const payload = { flipX, flipY, nudgeSeamY, nudgeMirrorY };
  ...
  async function odqWriteMirrorTuneViaServer(dateKey, tune, writeOptions = {}) {
    ...
    const blockCount = Number.isFinite(writeOptions.blockCount)
      ...
      : Array.isArray(writeOptions.blocks)
        ...
        : null;
    const historyEntry = odqBuildMirrorTuneHistoryEntry(
      beforeTune,
      { flipX, flipY, nudgeSeamY, nudgeMirrorY },
      {
        at: updatedAt,
