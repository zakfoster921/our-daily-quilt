...
(function (root) {
  ...
  /** Split-band single mirror: flip at fold without meet-mode vertical stack offset. */
  function splitBandBottomFieldTransformFromFlags(minX, minY, quiltW, quiltH, flags = {}, fillY = 1) {
    ...
    const stretchY = Math.max(1, Number(fillY) || 1);
    if (Math.abs(stretchY - 1) < 1e-6) return transform;
    return `${transform} translate(0 ${minY}) scale(1 ${stretchY}) translate(0 ${-minY})`;
  }
  ...
  function computeSplitBandLayout(options = {}) {
    ...
    const primaryScale = vpW / primaryVisualW;
    const primaryScreenH = primaryScale * quiltH;

    const seamOverlapPx = primaryScale * MIRROR_SEAM_OVERLAP_PX;
    let mirrorBandTopPx = Math.max(0, primaryScreenH - seamOverlapPx);
    if (nudgeSeamY !== 0) {
    ...
    /** Dup×2: two full-width tiles offset by halfW → 1.5× visual span (meet crop factor). */
    const mirrorAlignW = doubleSideBySide ? primaryVisualW * DOUBLE_BOTTOM_WIDTH_FACTOR : mirrorVisualW;
    const mirrorScaleX = vpW / (primaryScale * mirrorAlignW);
    const mirrorScaleY = mirrorBandScreenH / (primaryScale * quiltH);
    const mirrorBandScaleX = primaryScale * mirrorScaleX;
    const mirrorBandScaleY = primaryScale;
    const mirrorDupFillY = mirrorScaleY;

    const primaryTx = (vpW - primaryScale * primaryVisualW) / 2 - primaryScale * minX;
    const primaryTransform = `translate(${primaryTx} 0) scale(${primaryScale}) translate(0 ${-minY})`;

    const mirrorTx = (vpW - mirrorBandScaleX * mirrorAlignW) / 2 - mirrorBandScaleX * minX;
    const mirrorContentTransform =
      `translate(${mirrorTx} 0) scale(${mirrorBandScaleX} ${mirrorBandScaleY}) translate(0 ${-mirrorContentAnchorY})`;
