...
(function (root) {
  ...
  function computeSplitBandLayout(options = {}) {
    ...
    const mirrorContentAnchorY = Number.isFinite(options.mirrorContentAnchorY)
      ...
      : minY + mirrorSeamOffset - MIRROR_SEAM_OVERLAP_PX;
    const viewport = resolveViewport(options.viewportW, options.viewportH);
    ...
    /** Dup×2 tiles span quiltW, not the wider meet viewBox crop factor. */
    const mirrorAlignW = doubleSideBySide ? primaryVisualW : mirrorVisualW;
    ...
    const mirrorBandScaleY = primaryScale * mirrorScaleY;
    ...
    const mirrorTransform =
      `translate(${mirrorTx} ${mirrorBandTopPx}) scale(${mirrorBandScaleX} ${mirrorBandScaleY}) translate(0 ${-mirrorContentAnchorY})`;
