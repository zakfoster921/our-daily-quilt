...
(function (root) {
  ...
  function computeSplitBandLayout(options = {}) {
    ...
    void blockTopRel;
    const primaryHeightFraction = doubleSideBySide
      ? Math.min(1, Math.max(0, (widthFitScaleY * quiltH) / Math.max(1, vpH)))
      : computeSplitBandPrimaryHeightFraction(contentSeamRel, blockSpanRel, quiltH);
    ...
    /** Single mirror: scale ink height (0→seam) to fill band; y=0 at seam, grows down. */
    const mirrorBandScaleY = doubleSideBySide ? primaryScaleY : mirrorBandScreenH / contentSeamRel;
