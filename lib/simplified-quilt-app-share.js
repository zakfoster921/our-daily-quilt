  ...
  class SimplifiedQuiltAppV2Share {
      ...
      _ensureQuiltSpotlightDimRect(svg = document.getElementById('quilt')) {
        ...
        if (!dim) {
          ...
          dim.setAttribute('fill', 'rgba(0, 0, 0, 0.36)');
      ...
      _setQuiltSpotlightDimActive(active, fadeMs = 500) {
        const svg = document.getElementById('quilt');
        if (!svg) return;
        const dim = svg.querySelector('#quiltSpotlightDim');
        const alreadyActive = svg.classList.contains('quilt-spotlight-active');
        if (!active && !alreadyActive && !dim && !this._myBlockSpotlightDimAnim) {
          return;
        }
        ...
        if (this._myBlockSpotlightDimAnim) {
          try {
            this._myBlockSpotlightDimAnim.cancel();
          } catch (_) {
            /* ignore */
          }
          this._myBlockSpotlightDimAnim = null;
        }

        const targetOpacity = active ? 1 : 0;
        const startOpacity = Number(dimEl.getAttribute('opacity'));
        const fromOpacity = Number.isFinite(startOpacity) ? startOpacity : active ? 0 : 1;
        if (!active && fromOpacity <= 0.001) {
          dimEl.setAttribute('opacity', '0');
          return;
        }
        if (fadeMs <= 0) {
          dimEl.setAttribute('opacity', String(targetOpacity));
          return;
        }

        try {
          const anim = dimEl.animate([{ opacity: fromOpacity }, { opacity: targetOpacity }], {
            duration: fadeMs,
            fill: 'forwards',
            easing: 'ease'
          });
          this._myBlockSpotlightDimAnim = anim;
          anim.onfinish = () => {
            dimEl.setAttribute('opacity', String(targetOpacity));
            if (this._myBlockSpotlightDimAnim === anim) this._myBlockSpotlightDimAnim = null;
          };
          anim.oncancel = () => {
            if (this._myBlockSpotlightDimAnim === anim) this._myBlockSpotlightDimAnim = null;
          };
      ...
      _finishMyBlockSpotlight() {
        ...
        if (!pending?.group && !hadTimers && !dimActive && !this._myBlockSpotlightDimAnim) {
          return;
        }
        ...
        if (pending?.group) {
          pending.group.classList.remove('quilt-my-block-spotlight');
          if (pending.synthetic) pending.group.remove();
          else this._restoreFromSpotlightOverlay(pending.group, pending.state);
        }

        const overlay = svg?.querySelector('#quiltSpotlightOverlay');
        if (overlay) {
          overlay.querySelectorAll(':scope > g:not(#quiltSpotlightDim)').forEach((node) => {
            if (node !== pending?.group) node.remove();
          });
        }
      ...
      _reparentToSpotlightOverlayAsync(group) {
        return new Promise((resolve) => {
          requestAnimationFrame(() => {
            requestAnimationFrame(() => {
              resolve(this._reparentToSpotlightOverlay(group));
            });
          });
        });
      }
      ...
      async handleShowMyBlock() {
        ...
        const restoreState = await this._reparentToSpotlightOverlayAsync(sourceGroup);
        if (!restoreState?.localMatrix) {
          if (restoreState) this._restoreFromSpotlightOverlay(sourceGroup, restoreState);
          if (synthetic) sourceGroup.remove();
          this.uiService.showToast('Could not highlight your square — try again.');
          return;
        }

        sourceGroup.classList.add('quilt-my-block-spotlight');
        this._myBlockSpotlightRestore = { group: sourceGroup, state: restoreState, synthetic };
