      ...
      async getQuiltBlobForTunePreview(options = {}) {
        ...
        if (tuneFast && this.archiveService?.getInstagramQuiltSourceBlob) {
          ...
          try {
            ...
            if (blob) {
              return { blob, quiltSource: carouselPostPreview ? 'carousel-quilt-screen' : 'quilt-screen' };
            }
          } catch (quiltScreenErr) {
            this.logger.warn('Tune preview quilt-screen source failed, using blocks:', quiltScreenErr);
          }
        }
        if (tuneFast) {
          ...
          const loadTuneAssets = async () => {
            try {
              ...
              const [
                ...
                odqPromiseWithTimeout(this.getQuiltBlobForTunePreview({ tunePreviewFast: true }), 20000, 'Quilt preview').then((r) =>
