              ...
              void (async () => {
                ...
                try {
                  ...
                  if (serverApiOk) {
                    ...
                    if (typeof globalThis.odqVerifyLayoutBTuneOnServer === 'function') {
                      try {
                        ...
                        verifyResult = {
                          ok: false,
                          reason: String(verifyErr?.message || verifyErr)
                        };
                      }
                    }
                    if (verifyResult && !verifyResult.ok) {
                      this.logger.warn('Tune save: server verify mismatch', verifyResult);
                      if (firebaseReady && window.db && window.firestore) {
                        try {
                          await writeTuneToClientFirestore();
                          if (typeof globalThis.odqVerifyLayoutBTuneOnServer === 'function') {
                          ...
                          if (verifyResult?.ok) {
                            serverApiMeta = {
                              ...serverApiMeta,
                              source: 'client-firestore-verify-retry'
                            };
                          }
                        } catch (clientRetryErr) {
                          verifyResult = {
                            ok: false,
                            reason: `${verifyResult.reason}; client retry: ${String(clientRetryErr?.message || clientRetryErr)}`
                          };
                        }
                      }
                    }
                    if (verifyResult && !verifyResult.ok) {
                      cloudOk = false;
                      serverVerify = verifyResult;
                    } else {
                      cloudOk = true;
                      serverVerify = {
                        ok: true,
                        layoutBTuneUpdatedAt:
                          verifyResult?.layoutBTuneUpdatedAt ||
                          serverApiMeta?.layoutBTuneUpdatedAt ||
                          tuneSavedAt,
