      ...
      async handleAdminTuneSpeakerCutout() {
        ...
        try {
          ...
          const restoreSavedPostStripPlan = (plan) => {
            ...
            /** Firestore plans are x/y/angle only — keep story-linked typography, replay positions. */
            if (
              typeof globalThis.odqLayoutBStripPlanHasTypography === 'function' &&
              globalThis.odqLayoutBStripPlanHasTypography(plan)
            ) {
              postQuoteStyleIndependent = true;
            }
          ...
          const loadTuneAssets = async () => {
            try {
              ...
              if (!tuneUiDirty) {
                ...
                restoreSavedPostStripPlan(savedPostStripPlan);
                if (!postQuoteStyleIndependent) {
                  copyStoryQuoteStyleToPost();
                }
                storyRefStripPlan = null;
