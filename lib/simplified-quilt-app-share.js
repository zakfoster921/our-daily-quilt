...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Share {
      ...
      _setQuiltSpotlightDimActive(active, fadeMs = 500) {
        ...
        const dim = this._ensureQuiltSpotlightDimRect(svg);
        if (!dim) return;
        ...
        const startOpacity = Number(dim.getAttribute('opacity'));
        const fromOpacity = Number.isFinite(startOpacity) ? startOpacity : active ? 0 : 1;
        if (fadeMs <= 0) {
          dim.setAttribute('opacity', String(targetOpacity));
          return;
        }

        try {
          const anim = dim.animate([{ opacity: fromOpacity }, { opacity: targetOpacity }], {
            duration: fadeMs,
            fill: 'forwards',
            easing: 'ease'
          });
