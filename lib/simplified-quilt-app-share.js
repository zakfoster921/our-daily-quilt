...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Share {
        ...
        try {
          ...
          modal.innerHTML = `
            <div class="odq-speaker-tune-panel" role="dialog" aria-modal="true" aria-label="Tune today's speaker cutout">
              ...
              <p class="odq-speaker-tune-aspect-hint" style="font-size:10px;line-height:1.3;color:#555;margin:0;">Story changes also apply to Post. Post-only changes do not affect Story.</p>
          ...
          spinner.style.cssText = 'position:absolute;inset:0;display:flex;align-items:center;justify-content:center;color:#fff;font-size:14px;background:rgba(0,0,0,0.5);';
          ...
          const copyStoryDraftToPost = () => {
            ...
            p.stripLayoutSeed = s.stripLayoutSeed;
            p.keywordEmphasis = s.keywordEmphasis
              ? {
          ...
          const captureDraftFromUi = (aspect) => {
            ...
            if (a === 'story') copyStoryDraftToPost();
          };
          ...
          const renderPreview = async (presetName) => {
            if (!tuneAssetsReady || !highResBlob) {
              ...
              spinner.textContent = 'Still loading quilt and speaker…';
              return;
            }
            renderToken += 1;
            const myToken = renderToken;
            spinner.hidden = false;
            spinner.textContent = previewAspect === 'post' ? 'Rendering post…' : 'Rendering story…';
            try {
              const layoutW = 1080;
              const layoutH = previewAspect === 'post' ? 1350 : 1920;
              const composeOpts = {
                tuneAspect: previewAspect,
                keywordEmphasis: getKeywordEmphasisFromForm(),
                keywordEmphasisExplicit: true,
                stripLayoutSeed,
                stripLayoutSeedExplicit: true
              };
              const speakerOverlayBase = {
                enabled: true,
                imageUrl: speakerImageForCanvas,
                name: speakerName,
                washColor
              };
              const transform = odqSpeakerCutoutTransformResolved(presetName, {
                cx: nudgeCx,
                cy: nudgeCy,
                rotateDeg: nudgeRotateDeg
              });
              if (previewAspect === 'post') {
              ...
              const speakerKind = /^data:/i.test(String(speakerImageForCanvas || ''))
                ? 'data'
                : /^https?:/i.test(String(speakerImageForCanvas || ''))
                  ? 'url'
                  : 'other';
              ...
              setTuneDebug(
                `ok ${layoutW}×${layoutH} ${previewAspect} ${tuneQuiltSource} ${speakerKind} tok ${myToken}/${renderToken}`
              );
            } catch (renderErr) {
              ...
              const hideSpinner = myToken === renderToken;
              if (hideSpinner) spinner.hidden = true;
            }
          };
          ...
          kwInput.addEventListener('input', scheduleKeywordPreview);
          kwInput.addEventListener('change', scheduleKeywordPreview);
          ...
          const loadTuneAssets = async () => {
            try {
              ...
              const [quiltBlob, kwStory, kwPost, stripStory, stripPost] = await Promise.all([
      ...
      getMySpotlightBlockId() {
        const engineUid = this.quiltEngine.deviceId;
        const appUid = this.currentUserId;
        const subs = this.quiltEngine.getUserContributions().submissions;
        let lastContribId = null;
        for (let i = 0; i < subs.length; i++) {
          const c = subs[i];
          if (c && (c.userId === engineUid || c.userId === appUid)) {
        ...
        const mine = this.quiltEngine.blocks.filter((b) => {
          if (b.contributorId === appUid || b.contributorId === engineUid) return true;
          const ids = Array.isArray(b.contributorIds) ? b.contributorIds : [];
          return ids.some((id) => id === appUid || id === engineUid);
        });
        if (mine.length) {
      ...
      isExitChamberCtaEnabled() {
        return CONFIG.APP.exitChamberCtaEnabled === true;
      }
      ...
      ensureExitChamberCtaDisabled() {
        if (this.isExitChamberCtaEnabled()) return;
        document.documentElement.dataset.exitChamberCtaDisabled = '1';
        const btn = document.getElementById('quiltDailyColorsCtaBtn');
        if (btn) {
          btn.hidden = true;
          btn.disabled = true;
      ...
      getDevicePersonalColorSubmissions() {
        ...
        return submissions.filter((c) => {
          ...
          if (!Utils.validateHexColor(color)) return false;
          return true;
        });
      }
      ...
      getExitChamberTodayPieceData(dateKey = Utils.getTodayKey()) {
        ...
        if (!block) {
          const color = String(submission.color || '').trim().toLowerCase();
          block = blocks
            .filter((b) => belongsToDevice(b) && String(b?.color || '').trim().toLowerCase() === color)
            .sort((a, b) => (Number(a.submissionIndex) || 0) - (Number(b.submissionIndex) || 0))
            .pop() || null;
        }
        const color = Utils.validateHexColor(block?.color) ? block.color : submission.color;
        return Utils.validateHexColor(color) ? { submission, block, color } : null;
      }

      openExitChamberClassic() {
        if (!this.isExitChamberCtaEnabled()) return;
        this.applyExitChamberBackgroundFromTodayColor();
        this._exitChamberScrapSeed = 0;
        this.bindExitChamberTextureCycleGestures();
        const kicker = document.getElementById('exitChamberKickerBottom');
        if (kicker) {
          window.clearTimeout(this._exitChamberKickerRevealTimer);
          this._exitChamberKickerRevealTimer = null;
          kicker.classList.remove('is-revealed');
        }
        this.setNextExitChamberFarewellPhrase();
        this.uiService.showScreen('screen-exit-chamber');
        this.refreshExitChamberTodayPiece();
        this.refreshExitChamberMoodPaper();
        requestAnimationFrame(() => {
          this.applyExitChamberSpeakerTexture();
        });
        if (kicker) {
          this.scheduleExitChamberKickerUnderlineReveal(kicker, 140);
        }
      }

      refreshExitChamberTodayPiece() {
        const section = document.getElementById('exitChamberTodayPiece');
        const mount = document.getElementById('exitChamberTodayPiecePatch');
        if (!section || !mount) return;
        const data = this.getExitChamberTodayPieceData();
        mount.innerHTML = '';
        if (!data) {
          section.hidden = true;
          const snapshot = document.getElementById('exitChamberQuiltSnapshot');
          if (snapshot) {
            snapshot.innerHTML = '';
        ...
        section.setAttribute('aria-label', `Your piece today, ${data.color}`);
        mount.style.setProperty('--exit-chamber-piece-color', data.color);
        this.renderExitChamberTodayPieceSvg(mount, data.block, data.color, 'exit');
        this.renderExitChamberFullQuiltSnapshotSvg(document.getElementById('exitChamberQuiltSnapshot'));
        this.scheduleExitChamberCollageLayout(data.block);
        this.refreshQuiltUserShapeCard();
      }
      ...
      renderExitChamberTodayPieceSvg(
        mount,
        block,
        color,
        gradientIdPrefix = 'exit',
        overlayName = '',
        overlayColorName = ''
      ) {
        if (!mount) return;
        ...
        const safeColor = Utils.validateHexColor(color) ? color : CONFIG.APP.defaultColor;
        const sourceBlock = block || {
          id: `today-piece-${Utils.getTodayKey()}`,
          x: 0,
          y: 0,
          width: 96,
          height: 96,
        ...
        const w = Math.max(24, Number(sourceBlock.width) || 96);
        const h = Math.max(24, Number(sourceBlock.height) || 96);
        const previewBlock = {
          ...sourceBlock,
          x: 0,
          y: 0,
          width: w,
          height: h,
          color: Utils.validateHexColor(sourceBlock.color) ? sourceBlock.color : safeColor
        };
        const sourceX = Number(sourceBlock.x) || 0;
        const sourceY = Number(sourceBlock.y) || 0;
        if (Number.isFinite(Number(sourceBlock.insetCx))) previewBlock.insetCx = Number(sourceBlock.insetCx) - sourceX;
        if (Number.isFinite(Number(sourceBlock.insetCy))) previewBlock.insetCy = Number(sourceBlock.insetCy) - sourceY;

        const isQuiltDeboss = gradientIdPrefix === 'quilt';
        const pad = isQuiltDeboss
          ? Math.max(3, Math.min(w, h) * 0.06)
          : Math.max(8, Math.min(w, h) * 0.2);
        const svg = make('svg');
        svg.setAttribute('viewBox', `${-pad} ${-pad} ${w + pad * 2} ${h + pad * 2}`);
        svg.setAttribute('preserveAspectRatio', 'xMidYMid meet');
        svg.setAttribute('role', 'img');
        svg.setAttribute('aria-label', `Your piece today, ${safeColor}`);

        const defs = make('defs');
        const grainId = `${gradientIdPrefix}TodayPieceGrain`;
        const washId = `${gradientIdPrefix}TodayPieceColorWash`;
        const liftId = `${gradientIdPrefix}TodayPieceLiftShadow`;
        const cornerId = `${gradientIdPrefix}TodayPieceLiftedCorner`;
        const grainPattern = make('pattern');
        grainPattern.setAttribute('id', grainId);
        grainPattern.setAttribute('patternUnits', 'userSpaceOnUse');
        grainPattern.setAttribute('width', '17');
        grainPattern.setAttribute('height', '17');
        grainPattern.setAttribute('patternTransform', 'rotate(-13)');
        [
          ['path', { d: 'M 0 3.2 C 4.2 2.7, 8.6 3.8, 17 2.9', fill: 'none', stroke: 'rgba(255, 250, 236, 0.42)', 'stroke-width': '0.42' }],
          ['path', { d: 'M 0 10.8 C 5.4 11.6, 9.6 10.4, 17 11.2', fill: 'none', stroke: 'rgba(45, 36, 29, 0.12)', 'stroke-width': '0.34' }],
          ['circle', { cx: '3.2', cy: '4.1', r: '0.72', fill: 'rgba(255, 244, 220, 0.42)' }],
          ['circle', { cx: '12.6', cy: '12.2', r: '0.58', fill: 'rgba(45, 36, 29, 0.13)' }],
          ['rect', { x: '8.2', y: '2.1', width: '3.4', height: '0.48', rx: '0.24', fill: 'rgba(250, 244, 226, 0.48)' }],
          ['rect', { x: '1.1', y: '14.1', width: '5.2', height: '0.38', rx: '0.19', fill: 'rgba(45, 36, 29, 0.08)' }]
        ].forEach(([tag, attrs]) => {
          const node = make(tag);
          Object.entries(attrs).forEach(([key, value]) => node.setAttribute(key, value));
          grainPattern.appendChild(node);
        });
        defs.appendChild(grainPattern);

        const innerShadowFilterId = `${gradientIdPrefix}TodayPieceInnerShadow`;
        if (isQuiltDeboss) {
          /* Light from top-left → soft inner shadow on bottom + right edges only. */
          const innerRimRadius = Math.max(0.9, Math.min(w, h) * 0.018);
          const innerShadowFilter = make('filter');
          innerShadowFilter.setAttribute('id', innerShadowFilterId);
          innerShadowFilter.setAttribute('x', '-35%');
          innerShadowFilter.setAttribute('y', '-35%');
          innerShadowFilter.setAttribute('width', '170%');
          innerShadowFilter.setAttribute('height', '170%');
          innerShadowFilter.setAttribute('color-interpolation-filters', 'sRGB');

          const shadowLightMaskSvg = [
          ...
          const lightMaskImage = make('feImage');
          lightMaskImage.setAttribute('href', `data:image/svg+xml,${encodeURIComponent(shadowLightMaskSvg)}`);
          lightMaskImage.setAttribute('preserveAspectRatio', 'none');
          lightMaskImage.setAttribute('result', 'lightMask');

          const erode = make('feMorphology');
          erode.setAttribute('in', 'SourceAlpha');
          erode.setAttribute('operator', 'erode');
          erode.setAttribute('radius', String(innerRimRadius));
          erode.setAttribute('result', 'eroded');

          const rimMask = make('feComposite');
          rimMask.setAttribute('in', 'SourceAlpha');
          rimMask.setAttribute('in2', 'eroded');
          rimMask.setAttribute('operator', 'out');
          rimMask.setAttribute('result', 'innerRim');

          const directionalRim = make('feComposite');
          directionalRim.setAttribute('in', 'innerRim');
          directionalRim.setAttribute('in2', 'lightMask');
          directionalRim.setAttribute('operator', 'in');
          directionalRim.setAttribute('result', 'directionalRim');

          const softRim = make('feGaussianBlur');
          softRim.setAttribute('in', 'directionalRim');
          softRim.setAttribute('stdDeviation', '1.65');
          softRim.setAttribute('result', 'softRim');

          const clipInside = make('feComposite');
          clipInside.setAttribute('in', 'softRim');
          clipInside.setAttribute('in2', 'SourceAlpha');
          clipInside.setAttribute('operator', 'in');
          clipInside.setAttribute('result', 'softRimInside');

          const flood = make('feFlood');
          flood.setAttribute('flood-color', '#2d241d');
          flood.setAttribute('flood-opacity', '0.07');
          flood.setAttribute('result', 'shadowColor');

          const innerShadow = make('feComposite');
          innerShadow.setAttribute('in', 'shadowColor');
          innerShadow.setAttribute('in2', 'softRimInside');
          innerShadow.setAttribute('operator', 'in');
          innerShadow.setAttribute('result', 'innerShadow');

          const merge = make('feMerge');
          const mergeGraphic = make('feMergeNode');
          mergeGraphic.setAttribute('in', 'SourceGraphic');
          const mergeShadow = make('feMergeNode');
          mergeShadow.setAttribute('in', 'innerShadow');
          merge.appendChild(mergeGraphic);
          merge.appendChild(mergeShadow);

          innerShadowFilter.appendChild(lightMaskImage);
          innerShadowFilter.appendChild(erode);
          innerShadowFilter.appendChild(rimMask);
          innerShadowFilter.appendChild(directionalRim);
          innerShadowFilter.appendChild(softRim);
          innerShadowFilter.appendChild(clipInside);
          innerShadowFilter.appendChild(flood);
          innerShadowFilter.appendChild(innerShadow);
          innerShadowFilter.appendChild(merge);
          defs.appendChild(innerShadowFilter);
        }

        const washGradient = make('linearGradient');
        washGradient.setAttribute('id', washId);
        washGradient.setAttribute('x1', '0%');
        washGradient.setAttribute('y1', '0%');
        washGradient.setAttribute('x2', '100%');
        washGradient.setAttribute('y2', '100%');
        [
          ['0%', 'rgba(255, 255, 255, 0.48)'],
          ['42%', 'rgba(255, 255, 255, 0)'],
          ['100%', 'rgba(37, 29, 22, 0.26)']
        ].forEach(([offset, colorValue]) => {
          const stop = make('stop');
          stop.setAttribute('offset', offset);
          stop.setAttribute('stop-color', colorValue);
          washGradient.appendChild(stop);
        });
        defs.appendChild(washGradient);

        const liftGradient = make('linearGradient');
        liftGradient.setAttribute('id', liftId);
        liftGradient.setAttribute('x1', '0%');
        liftGradient.setAttribute('y1', '0%');
        liftGradient.setAttribute('x2', '100%');
        liftGradient.setAttribute('y2', '100%');
        [
          ['0%', 'rgba(255, 255, 255, 0)'],
          ['58%', 'rgba(45, 36, 29, 0.04)'],
          ['100%', 'rgba(45, 36, 29, 0.24)']
        ].forEach(([offset, colorValue]) => {
          const stop = make('stop');
          stop.setAttribute('offset', offset);
          stop.setAttribute('stop-color', colorValue);
          liftGradient.appendChild(stop);
        });
        defs.appendChild(liftGradient);

        const liftedCorner = make('radialGradient');
        liftedCorner.setAttribute('id', cornerId);
        liftedCorner.setAttribute('cx', '88%');
        liftedCorner.setAttribute('cy', '90%');
        liftedCorner.setAttribute('r', '46%');
        [
          ['0%', 'rgba(255, 250, 236, 0.5)'],
          ['44%', 'rgba(255, 250, 236, 0.15)'],
          ['100%', 'rgba(255, 250, 236, 0)']
        ].forEach(([offset, colorValue]) => {
          const stop = make('stop');
          stop.setAttribute('offset', offset);
          stop.setAttribute('stop-color', colorValue);
          liftedCorner.appendChild(stop);
        });
        defs.appendChild(liftedCorner);

        svg.appendChild(defs);

        const contactShadowLayer = make('g');
        contactShadowLayer.setAttribute('class', 'exit-chamber-today-piece__contact-shadow');
        const edgeLayer = make('g');
        edgeLayer.setAttribute('class', 'exit-chamber-today-piece__edge');
        const mainLayer = make('g');
        mainLayer.setAttribute('class', 'exit-chamber-today-piece__main-layer');
        const washLayer = make('g');
        washLayer.setAttribute('class', 'exit-chamber-today-piece__wash');
        const grainLayer = make('g');
        grainLayer.setAttribute('class', 'exit-chamber-today-piece__grain');
        const bevelLayer = make('g');
        bevelLayer.setAttribute('class', 'exit-chamber-today-piece__bevel');
        const pencilLayer = make('g');
        pencilLayer.setAttribute('class', 'exit-chamber-today-piece__pencil-outline');
        svg.appendChild(contactShadowLayer);
        svg.appendChild(edgeLayer);
        const pieceVisual = make('g');
        pieceVisual.setAttribute('class', 'exit-chamber-today-piece__visual');
        if (isQuiltDeboss) {
          pieceVisual.setAttribute('filter', `url(#${innerShadowFilterId})`);
        }
        pieceVisual.appendChild(mainLayer);
        svg.appendChild(pieceVisual);
        svg.appendChild(washLayer);
        svg.appendChild(grainLayer);
        svg.appendChild(bevelLayer);
        svg.appendChild(pencilLayer);

        const pencilPath = make('path');
        pencilPath.setAttribute(
          'd',
          [
            `M ${-w * 0.1} ${h * 0.25}`,
            `C ${w * 0.08} ${-h * 0.03}, ${w * 0.63} ${-h * 0.06}, ${w * 0.9} ${h * 0.22}`,
            `C ${w * 1.08} ${h * 0.42}, ${w * 0.9} ${h * 0.98}, ${w * 0.48} ${h * 1.06}`,
            `C ${w * 0.09} ${h * 1.13}, ${-w * 0.18} ${h * 0.9}, ${-w * 0.16} ${h * 0.55}`,
            `C ${-w * 0.15} ${h * 0.4}, ${-w * 0.12} ${h * 0.31}, ${-w * 0.1} ${h * 0.25}`
          ].join(' ')
        );
        pencilLayer.appendChild(pencilPath);

        const pencilLeadIn = make('path');
        pencilLeadIn.setAttribute(
          'd',
          [
            `M ${-w * 0.13} ${h * 0.3}`,
            `C ${w * 0.02} ${-h * 0.03}, ${w * 0.34} ${-h * 0.11}, ${w * 0.67} ${h * 0.03}`
          ].join(' ')
        );
        pencilLayer.appendChild(pencilLeadIn);

        const appendMaterialCopies = (shape, fill) => {
          if (!shape) return null;
          const fillColor = Utils.validateHexColor(fill) ? fill : safeColor;
          shape.setAttribute('fill', fillColor);
          shape.setAttribute('stroke', 'none');

          if (!isQuiltDeboss) {
            const contact = shape.cloneNode(true);
            contact.setAttribute('fill', 'rgba(45, 36, 29, 0.18)');
            contact.setAttribute('stroke', 'none');
            contact.setAttribute('transform', 'translate(2.2 3.2)');
            contactShadowLayer.appendChild(contact);

            const edge = shape.cloneNode(true);
            edge.setAttribute('fill', 'rgba(232, 219, 191, 0.98)');
            edge.setAttribute('stroke', 'rgba(92, 74, 54, 0.2)');
            edge.setAttribute('stroke-width', String(Math.max(1, Math.min(w, h) * 0.018)));
            edge.setAttribute('stroke-linejoin', 'round');
            edge.setAttribute('transform', 'translate(1.9 2.1)');
            edgeLayer.appendChild(edge);
          }

          mainLayer.appendChild(shape);

          if (!isQuiltDeboss) {
            const wash = shape.cloneNode(true);
            wash.setAttribute('fill', `url(#${washId})`);
            wash.setAttribute('stroke', 'none');
            washLayer.appendChild(wash);

            const bevel = shape.cloneNode(true);
            bevel.setAttribute('fill', `url(#${liftId})`);
            bevel.setAttribute('stroke', 'none');
            bevelLayer.appendChild(bevel);

            const liftedCornerHighlight = shape.cloneNode(true);
            liftedCornerHighlight.setAttribute('fill', `url(#${cornerId})`);
            liftedCornerHighlight.setAttribute('stroke', 'none');
            bevelLayer.appendChild(liftedCornerHighlight);
          }

          const grain = shape.cloneNode(true);
          grain.setAttribute('fill', `url(#${grainId})`);
          grain.setAttribute('stroke', 'none');
          grainLayer.appendChild(grain);

          return shape;
        };

        const appendPolygon = (points, fill, className = '') => {
          const pts = (points || [])
            .map((p) => {
          ...
          poly.setAttribute('fill', Utils.validateHexColor(fill) ? fill : safeColor);
          poly.setAttribute('stroke', 'none');
          if (className) poly.setAttribute('class', className);
          return appendMaterialCopies(poly, fill);
        };

        if (Array.isArray(previewBlock.polygonPieces) && previewBlock.polygonPieces.length) {
          previewBlock.polygonPieces.forEach((piece) => {
            appendPolygon(piece?.points, piece?.color || previewBlock.color, 'exit-chamber-today-piece__shape-part');
          });
        } else if (previewBlock.patternType === 'special' && previewBlock.specialPatternType === 'hst') {
          const triangles =
            typeof Utils.getHstOrganicRenderTriangles === 'function'
              ? Utils.getHstOrganicRenderTriangles(previewBlock, 0.7)
              : [];
          const userColor = String(previewBlock.color || safeColor).trim().toLowerCase();
          const userTriangles = triangles.filter((tri) => String(tri?.color || '').trim().toLowerCase() === userColor);
          (userTriangles.length ? userTriangles : triangles).forEach((tri) => {
            appendPolygon(tri?.points, tri?.color || previewBlock.color, 'exit-chamber-today-piece__shape-part');
          });
        } else {
          const main =
            this.renderer?.createHandCutPolygon?.(0, 0, w, h, 0.72, `exit-piece:${previewBlock.id || safeColor}`) ||
            appendPolygon([[0, 0], [w, 0], [w, h], [0, h]], previewBlock.color);
          if (main) {
            if (!main.parentNode) appendMaterialCopies(main, previewBlock.color);
          }
          if (previewBlock.patternType === 'special' && previewBlock.specialPatternType === 'insetCircle') {
            const innerColor = Utils.validateHexColor(previewBlock.insetInnerColor)
              ? previewBlock.insetInnerColor
              : previewBlock.color;
            const spec =
              typeof Utils.insetCircleOrganicSectorPointsLocal === 'function'
                ? Utils.insetCircleOrganicSectorPointsLocal(previewBlock, 0.7)
                : { kind: 'none', points: [] };
            if (spec.kind !== 'none' && spec.points?.length >= 3) {
              appendPolygon(spec.points, innerColor, 'exit-chamber-today-piece__shape-part');
            }
          }
        }

        const nameOnPiece = String(overlayName || '').trim();
        const colorNameOnPiece = String(overlayColorName || '').trim();
        if (isQuiltDeboss && (nameOnPiece || colorNameOnPiece)) {
          const cx = w * 0.5;
          const cy = h * 0.5;
          const fontSize = Math.max(13, Math.min(w, h) * 0.26);
          const colorFontSize = Math.max(9, fontSize * 0.4);
          const rr = parseInt(safeColor.slice(1, 3), 16);
          const rg = parseInt(safeColor.slice(3, 5), 16);
          const rb = parseInt(safeColor.slice(5, 7), 16);
          const pieceLuminance = (0.299 * rr + 0.587 * rg + 0.114 * rb) / 255;
          const labelColor = pieceLuminance > 0.56 ? '#1a2438' : '#f7f4ee';
          const labelOpacity = 0.72;
          const nameLayer = make('g');
          nameLayer.setAttribute('class', 'quilt-user-shape-card__name-label');
          nameLayer.setAttribute('opacity', String(labelOpacity));
          nameLayer.setAttribute('transform', `rotate(-5 ${cx} ${cy})`);
          let labelStackOffset = 0;
          if (nameOnPiece) {
            const nameText = make('text');
            nameText.setAttribute('class', 'quilt-user-shape-card__name-user');
            nameText.setAttribute('x', String(cx));
            nameText.setAttribute('y', String(cy - fontSize * 0.62));
            nameText.setAttribute('text-anchor', 'middle');
            nameText.setAttribute('dominant-baseline', 'middle');
            nameText.setAttribute('fill', labelColor);
            nameText.setAttribute('font-size', String(fontSize));
            nameText.setAttribute(
              'font-family',
              "'Zakarack', Georgia, 'Times New Roman', serif"
            );
            nameText.textContent = nameOnPiece;
            nameLayer.appendChild(nameText);
            labelStackOffset = fontSize * 0.34;
          }
          if (colorNameOnPiece) {
            const colorText = make('text');
            colorText.setAttribute('class', 'quilt-user-shape-card__name-color');
            colorText.setAttribute('x', String(cx));
            colorText.setAttribute(
              'y',
              String(cy - fontSize * 0.2 + (nameOnPiece ? 0 : colorFontSize * 0.15))
            );
            colorText.setAttribute('text-anchor', 'middle');
            colorText.setAttribute('dominant-baseline', 'middle');
            colorText.setAttribute('fill', labelColor);
            colorText.setAttribute('font-size', String(colorFontSize));
            colorText.setAttribute(
              'font-family',
              "Georgia, 'Times New Roman', Times, serif"
            );
            colorText.setAttribute('font-style', 'italic');
            colorText.textContent = colorNameOnPiece;
            nameLayer.appendChild(colorText);
            labelStackOffset = Math.max(labelStackOffset, fontSize * 0.22 + colorFontSize * 0.5);
          }

          const heartSize = fontSize * 0.36;
          const heartCy = cy + fontSize * 0.38 + labelStackOffset * 0.35;
          const heart = make('path');
          heart.setAttribute('class', 'quilt-user-shape-card__name-heart');
          heart.setAttribute(
            'd',
            [
              `M ${cx - heartSize * 0.06} ${heartCy + heartSize * 0.12}`,
              `C ${cx - heartSize * 1.02} ${heartCy - heartSize * 0.42}`,
              `${cx - heartSize * 0.58} ${heartCy + heartSize * 1.02}`,
              `${cx} ${heartCy + heartSize * 1.08}`,
              `C ${cx + heartSize * 0.62} ${heartCy + heartSize * 0.96}`,
              `${cx + heartSize * 1.05} ${heartCy - heartSize * 0.34}`,
              `${cx + heartSize * 0.1} ${heartCy + heartSize * 0.16}`,
              `C ${cx - heartSize * 0.38} ${heartCy - heartSize * 0.1}`,
              `${cx - heartSize * 0.04} ${heartCy + heartSize * 0.02}`,
              `${cx - heartSize * 0.06} ${heartCy + heartSize * 0.12}`
            ].join(' ')
          );
          heart.setAttribute('fill', 'none');
          heart.setAttribute('stroke', labelColor);
          heart.setAttribute('stroke-width', String(Math.max(0.7, fontSize * 0.032)));
          heart.setAttribute('stroke-linecap', 'round');
          heart.setAttribute('stroke-linejoin', 'round');
          nameLayer.appendChild(heart);
          svg.appendChild(nameLayer);
        }

        mount.appendChild(svg);
      }

      renderExitChamberFullQuiltSnapshotSvg(mount) {
        const snapshot = mount || document.getElementById('exitChamberQuiltSnapshot');
        if (!snapshot) return;
        snapshot.innerHTML = '';
        const blocks = Array.isArray(this.quiltEngine?.blocks) ? this.quiltEngine.blocks : [];
        const liveSvg = this.quiltSVG;
        const liveHasMirroredField = !!liveSvg?.querySelector?.('#quiltMirroredFieldLayer');
        if (liveHasMirroredField && liveSvg.childNodes.length > 0 && liveSvg.getAttribute('viewBox')) {
          const clone = liveSvg.cloneNode(true);
          const suffix = `exitSnapshot${Date.now().toString(36)}`;
          const idMap = new Map();
          ...
          clone.querySelectorAll('*').forEach((node) => {
            Array.from(node.attributes || []).forEach((attr) => {
              const replaced = replaceIdRefs(attr.value);
              if (replaced !== attr.value) node.setAttribute(attr.name, replaced);
            });
          });
          clone.setAttribute('width', '100%');
          clone.setAttribute('height', '100%');
          clone.setAttribute('preserveAspectRatio', liveSvg.getAttribute('preserveAspectRatio') || 'xMidYMin slice');
          clone.setAttribute('role', 'img');
          clone.setAttribute('aria-label', 'Full quilt in this moment');
        ...
        const sourceBlocks = blocks.filter((block) => {
          ...
          return [x, y, width, height].every(Number.isFinite) && width > 0 && height > 0 && Utils.validateHexColor(block.color);
        });
        ...
        const NS = 'http://www.w3.org/2000/svg';
        const make = (tag) => document.createElementNS(NS, tag);
        const minX = Math.min(...sourceBlocks.map((b) => Number(b.x)));
        const minY = Math.min(...sourceBlocks.map((b) => Number(b.y)));
        const maxX = Math.max(...sourceBlocks.map((b) => Number(b.x) + Number(b.width)));
        const maxY = Math.max(...sourceBlocks.map((b) => Number(b.y) + Number(b.height)));
        const quiltW = Math.max(1, maxX - minX);
        const quiltH = Math.max(1, maxY - minY);
        const horizontalStretch = 1.16;
        const pad = -Math.max(6, quiltW * 0.02);
        const mirrorSeamOffsetRatio =
          typeof this.renderer?.getMirrorSeamOffsetRatio === 'function'
            ? this.renderer.getMirrorSeamOffsetRatio()
            : 0.68;
        const mirrorSeamOffset = quiltH * mirrorSeamOffsetRatio;
        const mirrorSeamOverlap = 8;
        const mirroredFieldWidth = quiltW * horizontalStretch;
        const mirroredFieldHeight = quiltH + mirrorSeamOffset;
        const svg = make('svg');
        svg.setAttribute('viewBox', `${minX - pad} ${minY - pad} ${mirroredFieldWidth + pad * 2} ${mirroredFieldHeight + pad * 2}`);
        svg.setAttribute('preserveAspectRatio', 'xMidYMin slice');
        svg.setAttribute('role', 'img');
        svg.setAttribute('aria-label', 'Full quilt in this moment');

        const bg = make('rect');
        bg.setAttribute('x', String(minX - pad));
        bg.setAttribute('y', String(minY - pad));
        bg.setAttribute('width', String(mirroredFieldWidth + pad * 2));
        bg.setAttribute('height', String(mirroredFieldHeight + pad * 2));
        bg.setAttribute('fill', '#ebe8e3');
        svg.appendChild(bg);

        const fieldLayer = make('g');
        fieldLayer.setAttribute('transform', `translate(${minX} 0) scale(${horizontalStretch} 1) translate(${-minX} 0)`);
        svg.appendChild(fieldLayer);

        const parallaxLayer = make('g');
        fieldLayer.appendChild(parallaxLayer);

        const appendPoly = (points, fill) => {
          const pts = (points || [])
            .map((p) => {
              const x = Number(Array.isArray(p) ? p[0] : p?.x);
              const y = Number(Array.isArray(p) ? p[1] : p?.y);
          ...
          const poly = make('polygon');
          poly.setAttribute('points', pts);
          poly.setAttribute('fill', Utils.validateHexColor(fill) ? fill : CONFIG.APP.defaultColor);
          poly.setAttribute('stroke', 'none');
          parallaxLayer.appendChild(poly);
        };
        ...
        sourceBlocks.forEach((block) => {
          ...
          if (Array.isArray(block.polygonPieces) && block.polygonPieces.length) {
            block.polygonPieces.forEach((piece) => {
              ...
              appendPoly(points, piece.color || block.color);
            });
            return;
          }
          if (block.patternType === 'special' && block.specialPatternType === 'hst') {
            const tris = typeof Utils.getHstOrganicRenderTriangles === 'function'
              ? Utils.getHstOrganicRenderTriangles(block, 0.35)
              : [];
            tris.forEach((tri) => {
              appendPoly((tri.points || []).map((p) => [x + Number(p[0]), y + Number(p[1])]), tri.color || block.color);
            });
            return;
          }
          ...
          parallaxLayer.appendChild(rect);
          if (block.patternType === 'special' && block.specialPatternType === 'insetCircle') {
            const innerColor = Utils.validateHexColor(block.insetInnerColor) ? block.insetInnerColor : block.color;
            const spec = typeof Utils.insetCircleSectorPointsWorld === 'function'
              ? Utils.insetCircleSectorPointsWorld(block, x, y)
              : { kind: 'none', points: [] };
            if (spec.kind !== 'none' && spec.points?.length >= 3) appendPoly(spec.points, innerColor);
          }
        });
        ...
        mirroredLayer.setAttribute(
          'transform',
          `translate(${minX + quiltW} ${minY + quiltH + mirrorSeamOffset - mirrorSeamOverlap}) scale(-1 -1)`
        );
        fieldLayer.appendChild(mirroredLayer);

        snapshot.appendChild(svg);
        snapshot.hidden = false;
      }

      scheduleExitChamberCollageLayout(block = null) {
        requestAnimationFrame(() => this.layoutExitChamberCollage(block));
      }

      layoutExitChamberCollage(block = null) {
        const stage = document.getElementById('exitChamberTodayPiece');
        if (!stage || stage.hidden) {
          document.getElementById('exitChamberShapeThanks')?.remove();
          return;
        }
        const mood = document.getElementById('exitChamberMoodPaper');
        const snapshot = document.getElementById('exitChamberQuiltSnapshot');
        ...
        const seedKey = [
          Utils.getTodayKey(),
          block?.id || this._latestDedicatedBlockId || '',
          block?.color || '',
          'exit-collage'
        ].join('|');
        const baseSeed = Utils.hashStringToUint(seedKey) || 1;
        const randomUnit = (salt) => (Utils.hashStringToUint(`${baseSeed}:${salt}`) % 10000) / 9999;
        const clamp = (value, min, max) => Math.min(max, Math.max(min, value));
        const applyPlacement = (el, rect, rot) => {
          el.style.setProperty('--exit-collage-x', `${Math.round(rect.x)}px`);
          el.style.setProperty('--exit-collage-y', `${Math.round(rect.y)}px`);
          el.style.setProperty('--exit-collage-w', `${Math.round(rect.w)}px`);
          el.style.setProperty('--exit-collage-rot', `${rot.toFixed(2)}deg`);
        };

        const visibleMood = mood && !mood.hidden;
        const visibleSnapshot = snapshot && !snapshot.hidden;
        const speakerCutout = document.getElementById('exitChamberSpeakerCutout');
        const visibleSpeaker = speakerCutout && !speakerCutout.hidden;
        const viewport = window.visualViewport || null;
        const viewportW = Number(viewport?.width) || window.innerWidth || document.documentElement.clientWidth || W;
        const viewportH = Number(viewport?.height) || window.innerHeight || document.documentElement.clientHeight || H;
        const screenPad = 22;
        const quiltFloatCardMax = 460;
        const quiltFloatGutter = 16;
        const quiltMoodFrame = clamp(viewportW * 0.032, 16, 20);
        const quiltMoodWidgetW = Math.min(quiltFloatCardMax * 0.9, (viewportW - quiltFloatGutter * 2) * 0.9);
        const fillScale = clamp((viewportH - 40) / 700, 0.92, 1.14);
        const speakerSizeMax = Math.min(360, viewportW * 0.86, viewportH * 0.48);
        const speakerSizeMin = Math.min(225, speakerSizeMax);
        const speakerSize = visibleSpeaker
          ? clamp(Math.min(viewportW * 0.76, viewportH * 0.44) * fillScale, speakerSizeMin, speakerSizeMax)
          : 0;
        if (visibleSpeaker) speakerCutout.style.setProperty('--exit-speaker-size', `${Math.round(speakerSize)}px`);
        const quiltMoodPaperW = Math.max(162, quiltMoodWidgetW - quiltMoodFrame * 2);
        const quiltScreenMoodPaperW = clamp(quiltMoodPaperW, 162, viewportW - screenPad * 2);
        let moodW = visibleMood ? quiltScreenMoodPaperW : 0;
        let snapW = visibleSnapshot ? clamp(viewportW * 0.49 * fillScale, 168, Math.min(290, viewportW - screenPad * 2)) : 0;
        if (visibleMood) mood.style.setProperty('--exit-collage-w', `${Math.round(moodW)}px`);
        if (visibleSnapshot) snapshot.style.setProperty('--exit-collage-w', `${Math.round(snapW)}px`);

        let moodH = visibleMood ? Math.max(82, mood.getBoundingClientRect().height || moodW * 0.46) : 0;
        let snapH = visibleSnapshot ? snapW * (16 / 9) : 0;
        const speakerRect = visibleSpeaker ? speakerCutout.getBoundingClientRect() : null;
        const speakerW = speakerRect?.width || 0;
        const speakerH = speakerRect?.height || 0;
        const footerRow = document.querySelector('#screen-exit-chamber > .quilt-footer-icon-row');
        const footerRect = footerRow?.getBoundingClientRect?.();
        const footerGapPx = 6;
        const bottomLimit = footerRect && footerRect.top > 0
          ? Math.min(viewportH, footerRect.top - footerGapPx)
          : viewportH;
        const quietGap = clamp(Math.min(viewportW, viewportH) * 0.045, 24, 36);
        const clampScreenRect = (position, w, h) => {
          const rotationBleed = 12;
          const minX = screenPad + rotationBleed;
          const maxX = viewportW - screenPad - rotationBleed - w;
          const minY = screenPad + rotationBleed;
          const maxY = bottomLimit - screenPad - rotationBleed - h;
          return {
        ...
        const rectFromPoint = (position, w, h) => ({ x: position.x, y: position.y, w, h });
        const zonePoint = (corner, w, h, salt) => {
          const xJitter = (randomUnit(`${salt}:x`) - 0.5) * Math.min(18, viewportW * 0.045);
          const yJitter = (randomUnit(`${salt}:y`) - 0.5) * Math.min(18, viewportH * 0.035);
          const x = corner.includes('right')
            ? viewportW - screenPad - w + xJitter
            : screenPad + xJitter;
          const y = corner.includes('bottom')
            ? bottomLimit - screenPad - h + yJitter
            : screenPad + yJitter;
          return { x, y };
        };
        const rawZoneRect = (corner, w, h, salt) => clampScreenRect(zonePoint(corner, w, h, salt), w, h);
        const speakerFaceZoneFrom = (pos) => {
          if (!pos || !(speakerW > 0) || !(speakerH > 0)) return null;
          return {
            x: pos.x + speakerW * 0.3,
            y: pos.y - speakerH * 0.02,
            w: speakerW * 0.48,
            h: speakerH * 0.46
          };
        };
        const clampSpeakerRect = (position) => {
          if (!(speakerW > 0) || !(speakerH > 0)) return position;
          const face = speakerFaceZoneFrom(position);
          if (!face) return position;
          let x = position.x;
          let y = position.y;
          const minFaceX = screenPad;
          const maxFaceX = viewportW - screenPad - face.w;
          const minFaceY = screenPad;
          const maxFaceY = bottomLimit - screenPad - face.h;
          x += clamp(face.x, minFaceX, Math.max(minFaceX, maxFaceX)) - face.x;
          y += clamp(face.y, minFaceY, Math.max(minFaceY, maxFaceY)) - face.y;
          return { x, y };
        };
        const rawSpeakerRect = (corner, salt) => clampSpeakerRect(zonePoint(corner, speakerW, speakerH, salt));
        const footerProtectedZone = footerRect && footerRect.top > 0
          ? { x: 0, y: Math.max(screenPad, footerRect.top - footerGapPx), w: viewportW, h: viewportH - Math.max(screenPad, footerRect.top - footerGapPx) }
          : null;
        const speakerRot = -7.5;
        const moodRot = -2.6;
        const snapRot = 3.6;

        const usableTop = screenPad + quietGap;
        const usableBottom = bottomLimit - screenPad - quietGap;
        const usableHeight = Math.max(120, usableBottom - usableTop);
        const topBandH = clamp(usableHeight * 0.31, 190, Math.min(275, usableHeight * 0.38));
        let shapeSize = clamp(topBandH * 1.04, 172, Math.min(285, viewportW - screenPad * 2, topBandH * 1.12));
        const bandOverlap = quietGap * 0.35;
        const bottomBandH = visibleMood
          ? clamp(moodH + quietGap * 0.75, 130, Math.min(260, usableHeight * 0.34))
          : clamp(usableHeight * 0.22, 120, 220);
        const middleTop = usableTop + topBandH - bandOverlap;
        const bottomBandTop = usableBottom - bottomBandH;
        const middleBottom = bottomBandTop + bandOverlap;
        const middleH = Math.max(120, middleBottom - middleTop);
        const gutter = clamp(viewportW * 0.035, 14, 24);
        const columnW = (viewportW - screenPad * 2 - gutter) / 2;
        const leftX = screenPad;
        const rightX = screenPad + columnW + gutter;
        const leftCenterX = leftX + columnW * 0.5;
        const rightCenterX = rightX + columnW * 0.5;
        const middleMidY = middleTop + middleH * 0.5;
        document.getElementById('exitChamberTemplateDebugZones')?.remove();

        if (visibleSnapshot) {
          const targetSnapW = clamp(Math.min(columnW * 0.98, middleH * 0.55), 128, Math.min(230, viewportW - screenPad * 2));
          if (targetSnapW !== snapW) {
            snapW = targetSnapW;
            snapH = snapW * (16 / 9);
            snapshot.style.setProperty('--exit-collage-w', `${Math.round(snapW)}px`);
          }
        }

        let snapPos = visibleSnapshot
          ? clampScreenRect({
              x: rightCenterX - snapW * 0.5 + (randomUnit('snap-zone-x') - 0.5) * 8,
              y: middleMidY - snapH * 0.5 + (randomUnit('snap-zone-y') - 0.5) * 10
            }, snapW, snapH)
          : null;

        const targetSpeakerSize = visibleSpeaker
          ? clamp(Math.min(columnW * 1.55, middleH * 0.92), 155, speakerSizeMax)
          : 0;
        if (visibleSpeaker) speakerCutout.style.setProperty('--exit-speaker-size', `${Math.round(targetSpeakerSize)}px`);
        let curSpeakerW = targetSpeakerSize || speakerW;
        let curSpeakerH = targetSpeakerSize || speakerH;
        let speakerPos = visibleSpeaker && curSpeakerW > 0 && curSpeakerH > 0
          ? clampSpeakerRect({
              x: leftCenterX - curSpeakerW * 0.5 + (randomUnit('speaker-zone-x') - 0.5) * 8,
              y: middleTop + middleH * 0.02 + (randomUnit('speaker-zone-y') - 0.5) * 8
            })
          : null;

        let moodPos = visibleMood
          ? clampScreenRect({
              x: viewportW * 0.5 - moodW * 0.5 + (randomUnit('mood-zone-x') - 0.5) * 8,
              y: bottomBandTop + bottomBandH * 0.5 - moodH * 0.5 - bandOverlap * 0.25 + (randomUnit('mood-zone-y') - 0.5) * 6
            }, moodW, moodH)
          : null;

        const shapeX = visibleSpeaker
          ? clamp(
              viewportW - screenPad - shapeSize * 0.44 + (randomUnit('shape-zone-x') - 0.5) * 14,
              screenPad + shapeSize / 2,
              viewportW - screenPad - shapeSize / 2
            )
          : clamp(
              viewportW * 0.5 + (randomUnit('shape-zone-x') - 0.5) * Math.min(18, viewportW * 0.035),
              screenPad + shapeSize / 2,
              viewportW - screenPad - shapeSize / 2
            );
        const shapeY = clamp(
          usableTop + topBandH * 0.46 + (randomUnit('shape-zone-y') - 0.5) * Math.min(10, topBandH * 0.08),
          usableTop + shapeSize / 2,
          usableTop + topBandH - shapeSize / 2
        );
        stage.style.setProperty('--exit-shape-size', `${Math.round(shapeSize)}px`);
        stage.style.setProperty('--exit-shape-x', `${Math.round(shapeX)}px`);
        stage.style.setProperty('--exit-shape-y', `${Math.round(shapeY)}px`);

        document.getElementById('exitChamberShapeThanksArrow')?.remove();
        const screen = document.getElementById('screen-exit-chamber');
        let shapeThanks = document.getElementById('exitChamberShapeThanks');
        if (screen) {
          if (!shapeThanks) {
            shapeThanks = document.createElement('div');
            shapeThanks.id = 'exitChamberShapeThanks';
            shapeThanks.className = 'exit-chamber-shape-thanks';
            shapeThanks.textContent = 'thanks for adding your block';
            screen.appendChild(shapeThanks);
          }
          const shapeLeft = shapeX - shapeSize / 2;
          const thanksShapeGap = 6;
          const availableThanksW = Math.max(80, shapeLeft - screenPad - thanksShapeGap);
          const thanksW = clamp(availableThanksW, 108, Math.min(248, viewportW * 0.48));
          let thanksSize = clamp(Math.min(topBandH * 0.36, thanksW * 0.38), 30, 82);
          shapeThanks.style.setProperty('--exit-shape-thanks-w', `${Math.round(thanksW)}px`);
          shapeThanks.style.setProperty('--exit-shape-thanks-size', `${thanksSize.toFixed(1)}px`);
          shapeThanks.style.setProperty(
            '--exit-shape-thanks-rot',
            `${(-2 + (randomUnit('thanks-rot') - 0.5) * 2).toFixed(2)}deg`
          );
          shapeThanks.style.visibility = 'hidden';
          shapeThanks.style.setProperty('--exit-shape-thanks-x', `${screenPad}px`);
          shapeThanks.style.setProperty('--exit-shape-thanks-y', `${usableTop}px`);
          void shapeThanks.offsetHeight;
          const maxThanksH = topBandH * 0.92;
          while (thanksSize > 26 && shapeThanks.scrollHeight > maxThanksH) {
            thanksSize -= 1.5;
            shapeThanks.style.setProperty('--exit-shape-thanks-size', `${thanksSize.toFixed(1)}px`);
          }
          const thanksH = shapeThanks.offsetHeight || thanksSize * 2.2;
          const thanksX = clamp(
            shapeLeft - thanksW - thanksShapeGap,
            screenPad,
            Math.max(screenPad, shapeLeft - thanksW - thanksShapeGap)
          );
          const thanksY = clamp(
            shapeY - thanksH * 0.48,
            usableTop + 2,
            usableTop + topBandH - thanksH - 2
          );
          shapeThanks.style.setProperty('--exit-shape-thanks-x', `${Math.round(thanksX)}px`);
          shapeThanks.style.setProperty('--exit-shape-thanks-y', `${Math.round(thanksY)}px`);
          shapeThanks.style.visibility = '';
        }

        const placedStageRect = stage.getBoundingClientRect();

        const mkStageRect = (screenPosition, w, h) => {
          const onScreen = clampScreenRect(screenPosition, w, h);
          return {
            x: onScreen.x - placedStageRect.left,
            y: onScreen.y - placedStageRect.top,
            w,
            h
          };
        };

        const applyMoodPlacement = () => {
          if (!visibleMood || !moodPos) return null;
          const moodRect = mkStageRect(moodPos, moodW, moodH);
          applyPlacement(mood, moodRect, moodRot + (randomUnit('mood-rot') - 0.5) * 2.2);
          return rectFromPoint(moodPos, moodW, moodH);
        };
        const applySpeakerPlacement = () => {
          if (!visibleSpeaker || !speakerPos || !(curSpeakerW > 0) || !(curSpeakerH > 0)) {
            return null;
          }
          speakerCutout.style.setProperty('--exit-speaker-x', `${Math.round(speakerPos.x + curSpeakerW / 2)}px`);
          speakerCutout.style.setProperty('--exit-speaker-y', `${Math.round(speakerPos.y + curSpeakerH / 2)}px`);
          speakerCutout.style.setProperty('--exit-speaker-rot', `${speakerRot.toFixed(2)}deg`);
          return rectFromPoint(speakerPos, curSpeakerW, curSpeakerH);
        };
        const applySnapshotPlacement = () => {
          if (!visibleSnapshot || !snapPos) return null;
          const snapRect = mkStageRect(snapPos, snapW, snapH);
          applyPlacement(snapshot, snapRect, snapRot + (randomUnit('snap-rot') - 0.5) * 2.4);
          return rectFromPoint(snapPos, snapW, snapH);
        };

        applyMoodPlacement();
        applySpeakerPlacement();
        applySnapshotPlacement();
      }

      buildColorByAppDateKey() {
        // Map of app-date-key -> { color, submissionIndex } using last-wins on submissionIndex.
        const submissions = this.getDevicePersonalColorSubmissions();
        const map = new Map();
      ...
      getColorCalendarCellJitter(dateKey) {
        // Deterministic per-day jitter so each date's cell tilts in its own direction.
        // Plain square fill — no clip-path. Slight overscale keeps neighbors touching
        // after rotation so the grid doesn't show diagonal gaps.
        ...
        return {
          rotate: rangeSigned(2.6),   // ±2.6° — varied tilts in both directions
          shiftX: rangeSigned(1.6),
          shiftY: rangeSigned(1.6),
          scale: 1.04 + r() * 0.04    // 1.04–1.08, light overlap to hide rotation gaps
        };
      }
      ...
      getCurrentAppMonthDate() {
        // First-of-month UTC date for the current app-day.
        const todayKey = Utils.getTodayKey();
      ...
      applyExitChamberBackgroundFromTodayColor() {
        const screen = document.getElementById('screen-exit-chamber');
        if (!screen) return null;
        // TEMP: exit chamber now uses the app's warm neutral (#ebe8e3) from
        // the CSS fallback instead of today's color. Clear any prior inline
        // overrides so the warm neutral wins. Restore the today-color body
        // below to bring the daily tint back.
        screen.style.removeProperty('--exit-chamber-bg');
        screen.style.removeProperty('--exit-chamber-text');
        screen.classList.remove('exit-chamber--dark-bg');
        return null;
      }

      getExitChamberAssignmentDateKey() {
        return (
          this.quoteService?.getQuoteCalendarKeyNow?.() ||
          Utils.getTodayKey() ||
          ''
      ...
      getExitChamberScrapColors() {
        const uniqueColors = [];
        const pushColor = (color) => {
          const value = String(color || '').trim();
          if (!Utils.validateHexColor(value)) return;
          if (!uniqueColors.includes(value)) uniqueColors.push(value);
        };
        pushColor(this._getLatestUserQuiltColor());
        const personalColors =
          typeof this.getDevicePersonalColorHistory === 'function'
            ? this.getDevicePersonalColorHistory()
            : [];
        if (Array.isArray(personalColors)) {
          for (let i = personalColors.length - 1; i >= 0 && uniqueColors.length < 6; i--) {
            pushColor(personalColors[i]);
          }
        }
      ...
      renderExitChamberPaperScraps() {
        const stage = document.getElementById('exitChamberScraps');
        if (!stage) return;
        const sortedColors = this.getExitChamberScrapColors();
        const visit = Number(this._exitChamberScrapSeed) || 0;
        const dateKey = this.getExitChamberAssignmentDateKey() || Utils.getTodayKey();
        const seed = Utils.hashStringToUint(
          `${dateKey}:exit-chamber-scraps:${visit}:${sortedColors.join('|')}`
        );
        ...
        const baseRotations = [-6, 3, 8, 7, -4, -7];
        const scrapCuts = [
        ...
        const scraps = Array.from(stage.querySelectorAll('.exit-chamber-scrap'));
        scraps.forEach((scrap, i) => {
          ...
          const textureOpacity = (0.12 + randomUnit(`texture:${i}`) * 0.1) * darkerColorFactor;
          const paperOpacity = (ink.luminance <= 118 ? 0.1 : 0.14) + randomUnit(`paper:${i}`) * 0.06;
          scrap.style.setProperty('--exit-chamber-scrap-color', color);
          scrap.style.setProperty('--exit-chamber-scrap-rgb', ink.rgb);
          const x = (randomUnit(`x:${i}`) * 2.4 - 1.2).toFixed(2);
          const y = (randomUnit(`y:${i}`) * 1.8 - 0.9).toFixed(2);
          ...
          const scale = (0.88 + randomUnit(`s:${i}`) * 0.28).toFixed(2);
          scrap.style.setProperty('--exit-chamber-scrap-x', `${x}rem`);
          scrap.style.setProperty('--exit-chamber-scrap-y', `${y}rem`);
          scrap.style.setProperty('--exit-chamber-scrap-ink-x', `${inkX}rem`);
          scrap.style.setProperty('--exit-chamber-scrap-ink-y', `${inkY}rem`);
          scrap.style.setProperty('--exit-chamber-scrap-tooth-x', `${toothX}rem`);
          scrap.style.setProperty('--exit-chamber-scrap-tooth-y', `${toothY}rem`);
          scrap.style.setProperty('--exit-chamber-scrap-texture-opacity', textureOpacity.toFixed(3));
          scrap.style.setProperty('--exit-chamber-scrap-paper-opacity', paperOpacity.toFixed(3));
          scrap.style.setProperty('--exit-chamber-scrap-rotation', `${rotation}deg`);
          scrap.style.setProperty('--exit-chamber-scrap-scale', scale);
          scrap.style.setProperty('--exit-chamber-scrap-cut', scrapCuts[i % scrapCuts.length]);
        });
      }

      getExitChamberTextureLayer() {
        return document.getElementById('exitChamberTexture');
      }

      revealExitChamberColorWash() {
        const layer = this.getExitChamberTextureLayer();
        if (!layer) return false;
        this.renderExitChamberPaperScraps();
        layer.classList.add('is-visible');
        return true;
      }

      clearExitChamberSpeakerTexture() {
        const layer = this.getExitChamberTextureLayer();
        const cutout = document.getElementById('exitChamberSpeakerCutout');
        if (cutout) {
          cutout.hidden = true;
          cutout.style.removeProperty('--speaker-image-url');
        }
        if (!layer) return;
        layer.classList.remove('is-visible');
      }

      /** Re-apply scraps when the quilt color updates while the exit chamber is open. */
      syncExitChamberSpeakerTextureFromWidget() {
        const screen = document.getElementById('screen-exit-chamber');
        if (!screen?.classList.contains('active')) return;
        this.revealExitChamberColorWash();
      }

      /** Paper-scrap collage on the exit chamber background. */
      applyExitChamberSpeakerTexture() {
        const layer = this.getExitChamberTextureLayer();
        const cutout = document.getElementById('exitChamberSpeakerCutout');
        if (!layer || !cutout) return false;
        const quote = this.quoteService?.getTodayQuote?.() || null;
        const profile = this.getQuoteSpeakerProfile?.(quote) || null;
        const imageUrl = String(this._quoteSpeakerCutoutUrl || profile?.imageUrl || '').trim();
        if (!imageUrl) {
          this.clearExitChamberSpeakerTexture();
          return false;
        }

        const washColor = String(this.getMostPopularQuiltColor?.()?.color || CONFIG.APP.defaultColor || '#ea9b9a').trim();
        const safeWashColor = /^#[0-9A-Fa-f]{6}$/.test(washColor) ? washColor : '#ea9b9a';
        const palette = this.getQuiltPaletteSummary?.()?.palette || [];
        const haloColor =
          palette.find((item) => {
            const value = String(item?.color || '').trim();
            return /^#[0-9A-Fa-f]{6}$/.test(value) && value.toLowerCase() !== safeWashColor.toLowerCase();
          })?.color ||
          this.getContrastingQuiltColorExcludingDominant?.() ||
          CONFIG.APP.defaultColor ||
          '#d6977e';
        const safeHaloColor = /^#[0-9A-Fa-f]{6}$/.test(String(haloColor || '').trim())
          ? String(haloColor).trim()
          : '#d6977e';

        cutout.style.setProperty('--speaker-image-url', `url("${imageUrl.replace(/"/g, '%22')}")`);
        cutout.style.setProperty('--speaker-halo-color', safeHaloColor);
        cutout.hidden = false;
        layer.classList.add('is-visible');
        this.scheduleExitChamberCollageLayout?.();
        return true;
      }

      cycleExitChamberPaperScraps() {
        this._exitChamberScrapSeed = (Number(this._exitChamberScrapSeed) || 0) + 1;
        const layer = this.getExitChamberTextureLayer();
        if (!layer) return;
        layer.classList.remove('is-visible');
      ...
      bindExitChamberTextureCycleGestures() {
        if (this._exitChamberTextureGesturesBound) return;
        this._exitChamberTextureGesturesBound = true;
        let lastTapAt = 0;
        const onCycleGesture = (e) => {
          if (e.target.closest('button, a, .settings-color-calendar__cell')) return;
          const now = Date.now();
          if (now - lastTapAt < 380) {
            e.preventDefault();
            lastTapAt = 0;
            this.cycleExitChamberPaperScraps();
            return;
          }
          lastTapAt = now;
        };
        const inner = document.querySelector('#screen-exit-chamber .exit-chamber-inner');
        const footer = document.querySelector('#screen-exit-chamber .exit-chamber-footer');
        [inner, footer].forEach((el) => {
          if (!el) return;
          el.addEventListener('dblclick', onCycleGesture);
          el.addEventListener('touchend', onCycleGesture, { passive: false });
        });
      }
