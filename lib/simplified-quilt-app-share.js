  ...
  class SimplifiedQuiltAppV2Share {
      ...
      async getQuiltBlobForTunePreview(options = {}) {
        ...
        if (tuneFast && this.archiveService?.getInstagramQuiltSourceBlob) {
          const screenOpts = {
            dateKey: dk,
            exportModeLabel: carouselPostPreview
            ...
            skipRendererExport: true
          };
          const budgetMs = Math.max(4000, Number(options.quiltScreenBudgetMs) || 12000);
          const screenPromise = this.archiveService
            .getInstagramQuiltSourceBlob(blocks, screenOpts)
            .then((blob) => (blob ? { blob, quiltSource: carouselPostPreview ? 'carousel-quilt-screen' : 'quilt-screen' } : null))
            .catch((quiltScreenErr) => {
              this.logger.warn('Tune preview quilt-screen source failed:', quiltScreenErr);
              return null;
            });
          const fastPromise = blocksFastMirror();
          let resolved = false;
          const firstPack = await new Promise((resolve) => {
            const tryResolveFast = () => {
              void fastPromise.then((fastPack) => {
                if (!resolved && fastPack?.blob) {
                  resolved = true;
                  resolve(fastPack);
                }
              });
            };
            screenPromise.then((pack) => {
              if (pack?.blob && !resolved) {
                resolved = true;
                resolve(pack);
                return;
              }
              if (pack === null) tryResolveFast();
            });
            window.setTimeout(() => {
              if (!resolved) tryResolveFast();
            }, budgetMs);
          });
          if (!firstPack?.blob) {
            return (await fastPromise) || { blob: null, quiltSource: 'none' };
          }
          const onUpgrade = typeof options.onQuiltUpgrade === 'function' ? options.onQuiltUpgrade : null;
          if (onUpgrade && firstPack.quiltSource !== 'quilt-screen' && firstPack.quiltSource !== 'carousel-quilt-screen') {
            void screenPromise.then((upgradePack) => {
              if (
                upgradePack?.blob &&
                (upgradePack.quiltSource === 'quilt-screen' || upgradePack.quiltSource === 'carousel-quilt-screen')
              ) {
                onUpgrade(upgradePack);
              }
            });
          }
          return firstPack;
