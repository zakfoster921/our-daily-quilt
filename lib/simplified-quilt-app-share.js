...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Share {
      ...
      /**
       * Long-press share — fortune-card timing (timer while finger is down).
       */
      async shareLayoutBStoryPreviewFromLongPress() {
        ...
        if (typeof window.odqTrack === 'function') {
          window.odqTrack('download_story_image', { method: 'story_preview_long_press_compose' });
        }
      }

      shareLayoutBStoryPreviewFromLongPressSync() {
        const shareBlob = this._resolveLayoutBStoryPreviewShareBlobSync?.();
        if (!shareBlob?.size) return false;
        const dateStr = new Date().toISOString().split('T')[0];
        const filename = `our-daily-quilt-story-collage-${dateStr}.png`;
        const shareText = `Today's OUR DAILY QUILT has ${this.quiltEngine?.submissionCount ?? 0} contributors — take a look!`;
        const imageFile = new File([shareBlob], filename, { type: 'image/png' });
        const shareData = {
