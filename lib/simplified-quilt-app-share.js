      ...
      async getQuiltBlobForTunePreview(options = {}) {
        ...
        if (carouselPostPreview && this.archiveService?.getInstagramQuiltSourceBlob) {
          ...
          const blob = await this.archiveService.getInstagramQuiltSourceBlob(blocks, {
            dateKey: dk,
            exportModeLabel: 'tune_carousel_post_quilt_screen_cover_source',
            maxEdge,
          ...
          return { blob, quiltSource: 'carousel-quilt-screen' };
        }
        if (tuneFast) {
                    ...
                    try {
                      ...
                      if (!cloudOk && serverApiOk) {
                        try {
                          ...
                          cloudOk = !!serverVerify?.ok;
                        } catch (clientRetryErr) {
                          serverVerify = {
                            ok: false,
                            reason: `${String(serverVerify?.reason || 'verify-failed')}; client retry: ${String(clientRetryErr?.message || clientRetryErr)}`
                          };
                        }
                      }
                    } catch (verifyErr) {
