...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Share {
      ...
      async handleAdminTuneSpeakerCutout() {
        ...
        try {
          ...
          const syncLinkedSpeakerDrafts = (sourceAspect = previewAspect) => {
            ...
            const a = odqNormalizeTuneAspect(sourceAspect);
          ...
          const aspectPreviewCacheKey = (aspect) => {
            ...
            return [
              ...
              odqNormalizeQuiltBgOffsetY?.(tuneDraftByAspect[a].quiltBgOffsetY) ?? 0,
          ...
          const renderPreview = async (presetName, opts = {}) => {
            ...
            const aspect = odqNormalizeTuneAspect(opts.aspect ?? previewAspect);
            const isPrefetch = opts.prefetch === true;
            const updatesVisibleTab = aspect === odqNormalizeTuneAspect(previewAspect);
            ...
            try {
              ...
              setTuneDebug(
                `ok ${layoutW}×${layoutH} ${aspect}${aspect === 'post' ? ' carousel-story' : ''}${isPrefetch ? ' prefetch' : ''} seed story:${tuneDraftByAspect.story.stripLayoutSeed} post:${tuneDraftByAspect.post.stripLayoutSeed}${postQuoteStyleIndependent ? ' post-indep' : ''} ${aspect === 'post' ? postTuneQuiltSource || tuneQuiltSource : tuneQuiltSource} tok ${myToken}/${renderToken}`
              );
            } catch (renderErr) {
              if (updatesVisibleTab) {
          ...
          const switchPreviewAspect = async (nextAspect) => {
            ...
            captureDraftFromUi(previewAspect);
            if (!postQuoteStyleIndependent) {
              syncLinkedSpeakerDrafts(previewAspect);
            }
      ...
      async handleAdminTuneQuiltMirror() {
        ...
        const normalizeBottomLayout = (value) =>
          ...
          typeof QuiltMirrorLayout.odqNormalizeMirrorBottomLayout === 'function'
            ...
            : String(value || '').trim() === 'doubleSideBySide'
              ? 'doubleSideBySide'
              : 'single';
        ...
        const formatNudge = (value) => {
          const n = clampNudge(value);
        ...
        modal.innerHTML = `
          <div class="odq-mirror-tune-panel" role="dialog" aria-modal="true" aria-label="Tune quilt mirror">
            <h2 style="margin:0 0 8px;font-size:16px;">Tune quilt mirror</h2>
            <p style="margin:0 0 10px;font-size:12px;opacity:.75;">Bottom field for ${todayKey}</p>
            <div class="odq-mirror-tune-preview-wrap" style="position:relative;background:#222;border-radius:8px;overflow:hidden;width:min(100%,240px);margin:0 auto 12px;aspect-ratio:9/16;max-height:46vh;">
              <svg id="odqMirrorTunePreviewSvg" xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="xMidYMin meet" aria-hidden="true" style="display:block;"></svg>
            </div>
            <div class="odq-mirror-tune-mode" style="margin-bottom:10px;font-size:13px;text-align:center;">Mode: <strong data-mode-label></strong></div>
            <details class="odq-mirror-tune-details" style="margin-top:8px;">
              <summary style="cursor:pointer;font-size:13px;font-weight:600;">Layout</summary>
              <label style="display:flex;align-items:center;gap:8px;margin:8px 0;font-size:14px;">
                <input type="radio" name="odq-mirror-bottom-layout" value="single" data-bottom-layout="single" />
                Single bottom field
              </label>
              <label style="display:flex;align-items:center;gap:8px;margin:8px 0;font-size:14px;">
                <input type="radio" name="odq-mirror-bottom-layout" value="doubleSideBySide" data-bottom-layout="doubleSideBySide" />
                Dup ×2 side-by-side
              </label>
            </details>
            <details class="odq-mirror-tune-details" data-single-flip-section style="margin-top:8px;">
              <summary style="cursor:pointer;font-size:13px;font-weight:600;">Flip</summary>
              <label style="display:flex;align-items:center;gap:8px;margin:8px 0;font-size:14px;">
                <input type="checkbox" data-flip="x" />
                Flip X (horizontal)
              </label>
              <label style="display:flex;align-items:center;gap:8px;margin:8px 0;font-size:14px;">
                <input type="checkbox" data-flip="y" />
                Flip Y (vertical)
              </label>
            </details>
            <details class="odq-mirror-tune-details" data-double-flip-section hidden style="margin-top:8px;">
              <summary style="cursor:pointer;font-size:13px;font-weight:600;">Tile flips</summary>
              <div style="font-size:12px;font-weight:600;margin:8px 0 4px;">Left tile</div>
              <label style="display:flex;align-items:center;gap:8px;margin:8px 0;font-size:14px;">
                <input type="checkbox" data-tile-flip="left-x" />
                Flip X
              </label>
              <label style="display:flex;align-items:center;gap:8px;margin:8px 0;font-size:14px;">
                <input type="checkbox" data-tile-flip="left-y" />
                Flip Y
              </label>
              <div style="font-size:12px;font-weight:600;margin:12px 0 4px;">Right tile</div>
              <label style="display:flex;align-items:center;gap:8px;margin:8px 0;font-size:14px;">
                <input type="checkbox" data-tile-flip="right-x" />
                Flip X
              </label>
              <label style="display:flex;align-items:center;gap:8px;margin:8px 0;font-size:14px;">
                <input type="checkbox" data-tile-flip="right-y" />
                Flip Y
              </label>
            </details>
            <details class="odq-mirror-tune-details" data-tile-position-section hidden style="margin-top:8px;">
              <summary style="cursor:pointer;font-size:13px;font-weight:600;">Tile position</summary>
              <div style="font-size:12px;font-weight:600;margin:8px 0 4px;">Left tile</div>
              <div style="font-size:11px;opacity:.72;margin:0 0 2px;" data-left-tile-x-nudge-status></div>
              <div style="font-size:11px;opacity:.72;margin:0 0 6px;" data-left-tile-nudge-status></div>
              <div class="odq-mirror-tune-nudge" aria-label="Nudge left tile left, right, up or down">
                <div class="odq-mirror-tune-nudge-row" style="display:flex;flex-wrap:wrap;align-items:center;gap:6px;margin:6px 0;">
                  ...
                  <button type="button" data-action="reset-left-tile" style="margin-left:4px;">Reset left tile</button>
                </div>
              </div>
              <div style="font-size:12px;font-weight:600;margin:12px 0 4px;">Right tile</div>
              <div style="font-size:11px;opacity:.72;margin:0 0 2px;" data-tile-seam-nudge-status></div>
              <div style="font-size:11px;opacity:.72;margin:0 0 6px;" data-right-tile-nudge-status></div>
              <div class="odq-mirror-tune-nudge" aria-label="Nudge right tile left, right, up or down">
                <div class="odq-mirror-tune-nudge-row" style="display:flex;flex-wrap:wrap;align-items:center;gap:6px;margin:6px 0;">
                  ...
                  <button type="button" data-action="reset-right-tile" style="margin-left:4px;">Reset right tile</button>
                </div>
              </div>
            </details>
            <details class="odq-mirror-tune-details" style="margin-top:8px;">
              <summary style="cursor:pointer;font-size:13px;font-weight:600;">Seam position</summary>
              <div style="font-size:11px;opacity:.72;margin:6px 0 8px;" data-seam-nudge-status></div>
              <div class="odq-mirror-tune-nudge" aria-label="Nudge seam up or down">
                <div class="odq-mirror-tune-nudge-row" style="display:flex;flex-wrap:wrap;align-items:center;gap:6px;margin:6px 0;">
                  <span style="font-size:12px;min-width:52px;">Nudge</span>
                ...
                <div class="odq-mirror-tune-nudge-row" style="display:flex;flex-wrap:wrap;align-items:center;gap:6px;margin:6px 0;">
                  <span style="font-size:12px;min-width:52px;">Big nudge</span>
                  ...
                  <button type="button" data-action="reset-seam-nudge" style="margin-left:4px;">Reset seam</button>
                </div>
              </div>
            </details>
            <details class="odq-mirror-tune-details" style="margin-top:8px;">
              <summary style="cursor:pointer;font-size:13px;font-weight:600;">Mirror field position</summary>
              <div style="font-size:11px;opacity:.72;margin:6px 0 8px;" data-mirror-nudge-status></div>
              <div class="odq-mirror-tune-nudge" aria-label="Move mirrored half up or down">
                ...
                <div class="odq-mirror-tune-nudge-row" style="display:flex;flex-wrap:wrap;align-items:center;gap:6px;margin:6px 0;">
                  ...
                  <button type="button" data-action="reset-mirror-nudge" style="margin-left:4px;">Reset mirror</button>
                </div>
              </div>
            </details>
            <div class="odq-mirror-tune-actions" style="display:flex;gap:8px;margin-top:14px;flex-wrap:wrap;">
              <button type="button" data-action="reset">Reset all</button>
              <button type="button" data-action="save">Save</button>
              <button type="button" data-action="close">Close</button>
            </div>
            <div data-status style="margin-top:8px;font-size:11px;opacity:.7;min-height:14px;"></div>
          </div>
        `;
        modal.style.cssText =
          'position:fixed;inset:0;background:rgba(0,0,0,0.72);z-index:100060;display:flex;align-items:center;justify-content:center;padding:16px;';
        const panel = modal.querySelector('.odq-mirror-tune-panel');
        panel.style.cssText =
          'background:#fff;border-radius:10px;padding:14px 16px;max-width:420px;width:100%;max-height:96vh;overflow-y:auto;box-shadow:0 8px 32px rgba(0,0,0,.35);';
        ...
        const applyDraftToUi = () => {
          const doubleLayout = isDoubleLayout(draftTune);
          if (layoutSingleInput) layoutSingleInput.checked = !doubleLayout;
          if (layoutDoubleInput) layoutDoubleInput.checked = doubleLayout;
          if (singleFlipSection) singleFlipSection.hidden = doubleLayout;
          if (doubleFlipSection) doubleFlipSection.hidden = !doubleLayout;
          if (tilePositionSection) tilePositionSection.hidden = !doubleLayout;
          ...
          modeLabelEl.textContent = modeLabel(draftTune);
          ...
          if (tileSeamNudgeStatusEl) {
            tileSeamNudgeStatusEl.textContent = `Horizontal offset ${formatNudge(draftTune.nudgeTileSeamX)}`;
          }
          if (leftTileXNudgeStatusEl) {
            leftTileXNudgeStatusEl.textContent = `Horizontal offset ${formatNudge(draftTune.nudgeLeftTileX)}`;
          }
          if (leftTileNudgeStatusEl) {
            leftTileNudgeStatusEl.textContent = `Vertical offset ${formatNudge(draftTune.nudgeLeftTileY)}`;
          }
          if (rightTileNudgeStatusEl) {
            rightTileNudgeStatusEl.textContent = `Vertical offset ${formatNudge(draftTune.nudgeRightTileY)}`;
          }
        };
        ...
        const refreshMirrorTunePreview = () => {
          ...
          const viewportW = rect && rect.width > 0 ? rect.width : 240;
          const viewportH = rect && rect.height > 0 ? rect.height : 420;
        ...
        const applySeamNudge = (direction, big) => {
          ...
          if (direction === 'up') draftTune.nudgeSeamY = clampNudge(draftTune.nudgeSeamY - step);
          else if (direction === 'down') draftTune.nudgeSeamY = clampNudge(draftTune.nudgeSeamY + step);
          dirty = true;
          applyDraftToUi();
          applyDraftPreview();
        };
        ...
        const applyMirrorFieldNudge = (direction, big) => {
          const step = nudgeStep(big);
          if (direction === 'up') draftTune.nudgeMirrorY = clampNudge(draftTune.nudgeMirrorY + step);
          else if (direction === 'down') draftTune.nudgeMirrorY = clampNudge(draftTune.nudgeMirrorY - step);
          dirty = true;
          applyDraftToUi();
          applyDraftPreview();
        };
        ...
        const applyTileHorizontalNudge = (key, direction, big) => {
          const step = nudgeStep(big);
          if (direction === 'left') draftTune[key] = clampNudge(draftTune[key] - step);
          else if (direction === 'right') draftTune[key] = clampNudge(draftTune[key] + step);
          dirty = true;
          applyDraftToUi();
          applyDraftPreview();
        };
        ...
        const applyTileVerticalNudge = (key, direction, big) => {
          const step = nudgeStep(big);
          if (direction === 'up') draftTune[key] = clampNudge(draftTune[key] - step);
          else if (direction === 'down') draftTune[key] = clampNudge(draftTune[key] + step);
          dirty = true;
          applyDraftToUi();
          applyDraftPreview();
        };
      ...
      _reparentToSpotlightOverlay(group) {
        ...
        const worldMatrix = typeof group.getCTM === 'function' ? group.getCTM() : null;
        const state = {
        ...
        if (worldMatrix && typeof overlay.getCTM === 'function') {
      ...
      _cloneGroupToSpotlightOverlay(sourceGroup) {
        const svg = sourceGroup?.ownerSVGElement || document.getElementById('quilt');
        const overlay = this._ensureQuiltSpotlightOverlay(svg);
        if (!svg || !overlay || !sourceGroup) return null;

        const worldMatrix = typeof sourceGroup.getCTM === 'function' ? sourceGroup.getCTM() : null;
        const clone = sourceGroup.cloneNode(true);
        clone.setAttribute('data-spotlight-clone', '1');
      ...
      _createSyntheticSpotlightTarget(block, submissionColor = '') {
        ...
        marker.setAttribute('stroke', 'rgba(255, 220, 140, 0.95)');
      ...
      async handleShowMyBlock() {
        ...
        const target = await this._resolveMySpotlightTarget({ rerender: true });
        ...
        const sourceMarker = target.marker;
        const synthetic = target.synthetic === true;

        sourceGroup.querySelectorAll('.quilt-my-block-sparkle-burst').forEach((el) => el.remove());
        document.getElementById('quilt')?.querySelectorAll('[data-spotlight-clone="1"]').forEach((el) => el.remove());
        ...
        if (reduceMotion) {
          sourceMarker.classList.add('user-piece-highlight');
          sourceMarker.classList.add('wiggle');
          setTimeout(() => {
            sourceMarker.classList.remove('wiggle');
        ...
        let animGroup = sourceGroup;
        let animMarker = sourceMarker;
        let restoreOrder = () => {};

        if (synthetic) {
          const layer = sourceGroup.parentNode;
          if (!layer) {
            sourceGroup.remove();
            return;
          }
          restoreOrder = () => {
            animGroup.style.transform = '';
            animGroup.style.transformBox = '';
            animGroup.style.transformOrigin = '';
            sourceGroup.remove();
          };
          this._reparentToSpotlightOverlay(sourceGroup);
          ...
          const clone = this._cloneGroupToSpotlightOverlay(sourceGroup);
          if (!clone) return;
          animGroup = clone;
          animMarker = clone.querySelector('[data-block-id]') || clone.querySelector('polygon, rect, path') || sourceMarker;
          restoreOrder = () => {
            animGroup.style.transform = '';
            animGroup.style.transformBox = '';
            animGroup.style.transformOrigin = '';
        ...
        animGroup.classList.add('quilt-my-block-spotlight');

        let bbox;
        try {
          bbox = animGroup.getBBox();
        } catch (e) {
        ...
        const compose = (scale, rotDeg) => {
          const r = rotDeg || 0;
          return `scale(${scale}) rotate(${r}deg)`;
        };
        animGroup.style.transformBox = 'fill-box';
        animGroup.style.transformOrigin = 'center';

        const pad = Math.max(10, Math.min(bbox.width, bbox.height) * 0.12);
        const sparkleBurst = this._createMyBlockSparkleBurst(animGroup, bbox, cx, cy, pad);

        const keyframes = [
          { transform: compose(1, 0), easing: 'cubic-bezier(0.18, 0.9, 0.32, 1.15)' },
          { transform: compose(1.14, 0), offset: 0.12 },
          { transform: compose(1.14, -2.8), offset: 0.18, easing: 'ease-in-out' },
          { transform: compose(1.14, 2.8), offset: 0.24, easing: 'ease-in-out' },
          { transform: compose(1.14, -2.2), offset: 0.3, easing: 'ease-in-out' },
          { transform: compose(1.14, 2.2), offset: 0.36, easing: 'ease-in-out' },
          { transform: compose(1.14, -1.4), offset: 0.42, easing: 'ease-in-out' },
          { transform: compose(1.14, 1.4), offset: 0.48, easing: 'ease-in-out' },
          { transform: compose(1.14, 0), offset: 0.56 },
          { transform: compose(1.05, 0), offset: 0.82, easing: 'cubic-bezier(0.33, 1, 0.68, 1)' },
          { transform: compose(1, 0), offset: 1 }
        ];

        const anim = animGroup.animate(keyframes, { duration: 5600, fill: 'none' });
        this._myBlockSpotlightAnim = anim;

        const done = () => {
          animGroup.classList.remove('quilt-my-block-spotlight');
      ...
      playQuiltUserColorCardBounce() {
        ...
        this._quiltUserColorCardBounceTimer = window.setTimeout(done, 2200);
      }
