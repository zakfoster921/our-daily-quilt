      ...
      async handleAdminTuneSpeakerCutout() {
        ...
        try {
          ...
          const syncKeywordEmphasisFromUi = () => {
            if (isSlide0Preview(previewAspect)) return;
          ...
          const syncSlide0TuneControls = () => {
            const slide0 = isSlide0Preview(previewAspect);
            if (keywordsDetailsEl) keywordsDetailsEl.hidden = slide0;
            if (stripDetailsEl) stripDetailsEl.hidden = slide0;
            if (speakerDetailsEl) speakerDetailsEl.hidden = slide0;
            if (dragModeWrap) dragModeWrap.hidden = slide0;
            ...
            if (slide0 && previewDragMode !== 'off') previewDragMode = 'off';
            syncPreviewDragUi();
          };
          updateStripLayoutLabel();
          updateQuiltBgZoomLabel();
          ...
          const renderPreview = async (presetName, opts = {}) => {
            ...
            try {
              ...
              setTuneDebug(
                `ok ${layoutW}×${layoutH} ${aspect}${aspect === 'slide0' ? ' quote-fit' : aspect === 'post' ? ' carousel-story' : ''}${isPrefetch ? ' prefetch' : ''} seed story:${tuneDraftByAspect.story.stripLayoutSeed} post:${tuneDraftByAspect.post.stripLayoutSeed} ${isPostLikePreview(aspect) ? postTuneQuiltSource || tuneQuiltSource : tuneQuiltSource} tok ${myToken}/${renderToken}`
              );
            } catch (renderErr) {
              if (updatesVisibleTab) {
          ...
          syncPreviewDragUi = () => {
            const slide0 = isSlide0Preview(previewAspect);
            const enabled = !slide0 && previewDragMode !== 'off' && previewDragMode !== 'strip';
            const postTab = normalizePreviewAspect(previewAspect) === 'post';
            ...
            if ((!postTab || slide0) && previewDragMode === 'strip') previewDragMode = 'speaker';
            if (dragHintEl) {
              if (slide0) dragHintEl.textContent = '';
            ...
            if (action === 'close') {
              ...
              void (async () => {
                ...
                try {
                  ...
                  if (!isSlide0Preview(previewAspect)) {
                    syncKeywordEmphasisFromUi();
                  }
                  if (!postQuoteStyleIndependent) {
                    copyStoryQuoteStyleToPost();
