  ...
  class SimplifiedQuiltAppV2Share {
      ...
      async handleAdminTuneSpeakerCutout() {
        ...
        try {
          ...
          if (
            !/^data:/i.test(speakerImageForCanvas) &&
            this.archiveService?._prepareSpeakerImageUrlForCanvas
          ) {
            try {
              const prepared = await odqPromiseWithTimeout(
                this.archiveService._prepareSpeakerImageUrlForCanvas(speakerImageForCanvas, { quote }),
                15000,
                'Speaker canvas prep'
              );
              if (prepared) speakerImageForCanvas = prepared;
            } catch (prepErr) {
          ...
          previewWrap.style.cssText =
            'position:relative;background:#222;border-radius:6px;overflow:hidden;width:100%;aspect-ratio:9/16;flex:0 0 auto;min-height:42vh;max-height:62vh;';
          ...
          /** Speaker preset/nudge/rotate always live on story (carousel post uses story placement too). */
          const loadSpeakerDraftIntoUi = () => {
            const s = tuneDraftByAspect.story;
          ...
          const aspectPreviewCacheKey = (aspect) => {
            const a = odqNormalizeTuneAspect(aspect);
            const speakerDraft = tuneDraftByAspect.story;
            const layoutDraft =
          ...
          const showCachedPreview = (aspect) => {
            ...
            if (a === 'post' && previewSlide2PeekUrlByAspect.post && slide2PeekImg && slide2PeekWrap) {
          ...
          const syncPreviewWrapAspect = () => {
            ...
            previewWrap.style.minHeight = isPost ? '32vh' : '42vh';
          ...
          const renderPreview = async (presetName) => {
            ...
            try {
              ...
              if (aspect === 'post' && previewSlide2PeekUrlByAspect.post && slide2PeekImg && slide2PeekWrap) {
                slide2PeekImg.src = previewSlide2PeekUrlByAspect.post;
                slide2PeekWrap.hidden = false;
                slide2PeekWrap.style.display = 'block';
              } else if (slide2PeekWrap) {
              ...
              syncPreviewWrapAspect();
              setTuneDebug(
                `ok ${layoutW}×${layoutH} ${aspect}${aspect === 'post' ? ' carousel-story' : ''} seed story:${tuneDraftByAspect.story.stripLayoutSeed} post:${tuneDraftByAspect.post.stripLayoutSeed}${postQuoteStyleIndependent ? ' post-indep' : ''} ${tuneQuiltSource} tok ${myToken}/${renderToken}`
              );
            } catch (renderErr) {
            ...
            if (action === 'close') {
              ...
              void (async () => {
                ...
                try {
                  ...
                  if (!serverVerify && firebaseReady && window.db && window.firestore) {
                    try {
