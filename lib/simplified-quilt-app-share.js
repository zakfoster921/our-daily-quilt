  ...
  class SimplifiedQuiltAppV2Share {
      ...
      async handleAdminTuneSpeakerCutout() {
        ...
        try {
          ...
          let renderToken = 0;
          let kwRenderTimer = 0;
          /** Last story strip plan (9:16 coords) so post preview reuses the same text collage. */
          ...
          const stripPlanCacheKey = () =>
            [
              ...
              nudgeCy,
              nudgeRotateDeg,
              JSON.stringify(getKeywordEmphasisFromForm())
            ].join('\0');
          ...
          const loadTuneAssets = async () => {
            try {
              ...
              const [quiltBlob, kwStory, stripStory] = await Promise.all([
