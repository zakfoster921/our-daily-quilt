...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Share {
      ...
      getMyBlockGroupElement() {
        ...
        return this.findQuiltBlockGroup(blockId)?.group || null;
      }
      ...
      getMySpotlightBlockId() {
        ...
        const isInEngine = (id) =>
          this.quiltEngine.blocks.some((b) => String(b?.id || '').trim() === String(id || '').trim());
        const isRendered = (id) => !!this.findQuiltBlockMarker(id);

        const pushUnique = (list, seen, id) => {
        ...
        const todayPiece = this._todayPieceFromDedicatedBlock?.();
        pushUnique(candidateIds, seen, todayPiece?.block?.id);

        const todaySubmission = this.getDevicePersonalColorSubmissionForDate?.();
        pushUnique(candidateIds, seen, todaySubmission?.blockId);
        ...
        for (const c of subs) {
          if (c && ids.has(String(c.userId || '').trim())) {
            pushUnique(candidateIds, seen, c.blockId);
          }
        }

        for (const block of this.quiltEngine.blocks.filter(belongs)) {
          pushUnique(candidateIds, seen, block.id);
        }

        for (const id of candidateIds) {
          if (isInEngine(id) && isRendered(id)) return id;
        }
        for (const id of candidateIds) {
          if (isInEngine(id)) return id;
        }
        return null;
      }
      ...
      async handleShowMyBlock() {
        let blockId = this.getMySpotlightBlockId();
        if (!blockId) {
          this.uiService.showToast('Add your color first — your square will show up here once you join the quilt.');
          return;
        }

        let target = this.findQuiltBlockGroup(blockId);
        if (!target && this.quiltEngine.blocks.some((b) => String(b?.id || '').trim() === String(blockId).trim())) {
          await this.renderQuilt?.();
          target = this.findQuiltBlockGroup(blockId);
        }
        if (!target) {
          const ids = this._devicePersonalColorUserIds();
          const belongs = (block) => this._todayPieceBelongsToDevice(block, ids);
          for (const block of this.quiltEngine.blocks.filter(belongs)) {
            const fallback = this.findQuiltBlockGroup(block.id);
            if (fallback) {
              blockId = block.id;
              target = fallback;
              break;
