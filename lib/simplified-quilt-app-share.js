        ...
        try {
          ...
          const manualStripPlanFingerprint = () => {
            ...
            return manualPostStripPlan
              .map((s) => `${s.role || 'quote'}:${Math.round(Number(s.x) || 0)}:${Math.round(Number(s.y) || 0)}`)
              .join('|');
          };
          const stripPlanAxisRect = (spec, pad = 14) => {
          ...
          const stripDisplayLabel = (spec, index) => {
            if (spec?.role === 'author') return 'Author';
          ...
          syncStripHitOverlay = () => {
            ...
            const layoutW = previewImg.naturalWidth || 1080;
            const layoutH = previewImg.naturalHeight || 1350;
            ...
            plan.forEach((spec, index) => {
              if (spec.role === 'author' && spec.authorCutoutLabel) return;
              const axis = stripPlanAxisRect(spec);
            ...
            if (dragHintEl) {
              if (previewDragMode === 'strip' && postTab) {
                dragHintEl.textContent = manualPostStripPlanActive
                  ? 'Tap a strip outline, then drag — quilt and speaker stay put until release'
          ...
          if (stripLayoutSection) {
            stripLayoutSection.addEventListener('click', (e) => {
              ...
              if (feed || btn.dataset.action !== 'shuffle-strips') {
