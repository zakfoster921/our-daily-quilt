      ...
      async handleAdminTuneSpeakerCutout() {
        ...
        try {
            ...
            if (a === 'post') {
              ...
              composeOpts.onPostStripPlan = (plan) => {
                ...
                if (manualPostStripPlanActive && manualPostStripPlan?.length) {
                  manualPostStripPlan = plan.map((fresh, i) => {
                    const manual = manualPostStripPlan[i];
                    if (!manual) return { ...fresh };
                    return {
                      ...fresh,
                      x: manual.x,
                      y: manual.y,
          ...
          syncStripHitOverlay = () => {
            ...
            const show =
              postTab && previewDragMode === 'strip' && !!previewImg.src && spinner.hidden;
            if (!show || !plan.length) {
              stripHitLayer.hidden = true;
          ...
          syncPreviewDragUi = () => {
            ...
            if (resetStripPlanBtn) {
              resetStripPlanBtn.hidden = !postTab || !manualPostStripPlanActive;
            }
