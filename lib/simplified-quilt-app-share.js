...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Share {
      ...
      _domMatrixFromSvgMatrix(matrix) {
        if (!matrix) return new DOMMatrix();
        if (typeof DOMMatrix === 'undefined') return null;
        return new DOMMatrix([matrix.a, matrix.b, matrix.c, matrix.d, matrix.e, matrix.f]);
      }

      _spotlightMotionMatrix(baseMatrix, cx, cy, scale, rotDeg = 0) {
        const base = this._domMatrixFromSvgMatrix(baseMatrix);
        if (!base) return baseMatrix;
        const motion = new DOMMatrix()
          .translateSelf(cx, cy)
          .rotateSelf(rotDeg)
          .scaleSelf(scale)
          .translateSelf(-cx, -cy);
        return base.multiply(motion);
      }

      _matrixToTransformAttr(matrix) {
        if (!matrix) return '';
        return `matrix(${matrix.a} ${matrix.b} ${matrix.c} ${matrix.d} ${matrix.e} ${matrix.f})`;
      }

      _buildSpotlightMotionKeyframes(baseMatrix, cx, cy) {
        const steps = [
          { scale: 1, rot: 0, easing: 'cubic-bezier(0.18, 0.9, 0.32, 1.15)' },
          { scale: 1.14, rot: 0, offset: 0.12 },
          { scale: 1.14, rot: -2.8, offset: 0.18, easing: 'ease-in-out' },
          { scale: 1.14, rot: 2.8, offset: 0.24, easing: 'ease-in-out' },
          { scale: 1.14, rot: -2.2, offset: 0.3, easing: 'ease-in-out' },
          { scale: 1.14, rot: 2.2, offset: 0.36, easing: 'ease-in-out' },
          { scale: 1.14, rot: -1.4, offset: 0.42, easing: 'ease-in-out' },
          { scale: 1.14, rot: 1.4, offset: 0.48, easing: 'ease-in-out' },
          { scale: 1.14, rot: 0, offset: 0.56 },
          { scale: 1.05, rot: 0, offset: 0.82, easing: 'cubic-bezier(0.33, 1, 0.68, 1)' },
          { scale: 1, rot: 0, offset: 1 }
        ];
        return steps.map((step) => {
          const frame = {
            transform: this._matrixToTransformAttr(
              this._spotlightMotionMatrix(baseMatrix, cx, cy, step.scale, step.rot)
            )
          };
          if (step.offset != null) frame.offset = step.offset;
          if (step.easing) frame.easing = step.easing;
          return frame;
        });
      }
      ...
      async handleShowMyBlock() {
        ...
        const restoreState = this._reparentToSpotlightOverlay(sourceGroup);
        if (!restoreState?.localMatrix) {
          if (synthetic) sourceGroup.remove();
          else this._restoreFromSpotlightOverlay(sourceGroup, restoreState);
          return;
        }
        ...
        const keyframes = this._buildSpotlightMotionKeyframes(restoreState.localMatrix, cx, cy);
