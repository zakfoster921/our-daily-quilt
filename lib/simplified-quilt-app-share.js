  ...
  class SimplifiedQuiltAppV2Share {
      ...
      async handleAdminTuneSpeakerCutout() {
        ...
        try {
          ...
          const quoteStyleHasRepeatedKeyword = (kw) => {
            if (!kw || !Array.isArray(kw.keywords) || !kw.keywords.length) return false;
            const quoteLower = String(qText || '').replace(/\s+/g, ' ').trim().toLowerCase();
            if (!quoteLower) return false;
            for (const keyword of kw.keywords) {
              const needle = String(keyword || '').replace(/\s+/g, ' ').trim().toLowerCase();
              if (!needle) continue;
              let count = 0;
              let idx = 0;
              while ((idx = quoteLower.indexOf(needle, idx)) >= 0) {
                count += 1;
          ...
          const buildTuneComposeOpts = (presetName, aspect, extra = {}) => {
            ...
            const repeatedStoryKeyword = a === 'story' && quoteStyleHasRepeatedKeyword(quoteStyle.keywordEmphasis);
            const composeOpts = {
              tuneAspect: a,
            ...
            const activePostPlan =
              !repeatedStoryKeyword &&
              (capturedPostStripPlan && capturedPostStripPlan.length && capturedPostStripPlanKey === stripPlanCacheKey())
                ? capturedPostStripPlan
                : (!repeatedStoryKeyword && preloadedPostStripPlan && preloadedPostStripPlan.length ? preloadedPostStripPlan : null);
            if (activePostPlan) composeOpts.savedPostStripPlan = activePostPlan;
            if (!repeatedStoryKeyword && storyRefStripPlan && storyRefStripPlanKey === stripPlanCacheKey()) {
              composeOpts.storyRefStripPlan = storyRefStripPlan;
      ...
      playQuiltUserColorCardBounce() {
        ...
        wrap.classList.remove('is-color-card-bounce');
        void wrap.offsetWidth;
        wrap.classList.add('is-color-card-bounce');
        const done = () => wrap.classList.remove('is-color-card-bounce');
