      ...
      async handleAdminTuneQuiltMirror() {
        ...
        try {
          if (typeof globalThis.odqReadMirrorTune === 'function') {
            await globalThis.odqReadMirrorTune(todayKey);
          }
        } catch (prefetchErr) {
          this.logger?.warn?.('Mirror tune prefetch skipped', prefetchErr);
        }
        ...
        let draftTune = {
          bottomLayout: normalizeBottomLayout(savedTune.bottomLayout),
          flipX: savedTune.flipX !== false,
          flipY: savedTune.flipY !== false,
          leftFlipX: savedTune.leftFlipX === true,
          leftFlipY: savedTune.leftFlipY === true,
          rightFlipX: savedTune.rightFlipX === true,
          rightFlipY: savedTune.rightFlipY === true,
          nudgeSeamY: Number(savedTune.nudgeSeamY) || 0,
          nudgeMirrorY: Number(savedTune.nudgeMirrorY) || 0
        };
        let dirty = false;
        const nudgeStep = (big) =>
        ...
        modal.innerHTML = `
          <div class="odq-mirror-tune-panel" role="dialog" aria-modal="true" aria-label="Tune quilt mirror">
            ...
            <div class="odq-mirror-tune-preview-wrap" style="position:relative;background:#222;border-radius:8px;overflow:hidden;width:min(100%,240px);margin:0 auto 12px;aspect-ratio:9/16;max-height:46vh;">
              <svg id="odqMirrorTunePreviewSvg" xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" preserveAspectRatio="xMidYMin slice" aria-hidden="true" style="display:block;"></svg>
            </div>
