...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Share {
      ...
      /**
       * Long-press share must call navigator.share synchronously in the hold handler.
       * Any await before share() loses iOS user activation — sheet never opens.
       */
      shareLayoutBStoryPreviewFromLongPressSync() {
        const blob = this._layoutBStoryPreviewShareBlob;
        if (!blob?.size) {
          this.uiService.showToast('Still preparing — hold again in a moment');
        ...
        const imageFile = new File([blob], filename, { type: 'image/png' });
        const shareData = {
          title: 'OUR DAILY QUILT — Story collage',
          text: shareText,
          files: [imageFile]
        };
        if (
          ...
          this.errorHandler.handleError(new Error('Share not available on this device'), 'layoutBPreviewShare');
          return;
        }
        navigator
          .share(shareData)
