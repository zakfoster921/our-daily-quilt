  ...
  class SimplifiedQuiltAppV2Share {
      ...
      async handleAdminTuneSpeakerCutout() {
        ...
        try {
          ...
          const applyDraftToUi = (aspect) => {
            ...
            const d = tuneDraftByAspect[a];
            if (preview !== 'feed' && a === 'post' && !postQuoteStyleIndependent) {
              copyStoryQuoteStyleToPost();
          ...
          const manualStripPlanFingerprint = () => {
            ...
            return manualPostStripPlan
              .map(
                (s) =>
                  `${s.role || 'quote'}:${Math.round(Number(s.x) || 0)}:${Math.round(Number(s.y) || 0)}:${Math.round((Number(s.angle) || 0) * 1000)}`
              )
              .join('|');
          };
            ...
            if (a === 'post') {
              ...
              composeOpts.onPostStripPlan = (plan) => {
                ...
                if (manualPostStripPlanActive && manualPostStripPlan?.length) {
                  const hasTypography =
                    typeof globalThis.odqLayoutBStripPlanHasTypography === 'function' &&
                    globalThis.odqLayoutBStripPlanHasTypography(manualPostStripPlan);
                  if (hasTypography) {
                    previewBlobCacheKeyByAspect.story = '';
                    syncStripHitOverlay();
                  ...
                  manualPostStripPlan =
                    globalThis.odqCloneLayoutBStripPlan?.(plan) || plan.map((s) => ({ ...s }));
                  if (
                    typeof globalThis.odqApplySavedLayoutBStripPlanPositionsByReadingOrder ===
                    'function'
                  ) {
                  ...
                  if (selectedStripIndex >= manualPostStripPlan.length) {
                    selectedStripIndex = -1;
                  }
                }
                previewBlobCacheKeyByAspect.story = '';
          ...
          if (stripLayoutSection) {
            stripLayoutSection.addEventListener('click', (e) => {
              const btn = e.target.closest(
                'button[data-action], button[data-strip-offset], button[data-strip-big-offset], button[data-strip-rotate]'
              );
              if (!btn) return;
              const feed = isFeedPreview(previewAspect);
              ...
              if (feed || (btn.dataset.action !== 'shuffle-strips' && !btn.dataset.stripRotate)) {
              ...
              void (async () => {
                ...
                try {
                  ...
                  if (!postQuoteStyleIndependent) {
                    syncLinkedSpeakerDrafts(previewAspect);
                    copyStoryQuoteStyleToPost();
                  } else if (normalizePreviewAspect(previewAspect) === 'story') {
                    copyStoryQuoteStyleToPost();
                    postQuoteStyleIndependent = false;
                  ...
                  const activeKw =
                    tuneDraftByAspect[draftAspectFor(previewAspect)].keywordEmphasis;
                  if (
                    !isFeedPreview(previewAspect) &&
                    rawKw &&
                    !activeKw?.keywords?.length
                  ) {
                    const hint = keywordValidationHint();
                  ...
                  const writeTuneToClientFirestore = async () => {
                    ...
                    const stripPlanToSave =
                      manualPostStripPlanActive && manualPostStripPlan?.length
                        ? manualPostStripPlan
          ...
          const loadTuneAssets = async () => {
            try {
              ...
              const [
                ...
                odqPromiseWithTimeout(odqReadSpeakerCutoutTweak(todayKey, 'story'), 5000, 'Story speaker').catch(
                  () => odqReadSpeakerCutoutTweakFromLocal(todayKey, 'story')
                ),
                odqPromiseWithTimeout(odqReadSpeakerCutoutTweak(todayKey, 'post'), 5000, 'Post speaker').catch(
                  () => odqReadSpeakerCutoutTweakFromLocal(todayKey, 'post')
                ),
              ...
              preloadedPostStripPlan = savedPostStripPlan;
              ...
              if (!tuneUiDirty) {
                ...
                postQuoteStyleIndependent = resolvePostQuoteStyleIndependent(
                  igDoc,
                  stripPost,
                  stripStory,
                  kwPost,
                ...
                if (restoreSavedPostStripPlan(savedPostStripPlan)) {
                  postQuoteStyleIndependent = true;
                }
                if (!postQuoteStyleIndependent) {
                  copyStoryQuoteStyleToPost();
      ...
      async handleAdminTuneQuiltMirror() {
        ...
        modal.innerHTML = `
          <div class="odq-mirror-tune-panel" role="dialog" aria-modal="true" aria-label="Tune quilt mirror">
            ...
            <div class="odq-mirror-tune-preview-wrap" style="position:relative;background:#f6f4f1;border-radius:8px;overflow:hidden;margin:0 auto 12px;">
        ...
        /** Size preview box to live quilt aspect (scaled down to fit the modal). */
        const sizeMirrorTunePreviewWrap = () => {
          const live = getLiveQuiltViewport();
          const aspect = live.aspect || live.w / live.h;
          const maxW = Math.min(
            Math.max(160, (panel?.clientWidth || 388) - 32),
            typeof window !== 'undefined' ? window.innerWidth * 0.72 : 280
          );
          const maxH =
            typeof window !== 'undefined' ? Math.min(window.innerHeight * 0.46, 520) : 420;
          let h = maxH;
          let w = h * aspect;
          if (w > maxW) {
            w = maxW;
      ...
      playQuiltUserColorCardBounce() {
        ...
        const bounceDurationMs =
          parseFloat(screenStyle?.getPropertyValue('--quilt-color-card-bounce-duration') || '') * 1000;
