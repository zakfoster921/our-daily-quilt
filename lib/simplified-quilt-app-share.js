...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Share {
      ...
      async handleAdminTuneQuiltMirror() {
        ...
        let draftTune = { flipX: savedTune.flipX !== false, flipY: savedTune.flipY !== false };
        let dirty = false;
        ...
        modal.querySelector('[data-action="reset"]')?.addEventListener('click', () => {
          draftTune = { flipX: true, flipY: true };
          dirty = true;
          applyDraftToUi();
          applyDraftPreview();
          statusEl.textContent = 'Reset to default (Mirror). Save to persist.';
        });
