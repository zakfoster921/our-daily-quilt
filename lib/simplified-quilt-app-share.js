  ...
  class SimplifiedQuiltAppV2Share {
      ...
      async shareLayoutBStoryPreviewFromLongPress() {
        ...
        if (!shareBlob?.size) {
          ...
          const quote =
            ...
            { text: '', author: '' };
          /** Prefer vinyl quote-fit story (matches quilt-screen preview). */
          if (arch?.generateInstagramStoryQuoteFitImageData && typeof Utils?.dataUrlToBlob === 'function') {
            try {
              const result = await arch.generateInstagramStoryQuoteFitImageData(blocks, quote, dk);
              if (result?.dataUrl) {
                shareBlob = Utils.dataUrlToBlob(result.dataUrl);
              }
            } catch (_) {
              shareBlob = null;
            }
          }
          if (!shareBlob?.size && arch?.generateInstagramStoryLayoutBBlob) {
      ...
      async handleAdminTuneSpeakerCutout() {
        ...
        try {
          ...
          modal.innerHTML = `
            <div class="odq-speaker-tune-panel" role="dialog" aria-modal="true" aria-label="Tune today's speaker cutout">
              <div class="odq-speaker-tune-preview-wrap">
                <div class="odq-speaker-tune-preview-composite">
                  <div class="odq-speaker-tune-preview-slide1-wrap">
                    <img class="odq-speaker-tune-preview" alt="Story / Layout B preview" />
                  </div>
                </div>
              ...
              <div class="odq-speaker-tune-aspect">
                ...
                <button type="button" data-aspect="slide0">Slide 0</button>
                <button type="button" data-aspect="post">Post 4:5</button>
              </div>
              <div class="odq-speaker-tune-style-amount" data-speaker-style-amount>
                <span>Cutout style:</span>
                <strong data-speaker-style-amount-label>Full</strong>
                <button type="button" data-action="style-soft" aria-label="Softest cutout style">Soft</button>
                <button type="button" data-action="style-less" aria-label="Less cutout style">−</button>
                <button type="button" data-action="style-more" aria-label="More cutout style">+</button>
                <button type="button" data-action="style-full" aria-label="Full cutout style">Full</button>
              </div>
          ...
          const styleAmountWrap = modal.querySelector('[data-speaker-style-amount]');
          if (styleAmountWrap) {
            styleAmountWrap.style.cssText =
              'display:flex;align-items:center;gap:4px;flex-wrap:wrap;font-size:11px;';
            for (const btn of styleAmountWrap.querySelectorAll('button')) {
              btn.style.cssText =
                'padding:4px 8px;border:1px solid #aaa;background:#fff;cursor:pointer;border-radius:3px;font-size:11px;touch-action:manipulation;';
            }
          }
          ...
          /** Modal tabs: story | slide0 | post. Slide 0 is UI-only; quilt draft shares Post. */
          const normalizePreviewAspect = (aspect) => {
            const a = String(aspect || '').trim().toLowerCase();
            if (a === 'post' || a === 'slide0') return a;
            return 'story';
          };
          const draftAspectFor = (aspect) => {
            const a = normalizePreviewAspect(aspect);
            return a === 'slide0' ? 'post' : a;
          };
          const isSlide0Preview = (aspect) => normalizePreviewAspect(aspect) === 'slide0';
          /** Story + Slide 0 use vinyl quote-fit (no Layout B strips / speaker drag). */
          const isQuoteFitPreview = (aspect) => {
            const a = normalizePreviewAspect(aspect);
            return a === 'story' || a === 'slide0';
          };
          const isPostLikePreview = (aspect) => {
            const a = normalizePreviewAspect(aspect);
            return a === 'post' || a === 'slide0';
          };
          const odqNormalizeSpeakerStyleAmount =
          ...
          const odqFormatSpeakerStyleAmountLabel =
            typeof globalThis.odqFormatSpeakerStyleAmountLabel === 'function'
              ? globalThis.odqFormatSpeakerStyleAmountLabel
              : (amount) => {
          ...
          const SPEAKER_STYLE_AMOUNT_STEP = 0.1;
          const speakerStyleAmountInit = odqNormalizeSpeakerStyleAmount(
            (typeof globalThis.odqGetCachedLayoutBSpeakerStyleAmount === 'function'
              ? globalThis.odqGetCachedLayoutBSpeakerStyleAmount(todayKey)
              : undefined) ?? 1
          );
          const tuneDraftByAspect = {
            story: {
              ...
              speakerStyleAmount: speakerStyleAmountInit,
          ...
          const syncKeywordEmphasisFromUi = () => {
            if (isQuoteFitPreview(previewAspect)) return;
          ...
          const lockPostTuneIndependent = () => {
            if (normalizePreviewAspect(previewAspect) === 'post') {
          ...
          const maybeLockPostTuneIndependent = () => {
            ...
            if (preview === 'post') lockPostTuneIndependent();
          };

          /** Speaker preset/nudge/rotate are aspect-specific; Slide 0 reuses the Post quilt draft. */
          ...
          const captureLayoutDraftFromUi = (aspect) => {
            const preview = normalizePreviewAspect(aspect);
            const a = draftAspectFor(preview);
            if (a === 'post' && preview === 'post') lockPostTuneIndependent();
          ...
          let speakerStyleAmount = odqNormalizeSpeakerStyleAmount(
            tuneDraftByAspect.story.speakerStyleAmount ?? 1
          );
          let quoteStripOffset =
            odqNormalizeQuoteStripOffset?.(tuneDraftByAspect.story.quoteStripOffset) || { x: 0, y: 0 };
          let previewAspect = 'story';
          /** Declared early: Story is quote-fit and syncSlide0TuneControls runs before drag wiring. */
          let previewDragMode = 'speaker';
          let previewDragModeAtStart = 'speaker';
          /** Cached preview blobs per aspect — Post/Slide 0 built lazily on first tab switch. */
          const previewBlobUrlByAspect = { story: null, post: null, slide0: null };
          const previewBlobCacheKeyByAspect = { story: '', post: '', slide0: '' };
          const previewStaleByAspect = { story: true, post: true, slide0: true };
          ...
          const revokeAllPreviewBlobs = () => {
            ...
            revokeAspectPreviewBlob('slide0');
          };
          ...
          const aspectPreviewCacheKey = (aspect) => {
            ...
            if (preview === 'slide0') {
              return [
                'slide0',
                odqNormalizeQuiltBgZoom(tuneDraftByAspect.post.quiltBgZoom),
                odqNormalizeQuiltBgOffsetY?.(tuneDraftByAspect.post.quiltBgOffsetY) ?? 0,
                odqNormalizeSpeakerStyleAmount(tuneDraftByAspect.story.speakerStyleAmount ?? 1),
                String(qText || '').slice(0, 120)
              ].join('\0');
            }
            if (preview === 'story') {
              return [
                'story-quote-fit',
                odqNormalizeQuiltBgZoom(tuneDraftByAspect.story.quiltBgZoom),
                odqNormalizeQuiltBgOffsetY?.(tuneDraftByAspect.story.quiltBgOffsetY) ?? 0,
                odqNormalizeSpeakerStyleAmount(tuneDraftByAspect.story.speakerStyleAmount ?? 1),
                String(speakerName || '').slice(0, 80),
                String(qText || '').slice(0, 120)
              ].join('\0');
            }
          ...
          /** Story speaker/quote edits invalidate inherited Post + Slide 0 previews. */
          const markTunePreviewStaleForEdit = (opts = {}) => {
            ...
            if (!postQuoteStyleIndependent) {
              ...
              previewStaleByAspect.slide0 = true;
              previewBlobCacheKeyByAspect.slide0 = '';
              if (opts.invalidateStripPlan !== false) {
                invalidateStoryStripPlanCache();
              }
            } else if (previewAspect === 'post' || previewAspect === 'slide0') {
              previewStaleByAspect.post = true;
              previewBlobCacheKeyByAspect.post = '';
              previewStaleByAspect.slide0 = true;
              previewBlobCacheKeyByAspect.slide0 = '';
          ...
          const syncSpeakerStyleAmountDrafts = (amount) => {
            const a = odqNormalizeSpeakerStyleAmount(amount);
            speakerStyleAmount = a;
            tuneDraftByAspect.story.speakerStyleAmount = a;
            tuneDraftByAspect.post.speakerStyleAmount = a;
            if (typeof globalThis.odqSetCachedLayoutBSpeakerStyleAmount === 'function') {
              globalThis.odqSetCachedLayoutBSpeakerStyleAmount(todayKey, a);
            }
            return a;
          };
          const updateSpeakerStyleAmountLabel = () => {
            const labelEl = modal.querySelector('[data-speaker-style-amount-label]');
            if (labelEl) {
              labelEl.textContent = odqFormatSpeakerStyleAmountLabel(speakerStyleAmount);
            }
            const softBtn = modal.querySelector('button[data-action="style-soft"]');
            const lessBtn = modal.querySelector('button[data-action="style-less"]');
            const moreBtn = modal.querySelector('button[data-action="style-more"]');
            const fullBtn = modal.querySelector('button[data-action="style-full"]');
            if (softBtn) {
              softBtn.disabled = speakerStyleAmount <= 0.001;
              softBtn.style.opacity = softBtn.disabled ? '0.45' : '1';
            }
            if (lessBtn) {
              lessBtn.disabled = speakerStyleAmount <= 0.001;
              lessBtn.style.opacity = lessBtn.disabled ? '0.45' : '1';
            }
            if (moreBtn) {
              moreBtn.disabled = speakerStyleAmount >= 0.999;
              moreBtn.style.opacity = moreBtn.disabled ? '0.45' : '1';
            }
            if (fullBtn) {
              fullBtn.disabled = speakerStyleAmount >= 0.999;
              fullBtn.style.opacity = fullBtn.disabled ? '0.45' : '1';
            }
          };
          ...
          const speakerDetailsEl = [...modal.querySelectorAll('details.odq-speaker-tune-details')].find(
            (el) => /speaker placement/i.test(el.querySelector('summary')?.textContent || '')
          );
          ...
          const syncSlide0TuneControls = () => {
            const quoteFit = isQuoteFitPreview(previewAspect);
            if (keywordsDetailsEl) keywordsDetailsEl.hidden = quoteFit;
            if (stripDetailsEl) stripDetailsEl.hidden = quoteFit;
            if (speakerDetailsEl) speakerDetailsEl.hidden = quoteFit;
            if (dragModeWrap) dragModeWrap.hidden = quoteFit;
            if (stripLayoutStatusEl) stripLayoutStatusEl.hidden = false;
            if (shuffleStripsBtn) shuffleStripsBtn.hidden = false;
            if (stripSectionSummaryEl) stripSectionSummaryEl.textContent = 'Text strip layout';
            if (stripOffsetMoveLabelEl) stripOffsetMoveLabelEl.textContent = 'Move strips';
            if (stripOffsetWrapEl) {
              stripOffsetWrapEl.setAttribute('aria-label', 'Move all quote strips');
            }
            if (quoteFit && previewDragMode !== 'off') previewDragMode = 'off';
            syncPreviewDragUi();
          };
          updateStripLayoutLabel();
          updateQuiltBgZoomLabel();
          ...
          const buildTuneComposeOpts = (presetName, aspect, extra = {}) => {
            ...
            const composeOpts = {
              ...
              styleAmount: odqNormalizeSpeakerStyleAmount(
                tuneDraftByAspect.story.speakerStyleAmount ?? speakerStyleAmount ?? 1
              ),
              styleAmountExplicit: true,
              ...extra
            };
            if (a === 'post') {
          ...
          const composeSlide0QuoteFitPreview = async (previewQuiltBlob) => {
            const compose =
            ...
            if (typeof compose !== 'function') {
              throw new Error('Slide 0 quote-fit compose unavailable');
            }
            const d = tuneDraftByAspect.post;
            const result = await compose(previewQuiltBlob, {
              dateKey: todayKey,
              quoteText: qText,
              quiltBgZoom: d.quiltBgZoom,
              quiltBgOffsetY: d.quiltBgOffsetY,
              quiltFit: 'cover',
              tunePreviewFast: true
            });
          ...
          const composeStoryQuoteFitPreview = async (previewQuiltBlob) => {
            const compose =
            ...
            const d = tuneDraftByAspect.story;
            const result = await compose(previewQuiltBlob, {
              dateKey: todayKey,
              quoteText: qText,
              speakerName,
              ...
              washColor,
              styleAmount: odqNormalizeSpeakerStyleAmount(
                d.speakerStyleAmount ?? speakerStyleAmount ?? 1
              ),
              quiltBgZoom: d.quiltBgZoom,
              quiltBgOffsetY: d.quiltBgOffsetY,
              quiltFit: 'cover',
            ...
            try {
              ...
              if (aspect === 'slide0') {
                outBlob = await odqPromiseWithTimeout(
                  composeSlide0QuoteFitPreview(previewQuiltBlob),
                  45000,
                  'Slide 0 quote-fit preview'
                );
              } else if (aspect === 'story') {
                outBlob = await odqPromiseWithTimeout(
                  composeStoryQuoteFitPreview(previewQuiltBlob),
                  45000,
          ...
          syncPreviewDragUi = () => {
            const quoteFit = isQuoteFitPreview(previewAspect);
            const enabled = !quoteFit && previewDragMode !== 'off' && previewDragMode !== 'strip';
            const postTab = normalizePreviewAspect(previewAspect) === 'post';
            ...
            if ((!postTab || quoteFit) && previewDragMode === 'strip') previewDragMode = 'speaker';
            if (dragHintEl) {
              if (quoteFit) dragHintEl.textContent = '';
              else if (previewDragMode === 'strip' && postTab) {
                ...
                dragHintEl.textContent = 'Drag to move text — quilt and speaker stay put until release';
              }
            }
            const textBtn = dragModeWrap?.querySelector('button[data-drag-mode="text"]');
            if (textBtn) textBtn.textContent = 'Text';
            if (previewSlide1Wrap) {
              ...
              const previewAspectNow = normalizePreviewAspect(previewAspect);
              previewSlide1Wrap.setAttribute(
                ...
                previewDragMode === 'strip'
                  ...
                  : enabled
                    ...
                    : previewAspectNow === 'slide0'
                      ? 'Slide 0 quote-fit preview'
                      : previewAspectNow === 'story'
                        ? 'Story quote-fit preview'
                        : 'Layout B preview'
              );
            }
          ...
          if (quiltZoomSection) {
            quiltZoomSection.addEventListener('click', (e) => {
              ...
              if (quiltOffsetBtn) {
                ...
                markPreviewStale(
                  isPostLikePreview(previewAspect) ? ['post', 'slide0'] : [previewAspect]
                );
                captureDraftFromUi(previewAspect);
                updateQuiltBgOffsetLabel();
                scheduleNudgePreview();
          ...
          if (styleAmountWrap) {
            styleAmountWrap.addEventListener('click', (e) => {
              const btn = e.target.closest('button[data-action]');
              if (!btn) return;
              const action = btn.dataset.action;
              let next = speakerStyleAmount;
              if (action === 'style-soft') next = 0;
              else if (action === 'style-full') next = 1;
              else if (action === 'style-less') next = speakerStyleAmount - SPEAKER_STYLE_AMOUNT_STEP;
              else if (action === 'style-more') next = speakerStyleAmount + SPEAKER_STYLE_AMOUNT_STEP;
              else return;
              markTuneUiDirty();
              syncSpeakerStyleAmountDrafts(next);
              updateSpeakerStyleAmountLabel();
              markPreviewStale(['story', 'post', 'slide0']);
              scheduleNudgePreview();
            });
          }
          ...
          actionsWrap.addEventListener('click', (e) => {
            ...
            if (action === 'close') {
              ...
              quiltBgOffsetY = 0;
              quoteStripOffset = { x: 0, y: 0 };
              syncSpeakerStyleAmountDrafts(1);
              updateActiveButton();
              updateStripLayoutLabel();
              updateQuiltBgZoomLabel();
              updateQuiltBgOffsetLabel();
              ...
              void (async () => {
                ...
                try {
                  ...
                  if (!isQuoteFitPreview(previewAspect)) {
                    syncKeywordEmphasisFromUi();
                  }
                  if (!postQuoteStyleIndependent) {
                    copyStoryQuoteStyleToPost();
                  ...
                  if (
                    !isQuoteFitPreview(previewAspect) &&
                    rawKw &&
                  ...
                  syncSpeakerStyleAmountDrafts(speakerStyleAmount);
                  ...
                  if (typeof globalThis.odqSetCachedLayoutBSpeakerStyleAmount === 'function') {
                    globalThis.odqSetCachedLayoutBSpeakerStyleAmount(
                      saveDateKey,
                      tuneDraftByAspect.story.speakerStyleAmount ?? speakerStyleAmount
                    );
                  }
                  ...
                  // Saving tune assets is not the morning IG post — leave the pill for Zapier/tap.
                  close();
                  try {
          ...
          const loadTuneAssets = async () => {
            try {
              ...
              if (!tuneUiDirty) {
                ...
                syncSpeakerStyleAmountDrafts(
                  igDoc?.layoutBSpeakerStyleAmount ??
                    (typeof globalThis.odqGetCachedLayoutBSpeakerStyleAmount === 'function'
                      ? globalThis.odqGetCachedLayoutBSpeakerStyleAmount(todayKey)
                      : undefined) ??
                    speakerStyleAmount
      ...
      async handleAdminTuneQuiltMirror() {
        ...
        const QUAD_TILE_IDS = ['tl', 'tr', 'bl', 'br'];
        const quadFieldsFromTune = (tune) => {
          if (
            typeof QuiltMirrorLayout !== 'undefined' &&
            typeof QuiltMirrorLayout.odqQuadTileFieldsPlain === 'function'
          ) {
            return QuiltMirrorLayout.odqQuadTileFieldsPlain(tune);
          }
          const out = {};
          QUAD_TILE_IDS.forEach((id) => {
            out[`${id}FlipX`] = tune?.[`${id}FlipX`] === true;
            out[`${id}FlipY`] =
              tune?.[`${id}FlipY`] === true ||
              (tune?.[`${id}FlipY`] == null && (id === 'bl' || id === 'br'));
            out[`${id}NudgeX`] = Number(tune?.[`${id}NudgeX`]) || 0;
            out[`${id}NudgeY`] = Number(tune?.[`${id}NudgeY`]) || 0;
          });
          return out;
        };
        ...
        modal.innerHTML = `
          <div class="odq-mirror-tune-panel" role="dialog" aria-modal="true" aria-label="Tune quilt mirror">
            <style>
              ...
              .odq-mt-quad-cell {
                border:1px solid #eceae6;border-radius:6px;padding:5px 6px;min-width:0;
              }
              ...
              .odq-mirror-tune-nudge { display:flex;flex-direction:column;gap:4px; }
            ...
            <div class="odq-mirror-tune-body">
              ...
              <div class="odq-mirror-tune-controls">
                ...
                <details class="odq-mirror-tune-details" data-quad-tiles-section hidden open>
                  <summary>Tiles (2×2)</summary>
                  <div class="odq-mt-quad">
                    <div class="odq-mt-quad-cell" data-quad-tile="tl">
                      <div class="odq-mt-col-title">TL</div>
                      <div class="odq-mt-row">
                        <label><input type="checkbox" data-quad-flip="tl-x" /> X</label>
                        <label><input type="checkbox" data-quad-flip="tl-y" /> Y</label>
                      </div>
                      <div class="odq-mt-status" data-quad-nudge-status="tl"></div>
                      <div class="odq-mirror-tune-nudge" aria-label="Nudge TL tile">
                        <div class="odq-mirror-tune-nudge-row">
                          <button type="button" data-quad-nudge="tl" data-axis="x" data-dir="-1" aria-label="Nudge TL left">←</button>
                          <button type="button" data-quad-nudge="tl" data-axis="x" data-dir="1" aria-label="Nudge TL right">→</button>
                          <button type="button" data-quad-nudge="tl" data-axis="y" data-dir="-1" aria-label="Nudge TL up">↑</button>
                          <button type="button" data-quad-nudge="tl" data-axis="y" data-dir="1" aria-label="Nudge TL down">↓</button>
                          <button type="button" data-action="reset-quad-tl">Reset</button>
                        </div>
                        <div class="odq-mirror-tune-nudge-row">
                          <button type="button" data-quad-nudge="tl" data-axis="x" data-dir="-1" data-big="1" aria-label="Big nudge TL left">⇐</button>
                          <button type="button" data-quad-nudge="tl" data-axis="x" data-dir="1" data-big="1" aria-label="Big nudge TL right">⇒</button>
                          <button type="button" data-quad-nudge="tl" data-axis="y" data-dir="-1" data-big="1" aria-label="Big nudge TL up">⇑</button>
                          <button type="button" data-quad-nudge="tl" data-axis="y" data-dir="1" data-big="1" aria-label="Big nudge TL down">⇓</button>
                        </div>
                      </div>
                    </div>
                    <div class="odq-mt-quad-cell" data-quad-tile="tr">
                      <div class="odq-mt-col-title">TR</div>
                      <div class="odq-mt-row">
                        <label><input type="checkbox" data-quad-flip="tr-x" /> X</label>
                        <label><input type="checkbox" data-quad-flip="tr-y" /> Y</label>
                      </div>
                      <div class="odq-mt-status" data-quad-nudge-status="tr"></div>
                      <div class="odq-mirror-tune-nudge" aria-label="Nudge TR tile">
                        <div class="odq-mirror-tune-nudge-row">
                          <button type="button" data-quad-nudge="tr" data-axis="x" data-dir="-1" aria-label="Nudge TR left">←</button>
                          <button type="button" data-quad-nudge="tr" data-axis="x" data-dir="1" aria-label="Nudge TR right">→</button>
                          <button type="button" data-quad-nudge="tr" data-axis="y" data-dir="-1" aria-label="Nudge TR up">↑</button>
                          <button type="button" data-quad-nudge="tr" data-axis="y" data-dir="1" aria-label="Nudge TR down">↓</button>
                          <button type="button" data-action="reset-quad-tr">Reset</button>
                        </div>
                        <div class="odq-mirror-tune-nudge-row">
                          <button type="button" data-quad-nudge="tr" data-axis="x" data-dir="-1" data-big="1" aria-label="Big nudge TR left">⇐</button>
                          <button type="button" data-quad-nudge="tr" data-axis="x" data-dir="1" data-big="1" aria-label="Big nudge TR right">⇒</button>
                          <button type="button" data-quad-nudge="tr" data-axis="y" data-dir="-1" data-big="1" aria-label="Big nudge TR up">⇑</button>
                          <button type="button" data-quad-nudge="tr" data-axis="y" data-dir="1" data-big="1" aria-label="Big nudge TR down">⇓</button>
                        </div>
                      </div>
                    </div>
                    <div class="odq-mt-quad-cell" data-quad-tile="bl">
                      <div class="odq-mt-col-title">BL</div>
                      <div class="odq-mt-row">
                        <label><input type="checkbox" data-quad-flip="bl-x" /> X</label>
                        <label><input type="checkbox" data-quad-flip="bl-y" /> Y</label>
                      </div>
                      <div class="odq-mt-status" data-quad-nudge-status="bl"></div>
                      <div class="odq-mirror-tune-nudge" aria-label="Nudge BL tile">
                        <div class="odq-mirror-tune-nudge-row">
                          <button type="button" data-quad-nudge="bl" data-axis="x" data-dir="-1" aria-label="Nudge BL left">←</button>
                          <button type="button" data-quad-nudge="bl" data-axis="x" data-dir="1" aria-label="Nudge BL right">→</button>
                          <button type="button" data-quad-nudge="bl" data-axis="y" data-dir="-1" aria-label="Nudge BL up">↑</button>
                          <button type="button" data-quad-nudge="bl" data-axis="y" data-dir="1" aria-label="Nudge BL down">↓</button>
                          <button type="button" data-action="reset-quad-bl">Reset</button>
                        </div>
                        <div class="odq-mirror-tune-nudge-row">
                          <button type="button" data-quad-nudge="bl" data-axis="x" data-dir="-1" data-big="1" aria-label="Big nudge BL left">⇐</button>
                          <button type="button" data-quad-nudge="bl" data-axis="x" data-dir="1" data-big="1" aria-label="Big nudge BL right">⇒</button>
                          <button type="button" data-quad-nudge="bl" data-axis="y" data-dir="-1" data-big="1" aria-label="Big nudge BL up">⇑</button>
                          <button type="button" data-quad-nudge="bl" data-axis="y" data-dir="1" data-big="1" aria-label="Big nudge BL down">⇓</button>
                        </div>
                      </div>
                    </div>
                    <div class="odq-mt-quad-cell" data-quad-tile="br">
                      <div class="odq-mt-col-title">BR</div>
                      <div class="odq-mt-row">
                        <label><input type="checkbox" data-quad-flip="br-x" /> X</label>
                        <label><input type="checkbox" data-quad-flip="br-y" /> Y</label>
                      </div>
                      <div class="odq-mt-status" data-quad-nudge-status="br"></div>
                      <div class="odq-mirror-tune-nudge" aria-label="Nudge BR tile">
                        <div class="odq-mirror-tune-nudge-row">
                          <button type="button" data-quad-nudge="br" data-axis="x" data-dir="-1" aria-label="Nudge BR left">←</button>
                          <button type="button" data-quad-nudge="br" data-axis="x" data-dir="1" aria-label="Nudge BR right">→</button>
                          <button type="button" data-quad-nudge="br" data-axis="y" data-dir="-1" aria-label="Nudge BR up">↑</button>
                          <button type="button" data-quad-nudge="br" data-axis="y" data-dir="1" aria-label="Nudge BR down">↓</button>
                          <button type="button" data-action="reset-quad-br">Reset</button>
                        </div>
                        <div class="odq-mirror-tune-nudge-row">
                          <button type="button" data-quad-nudge="br" data-axis="x" data-dir="-1" data-big="1" aria-label="Big nudge BR left">⇐</button>
                          <button type="button" data-quad-nudge="br" data-axis="x" data-dir="1" data-big="1" aria-label="Big nudge BR right">⇒</button>
                          <button type="button" data-quad-nudge="br" data-axis="y" data-dir="-1" data-big="1" aria-label="Big nudge BR up">⇑</button>
                          <button type="button" data-quad-nudge="br" data-axis="y" data-dir="1" data-big="1" aria-label="Big nudge BR down">⇓</button>
                        </div>
                      </div>
                    </div>
                  </div>
                </details>
                <div class="odq-mt-cols" data-single-band-nudge-section>
                  <details class="odq-mirror-tune-details" open>
        ...
        const quadTilesSection = modal.querySelector('[data-quad-tiles-section]');
        const singleBandNudgeSection = modal.querySelector('[data-single-band-nudge-section]');
        ...
        const applyDraftToUi = () => {
          ...
          const isDouble = layout === 'doubleSideBySide';
          const isQuad = layout === 'quadSideBySide';
          if (layoutSingleInput) layoutSingleInput.checked = layout === 'single';
          if (layoutDoubleInput) layoutDoubleInput.checked = isDouble;
          if (layoutQuadInput) layoutQuadInput.checked = isQuad;
          if (singleFlipSection) singleFlipSection.hidden = isDouble || isQuad;
          if (doubleFlipSection) doubleFlipSection.hidden = !isDouble;
          if (tilePositionSection) tilePositionSection.hidden = !isDouble;
          if (quadTilesSection) quadTilesSection.hidden = !isQuad;
          if (singleBandNudgeSection) singleBandNudgeSection.hidden = isQuad;
          ...
          QUAD_TILE_IDS.forEach((id) => {
            const fx = modal.querySelector(`[data-quad-flip="${id}-x"]`);
            const fy = modal.querySelector(`[data-quad-flip="${id}-y"]`);
            if (fx) fx.checked = !!draftTune[`${id}FlipX`];
            if (fy) fy.checked = !!draftTune[`${id}FlipY`];
            const status = modal.querySelector(`[data-quad-nudge-status="${id}"]`);
            if (status) {
              const nx = formatNudge(draftTune[`${id}NudgeX`], clampTileNudge);
              const ny = formatNudge(draftTune[`${id}NudgeY`], clampTileNudge);
              status.textContent = `x ${nx} · y ${ny}`;
            }
          });
        ...
        QUAD_TILE_IDS.forEach((id) => {
          const fx = modal.querySelector(`[data-quad-flip="${id}-x"]`);
          const fy = modal.querySelector(`[data-quad-flip="${id}-y"]`);
          bindTileFlipInput(fx, `${id}FlipX`);
          bindTileFlipInput(fy, `${id}FlipY`);
        });

        modal.querySelectorAll('[data-quad-nudge]').forEach((btn) => {
          btn.addEventListener('click', () => {
            const id = btn.getAttribute('data-quad-nudge');
            const axis = btn.getAttribute('data-axis') === 'y' ? 'y' : 'x';
            const dir = Number(btn.getAttribute('data-dir')) || 0;
            const big = btn.getAttribute('data-big') === '1';
            const key = axis === 'y' ? `${id}NudgeY` : `${id}NudgeX`;
            const step = nudgeStep(big);
            draftTune[key] = clampTileNudge((Number(draftTune[key]) || 0) + dir * step);
            dirty = true;
            applyDraftToUi();
            applyDraftPreview();
          });
        ...
        QUAD_TILE_IDS.forEach((id) => {
          modal.querySelector(`[data-action="reset-quad-${id}"]`)?.addEventListener('click', () => {
            draftTune[`${id}NudgeX`] = 0;
            draftTune[`${id}NudgeY`] = 0;
            dirty = true;
            applyDraftToUi();
            applyDraftPreview();
            statusEl.textContent = `${id.toUpperCase()} position reset. Save to persist.`;
          });
        });
        ...
        modal.querySelector('[data-action="reset"]')?.addEventListener('click', () => {
          draftTune = tuneToDraft(
            typeof QuiltMirrorLayout !== 'undefined' && QuiltMirrorLayout.MIRROR_TUNE_DEFAULT
              ? { ...QuiltMirrorLayout.MIRROR_TUNE_DEFAULT }
              : {
                  bottomLayout: 'single',
                  flipX: true,
                  flipY: true,
        ...
        const snapshotDraftTune = (tune) => tuneToDraft(tune || draftTune);
      ...
      async handlePushInstagramAssetsToFirestore() {
        ...
        try {
          ...
          let carouselSlide0ImageData = null;
          let carouselSlide0bImageData = null;
          let carouselSlide1ImageData = null;
          let carouselSlide2ImageData = null;
          let carouselSlide3ImageData = null;
          let carouselSlide4ImageData = null;
          if (arch.buildIntegratedInstagramCarouselImageData) {
            ...
            carouselSlide0ImageData = integrated?.carouselSlide0 || null;
            carouselSlide0bImageData = integrated?.carouselSlide0b || null;
            ...
            carouselSlide4ImageData = integrated?.carouselSlide4 || null;
          } else {
          ...
          // Asset push ≠ published post; Zapier fetch or banner tap marks IG post.
