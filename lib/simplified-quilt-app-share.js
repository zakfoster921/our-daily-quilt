  ...
  class SimplifiedQuiltAppV2Share {
      ...
      async handleAdminTuneSpeakerCutout() {
        ...
        try {
          ...
          modal.innerHTML = `
            <div class="odq-speaker-tune-panel" role="dialog" aria-modal="true" aria-label="Tune today's speaker cutout">
              ...
              <div class="odq-speaker-tune-drag-mode">
                ...
                <button type="button" data-drag-mode="quilt">Quilt</button>
              ...
              <details class="odq-speaker-tune-details odq-speaker-tune-quilt-details">
                ...
                <div class="odq-speaker-tune-quilt-zoom">
                  ...
                  <div class="odq-speaker-tune-quilt-offset" aria-label="Nudge quilt background up or down">
                    <div class="odq-speaker-tune-nudge-row">
                      <span class="odq-speaker-tune-nudge-label">Nudge</span>
                      <button type="button" data-quilt-offset="up" aria-label="Nudge quilt background up">↑</button>
                      <button type="button" data-quilt-offset="down" aria-label="Nudge quilt background down">↓</button>
                      <button type="button" data-action="quilt-offset-reset">Reset position</button>
                    </div>
                    <div class="odq-speaker-tune-nudge-row">
                      <span class="odq-speaker-tune-nudge-label">Big nudge</span>
                      <button type="button" data-quilt-big-offset="up" aria-label="Nudge quilt background up more">⇑</button>
                      <button type="button" data-quilt-big-offset="down" aria-label="Nudge quilt background down more">⇓</button>
                    </div>
                  </div>
                </div>
              </details>
          ...
          const quiltDragModeBtn = dragModeWrap?.querySelector('button[data-drag-mode="quilt"]');
          ...
          const quiltDetailsEl = modal.querySelector('.odq-speaker-tune-quilt-details');
          ...
          const syncSlide0TuneControls = () => {
            ...
            if (dragModeWrap) dragModeWrap.hidden = false;
            ...
            if (quoteFit && previewDragMode !== 'off' && previewDragMode !== 'quilt') {
              previewDragMode = 'off';
            }
            syncPreviewDragUi();
          };
          ...
          const updateQuiltDragLiveLabel = (clientX, clientY) => {
            const delta = previewDragFractionDelta(clientX, clientY);
            if (!delta) return;
            const y = odqNormalizeQuiltBgOffsetY?.(previewDragStartQuiltY + delta.dyFrac) ??
              previewDragStartQuiltY + delta.dyFrac;
            const label = modal.querySelector('[data-quilt-bg-offset-label]');
            if (label) {
              label.textContent = odqFormatQuiltBgOffsetYLabel
                ? odqFormatQuiltBgOffsetYLabel(y)
                : 'Centered';
            }
          };
          ...
          syncPreviewDragUi = () => {
            const quoteFit = isQuoteFitPreview(previewAspect);
            const postTab = normalizePreviewAspect(previewAspect) === 'post';
            const quiltMode = previewDragMode === 'quilt';
            const enabled =
              previewDragMode !== 'off' &&
              previewDragMode !== 'strip' &&
              (!quoteFit || quiltMode);
            if (stripDragModeBtn) stripDragModeBtn.hidden = !postTab;
            const speakerBtn = dragModeWrap?.querySelector('button[data-drag-mode="speaker"]');
            const textBtn = dragModeWrap?.querySelector('button[data-drag-mode="text"]');
            if (speakerBtn) speakerBtn.hidden = quoteFit;
            if (textBtn) textBtn.hidden = quoteFit;
            if (quiltDragModeBtn) quiltDragModeBtn.hidden = false;
            if (resetStripPlanBtn) {
              resetStripPlanBtn.hidden =
                !postTab || (!manualPostStripPlanActive && !capturedPostStripPlan?.length);
            }
            if ((!postTab || quoteFit) && previewDragMode === 'strip') {
              previewDragMode = quoteFit ? 'off' : 'speaker';
            }
            if (quoteFit && previewDragMode !== 'off' && previewDragMode !== 'quilt') {
              previewDragMode = 'off';
            }
            if (dragHintEl) {
              if (previewDragMode === 'quilt') {
                dragHintEl.textContent = 'Drag up or down to shift the quilt background';
              } else if (quoteFit) dragHintEl.textContent = '';
            ...
            if (previewSlide1Wrap) {
              ...
              previewSlide1Wrap.setAttribute(
                ...
                previewDragMode === 'strip'
                  ...
                  : previewDragMode === 'quilt'
                    ? 'Drag up or down to shift quilt background'
                    : enabled
            ...
            if (previewDragMode === 'quilt' && quiltDetailsEl) {
              quiltDetailsEl.open = true;
            }
            syncStripHitOverlay();
            syncSelectedStripRotateUi();
          };
          ...
          const commitPreviewDrag = (clientX, clientY) => {
            ...
            if (mode === 'quilt') {
              maybeLockPostTuneIndependent();
              quiltBgOffsetY = odqNormalizeQuiltBgOffsetY?.(previewDragStartQuiltY + dyFrac) ??
                previewDragStartQuiltY + dyFrac;
              markPreviewStale(
          ...
          const endPreviewDrag = (e) => {
            ...
            if (e && Number.isFinite(e.clientX) && Number.isFinite(e.clientY)) {
              ...
              updateQuoteStripOffsetLabel();
            } else if (previewDragModeAtStart === 'quilt') {
              updateQuiltBgOffsetLabel();
            }
          };
          ...
          const onPreviewDragPointerDown = (e) => {
            ...
            previewDragStartQuiltY = odqNormalizeQuiltBgOffsetY?.(quiltBgOffsetY) ?? 0;
            if (previewDragModeAtStart === 'text' && stripDetailsEl) stripDetailsEl.open = true;
            if (previewDragModeAtStart === 'quilt' && quiltDetailsEl) quiltDetailsEl.open = true;
            if (panel) {
          ...
          const onPreviewDragPointerMove = (e) => {
            ...
            if (previewDragModeAtStart === 'quilt') {
              updateQuiltDragLiveLabel(e.clientX, e.clientY);
            }
          };
          ...
          const switchPreviewAspect = async (nextAspect) => {
            ...
            updateAspectButtons();
            if (next === 'post' && quiltDetailsEl) quiltDetailsEl.open = true;
