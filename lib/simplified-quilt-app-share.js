  ...
  class SimplifiedQuiltAppV2Share {
      ...
      async handleAdminTuneSpeakerCutout() {
        ...
        try {
          ...
          syncPreviewDragUi = () => {
            const enabled = previewDragMode !== 'off';
            const feed = isFeedPreview(previewAspect);
            if (dragHintEl) {
              if (!enabled) dragHintEl.textContent = '';
              else if (previewDragMode === 'speaker') {
                dragHintEl.textContent = 'Drag preview to move speaker';
              } else {
                dragHintEl.textContent = feed
                  ? 'Drag preview to move quote clipping'
                  : 'Drag preview to move text strips';
              }
            }
            ...
            if (previewSlide1Wrap) {
              previewSlide1Wrap.style.cursor = enabled
                ? previewDragActive
                  ? 'grabbing'
                  : 'grab'
                : '';
              previewSlide1Wrap.style.touchAction = enabled ? 'none' : '';
          ...
          const commitPreviewDrag = (clientX, clientY) => {
            ...
            const dxFrac = (clientX - previewDragStartClient.x) / m.dispW;
            const dyFrac = (clientY - previewDragStartClient.y) / m.dispH;
            if (Math.abs(dxFrac) < 0.001 && Math.abs(dyFrac) < 0.001) return;
            markTuneUiDirty();
            if (previewDragMode === 'speaker') {
          ...
          if (previewSlide1Wrap) {
            previewSlide1Wrap.addEventListener('pointerdown', (e) => {
              if (previewDragMode === 'off') return;
            ...
            previewSlide1Wrap.addEventListener('pointermove', (e) => {
              if (!previewDragActive || e.pointerId !== previewDragPointerId) return;
              e.preventDefault();
              const dx = e.clientX - previewDragStartClient.x;
              const dy = e.clientY - previewDragStartClient.y;
              if (previewImg) previewImg.style.transform = `translate(${dx}px, ${dy}px)`;
            });
          ...
          if (stripLayoutSection) {
            stripLayoutSection.addEventListener('click', (e) => {
              ...
              if (feed || btn.dataset.action !== 'shuffle-strips') return;
              markTuneUiDirty();
