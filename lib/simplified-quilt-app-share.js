  ...
  class SimplifiedQuiltAppV2Share {
      ...
      async handleAdminTuneSpeakerCutout() {
        ...
        try {
          ...
          try {
            speakerImageForCanvas = await odqPromiseWithTimeout(
          ...
          modal.innerHTML = `
            <div class="odq-speaker-tune-panel" role="dialog" aria-modal="true" aria-label="Tune today's speaker cutout">
              ...
              <p class="odq-speaker-tune-aspect-hint" style="font-size:10px;line-height:1.3;color:#555;margin:0;">Post 4:5 previews layout-b.png (quote strips + speaker name, no cutout). Keyword and strip edits on Story apply to Post too; Post-only quote edits stay on Post. Speaker placement is Story only.</p>
          ...
          const buildTuneComposeOpts = (presetName, aspect) => {
            ...
            if (a === 'post') {
              /** layout-b.png: native 4:5 quote strips (not story positions scaled to post). */
              ...
              /** Speaker name band only — cutout is not drawn on post (matches nightly layout-b.png). */
              if (speakerImageForCanvas) {
          ...
          const renderPreview = async (presetName) => {
            ...
            try {
              ...
              let quiltBlobForCompose = highResBlob;
              if (
                previewAspect === 'post' &&
                this.quiltEngine?.blocks?.length &&
                this.archiveService?.getInstagramQuiltSourceBlob
              ) {
                try {
                  const postSource = await this.archiveService.getInstagramQuiltSourceBlob(
                    this.quiltEngine.blocks,
                    {
                      dateKey: todayKey,
                      exportModeLabel: 'layout_b_tune_post_preview',
                      skipFilmGrain: true
                    }
                  );
                  if (postSource) quiltBlobForCompose = postSource;
                } catch (_) {
                  /* keep tune preview blob */
                }
              }
              const outBlob = await composeInstagramLayoutBFromQuiltBlob(
                quiltBlobForCompose,
              ...
              setTuneDebug(
                `ok ${layoutW}×${layoutH} ${previewAspect}${previewAspect === 'post' ? ' layout-b-plain' : ''} seed story:${tuneDraftByAspect.story.stripLayoutSeed} post:${tuneDraftByAspect.post.stripLayoutSeed}${postQuoteStyleIndependent ? ' post-indep' : ''} ${tuneQuiltSource} tok ${myToken}/${renderToken}`
              );
            } catch (renderErr) {
      ...
      async handleAdminVerifyLayoutBTuneSettings() {
        ...
        const deviceLine = `Device: Story ${storyTweak.preset}${fmtNudge(storyTweak)}${fmtQuiltZoom(storyQuiltZoom)}, ${fmtKw(storyKw)}, ${fmtStrip(storyStrip)} · Post ${postTweak.preset}${fmtNudge(postTweak)}${fmtQuiltZoom(postQuiltZoom)}, ${fmtKw(postKw)}, ${fmtStrip(postStrip)}`;
        ...
        try {
          if (window.db && window.firestore) {
            ...
            if (!data) {
              ...
              serverLine = `Server instagram-images/${dk}${srcTag}: Story ${ss}${serverTweak(snCx, snCy, snRot)}${fmtQuiltZoom(sQuiltZoom)}, ${fmtKw(sk)}, ${fmtStrip(sStrip)} · Post ${ps}${serverTweak(pnCx, pnCy, pnRot)}${fmtQuiltZoom(pQuiltZoom)}, ${fmtKw(pk)}, ${fmtStrip(pStrip)}`;
              if (serverError) {
      ...
      async handlePushInstagramAssetsToFirestore() {
        ...
        try {
          ...
          const carouselSlides = await odqPromiseWithTimeout(
            arch.generateInstagramCarouselSlideImageData(blocks, contributors, today),
            120000,
            'IG carousel slide generation'
          );
          const carouselSlide1ImageData = carouselSlides?.slide1 || null;
          const carouselSlide2ImageData = carouselSlides?.slide2 || null;
