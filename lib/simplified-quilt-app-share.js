  ...
  class SimplifiedQuiltAppV2Share {
        ...
        try {
          ...
          modal.innerHTML = `
            <div class="odq-speaker-tune-panel" role="dialog" aria-modal="true" aria-label="Tune today's speaker cutout">
              ...
              <div class="odq-speaker-tune-aspect">
                ...
                <button type="button" data-aspect="post">Feed slide 1</button>
              </div>
          ...
          const aspectPreviewCacheKey = (aspect) => {
            ...
            const stripOff =
              a === 'post'
                ? clippingOff
                : odqNormalizeQuoteStripOffset?.(layoutDraft.quoteStripOffset) || { x: 0, y: 0 };
            return [
              a,
              speakerDraft.preset,
              ...
              a === 'post' ? 0 : odqNormalizeStripLayoutSeed(layoutDraft.stripLayoutSeed),
              stripOff.x || 0,
              stripOff.y || 0,
              a === 'post' ? '' : JSON.stringify(layoutDraft.keywordEmphasis),
              odqNormalizeQuiltBgZoom(tuneDraftByAspect[a].quiltBgZoom),
              odqNormalizeQuiltBgOffsetY?.(tuneDraftByAspect[a].quiltBgOffsetY) ?? 0,
              a === 'post' ? 'clip' : postQuoteStyleIndependent ? 1 : 0
            ].join('\0');
          };
          ...
          const syncFeedSlide1TuneControls = () => {
            const isPost = odqNormalizeTuneAspect(previewAspect) === 'post';
            if (keywordsDetailsEl) keywordsDetailsEl.hidden = isPost;
            if (stripLayoutStatusEl) stripLayoutStatusEl.hidden = isPost;
            if (shuffleStripsBtn) shuffleStripsBtn.hidden = isPost;
            if (stripSectionSummaryEl) {
              stripSectionSummaryEl.textContent = isPost ? 'Quote clipping' : 'Text strip layout';
            }
            if (stripOffsetMoveLabelEl) {
              stripOffsetMoveLabelEl.textContent = isPost ? 'Move clipping' : 'Move strips';
            }
            if (stripOffsetWrapEl) {
              stripOffsetWrapEl.setAttribute(
                'aria-label',
                isPost ? 'Move quote clipping' : 'Move all quote strips'
              );
            }
            if (stripDetailsEl) stripDetailsEl.open = isPost ? true : stripDetailsEl.open;
          };
          updateStripLayoutLabel();
          updateQuiltBgZoomLabel();
          updateQuiltBgOffsetLabel();
          ...
          const ensureTuneClippingAssets = async () => {
            ...
            const dataUrl = await selfRef.archiveService.generateNewspaperClippingImageData(todayKey);
          ...
          const composeFeedSlide1ClippingPreview = async (presetName, previewQuiltBlob) => {
            ...
            const baseBlob = await composeInstagramLayoutBFromQuiltBlob(
              previewQuiltBlob,
              qText,
              qAuthor,
              1080,
            ...
            const clipping = await ensureTuneClippingAssets();
          ...
          const renderPreview = async (presetName, opts = {}) => {
            ...
            if (updatesVisibleTab) {
              ...
              spinner.textContent =
                aspect === 'post' ? 'Rendering feed slide 1…' : 'Rendering preview…';
            }
            try {
              const layoutW = 1080;
              ...
              setTuneDebug(
                `ok ${layoutW}×${layoutH} ${aspect}${aspect === 'post' ? ` feed-clip ${clipOff.x}/${clipOff.y}` : ''}${isPrefetch ? ' prefetch' : ''} seed story:${tuneDraftByAspect.story.stripLayoutSeed} ${aspect === 'post' ? postTuneQuiltSource || tuneQuiltSource : tuneQuiltSource} tok ${myToken}/${renderToken}`
              );
            } catch (renderErr) {
              if (updatesVisibleTab) {
          ...
          const switchPreviewAspect = async (nextAspect) => {
            ...
            if (next === 'post' && postPreviewPrefetchPromise) {
              spinner.hidden = false;
              spinner.style.display = 'flex';
              spinner.textContent = 'Loading feed slide 1…';
              try {
          ...
          if (stripLayoutSection) {
            stripLayoutSection.addEventListener('click', (e) => {
              ...
              if (isPost || btn.dataset.action !== 'shuffle-strips') return;
              markTuneUiDirty();
            ...
            if (action === 'close') {
              ...
              void (async () => {
                ...
                try {
                  ...
                  if (typeof odqSetCachedLayoutBCarouselClippingOffset === 'function') {
                  ...
                  const writeTuneToClientFirestore = async () => {
                    ...
                    if (typeof odqWriteLayoutBCarouselClippingOffset === 'function') {
                      await odqPromiseWithTimeout(
      ...
      async handleAdminTuneQuiltMirror() {
        ...
        const refreshMirrorTunePreview = () => {
          ...
          const previousDisableSplitBand = this.renderer._exportDisableSplitBandFit;
          try {
            /** Single-field Flip Y needs classic stack; dup×2 should match live split-band layout. */
            this.renderer._exportDisableSplitBandFit = !isDoubleLayout(draftTune);
            ...
            this.renderer._exportDisableSplitBandFit = previousDisableSplitBand;
      ...
      async handleAdminVerifyLayoutBTuneSettings() {
        ...
        const deviceLine = `Device: Story ${storyTweak.preset}${fmtNudge(storyTweak)}${fmtQuiltZoom(storyQuiltZoom)}${fmtQuiltOffset(storyQuiltOffsetY)}${fmtStripOffset(storyStripOffset)}, ${fmtKw(storyKw)}, ${fmtStrip(storyStrip)} · Feed slide 1 ${postTweak.preset}${fmtNudge(postTweak)}${fmtQuiltZoom(postQuiltZoom)}${fmtQuiltOffset(postQuiltOffsetY)}${fmtClippingOffset(clippingOffset)}`;
