...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Share {
      ...
      _isTrueQuiltScreenExport(exportMeta) {
        return !!(
          exportMeta &&
          (exportMeta.quiltBlobFromLiveSvg === true || exportMeta.quiltBlobFromRendererExport === true)
        );
      }
      ...
      async _resolveStoredQuiltScreenBlobForTune(dateKey, blocks) {
        ...
        const blockCountMatches =
          Number.isFinite(storedBlockCount) && storedBlockCount > 0 && storedBlockCount === currentBlockCount;
      ...
      async _packFromQuiltScreenExport(blocks, opts, quiltSource) {
        const blob = await this.archiveService.getInstagramQuiltSourceBlob(blocks, opts);
        if (!blob) return null;
        const exportMeta = this.archiveService._igQuiltSourceExportMeta
          ? { ...this.archiveService._igQuiltSourceExportMeta }
          : {};
        return {
          blob,
          quiltSource,
      ...
      async getQuiltBlobForTunePreview(options = {}) {
        ...
        if (tuneFast && this.archiveService?.getInstagramQuiltSourceBlob) {
          const modeLabel = carouselPostPreview
            ? 'tune_carousel_post_quilt_screen_cover_source'
            : 'tune_story_quilt_screen_source';
          const baseOpts = {
            dateKey: dk,
            exportModeLabel: modeLabel,
            maxEdge,
            imageSmoothingQuality: 'low',
            skipFilmGrain: true,
          ...
          const fullOpts = { ...baseOpts, skipRendererExport: false };
          const onUpgrade = typeof options.onQuiltUpgrade === 'function' ? options.onQuiltUpgrade : null;
          const tuneLoadState = { initialPack: null };
          const storedPromise = this._resolveStoredQuiltScreenBlobForTune(dk, blocks);
          const fullPromise = (async () => {
            ...
            return this._packFromQuiltScreenExport(
              blocks,
              fullOpts,
              carouselPostPreview ? 'carousel-quilt-screen' : 'quilt-screen'
            );
          })();
          if (onUpgrade) {
            void fullPromise.then((fullPack) => {
              if (!fullPack?.blob || !this._isTrueQuiltScreenExport(fullPack.exportMeta)) return;
              const initial = tuneLoadState.initialPack;
              if (
                initial?.blob &&
                initial.quiltSource === 'stored-quilt-screen' &&
                initial.blockCount === fullPack.blockCount
              ) {
                return;
              }
              onUpgrade(fullPack);
          ...
          tuneLoadState.initialPack = await storedPromise;
          if (tuneLoadState.initialPack?.blob) {
            return tuneLoadState.initialPack;
          }
          const fullResult = await (onUpgrade
            ? Promise.race([
                fullPromise,
                new Promise((resolve) => window.setTimeout(() => resolve(null), 45000))
              ])
            : fullPromise);
          if (fullResult?.blob) return fullResult;
          return (await blocksFastMirror()) || { blob: null, quiltSource: 'none' };
        }
        if (tuneFast) {
          return blocksFastMirror();
        }
            ...
            try {
              ...
              const [
                ...
                this.getQuiltBlobForTunePreview({
                  tunePreviewFast: true,
                  onQuiltUpgrade: (upgradePack) => {
                    if (!upgradePack?.blob) return;
                    const gen = (quiltUpgradeGeneration += 1);
                    highResBlob = upgradePack.blob;
                    postHighResBlob = highResBlob;
                    tuneQuiltSource = upgradePack.quiltSource || tuneQuiltSource;
                    postTuneQuiltSource = tuneQuiltSource;
                    if (!tuneAssetsReady || gen !== quiltUpgradeGeneration) return;
                    markTunePreviewStaleForEdit({ invalidateStripPlan: false });
                    setTuneDebug(`upgrade ${tuneQuiltSource}`);
                    void renderPreview(currentPreset).then(() => schedulePostPreviewPrefetch(currentPreset));
                  }
                }).then((r) => (r && typeof r === 'object' ? r : { blob: r, quiltSource: 'legacy' })),
