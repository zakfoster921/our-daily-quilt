  ...
  class SimplifiedQuiltAppV2Share {
      ...
      async handleAdminTuneSpeakerCutout() {
        ...
        try {
          ...
          const previewSlide2PeekUrlByAspect = { story: null, post: null, feed: null };
            ...
            if (_prefetchDk && typeof globalThis.odqReadLayoutBPostStripPlan === 'function' && window.db && window.firestore) {
              globalThis.odqReadLayoutBPostStripPlan(_prefetchDk).then((plan) => {
                if (plan && plan.length) {
                  ...
                  manualPostStripPlan = plan.map((s) => ({ ...s }));
          ...
          const syncSelectedStripRotateUi = () => {
            ...
            const show =
              ...
              !!(manualPostStripPlan || capturedPostStripPlan)?.[selectedStripIndex];
            if (selectedStripRotateRowEl) selectedStripRotateRowEl.hidden = !show;
            if (!show || !selectedStripLabelEl) return;
            const plan = manualPostStripPlan || capturedPostStripPlan;
            selectedStripLabelEl.textContent = stripDisplayLabel(plan[selectedStripIndex], selectedStripIndex);
          };
          ...
          const syncPreviewWrapAspect = () => {
            ...
            const showSlide2Peek =
              normalizePreviewAspect(previewAspect) === 'post' &&
              !!previewSlide2PeekUrlByAspect.post &&
              !!slide2PeekWrap &&
              !slide2PeekWrap.hidden;
          ...
          const buildTuneComposeOpts = (presetName, aspect, extra = {}) => {
            ...
            if (a === 'post') {
              ...
              composeOpts.onPostStripPlan = (plan) => {
                ...
                if (manualPostStripPlanActive && prevManual?.length) {
                  manualPostStripPlan =
