...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Share {
      ...
      _getPrimaryQuiltMarkerRoot() {
        ...
        if (!root) return null;
        return (
          root.querySelector('#quiltPrimaryBand #quiltParallaxLayer') ||
          root.querySelector('#quiltFieldLayer > #quiltParallaxLayer') ||
          root.querySelector('#quiltParallaxLayer') ||
          root
        );
      }
      ...
      findQuiltBlockMarker(blockId) {
        ...
        const esc = this._escapeBlockIdForSelector(id);
        let node = scope.querySelector(`[data-block-id="${esc}"]`);
        if (node && !this._isMirroredQuiltMarker(node)) return node;
        for (const el of scope.querySelectorAll('[data-block-id]')) {
          if (this._isMirroredQuiltMarker(el)) continue;
          if (String(el.getAttribute('data-block-id') || '').trim() === id) return el;
        }
        return null;
      }
      ...
      _listQuiltBlockMarkers() {
        const scope = this._getPrimaryQuiltMarkerRoot();
        if (!scope) return [];
        return Array.from(scope.querySelectorAll('[data-block-id]')).filter(
          (node) => !this._isMirroredQuiltMarker(node)
        );
      }
      ...
      _findRenderedMarkerForBlock(block, submissionColor = '') {
        ...
        for (const marker of this._listQuiltBlockMarkers()) {
          ...
          if (this._spotlightBlockIdsRelated(markerId, blockId)) return marker;
          const engineBlock = blockById.get(markerId);
          if (!engineBlock) continue;
          if (engineBlock === block || belongs(engineBlock)) return marker;
        ...
        if (targetColor) {
          for (const marker of this._listQuiltBlockMarkers()) {
            if (this._spotlightNormalizeColor(marker.getAttribute('fill')) === targetColor) return marker;
          }
        }
        return null;
      }
      ...
      async _resolveMySpotlightTarget(options = {}) {
        ...
        const buildTarget = (buildOptions = {}) => {
          ...
          const colorMarker = submissionColor
            ? this._listQuiltBlockMarkers().find(
                (marker) =>
      ...
      async handleShowMyBlock() {
        ...
        if (reduceMotion) {
          ...
          setTimeout(() => {
            poly.classList.remove('wiggle');
            if (synthetic) g.remove();
          }, 1300);
        ...
        const restoreState = synthetic
          ? null
          : {
              parent: layer,
              nextSibling: g.nextSibling,
              transform: g.getAttribute('transform') || '',
              baseTransform: g.dataset.baseTransform || ''
            };
        const restoreOrder = () => {
          g.style.transform = '';
          ...
          this._restoreFromSpotlightOverlay(g, restoreState);
        };

        this._reparentToSpotlightOverlay(g);
