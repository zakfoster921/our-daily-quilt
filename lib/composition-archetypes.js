...
(function (root) {
  ...
  function installColumnsArchetype(engine, options = {}) {
    ...
    const preserveProductionOdds = options.preserveProductionOdds === true;
    if (!preserveProductionOdds) {
  ...
  function ninePatchCellForBlock(engine, block) {
    const cells = engine?._macroNinePatchCells;
    if (!Array.isArray(cells) || !cells.length || !block) return null;
    const cx = (Number(block.x) || 0) + (Number(block.width) || 0) / 2;
  ...
  function checkerboardParityForColor(engine, newColor) {
    const dateKey = engine?.quiltDateKey || 'ninepatch';
    const flip = hashRatioFromKey(`${dateKey}:9p-parity`) >= 0.5;
    const light = colorLightness(newColor) >= 0.48;
    const parity = light ? 0 : 1;
    return flip ? 1 - parity : parity;
  }

  function splitDirectionForNinePatch(block) {
    const { w: quiltW, h: quiltH } = quiltDims();
    const bx = Number(block?.x) || 0;
    const by = Number(block?.y) || 0;
    const bw = Number(block?.width) || 0;
    const bh = Number(block?.height) || 0;
    if (bx <= 4 && bw >= quiltW * 0.55) return 'vertical';
    if (by <= 4 && bh >= quiltH * 0.55) return 'horizontal';
    if (bw >= quiltW * 0.36) return 'vertical';
    if (bh >= quiltH * 0.36) return 'horizontal';
    return bw >= bh ? 'vertical' : 'horizontal';
  }

  /**
   * Big 9-patch: 3×3 macro grid with checkerboard light/dark cell routing.
   */
  function installNinePatchArchetype(engine, options = {}) {
    if (!engine || typeof engine.addColor !== 'function') return;
    clearArchetypeBiases(engine);

    const dateKey = String(options.dateKey || 'ninepatch-preview').trim() || 'ninepatch-preview';
    if (typeof engine.setQuiltDateKey === 'function') engine.setQuiltDateKey(dateKey);
    if (typeof engine.hydrateMacroLayoutFromPersistence === 'function') {
    ...
    const preserveProductionOdds = options.preserveProductionOdds === true;
    if (!preserveProductionOdds) {
      engine.specialStructureScaleMultiplier = Math.max(
        Number(engine.specialStructureScaleMultiplier) || 0,
        1
      );
      engine.macroFlattenedAngledSplitProb =
        Number.isFinite(Number(engine.macroFlattenedAngledSplitProb)) &&
    ...
    if (stash.selectPatternType) {
      engine.selectPatternType = (availablePatterns, block, newColor) => {
        for (const preferred of NINEPATCH_PATTERN_PREF) {
    ...
    engine._compositionSplitDirection = (block) => splitDirectionForNinePatch(block);

    if (stash.routeMacro) {
      engine._routeSplittableBlocksByMacroColor = (newColor, candidateBlocks) => {
        let routed = stash.routeMacro(newColor, candidateBlocks);
        if (!Array.isArray(routed) || routed.length <= 1) return routed;
        if (!Array.isArray(engine._macroNinePatchCells)) return routed;
        const targetParity = checkerboardParityForColor(engine, newColor);
        routed = [...routed].sort((a, b) => {
          const cellA = ninePatchCellForBlock(engine, a);
          const cellB = ninePatchCellForBlock(engine, b);
          const matchA = cellA && Number(cellA.parity) === targetParity ? 0 : 1;
          const matchB = cellB && Number(cellB.parity) === targetParity ? 0 : 1;
    ...
    engine.__compositionArchetypeStash = stash;
    engine.__compositionArchetypeKey = 'ninepatch';
  }

  root.CompositionArchetypes = {
    installColumnsArchetype,
    installNinePatchArchetype,
