...
(function (root) {
  ...
  /**
   * Columns framework:
   * - Day opens with a fixed grid of vertical, parallel, full-height bands (date-seeded widths).
   * - Pre-freeze splits are vertical only (carve column boundaries).
   * - Post-freeze splits are horizontal only (stack rows inside a column).
   * - No diagonal starter axes or angled macro shards (keeps edges parallel).
   */
