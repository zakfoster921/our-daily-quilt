...
(function (root, factory) {
  ...
  function (root) {
    ...
    function planOneMockCut(wonkySquare, remaining, panelW, panelH, cutIndex) {
      const [cx, cy] = polygonCentroid(remaining);
      const shrink = clamp(MOCK_TARGET_SHRINK - cutIndex * 0.03, 0.52, MOCK_TARGET_SHRINK);
      const fakeTarget = remaining.map(([x, y]) => [
        cx + (x - cx) * shrink,
        cy + (y - cy) * shrink
      ]);
      const plan = planCutsForShape(
        wonkySquare,
      ...
      return plan.cuts[0] || null;
    }

    function planFallbackMockCut(wonkySquare, remaining, panelW, panelH) {
      const [cx, cy] = polygonCentroid(wonkySquare);
      const fakeTarget = wonkySquare.map(([x, y]) => [
        cx + (x - cx) * 0.88,
        cy + (y - cy) * 0.88
      ]);
      const plan = planCutsForShape(
    ...
    function planMockCuts(basePlan, maxMockCuts = MAX_MOCK_CUTS) {
      const [cx, cy] = basePlan.targetCentroid;
    ...
    class PostColorCutRevealController {
      ...
      async _playMockCutsWhileWaiting(fetchPromise, color, cutMs, maxMockCuts) {
        ...
        while (!fetchSettled) {
          ...
          if (!cut) cut = planFallbackMockCut(wonkySquare, remainingAfterMock, panelW, panelH);
          if (!cut) {
            await wait(120);
            continue;
          }

          remainingAfterMock = await this._executeCut(
            cut,
            color,
            cutMs,
            () => fetchSettled
          );
          if (mockCutCount < maxMockCuts) mockCutCount += 1;
          cutIndex += 1;
        }
      ...
      async play(options = {}) {
        ...
        if (useLiveServer) {
          const fetchPromise = fetchLivePreview({
            color,
