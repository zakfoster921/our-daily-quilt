...
(function (root, factory) {
  ...
  function (root) {
    ...
    function renderPaperColorPiece(options = {}) {
      ...
      if (PaperDepth?.drawPaperPieceWithDepth) {
        PaperDepth.drawPaperPieceWithDepth(
          ...
          PaperDepth.scrapLikeDepthOptions(paperTextureImg, seed, depthScale)
        );
        return canvas;
      }
    ...
    class PostColorCutRevealController {
      ...
      _pieceRenderOptions(color, points, seed, depthScale) {
        return {
          points,
          color,
      ...
      resetLayers(color) {
        ...
        mountRemainingPiece(
          remainingLayer,
          this._pieceRenderOptions(color, this._plan.wonkySquare, `${seedBase}:initial`)
        );
      }
      ...
      async play(options = {}) {
        ...
        const seedBase = String(options.shapePreset || this.options.shapePreset || 'block');
        ...
        for (let i = 0; i < cuts.length; i++) {
          ...
          const scrapWrap = mountScrapPiece(
            ...
            this._pieceRenderOptions(color, cut.scrap, `${seedBase}:scrap:${i}`),
            cut.slide
          );

          mountRemainingPiece(
            this._els.remainingLayer,
            this._pieceRenderOptions(color, cut.remainingAfter, `${seedBase}:remaining:${i}`)
          );
        ...
        mountRemainingPiece(
          this._els.remainingLayer,
          this._pieceRenderOptions(color, target, `${seedBase}:final`, 1.15)
        );
