...
(function (root, factory) {
  ...
  function (root) {
    ...
    class PostColorCutRevealController {
      ...
      _sealFinalBlockSync(renderColor, targetPoints) {
        mountRemainingPiece(
      ...
      async play(options = {}) {
        ...
        for (let i = 0; i < cuts.length; i++) {
          ...
          await this._executeCut(cuts[i], fabricColor, cutMs, rushReal, pullMode);
        }

        const liveHandoff = typeof options.onHandoff === 'function';

        if (liveHandoff) {
          this._sealFinalBlockSync(fabricColor, target);
          options.onHandoff();
        } else {
          mountRemainingPiece(
            this._els.remainingLayer,
