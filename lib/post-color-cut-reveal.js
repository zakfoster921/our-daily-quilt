...
(function (root, factory) {
  ...
  function (root) {
    ...
    const FAST_CUT_LEAD_MS = 10;
    const FAST_SCRAP_FRAME_MS = 12;
    const FAST_SCRAP_STEPS = 2;
    ...
    class PostColorCutRevealController {
      ...
      _resolveScrapPullMode(shouldRush, explicitMode) {
        if (explicitMode === 'snap' || explicitMode === 'fast') return explicitMode;
        return shouldRush?.() ? 'snap' : 'normal';
      }
      ...
      _sealFinalBlock(renderColor, targetPoints) {
        mountRemainingPiece(
          this._els.remainingLayer,
          this._pieceRenderOptions(renderColor, targetPoints, 1.15)
        );
        const scraps = this._els.scrapLayer.querySelectorAll('.post-color-cut-reveal__scrap');
        scraps.forEach((wrap) => {
      ...
      async _executeCut(cut, renderColor, cutMs, shouldRush, pullMode) {
        ...
        const rush = typeof shouldRush === 'function' ? shouldRush : null;
        const mode = this._resolveScrapPullMode(rush, pullMode);
        const leadMs = mode === 'snap' ? FAST_CUT_LEAD_MS : mode === 'fast' ? 18 : rush?.() ? 40 : 80;
      ...
      async play(options = {}) {
        ...
        if (liveHandoff) {
          this._sealFinalBlock(fabricColor, target);
          options.onHandoff();
        } else {
          mountRemainingPiece(
            this._els.remainingLayer,
