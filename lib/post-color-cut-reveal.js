    ...
    class PostColorCutRevealController {
      ...
      async play(options = {}) {
        ...
        const landed = await this._playStopMotionLand(color, {
          ...
          shouldStop: useLiveServer ? () => pacingGate.shouldStop() : null
        });
        ...
        for (let i = 0; i < cuts.length; i++) {
          if (this._abort) return { aborted: true };
          const rushRealCut =
            useLiveServer && pacingGate.elapsed() > 4200 ? () => pacingGate.shouldStop() : null;
        ...
        const finalRestMs = useLiveServer ? pacingGate.tailMs(restMs, 320) : restMs;
        const finalQuiltMs = useLiveServer
