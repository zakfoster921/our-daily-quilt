...
(function (root, factory) {
  ...
  function (root) {
    ...
    const REAL_CUT_LEAD_MS = 48;
    const REAL_SCRAP_FRAME_MS = 48;
    ...
    class PostColorCutRevealController {
      ...
      async play(options = {}) {
        ...
        for (let i = 0; i < cuts.length; i++) {
          if (this._abort) return { aborted: true };
          // Once the polygon is known, keep every cut (including last) on the same
          // comfortable pace — a forced-slow final scrap reads as a freeze.
          const pullMode = serverReadyForCuts ? 'fast' : 'normal';
          await this._executeCut(
            cuts[i],
            fabricColor,
            cutMs,
        ...
        if (liveHandoff) {
          this._sealFinalBlockBeforeHandoff();
