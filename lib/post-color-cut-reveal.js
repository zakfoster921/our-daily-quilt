...
(function (root, factory) {
  ...
  function (root) {
    ...
    const COPY_STRIP_WORDS = [
      { text: 'Finding', xPct: 0.2, yPct: 0.1, rotate: -2.6 },
      { text: 'the', xPct: 0.56, yPct: 0.07, rotate: 3.1 },
      { text: 'perfect', xPct: 0.3, yPct: 0.18, rotate: -1.4 },
      { text: 'spot', xPct: 0.74, yPct: 0.155, rotate: 2.2 }
    ];
    ...
    function buildScrapPullFrames(slide) {
      const steps = 5;
      const frames = [];
    ...
    class PostColorCutRevealController {
      ...
      async _playScrapPullBack(scrapWrap, scrapPoints, slide, frameMs, shouldRush) {
        ...
        const frames = buildScrapPullFrames(slide);
        const stepMs = frameMs ?? DEFAULT_SCRAP_PULL_FRAME_MS;
      ...
      async _executeCut(cut, renderColor, cutMs, shouldRush) {
        ...
        await this._playScrapPullBack(
          ...
          cut.scrap,
          cut.slide,
          rush?.() ? 32 : DEFAULT_SCRAP_PULL_FRAME_MS,
          rush
        );
      ...
      _mountCopyStrips() {
        ...
        const panelH = this._plan.panelH;
        ...
        this._copyStripStates = COPY_STRIP_WORDS.map((word, index) => {
          ...
          el.style.left = `${Math.round(word.xPct * panelW)}px`;
          el.style.top = `${Math.round(word.yPct * panelH)}px`;
          el.style.fontSize = `${fontPx}px`;
          copyStage.appendChild(el);
          return {
            el,
      ...
      _applyCopyStripMotionFrame() {
        for (const strip of this._copyStripStates || []) {
          ...
          strip.el.style.transform = `translate(calc(-50% + ${frame.x}px), calc(-50% + ${frame.y}px)) rotate(${(strip.baseRotate + frame.rotate).toFixed(2)}deg)`;
        }
      }
      ...
      async play(options = {}) {
        ...
        for (let i = 0; i < cuts.length; i++) {
          if (this._abort) return { aborted: true };
          const rushRealCut =
            useServerPacing && pacingGate.elapsed() > 4200 ? () => true : null;
          await this._executeCut(cuts[i], renderColor, cutMs, rushRealCut);
        }
        ...
        const finalRestMs = useServerPacing ? pacingGate.tailMs(restMs, 320) : restMs;
        const finalQuiltMs = useServerPacing
          ? pacingGate.tailMs(quiltTransitionMs, 280)
          : quiltTransitionMs;
