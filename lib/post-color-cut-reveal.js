...
(function (root, factory) {
  ...
  function (root) {
    ...
    class PostColorCutRevealController {
      ...
      _scrapNeedsPull(scrapWrap) {
        const transform = String(scrapWrap?.style?.transform || '').trim();
        if (!transform || transform === 'none') return true;
        return /^translate\(\s*0(?:px)?,\s*0(?:px)?\s*(?:,\s*0(?:px)?)?\s*\)/.test(transform);
      }
      ...
      async _finalizeBeforeLiveHandoff(renderColor, targetPoints, lastCutPlayback) {
        if (
          lastCutPlayback?.scrapWrap &&
          lastCutPlayback?.slide &&
          this._scrapNeedsPull(lastCutPlayback.scrapWrap)
        ) {
          await this._playScrapPullBack(
            lastCutPlayback.scrapWrap,
            lastCutPlayback.scrap,
            lastCutPlayback.slide,
            REAL_SCRAP_FRAME_MS,
            null,
            'normal'
        ...
        mountRemainingPiece(
          ...
          this._pieceRenderOptions(renderColor, targetPoints, 1.15)
        );
        this._els.scrapLayer.innerHTML = '';
      }

      async _executeCut(cut, renderColor, cutMs, shouldRush, pullMode) {
      ...
      async play(options = {}) {
        ...
        let lastCutPlayback = null;

        for (let i = 0; i < cuts.length; i++) {
          if (this._abort) return { aborted: true };
          const isLast = i === cuts.length - 1;
          const runningLong = useServerPacing && pacingGate.elapsed() > 5200;
          const pullMode = isLast ? 'normal' : runningLong ? 'fast' : 'normal';
          const rushReal = !isLast && runningLong ? () => true : null;
          lastCutPlayback = await this._executeCut(
            cuts[i],
            fabricColor,
            cutMs,
            rushReal,
        ...
        if (liveHandoff) {
          await this._finalizeBeforeLiveHandoff(fabricColor, target, lastCutPlayback);
          options.onHandoff();
        } else {
          mountRemainingPiece(
            this._els.remainingLayer,
