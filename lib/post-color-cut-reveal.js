...
(function (root, factory) {
  ...
  function (root) {
    ...
    function computeSlideVector(scrapPoints, targetCentroid, panelW, panelH) {
      ...
      const pull = clamp(Math.sqrt(scrapArea) * 0.42, 32, 78);
    ...
    class PostColorCutRevealController {
      ...
      async _executeCut(cut, renderColor, cutMs, shouldRush) {
        ...
        const pullMs = DEFAULT_SCRAP_PULL_FRAME_MS * 5;
        const tailMs = Math.max(0, cutMs - leadMs - pullMs);
        if (tailMs) {
          if (rush) await waitMsOrUntil(tailMs, rush, 40);
          else await wait(tailMs);
        }
