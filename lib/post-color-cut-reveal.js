  ...
  function (root) {
    ...
    class PostColorCutRevealController {
      ...
      async _playScrapPullBack(scrapWrap, scrapPoints, slide, frameMs, shouldRush) {
        ...
        const stepMs = rush?.() ? Math.min(32, baseStepMs) : baseStepMs;
        ...
        for (const frame of frames) {
          ...
          // Never bail early on rush — server-ready pacing must still pull scraps off the mat.
          await wait(stepMs);
        }
      }
      ...
      async play(options = {}) {
        ...
        for (let i = 0; i < cuts.length; i++) {
          if (this._abort) return { aborted: true };
          const serverReady = useServerPacing ? () => pacingGate.shouldStop() : null;
          const rushRealCut =
            useServerPacing && (pacingGate.elapsed() > 4200 || serverReady?.())
              ? () => true
              : null;
