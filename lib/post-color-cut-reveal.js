    ...
    class PostColorCutRevealController {
      ...
      async play(options = {}) {
        ...
        for (let i = 0; i < cuts.length; i++) {
          ...
          await this._executeCut(cuts[i], fabricColor, cutMs, rushRealCut);
        }
        ...
        const hostEl =
          this.container.closest('.post-color-cut-reveal-host') || this.container.parentElement;
        ...
        if (liveHandoff) {
          ...
          if (this._abort) return { aborted: true };
          options.onHandoff();
          hostEl?.classList.add('post-color-cut-reveal-host--handoff');
          this._els.stage.classList.add('post-color-cut-reveal__stage--to-quilt');
          await Promise.all([wait(220), applyPromise || Promise.resolve()]);
        } else {
