 ...
 *
 * Packaged separately from the live submit path (which uses the solid-color spinner overlay).
 * - Lab / workshop: open post-color-lab.html
 * - Live app (optional): load this script + set CONFIG.APP.postColorCutRevealEnabled = true
 *   (or ?postColorCutReveal=1). Wiring stays in simplified-quilt-app-quilt.js
 *   (isPostColorCutRevealEnabled / runPostColorCutRevealSubmit).
 */
(function (root, factory) {
  ...
  function (root) {
    ...
    const MAX_CUTS = 8;
    /** Live handoff: after server returns we only need a short final trim, not 8 more cuts. */
    const LIVE_REAL_CUT_MAX = 4;
    ...
    /** Live submit: short green beat, then land — don't sit on a dark host waiting on assets. */
    const LIVE_MAT_HOLD_MS = 180;
    const DEFAULT_LAND_FRAME_MS = 195;
    const LIVE_LAND_FRAME_MS = 110;
    const DEFAULT_SCRAP_PULL_FRAME_MS = 95;
    /** Discrete holds — long enough to read as stop-motion, short enough to stay snappy. */
    const REAL_CUT_LEAD_MS = 90;
    const REAL_SCRAP_FRAME_MS = 88;
    const REAL_SCRAP_STEPS = 4;
    const FAST_CUT_LEAD_MS = 18;
    const FAST_SCRAP_FRAME_MS = 28;
    const FAST_SCRAP_STEPS = 3;
    const LIVE_MAT_PHOTO_WAIT_MS = 700;
    ...
    /** Fewer stop-motion steps for live so land doesn't feel like a hitchy slideshow. */
    const LIVE_LAND_FRAMES = DEFAULT_LAND_FRAMES.filter((_, i) => i % 2 === 0 || i === DEFAULT_LAND_FRAMES.length - 1);
    ...
    /** Hold a stop-motion pose on animation frames (avoids setTimeout drift/jank). */
    ...
    function waitMsOrUntil(ms, shouldStop, pollMs = 40) {
      if (ms <= 0) return Promise.resolve();
      if (typeof shouldStop !== 'function') return waitHold(ms);
      return new Promise((resolve) => {
        const started =
          typeof performance !== 'undefined' ? performance.now() : Date.now();
        const tick = (now) => {
    ...
    function snapLandFrame(wrap, frames) {
      ...
      wrap.style.transform =
        `translate3d(${final.x}px, ${final.y}px, 0) rotate(${final.rotate}deg)`;
    ...
    function renderPaperColorPiece(options = {}) {
      ...
      if (!ctx || !points?.length) {
        canvas.width = Math.max(1, panelW || 1);
        canvas.height = Math.max(1, panelH || 1);
        return canvas;
      }

      if (lightweight) {
        // BBox canvas — full-panel canvases every cut were hitching the stop-motion holds.
        const pad = 2;
        ...
        canvas.width = w;
        canvas.height = h;
        canvas.classList.add('post-color-cut-reveal__piece-canvas--bbox');
    ...
    function paintProceduralMat(canvas, panelW, panelH) {
    ...
    /**
     * Procedural mat first (instant). Then photo mat when ready.
     * timeoutMs: max wait for photo before continuing (0 = don't wait; omit = wait fully).
     */
    async function drawMatBackground(canvas, panelW, panelH, options = {}) {
      paintProceduralMat(canvas, panelW, panelH);
      if (options.photo === false) return false;
      const MatBg = root.CarouselCuttingMatBg;
      if (!MatBg?.loadCarouselCuttingMatImage || !MatBg?.drawCarouselCuttingMatBackground) {
        return false;
      }
      try {
        const loadPromise = MatBg.loadCarouselCuttingMatImage();
        ...
        const img =
          timeoutMs == null
            ...
            : await Promise.race([
                loadPromise,
                wait(Math.max(0, Number(timeoutMs) || 0)).then(() => null)
              ]);
        if (!img || !canvas.isConnected) return false;
        const ctx = canvas.getContext('2d');
        if (!ctx) return false;
    ...
    class PostColorCutRevealController {
      ...
      async _playScrapPullBack(
        ...
        const frameSteps = mode === 'fast' ? FAST_SCRAP_STEPS : REAL_SCRAP_STEPS;
          ...
          mode === 'snap'
            ...
            : mode === 'fast'
              ...
              : rush?.()
                ...
                : baseStepMs;
        const onNearComplete =
          typeof pullOptions.onNearComplete === 'function' ? pullOptions.onNearComplete : null;
        let nearCompleteFired = false;
        const maybeNearComplete = (frameIndex) => {
          if (!onNearComplete || nearCompleteFired || !frames.length) return;
          // Begin quilt fade while the scrap is still leaving (~60% through pull).
          if (frameIndex >= Math.max(0, Math.floor(frames.length * 0.6))) {
        ...
        if (mode === 'snap') {
          ...
          maybeNearComplete(frames.length - 1);
          return;
        }
        ...
        for (let fi = 0; fi < frames.length; fi++) {
          ...
          maybeNearComplete(fi);
          if (stepMs > 0) await waitHold(stepMs);
        }
        this._snapScrapPullFrame(scrapWrap, frames);
        maybeNearComplete(frames.length - 1);
      }

      _sealFinalBlockBeforeHandoff() {
        const { scrapLayer, cutLayer } = this._els;
      ...
      async _executeCut(cut, renderColor, cutMs, shouldRush, pullMode, cutOptions = {}) {
            ...
            : mode === 'fast'
              ...
              : mode === 'normal'
                ...
                : rush?.()
                  ? 40
                  : 80;
        if (leadMs > 0) await waitHold(leadMs);
        await this._playScrapPullBack(
          ...
          mode,
          {
            onNearComplete: cutOptions.onScrapNearComplete
          }
        );
      ...
      async prepare(options = {}) {
        ...
        const lightweightPieces = options.lightweightPieces === true;
        // Instant procedural mat, then wait briefly for the photo mat (usually cached).
        paintProceduralMat(this._els.mat, this._plan.panelW, this._plan.panelH);
        this.resetLayers(this.options.color || '#6fa8dc');
        if (!lightweightPieces) {
          const PaperDepth = root.IgCarouselPaperDepth;
          ...
          await drawMatBackground(this._els.mat, this._plan.panelW, this._plan.panelH, {
            timeoutMs: options.matPhotoWaitMs ?? LIVE_MAT_PHOTO_WAIT_MS
          });
        }
        return this._plan;
      }
      ...
      async _playStopMotionLand(color, options = {}) {
        ...
        const lightweight =
          options.lightweightPieces === true || this.options.lightweightPieces === true;
        const shouldStop = typeof options.shouldStop === 'function' ? options.shouldStop : null;
        const { remainingLayer, stage } = this._els;
        const pieceOpts = (points) => this._pieceRenderOptions(color, points, undefined, lightweight);
        ...
        if (shouldStop?.()) {
          mountRemainingPiece(remainingLayer, pieceOpts(this._plan.wonkySquare));
          return true;
        }
        ...
        wrap.appendChild(renderPaperColorPiece(pieceOpts(this._plan.wonkySquare)));
        ...
        for (let i = 0; i < frames.length; i++) {
          ...
          await waitMsOrUntil(frameMs, shouldStop);
      ...
      async play(options = {}) {
        ...
        // Live path: static strips only — strip stop-motion steals frames from scrap holds.
        if (!useLiveServer && !options.submitPromise) {
        ...
        // Let the host flip the quilt under the mat here (awaited) — not mid-scrap.
        if (typeof options.onBeforeCuts === 'function') {
          try {
            await Promise.resolve(options.onBeforeCuts());
          } catch (_) {
            /* host activate is best-effort */
          }
        }
        ...
        if (useServerPacing) {
          ...
          // Server already returned during mock wait — keep final trim short.
          const realCutBudget =
            typeof options.onHandoff === 'function' ? LIVE_REAL_CUT_MAX : MAX_CUTS;
          ...
          // Between mock→real: host may start data apply (not render) in this gap.
          if (typeof options.onAfterCut === 'function') {
            try {
              options.onAfterCut({
                index: -1,
                phase: 'after-mocks',
          ...
          // Let the last mock frame paint before real cuts (avoids a visible hitch).
          await new Promise((resolve) => requestAnimationFrame(resolve));
        ...
        if (useServerPacing && typeof console !== 'undefined') {
          console.log('[post-color-cut-reveal] pacing', {
            serverMs: pacingGate.serverWaitMs(),
            elapsedMs: pacingGate.elapsed(),
            mockCutCount,
            realCutCount: cuts.length
          });
        }

        let liveHandoffStarted = false;
        const beginLiveHandoff = async () => {
          if (!liveHandoff || liveHandoffStarted) return;
          liveHandoffStarted = true;
          if (typeof console !== 'undefined') {
            console.log('[post-color-cut-reveal] handoff', {
              totalMs: Date.now() - playStartedAt,
              serverMs: pacingGate.serverWaitMs()
            });
          }
          // Await host: paint under opaque mat, then fast fade. No quilt work before this.
          await Promise.resolve(options.onHandoff());
        };

        for (let i = 0; i < cuts.length; i++) {
          if (this._abort) return { aborted: true };
          const isLast = i === cuts.length - 1;
          // Still gap before the last cut: host paints quilt under the mat here
          // so the ending can be fade-only (moves the mosaic cost off the finale).
          if (isLast && typeof options.onBeforeLastCut === 'function') {
            try {
              await Promise.resolve(options.onBeforeLastCut());
            } catch (_) {
              /* host paint is best-effort */
            }
            if (this._abort) return { aborted: true };
          }
          // Server is usually ready before real cuts — still use normal pace so
          // the trim reads as stop-motion, not a blur. Only rush if we're overtime.
          const runningLong = useServerPacing && pacingGate.elapsed() > 5200;
          const pullMode = runningLong ? 'fast' : 'normal';
          await this._executeCut(
            cuts[i],
            fabricColor,
            cutMs,
          ...
          if (this._abort) return { aborted: true };
          if (isLast && liveHandoff) {
            await beginLiveHandoff();
            break;
          }
          // Fire-and-forget between poses — do not await (keeps scrap motion smooth).
          if (typeof options.onAfterCut === 'function') {
            try {
              options.onAfterCut({ index: i, isLast, cutCount: cuts.length });
            } catch (_) {
              /* */
            }
          }
      ...
      async playWithSubmission(submitPromise, options = {}) {
        const liveOptions = {
          ...
          matHoldMs: options.matHoldMs ?? LIVE_MAT_HOLD_MS,
          landFrameMs: options.landFrameMs ?? LIVE_LAND_FRAME_MS,
          // Full land pose set — holds are rAF-timed so denser frames stay readable.
          landFrames: options.landFrames || DEFAULT_LAND_FRAMES
        };
        if (!this._plan) await this.prepare(liveOptions);
