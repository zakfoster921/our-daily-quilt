...
(function (root, factory) {
  ...
  function (root) {
    ...
    const DEFAULT_LAND_FRAME_MS = 125;
    const DEFAULT_LAND_FRAMES = [
      { x: 0, y: -132, scale: 0.56, rotate: -10, opacity: 0.32 },
      { x: 10, y: -92, scale: 0.66, rotate: -7, opacity: 0.5 },
      { x: -8, y: -58, scale: 0.76, rotate: 5, opacity: 0.66 },
      { x: 6, y: -34, scale: 0.86, rotate: -3, opacity: 0.78 },
      { x: -4, y: -16, scale: 0.94, rotate: 2, opacity: 0.88 },
      { x: 2, y: -5, scale: 0.98, rotate: -0.5, opacity: 0.96 },
      ...
      async _playStopMotionLand(color, options = {}) {
        ...
        for (let i = 0; i < frames.length; i++) {
          ...
          wrap.style.transform =
            `translate(${frame.x}px, ${frame.y}px) rotate(${frame.rotate}deg) scale(${frame.scale})`;
          wrap.style.opacity = String(frame.opacity ?? 1);
          stage.classList.toggle('post-color-cut-reveal__stage--land-tick', i % 2 === 1);
          await wait(frameMs);
        }
