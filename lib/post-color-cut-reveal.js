...
(function (root, factory) {
  ...
  function (root) {
    ...
    const LIVE_MAT_HOLD_MS = 140;
    const DEFAULT_LAND_FRAME_MS = 195;
    const LIVE_LAND_FRAME_MS = 105;
    const DEFAULT_SCRAP_PULL_FRAME_MS = 95;
    const REAL_CUT_LEAD_MS = 70;
    const REAL_SCRAP_FRAME_MS = 64;
    ...
    /** Instant green mat first; photo mat upgrades when already cached / idle. */
    async function drawMatBackground(canvas, panelW, panelH, options = {}) {
      paintProceduralMat(canvas, panelW, panelH);
      if (options.photo === false) return;
      const MatBg = root.CarouselCuttingMatBg;
      if (!MatBg?.loadCarouselCuttingMatImage || !MatBg?.drawCarouselCuttingMatBackground) return;
      try {
        ...
        /* procedural mat already on screen */
      }
    }
    ...
    class PostColorCutRevealController {
      ...
      async prepare(options = {}) {
        ...
        // Never block first paint on paper/mat photo decode — that was the black screen.
        ...
        // Live path keeps the instant procedural mat — photo upgrade mid-cuts hitchs.
        return this._plan;
      }
      ...
      async playWithSubmission(submitPromise, options = {}) {
        const liveOptions = {
          ...
          landFrames: options.landFrames || LIVE_LAND_FRAMES
        };
