    ...
    class PostColorCutRevealController {
      ...
      async play(options = {}) {
        ...
        if (useServerPacing) {
          ...
          // Server is ready — let the host paint the updated quilt under the mat
          // before real cuts continue (not during scrap-pull frames).
        ...
        const beginLiveHandoff = () => {
          ...
          // Host should already have quilt painted under the mat — fade only.
        ...
        for (let i = 0; i < cuts.length; i++) {
          ...
          await this._executeCut(
            ...
            {
              lightweight: true,
              // Start the fast fade while the last scrap is still leaving.
              onScrapNearComplete: isLast && liveHandoff ? beginLiveHandoff : null
            }
          );
          if (this._abort) return { aborted: true };
          ...
          // Between cuts: hydrate the quilt under the mat (awaited, not during scrap pull).
          if (typeof options.onAfterCut === 'function') {
            try {
              await Promise.resolve(
