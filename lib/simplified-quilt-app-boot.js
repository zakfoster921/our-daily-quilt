...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    initReflectionCarouselLoop(notes) {
      ...
      const finishCarouselTouchGesture = () => {
        if (dragScrollFrame != null) {
          cancelAnimationFrame(dragScrollFrame);
          dragScrollFrame = null;
        }
        flushDragScroll();
        const wasHorizontal = carouselTouchGesture?.axis === 'x';
      ...
      viewport.addEventListener(
        ...
        (event) => {
          ...
          if (
            ...
            if (absDx > absDy * carouselTouchAxisRatio) {
              ...
              if (quiltScrollEl && carouselTouchGesture.lockScrollTop == null) {
                carouselTouchGesture.lockScrollTop = quiltScrollEl.scrollTop;
              }
            }
          }
          if (carouselTouchGesture?.axis === 'x') {
            ...
            queueDragScroll(carouselTouchGesture.startScrollLeft - dx);
          }
        },
        { passive: false }
      );
      viewport.addEventListener('touchend', finishCarouselTouchGesture, { passive: true });
      viewport.addEventListener('touchcancel', finishCarouselTouchGesture, { passive: true });
