  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    async loadDeferredAdminSlice() {
      ...
      const SliceCtor = await this._loadDeferredScript(
        'lib/simplified-quilt-app-admin.js?v=30',
    ...
    async ensureLayoutBComposeReady() {
      ...
      await this._loadDeferredScript('lib/layout-b-compose.js?v=161', 'composeInstagramLayoutBFromQuiltBlob');
      ...
      document.addEventListener('click', (event) => {
        ...
        if (heartBtn) {
          ...
          const responseId = String(
            heartBtn.dataset.reflectionResponseId || slide.dataset.reflectionResponseId || ''
          ).trim();
    ...
    formatReflectionCarouselStripHtml(strip, dateKey, stripIndex = 0, slideIndex = 0) {
      ...
      const heartMarkup = heartId
        ? this.renderReflectionCarouselEngagementHtml(heartId, normalized?.heartCount || 0, {
            ...
            'class="quilt-reflection-carousel-engagement',
            `data-reflection-response-id="${this.escapeQuiltFortuneText(responseId)}" class="quilt-reflection-carousel-engagement`
          )
        : '';
      const cardClass =
        stripIndex === 0
    ...
    renderReflectionCarouselEngagementHtml(heartId, heartCount, options = {}) {
      ...
      const heartHtml = this.renderReflectionCarouselHeartHtml(heartId, heartCount);
    ...
    async toggleReflectionAdminHighlight({ responseId, dateKey, slideEl, buttonEl } = {}) {
      ...
      try {
        ...
        this._reflectionWallLastContentKey = null;
        await this.loadReflectionThemesForToday?.();
    ...
    /** Admin highlight: washed paper tint from a real quilt block color. */
    getReflectionHighlightQuiltPalette() {
      const fromSpeakerPalette = this.getSpeakerCutoutPaperPalette?.() || [];
      const palette = (Array.isArray(fromSpeakerPalette) ? fromSpeakerPalette : [])
        .map((color) => String(color || '').trim())
        .filter((color) => /^#[0-9A-Fa-f]{6}$/.test(color));
      if (palette.length) return palette;
      const summaryPalette = (this.getQuiltPaletteSummary?.()?.palette || [])
        .map((item) => String(item?.color || '').trim())
        .filter((color) => /^#[0-9A-Fa-f]{6}$/.test(color));
      if (summaryPalette.length) return summaryPalette;
      const blocks = Array.isArray(this.quiltEngine?.blocks) ? this.quiltEngine.blocks : [];
      const seen = new Set();
      const fromBlocks = [];
      blocks.forEach((block) => {
        ...
        seen.add(key);
        fromBlocks.push(color);
      });
      if (fromBlocks.length) return fromBlocks;
      const fallback = String(CONFIG.APP.defaultColor || '#ea9b9a').trim();
      return /^#[0-9A-Fa-f]{6}$/.test(fallback) ? [fallback] : ['#ea9b9a'];
    }

    pickReflectionHighlightQuiltHex(seed = 0) {
      const palette = this.getReflectionHighlightQuiltPalette();
      const safeSeed = Math.abs(Number(seed) || 0);
      return palette[safeSeed % palette.length] || CONFIG.APP.defaultColor || '#ea9b9a';
    }

    reflectionFabricHexToRgbCsv(hex) {
    ...
    washReflectionHighlightQuiltHex(hex) {
      const source = String(hex || '').trim();
      const normalized = source.startsWith('#') ? source : `#${source}`;
      if (!/^#[0-9A-Fa-f]{6}$/.test(normalized)) {
        return this.brightenReflectionCarouselPatchRgb(REFLECTION_NEWSPAPER_PATCH_RGB);
      }
      const hsv = Utils.hexToHsv(normalized);
      const wash = Utils.hsvToHex(
        hsv.h,
        Math.max(18, Math.min(38, hsv.s * 0.52 + 12)),
        Math.max(90, Math.min(97, hsv.v < 90 ? hsv.v + (95 - hsv.v) * 0.62 : hsv.v + 4))
      );
      const match = /^#?([0-9a-f]{6})$/i.exec(wash);
      if (!match) return this.reflectionFabricHexToRgbCsv(normalized);
      const h = match[1];
      const lighten = 0.1;
      const channels = [
        parseInt(h.slice(0, 2), 16),
    ...
    getReflectionHighlightPatchRgb(seed = 0) {
      return this.washReflectionHighlightQuiltHex(this.pickReflectionHighlightQuiltHex(seed));
    }

    getReflectionHighlightPatchSeed(theme, slideIndex = 0) {
      const normalized = this.normalizeReflectionWallTheme(theme);
      const dateKey = String(Utils.getTodayKey() || 'nodate').trim() || 'nodate';
      const responseId = normalized?.split && Array.isArray(normalized.strips)
        ? String(normalized.strips.find((strip) => strip.adminHighlight)?.responseId || normalized.strips[0]?.responseId || '').trim()
        : String(normalized?.responseId || '').trim();
      if (responseId) {
        return Utils.hashStringToUint(`${dateKey}:reflection-highlight:${responseId}`);
      }
      return Math.max(0, Number(slideIndex) || 0);
    }

    /** Admin highlight fallback when quilt palette is unavailable. */
    brightenReflectionCarouselPatchRgb(rgbCsv) {
      const hex = this.reflectionFabricRgbCsvToHex(rgbCsv);
      const hsv = Utils.hexToHsv(hex);
      let hue = hsv.s < 6 ? 36 : hsv.h;
      if (hsv.s >= 10 && hue > 55 && hue < 185) {
        hue = 30 + (hue % 16);
      }
      const wash = Utils.hsvToHex(
        hue,
        Math.max(18, Math.min(30, hsv.s < 8 ? 24 : hsv.s * 1.1)),
        Math.max(91, Math.min(97, Math.max(hsv.v, 92)))
      );
      const match = /^#?([0-9a-f]{6})$/i.exec(wash);
      if (!match) return this.washReflectionCarouselPatchRgb(rgbCsv);
      const h = match[1];
      const lighten = 0.07;
      const channels = [
        parseInt(h.slice(0, 2), 16),
    ...
    getReflectionFabricPatchStyle(index, themeText, allThemes, rgbOverride, options = {}) {
      ...
      const rgb = options.adminHighlight
        ? this.getReflectionHighlightPatchRgb(options.highlightSeed ?? index)
    ...
    buildReflectionThemePatchSlide(slideIndex, theme, allThemes, rgb, edgeFray = {}, myReflection = null) {
      ...
      const highlightSeed = this.getReflectionHighlightPatchSeed(theme, slideIndex);
      const previewRgb = adminHighlighted
        ? this.getReflectionHighlightPatchRgb(highlightSeed)
    ...
    _normalizeReflectionThemesPayload(key, data) {
      ...
      const adminFirstResponseHighlight = data.adminFirstResponseHighlight === true;
      const adminFirstResponseHighlightAtIso = String(data.adminFirstResponseHighlightAtIso || '').trim();
    ...
    _applyQuoteSpeakerCutoutAnchor(imageWrap, img) {
      ...
      const plainComposite =
        ...
        imageWrap.querySelector('.quote-speaker-image')?.dataset?.plainCutoutDirect === '1';
