...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    setupEventListeners() {
      ...
      document.addEventListener('screenChange', (e) => {
        if (e?.detail?.screenId === 'screen-quilt') {
          ...
          if (!this._footerIconChromePrewarmedThisVisit) {
    ...
    prewarmFooterIconChrome() {
      ...
      if (this._footerIconChromePrewarmedThisVisit) {
        return this._footerIconChromePrewarmResult === true;
      }
      ...
      try {
        ...
        this._footerIconChromePrewarmResult = ready;
        return ready;
      } finally {
    ...
    updateFooterIconPaperLayout(options = {}) {
      ...
      this._footerIconPaperElements().forEach((paper) => {
        ...
        if (!slotRect || rect.width < 1 || rect.height < 1) {
          if (!projectAtDock) {
            paper.style.setProperty('--quilt-footer-icon-paper-height', '0px');
          }
          return;
        ...
        const effectiveTop = rect.top - scrollDelta;
        if (
          !projectAtDock &&
        ...
        const slotTop = slotRect.top - scrollDelta;
        const topPx = Math.round(effectiveTop - slotTop + (rect.height - squarePx) / 2);
    ...
    _applyQuoteSpeakerPlainCutoutStack(cutout, img) {
      ...
      const cutoutMat = document.getElementById('quoteSpeakerCutoutMat');
      if (cutoutMat) cutoutMat.hidden = false;
    ...
    _applyQuoteSpeakerImageLoaded(
      ...
      const cutoutMat = document.getElementById('quoteSpeakerCutoutMat');
      if (cutoutMat) cutoutMat.hidden = usePopArtStack || useDataUrlPaint;
      if (colorWash) {
    ...
    _refreshQuoteSpeakerWidgetNowInner(quote = null) {
      ...
      if (canKeepLoadedImage) {
        ...
        const cutoutMat = document.getElementById('quoteSpeakerCutoutMat');
        if (cutoutMat) {
          cutoutMat.hidden = imageWrap.classList.contains('quote-speaker-cutout--paper-shapes');
        }
        if (colorWash) {
