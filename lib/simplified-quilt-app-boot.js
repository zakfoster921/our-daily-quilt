...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    /** Pull mirror tune from Railway (native iOS misses Firestore snapshot updates). */
    async syncMirrorTuneFromServer(dateKey = Utils.getTodayKey()) {
      if (this._mirrorTuneModalOpen) return { ok: false, reason: 'modal_open' };
      const key = String(dateKey || '').trim();
      if (!key) return { ok: false, reason: 'dateKey required' };
      const fetchPayload =
        typeof globalThis.odqFetchMirrorTunePayloadViaServer === 'function'
          ? globalThis.odqFetchMirrorTunePayloadViaServer
          : null;
      if (!fetchPayload) return { ok: false, reason: 'fetch_unavailable' };
      try {
        const payload = await fetchPayload(key);
        if (!payload) return { ok: false, reason: 'fetch_failed' };
        this._reconcileMirrorTuneFromRemotePayload(key, payload);
        return { ok: true };
      } catch (err) {
        this.logger?.warn?.('syncMirrorTuneFromServer failed:', err);
