...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    /**
     * Seat + scale the birthday hat to the measured forehead.
     * Measure from a canvas-readable layout source first (remote #quoteSpeakerImage often
     * taints getImageData → silent fallback to fixed 52%/50%, which reads small + left).
     * Map through the cutout's paint offsets so the brim matches the drawn portrait.
     */
    _positionQuoteSpeakerBirthdayHat(cutout, img) {
      ...
      const measureCandidates = [
        ...
        img,
        portraitImg
      ].filter((el, i, arr) => el?.naturalWidth && arr.indexOf(el) === i);

      let measureImg = null;
      let crown = null;
      let bounds =
        cutout._speakerSubjectBounds || cutout._speakerOpaqueBounds || null;
      for (const candidate of measureCandidates) {
        const candidateBounds =
          bounds ||
          this._quoteSpeakerMeasureSubjectBounds(candidate) ||
          this._quoteSpeakerMeasureOpaqueBounds(candidate);
        if (candidateBounds && !bounds) bounds = candidateBounds;
        const measured = this._quoteSpeakerMeasureCrownTop(candidate, candidateBounds);
        if (measured) {
          measureImg = candidate;
          crown = measured;
          break;
        }
      }
      // Last resort: opaque bounds alone (bust/head cutout) — still better than frame center.
      if (!crown && bounds && layoutImg?.naturalWidth) {
        measureImg = layoutImg;
        crown = {
          x: (bounds.minX + bounds.maxX) / 2,
          y: bounds.minY,
          foreheadWidth: Math.max(8, bounds.width * 0.72)
        };
      }
      if (!crown || !measureImg?.naturalWidth) {
        clear();
        return;
      }
      ...
      const drawW = readPx('--speaker-cutout-bg-w', frameW);
      const drawH = readPx('--speaker-cutout-bg-h', frameH);
      if (drawW < 1 || drawH < 1) {
        clear();
        return;
      }
      ...
      // Brim ≈ forehead; slight pad so it reads as on the temples.
      const hatW = Math.max(28, Math.min(frameW * 0.95, foreheadW * 1.06));
      const dip = Math.max(6, Math.min(24, frameH * 0.055));
      const brimY = crownY + dip;
      const bottomPct = ((frameH - brimY) / frameH) * 100;
      const leftPct = (crownX / frameW) * 100;
      const widthPct = (hatW / frameW) * 100;
      cutout.style.setProperty(
        '--speaker-hat-bottom',
        `${Math.max(55, Math.min(94, bottomPct)).toFixed(2)}%`
      );
      cutout.style.setProperty(
        '--speaker-hat-left',
        `${Math.max(18, Math.min(82, leftPct)).toFixed(2)}%`
      );
      cutout.style.setProperty(
        '--speaker-hat-width',
        `${Math.max(30, Math.min(95, widthPct)).toFixed(2)}%`
      );
    }
    ...
    _refreshQuoteSpeakerWidgetNowInner(quote = null) {
      ...
      if (hatEl || glitterEl || birthdayCaptionEl) {
        ...
        if (isBirthday && imageWrap) {
          ...
          imageWrap.style.removeProperty('--speaker-hat-bottom');
          imageWrap.style.removeProperty('--speaker-hat-left');
          imageWrap.style.removeProperty('--speaker-hat-width');
        }
      }
