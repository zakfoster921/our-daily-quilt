...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    _applyQuoteSpeakerBirthdayHatSeat(cutout, seat) {
      ...
      const bottom = `${Math.max(38, Math.min(90, seat.bottomPct)).toFixed(2)}%`;
    ...
    _quoteSpeakerBirthdayHatPaintBoxSeat(cutout) {
      ...
      // Target: brim ~18% down the portrait — into hair/forehead, not perched on the crown tip.
      const brimY = boxY + boxH * 0.18;
      return {
    ...
    _quoteSpeakerBirthdayHatSeatFromCrown(cutout, measureImg, crown) {
      ...
      const seatFromBox = (originX, originY, boxW, boxH) => {
        ...
        // Small seat into the scalp — foreheadW*0.28 buried the brim mid-face on head crops.
        const dip = Math.max(8, Math.min(foreheadW * 0.1, boxH * 0.045));
