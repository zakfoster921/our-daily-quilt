...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    _applyQuoteSpeakerBirthdayHatSeat(cutout, seat) {
      ...
      const bottom = `${Math.max(55, Math.min(94, seat.bottomPct)).toFixed(2)}%`;
      const left = `${Math.max(15, Math.min(85, seat.leftPct)).toFixed(2)}%`;
      const width = `${Math.max(32, Math.min(96, seat.widthPct)).toFixed(2)}%`;
    ...
    _quoteSpeakerBirthdayHatPaintBoxSeat(cutout) {
      ...
      const crownY = boxY + boxH * 0.06;
      const foreheadW = boxW * 0.84;
      const hatW = Math.max(28, Math.min(frameW * 0.96, foreheadW));
      const dip = Math.max(6, Math.min(22, frameH * 0.05));
    ...
    _quoteSpeakerBirthdayHatSeatFromCrown(cutout, measureImg, crown) {
      ...
      if (imgRect && imgRect.width > 8 && cutoutRect.width > 8) {
        ...
        const hatW = Math.max(28, Math.min(frameW * 0.96, foreheadW * 1.05));
        const dip = Math.max(6, Math.min(24, frameH * 0.055));
        const brimY = crownY + dip;
        return {
          bottomPct: ((frameH - brimY) / frameH) * 100,
          leftPct: (crownX / frameW) * 100,
