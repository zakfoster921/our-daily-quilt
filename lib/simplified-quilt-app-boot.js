...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    _scheduleReflectionThemesForLaunch() {
      ...
      // Start during portal dwell (idle) so fast scrollers do not outrun the wall.
      // Still yields to quilt work via requestIdleCallback — not a hard delay after splash.
      this._schedulePostLaunchIdleTask(
    ...
    syncReflectionCarouselHighlight(slideEl, { adminHighlight } = {}) {
      ...
      if (slideEl?.matches?.('[data-reflection-split-card]')) {
        ...
        const currentRgb = slideEl.style.getPropertyValue('--reflection-fabric-rgb').trim();
        if (currentRgb) {
    ...
    async _pinQuiltScrollPaperImage(img) {
      ...
      if (!src || img.naturalWidth < 1) return;
      try {
