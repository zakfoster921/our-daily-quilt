  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    async syncLiveDailyData(options = {}) {
      ...
      try {
        ...
        if (quiltOnly) {
          void this._ensureTodayQuotePinnedForLaunch({ quiltOnlyLaunch: true }).then((pinned) => {
            if (!pinned) return;
            this._refreshQuoteScreenIfActive();
            void this._primeQuiltQuoteChrome();
          });
          quotesPinned = this.quoteService?.hasTodayQuotePinned?.() || false;
          quiltResult = await this.dataService.loadQuiltFromServer(todayKey);
        } else {
        ...
        if (!quotesPinned) {
          ...
          if (!hasPinnedQuote) {
            if (quiltOnly && quiltResult.ok) {
              this.logger.warn('Quote pin still pending during quilt-only sync; continuing with live quilt');
            } else {
              return { ok: false, reason: 'quote_assignment' };
            }
          }
