          ...
          .then(async ({ remote, prepared }) => {
            ...
            if (/^https?:\/\//i.test(base)) {
              if (this._quoteSpeakerCanvasPipelineBlocked()) {
                // Native WebViews can hang on the proxy/canvas prep path; the widget can display
                // the remote cutout directly while export paths keep using the canvas-safe proxy.
                display = base;
              } else {
