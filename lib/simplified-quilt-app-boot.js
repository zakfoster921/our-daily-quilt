...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    async ensureLayoutBComposeReady() {
      ...
      await this._loadDeferredScript('lib/layout-b-compose.js?v=91', 'composeInstagramLayoutBFromQuiltBlob');
    ...
    setupReflectionResponsePrototype() {
      ...
      form.addEventListener('submit', async (event) => {
        ...
        try {
          ...
          try {
            await this.fetchMyReflectionForDate(dateKey);
          } catch (_) {
    ...
    async loadReflectionThemesForToday() {
      ...
      const dateKey =
        typeof this.getEffectiveAppDateKey === 'function' && this.isAdminTomorrowPreviewActive?.()
          ...
          : Utils.getTodayKey();
      if (!this._reflectionThemesNotFoundKeys) this._reflectionThemesNotFoundKeys = new Set();
      const [todayThemes, assignmentFirst, myReflection] = await Promise.all([
        this._readReflectionThemesForDate(dateKey),
        this.fetchTodayFirstResponseFields(),
        this.syncMyReflectionForToday(dateKey, { force: true })
      ]);
      ...
      const wallThemes = await this.resolveReflectionWallThemes(
        ...
        myReflection || this._readMyReflectionCache(dateKey)
      );
      if (wallThemes.length) {
        this.renderReflectionAnonymousWall(wallThemes, myReflection || this._readMyReflectionCache(dateKey));
        return;
      }
      if (!this._reflectionWallLastContentKey) {
        this.renderReflectionAnonymousWall([], myReflection || this._readMyReflectionCache(dateKey));
      }
      })().finally(() => {
