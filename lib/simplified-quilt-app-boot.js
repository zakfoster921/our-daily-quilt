  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    formatReflectionCarouselStripHtml(strip, dateKey, stripIndex = 0, slideIndex = 0) {
      ...
      const heartMarkup = heartId
        ? this.renderReflectionCarouselHeartHtml(heartId, normalized?.heartCount || 0).replace(
            'class="quilt-reflection-carousel-heart',
            `data-reflection-response-id="${this.escapeQuiltFortuneText(responseId)}" class="quilt-reflection-carousel-heart`
          )
        : '';
      const cardClass =
        stripIndex === 0
      ...
      return `<div class="${cardClass}" data-reflection-split-card data-reflection-hand-cut-seed="${this.escapeQuiltFortuneText(handCutSeed)}">
        <div class="quilt-reflection-carousel-split-card-copy quilt-reflection-carousel-copy">
          ...
          <div class="quilt-reflection-carousel-split-strip-footer">
    ...
    formatReflectionCarouselTextHtml(entry, slideIndex = 0) {
      const normalized = this.normalizeReflectionWallTheme(entry);
      if (normalized?.split && Array.isArray(normalized.strips)) {
    ...
    buildReflectionThemePatchSlide(slideIndex, theme, allThemes, rgb, edgeFray = {}, myReflection = null) {
      ...
      const heartMarkup = isSplit
        ? ''
        : this.renderReflectionCarouselHeartHtml(heartId, normalized?.heartCount || 0);
      const splitClass = isSplit ? ' quilt-reflection-carousel-slide--split' : '';
      return `
            <article class="quilt-reflection-carousel-slide quilt-reflection-carousel-slide--clipping${splitClass}${wideCopyClass}${lightClass}" data-reflection-theme-slide data-reflection-slide="${slideIndex}" data-reflection-hand-cut-seed="${dateKey}:reflection-response:${slideIndex}"${mineAttrs} style="${this.getReflectionFabricPatchStyle(slideIndex, theme, allThemes, rgb)}">
              <div class="quilt-reflection-carousel-text">${this.formatReflectionCarouselTextHtml(theme, slideIndex)}</div>
    ...
    _readLocalReflectionThemesCache(dateKey) {
      try {
        ...
        if (!entry || typeof entry !== 'object') return null;
        if (Number(entry.schemaVersion) !== 2) return null;
        return entry;
      } catch (_) {
        return null;
      }
    ...
    async _readReflectionThemesForDate(key) {
      ...
      let winner = null;
      let winnerSource = '';
      const claimIfRicher = (data, source) => {
        const normalized = this._normalizeReflectionThemesPayload(k, data);
        if (!normalized) return;
        const splitCount = (normalized.themes || []).filter((entry) => entry?.split).length;
        const priorSplitCount = (winner?.themes || []).filter((entry) => entry?.split).length;
        if (!winner || splitCount > priorSplitCount || (normalized.themes?.length || 0) > (winner.themes?.length || 0)) {
          winner = normalized;
          winnerSource = source;
        }
      };
