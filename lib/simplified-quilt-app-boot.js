...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    _quoteSpeakerStoryCutoutBakeOptions(bakeLayout = {}) {
      ...
      const washColor = String(cardColors.washColor || this.getSpeakerCutoutWashColor?.() || '#ea9b9a').trim();
      const paperSeed = this._quoteSpeakerPlainCutoutPaperSeed();
      return {
        bakeKey: `${washColor}|quilt-bg-bright-v1`,
        options: {
          isCutoutPng: true,
          skipScannerBed: true,
    ...
    _applyQuoteSpeakerPlainCutoutBacking(cutout, img, sourceUrl = '') {
      ...
      const paperSig = `${layoutSig}|${bakeKey}|story-over-quilt-v10`;
      const compositeReady =
