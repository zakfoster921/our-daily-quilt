...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    syncIgReflectionCaptureClusterJoinTape(gridEl, dateKey) {
      ...
      const appendHorizontalTape = (topPatch, bottomPatch, tapeVariant) => {
        ...
        const visualTopPaperBottom = topR.bottom + this._readReflectionPatchPaperEdgeY(topPatch);
        ...
        const joinLeft = overlapLeft + overlapW / 2 + tapeVariant.verticalShift - gridRect.left;
        ...
        tape.style.setProperty('--reflection-join-tape-tilt-jitter', `${tapeVariant.tiltJitterDeg}deg`);
        gridEl.appendChild(tape);
      };

      const appendHorizontalTapeAtTopSeam = (topPatches, bottomPatch, tapeVariant) => {
        const above = Array.isArray(topPatches) ? topPatches.filter(Boolean) : [];
        if (above.length < 2 || !bottomPatch || !tapeVariant) return;

        const bottomR = bottomPatch.getBoundingClientRect();
        let visualTopRowBottom = -Infinity;
        above.forEach((patch) => {
          const rect = patch.getBoundingClientRect();
          visualTopRowBottom = Math.max(
            visualTopRowBottom,
            rect.bottom + this._readReflectionPatchPaperEdgeY(patch)
          );
        });
        const bottomEdgeY = this._readReflectionPatchPaperEdgeY(bottomPatch);
        const visualBottomTop = bottomR.top - bottomEdgeY;
        const seamY = (visualTopRowBottom + visualBottomTop) / 2;
        const joinLeft = above[1].getBoundingClientRect().left + tapeVariant.verticalShift - gridRect.left;
        ...
        tape.setAttribute('aria-hidden', 'true');
        tape.style.setProperty('--reflection-join-tape-left', `${joinLeft}px`);
        tape.style.setProperty('--reflection-join-tape-top', `${seamY - gridRect.top}px`);
        tape.style.setProperty('--reflection-join-tape-span', `${tapeVariant.spanPx}px`);
        tape.style.setProperty('--reflection-join-tape-width-scale', tapeVariant.widthScale);
        tape.style.setProperty('--reflection-join-tape-clip', tapeVariant.clipPath);
        tape.style.setProperty('--reflection-join-tape-texture-y', tapeVariant.textureY);
