...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    refreshQuiltMoodWidget(quote = null) {
      if (
        this._parallaxScrollActive &&
        this._isCapacitorNativeClient?.() &&
        !this._moodRefreshForceImmediate
      ) {
        this._runAfterQuiltScrollQuiet(() => {
          this._moodRefreshForceImmediate = true;
          try {
            this.refreshQuiltMoodWidget(quote);
          } finally {
            this._moodRefreshForceImmediate = false;
          }
        });
        return;
      }
