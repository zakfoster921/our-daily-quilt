...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    /** Scan cutout alpha — full envelope + torso-band width for subject-aware zoom. */
    _quoteSpeakerMeasureOpaqueBounds(img) {
      ...
      const alphaAt = (x, y) => data[(y * scanW + x) * 4 + 3];
      let minX = scanW;
      let minY = scanH;
      let maxX = -1;
      let maxY = -1;
      ...
      for (let y = 0; y < scanH; y += 1) {
        ...
        if (rowMax >= rowMin) rowWidths.push({ y, width: rowMax - rowMin + 1 });
      }
      ...
      const bandTop = minY + (maxY - minY) * 0.28;
      const bandBottom = minY + (maxY - minY) * 0.62;
      const bandRowWidths = rowWidths
        .filter((row) => row.y >= bandTop && row.y <= bandBottom)
        .map((row) => row.width)
        .sort((a, b) => a - b);
      if (bandRowWidths.length) {
        const medianScanW = bandRowWidths[Math.floor(bandRowWidths.length / 2)];
        const coreWidth = Math.max(1, medianScanW * sx);
        const cx = bounds.minX + bounds.width / 2;
        bounds.zoomWidth = Math.min(bounds.width, coreWidth);
        bounds.zoomMinX = cx - bounds.zoomWidth / 2;
        bounds.zoomMaxX = cx + bounds.zoomWidth / 2;
      }

      return bounds;
    }

    /** Bounds slice used for zoom — torso band when wide hats/robes inflate full envelope. */
    _quoteSpeakerZoomBounds(bounds) {
      if (!bounds?.width || !bounds?.height) return bounds;
      if (Number.isFinite(bounds.zoomWidth) && bounds.zoomWidth > 0) {
        return {
          minX: bounds.zoomMinX,
          maxX: bounds.zoomMaxX,
          minY: bounds.minY,
          maxY: bounds.maxY,
          width: bounds.zoomWidth,
    ...
    /**
     * Zoom multiplier from opaque subject bounds — tight headshots stay at 1×;
     * loose full-body PNGs scale up so the person reads closer to prior days.
     */
    _quoteSpeakerSubjectZoomBoost(iw, ih, boxW, bounds) {
      if (!bounds?.width || !bounds?.height) return 1;
      const zoomBounds = this._quoteSpeakerZoomBounds(bounds);
      const widthFill = 0.9;
      const heightFill = 0.78;
      const maxZoom = 2.4;
      const baseScale = boxW / Math.max(1, iw);
      const subW = zoomBounds.width * baseScale;
      const subH = zoomBounds.height * baseScale;
      const frameH = this._quoteSpeakerCutoutFrameHeight(iw, ih, boxW);
      const widthBoost = (boxW * widthFill) / Math.max(1, subW);
      const heightBoost = (frameH * heightFill) / Math.max(1, subH);
      return Math.max(1, Math.min(maxZoom, Math.max(widthBoost, heightBoost)));
    }

    _quoteSpeakerClampCutoutOffsetX(offsetX, boxW, drawW) {
      if (drawW <= boxW) return (boxW - drawW) / 2;
      return Math.min(0, Math.max(boxW - drawW, offsetX));
    }

    /** Width-fill in a portrait-shaped frame; zooms into opaque bounds when present. */
    _quoteSpeakerCutoutFraming(naturalW, naturalH, boxW, boxH, bounds = null) {
      const iw = Math.max(1, naturalW);
      const ih = Math.max(1, naturalH);
      let scale = boxW / iw;
      const zoomBounds = bounds?.height ? this._quoteSpeakerZoomBounds(bounds) : null;
      if (zoomBounds?.height) {
        scale *= this._quoteSpeakerSubjectZoomBoost(iw, ih, boxW, bounds);
      }
      ...
      const subjectCx = zoomBounds.minX + zoomBounds.width / 2;
      offsetX = this._quoteSpeakerClampCutoutOffsetX(
        boxW / 2 - subjectCx * scale,
        boxW,
        drawW
      );
      const padTop = 8;
      const downNudge = 28;
      let offsetY = padTop + downNudge - bounds.minY * scale;
      offsetY = Math.max(padTop, offsetY);
      const layoutH = Number(boxH);
      if (Number.isFinite(layoutH) && layoutH > 0 && layoutH < 6000 && drawH > layoutH) {
        offsetY = Math.min(offsetY, layoutH - drawH);
      }
    ...
    _applyQuoteSpeakerCutoutAnchor(imageWrap, img) {
      ...
      if (!bounds) {
        ...
        imageWrap.dataset.subjectZoomed = '0';
        this._pinQuoteSpeakerSingleLayerOverlays(imageWrap);
        return;
      }
      const { offsetX, offsetY, drawW, drawH, scale } = this._quoteSpeakerCutoutFraming(
        img.naturalWidth,
        img.naturalHeight,
        boxW,
        8192,
      ...
      const subjectBottom = offsetY + bounds.maxY * scale + 12;
      const frameH = Math.max(120, Math.ceil(Math.min(offsetY + drawH + 4, subjectBottom)));
      imageWrap.dataset.subjectZoomed = drawW > boxW + 1 ? '1' : '0';
      imageWrap.style.height = `${frameH}px`;
