  ...
  class SimplifiedQuiltAppV2Boot {
        ...
        try {
          ...
          if (cutoutUrl && this.archiveService?._prepareSpeakerImageUrlForCanvas) {
            void this.archiveService
              ._prepareSpeakerImageUrlForCanvas(cutoutUrl, {
                quote,
    ...
    async syncLiveDailyData(options = {}) {
      ...
      try {
        ...
        if (!quotesPinned) {
          ...
          if (!hasPinnedQuote) {
            if (quiltOnly && quiltResult.ok) {
              this.logger.warn('Quote pin failed during quilt-only sync; continuing with live quilt');
            } else {
              return { ok: false, reason: 'quote_assignment' };
            }
          }
    ...
    async initialize() {
      ...
      try {
        ...
        const nativeLaunch =
          typeof window.odqIsCapacitorNative === 'function' && window.odqIsCapacitorNative();
        const bootstrapped = nativeLaunch ? false : await this._tryBootstrapTodayQuiltFromLocalStorage();
        ...
        const finishLaunchAfterQuilt = async (live) => {
          if (!live.ok) {
            ...
            if (this._hasCachedTodayQuilt()) {
              const nativeStaleGuard =
                typeof window.odqIsCapacitorNative === 'function' && window.odqIsCapacitorNative();
              if (nativeStaleGuard) {
                this.logger.warn('Launch sync failed on native; retrying without stale count:', live.reason);
                this._scheduleLaunchQuiltSyncRetry?.(2000);
                this._showConnectionProblemConnecting('native-launch-retry');
                return;
              }
