  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    async loadDeferredAdminSlice() {
      ...
      const SliceCtor = await this._loadDeferredScript(
        'lib/simplified-quilt-app-admin.js?v=29',
    ...
    async ensureLayoutBComposeReady() {
      ...
      await this._loadDeferredScript('lib/layout-b-compose.js?v=150', 'composeInstagramLayoutBFromQuiltBlob');
    ...
    _kickLaunchQuiltPipeline(options = {}) {
      ...
      this._launchQuiltPipelinePromise = (async () => {
        try {
          ...
          if (quiltScreenActive && quiltAlreadyPainted && this._launchQuiltPipelineRendered) {
            this._agentDebugLog?.('H8', 'lib/simplified-quilt-app-boot.js:_kickLaunchQuiltPipeline', 'skipped active quilt rerender during launch sync', {
              refresh: options.refresh === true,
              childElementCount: Number(this.renderer?.quiltSVG?.childElementCount || 0)
            });
    ...
    setupLayoutBStoryPreviewLongPressShare() {
      ...
      sheet.addEventListener('pointerdown', () => {
        ...
        shareTimer = setTimeout(async () => {
          shareTimer = null;
          try {
            let shareBlob = this._layoutBStoryPreviewShareBlob;
            const layerUrls = this._layoutBStoryPreviewLayerUrls;
            if (!shareBlob && layerUrls?.quiltUrl && layerUrls?.overlayUrl) {
              shareBlob = await this._composeStoredLayoutBStoryPreviewLayers(
                layerUrls.quiltUrl,
                layerUrls.overlayUrl
              );
              this._layoutBStoryPreviewShareBlob = shareBlob;
            }
            if (!shareBlob && this._layoutBStoryPreviewShareUrl) {
              await this._prepareStoredLayoutBStoryPreviewShareBlob?.(
                this._layoutBStoryPreviewShareUrl,
                this._layoutBStoryPreviewContentKey
              );
              shareBlob = this._layoutBStoryPreviewShareBlob;
            }
            if (!shareBlob) return;
    ...
    async toggleReflectionCarouselHeart({ responseId, dateKey, slideEl } = {}) {
      ...
      try {
        ...
        this.syncReflectionCarouselHeart(slideEl, { hearted: wasHearted, heartCount: priorCount });
