...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    _applyQuoteSpeakerCutoutAnchor(imageWrap, img) {
      ...
      if (!bounds) {
        imageWrap.style.height = `${Math.max(120, naturalFrameH + 8)}px`;
      ...
      const frameH = Math.max(120, Math.ceil(offsetY + drawH + 10));
      if (frameH !== draftH) {
    ...
    _syncQuoteSpeakerCutoutLayout(cutout, img) {
      ...
      if (typeof ResizeObserver !== 'undefined' && cutout) {
        if (!this._quoteSpeakerHandCutObserver) {
          this._quoteSpeakerHandCutObserver = new ResizeObserver(() => {
            ...
            if (img?.naturalWidth) this._applyQuoteSpeakerCutoutAnchor(c, img);
      ...
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          if (cutout && !cutout.hidden) {
            const img = cutout.querySelector('.quote-speaker-image');
            if (img) this._syncQuoteSpeakerCutoutLayout(cutout, img);
          }
        });
      });
    }
    ...
    _applyQuoteSpeakerImageLoaded(displayUrl, imageToken, profile, imageWrap, img, neutralSilhouette, colorWash, avatar) {
      ...
      if (imageWrap && img && !imageWrap.hidden) {
        this._syncQuoteSpeakerCutoutLayout(imageWrap, img);
      }
    ...
    async _refreshQuiltMoodSpread(
      ...
      if (!canComposeQuote && (!window.db || !window.firestore)) {
        this._unmountQuiltMoodSpread();
        this._fallbackQuiltQuoteClipping(todayQuote);
        return;
      }
      ...
      try {
        ...
        const storageUrl = dk ? await this.fetchNewspaperClippingUrl(dk) : null;
        if (spreadHost._moodSpreadComposePending !== sig) {
          return;
        }
        if (storageUrl && (await this._probeRemoteImageUrl(storageUrl))) {
          quoteDataUrl = storageUrl;
        } else if (storageUrl) {
          console.warn(
            '[our-daily] Triptych quote Storage PNG missing or failed to load; composing in browser'
          );
        }
        if (!quoteDataUrl && canComposeQuote) {
          ...
          quoteDataUrl = await QNC.composeDataUrl({
            today: {
