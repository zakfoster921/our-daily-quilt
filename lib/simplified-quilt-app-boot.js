...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    async loadDeferredShareSlice() {
      if (this._deferredShareSliceReady) return true;
      const SliceCtor = await this._loadDeferredScript(
        'lib/simplified-quilt-app-share.js?v=45',
        'SimplifiedQuiltAppV2Share'
      );
      if (!this._mergeAppSlicePrototype(SliceCtor)) return false;
      this._deferredShareSliceReady = true;
      return true;
    }

    async _invokeDeferredShareMethod(methodName, ...args) {
      await this.loadDeferredShareSlice();
      const fn = this[methodName];
      if (typeof fn !== 'function') {
        this.logger?.warn?.(`Share method unavailable: ${methodName}`);
        return undefined;
      }
    ...
    scheduleDeferredAppSlices() {
      this._schedulePostLaunchIdleTask(
        'deferred-share-slice',
        () => this.loadDeferredShareSlice(),
        {
          delayMs: 4500,
          timeoutMs: 12000,
          label: 'Deferred share slice'
        }
      );
    ...
    _attachQuiltLiveListenerWhenReady(dateKey, maxWaitMs = 20000) {
      const key = String(dateKey || Utils.getTodayKey?.() || '').trim();
      if (!key) return;
      this.attachQuiltLiveListener?.(key);
      if (window.db && window.firestore) return;
      void LiveDailyDataSync.waitForFirebaseReady(maxWaitMs).then((ready) => {
        if (ready) this.attachQuiltLiveListener?.(key);
      });
    }
    ...
    async syncLiveDailyData(options = {}) {
      ...
      try {
        ...
        const firebaseWaitMs = quiltOnly
          ? options.mode === 'resume'
            ...
            : isNative
              ...
              : 12000
          : 20000;
        const skipFirebaseWaitForNativeRailway = isNative && quiltOnly;
        let firebaseReady = !!(window.db && window.firestore);
        if (!skipFirebaseWaitForNativeRailway) {
          firebaseReady = await LiveDailyDataSync.waitForFirebaseReady(firebaseWaitMs);
          if (!firebaseReady && !(isNative && quiltOnly)) {
            return { ok: false, reason: 'firebase_unavailable' };
          }
        } else if (window.odqPerfMark) {
          window.odqPerfMark('launch-sync-start-without-firebase-wait');
        }
        ...
        this._attachQuiltLiveListenerWhenReady(todayKey, firebaseWaitMs);
    ...
    setupEventListeners() {
      ...
      if (shareBtnCompleted) {
        shareBtnCompleted.addEventListener('click', (e) => {
          void this._invokeDeferredShareMethod('handleShare', e);
        });
      }
    ...
    setupLayoutBStoryPreviewLongPressShare() {
      ...
      sheet.addEventListener('pointerdown', (event) => {
        ...
        shareTimer = window.setTimeout(async () => {
          ...
          try {
            await this._invokeDeferredShareMethod('shareLayoutBStoryPreviewFromLongPress');
          } catch (error) {
