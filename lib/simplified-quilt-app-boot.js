...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    /** PNG width for triptych quote — display width × DPR (avoids upscaling blur on 3× phones). */
    _triptychQuoteComposeWidthPx() {
      const triptychScale = 0.85;
      const sheetDom = 428;
      const dpr =
        typeof window !== 'undefined'
          ? Math.min(Math.max(window.devicePixelRatio || 2, 2), 3)
          : 2;
      let displayPx = Math.round(sheetDom * triptychScale);
      try {
        const stack = document.querySelector('#screen-quilt .quote-card-stack');
        if (stack && typeof getComputedStyle === 'function') {
          const cs = getComputedStyle(stack);
          const gutter = parseFloat(cs.getPropertyValue('--quilt-float-card-gutter')) || 20;
          const max = parseFloat(cs.getPropertyValue('--quilt-float-card-max')) || 460;
          const cssW = Math.min(max * triptychScale, window.innerWidth - 2 * gutter);
          if (Number.isFinite(cssW) && cssW > 100) displayPx = Math.round(cssW);
        }
      } catch (_) {
        /* */
      }
      return Math.max(320, Math.round(displayPx * dpr));
    }
      ...
      try {
        ...
        if (canComposeQuote) {
          ...
          quoteDataUrl = await QNC.composeDataUrl({
            ...
            dateKey: dk,
            paperTextureUrl,
            width: this._triptychQuoteComposeWidthPx(),
            exportScale: 1
          });
        }
        if (!quoteDataUrl && storageUrl && (await this._probeRemoteImageUrl(storageUrl))) {
          quoteDataUrl = storageUrl;
        } else if (!quoteDataUrl && storageUrl) {
          console.warn(
            '[our-daily] Triptych quote Storage PNG missing or failed to load; compose also failed'
          );
        }
        if (spreadHost._moodSpreadComposePending !== sig) {
          return;
