...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    /** Direct Storage cutout: contain the full PNG inside the 90% portrait column. */
    ...
    _applyQuoteSpeakerCutoutAnchor(imageWrap, img) {
      ...
      /** Speaker card: width-fit the full PNG in the 90% column (subject-fill spills past the strip). */
      const subjectBounds = null;
      const layoutBounds = opaqueBounds;
      ...
      if (
        ...
        this._applyQuoteSpeakerPlainCutoutDirectFit(imageWrap, img, boxW);
      }
      if (wrap) {
        requestAnimationFrame(() => {
          const wrapRect = wrap.getBoundingClientRect();
    ...
    /** Speaker card: portrait column is 90% of the identity name strip (slab stays full width). */
    _syncQuoteSpeakerPortraitWidthFromNameStrip(widget) {
      ...
      const stripW = Math.max(identity.offsetWidth || 0, identity.clientWidth || 0);
      if (stripW < 48) return false;
      const target = Math.max(48, Math.round(stripW * this._quoteSpeakerCutoutWidthFrac(widget)));
    ...
    _applyQuoteSpeakerPlainCutoutBacking(cutout, img, sourceUrl = '') {
      ...
      const applyDirectCutoutDisplay = async (sourceImg) => {
        ...
        if (portraitImg.dataset.plainCutoutDirect === '1' && portraitImg.naturalWidth) {
          portraitImg.style.removeProperty('opacity');
          this._applyQuoteSpeakerPlainCutoutDirectFit(cutout, srcImg || portraitImg);
          return;
        }
