...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    /**
     * Pin today's quote before `_liveDailyDataConfirmed`. Uses local assignment cache, then Firestore.
     */
    async _ensureTodayQuotePinnedForLaunch() {
      this.quoteService.primeTodayQuoteFromLocalAssignment();
      if (this.quoteService.hasTodayQuotePinned()) return true;
      const quotes = await this._syncLiveDailyQuotes();
      if (quotes.ok) return true;
    ...
    async syncLiveDailyData(options = {}) {
      ...
      try {
        ...
        const todayKey = Utils.getTodayKey();
        const quiltResult = await this.dataService.loadQuiltFromServer(todayKey);
        if (!quiltResult.ok) {
          return { ok: false, reason: quiltResult.reason || 'quilt' };
        }
        await this.applyQuiltDataFromPayload(quiltResult.data);

        const quotesPinned = await this._ensureTodayQuotePinnedForLaunch();
        if (!quotesPinned) {
          return { ok: false, reason: 'quote_assignment' };
        }
        ...
        if (quiltOnly) {
          ...
          void odqPrefetchSpeakerCutoutTweak(todayKey);
    ...
    async applyQuiltDataFromPayload(data) {
      ...
      if (data.blocks && data.blocks.length > 0) {
        ...
        if (this.quiltEngine.blocks.length === 0) {
          ...
          this.quiltEngine.submissionCount = Math.max(
            Number(data.contributorCount) || 1,
    ...
    async _continueAfterLiveSync() {
      ...
      const rememberViewDateKey = await this._awaitRememberTodayNotificationOpenIntent(900);
    ...
    setupEventListeners() {
      ...
      document.addEventListener('screenChange', (e) => {
        if (e?.detail?.screenId === 'screen-quilt' || e?.detail?.screenId === 'screen-exit-chamber') {
          this.refreshRememberTodayFooterVisibility();
        }
      ...
      if (document.documentElement.dataset.odqSettingsGearRectNav !== '1') {
        ...
        const activeSettingsGearEl = () => {
          ...
          const exit = document.getElementById('screen-exit-chamber');
          if (quilt?.classList.contains('active')) {
            return document.getElementById('settingsGearBtn');
          }
          if (exit?.classList.contains('active')) {
            return (
              document.getElementById('exitChamberSettingsGearBtn') ||
              exit.querySelector?.('.quilt-settings-icon-btn[data-next="screen-settings"]')
            );
          }
          return null;
        };
      ...
      this.ensureExitChamberCtaDisabled();
      this.ensureMilestoneQuiltsDisabled();
      const quiltDailyColorsCtaBtn = document.getElementById('quiltDailyColorsCtaBtn');
      if (quiltDailyColorsCtaBtn) {
        quiltDailyColorsCtaBtn.addEventListener('click', (e) => {
          if (!this.isExitChamberCtaEnabled()) return;
          e.preventDefault();
          e.stopPropagation();
          this.openExitChamberClassic();
        });
      }
      const exitChamberBackBtn = document.getElementById('exitChamberBackBtn');
      if (exitChamberBackBtn) {
        exitChamberBackBtn.addEventListener('click', (e) => {
          e.preventDefault();
          e.stopPropagation();
          this.uiService.showScreen('screen-quilt');
        });
    ...
    getStoredExitChamberFarewellPhraseIndex() {
      try {
        const storedIndex = Number.parseInt(
          localStorage.getItem('ourDailyExitChamberFarewellPhraseIndex') || '0',
          10
        );
    ...
    setNextExitChamberFarewellPhrase() {
      const phrases = Array.isArray(this.exitChamberFarewellPhrases)
        ? this.exitChamberFarewellPhrases.filter(Boolean)
        : [];
      if (!phrases.length) return;

      const phraseIndex = this.exitChamberFarewellPhraseIndex % phrases.length;
      const phrase = phrases[phraseIndex];
      this.exitChamberFarewellPhraseIndex = (phraseIndex + 1) % phrases.length;

      try {
        localStorage.setItem(
          'ourDailyExitChamberFarewellPhraseIndex',
          String(this.exitChamberFarewellPhraseIndex)
        );
      } catch (error) {
        // Rotation is decorative; keep going if storage is unavailable.
      }

      const kicker = document.getElementById('exitChamberKickerBottom');
      const textEl = kicker?.querySelector?.('.exit-chamber-farewell__text');
      if (textEl) textEl.textContent = phrase;
    }

    /** L→R pen-stroke underline under the exit-chamber farewell (dashoffset reset + reveal). */
    scheduleExitChamberKickerUnderlineReveal(kicker, delayMs = 0) {
      if (!kicker) return;
      ...
      const path = kicker.querySelector('.exit-chamber-farewell__rule path');
      if (path) {
      ...
      this._exitChamberKickerRevealTimer = window.setTimeout(() => {
        this._exitChamberKickerRevealTimer = null;
    ...
    setupQuiltUserColorTogetherNoteLongPress() {
      const wrap = document.getElementById('quiltUserColorTogetherNoteWrap');
      const note = wrap?.querySelector('.quilt-user-color-together-note');
      if (!wrap || !note || note.dataset.longPressAttached === '1') return;
      note.dataset.longPressAttached = '1';
      ...
      const HOLD_MS_TOUCH = 480;
      const HOLD_MS_DEFAULT = 650;
      ...
      /** Unlock audio during the touch (iOS needs this before the hold timer fires). */
      ...
      /** Visual + quiet thump sound (iOS) + vibrate when the browser allows it (mostly Android). */
      ...
      const endGlimmerHold = () => {
        clearPressTimer();
        detachReleaseListeners();
        if (!glimmerHeld) return;
        glimmerHeld = false;
        this.setContributorNameGlimmerActive(false);
      };

      const attachReleaseListeners = () => {
        if (releaseOnWindow) return;
        releaseOnWindow = () => endGlimmerHold();
      ...
      note.addEventListener(
        'touchstart',
        (event) => {
          primeHoldThumpFeedback();
          blockNativeTextGesture(event);
        },
        { passive: false }
      );
      note.addEventListener('selectstart', blockNativeTextGesture);
      note.addEventListener('contextmenu', blockNativeTextGesture);
      ...
      note.addEventListener('pointerdown', (event) => {
        ...
        const holdMs =
          event.pointerType === 'touch' ? HOLD_MS_TOUCH : HOLD_MS_DEFAULT;
      ...
      note.addEventListener('pointermove', (event) => {
        ...
        if (Math.hypot(dx, dy) > HOLD_MOVE_CANCEL_PX) clearPressTimer();
      });
      note.addEventListener('pointerup', (event) => {
        if (event.pointerId !== pressPointerId && pressPointerId != null && !glimmerHeld) {
          return;
        }
        if (glimmerHeld) endGlimmerHold();
      ...
      note.addEventListener('keydown', (event) => {
        ...
        if (event.repeat || wrap.hidden || glimmerHeld) return;
        event.preventDefault();
        glimmerHeld = true;
        triggerGentleWobble();
        this.setContributorNameGlimmerActive(true);
      ...
      note.addEventListener('blur', endGlimmerHold);
    }
    ...
    setupQuiltFortuneReveal() {
      ...
      reveal.addEventListener('click', (event) => {
        ...
        const kicker = document.getElementById('exitChamberKickerBottom');
        if (kicker && !kicker.classList.contains('is-revealed')) {
          this.scheduleExitChamberKickerUnderlineReveal(kicker, 1200);
        }
      });
    }
    ...
    static REFLECTION_FIRST_PATCH_AUTHOR = 'Zak';
    ...
    formatReflectionCarouselTextHtml(entry) {
      ...
      return `<span class="quilt-reflection-carousel-body">${bodyHtml}</span><span class="quilt-reflection-carousel-author">— ${authorHtml}</span>`;
    }
    ...
    setupReflectionResponsePrototype() {
      ...
      const idleSubmitLabel = 'add yours';
      ...
      const refreshThemes = () => this.loadReflectionThemesForToday().catch((error) => {
        this.logger?.warn?.('Reflection themes load failed:', error);
      });
      refreshThemes();
      document.addEventListener('firebaseReady', refreshThemes, { once: true });
      setTimeout(refreshThemes, 1200);
      setTimeout(refreshThemes, 3500);
      Utils.syncReflectionPatchStarElement();
      form.addEventListener('submit', async (event) => {
        event.preventDefault();
        // First click while still collapsed: reveal the response field, don't submit yet.
        if (form.classList.contains('is-collapsed')) {
          form.classList.remove('is-collapsed');
          if (submit) submit.setAttribute('aria-expanded', 'true');
    ...
    buildReflectionWallThemes(communityThemes = [], firstResponseOverride = null) {
      const firstRaw =
        firstResponseOverride != null && String(firstResponseOverride).trim()
          ? String(firstResponseOverride)
          : this._firstResponseFromPayload(this.quoteService?.getTodayQuote?.() || {});
      const first = firstRaw
        ? {
            text: this.formatFirstResponsePatchText(firstRaw),
            author: this.constructor.REFLECTION_FIRST_PATCH_AUTHOR
          }
        : null;
    ...
    async fetchTodayFirstResponseFields() {
      ...
      if (!dateKey) return { first_response: '', user_name: '' };
      ...
      return { first_response: '', user_name: '' };
    }
    ...
    async resolveReflectionWallThemes(communityThemes = [], firstResponseContext = null) {
      const ctx =
        firstResponseContext && typeof firstResponseContext === 'object'
          ...
          : firstResponseContext != null && String(firstResponseContext).trim()
            ...
            : {};
      const quotePayload = this.quoteService?.getTodayQuote?.() || {};
      ...
      return this.buildReflectionWallThemes(communityThemes, first);
    }

    /** Pastel wash: brighter, lower chroma so carousel patches read apart from live quilt blocks. */
    washReflectionCarouselPatchRgb(rgbCsv) {
      ...
      const wash = Utils.hsvToHex(
        hsv.h,
        hsv.s < 10 ? 8 : Math.max(12, Math.min(32, hsv.s * 0.35)),
        Math.max(92, Math.min(99, hsv.v + (100 - hsv.v) * 0.68))
      );
      const match = /^#?([0-9a-f]{6})$/i.exec(wash);
      if (!match) return '254, 248, 242';
      const h = match[1];
      const lighten = 0.16;
      const channels = [
    ...
    getReflectionFabricPatchPalette() {
      const isHex = (color) => /^#[0-9A-Fa-f]{6}$/.test(String(color || '').trim());
      ...
      ['#e7d8bc', '#ea9b9a', '#b9a7d8', '#8bd5d8', '#f2c7ba', '#c7b299'].forEach((color) => {
        if (unique.length < 6 && !unique.includes(color.toLowerCase())) unique.push(color.toLowerCase());
      });
      const paletteRgb = unique.map((color) => {
        const match = /^#?([0-9a-f]{6})$/i.exec(color);
        if (!match) return '231, 216, 188';
        const hex = match[1];
    ...
    /** Narrow by default; only approach full width when copy needs it (~200 chars). */
    getReflectionFabricPatchWidth(themeEntry, allThemes = []) {
      if (themeEntry === 'add yours' || themeEntry === 'add yours ?') return '42vw';
      const chars = this.getReflectionFabricPatchContentWeight(themeEntry);
      if (!chars) return '42vw';
      if (chars <= 62) return '50vw';
      if (chars <= 95) return '58vw';
      if (chars <= 125) return '68vw';
      if (chars <= 158) return '78vw';
      return '90vw';
    }
    ...
    reflectionFabricRgbCsvToHex(rgbCsv) {
      const parts = String(rgbCsv || '')
        .split(',')
        .map((value) => parseInt(String(value).trim(), 10));
      if (parts.length < 3 || parts.some((channel) => !Number.isFinite(channel))) return '#e7d8bc';
      return `#${parts
    ...
    getReflectionFabricPatchHueVariant(rgbCsv, hueShiftDeg = 26) {
      ...
      const next = Utils.hsvToHex(
        (hsv.h + hueShiftDeg + 360) % 360,
        Math.max(12, Math.min(32, hsv.s < 10 ? 16 : hsv.s * 0.35)),
        Math.max(92, Math.min(99, hsv.v + (100 - hsv.v) * 0.62))
      );
      const match = /^#?([0-9a-f]{6})$/i.exec(next);
      if (!match) return rgbCsv || '231, 216, 188';
      const h = match[1];
      return `${parseInt(h.slice(0, 2), 16)}, ${parseInt(h.slice(2, 4), 16)}, ${parseInt(h.slice(4, 6), 16)}`;
    }

    ensureDistinctReflectionPatchRgb(referenceRgb, preferredIndex = 0) {
      const fallback = '231, 216, 188';
    ...
    getReflectionFabricPatchStyle(index, themeText, allThemes, rgbOverride) {
      ...
      const rawRgb = rgbOverride || palette[index % palette.length] || '231, 216, 188';
    ...
    buildReflectionInvitePatchSlide(index, allThemesForHeight = [], rgbOverride, edgeFray = {}) {
      ...
      return `
            <article class="quilt-reflection-carousel-slide quilt-reflection-carousel-slide--invite${lightClass}" data-reflection-invite-slide data-reflection-slide="${index}" data-reflection-hand-cut-seed="${dateKey}:reflection-invite:${index}" style="${style}">
              ${this.getReflectionCarouselFrayMarkup(edgeFray)}
              <span class="quilt-reflection-carousel-seam quilt-reflection-carousel-seam--join" aria-hidden="true"></span>
              <button type="button" class="quilt-reflection-carousel-invite-btn" aria-label="Add your reflection?">add yours ?</button>
            </article>
          `;
    }

    getReflectionDecorPatchPalette() {
      return [
        '252, 226, 232',
        '230, 236, 250',
        '246, 234, 246',
    ...
    getReflectionCarouselDecorCount(afterIndex, responseCount, dateKey, beforeInvite = false) {
      const key = String(dateKey || Utils.getTodayKey() || 'nodate').trim() || 'nodate';
      const slot = beforeInvite ? 'pre-invite' : 'gap';
      const h =
        Utils.hashStringToUint(`${key}:odq-reflection-decor:${slot}:${afterIndex}:${responseCount}`) || 1;
      const pick = h % 100;
      if (responseCount === 0 && beforeInvite) {
        if (pick < 28) return 0;
        if (pick < 72) return 1;
        return 2;
      }
    ...
    getReflectionDecorPatchDimensions(slotKey) {
      const h = Utils.hashStringToUint(String(slotKey || 'decor')) || 1;
      const widthVw = 12 + (h % 11);
      const minRem = 11;
      const maxRem = 13.25;
      const heightRem = minRem + ((h >>> 8) % 10) * ((maxRem - minRem) / 9);
      return {
        width: `${widthVw}vw`,
    ...
    pickReflectionDecorPatchRgb(leftRgb, rightRgb, slotKey) {
      const palette = this.getReflectionDecorPatchPalette();
      const h = Utils.hashStringToUint(String(slotKey || 'decor')) || 1;
      for (let offset = 0; offset < palette.length; offset++) {
        const candidate = palette[(h + offset) % palette.length];
        const distinctLeft = !leftRgb || this.isReflectionFabricRgbDistinctFrom(candidate, leftRgb, 40);
        const distinctRight = !rightRgb || this.isReflectionFabricRgbDistinctFrom(candidate, rightRgb, 40);
        if (distinctLeft && distinctRight) return candidate;
      }
      const ref = leftRgb || rightRgb || palette[0];
      return this.getReflectionFabricPatchHueVariant(ref, 48 + (h % 36));
    }

    getReflectionDecorPatchStyle(slideIndex, slotKey, rgbOverride, dims) {
      const rawRgb = rgbOverride || this.pickReflectionDecorPatchRgb(null, null, slotKey);
      const rgb = this.washReflectionCarouselPatchRgb(rawRgb);
      const weaveStrength = this.getReflectionFabricPatchWeaveStrength(rgb);
      const h = Utils.hashStringToUint(String(slotKey || 'decor-cut')) || 1;
      const tilt = `${((h % 7) - 3) * 0.35}deg`;
      return [
        `--reflection-fabric-rgb: ${rgb}`,
        `--reflection-fabric-text: ${this.getReflectionFabricPatchTextColor(rgb)}`,
        `--reflection-fabric-weave: ${weaveStrength}`,
        `--reflection-patch-w: ${dims.width}`,
        `--reflection-patch-h: ${dims.height}`,
        `--reflection-fabric-tilt: ${tilt}`,
        `--reflection-fabric-y: 0rem`,
        `--reflection-slide-z: ${slideIndex + 1}`
      ].join('; ');
    }

    buildReflectionDecorPatchSlide(slideIndex, slotKey, leftRgb, rightRgb, edgeFray = {}) {
      const rawRgb = this.pickReflectionDecorPatchRgb(leftRgb, rightRgb, slotKey);
      const washedRgb = this.washReflectionCarouselPatchRgb(rawRgb);
      const dims = this.getReflectionDecorPatchDimensions(slotKey);
      const lightClass =
        this.getReflectionFabricPatchWeaveStrength(washedRgb) < 1 ? ' quilt-reflection-carousel-slide--light' : '';
      const style = this.getReflectionDecorPatchStyle(slideIndex, slotKey, rawRgb, dims);
      return `
            <article class="quilt-reflection-carousel-slide quilt-reflection-carousel-slide--decor${lightClass}" data-reflection-decor-slide data-reflection-slide="${slideIndex}" data-reflection-hand-cut-seed="${slotKey}" aria-hidden="true" style="${style}">
              ${this.getReflectionCarouselFrayMarkup(edgeFray)}
              <span class="quilt-reflection-carousel-seam quilt-reflection-carousel-seam--join" aria-hidden="true"></span>
            </article>
          `;
    ...
    buildReflectionThemePatchSlide(slideIndex, theme, allThemes, rgb, edgeFray = {}) {
      ...
      const dateKey = String(Utils.getTodayKey() || 'nodate').trim() || 'nodate';
      return `
            <article class="quilt-reflection-carousel-slide${lightClass}" data-reflection-theme-slide data-reflection-slide="${slideIndex}" data-reflection-hand-cut-seed="${dateKey}:reflection-patch:${slideIndex}" style="${this.getReflectionFabricPatchStyle(slideIndex, theme, allThemes, rgb)}">
              ${this.getReflectionCarouselFrayMarkup(edgeFray)}
              <span class="quilt-reflection-carousel-seam quilt-reflection-carousel-seam--join" aria-hidden="true"></span>
              <p class="quilt-reflection-carousel-text">${this.formatReflectionCarouselTextHtml(theme)}</p>
            </article>
          `;
    }

    appendReflectionDecorSlides(slideParts, slideIndex, count, slotPrefix, leftRgb, rightRgb, dateKey) {
      let nextIndex = slideIndex;
      for (let d = 0; d < count; d++) {
        const slotKey = `${dateKey}:${slotPrefix}:${d}`;
        slideParts.push(
          this.buildReflectionDecorPatchSlide(nextIndex, slotKey, leftRgb, rightRgb, {})
        );
        nextIndex += 1;
      }
      return nextIndex;
    ...
    getReflectionCarouselMarkup(themeTexts) {
      ...
      const pushDecorRun = (count, slotPrefix, leftRgb, rightRgb) => {
        slideIndex = this.appendReflectionDecorSlides(
          slideParts,
          slideIndex,
          count,
          slotPrefix,
      ...
      // Always open with a columnar decorative patch on the left edge.
      pushDecorRun(
        1,
        'leading',
        null,
        responseCount > 0 ? themeRgbs[0] : inviteRgb
      );

      if (!responseCount) {
        pushDecorRun(
          this.getReflectionCarouselDecorCount(0, 0, dateKey, true),
          'pre-invite-empty',
          null,
          inviteRgb
        );
        ...
        for (let i = 0; i < responseCount; i++) {
          slideParts.push(
            this.buildReflectionThemePatchSlide(
              slideIndex,
              ideas[i],
              ideas,
              themeRgbs[i],
          ...
          if (i < responseCount - 1) {
            pushDecorRun(
              this.getReflectionCarouselDecorCount(i, responseCount, dateKey, false),
              `gap-${i}`,
              themeRgbs[i],
              themeRgbs[i + 1]
            );
            ...
            pushDecorRun(
              this.getReflectionCarouselDecorCount(i, responseCount, dateKey, true),
              'pre-invite',
              themeRgbs[i],
              inviteRgb
            );
    ...
    syncReflectionCarouselEdgeFray(slides) {
      ...
      insertSideFray(list[list.length - 1], 'right');
    }

    syncReflectionCarouselSeams(slides) {
      const list = Array.isArray(slides) ? slides : [];
      list.forEach((slide) => {
        const seam = slide.querySelector('.quilt-reflection-carousel-seam--join');
        if (!seam) return;
        seam.classList.remove('is-visible', 'is-left', 'is-right');
        seam.style.removeProperty('--reflection-seam-top');
        seam.style.removeProperty('--reflection-seam-span');
        slide.classList.remove('has-join-shadow-left');
      });
      for (let i = 1; i < list.length; i++) {
        ...
        const seam = next.querySelector('.quilt-reflection-carousel-seam--join');
        if (!seam) continue;
        seam.classList.add('is-visible', 'is-left');
        seam.style.setProperty('--reflection-seam-top', `${(tallerH - shorterH) / 2}px`);
        seam.style.setProperty('--reflection-seam-span', `${shorterH}px`);
        next.classList.add('has-join-shadow-left');
      }
    }
    ...
    syncReflectionCarouselHandCuts(slides, dateKey = '') {
      ...
      const cfg = this._reflectionPatchHandCutCfg();
      list.forEach((slide, index) => {
        ...
        if (w < 40 || h < 32) return;
        const clip = QNC.buildHandCutCssClipPath(w, h, seed, cfg);
    ...
    initReflectionCarouselLoop(notes) {
      ...
      const layoutCarousel = () => {
        ...
        this.syncReflectionCarouselSeams(slides);
        ...
        if (this._reflectionCarouselHandCutObservedEl !== carousel) {
          ...
          if (!this._reflectionCarouselHandCutObserver) {
            this._reflectionCarouselHandCutObserver = new ResizeObserver(() => {
              ...
              this.syncReflectionCarouselSeams(list);
      ...
      const getScrollHintDistance = () => {
        ...
        return Math.min(68, Math.max(36, viewport.clientWidth * 0.105));
      };
      const playScrollHintBounce = () => {
        ...
        const sampleShift = (progress) => {
          const frames = bounceKeyframes;
          for (let i = 0; i < frames.length - 1; i += 1) {
            const from = frames[i];
            const to = frames[i + 1];
            if (progress > to.t) continue;
            const span = Math.max(0.0001, to.t - from.t);
            const local = (progress - from.t) / span;
            const eased = easeGentle(local);
            return peek * (from.v + (to.v - from.v) * eased);
          }
          return 0;
        };
        const step = (now) => {
      ...
      viewport.addEventListener('touchstart', markUserScrolled, { passive: true });
    ...
    async loadReflectionThemesForToday() {
      ...
      const readThemes = async (key) => {
        if (!key) return null;
        let data = null;
        const firestoreReady = await this.waitForReflectionFirestore(1200);
        if (firestoreReady) {
          try {
            const docRef = window.firestore.doc(window.db, 'reflectionThemes', key);
            const readDoc = window.firestore.getDocFromServer || window.firestore.getDoc;
            const snap = await readDoc(docRef);
            if (snap.exists()) {
              this._reflectionThemesNotFoundKeys?.delete(key);
              data = snap.data() || {};
            } else {
              data = null;
            ...
            this.logger?.warn?.(`Reflection themes SDK read failed for ${key}:`, error);
          }
        }
        if (!data && !this._reflectionThemesNotFoundKeys.has(key)) {
          data = await this.fetchReflectionThemesViaRest(key);
        }
        if (!data && !this._reflectionThemesNotFoundKeys.has(key)) {
          data = await this.fetchReflectionThemesViaBackend(key);
        }
        if (!data) return null;
      ...
      const todayThemes = await readThemes(dateKey);
      const communityThemes = todayThemes?.themes || [];
      const wallThemes = await this.resolveReflectionWallThemes(communityThemes, {
        first_response: todayThemes?.first_response || '',
        user_name: todayThemes?.user_name || ''
      });
    ...
    _quoteSpeakerImageContainRect(naturalW, naturalH, boxW, boxH) {
    ...
    _quoteSpeakerSilhouetteRightEdge(img, boxW, boxH) {
      ...
      if (!ctx) return null;
      const { offsetX, offsetY, drawW, drawH } = this._quoteSpeakerImageContainRect(
        img.naturalWidth,
        img.naturalHeight,
        boxW,
        boxH
    ...
    _quoteSpeakerAttributionNameCardClearancePx(wrap) {
      ...
      const gap = 8;
      const maxBottomLocalY = slabRect.top - wrapRect.top - gap;
      if (!Number.isFinite(maxBottomLocalY)) return 0;
      return Math.max(0, wrapRect.height - maxBottomLocalY);
    }
    ...
    _quoteSpeakerAttributionBottomEdge(edge, img, boxW, boxH, text, fontSize, wrap) {
      ...
      const imageRight = offsetX + drawW;
      const lineHeight = fontSize * 1.4;
      const colWidth = fontSize * 1.32;
      const maxCols = 3;
      ...
      const padPx = Math.max(1, fontSize * 0.15);
      const cornerX = imageRight - padPx;
      const maxBottomY = Math.max(
        edge[0].y + neededHeight,
        Math.min(boxH - 1, imageBottom - padPx - clearancePx)
      );
      ...
      segment = segment.map((p, i, arr) => {
        const t = i / Math.max(1, arr.length - 1);
        const x = p.x + (Math.max(p.x, cornerX - colWidth * 0.15) - p.x) * (0.2 + 0.8 * t);
        return { x, y: Math.min(p.y, cornerY) };
      });
      const last = segment[segment.length - 1];
      if (!last || last.y < cornerY - 1) {
        const anchorX = last ? last.x : cornerX;
        segment.push({ x: Math.max(anchorX, cornerX - colWidth * 0.15), y: cornerY });
      } else {
        segment[segment.length - 1] = {
          x: Math.max(last.x, cornerX - colWidth * 0.15),
          y: cornerY
        };
      }
      return { segment, columns, colWidth, padPx: padPx * 0.5, clearancePx };
    }
    ...
    _layoutQuoteSpeakerAttributionAlongSilhouette(attributionEl, imgEl, cutoutEl) {
      ...
      const boxW = Math.max(1, Math.round(wrap.clientWidth));
      const boxH = Math.max(1, Math.round(wrap.clientHeight));
      ...
      const paint = () => {
        ...
        const edge = this._quoteSpeakerSilhouetteRightEdge(img, boxW, boxH);
        if (!edge?.length) {
          this._setQuoteSpeakerAttributionFallback(attributionEl, text, wrap);
          return;
        }
        const fontSize = this._quoteSpeakerAttributionFontSizePx();
        const layout = this._quoteSpeakerAttributionBottomEdge(edge, img, boxW, boxH, text, fontSize, wrap);
        const { segment, columns, colWidth, padPx, clearancePx } = layout || {};
        if (!segment?.length || !columns?.length) {
          this._setQuoteSpeakerAttributionFallback(attributionEl, text, wrap);
          return;
        }
        ...
        columns.forEach((colText, colIdx) => {
          const offsetX = padPx - colIdx * colWidth;
          const colPoints = segment.map((p) => ({ x: p.x + offsetX, y: p.y }));
    ...
    _ensureQuoteSpeakerAttributionFit(widget) {
      ...
      const identitySlab = document.getElementById('quoteSpeakerIdentitySlab');
      const targets = [wrap, identitySlab].filter(Boolean);
      targets.forEach((el) => {
    ...
    fitQuoteSpeakerNameToOneLine(nameEl, opts = {}) {
      ...
      const heroSizeRaw = quiltScreen
        ? getComputedStyle(quiltScreen).getPropertyValue('--quilt-hero-display-size').trim()
        : '5.75rem';
      const heroRem = parseFloat(heroSizeRaw) || 5.75;
      const maxPx = heroRem * rootRem;
    ...
    _quoteSpeakerHandCutCfg() {
      return {
        ...
        handCutMacroDomPx: 10,
        handCutBiteMaxDomPx: 20,
        handCutSecondaryBiteDomPx: 12,
    ...
    _applyQuoteSpeakerWashColors(widget) {
      ...
      const washColor = String(
        this.getMostPopularQuiltColor()?.color || CONFIG.APP.defaultColor || '#ea9b9a'
      ).trim();
    ...
    _applyQuoteSpeakerImageLoaded(displayUrl, imageToken, profile, imageWrap, img, neutralSilhouette, colorWash, avatar) {
      const speakerImageCssUrl = `url("${String(displayUrl).replace(/"/g, '%22')}")`;
      if (imageWrap) {
        imageWrap.hidden = false;
        imageWrap.classList.add('quote-speaker-cutout--loaded');
        imageWrap.style.setProperty('--speaker-image-url', speakerImageCssUrl);
      }
      if (neutralSilhouette) neutralSilhouette.hidden = false;
      if (colorWash) colorWash.hidden = false;
      if (img) {
        img.src = displayUrl;
        img.hidden = false;
        img.classList.add('is-loaded');
      ...
      const widget = document.getElementById('quoteSpeakerWidget');
      if (widget && imageWrap && !imageWrap.hidden) {
        this._applyQuoteSpeakerHandCut(
          imageWrap,
          widget.dataset.handCutDateKey || '',
          'speaker-cutout'
    ...
    _showQuoteSpeakerAvatarFallback(imageWrap, img, neutralSilhouette, colorWash, avatar) {
      if (imageWrap) {
        imageWrap.hidden = false;
        imageWrap.classList.remove('quote-speaker-cutout--loaded');
    ...
    _refreshQuoteSpeakerWidgetNow(quote = null) {
      const widget = document.getElementById('quoteSpeakerWidget');
      if (!widget) return;
      const resolvedQuote = quote || this.quoteService?.getTodayQuote?.() || null;
      ...
      this._applyQuoteSpeakerWashColors(widget);
      if (nameEl) {
      ...
      if (imageWrap) {
        imageWrap.hidden = !showSpeakerImage;
        imageWrap.classList.remove('quote-speaker-cutout--loaded');
        imageWrap.style.removeProperty('--speaker-image-url');
        imageWrap.style.removeProperty('clip-path');
        imageWrap.style.removeProperty('-webkit-clip-path');
      ...
      if (img) {
        img.alt = showSpeakerImage ? `Portrait of ${profile.name}` : '';
        img.classList.remove('is-loaded');
        img.hidden = true;
        img.removeAttribute('src');
      }
      if (showSpeakerImage && imageWrap) {
        const applyLoaded = (displayUrl) => {
        ...
        const pickUrl = async () => {
          ...
          const portraitForResolve =
            profile.portraitUrl ||
            (arch?._layoutBSpeakerPortraitUrl ? arch._layoutBSpeakerPortraitUrl(resolvedQuote) : '') ||
        ...
        const prepareDisplayUrl = async (displayUrl) => {
          const url = String(displayUrl || '').trim();
          if (!url) return '';
          if (!prepare || !/^https?:\/\//i.test(url)) return url;
          const fallback =
            profile.portraitUrl && url !== profile.portraitUrl ? profile.portraitUrl : '';
          const prepared = await prepare(url, { fallbackUrl: fallback });
          return prepared || url;
        };
        pickUrl()
          .then(prepareDisplayUrl)
          .then(applyLoaded)
          .catch(() => applyLoaded(''));
      }
    ...
    _quiltMoodSpreadEnabled() {
      return (
        typeof globalThis.QuiltMoodTriptychWidget?.mount === 'function' &&
    ...
    _quiltMoodTerminalQuotePayload(quote, dateKey = null) {
      ...
      const dateKeyOut = String(
        dateKey || this.quoteService?.getQuoteCalendarKeyNow?.() || q.dateKey || ''
      ).trim();
      ...
      if (q.keyword) payload.keyword = String(q.keyword).trim();
      return payload;
    }
    ...
    _onQuiltMoodTerminalPicked(host, mood, line, meta = {}) {
      ...
      if (spreadHost?._moodTriptychWidget && TriptychW?.moodToPanel) {
        spreadHost._moodTriptychWidget.setMood(mood, { instant: false });
      }
    ...
    _scheduleQuiltMoodFieldsHydrate(quote, widget) {
      ...
      void qs
        .hydrateMoodFieldsForCalendarKey(qs.getQuoteCalendarKeyNow(), quote)
        .finally(() => {
    ...
    async _refreshQuiltMoodSpread(
      ...
      if (!TriptychW?.mount || !QNC?.composeDataUrl || !this._quiltMoodSpreadEnabled()) {
      ...
      const sig = `${g}\u0001${r}`;
      const exportRev = String(QNC?.CLIPPING_EXPORT_REV || '').trim();
      if (
        spreadHost.dataset.contentSig === sig &&
        spreadHost._moodTriptychWidget &&
        spreadHost.dataset.moodExportRev === exportRev
      ) {
        if (initialMood === 'good' || initialMood === 'rough') {
          spreadHost._moodTriptychWidget.setMood(initialMood, { instant });
        }
        spreadHost._moodTriptychWidget.remeasure?.();
      ...
      const dk = String(quotePayload.dateKey || '').trim();
      try {
        const quoteDataUrl = await QNC.composeDataUrl({
          today: {
        ...
        if (!quoteDataUrl) {
          console.warn('[our-daily] Triptych quote compose returned empty');
          this._unmountQuiltMoodSpread();
