...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    /**
     * Top-of-head seat in natural image pixels.
     * Y = highest subject ink in a center band (ignores white paper / side flyaways).
     * X = midpoint of the widest temple/forehead row under the crown — hair spikes and
     * 3/4 silhouettes bias a scalp-only midpoint left; the widest upper-head row tracks
     * the skull center that the hat should sit on.
     */
    _quoteSpeakerMeasureCrownTop(img, bounds = null) {
      ...
      // Skip the hair-tip slab; seek the widest row through forehead/temples.
      const yFore0 = Math.min(scanH - 1, minY + Math.max(2, Math.round(boundH * 0.08)));
      const yFore1 = Math.min(scanH - 1, minY + Math.max(yFore0 - minY + 2, Math.round(boundH * 0.28)));
      let bestWidth = -1;
    ...
    /**
     * Seat the birthday hat on the measured crown (not the frame top). Uses cutout draw
     * offsets so plain-cutout bottom-pinning and letterboxing stay correct.
     */
    _positionQuoteSpeakerBirthdayHat(cutout, img) {
      ...
      const bounds =
        ...
        this._quoteSpeakerMeasureSubjectBounds(img) ||
        this._quoteSpeakerMeasureOpaqueBounds(img);
      const crown = this._quoteSpeakerMeasureCrownTop(img, bounds);
      if (!crown) {
        clear();
        return;
      }
      ...
      const drawW = readPx('--speaker-cutout-bg-w', cutout.clientWidth || 0);
      const drawH = readPx('--speaker-cutout-bg-h', cutout.clientHeight || 0);
      const frameW = Math.max(1, cutout.clientWidth || drawW);
      const frameH = Math.max(1, cutout.clientHeight || drawH);
      if (drawW < 1 || drawH < 1) {
        clear();
        return;
      }
      ...
      // Dip the brim into the scalp so it reads as "on" the head, not hovering.
      const dip = Math.max(8, Math.min(28, frameH * 0.07));
      const brimY = crownY + dip;
      const bottomPct = ((frameH - brimY) / frameH) * 100;
      const leftPct = (crownX / frameW) * 100;
      cutout.style.setProperty(
        '--speaker-hat-bottom',
        `${Math.max(58, Math.min(92, bottomPct)).toFixed(2)}%`
      );
      cutout.style.setProperty(
        '--speaker-hat-left',
        `${Math.max(38, Math.min(68, leftPct)).toFixed(2)}%`
      );
    }
