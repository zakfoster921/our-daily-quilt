...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    _quoteSpeakerMeasureCrownTop(img, bounds = null) {
      ...
      const yFore0 = Math.min(scanH - 1, minY + Math.max(2, Math.round(boundH * 0.1)));
      const yFore1 = Math.min(scanH - 1, minY + Math.max(yFore0 - minY + 2, Math.round(boundH * 0.26)));
      let bestWidth = -1;
      ...
      for (let y = yFore0; y <= yFore1; y += 1) {
        ...
        const width = rowMax - rowMin;
        if (width > bestWidth) {
          bestWidth = width;
          bestMidX = (rowMin + rowMax) / 2;
        }
      }
    ...
    _quoteSpeakerBirthdayHatSeatFromCrown(cutout, measureImg, crown) {
      ...
      const seatFromBox = (originX, originY, boxW, boxH) => {
        ...
        const hatH = hatW * (1285 / 744);
        // Into the hair, then compensate for curved brim so the wide sit-line lands on the scalp.
        const dip = Math.max(18, Math.min(foreheadW * 0.2, boxH * 0.085));
