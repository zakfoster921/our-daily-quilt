...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    /** Dev/preview: ?screen=zak|mission|welcome|bridge|first-name|portal|quote|color|leaderboard|qnlb=submissions|birthdayPreview=tomorrow|firstTime=1|adminMenu=1|admin=1|screen=admin */
    _firstTimePreviewRequestedFromQuery() {
      try {
        const raw = String(new URLSearchParams(window.location.search || '').get('firstTime') || '')
          .trim()
          .toLowerCase();
    ...
    _applyFirstTimePreviewFromQuery() {
      ...
      try {
        ...
        localStorage.setItem(
          typeof Utils !== 'undefined' && Utils.FIRST_NAME_SKIPPED_KEY
            ? Utils.FIRST_NAME_SKIPPED_KEY
            : 'ourDailyFirstNameSkipped',
          '1'
        );
        localStorage.setItem(
          typeof Utils !== 'undefined' && Utils.USER_FIRST_NAME_KEY
            ? Utils.USER_FIRST_NAME_KEY
            : 'ourDailyUserFirstName',
          'Preview'
        );
        ...
        localStorage.setItem('ourDailyLastVisitDate', Utils.getTodayKey());
      } catch (_) {
        /* ignore */
      }
    ...
    _previewQuoteColorPickerRequestedFromQuery() {
      try {
        ...
        if (['color', 'color-picker', 'colorpicker', 'picker'].includes(raw)) return true;
