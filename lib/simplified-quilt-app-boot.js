  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    async loadDeferredAdminSlice() {
      ...
      const SliceCtor = await this._loadDeferredScript(
        'lib/simplified-quilt-app-admin.js?v=36',
    ...
    async ensureLayoutBComposeReady() {
      ...
      await this._loadDeferredScript('lib/layout-b-compose.js?v=165', 'composeInstagramLayoutBFromQuiltBlob');
    ...
    async _drawIgReflectionClippingPaperCanvas(w, h, slide, dateKey, scale) {
      ...
      const paperHex = this.reflectionFabricRgbCsvToHex(rgbCsv);
    ...
    async _drawIgReflectionQuestionPaperSheetCanvas(w, h, root, scale) {
      const canvas = document.createElement('canvas');
      canvas.width = Math.max(1, Math.ceil(w * scale));
      canvas.height = Math.max(1, Math.ceil(h * scale));
      const ctx = canvas.getContext('2d');
      if (!ctx) return null;
      const img = root.querySelector('.quilt-reflection-question-paper__sheet img');
    ...
    async _captureIgReflectionQuestionPaperBodyText(root, html2canvas, scale) {
      const body = root.querySelector('.quilt-reflection-question-paper-body');
      if (!body) return null;
      const rootRect = root.getBoundingClientRect();
      const bodyRect = body.getBoundingClientRect();
      const bw = Math.max(1, Math.ceil(bodyRect.width));
      const bh = Math.max(1, Math.ceil(bodyRect.height));
      const bx = Math.max(0, bodyRect.left - rootRect.left);
      const by = Math.max(0, bodyRect.top - rootRect.top);

      const box = document.createElement('div');
      box.className = 'ig-reflection-question-body-capture';
      box.style.cssText = [
        'position:relative',
        `width:${bw}px`,
        `height:${bh}px`,
      ...
      box.innerHTML = body.outerHTML;
      const bodyClone = box.firstElementChild;
      if (bodyClone) {
        bodyClone.style.position = 'relative';
        bodyClone.style.inset = 'auto';
        bodyClone.style.top = '0';
        bodyClone.style.left = '0';
        bodyClone.style.right = 'auto';
        bodyClone.style.bottom = 'auto';
        bodyClone.style.width = '100%';
        bodyClone.style.height = '100%';
        bodyClone.style.overflow = 'hidden';
        bodyClone.style.boxSizing = 'border-box';
        const promptClone = bodyClone.querySelector('.quilt-reflection-scrap-text');
        const promptSrc = body.querySelector('.quilt-reflection-scrap-text');
        if (promptClone && promptSrc) {
          promptClone.style.cssText = promptSrc.style.cssText;
        }
      }
      ...
      try {
        const bodyCanvas = await html2canvas(box, {
        ...
        return { canvas: bodyCanvas, x: bx, y: by, w: bw, h: bh };
      } finally {
        box.remove();
      }
    }

    _mergeIgReflectionQuestionPaperCaptureLayers(sheetCanvas, bodyCapture, w, h, scale) {
      if (!sheetCanvas && !bodyCapture?.canvas) return null;
      const merged = document.createElement('canvas');
      merged.width = Math.max(1, Math.ceil(w * scale));
      merged.height = Math.max(1, Math.ceil(h * scale));
      const ctx = merged.getContext('2d');
      if (!ctx) return sheetCanvas || bodyCapture?.canvas || null;
      if (sheetCanvas) ctx.drawImage(sheetCanvas, 0, 0, merged.width, merged.height);
      if (bodyCapture?.canvas) {
        ctx.drawImage(
          bodyCapture.canvas,
          bodyCapture.x * scale,
          bodyCapture.y * scale,
    ...
    /** IG slide 2 export — response patches only (no invite), stable dateKey for hand-cut seeds. */
    buildIgReflectionCarouselMarkupForExport(themeEntries, dateKey) {
    ...
    /** IG slide 2 raster width — ~panel width so type stays readable at 1080px. */
    ...
    _igReflectionExportPromptQuestionPx() {
      return 50;
    }

    _igReflectionExportPromptQuestionMinPx() {
      return 34;
    }
    ...
    /** Shrink IG prompt type until it fits inside the notebook safe area. */
    _syncIgReflectionQuestionPaperExportType(scope) {
      const root = scope?.querySelector?.('.quilt-reflection-question-paper') ? scope : null;
      const paper =
        scope?.classList?.contains?.('quilt-reflection-question-paper')
          ? scope
          : root?.querySelector('.quilt-reflection-question-paper') ||
            scope?.querySelector?.('.quilt-reflection-question-paper');
      if (!paper) return;
      const body = paper.querySelector('.quilt-reflection-question-paper-body');
      const promptEl = paper.querySelector('.quilt-reflection-scrap-text');
      if (!body || !promptEl) return;

      const maxPx = this._igReflectionExportPromptQuestionPx();
      const minPx = this._igReflectionExportPromptQuestionMinPx();
      const lineHeight = 1.12;
      const html2canvasSlackPx = 10;

      promptEl.style.setProperty('max-width', '100%', 'important');
      promptEl.style.setProperty('text-align', 'center', 'important');
      promptEl.style.setProperty('margin', '0', 'important');
      promptEl.style.setProperty('padding', '0', 'important');
      ...
      const fitsBodyBox = () => {
        ...
        void promptEl.offsetHeight;
        const maxH = Math.max(0, body.clientHeight - html2canvasSlackPx);
        const maxW = Math.max(0, body.clientWidth - 4);
        if (maxH < 12 || maxW < 12) return false;
        return promptEl.scrollHeight <= maxH && promptEl.scrollWidth <= maxW;
      };

      let bestPx = minPx;
      for (let px = maxPx; px >= minPx; px -= 1) {
        promptEl.style.setProperty('font-size', `${px}px`, 'important');
        promptEl.style.setProperty('line-height', String(lineHeight), 'important');
        if (fitsBodyBox()) {
          bestPx = px;
          break;
        }
      }
      promptEl.style.setProperty('font-size', `${bestPx}px`, 'important');
      promptEl.style.setProperty('line-height', String(lineHeight), 'important');
      body.classList.toggle('is-long-prompt', bestPx <= maxPx - 4);
    }

    _ensureIgReflectionExportStyles() {
      const exportW = this._igReflectionExportViewportWidth();
      const patchW = exportW - 56;
      const patchBodyPx = this._igReflectionExportPatchBodyPx();
      const promptPx = this._igReflectionExportPromptQuestionPx();
      ...
      style.textContent = `
        ...
        #ig-reflection-capture-clone .quilt-reflection-carousel-footer {
        ...
        #screen-quilt.is-ig-export {
          ...
          --quilt-reflection-prompt-question-size: ${promptPx}px;
          --quilt-reflection-prompt-question-size-scale: 1;
          --reflection-clipping-card-max: ${patchW}px;
          --reflection-patch-w: ${patchW}px;
          --reflection-clipping-pad-y: calc(12px + var(--reflection-clipping-edge-y));
          --reflection-clipping-pad-x-outer: 34px;
          --reflection-clipping-edge-y: 12px;
          --reflection-question-card-width: min(100%, 520px);
          --reflection-card-inset-top: 21%;
          --reflection-card-inset-bottom: 32%;
          --reflection-card-inset-x: 8%;
        ...
        #screen-quilt.is-ig-export .quilt-reflection-carousel-slide--clipping {
          width: ${patchW}px !important;
          max-width: ${patchW}px !important;
          flex: 0 0 ${patchW}px !important;
        }
        ...
        #screen-quilt.is-ig-export .quilt-reflection-question-paper .quilt-reflection-scrap-text,
        #screen-quilt.is-ig-export .quilt-reflection-prompt-question {
          line-height: 1.12 !important;
        }
        #screen-quilt.is-ig-export .quilt-reflection-question-paper-body {
          overflow: hidden !important;
          justify-content: center !important;
        }
        #screen-quilt.is-ig-export .quilt-reflection-question-paper {
          overflow: hidden !important;
        ...
        #screen-quilt.is-ig-export .quilt-reflection-carousel-footer,
        ...
        #screen-quilt.is-ig-export .quilt-reflection-carousel-track {
          ...
          gap: clamp(0.2rem, 0.75vw, 0.38rem) !important;
          transform: none !important;
        }
        ...
        #screen-quilt.is-ig-export .quilt-reflection-question-paper {
          ...
          margin-bottom: clamp(0.12rem, 0.55vw, 0.32rem) !important;
        }
        #screen-quilt.is-ig-export .quilt-reflection-scrap-widget {
    ...
    _ensureIgReflectionCaptureStyles() {
      ...
      css += `
        ${ROOT} {
          ...
          --quilt-reflection-prompt-question-size: ${promptPx}px;
          --quilt-reflection-prompt-question-size-scale: 1;
          --reflection-clipping-card-max: ${exportW - 56}px;
          --reflection-patch-w: ${exportW - 56}px;
        }
        ${ROOT} .ig-reflection-question-body-capture .quilt-reflection-question-paper-body {
          overflow: hidden !important;
          justify-content: center !important;
        }
        ${ROOT} .ig-reflection-question-body-capture .quilt-reflection-scrap-text,
        ${ROOT} .ig-reflection-question-body-capture .quilt-reflection-prompt-question,
        ${ROOT} .quilt-reflection-question-paper .quilt-reflection-scrap-text,
        ${ROOT} .quilt-reflection-prompt-question {
          line-height: 1.12 !important;
        }
        ${ROOT} .quilt-reflection-question-paper-body {
          overflow: hidden !important;
          justify-content: center !important;
        }
        ${ROOT} .quilt-reflection-question-paper {
          overflow: hidden !important;
          transform: none !important;
          --reflection-card-inset-top: 21%;
          --reflection-card-inset-bottom: 32%;
        ...
        ${ROOT} .quilt-reflection-carousel-slide--clipping {
          width: ${exportW - 56}px !important;
          max-width: ${exportW - 56}px !important;
        }
        ...
        ${ROOT} .quilt-reflection-carousel-footer,
        ...
        ${ROOT} .quilt-reflection-carousel-track {
          ...
          flex-direction: column !important;
          align-items: flex-start !important;
          gap: clamp(0.35rem, 1.5vw, 0.65rem) !important;
          transform: none !important;
        }
        ...
        ${ROOT} .quilt-reflection-question-paper {
          ...
          margin-right: auto !important;
          margin-top: 0 !important;
          margin-bottom: clamp(0.35rem, 1.5vw, 0.65rem) !important;
        }
    ...
    _syncIgReflectionCaptureClone(captureClone, dateKey) {
      ...
      const dk = String(dateKey || Utils.getTodayKey() || 'nodate').trim() || 'nodate';
      const paperBody = captureClone.querySelector('.quilt-reflection-question-paper-body');
      const promptEl = captureClone.querySelector('.quilt-reflection-scrap-text');
      if (paperBody && promptEl) {
        this._syncIgReflectionQuestionPaperExportType(captureClone);
      }
      this.syncReflectionCarouselEdgeFray(slides);
      this.syncReflectionCarouselJoinTape(slides);
      this.syncReflectionCarouselSplitTape(slides);
    ...
    async _captureIgReflectionExportElement(sourceEl, dateKey) {
      ...
      } else if (root.classList.contains('quilt-reflection-question-paper')) {
        const srcCs = getComputedStyle(sourceEl);
        root.style.width = srcCs.width;
        root.style.height = srcCs.height;
        root.style.transform = 'none';
        root.style.margin = '0';
        root.style.aspectRatio = srcCs.aspectRatio;

        await this._waitForIgReflectionExportImages(root);
        this._syncIgReflectionQuestionPaperExportType(root);

        const scale = Math.max(2, window.devicePixelRatio || 2);
        try {
          const sheetCanvas = await this._drawIgReflectionQuestionPaperSheetCanvas(w, h, root, scale);
          const bodyCapture = await this._captureIgReflectionQuestionPaperBodyText(root, html2canvas, scale);
          return await this._mergeIgReflectionQuestionPaperCaptureLayers(
            sheetCanvas,
            bodyCapture,
            w,
            h,
    ...
    async _compositeIgReflectionWidgetCapture(sourceWidget, dateKey) {
      ...
      const prompt = sourceWidget.querySelector('.quilt-reflection-question-paper');
      if (prompt) {
        const rect = prompt.getBoundingClientRect();
        const canvas = await this._captureIgReflectionExportElement(prompt, dateKey);
        if (canvas) {
          layers.push({
            canvas,
    ...
    _buildIgReflectionSlideExportMarkup(reflectionPrompt, themeEntries, dateKey) {
      ...
      const carouselMarkup = this.buildIgReflectionCarouselMarkupForExport(themeEntries, dateKey);
      return `
        <div id="screen-quilt-ig-export" class="is-ig-export">
          <div class="quilt-reflection-scrap-widget">
            <div class="quilt-reflection-question-paper">
              <picture class="quilt-reflection-question-paper__sheet" aria-hidden="true">
                <source srcset="assets/reflection-prompt-card.webp?v=5" type="image/webp" />
                <img src="assets/reflection-prompt-card.png?v=5" alt="" width="450" height="563" decoding="sync" />
              </picture>
              <div class="quilt-reflection-question-paper-body">
                <p class="quilt-reflection-scrap-text quilt-reflection-prompt-question">${promptHtml}</p>
              </div>
            </div>
    ...
    _syncIgReflectionSlideExportLayout(exportScreen, dateKey) {
      const slides = Array.from(exportScreen.querySelectorAll('.quilt-reflection-carousel-slide'));
      const dk = String(dateKey || Utils.getTodayKey() || 'nodate').trim() || 'nodate';
      const paperBody = exportScreen.querySelector('.quilt-reflection-question-paper-body');
      const promptEl = exportScreen.querySelector('.quilt-reflection-scrap-text');
      if (paperBody && promptEl) {
        this._syncIgReflectionQuestionPaperExportType(exportScreen);
      }
      this.syncReflectionCarouselEdgeFray(slides);
      this.syncReflectionCarouselJoinTape(slides);
      this.syncReflectionCarouselSplitTape(slides);
     ...
     * Uses live CSS + carousel sync helpers; caller composites onto quilt bg.
     */
    async rasterizeIgReflectionSlideCardsLayer(options = {}) {
      ...
      try {
        this._syncIgReflectionSlideExportLayout(exportScreen, dateKey);
