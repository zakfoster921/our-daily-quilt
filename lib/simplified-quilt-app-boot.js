        ...
        const pickUrl = async () => {
          ...
          if (
            ...
            const immediateNativeUrl = [
              ...
              .find((value) => /^(https?:|data:)/i.test(value));
            if (immediateNativeUrl) {
              return immediateNativeUrl;
            }
          }
          if (
            typeof window !== 'undefined' &&
            typeof window.odqIsCapacitorNative === 'function' &&
            window.odqIsCapacitorNative() &&
            !String(profile.cutoutUrl || cutoutSnap || '').trim()
          ) {
            const immediate = portraitForResolve || profile.imageUrl || '';
            if (immediate) {
              return immediate;
            }
          }
