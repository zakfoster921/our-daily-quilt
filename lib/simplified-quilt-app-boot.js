...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    _quoteSpeakerBirthdayHatPaintBoxSeat(cutout) {
      ...
      // Target: brim ~22% down the portrait — into hair/forehead, not floating above the scalp.
      const brimY = boxY + boxH * 0.22;
      return {
    ...
    _quoteSpeakerBirthdayHatSeatFromCrown(cutout, measureImg, crown) {
      ...
      const seatFromBox = (originX, originY, boxW, boxH) => {
        ...
        // Seat into the hairline — enough to close the float gap without covering eyes.
        const dip = Math.max(14, Math.min(foreheadW * 0.16, boxH * 0.07));
