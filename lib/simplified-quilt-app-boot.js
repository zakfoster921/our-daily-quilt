...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    /** Plain cutout layout: contain full PNG (matches Layout B story speaker rect). */
    ...
    _quoteSpeakerStoryCutoutBakeOptions(bakeLayout = {}) {
      ...
      return {
        ...
        options: {
          ...
          washColor,
          seed: paperSeed,
          layout: bakeLayout.compositeLayout || 'contain'
        }
      };
    }
    ...
    _resolveQuoteSpeakerPlainCutoutBakeLayout(cutout, sourceImg) {
      ...
      const naturalDrawH = Math.max(120, Math.ceil((ih * boxW) / iw));
      const tuckBand = Math.max(18, Math.round(naturalDrawH * 0.1));
      const frameH = Math.max(120, naturalDrawH + tuckBand);
    ...
    _applyQuoteSpeakerPlainCutoutBacking(cutout, img, sourceUrl = '') {
      ...
      const paperSig = `${layoutSig}|${bakeKey}|story-over-quilt-v1`;
      const compositeReady =
