    ...
    patchReflectionArchiveEntryInDom(dateKey, entry) {
      ...
      if (quiltName) {
        if (!quiltNameEl) {
          ...
          const clipping = quoteInsertAnchor();
          if (frame) details.insertBefore(quiltNameEl, frame);
          else if (clipping) details.insertBefore(quiltNameEl, clipping);
          else details.appendChild(quiltNameEl);
        }
