...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    _igReflectionCaptureStyleFilter(text) {
      return /quilt-reflection|reflection-prompt|reflection-question|reflection-scrap|reflection-clipping|reflection-patch|reflection-fabric|reflection-join|reflection-split|reflection-card|reflection-fray|quilt-byg|join-tape|split-tape|question-paper|quilt-quote-copy-size|quilt-reflection-prompt|scrap-text|prompt-question|odq-paper-tape|before-you-go-tape/i.test(
        String(text || '')
      );
    }
    ...
    async rasterizeIgReflectionSlideCardsLayer(options = {}) {
      ...
      try {
        ...
        let captureClone = document.getElementById('ig-reflection-capture-clone');
        if (!captureClone) {
          captureClone = document.createElement('div');
          captureClone.id = 'ig-reflection-capture-clone';
          captureClone.setAttribute('aria-hidden', 'true');
          document.body.appendChild(captureClone);
        }
        captureClone.style.width = `${logicalW}px`;
        captureClone.innerHTML = captureTarget.innerHTML;
        this._ensureIgReflectionCaptureStyles();
        this._syncIgReflectionCaptureClone(captureClone, dateKey);
        await this._waitForIgReflectionExportImages(captureClone);
        this._syncIgReflectionCaptureClone(captureClone, dateKey);

        logicalW = Math.max(
          1,
          Math.round(captureClone.scrollWidth || captureClone.offsetWidth || logicalW)
        );
        logicalH = Math.max(
          1,
          Math.round(captureClone.scrollHeight || captureClone.offsetHeight || 1)
        );

        const captureScale = Math.max(2, window.devicePixelRatio || 2);
        captureCanvas = await html2canvas(captureClone, {
        ...
        logicalH = Math.max(1, Math.round(captureCanvas.height / captureScale));
      } finally {
