...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    initReflectionCarouselLoop(notes) {
      ...
      const carouselTouchAxisRatio = 1.25;
      ...
      const resetCarouselTouchGesture = () => {
        ...
        viewport.classList.remove('is-vertical-touch-scroll');
      };
      const syncCarouselVerticalPageScroll = (touch, event) => {
        if (!quiltScrollEl || !carouselTouchGesture || carouselTouchGesture.axis !== 'y') return;
        event?.preventDefault?.();
        const deltaY = carouselTouchGesture.lastY - touch.clientY;
        if (Math.abs(deltaY) >= 0.5) {
          quiltScrollEl.scrollTop += deltaY;
          carouselTouchGesture.lastY = touch.clientY;
        }
        if (carouselTouchGesture.lockScrollLeft != null) {
          viewport.scrollLeft = carouselTouchGesture.lockScrollLeft;
        }
      };
        ...
        (event) => {
          ...
          if (
            ...
            if (absDy > absDx * carouselTouchAxisRatio) {
              carouselTouchGesture.axis = 'y';
              carouselTouchGesture.lockScrollLeft = viewport.scrollLeft;
              viewport.classList.add('is-vertical-touch-scroll');
            } else if (absDx > absDy * carouselTouchAxisRatio) {
              carouselTouchGesture.axis = 'x';
            }
          }
          syncCarouselVerticalPageScroll(touch, event);
        },
        { passive: false }
      );
    ...
    /** file:// / null origin cannot read cross-origin pixels — display-only is fine. */
    _quoteSpeakerCanvasPipelineBlocked() {
      if (typeof location === 'undefined') return true;
      const origin = String(location.origin || '').trim();
      return !origin || origin === 'null' || location.protocol === 'file:';
    }
    ...
    _quoteSpeakerMaskSubjectComponent(d, iw, ih) {
      ...
      for (let y = y0; y < y1; y += 1) {
        for (let x = x0; x < x1; x += 1) {
          const i = (y * iw + x) * 4;
          if (!this._quoteSpeakerPixelIsSubject(d[i], d[i + 1], d[i + 2], d[i + 3])) continue;
    ...
    /** Complementary / contrasting wash for Warhol speaker cutout (not the dominant quilt swatch). */
    getSpeakerCutoutWashColor() {
      const complementary = String(this.getContrastingQuiltColorExcludingDominant?.() || '').trim();
      if (/^#[0-9A-Fa-f]{6}$/.test(complementary)) return complementary;
      const dominant = String(
        this.getMostPopularQuiltColor()?.color || CONFIG.APP.defaultColor || '#ea9b9a'
      ).trim();
      const safe = /^#[0-9A-Fa-f]{6}$/.test(dominant) ? dominant : '#ea9b9a';
      const hsv = Utils.hexToHsv(safe);
      return Utils.hsvToHex((hsv.h + 180) % 360, Math.max(28, hsv.s * 0.55), Math.max(68, hsv.v));
    }
    ...
    _applyQuoteSpeakerWashColors(widget) {
      ...
      const washColor = String(
        this.getSpeakerCutoutWashColor?.() ||
          this.getMostPopularQuiltColor()?.color ||
      ...
      const cutout = document.getElementById('quoteSpeakerImageWrap');
      if (cutout && !cutout.hidden) this._applyQuoteSpeakerCutoutRenderPlan(cutout);
    }

    _applyQuoteSpeakerCutoutRenderPlan(cutout) {
      const SCR = globalThis.SpeakerCutoutRender;
      if (!SCR?.buildSpeakerCutoutPlan || !SCR?.applySpeakerCutoutPlanToElement || !cutout) return;
      const widget = document.getElementById('quoteSpeakerWidget');
      const washColor = String(
        widget?.style.getPropertyValue('--speaker-wash-color') ||
          this.getSpeakerCutoutWashColor?.() ||
          CONFIG.APP.defaultColor ||
          '#ea9b9a'
      ).trim();
      ...
      const boxH = Math.max(1, Math.round(cutout.clientHeight || boxW * 1.2));
    ...
    _applyQuoteSpeakerImageLoaded(
      ...
      const speakerImageCssUrl = washPaintUrl
        ...
        : 'none';
      const paintUrl = String(remoteUrlForCss || cssPaintUrl || displayUrl || '').trim();
      const isSpeakerCutout = /speaker-cutouts(?:%2F|\/)/i.test(paintUrl);
      ...
      if (imageWrap) {
        ...
        imageWrap.classList.toggle('quote-speaker-cutout--single-layer', isSpeakerCutout);
      ...
      if (neutralSilhouette) neutralSilhouette.hidden = isSpeakerCutout || useDataUrlPaint;
      const cutoutMat = document.getElementById('quoteSpeakerCutoutMat');
      if (cutoutMat) cutoutMat.hidden = !isSpeakerCutout || useDataUrlPaint;
      if (colorWash) {
        colorWash.hidden = isSpeakerCutout ? false : useDataUrlPaint ? true : !washPaintUrl;
      }
      if (img) {
        img.src = String(displayUrl || cssPaintUrl).trim();
        img.hidden = false;
        img.classList.toggle('is-loaded', isSpeakerCutout || useDataUrlPaint);
      }
    ...
    _refreshQuoteSpeakerWidgetNow(quote = null) {
      ...
      if (showSpeakerImage && imageWrap) {
        const applyLoaded = (displayUrl, remoteUrlForCss = '') => {
        ...
        pickUrl()
          ...
          .then(async ({ remote, prepared }) => {
            ...
            const cleaned = base
              ? await this._quoteSpeakerCleanCutoutDisplayUrl(base, resolvedQuote)
              : '';
            return { remote, prepared: cleaned || base };
          })
          .then(({ remote, prepared }) => applyLoaded(prepared || remote, remote))
          .catch(() => applyLoaded(''));
      }
    ...
    async _refreshQuiltMoodSpread(
      ...
      const sig = `${g}\u0001${r}`;
      if (spreadHost.dataset.contentSig === sig && spreadHost._moodTriptychWidget) {
      ...
      try {
        ...
        quoteDataUrl = dk
          ? await this.resolveNightlyNewspaperClippingUrl(dk, { waitForFirebaseMs: 12000 })
          : null;
        if (quoteDataUrl) {
          console.info('[our-daily] Triptych quote: nightly PNG (source of truth)');
          void this.applyQuoteScreenClipping({ dateKey: dk, quote: todayQuote });
