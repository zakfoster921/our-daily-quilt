...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    _applyQuoteSpeakerImageLoaded(
      ...
      const useImgPaint =
        /^data:/i.test(String(displayUrl || '')) && cssPaintUrl === String(displayUrl || '').trim();
      const speakerImageCssUrl = useImgPaint
        ? 'none'
        : `url("${String(cssPaintUrl).replace(/"/g, '%22')}")`;
      if (imageWrap) {
        imageWrap.hidden = false;
        imageWrap.classList.add('quote-speaker-cutout--loaded');
        imageWrap.classList.toggle('quote-speaker-cutout--img-paint', useImgPaint);
    ...
    _showQuoteSpeakerAvatarFallback(imageWrap, img, neutralSilhouette, colorWash, avatar) {
      if (imageWrap) {
        imageWrap.hidden = false;
        imageWrap.classList.remove('quote-speaker-cutout--loaded', 'quote-speaker-cutout--img-paint');
      ...
      if (showSpeakerImage && imageWrap) {
        ...
        pickUrl()
          .then(async (remoteUrl) => {
            ...
            const prepared = await prepareDisplayUrl(remote);
            return { remote, prepared: prepared || remote };
          })
          .then(({ remote, prepared }) => applyLoaded(prepared, remote))
          .catch(() => applyLoaded(''));
      }
    ...
    async _refreshQuiltMoodSpread(
      ...
      try {
        ...
        const storageUrl = dk
          ? await this.fetchNewspaperClippingUrl(dk, { waitForFirebaseMs: 12000 })
          : null;
        if (spreadHost._moodSpreadComposePending !== sig) {
          return;
        }
        if (storageUrl && (await this._probeRemoteImageUrl(storageUrl))) {
          ...
          console.warn('[our-daily] Nightly newspaper clipping PNG failed to load');
        }
        ...
        if (!quoteDataUrl && storageUrl) {
          console.warn(
            '[our-daily] Triptych quote unavailable: nightly PNG unloadable and compose failed'
          );
        }
        if (spreadHost._moodSpreadComposePending !== sig) {
          return;
        ...
        if (String(quoteDataUrl || '').startsWith('data:') && dk) {
          ...
          global.setTimeout(upgrade, 2000);
