  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    setupLayoutBStoryPreviewLongPressShare() {
      ...
      const canShareLayoutBStoryPreview = () => {
        ...
        if (!img || img.hidden || !(img.naturalWidth > 0)) return false;
        return !!(
          this._layoutBStoryPreviewShareBlob ||
          this._layoutBStoryPreviewLayerUrls ||
          this._layoutBStoryPreviewShareUrl ||
          this._layoutBPreviewObjectUrl ||
          String(img.currentSrc || img.src || '').trim()
        );
      };

      const shareLayoutBStoryPreview = async () => {
        try {
          let shareBlob = null;
          if (this._layoutBStoryPreviewHasVisibleOverlay?.()) {
            shareBlob = await this._composeLayoutBStoryPreviewLayersFromDom?.();
          }
          if (!shareBlob) {
            shareBlob = await this._ensureLayoutBStoryPreviewShareBlob?.();
          }
          if (!shareBlob?.size) {
            throw new Error('Story preview share image is not ready yet');
          }
          ...
          if (typeof window.odqTrack === 'function') {
            window.odqTrack('download_story_image', { method: 'story_preview_long_press' });
          }
        } catch (error) {
      ...
      this._bindSustainHoldGesture(sheet, {
        isEnabled: canShareLayoutBStoryPreview,
        onPrime: () => {
          void this._ensureLayoutBStoryPreviewShareBlob?.();
        },
        onHoldStart: () => {
          void shareLayoutBStoryPreview();
        },
