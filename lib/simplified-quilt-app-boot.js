...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    _igReflectionCaptureStackGap(responseCount) {
      const n = Math.max(1, Math.min(6, Math.floor(Number(responseCount) || 1)));
      if (n >= 5) return '0.72rem';
      if (n >= 3) return '0.95rem';
      return '1.1rem';
    }
    ...
    syncIgReflectionCaptureClusterJoinTape(gridEl, dateKey) {
      ...
      const appendHorizontalTape = (topPatch, bottomPatch, seedKey) => {
        ...
        const variant = this._reflectionJoinTapeVariant(seedKey, overlapW);
        const joinTop = (topR.bottom + bottomR.top) / 2 - gridRect.top;
        const joinLeft = overlapLeft + overlapW / 2 - gridRect.left;
