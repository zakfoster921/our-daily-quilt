...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    _mergeIgReflectionClippingCaptureLayers(paperCanvas, textCanvas, w, h, scale) {
      if (!paperCanvas) return textCanvas || null;
      if (!textCanvas) return paperCanvas;

      const merged = document.createElement('canvas');
      merged.width = Math.max(paperCanvas.width, textCanvas.width);
      merged.height = Math.max(paperCanvas.height, textCanvas.height);
