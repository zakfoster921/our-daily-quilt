...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    /** Row sizes for IG capture — pack patches into a tight cluster (1–6). */
    ...
    _fitIgReflectionResponsePatchHeights(responseSlides) {
      ...
      list.forEach((slide) => {
        ...
        const needed = Math.max(168, Math.ceil(contentH + edgeY * 2 + 4));
    ...
    async _igReflectionCardsLayerFromPngBase64(b64, sizeHints = {}) {
      ...
      return new Promise((resolve) => {
        ...
        img.onload = () => {
          ...
          const trimmed = this._trimIgReflectionCardsCanvas(canvas);
    ...
    _ensureIgReflectionPlaywrightCaptureStyles(responseCount = 3) {
      ...
      style.textContent = `
        ...
        #screen-quilt.is-ig-reflection-playwright-capture .quilt-reflection-carousel-join-tape--cross-row {
          width: var(--reflection-join-tape-span, 100%);
          height: calc(var(--reflection-join-tape-thickness) * var(--reflection-join-tape-width-scale, 1));
          transform: translate(-50%, -50%)
            rotate(calc(var(--reflection-fabric-tilt, 0deg) + var(--reflection-join-tape-tilt-jitter, 0deg)))
    ...
    _syncIgReflectionPlaywrightCaptureLayout(captureScreen, dateKey) {
      ...
      if (promptSlide) {
        promptSlide.style.width = `${columnW}px`;
        promptSlide.style.maxWidth = `${columnW}px`;
        promptSlide.style.setProperty('--reflection-patch-w', `${columnW}px`);
      }
      ...
      gridRows.forEach((rowEl, rowIndex) => {
        ...
        if (n === 1) {
          const soloRatio =
            responseSlides.length === 1 ? 0.88 : responseSlides.length <= 4 ? 0.76 : 0.68;
          const soloW = Math.min(columnW, Math.floor(columnW * soloRatio));
          ...
          const prevCount = prevRow
            ...
            : 0;
          if (prevRow && prevCount >= 2) {
            const prevOverlap = overlapPx;
            const prevEqualW = Math.floor((columnW + prevOverlap * (prevCount - 1)) / prevCount);
            const seamX = prevEqualW - prevOverlap;
        ...
        const equalW = Math.floor((columnW + overlapPx * (n - 1)) / n);
        rowSlides.forEach((slide, index) => {
          slide.style.width = `${equalW}px`;
          slide.style.maxWidth = `${equalW}px`;
          slide.style.flex = `0 0 ${equalW}px`;
          slide.style.setProperty('--reflection-patch-w', `${equalW}px`);
    ...
    _buildReflectionJoinTapeHorizontalScrapClipPath(seedKey = '') {
      ...
      if (rnd() > 0.38) {
        const peakY = 34 + rnd() * 32;
        if (rnd() > 0.5) {
          const peakX = rnd() * 5;
          return `polygon(${pct(peakX)} ${pct(peakY)}, ${pct(rightTopX)} ${pct(rightTopY)}, ${pct(rightBotX)} ${pct(rightBotY)}, ${pct(leftBotX)} ${pct(leftBotY)}, ${pct(leftTopX)} ${pct(leftTopY)})`;
        }
        const valleyX = 95 + rnd() * 4;
        return `polygon(${pct(leftTopX)} ${pct(leftTopY)}, ${pct(rightTopX)} ${pct(rightTopY)}, ${pct(valleyX)} ${pct(peakY)}, ${pct(rightBotX)} ${pct(rightBotY)}, ${pct(leftBotX)} ${pct(leftBotY)})`;
      }
    ...
    syncIgReflectionCaptureClusterJoinTape(gridEl, dateKey) {
      ...
      const appendHorizontalTapeAtSeam = (topPatches, bottomPatch, seedKey) => {
        const above = Array.isArray(topPatches) ? topPatches.filter(Boolean) : [];
        if (!above.length || !bottomPatch) return;

        const bottomR = bottomPatch.getBoundingClientRect();
        if (!bottomR.width || !bottomR.height) return;

        const overlappingAbove = above.filter((patch) => {
          const topR = patch.getBoundingClientRect();
          const overlapW = Math.min(topR.right, bottomR.right) - Math.max(topR.left, bottomR.left);
          return overlapW >= 24;
        });
        if (!overlappingAbove.length) return;

        const seamPatches = [...overlappingAbove, bottomPatch];
        const sideBleed = Math.max(
          this._readReflectionPatchSideBleedPx(bottomPatch),
          ...seamPatches.map((patch) => this._readReflectionPatchSideBleedPx(patch))
        );
        const spanLeft =
          Math.min(...seamPatches.map((patch) => patch.getBoundingClientRect().left)) - sideBleed;
        const spanRight =
          Math.max(...seamPatches.map((patch) => patch.getBoundingClientRect().right)) + sideBleed;
        const overlapW = spanRight - spanLeft;
        if (overlapW < 24) return;

        let visualTopRowBottom = -Infinity;
        overlappingAbove.forEach((patch) => {
          const rect = patch.getBoundingClientRect();
          const edgeY = this._readReflectionPatchPaperEdgeY(patch);
          visualTopRowBottom = Math.max(visualTopRowBottom, rect.bottom + edgeY);
        });
        const bottomEdgeY = this._readReflectionPatchPaperEdgeY(bottomPatch);
        const visualBottomTop = bottomR.top - bottomEdgeY;
        const seamY = (visualTopRowBottom + visualBottomTop) / 2;

        const variant = this._reflectionJoinTapeVariant(seedKey, overlapW, { orientation: 'horizontal' });
        const joinLeft = spanLeft + overlapW / 2 + variant.verticalShift - gridRect.left;
        const joinTop = seamY - gridRect.top;
        const avgTilt =
          [...overlappingAbove, bottomPatch].reduce((sum, patch) => sum + readFabricTiltDeg(patch), 0) /
          (overlappingAbove.length + 1);
        const tape = document.createElement('span');
        tape.className =
      ...
      for (let rowIndex = 1; rowIndex < rows.length; rowIndex += 1) {
        ...
        below.forEach((bottomPatch, bottomIndex) => {
          appendHorizontalTapeAtSeam(
            above,
            bottomPatch,
            `${dk}:ig-capture-join-v:${rowIndex}:${bottomIndex}`
          );
        });
      }
    }
