...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    /** ScrollTop where the icon footer sits on the viewport bottom (image-2 stop). */
    getQuiltFooterScrollStopTop(scroller = null) {
      ...
      const el =
        ...
        quiltScreen;
      if (!quiltScreen?.classList.contains('active') || !footer || !el) return 0;

      const viewportBottom = this._footerIconPaperViewportBottom();
      const footerRect = footer.getBoundingClientRect();
      const stopTop = Math.round(el.scrollTop + (footerRect.bottom - viewportBottom));
    ...
    _footerIconPaperAtScrollStop() {
      ...
      const viewportBottom = this._footerIconPaperViewportBottom();
      const footerRect = footer.getBoundingClientRect();
      return Math.abs(footerRect.bottom - viewportBottom) <= 8;
    }
    ...
    _readFooterIconPaperLayout(paper) {
      const top = parseFloat(paper.style.getPropertyValue('--quilt-footer-icon-paper-top'));
      ...
      if (![top, height, width].every((value) => Number.isFinite(value))) return null;
      return {
        top,
    ...
    updateFooterIconPaperLayout() {
      ...
      this._footerIconPaperElements().forEach((paper) => {
        ...
        if (!slotRect || rect.bottom < 0 || rect.top > viewportBottom + 2) {
          paper.style.setProperty('--quilt-footer-icon-paper-height', '0px');
          return;
        }
        ...
        const topPadPx = pad.topExtra * size.topPadScale;
        const stripTopViewport = rect.top - topPadPx;
        const topPx = Math.round(rect.top - slotRect.top - topPadPx);
        const atScrollStop = this._footerIconPaperAtScrollStop();
        const heightPx = atScrollStop
          ? Math.max(1, Math.ceil(viewportBottom - stripTopViewport + size.bottomExtra))
          : Math.max(
              28,
              Math.ceil(control.offsetHeight + topPadPx + pad.bottomExtra + size.bottomExtra)
            );
        const widthPx = Math.ceil((rect.width + pad.widthExtra) * size.widthScale);
    ...
    _bindQuiltFooterPaperLayoutSync() {
      ...
      scrollers.forEach((el) => {
        if (seen.has(el)) return;
        seen.add(el);
        el.addEventListener('scroll', onLayoutChange, { passive: true });
      });
    ...
    async loadReflectionThemesForToday() {
      ...
      const assignmentFirst = await this.fetchTodayFirstResponseFields();
      const wallThemes = await this.resolveReflectionWallThemes(communityThemes, {
        first_response:
          assignmentFirst.first_response || todayThemes?.first_response || '',
        user_name: assignmentFirst.user_name || todayThemes?.user_name || ''
      });
    ...
    _applyQuoteSpeakerImageLoaded(
      ...
      if (
        ...
        if (usePlainCutout && imageWrap && img?.naturalWidth) {
      ...
      if (canKeepLoadedImage) {
        ...
        pickUrl()
          ...
          .then(async ({ remote, prepared }) => {
            ...
            const cutoutSource = base;
            const cleaned = base
              ? await this._quoteSpeakerCleanCutoutDisplayUrl(base, resolvedQuote)
              : '';
            return { remote, prepared: cleaned || base, cutoutSource };
          })
