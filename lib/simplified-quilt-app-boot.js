...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    /** IG slide 2 — shorter response strips (inline author, one line). */
    getIgReflectionResponsePatchHeight(themeText, allThemes = []) {
      const minRem = 6.5;
      const maxRem = 8.25;
      const normalizeLen = (value) => {
        const normalized = this.normalizeReflectionWallTheme(value);
        if (normalized?.text) {
          const author = String(normalized.author || '').trim();
    ...
    getReflectionFabricPatchStyle(index, themeText, allThemes, rgbOverride, options = {}) {
      ...
      return [
        ...
        `--reflection-patch-h: ${
          options.igExport
            ? this.getIgReflectionResponsePatchHeight(themeText, allThemes)
            : this.getReflectionFabricPatchHeight(themeText, allThemes)
        }`,
    ...
    buildReflectionThemePatchSlide(
      ...
      return `
            <article class="quilt-reflection-carousel-slide quilt-reflection-carousel-slide--clipping${splitClass}${wideCopyClass}${lightClass}${highlightClass}${igExportClass}" data-reflection-theme-slide data-reflection-slide="${slideIndex}" data-reflection-hand-cut-seed="${dateKey}:reflection-response:${slideIndex}"${mineAttrs} style="${this.getReflectionFabricPatchStyle(slideIndex, theme, allThemes, rgb, { adminHighlight: highlightForArticle, igExport })}">
              <div class="quilt-reflection-carousel-text">${this.formatReflectionCarouselTextHtml(theme, slideIndex, { heartMarkup: igExport ? '' : heartMarkup, rgb, inlineAuthor: igExport })}</div>
              ${footerMarkup}
            </article>
          `;
    }
    ...
    getIgReflectionPromptStripHeight(promptText) {
      ...
      if (len <= 42) return '9.25rem';
      if (len <= 68) return '10.5rem';
      if (len <= 92) return '11.75rem';
      return '13rem';
    }
    ...
    buildIgReflectionPromptStripSlide(reflectionPrompt, dateKey) {
      ...
      return `
            <article class="quilt-reflection-carousel-slide quilt-reflection-carousel-slide--clipping quilt-reflection-carousel-slide--ig-prompt quilt-reflection-carousel-slide--wide-copy" data-reflection-ig-prompt-slide data-reflection-slide="0" data-reflection-hand-cut-seed="${this.escapeQuiltFortuneText(dk)}:reflection-ig-prompt" style="${style}">
              <div class="quilt-reflection-carousel-text">
                <span class="quilt-reflection-carousel-ig-prompt-kicker" aria-hidden="true">Reflection</span>
    ...
    buildIgReflectionCarouselMarkupForExport(themeEntries, dateKey, reflectionPrompt = '') {
      ...
      for (let i = 0; i < ideas.length; i += 1) {
        slideParts.push(
          this.buildReflectionThemePatchSlide(i + 1, ideas[i], ideas, themeRgbs[i], {}, null, dk, {
            igExport: true
          })
        );
      }
    ...
    _igReflectionExportResponsePatchWidth(exportViewportWidth) {
      const full = Math.max(1, (exportViewportWidth || this._igReflectionExportViewportWidth()) - 56);
      return Math.max(1, Math.round(full * 0.9));
    }

    _ensureIgReflectionExportStyles() {
      const exportW = this._igReflectionExportViewportWidth();
      const patchW = exportW - 56;
      const responsePatchW = this._igReflectionExportResponsePatchWidth(exportW);
      ...
      style.textContent = `
        ...
        #screen-quilt.is-ig-export .quilt-reflection-carousel-slide--ig-export-response {
          width: ${responsePatchW}px !important;
          max-width: ${responsePatchW}px !important;
          flex: 0 0 ${responsePatchW}px !important;
          --reflection-patch-w: ${responsePatchW}px;
        ...
        #screen-quilt.is-ig-export .quilt-reflection-carousel-slide--clipping:not(.quilt-reflection-carousel-slide--ig-export-response) {
          width: ${patchW}px !important;
          max-width: ${patchW}px !important;
          flex: 0 0 ${patchW}px !important;
        }
        ...
        #screen-quilt.is-ig-export .quilt-reflection-carousel-slide--ig-export-response .quilt-reflection-carousel-body--inline-author {
          font-size: ${patchBodyPx}px !important;
          line-height: 1.18 !important;
        }
        #screen-quilt.is-ig-export .quilt-reflection-carousel-author--inline {
        ...
        #screen-quilt.is-ig-export .quilt-reflection-carousel-track {
          ...
          gap: clamp(0.12rem, 0.5vw, 0.28rem) !important;
    ...
    _ensureIgReflectionCaptureStyles() {
      ...
      css += `
        ...
        ${ROOT} .quilt-reflection-carousel-slide--ig-export-response .quilt-reflection-carousel-body--inline-author {
          font-size: ${patchBodyPx}px !important;
          line-height: 1.18 !important;
        }
        ${ROOT} .quilt-reflection-carousel-author--inline {
          display: inline !important;
          font-size: ${patchBodyPx}px !important;
          line-height: inherit !important;
        }
        ${ROOT} .quilt-reflection-carousel-slide--ig-export-response {
          width: ${responsePatchW}px !important;
          max-width: ${responsePatchW}px !important;
          --reflection-patch-w: ${responsePatchW}px;
          --reflection-clipping-pad-y: calc(5px + var(--reflection-clipping-edge-y));
        ...
        ${ROOT} .quilt-reflection-carousel-slide--clipping:not(.quilt-reflection-carousel-slide--ig-export-response) {
    ...
    _syncIgReflectionSlideExportLayout(exportScreen, dateKey) {
      ...
      const dk = String(dateKey || Utils.getTodayKey() || 'nodate').trim() || 'nodate';
      const responsePatchW = this._igReflectionExportResponsePatchWidth();
      exportScreen.querySelectorAll('.quilt-reflection-carousel-slide--ig-export-response').forEach((slide) => {
        slide.style.width = `${responsePatchW}px`;
        slide.style.maxWidth = `${responsePatchW}px`;
        slide.style.flex = `0 0 ${responsePatchW}px`;
        slide.style.setProperty('--reflection-patch-w', `${responsePatchW}px`);
      });
      this.syncReflectionCarouselEdgeFray(slides);
      this.syncReflectionCarouselJoinTape(slides);
      this.syncReflectionCarouselSplitTape(slides);
