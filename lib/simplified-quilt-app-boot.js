  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    async loadDeferredAdminSlice() {
      ...
      const SliceCtor = await this._loadDeferredScript(
        'lib/simplified-quilt-app-admin.js?v=28',
    ...
    _adminDailyTaskDateKey() {
      return String(
        this.quoteService?.getQuoteCalendarKeyNow?.() ||
          Utils.getTodayKey?.() ||
          this._adminTuneReminderDateKey?.() ||
          ''
      ).trim();
    }
    ...
    async refreshAdminDailyTaskBanner() {
      ...
      if (localTasks.igPostCompleted || localTasks.previewTomorrowCompleted) {
        void this._writeAdminDailyTasksViaBackend(dateKey, localTasks);
      }

      const backendTasks = await this._readAdminDailyTasksViaBackend(dateKey);
      if (backendTasks) {
        const tasks = this._writeLocalAdminDailyTasks(dateKey, backendTasks);
      ...
      const readDoc = window.firestore?.getDocFromServer || window.firestore?.getDoc;
      if (!ref || typeof readDoc !== 'function') return;

      try {
        const snap = await readDoc(ref);
        if (!snap.exists?.()) return;
        const tasks = this._writeLocalAdminDailyTasks(dateKey, snap.data?.() || {});
        this._renderAdminDailyTaskBanner(tasks);
      } catch (error) {
    ...
    _scheduleAdminTuneReminderAfterAdminReady() {
      ...
      window.setTimeout(() => {
        void this.maybeShowAdminTuneReminder?.();
      }, 1400);
    }

    _showAdminTuneReminderModal() {
      ...
      return new Promise((resolve) => {
        ...
        overlay.className = 'odq-whats-new-overlay odq-admin-tune-reminder-overlay';
        overlay.setAttribute('role', 'dialog');
        overlay.setAttribute('aria-modal', 'true');
        overlay.setAttribute('aria-labelledby', 'odq-admin-tune-reminder-title');
        overlay.innerHTML = `
          <div class="odq-whats-new-scrim" data-odq-admin-tune-dismiss="1" aria-hidden="true"></div>
          <div class="odq-whats-new-dialog">
            <h2 id="odq-admin-tune-reminder-title" class="odq-whats-new-title">${escapeHtml(
              'HEY CUTIE you have two jobs: set IG post and preview tomorrow. Better get on it !'
            )}</h2>
            <div class="odq-whats-new-actions">
              <button type="button" class="odq-whats-new-btn odq-whats-new-btn--primary" data-odq-admin-tune-primary>yes</button>
              <button type="button" class="odq-whats-new-btn odq-whats-new-btn--secondary" data-odq-admin-tune-secondary>ask me next visit</button>
            </div>
          </div>
        `;
        ...
        const finish = (result) => {
          ...
          window.setTimeout(() => cleanup(result), 180);
        };
        const onKey = (e) => {
          if (e.key === 'Escape') finish('secondary');
        };

        overlay.addEventListener('click', (e) => {
          const target = e.target;
          if (target?.closest?.('[data-odq-admin-tune-primary]')) {
            e.preventDefault();
            finish('primary');
            return;
          }
          if (
            target?.closest?.('[data-odq-admin-tune-secondary]') ||
            target?.getAttribute?.('data-odq-admin-tune-dismiss') === '1'
          ) {
            e.preventDefault();
            finish('secondary');
          }
        ...
        requestAnimationFrame(() => {
          ...
          overlay.querySelector('[data-odq-admin-tune-primary]')?.focus?.({ preventScroll: true });
        });
      });
    }

    async maybeShowAdminTuneReminder() {
      if (this._adminTuneReminderInFlight) return false;
      if (!CONFIG.APP.enableAdminTools) return false;
      if (!this._isAdminTuneReminderEnabledForThisDevice()) return false;

      const dateKey = this._adminTuneReminderDateKey();
      if (!dateKey) return false;
      if (await this._isSharedAdminTuneReminderCompleted(dateKey)) return false;

      if (
        document.querySelector(
          '.odq-whats-new-overlay, .odq-push-optin-overlay, .odq-speaker-tune-modal, .odq-admin-passcode-modal'
        )
      ) {
        return false;
      }

      this._adminTuneReminderInFlight = true;
      let result = 'dismiss';
      try {
        result = await this._showAdminTuneReminderModal();
      } catch (error) {
        this.logger?.warn?.('Admin tune reminder failed:', error);
      } finally {
        this._adminTuneReminderInFlight = false;
      }

      if (result === 'primary') {
        await this._markSharedAdminTuneReminderCompleted(dateKey);
        if (typeof this.handleAdminTuneSpeakerCutout === 'function') {
          void this.handleAdminTuneSpeakerCutout();
        } else {
          this.uiService?.showToast?.('Tune tools are still loading');
        }
        return true;
      }
      return false;
    ...
    async loadDeferredSocialSlice() {
      ...
      const SliceCtor = await this._loadDeferredScript(
        'lib/simplified-quilt-app-social.js?v=3',
    ...
    async ensureLayoutBComposeReady() {
      ...
      await this._loadDeferredScript('lib/layout-b-compose.js?v=147', 'composeInstagramLayoutBFromQuiltBlob');
    ...
    _reflectionWallContentKey(themeEntries = [], myReflection = null) {
      return JSON.stringify({
        themes: (Array.isArray(themeEntries) ? themeEntries : []).map((entry) => ({
    ...
    _scheduleStoryPreviewOnce() {
      ...
      this._schedulePostLaunchIdleTask(
        ...
        async () => {
          const latestBlockCount = Array.isArray(this.quiltEngine?.blocks)
            ? this.quiltEngine.blocks.length
            : 0;
          if (latestBlockCount <= 1) return;
          void globalThis.ensureOdqCanvasFontsReady?.();
          void globalThis.odqPrefetchLayoutBKeywordEmphasis?.(dateKey);
          // Ensure layout-b-compose.js is loaded before scheduling — the heavy render
          // needs composeInstagramLayoutBFromQuiltBlob and won't self-load it on mobile.
          await this.ensureLayoutBComposeReady?.();
    ...
    /** Dev/preview: ?screen=zak|mission|welcome|bridge|first-name|portal|quote */
    ...
    setupQuiltUserColorTogetherNoteLongPress() {
      ...
      note.addEventListener('click', () => {
        if (wrap.hidden || typeof window.odqTrack !== 'function') return;
        window.odqTrack('tap_what_else', { gesture: 'tap' });
      });
      ...
      this._bindSustainHoldGesture(note, {
        ...
        onHoldStart: () => {
          ...
          if (typeof window.odqTrack === 'function') {
            window.odqTrack('tap_what_else', { gesture: 'long_press' });
          }
        },
    ...
    bindReflectionCarouselMineActions() {
      ...
      document.addEventListener('click', (event) => {
        ...
        if (heartBtn) {
          ...
          this.toggleReflectionCarouselHeart({ responseId, dateKey, slideEl: slide });
          return;
        }
    ...
    buildReflectionWallThemes(
      ...
      const rest = community.filter((entry) => {
        if (myReflection?.responseId) {
          if (String(entry.responseId || '').trim() === myReflection.responseId) return true;
          if (this.reflectionThemeMatchesMine(entry, myReflection)) return true;
        }
    ...
    async resolveReflectionWallThemes(communityThemes = [], firstResponseContext = null, myReflection = null) {
      ...
      return this.buildReflectionWallThemes(communityThemes, first, userName, myReflection, firstHC);
    }
    ...
    renderReflectionCarouselHeartHtml(heartId, heartCount) {
      ...
      return `<button type="button" class="quilt-reflection-carousel-heart${hearted ? ' quilt-reflection-carousel-heart--active' : ''}" aria-label="${hearted ? 'Unlike' : 'Like'}" aria-pressed="${hearted ? 'true' : 'false'}">
        <span class="material-symbols-outlined" aria-hidden="true" translate="no">favorite</span>
        ${countHtml}
      </button>`;
    }

    syncReflectionCarouselHeart(slideEl, { hearted, heartCount } = {}) {
      const btn = slideEl?.querySelector?.('.quilt-reflection-carousel-heart');
      if (!btn) return;
    ...
    async toggleReflectionCarouselHeart({ responseId, dateKey, slideEl } = {}) {
      ...
      this.syncReflectionCarouselHeart(slideEl, { hearted: optimisticHearted, heartCount: optimisticCount });
      const baseUrl = (typeof CONFIG !== 'undefined' && CONFIG.API_BASE_URL) || '';
      try {
        const clientId = (typeof UtilsCore !== 'undefined' && UtilsCore.getOrCreateUserId?.()) || '';
        ...
        this.syncReflectionCarouselHeart(slideEl, { hearted: !!data.hearted, heartCount: data.heartCount });
        this.setReflectionHeartedLocally(heartId, !!data.hearted);
      } catch (_) {
        this.setReflectionHeartedLocally(heartId, optimisticHearted);
        this.syncReflectionCarouselHeart(slideEl, { hearted: optimisticHearted, heartCount: optimisticCount });
      } finally {
    ...
    buildReflectionThemePatchSlide(slideIndex, theme, allThemes, rgb, edgeFray = {}, myReflection = null) {
      ...
      const responseId = isMine
        ? String(myReflection?.responseId || normalized?.responseId || '').trim()
      ...
      const heartMarkup = this.renderReflectionCarouselHeartHtml(heartId, normalized?.heartCount || 0);
      return `
            <article class="quilt-reflection-carousel-slide quilt-reflection-carousel-slide--clipping${wideCopyClass}${lightClass}" data-reflection-theme-slide data-reflection-slide="${slideIndex}" data-reflection-hand-cut-seed="${dateKey}:reflection-response:${slideIndex}"${mineAttrs} style="${this.getReflectionFabricPatchStyle(slideIndex, theme, allThemes, rgb)}">
              <div class="quilt-reflection-carousel-text quilt-reflection-carousel-copy">${this.formatReflectionCarouselTextHtml(theme)}</div>
              <div class="quilt-reflection-carousel-footer">
                ${mineActions}
                ${heartMarkup}
              </div>
            </article>
    ...
    _normalizeReflectionThemesPayload(key, data) {
      ...
      return { key, themes, first_response, user_name, firstResponseHeartCount };
    }
    ...
    _paintReflectionWallBootstrap(dateKey) {
      ...
      const wallThemes = this.buildReflectionWallThemes(themes, first || null, user, this._readMyReflectionCache(key), firstHC);
    ...
    async _readReflectionThemesForDate(key) {
      ...
      // Run SDK and REST reads in parallel. Resolve as soon as either produces a winner,
      // or when both have settled (both failed). A 6-second ceiling prevents an indefinite wait.
      let resolveRace;
      const racePromise = new Promise((res) => { resolveRace = res; });
      const claimAndResolve = (data, source) => {
        claim(data, source);
        if (winner) resolveRace();
      };
      const sdkRead = (async () => {
        const firestoreReady = await this.waitForReflectionFirestore(
          window.db && window.firestore?.doc ? 0 : 400
      ...
      const restRead = (async () => {
        ...
        if (data) {
          ...
          claimAndResolve(data, 'rest');
        }
      })();
      // Resolve race when a winner is found, when both reads settle, or after 6s ceiling.
      Promise.allSettled([sdkRead, restRead]).then(resolveRace);
      await Promise.race([racePromise, new Promise((res) => setTimeout(res, 6000))]);
    ...
    async loadReflectionThemesForToday() {
      ...
      if (lsCache || lsAssign.first_response) {
        ...
        const lsWallThemes = this.buildReflectionWallThemes(lsThemes, lsFirst || null, lsUser, lsMine, lsFirstHC);
      ...
      if (myReflection && myReflection.responseId !== cachedReflection?.responseId) {
        const updatedWallThemes = await this.resolveReflectionWallThemes(
          communityThemes,
          { first_response: mergedFirst, user_name: mergedUser, firstResponseHeartCount: mergedFirstHC },
          myReflection
        );
    ...
    patchReflectionArchiveEntryInDom(dateKey, entry) {
      ...
      if (details) {
        ...
        const detailsBottomAnchor =
          () =>
            details.querySelector('.reflection-themes-archive-quote-clipping') ||
            details.querySelector('.reflection-themes-archive-date');
        ...
        if (quiltUrl) {
          if (!img) {
            ...
            const anchor = detailsBottomAnchor();
        ...
        if (clippingUrl) {
          ...
          if (clippingFrame) {
            ...
            if (node) {
              if (dateEl) details.insertBefore(node, dateEl);
    ...
    _quoteSpeakerCutoutSubjectFraming(naturalW, naturalH, boxW, subjectBounds) {
      ...
      const offsetX = -subjectBounds.minX * scale;
      const padTop = 8;
      const downNudge = 28;
    ...
    _syncQuoteSpeakerNameTuck(cutout, img) {
      ...
      const boxW = Math.max(
        1,
        Math.round(cutout.clientWidth || wrap?.clientWidth || 0)
      );
    ...
    _applyQuoteSpeakerCutoutAnchor(imageWrap, img) {
      ...
      const plainComposite =
        imageWrap.classList.contains('quote-speaker-cutout--plain-composite') ||
        imageWrap.querySelector('.quote-speaker-image')?.dataset?.plainCutoutComposite === '1';
      const opaqueBounds = this._quoteSpeakerMeasureOpaqueBounds(img);
      const subjectBounds =
        plainCutout && !plainComposite ? this._quoteSpeakerMeasureSubjectBounds(img) : null;
      const layoutBounds = subjectBounds || opaqueBounds;
    ...
    _resolveQuoteSpeakerPlainCutoutBakeLayout(cutout, sourceImg) {
      ...
      const boxW = Math.max(
        1,
        Math.round(wrap?.clientWidth || cutout.clientWidth || 0)
      );
    ...
    _applyQuoteSpeakerPlainCutoutBacking(cutout, img, sourceUrl = '') {
      ...
      const paperSig = `${layoutSig}|${paper}|layout-b-story-v4-warmtape`;
      const compositeReady =
        portraitImg?.dataset?.plainCutoutComposite === '1' &&
        /^data:image\//i.test(String(portraitImg.currentSrc || portraitImg.src || ''));
      if (cutout._plainCutoutPaperSig === paperSig && compositeReady) return;

      if (!bakeLayout) {
        if (portraitImg && remote && !compositeReady) {
          portraitImg.hidden = false;
          portraitImg.classList.add('is-loaded');
      ...
      if (portraitImg) {
        ...
        portraitImg.hidden = false;
        portraitImg.classList.add('is-loaded');
        if (!compositeReady && remote && portraitImg.src !== remote) portraitImg.src = remote;
      }
      cutout.style.setProperty('--speaker-image-url', 'none');

      const { drawW, drawH, compositeLayout, subjectCrop } = bakeLayout;
      ...
      const resolveSourceImg = async () => {
        ...
        if (/^https?:\/\//i.test(loadUrl) && this._quoteSpeakerShouldStripFringe(loadUrl)) {
          const cleaned = await this._quoteSpeakerCleanCutoutDisplayUrl(loadUrl);
          if (cleaned && cleaned !== loadUrl) {
            loadUrl = cleaned;
            cutout._plainCutoutSourceUrl = cleaned;
          }
        }
        if (
        ...
        if (portraitImg?.naturalWidth && portraitImg.dataset.plainCutoutComposite !== '1') {
          const cur = String(portraitImg.currentSrc || portraitImg.src || '').trim();
          if (cur === loadUrl) {
        ...
        if (img?.naturalWidth && img !== portraitImg) {
          const cur = String(img.currentSrc || img.src || '').trim();
          if (cur === loadUrl) {
      ...
      const applyLithographComposite = async (sourceImg) => {
        if (paperGen !== cutout._plainCutoutPaperGen || cutout.hidden || !portraitImg) return;
        const srcImg = sourceImg || (img?.naturalWidth ? img : null) || (portraitImg?.naturalWidth ? portraitImg : null);
        if (!srcImg?.naturalWidth) {
          this._showQuoteSpeakerPlainCutoutFallback(cutout, portraitImg);
          return;
        }
        if (!this._quoteSpeakerCanCanvasProcessImage(srcImg, cutout, portraitImg)) {
          this._showQuoteSpeakerPlainCutoutFallback(cutout, portraitImg);
          return;
        }
        const SCR = globalThis.SpeakerCutoutRender;
        const cutoutSeed = this._quoteSpeakerPlainCutoutPaperSeed();
        const compositeUrl = await SCR?.buildSpeakerPlainCutoutCompositeDataUrlAsync?.(
          srcImg,
          drawW,
          drawH,
          compositeLayout,
        ...
        if (paperGen !== cutout._plainCutoutPaperGen || cutout.hidden) return;
        if (!compositeUrl || compositeUrl.length < 500) {
          this._showQuoteSpeakerPlainCutoutFallback(cutout, portraitImg);
          return;
        }
        cutout.classList.remove('quote-speaker-cutout--plain-fallback');
        portraitImg.src = compositeUrl;
        portraitImg.dataset.plainCutoutComposite = '1';
        portraitImg.hidden = false;
        portraitImg.classList.add('is-loaded');
        portraitImg.style.removeProperty('opacity');
        ...
        await new Promise((resolve) => {
          if (portraitImg.complete && portraitImg.naturalWidth) resolve();
          else portraitImg.addEventListener('load', () => resolve(), { once: true });
        });
        if (paperGen !== cutout._plainCutoutPaperGen || cutout.hidden) return;
        requestAnimationFrame(() => {
          if (!cutout.hidden && portraitImg?.naturalWidth) {
            const layoutSource = this._quoteSpeakerPlainCutoutSourceImg(cutout, sourceImg) || sourceImg;
      ...
      void resolveSourceImg()
        .then((sourceImg) => applyLithographComposite(sourceImg))
        .catch(() => {
          this._showQuoteSpeakerPlainCutoutFallback(cutout, portraitImg);
        });
    }
    ...
    async _resolveCollageQuoteFast(dk, quotePayload, { canComposeQuote = false } = {}) {
      ...
      if (dateKey) {
        ...
        const nightly = await this.resolveNightlyNewspaperClippingUrl(dateKey, {
          waitForFirebaseMs: 900
        });
        if (nightly) {
          return { quoteDataUrl: nightly, quoteDisplayWidthPx: 0, composed: null, source: 'nightly' };
        }
      }
    ...
    _stampQuiltMoodSpreadQuoteMeta(spreadHost, quoteDataUrl, composed) {
    ...
    async _resolveQuiltMoodSpreadQuoteUrl(
      ...
      return { quoteDataUrl, quoteDisplayWidthPx, composed, source };
    }
    ...
    async _refreshQuiltMoodCollageSpread({
      ...
      this._stampQuiltMoodSpreadQuoteMeta(spreadHost, quoteDataUrl, composed);
      const spreadApi = this._quiltMoodSpreadApi(spreadHost);
