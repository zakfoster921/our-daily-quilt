...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    /** Row sizes for IG capture grid (1–6 highlighted responses). */
    ...
    _ensureIgReflectionPlaywrightCaptureStyles(responseCount = 3) {
      ...
      const gridGap = responseCount >= 5 ? '0.55rem' : responseCount >= 3 ? '0.72rem' : '0.85rem';
    ...
    _syncIgReflectionPlaywrightCaptureLayout(captureScreen, dateKey) {
      ...
      if (stack) {
        ...
        stack.style.minHeight = `${this._igReflectionCaptureStackTargetHeight(responseSlides.length || 1)}px`;
      ...
      gridRows.forEach((rowEl) => {
        ...
        if (!n) return;
        if (n === 1) {
          const soloRatio = responseSlides.length === 1 ? 0.86 : responseSlides.length <= 3 ? 0.78 : 0.68;
      ...
      gridRows.forEach((rowEl) => {
        ...
        this.syncReflectionCarouselJoinTape(rowSlides.length >= 2 ? rowSlides : []);
      });
      this.syncReflectionCarouselSplitTape(slides);
      this.syncReflectionCarouselHandCuts(slides, dk);
      this.syncReflectionCarouselCopyAlign(slides);
    ...
    async prepareIgReflectionSlidePlaywrightCapture(options = {}) {
      ...
      const gridRowsHtml = rowSizes
        .map((rowCount) => {
          ...
          return `<div class="${rowClasses.join(' ')}" data-reflection-carousel-track>${rowPatches.join('')}</div>`;
        })
        .join('');
              ...
              <div class="quilt-reflection-theme-cards-shell">
                <div class="quilt-reflection-wall-notes">
                  <div class="quilt-reflection-ig-capture-stack">
                    ${promptSlide}
                    <div class="quilt-reflection-ig-capture-grid quilt-reflection-carousel" data-reflection-carousel data-reflection-hand-cut-date-key="${escapedDateKey}">
                      ${gridRowsHtml}
                    </div>
                  </div>
                </div>
