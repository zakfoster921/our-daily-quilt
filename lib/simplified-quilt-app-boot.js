...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    _igReflectionExportColumnWidth(exportViewportWidth) {
      return Math.max(1, (exportViewportWidth || this._igReflectionExportViewportWidth()) - 56);
    }
    ...
    async _igReflectionCardsLayerFromPngBase64(b64, sizeHints = {}) {
      ...
      return new Promise((resolve) => {
        ...
        img.onload = () => {
          ...
          resolve({
            canvas,
            logicalWidth: hintW > 0 ? hintW : Math.max(1, Math.round(w / dpr)),
            logicalHeight: hintH > 0 ? hintH : Math.max(1, Math.round(h / dpr))
          });
        };
    ...
    _ensureIgReflectionPlaywrightCaptureStyles() {
      const exportW = this._igReflectionPlaywrightCaptureViewportWidth();
      const columnW = Math.max(1, exportW - 28);
      const patchBodyPx = Math.max(36, Math.round(this._igReflectionExportPatchBodyPx() * 0.82));
      ...
      style.textContent = `
        ...
        #screen-quilt.is-ig-reflection-playwright-capture .quilt-reflection-carousel {
        ...
        #screen-quilt.is-ig-reflection-playwright-capture .quilt-reflection-carousel-viewport {
          overflow: visible !important;
          scroll-snap-type: none !important;
          cursor: default !important;
        }
        #screen-quilt.is-ig-reflection-playwright-capture .quilt-reflection-carousel-track {
    ...
    _syncIgReflectionPlaywrightCaptureLayout(captureScreen, dateKey) {
      const dk = String(dateKey || Utils.getTodayKey() || 'nodate').trim() || 'nodate';
      const columnW = Math.max(1, this._igReflectionPlaywrightCaptureViewportWidth() - 28);
      const stack = captureScreen.querySelector('.quilt-reflection-ig-capture-stack');
      const carousel = captureScreen.querySelector('[data-reflection-carousel]');
      const viewport = carousel?.querySelector('[data-reflection-carousel-viewport]');
      const track = carousel?.querySelector('[data-reflection-carousel-track]');
      ...
      if (track && viewport) {
        ...
        const naturalW = Math.max(track.scrollWidth, track.getBoundingClientRect().width);
        if (naturalW > columnW + 2) {
          const scale = columnW / naturalW;
          track.style.transform = `scale(${scale})`;
          track.style.transformOrigin = 'left bottom';
          const scaledH = Math.ceil(track.offsetHeight * scale);
          viewport.style.height = `${scaledH}px`;
          viewport.style.minHeight = `${scaledH}px`;
          track.style.marginBottom = `${Math.ceil(track.offsetHeight * (scale - 1))}px`;
        } else {
    ...
    async prepareIgReflectionSlidePlaywrightCapture(options = {}) {
      ...
      this._ensureIgReflectionPlaywrightCaptureStyles();
      ...
      const carouselMarkup = this.getReflectionCarouselMarkup(ideas, null, {
        omitInviteSlide: true,
        dateKey
      });
      host.innerHTML = `
      ...
      const root = captureScreen.querySelector('[data-ig-reflection-capture-root]') || captureScreen;
      const rect = root.getBoundingClientRect();
      this._igReflectionPlaywrightCaptureActive = true;

      return {
        selector: '#ig-reflection-playwright-capture-host [data-ig-reflection-capture-root]',
        hostSelector: '#ig-reflection-playwright-capture-host',
        logicalWidth: Math.max(1, Math.round(rect.width) || this._igReflectionPlaywrightCaptureViewportWidth()),
