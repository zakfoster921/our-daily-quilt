...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    setupEventListeners() {
      ...
      const showIntroZakAfterFirstName = () => {
      ...
      const setupIntroPersonaTapAdvance = () => {
        ...
        const quoteBridge = document.getElementById('screen-first-quote-bridge');
        if (quoteBridge && !quoteBridge.dataset.introTapBound) {
          quoteBridge.dataset.introTapBound = '1';
          quoteBridge.addEventListener('click', (e) => {
            if (!quoteBridge.classList.contains('active')) return;
            e.preventDefault();
            this.advanceFromFirstQuoteBridge();
          });
        }
      };
    ...
    _quoteSpeakerPlainCutoutLayoutSig(cutout) {
      if (!cutout) return '';
      const read = (name) => String(cutout.style.getPropertyValue(name) || '').trim();
      return [
        read('--speaker-cutout-bg-w'),
        read('--speaker-cutout-bg-h'),
        read('--speaker-cutout-pos-x'),
        read('--speaker-cutout-pos-y'),
        String(cutout.style.height || cutout.clientHeight || '')
      ].join('|');
    }
    ...
    _applyQuoteSpeakerPlainCutoutBacking(cutout, img, sourceUrl = '') {
      ...
      const layoutSig = this._quoteSpeakerPlainCutoutLayoutSig(cutout);
      ...
      const subjectCrop = cutout._speakerSubjectCrop || null;
      const boxW = Math.max(1, Math.round(cutout.clientWidth || 0));
      const frameH = Math.max(
        1,
        Math.round(parseFloat(cutout.style.height) || cutout.clientHeight || 0)
      );
      const drawW = subjectCrop ? boxW : parsePxVar('--speaker-cutout-bg-w', cutout.clientWidth || 168);
      const drawH = subjectCrop
        ? Math.max(frameH, parsePxVar('--speaker-cutout-bg-h', frameH || 200))
        : parsePxVar('--speaker-cutout-bg-h', cutout.clientHeight || 200);
      const compositeLayout = subjectCrop ? 'subject-fill' : 'width-fill';
      ...
      const applyLithographComposite = async (sourceImg) => {
        ...
        requestAnimationFrame(() => {
          if (!cutout.hidden && portraitImg?.naturalWidth) {
            this._applyQuoteSpeakerCutoutAnchor(cutout, portraitImg);
          }
        });
      };
    ...
    _moodSpreadOwnsQuoteUi() {
      ...
      if (document.getElementById('quiltMoodDuo')?.classList.contains('is-duo-ready')) return true;
      if (!host) return false;
      if (host.classList.contains('is-ready') || host.classList.contains('is-layer-ready')) return true;
      if (host._moodSpreadWidget || host._moodTriptychWidget || host._moodCollageWidget) return true;
      if (host._moodSpreadComposePending) return true;
      const src = this._quiltMoodSpreadQuoteImg(host)?.src;
