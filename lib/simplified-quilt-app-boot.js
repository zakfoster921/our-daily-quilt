...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    /** Plain cutout: bake IG slide-1 collage (colored paper + lithograph multiply). */
    ...
    /** Match Layout B carousel slide 1 / feed slide 1 speaker cutout bake options. */
    _quoteSpeakerSlide1CutoutBakeOptions(bakeLayout = {}) {
      ...
      const igPaper =
        SCR?.resolveIgCollagePaperColor?.({
          palette,
          washColor: cardColors.washColor
        }) ||
        this.getPaleQuiltPaperColor?.() ||
        '#f2eee6';
    ...
    _applyQuoteSpeakerPlainCutoutBacking(cutout, img, sourceUrl = '') {
      ...
      const { igPaper } = this._quoteSpeakerSlide1CutoutBakeOptions(bakeLayout || {});
      ...
      const paperSig = `${layoutSig}|${igPaper}|slide1-ig-collage-v3`;
      ...
      const applySlide1CutoutDisplay = async (sourceImg) => {
        ...
        if (
          !SCR?.buildSpeakerPlainCutoutCompositeDataUrlAsync ||
          !this._quoteSpeakerCanCanvasProcessImage(bakeImg, cutout, portraitImg)
        ) {
          applyPlainCutoutDirectFallback(srcImg);
          return;
        }
        const { options } = this._quoteSpeakerSlide1CutoutBakeOptions(bakeLayout);
        let dataUrl = '';
        try {
          dataUrl = await SCR.buildSpeakerPlainCutoutCompositeDataUrlAsync(
            bakeImg,
            bakeLayout.drawW,
            bakeLayout.drawH,
