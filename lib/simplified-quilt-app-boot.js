...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    formatReflectionCarouselStripHtml(strip, dateKey) {
      ...
      const heartMarkup = heartId
        ...
        : '';
      return `<div class="quilt-reflection-carousel-split-strip">
    ...
    formatReflectionCarouselTextHtml(entry) {
      ...
      if (normalized?.split && Array.isArray(normalized.strips)) {
        ...
        const strips = normalized.strips
          .map((strip) => this.formatReflectionCarouselStripHtml(strip, dateKey))
          .join('<div class="quilt-reflection-carousel-split-divider" aria-hidden="true"></div>');
        return `<div class="quilt-reflection-carousel-split">${strips}</div>`;
      }
    ...
    /** Hand-cut vertical tape scrap — angled top/bottom, occasional side nick. */
    _buildReflectionJoinTapeScrapClipPath(seedKey = '') {
      ...
      const topLeftX = 2 + rnd() * 12;
      const topRightX = 86 + rnd() * 12;
      const topLeftY = rnd() * 11;
      const topRightY = rnd() * 11;
      const botLeftX = 1 + rnd() * 14;
      const botRightX = 84 + rnd() * 14;
      const botLeftY = 88 + rnd() * 11;
      const botRightY = 88 + rnd() * 11;
      const pts = [
        `${pct(topLeftX)} ${pct(topLeftY)}`,
        `${pct(topRightX)} ${pct(topRightY)}`
      ];
      if (rnd() > 0.42) {
        pts.push(`${pct(52 + (rnd() - 0.5) * 18)} ${pct(rnd() * 4)}`);
      }
      pts.push(`${pct(botRightX)} ${pct(botRightY)}`, `${pct(botLeftX)} ${pct(botLeftY)}`);
      if (rnd() > 0.55) {
        pts.splice(2, 0, `${pct(96 + rnd() * 3)} ${pct(38 + rnd() * 24)}`);
      } else if (rnd() > 0.5) {
        pts.push(`${pct(rnd() * 4)} ${pct(42 + rnd() * 20)}`);
      }
