...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    /** Shrink IG prompt type until it fits the notebook body box. */
    _syncIgReflectionQuestionPaperExportType(scope) {
      ...
      promptEl.style.maxWidth = '100%';
      promptEl.style.textAlign = 'center';
      promptEl.style.margin = '0';

      let bestPx = minPx;
      for (let px = maxPx; px >= minPx; px -= 1) {
        promptEl.style.fontSize = `${px}px`;
        promptEl.style.lineHeight = String(lineHeight);
        const overflowY = promptEl.scrollHeight - body.clientHeight;
        const overflowX = promptEl.scrollWidth - body.clientWidth;
        if (overflowY <= 2 && overflowX <= 2) {
          bestPx = px;
          break;
        }
      }
      promptEl.style.fontSize = `${bestPx}px`;
      promptEl.style.lineHeight = String(lineHeight);
    ...
    _ensureIgReflectionExportStyles() {
      ...
      style.textContent = `
        ...
        #screen-quilt.is-ig-export {
          ...
          --reflection-question-card-width: min(100%, 500px);
          --reflection-card-inset-top: 15%;
          --reflection-card-inset-bottom: 11%;
          --reflection-card-inset-x: 7%;
    ...
    _syncIgReflectionCaptureClone(captureClone, dateKey) {
      ...
      if (paperBody && promptEl) {
        ...
        const height = promptEl.getBoundingClientRect().height;
    ...
    async _captureIgReflectionExportElement(sourceEl, dateKey) {
      ...
      if (root.classList.contains('quilt-reflection-carousel-slide--clipping')) {
        ...
        root.style.transform = srcCs.transform;
        root.style.margin = '0';
