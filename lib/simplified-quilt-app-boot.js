...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    _quoteSpeakerCutoutSubjectFraming(naturalW, naturalH, boxW, subjectBounds) {
      ...
      const padTop = 8;
      const tuckBand = Math.max(14, Math.round(sh * scale * 0.1));
      const frameH = Math.max(120, Math.ceil(padTop + sh * scale + tuckBand));
      /** Top-align subject ink — bottom-aligning the full PNG windowed the face behind name strips. */
      const offsetY = padTop - subjectBounds.minY * scale;
    ...
    _applyQuoteSpeakerPlainCutoutBacking(cutout, img, sourceUrl = '') {
      ...
      const paperSig = `${layoutSig}|${bakeKey}|story-over-quilt-v4`;
      const compositeReady =
