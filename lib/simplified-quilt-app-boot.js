    ...
    setupLayoutBStoryPreviewLongPressShare() {
      ...
      const canShareLayoutBStoryPreview = () =>
        !!(
          ...
          (this._layoutBPreviewObjectUrl && this._layoutBPreviewOverlayObjectUrl)
        );

      sheet.addEventListener('pointerdown', () => {
        const img = document.getElementById('quiltLayoutBPreviewImg');
        if (!img || img.hidden || !canShareLayoutBStoryPreview()) return;
        clearShareTimer();
        const prepPromise = this._ensureLayoutBStoryPreviewShareBlob?.();
        shareTimer = setTimeout(async () => {
          shareTimer = null;
          try {
            let shareBlob = null;
            ...
            if (!shareBlob && prepPromise) {
              shareBlob = await prepPromise;
            }
            if (!shareBlob) {
