...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    _applyQuoteSpeakerCutoutFrameHeight(imageWrap, img, boxW, bounds) {
    ...
    _applyQuoteSpeakerCutoutAnchor(imageWrap, img) {
      ...
      const subjectBounds = imageWrap.classList.contains('quote-speaker-cutout--plain-cutout')
      ...
      this._applyQuoteSpeakerCutoutFrameHeight(imageWrap, img, boxW, bounds);
    ...
    _applyQuoteSpeakerIdentityTuck(cutout, wrapRect, cutoutRect, wrapHeightPx) {
      ...
      const bounds = cutout._speakerOpaqueBounds;
      const offsetY = parseFloat(String(cutout.style.getPropertyValue('--speaker-cutout-pos-y') || '0')) || 0;
      const scale = cutoutRect.width / Math.max(1, img?.naturalWidth || 1);
    ...
    _quoteSpeakerAttributionVisualBottomY(img, boxW, boxH, bounds, cutout) {
      const { offsetY, drawH, scale } = this._quoteSpeakerImageContainRect(
        img.naturalWidth,
        img.naturalHeight,
        boxW,
        boxH,
    ...
    _ensureQuoteSpeakerPlainCutoutPaperShell(cutout, neutralSilhouette = null) {
      ...
      const widget = document.getElementById('quoteSpeakerWidget');
      const paper = this.getPaleQuiltPaperColor?.() || '#f6f4f1';
    ...
    _applyQuoteSpeakerPlainCutoutBacking(cutout, img, sourceUrl = '') {
      ...
      const paper = shell?.paper || this.getPaleQuiltPaperColor?.() || '#f6f4f1';
      ...
      const paperSig = `${layoutSig}|${paper}|bw-litho-v2`;
    ...
    _buildQuiltMoodSpreadMountOpts({
      ...
      const mountOpts = {
        ...
        instant: mountMoodFinal === 'good' || mountMoodFinal === 'rough',
