  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    async loadDeferredAdminSlice() {
      ...
      const SliceCtor = await this._loadDeferredScript(
        'lib/simplified-quilt-app-admin.js?v=43',
    ...
    _normalizeAdminDailyTasks(data = {}) {
      return {
        ...
        igPostCompletedAtIso: String(data.igPostCompletedAtIso || '').trim(),
        igPostClearedAtIso: String(data.igPostClearedAtIso || '').trim(),
        igPostSource: String(data.igPostSource || '').trim()
      };
    }
    ...
    _mergeAdminDailyTasks(...sources) {
      ...
      for (const source of sources) {
        ...
        if (next.igPostCompletedAtIso) {
          merged.igPostCompletedAtIso = next.igPostCompletedAtIso;
        }
        if (next.igPostClearedAtIso) {
          merged.igPostClearedAtIso = next.igPostClearedAtIso;
        }
        if (next.igPostSource) {
          merged.igPostSource = next.igPostSource;
        }
      }
      // Remote clear beats a sticky local complete when the clear is newer.
      if (
        merged.igPostClearedAtIso &&
        (!merged.igPostCompletedAtIso || merged.igPostClearedAtIso >= merged.igPostCompletedAtIso)
      ) {
        merged.igPostCompleted = false;
      }
      return merged;
    }
    ...
    _adminDailyTaskPushPatch(tasks = {}) {
      ...
      const patch = {};
      if (normalized.igPostCompleted) {
        patch.igPostCompleted = true;
        // Provenance required server-side (blocks stale tune/push + bare sticky sync).
        if (normalized.igPostSource) patch.igPostSource = normalized.igPostSource;
        if (normalized.igPostCompletedAtIso) {
          patch.igPostCompletedAtIso = normalized.igPostCompletedAtIso;
        }
      }
    ...
    async _readAdminDailyTasksFromFirestore(dateKey) {
      ...
      try {
        ...
        return this._normalizeAdminDailyTasks({
          ...data,
          // Explicit false from a clear must survive normalize sticky merges.
          igPostClearedAtIso: data.igPostClearedAtIso || '',
          igPostSource: data.igPostSource || ''
        });
      } catch (error) {
    ...
    _setupAdminDailyTaskBannerHandlers() {
      ...
      banner.addEventListener(
        ...
        (e) => {
          ...
          } else if (action === 'markIgPost') {
            void this._handleAdminDailyTaskIgPostTap();
          }
        },
        { capture: true }
      );
    ...
    async _handleAdminDailyTaskIgPostTap() {
      ...
      const local = this._readLocalAdminDailyTasks(dateKey);
      if (local.igPostCompleted) {
        this._renderAdminDailyTaskBanner(local);
        return;
      }
      await this.markAdminDailyTaskCompleted?.('igPost', { source: 'admin_banner_tap' });
    }
    ...
    async ensureLayoutBComposeReady() {
      ...
      await this._loadDeferredScript('lib/layout-b-compose.js?v=190', 'composeInstagramLayoutBFromQuiltBlob');
    ...
    _refreshQuiltQuoteUiAfterPin(beforeQuote) {
      ...
      // Episode catalog often lands after the first quote paint. Always retry the player
      // even when the quote itself did not change (submitted_via already SEAMSIDE).
    ...
    _ensureQuiltLowerSections(options = {}) {
      ...
      if (phase === 'bootstrap') {
        ...
        // Paint scrap / speaker / podcast chrome from cache during portal dwell — do not wait for quilt render.
        this.refreshQuiltReflectionScrapWidget?.(quote);
        this.refreshQuoteSpeakerWidget?.(quote);
    ...
    _scheduleStoryPreviewOnce() {
      ...
      // Hidden = feature off; mark scheduled so callers stop retrying.
      if (!wrap || wrap.hidden) {
    ...
    /** Dev/preview: reset local first-time state (?firstTime=1, ?firstTimeColor=1, ?firstTime=color, …). */
    _firstTimeColorPickerPreviewRequestedFromQuery() {
      try {
        const params = new URLSearchParams(window.location.search || '');
        const alias = String(params.get('firstTimeColor') || params.get('firstTimePicker') || '')
          .trim()
          .toLowerCase();
        if (['1', 'true', 'yes'].includes(alias)) return true;
        const firstTime = String(params.get('firstTime') || params.get('firstVisit') || '')
          .trim()
          .toLowerCase();
        return firstTime === 'color' || firstTime === 'picker';
      } catch (_) {
        return false;
      }
    }

    _firstTimePreviewRequestedFromQuery() {
      try {
        if (this._firstTimeColorPickerPreviewRequestedFromQuery()) return true;
        const params = new URLSearchParams(window.location.search || '');
        const raw = String(params.get('firstTime') || params.get('firstVisit') || '')
          .trim()
          .toLowerCase();
        if (['1', 'true', 'yes', 'fresh', 'reset'].includes(raw)) return true;
        const screen = String(params.get('screen') || params.get('preview') || '').trim().toLowerCase();
        return (
          screen === 'first-time' ||
          screen === 'firsttime' ||
          screen === 'first-visit' ||
          screen === 'firstvisit' ||
          screen === 'first-time-color' ||
          screen === 'firsttimecolor'
        );
      } catch (_) {
        return false;
      }
    ...
    _applyFirstTimePreviewFromQuery() {
      if (!this._firstTimePreviewRequestedFromQuery()) return;
      if (typeof this.resetFirstTimeUserState === 'function') {
        this.resetFirstTimeUserState();
      }
      try {
        localStorage.setItem('ourDailyVisitCount', '0');
      } catch (_) {
        /* ignore */
      }
      this._firstTimePreviewFromQuery = true;
      if (this._firstTimeColorPickerPreviewRequestedFromQuery()) {
        this._previewQuoteColorPicker = true;
      }
      this.logger?.log?.('First-time preview dev override active');
    }

    /** Dev/preview: jump to quote color picker (?screen=color, ?color=1, ?firstTimeColor=1, …). */
    _previewQuoteColorPickerRequestedFromQuery() {
      try {
        if (this._firstTimeColorPickerPreviewRequestedFromQuery()) return true;
        const params = new URLSearchParams(window.location.search || '');
        const raw = String(params.get('screen') || params.get('preview') || '').trim().toLowerCase();
        if (['color', 'color-picker', 'colorpicker', 'picker', 'first-time-color', 'firsttimecolor'].includes(raw)) {
          return true;
        }
        const colorFlag = String(params.get('color') || params.get('colorPicker') || '')
          .trim()
          .toLowerCase();
        return colorFlag === '1' || colorFlag === 'true' || colorFlag === 'yes';
      } catch (_) {
        return false;
      }
    }

    _activatePreviewQuoteColorPickerIfNeeded() {
      if (!this._previewQuoteColorPicker) return;
      const run = () => {
        ...
        if (typeof this.revealQuoteColorPickerPane === 'function') {
          this.revealQuoteColorPickerPane();
          return;
        }
        if (typeof this.scheduleQuoteColorPickerReveal === 'function') {
          this.scheduleQuoteColorPickerReveal(0);
        }
      };
    ...
    _getPreviewScreenIdFromQuery() {
      try {
        const params = new URLSearchParams(window.location.search || '');
        this._previewQuoteColorPicker = this._previewQuoteColorPickerRequestedFromQuery();
        ...
        const raw = String(params.get('screen') || params.get('preview') || '').trim().toLowerCase();
        if (!raw && !this._previewQuoteColorPicker) return null;
        if (this._previewQuoteColorPicker && (!raw || ['color', 'color-picker', 'colorpicker', 'picker'].includes(raw))) {
          return document.getElementById('screen-quote') ? 'screen-quote' : null;
        }
        if (!raw) return null;
        const map = {
          ...
          quote: 'screen-quote',
          color: 'screen-quote',
          'color-picker': 'screen-quote',
          colorpicker: 'screen-quote',
          picker: 'screen-quote',
        ...
        if (
          ...
          (this._previewQuoteColorPicker ||
            ['color', 'color-picker', 'colorpicker', 'picker'].includes(raw))
        ) {
          this._previewQuoteColorPicker = true;
        }
        return screenId;
    ...
    _adminMenuPreviewRequestedFromQuery() {
      try {
        const params = new URLSearchParams(window.location.search || '');
        const adminMenu = String(params.get('adminMenu') || '').trim().toLowerCase();
        if (['1', 'true', 'yes', 'open'].includes(adminMenu)) return true;
        const admin = String(params.get('admin') || '').trim().toLowerCase();
        if (['1', 'true', 'yes', 'open', 'menu'].includes(admin)) return true;
        const screen = String(params.get('screen') || params.get('preview') || '').trim().toLowerCase();
        return screen === 'admin' || screen === 'admin-menu';
      } catch (_) {
        return false;
      }
    }

    _applyAdminMenuPreviewFromQuery() {
      if (!this._adminMenuPreviewRequestedFromQuery()) return;
      this._previewAdminMenuOpen = true;
      if (CONFIG.APP.enableAdminTools) {
        void this.loadDeferredAdminSlice?.();
      }
      this.logger?.log?.('Admin menu preview query active');
    }

    async _activateAdminMenuPreviewIfNeeded() {
      if (!this._previewAdminMenuOpen || this._previewAdminMenuActivated) return;
      if (!CONFIG.APP.enableAdminTools) {
        this.logger?.warn?.('Admin menu preview ignored: admin tools disabled on this host');
        this._previewAdminMenuActivated = true;
        return;
      }
      const adminReady = await this.loadDeferredAdminSlice?.().catch(() => false);
      if (!adminReady || typeof this.showAdminMenu !== 'function') {
        this.logger?.warn?.('Admin menu preview waiting for admin slice');
        return;
      }
      ...
      if (localhostDev && !this.isCurrentUserAdmin?.()) {
    ...
    _reconcileMirrorTuneFromRemotePayload(dateKey, payload) {
      ...
      try {
        ...
        if (QuiltMirrorLayout.odqMirrorTuneSnapshotsEqual(remoteTune, localTune)) return;

        const remoteAt = String(remoteTune?.updatedAt || payload.mirrorTuneUpdatedAt || '').trim();
        const localAt = String(localTune?.updatedAt || '').trim();
        const remoteIsNewer =
          typeof QuiltMirrorLayout.odqIsoTimestampIsNewer === 'function'
            ? QuiltMirrorLayout.odqIsoTimestampIsNewer(remoteAt, localAt)
            : Date.parse(remoteAt) > Date.parse(localAt);
        const localCustom =
          typeof QuiltMirrorLayout.odqMirrorTuneIsCustomized === 'function' &&
          QuiltMirrorLayout.odqMirrorTuneIsCustomized(localTune);
        // Never clobber a customized local tune with an older/equal remote snapshot.
        if (!remoteIsNewer && localCustom) return;

        // Dup ×4 per-tile fields: if remote payload omits them, synthesized defaults
        // must not wipe a local quad tune that already has explicit tile settings.
        const remoteIsQuad =
          String(remoteTune?.bottomLayout || '').trim() === 'quadSideBySide';
        const payloadHasQuadTiles = ['TL', 'TR', 'BL', 'BR'].some((id) =>
          Object.prototype.hasOwnProperty.call(payload, `mirrorQuad${id}FlipX`) ||
          Object.prototype.hasOwnProperty.call(payload, `mirrorQuad${id}FlipY`) ||
          Object.prototype.hasOwnProperty.call(payload, `mirrorQuad${id}NudgeX`) ||
          Object.prototype.hasOwnProperty.call(payload, `mirrorQuad${id}NudgeY`)
        );
        if (
          remoteIsQuad &&
          !payloadHasQuadTiles &&
          String(localTune?.bottomLayout || '').trim() === 'quadSideBySide' &&
          localCustom
        ) {
          return;
        }

        QuiltMirrorLayout.odqWriteMirrorTuneLocal(key, {
          ...remoteTune,
          updatedAt: remoteAt || localAt
        });
    ...
    async applyQuiltDataFromPayload(data, options = {}) {
      ...
      if (data.blocks && data.blocks.length > 0) {
        ...
        if (this.quiltEngine.blocks.length === 0) {
          ...
          this.quiltEngine.setQuiltDateKey?.(incomingDateKey);
          this.quiltEngine.hydrateMacroLayoutFromPersistence?.(data.macroLayoutMode);
    ...
    async _continueAfterLiveSync() {
      ...
      if (this._previewAdminMenuOpen) {
        const tryOpenAdminMenuPreview = () => {
          void this._activateAdminMenuPreviewIfNeeded();
        };
        tryOpenAdminMenuPreview();
        window.setTimeout(tryOpenAdminMenuPreview, 450);
        window.setTimeout(tryOpenAdminMenuPreview, 1800);
      }
      if (rememberViewDateKey) {
        ...
        if (previewScreenId) {
          ...
          if (previewScreenId === 'screen-quote' && this._previewQuoteColorPicker) {
            this._activatePreviewQuoteColorPickerIfNeeded();
          }
    ...
    async initialize() {
      ...
      try {
        ...
        this._applyAdminMenuPreviewFromQuery();
        this._applyFirstTimePreviewFromQuery();
    ...
    setupEventListeners() {
      ...
      document.addEventListener('screenChange', (e) => {
        ...
        if (e?.detail?.screenId === 'screen-quilt') {
          // Cut-reveal handoff: skip admin pill + footer paper work — they compete
          // with the last animation frame and make the mat feel frozen.
          if (e?.detail?.fastQuiltHandoff === true) {
            this.refreshRememberTodayFooterVisibility?.();
          } else {
            this.refreshRememberTodayFooterVisibility();
      ...
      const introSkipInteractive =
        'button, a, input, textarea, select, label, [data-next], [data-intro-next], [role="button"]';
      const setupFirstVisitIntroTapSkip = () => {
        const portalEl = document.getElementById('screen-portal');
        if (portalEl && !portalEl.dataset.introSkipBound) {
          portalEl.dataset.introSkipBound = '1';
          portalEl.addEventListener('click', (e) => {
            if (!portalEl.classList.contains('active')) return;
            if (e.target.closest(introSkipInteractive)) return;
            this.skipPortalIntroDwell?.();
          });
        }

        const welcomeEl = document.getElementById('screen-welcome');
        if (welcomeEl && !welcomeEl.dataset.introSkipBound) {
          welcomeEl.dataset.introSkipBound = '1';
          welcomeEl.addEventListener('click', (e) => {
            if (!welcomeEl.classList.contains('active')) return;
            if (e.target.closest(introSkipInteractive)) return;
            this.snapWelcomeIntroFadeIns?.();
            void this.goToQuoteWithColorPicker?.();
          });
        }

        const bridgeEl = document.getElementById('screen-first-quote-bridge');
        if (bridgeEl && !bridgeEl.dataset.introSkipBound) {
          bridgeEl.dataset.introSkipBound = '1';
          bridgeEl.addEventListener('click', (e) => {
            if (!bridgeEl.classList.contains('active')) return;
            if (e.target.closest(introSkipInteractive)) return;
            void this.goToQuoteWithColorPicker?.();
          });
        }
        ...
        if (quoteEl && !quoteEl.dataset.introSkipBound) {
          quoteEl.dataset.introSkipBound = '1';
          quoteEl.addEventListener('click', (e) => {
            if (!quoteEl.classList.contains('active')) return;
            if (this.isColorPickerActive?.()) return;
            if (
              e.target.closest(
                '#quoteColorPickerPane, button, a, input, textarea, select, label, [role="button"]'
              )
            ) {
              return;
            }
            this.skipQuoteColorPickerRevealWait?.();
          });
        }
      };
      setupFirstVisitIntroTapSkip();

      if (firstNameContinueBtn) {
        firstNameContinueBtn.addEventListener('click', async (e) => {
          e.preventDefault();
          ...
          if (t) {
            if (typeof Utils.isPendingFriendTermInputValue === 'function' && Utils.isPendingFriendTermInputValue(t)) {
              ...
              if (typeof Utils.validateContributorName === 'function') {
                const validation = await Utils.validateContributorName(t);
                if (!validation.ok) {
    ...
    setupLayoutBStoryPreviewLongPressShare() {
      ...
      if (!wrap || wrap.hidden || !sheet || sheet.dataset.listenerAttached === '1') return;
    ...
    _igContributorCarouselBodyFontStack() {
      const QNC = globalThis.QuiltNewspaperClipping;
      return QNC?.CLIPPING_FONT_FAMILY || '"DM Sans", system-ui, -apple-system, "Segoe UI", sans-serif';
    }

    /** IG carousel slide 2 reflection prompt + response patches. */
    _igReflectionSlideBodyFontStack() {
      return '"Barlow", "Helvetica Neue", Helvetica, Arial, sans-serif';
    }

    _igReflectionPromptFontWeight() {
      return 600;
    }

    _igContributorCarouselBodyPx() {
      const QNC = globalThis.QuiltNewspaperClipping;
      if (typeof QNC?.contributorListBodyPxAtExportWidth === 'function') {
        return QNC.contributorListBodyPxAtExportWidth();
      }
      return 48;
    }

    _igReflectionExportPatchBodyPx() {
      return this._igContributorCarouselBodyPx();
    }

    _igReflectionExportAuthorPx() {
      return Math.max(14, Math.round(this._igReflectionExportPatchBodyPx() * 0.8));
    }
    ...
    _ensureIgReflectionExportStyles() {
      ...
      const igReflectionFont = this._igReflectionSlideBodyFontStack();
      ...
      style.textContent = `
        ...
        #screen-quilt.is-ig-export {
          ...
          background: transparent;
          overflow: visible;
          --reflection-clipping-font: ${igReflectionFont};
          --quilt-clipping-font: ${igReflectionFont};
          --quilt-quote-copy-size: ${patchBodyPx}px;
          --quilt-reflection-theme-size: ${patchBodyPx}px;
        ...
        #screen-quilt.is-ig-export .quilt-reflection-carousel-slide--clipping .quilt-reflection-carousel-author {
          font-family: ${igReflectionFont} !important;
        ...
        #screen-quilt.is-ig-export .quilt-reflection-carousel-slide--ig-prompt .quilt-reflection-carousel-body--ig-prompt {
          font-family: ${igReflectionFont} !important;
          font-weight: ${this._igReflectionPromptFontWeight()} !important;
    ...
    _ensureIgReflectionCaptureStyles() {
      ...
      css += `
        ...
        ${ROOT} .quilt-reflection-carousel-slide--clipping .quilt-reflection-carousel-author {
          font-family: ${igReflectionFont} !important;
          font-weight: 400 !important;
          font-style: normal !important;
        }
        ...
        ${ROOT} .quilt-reflection-carousel-slide--ig-prompt .quilt-reflection-carousel-body--ig-prompt {
          font-family: ${igReflectionFont} !important;
          font-weight: ${this._igReflectionPromptFontWeight()} !important;
          font-style: italic !important;
        }
    ...
    _syncIgReflectionSlideExportLayout(exportScreen, dateKey, themeEntries) {
      ...
      exportScreen.style.setProperty('--quilt-reflection-theme-size', `${bodyPx}px`);
    ...
    _ensureIgReflectionPlaywrightCaptureStyles(responseCount = 3) {
      ...
      const igReflectionFont = this._igReflectionSlideBodyFontStack();
      const igReflectionPromptWeight = this._igReflectionPromptFontWeight();
      ...
      style.textContent = `
        ...
        #screen-quilt.is-ig-reflection-playwright-capture .quilt-reflection-carousel-slide--ig-prompt .quilt-reflection-carousel-text,
        #screen-quilt.is-ig-reflection-playwright-capture .quilt-reflection-carousel-slide--ig-prompt .quilt-reflection-carousel-body,
        #screen-quilt.is-ig-reflection-playwright-capture .quilt-reflection-carousel-slide--ig-prompt .quilt-reflection-carousel-body--ig-prompt {
          font-family: ${igReflectionFont} !important;
          font-weight: ${igReflectionPromptWeight} !important;
          font-style: italic !important;
          font-variation-settings: normal !important;
        }
    ...
    /** Positive when visible mat shows between stacked patch paper edges. */
    _reflectionPatchPairVerticalPaperGapPx(topPatch, bottomPatch) {
      if (!topPatch || !bottomPatch) return 0;
      const topR = topPatch.getBoundingClientRect();
      const bottomR = bottomPatch.getBoundingClientRect();
      const topPaperBottom = topR.bottom + this._readReflectionPatchPaperEdgeY(topPatch);
      const bottomPaperTop = bottomR.top - this._readReflectionPatchPaperEdgeY(bottomPatch);
      return bottomPaperTop - topPaperBottom;
    }

    _reflectionPatchPairHasVisibleVerticalPaperGap(topPatch, bottomPatch, minGapPx = 4) {
      return this._reflectionPatchPairVerticalPaperGapPx(topPatch, bottomPatch) >= minGapPx;
    }

    /** Positive when visible mat shows between side-by-side patch boxes. */
    _reflectionPatchPairHorizontalGapPx(leftPatch, rightPatch) {
      if (!leftPatch || !rightPatch) return 0;
      const leftR = leftPatch.getBoundingClientRect();
      const rightR = rightPatch.getBoundingClientRect();
      return rightR.left - leftR.right;
    }

    _reflectionPatchPairHasVisibleHorizontalGap(leftPatch, rightPatch, minGapPx = 4) {
      return this._reflectionPatchPairHorizontalGapPx(leftPatch, rightPatch) >= minGapPx;
    }
    ...
    syncReflectionCarouselSplitTape(slides) {
      ...
      list.forEach((slide) => {
        ...
        if (visualBottomPaperTop - visualTopPaperBottom >= 4) {
    ...
    syncIgReflectionCaptureClusterJoinTape(gridEl, dateKey) {
      ...
      const appendVerticalTape = (prev, next, seedKey) => {
        if (this._reflectionPatchPairHasVisibleHorizontalGap(prev, next)) return null;
      ...
      const appendHorizontalTape = (topPatch, bottomPatch, tapeVariant, joinLeftOverride = null) => {
        if (this._reflectionPatchPairHasVisibleVerticalPaperGap(topPatch, bottomPatch)) return;
        const topR = topPatch.getBoundingClientRect();
        const bottomR = bottomPatch.getBoundingClientRect();
    ...
    syncReflectionCarouselJoinTape(slides) {
      ...
      for (let i = 1; i < list.length; i++) {
        ...
        if (this._reflectionPatchPairHasVisibleHorizontalGap(prev, next)) continue;
    ...
    _staticQuiltPaperAssetUrls() {
      const moodBuild =
        String(globalThis.QuiltMoodCollageWidget?.COLLAGE_BUILD || 'v153').trim() || 'v153';
      return [
        ...
        `assets/mood-collage/tape-good-foil.webp?${moodBuild}`,
        `assets/mood-collage/tape-rough-foil.webp?${moodBuild}`,
    ...
    async _ensureQuoteSpeakerImageForCanvas(url, options = {}) {
      const src = String(url || '').trim();
      if (!src) return null;
      if (/^data:/i.test(src)) {
        const dataImg = await this._loadQuoteSpeakerImageElement(src, { forCanvas: true });
        if (dataImg?.naturalWidth) {
          dataImg._loadedUrl = src;
          dataImg._canvasSafe = true;
          return dataImg;
        }
      }
      let img = await this._loadQuoteSpeakerImageElement(src, { forCanvas: true });
      const shouldProxy =
        this._quoteSpeakerShouldStripFringe(src) && this.archiveService?._prepareSpeakerImageUrlForCanvas;
      if ((!img?.naturalWidth || shouldProxy) && shouldProxy) {
        try {
          const proxied = await this.archiveService._prepareSpeakerImageUrlForCanvas(src, {
            quote: options.quote || null,
            retry: options.retry === true,
            skipCutoutExportFinalize: options.skipCutoutExportFinalize !== false
          });
    ...
    _quoteSpeakerCutoutSubjectFraming(naturalW, naturalH, boxW, subjectBounds, options = {}) {
      const iw = Math.max(1, naturalW);
      const ih = Math.max(1, naturalH);
      const sw = Math.max(1, subjectBounds.width);
      const sh = Math.max(1, subjectBounds.height);
      const minX = Math.max(0, Number(subjectBounds.minX) || 0);
      const minY = Math.max(0, Number(subjectBounds.minY) || 0);
      /** Scale so opaque subject width fills the 90% name-strip column. */
      const scale = boxW / sw;
      const subjectDrawH = Math.ceil(sh * scale);
      const padTop = 8;
      const tuckBand = Math.max(14, Math.round(subjectDrawH * 0.1));
      const frameH = Math.max(120, Math.ceil(padTop + subjectDrawH + tuckBand));
      const subjectBottom = padTop + subjectDrawH;
      // Direct Storage PNGs keep transparent padding — paint the full bitmap so the
      // subject lands at boxW; composite bakes are already subject-cropped.
      if (options.fullPngPaintBox === true) {
        return {
          offsetX: Math.round(-minX * scale),
          offsetY: Math.round(padTop - minY * scale),
    ...
    /** Plain cutout layout: subject-fill so the figure reads at 90% strip width. */
    ...
    /** Plain cutout: vertical anchor inside the cutout frame (top/center — not bottom). */
    _quoteSpeakerPlainCutoutVerticalPosY(frameH, drawH, mode = 'center') {
      const fh = Math.max(0, Number(frameH) || 0);
      const dh = Math.max(0, Number(drawH) || 0);
      if (fh < 1 || dh < 1) return 0;
      if (mode === 'top') return 8;
      return Math.max(8, Math.round((fh - dh) / 2));
    }
    ...
    /** Plain cutout: bake Layout B story lithograph (same stack as layout-b-story.png). */
    ...
    /** Same cream tape as `.quote-speaker-slab` / identity name strip. */
    _quoteSpeakerNameStripPaperColor(widget = null) {
      const widgetEl = widget || document.getElementById('quoteSpeakerWidget');
      if (widgetEl && typeof getComputedStyle === 'function') {
        const tape = String(getComputedStyle(widgetEl).getPropertyValue('--quilt-byg-tape') || '').trim();
        if (/^#[0-9A-Fa-f]{6}$/.test(tape)) return tape;
      }
      return String(this.getPaleQuiltPaperColor?.() || '#f2eee6').trim();
    }

    /** Quilt-screen speaker cutout bake (lithograph stack; darker shadow ink than IG story). */
    _quoteSpeakerStoryCutoutBakeOptions(bakeLayout = {}) {
      const nameStripPaper = this._quoteSpeakerNameStripPaperColor();
      const paperSeed = this._quoteSpeakerPlainCutoutPaperSeed();
      const normalizeStyle =
        typeof globalThis.odqNormalizeSpeakerStyleAmount === 'function'
          ? globalThis.odqNormalizeSpeakerStyleAmount
          : (raw) => {
              const n = Number(raw);
              if (!Number.isFinite(n)) return 1;
              return Math.round(Math.min(1, Math.max(0, n)) * 1000) / 1000;
            };
      const styleAmount = normalizeStyle(
        (typeof globalThis.odqGetCachedLayoutBSpeakerStyleAmount === 'function'
          ? globalThis.odqGetCachedLayoutBSpeakerStyleAmount(paperSeed)
          : undefined) ?? 1
      );
      return {
        bakeKey: `${nameStripPaper}|quilt-name-strip-v4|style:${styleAmount}`,
        options: {
          isCutoutPng: true,
          skipScannerBed: true,
          drawScannerBed: false,
          bleedPhase: 'quilt-screen',
          matchNameStripPaper: true,
          washColor: nameStripPaper,
          seed: paperSeed,
          styleAmount,
          layout: bakeLayout.compositeLayout || 'width-fill',
          subjectCrop: bakeLayout.subjectCrop || null
        }
      };
    }
    ...
    /** Baked lithograph PNG is on the portrait <img> — skip rebake unless layout sig changes. */
    _quoteSpeakerPlainCutoutCompositeReady(cutout, img = null) {
      if (!cutout) return false;
      const portrait = img || cutout.querySelector?.('.quote-speaker-image');
      return !!(
        cutout.classList.contains('quote-speaker-cutout--plain-composite') &&
        portrait?.dataset?.plainCutoutComposite === '1' &&
        portrait.complete &&
        portrait.naturalWidth > 0
      );
    }

    _quoteSpeakerPlainCutoutLayoutSig(cutout, sourceImg = null, boxW = 0, bakeLayout = null) {
      if (!cutout) return '';
      const src =
        sourceImg ||
      ...
      const subjKey = subj?.width
        ...
        : '';
      const bakeKey = bakeLayout?.drawW
        ? `${Math.round(bakeLayout.drawW)}x${Math.round(bakeLayout.drawH)}`
        : '';
      return [w, src?.naturalWidth || 0, src?.naturalHeight || 0, subjKey, bakeKey].join('|');
    }
    ...
    _applyQuoteSpeakerPlainCutoutFit(cutout, img, boxW = 0) {
      ...
      const tuckBand = Math.max(18, Math.round(drawH * 0.1));
      const frameH = Math.max(120, drawH + tuckBand);
      cutout.classList.remove('quote-speaker-cutout--subject-fill');
      delete cutout._speakerSubjectCrop;
      cutout._speakerSubjectBounds = null;
      ...
      cutout.style.setProperty(
        '--speaker-cutout-pos-y',
        `${this._quoteSpeakerPlainCutoutVerticalPosY(frameH, drawH, 'top')}px`
      );
    ...
    _scheduleQuoteSpeakerIdentityTuckOnly(cutout) {
      const wrap = cutout?.closest('.quote-speaker-portrait-wrap');
      if (!cutout || cutout.hidden || !wrap) return;
      requestAnimationFrame(() => {
    ...
    _syncQuoteSpeakerNameTuck(cutout, img) {
      ...
      if (cutout._speakerTuckAnchorSig !== tuckSig) {
        ...
        const compositeReady =
          cutout.classList.contains('quote-speaker-cutout--plain-composite') &&
          cutout.querySelector('.quote-speaker-image')?.dataset?.plainCutoutComposite === '1';
        if (cutout.classList.contains('quote-speaker-cutout--plain-cutout') && compositeReady) {
          if (!wrap) return;
          requestAnimationFrame(() => {
            const wrapRect = wrap.getBoundingClientRect();
            const cutoutRect = cutout.getBoundingClientRect();
        ...
        if (cutout.classList.contains('quote-speaker-cutout--plain-cutout')) {
          const displayReady =
            ...
            cutout.querySelector('.quote-speaker-image')?.dataset?.plainCutoutComposite === '1';
          if (!displayReady && !cutout._plainCutoutBackingInFlight) {
    ...
    _applyQuoteSpeakerCutoutFrameHeight(imageWrap, img, boxW, bounds, subjectBounds = null) {
      ...
      const portraitImg = imageWrap.querySelector?.('.quote-speaker-image');
      const directPng =
        portraitImg?.dataset?.plainCutoutDirect === '1' &&
        !imageWrap.classList.contains('quote-speaker-cutout--plain-composite');
      let offsetX = 0;
      let offsetY = 0;
      let drawW = boxW;
      ...
      if (useSubjectFraming) {
        ...
        ({ offsetX, offsetY, drawW, drawH, frameH, subjectBottom } = this._quoteSpeakerCutoutSubjectFraming(
          img.naturalWidth,
          img.naturalHeight,
          boxW,
          subjectBounds,
          { fullPngPaintBox: directPng }
        ));
        imageWrap._speakerSubjectBottom = subjectBottom;
      } else {
        imageWrap.classList.remove('quote-speaker-cutout--subject-fill');
        delete imageWrap._speakerSubjectCrop;
        delete imageWrap._speakerSubjectBottom;
    ...
    _applyQuoteSpeakerCutoutAnchor(imageWrap, img) {
      ...
      if (plainCutout) {
        const layoutImg = this._quoteSpeakerLayoutImg(imageWrap, img) || img;
        const subjectBounds =
          imageWrap._speakerSubjectBounds ||
          this._quoteSpeakerMeasureSubjectBounds(layoutImg);
        if (subjectBounds?.width > 0) {
          imageWrap._speakerSubjectBounds = subjectBounds;
          imageWrap._speakerSubjectCrop = subjectBounds;
        }
        if (widgetEl) this._syncQuoteSpeakerPortraitWidthFromNameStrip(widgetEl);
        this._applyQuoteSpeakerPlainCutoutLayout(
          imageWrap,
          img,
          this._quoteSpeakerLayoutBoxW(imageWrap)
        );
      } else {
        ...
        this._refineQuoteSpeakerPortraitWidthAfterLayout(widgetEl, imageWrap, img);
      }
      if (widgetEl) {
        requestAnimationFrame(() => {
    ...
    /** Full identity name strip width (matches Layout B author strip). */
    _quoteSpeakerIdentityNameStripWidth(widget, identity) {
      const outerW = Math.max(identity?.offsetWidth || 0, identity?.clientWidth || 0);
      return outerW >= 48 ? outerW : 0;
    }

    _quoteSpeakerPortraitLayoutWidth(widget, stripW, heightPx = 0, subjectAspect = 0) {
      const maxVisualW = stripW * this._quoteSpeakerCutoutWidthFrac(widget);
      const aspect = Math.max(0, Number(subjectAspect) || 0);
      if (aspect > 0) {
        const sinT = Math.sin((1.35 * Math.PI) / 180);
        return Math.max(48, Math.round(maxVisualW / (1 + aspect * sinT)));
      }
      if (heightPx > 0) {
        const bleed = this._quoteSpeakerPortraitTiltBleedPx(widget, heightPx);
        return Math.max(48, Math.round(maxVisualW - bleed));
      }
      return Math.max(48, Math.round(maxVisualW));
    }

    /** Speaker card: portrait column fits inside 90% of the name strip after tilt bleed. */
    _syncQuoteSpeakerPortraitWidthFromNameStrip(widget, heightPx = 0) {
      ...
      const subj = cutout?._speakerSubjectBounds || cutout?._speakerSubjectCrop;
      const subjectAspect = subj?.width > 0 ? subj.height / subj.width : 0;
      let estH = Math.max(0, Number(heightPx) || 0);
      if (estH < 1 && subjectAspect <= 0) {
        estH = Math.max(
          cutout?.clientHeight || 0,
          parseFloat(String(cutout?.style?.height || '').replace(/px$/, '')) || 0
        );
      }
      const target = this._quoteSpeakerPortraitLayoutWidth(widget, stripW, estH, subjectAspect);
    ...
    /** Final pass: keep layout column at/under 90% of the name strip width. */
    ...
    _applyQuoteSpeakerIdentityTuck(cutout, wrapRect, cutoutRect) {
      ...
      const bgW =
        ...
        0;
      const bgH =
        parseFloat(String(cutout.style.getPropertyValue('--speaker-cutout-bg-h') || '0').replace(/px$/, '')) ||
        0;
      const scale =
      ...
      const compositeReady =
        ...
        img?.naturalWidth > 0;
      const directReady =
        img?.dataset?.plainCutoutDirect === '1' && img?.naturalWidth > 0;
      let opaqueBottomLayout;
      // Direct Storage PNGs are full-frame paint boxes — tuck to subject feet, not PNG bottom.
      if (directReady && Number(cutout._speakerSubjectBottom) > 0) {
        opaqueBottomLayout = Number(cutout._speakerSubjectBottom);
      } else if (
        directReady &&
        bounds &&
        scale > 0 &&
        ...
        opaqueBottomLayout = offsetY + (Number(bounds.minY) + Number(bounds.height)) * scale;
      } else if ((compositeReady || directReady) && bgH > 0) {
        opaqueBottomLayout = offsetY + bgH;
      } else if (bounds && scale > 0 && Number.isFinite(Number(bounds.maxY))) {
    ...
    _syncQuoteSpeakerCutoutLayout(cutout, img, quote = null) {
      ...
      const run = (layoutOnly = false) => {
        ...
        if (this._quoteSpeakerWantsFirebasePopArt(profile, resolvedQuote)) {
          ...
          if (this._quoteSpeakerPlainCutoutCompositeReady(cutout, img)) {
            this._syncQuoteSpeakerCutoutLayoutOnly(cutout, img);
          } else {
    ...
    _quoteSpeakerCutoutPaintRect(cutout, img, boxW, boxH, bounds = null) {
      if (this._quoteSpeakerCutoutUsesPaintBox(cutout)) {
    ...
    _layoutQuoteSpeakerAttributionAlongSilhouette(attributionEl, imgEl, cutoutEl) {
      ...
      const portraitImg = cutout?.querySelector('.quote-speaker-image');
      const img =
        this._quoteSpeakerLayoutImg(cutout, imgEl || portraitImg) || imgEl || portraitImg;
    ...
    _ensureQuoteSpeakerPlainCutoutLayersHidden(cutout) {
      ...
      if (grain) {
        const plainCutout = cutout.classList.contains('quote-speaker-cutout--plain-cutout');
        const plainFallback = cutout.classList.contains('quote-speaker-cutout--plain-fallback');
        const compositeReady =
          cutout.classList.contains('quote-speaker-cutout--plain-composite') ||
          cutout.querySelector('.quote-speaker-image')?.dataset?.plainCutoutComposite === '1';
        if ((plainCutout && !plainFallback) || compositeReady) {
          grain.hidden = true;
          grain.setAttribute('hidden', '');
        } else {
          grain.hidden = false;
    ...
    _quoteSpeakerPlainCutoutSourceImg(cutout, img) {
      if (
        ...
        cutout._plainCutoutSourceImage._canvasSafe
      ) {
    ...
    /** Plain cutout uses explicit draw box CSS vars — attribution must match, not generic contain. */
    _quoteSpeakerCutoutUsesPaintBox(cutout) {
      if (!cutout) return false;
      if (cutout.classList.contains('quote-speaker-cutout--subject-fill')) return true;
      if (!cutout.classList.contains('quote-speaker-cutout--plain-cutout')) return false;
      const portrait = cutout.querySelector('.quote-speaker-image');
      if (
        portrait?.dataset?.plainCutoutDirect !== '1' &&
        portrait?.dataset?.plainCutoutComposite !== '1'
      ) {
        return false;
      }
      const drawW = parseFloat(
        String(cutout.style.getPropertyValue('--speaker-cutout-bg-w') || '').replace(/px$/, '')
      );
      return Number.isFinite(drawW) && drawW > 0;
    }
    ...
    _resolveQuoteSpeakerPlainCutoutBakeLayout(cutout, sourceImg) {
      ...
      if (subjectCrop?.width > 0) {
        const framed = this._quoteSpeakerCutoutSubjectFraming(iw, ih, boxW, subjectCrop);
        drawW = Math.max(1, Math.round(framed.drawW));
        drawH = Math.max(1, Math.round(framed.drawH));
      } else {
        drawW = boxW;
        drawH = Math.max(120, Math.ceil((ih * boxW) / iw));
      }
      ...
      return {
        ...
        drawW,
        drawH,
        compositeLayout: subjectCrop?.width > 0 ? 'subject-fill' : 'width-fill',
        subjectCrop: subjectCrop?.width > 0 ? subjectCrop : null
      };
    }
    ...
    _applyQuoteSpeakerPlainCutoutBacking(cutout, img, sourceUrl = '') {
      ...
      const boxW = this._quoteSpeakerLayoutBoxW(cutout);
      const layoutSig = this._quoteSpeakerPlainCutoutLayoutSig(cutout, sourceImg, boxW, bakeLayout);
      const { bakeKey } = this._quoteSpeakerStoryCutoutBakeOptions(bakeLayout || {});
      ...
      const storageUrl = String(cutout._plainCutoutSourceUrl || remote || '').trim();
      const useDirectStorageCutout = this._quoteSpeakerShouldStripFringe(storageUrl);
      const paperSig = `${layoutSig}|${bakeKey}|${
        useDirectStorageCutout ? 'storage-direct-v2-subject-fill' : 'story-over-quilt-v18'
      }`;
      const compositeReady =
      ...
      const directReady =
        useDirectStorageCutout &&
        portraitImg?.dataset?.plainCutoutDirect === '1' &&
        portraitImg.complete &&
        portraitImg.naturalWidth > 0;
      if (cutout._plainCutoutPaperSig === paperSig && (directReady || compositeReady)) return;
      if (cutout._plainCutoutBackingInFlight) {
        cutout._plainCutoutBackingQueuedSig = paperSig;
        return;
      }
      ...
      cutout._plainCutoutBackingInFlight = true;
      cutout._plainCutoutBackingQueuedSig = '';

      const finishPlainCutoutBacking = () => {
        cutout._plainCutoutBackingInFlight = false;
        const queued = String(cutout._plainCutoutBackingQueuedSig || '').trim();
        cutout._plainCutoutBackingQueuedSig = '';
        const compositeStable =
          portraitImg?.dataset?.plainCutoutComposite === '1' &&
          portraitImg.complete &&
          portraitImg.naturalWidth > 0;
        if (queued && queued !== cutout._plainCutoutPaperSig && !cutout.hidden) {
          if (compositeStable) {
            cutout._plainCutoutPaperSig = queued;
            return;
          }
          this._applyQuoteSpeakerPlainCutoutBacking(cutout, img, sourceUrl);
        }
      ...
      if (portraitImg && (prevPaperSig !== paperSig || useDirectStorageCutout)) {
        delete portraitImg.dataset.plainCutoutComposite;
        delete portraitImg.dataset.plainCutoutDirect;
        cutout.classList.remove('quote-speaker-cutout--plain-composite', 'quote-speaker-cutout--plain-fallback');
      }
      if (portraitImg) {
        portraitImg.hidden = false;
        portraitImg.classList.add('is-loaded');
      ...
      const resolveSourceImg = async () => {
        ...
        const decoded = await this._ensureQuoteSpeakerImageForCanvas(loadUrl, {
          skipCutoutExportFinalize: true
        });
      ...
      const applyPlainCutoutDirectFallback = (srcImg) => {
        if (paperGen !== cutout._plainCutoutPaperGen || cutout.hidden || !portraitImg) {
          finishPlainCutoutBacking();
          return;
        }
        ...
        requestAnimationFrame(() => {
          if (!cutout.hidden && portraitImg?.naturalWidth) {
            ...
            const attributionEl = document.getElementById('quoteSpeakerAttribution');
            if (attributionEl && !attributionEl.hidden && layoutSource?.naturalWidth) {
              this._scheduleQuoteSpeakerAttributionLayout(attributionEl, layoutSource, cutout);
            }
          }
          finishPlainCutoutBacking();
        });
      ...
      const applySlide1CutoutDisplay = async (sourceImg) => {
        if (paperGen !== cutout._plainCutoutPaperGen || cutout.hidden || !portraitImg) {
          finishPlainCutoutBacking();
          return;
        }
        ...
        if (portraitImg.dataset.plainCutoutComposite === '1' && portraitImg.naturalWidth) {
          portraitImg.style.removeProperty('opacity');
          const liveBoxW = this._quoteSpeakerLayoutBoxW(cutout);
          this._applyQuoteSpeakerPlainCutoutLayout(cutout, srcImg || portraitImg, liveBoxW);
          this._scheduleQuoteSpeakerIdentityTuckOnly(cutout);
          finishPlainCutoutBacking();
          return;
        }
        if (useDirectStorageCutout) {
          if (portraitImg && storageUrl && !/^data:/i.test(storageUrl) && portraitImg.src !== storageUrl) {
            portraitImg.src = storageUrl;
          }
          applyPlainCutoutDirectFallback(srcImg);
          return;
        }
        const SCR = globalThis.SpeakerCutoutRender;
        const canvasSource = await this._ensureQuoteSpeakerImageForCanvas(
          String(cutout._plainCutoutSourceUrl || remote || '').trim(),
          { skipCutoutExportFinalize: true }
        );
        const bakeImg = canvasSource?.naturalWidth ? canvasSource : srcImg;
        if (
          !SCR?.buildSpeakerOverQuiltCompositeDataUrlAsync ||
          !this._quoteSpeakerCanCanvasProcessImage(bakeImg, cutout, portraitImg)
        ) {
          applyPlainCutoutDirectFallback(srcImg);
          return;
        }
        const { options } = this._quoteSpeakerStoryCutoutBakeOptions(bakeLayout);
        const dpr = Math.min(
          3,
          Math.max(1, typeof window !== 'undefined' ? window.devicePixelRatio || 1 : 1)
        );
        const bakeW = Math.max(1, Math.round(bakeLayout.drawW * dpr));
        const bakeH = Math.max(1, Math.round(bakeLayout.drawH * dpr));
        let dataUrl = '';
        try {
          dataUrl = await SCR.buildSpeakerOverQuiltCompositeDataUrlAsync(
            bakeImg,
            bakeW,
            bakeH,
            bakeLayout.compositeLayout || 'contain',
            { ...options, filmGrain: false }
          );
        } catch (_) {
          dataUrl = '';
        }
        if (paperGen !== cutout._plainCutoutPaperGen || cutout.hidden) {
          finishPlainCutoutBacking();
          return;
        }
        ...
        cutout.classList.remove('quote-speaker-cutout--plain-fallback');
        cutout.classList.add('quote-speaker-cutout--plain-composite');
        delete portraitImg.dataset.plainCutoutDirect;
        portraitImg.dataset.plainCutoutComposite = '1';
        portraitImg.hidden = false;
        portraitImg.classList.add('is-loaded');
        ...
        if (portraitImg.src !== dataUrl) portraitImg.src = dataUrl;
        if (neutralSilhouette) {
          neutralSilhouette.hidden = true;
          neutralSilhouette.setAttribute('hidden', '');
        }
        const layoutAfterComposite = () => {
          if (paperGen !== cutout._plainCutoutPaperGen || cutout.hidden || !portraitImg?.naturalWidth) {
            finishPlainCutoutBacking();
            return;
          }
          const liveBoxW = this._quoteSpeakerLayoutBoxW(cutout);
          this._applyQuoteSpeakerPlainCutoutLayout(cutout, srcImg, liveBoxW);
          cutout._speakerTuckAnchorSig = `${srcImg.naturalWidth}x${srcImg.naturalHeight}:${liveBoxW}`;
          this._scheduleQuoteSpeakerIdentityTuckOnly(cutout);
          finishPlainCutoutBacking();
        };
        if (portraitImg.complete && portraitImg.naturalWidth) layoutAfterComposite();
        else portraitImg.addEventListener('load', layoutAfterComposite, { once: true });
      };

      void resolveSourceImg()
        .then((sourceImg) => applySlide1CutoutDisplay(sourceImg))
        .catch(() => {
          this._showQuoteSpeakerPlainCutoutFallback(cutout, portraitImg);
          finishPlainCutoutBacking();
        });
    ...
    _applyQuoteSpeakerPlainCutoutStack(cutout, img) {
      if (!cutout || cutout.hidden || !img?.naturalWidth) return;
      const portraitImg = cutout.querySelector('.quote-speaker-image') || img;
      if (this._quoteSpeakerPlainCutoutCompositeReady(cutout, portraitImg)) {
        this._scheduleQuoteSpeakerIdentityTuckOnly(cutout);
        return;
      }
      this._ensureQuoteSpeakerPlainCutoutLayersHidden(cutout);
      ...
      this._applyQuoteSpeakerCutoutAnchor(cutout, img);
      requestAnimationFrame(() => {
        if (cutout.hidden || !img?.naturalWidth) return;
    ...
    _applyQuoteSpeakerImageLoaded(
      ...
      if (img) {
        ...
        const plainCompositeReady =
          usePlainCutout && this._quoteSpeakerPlainCutoutCompositeReady(imageWrap, img);
        if (!plainCompositeReady) {
        ...
        const syncLoadedSpeakerImage = () => {
          if (plainCompositeReady || this._quoteSpeakerPlainCutoutCompositeReady(imageWrap, img)) {
            if (imageWrap) this._syncQuoteSpeakerCutoutLayoutOnly(imageWrap, img);
            return;
          }
      ...
      if (imageWrap && img && !imageWrap.hidden) {
        if (usePlainCutout && this._quoteSpeakerPlainCutoutCompositeReady(imageWrap, img)) {
    ...
    refreshQuoteSpeakerWidget(quote = null) {
      ...
      this._quoteSpeakerRefreshTimer = setTimeout(run, 100);
    }
    ...
    async _refreshSeamsidePodcastWidgetEntry(quote = null) {
      ...
      const paint = () => {
        ...
        if (host && !host.hidden) {
          document.getElementById('quoteSpeakerStage')?.classList?.add?.('quote-speaker-stage--visible');
        }
      };
      paint();
      const hadEpisodes = !!(this.quoteService?._seamsideEpisodes?.length);
      if (hadEpisodes) return;
    ...
    async _refreshQuoteSpeakerWidgetEntry(quote = null) {
      ...
      if (this.isAdminTomorrowPreviewActive?.()) {
        ...
        if (dk && this.quoteService) {
          const paintMerged = (merged) => {
            if (!merged?.text || this._adminPreview?.dateKey !== dk) return merged;
        ...
        resolvedQuote =
          quote || this.getEffectiveQuiltQuote?.() || this.quoteService?.getTodayQuote?.() || null;
        // Paint from the pin immediately so speaker chrome occupies scroll height
        // before cutout/episode network work finishes.
        this._refreshQuoteSpeakerWidgetNow(resolvedQuote);
        if (resolvedQuote?.text && dk && this.quoteService) {
          ...
          const hydrateInBackground = isNativeSpeakerHydrate || hasUsableSpeakerImage;
      ...
      if (
        resolvedQuote?.text &&
        !this.isAdminTomorrowPreviewActive?.() &&
        this.quoteService?.isSeamsideQuote?.(resolvedQuote)
      ) {
        void this.quoteService.ensureSeamsideEpisodesLoaded?.({ requireServer: false });
      }
    }
    ...
    _refreshQuoteSpeakerWidgetNowInner(quote = null) {
      ...
      const currentSpeakerSrc = String(img?.currentSrc || img?.src || '').trim();
      const canKeepLoadedImage =
        ...
        (this._quoteSpeakerImageUrlsMatch(currentSpeakerSrc, profile.imageUrl) ||
          this._quoteSpeakerImageUrlsMatch(
            currentSpeakerSrc,
            imageWrap._plainCutoutSourceUrl || this._quoteSpeakerCutoutUrl
          ));
      ...
      if (canKeepLoadedImage) {
        ...
        if (
          ...
          if (this._quoteSpeakerPlainCutoutCompositeReady(imageWrap, img)) {
            this._syncQuoteSpeakerCutoutLayoutOnly(imageWrap, img);
          } else {
    ...
    async _refreshQuiltMoodSpread(
      ...
      const dk = String(
        quotePayload?.dateKey ||
          (typeof this.getEffectiveAppDateKey === 'function' ? this.getEffectiveAppDateKey() : '') ||
          ''
      ).trim();
      if (!quotePayload && spreadMode !== 'collage') {
        this._unmountQuiltMoodSpread();
        this._fallbackQuiltQuoteClipping(todayQuote);
        return;
      }
      ...
      try {
        ...
        if (spreadMode === 'collage') {
          await this._refreshQuiltMoodCollageSpread({
            ...
            r,
            dk,
            quotePayload: quotePayload || { dateKey: dk, text: '', author: '' },
            todayQuote,
            canComposeQuote,
            mountMood,
            moodHost,
