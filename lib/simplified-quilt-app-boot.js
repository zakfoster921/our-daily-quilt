...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
     ...
     * X = midpoint of the head's width in a short band just under that crown — not the
     * tip of a left/right hair spike, which pulled the hat off-center on 3/4 portraits.
     */
    _quoteSpeakerMeasureCrownTop(img, bounds = null) {
      ...
      // Head width just under the crown (not the single peak row).
      const slabH = Math.max(3, Math.round(scanH * 0.06));
      const y1 = Math.min(scanH - 1, minY + slabH);
      let headMinX = scanW;
      let headMaxX = -1;
      for (let y = minY; y <= y1; y += 1) {
        for (let x = x0; x <= x1; x += 1) {
          if (!isSubject((y * scanW + x) * 4)) continue;
