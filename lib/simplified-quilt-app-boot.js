...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    async _igReflectionCardsLayerFromPngBase64(b64, sizeHints = {}) {
      ...
      return new Promise((resolve) => {
        ...
        img.onload = () => {
          ...
          resolve({
            canvas,
            logicalWidth: Math.max(1, Math.round(hintW > 0 ? hintW : w / dpr)),
            logicalHeight: Math.max(1, Math.round(hintH > 0 ? hintH : h / dpr))
          });
        };
    ...
    _ensureIgReflectionPlaywrightCaptureStyles(responseCount = 3) {
      ...
      const gridGap = responseCount >= 5 ? '0.28rem' : '0.35rem';
    ...
    _syncIgReflectionPlaywrightCaptureLayout(captureScreen, dateKey) {
      ...
      gridRows.forEach((rowEl) => {
        ...
        if (!n) return;
        if (n === 1) {
          const soloW = Math.min(columnW, Math.floor(columnW * (responseSlides.length === 1 ? 0.82 : 0.72)));
