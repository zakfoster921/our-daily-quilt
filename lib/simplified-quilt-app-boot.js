...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    _applyPickerHSVFromHex(hex) {
      ...
      const vividSat = Number(CONFIG.COLOR_PICKER?.hueBarSaturation) || 68;
    ...
    _applyQuoteSpeakerPlainCutoutBacking(cutout, img, sourceUrl = '') {
      ...
      if (portraitImg) portraitImg.style.opacity = '0';
      if (neutralSilhouette) {
        neutralSilhouette.hidden = true;
        this._clearQuoteSpeakerPlainCutoutBackingStyles(neutralSilhouette);
      }
      ...
      const applyComposite = async (sourceImg) => {
        ...
        const plainCutoutSeed =
          ...
          (typeof Utils !== 'undefined' && typeof Utils.getTodayKey === 'function'
            ? Utils.getTodayKey()
            : 'odq');
        const plainCutoutOpts = { seed: plainCutoutSeed };
        const composite =
          (await SCR?.buildSpeakerPlainCutoutCompositeDataUrlAsync?.(
            sourceImg,
            drawW,
            drawH,
            'width-fill',
          ...
          SCR?.buildSpeakerPlainCutoutCompositeDataUrl?.(
            sourceImg,
            drawW,
            drawH,
            'width-fill',
    ...
    _applyQuoteSpeakerImageLoaded(
      ...
      if (img) {
        ...
        img.style.opacity = usePlainCutout ? '0' : '1';
        img.src = displaySrc;
        img.hidden = false;
      ...
      const term = T.createInteractiveTerminalElement({
        ...
        resolveQuoteClippingUrl: async (dateKey) => {
          ...
          const stale =
            ...
            SimplifiedQuiltAppV2Quilt.isStoredClippingExportStale(meta.exportRev);
          if (stale) {
            console.info(
              `[our-daily] Mood terminal: Storage rev ${meta.exportRev} < bundled ${globalThis.QuiltNewspaperClipping?.CLIPPING_EXPORT_REV}; composing locally`
            );
            return '';
          }
    ...
    async _resolveQuiltMoodSpreadQuoteUrl(
      ...
      const triptychClippingMeta = dk
        ...
        : { exportRev: '' };
      const triptychSkipStorage =
        triptychClippingMeta.exportRev &&
        typeof SimplifiedQuiltAppV2Quilt?.isStoredClippingExportStale === 'function' &&
        SimplifiedQuiltAppV2Quilt.isStoredClippingExportStale(triptychClippingMeta.exportRev);
      if (triptychSkipStorage) {
        console.info(
          `[our-daily] Triptych quote: Storage rev ${triptychClippingMeta.exportRev} < bundled ${globalThis.QuiltNewspaperClipping?.CLIPPING_EXPORT_REV}; composing locally`
        );
      }
      if (!triptychSkipStorage) {
