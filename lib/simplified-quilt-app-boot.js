...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    /**
     * Seat + scale the birthday hat to the measured forehead on the painted portrait box
     * (DOM rects), so letterboxing / CSS offsets can't drift the brim left of the skull.
     */
    _positionQuoteSpeakerBirthdayHat(cutout, img) {
      ...
      if (!hatEl || hatEl.hidden || !img?.naturalWidth) {
        clear();
        return;
      }
      const portraitImg =
      ...
      const imgRect = portraitImg?.getBoundingClientRect?.() || null;
      const frameW = Math.max(1, cutoutRect.width || cutout.clientWidth || 1);
      const frameH = Math.max(1, cutoutRect.height || cutout.clientHeight || 1);
      let crownX;
      let crownY;
      let foreheadW;
      if (imgRect && imgRect.width > 1 && imgRect.height > 1) {
        ...
        crownX = originX + (crown.x / iw) * imgRect.width;
        crownY = originY + (crown.y / ih) * imgRect.height;
        foreheadW = (crown.foreheadWidth / iw) * imgRect.width;
      } else {
        ...
        crownX = offsetX + (crown.x / iw) * drawW;
        crownY = offsetY + (crown.y / ih) * drawH;
        foreheadW = (crown.foreheadWidth / iw) * drawW;
      }
      // Brim matches forehead width; tiny pad so it reads as sitting on the temples.
      const hatW = Math.max(24, Math.min(frameW * 0.92, foreheadW * 1.04));
      ...
      cutout.style.setProperty(
        '--speaker-hat-left',
        `${Math.max(20, Math.min(80, leftPct)).toFixed(2)}%`
      );
      cutout.style.setProperty(
        '--speaker-hat-width',
        `${Math.max(28, Math.min(92, widthPct)).toFixed(2)}%`
      );
    }
