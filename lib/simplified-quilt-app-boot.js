...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    syncIgReflectionCaptureClusterJoinTape(gridEl, dateKey) {
      ...
      const appendHorizontalTape = (topPatch, bottomPatch, seedKey) => {
        ...
        let overlapTop = Math.max(topR.top, bottomR.top);
        let overlapBottom = Math.min(topR.bottom, bottomR.bottom);
        let overlapSpan = overlapBottom - overlapTop;
        if (overlapSpan <= 0) {
          overlapSpan = Math.min(topR.height, bottomR.height);
        }

        let visualTopPaperBottom = topR.bottom + this._readReflectionPatchPaperEdgeY(topPatch);
        const bottomEdgeY = this._readReflectionPatchPaperEdgeY(bottomPatch);
        const visualBottomTop = bottomR.top - bottomEdgeY;
        const seamY = (visualTopPaperBottom + visualBottomTop) / 2;

        const variant = this._reflectionJoinTapeVariant(seedKey, overlapSpan, { orientation: 'horizontal' });
        const joinLeft = overlapLeft + overlapW / 2 + variant.verticalShift - gridRect.left;
        const joinTop = seamY - gridRect.top;
        const avgTilt = (readFabricTiltDeg(topPatch) + readFabricTiltDeg(bottomPatch)) / 2;
        const tape = document.createElement('span');
        tape.className =
      ...
      for (let rowIndex = 1; rowIndex < rows.length; rowIndex += 1) {
        ...
        below.forEach((bottomPatch, bottomIndex) => {
          above.forEach((topPatch, topIndex) => {
            appendHorizontalTape(
              topPatch,
              bottomPatch,
              `${dk}:ig-capture-join-v:${rowIndex}:${topIndex}:${bottomIndex}`
            );
          });
        });
      }
