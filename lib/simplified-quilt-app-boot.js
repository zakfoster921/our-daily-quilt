  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    async loadDeferredAdminSlice() {
      ...
      const SliceCtor = await this._loadDeferredScript(
        'lib/simplified-quilt-app-admin.js?v=34',
    ...
    async _waitForAdminTuneReminderAuthReady() {
    ...
    _writeLocalAdminDailyTasks(dateKey, patch = {}) {
      ...
      const next = this._normalizeAdminDailyTasks({ ...current, ...patch });
      try {
    ...
    _renderAdminDailyTaskBanner(state = null) {
      ...
      if (!banner) return;
      const show = CONFIG.APP.enableAdminTools && this._isAdminTuneReminderEnabledForThisDevice();
      banner.hidden = !show;
    ...
    async ensureLayoutBComposeReady() {
      ...
      await this._loadDeferredScript('lib/layout-b-compose.js?v=164', 'composeInstagramLayoutBFromQuiltBlob');
    ...
    _maybeSnapPickerToDailyFirstPalette(opts = {}) {
      ...
      try {
        ...
        this._userExplicitSatPick = false;
    ...
    _applyPickerHSVFromHex(hex) {
      ...
      const vividSat = Number(CONFIG.COLOR_PICKER?.hueBarSaturation) || 50;
      this.selectedHue = hsv.h;
      this.selectedSaturation = vividSat;
    ...
    /** Dev/preview: ?screen=zak|mission|welcome|bridge|first-name|portal|quote|leaderboard|qnlb=submissions */
    ...
    async applyQuiltDataFromPayload(data) {
      ...
      if (this.isAdminTomorrowPreviewActive?.() && incomingDateKey !== this.getEffectiveAppDateKey?.()) {
    ...
    formatReflectionCarouselStripHtml(strip, dateKey, stripIndex = 0, slideIndex = 0) {
      ...
      const heartMarkup = heartId
        ? this.renderReflectionCarouselEngagementHtml(heartId, normalized?.heartCount || 0, {
            responseId,
            adminHighlight: normalized?.adminHighlight === true
          })
        : '';
      const cardClass =
        stripIndex === 0
      ...
      return `<div class="${cardClass}" data-reflection-split-card${responseId ? ` data-reflection-response-id="${this.escapeQuiltFortuneText(responseId)}"` : ''} data-reflection-hand-cut-seed="${this.escapeQuiltFortuneText(handCutSeed)}">
    ...
    formatReflectionCarouselTextHtml(entry, slideIndex = 0, options = {}) {
      ...
      if (normalized?.split && Array.isArray(normalized.strips)) {
        ...
        const top = this.formatReflectionCarouselStripHtml(normalized.strips[0], dateKey, 0, slideIndex);
        const bottom = this.formatReflectionCarouselStripHtml(normalized.strips[1], dateKey, 1, slideIndex);
    ...
    syncReflectionCarouselHighlight(slideEl, { adminHighlight } = {}) {
      const btn = slideEl?.querySelector?.('.quilt-reflection-carousel-highlight');
      if (!btn) return;
    ...
    async toggleReflectionAdminHighlight({ responseId, dateKey, slideEl, buttonEl } = {}) {
      ...
      if (btn?.dataset?.highlightPending === '1') return;
      ...
      if (btn) btn.dataset.highlightPending = '1';
      ...
      try {
        ...
        this.syncReflectionCarouselHighlight(slideEl, { adminHighlight: wasHighlighted });
        this.uiService?.showToast?.(error?.message || 'Highlight did not save. Try again?');
      } finally {
        if (btn) delete btn.dataset.highlightPending;
      }
    }
    ...
    buildReflectionThemePatchSlide(slideIndex, theme, allThemes, rgb, edgeFray = {}, myReflection = null) {
      const dateKey = String(Utils.getTodayKey() || 'nodate').trim() || 'nodate';
      const normalized = this.normalizeReflectionWallTheme(theme);
      const adminHighlighted = this.reflectionThemeIsAdminHighlighted(theme);
      const previewRgb = adminHighlighted
      ...
      const highlightClass = adminHighlighted ? ' quilt-reflection-carousel-slide--admin-highlight' : '';
      const footerMarkup = mineActions
        ? `<div class="quilt-reflection-carousel-footer">${mineActions}</div>`
        : '';
      return `
            <article class="quilt-reflection-carousel-slide quilt-reflection-carousel-slide--clipping${splitClass}${wideCopyClass}${lightClass}${highlightClass}" data-reflection-theme-slide data-reflection-slide="${slideIndex}" data-reflection-hand-cut-seed="${dateKey}:reflection-response:${slideIndex}"${mineAttrs} style="${this.getReflectionFabricPatchStyle(slideIndex, theme, allThemes, rgb, { adminHighlight: adminHighlighted })}">
              <div class="quilt-reflection-carousel-text">${this.formatReflectionCarouselTextHtml(theme, slideIndex, { heartMarkup })}</div>
              ${footerMarkup}
            </article>
          `;
    }
    ...
    getReflectionCarouselMarkup(themeTexts, myReflection = null) {
      ...
      for (let i = 0; i < responseCount; i++) {
        slideParts.push(
          this.buildReflectionThemePatchSlide(slideIndex, ideas[i], ideas, themeRgbs[i], {}, myReflection)
        );
        slideIndex += 1;
      }
      slideParts.push(
    ...
    _reflectionJoinTapeVariant(seedKey = '', overlapSpanPx = 0) {
      ...
      return {
        ...
        spanPx,
        verticalShift,
        tiltJitterDeg: ((rnd() - 0.5) * 10).toFixed(2),
        opacity: (0.84 + rnd() * 0.14).toFixed(3),
        textureY: `${Math.round(rnd() * 100)}%`,
        clipPath: this._buildReflectionJoinTapeScrapClipPath(`${seedKey}:clip`)
      };
    }
    ...
    _patchLocalReflectionThemeHighlight(dateKey, responseId, highlighted, highlightAtIso = '') {
      ...
      const themes = (Array.isArray(cache.themes) ? cache.themes : []).map(patchEntry);
      const highlightMap = this._normalizeReflectionAdminHighlightMap(cache.adminHighlightByResponseId);
      if (highlighted) {
      ...
      if (rid === 'first') {
        nextCache.adminFirstResponseHighlight = highlighted === true;
        if (highlighted && nowIso) nextCache.adminFirstResponseHighlightAtIso = nowIso;
        else delete nextCache.adminFirstResponseHighlightAtIso;
      }
      this._writeLocalReflectionThemesCache(key, nextCache);
    }
    ...
    _appendSeamsideSpeakerGuideLearnMoreLink(guideEl, quote, profile) {
      ...
      if (guideEl.textContent.trim()) guideEl.append(' ');
    ...
    /**
     * Crown seat + forehead span in natural image pixels.
     * Y = top of subject ink. X/width = widest temple/forehead row under the crown
     * (hat scales to that width and centers on its midpoint).
     */
    _quoteSpeakerMeasureCrownTop(img, bounds = null) {
      const iw = img?.naturalWidth;
      const ih = img?.naturalHeight;
      if (!iw || !ih) return null;
      const scanW = Math.min(256, iw);
      ...
      const paperCutoff = 232;
      const isSubject = (o) => {
        if (data[o + 3] <= 28) return false;
        if (data[o] >= paperCutoff && data[o + 1] >= paperCutoff && data[o + 2] >= paperCutoff) {
          return false;
        }
        return true;
      };
      ...
      if (bounds && Number.isFinite(bounds.minX) && Number.isFinite(bounds.maxX)) {
        x0 = Math.max(0, Math.floor(bounds.minX / sx));
        x1 = Math.min(scanW - 1, Math.ceil(bounds.maxX / sx));
        if (Number.isFinite(bounds.height) && bounds.height > 0) {
          boundH = Math.max(8, Math.round(bounds.height / sy));
        }
      }
      const bandPad = Math.floor((x1 - x0 + 1) * 0.3);
      const bandX0 = x0 + bandPad;
      const bandX1 = x1 - bandPad;
      if (bandX1 <= bandX0) return null;
      let minY = -1;
      for (let y = 0; y < scanH; y += 1) {
        for (let x = bandX0; x <= bandX1; x += 1) {
          if (!isSubject((y * scanW + x) * 4)) continue;
          minY = y;
          break;
        }
        if (minY >= 0) break;
      ...
      // Forehead/temple band — below hair tip, above eyes.
      const yFore0 = Math.min(scanH - 1, minY + Math.max(2, Math.round(boundH * 0.08)));
      const yFore1 = Math.min(scanH - 1, minY + Math.max(yFore0 - minY + 2, Math.round(boundH * 0.32)));
      let bestWidth = -1;
      let bestMidX = (bandX0 + bandX1) / 2;
      const rowWidthAt = (y) => {
        let rowMin = scanW;
        let rowMax = -1;
        for (let x = x0; x <= x1; x += 1) {
        ...
        if (rowMax < rowMin) return { width: -1, midX: bestMidX };
        return { width: rowMax - rowMin, midX: (rowMin + rowMax) / 2 };
      };
      for (let y = yFore0; y <= yFore1; y += 1) {
        const row = rowWidthAt(y);
        if (row.width > bestWidth) {
          bestWidth = row.width;
          bestMidX = row.midX;
        }
      }
      if (bestWidth < 0) return null;
      // Skip flyaway hair tips (Twain) — sit where the silhouette is already ~half forehead-wide.
      const solidThresh = Math.max(4, Math.round(bestWidth * 0.55));
      let sitY = minY;
      for (let y = minY; y <= yFore1; y += 1) {
        const row = rowWidthAt(y);
        if (row.width >= solidThresh) {
          sitY = y;
          bestMidX = row.midX;
          break;
        }
      ...
      return {
        x: bestMidX * sx,
        y: sitY * sy,
        foreheadWidth: bestWidth * sx
      };
    }

    /**
     * party-hat-alpha.png: the wide sit-line is above the image bottom (curved brim).
     * Fraction of rendered hat height from tip → visual sit-line (~widest row / image height).
     */
    _quoteSpeakerBirthdayHatVisualBrimFromTop() {
      return 1185 / 1285;
    }

    /** Write hat seat onto cutout CSS vars + hat inline styles (inline wins over stale caches). */
    _applyQuoteSpeakerBirthdayHatSeat(cutout, seat) {
      const hatEl = document.getElementById('quoteSpeakerHat');
      if (!cutout || !seat) return;
      // Lower min bottom = allow deeper seat (52% was clamping the dip back up to the scalp tip).
      const bottom = `${Math.max(34, Math.min(90, seat.bottomPct)).toFixed(2)}%`;
      const left = `${Math.max(22, Math.min(78, seat.leftPct)).toFixed(2)}%`;
      // Bust cutouts are much wider than a forehead — never let the brim span the whole frame.
      const width = `${Math.max(28, Math.min(56, seat.widthPct)).toFixed(2)}%`;
      cutout.style.setProperty('--speaker-hat-bottom', bottom);
      cutout.style.setProperty('--speaker-hat-left', left);
      cutout.style.setProperty('--speaker-hat-width', width);
      if (hatEl) {
        hatEl.style.bottom = bottom;
        hatEl.style.left = left;
        hatEl.style.width = width;
        hatEl.style.transform = 'translateX(-50%)';
      }
    }

    _clearQuoteSpeakerBirthdayHatSeat(cutout) {
      if (!cutout) return;
      cutout.style.removeProperty('--speaker-hat-bottom');
      cutout.style.removeProperty('--speaker-hat-left');
      cutout.style.removeProperty('--speaker-hat-width');
      const hatEl = document.getElementById('quoteSpeakerHat');
      if (hatEl) {
        hatEl.style.removeProperty('bottom');
        hatEl.style.removeProperty('left');
        hatEl.style.removeProperty('width');
        hatEl.style.removeProperty('transform');
      }
    }

    /**
     * Paint-box seat (no pixels): brim near top of the drawn portrait, width ~forehead share
     * of that box. Used immediately so we never stick on the 52%/50% CSS fallback when
     * Firebase portraits taint canvas reads.
     */
    _quoteSpeakerBirthdayHatPaintBoxSeat(cutout) {
      if (!cutout) return null;
      const portraitImg =
        cutout.querySelector('.quote-speaker-image.is-loaded') ||
        cutout.querySelector('.quote-speaker-image');
      const cutoutRect = cutout.getBoundingClientRect();
      const frameW = Math.max(1, cutout.clientWidth || cutoutRect.width || 1);
      const frameH = Math.max(1, cutout.clientHeight || cutoutRect.height || 1);
      if (frameW < 8 || frameH < 8) return null;
      const readPx = (name, fallback = 0) => {
        const n = parseFloat(String(cutout.style.getPropertyValue(name) || '').replace(/px$/, ''));
        return Number.isFinite(n) && n > 0 ? n : fallback;
      };
      let boxX = readPx('--speaker-cutout-pos-x', 0);
      let boxY = readPx('--speaker-cutout-pos-y', 0);
      let boxW = readPx('--speaker-cutout-bg-w', 0);
      let boxH = readPx('--speaker-cutout-bg-h', 0);
      const imgRect = portraitImg?.getBoundingClientRect?.();
      if (imgRect && imgRect.width > 8 && imgRect.height > 8 && cutoutRect.width > 8) {
        boxX = imgRect.left - cutoutRect.left;
        boxY = imgRect.top - cutoutRect.top;
        boxW = imgRect.width;
        boxH = imgRect.height;
      }
      if (boxW < 8) {
        boxW = frameW;
        boxH = boxH > 8 ? boxH : frameH;
        boxX = 0;
        boxY = 0;
      }
      // Bust portraits: head ≈ ~45% of paint box width; brim into the upper forehead.
      const crownX = boxX + boxW * 0.5;
      const foreheadW = boxW * 0.44;
      const hatW = Math.max(28, Math.min(frameW * 0.54, foreheadW));
      const hatH = hatW * (1285 / 744);
      // Sit-line on scalp (~20% down), then push image-bottom down so the wide brim (not the
      // curved tip) is what contacts the head.
      const sitY = boxY + boxH * 0.2;
      const brimY = sitY + hatH * (1 - this._quoteSpeakerBirthdayHatVisualBrimFromTop());
      return {
        bottomPct: ((frameH - brimY) / frameH) * 100,
        leftPct: (crownX / frameW) * 100,
        widthPct: (hatW / frameW) * 100
      };
    }

    _quoteSpeakerBirthdayHatSeatFromCrown(cutout, measureImg, crown) {
      if (!cutout || !measureImg?.naturalWidth || !crown) return null;
      const iw = measureImg.naturalWidth;
      const ih = measureImg.naturalHeight;
      const readPx = (name, fallback = 0) => {
        const n = parseFloat(String(cutout.style.getPropertyValue(name) || '').replace(/px$/, ''));
        return Number.isFinite(n) ? n : fallback;
      };
      const frameW = Math.max(1, cutout.clientWidth || cutout.getBoundingClientRect().width || 1);
      const frameH = Math.max(1, cutout.clientHeight || cutout.getBoundingClientRect().height || 1);
      const offsetX = readPx('--speaker-cutout-pos-x', 0);
      const offsetY = readPx('--speaker-cutout-pos-y', 0);
      let drawW = readPx('--speaker-cutout-bg-w', 0);
      let drawH = readPx('--speaker-cutout-bg-h', 0);
      const portraitImg = cutout.querySelector('.quote-speaker-image');
      const cutoutRect = cutout.getBoundingClientRect();
      const imgRect = portraitImg?.getBoundingClientRect?.();
      const seatFromBox = (originX, originY, boxW, boxH) => {
        const crownX = originX + (crown.x / iw) * boxW;
        const crownY = originY + (crown.y / ih) * boxH;
        const foreheadW = (crown.foreheadWidth / iw) * boxW;
        const hatW = Math.max(28, Math.min(frameW * 0.54, boxW * 0.56, foreheadW * 0.98));
        const hatH = hatW * (1285 / 744);
        // crown.y is already the solid-hair sit line (not flyaway tips); small extra into forehead.
        const dip = Math.max(12, Math.min(foreheadW * 0.14, boxH * 0.055));
        const sitY = crownY + dip;
        const brimY = sitY + hatH * (1 - this._quoteSpeakerBirthdayHatVisualBrimFromTop());
        return {
          bottomPct: ((frameH - brimY) / frameH) * 100,
          leftPct: (crownX / frameW) * 100,
      ...
      if (imgRect && imgRect.width > 8 && cutoutRect.width > 8) {
        return seatFromBox(
          imgRect.left - cutoutRect.left,
          imgRect.top - cutoutRect.top,
          imgRect.width,
          imgRect.height
        );
      }
      if (drawW < 1) drawW = frameW;
      if (drawH < 1) drawH = frameH;
      return seatFromBox(offsetX, offsetY, drawW, drawH);
    }

    /**
     * Seat + scale the birthday hat.
     * 1) Immediate paint-box seat (no canvas) — always moves size/position off the CSS fallback.
     * 2) Async same-origin proxy + crown scan for true forehead width/center (Firebase imgs taint).
     */
    _positionQuoteSpeakerBirthdayHat(cutout, img) {
      if (!cutout) return;
      const hatEl = document.getElementById('quoteSpeakerHat');
      if (!hatEl || hatEl.hidden) {
        this._clearQuoteSpeakerBirthdayHatSeat(cutout);
        return;
      }
      const paintSeat = this._quoteSpeakerBirthdayHatPaintBoxSeat(cutout);
      if (paintSeat) this._applyQuoteSpeakerBirthdayHatSeat(cutout, paintSeat);

      const portraitImg =
        cutout.querySelector('.quote-speaker-image.is-loaded') ||
        cutout.querySelector('.quote-speaker-image') ||
        img;
      const tryPixelSeat = (measureImg) => {
        if (!measureImg?.naturalWidth || !cutout.isConnected || hatEl.hidden) return false;
        const bounds =
          cutout._speakerSubjectBounds ||
          cutout._speakerOpaqueBounds ||
          this._quoteSpeakerMeasureSubjectBounds(measureImg) ||
          this._quoteSpeakerMeasureOpaqueBounds(measureImg);
        if (bounds) {
          cutout._speakerOpaqueBounds = cutout._speakerOpaqueBounds || bounds;
        }
        const crown = this._quoteSpeakerMeasureCrownTop(measureImg, bounds);
        if (!crown) return false;
        const seat = this._quoteSpeakerBirthdayHatSeatFromCrown(cutout, measureImg, crown);
        if (!seat) return false;
        this._applyQuoteSpeakerBirthdayHatSeat(cutout, seat);
        return true;
      };

      // Sync path if the layout source is already canvas-readable (data:/blob:).
      const layoutImg =
        this._quoteSpeakerLayoutImg?.(cutout, img || portraitImg) ||
        cutout._plainCutoutSourceImage ||
        img;
      if (layoutImg?.naturalWidth && tryPixelSeat(layoutImg)) return;

      const sourceUrl = String(
        cutout._plainCutoutSourceUrl ||
          portraitImg?.currentSrc ||
          portraitImg?.src ||
          layoutImg?.currentSrc ||
          layoutImg?.src ||
          ''
      ).trim();
      if (!sourceUrl || !this.archiveService?._prepareSpeakerImageUrlForCanvas) return;
      const token = `${sourceUrl}|${cutout.clientWidth}x${cutout.clientHeight}`;
      if (cutout._birthdayHatMeasureToken === token) return;
      cutout._birthdayHatMeasureToken = token;
      const quote = this.getEffectiveQuiltQuote?.() || null;
      void this.archiveService
        ._prepareSpeakerImageUrlForCanvas(sourceUrl, { quote })
        .then(async (safeUrl) => {
          if (!safeUrl || !cutout.isConnected || hatEl.hidden) return;
          if (cutout._birthdayHatMeasureToken !== token) return;
          const safeImg = await this._loadQuoteSpeakerImageElement(safeUrl);
          if (!safeImg?.naturalWidth) return;
          tryPixelSeat(safeImg);
        })
        .catch(() => {
          /* paint-box seat already applied */
        });
    }

    /** Subject ink bounds — skips white paper mat / watermark strip (plain cutout crop). */
    ...
    _quoteSpeakerCutoutSubjectFraming(naturalW, naturalH, boxW, subjectBounds) {
      ...
      /** Fit full PNG width inside boxW — subject-fill must not spill past the 90% column. */
      const scale = Math.min(boxW / sw, boxW / iw);
      const drawW = iw * scale;
      const drawH = ih * scale;
      let offsetX = -subjectBounds.minX * scale;
      offsetX = Math.max(0, Math.min(Math.max(0, boxW - drawW), offsetX));
      const padTop = 8;
      const downNudge = 28;
    ...
    _applyQuoteSpeakerPlainCutoutFit(cutout, img, boxW = 0) {
      ...
      const portraitImg = cutout.querySelector('.quote-speaker-image');
      if (portraitImg) {
        portraitImg.style.width = `${width}px`;
        portraitImg.style.height = `${drawH}px`;
        portraitImg.style.maxWidth = '100%';
        portraitImg.style.left = '0px';
        portraitImg.style.top = `${Math.max(0, frameH - drawH - 2)}px`;
        portraitImg.style.objectFit = 'contain';
        portraitImg.style.objectPosition = 'bottom center';
      }
    }
    ...
    _syncQuoteSpeakerNameTuck(cutout, img) {
      ...
      requestAnimationFrame(() => {
        ...
        const cutoutBottomInWrap = cutoutRect.top - wrapRect.top + cutoutRect.height;
        this._applyQuoteSpeakerIdentityTuck(cutout, wrapRect, cutoutRect, cutoutBottomInWrap);
      });
    }
    ...
    _applyQuoteSpeakerCutoutAnchor(imageWrap, img) {
      ...
      if (plainCutout) {
        imageWrap.classList.remove('quote-speaker-cutout--subject-fill');
        delete imageWrap._speakerSubjectCrop;
        this._applyQuoteSpeakerPlainCutoutFit(imageWrap, img, boxW);
      ...
      if (wrap) {
        requestAnimationFrame(() => {
          ...
          if (wrapRect.width < 1 || cutoutRect.width < 1) return;
          const cutoutBottomInWrap = cutoutRect.top - wrapRect.top + cutoutRect.height;
          this._applyQuoteSpeakerIdentityTuck(imageWrap, wrapRect, cutoutRect, cutoutBottomInWrap);
        });
      }
      requestAnimationFrame(() => {
        if (!imageWrap.isConnected || !img?.naturalWidth) return;
        this._positionQuoteSpeakerBirthdayHat(imageWrap, img);
      });
    }
    ...
    _refineQuoteSpeakerPortraitWidthAfterLayout(widget, cutout, img) {
      ...
      if (cutout.classList.contains('quote-speaker-cutout--plain-cutout')) {
        this._applyQuoteSpeakerPlainCutoutFit(cutout, img, nextBoxW);
      } else {
        this._applyQuoteSpeakerCutoutFrameHeight(
          cutout,
          img,
    ...
    /** Final pass: shrink layout if painted bbox still exceeds 90% of center tape band. */
    _enforceQuoteSpeakerPortraitVisualCap(widget, cutout, img) {
      ...
      const visualW = Math.max(
        cutout.getBoundingClientRect().width || 0,
      ...
      const nextW = Math.max(48, Math.floor(currentLayoutW * (maxVisualW / visualW)));
      ...
      if (cutout.classList.contains('quote-speaker-cutout--plain-cutout')) {
        this._applyQuoteSpeakerPlainCutoutFit(cutout, img, nextW);
      } else {
        this._applyQuoteSpeakerCutoutFrameHeight(
          cutout,
          img,
    ...
    _applyQuoteSpeakerIdentityTuck(cutout, wrapRect, cutoutRect, wrapHeightPx) {
      const widget = cutout?.closest('.quote-speaker-widget');
      const identity = widget?.querySelector('.quote-speaker-slab--identity');
      if (!identity || cutout?.hidden || !wrapRect || !cutoutRect || wrapHeightPx < 1) {
      ...
      const bgW = parseFloat(String(cutout.style.getPropertyValue('--speaker-cutout-bg-w') || '0').replace(/px$/, '')) || 0;
      const scale =
        bgW > 0 && img?.naturalWidth
          ? bgW / Math.max(1, img.naturalWidth)
          : cutoutRect.width / Math.max(1, img?.naturalWidth || 1);
      let anchorBottomInWrap;
      if (bounds && img?.naturalWidth) {
        const cutoutTopInWrap = cutoutRect.top - wrapRect.top;
        anchorBottomInWrap = cutoutTopInWrap + offsetY + bounds.maxY * scale;
      } else {
        anchorBottomInWrap = cutoutRect.bottom - wrapRect.top;
      }
      const marginTop = Math.round(anchorBottomInWrap - wrapHeightPx - tuckPx);
    ...
    _ensureQuoteSpeakerPlainCutoutLayersHidden(cutout) {
      ...
      hide('.quote-speaker-xerox-grain');
    ...
    _applyQuoteSpeakerPlainCutoutBacking(cutout, img, sourceUrl = '') {
      ...
      const applyDirectCutoutDisplay = async (sourceImg) => {
        ...
        if (portraitImg.dataset.plainCutoutDirect === '1' && portraitImg.naturalWidth) {
          portraitImg.style.removeProperty('opacity');
          this._applyQuoteSpeakerPlainCutoutFit(cutout, srcImg || portraitImg);
          return;
        }
    ...
    _applyQuoteSpeakerImageLoaded(
      ...
      if (img) {
        ...
        if (!usePlainCutout && /^https?:\/\//i.test(displaySrc) && !this._quoteSpeakerCanvasPipelineBlocked()) {
          img.crossOrigin = 'anonymous';
        }
        ...
        img.onerror = () => {
          const fallbackSrc = String(profile?.portraitUrl || profile?.imageUrl || '').trim();
          if (fallbackSrc && img.src !== fallbackSrc && /^https?:/i.test(fallbackSrc)) {
            imageWrap?.classList.remove('quote-speaker-cutout--plain-cutout', 'quote-speaker-cutout--plain-fallback');
            img.style.opacity = '1';
            img.src = fallbackSrc;
            return;
          }
    ...
    async _refreshQuoteSpeakerWidgetEntry(quote = null) {
      ...
      if (this.isAdminTomorrowPreviewActive?.()) {
        ...
        if (dk && this.quoteService) {
          ...
          const hasPreviewSpeakerAsset = !!String(
            merged.speakerCutoutUrl ??
              merged.speaker_cutout_url ??
              merged.speakerImageUrl ??
              merged.speaker_image_url ??
              ''
          ).trim();
          if (hasPreviewSpeakerAsset) {
            void this.quoteService
              ._mergeSpeakerFieldsFromFirestoreSource(dk, merged)
              .then((next) => next?.text && this.quoteService.hydrateSpeakerCutoutFieldsForCalendarKey(dk, next))
              .then((next) => next?.text && this.quoteService.hydrateSpeakerGuideFieldsForCalendarKey(dk, next))
              .then((next) => {
                if (!next?.text || !this.isAdminTomorrowPreviewActive?.()) return;
                this.quoteService._pinnedByDateKey[dk] = next;
                if (this._adminPreview?.dateKey === dk) this._adminPreview.quote = next;
                this._refreshQuoteSpeakerWidgetNow(next);
              })
              .catch(() => {});
          } else {
            merged =
            ...
            merged =
              (await this.quoteService.hydrateSpeakerCutoutFieldsForCalendarKey(dk, merged)) || merged;
            merged =
              (await this.quoteService.hydrateSpeakerGuideFieldsForCalendarKey(dk, merged)) || merged;
          }
    ...
    _ensureQuoteSpeakerGlitterSparkles(glitterEl) {
      ...
      const COUNT = 70;
      const COLORS = ['#f6c945', '#7fd8e0', '#ff8a5c'];
      const frag = document.createDocumentFragment();
      for (let i = 0; i < COUNT; i += 1) {
        const angle = Math.random() * Math.PI * 2;
        const radius = 40 + Math.random() * 28; // halo band around the frame edge, in %
    ...
    _refreshQuoteSpeakerWidgetNowInner(quote = null) {
      ...
      if (hatEl || glitterEl || birthdayCaptionEl) {
        ...
        if (hatEl) hatEl.hidden = !isBirthday;
        if (glitterEl) {
        ...
        if (isBirthday && imageWrap) {
          const hatImg = this._quoteSpeakerLayoutImg(
            imageWrap,
            document.getElementById('quoteSpeakerImage')
          );
          const seatHat = () => {
            if (!imageWrap.isConnected) return;
            this._positionQuoteSpeakerBirthdayHat(imageWrap, hatImg);
          };
          // Two frames: first after hat unhides, second after cutout paint offsets settle.
          requestAnimationFrame(() => {
            seatHat();
            requestAnimationFrame(seatHat);
          });
        } else if (imageWrap) {
          this._clearQuoteSpeakerBirthdayHatSeat(imageWrap);
          imageWrap._birthdayHatMeasureToken = '';
        }
      }
