  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    async loadDeferredAdminSlice() {
      ...
      const SliceCtor = await this._loadDeferredScript(
        'lib/simplified-quilt-app-admin.js?v=33',
    ...
    _applyQuoteSpeakerBirthdayHatSeat(cutout, seat) {
      ...
      const bottom = `${Math.max(52, Math.min(92, seat.bottomPct)).toFixed(2)}%`;
      const left = `${Math.max(20, Math.min(80, seat.leftPct)).toFixed(2)}%`;
      // Bust cutouts are much wider than a forehead — never let the brim span the whole frame.
      const width = `${Math.max(28, Math.min(62, seat.widthPct)).toFixed(2)}%`;
    ...
    _quoteSpeakerBirthdayHatPaintBoxSeat(cutout) {
      ...
      // Bust portraits: head ≈ half the paint box, not ~all of it (0.84 made the hat enormous).
      const crownX = boxX + boxW * 0.52;
      const crownY = boxY + boxH * 0.1;
      const foreheadW = boxW * 0.48;
      const hatW = Math.max(28, Math.min(frameW * 0.58, foreheadW));
      // Deep enough that the brim sits in the hair/forehead, not balanced on the tip of the scalp.
      const dip = Math.max(14, Math.min(36, frameH * 0.1));
      const brimY = crownY + dip;
      return {
    ...
    _quoteSpeakerBirthdayHatSeatFromCrown(cutout, measureImg, crown) {
      ...
      if (imgRect && imgRect.width > 8 && cutoutRect.width > 8) {
        // Prefer live painted box when CSS paint vars are missing (common on plain cutouts).
        const originX = imgRect.left - cutoutRect.left;
        const originY = imgRect.top - cutoutRect.top;
        const crownX = originX + (crown.x / iw) * imgRect.width;
        const crownY = originY + (crown.y / ih) * imgRect.height;
        const foreheadW = (crown.foreheadWidth / iw) * imgRect.width;
        const hatW = Math.max(28, Math.min(frameW * 0.58, imgRect.width * 0.62, foreheadW * 1.02));
        const dip = Math.max(14, Math.min(36, frameH * 0.1));
        const brimY = crownY + dip;
        return {
          bottomPct: ((frameH - brimY) / frameH) * 100,
      ...
      const crownX = offsetX + (crown.x / iw) * drawW;
      const crownY = offsetY + (crown.y / ih) * drawH;
      const foreheadW = (crown.foreheadWidth / iw) * drawW;
      const hatW = Math.max(28, Math.min(frameW * 0.58, drawW * 0.62, foreheadW * 1.02));
      const dip = Math.max(14, Math.min(36, frameH * 0.1));
      const brimY = crownY + dip;
      return {
        bottomPct: ((frameH - brimY) / frameH) * 100,
