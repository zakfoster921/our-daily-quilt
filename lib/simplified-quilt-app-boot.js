  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    async loadDeferredAdminSlice() {
      ...
      const SliceCtor = await this._loadDeferredScript(
        'lib/simplified-quilt-app-admin.js?v=32',
    ...
    buildReflectionArchiveQuiltNameHtml(quiltName) {
      const name = String(quiltName || '').trim();
      if (!name) return '';
      return `<p class="reflection-themes-archive-quilt-name">${this.escapeQuiltFortuneText(name)}</p>`;
    }

    getQuiltNameFromQuiltNamesData(data) {
      if (!data || typeof data !== 'object') return '';
      return String(data.winningQuiltName || data.quiltName || '').trim();
    }

    async resolveQuiltNameForReflectionArchive(dateKey) {
      const key = String(dateKey || '').trim();
      if (!key) return '';
      try {
        const result = await this.getReflectionArchiveFirestoreDoc('quiltNames', key);
        return this.getQuiltNameFromQuiltNamesData(result?.data);
      } catch (error) {
        this.logger?.warn?.(`resolveQuiltNameForReflectionArchive(${key}):`, error);
        return '';
      }
    }
    ...
    async hydrateReflectionArchiveEntry(entry) {
      ...
      const [context, quiltImage, newspaperClippingUrl, quiltName] = await Promise.all([
        ...
        entry.quiltName
          ? Promise.resolve(String(entry.quiltName).trim())
          : this.resolveQuiltNameForReflectionArchive(entry.dateKey)
      ]);
      ...
      return {
        ...
        quiltName: String(quiltName || entry.quiltName || '').trim(),
        ...
        .map((row) => {
          ...
          return {
            ...
            quiltImageFallbackBlocks: Array.isArray(row.quiltImageFallbackBlocks)
              ...
              : null,
            quiltImageIsClassic,
            quiltName: String(row.quiltName || row.winningQuiltName || '').trim(),
    ...
    patchReflectionArchiveEntryInDom(dateKey, entry) {
      ...
      const findArchiveKicker = (label) =>
        [...details.querySelectorAll('.reflection-themes-archive-kicker')].find(
          (el) => String(el.textContent || '').trim().toLowerCase() === label
        );

      const ensureArchiveKicker = (label, insertBefore = null) => {
        let kicker = findArchiveKicker(label);
        if (kicker) return kicker;
        kicker = document.createElement('p');
        kicker.className = 'reflection-themes-archive-kicker';
        kicker.textContent = label.charAt(0).toUpperCase() + label.slice(1);
        if (insertBefore) details.insertBefore(kicker, insertBefore);
        else details.appendChild(kicker);
        return kicker;
      };

      const quoteInsertAnchor = () =>
        details.querySelector('.reflection-themes-archive-quote-clipping') ||
        findArchiveKicker('quote');

      const quiltName = String(entry.quiltName || '').trim();
      let quiltNameEl = details.querySelector('.reflection-themes-archive-quilt-name');
      if (quiltName) {
        if (!quiltNameEl) {
          quiltNameEl = document.createElement('p');
          quiltNameEl.className = 'reflection-themes-archive-quilt-name';
          const frame = details.querySelector('.reflection-themes-archive-quilt-frame');
          const quiltKicker = ensureArchiveKicker('quilt', frame || quoteInsertAnchor());
          if (frame) details.insertBefore(quiltNameEl, frame);
          else details.insertBefore(quiltNameEl, quiltKicker.nextSibling);
        }
        quiltNameEl.textContent = quiltName;
      } else if (quiltNameEl) {
        quiltNameEl.remove();
      }
      ...
      if (quiltUrl) {
        if (!img) {
          ...
          ensureArchiveKicker('quilt', quoteInsertAnchor());
          const anchor = quoteInsertAnchor();
    ...
    buildReflectionThemeArchiveEntryHtml(entry, index, idPrefix = 'reflectionThemesArchiveDetails') {
      ...
      const quiltNameHtml = this.buildReflectionArchiveQuiltNameHtml(entry.quiltName);
      ...
      const quoteSectionHtml = quoteClippingHtml
        ? `${this.buildReflectionArchiveSectionKickerHtml('Quote')}${quoteClippingHtml}`
        : '';
      const quiltSectionHtml = hasQuiltVisual
        ? `${this.buildReflectionArchiveSectionKickerHtml('Quilt')}${quiltNameHtml}${quiltImageHtml}`
        : quiltNameHtml
          ? `${this.buildReflectionArchiveSectionKickerHtml('Quilt')}${quiltNameHtml}`
          : '';
      return `
