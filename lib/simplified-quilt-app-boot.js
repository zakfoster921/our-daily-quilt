...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    /**
     * remove.bg and similar often leave a light semi-transparent matte (reads as a polygon on the quilt).
     * Crop to the opaque subject core and drop halo pixels before paint.
     */
    _quoteSpeakerStripFringeFromImageElement(img) {
      ...
      const pad = Math.max(2, Math.round(Math.min(iw, ih) * 0.01));
    ...
    async _quoteSpeakerCleanCutoutDisplayUrl(url) {
      const src = String(url || '').trim();
      ...
      const cleaned = this._quoteSpeakerStripFringeFromImageElement(img);
      return cleaned || src;
    }
    ...
    _applyQuoteSpeakerImageLoaded(
      ...
      if (imageWrap) {
        ...
        imageWrap.classList.add('quote-speaker-cutout--loaded', 'quote-speaker-cutout--single-layer');
        imageWrap.classList.remove('quote-speaker-cutout--img-paint');
      ...
      if (
        ...
        /speaker-cutouts(?:%2F|\/)/i.test(paintUrl) &&
      ...
      if (/speaker-cutouts(?:%2F|\/)/i.test(paintUrl)) {
      ...
      if (showSpeakerImage && imageWrap) {
        ...
        pickUrl()
          ...
          .then(async ({ remote, prepared }) => {
            ...
            const cleaned = base ? await this._quoteSpeakerCleanCutoutDisplayUrl(base) : '';
