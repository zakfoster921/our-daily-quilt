...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    setupLayoutBStoryPreviewLongPressShare() {
      ...
      const frame = wrap?.querySelector?.('.quilt-layout-b-preview-frame');
      ...
      let holdSharePending = false;

      const shareLayoutBStoryPreview = () => {
        try {
          this.shareLayoutBStoryPreviewFromLongPressSync?.();
        } catch (error) {
      ...
      const bindHold = (target) => {
        if (!target || target.dataset.storyPreviewHoldAttached === '1') return;
        target.dataset.storyPreviewHoldAttached = '1';
        this._bindSustainHoldGesture(target, {
          isEnabled: canShareLayoutBStoryPreview,
          onPrime: () => {
            holdSharePending = false;
            void this._ensureLayoutBStoryPreviewShareBlob?.();
          },
          onHoldStart: () => {
            holdSharePending = true;
          },
          onHoldEnd: () => {
            if (!holdSharePending) return;
            holdSharePending = false;
            shareLayoutBStoryPreview();
          },
