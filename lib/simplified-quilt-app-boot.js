...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    /** Full-width strip: straight sides + bottom; 1–2 gentle peaks on the top edge only. */
    _buildFooterIconStripTopCutClipPath(w, h, seedKey = '') {
      ...
      const twoPoints = rnd() < 0.5;
      const leanLeft = rnd() < 0.5;
      const peakDepthPx = Math.max(11, Math.min(18, Math.round(ih * 0.1)));
      const cornerYPx = Math.max(2, Math.round(peakDepthPx * 0.16));
      const peakYPx = 0;
      const valleyYPx = peakDepthPx + Math.max(4, Math.round(peakDepthPx * 0.28));
      const pts = [`0px ${ih}px`, `0px ${cornerYPx}px`];

      if (twoPoints) {
        const span = 0.14 + rnd() * 0.08;
        const start = leanLeft ? 0.16 + rnd() * 0.08 : 0.58 + rnd() * 0.08;
        const x1 = Math.round(iw * start);
        const x2 = Math.round(iw * (start + span));
        const valleyX = Math.round((x1 + x2) / 2);
        pts.push(`${x1}px ${peakYPx}px`);
        pts.push(`${valleyX}px ${valleyYPx}px`);
        pts.push(`${x2}px ${peakYPx}px`);
      } else {
        const xPeak = Math.round(
          iw * (leanLeft ? 0.2 + rnd() * 0.14 : 0.66 + rnd() * 0.14)
        );
        pts.push(`${xPeak}px ${peakYPx}px`);
      }

      pts.push(`${iw}px ${cornerYPx}px`, `${iw}px ${ih}px`);
      return {
        clip: `polygon(${pts.join(', ')})`,
        /* Peaks lean off-center; centered icons only need the shallow top clearance. */
        peakOverhangPx: cornerYPx + 2
      };
    }
    ...
    initReflectionCarouselLoop(notes) {
      ...
      const lockCarouselHorizontalGesture = () => {
        if (!carouselTouchGesture || carouselTouchGesture.axis !== 'x') return;
        if (quiltScrollEl && carouselTouchGesture.lockScrollTop != null) {
          quiltScrollEl.scrollTop = carouselTouchGesture.lockScrollTop;
        }
      };
        ...
        (event) => {
          ...
          if (
            ...
            if (absDy > absDx * carouselTouchAxisRatio) {
              // Vertical intent — let #screen-quilt scroll natively (manual scrollTop fought it).
              resetCarouselTouchGesture();
              return;
            }
            if (absDx > absDy * carouselTouchAxisRatio) {
              carouselTouchGesture.axis = 'x';
              carouselTouchGesture.lockScrollTop = quiltScrollEl?.scrollTop ?? 0;
            }
          }
          if (carouselTouchGesture?.axis === 'x') {
            event.preventDefault();
            lockCarouselHorizontalGesture();
          }
        },
        { passive: false }
      );
    ...
    renderReflectionAnonymousWall(themes = null) {
      ...
      const quiltScroll = document.getElementById('screen-quilt');
      const wallTopBefore = wall.getBoundingClientRect().top;
      ...
      wall.hidden = false;
      const stabilizeWallScroll = () => {
        if (!quiltScroll) return;
        const drift = wall.getBoundingClientRect().top - wallTopBefore;
        if (Math.abs(drift) > 0.5) quiltScroll.scrollTop += drift;
      };
      stabilizeWallScroll();
      requestAnimationFrame(() => requestAnimationFrame(stabilizeWallScroll));
    }
