...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    /** Dev/preview: ?screen=zak|mission|welcome|bridge|first-name|portal|quote|leaderboard */
    _getPreviewScreenIdFromQuery() {
      try {
        const params = new URLSearchParams(window.location.search || '');
        if (params.get('qnlb') === '1' || params.get('leaderboard') === '1') {
          return document.getElementById('screen-quilt-name-leaderboard')
            ? 'screen-quilt-name-leaderboard'
            : null;
        }
        ...
        const map = {
          ...
          quote: 'screen-quote',
          leaderboard: 'screen-quilt-name-leaderboard',
          qnlb: 'screen-quilt-name-leaderboard',
          'quilt-name-leaderboard': 'screen-quilt-name-leaderboard'
        };
    ...
    async _continueAfterLiveSync() {
      ...
      if (rememberViewDateKey) {
        ...
        if (previewScreenId) {
          ...
          if (previewScreenId === 'screen-quilt-name-leaderboard') {
      ...
      if (fastReturningToday) {
        ...
        this._portalToQuoteTimer = setTimeout(() => {
          ...
          if (document.getElementById('screen-quilt-name-leaderboard')?.classList.contains('active')) {
            return;
          }
    ...
    /** Console/local preview: load catalog + show Demetri (or named artist) widget on quilt screen. */
    async previewSeamsidePodcastWidget(authorName = 'Demetri Broxton') {
      const author = String(authorName || 'Demetri Broxton').trim();
      await this.quoteService?.ensureSeamsideEpisodesLoaded?.({ requireServer: false });
      this.uiService?.showScreen?.('screen-quilt');
      await this._refreshSeamsidePodcastWidgetEntry({
        text: 'SEAMSIDE preview',
      ...
      if (host?.hidden) {
        console.warn(
          '[SEAMSIDE preview] widget hidden — episodes loaded:',
