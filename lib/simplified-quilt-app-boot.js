...
(function (root) {
  ...
  class SimplifiedQuiltAppV2Boot {
    ...
    setupEventListeners() {
      ...
      document.querySelectorAll('.quilt-instagram-icon-link').forEach((link) => {
        if (link.dataset.odqAnalyticsBound === '1') return;
    ...
    _onQuiltMoodTerminalPicked(host, mood, line, meta = {}) {
      ...
      if (spreadHost?._moodTriptychWidget && TriptychW?.moodToPanel) {
        ...
        if (!spreadDone && triptych.getMood?.() !== mood) {
          triptych.setMood(mood, { instant: !!meta.instant });
        }
      }
      ...
      if (spreadHost.dataset.contentSig === sig && spreadHost._moodTriptychWidget && spreadDateMatches) {
        ...
        if (!remountForNewClipping) {
          ...
          if (effectiveMood === 'good' || effectiveMood === 'rough') {
            if (triptych.getMood?.() !== effectiveMood) {
              triptych.setMood(effectiveMood, { instant: true });
            }
            if (moodHost) {
