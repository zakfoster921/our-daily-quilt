    ...
    async _refreshQuiltMoodSpread(
      ...
      try {
        ...
        quoteDataUrl = dk
          ...
          : null;
        if (quoteDataUrl && /^https?:\/\//i.test(quoteDataUrl)) {
          const meta = await this.fetchNewspaperClippingMeta(dk, { waitForFirebaseMs: 4000 });
          const bustRev =
            meta.exportRev ||
            globalThis.readNewspaperClippingUrlOdqT?.(quoteDataUrl) ||
            Date.now();
          quoteDataUrl = globalThis.withNewspaperClippingBrowserCacheBust?.(quoteDataUrl, bustRev) || quoteDataUrl;
        }
        if (quoteDataUrl) {
